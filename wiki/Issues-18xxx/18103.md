# Issue 18103: cleanup number fields coerce embeddings

archive/issues_017866.json:
```json
{
    "body": "This ticket is a duplication of #12715.\n\nCurrently, the default coerce embedding uses RLF/CLF but heavily relies on AA/QQbar. It is bad for several reasons:\n- there is no coercion from RLF/CLF to AA/QQbar\n- equality and comparisons in RLF are completely broken (because it relies on finite approximations)\n\n```\nsage: K.<cbrt3> = NumberField(x^3 - 3, embedding=1)\nsage: b=(1+RLF(cbrt3))/2\nsage: c=1/2 + 3/2*(RLF(cbrt3)/3)\nsage: b == c\nFalse\n```\n\nIn this ticket:\n- we get rid of RLF/CLF for number field embeddings and use AA/QQbar instead\n- we make the interaction between AA and number fields more friendly\n- we speed up a lot the approximations of number field elements using Cython\n\nIn a ticket #18104, we will consider number fields with complex embeddings.\n\n**CC:**  @staroste\n\n**Keywords:** sd66\n\nIssue created by migration from https://trac.sagemath.org/ticket/18103\n\n",
    "closed_at": "2015-04-13T17:47:47Z",
    "created_at": "2015-04-01T22:31:49Z",
    "labels": [
        "component: number fields",
        "enhancement",
        "duplicate"
    ],
    "title": "cleanup number fields coerce embeddings",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/18103",
    "user": "https://github.com/videlec"
}
```
This ticket is a duplication of #12715.

Currently, the default coerce embedding uses RLF/CLF but heavily relies on AA/QQbar. It is bad for several reasons:
- there is no coercion from RLF/CLF to AA/QQbar
- equality and comparisons in RLF are completely broken (because it relies on finite approximations)

```
sage: K.<cbrt3> = NumberField(x^3 - 3, embedding=1)
sage: b=(1+RLF(cbrt3))/2
sage: c=1/2 + 3/2*(RLF(cbrt3)/3)
sage: b == c
False
```

In this ticket:
- we get rid of RLF/CLF for number field embeddings and use AA/QQbar instead
- we make the interaction between AA and number fields more friendly
- we speed up a lot the approximations of number field elements using Cython

In a ticket #18104, we will consider number fields with complex embeddings.

**CC:**  @staroste

**Keywords:** sd66

Issue created by migration from https://trac.sagemath.org/ticket/18103





---

archive/issue_comments_249419.json:
```json
{
    "body": "**Dependencies:** #17830",
    "created_at": "2015-04-01T22:40:57Z",
    "issue": "https://github.com/sagemath/sage/issues/18103",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18103#issuecomment-249419",
    "user": "https://github.com/videlec"
}
```

**Dependencies:** #17830



---

archive/issue_comments_249420.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,5 +1,5 @@\n-Currently, the default coerce embedding uses RLF (or CLF) but heavily relies on AA (or QQbar). It is bad for several reasons:\n-- there is no coercion from RLF to AA\n+Currently, the default coerce embedding uses RLF/CLF but heavily relies on AA/QQbar. It is bad for several reasons:\n+- there is no coercion from RLF/CLF to AA/QQbar\n - equality and comparisons in RLF are completely broken (because it relies on finite approximations)\n \n ```\n@@ -10,4 +10,9 @@\n False\n ```\n \n-In this ticket we get rid of RLF/CLF for number field embeddings.\n+In this ticket:\n+- we get rid of RLF/CLF for number field embeddings and use AA/QQbar instead\n+- we make the interaction between AA and number fields more friendly\n+- we speed up a lot the approximations of number field elements using Cython\n+\n+In a ticket #18104, we will consider number fields with complex embeddings.\n``````\n",
    "created_at": "2015-04-01T22:40:57Z",
    "issue": "https://github.com/sagemath/sage/issues/18103",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18103#issuecomment-249420",
    "user": "https://github.com/videlec"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,5 +1,5 @@
-Currently, the default coerce embedding uses RLF (or CLF) but heavily relies on AA (or QQbar). It is bad for several reasons:
-- there is no coercion from RLF to AA
+Currently, the default coerce embedding uses RLF/CLF but heavily relies on AA/QQbar. It is bad for several reasons:
+- there is no coercion from RLF/CLF to AA/QQbar
 - equality and comparisons in RLF are completely broken (because it relies on finite approximations)
 
 ```
@@ -10,4 +10,9 @@
 False
 ```
 
-In this ticket we get rid of RLF/CLF for number field embeddings.
+In this ticket:
+- we get rid of RLF/CLF for number field embeddings and use AA/QQbar instead
+- we make the interaction between AA and number fields more friendly
+- we speed up a lot the approximations of number field elements using Cython
+
+In a ticket #18104, we will consider number fields with complex embeddings.
``````




---

archive/issue_comments_249421.json:
```json
{
    "body": "**Changing keywords** from \"\" to \"sd66\".",
    "created_at": "2015-04-01T22:40:57Z",
    "issue": "https://github.com/sagemath/sage/issues/18103",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18103#issuecomment-249421",
    "user": "https://github.com/videlec"
}
```

**Changing keywords** from "" to "sd66".



---

archive/issue_comments_249422.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks like a duplicate of #5355 and #12715.",
    "created_at": "2015-04-13T14:07:05Z",
    "issue": "https://github.com/sagemath/sage/issues/18103",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18103#issuecomment-249422",
    "user": "https://github.com/mezzarobba"
}
```

<a id='comment:2'></a>
Looks like a duplicate of #5355 and #12715.



---

archive/issue_events_163728.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-04-13T14:15:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18103",
    "milestone": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18103#event-163728"
}
```



---

archive/issue_events_163729.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-04-13T14:15:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18103",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18103#event-163729"
}
```



---

archive/issue_comments_249423.json:
```json
{
    "body": "<a id='comment:3'></a>\nReplying to [@mezzarobba](#comment%3A2):\n> Looks like a duplicate of #5355 and #12715.\n\nIt is (of #12715). Could you close it?\n\nI will add some details in #12715 (I have an implementation which works except for cyclotomic fields).\n\nVincent",
    "created_at": "2015-04-13T14:15:47Z",
    "issue": "https://github.com/sagemath/sage/issues/18103",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18103#issuecomment-249423",
    "user": "https://github.com/videlec"
}
```

<a id='comment:3'></a>
Replying to [@mezzarobba](#comment%3A2):
> Looks like a duplicate of #5355 and #12715.

It is (of #12715). Could you close it?

I will add some details in #12715 (I have an implementation which works except for cyclotomic fields).

Vincent



---

archive/issue_comments_249424.json:
```json
{
    "body": "**Changing dependencies** from \"#17830\" to \"\".",
    "created_at": "2015-04-13T14:15:47Z",
    "issue": "https://github.com/sagemath/sage/issues/18103",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18103#issuecomment-249424",
    "user": "https://github.com/videlec"
}
```

**Changing dependencies** from "#17830" to "".



---

archive/issue_comments_249425.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n+This ticket is a duplication of #12715.\n+\n Currently, the default coerce embedding uses RLF/CLF but heavily relies on AA/QQbar. It is bad for several reasons:\n - there is no coercion from RLF/CLF to AA/QQbar\n - equality and comparisons in RLF are completely broken (because it relies on finite approximations)\n``````\n",
    "created_at": "2015-04-13T14:15:47Z",
    "issue": "https://github.com/sagemath/sage/issues/18103",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18103#issuecomment-249425",
    "user": "https://github.com/videlec"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
+This ticket is a duplication of #12715.
+
 Currently, the default coerce embedding uses RLF/CLF but heavily relies on AA/QQbar. It is bad for several reasons:
 - there is no coercion from RLF/CLF to AA/QQbar
 - equality and comparisons in RLF are completely broken (because it relies on finite approximations)
``````




---

archive/issue_events_163730.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-04-13T15:17:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18103",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18103#event-163730"
}
```



---

archive/issue_events_163731.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-04-13T15:17:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18103",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18103#event-163731"
}
```



---

archive/issue_events_163732.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-13T17:47:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18103",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18103#event-163732"
}
```



---

archive/issue_events_163733.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-13T17:47:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18103",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18103#event-163733"
}
```



---

archive/issue_events_163734.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-13T17:47:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18103",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18103#event-163734"
}
```

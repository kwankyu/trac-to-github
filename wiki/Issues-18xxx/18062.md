# Issue 18062: Add homomesy detection code.

archive/issues_017825.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nAdd code to detect homomesies in posets for cardinality of order ideals, antichains for any toggle order. \n\nCC:  @kevindilks\n\nComponent: **combinatorics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/18062_\n\n",
    "created_at": "2015-03-26T18:37:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add homomesy detection code.",
    "type": "issue",
    "updated_at": "2022-12-29T01:33:44Z",
    "url": "https://github.com/sagemath/sage/issues/18062",
    "user": "https://github.com/deinst"
}
```
<div id="comment:0"></div>

Add code to detect homomesies in posets for cardinality of order ideals, antichains for any toggle order. 

CC:  @kevindilks

Component: **combinatorics**

_Issue created by migration from https://trac.sagemath.org/ticket/18062_





---

archive/issue_events_254633.json:
```json
{
    "actor": "https://github.com/deinst",
    "created_at": "2015-03-26T18:37:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18062",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18062#event-254633"
}
```



---

archive/issue_events_254634.json:
```json
{
    "actor": "https://github.com/deinst",
    "created_at": "2015-03-26T18:37:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18062",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18062#event-254634"
}
```



---

archive/issue_comments_246107.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nI have some code to do this that needs cleaning up.",
    "created_at": "2015-03-26T18:37:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18062",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18062#issuecomment-246107",
    "user": "https://github.com/deinst"
}
```

<div id="comment:1" align="right">comment:1</div>

I have some code to do this that needs cleaning up.



---

archive/issue_events_254635.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2015-09-29T05:14:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18062",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18062#event-254635"
}
```



---

archive/issue_events_254636.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2015-09-29T05:14:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18062",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18062#event-254636"
}
```



---

archive/issue_events_254637.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2015-09-29T05:14:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18062",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18062#event-254637"
}
```



---

archive/issue_events_254638.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2015-09-29T05:14:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18062",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18062#event-254638"
}
```



---

archive/issue_events_254639.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2015-09-29T05:14:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/18062",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18062#event-254639"
}
```



---

archive/issue_events_254640.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2015-09-29T05:14:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18062",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18062#event-254640"
}
```



---

archive/issue_comments_246108.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nThis is now done in #24128 for invertible actions.",
    "created_at": "2018-10-06T21:06:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/18062",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18062#issuecomment-246108",
    "user": "https://github.com/darijgr"
}
```

<div id="comment:3" align="right">comment:3</div>

This is now done in #24128 for invertible actions.



---

archive/issue_events_254641.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:33:44Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18062",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18062#event-254641"
}
```

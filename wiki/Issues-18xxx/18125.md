# Issue 18125: Loading/attaching Cython files might be broken

archive/issues_017888.json:
```json
{
    "body": "See [https://groups.google.com/forum/#!topic/sage-devel/1iePzvZT8Ns](https://groups.google.com/forum/#!topic/sage-devel/1iePzvZT8Ns)\n\n**Reviewer:** Jeroen Demeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/18125\n\n",
    "closed_at": "2015-04-09T12:11:35Z",
    "created_at": "2015-04-05T07:21:06Z",
    "labels": [
        "component: cython",
        "blocker",
        "bug",
        "worksforme",
        "invalid"
    ],
    "title": "Loading/attaching Cython files might be broken",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/18125",
    "user": "https://github.com/jdemeyer"
}
```
See [https://groups.google.com/forum/#!topic/sage-devel/1iePzvZT8Ns](https://groups.google.com/forum/#!topic/sage-devel/1iePzvZT8Ns)

**Reviewer:** Jeroen Demeyer

Issue created by migration from https://trac.sagemath.org/ticket/18125





---

archive/issue_comments_249931.json:
```json
{
    "body": "<a id='comment:1'></a>\nIs this really a blocker, you can't even reproduce the problem...",
    "created_at": "2015-04-09T08:52:28Z",
    "issue": "https://github.com/sagemath/sage/issues/18125",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18125#issuecomment-249931",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:1'></a>
Is this really a blocker, you can't even reproduce the problem...



---

archive/issue_events_163907.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-09T09:09:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/18125",
    "milestone": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18125#event-163907"
}
```



---

archive/issue_comments_249932.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2015-04-09T09:09:25Z",
    "issue": "https://github.com/sagemath/sage/issues/18125",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18125#issuecomment-249932",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_events_163908.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-09T09:09:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18125",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18125#event-163908"
}
```



---

archive/issue_comments_249933.json:
```json
{
    "body": "<a id='comment:3'></a>\nSuch a problem is certainly a blocker. Since I didn't receive any more feedback from the reporter, let's close as \"worksforme\".",
    "created_at": "2015-04-09T09:10:29Z",
    "issue": "https://github.com/sagemath/sage/issues/18125",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/18125#issuecomment-249933",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'></a>
Such a problem is certainly a blocker. Since I didn't receive any more feedback from the reporter, let's close as "worksforme".



---

archive/issue_events_163909.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-09T09:10:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18125",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18125#event-163909"
}
```



---

archive/issue_events_163910.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-09T09:10:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18125",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18125#event-163910"
}
```



---

archive/issue_events_163911.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-09T12:11:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/18125",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18125#event-163911"
}
```



---

archive/issue_events_163912.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-09T12:11:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/18125",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18125#event-163912"
}
```



---

archive/issue_events_163913.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-09T12:11:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/18125",
    "label": "worksforme",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/18125#event-163913"
}
```

# Issue 4911: convert sage.games.* docstrings to Sphinx

archive/issues_004911.json:
```json
{
    "body": "\n\nIssue created by migration from https://trac.sagemath.org/ticket/4911\n\n",
    "closed_at": "2009-02-24T18:01:53Z",
    "created_at": "2009-01-01T22:51:10Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4",
    "title": "convert sage.games.* docstrings to Sphinx",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/4911",
    "user": "https://github.com/mwhansen"
}
```


Issue created by migration from https://trac.sagemath.org/ticket/4911





---

archive/attachments_005700.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_4911.patch",
    "asset_url": "tarball://root/attachments/ticket4911/trac_4911.patch",
    "created_at": "2009-01-02T02:24:45Z",
    "issue": "https://github.com/sagemath/sage/issues/4911",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket4911/trac_4911.patch",
    "user": "https://github.com/mwhansen"
}
```



---

archive/issue_comments_030840.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_4911.patch](https://github.com/sagemath/sage/files/ticket4911/trac_4911.patch)",
    "created_at": "2009-01-02T02:24:45Z",
    "issue": "https://github.com/sagemath/sage/issues/4911",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4911#issuecomment-30840",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_4911.patch](https://github.com/sagemath/sage/files/ticket4911/trac_4911.patch)



---

archive/issue_events_030012.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-02T02:24:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4911",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4911#event-30012"
}
```



---

archive/issue_events_030013.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-01-07T22:05:04Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4911",
    "rename": {
        "from": "convert sage.games.* docstrings to Sphinx",
        "to": "[needs a little work] convert sage.games.* docstrings to Sphinx"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4911#event-30013"
}
```



---

archive/issue_comments_030841.json:
```json
{
    "body": "<a id='comment:2'></a>\nline 56: \"<code>contains the :math:\\`i\\`,:math:\\`j\\` position</code>\" is not being rendered properly.  Maybe replace it with \"<code>contains the :math:\\`i,j\\` position</code>\".\n\nActually, why do you need :math: here at all?  I don't remember seeing it in the other files that I've looked at...",
    "created_at": "2009-01-07T22:05:04Z",
    "issue": "https://github.com/sagemath/sage/issues/4911",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4911#issuecomment-30841",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'></a>
line 56: "<code>contains the :math:\`i\`,:math:\`j\` position</code>" is not being rendered properly.  Maybe replace it with "<code>contains the :math:\`i,j\` position</code>".

Actually, why do you need :math: here at all?  I don't remember seeing it in the other files that I've looked at...



---

archive/attachments_005701.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_4911-2.patch",
    "asset_url": "tarball://root/attachments/ticket4911/trac_4911-2.patch",
    "created_at": "2009-01-08T21:16:25Z",
    "issue": "https://github.com/sagemath/sage/issues/4911",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket4911/trac_4911-2.patch",
    "user": "https://github.com/mwhansen"
}
```



---

archive/issue_comments_030842.json:
```json
{
    "body": "<a id='comment:3'></a>\n**Attachment:** [trac_4911-2.patch](https://github.com/sagemath/sage/files/ticket4911/trac_4911-2.patch)\n\nI've posted a patch which fixes this issue.  The :math: was there because I had forgot to go through this file and remove them.",
    "created_at": "2009-01-08T21:16:25Z",
    "issue": "https://github.com/sagemath/sage/issues/4911",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4911#issuecomment-30842",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'></a>
**Attachment:** [trac_4911-2.patch](https://github.com/sagemath/sage/files/ticket4911/trac_4911-2.patch)

I've posted a patch which fixes this issue.  The :math: was there because I had forgot to go through this file and remove them.



---

archive/issue_events_030014.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-08T21:16:25Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4911",
    "rename": {
        "from": "[needs a little work] convert sage.games.* docstrings to Sphinx",
        "to": "convert sage.games.* docstrings to Sphinx"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4911#event-30014"
}
```



---

archive/issue_comments_030843.json:
```json
{
    "body": "<a id='comment:4'></a>\nGreat, positive review.",
    "created_at": "2009-01-08T23:36:14Z",
    "issue": "https://github.com/sagemath/sage/issues/4911",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4911#issuecomment-30843",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:4'></a>
Great, positive review.



---

archive/issue_events_030015.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-01-08T23:36:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4911",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4911#event-30015"
}
```



---

archive/issue_events_030016.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-01-08T23:36:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4911",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4911#event-30016"
}
```



---

archive/attachments_005702.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "sage.games-final.patch",
    "asset_url": "tarball://root/attachments/ticket4911/sage.games-final.patch",
    "created_at": "2009-02-24T18:01:53Z",
    "issue": "https://github.com/sagemath/sage/issues/4911",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket4911/sage.games-final.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```



---

archive/issue_comments_030844.json:
```json
{
    "body": "<a id='comment:5'></a>\n**Attachment:** [sage.games-final.patch](https://github.com/sagemath/sage/files/ticket4911/sage.games-final.patch)\n\nMerged sage.games-final.patch in Sage 3.4.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-24T18:01:53Z",
    "issue": "https://github.com/sagemath/sage/issues/4911",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4911#issuecomment-30844",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
**Attachment:** [sage.games-final.patch](https://github.com/sagemath/sage/files/ticket4911/sage.games-final.patch)

Merged sage.games-final.patch in Sage 3.4.alpha0.

Cheers,

Michael



---

archive/issue_events_030017.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-24T18:01:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4911",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4911#event-30017"
}
```



---

archive/issue_events_030018.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-24T18:01:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4911",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4911#event-30018"
}
```

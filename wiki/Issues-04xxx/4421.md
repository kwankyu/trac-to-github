# Issue 4421: create an optional singular_surf.spkg

archive/issues_004421.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nWe have removed the Java binaries required for surf (which is part of Singular). Since there are optional doctests in sage/rings/polynomial/multi_polynomial_ideal.py that depend on it and since people have complained about it not being available we should add it back via an optional spkg.\n\nCheers,\n\nMichael\n\nComponent: **packages: optional**\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4421_\n\n",
    "closed_at": "2017-09-13T18:05:25Z",
    "created_at": "2008-11-02T02:31:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "create an optional singular_surf.spkg",
    "type": "issue",
    "updated_at": "2017-09-13T18:05:25Z",
    "url": "https://github.com/sagemath/sage/issues/4421",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
<div id="comment:0"></div>

We have removed the Java binaries required for surf (which is part of Singular). Since there are optional doctests in sage/rings/polynomial/multi_polynomial_ideal.py that depend on it and since people have complained about it not being available we should add it back via an optional spkg.

Cheers,

Michael

Component: **packages: optional**

Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/4421_





---

archive/issue_events_049552.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-02T02:31:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49552"
}
```



---

archive/issue_events_049553.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-02T02:31:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20optional",
    "label_color": "0000b0",
    "label_name": "c: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49553"
}
```



---

archive/issue_events_049554.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-02T02:31:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49554"
}
```



---

archive/issue_events_049555.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-02T02:31:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49555"
}
```



---

archive/issue_events_049556.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-02T02:31:43Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49556"
}
```



---

archive/issue_events_049557.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49557"
}
```



---

archive/issue_events_049558.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49558"
}
```



---

archive/issue_events_049559.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49559"
}
```



---

archive/issue_events_049560.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49560"
}
```



---

archive/issue_events_049561.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49561"
}
```



---

archive/issue_events_049562.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49562"
}
```



---

archive/issue_events_049563.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49563"
}
```



---

archive/issue_events_049564.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49564"
}
```



---

archive/issue_comments_025606.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nThere is already a surf package so I think this can be closed. The surf package however doesn't build but we have #6316 for this. Putting up for review so someone else can look wether this can be closed.",
    "created_at": "2017-09-13T12:49:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4421#issuecomment-25606",
    "user": "https://github.com/koffie"
}
```

<div id="comment:5" align="right">comment:5</div>

There is already a surf package so I think this can be closed. The surf package however doesn't build but we have #6316 for this. Putting up for review so someone else can look wether this can be closed.



---

archive/issue_events_049565.json:
```json
{
    "actor": "https://github.com/koffie",
    "created_at": "2017-09-13T12:49:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49565"
}
```



---

archive/issue_events_049566.json:
```json
{
    "actor": "https://github.com/koffie",
    "created_at": "2017-09-13T12:49:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49566"
}
```



---

archive/issue_events_049567.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-13T18:05:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49567"
}
```



---

archive/issue_events_049568.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-13T18:05:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "c6c6c6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49568"
}
```



---

archive/issue_comments_025607.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2017-09-13T18:05:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4421#issuecomment-25607",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_049569.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-13T18:05:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49569"
}
```



---

archive/issue_events_049570.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-13T18:05:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4421",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4421#event-49570"
}
```

# Issue 4707: magma/sage interface -- another trivial easy-to-fix failure hopefully

archive/issues_004707.json:
```json
{
    "body": "\n```\nsage: E = EllipticCurve(GF(25,'a'), [0,0,1,4,0])\nsage: magma(E)\nboom\n```\n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/4707\n\n",
    "closed_at": "2008-12-11T11:09:55Z",
    "created_at": "2008-12-05T02:28:26Z",
    "labels": [
        "component: interfaces",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2.2",
    "title": "magma/sage interface -- another trivial easy-to-fix failure hopefully",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/4707",
    "user": "https://github.com/williamstein"
}
```

```
sage: E = EllipticCurve(GF(25,'a'), [0,0,1,4,0])
sage: magma(E)
boom
```

**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/4707





---

archive/attachments_005429.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_4707.patch",
    "asset_url": "tarball://root/attachments/ticket4707/trac_4707.patch",
    "created_at": "2008-12-05T02:55:03Z",
    "issue": "https://github.com/sagemath/sage/issues/4707",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket4707/trac_4707.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_029328.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_4707.patch](https://github.com/sagemath/sage/files/ticket4707/trac_4707.patch)",
    "created_at": "2008-12-05T02:55:03Z",
    "issue": "https://github.com/sagemath/sage/issues/4707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4707#issuecomment-29328",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_4707.patch](https://github.com/sagemath/sage/files/ticket4707/trac_4707.patch)



---

archive/issue_events_028326.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-12-05T02:55:03Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4707",
    "rename": {
        "from": "magma/sage interface -- another trivial easy-to-fix failure hopefully",
        "to": "[with patch; needs review (EASY!)] magma/sage interface -- another trivial easy-to-fix failure hopefully"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4707#event-28326"
}
```



---

archive/issue_events_028327.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2008-12-06T23:49:58Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4707",
    "rename": {
        "from": "[with patch; needs review (EASY!)] magma/sage interface -- another trivial easy-to-fix failure hopefully",
        "to": "[with postitive review] magma/sage interface -- another trivial easy-to-fix failure hopefully"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4707#event-28327"
}
```



---

archive/attachments_005430.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_4707_tiny_followup.patch",
    "asset_url": "tarball://root/attachments/ticket4707/trac_4707_tiny_followup.patch",
    "created_at": "2008-12-06T23:49:58Z",
    "issue": "https://github.com/sagemath/sage/issues/4707",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket4707/trac_4707_tiny_followup.patch",
    "user": "https://github.com/ncalexan"
}
```



---

archive/issue_comments_029329.json:
```json
{
    "body": "<a id='comment:2'></a>\n**Attachment:** [trac_4707_tiny_followup.patch](https://github.com/sagemath/sage/files/ticket4707/trac_4707_tiny_followup.patch)\n\nWorks for me.  Apply both patches.  We will open a separate ticket for the corresponding magma -> sage conversion.",
    "created_at": "2008-12-06T23:49:58Z",
    "issue": "https://github.com/sagemath/sage/issues/4707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4707#issuecomment-29329",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:2'></a>
**Attachment:** [trac_4707_tiny_followup.patch](https://github.com/sagemath/sage/files/ticket4707/trac_4707_tiny_followup.patch)

Works for me.  Apply both patches.  We will open a separate ticket for the corresponding magma -> sage conversion.



---

archive/issue_comments_029330.json:
```json
{
    "body": "<a id='comment:3'></a>\nfix typo in title",
    "created_at": "2008-12-11T04:58:07Z",
    "issue": "https://github.com/sagemath/sage/issues/4707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4707#issuecomment-29330",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
fix typo in title



---

archive/issue_events_028328.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-12-11T04:58:07Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4707",
    "rename": {
        "from": "[with postitive review] magma/sage interface -- another trivial easy-to-fix failure hopefully",
        "to": "magma/sage interface -- another trivial easy-to-fix failure hopefully"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4707#event-28328"
}
```



---

archive/issue_events_028329.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-12-11T04:58:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4707",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4707#event-28329"
}
```



---

archive/issue_comments_029331.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged both patches in Sage 3.2.2.alpha2",
    "created_at": "2008-12-11T11:09:55Z",
    "issue": "https://github.com/sagemath/sage/issues/4707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4707#issuecomment-29331",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged both patches in Sage 3.2.2.alpha2



---

archive/issue_events_028330.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-12-11T11:09:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4707",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4707#event-28330"
}
```



---

archive/issue_events_028331.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-12-11T11:09:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4707",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4707#event-28331"
}
```

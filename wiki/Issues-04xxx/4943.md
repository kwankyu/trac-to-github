# Issue 4943: make sage -tp run sage -t when only one file is specified

archive/issues_004943.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nsage -tp is great, except it doesn't print synchronously when only one file is being tested.  It's irritating to wait for output when there are no race problems due to only a single parallel process.  Could we make sage -tp detect a single file and just run sage -t when that's the case?\n\nComponent: **doctest coverage**\n\nKeywords: **parallel doctest testing**\n\nReviewer: **David Roe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4943_\n\n",
    "closed_at": "2013-03-15T13:01:09Z",
    "created_at": "2009-01-05T22:35:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "make sage -tp run sage -t when only one file is specified",
    "type": "issue",
    "updated_at": "2013-03-15T13:01:09Z",
    "url": "https://github.com/sagemath/sage/issues/4943",
    "user": "https://github.com/ncalexan"
}
```
<div id="comment:0"></div>

sage -tp is great, except it doesn't print synchronously when only one file is being tested.  It's irritating to wait for output when there are no race problems due to only a single parallel process.  Could we make sage -tp detect a single file and just run sage -t when that's the case?

Component: **doctest coverage**

Keywords: **parallel doctest testing**

Reviewer: **David Roe**

_Issue created by migration from https://trac.sagemath.org/ticket/4943_





---

archive/issue_events_056778.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-01-05T22:35:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "milestone_number": null,
    "milestone_title": "sage-3.2.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56778"
}
```



---

archive/issue_events_056779.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-01-05T22:35:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56779"
}
```



---

archive/issue_events_056780.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-01-05T22:35:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56780"
}
```



---

archive/issue_events_056781.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-01-05T22:35:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56781"
}
```



---

archive/issue_events_056782.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-01-05T22:35:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56782"
}
```



---

archive/issue_events_056783.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-05T22:35:56Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "subject": "https://github.com/ncalexan",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56783"
}
```



---

archive/issue_events_056784.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-06T01:08:29Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "milestone_number": null,
    "milestone_title": "sage-3.2.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56784"
}
```



---

archive/issue_events_056785.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-06T01:08:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56785"
}
```



---

archive/issue_events_056786.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-12T02:27:21Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56786"
}
```



---

archive/issue_events_056787.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-12T02:27:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56787"
}
```



---

archive/issue_events_056788.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-01-22T18:25:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56788"
}
```



---

archive/issue_events_056789.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-01-22T18:25:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56789"
}
```



---

archive/issue_events_056790.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-14T20:37:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56790"
}
```



---

archive/issue_comments_029900.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThis is resolved by #12415.",
    "created_at": "2013-03-14T20:37:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4943#issuecomment-29900",
    "user": "https://github.com/roed314"
}
```

<div id="comment:4" align="right">comment:4</div>

This is resolved by #12415.



---

archive/issue_events_056791.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-14T20:38:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "milestone_number": null,
    "milestone_title": "sage-5.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56791"
}
```



---

archive/issue_events_056792.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-14T20:38:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56792"
}
```



---

archive/issue_events_056793.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-14T20:38:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56793"
}
```



---

archive/issue_events_056794.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-15T13:01:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56794"
}
```



---

archive/issue_events_056795.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-15T13:01:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56795"
}
```



---

archive/issue_comments_029901.json:
```json
{
    "body": "Reviewer: **David Roe**",
    "created_at": "2013-03-15T13:01:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4943#issuecomment-29901",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **David Roe**



---

archive/issue_events_056796.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-15T13:01:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56796"
}
```



---

archive/issue_events_056797.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-15T13:01:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4943",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "c6c6c6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4943#event-56797"
}
```

# Issue 4852: graph plotting using @option and @suboption

archive/issues_004852.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nWe really probably ought to use the very nice `@`option and `@`suboption functionality for doing plots of graphs. That would make it easier to specify edge/vertex options, for example, and to retrieve existing defaults, etc.\n\n\nCC:  @sagetrac-ekirkman\n\nComponent: **graph theory**\n\nReviewer: **Robert Miller, Karl-Dieter Crisman**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4852_\n\n",
    "closed_at": "2012-06-02T12:33:18Z",
    "created_at": "2008-12-22T18:37:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/worksforme"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "graph plotting using @option and @suboption",
    "type": "issue",
    "updated_at": "2012-06-02T12:33:18Z",
    "url": "https://github.com/sagemath/sage/issues/4852",
    "user": "https://github.com/jasongrout"
}
```
<div id="comment:0"></div>

We really probably ought to use the very nice `@`option and `@`suboption functionality for doing plots of graphs. That would make it easier to specify edge/vertex options, for example, and to retrieve existing defaults, etc.


CC:  @sagetrac-ekirkman

Component: **graph theory**

Reviewer: **Robert Miller, Karl-Dieter Crisman**

_Issue created by migration from https://trac.sagemath.org/ticket/4852_





---

archive/issue_events_055601.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-12-22T18:37:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4852#event-55601"
}
```



---

archive/issue_events_055602.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-12-22T18:37:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4852#event-55602"
}
```



---

archive/issue_events_055603.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-12-22T18:37:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4852#event-55603"
}
```



---

archive/issue_events_055604.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-12-22T18:37:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4852#event-55604"
}
```



---

archive/issue_events_055605.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-12-22T18:37:25Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4852#event-55605"
}
```



---

archive/issue_events_055606.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-12-22T18:40:03Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4852#event-55606"
}
```



---

archive/issue_events_055607.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-12-22T18:40:03Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4852#event-55607"
}
```



---

archive/issue_comments_029266.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nEmily was working on this at one point. I don't know where that code is now...",
    "created_at": "2008-12-22T19:06:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4852#issuecomment-29266",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:2" align="right">comment:2</div>

Emily was working on this at one point. I don't know where that code is now...



---

archive/issue_comments_029267.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nThis is related a little to #2817.\n\nEmily, were you working on using `@`option and `@`suboption to plot graphs?",
    "created_at": "2008-12-23T03:00:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4852#issuecomment-29267",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:3" align="right">comment:3</div>

This is related a little to #2817.

Emily, were you working on using `@`option and `@`suboption to plot graphs?



---

archive/issue_comments_029268.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nGraph plots currently use `@`option. See `sage/graphs/graph_plot.py`\n\n\n...invalid?",
    "created_at": "2009-07-13T21:34:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4852#issuecomment-29268",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:4" align="right">comment:4</div>

Graph plots currently use `@`option. See `sage/graphs/graph_plot.py`


...invalid?



---

archive/issue_comments_029269.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\n> Graph plots currently use `@`option. See `sage/graphs/graph_plot.py`\n\nOr at least see `sage.graphs.generic_graph.py`, in particular (in Sage 5.0) [http://hg.sagemath.org/sage-main/file/9ab4ab6e12d0/sage/graphs/generic_graph.py#l13907](http://hg.sagemath.org/sage-main/file/9ab4ab6e12d0/sage/graphs/generic_graph.py#l13907)\n\n> \n> ...invalid?\n\nProbably it could be improved somehow, but we'd want a more specific ticket for this.  Jason, I'm recommending closing this - speak now!",
    "created_at": "2012-06-01T18:55:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4852#issuecomment-29269",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:5" align="right">comment:5</div>

> Graph plots currently use `@`option. See `sage/graphs/graph_plot.py`

Or at least see `sage.graphs.generic_graph.py`, in particular (in Sage 5.0) [http://hg.sagemath.org/sage-main/file/9ab4ab6e12d0/sage/graphs/generic_graph.py#l13907](http://hg.sagemath.org/sage-main/file/9ab4ab6e12d0/sage/graphs/generic_graph.py#l13907)

> 
> ...invalid?

Probably it could be improved somehow, but we'd want a more specific ticket for this.  Jason, I'm recommending closing this - speak now!



---

archive/issue_events_055608.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-06-01T18:55:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4852#event-55608"
}
```



---

archive/issue_events_055609.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-06-01T18:55:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4852#event-55609"
}
```



---

archive/issue_comments_029270.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\n+1 to closing this vague ticket that seems to be minimally satisfied.",
    "created_at": "2012-06-01T18:58:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4852#issuecomment-29270",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:6" align="right">comment:6</div>

+1 to closing this vague ticket that seems to be minimally satisfied.



---

archive/issue_events_055610.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-06-01T19:00:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4852#event-55610"
}
```



---

archive/issue_events_055611.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-06-01T19:00:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4852#event-55611"
}
```



---

archive/issue_comments_029271.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nGreat.",
    "created_at": "2012-06-01T19:00:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4852#issuecomment-29271",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:7" align="right">comment:7</div>

Great.



---

archive/issue_comments_029272.json:
```json
{
    "body": "Reviewer: **Robert Miller, Karl-Dieter Crisman**",
    "created_at": "2012-06-01T19:00:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4852#issuecomment-29272",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **Robert Miller, Karl-Dieter Crisman**



---

archive/issue_events_055612.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T12:33:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4852#event-55612"
}
```



---

archive/issue_events_055613.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T12:33:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "label": "https://github.com/sagemath/sage/labels/worksforme",
    "label_color": "c6c6c6",
    "label_name": "worksforme",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4852#event-55613"
}
```



---

archive/issue_events_055614.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T12:33:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4852#event-55614"
}
```



---

archive/issue_events_055615.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T12:33:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4852",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4852#event-55615"
}
```

# Issue 4221: [with package] add initial pynac-0.1.p0 package

archive/issues_004221.json:
```json
{
    "body": "To split the task of testing the new pynac package and the interface code from #3872, the new version of the pynac package is moved here.\n\nLatest version of the package can be downloaded from:\n\nhttp://www.risc.jku.at/people/berocal/sage/pynac-0.1.p0.spkg\n\n**Assignee:** @burcin\n\n**Keywords:** symbolics, pynac\n\nIssue created by migration from https://trac.sagemath.org/ticket/4221\n\n",
    "closed_at": "2008-09-30T12:55:08Z",
    "created_at": "2008-09-30T12:34:07Z",
    "labels": [
        "component: packages: standard",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.3",
    "title": "[with package] add initial pynac-0.1.p0 package",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/4221",
    "user": "https://github.com/burcin"
}
```
To split the task of testing the new pynac package and the interface code from #3872, the new version of the pynac package is moved here.

Latest version of the package can be downloaded from:

http://www.risc.jku.at/people/berocal/sage/pynac-0.1.p0.spkg

**Assignee:** @burcin

**Keywords:** symbolics, pynac

Issue created by migration from https://trac.sagemath.org/ticket/4221





---

archive/issue_events_024400.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-30T12:55:08Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4221",
    "rename": {
        "from": "[with package] pynac package",
        "to": "[with package] add initial pynac-0.1.p0 package"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4221#event-24400"
}
```



---

archive/issue_events_024401.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-30T12:55:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4221",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4221#event-24401"
}
```



---

archive/issue_comments_025026.json:
```json
{
    "body": "<a id='comment:1'></a>\nBuild tested on \n\n* OSX 10.4, 10.5\n* Linux x86, x86-64, Itanium \n* Solaris x86\n\nIn total I ran 10 builds including all SkyNet machines. That obviously does not mean it actually works as intended, but the spkg passes the required build tests. Positive review.\n\nBurcin's spkg with some small SPKG.txt cleanups can be found at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.1.3/alpha2/pynac-0.1.p0.spkg\n\n\nCheers,\n\nMichael",
    "created_at": "2008-09-30T12:55:08Z",
    "issue": "https://github.com/sagemath/sage/issues/4221",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4221#issuecomment-25026",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Build tested on 

* OSX 10.4, 10.5
* Linux x86, x86-64, Itanium 
* Solaris x86

In total I ran 10 builds including all SkyNet machines. That obviously does not mean it actually works as intended, but the spkg passes the required build tests. Positive review.

Burcin's spkg with some small SPKG.txt cleanups can be found at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.1.3/alpha2/pynac-0.1.p0.spkg


Cheers,

Michael



---

archive/issue_events_024402.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-30T12:55:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4221",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4221#event-24402"
}
```



---

archive/issue_events_024403.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-30T12:55:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4221",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4221#event-24403"
}
```



---

archive/issue_comments_025027.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.1.3.alpha2",
    "created_at": "2008-09-30T12:56:44Z",
    "issue": "https://github.com/sagemath/sage/issues/4221",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4221#issuecomment-25027",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.1.3.alpha2

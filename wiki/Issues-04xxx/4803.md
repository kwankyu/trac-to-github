# Issue 4803: improvements to the polyhedra module documentation

archive/issues_004803.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-sbarthelemy"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe attached patch corrects some imprecisions in the polyhedra module documentation.\n\nCC:  @sagetrac-mhampton\n\nComponent: **geometry**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4803_\n\n",
    "closed_at": "2008-12-17T14:37:46Z",
    "created_at": "2008-12-15T09:50:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "improvements to the polyhedra module documentation",
    "type": "issue",
    "updated_at": "2008-12-17T14:37:46Z",
    "url": "https://github.com/sagemath/sage/issues/4803",
    "user": "https://github.com/sagetrac-sbarthelemy"
}
```
<div id="comment:0"></div>

The attached patch corrects some imprecisions in the polyhedra module documentation.

CC:  @sagetrac-mhampton

Component: **geometry**

_Issue created by migration from https://trac.sagemath.org/ticket/4803_





---

archive/issue_events_054907.json:
```json
{
    "actor": "https://github.com/sagetrac-sbarthelemy",
    "created_at": "2008-12-15T09:50:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54907"
}
```



---

archive/issue_events_054908.json:
```json
{
    "actor": "https://github.com/sagetrac-sbarthelemy",
    "created_at": "2008-12-15T09:50:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54908"
}
```



---

archive/issue_events_054909.json:
```json
{
    "actor": "https://github.com/sagetrac-sbarthelemy",
    "created_at": "2008-12-15T09:50:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54909"
}
```



---

archive/issue_events_054910.json:
```json
{
    "actor": "https://github.com/sagetrac-sbarthelemy",
    "created_at": "2008-12-15T09:50:23Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "subject": "https://github.com/sagetrac-sbarthelemy",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54910"
}
```



---

archive/issue_events_054911.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-15T09:52:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "milestone_number": null,
    "milestone_title": "sage-3.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54911"
}
```



---

archive/issue_comments_028974.json:
```json
{
    "body": "Attachment: **[11113.patch.gz](https://github.com/sagemath/sage/files/ticket4803/11113.patch.gz)**",
    "created_at": "2008-12-15T09:57:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4803#issuecomment-28974",
    "user": "https://github.com/sagetrac-sbarthelemy"
}
```

Attachment: **[11113.patch.gz](https://github.com/sagemath/sage/files/ticket4803/11113.patch.gz)**



---

archive/issue_events_054912.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-12-15T10:15:00Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "title_is": "[with review] improvements to the polyhedra module documentation",
    "title_was": "improvements to the polyhedra module documentation",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54912"
}
```



---

archive/issue_events_054913.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-12-15T10:15:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54913"
}
```



---

archive/issue_comments_028975.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThere are a couple typos in this proposed documentation change:\n\n\"minkowsky sum\" should be \"Minkowski sum\"\n\"positive (aka conic) combination\" should be \"positive (aka convex) combination\"\n\n(unless there is a meaning of conic combination that I don't know).\n\nThe Fukuda reference is good, it would be better to add a \"REFERENCES\" section since eventually such sections might be searched for and organized.\n\nThanks for working on this!\nMarshall Hampton",
    "created_at": "2008-12-15T10:15:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4803#issuecomment-28975",
    "user": "https://github.com/sagetrac-mhampton"
}
```

<div id="comment:2" align="right">comment:2</div>

There are a couple typos in this proposed documentation change:

"minkowsky sum" should be "Minkowski sum"
"positive (aka conic) combination" should be "positive (aka convex) combination"

(unless there is a meaning of conic combination that I don't know).

The Fukuda reference is good, it would be better to add a "REFERENCES" section since eventually such sections might be searched for and organized.

Thanks for working on this!
Marshall Hampton



---

archive/issue_events_054914.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-12-15T10:15:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54914"
}
```



---

archive/issue_events_054915.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-12-15T10:15:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "c: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54915"
}
```



---

archive/issue_comments_028976.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nHere is a new patch, against the previous\n\n> \"minkowsky sum\" should be \"Minkowski sum\"\n\nI fixed that thanks\n\n> \"positive (aka conic) combination\" should be \"positive (aka convex) combination\"\n> (unless there is a meaning of conic combination that I don't know).\n\nsee http://en.wikipedia.org/wiki/Conical_combination\n\nDattorro [1] uses \"conic hull\" instead of \"conical hull\". Fukuda uses noneg(ray1, ray2,...). I don't know which term is better. In the new patch I chose \"conic hull\", change it to what sounds best to you, english speakers.\n\n> The Fukuda reference is good, it would be better to add a \"REFERENCES\" section since eventually such sections might be searched for and organized.\n\nYou're right, fixed in the patch\n\n> Thanks for working on this!\n\nThanks for the feedback!\n\nS\u00e9bastien\n\n[1] http://meboo.convexoptimization.com/BOOK/convexgeometry.pdf",
    "created_at": "2008-12-15T11:04:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4803#issuecomment-28976",
    "user": "https://github.com/sagetrac-sbarthelemy"
}
```

<div id="comment:3" align="right">comment:3</div>

Here is a new patch, against the previous

> "minkowsky sum" should be "Minkowski sum"

I fixed that thanks

> "positive (aka conic) combination" should be "positive (aka convex) combination"
> (unless there is a meaning of conic combination that I don't know).

see http://en.wikipedia.org/wiki/Conical_combination

Dattorro [1] uses "conic hull" instead of "conical hull". Fukuda uses noneg(ray1, ray2,...). I don't know which term is better. In the new patch I chose "conic hull", change it to what sounds best to you, english speakers.

> The Fukuda reference is good, it would be better to add a "REFERENCES" section since eventually such sections might be searched for and organized.

You're right, fixed in the patch

> Thanks for working on this!

Thanks for the feedback!

Sébastien

[1] http://meboo.convexoptimization.com/BOOK/convexgeometry.pdf



---

archive/issue_comments_028977.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nAttachment: **[11114.patch.gz](https://github.com/sagemath/sage/files/ticket4803/11114.patch.gz)**",
    "created_at": "2008-12-15T11:08:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4803#issuecomment-28977",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Attachment: **[11114.patch.gz](https://github.com/sagemath/sage/files/ticket4803/11114.patch.gz)**



---

archive/issue_events_054916.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-15T11:08:01Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "title_is": "improvements to the polyhedra module documentation",
    "title_was": "[with review] improvements to the polyhedra module documentation",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54916"
}
```



---

archive/issue_events_054917.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-15T11:08:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54917"
}
```



---

archive/issue_events_054918.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-15T11:08:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54918"
}
```



---

archive/issue_comments_028978.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nGreat, I think with those changes it should go in.  Its unclear to me what the best term is for the \"conic hull\" but that can be tweaked later.",
    "created_at": "2008-12-16T14:09:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4803#issuecomment-28978",
    "user": "https://github.com/sagetrac-mhampton"
}
```

<div id="comment:5" align="right">comment:5</div>

Great, I think with those changes it should go in.  Its unclear to me what the best term is for the "conic hull" but that can be tweaked later.



---

archive/issue_events_054919.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-12-16T14:09:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54919"
}
```



---

archive/issue_events_054920.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-12-16T14:09:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54920"
}
```



---

archive/issue_comments_028979.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nMerged in Sage 3.2.2.rc1",
    "created_at": "2008-12-17T14:37:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4803#issuecomment-28979",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Merged in Sage 3.2.2.rc1



---

archive/issue_events_054921.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-17T14:37:46Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54921"
}
```



---

archive/issue_events_054922.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-17T14:37:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "milestone_number": null,
    "milestone_title": "sage-3.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54922"
}
```



---

archive/issue_events_054923.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-17T14:37:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54923"
}
```



---

archive/issue_events_054924.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-17T14:37:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-54924"
}
```

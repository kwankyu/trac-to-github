# Issue 4923: convert sage.plot.* docstrings to Sphinx (open issues at #5376)

archive/issues_004923.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\n\n\nComponent: **documentation**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4923_\n\n",
    "closed_at": "2009-02-24T19:09:28Z",
    "created_at": "2009-01-01T22:55:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "convert sage.plot.* docstrings to Sphinx (open issues at #5376)",
    "type": "issue",
    "updated_at": "2009-02-26T16:19:06Z",
    "url": "https://github.com/sagemath/sage/issues/4923",
    "user": "https://github.com/mwhansen"
}
```
<div id="comment:0"></div>



Component: **documentation**

_Issue created by migration from https://trac.sagemath.org/ticket/4923_





---

archive/issue_events_056510.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-01T22:55:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4923#event-56510"
}
```



---

archive/issue_events_056511.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-01T22:55:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4923#event-56511"
}
```



---

archive/issue_events_056512.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-01T22:55:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4923#event-56512"
}
```



---

archive/issue_events_056513.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-01T22:55:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4923#event-56513"
}
```



---

archive/issue_comments_029752.json:
```json
{
    "body": "Attachment: **[trac_4923.patch.gz](https://github.com/sagemath/sage/files/ticket4923/trac_4923.patch.gz)**",
    "created_at": "2009-01-02T02:58:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4923#issuecomment-29752",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[trac_4923.patch.gz](https://github.com/sagemath/sage/files/ticket4923/trac_4923.patch.gz)**



---

archive/issue_events_056514.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-02T02:59:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4923#event-56514"
}
```



---

archive/issue_comments_029753.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI don't have the time or energy to review this fully right now, but I noticed one small issue: in line 61 in plot.py, \"Type {?} after each primitive\", change \"{?}\" to \"?\"",
    "created_at": "2009-01-07T22:20:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4923#issuecomment-29753",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:2" align="right">comment:2</div>

I don't have the time or energy to review this fully right now, but I noticed one small issue: in line 61 in plot.py, "Type {?} after each primitive", change "{?}" to "?"



---

archive/issue_comments_029754.json:
```json
{
    "body": "Attachment: **[sage.plot-final.patch.gz](https://github.com/sagemath/sage/files/ticket4923/sage.plot-final.patch.gz)**",
    "created_at": "2009-02-21T19:21:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4923#issuecomment-29754",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[sage.plot-final.patch.gz](https://github.com/sagemath/sage/files/ticket4923/sage.plot-final.patch.gz)**



---

archive/issue_comments_029755.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nLooks good except for the following mostly minor changes:\n\nplot.py, line 990: change 'suming' to 'summing'  (not your fault, an old typo)\n\nline 1141: I think EXAMPLES is indented too far: check the page [http://sage.math.washington.edu/home/mhansen/sage/devel/sage/doc/output/html/en/reference/sage/plot/plot.html#sage.plot.plot.Graphics.show](http://sage.math.washington.edu/home/mhansen/sage/devel/sage/doc/output/html/en/reference/sage/plot/plot.html#sage.plot.plot.Graphics.show)\n\nline 1827: `'-' (dashed)` should say `'--' (dashed)`\n\nline 2559: a dash \"--\" in some text got turned into a hyphen \"-\"\n\nline 2913: change \"increase\" to \"increases\" (the subject of the sentence is \"lowering\", which is singular) (not your fault)",
    "created_at": "2009-02-21T23:33:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4923#issuecomment-29755",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:3" align="right">comment:3</div>

Looks good except for the following mostly minor changes:

plot.py, line 990: change 'suming' to 'summing'  (not your fault, an old typo)

line 1141: I think EXAMPLES is indented too far: check the page [http://sage.math.washington.edu/home/mhansen/sage/devel/sage/doc/output/html/en/reference/sage/plot/plot.html#sage.plot.plot.Graphics.show](http://sage.math.washington.edu/home/mhansen/sage/devel/sage/doc/output/html/en/reference/sage/plot/plot.html#sage.plot.plot.Graphics.show)

line 1827: `'-' (dashed)` should say `'--' (dashed)`

line 2559: a dash "--" in some text got turned into a hyphen "-"

line 2913: change "increase" to "increases" (the subject of the sentence is "lowering", which is singular) (not your fault)



---

archive/issue_events_056515.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-02-21T23:33:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4923#event-56515"
}
```



---

archive/issue_events_056516.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-02-21T23:33:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4923#event-56516"
}
```



---

archive/issue_events_056517.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-24T19:09:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4923#event-56517"
}
```



---

archive/issue_events_056518.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-24T19:09:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4923#event-56518"
}
```



---

archive/issue_comments_029756.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in Sage 3.4.alpha0.\n\nMike: please open a ticket for the review problems.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-24T19:09:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4923#issuecomment-29756",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in Sage 3.4.alpha0.

Mike: please open a ticket for the review problems.

Cheers,

Michael



---

archive/issue_comments_029757.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nSee #5376.",
    "created_at": "2009-02-26T15:24:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4923#issuecomment-29757",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:5" align="right">comment:5</div>

See #5376.



---

archive/issue_events_056519.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-26T16:19:06Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "title_is": "convert sage.plot.* docstrings to Sphinx (open issues at #5376)",
    "title_was": "convert sage.plot.* docstrings to Sphinx",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4923#event-56519"
}
```



---

archive/issue_events_056520.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-26T16:19:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4923#event-56520"
}
```



---

archive/issue_events_056521.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-26T16:19:06Z",
    "event": "reopened",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4923#event-56521"
}
```



---

archive/issue_comments_029758.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nThanks John.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-26T16:19:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4923",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4923#issuecomment-29758",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Thanks John.

Cheers,

Michael

# Issue 4686: Documentation for eta products

archive/issues_004686.json:
```json
{
    "assignees": [
        "https://github.com/loefflerd"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nWhen I wrote the eta products code I didn't know how to add it to the reference manual. Here is the requisite patch, together with a patch to the code itself correcting LaTeX errors in the docstrings.\n\nComponent: **modular forms**\n\nAuthor: **David Loeffler**\n\nReviewer: **Michael Abshoff**\n\nMerged: **3.2.2.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4686_\n\n",
    "closed_at": "2008-12-04T15:37:16Z",
    "created_at": "2008-12-03T10:20:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Documentation for eta products",
    "type": "issue",
    "updated_at": "2009-06-07T09:41:01Z",
    "url": "https://github.com/sagemath/sage/issues/4686",
    "user": "https://github.com/loefflerd"
}
```
<div id="comment:0"></div>

When I wrote the eta products code I didn't know how to add it to the reference manual. Here is the requisite patch, together with a patch to the code itself correcting LaTeX errors in the docstrings.

Component: **modular forms**

Author: **David Loeffler**

Reviewer: **Michael Abshoff**

Merged: **3.2.2.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/4686_





---

archive/issue_events_053289.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2008-12-03T10:20:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4686",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
    "label_color": "0000ff",
    "label_name": "c: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4686#event-53289"
}
```



---

archive/issue_events_053290.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2008-12-03T10:20:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4686",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4686#event-53290"
}
```



---

archive/issue_events_053291.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2008-12-03T10:20:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4686",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4686#event-53291"
}
```



---

archive/issue_events_053292.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2008-12-03T10:20:01Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4686",
    "subject": "https://github.com/loefflerd",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4686#event-53292"
}
```



---

archive/issue_comments_028071.json:
```json
{
    "body": "Attachment: **[4686-code.patch.gz](https://github.com/sagemath/sage/files/ticket4686/4686-code.patch.gz)**",
    "created_at": "2008-12-03T10:22:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4686",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4686#issuecomment-28071",
    "user": "https://github.com/loefflerd"
}
```

Attachment: **[4686-code.patch.gz](https://github.com/sagemath/sage/files/ticket4686/4686-code.patch.gz)**



---

archive/issue_comments_028072.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[4686-doc.patch.gz](https://github.com/sagemath/sage/files/ticket4686/4686-doc.patch.gz)**\n\nHere are two patches (one each for the hg_sage and hg_doc repositories).",
    "created_at": "2008-12-03T10:23:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4686",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4686#issuecomment-28072",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[4686-doc.patch.gz](https://github.com/sagemath/sage/files/ticket4686/4686-doc.patch.gz)**

Here are two patches (one each for the hg_sage and hg_doc repositories).



---

archive/issue_events_053293.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2008-12-03T10:23:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4686",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4686#event-53293"
}
```



---

archive/issue_events_053294.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2008-12-03T10:23:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4686",
    "milestone_number": null,
    "milestone_title": "sage-3.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4686#event-53294"
}
```



---

archive/issue_events_053295.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2008-12-03T10:23:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4686",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4686#event-53295"
}
```



---

archive/issue_comments_028073.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nPatch looks good to me. One tiny spelling issue I can take care when applying the patch:\n\n```\nGrobner basis\n```\nis missing an \"e\".\n\nCheers,\n\nMichael",
    "created_at": "2008-12-03T10:26:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4686",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4686#issuecomment-28073",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Patch looks good to me. One tiny spelling issue I can take care when applying the patch:

```
Grobner basis
```
is missing an "e".

Cheers,

Michael



---

archive/issue_events_053296.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-03T10:26:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4686",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4686#event-53296"
}
```



---

archive/issue_comments_028074.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged both patches in Sage 3.2.2.alpha0",
    "created_at": "2008-12-04T15:37:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4686",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4686#issuecomment-28074",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged both patches in Sage 3.2.2.alpha0



---

archive/issue_events_053297.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-04T15:37:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4686",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4686#event-53297"
}
```



---

archive/issue_events_053298.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-04T15:37:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4686",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4686#event-53298"
}
```



---

archive/issue_comments_028075.json:
```json
{
    "body": "Merged: **3.2.2.alpha0**",
    "created_at": "2009-06-07T09:41:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4686",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4686#issuecomment-28075",
    "user": "https://github.com/loefflerd"
}
```

Merged: **3.2.2.alpha0**



---

archive/issue_comments_028076.json:
```json
{
    "body": "Author: **David Loeffler**",
    "created_at": "2009-06-07T09:41:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4686",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4686#issuecomment-28076",
    "user": "https://github.com/loefflerd"
}
```

Author: **David Loeffler**



---

archive/issue_comments_028077.json:
```json
{
    "body": "Reviewer: **Michael Abshoff**",
    "created_at": "2009-06-07T09:41:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4686",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4686#issuecomment-28077",
    "user": "https://github.com/loefflerd"
}
```

Reviewer: **Michael Abshoff**

# Issue 4682: Add the first set of 12 people missing from the DevMap for Sage 3.0.6-3.2.1

archive/issues_004682.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThere are about 36 people missing at \n\n\n  http://sagemath.org/development-map.html\n\nthat have contributed to Sage. Contact them and if they would like to be added get them on there. This is a join project by Michael Abshoff, Minh Van Nguyen and Harald Schilly.\n\nCheers,\n\nMichael\n\nCC:  @sagetrac-mvngu @haraldschilly\n\nComponent: **website/wiki**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4682_\n\n",
    "closed_at": "2008-12-04T21:55:45Z",
    "created_at": "2008-12-03T00:38:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20website%20/%20wiki",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add the first set of 12 people missing from the DevMap for Sage 3.0.6-3.2.1",
    "type": "issue",
    "updated_at": "2008-12-05T00:24:22Z",
    "url": "https://github.com/sagemath/sage/issues/4682",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
<div id="comment:0"></div>

There are about 36 people missing at 


  http://sagemath.org/development-map.html

that have contributed to Sage. Contact them and if they would like to be added get them on there. This is a join project by Michael Abshoff, Minh Van Nguyen and Harald Schilly.

Cheers,

Michael

CC:  @sagetrac-mvngu @haraldschilly

Component: **website/wiki**

_Issue created by migration from https://trac.sagemath.org/ticket/4682_





---

archive/issue_events_053243.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-03T00:38:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "milestone_number": null,
    "milestone_title": "sage-3.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4682#event-53243"
}
```



---

archive/issue_events_053244.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-03T00:38:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20website%20/%20wiki",
    "label_color": "0075ca",
    "label_name": "c: website / wiki",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4682#event-53244"
}
```



---

archive/issue_events_053245.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-03T00:38:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4682#event-53245"
}
```



---

archive/issue_events_053246.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-03T00:38:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4682#event-53246"
}
```



---

archive/issue_events_053247.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-12-03T00:38:16Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4682#event-53247"
}
```



---

archive/issue_events_053248.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-12-03T00:38:47Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4682#event-53248"
}
```



---

archive/issue_events_053249.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-03T00:38:47Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4682#event-53249"
}
```



---

archive/issue_comments_028048.json:
```json
{
    "body": "Attachment: **[trac_4682.patch.gz](https://github.com/sagemath/sage/files/ticket4682/trac_4682.patch.gz)**\n\nfirst draft of revised devmap",
    "created_at": "2008-12-04T21:20:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4682#issuecomment-28048",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[trac_4682.patch.gz](https://github.com/sagemath/sage/files/ticket4682/trac_4682.patch.gz)**

first draft of revised devmap



---

archive/issue_comments_028049.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThe patch **trac_4682.patch** is an updated contributors list as found on the development map. I've re-organized the whole XML file for consistency, and added new details since they've arrived in the last few days.",
    "created_at": "2008-12-04T21:25:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4682#issuecomment-28049",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:2" align="right">comment:2</div>

The patch **trac_4682.patch** is an updated contributors list as found on the development map. I've re-organized the whole XML file for consistency, and added new details since they've arrived in the last few days.



---

archive/issue_events_053250.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2008-12-04T21:26:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4682#event-53250"
}
```



---

archive/issue_events_053251.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2008-12-04T21:55:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4682#event-53251"
}
```



---

archive/issue_events_053252.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2008-12-04T21:55:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4682#event-53252"
}
```



---

archive/issue_comments_028050.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nReplying to [@sagetrac-mvngu](#comment%3A2):\n> The patch **trac_4682.patch** is an updated contributors list as found on the development map. I've re-organized the whole XML file for consistency, and added new details since they've arrived in the last few days.\n\nthx and very cool, worked out of the box ;)\n\npatch added, site online, for future patches please use the current version to avoid confusions: http://www.sagemath.org/res/contributors.xml\n\nh",
    "created_at": "2008-12-04T21:55:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4682#issuecomment-28050",
    "user": "https://github.com/haraldschilly"
}
```

<div id="comment:4" align="right">comment:4</div>

Replying to [@sagetrac-mvngu](#comment%3A2):
> The patch **trac_4682.patch** is an updated contributors list as found on the development map. I've re-organized the whole XML file for consistency, and added new details since they've arrived in the last few days.

thx and very cool, worked out of the box ;)

patch added, site online, for future patches please use the current version to avoid confusions: http://www.sagemath.org/res/contributors.xml

h



---

archive/issue_events_053253.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2008-12-04T21:55:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4682#event-53253"
}
```



---

archive/issue_events_053254.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2008-12-04T21:55:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4682#event-53254"
}
```



---

archive/issue_comments_028051.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nWell, the idea of this ticket was to add all people and then close it. Since now the first dozen has been added I changed the subject.\n\nMinh: You should open a ticket for the next batch of say a dozen people once you are ready to go.\n\nCheers,\n\nMichae",
    "created_at": "2008-12-04T23:31:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4682#issuecomment-28051",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Well, the idea of this ticket was to add all people and then close it. Since now the first dozen has been added I changed the subject.

Minh: You should open a ticket for the next batch of say a dozen people once you are ready to go.

Cheers,

Michae



---

archive/issue_events_053255.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-04T23:31:03Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "title_is": "Add the first set of 12 people missing from the DevMap for Sage 3.0.6-3.2.1",
    "title_was": "Add about 36 people missing from the DevMap for Sage 3.0.6-3.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4682#event-53255"
}
```



---

archive/issue_events_053256.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-04T23:31:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4682#event-53256"
}
```



---

archive/issue_events_053257.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-04T23:31:03Z",
    "event": "reopened",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4682#event-53257"
}
```



---

archive/issue_comments_028052.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nHarald,\n\nYou also need to give the ticket a formal positive review. Then usually the release manager will close the ticket. For website tickets this might be handled slightly different.\n\nOne more thing: Minh, can you give us a listing which people were added in the patch? Since it is a reorg it is hard to follow. For future reference please post two patches in the future: One to move things around, one with the actual changes.\n\nCheers,\n\nMichael",
    "created_at": "2008-12-04T23:41:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4682#issuecomment-28052",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Harald,

You also need to give the ticket a formal positive review. Then usually the release manager will close the ticket. For website tickets this might be handled slightly different.

One more thing: Minh, can you give us a listing which people were added in the patch? Since it is a reorg it is hard to follow. For future reference please post two patches in the future: One to move things around, one with the actual changes.

Cheers,

Michael



---

archive/issue_events_053258.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-04T23:41:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4682#event-53258"
}
```



---

archive/issue_events_053259.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-04T23:41:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4682#event-53259"
}
```



---

archive/issue_comments_028053.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nReplying to [@sagetrac-mabshoff](#comment%3A6):\n> One more thing: Minh, can you give us a listing which people were added in the patch? Since it is a reorg it is hard to follow. \n\n\n\nThe following people were added to the devmap by **trac_4682.patch**:\n\n1. Arnaud Bergeron\n2. Nicolas Borie\n3. Hamish Ivey-Law\n4. Kwankyu Lee\n5. David Loeffler\n6. Jason Merrill\n7. Guillaume Moroz\n8. Brett Nakashima\n9. John Perry\n10. Nils-Peter Skoruppa\n11. Philippe Theveny\n12. Igor Tolkov\n\nThe above list is sorted in alphabetical order by last name.\n\n\n\n> For future reference please post two patches in the future: One to move things around, one with the actual changes.\n\n\n\nSure. I'll keep this in mind.",
    "created_at": "2008-12-05T00:24:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4682#issuecomment-28053",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:7" align="right">comment:7</div>

Replying to [@sagetrac-mabshoff](#comment%3A6):
> One more thing: Minh, can you give us a listing which people were added in the patch? Since it is a reorg it is hard to follow. 



The following people were added to the devmap by **trac_4682.patch**:

1. Arnaud Bergeron
2. Nicolas Borie
3. Hamish Ivey-Law
4. Kwankyu Lee
5. David Loeffler
6. Jason Merrill
7. Guillaume Moroz
8. Brett Nakashima
9. John Perry
10. Nils-Peter Skoruppa
11. Philippe Theveny
12. Igor Tolkov

The above list is sorted in alphabetical order by last name.



> For future reference please post two patches in the future: One to move things around, one with the actual changes.



Sure. I'll keep this in mind.

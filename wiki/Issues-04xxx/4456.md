# Issue 4456: sage-3.2.alpha3 -- numerical noise on osx 32-bit intel

archive/issues_004456.json:
```json
{
    "body": "\n```\nsage -t  devel/sage/sage/calculus/wester.py\n**********************************************************************\nFile \"/Users/was/build/sage-3.2.alpha3/tmp/wester.py\", line 261:\n   : [float(f(i/10)) for i in range(1,5)]\nExpected:\n   <BLANKLINE>\n   [-0.00033670040754082975,\n    -0.0027778004096620235,\n    -0.00989099409140...,\n    -0.025411145508414...]\nGot:\n   [-0.00033670040754081587, -0.0027778004096621622,\n-0.0098909940914039818, -0.025411145508414779]\n**********************************************************************\n1 items had failures:\n  1 of 193 in __main__.example_0\n***Test Failed*** 1 failures.\nFor whitespace errors, see the file\n/Users/was/build/sage-3.2.alpha3/tmp/.doctest_wester.py\n        [11.4 s]\n```\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/4456\n\n",
    "closed_at": "2008-11-09T00:29:00Z",
    "created_at": "2008-11-06T21:28:19Z",
    "labels": [
        "component: doctest coverage",
        "blocker",
        "bug",
        "duplicate"
    ],
    "title": "sage-3.2.alpha3 -- numerical noise on osx 32-bit intel",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/4456",
    "user": "https://github.com/williamstein"
}
```

```
sage -t  devel/sage/sage/calculus/wester.py
**********************************************************************
File "/Users/was/build/sage-3.2.alpha3/tmp/wester.py", line 261:
   : [float(f(i/10)) for i in range(1,5)]
Expected:
   <BLANKLINE>
   [-0.00033670040754082975,
    -0.0027778004096620235,
    -0.00989099409140...,
    -0.025411145508414...]
Got:
   [-0.00033670040754081587, -0.0027778004096621622,
-0.0098909940914039818, -0.025411145508414779]
**********************************************************************
1 items had failures:
  1 of 193 in __main__.example_0
***Test Failed*** 1 failures.
For whitespace errors, see the file
/Users/was/build/sage-3.2.alpha3/tmp/.doctest_wester.py
        [11.4 s]
```

**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/4456





---

archive/issue_events_026192.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-09T00:29:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4456",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4456#event-26192"
}
```



---

archive/issue_comments_026938.json:
```json
{
    "body": "<a id='comment:1'></a>\nOops, I opened a dupe of this at #4472, but since I will post a patch there I am closing this ticket :)\n\nCheers,\n\nMichael",
    "created_at": "2008-11-09T00:29:00Z",
    "issue": "https://github.com/sagemath/sage/issues/4456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4456#issuecomment-26938",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Oops, I opened a dupe of this at #4472, but since I will post a patch there I am closing this ticket :)

Cheers,

Michael



---

archive/issue_events_026193.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-09T00:29:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4456",
    "milestone": "sage-3.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4456#event-26193"
}
```



---

archive/issue_events_026194.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-09T00:29:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4456",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4456#event-26194"
}
```

# Issue 4898: Add style and labels to contour_plot()

archive/issues_004898.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-abergeron"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis patch add the option of styling lines, either individually or all at once, and adding contour level labels.\n\nAnother one is coming to support this in combination with #2770.\n\nCC:  @sagetrac-wcauchois @jasongrout\n\nComponent: **graphics**\n\nAuthor: **Arnaud Bergeron, Karl-Dieter Crisman**\n\nReviewer: **Jason Grout**\n\nMerged: **sage-4.2.1.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4898_\n\n",
    "closed_at": "2009-11-12T06:53:24Z",
    "created_at": "2008-12-31T21:49:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graphics",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.2.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add style and labels to contour_plot()",
    "type": "issue",
    "updated_at": "2009-11-22T05:37:58Z",
    "url": "https://github.com/sagemath/sage/issues/4898",
    "user": "https://github.com/sagetrac-abergeron"
}
```
<div id="comment:0"></div>

This patch add the option of styling lines, either individually or all at once, and adding contour level labels.

Another one is coming to support this in combination with #2770.

CC:  @sagetrac-wcauchois @jasongrout

Component: **graphics**

Author: **Arnaud Bergeron, Karl-Dieter Crisman**

Reviewer: **Jason Grout**

Merged: **sage-4.2.1.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/4898_





---

archive/issue_events_056240.json:
```json
{
    "actor": "https://github.com/sagetrac-abergeron",
    "created_at": "2008-12-31T21:49:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "milestone_number": null,
    "milestone_title": "sage-4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56240"
}
```



---

archive/issue_events_056241.json:
```json
{
    "actor": "https://github.com/sagetrac-abergeron",
    "created_at": "2008-12-31T21:49:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "c: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56241"
}
```



---

archive/issue_events_056242.json:
```json
{
    "actor": "https://github.com/sagetrac-abergeron",
    "created_at": "2008-12-31T21:49:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56242"
}
```



---

archive/issue_events_056243.json:
```json
{
    "actor": "https://github.com/sagetrac-abergeron",
    "created_at": "2008-12-31T21:49:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56243"
}
```



---

archive/issue_events_056244.json:
```json
{
    "actor": "https://github.com/sagetrac-abergeron",
    "created_at": "2008-12-31T21:49:27Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "subject": "https://github.com/sagetrac-abergeron",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56244"
}
```



---

archive/issue_events_056245.json:
```json
{
    "actor": "https://github.com/sagetrac-abergeron",
    "created_at": "2008-12-31T21:49:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56245"
}
```



---

archive/issue_events_056246.json:
```json
{
    "actor": "https://github.com/sagetrac-abergeron",
    "created_at": "2008-12-31T22:02:04Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "title_is": "[with patch; don't review yet] Add style and labels to contour_plot()",
    "title_was": "Add style and labels to contour_plot()",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56246"
}
```



---

archive/issue_comments_029567.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nI think it would just be better to wait until #2770 and #4884 are merged since they conflict with this.  I'll post an updated patch then.",
    "created_at": "2008-12-31T22:02:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29567",
    "user": "https://github.com/sagetrac-abergeron"
}
```

<div id="comment:1" align="right">comment:1</div>

I think it would just be better to wait until #2770 and #4884 are merged since they conflict with this.  I'll post an updated patch then.



---

archive/issue_events_056247.json:
```json
{
    "actor": "https://github.com/sagetrac-abergeron",
    "created_at": "2009-01-24T05:02:25Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "title_is": "Add style and labels to contour_plot()",
    "title_was": "[with patch; don't review yet] Add style and labels to contour_plot()",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56247"
}
```



---

archive/issue_events_056248.json:
```json
{
    "actor": "https://github.com/sagetrac-abergeron",
    "created_at": "2009-01-24T05:02:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56248"
}
```



---

archive/issue_comments_029568.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAttachment: **[trac_4898.patch.gz](https://github.com/sagemath/sage/files/ticket4898/trac_4898.patch.gz)**\n\nUpdate patch is posted.  Start reviewing!",
    "created_at": "2009-01-24T05:02:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29568",
    "user": "https://github.com/sagetrac-abergeron"
}
```

<div id="comment:2" align="right">comment:2</div>

Attachment: **[trac_4898.patch.gz](https://github.com/sagemath/sage/files/ticket4898/trac_4898.patch.gz)**

Update patch is posted.  Start reviewing!



---

archive/issue_events_056249.json:
```json
{
    "actor": "https://github.com/sagetrac-abergeron",
    "created_at": "2009-01-24T05:02:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56249"
}
```



---

archive/issue_comments_029569.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nWhat was this updated to?  I get conflicts applying it to 3.3alpha3.",
    "created_at": "2009-02-11T05:33:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29569",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:3" align="right">comment:3</div>

What was this updated to?  I get conflicts applying it to 3.3alpha3.



---

archive/issue_comments_029570.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nI tried applying it to 3.3rc0:\n\n```\njason@sage:~$ patch contour_plot.py < trac_4898.patch \npatching file contour_plot.py\nHunk #2 FAILED at 53.\nHunk #3 FAILED at 75.\nHunk #4 FAILED at 136.\nHunk #5 succeeded at 167 (offset -1 lines).\nHunk #6 succeeded at 180 (offset -1 lines).\nHunk #7 FAILED at 226.\nHunk #8 succeeded at 272 (offset -2 lines).\nHunk #9 succeeded at 293 (offset -2 lines).\nHunk #10 succeeded at 308 (offset -2 lines).\nHunk #11 succeeded at 317 (offset -2 lines).\nHunk #12 succeeded at 345 (offset -2 lines).\n4 out of 12 hunks FAILED -- saving rejects to file contour_plot.py.rej\n```",
    "created_at": "2009-02-11T06:44:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29570",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:4" align="right">comment:4</div>

I tried applying it to 3.3rc0:

```
jason@sage:~$ patch contour_plot.py < trac_4898.patch 
patching file contour_plot.py
Hunk #2 FAILED at 53.
Hunk #3 FAILED at 75.
Hunk #4 FAILED at 136.
Hunk #5 succeeded at 167 (offset -1 lines).
Hunk #6 succeeded at 180 (offset -1 lines).
Hunk #7 FAILED at 226.
Hunk #8 succeeded at 272 (offset -2 lines).
Hunk #9 succeeded at 293 (offset -2 lines).
Hunk #10 succeeded at 308 (offset -2 lines).
Hunk #11 succeeded at 317 (offset -2 lines).
Hunk #12 succeeded at 345 (offset -2 lines).
4 out of 12 hunks FAILED -- saving rejects to file contour_plot.py.rej
```



---

archive/issue_events_056250.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-11T06:44:49Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "title_is": "[needs rebase] Add style and labels to contour_plot()",
    "title_was": "Add style and labels to contour_plot()",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56250"
}
```



---

archive/issue_comments_029571.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nabergeron: what is the status of your work on this patch (and how can we help?)  It looks like it is very useful.  Thanks for doing this!",
    "created_at": "2009-05-30T08:27:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29571",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:5" align="right">comment:5</div>

abergeron: what is the status of your work on this patch (and how can we help?)  It looks like it is very useful.  Thanks for doing this!



---

archive/issue_comments_029572.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nIt is on my TODO list (which is rather long) and which I started attacking since the end of my finals.\n\nI don't have a recent version of Sage on my machine right now so it'll be a couple of days before I can rebase it.\n\nIf somebody else wants to do it in the meantime, that could help.",
    "created_at": "2009-05-31T02:48:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29572",
    "user": "https://github.com/sagetrac-abergeron"
}
```

<div id="comment:6" align="right">comment:6</div>

It is on my TODO list (which is rather long) and which I started attacking since the end of my finals.

I don't have a recent version of Sage on my machine right now so it'll be a couple of days before I can rebase it.

If somebody else wants to do it in the meantime, that could help.



---

archive/issue_comments_029573.json:
```json
{
    "body": "Author: **Arnaud Bergeron, Karl-Dieter Crisman**",
    "created_at": "2009-10-05T20:24:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29573",
    "user": "https://github.com/kcrisman"
}
```

Author: **Arnaud Bergeron, Karl-Dieter Crisman**



---

archive/issue_comments_029574.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nHere is a somewhat significant rebase based on 4.1.2.alpha2, also more or less ReSTified.  There have been some definite changes in this file and matplotlib since then, so a few things now behave slightly differently from the original patch; however, I think they are just different, not wrong (especially when it comes to axes).   In particular I hope I got the option popping in the right spots.\n\nGreat thanks to Arnaud's work here - especially labeling contours is a great thing to have now!",
    "created_at": "2009-10-05T20:24:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29574",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:7" align="right">comment:7</div>

Here is a somewhat significant rebase based on 4.1.2.alpha2, also more or less ReSTified.  There have been some definite changes in this file and matplotlib since then, so a few things now behave slightly differently from the original patch; however, I think they are just different, not wrong (especially when it comes to axes).   In particular I hope I got the option popping in the right spots.

Great thanks to Arnaud's work here - especially labeling contours is a great thing to have now!



---

archive/issue_events_056251.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-10-05T20:24:53Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "title_is": "Add style and labels to contour_plot()",
    "title_was": "[needs rebase] Add style and labels to contour_plot()",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56251"
}
```



---

archive/issue_events_056252.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-10-05T20:24:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56252"
}
```



---

archive/issue_comments_029575.json:
```json
{
    "body": "Work Issues: **rebase**",
    "created_at": "2009-10-06T20:52:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29575",
    "user": "https://github.com/jasongrout"
}
```

Work Issues: **rebase**



---

archive/issue_events_056253.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-10-06T20:52:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56253"
}
```



---

archive/issue_events_056254.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-10-06T20:52:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56254"
}
```



---

archive/issue_comments_029576.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nUnfortunately, it still needs more rebasing.  Here is the result of applying it to 4.1.2.rc0:\n\n```\njason@sage:~/sage-4.1.2.rc0-sage.math.washington.edu-x86_64-Linux/devel/sage/sage$ hg qpush\napplying trac_4898-contour-labels-rebase.patch\npatching file sage/plot/contour_plot.py\nHunk #11 succeeded at 517 with fuzz 1 (offset 3 lines).\nHunk #12 FAILED at 549\n1 out of 12 hunks FAILED -- saving rejects to file sage/plot/contour_plot.py.rej\npatch failed, unable to continue (try -v)\npatch failed, rejects left in working dir\nErrors during apply, please fix and refresh trac_4898-contour-labels-rebase.patch\n```\n\nI'm excited for this to get in!",
    "created_at": "2009-10-06T20:52:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29576",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:8" align="right">comment:8</div>

Unfortunately, it still needs more rebasing.  Here is the result of applying it to 4.1.2.rc0:

```
jason@sage:~/sage-4.1.2.rc0-sage.math.washington.edu-x86_64-Linux/devel/sage/sage$ hg qpush
applying trac_4898-contour-labels-rebase.patch
patching file sage/plot/contour_plot.py
Hunk #11 succeeded at 517 with fuzz 1 (offset 3 lines).
Hunk #12 FAILED at 549
1 out of 12 hunks FAILED -- saving rejects to file sage/plot/contour_plot.py.rej
patch failed, unable to continue (try -v)
patch failed, rejects left in working dir
Errors during apply, please fix and refresh trac_4898-contour-labels-rebase.patch
```

I'm excited for this to get in!



---

archive/issue_comments_029577.json:
```json
{
    "body": "Attachment: **[trac_4898-contour-labels-rebase.2.patch.gz](https://github.com/sagemath/sage/files/ticket4898/trac_4898-contour-labels-rebase.2.patch.gz)**\n\nBased on 4.1.2.rc1.alpha3",
    "created_at": "2009-10-08T02:39:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29577",
    "user": "https://github.com/kcrisman"
}
```

Attachment: **[trac_4898-contour-labels-rebase.2.patch.gz](https://github.com/sagemath/sage/files/ticket4898/trac_4898-contour-labels-rebase.2.patch.gz)**

Based on 4.1.2.rc1.alpha3



---

archive/issue_events_056255.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-10-08T02:41:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56255"
}
```



---

archive/issue_events_056256.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-10-08T02:41:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56256"
}
```



---

archive/issue_comments_029578.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nHmm, I had intended to delete the previous version...\n\nAnyway, there was a one-character (in fact, one space) thing that prevented it from applying.  So silly.  It should actually apply to rc0 as well, I suspect, but you never know... anyway, can you check that if the only issue is a space, that you fix it manually to at least see if doctests pass?",
    "created_at": "2009-10-08T02:41:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29578",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:9" align="right">comment:9</div>

Hmm, I had intended to delete the previous version...

Anyway, there was a one-character (in fact, one space) thing that prevented it from applying.  So silly.  It should actually apply to rc0 as well, I suspect, but you never know... anyway, can you check that if the only issue is a space, that you fix it manually to at least see if doctests pass?



---

archive/issue_comments_029579.json:
```json
{
    "body": "Attachment: **[trac_4898-contour-labels-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket4898/trac_4898-contour-labels-rebase.patch.gz)**\n\nBased on 4.2 - apply only this!",
    "created_at": "2009-11-05T19:55:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29579",
    "user": "https://github.com/kcrisman"
}
```

Attachment: **[trac_4898-contour-labels-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket4898/trac_4898-contour-labels-rebase.patch.gz)**

Based on 4.2 - apply only this!



---

archive/issue_comments_029580.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nI tweaked two of the defaults and add a bit more documentation.  This is great; I'm excited to see this go in!",
    "created_at": "2009-11-10T16:50:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29580",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:10" align="right">comment:10</div>

I tweaked two of the defaults and add a bit more documentation.  This is great; I'm excited to see this go in!



---

archive/issue_events_056257.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-11-10T16:50:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56257"
}
```



---

archive/issue_events_056258.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-11-10T16:50:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56258"
}
```



---

archive/issue_comments_029581.json:
```json
{
    "body": "Reviewer: **Jason Grout**",
    "created_at": "2009-11-10T16:50:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29581",
    "user": "https://github.com/jasongrout"
}
```

Reviewer: **Jason Grout**



---

archive/issue_comments_029582.json:
```json
{
    "body": "Changed work issues from **rebase** to none",
    "created_at": "2009-11-10T16:50:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29582",
    "user": "https://github.com/jasongrout"
}
```

Changed work issues from **rebase** to none



---

archive/issue_comments_029583.json:
```json
{
    "body": "Attachment: **[trac-4898-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket4898/trac-4898-reviewer.patch.gz)**\n\napply on top of previous patch",
    "created_at": "2009-11-10T17:16:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29583",
    "user": "https://github.com/jasongrout"
}
```

Attachment: **[trac-4898-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket4898/trac-4898-reviewer.patch.gz)**

apply on top of previous patch



---

archive/issue_comments_029584.json:
```json
{
    "body": "Merged: **sage-4.2.1.rc0**",
    "created_at": "2009-11-12T06:53:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4898#issuecomment-29584",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.2.1.rc0**



---

archive/issue_events_056259.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-12T06:53:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56259"
}
```



---

archive/issue_events_056260.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-12T06:53:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56260"
}
```



---

archive/issue_events_056261.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-22T05:37:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "milestone_number": null,
    "milestone_title": "sage-4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56261"
}
```



---

archive/issue_events_056262.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-22T05:37:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4898",
    "milestone_number": null,
    "milestone_title": "sage-4.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4898#event-56262"
}
```

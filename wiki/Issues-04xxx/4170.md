# Issue 4170: symbolic ring does not accept python longs

archive/issues_004170.json:
```json
{
    "body": "The easy fix is to add it to the big list in `_coerce_impl` at sage.calculus.calculus.py:481. Because\n\n```\nsage: ZZ.has_coerce_map_from(long)\nTrue\nsage: SR.has_coerce_map_from(ZZ)\nTrue\n```\n\nThis should be handled in the new model, but symbolics are being changed anyways. \n\n**Assignee:** @burcin\n\nIssue created by migration from https://trac.sagemath.org/ticket/4170\n\n",
    "closed_at": "2008-09-23T01:51:33Z",
    "created_at": "2008-09-23T01:10:30Z",
    "labels": [
        "component: calculus",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.3",
    "title": "symbolic ring does not accept python longs",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/4170",
    "user": "https://github.com/robertwb"
}
```
The easy fix is to add it to the big list in `_coerce_impl` at sage.calculus.calculus.py:481. Because

```
sage: ZZ.has_coerce_map_from(long)
True
sage: SR.has_coerce_map_from(ZZ)
True
```

This should be handled in the new model, but symbolics are being changed anyways. 

**Assignee:** @burcin

Issue created by migration from https://trac.sagemath.org/ticket/4170





---

archive/issue_events_024075.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2008-09-23T01:15:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4170",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4170#event-24075"
}
```



---

archive/attachments_004606.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "4170-SR-long.patch",
    "asset_url": "tarball://root/attachments/ticket4170/4170-SR-long.patch",
    "created_at": "2008-09-23T01:15:49Z",
    "issue": "https://github.com/sagemath/sage/issues/4170",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket4170/4170-SR-long.patch",
    "user": "https://github.com/robertwb"
}
```



---

archive/issue_comments_024695.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [4170-SR-long.patch](https://github.com/sagemath/sage/files/ticket4170/4170-SR-long.patch)",
    "created_at": "2008-09-23T01:15:49Z",
    "issue": "https://github.com/sagemath/sage/issues/4170",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4170#issuecomment-24695",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:1'></a>
**Attachment:** [4170-SR-long.patch](https://github.com/sagemath/sage/files/ticket4170/4170-SR-long.patch)



---

archive/issue_events_024076.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-23T01:17:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4170",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4170#event-24076"
}
```



---

archive/issue_events_024077.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-23T01:17:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4170",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4170#event-24077"
}
```



---

archive/issue_comments_024696.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good to me. Assuming this passes doctests positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-09-23T01:17:34Z",
    "issue": "https://github.com/sagemath/sage/issues/4170",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4170#issuecomment-24696",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Looks good to me. Assuming this passes doctests positive review.

Cheers,

Michael



---

archive/issue_events_024078.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-23T01:51:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4170",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4170#event-24078"
}
```



---

archive/issue_events_024079.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-23T01:51:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4170",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4170#event-24079"
}
```



---

archive/issue_comments_024697.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.1.3.alpha1",
    "created_at": "2008-09-23T01:51:33Z",
    "issue": "https://github.com/sagemath/sage/issues/4170",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4170#issuecomment-24697",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.1.3.alpha1

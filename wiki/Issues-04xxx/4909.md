# Issue 4909: [needs little work] convert sage.dsage.* docstrings to Sphinx

archive/issues_004909.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\n\n\nComponent: **documentation**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4909_\n\n",
    "closed_at": "2009-02-24T18:33:39Z",
    "created_at": "2009-01-01T22:50:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "[needs little work] convert sage.dsage.* docstrings to Sphinx",
    "type": "issue",
    "updated_at": "2009-02-24T18:33:39Z",
    "url": "https://github.com/sagemath/sage/issues/4909",
    "user": "https://github.com/mwhansen"
}
```
<div id="comment:0"></div>



Component: **documentation**

_Issue created by migration from https://trac.sagemath.org/ticket/4909_





---

archive/issue_events_056369.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-01T22:50:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4909",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4909#event-56369"
}
```



---

archive/issue_events_056370.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-01T22:50:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4909",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4909#event-56370"
}
```



---

archive/issue_events_056371.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-01T22:50:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4909",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4909#event-56371"
}
```



---

archive/issue_events_056372.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-01T22:50:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4909",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4909#event-56372"
}
```



---

archive/issue_comments_029670.json:
```json
{
    "body": "Attachment: **[trac_4909.patch.gz](https://github.com/sagemath/sage/files/ticket4909/trac_4909.patch.gz)**",
    "created_at": "2009-01-02T02:25:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4909#issuecomment-29670",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[trac_4909.patch.gz](https://github.com/sagemath/sage/files/ticket4909/trac_4909.patch.gz)**



---

archive/issue_events_056373.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-02T02:25:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4909",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4909#event-56373"
}
```



---

archive/issue_comments_029671.json:
```json
{
    "body": "Attachment: **[sage.dsage-final.patch.gz](https://github.com/sagemath/sage/files/ticket4909/sage.dsage-final.patch.gz)**",
    "created_at": "2009-02-21T19:17:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4909#issuecomment-29671",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[sage.dsage-final.patch.gz](https://github.com/sagemath/sage/files/ticket4909/sage.dsage-final.patch.gz)**



---

archive/issue_events_056374.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-02-24T17:42:44Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4909",
    "title_is": "[needs little work] convert sage.dsage.* docstrings to Sphinx",
    "title_was": "convert sage.dsage.* docstrings to Sphinx",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4909#event-56374"
}
```



---

archive/issue_comments_029672.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nA little problem:\n\n```\nNote that configuration files will be stored in the  \n    directory \\code{\\$DOT\\_SAGE/dsage}.\n```\nIs replaced now by\n\n```\nNote that configuration files will be stored in the directory \n``$DOT \nSage/dsage``. \n``` \nthe \"_\" must be kept. \n\nOtherwise this is correct. \n\nCheers,\n\nFlorent",
    "created_at": "2009-02-24T17:42:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4909#issuecomment-29672",
    "user": "https://github.com/hivert"
}
```

<div id="comment:2" align="right">comment:2</div>

A little problem:

```
Note that configuration files will be stored in the  
    directory \code{\$DOT\_SAGE/dsage}.
```
Is replaced now by

```
Note that configuration files will be stored in the directory 
``$DOT 
Sage/dsage``. 
``` 
the "_" must be kept. 

Otherwise this is correct. 

Cheers,

Florent



---

archive/issue_events_056375.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-24T18:33:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4909",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4909#event-56375"
}
```



---

archive/issue_events_056376.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-24T18:33:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4909",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4909#event-56376"
}
```



---

archive/issue_comments_029673.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.4.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-24T18:33:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4909#issuecomment-29673",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.4.alpha0.

Cheers,

Michael

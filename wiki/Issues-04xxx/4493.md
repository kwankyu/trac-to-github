# Issue 4493: derivative of a vector and a matrix

archive/issues_004493.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nIt would be handy in differential equations to be able to do differentiation and integration of matrices and vectors, with the exact same answer as obtained by using the apply_map method.\n\nComponent: **calculus**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4493_\n\n",
    "closed_at": "2008-12-07T09:59:29Z",
    "created_at": "2008-11-11T18:22:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20calculus",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "derivative of a vector and a matrix",
    "type": "issue",
    "updated_at": "2008-12-07T09:59:29Z",
    "url": "https://github.com/sagemath/sage/issues/4493",
    "user": "https://github.com/jasongrout"
}
```
<div id="comment:0"></div>

It would be handy in differential equations to be able to do differentiation and integration of matrices and vectors, with the exact same answer as obtained by using the apply_map method.

Component: **calculus**

_Issue created by migration from https://trac.sagemath.org/ticket/4493_





---

archive/issue_events_050559.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-11-11T18:22:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4493",
    "milestone_number": null,
    "milestone_title": "sage-3.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4493#event-50559"
}
```



---

archive/issue_events_050560.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-11-11T18:22:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4493",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "c: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4493#event-50560"
}
```



---

archive/issue_events_050561.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-11-11T18:22:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4493",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4493#event-50561"
}
```



---

archive/issue_events_050562.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-11-11T18:22:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4493",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4493#event-50562"
}
```



---

archive/issue_events_050563.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2008-11-11T18:22:50Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4493",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4493#event-50563"
}
```



---

archive/issue_events_050564.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2008-12-05T06:45:03Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/4493",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4493#event-50564"
}
```



---

archive/issue_events_050565.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-12-05T06:45:03Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4493",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4493#event-50565"
}
```



---

archive/issue_comments_026173.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis depends on #4713\n\nI guess the integral of a matrix doesn't come up that often, so I'm changing my request.  Besides, the integral can calculated using the apply_map function at #4713 for vectors or the existing apply_map function for matrices.",
    "created_at": "2008-12-05T08:14:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4493#issuecomment-26173",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:2" align="right">comment:2</div>

This depends on #4713

I guess the integral of a matrix doesn't come up that often, so I'm changing my request.  Besides, the integral can calculated using the apply_map function at #4713 for vectors or the existing apply_map function for matrices.



---

archive/issue_events_050566.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-12-05T08:14:51Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4493",
    "title_is": "derivative of a vector and a matrix",
    "title_was": "derivative and integral of a matrix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4493#event-50566"
}
```



---

archive/issue_comments_026174.json:
```json
{
    "body": "Attachment: **[vector_derivative.patch.gz](https://github.com/sagemath/sage/files/ticket4493/vector_derivative.patch.gz)**",
    "created_at": "2008-12-05T08:15:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4493#issuecomment-26174",
    "user": "https://github.com/jasongrout"
}
```

Attachment: **[vector_derivative.patch.gz](https://github.com/sagemath/sage/files/ticket4493/vector_derivative.patch.gz)**



---

archive/issue_comments_026175.json:
```json
{
    "body": "Attachment: **[matrix-derivative.patch.gz](https://github.com/sagemath/sage/files/ticket4493/matrix-derivative.patch.gz)**",
    "created_at": "2008-12-05T08:31:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4493#issuecomment-26175",
    "user": "https://github.com/jasongrout"
}
```

Attachment: **[matrix-derivative.patch.gz](https://github.com/sagemath/sage/files/ticket4493/matrix-derivative.patch.gz)**



---

archive/issue_events_050567.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-12-05T08:32:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4493",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4493#event-50567"
}
```



---

archive/issue_events_050568.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-12-06T22:21:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4493",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4493#event-50568"
}
```



---

archive/issue_events_050569.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-12-06T22:21:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4493",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4493#event-50569"
}
```



---

archive/issue_events_050570.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-07T09:59:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4493",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4493#event-50570"
}
```



---

archive/issue_events_050571.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-07T09:59:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4493",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4493#event-50571"
}
```



---

archive/issue_comments_026176.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nMerged both patches in Sage 3.2.2.alpha1",
    "created_at": "2008-12-07T09:59:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4493#issuecomment-26176",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Merged both patches in Sage 3.2.2.alpha1

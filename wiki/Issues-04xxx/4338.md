# Issue 4338: optimize computation of victor_miller basis and use this for level 1 modular forms

archive/issues_004338.json:
```json
{
    "body": "Craig Citro is working on this.\n\n**Assignee:** citro\n\nIssue created by migration from https://trac.sagemath.org/ticket/4338\n\n",
    "closed_at": "2008-10-22T19:07:41Z",
    "created_at": "2008-10-22T17:47:26Z",
    "labels": [
        "component: modular forms",
        "enhancement",
        "duplicate"
    ],
    "title": "optimize computation of victor_miller basis and use this for level 1 modular forms",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/4338",
    "user": "https://github.com/williamstein"
}
```
Craig Citro is working on this.

**Assignee:** citro

Issue created by migration from https://trac.sagemath.org/ticket/4338





---

archive/issue_comments_026044.json:
```json
{
    "body": "**Changing assignee** from @craigcitro to citro.",
    "created_at": "2008-10-22T17:47:39Z",
    "issue": "https://github.com/sagemath/sage/issues/4338",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4338#issuecomment-26044",
    "user": "https://github.com/williamstein"
}
```

**Changing assignee** from @craigcitro to citro.



---

archive/issue_events_025307.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-22T19:07:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4338",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4338#event-25307"
}
```



---

archive/issue_comments_026045.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis is a dupe of #4340.\n\nCheers,\n\nMichael",
    "created_at": "2008-10-22T19:07:41Z",
    "issue": "https://github.com/sagemath/sage/issues/4338",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4338#issuecomment-26045",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
This is a dupe of #4340.

Cheers,

Michael



---

archive/issue_events_025308.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-22T19:07:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4338",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4338#event-25308"
}
```



---

archive/issue_events_025309.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-22T19:07:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4338",
    "milestone": "sage-3.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4338#event-25309"
}
```

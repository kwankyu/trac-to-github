# Issue 4427: use RTLD symbols instead of constants in init_singular

archive/issues_004427.json:
```json
{
    "body": "not doing so is very, very, **very** bad for portability.\n\n**Assignee:** @malb\n\nIssue created by migration from https://trac.sagemath.org/ticket/4427\n\n",
    "closed_at": "2008-11-04T21:33:42Z",
    "created_at": "2008-11-02T19:23:17Z",
    "labels": [
        "component: commutative algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2",
    "title": "use RTLD symbols instead of constants in init_singular",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/4427",
    "user": "https://github.com/malb"
}
```
not doing so is very, very, **very** bad for portability.

**Assignee:** @malb

Issue created by migration from https://trac.sagemath.org/ticket/4427





---

archive/issue_events_025956.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-02T19:26:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4427",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4427#event-25956"
}
```



---

archive/attachments_004951.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "rtld_consts.patch",
    "asset_url": "tarball://root/attachments/ticket4427/rtld_consts.patch",
    "created_at": "2008-11-02T19:26:16Z",
    "issue": "https://github.com/sagemath/sage/issues/4427",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket4427/rtld_consts.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```



---

archive/issue_comments_026661.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [rtld_consts.patch](https://github.com/sagemath/sage/files/ticket4427/rtld_consts.patch)\n\nNice, positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-11-02T19:26:16Z",
    "issue": "https://github.com/sagemath/sage/issues/4427",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4427#issuecomment-26661",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
**Attachment:** [rtld_consts.patch](https://github.com/sagemath/sage/files/ticket4427/rtld_consts.patch)

Nice, positive review.

Cheers,

Michael



---

archive/issue_comments_026662.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.2.alpha3",
    "created_at": "2008-11-04T21:33:42Z",
    "issue": "https://github.com/sagemath/sage/issues/4427",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4427#issuecomment-26662",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.2.alpha3



---

archive/issue_events_025957.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-04T21:33:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4427",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4427#event-25957"
}
```



---

archive/issue_events_025958.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-04T21:33:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4427",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4427#event-25958"
}
```

# Issue 4963: Make a screen-full of whitespace at the bottom of the notebook cells

archive/issues_004963.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nNot having this causes interacts to jump off the page when updating, for example.  See http://groups.google.com/group/sage-devel/browse_thread/thread/51b538c8212fa2a/b61655921eb0ebab\n\nCC:  @mwhansen @TimothyClemans\n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4963_\n\n",
    "closed_at": "2009-01-23T09:40:00Z",
    "created_at": "2009-01-11T00:55:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make a screen-full of whitespace at the bottom of the notebook cells",
    "type": "issue",
    "updated_at": "2009-01-23T09:40:00Z",
    "url": "https://github.com/sagemath/sage/issues/4963",
    "user": "https://github.com/jasongrout"
}
```
<div id="comment:0"></div>

Not having this causes interacts to jump off the page when updating, for example.  See http://groups.google.com/group/sage-devel/browse_thread/thread/51b538c8212fa2a/b61655921eb0ebab

CC:  @mwhansen @TimothyClemans

Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/4963_





---

archive/issue_events_057054.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-01-11T00:55:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4963#event-57054"
}
```



---

archive/issue_events_057055.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-01-11T00:55:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4963#event-57055"
}
```



---

archive/issue_events_057056.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-01-11T00:55:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4963#event-57056"
}
```



---

archive/issue_events_057057.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-01-11T00:55:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4963#event-57057"
}
```



---

archive/issue_events_057058.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-11T00:55:31Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4963#event-57058"
}
```



---

archive/issue_comments_030071.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nFixing this might also help in resolving #2629",
    "created_at": "2009-01-11T00:58:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4963#issuecomment-30071",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:1" align="right">comment:1</div>

Fixing this might also help in resolving #2629



---

archive/issue_events_057059.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-14T08:01:53Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4963#event-57059"
}
```



---

archive/issue_events_057060.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-01-14T08:01:53Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4963#event-57060"
}
```



---

archive/issue_events_057061.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-01-21T13:30:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4963#event-57061"
}
```



---

archive/issue_comments_030072.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nAfter testing with Firefox 2 and 3 and Safari on OSX.4 PPC, it seems to perform as advertised.  So positive review of its effect!  This is very useful to those of us who use interacts a lot in class, where the jumping away from the interact makes Sage look cheap as in beer, not free as in speech (or beer).\n\nIt does not resolve #2629 (though it makes it less likely to happen; see comments on #2629).\n\nBut I request another review or additional work, because the aesthetics of adding that margin-bottom 80% are still hackish at best.  For instance, very often the browser does not now jump to the next cell (where the focus has shifted), even if the jump would be fairly minimal but at the bottom of the browser.  Is this a somewhat different behavior than in the past?  It could be especially weird if someone hasn't used the notebook before, and hence doesn't know what happened to the focus after evaluating - again, if this is new behavior; perhaps it is not.\n\nGiven the discussion mentioned in the description, it sounds like this is not a big deal, but I don't feel comfortable giving final positive review on that aspect of things, since I haven't worked on the notebook, I just use it.",
    "created_at": "2009-01-21T17:41:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4963#issuecomment-30072",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:4" align="right">comment:4</div>

After testing with Firefox 2 and 3 and Safari on OSX.4 PPC, it seems to perform as advertised.  So positive review of its effect!  This is very useful to those of us who use interacts a lot in class, where the jumping away from the interact makes Sage look cheap as in beer, not free as in speech (or beer).

It does not resolve #2629 (though it makes it less likely to happen; see comments on #2629).

But I request another review or additional work, because the aesthetics of adding that margin-bottom 80% are still hackish at best.  For instance, very often the browser does not now jump to the next cell (where the focus has shifted), even if the jump would be fairly minimal but at the bottom of the browser.  Is this a somewhat different behavior than in the past?  It could be especially weird if someone hasn't used the notebook before, and hence doesn't know what happened to the focus after evaluating - again, if this is new behavior; perhaps it is not.

Given the discussion mentioned in the description, it sounds like this is not a big deal, but I don't feel comfortable giving final positive review on that aspect of things, since I haven't worked on the notebook, I just use it.



---

archive/issue_events_057062.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-01-21T17:41:39Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "title_is": "[needs work or second review] Make a screen-full of whitespace at the bottom of the notebook cells",
    "title_was": "Make a screen-full of whitespace at the bottom of the notebook cells",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4963#event-57062"
}
```



---

archive/issue_comments_030073.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nBefore this patch, we still have problems with the browser scrolling not following the focus or tab-completion menu, like described above.  I thought maybe this fix would help, but apparently more needs to be done.\n\nI did notice that this patch also extends the \"Home\" page of the notebook which lists the worksheets.  The problem there is that the \"body\" command used in each page is the same.  I've updated the patch to fix this.  This should probably be re-reviewed.  One thing that might be reviewed is that I actually changed *two* body elements; is one of them redundant?",
    "created_at": "2009-01-21T23:31:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4963#issuecomment-30073",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:5" align="right">comment:5</div>

Before this patch, we still have problems with the browser scrolling not following the focus or tab-completion menu, like described above.  I thought maybe this fix would help, but apparently more needs to be done.

I did notice that this patch also extends the "Home" page of the notebook which lists the worksheets.  The problem there is that the "body" command used in each page is the same.  I've updated the patch to fix this.  This should probably be re-reviewed.  One thing that might be reviewed is that I actually changed *two* body elements; is one of them redundant?



---

archive/issue_events_057063.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-01-21T23:31:57Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "title_is": "Make a screen-full of whitespace at the bottom of the notebook cells",
    "title_was": "[needs work or second review] Make a screen-full of whitespace at the bottom of the notebook cells",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4963#event-57063"
}
```



---

archive/issue_comments_030074.json:
```json
{
    "body": "Attachment: **[notebook-whitespace.patch.gz](https://github.com/sagemath/sage/files/ticket4963/notebook-whitespace.patch.gz)**",
    "created_at": "2009-01-21T23:32:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4963#issuecomment-30074",
    "user": "https://github.com/jasongrout"
}
```

Attachment: **[notebook-whitespace.patch.gz](https://github.com/sagemath/sage/files/ticket4963/notebook-whitespace.patch.gz)**



---

archive/issue_comments_030075.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nThat is, in fact, a large amount of space.  Well done.",
    "created_at": "2009-01-22T18:59:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4963#issuecomment-30075",
    "user": "https://github.com/boothby"
}
```

<div id="comment:6" align="right">comment:6</div>

That is, in fact, a large amount of space.  Well done.



---

archive/issue_events_057064.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-22T18:59:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4963#event-57064"
}
```



---

archive/issue_events_057065.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-22T18:59:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4963#event-57065"
}
```



---

archive/issue_comments_030076.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nMerged in Sage 3.3.alpha1\n\nCheers,\n\nMichael",
    "created_at": "2009-01-23T09:40:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4963#issuecomment-30076",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:7" align="right">comment:7</div>

Merged in Sage 3.3.alpha1

Cheers,

Michael



---

archive/issue_events_057066.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-23T09:40:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4963#event-57066"
}
```



---

archive/issue_events_057067.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-23T09:40:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4963",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4963#event-57067"
}
```

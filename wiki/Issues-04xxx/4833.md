# Issue 4833: sage -upgrade should complain when queues are used and patches are applied

archive/issues_004833.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nI've messed up at least one upgrade by accidentally having mercurial queue patches applied when trying to upgrade.  sage -upgrade should at least warn the user, and maybe give an option of popping off all of the patches before upgrading or just quitting.\n\n\nCC:  @sagetrac-jthurber\n\nComponent: **build**\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4833_\n\n",
    "closed_at": "2014-01-10T08:47:31Z",
    "created_at": "2008-12-19T19:32:57Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20build",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage -upgrade should complain when queues are used and patches are applied",
    "type": "issue",
    "updated_at": "2014-01-10T08:47:31Z",
    "url": "https://github.com/sagemath/sage/issues/4833",
    "user": "https://github.com/jasongrout"
}
```
<div id="comment:0"></div>

I've messed up at least one upgrade by accidentally having mercurial queue patches applied when trying to upgrade.  sage -upgrade should at least warn the user, and maybe give an option of popping off all of the patches before upgrading or just quitting.


CC:  @sagetrac-jthurber

Component: **build**

Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/4833_





---

archive/issue_events_055349.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-12-19T19:32:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4833#event-55349"
}
```



---

archive/issue_events_055350.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-12-19T19:32:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20build",
    "label_color": "0000b0",
    "label_name": "c: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4833#event-55350"
}
```



---

archive/issue_events_055351.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-12-19T19:32:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4833#event-55351"
}
```



---

archive/issue_events_055352.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-12-19T19:32:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4833#event-55352"
}
```



---

archive/issue_events_055353.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-19T19:32:57Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4833#event-55353"
}
```



---

archive/issue_comments_029147.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nSee also #4759",
    "created_at": "2009-02-21T15:42:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4833#issuecomment-29147",
    "user": "https://github.com/robertwb"
}
```

<div id="comment:1" align="right">comment:1</div>

See also #4759



---

archive/issue_events_055354.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4833#event-55354"
}
```



---

archive/issue_events_055355.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4833#event-55355"
}
```



---

archive/issue_comments_029148.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2014-01-09T09:36:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4833#issuecomment-29148",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_055356.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-01-09T09:36:59Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4833#event-55356"
}
```



---

archive/issue_events_055357.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-01-09T09:36:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4833#event-55357"
}
```



---

archive/issue_comments_029149.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nObsolete because of `git`",
    "created_at": "2014-01-09T09:36:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4833#issuecomment-29149",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">comment:4</div>

Obsolete because of `git`



---

archive/issue_events_055358.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-01-09T09:37:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4833#event-55358"
}
```



---

archive/issue_events_055359.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-01-09T09:37:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4833#event-55359"
}
```



---

archive/issue_events_055360.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-01-10T08:47:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4833#event-55360"
}
```



---

archive/issue_events_055361.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-01-10T08:47:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "c6c6c6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4833#event-55361"
}
```



---

archive/issue_events_055362.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-01-10T08:47:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4833#event-55362"
}
```



---

archive/issue_events_055363.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-01-10T08:47:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4833",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4833#event-55363"
}
```

# Issue 4679: Spelling errors in documentation for sage.rings.polynomial.polynomial_element

archive/issues_004679.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nThere are a number of small spelling errors in the docstrings for the class sage.rings.polynomial.polynomial_element. \n\nA patch will be forthcoming.\n\nComponent: **documentation**\n\nKeywords: **polynomial**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4679_\n\n",
    "closed_at": "2008-12-04T13:36:25Z",
    "created_at": "2008-12-02T18:17:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20documentation"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Spelling errors in documentation for sage.rings.polynomial.polynomial_element",
    "type": "issue",
    "updated_at": "2008-12-04T13:36:25Z",
    "url": "https://github.com/sagemath/sage/issues/4679",
    "user": "https://github.com/tjl"
}
```
<div id="comment:0"></div>

There are a number of small spelling errors in the docstrings for the class sage.rings.polynomial.polynomial_element. 

A patch will be forthcoming.

Component: **documentation**

Keywords: **polynomial**

_Issue created by migration from https://trac.sagemath.org/ticket/4679_





---

archive/issue_events_053205.json:
```json
{
    "actor": "https://github.com/tjl",
    "created_at": "2008-12-02T18:17:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-53205"
}
```



---

archive/issue_events_053206.json:
```json
{
    "actor": "https://github.com/tjl",
    "created_at": "2008-12-02T18:17:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-53206"
}
```



---

archive/issue_events_053207.json:
```json
{
    "actor": "https://github.com/tjl",
    "created_at": "2008-12-02T18:17:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-53207"
}
```



---

archive/issue_events_053208.json:
```json
{
    "actor": "https://github.com/tjl",
    "created_at": "2008-12-02T18:17:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-53208"
}
```



---

archive/issue_comments_028007.json:
```json
{
    "body": "Spelling errors patch for Trac 4679.",
    "created_at": "2008-12-02T18:19:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4679#issuecomment-28007",
    "user": "https://github.com/tjl"
}
```

Spelling errors patch for Trac 4679.



---

archive/issue_comments_028008.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_4679.patch.gz](https://github.com/sagemath/sage/files/ticket4679/trac_4679.patch.gz)**\n\nAdded patch to fix spelling errors.",
    "created_at": "2008-12-02T18:20:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4679#issuecomment-28008",
    "user": "https://github.com/tjl"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_4679.patch.gz](https://github.com/sagemath/sage/files/ticket4679/trac_4679.patch.gz)**

Added patch to fix spelling errors.



---

archive/issue_events_053209.json:
```json
{
    "actor": "https://github.com/tjl",
    "created_at": "2008-12-02T18:20:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-53209"
}
```



---

archive/issue_events_053210.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-12-03T00:35:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-53210"
}
```



---

archive/issue_events_053211.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-12-03T00:35:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-53211"
}
```



---

archive/issue_comments_028009.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nTim,\n\nplease make sure to assign a milestone when opening a ticket. This is usually the current merge milestone, i.e. 3.2.2 at the moment.\n\nCheers,\n\nMichael",
    "created_at": "2008-12-03T00:41:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4679#issuecomment-28009",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Tim,

please make sure to assign a milestone when opening a ticket. This is usually the current merge milestone, i.e. 3.2.2 at the moment.

Cheers,

Michael



---

archive/issue_events_053212.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-03T00:41:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "milestone_number": null,
    "milestone_title": "sage-3.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-53212"
}
```



---

archive/issue_comments_028010.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThanks. I wasn't sure what milestone to assign.",
    "created_at": "2008-12-03T00:52:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4679#issuecomment-28010",
    "user": "https://github.com/tjl"
}
```

<div id="comment:4" align="right">comment:4</div>

Thanks. I wasn't sure what milestone to assign.



---

archive/issue_comments_028011.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nReplying to [@tjl](#comment%3A4):\n> Thanks. I wasn't sure what milestone to assign.\n\nNo problem. The new trac release 0.11.2 we will be upgrading to in about a week let's the admin assign default milestones from a panel, so in the future this will become a non-issue.\n\nCheers,\n\nMichael",
    "created_at": "2008-12-03T00:53:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4679#issuecomment-28011",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Replying to [@tjl](#comment%3A4):
> Thanks. I wasn't sure what milestone to assign.

No problem. The new trac release 0.11.2 we will be upgrading to in about a week let's the admin assign default milestones from a panel, so in the future this will become a non-issue.

Cheers,

Michael



---

archive/issue_events_053213.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-04T13:36:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-53213"
}
```



---

archive/issue_events_053214.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-04T13:36:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-53214"
}
```



---

archive/issue_comments_028012.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nMerged in Sage 3.2.2.alpha0",
    "created_at": "2008-12-04T13:36:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4679#issuecomment-28012",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Merged in Sage 3.2.2.alpha0

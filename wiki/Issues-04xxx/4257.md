# Issue 4257: support for Singular's  'intmat' and 'intvec'

archive/issues_004257.json:
```json
{
    "body": "This now works:\n\n```\nsage: A = random_matrix(ZZ,3,3); A\n[ -8   2   0]\n[  0   1  -1]\n[  2   1 -95]\nsage: As = singular(A); As\n-8     2     0\n 0     1    -1\n 2     1   -95\nsage: As._sage_()\n[ -8   2   0]\n[  0   1  -1]\n[  2   1 -95]\n```\n\n**Assignee:** @malb\n\n**CC:**  singular\n\nIssue created by migration from https://trac.sagemath.org/ticket/4257\n\n",
    "closed_at": "2008-10-11T06:40:58Z",
    "created_at": "2008-10-09T21:54:29Z",
    "labels": [
        "component: interfaces",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.3",
    "title": "support for Singular's  'intmat' and 'intvec'",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/4257",
    "user": "https://github.com/malb"
}
```
This now works:

```
sage: A = random_matrix(ZZ,3,3); A
[ -8   2   0]
[  0   1  -1]
[  2   1 -95]
sage: As = singular(A); As
-8     2     0
 0     1    -1
 2     1   -95
sage: As._sage_()
[ -8   2   0]
[  0   1  -1]
[  2   1 -95]
```

**Assignee:** @malb

**CC:**  singular

Issue created by migration from https://trac.sagemath.org/ticket/4257





---

archive/attachments_004705.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "singular_intmat.patch",
    "asset_url": "tarball://root/attachments/ticket4257/singular_intmat.patch",
    "created_at": "2008-10-10T08:46:24Z",
    "issue": "https://github.com/sagemath/sage/issues/4257",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket4257/singular_intmat.patch",
    "user": "https://github.com/malb"
}
```



---

archive/issue_comments_025286.json:
```json
{
    "body": "**Attachment:** [singular_intmat.patch](https://github.com/sagemath/sage/files/ticket4257/singular_intmat.patch)",
    "created_at": "2008-10-10T08:46:24Z",
    "issue": "https://github.com/sagemath/sage/issues/4257",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4257#issuecomment-25286",
    "user": "https://github.com/malb"
}
```

**Attachment:** [singular_intmat.patch](https://github.com/sagemath/sage/files/ticket4257/singular_intmat.patch)



---

archive/issue_events_024654.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-10-10T16:18:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4257",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4257#event-24654"
}
```



---

archive/issue_comments_025287.json:
```json
{
    "body": "<a id='comment:1'></a>\nLooks good to me.",
    "created_at": "2008-10-10T16:18:51Z",
    "issue": "https://github.com/sagemath/sage/issues/4257",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4257#issuecomment-25287",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Looks good to me.



---

archive/issue_events_024655.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-11T06:40:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4257",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4257#event-24655"
}
```



---

archive/issue_events_024656.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-11T06:40:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4257",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4257#event-24656"
}
```



---

archive/issue_comments_025288.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.1.3.rc0",
    "created_at": "2008-10-11T06:40:58Z",
    "issue": "https://github.com/sagemath/sage/issues/4257",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4257#issuecomment-25288",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.1.3.rc0

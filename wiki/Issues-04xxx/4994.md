# Issue 4994: OSX 64 bit: add 64 bit build support for ghmm

archive/issues_004994.json:
```json
{
    "body": "spkg coming up.\n\nCheers,\n\nMichael\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/4994\n\n",
    "closed_at": "2009-01-23T00:25:50Z",
    "created_at": "2009-01-17T15:35:54Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "title": "OSX 64 bit: add 64 bit build support for ghmm",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/4994",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
spkg coming up.

Cheers,

Michael

**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/4994





---

archive/issue_events_030692.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-22T19:10:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4994",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4994#event-30692"
}
```



---

archive/issue_comments_031576.json:
```json
{
    "body": "<a id='comment:2'></a>\nThe spkg is at\n\nhttp://sage.math.washington.edu/home/mabshoff/spkgs/ghmm-20080813.p0.spkg\n\nCheers,\n\nMichael",
    "created_at": "2009-01-22T19:10:14Z",
    "issue": "https://github.com/sagemath/sage/issues/4994",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4994#issuecomment-31576",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
The spkg is at

http://sage.math.washington.edu/home/mabshoff/spkgs/ghmm-20080813.p0.spkg

Cheers,

Michael



---

archive/issue_events_030693.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-01-22T20:11:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4994",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4994#event-30693"
}
```



---

archive/issue_events_030694.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-01-22T20:11:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4994",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4994#event-30694"
}
```



---

archive/issue_comments_031577.json:
```json
{
    "body": "<a id='comment:3'></a>\n+1",
    "created_at": "2009-01-22T20:11:42Z",
    "issue": "https://github.com/sagemath/sage/issues/4994",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4994#issuecomment-31577",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:3'></a>
+1



---

archive/issue_comments_031578.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.3.alpha1",
    "created_at": "2009-01-23T00:25:50Z",
    "issue": "https://github.com/sagemath/sage/issues/4994",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4994#issuecomment-31578",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.3.alpha1



---

archive/issue_events_030695.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-23T00:25:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4994",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4994#event-30695"
}
```



---

archive/issue_events_030696.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-23T00:25:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4994",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4994#event-30696"
}
```

# Issue 4718: notebook -- move source code, upload, and help window html to template

archive/issues_004718.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "<div id=\"comment:0\"></div>\n\n\n\nCC:  @mwhansen\n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4718_\n\n",
    "closed_at": "2009-01-19T09:51:52Z",
    "created_at": "2008-12-05T18:45:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "notebook -- move source code, upload, and help window html to template",
    "type": "issue",
    "updated_at": "2009-01-19T09:53:02Z",
    "url": "https://github.com/sagemath/sage/issues/4718",
    "user": "https://github.com/TimothyClemans"
}
```
<div id="comment:0"></div>



CC:  @mwhansen

Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/4718_





---

archive/issue_events_053678.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-12-05T18:45:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4718#event-53678"
}
```



---

archive/issue_events_053679.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-12-05T18:45:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4718#event-53679"
}
```



---

archive/issue_events_053680.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-12-05T18:45:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4718#event-53680"
}
```



---

archive/issue_events_053681.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-12-05T18:45:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4718#event-53681"
}
```



---

archive/issue_events_053682.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-12-05T18:45:52Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "subject": "https://github.com/TimothyClemans",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4718#event-53682"
}
```



---

archive/issue_events_053683.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-12-05T18:51:28Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "title_is": "notebook -- move source code, upload, and help window html to template",
    "title_was": "notebook -- move source code and help window html to template",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4718#event-53683"
}
```



---

archive/issue_events_053684.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-12-05T18:51:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4718#event-53684"
}
```



---

archive/issue_comments_028293.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nTimothy,\n\nplease learn how to use hg queues and use them. This ticket will otherwise end up as potentially another giant mess and someone will have to clean it up/collapse all the patches.\n\nCheers,\n\nMichael",
    "created_at": "2008-12-05T19:00:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4718#issuecomment-28293",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Timothy,

please learn how to use hg queues and use them. This ticket will otherwise end up as potentially another giant mess and someone will have to clean it up/collapse all the patches.

Cheers,

Michael



---

archive/issue_comments_028294.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nDepends on #4552",
    "created_at": "2008-12-05T19:02:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4718#issuecomment-28294",
    "user": "https://github.com/TimothyClemans"
}
```

<div id="comment:3" align="right">comment:3</div>

Depends on #4552



---

archive/issue_comments_028295.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+This depends on #4552\n``````\n",
    "created_at": "2008-12-06T21:56:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4718#issuecomment-28295",
    "user": "https://github.com/williamstein"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+This depends on #4552
``````




---

archive/issue_events_053685.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-19T07:35:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4718#event-53685"
}
```



---

archive/issue_events_053686.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-19T07:35:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4718#event-53686"
}
```



---

archive/issue_comments_028296.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nAttachment: **[trac_4718.patch.gz](https://github.com/sagemath/sage/files/ticket4718/trac_4718.patch.gz)**\n\nLooks good and works for me.  Note that this does NOT depend on #4552.",
    "created_at": "2009-01-19T07:35:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4718#issuecomment-28296",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:5" align="right">comment:5</div>

Attachment: **[trac_4718.patch.gz](https://github.com/sagemath/sage/files/ticket4718/trac_4718.patch.gz)**

Looks good and works for me.  Note that this does NOT depend on #4552.



---

archive/issue_comments_028297.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-This depends on #4552\n+\n``````\n",
    "created_at": "2009-01-19T07:35:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4718#issuecomment-28297",
    "user": "https://github.com/mwhansen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-This depends on #4552
+
``````




---

archive/issue_events_053687.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-19T07:35:06Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4718#event-53687"
}
```



---

archive/issue_events_053688.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-19T07:35:06Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4718#event-53688"
}
```



---

archive/issue_comments_028298.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nMerged in Sage 3.3.alpha0",
    "created_at": "2009-01-19T09:51:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4718#issuecomment-28298",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Merged in Sage 3.3.alpha0



---

archive/issue_events_053689.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-19T09:51:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4718#event-53689"
}
```



---

archive/issue_events_053690.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-19T09:51:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4718#event-53690"
}
```



---

archive/issue_events_053691.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-19T09:53:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4718#event-53691"
}
```



---

archive/issue_events_053692.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-19T09:53:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4718",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4718#event-53692"
}
```

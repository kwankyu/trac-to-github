# Issue 4763: fixes to build reference manual in 3.2.2.alpha1

archive/issues_004763.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nAs reported on [sage-devel](http://groups.google.com/group/sage-devel/browse_frm/thread/1e5f8fa4c33ef622), there are some problems trying to build the reference manual.  I think the attached patch fixes them.\n\ndavidloeffler should get partial credit for this, since he reported several of these...\n\n\nCC:  @loefflerd\n\nComponent: **documentation**\n\nKeywords: **reference**\n\nAuthor: **John Palmieri, David Loeffler**\n\nReviewer: **Michael Abshoff**\n\nMerged: **3.2.2.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4763_\n\n",
    "closed_at": "2008-12-12T06:00:32Z",
    "created_at": "2008-12-11T21:47:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20documentation"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fixes to build reference manual in 3.2.2.alpha1",
    "type": "issue",
    "updated_at": "2009-06-07T09:36:07Z",
    "url": "https://github.com/sagemath/sage/issues/4763",
    "user": "https://github.com/jhpalmieri"
}
```
<div id="comment:0"></div>

As reported on [sage-devel](http://groups.google.com/group/sage-devel/browse_frm/thread/1e5f8fa4c33ef622), there are some problems trying to build the reference manual.  I think the attached patch fixes them.

davidloeffler should get partial credit for this, since he reported several of these...


CC:  @loefflerd

Component: **documentation**

Keywords: **reference**

Author: **John Palmieri, David Loeffler**

Reviewer: **Michael Abshoff**

Merged: **3.2.2.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/4763_





---

archive/issue_events_054401.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2008-12-11T21:47:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4763",
    "milestone_number": null,
    "milestone_title": "sage-3.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4763#event-54401"
}
```



---

archive/issue_events_054402.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2008-12-11T21:47:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4763",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4763#event-54402"
}
```



---

archive/issue_events_054403.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2008-12-11T21:47:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4763",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4763#event-54403"
}
```



---

archive/issue_events_054404.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2008-12-11T21:47:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4763",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4763#event-54404"
}
```



---

archive/issue_events_054405.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2008-12-11T21:47:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4763",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4763#event-54405"
}
```



---

archive/issue_events_054406.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2008-12-11T21:47:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4763",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4763#event-54406"
}
```



---

archive/issue_comments_028729.json:
```json
{
    "body": "Attachment: **[4763.patch.gz](https://github.com/sagemath/sage/files/ticket4763/4763.patch.gz)**",
    "created_at": "2008-12-11T21:49:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4763#issuecomment-28729",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment: **[4763.patch.gz](https://github.com/sagemath/sage/files/ticket4763/4763.patch.gz)**



---

archive/issue_events_054407.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-12T03:40:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4763",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4763#event-54407"
}
```



---

archive/issue_comments_028730.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nPositive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-12-12T03:40:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4763#issuecomment-28730",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

Positive review.

Cheers,

Michael



---

archive/issue_comments_028731.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged in Sage 3.2.2.alpha2",
    "created_at": "2008-12-12T06:00:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4763#issuecomment-28731",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged in Sage 3.2.2.alpha2



---

archive/issue_events_054408.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-12T06:00:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4763",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4763#event-54408"
}
```



---

archive/issue_events_054409.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-12T06:00:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4763",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4763#event-54409"
}
```



---

archive/issue_comments_028732.json:
```json
{
    "body": "Author: **John Palmieri, David Loeffler**",
    "created_at": "2009-06-07T09:36:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4763#issuecomment-28732",
    "user": "https://github.com/loefflerd"
}
```

Author: **John Palmieri, David Loeffler**



---

archive/issue_comments_028733.json:
```json
{
    "body": "Merged: **3.2.2.alpha2**",
    "created_at": "2009-06-07T09:36:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4763#issuecomment-28733",
    "user": "https://github.com/loefflerd"
}
```

Merged: **3.2.2.alpha2**



---

archive/issue_comments_028734.json:
```json
{
    "body": "Reviewer: **Michael Abshoff**",
    "created_at": "2009-06-07T09:36:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4763#issuecomment-28734",
    "user": "https://github.com/loefflerd"
}
```

Reviewer: **Michael Abshoff**

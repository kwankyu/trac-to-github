# Issue 5394: Remove the remnants of the docs from sage-ptest and make it ignore the devel/sage/build directory

archive/issues_005394.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-cwitty"
    ],
    "body": "<div id=\"comment:0\"></div>\n\n\n\nComponent: **misc**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5394_\n\n",
    "closed_at": "2009-02-27T23:07:33Z",
    "created_at": "2009-02-27T17:14:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove the remnants of the docs from sage-ptest and make it ignore the devel/sage/build directory",
    "type": "issue",
    "updated_at": "2009-02-28T15:50:50Z",
    "url": "https://github.com/sagemath/sage/issues/5394",
    "user": "https://github.com/mwhansen"
}
```
<div id="comment:0"></div>



Component: **misc**

_Issue created by migration from https://trac.sagemath.org/ticket/5394_





---

archive/issue_events_062880.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-27T17:14:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5394",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5394#event-62880"
}
```



---

archive/issue_events_062881.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-27T17:14:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5394",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5394#event-62881"
}
```



---

archive/issue_events_062882.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-27T17:14:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5394",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5394#event-62882"
}
```



---

archive/issue_events_062883.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2009-02-27T17:14:13Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5394",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5394#event-62883"
}
```



---

archive/issue_events_062884.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-27T17:14:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5394",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5394#event-62884"
}
```



---

archive/issue_events_062885.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-27T21:03:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5394",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5394#event-62885"
}
```



---

archive/issue_comments_033260.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_5394.patch.gz](https://github.com/sagemath/sage/files/ticket5394/trac_5394.patch.gz)**\n\nExcellent, couldn't have done it better myself :)\n\nCheers,\n\nMichael",
    "created_at": "2009-02-27T21:03:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5394",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5394#issuecomment-33260",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_5394.patch.gz](https://github.com/sagemath/sage/files/ticket5394/trac_5394.patch.gz)**

Excellent, couldn't have done it better myself :)

Cheers,

Michael



---

archive/issue_comments_033261.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged in Sage 3.4.rc0.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-27T23:07:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5394",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5394#issuecomment-33261",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged in Sage 3.4.rc0.

Cheers,

Michael



---

archive/issue_events_062886.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-27T23:07:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5394",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5394#event-62886"
}
```



---

archive/issue_events_062887.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-27T23:07:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5394",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5394#event-62887"
}
```



---

archive/issue_comments_033262.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nThis patch actually does not ignore the bits in build and I am not quite sure why the patch doesn't work.\n\nMike: Thoughts?\n\nCheers,\n\nMichael",
    "created_at": "2009-02-28T12:19:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5394",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5394#issuecomment-33262",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

This patch actually does not ignore the bits in build and I am not quite sure why the patch doesn't work.

Mike: Thoughts?

Cheers,

Michael



---

archive/issue_comments_033263.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nOk, figured it out - reviewer patch coming up.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-28T12:35:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5394",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5394#issuecomment-33263",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Ok, figured it out - reviewer patch coming up.

Cheers,

Michael



---

archive/issue_comments_033264.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nAttachment: **[trac_5394-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket5394/trac_5394-reviewer.patch.gz)**\n\nOops, the reviewer patch had some debug output in it. I removed that in the tree and did another checkin.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-28T15:50:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5394",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5394#issuecomment-33264",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Attachment: **[trac_5394-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket5394/trac_5394-reviewer.patch.gz)**

Oops, the reviewer patch had some debug output in it. I removed that in the tree and did another checkin.

Cheers,

Michael

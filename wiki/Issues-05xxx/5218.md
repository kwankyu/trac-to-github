# Issue 5218: Update Python to 2.5.4 (latest 2.5 upstream) (depends on #5286)

archive/issues_005218.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nPython 2.5.4 fixes a couple of security problems in Python 2.5.2. So let's update to it since the difference to 2.5.2 are minimal.\n\nCheers,\n\nMichael\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5218_\n\n",
    "closed_at": "2009-05-29T17:34:02Z",
    "created_at": "2009-02-09T12:31:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update Python to 2.5.4 (latest 2.5 upstream) (depends on #5286)",
    "type": "issue",
    "updated_at": "2009-05-29T17:34:02Z",
    "url": "https://github.com/sagemath/sage/issues/5218",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
<div id="comment:0"></div>

Python 2.5.4 fixes a couple of security problems in Python 2.5.2. So let's update to it since the difference to 2.5.2 are minimal.

Cheers,

Michael

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/5218_





---

archive/issue_events_060486.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-09T12:31:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60486"
}
```



---

archive/issue_events_060487.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-09T12:31:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "0000b0",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60487"
}
```



---

archive/issue_events_060488.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-09T12:31:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60488"
}
```



---

archive/issue_events_060489.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-09T12:31:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60489"
}
```



---

archive/issue_events_060490.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-09T12:31:43Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60490"
}
```



---

archive/issue_events_060491.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-16T05:04:01Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "title_is": "Update Python to 2.5.4",
    "title_was": "Update Pyhton to 2.5.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60491"
}
```



---

archive/issue_comments_031954.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAn spkg is coming up :), but I needed to correct the spelling problem in the meantime.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-16T05:04:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5218#issuecomment-31954",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

An spkg is coming up :), but I needed to correct the spelling problem in the meantime.

Cheers,

Michael



---

archive/issue_events_060492.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-16T06:05:21Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "title_is": "Update Python to 2.5.4 (latest 2.5 upstream)",
    "title_was": "Update Python to 2.5.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60492"
}
```



---

archive/issue_events_060493.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-16T06:05:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60493"
}
```



---

archive/issue_comments_031955.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nThe spkg at\n\n http://sage.math.washington.edu/home/mabshoff/release-cycles-3.3/rc1/python-2.5.4.spkg\n\nswitches to Python 2.5.4. Note that no changes were made aside from \n\n* rebasing the ctypes/__init__.py patch\n* update SPKG.txt, especially with patch information\n* rename two patches to be consistent with SPKG guidelines\n\nCheers,\n\nMichael",
    "created_at": "2009-02-16T06:05:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5218#issuecomment-31955",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

The spkg at

 http://sage.math.washington.edu/home/mabshoff/release-cycles-3.3/rc1/python-2.5.4.spkg

switches to Python 2.5.4. Note that no changes were made aside from 

* rebasing the ctypes/__init__.py patch
* update SPKG.txt, especially with patch information
* rename two patches to be consistent with SPKG guidelines

Cheers,

Michael



---

archive/issue_events_060494.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-02-16T11:31:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60494"
}
```



---

archive/issue_events_060495.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-02-16T11:31:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60495"
}
```



---

archive/issue_comments_031956.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nI have not extensively tested this, but Michael says he has, and I have installed it and done some limited tests.  Seems to be fine.",
    "created_at": "2009-02-16T11:31:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5218#issuecomment-31956",
    "user": "https://github.com/sagetrac-mhampton"
}
```

<div id="comment:4" align="right">comment:4</div>

I have not extensively tested this, but Michael says he has, and I have installed it and done some limited tests.  Seems to be fine.



---

archive/issue_comments_031957.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nMerged in Sage 3.3.rc1.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-16T11:34:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5218#issuecomment-31957",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Merged in Sage 3.3.rc1.

Cheers,

Michael



---

archive/issue_events_060496.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-16T11:34:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60496"
}
```



---

archive/issue_events_060497.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-16T11:34:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60497"
}
```



---

archive/issue_events_060498.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-16T15:59:47Z",
    "event": "reopened",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60498"
}
```



---

archive/issue_comments_031958.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nUnfortunately this introduced an unpleasant side effect since -sdist now does not include the hg repo in the sage-x.y.z.spkg. I know too little about distutils to deal with this in 3.3.rc1, so I am reverting the spkg until we fix the bug in setup.py.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-16T15:59:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5218#issuecomment-31958",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Unfortunately this introduced an unpleasant side effect since -sdist now does not include the hg repo in the sage-x.y.z.spkg. I know too little about distutils to deal with this in 3.3.rc1, so I am reverting the spkg until we fix the bug in setup.py.

Cheers,

Michael



---

archive/issue_events_060499.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-16T16:03:59Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "title_is": "Update Python to 2.5.4 (latest 2.5 upstream) (depends on #5286)",
    "title_was": "Update Python to 2.5.4 (latest 2.5 upstream)",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60499"
}
```



---

archive/issue_events_060500.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-16T16:03:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60500"
}
```



---

archive/issue_events_060501.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T06:11:39Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60501"
}
```



---

archive/issue_events_060502.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T06:11:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60502"
}
```



---

archive/issue_comments_031959.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nBumped to 3.4.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-20T06:11:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5218#issuecomment-31959",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:8" align="right">comment:8</div>

Bumped to 3.4.

Cheers,

Michael



---

archive/issue_comments_031960.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nBetter luck in Sage 3.4.1.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-27T23:10:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5218#issuecomment-31960",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:9" align="right">comment:9</div>

Better luck in Sage 3.4.1.

Cheers,

Michael



---

archive/issue_events_060503.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-27T23:10:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60503"
}
```



---

archive/issue_events_060504.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-27T23:10:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60504"
}
```



---

archive/issue_events_060505.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T20:25:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60505"
}
```



---

archive/issue_events_060506.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T20:25:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60506"
}
```



---

archive/issue_comments_031961.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nA new spkg which fixes the sdist issue is at http://sage.math.washington.edu/home/mhansen/python-2.5.4.p0.spkg",
    "created_at": "2009-05-28T20:25:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5218#issuecomment-31961",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:10" align="right">comment:10</div>

A new spkg which fixes the sdist issue is at http://sage.math.washington.edu/home/mhansen/python-2.5.4.p0.spkg



---

archive/issue_events_060507.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T21:04:08Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60507"
}
```



---

archive/issue_events_060508.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T21:04:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60508"
}
```



---

archive/issue_comments_031962.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nNote:\nThe patch that fixes this sdist issue is:\n\n```\nwstein@sage:~/tmp/python-2.5.4.p0$ diff src/Lib/distutils/command/sdist.py patches/sdist.py\n350c350\n<           * any RCS, CVS, .svn, .hg, .git, .bzr, _darcs directories\n---\n>           * any RCS, CVS, .svn, .git, .bzr, _darcs directories\n357c357\n<         self.filelist.exclude_pattern(r'(^|/)(RCS|CVS|\\.svn|\\.hg|\\.git|\\.bzr|_darcs)/.*', is_regex=1)\n---\n>         self.filelist.exclude_pattern(r'(^|/)(RCS|CVS|\\.svn|\\.git|\\.bzr|_darcs)/.*', is_regex=1)\n```\n\nI'm concerned since I bet this problem will just come back later again when we upgrade to some newer python, since it seems that it is the intention of the python devs to make it impossible to include repo's in sdists.  Thus, in the long run, we may have to fix this by (1) doing sdist, then (2) extract the sdist tar ball and manually copying over the hg repo, then (3) remaking the sdist tar ball.    We already extract and remake the tar ball to go from .tar.gz to .tar.bz2 format, so this isn't so bad. \n\nSo, positive review, but with a \"just for the record\" caveat.",
    "created_at": "2009-05-28T22:07:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5218#issuecomment-31962",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:12" align="right">comment:12</div>

Note:
The patch that fixes this sdist issue is:

```
wstein@sage:~/tmp/python-2.5.4.p0$ diff src/Lib/distutils/command/sdist.py patches/sdist.py
350c350
<           * any RCS, CVS, .svn, .hg, .git, .bzr, _darcs directories
---
>           * any RCS, CVS, .svn, .git, .bzr, _darcs directories
357c357
<         self.filelist.exclude_pattern(r'(^|/)(RCS|CVS|\.svn|\.hg|\.git|\.bzr|_darcs)/.*', is_regex=1)
---
>         self.filelist.exclude_pattern(r'(^|/)(RCS|CVS|\.svn|\.git|\.bzr|_darcs)/.*', is_regex=1)
```

I'm concerned since I bet this problem will just come back later again when we upgrade to some newer python, since it seems that it is the intention of the python devs to make it impossible to include repo's in sdists.  Thus, in the long run, we may have to fix this by (1) doing sdist, then (2) extract the sdist tar ball and manually copying over the hg repo, then (3) remaking the sdist tar ball.    We already extract and remake the tar ball to go from .tar.gz to .tar.bz2 format, so this isn't so bad. 

So, positive review, but with a "just for the record" caveat.



---

archive/issue_events_060509.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-28T22:07:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60509"
}
```



---

archive/issue_events_060510.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-28T22:07:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60510"
}
```



---

archive/issue_comments_031963.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nI made http://sage.math.washington.edu/home/mhansen/python-2.5.4.p1.spkg which includes the fix done in sage-2.5.2.p9.spkg.",
    "created_at": "2009-05-29T04:25:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5218#issuecomment-31963",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:13" align="right">comment:13</div>

I made http://sage.math.washington.edu/home/mhansen/python-2.5.4.p1.spkg which includes the fix done in sage-2.5.2.p9.spkg.



---

archive/issue_events_060511.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-29T04:25:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60511"
}
```



---

archive/issue_events_060512.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-29T04:25:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60512"
}
```



---

archive/issue_comments_031964.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nThis looks suspicous:\n\n```\nwstein@sage:~/tmp/python-2.5.4.p1$ hg status\n? patches/posixmodule.c.patch.rej\n```\n\nOtherwise this looks perfect.  \n\nhttp://sage.math.washington.edu/home/wstein/tmp/python-2.5.4.p1.spkg\n\nis the spkg but with that rej file deleted.",
    "created_at": "2009-05-29T13:36:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5218#issuecomment-31964",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:14" align="right">comment:14</div>

This looks suspicous:

```
wstein@sage:~/tmp/python-2.5.4.p1$ hg status
? patches/posixmodule.c.patch.rej
```

Otherwise this looks perfect.  

http://sage.math.washington.edu/home/wstein/tmp/python-2.5.4.p1.spkg

is the spkg but with that rej file deleted.



---

archive/issue_events_060513.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-29T13:36:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60513"
}
```



---

archive/issue_events_060514.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-29T13:36:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60514"
}
```



---

archive/issue_comments_031965.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nThis also seems to fix some problems I was having in 64-bit Fedora 10: segmentation faults when trying to use tab completion, and mysterious \"out of memory\" errors. (I tried it against 4.0.rc1.)",
    "created_at": "2009-05-29T13:47:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5218#issuecomment-31965",
    "user": "https://github.com/kedlaya"
}
```

<div id="comment:15" align="right">comment:15</div>

This also seems to fix some problems I was having in 64-bit Fedora 10: segmentation faults when trying to use tab completion, and mysterious "out of memory" errors. (I tried it against 4.0.rc1.)



---

archive/issue_comments_031966.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">comment:16</div>\n\nMerged http://sage.math.washington.edu/home/wstein/tmp/python-2.5.4.p1.spkg in 4.0.rc2.",
    "created_at": "2009-05-29T17:33:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5218#issuecomment-31966",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:16" align="right">comment:16</div>

Merged http://sage.math.washington.edu/home/wstein/tmp/python-2.5.4.p1.spkg in 4.0.rc2.



---

archive/issue_events_060515.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-29T17:34:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60515"
}
```



---

archive/issue_events_060516.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-29T17:34:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5218",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5218#event-60516"
}
```

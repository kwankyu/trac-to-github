# Issue 5215: Remove ipython1-20070130.spkg from Sage

archive/issues_005215.json:
```json
{
    "body": "ipython1-20070130.spkg is very outdated and since ipython 0.9.0 the functionality has been migrated into ipython itself. Since we are now shipping ipython 0.9.1 remove the ipython1-20070130.spkg from the Sage distribution. Besides deleting the spkg one also needs to change deps and install.\n\nI will post diff for deps and install momentarily.\n\nCheers,\n\nMichael\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/5215\n\n",
    "closed_at": "2009-02-11T06:10:33Z",
    "created_at": "2009-02-09T12:20:24Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "title": "Remove ipython1-20070130.spkg from Sage",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/5215",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
ipython1-20070130.spkg is very outdated and since ipython 0.9.0 the functionality has been migrated into ipython itself. Since we are now shipping ipython 0.9.1 remove the ipython1-20070130.spkg from the Sage distribution. Besides deleting the spkg one also needs to change deps and install.

I will post diff for deps and install momentarily.

Cheers,

Michael

**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/5215





---

archive/attachments_006084.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_5215_deps.patch",
    "asset_url": "tarball://root/attachments/ticket5215/trac_5215_deps.patch",
    "created_at": "2009-02-11T04:37:53Z",
    "issue": "https://github.com/sagemath/sage/issues/5215",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket5215/trac_5215_deps.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```



---

archive/issue_comments_033168.json:
```json
{
    "body": "**Attachment:** [trac_5215_deps.patch](https://github.com/sagemath/sage/files/ticket5215/trac_5215_deps.patch)",
    "created_at": "2009-02-11T04:37:53Z",
    "issue": "https://github.com/sagemath/sage/issues/5215",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5215#issuecomment-33168",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

**Attachment:** [trac_5215_deps.patch](https://github.com/sagemath/sage/files/ticket5215/trac_5215_deps.patch)



---

archive/issue_events_032403.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-11T05:48:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5215",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5215#event-32403"
}
```



---

archive/attachments_006085.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_5215_install.patch",
    "asset_url": "tarball://root/attachments/ticket5215/trac_5215_install.patch",
    "created_at": "2009-02-11T05:48:38Z",
    "issue": "https://github.com/sagemath/sage/issues/5215",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket5215/trac_5215_install.patch",
    "user": "https://github.com/mwhansen"
}
```



---

archive/issue_comments_033169.json:
```json
{
    "body": "<a id='comment:2'></a>\n**Attachment:** [trac_5215_install.patch](https://github.com/sagemath/sage/files/ticket5215/trac_5215_install.patch)\n\nLooks good.  Hurray for removing cruft!",
    "created_at": "2009-02-11T05:48:38Z",
    "issue": "https://github.com/sagemath/sage/issues/5215",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5215#issuecomment-33169",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
**Attachment:** [trac_5215_install.patch](https://github.com/sagemath/sage/files/ticket5215/trac_5215_install.patch)

Looks good.  Hurray for removing cruft!



---

archive/issue_comments_033170.json:
```json
{
    "body": "<a id='comment:3'></a>\n(Un)Merged in Sage 3.3.rc0 :)\n\nCheers,\n\nMichael",
    "created_at": "2009-02-11T06:10:33Z",
    "issue": "https://github.com/sagemath/sage/issues/5215",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5215#issuecomment-33170",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
(Un)Merged in Sage 3.3.rc0 :)

Cheers,

Michael



---

archive/issue_events_032404.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-11T06:10:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5215",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5215#event-32404"
}
```



---

archive/issue_events_032405.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-11T06:10:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5215",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5215#event-32405"
}
```

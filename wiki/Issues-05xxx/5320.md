# Issue 5320: update Sphinx to 0.5.1, setuptools to 0.6c9

archive/issues_005320.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe spkg can be found at http://sage.math.washington.edu/home/mhansen/sphinx-0.5.1.spkg\n\nThis requires an update to setuptools 0.6c9 as well.\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5320_\n\n",
    "closed_at": "2009-02-20T12:07:55Z",
    "created_at": "2009-02-20T11:20:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "update Sphinx to 0.5.1, setuptools to 0.6c9",
    "type": "issue",
    "updated_at": "2009-02-20T12:07:55Z",
    "url": "https://github.com/sagemath/sage/issues/5320",
    "user": "https://github.com/mwhansen"
}
```
<div id="comment:0"></div>

The spkg can be found at http://sage.math.washington.edu/home/mhansen/sphinx-0.5.1.spkg

This requires an update to setuptools 0.6c9 as well.

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/5320_





---

archive/issue_events_061966.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-20T11:20:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-61966"
}
```



---

archive/issue_events_061967.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-20T11:20:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "0000b0",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-61967"
}
```



---

archive/issue_events_061968.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-20T11:20:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-61968"
}
```



---

archive/issue_events_061969.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-20T11:20:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-61969"
}
```



---

archive/issue_events_061970.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T11:20:32Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-61970"
}
```



---

archive/issue_events_061971.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-20T11:27:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-61971"
}
```



---

archive/issue_events_061972.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-20T11:27:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-61972"
}
```



---

archive/issue_events_061973.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-20T11:27:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-61973"
}
```



---

archive/issue_events_061974.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T11:27:41Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-61974"
}
```



---

archive/issue_events_061975.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-20T11:27:41Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-61975"
}
```



---

archive/issue_comments_032794.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThe setup tools spkg can be found at http://sage.math.washington.edu/home/mhansen/setuptools-0.6c9.spkg",
    "created_at": "2009-02-20T11:27:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5320#issuecomment-32794",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

The setup tools spkg can be found at http://sage.math.washington.edu/home/mhansen/setuptools-0.6c9.spkg



---

archive/issue_events_061976.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T12:07:31Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "title_is": "update Sphinx to 0.5.1, setuptools to 0.6c9",
    "title_was": "update Sphinx to 0.5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-61976"
}
```



---

archive/issue_events_061977.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T12:07:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-61977"
}
```



---

archive/issue_events_061978.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T12:07:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-61978"
}
```



---

archive/issue_comments_032795.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nBoth spkgs are clean, all changes checked in and documented. Positive review. I made spkg-install in setuptools-0.6c9.spkg executable without bumping the patch level, but if that would get lost it wouldn't matter too much.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-20T12:07:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5320#issuecomment-32795",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Both spkgs are clean, all changes checked in and documented. Positive review. I made spkg-install in setuptools-0.6c9.spkg executable without bumping the patch level, but if that would get lost it wouldn't matter too much.

Cheers,

Michael



---

archive/issue_events_061979.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T12:07:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-61979"
}
```



---

archive/issue_events_061980.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T12:07:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5320#event-61980"
}
```



---

archive/issue_comments_032796.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.3.rc3.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-20T12:07:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5320#issuecomment-32796",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.3.rc3.

Cheers,

Michael

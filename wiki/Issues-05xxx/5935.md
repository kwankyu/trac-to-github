# Issue 5935: graph theory docs -- sage: graphs_query.[tab]             # not tested

archive/issues_005935.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-ekirkman"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nRight at the top overview of graph theory the ref manual has the line\n\n```\nsage: graphs_query.[tab]             # not tested\n```\n\nhttp://www.sagemath.org/doc/reference/sage/graphs/graph.html\n\nIf it isn't tested it's broken, and that's the case here, since there is no graphs_query object available on the Sage command line.\n\nComponent: **graph theory**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5935_\n\n",
    "closed_at": "2009-07-08T20:30:52Z",
    "created_at": "2009-04-29T15:49:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "graph theory docs -- sage: graphs_query.[tab]             # not tested",
    "type": "issue",
    "updated_at": "2009-07-08T20:30:52Z",
    "url": "https://github.com/sagemath/sage/issues/5935",
    "user": "https://github.com/williamstein"
}
```
<div id="comment:0"></div>

Right at the top overview of graph theory the ref manual has the line

```
sage: graphs_query.[tab]             # not tested
```

http://www.sagemath.org/doc/reference/sage/graphs/graph.html

If it isn't tested it's broken, and that's the case here, since there is no graphs_query object available on the Sage command line.

Component: **graph theory**

_Issue created by migration from https://trac.sagemath.org/ticket/5935_





---

archive/issue_events_070357.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-29T15:49:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5935#event-70357"
}
```



---

archive/issue_events_070358.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-29T15:49:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5935#event-70358"
}
```



---

archive/issue_events_070359.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-29T15:49:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5935#event-70359"
}
```



---

archive/issue_events_070360.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-29T15:49:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5935#event-70360"
}
```



---

archive/issue_events_070361.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-04-29T15:49:31Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5935#event-70361"
}
```



---

archive/issue_events_070362.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-05-21T21:55:34Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "subject": "https://github.com/sagetrac-ekirkman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5935#event-70362"
}
```



---

archive/issue_events_070363.json:
```json
{
    "actor": "https://github.com/sagetrac-ekirkman",
    "created_at": "2009-05-21T21:55:34Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "subject": "https://github.com/sagetrac-ekirkman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5935#event-70363"
}
```



---

archive/issue_comments_038368.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMarked as duplicate because this documentation change is covered in the patch for ticket #5936.",
    "created_at": "2009-05-21T21:57:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5935#issuecomment-38368",
    "user": "https://github.com/sagetrac-ekirkman"
}
```

<div id="comment:2" align="right">comment:2</div>

Marked as duplicate because this documentation change is covered in the patch for ticket #5936.



---

archive/issue_events_070364.json:
```json
{
    "actor": "https://github.com/sagetrac-ekirkman",
    "created_at": "2009-05-21T21:57:16Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5935#event-70364"
}
```



---

archive/issue_events_070365.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-08T20:30:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5935#event-70365"
}
```



---

archive/issue_events_070366.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-08T20:30:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "c6c6c6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5935#event-70366"
}
```



---

archive/issue_events_070367.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-08T20:30:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5935",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5935#event-70367"
}
```

# Issue 5274: Adapt matrices coherency check to (still future) Nicolas check framework.

archive/issues_005274.json:
```json
{
    "assignees": [
        "https://github.com/hivert"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe coherency check for matrices is now done by hand in matrix/matrix_spaces.py When the automated check framework for categories is finished and integrated, we should use it to automatically ensure that all matrix spaces are checked. \n\nCC:  @nthiery\n\nComponent: **linear algebra**\n\nKeywords: **matrix, category check framework**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5274_\n\n",
    "closed_at": "2009-10-19T17:21:40Z",
    "created_at": "2009-02-14T17:33:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Adapt matrices coherency check to (still future) Nicolas check framework.",
    "type": "issue",
    "updated_at": "2009-10-19T17:21:40Z",
    "url": "https://github.com/sagemath/sage/issues/5274",
    "user": "https://github.com/hivert"
}
```
<div id="comment:0"></div>

The coherency check for matrices is now done by hand in matrix/matrix_spaces.py When the automated check framework for categories is finished and integrated, we should use it to automatically ensure that all matrix spaces are checked. 

CC:  @nthiery

Component: **linear algebra**

Keywords: **matrix, category check framework**

_Issue created by migration from https://trac.sagemath.org/ticket/5274_





---

archive/issue_events_061304.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-02-14T17:33:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5274",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5274#event-61304"
}
```



---

archive/issue_events_061305.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-02-14T17:33:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5274",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "c: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5274#event-61305"
}
```



---

archive/issue_events_061306.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-02-14T17:33:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5274",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5274#event-61306"
}
```



---

archive/issue_events_061307.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-02-14T17:33:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5274",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5274#event-61307"
}
```



---

archive/issue_events_061308.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-02-14T17:33:26Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5274",
    "subject": "https://github.com/hivert",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5274#event-61308"
}
```



---

archive/issue_events_061309.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-19T17:21:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5274",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5274#event-61309"
}
```



---

archive/issue_events_061310.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-19T17:21:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5274",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5274#event-61310"
}
```



---

archive/issue_comments_032367.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nDuplicate of #6936.",
    "created_at": "2009-10-19T17:21:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5274#issuecomment-32367",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:2" align="right">comment:2</div>

Duplicate of #6936.



---

archive/issue_events_061311.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-19T17:21:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5274",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5274#event-61311"
}
```



---

archive/issue_events_061312.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-19T17:21:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5274",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "c6c6c6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5274#event-61312"
}
```

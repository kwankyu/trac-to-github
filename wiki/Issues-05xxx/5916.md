# Issue 5916: show mathematica expression using jsmath

archive/issues_005916.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-whuss"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nShow mathematica expressions in the notebook using jsmath.\n\nCC:  @jasongrout\n\nComponent: **interfaces**\n\nAuthor: **Wilfried Huss**\n\nReviewer: **Jason Grout**\n\nMerged: **4.0.1.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5916_\n\n",
    "closed_at": "2009-06-01T01:09:49Z",
    "created_at": "2009-04-28T08:35:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "show mathematica expression using jsmath",
    "type": "issue",
    "updated_at": "2009-06-06T22:36:19Z",
    "url": "https://github.com/sagemath/sage/issues/5916",
    "user": "https://github.com/sagetrac-whuss"
}
```
<div id="comment:0"></div>

Show mathematica expressions in the notebook using jsmath.

CC:  @jasongrout

Component: **interfaces**

Author: **Wilfried Huss**

Reviewer: **Jason Grout**

Merged: **4.0.1.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/5916_





---

archive/issue_events_070105.json:
```json
{
    "actor": "https://github.com/sagetrac-whuss",
    "created_at": "2009-04-28T08:35:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5916",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5916#event-70105"
}
```



---

archive/issue_events_070106.json:
```json
{
    "actor": "https://github.com/sagetrac-whuss",
    "created_at": "2009-04-28T08:35:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5916",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "c: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5916#event-70106"
}
```



---

archive/issue_events_070107.json:
```json
{
    "actor": "https://github.com/sagetrac-whuss",
    "created_at": "2009-04-28T08:35:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5916",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5916#event-70107"
}
```



---

archive/issue_events_070108.json:
```json
{
    "actor": "https://github.com/sagetrac-whuss",
    "created_at": "2009-04-28T08:35:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5916",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5916#event-70108"
}
```



---

archive/issue_events_070109.json:
```json
{
    "actor": "https://github.com/sagetrac-whuss",
    "created_at": "2009-04-28T08:35:25Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5916",
    "subject": "https://github.com/sagetrac-whuss",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5916#event-70109"
}
```



---

archive/issue_events_070110.json:
```json
{
    "actor": "https://github.com/sagetrac-whuss",
    "created_at": "2009-04-28T08:35:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5916",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5916#event-70110"
}
```



---

archive/issue_comments_038188.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[mathematica_jsmath.patch.gz](https://github.com/sagemath/sage/files/ticket5916/mathematica_jsmath.patch.gz)**",
    "created_at": "2009-05-06T03:28:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5916#issuecomment-38188",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[mathematica_jsmath.patch.gz](https://github.com/sagemath/sage/files/ticket5916/mathematica_jsmath.patch.gz)**



---

archive/issue_comments_038189.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis works great and passes doctests in the file.  Positive review.",
    "created_at": "2009-05-30T06:21:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5916#issuecomment-38189",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:2" align="right">comment:2</div>

This works great and passes doctests in the file.  Positive review.



---

archive/issue_events_070111.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-05-30T06:21:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5916",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5916#event-70111"
}
```



---

archive/issue_comments_038190.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in 4.0.1.alpha0.",
    "created_at": "2009-06-01T01:09:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5916#issuecomment-38190",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in 4.0.1.alpha0.



---

archive/issue_events_070112.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-01T01:09:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5916",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5916#event-70112"
}
```



---

archive/issue_events_070113.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-01T01:09:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5916",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5916#event-70113"
}
```



---

archive/issue_comments_038191.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nIt looks like we can use a screenshot to show the feature this patch introduces. If I'm not mistaken, can someone please upload such a screenshot so it can be put in the release tour.",
    "created_at": "2009-06-04T13:56:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5916#issuecomment-38191",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:4" align="right">comment:4</div>

It looks like we can use a screenshot to show the feature this patch introduces. If I'm not mistaken, can someone please upload such a screenshot so it can be put in the release tour.



---

archive/issue_comments_038192.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nAttachment: **[mathematica.png](https://github.com/sagemath/sage/files/ticket5916/mathematica.png)**\n\nReplying to [@sagetrac-mvngu](#comment%3A4):\n> It looks like we can use a screenshot to show the feature this patch introduces. If I'm not mistaken, can someone please upload such a screenshot so it can be put in the release tour.\n\nI attached the image which is produced by:\n\n```\nm = mathematica(\"Product[(3/2)^i, {i, 1, n}]\")\nshow(m)\n```",
    "created_at": "2009-06-04T15:09:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5916#issuecomment-38192",
    "user": "https://github.com/sagetrac-whuss"
}
```

<div id="comment:5" align="right">comment:5</div>

Attachment: **[mathematica.png](https://github.com/sagemath/sage/files/ticket5916/mathematica.png)**

Replying to [@sagetrac-mvngu](#comment%3A4):
> It looks like we can use a screenshot to show the feature this patch introduces. If I'm not mistaken, can someone please upload such a screenshot so it can be put in the release tour.

I attached the image which is produced by:

```
m = mathematica("Product[(3/2)^i, {i, 1, n}]")
show(m)
```



---

archive/issue_comments_038193.json:
```json
{
    "body": "Author: **Wilfried Huss**",
    "created_at": "2009-06-06T22:36:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5916#issuecomment-38193",
    "user": "https://github.com/mwhansen"
}
```

Author: **Wilfried Huss**



---

archive/issue_comments_038194.json:
```json
{
    "body": "Reviewer: **Jason Grout**",
    "created_at": "2009-06-06T22:36:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5916#issuecomment-38194",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **Jason Grout**



---

archive/issue_comments_038195.json:
```json
{
    "body": "Merged: **4.0.1.alpha0**",
    "created_at": "2009-06-06T22:36:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5916#issuecomment-38195",
    "user": "https://github.com/mwhansen"
}
```

Merged: **4.0.1.alpha0**

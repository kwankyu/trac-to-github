# Issue 5522: Fix segfault if libsingular.so can't be found

archive/issues_005522.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nIf for some reason libsingular.so doesn't exist, Sage will segfault when trying to call dlerror() when dlopen() was never called.\n\nI've attached a patch which avoids doing so.\n\nWith this patch, however, Sage then segfaults sometime after trying to raise the ImportError on the next line.\n\nI don't have time to investigate further, but it is easy to reproduce the condition by just moving aside the libsingular.so and trying to start sage.\n\nUpstream: **None of the above - read trac for reasoning.**\n\nCC:  @sagetrac-GeorgSWeber\n\nComponent: **packages: standard**\n\nAuthor: **Julien Puydt**\n\nBranch/Commit: **[`c17a703`](https://github.com/sagemath/sagetrac-mirror/commit/c17a703a99c3b59c58b64ee2f822323b3371ad6d)**\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5522_\n\n",
    "closed_at": "2015-03-24T10:48:31Z",
    "created_at": "2009-03-15T00:21:22Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix segfault if libsingular.so can't be found",
    "type": "issue",
    "updated_at": "2015-03-24T10:48:31Z",
    "url": "https://github.com/sagemath/sage/issues/5522",
    "user": "https://github.com/timabbott"
}
```
<div id="comment:0"></div>

If for some reason libsingular.so doesn't exist, Sage will segfault when trying to call dlerror() when dlopen() was never called.

I've attached a patch which avoids doing so.

With this patch, however, Sage then segfaults sometime after trying to raise the ImportError on the next line.

I don't have time to investigate further, but it is easy to reproduce the condition by just moving aside the libsingular.so and trying to start sage.

Upstream: **None of the above - read trac for reasoning.**

CC:  @sagetrac-GeorgSWeber

Component: **packages: standard**

Author: **Julien Puydt**

Branch/Commit: **[`c17a703`](https://github.com/sagemath/sagetrac-mirror/commit/c17a703a99c3b59c58b64ee2f822323b3371ad6d)**

Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/5522_





---

archive/issue_events_064733.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2009-03-15T00:21:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64733"
}
```



---

archive/issue_events_064734.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2009-03-15T00:21:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "0000b0",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64734"
}
```



---

archive/issue_events_064735.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2009-03-15T00:21:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64735"
}
```



---

archive/issue_events_064736.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2009-03-15T00:21:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64736"
}
```



---

archive/issue_events_064737.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2009-03-15T00:21:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64737"
}
```



---

archive/issue_events_064738.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-15T00:21:22Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "subject": "https://github.com/timabbott",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64738"
}
```



---

archive/issue_comments_034571.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[sage-libsingular-nocrash.patch.gz](https://github.com/sagemath/sage/files/ticket5522/sage-libsingular-nocrash.patch.gz)**\n\nHi,\n\nright after\n\n```\nif handle == NULL\n```\none could do\n\n```\nprint \"cannot load libSINGULAR library\"\n```\nand only then try to call dlerror() or raise ImportError or whatsoever. If then the SegFault (sometimes) occurs, the user still knows why. With this change, I would consider this such an enhancement over the current situation, that I would vote to have it in.",
    "created_at": "2009-03-25T23:36:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34571",
    "user": "https://github.com/sagetrac-GeorgSWeber"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[sage-libsingular-nocrash.patch.gz](https://github.com/sagemath/sage/files/ticket5522/sage-libsingular-nocrash.patch.gz)**

Hi,

right after

```
if handle == NULL
```
one could do

```
print "cannot load libSINGULAR library"
```
and only then try to call dlerror() or raise ImportError or whatsoever. If then the SegFault (sometimes) occurs, the user still knows why. With this change, I would consider this such an enhancement over the current situation, that I would vote to have it in.



---

archive/issue_events_064739.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64739"
}
```



---

archive/issue_events_064740.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64740"
}
```



---

archive/issue_events_064741.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64741"
}
```



---

archive/issue_events_064742.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64742"
}
```



---

archive/issue_events_064743.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64743"
}
```



---

archive/issue_events_064744.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64744"
}
```



---

archive/issue_events_064745.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64745"
}
```



---

archive/issue_events_064746.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64746"
}
```



---

archive/issue_comments_034572.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nThe attached patch doesn't apply, and the corresponding code in src/sage/libs/singular/singular.pyx looks like the target code already, so I'm putting that bug up as \"needs review\" with the proposition to just close it as obsolete!",
    "created_at": "2015-02-07T09:59:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34572",
    "user": "https://github.com/SnarkBoojum"
}
```

<div id="comment:6" align="right">comment:6</div>

The attached patch doesn't apply, and the corresponding code in src/sage/libs/singular/singular.pyx looks like the target code already, so I'm putting that bug up as "needs review" with the proposition to just close it as obsolete!



---

archive/issue_comments_034573.json:
```json
{
    "body": "Upstream: **None of the above - read trac for reasoning.**",
    "created_at": "2015-02-07T09:59:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34573",
    "user": "https://github.com/SnarkBoojum"
}
```

Upstream: **None of the above - read trac for reasoning.**



---

archive/issue_events_064747.json:
```json
{
    "actor": "https://github.com/SnarkBoojum",
    "created_at": "2015-02-07T09:59:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64747"
}
```



---

archive/issue_events_064748.json:
```json
{
    "actor": "https://github.com/SnarkBoojum",
    "created_at": "2015-02-07T09:59:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64748"
}
```



---

archive/issue_comments_034574.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nReplying to [@SnarkBoojum](#comment%3A6):\n> the corresponding code in `src/sage/libs/singular/singular.pyx` looks like the target code already\n\nThis statement is completely false.\n\nNevertheless, the patch might not be needed, I'll check.",
    "created_at": "2015-02-07T10:08:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34574",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:7" align="right">comment:7</div>

Replying to [@SnarkBoojum](#comment%3A6):
> the corresponding code in `src/sage/libs/singular/singular.pyx` looks like the target code already

This statement is completely false.

Nevertheless, the patch might not be needed, I'll check.



---

archive/issue_comments_034575.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nUh... indeed, I had \"applied\" the patch by hand yesterday and forgot about it overnight, so it's no surprise it was looking the same :-(\n\nI'll provide the adapted patch when I'll have checked its correctness.",
    "created_at": "2015-02-07T10:29:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34575",
    "user": "https://github.com/SnarkBoojum"
}
```

<div id="comment:8" align="right">comment:8</div>

Uh... indeed, I had "applied" the patch by hand yesterday and forgot about it overnight, so it's no surprise it was looking the same :-(

I'll provide the adapted patch when I'll have checked its correctness.



---

archive/issue_comments_034576.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nI would prefer to move the error inside the `for` loop, like\n\n```\nhandle = ...\nif not handle:\n    dlerror(...)\n```\n\nI think that's the simplest solution",
    "created_at": "2015-02-07T10:47:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34576",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:9" align="right">comment:9</div>

I would prefer to move the error inside the `for` loop, like

```
handle = ...
if not handle:
    dlerror(...)
```

I think that's the simplest solution



---

archive/issue_comments_034577.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nIt avoids the flag variable. Good idea.",
    "created_at": "2015-02-07T11:03:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34577",
    "user": "https://github.com/SnarkBoojum"
}
```

<div id="comment:10" align="right">comment:10</div>

It avoids the flag variable. Good idea.



---

archive/issue_events_064749.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-02-07T12:08:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64749"
}
```



---

archive/issue_events_064750.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-02-07T12:08:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64750"
}
```



---

archive/issue_comments_034578.json:
```json
{
    "body": "Branch: **[u/Snark/_with_patch__needs_work__fix_segfault_if_libsingular_so_can_t_be_found](https://github.com/sagemath/sagetrac-mirror/tree/u/Snark/_with_patch__needs_work__fix_segfault_if_libsingular_so_can_t_be_found)**",
    "created_at": "2015-02-07T12:09:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34578",
    "user": "https://github.com/SnarkBoojum"
}
```

Branch: **[u/Snark/_with_patch__needs_work__fix_segfault_if_libsingular_so_can_t_be_found](https://github.com/sagemath/sagetrac-mirror/tree/u/Snark/_with_patch__needs_work__fix_segfault_if_libsingular_so_can_t_be_found)**



---

archive/issue_comments_034579.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nI pushed the branch because it looked nice, but then I tested what happens without the patch... and the result is in fact the same! Indeed, if libsingular.so isn't available, we already have an ImportError because of missing symbols:\n\n```\n   /home/jpuydt/sage-6.5.beta5/local/lib/python2.7/site-packages/sage/libs/singular/__init__.py in <module>()\n   ---->\n   1 from sage.libs.singular.function import singular_function, lib\n        global sage.libs.singular.function = undefined\n        global singular_function = undefined\n        global lib = undefined\n      2 \n      3 from sage.libs.singular.function_factory import SingularFunctionFactory\n      4 \n      5 ff = SingularFunctionFactory()\n      6 \n\nsage/rings/polynomial/plural.pxd in init sage.libs.singular.function (build/cythonized/sage/libs/singular/function.cpp:18713)()\n\nsage/rings/polynomial/plural.pyx in init sage.rings.polynomial.plural (build/cythonized/sage/rings/polynomial/plural.cpp:24145)()\n\nsage/libs/singular/polynomial.pyx in init sage.libs.singular.polynomial (build/cythonized/sage/libs/singular/polynomial.cpp:6414)()\n\nImportError: /home/jpuydt/sage-6.5.beta5/local/lib/python2.7/site-packages/sage/libs/singular/singular.so: undefined symbol: _Z6naInitlP9sip_sring\n\n```\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/91185c85ffa5163af55caf28ae71a8956933443c\"><code>91185c8</code></a></td><td><code>Better handle if libsingular isn't found</code></td></tr></table>\n",
    "created_at": "2015-02-07T12:20:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34579",
    "user": "https://github.com/SnarkBoojum"
}
```

<div id="comment:13" align="right">comment:13</div>

I pushed the branch because it looked nice, but then I tested what happens without the patch... and the result is in fact the same! Indeed, if libsingular.so isn't available, we already have an ImportError because of missing symbols:

```
   /home/jpuydt/sage-6.5.beta5/local/lib/python2.7/site-packages/sage/libs/singular/__init__.py in <module>()
   ---->
   1 from sage.libs.singular.function import singular_function, lib
        global sage.libs.singular.function = undefined
        global singular_function = undefined
        global lib = undefined
      2 
      3 from sage.libs.singular.function_factory import SingularFunctionFactory
      4 
      5 ff = SingularFunctionFactory()
      6 

sage/rings/polynomial/plural.pxd in init sage.libs.singular.function (build/cythonized/sage/libs/singular/function.cpp:18713)()

sage/rings/polynomial/plural.pyx in init sage.rings.polynomial.plural (build/cythonized/sage/rings/polynomial/plural.cpp:24145)()

sage/libs/singular/polynomial.pyx in init sage.libs.singular.polynomial (build/cythonized/sage/libs/singular/polynomial.cpp:6414)()

ImportError: /home/jpuydt/sage-6.5.beta5/local/lib/python2.7/site-packages/sage/libs/singular/singular.so: undefined symbol: _Z6naInitlP9sip_sring

```

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/91185c85ffa5163af55caf28ae71a8956933443c"><code>91185c8</code></a></td><td><code>Better handle if libsingular isn't found</code></td></tr></table>




---

archive/issue_comments_034580.json:
```json
{
    "body": "Commit: **[`91185c8`](https://github.com/sagemath/sagetrac-mirror/commit/91185c85ffa5163af55caf28ae71a8956933443c)**",
    "created_at": "2015-02-07T12:20:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34580",
    "user": "https://github.com/SnarkBoojum"
}
```

Commit: **[`91185c8`](https://github.com/sagemath/sagetrac-mirror/commit/91185c85ffa5163af55caf28ae71a8956933443c)**



---

archive/issue_comments_034581.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nHi, I would suggest to improve the error handling for loading libSingular even further as in \n\nhttps://github.com/sagemath/sagetrac-mirror/commit/1e847897b9fce54fa25f1b91dea78f9e225ad0c9\nsee ticket #17254 (upgrade to Singular 4.0.1)\n\n\nThis issue puzzled me for two days, since the error handling was not clean,\nand I failed to interpret the crash messages.\n\n\nAttention: do not pick \n\n```\nlib = os.environ['SAGE_LOCAL']+\"/lib/libSingular.\"+extension # libsingular renamed to libSingular\n```\nsince this is only for upgrade to Singular 4.0.1 (libsingular renamed to libSingular)",
    "created_at": "2015-02-07T12:56:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34581",
    "user": "https://github.com/sagetrac-jakobkroeker"
}
```

<div id="comment:14" align="right">comment:14</div>

Hi, I would suggest to improve the error handling for loading libSingular even further as in 

https://github.com/sagemath/sagetrac-mirror/commit/1e847897b9fce54fa25f1b91dea78f9e225ad0c9
see ticket #17254 (upgrade to Singular 4.0.1)


This issue puzzled me for two days, since the error handling was not clean,
and I failed to interpret the crash messages.


Attention: do not pick 

```
lib = os.environ['SAGE_LOCAL']+"/lib/libSingular."+extension # libsingular renamed to libSingular
```
since this is only for upgrade to Singular 4.0.1 (libsingular renamed to libSingular)



---

archive/issue_comments_034582.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nReplying to [@sagetrac-jakobkroeker](#comment%3A14):\n> Hi, I would suggest to improve the error handling for loading libSingular even further as in \n> \n> https://github.com/sagemath/sagetrac-mirror/commit/1e847897b9fce54fa25f1b91dea78f9e225ad0c9\n> see ticket #17254 (upgrade to Singular 4.0.1)\n\n\nDid you check that this patch was of any use?\n\nIf libsingular.so can't be loaded, then that file can't either. So any error handling within that file won't ever run, because the missing library will already have triggered an ImportError.\n\nThat is my point in [comment:13](#comment%3A13).",
    "created_at": "2015-02-07T13:11:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34582",
    "user": "https://github.com/SnarkBoojum"
}
```

<div id="comment:15" align="right">comment:15</div>

Replying to [@sagetrac-jakobkroeker](#comment%3A14):
> Hi, I would suggest to improve the error handling for loading libSingular even further as in 
> 
> https://github.com/sagemath/sagetrac-mirror/commit/1e847897b9fce54fa25f1b91dea78f9e225ad0c9
> see ticket #17254 (upgrade to Singular 4.0.1)


Did you check that this patch was of any use?

If libsingular.so can't be loaded, then that file can't either. So any error handling within that file won't ever run, because the missing library will already have triggered an ImportError.

That is my point in [comment:13](#comment%3A13).



---

archive/issue_comments_034583.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">comment:16</div>\n\n> Did you check that this patch was of any use?\n\nI do not exactly understand, what you mean.\n\nMy point is that an error message should be descriptive as possible,\nso if a file cannot be found it should be \n\n```\n   raise OSError, \"did not find libsingular file!\"\n```\n\nand if dlopen fails ,it should be something like (I improved the message)\n\n```\n        raise ImportError, \" failed to dlopen \" + lib + \" \"\n```\n\nBefore your or mine patch it was even worse,\n(a SIGSEGV because dlerror() is NULL in that case) and that \npuzzled me, since because of the compiled cython stuff I could not exactly see what happened.",
    "created_at": "2015-02-07T13:21:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34583",
    "user": "https://github.com/sagetrac-jakobkroeker"
}
```

<div id="comment:16" align="right">comment:16</div>

> Did you check that this patch was of any use?

I do not exactly understand, what you mean.

My point is that an error message should be descriptive as possible,
so if a file cannot be found it should be 

```
   raise OSError, "did not find libsingular file!"
```

and if dlopen fails ,it should be something like (I improved the message)

```
        raise ImportError, " failed to dlopen " + lib + " "
```

Before your or mine patch it was even worse,
(a SIGSEGV because dlerror() is NULL in that case) and that 
puzzled me, since because of the compiled cython stuff I could not exactly see what happened.



---

archive/issue_comments_034584.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">comment:17</div>\n\n> That is my point in [comment:13](#comment%3A13).\n\nThat is a different issue. I recall that hit similar errors and for me './sage -ba' solved them, so I guess this are caching issues or  mixing c with c++ issues.\n\nIn case libsingular.so was missing, for me it usually ended in \na SIGSEGV.",
    "created_at": "2015-02-07T13:24:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34584",
    "user": "https://github.com/sagetrac-jakobkroeker"
}
```

<div id="comment:17" align="right">comment:17</div>

> That is my point in [comment:13](#comment%3A13).

That is a different issue. I recall that hit similar errors and for me './sage -ba' solved them, so I guess this are caching issues or  mixing c with c++ issues.

In case libsingular.so was missing, for me it usually ended in 
a SIGSEGV.



---

archive/issue_comments_034585.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">comment:18</div>\n\n> That is my point in [comment:13](#comment%3A13).\n\nThat is a different issue. I recall that I hit similar errors and for me './sage -ba' solved them, so I guess this are caching issues or  mixing C with C++ issues.\n\nIn case libsingular.so was missing, for me it usually ended in \na SIGSEGV or other strange crashes",
    "created_at": "2015-02-07T13:31:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34585",
    "user": "https://github.com/sagetrac-jakobkroeker"
}
```

<div id="comment:18" align="right">comment:18</div>

> That is my point in [comment:13](#comment%3A13).

That is a different issue. I recall that I hit similar errors and for me './sage -ba' solved them, so I guess this are caching issues or  mixing C with C++ issues.

In case libsingular.so was missing, for me it usually ended in 
a SIGSEGV or other strange crashes



---

archive/issue_comments_034586.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">comment:19</div>\n\nI'm happy with this patch (would set to positive_review) but I don't quite understand the issues raised above.",
    "created_at": "2015-02-07T13:45:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34586",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:19" align="right">comment:19</div>

I'm happy with this patch (would set to positive_review) but I don't quite understand the issues raised above.



---

archive/issue_comments_034587.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">comment:20</div>\n\n> I'm happy with this patch\n\nI'm not happy yet with this patch",
    "created_at": "2015-02-07T14:41:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34587",
    "user": "https://github.com/sagetrac-jakobkroeker"
}
```

<div id="comment:20" align="right">comment:20</div>

> I'm happy with this patch

I'm not happy yet with this patch



---

archive/issue_comments_034588.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">comment:21</div>\n\nI have compiled sage from scratch without my patch, moved libsingular.so, tried to launch sage, got a nice backtrace.\n\nI have compiled sage from scratch with my patch, moved libsingular.so, tried to launch sage, got the same nice backtrace.\n\nThis rules out the case where \"./sage -ba\" would have made things better.\n\nI reiterate my point of [comment:13](#comment%3A13) : that piece of code doesn't get executed if it can't even be loaded itself because libsingular is missing.\n\nI think the correct way to use the dl* functions is with dlsym : you dlopen a library, use dlsym to get the symbol you want, then call it (and you can check every step). That way you don't link-depend on the symbol you want. The current code tries to explicitly load the library, but since it uses directly the functions it wants, it can come into memory only if the linker already opened the lib and found those functions, so playing with dl* is pointless.",
    "created_at": "2015-02-09T07:40:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34588",
    "user": "https://github.com/SnarkBoojum"
}
```

<div id="comment:21" align="right">comment:21</div>

I have compiled sage from scratch without my patch, moved libsingular.so, tried to launch sage, got a nice backtrace.

I have compiled sage from scratch with my patch, moved libsingular.so, tried to launch sage, got the same nice backtrace.

This rules out the case where "./sage -ba" would have made things better.

I reiterate my point of [comment:13](#comment%3A13) : that piece of code doesn't get executed if it can't even be loaded itself because libsingular is missing.

I think the correct way to use the dl* functions is with dlsym : you dlopen a library, use dlsym to get the symbol you want, then call it (and you can check every step). That way you don't link-depend on the symbol you want. The current code tries to explicitly load the library, but since it uses directly the functions it wants, it can come into memory only if the linker already opened the lib and found those functions, so playing with dl* is pointless.



---

archive/issue_comments_034589.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">comment:22</div>\n\n> This rules out the case where `./sage -ba` would have made things better.\n\nThe issue happened to me when I tried to update Singular (from 3.1.7 to 4.0.1) in a working branch\n(and will of course not happen if you compile from scratch!; that is equivalent to\n`make distclean` and implies `./sage -ba`\n\nHere are some more hints and  I hope it is reproducible\n\n- Compile from scratch the develop branch  (in doubt use commit d27f8497dcd19d70ec08155888e6fec9c74b839a)\n- merge in changes from https://github.com/sagemath/sagetrac-mirror/commit/221c5d63e02d7f1d7fdc57cf6c819677553dd262 \n- rename \"/lib/libSingular.\" tp \"/lib/libsingular.\" in singular.pyx\n- build (parallelized) again with `make build -j 6` or something similar;\n\nThen I predict that you may hit something you did hit in [comment:13](#comment%3A13).\nIn fact, I have no idea, how you managed to trigger an `ImportError` as in [comment:13](#comment%3A13)\n``\n> so playing with dl* is pointless.\n\nare you sidetracking? I issued something different, namely that an error message should be as descriptive as possible;\nespecially when a file is missing a developer would be happy to see that a file is missing and not an `ImportError` or a SIGSEGV as before",
    "created_at": "2015-02-09T12:13:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34589",
    "user": "https://github.com/sagetrac-jakobkroeker"
}
```

<div id="comment:22" align="right">comment:22</div>

> This rules out the case where `./sage -ba` would have made things better.

The issue happened to me when I tried to update Singular (from 3.1.7 to 4.0.1) in a working branch
(and will of course not happen if you compile from scratch!; that is equivalent to
`make distclean` and implies `./sage -ba`

Here are some more hints and  I hope it is reproducible

- Compile from scratch the develop branch  (in doubt use commit d27f8497dcd19d70ec08155888e6fec9c74b839a)
- merge in changes from https://github.com/sagemath/sagetrac-mirror/commit/221c5d63e02d7f1d7fdc57cf6c819677553dd262 
- rename "/lib/libSingular." tp "/lib/libsingular." in singular.pyx
- build (parallelized) again with `make build -j 6` or something similar;

Then I predict that you may hit something you did hit in [comment:13](#comment%3A13).
In fact, I have no idea, how you managed to trigger an `ImportError` as in [comment:13](#comment%3A13)
``
> so playing with dl* is pointless.

are you sidetracking? I issued something different, namely that an error message should be as descriptive as possible;
especially when a file is missing a developer would be happy to see that a file is missing and not an `ImportError` or a SIGSEGV as before



---

archive/issue_comments_034590.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">comment:23</div>\n\n@SnarkBoojum\nit is likely that I cannot exactly recall, how I triggered  ImportError ,\nmaybe it was by removing the shared library and only keeping the static one.\nI hope that it does not matter anyway.",
    "created_at": "2015-02-09T15:11:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34590",
    "user": "https://github.com/sagetrac-jakobkroeker"
}
```

<div id="comment:23" align="right">comment:23</div>

@SnarkBoojum
it is likely that I cannot exactly recall, how I triggered  ImportError ,
maybe it was by removing the shared library and only keeping the static one.
I hope that it does not matter anyway.



---

archive/issue_comments_034591.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">comment:24</div>\n\nDeleted post to try the issue reported in \nhttps://groups.google.com/d/msg/sage-devel/IpeIDqTo4mE/hzs3cLA3N_YJ",
    "created_at": "2015-02-10T01:35:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34591",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:24" align="right">comment:24</div>

Deleted post to try the issue reported in 
https://groups.google.com/d/msg/sage-devel/IpeIDqTo4mE/hzs3cLA3N_YJ



---

archive/issue_comments_034592.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">comment:25</div>\n\nI'm a little at loss as to how I'll improve the patch : I don't exactly know how to reproduce the issue.\n\nIndeed, what kind of error will that code detect? It will only trigger when the library will have been loaded in memory (because it links to it), but for some reason cannot be reloaded with the new RTLD_* flags!",
    "created_at": "2015-02-18T10:50:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34592",
    "user": "https://github.com/SnarkBoojum"
}
```

<div id="comment:25" align="right">comment:25</div>

I'm a little at loss as to how I'll improve the patch : I don't exactly know how to reproduce the issue.

Indeed, what kind of error will that code detect? It will only trigger when the library will have been loaded in memory (because it links to it), but for some reason cannot be reloaded with the new RTLD_* flags!



---

archive/issue_comments_034593.json:
```json
{
    "body": "<div id=\"comment:26\" align=\"right\">comment:26</div>\n\n> Indeed, what kind of error will that code detect?\n> It will only trigger when the library will have been loaded in memory (because it links to it)\n\nI'm not sure, but it seems you can't see the case I described, because you are too focused on the scenario you have in mind. Try to think \"what may happen\", not \"what do we want to achieve\"\n``\nCase 1: \n The name of the library is `_hardcoded_` in 'singular.pyx', so we might get an issue if there is a typo or the name of the library changed. That happened in Singular 4.0.1, so I was able to compile sage, but then dlopen tried to open a file which did not exist.\n\ncase 2: \n> It will only trigger when the library will have been loaded in memory (because it links to it), but for some reason cannot be reloaded with the new RTLD_* flags!\n\nExactly. Who knows why dlopen() may fail, so better catch it and watch out for dlerror() == NULL",
    "created_at": "2015-02-18T11:11:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34593",
    "user": "https://github.com/sagetrac-jakobkroeker"
}
```

<div id="comment:26" align="right">comment:26</div>

> Indeed, what kind of error will that code detect?
> It will only trigger when the library will have been loaded in memory (because it links to it)

I'm not sure, but it seems you can't see the case I described, because you are too focused on the scenario you have in mind. Try to think "what may happen", not "what do we want to achieve"
``
Case 1: 
 The name of the library is `_hardcoded_` in 'singular.pyx', so we might get an issue if there is a typo or the name of the library changed. That happened in Singular 4.0.1, so I was able to compile sage, but then dlopen tried to open a file which did not exist.

case 2: 
> It will only trigger when the library will have been loaded in memory (because it links to it), but for some reason cannot be reloaded with the new RTLD_* flags!

Exactly. Who knows why dlopen() may fail, so better catch it and watch out for dlerror() == NULL



---

archive/issue_comments_034594.json:
```json
{
    "body": "Changed commit from **[`91185c8`](https://github.com/sagemath/sagetrac-mirror/commit/91185c85ffa5163af55caf28ae71a8956933443c)** to **[`c17a703`](https://github.com/sagemath/sagetrac-mirror/commit/c17a703a99c3b59c58b64ee2f822323b3371ad6d)**",
    "created_at": "2015-03-14T07:52:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34594",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`91185c8`](https://github.com/sagemath/sagetrac-mirror/commit/91185c85ffa5163af55caf28ae71a8956933443c)** to **[`c17a703`](https://github.com/sagemath/sagetrac-mirror/commit/c17a703a99c3b59c58b64ee2f822323b3371ad6d)**



---

archive/issue_comments_034595.json:
```json
{
    "body": "<div id=\"comment:27\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9f912af0882313acc72ff4acf1062a0782d4db28\"><code>9f912af</code></a></td><td><code>Better handle if libsingular isn't found</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e72d87370043cdad0064a439c4735237e179c2ca\"><code>e72d873</code></a></td><td><code>Try to check if dlerror worked before printing the result</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c17a703a99c3b59c58b64ee2f822323b3371ad6d\"><code>c17a703</code></a></td><td><code>Merge branch 'u/Snark/_with_patch__needs_work__fix_segfault_if_libsingular_so_can_t_be_found' of git://git.sagemath.org/sage into trac_5522</code></td></tr></table>\n",
    "created_at": "2015-03-14T07:52:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34595",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:27"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9f912af0882313acc72ff4acf1062a0782d4db28"><code>9f912af</code></a></td><td><code>Better handle if libsingular isn't found</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e72d87370043cdad0064a439c4735237e179c2ca"><code>e72d873</code></a></td><td><code>Try to check if dlerror worked before printing the result</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c17a703a99c3b59c58b64ee2f822323b3371ad6d"><code>c17a703</code></a></td><td><code>Merge branch 'u/Snark/_with_patch__needs_work__fix_segfault_if_libsingular_so_can_t_be_found' of git://git.sagemath.org/sage into trac_5522</code></td></tr></table>




---

archive/issue_comments_034596.json:
```json
{
    "body": "<div id=\"comment:28\" align=\"right\">comment:28</div>\n\nOk, I tried to rebase my branch and make a better change.",
    "created_at": "2015-03-14T07:54:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34596",
    "user": "https://github.com/SnarkBoojum"
}
```

<div id="comment:28" align="right">comment:28</div>

Ok, I tried to rebase my branch and make a better change.



---

archive/issue_events_064751.json:
```json
{
    "actor": "https://github.com/SnarkBoojum",
    "created_at": "2015-03-14T07:54:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64751"
}
```



---

archive/issue_events_064752.json:
```json
{
    "actor": "https://github.com/SnarkBoojum",
    "created_at": "2015-03-14T07:54:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64752"
}
```



---

archive/issue_comments_034597.json:
```json
{
    "body": "Author: **Julien Puydt**",
    "created_at": "2015-03-15T20:33:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34597",
    "user": "https://github.com/SnarkBoojum"
}
```

Author: **Julien Puydt**



---

archive/issue_events_064753.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-03-23T18:32:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64753"
}
```



---

archive/issue_events_064754.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-03-23T18:32:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64754"
}
```



---

archive/issue_comments_034598.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2015-03-23T18:32:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34598",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_064755.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-24T10:48:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64755"
}
```



---

archive/issue_events_064756.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "687a776e22fa9285e99407a8558e48e8dc0d5015",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-03-24T10:48:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5522#event-64756"
}
```



---

archive/issue_comments_034599.json:
```json
{
    "body": "Changed branch from **[u/Snark/_with_patch__needs_work__fix_segfault_if_libsingular_so_can_t_be_found](https://github.com/sagemath/sagetrac-mirror/tree/u/Snark/_with_patch__needs_work__fix_segfault_if_libsingular_so_can_t_be_found)** to **[`c17a703`](https://github.com/sagemath/sagetrac-mirror/commit/c17a703a99c3b59c58b64ee2f822323b3371ad6d)**",
    "created_at": "2015-03-24T10:48:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5522#issuecomment-34599",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/Snark/_with_patch__needs_work__fix_segfault_if_libsingular_so_can_t_be_found](https://github.com/sagemath/sagetrac-mirror/tree/u/Snark/_with_patch__needs_work__fix_segfault_if_libsingular_so_can_t_be_found)** to **[`c17a703`](https://github.com/sagemath/sagetrac-mirror/commit/c17a703a99c3b59c58b64ee2f822323b3371ad6d)**

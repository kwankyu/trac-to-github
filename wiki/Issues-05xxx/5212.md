# Issue 5212: bug with numbers in names in sage.structure.parent_gens.normalize_names

archive/issues_005212.json:
```json
{
    "body": "Try `PolynomialRing(QQ, 2, 'alpha0')` for hilarity.\n\n**Keywords:** numbers in names normalize_names\n\nIssue created by migration from https://trac.sagemath.org/ticket/5212\n\n",
    "closed_at": "2009-02-11T06:05:03Z",
    "created_at": "2009-02-09T04:20:51Z",
    "labels": [
        "component: basic arithmetic",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "title": "bug with numbers in names in sage.structure.parent_gens.normalize_names",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/5212",
    "user": "https://github.com/ncalexan"
}
```
Try `PolynomialRing(QQ, 2, 'alpha0')` for hilarity.

**Keywords:** numbers in names normalize_names

Issue created by migration from https://trac.sagemath.org/ticket/5212





---

archive/attachments_006080.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_5212-normalize-names.patch",
    "asset_url": "tarball://root/attachments/ticket5212/trac_5212-normalize-names.patch",
    "created_at": "2009-02-09T04:23:07Z",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket5212/trac_5212-normalize-names.patch",
    "user": "https://github.com/ncalexan"
}
```



---

archive/issue_comments_033150.json:
```json
{
    "body": "**Attachment:** [trac_5212-normalize-names.patch](https://github.com/sagemath/sage/files/ticket5212/trac_5212-normalize-names.patch)",
    "created_at": "2009-02-09T04:23:07Z",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5212#issuecomment-33150",
    "user": "https://github.com/ncalexan"
}
```

**Attachment:** [trac_5212-normalize-names.patch](https://github.com/sagemath/sage/files/ticket5212/trac_5212-normalize-names.patch)



---

archive/issue_comments_033151.json:
```json
{
    "body": "<a id='comment:1'></a>\nWith this patch applied to my current 3.3.rc0 merge tree all doctests pass.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-09T12:26:34Z",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5212#issuecomment-33151",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
With this patch applied to my current 3.3.rc0 merge tree all doctests pass.

Cheers,

Michael



---

archive/issue_events_032386.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2009-02-11T05:36:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5212#event-32386"
}
```



---

archive/issue_comments_033152.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.3.rc0.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-11T06:05:03Z",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5212#issuecomment-33152",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.3.rc0.

Cheers,

Michael



---

archive/issue_events_032387.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-11T06:05:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5212#event-32387"
}
```



---

archive/issue_events_032388.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-11T06:05:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5212",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5212#event-32388"
}
```

# Issue 5493: improve Sphinx documentation of sr.py

archive/issues_005493.json:
```json
{
    "assignees": [
        "https://github.com/malb"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe attached patch makes the Sphinx output for `mq.SR` look much nicer.\n\nComponent: **documentation**\n\nKeywords: **sphinx**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5493_\n\n",
    "closed_at": "2009-03-20T20:50:41Z",
    "created_at": "2009-03-11T23:03:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20cryptography"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "improve Sphinx documentation of sr.py",
    "type": "issue",
    "updated_at": "2009-03-20T20:50:41Z",
    "url": "https://github.com/sagemath/sage/issues/5493",
    "user": "https://github.com/malb"
}
```
<div id="comment:0"></div>

The attached patch makes the Sphinx output for `mq.SR` look much nicer.

Component: **documentation**

Keywords: **sphinx**

_Issue created by migration from https://trac.sagemath.org/ticket/5493_





---

archive/issue_events_064327.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-03-11T23:03:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5493",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5493#event-64327"
}
```



---

archive/issue_events_064328.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-03-11T23:03:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5493",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5493#event-64328"
}
```



---

archive/issue_events_064329.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-03-11T23:03:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5493",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5493#event-64329"
}
```



---

archive/issue_events_064330.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-03-11T23:03:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5493",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5493#event-64330"
}
```



---

archive/issue_events_064331.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-03-11T23:03:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5493",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20cryptography",
    "label_color": "0000ff",
    "label_name": "c: cryptography",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5493#event-64331"
}
```



---

archive/issue_events_064332.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-03-11T23:03:38Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5493",
    "subject": "https://github.com/malb",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5493#event-64332"
}
```



---

archive/issue_events_064333.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-03-11T23:03:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5493",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5493#event-64333"
}
```



---

archive/issue_comments_034298.json:
```json
{
    "body": "Attachment: **[sr_sphinx.patch.gz](https://github.com/sagemath/sage/files/ticket5493/sr_sphinx.patch.gz)**\n\nyay, my first sphinx patch",
    "created_at": "2009-03-11T23:03:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5493#issuecomment-34298",
    "user": "https://github.com/malb"
}
```

Attachment: **[sr_sphinx.patch.gz](https://github.com/sagemath/sage/files/ticket5493/sr_sphinx.patch.gz)**

yay, my first sphinx patch



---

archive/issue_events_064334.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-11T23:19:10Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5493",
    "title_is": "improve Sphinx documentation of sr.py",
    "title_was": "No title",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5493#event-64334"
}
```



---

archive/issue_events_064335.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-11T23:19:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5493",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5493#event-64335"
}
```



---

archive/issue_comments_034299.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nREFEREE REPORT\n\n\n\nThe patch **sr_sphinx.patch** applied fine against Sage version 3.4. All tests passed, even with the `-long` option. The reference manual (which the patch touches) builds OK and looks rather prettier, which is what Martin wants :-)  Positive review.\n\n\n\nNote that while reviewing this ticket, I also noticed some further enhancements that can be done to `sr.py`. But these are addressed in ticket #5527.",
    "created_at": "2009-03-16T07:50:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5493#issuecomment-34299",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:2" align="right">comment:2</div>

REFEREE REPORT



The patch **sr_sphinx.patch** applied fine against Sage version 3.4. All tests passed, even with the `-long` option. The reference manual (which the patch touches) builds OK and looks rather prettier, which is what Martin wants :-)  Positive review.



Note that while reviewing this ticket, I also noticed some further enhancements that can be done to `sr.py`. But these are addressed in ticket #5527.



---

archive/issue_events_064336.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-03-16T07:50:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5493",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5493#event-64336"
}
```



---

archive/issue_events_064337.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-03-16T07:50:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5493",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5493#event-64337"
}
```



---

archive/issue_comments_034300.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.4.1.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-03-20T20:50:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5493#issuecomment-34300",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.4.1.alpha0.

Cheers,

Michael



---

archive/issue_events_064338.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-20T20:50:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5493",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5493#event-64338"
}
```



---

archive/issue_events_064339.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-20T20:50:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5493",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5493#event-64339"
}
```

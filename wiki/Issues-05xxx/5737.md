# Issue 5737: pynac interface improvements

archive/issues_005737.json:
```json
{
    "body": "Attached patches fix some bugs in the pynac interface and add .find() and .is_polynomial() functions.\n\n**Assignee:** @burcin\n\n**CC:**  wstein @mwhansen @ncalexan\n\nIssue created by migration from https://trac.sagemath.org/ticket/5737\n\n",
    "closed_at": "2009-04-13T09:25:30Z",
    "created_at": "2009-04-10T21:09:13Z",
    "labels": [
        "component: symbolics",
        "trivial",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4.1",
    "title": "pynac interface improvements",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/5737",
    "user": "https://github.com/burcin"
}
```
Attached patches fix some bugs in the pynac interface and add .find() and .is_polynomial() functions.

**Assignee:** @burcin

**CC:**  wstein @mwhansen @ncalexan

Issue created by migration from https://trac.sagemath.org/ticket/5737





---

archive/issue_comments_037702.json:
```json
{
    "body": "add .find() method",
    "created_at": "2009-04-10T21:09:51Z",
    "issue": "https://github.com/sagemath/sage/issues/5737",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5737#issuecomment-37702",
    "user": "https://github.com/burcin"
}
```

add .find() method



---

archive/attachments_006810.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_5737-01-find.patch",
    "asset_url": "tarball://root/attachments/ticket5737/trac_5737-01-find.patch",
    "created_at": "2009-04-10T21:10:24Z",
    "issue": "https://github.com/sagemath/sage/issues/5737",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket5737/trac_5737-01-find.patch",
    "user": "https://github.com/burcin"
}
```



---

archive/attachments_006811.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_5737-02-real_imag.patch",
    "asset_url": "tarball://root/attachments/ticket5737/trac_5737-02-real_imag.patch",
    "created_at": "2009-04-10T21:10:24Z",
    "issue": "https://github.com/sagemath/sage/issues/5737",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket5737/trac_5737-02-real_imag.patch",
    "user": "https://github.com/burcin"
}
```



---

archive/issue_comments_037703.json:
```json
{
    "body": "**Attachment:** [trac_5737-01-find.patch](https://github.com/sagemath/sage/files/ticket5737/trac_5737-01-find.patch)\n\n**Attachment:** [trac_5737-02-real_imag.patch](https://github.com/sagemath/sage/files/ticket5737/trac_5737-02-real_imag.patch)\n\nfix py_real() and py_imag()",
    "created_at": "2009-04-10T21:10:24Z",
    "issue": "https://github.com/sagemath/sage/issues/5737",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5737#issuecomment-37703",
    "user": "https://github.com/burcin"
}
```

**Attachment:** [trac_5737-01-find.patch](https://github.com/sagemath/sage/files/ticket5737/trac_5737-01-find.patch)

**Attachment:** [trac_5737-02-real_imag.patch](https://github.com/sagemath/sage/files/ticket5737/trac_5737-02-real_imag.patch)

fix py_real() and py_imag()



---

archive/issue_comments_037704.json:
```json
{
    "body": "add .is_polynomial()",
    "created_at": "2009-04-10T21:10:40Z",
    "issue": "https://github.com/sagemath/sage/issues/5737",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5737#issuecomment-37704",
    "user": "https://github.com/burcin"
}
```

add .is_polynomial()



---

archive/issue_events_036649.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-04-10T21:11:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5737",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5737#event-36649"
}
```



---

archive/issue_events_036650.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-04-10T21:11:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5737",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5737#event-36650"
}
```



---

archive/attachments_006812.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_5737-03-is_polynomial.patch",
    "asset_url": "tarball://root/attachments/ticket5737/trac_5737-03-is_polynomial.patch",
    "created_at": "2009-04-10T21:11:55Z",
    "issue": "https://github.com/sagemath/sage/issues/5737",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket5737/trac_5737-03-is_polynomial.patch",
    "user": "https://github.com/burcin"
}
```



---

archive/issue_comments_037705.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_5737-03-is_polynomial.patch](https://github.com/sagemath/sage/files/ticket5737/trac_5737-03-is_polynomial.patch)",
    "created_at": "2009-04-10T21:11:55Z",
    "issue": "https://github.com/sagemath/sage/issues/5737",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5737#issuecomment-37705",
    "user": "https://github.com/burcin"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_5737-03-is_polynomial.patch](https://github.com/sagemath/sage/files/ticket5737/trac_5737-03-is_polynomial.patch)



---

archive/issue_comments_037706.json:
```json
{
    "body": "<a id='comment:2'></a>\nIt's not clear that defaulting all objects to be \"real\" is a good choice, but it'll do for now.  Apply!",
    "created_at": "2009-04-11T21:54:38Z",
    "issue": "https://github.com/sagemath/sage/issues/5737",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5737#issuecomment-37706",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:2'></a>
It's not clear that defaulting all objects to be "real" is a good choice, but it'll do for now.  Apply!



---

archive/issue_events_036651.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-04-11T21:54:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5737",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5737#event-36651"
}
```



---

archive/issue_comments_037707.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged all three patches in Sage 3.4.1.rc3.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-13T09:25:30Z",
    "issue": "https://github.com/sagemath/sage/issues/5737",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5737#issuecomment-37707",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged all three patches in Sage 3.4.1.rc3.

Cheers,

Michael



---

archive/issue_events_036652.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-04-13T09:25:30Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5737",
    "milestone": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5737#event-36652"
}
```



---

archive/issue_events_036653.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-04-13T09:25:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5737",
    "milestone": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5737#event-36653"
}
```



---

archive/issue_events_036654.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-04-13T09:25:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5737",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5737#event-36654"
}
```



---

archive/issue_events_036655.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-04-13T09:25:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5737",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5737#event-36655"
}
```

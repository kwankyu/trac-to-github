# Issue 5203: Update mpir to 0.9.rc3 release (latest upstream)

archive/issues_005203.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe spkg at\n\n  http://sage.math.washington.edu/home/mabshoff/release-cycles-3.3/alpha6/gmp-mpir-0.9.rc3.spkg\n \nupdates the gmp-mpir.spkg to latest upstream. For now the test suite is invoked automatically. It passes on\n\nSkyNet:\n\n* eno (x86_64, FC9)\n* mark (Sparc, Solaris)\n* fulvia (x86, Solaris)\n* cicero (x86, FC9)\n* menas (x86_64, OpenSUSE 10.3)\n* iras (Itanium, SLES 10)\n* cleo (Itanium, RHEL 5.2)\n* varro (PPC, OSX 10.4)\n\nMisc machines:\n\n* bsd (x86, OSX 10.5)\n* sage.math (x86_64, Ubuntu LTS 8.04)\n* sprocketer (x86-64, OSX 10.5)\n\nCheers,\n\nMichael\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5203_\n\n",
    "closed_at": "2009-02-08T01:58:34Z",
    "created_at": "2009-02-08T01:10:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update mpir to 0.9.rc3 release (latest upstream)",
    "type": "issue",
    "updated_at": "2009-02-08T01:58:34Z",
    "url": "https://github.com/sagemath/sage/issues/5203",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
<div id="comment:0"></div>

The spkg at

  http://sage.math.washington.edu/home/mabshoff/release-cycles-3.3/alpha6/gmp-mpir-0.9.rc3.spkg
 
updates the gmp-mpir.spkg to latest upstream. For now the test suite is invoked automatically. It passes on

SkyNet:

* eno (x86_64, FC9)
* mark (Sparc, Solaris)
* fulvia (x86, Solaris)
* cicero (x86, FC9)
* menas (x86_64, OpenSUSE 10.3)
* iras (Itanium, SLES 10)
* cleo (Itanium, RHEL 5.2)
* varro (PPC, OSX 10.4)

Misc machines:

* bsd (x86, OSX 10.5)
* sage.math (x86_64, Ubuntu LTS 8.04)
* sprocketer (x86-64, OSX 10.5)

Cheers,

Michael

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/5203_





---

archive/issue_events_060305.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:10:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5203",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5203#event-60305"
}
```



---

archive/issue_events_060306.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:10:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5203",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "0000b0",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5203#event-60306"
}
```



---

archive/issue_events_060307.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:10:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5203",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
    "label_color": "ff7700",
    "label_name": "p: critical / 2",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5203#event-60307"
}
```



---

archive/issue_events_060308.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:10:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5203",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5203#event-60308"
}
```



---

archive/issue_events_060309.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:10:15Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5203",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5203#event-60309"
}
```



---

archive/issue_events_060310.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:10:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5203",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5203#event-60310"
}
```



---

archive/issue_comments_031867.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,6 @@\n The spkg at\n \n+  http://sage.math.washington.edu/home/mabshoff/release-cycles-3.3/alpha6/gmp-mpir-0.9.rc3.spkg\n  \n updates the gmp-mpir.spkg to latest upstream. For now the test suite is invoked automatically. It passes on\n \n``````\n",
    "created_at": "2009-02-08T01:10:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5203#issuecomment-31867",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,6 @@
 The spkg at
 
+  http://sage.math.washington.edu/home/mabshoff/release-cycles-3.3/alpha6/gmp-mpir-0.9.rc3.spkg
  
 updates the gmp-mpir.spkg to latest upstream. For now the test suite is invoked automatically. It passes on
 
``````




---

archive/issue_events_060311.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-08T01:48:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5203",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5203#event-60311"
}
```



---

archive/issue_comments_031868.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nLooks good.  Passes all tests for me.",
    "created_at": "2009-02-08T01:48:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5203#issuecomment-31868",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:2" align="right">comment:2</div>

Looks good.  Passes all tests for me.



---

archive/issue_events_060312.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:58:34Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5203",
    "title_is": "Update mpir to 0.9.rc3 release (latest upstream)",
    "title_was": "Update mpir to 0.9.rc3 release [latest upstream]",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5203#event-60312"
}
```



---

archive/issue_comments_031869.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.3.alpha6.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-08T01:58:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5203#issuecomment-31869",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.3.alpha6.

Cheers,

Michael



---

archive/issue_events_060313.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:58:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5203",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5203#event-60313"
}
```



---

archive/issue_events_060314.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-08T01:58:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5203",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5203#event-60314"
}
```

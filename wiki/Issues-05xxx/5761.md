# Issue 5761: Bring doctests of sage/misc/latex.py to 100%

archive/issues_005761.json:
```json
{
    "assignees": [
        "https://github.com/rbeezer"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nApply only [attachment: trac_5761-latex_docstrings-ts.patch](https://github.com/sagemath/sage/files/ticket5761/trac_5761-latex_docstrings-ts.patch.gz).\n\nComponent: **doctest coverage**\n\nKeywords: **latex**\n\nAuthor: **Travis Scrimshaw**\n\nReviewer: **Andr\u00e9 Apitzsch**\n\nMerged: **sage-5.7.beta4**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5761_\n\n",
    "closed_at": "2013-02-09T12:13:02Z",
    "created_at": "2009-04-11T19:56:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/doctest%20coverage"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bring doctests of sage/misc/latex.py to 100%",
    "type": "issue",
    "updated_at": "2013-02-09T12:13:02Z",
    "url": "https://github.com/sagemath/sage/issues/5761",
    "user": "https://github.com/rbeezer"
}
```
<div id="comment:0"></div>

Apply only [attachment: trac_5761-latex_docstrings-ts.patch](https://github.com/sagemath/sage/files/ticket5761/trac_5761-latex_docstrings-ts.patch.gz).

Component: **doctest coverage**

Keywords: **latex**

Author: **Travis Scrimshaw**

Reviewer: **André Apitzsch**

Merged: **sage-5.7.beta4**

_Issue created by migration from https://trac.sagemath.org/ticket/5761_





---

archive/issue_events_067884.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-04-11T19:56:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67884"
}
```



---

archive/issue_events_067885.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-04-11T19:56:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67885"
}
```



---

archive/issue_events_067886.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-04-11T19:56:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67886"
}
```



---

archive/issue_events_067887.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-04-11T19:56:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67887"
}
```



---

archive/issue_events_067888.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-11T19:56:35Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "subject": "https://github.com/rbeezer",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67888"
}
```



---

archive/issue_comments_036517.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nI'm partway through this, but still need to do much more, and will be away from it for a few days.",
    "created_at": "2009-04-12T01:00:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5761#issuecomment-36517",
    "user": "https://github.com/rbeezer"
}
```

<div id="comment:1" align="right">comment:1</div>

I'm partway through this, but still need to do much more, and will be away from it for a few days.



---

archive/issue_events_067889.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-12T01:00:59Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "subject": "https://github.com/rbeezer",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67889"
}
```



---

archive/issue_events_067890.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-04-12T01:00:59Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "subject": "https://github.com/rbeezer",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67890"
}
```



---

archive/issue_comments_036518.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nBouncing to 3.4.2.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-13T03:40:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5761#issuecomment-36518",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Bouncing to 3.4.2.

Cheers,

Michael



---

archive/issue_events_067891.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-13T03:40:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67891"
}
```



---

archive/issue_events_067892.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-13T03:40:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67892"
}
```



---

archive/issue_comments_036519.json:
```json
{
    "body": "Author: **Travis Scrimshaw**",
    "created_at": "2013-02-01T16:15:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5761#issuecomment-36519",
    "user": "https://github.com/tscrim"
}
```

Author: **Travis Scrimshaw**



---

archive/issue_comments_036520.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI've gone through an brought the documentation for `latex.py` to full coverage and up to current standards. I've also removed the ~2 year old deprecated method `pdflatex()`.",
    "created_at": "2013-02-01T16:15:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5761#issuecomment-36520",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">comment:3</div>

I've gone through an brought the documentation for `latex.py` to full coverage and up to current standards. I've also removed the ~2 year old deprecated method `pdflatex()`.



---

archive/issue_events_067893.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-01T16:15:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67893"
}
```



---

archive/issue_comments_036521.json:
```json
{
    "body": "Changed keywords from none to **latex**",
    "created_at": "2013-02-01T16:15:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5761#issuecomment-36521",
    "user": "https://github.com/tscrim"
}
```

Changed keywords from none to **latex**



---

archive/issue_events_067894.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-01T16:15:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67894"
}
```



---

archive/issue_events_067895.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2013-02-02T16:08:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67895"
}
```



---

archive/issue_events_067896.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2013-02-02T16:08:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67896"
}
```



---

archive/issue_comments_036522.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThere are some `True, False` and `None` not surrounded by <code>\\`\\`</code>.\n\nAfter\n\n```\n\"\\\\ZZ[x]\n```\nin\n\n```\nEXAMPLES::\n\n    # sage: latex.eval(\"\\\\ZZ[x], locals(), filename=\"test\") # This would generate a file named \"test.png\"\n    # ''\n    # sage: latex.eval(\"\\\\ZZ[x], locals(), filename=\"/path/to/test\") # This would generate a file named \"/path/to/test.png\"\n    # ''\n```\na \" is missing, otherwise the command will fail. You can use *# not tested* at the end of the line instead of commenting out the examples.\n\nEverything else looks good to me.",
    "created_at": "2013-02-02T16:08:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5761#issuecomment-36522",
    "user": "https://github.com/a-andre"
}
```

<div id="comment:4" align="right">comment:4</div>

There are some `True, False` and `None` not surrounded by <code>\`\`</code>.

After

```
"\\ZZ[x]
```
in

```
EXAMPLES::

    # sage: latex.eval("\\ZZ[x], locals(), filename="test") # This would generate a file named "test.png"
    # ''
    # sage: latex.eval("\\ZZ[x], locals(), filename="/path/to/test") # This would generate a file named "/path/to/test.png"
    # ''
```
a " is missing, otherwise the command will fail. You can use *# not tested* at the end of the line instead of commenting out the examples.

Everything else looks good to me.



---

archive/issue_comments_036523.json:
```json
{
    "body": "Reviewer: **Andr\u00e9 Apitzsch**",
    "created_at": "2013-02-02T16:08:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5761#issuecomment-36523",
    "user": "https://github.com/a-andre"
}
```

Reviewer: **André Apitzsch**



---

archive/issue_events_067897.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-05T02:21:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67897"
}
```



---

archive/issue_events_067898.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-05T02:21:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67898"
}
```



---

archive/issue_comments_036524.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nI believe I caught them all; let me know where if I missed any others.\n\nThanks for the review,\n\nTravis",
    "created_at": "2013-02-05T02:21:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5761#issuecomment-36524",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:5" align="right">comment:5</div>

I believe I caught them all; let me know where if I missed any others.

Thanks for the review,

Travis



---

archive/issue_comments_036525.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nCould you add an additional patch to fix the trac reference in `repr_lincomb()`:\n\n```\nVerify that a certain corner case works (see trac 5707 and 5766)::\n```\nand in `latex_varify()`:\n\n```\nif \"is_fname\" flag is True or False.\n```",
    "created_at": "2013-02-05T22:24:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5761#issuecomment-36525",
    "user": "https://github.com/a-andre"
}
```

<div id="comment:6" align="right">comment:6</div>

Could you add an additional patch to fix the trac reference in `repr_lincomb()`:

```
Verify that a certain corner case works (see trac 5707 and 5766)::
```
and in `latex_varify()`:

```
if "is_fname" flag is True or False.
```



---

archive/issue_comments_036526.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nFixed. I've also made the patchbot offending test optional.\n\n---\n\nFor patchbot:\n\nApply only: trac_5791-latex_docstrings-ts.patch",
    "created_at": "2013-02-05T23:16:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5761#issuecomment-36526",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:7" align="right">comment:7</div>

Fixed. I've also made the patchbot offending test optional.

---

For patchbot:

Apply only: trac_5791-latex_docstrings-ts.patch



---

archive/issue_comments_036527.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+Apply only [attachment: trac_5791-latex_docstrings-ts.patch](https://github.com/sagemath/sage/files/ticket5761/trac_5791-latex_docstrings-ts.patch.gz).\n``````\n",
    "created_at": "2013-02-05T23:16:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5761#issuecomment-36527",
    "user": "https://github.com/tscrim"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+Apply only [attachment: trac_5791-latex_docstrings-ts.patch](https://github.com/sagemath/sage/files/ticket5761/trac_5791-latex_docstrings-ts.patch.gz).
``````




---

archive/issue_comments_036528.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nYou added one trailing whitespace after \"An error\"\n\n```\nsage: latex.eval(\"\\ThisIsAnInvalidCommand\", {}) # optional - requires 'convert'\nAn error\n```\nBut I think it's no problem. So I'm giving this a positive review. Let's hope the release manager does not complain about the wrong number in the filename. ;)",
    "created_at": "2013-02-06T18:03:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5761#issuecomment-36528",
    "user": "https://github.com/a-andre"
}
```

<div id="comment:8" align="right">comment:8</div>

You added one trailing whitespace after "An error"

```
sage: latex.eval("\ThisIsAnInvalidCommand", {}) # optional - requires 'convert'
An error
```
But I think it's no problem. So I'm giving this a positive review. Let's hope the release manager does not complain about the wrong number in the filename. ;)



---

archive/issue_events_067899.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2013-02-06T18:03:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67899"
}
```



---

archive/issue_events_067900.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2013-02-06T18:03:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67900"
}
```



---

archive/issue_comments_036529.json:
```json
{
    "body": "Attachment: **[trac_5761-latex_docstrings-ts.patch.gz](https://github.com/sagemath/sage/files/ticket5761/trac_5761-latex_docstrings-ts.patch.gz)**\n\nFixed trailing whitespace and matched ticket number",
    "created_at": "2013-02-06T18:30:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5761#issuecomment-36529",
    "user": "https://github.com/tscrim"
}
```

Attachment: **[trac_5761-latex_docstrings-ts.patch.gz](https://github.com/sagemath/sage/files/ticket5761/trac_5761-latex_docstrings-ts.patch.gz)**

Fixed trailing whitespace and matched ticket number



---

archive/issue_comments_036530.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-Apply only [attachment: trac_5791-latex_docstrings-ts.patch](https://github.com/sagemath/sage/files/ticket5761/trac_5791-latex_docstrings-ts.patch.gz).\n+Apply only [attachment: trac_5761-latex_docstrings-ts.patch](https://github.com/sagemath/sage/files/ticket5761/trac_5761-latex_docstrings-ts.patch.gz).\n``````\n",
    "created_at": "2013-02-06T18:31:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5761#issuecomment-36530",
    "user": "https://github.com/tscrim"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-Apply only [attachment: trac_5791-latex_docstrings-ts.patch](https://github.com/sagemath/sage/files/ticket5761/trac_5791-latex_docstrings-ts.patch.gz).
+Apply only [attachment: trac_5761-latex_docstrings-ts.patch](https://github.com/sagemath/sage/files/ticket5761/trac_5761-latex_docstrings-ts.patch.gz).
``````




---

archive/issue_comments_036531.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nFixed. Thank you.\n\nFor patchbot:\n\nApply only: trac_5761-latex_docstrings-ts.patch",
    "created_at": "2013-02-06T18:31:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5761#issuecomment-36531",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:9" align="right">comment:9</div>

Fixed. Thank you.

For patchbot:

Apply only: trac_5761-latex_docstrings-ts.patch



---

archive/issue_comments_036532.json:
```json
{
    "body": "Merged: **sage-5.7.beta4**",
    "created_at": "2013-02-09T12:13:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5761#issuecomment-36532",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.7.beta4**



---

archive/issue_events_067901.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-09T12:13:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67901"
}
```



---

archive/issue_events_067902.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-09T12:13:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5761",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5761#event-67902"
}
```

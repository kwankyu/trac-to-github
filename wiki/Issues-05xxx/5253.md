# Issue 5253: Make a jacobian function which computes the jacobian matrix

archive/issues_005253.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nAttached patch computes the Jacobian matrix, the matrix of partial derivatives of a multivariable function from `R^n` to `R^m`.\n\nComponent: **calculus**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5253_\n\n",
    "closed_at": "2009-02-14T09:03:39Z",
    "created_at": "2009-02-13T05:08:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20calculus",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make a jacobian function which computes the jacobian matrix",
    "type": "issue",
    "updated_at": "2009-02-14T09:03:39Z",
    "url": "https://github.com/sagemath/sage/issues/5253",
    "user": "https://github.com/jasongrout"
}
```
<div id="comment:0"></div>

Attached patch computes the Jacobian matrix, the matrix of partial derivatives of a multivariable function from `R^n` to `R^m`.

Component: **calculus**

_Issue created by migration from https://trac.sagemath.org/ticket/5253_





---

archive/issue_events_061013.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-13T05:08:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5253",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5253#event-61013"
}
```



---

archive/issue_events_061014.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-13T05:08:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5253",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "c: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5253#event-61014"
}
```



---

archive/issue_events_061015.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-13T05:08:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5253",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5253#event-61015"
}
```



---

archive/issue_events_061016.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-13T05:08:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5253",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5253#event-61016"
}
```



---

archive/issue_events_061017.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-02-13T05:08:19Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5253",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5253#event-61017"
}
```



---

archive/issue_events_061018.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-13T05:11:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5253",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5253#event-61018"
}
```



---

archive/issue_comments_032235.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_5253-jacobian.patch.gz](https://github.com/sagemath/sage/files/ticket5253/trac_5253-jacobian.patch.gz)**",
    "created_at": "2009-02-13T05:11:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5253#issuecomment-32235",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_5253-jacobian.patch.gz](https://github.com/sagemath/sage/files/ticket5253/trac_5253-jacobian.patch.gz)**



---

archive/issue_events_061019.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-13T05:49:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5253",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5253#event-61019"
}
```



---

archive/issue_events_061020.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-02-13T05:49:18Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/5253",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5253#event-61020"
}
```



---

archive/issue_events_061021.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-13T05:49:18Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5253",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5253#event-61021"
}
```



---

archive/issue_events_061022.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2009-02-13T18:52:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5253",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5253#event-61022"
}
```



---

archive/issue_comments_032236.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nDoes what it says, code looks reasonable, all doctests pass.  (I'm not sure if the set of special cases controlling which types it will handle is \"right\", but that can be fixed later if somebody has specific complaints.)\n\nPositive review.",
    "created_at": "2009-02-13T18:52:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5253#issuecomment-32236",
    "user": "https://github.com/sagetrac-cwitty"
}
```

<div id="comment:3" align="right">comment:3</div>

Does what it says, code looks reasonable, all doctests pass.  (I'm not sure if the set of special cases controlling which types it will handle is "right", but that can be fixed later if somebody has specific complaints.)

Positive review.



---

archive/issue_comments_032237.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nFYI, the special casing is because there is no good way right now to represent a function from R<sup>n</sup> to R<sup>m</sup>.  I think people would naturally use either a list, tuple, or vector of symbolic expressions.  The Matrix special case is so that you can do nested jacobians to compute the hessian matrix, like what is illustrated in the doctests.",
    "created_at": "2009-02-13T19:45:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5253#issuecomment-32237",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:4" align="right">comment:4</div>

FYI, the special casing is because there is no good way right now to represent a function from R<sup>n</sup> to R<sup>m</sup>.  I think people would naturally use either a list, tuple, or vector of symbolic expressions.  The Matrix special case is so that you can do nested jacobians to compute the hessian matrix, like what is illustrated in the doctests.



---

archive/issue_comments_032238.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-Attached patch computes the Jacobian matrix, the matrix of partial derivatives of a multivariable function from R^n to R^m.\n+Attached patch computes the Jacobian matrix, the matrix of partial derivatives of a multivariable function from `R^n` to `R^m`.\n``````\n",
    "created_at": "2009-02-14T08:47:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5253#issuecomment-32238",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-Attached patch computes the Jacobian matrix, the matrix of partial derivatives of a multivariable function from R^n to R^m.
+Attached patch computes the Jacobian matrix, the matrix of partial derivatives of a multivariable function from `R^n` to `R^m`.
``````




---

archive/issue_comments_032239.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nMerged in Sage 3.3.rc1.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-14T09:03:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5253#issuecomment-32239",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Merged in Sage 3.3.rc1.

Cheers,

Michael



---

archive/issue_events_061023.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-14T09:03:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5253",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5253#event-61023"
}
```



---

archive/issue_events_061024.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-14T09:03:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5253",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5253#event-61024"
}
```

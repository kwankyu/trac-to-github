# Issue 5299: add functions for Mercurial queues to the hg_sage interface

archive/issues_005299.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "<div id=\"comment:0\"></div>\n\n\n\nComponent: **misc**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5299_\n\n",
    "closed_at": "2013-07-23T14:21:05Z",
    "created_at": "2009-02-17T23:44:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add functions for Mercurial queues to the hg_sage interface",
    "type": "issue",
    "updated_at": "2013-07-23T14:21:05Z",
    "url": "https://github.com/sagemath/sage/issues/5299",
    "user": "https://github.com/mwhansen"
}
```
<div id="comment:0"></div>



Component: **misc**

_Issue created by migration from https://trac.sagemath.org/ticket/5299_





---

archive/issue_events_061667.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-17T23:44:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5299",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5299#event-61667"
}
```



---

archive/issue_events_061668.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-17T23:44:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5299",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5299#event-61668"
}
```



---

archive/issue_events_061669.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-17T23:44:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5299",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5299#event-61669"
}
```



---

archive/issue_events_061670.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-02-17T23:44:26Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5299",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5299#event-61670"
}
```



---

archive/issue_events_061671.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2013-07-23T14:21:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5299",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5299#event-61671"
}
```



---

archive/issue_comments_032618.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThis has already been done:\n\n```\nsage: hg_sage.q\nhg_sage.qapplied    hg_sage.qimport     hg_sage.qpush       hg_sage.qseries     \nhg_sage.qdelete     hg_sage.qnew        hg_sage.qrefresh    hg_sage.qunapplied  \nhg_sage.qdiff       hg_sage.qpop        hg_sage.qremove     \n```",
    "created_at": "2013-07-23T14:21:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5299#issuecomment-32618",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

This has already been done:

```
sage: hg_sage.q
hg_sage.qapplied    hg_sage.qimport     hg_sage.qpush       hg_sage.qseries     
hg_sage.qdelete     hg_sage.qnew        hg_sage.qrefresh    hg_sage.qunapplied  
hg_sage.qdiff       hg_sage.qpop        hg_sage.qremove     
```



---

archive/issue_events_061672.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2013-07-23T14:21:05Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5299",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5299#event-61672"
}
```

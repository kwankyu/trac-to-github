# Issue 5401: Fix the permission issues with the CC field after the trac 0.10.3 -> 0.11.3 update

archive/issues_005401.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nFrom #5400:\n\nMe:\n\n```\nI assume you know about the CC field, so what is the problem? \n```\nJason:\n\n```\nThe only CC option I have now with the new trac is to add \nmyself. It's not a text box anymore. \n```\n\nCheers,\n\nMichael\n\nCC:  @mwhansen @jasongrout\n\nComponent: **website/wiki**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5401_\n\n",
    "closed_at": "2009-03-01T06:15:13Z",
    "created_at": "2009-02-28T16:55:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20website%20/%20wiki",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix the permission issues with the CC field after the trac 0.10.3 -> 0.11.3 update",
    "type": "issue",
    "updated_at": "2009-03-01T06:15:13Z",
    "url": "https://github.com/sagemath/sage/issues/5401",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
<div id="comment:0"></div>

From #5400:

Me:

```
I assume you know about the CC field, so what is the problem? 
```
Jason:

```
The only CC option I have now with the new trac is to add 
myself. It's not a text box anymore. 
```

Cheers,

Michael

CC:  @mwhansen @jasongrout

Component: **website/wiki**

_Issue created by migration from https://trac.sagemath.org/ticket/5401_





---

archive/issue_events_062998.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-28T16:55:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5401#event-62998"
}
```



---

archive/issue_events_062999.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-28T16:55:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20website%20/%20wiki",
    "label_color": "0075ca",
    "label_name": "c: website / wiki",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5401#event-62999"
}
```



---

archive/issue_events_063000.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-28T16:55:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5401#event-63000"
}
```



---

archive/issue_events_063001.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-28T16:55:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5401#event-63001"
}
```



---

archive/issue_events_063002.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2009-02-28T16:55:46Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5401#event-63002"
}
```



---

archive/issue_comments_033415.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,7 +8,8 @@\n Jason:\n \n ```\n-The only CC option I have now with the new trac is to add myself. It's not a text box anymore. \n+The only CC option I have now with the new trac is to add \n+myself. It's not a text box anymore. \n ```\n \n Cheers,\n``````\n",
    "created_at": "2009-02-28T16:55:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5401#issuecomment-33415",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,7 +8,8 @@
 Jason:
 
 ```
-The only CC option I have now with the new trac is to add myself. It's not a text box anymore. 
+The only CC option I have now with the new trac is to add 
+myself. It's not a text box anymore. 
 ```
 
 Cheers,
``````




---

archive/issue_events_063003.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-01T04:51:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5401#event-63003"
}
```



---

archive/issue_events_063004.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-01T04:51:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5401#event-63004"
}
```



---

archive/issue_comments_033416.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nJason,\n\nI have granted authenticated users the permission \"TICKET_EDIT_CC\" - please check if you now can change the CC field and give this ticket a positive review in that case.\n\nCheers,\n\nMichael",
    "created_at": "2009-03-01T04:51:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5401#issuecomment-33416",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Jason,

I have granted authenticated users the permission "TICKET_EDIT_CC" - please check if you now can change the CC field and give this ticket a positive review in that case.

Cheers,

Michael



---

archive/issue_events_063005.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2009-03-01T04:51:38Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5401#event-63005"
}
```



---

archive/issue_events_063006.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-01T04:51:38Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5401#event-63006"
}
```



---

archive/issue_comments_033417.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI get a text box for the CC field now.",
    "created_at": "2009-03-01T06:13:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5401#issuecomment-33417",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:3" align="right">comment:3</div>

I get a text box for the CC field now.



---

archive/issue_events_063007.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-03-01T06:13:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5401#event-63007"
}
```



---

archive/issue_comments_033418.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nExcellent. Fixed during the Sage 3.4.rc0 release cycle.\n\nCheers,\n\nMichael",
    "created_at": "2009-03-01T06:15:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5401#issuecomment-33418",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Excellent. Fixed during the Sage 3.4.rc0 release cycle.

Cheers,

Michael



---

archive/issue_events_063008.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-01T06:15:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5401#event-63008"
}
```



---

archive/issue_events_063009.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-01T06:15:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5401#event-63009"
}
```



---

archive/issue_events_063010.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-01T06:15:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5401#event-63010"
}
```



---

archive/issue_events_063011.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-01T06:15:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5401",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5401#event-63011"
}
```

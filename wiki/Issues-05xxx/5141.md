# Issue 5141: tinymce should be disabled on published worksheets

archive/issues_005141.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nOtherwise bad things happen.\n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5141_\n\n",
    "closed_at": "2009-02-04T01:16:59Z",
    "created_at": "2009-01-30T21:51:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "tinymce should be disabled on published worksheets",
    "type": "issue",
    "updated_at": "2009-02-04T01:16:59Z",
    "url": "https://github.com/sagemath/sage/issues/5141",
    "user": "https://github.com/jasongrout"
}
```
<div id="comment:0"></div>

Otherwise bad things happen.

Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/5141_





---

archive/issue_events_059480.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-01-30T21:51:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59480"
}
```



---

archive/issue_events_059481.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-01-30T21:51:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59481"
}
```



---

archive/issue_events_059482.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-01-30T21:51:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59482"
}
```



---

archive/issue_events_059483.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-01-30T21:51:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59483"
}
```



---

archive/issue_events_059484.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-30T21:51:19Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59484"
}
```



---

archive/issue_events_059485.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-01-30T22:33:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59485"
}
```



---

archive/issue_comments_031383.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nPositive review.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-02T04:59:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5141#issuecomment-31383",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Positive review.

Cheers,

Michael



---

archive/issue_events_059486.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-02T04:59:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59486"
}
```



---

archive/issue_events_059487.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-02T04:59:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59487"
}
```



---

archive/issue_events_059488.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-02T05:01:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59488"
}
```



---

archive/issue_events_059489.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-02T05:01:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59489"
}
```



---

archive/issue_comments_031384.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.3.alpha4.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-02T05:01:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5141#issuecomment-31384",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.3.alpha4.

Cheers,

Michael



---

archive/issue_events_059490.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-02T05:25:12Z",
    "event": "reopened",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59490"
}
```



---

archive/issue_events_059491.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-02T05:25:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59491"
}
```



---

archive/issue_comments_031385.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nOoops, this did break some doctests in cell.py:\n\n```\nsage -t -long \"devel/sage/sage/server/notebook/cell.py\"     \n**********************************************************************\nFile \"/scratch/mabshoff/sage-3.3.alpha4/devel/sage/sage/server/notebook/cell.py\", line 175:\n    sage: C.html()\nException raised:\n    Traceback (most recent call last):\n      File \"/scratch/mabshoff/sage-3.3.alpha4/local/bin/ncadoctest.py\", line 1231, in run_one_test\n        self.run_one_example(test, example, filename, compileflags)\n      File \"/scratch/mabshoff/sage-3.3.alpha4/local/bin/sagedoctest.py\", line 38, in run_one_example\n        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)\n      File \"/scratch/mabshoff/sage-3.3.alpha4/local/bin/ncadoctest.py\", line 1172, in run_one_example\n        compileflags, 1) in test.globs\n      File \"<doctest __main__.example_9[3]>\", line 1, in <module>\n        C.html()###line 175:\n    sage: C.html()\n      File \"/scratch/mabshoff/sage-3.3.alpha4/local/lib/python2.5/site-packages/sage/server/notebook/cell.py\", line 184, in html\n        if JEDITABLE_TINYMCE and not self.worksheet().is_published():\n    AttributeError: 'NoneType' object has no attribute 'is_published'\n**********************************************************************\nFile \"/scratch/mabshoff/sage-3.3.alpha4/devel/sage/sage/server/notebook/cell.py\", line 178:\n    sage: C.html(do_math_parse=True)\nException raised:\n    Traceback (most recent call last):\n      File \"/scratch/mabshoff/sage-3.3.alpha4/local/bin/ncadoctest.py\", line 1231, in run_one_test\n        self.run_one_example(test, example, filename, compileflags)\n      File \"/scratch/mabshoff/sage-3.3.alpha4/local/bin/sagedoctest.py\", line 38, in run_one_example\n        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)\n      File \"/scratch/mabshoff/sage-3.3.alpha4/local/bin/ncadoctest.py\", line 1172, in run_one_example\n        compileflags, 1) in test.globs\n      File \"<doctest __main__.example_9[5]>\", line 1, in <module>\n        C.html(do_math_parse=True)###line 178:\n    sage: C.html(do_math_parse=True)\n      File \"/scratch/mabshoff/sage-3.3.alpha4/local/lib/python2.5/site-packages/sage/server/notebook/cell.py\", line 184, in html\n        if JEDITABLE_TINYMCE and not self.worksheet().is_published():\n    AttributeError: 'NoneType' object has no attribute 'is_published'\n**********************************************************************\n```\n\nShame on me for giving this a positive review :p\n\nReopened. \n\nCheers,\n\nMichel",
    "created_at": "2009-02-02T05:25:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5141#issuecomment-31385",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Ooops, this did break some doctests in cell.py:

```
sage -t -long "devel/sage/sage/server/notebook/cell.py"     
**********************************************************************
File "/scratch/mabshoff/sage-3.3.alpha4/devel/sage/sage/server/notebook/cell.py", line 175:
    sage: C.html()
Exception raised:
    Traceback (most recent call last):
      File "/scratch/mabshoff/sage-3.3.alpha4/local/bin/ncadoctest.py", line 1231, in run_one_test
        self.run_one_example(test, example, filename, compileflags)
      File "/scratch/mabshoff/sage-3.3.alpha4/local/bin/sagedoctest.py", line 38, in run_one_example
        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)
      File "/scratch/mabshoff/sage-3.3.alpha4/local/bin/ncadoctest.py", line 1172, in run_one_example
        compileflags, 1) in test.globs
      File "<doctest __main__.example_9[3]>", line 1, in <module>
        C.html()###line 175:
    sage: C.html()
      File "/scratch/mabshoff/sage-3.3.alpha4/local/lib/python2.5/site-packages/sage/server/notebook/cell.py", line 184, in html
        if JEDITABLE_TINYMCE and not self.worksheet().is_published():
    AttributeError: 'NoneType' object has no attribute 'is_published'
**********************************************************************
File "/scratch/mabshoff/sage-3.3.alpha4/devel/sage/sage/server/notebook/cell.py", line 178:
    sage: C.html(do_math_parse=True)
Exception raised:
    Traceback (most recent call last):
      File "/scratch/mabshoff/sage-3.3.alpha4/local/bin/ncadoctest.py", line 1231, in run_one_test
        self.run_one_example(test, example, filename, compileflags)
      File "/scratch/mabshoff/sage-3.3.alpha4/local/bin/sagedoctest.py", line 38, in run_one_example
        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)
      File "/scratch/mabshoff/sage-3.3.alpha4/local/bin/ncadoctest.py", line 1172, in run_one_example
        compileflags, 1) in test.globs
      File "<doctest __main__.example_9[5]>", line 1, in <module>
        C.html(do_math_parse=True)###line 178:
    sage: C.html(do_math_parse=True)
      File "/scratch/mabshoff/sage-3.3.alpha4/local/lib/python2.5/site-packages/sage/server/notebook/cell.py", line 184, in html
        if JEDITABLE_TINYMCE and not self.worksheet().is_published():
    AttributeError: 'NoneType' object has no attribute 'is_published'
**********************************************************************
```

Shame on me for giving this a positive review :p

Reopened. 

Cheers,

Michel



---

archive/issue_comments_031386.json:
```json
{
    "body": "Attachment: **[trac_5141-tinymce-published.patch.gz](https://github.com/sagemath/sage/files/ticket5141/trac_5141-tinymce-published.patch.gz)**",
    "created_at": "2009-02-02T20:19:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5141#issuecomment-31386",
    "user": "https://github.com/jasongrout"
}
```

Attachment: **[trac_5141-tinymce-published.patch.gz](https://github.com/sagemath/sage/files/ticket5141/trac_5141-tinymce-published.patch.gz)**



---

archive/issue_events_059492.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-02T20:20:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59492"
}
```



---

archive/issue_events_059493.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-02T20:20:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59493"
}
```



---

archive/issue_comments_031387.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\npatch updated to check for the existence of the .is_published() method.  It should work now.",
    "created_at": "2009-02-02T20:20:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5141#issuecomment-31387",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:5" align="right">comment:5</div>

patch updated to check for the existence of the .is_published() method.  It should work now.



---

archive/issue_events_059494.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-03T08:19:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59494"
}
```



---

archive/issue_events_059495.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-03T08:19:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
    "label_color": "ff7700",
    "label_name": "p: critical / 2",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59495"
}
```



---

archive/issue_events_059496.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-03T09:25:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59496"
}
```



---

archive/issue_events_059497.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-02-03T09:25:24Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59497"
}
```



---

archive/issue_events_059498.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-03T09:25:24Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59498"
}
```



---

archive/issue_events_059499.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2009-02-04T00:52:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59499"
}
```



---

archive/issue_comments_031388.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nMerged in Sage 3.3.alpha5.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-04T01:16:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5141#issuecomment-31388",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:9" align="right">comment:9</div>

Merged in Sage 3.3.alpha5.

Cheers,

Michael



---

archive/issue_events_059500.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-04T01:16:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59500"
}
```



---

archive/issue_events_059501.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-04T01:16:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5141",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5141#event-59501"
}
```

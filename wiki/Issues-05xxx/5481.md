# Issue 5481: devel/doc/output/* should be filtered from the list of files to doctest

archive/issues_005481.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThere can be many rst files under devel/doc/output - those should be filtered from the list of files to doctest since they are duplicate doctests from the main Sage library in many cases.\n\nCheers,\n\nMichael\n\nCC:  @mwhansen\n\nComponent: **doctest coverage**\n\nAuthor: **John Palmieri**\n\nReviewer: **Mike Hansen**\n\nMerged: **sage-4.1.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5481_\n\n",
    "closed_at": "2009-06-26T17:43:01Z",
    "created_at": "2009-03-11T06:31:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "devel/doc/output/* should be filtered from the list of files to doctest",
    "type": "issue",
    "updated_at": "2009-06-29T21:01:08Z",
    "url": "https://github.com/sagemath/sage/issues/5481",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
<div id="comment:0"></div>

There can be many rst files under devel/doc/output - those should be filtered from the list of files to doctest since they are duplicate doctests from the main Sage library in many cases.

Cheers,

Michael

CC:  @mwhansen

Component: **doctest coverage**

Author: **John Palmieri**

Reviewer: **Mike Hansen**

Merged: **sage-4.1.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/5481_





---

archive/issue_events_064138.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-11T06:31:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "milestone_number": null,
    "milestone_title": "sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5481#event-64138"
}
```



---

archive/issue_events_064139.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-11T06:31:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5481#event-64139"
}
```



---

archive/issue_events_064140.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-11T06:31:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5481#event-64140"
}
```



---

archive/issue_events_064141.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-11T06:31:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5481#event-64141"
}
```



---

archive/issue_events_064142.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-11T06:31:10Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5481#event-64142"
}
```



---

archive/issue_comments_034159.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nHas this been fixed?  The file sage-maketest looks good to me already.",
    "created_at": "2009-06-09T22:11:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5481#issuecomment-34159",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:1" align="right">comment:1</div>

Has this been fixed?  The file sage-maketest looks good to me already.



---

archive/issue_events_064143.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-15T23:24:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5481#event-64143"
}
```



---

archive/issue_events_064144.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-15T23:24:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
    "label_color": "ff7700",
    "label_name": "p: critical / 2",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5481#event-64144"
}
```



---

archive/issue_comments_034160.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMaybe sage-ptest needs changing.  I'm still trying to figure out what the issue is here.",
    "created_at": "2009-06-16T03:12:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5481#issuecomment-34160",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:3" align="right">comment:3</div>

Maybe sage-ptest needs changing.  I'm still trying to figure out what the issue is here.



---

archive/issue_events_064145.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-06-16T03:28:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5481#event-64145"
}
```



---

archive/issue_comments_034161.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nAttachment: **[trac_5481_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket5481/trac_5481_scripts.patch.gz)**\n\nHere's a patch.  (The first change -- deleting all.py and __init__.py -- comes from #6108.)",
    "created_at": "2009-06-16T03:28:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5481#issuecomment-34161",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:4" align="right">comment:4</div>

Attachment: **[trac_5481_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket5481/trac_5481_scripts.patch.gz)**

Here's a patch.  (The first change -- deleting all.py and __init__.py -- comes from #6108.)



---

archive/issue_comments_034162.json:
```json
{
    "body": "Author: **John Palmieri**",
    "created_at": "2009-06-20T01:29:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5481#issuecomment-34162",
    "user": "https://github.com/mwhansen"
}
```

Author: **John Palmieri**



---

archive/issue_comments_034163.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nLooks good to me.",
    "created_at": "2009-06-20T01:29:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5481#issuecomment-34163",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:5" align="right">comment:5</div>

Looks good to me.



---

archive/issue_comments_034164.json:
```json
{
    "body": "Reviewer: **Mike Hansen**",
    "created_at": "2009-06-20T01:29:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5481#issuecomment-34164",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **Mike Hansen**



---

archive/issue_events_064146.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-20T01:29:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5481#event-64146"
}
```



---

archive/issue_events_064147.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-06-20T01:29:00Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5481#event-64147"
}
```



---

archive/issue_events_064148.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-20T01:29:00Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5481#event-64148"
}
```



---

archive/issue_events_064149.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-20T01:29:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5481#event-64149"
}
```



---

archive/issue_comments_034165.json:
```json
{
    "body": "Merged: **sage-4.1.alpha2**",
    "created_at": "2009-06-26T17:43:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5481#issuecomment-34165",
    "user": "https://github.com/boothby"
}
```

Merged: **sage-4.1.alpha2**



---

archive/issue_events_064150.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-06-26T17:43:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5481#event-64150"
}
```



---

archive/issue_events_064151.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-06-26T17:43:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5481#event-64151"
}
```



---

archive/issue_comments_034166.json:
```json
{
    "body": "Changed merged from **sage-4.1.alpha2** to **sage-4.1.alpha3**",
    "created_at": "2009-06-29T21:01:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5481#issuecomment-34166",
    "user": "https://github.com/rlmill"
}
```

Changed merged from **sage-4.1.alpha2** to **sage-4.1.alpha3**

# Issue 5008: Solaris/gcc 4.3.2: fix matplotlib build

archive/issues_005008.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nMatplotlib has some build problems on Solaris when using gcc 4.3.2 that do not happen on other platforms.\n\nSpkg coming up.\n\nCheers,\n\nMichael\n\nComponent: **porting: Solaris**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5008_\n\n",
    "closed_at": "2009-01-19T12:01:43Z",
    "created_at": "2009-01-18T06:31:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20solaris",
        "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Solaris/gcc 4.3.2: fix matplotlib build",
    "type": "issue",
    "updated_at": "2009-01-19T12:01:43Z",
    "url": "https://github.com/sagemath/sage/issues/5008",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
<div id="comment:0"></div>

Matplotlib has some build problems on Solaris when using gcc 4.3.2 that do not happen on other platforms.

Spkg coming up.

Cheers,

Michael

Component: **porting: Solaris**

_Issue created by migration from https://trac.sagemath.org/ticket/5008_





---

archive/issue_events_057651.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-18T06:31:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5008",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5008#event-57651"
}
```



---

archive/issue_events_057652.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-18T06:31:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5008",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20solaris",
    "label_color": "0000b0",
    "label_name": "c: porting: solaris",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5008#event-57652"
}
```



---

archive/issue_events_057653.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-18T06:31:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5008",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
    "label_color": "ff7700",
    "label_name": "p: critical / 2",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5008#event-57653"
}
```



---

archive/issue_events_057654.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-18T06:31:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5008",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5008#event-57654"
}
```



---

archive/issue_events_057655.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-18T06:31:49Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5008",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5008#event-57655"
}
```



---

archive/issue_events_057656.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-19T11:54:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5008",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5008#event-57656"
}
```



---

archive/issue_comments_030452.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThe spkg at \n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.3/alpha0/matplotlib-0.98.3.p5.spkg\n\nadds a Solaris 10 specific workaround. On other platforms the workaround is not applied.\n\nCheers,\n\nMichael",
    "created_at": "2009-01-19T11:54:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5008",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5008#issuecomment-30452",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

The spkg at 

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.3/alpha0/matplotlib-0.98.3.p5.spkg

adds a Solaris 10 specific workaround. On other platforms the workaround is not applied.

Cheers,

Michael



---

archive/issue_events_057657.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-19T11:58:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5008",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5008#event-57657"
}
```



---

archive/issue_events_057658.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-19T11:58:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5008",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5008#event-57658"
}
```



---

archive/issue_comments_030453.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nWorks fine for me.",
    "created_at": "2009-01-19T11:58:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5008",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5008#issuecomment-30453",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">comment:3</div>

Works fine for me.



---

archive/issue_events_057659.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-19T12:01:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5008",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5008#event-57659"
}
```



---

archive/issue_events_057660.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-19T12:01:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5008",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5008#event-57660"
}
```



---

archive/issue_comments_030454.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in Sage 3.3.alpha0",
    "created_at": "2009-01-19T12:01:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5008",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5008#issuecomment-30454",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in Sage 3.3.alpha0

# Issue 5770: Bring doctests of modular/modsym/p1list.py up to 100%

archive/issues_005770.json:
```json
{
    "assignees": [
        "https://github.com/craigcitro"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe attached patch completes all docstrings and doctests for the file modular/modsym/p1list.py.  I also checked that the html and pdf output in the reference manual look good.\n\nCC:  @loefflerd\n\nComponent: **modular forms**\n\nKeywords: **modular form documentation**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5770_\n\n",
    "closed_at": "2009-04-13T22:06:08Z",
    "created_at": "2009-04-12T17:25:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/c%3A%20documentation"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bring doctests of modular/modsym/p1list.py up to 100%",
    "type": "issue",
    "updated_at": "2009-04-13T22:06:08Z",
    "url": "https://github.com/sagemath/sage/issues/5770",
    "user": "https://github.com/JohnCremona"
}
```
<div id="comment:0"></div>

The attached patch completes all docstrings and doctests for the file modular/modsym/p1list.py.  I also checked that the html and pdf output in the reference manual look good.

CC:  @loefflerd

Component: **modular forms**

Keywords: **modular form documentation**

_Issue created by migration from https://trac.sagemath.org/ticket/5770_





---

archive/issue_events_068046.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-04-12T17:25:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5770#event-68046"
}
```



---

archive/issue_events_068047.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-04-12T17:25:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
    "label_color": "0000ff",
    "label_name": "c: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5770#event-68047"
}
```



---

archive/issue_events_068048.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-04-12T17:25:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5770#event-68048"
}
```



---

archive/issue_events_068049.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-04-12T17:25:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5770#event-68049"
}
```



---

archive/issue_events_068050.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-04-12T17:25:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
    "label_color": "0000ff",
    "label_name": "c: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5770#event-68050"
}
```



---

archive/issue_events_068051.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-04-12T17:25:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5770#event-68051"
}
```



---

archive/issue_events_068052.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-04-12T17:25:24Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "subject": "https://github.com/JohnCremona",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5770#event-68052"
}
```



---

archive/issue_events_068053.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-04-12T17:25:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5770#event-68053"
}
```



---

archive/issue_comments_036614.json:
```json
{
    "body": "Attachment: **[p1list-doc.patch.gz](https://github.com/sagemath/sage/files/ticket5770/p1list-doc.patch.gz)**\n\nBased on 3.4.1.rc2",
    "created_at": "2009-04-12T17:25:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5770#issuecomment-36614",
    "user": "https://github.com/JohnCremona"
}
```

Attachment: **[p1list-doc.patch.gz](https://github.com/sagemath/sage/files/ticket5770/p1list-doc.patch.gz)**

Based on 3.4.1.rc2



---

archive/issue_comments_036615.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nPatch applies fine to 3.4.1.rc2, all doctests pass, and reference manual builds happily. There is just one tiny typo: in the docstring for \"apply_S\", it has \"[-0,1;1,0]\" instead of \"[0,-1;1,0]\". I have uploaded a patch that corrects this.",
    "created_at": "2009-04-13T12:32:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5770#issuecomment-36615",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:2" align="right">comment:2</div>

Patch applies fine to 3.4.1.rc2, all doctests pass, and reference manual builds happily. There is just one tiny typo: in the docstring for "apply_S", it has "[-0,1;1,0]" instead of "[0,-1;1,0]". I have uploaded a patch that corrects this.



---

archive/issue_events_068054.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-04-13T12:32:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5770#event-68054"
}
```



---

archive/issue_comments_036616.json:
```json
{
    "body": "apply over previous patch",
    "created_at": "2009-04-13T12:32:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5770#issuecomment-36616",
    "user": "https://github.com/loefflerd"
}
```

apply over previous patch



---

archive/issue_comments_036617.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAttachment: **[trac_5770-reviewer_fix.patch.gz](https://github.com/sagemath/sage/files/ticket5770/trac_5770-reviewer_fix.patch.gz)**\n\nThanks, David.  Getting the documentation to build & look ok takes a long time!  As you can imagine there was a lot of cutting and pasting.  Much more of the same to com with manin_symbols.py, where there are about 5 classes each of which has a very similar set of methods (but not quite identical).",
    "created_at": "2009-04-13T13:16:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5770#issuecomment-36617",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:3" align="right">comment:3</div>

Attachment: **[trac_5770-reviewer_fix.patch.gz](https://github.com/sagemath/sage/files/ticket5770/trac_5770-reviewer_fix.patch.gz)**

Thanks, David.  Getting the documentation to build & look ok takes a long time!  As you can imagine there was a lot of cutting and pasting.  Much more of the same to com with manin_symbols.py, where there are about 5 classes each of which has a very similar set of methods (but not quite identical).



---

archive/issue_events_068055.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-13T22:06:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5770#event-68055"
}
```



---

archive/issue_events_068056.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-13T22:06:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5770#event-68056"
}
```



---

archive/issue_comments_036618.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged both patches in Sage 3.4.1.rc3.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-13T22:06:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5770#issuecomment-36618",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged both patches in Sage 3.4.1.rc3.

Cheers,

Michael



---

archive/issue_events_068057.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-13T22:06:08Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5770#event-68057"
}
```



---

archive/issue_events_068058.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-13T22:06:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5770",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5770#event-68058"
}
```

# Issue 5258: escape html strings with cgi.escape instead of custom (and lacking) regexp

archive/issues_005258.json:
```json
{
    "body": "See http://groups.google.com/group/sage-devel/browse_thread/thread/3fd6f52b4b04108d for the discussion.\n\n\n**Assignee:** @jasongrout\n\nIssue created by migration from https://trac.sagemath.org/ticket/5258\n\n",
    "closed_at": "2009-02-14T02:13:46Z",
    "created_at": "2009-02-13T19:06:54Z",
    "labels": [
        "component: notebook",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "title": "escape html strings with cgi.escape instead of custom (and lacking) regexp",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/5258",
    "user": "https://github.com/jasongrout"
}
```
See http://groups.google.com/group/sage-devel/browse_thread/thread/3fd6f52b4b04108d for the discussion.


**Assignee:** @jasongrout

Issue created by migration from https://trac.sagemath.org/ticket/5258





---

archive/issue_events_032782.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-13T19:11:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5258",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5258#event-32782"
}
```



---

archive/attachments_006136.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_5258-escape-with-cgi.patch",
    "asset_url": "tarball://root/attachments/ticket5258/trac_5258-escape-with-cgi.patch",
    "created_at": "2009-02-13T19:11:03Z",
    "issue": "https://github.com/sagemath/sage/issues/5258",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket5258/trac_5258-escape-with-cgi.patch",
    "user": "https://github.com/jasongrout"
}
```



---

archive/issue_comments_033514.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_5258-escape-with-cgi.patch](https://github.com/sagemath/sage/files/ticket5258/trac_5258-escape-with-cgi.patch)",
    "created_at": "2009-02-13T19:11:03Z",
    "issue": "https://github.com/sagemath/sage/issues/5258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5258#issuecomment-33514",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_5258-escape-with-cgi.patch](https://github.com/sagemath/sage/files/ticket5258/trac_5258-escape-with-cgi.patch)



---

archive/issue_comments_033515.json:
```json
{
    "body": "<a id='comment:2'></a>\n+1 and positive review!",
    "created_at": "2009-02-13T19:27:00Z",
    "issue": "https://github.com/sagemath/sage/issues/5258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5258#issuecomment-33515",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>
+1 and positive review!



---

archive/issue_events_032783.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-02-13T19:27:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5258",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5258#event-32783"
}
```



---

archive/issue_events_032784.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-02-13T19:27:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5258",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5258#event-32784"
}
```



---

archive/issue_comments_033516.json:
```json
{
    "body": "**Changing assignee** from boothby to @jasongrout.",
    "created_at": "2009-02-13T23:08:21Z",
    "issue": "https://github.com/sagemath/sage/issues/5258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5258#issuecomment-33516",
    "user": "https://github.com/jasongrout"
}
```

**Changing assignee** from boothby to @jasongrout.



---

archive/issue_events_032785.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-13T23:08:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5258",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5258#event-32785"
}
```



---

archive/issue_events_032786.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-14T02:13:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5258",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5258#event-32786"
}
```



---

archive/issue_comments_033517.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.3.rc1.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-14T02:13:46Z",
    "issue": "https://github.com/sagemath/sage/issues/5258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5258#issuecomment-33517",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.3.rc1.

Cheers,

Michael



---

archive/issue_comments_033518.json:
```json
{
    "body": "<a id='comment:5'></a>\nI blew it on this review -- there is a major major bug in this patch in that it uses escape in twist.py but does *not* import it!!  See #5358.",
    "created_at": "2009-02-24T14:44:48Z",
    "issue": "https://github.com/sagemath/sage/issues/5258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5258#issuecomment-33518",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:5'></a>
I blew it on this review -- there is a major major bug in this patch in that it uses escape in twist.py but does *not* import it!!  See #5358.



---

archive/issue_comments_033519.json:
```json
{
    "body": "<a id='comment:6'></a>\nWell, I blew it even more by writing the error in the patch!",
    "created_at": "2009-02-24T15:20:37Z",
    "issue": "https://github.com/sagemath/sage/issues/5258",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5258#issuecomment-33519",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:6'></a>
Well, I blew it even more by writing the error in the patch!

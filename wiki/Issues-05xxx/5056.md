# Issue 5056: rename Ideal.reduced_basis to Ideal.interreduced_basis

archive/issues_005056.json:
```json
{
    "assignees": [
        "https://github.com/malb"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nIt seems like people get confused by the name and assume the function returns the reduced **Gr\u00f6bner** basis. Thus `reduced_basis()` should be deprecated and `interreduced_basis()` should take its place.\n\nCC:  @johnperry-math\n\nComponent: **commutative algebra**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5056_\n\n",
    "closed_at": "2009-01-25T02:21:05Z",
    "created_at": "2009-01-22T18:34:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "rename Ideal.reduced_basis to Ideal.interreduced_basis",
    "type": "issue",
    "updated_at": "2009-01-25T02:21:05Z",
    "url": "https://github.com/sagemath/sage/issues/5056",
    "user": "https://github.com/malb"
}
```
<div id="comment:0"></div>

It seems like people get confused by the name and assume the function returns the reduced **Gröbner** basis. Thus `reduced_basis()` should be deprecated and `interreduced_basis()` should take its place.

CC:  @johnperry-math

Component: **commutative algebra**

_Issue created by migration from https://trac.sagemath.org/ticket/5056_





---

archive/issue_events_058233.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-01-22T18:34:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5056",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5056#event-58233"
}
```



---

archive/issue_events_058234.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-01-22T18:34:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5056",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "c: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5056#event-58234"
}
```



---

archive/issue_events_058235.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-01-22T18:34:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5056",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5056#event-58235"
}
```



---

archive/issue_events_058236.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-01-22T18:34:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5056",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5056#event-58236"
}
```



---

archive/issue_events_058237.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-01-22T18:34:52Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5056",
    "subject": "https://github.com/malb",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5056#event-58237"
}
```



---

archive/issue_comments_030720.json:
```json
{
    "body": "\"patch\", not \"path\"",
    "created_at": "2009-01-22T23:09:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5056",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5056#issuecomment-30720",
    "user": "https://github.com/johnperry-math"
}
```

"patch", not "path"



---

archive/issue_events_058238.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-01-23T02:45:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5056",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5056#event-58238"
}
```



---

archive/issue_comments_030721.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[interreduced_basis_path.gz](https://github.com/sagemath/sage/files/ticket5056/interreduced_basis_path.gz)**",
    "created_at": "2009-01-23T02:45:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5056",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5056#issuecomment-30721",
    "user": "https://github.com/malb"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[interreduced_basis_path.gz](https://github.com/sagemath/sage/files/ticket5056/interreduced_basis_path.gz)**



---

archive/issue_events_058239.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2009-01-24T16:46:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5056",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5056#event-58239"
}
```



---

archive/issue_events_058240.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2009-01-24T16:46:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5056",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5056#event-58240"
}
```



---

archive/issue_comments_030722.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThe patch applies cleanly (in spite of its strange name :) ), there is a deprecation warning, and the doc tests of `multi_polynomial_ideal.py` pass. In other words, the patch does what it is supposed to.\nPositive review!",
    "created_at": "2009-01-24T16:46:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5056",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5056#issuecomment-30722",
    "user": "https://github.com/simon-king-jena"
}
```

<div id="comment:2" align="right">comment:2</div>

The patch applies cleanly (in spite of its strange name :) ), there is a deprecation warning, and the doc tests of `multi_polynomial_ideal.py` pass. In other words, the patch does what it is supposed to.
Positive review!



---

archive/issue_events_058241.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-25T02:21:05Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5056",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5056#event-58241"
}
```



---

archive/issue_events_058242.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-25T02:21:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5056",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5056#event-58242"
}
```



---

archive/issue_events_058243.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-25T02:21:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5056",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5056#event-58243"
}
```



---

archive/issue_events_058244.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-25T02:21:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5056",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5056#event-58244"
}
```



---

archive/issue_comments_030723.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.3.alpha2.\n\nCheers,\n\nMichael",
    "created_at": "2009-01-25T02:21:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5056",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5056#issuecomment-30723",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.3.alpha2.

Cheers,

Michael

# Issue 5227: add next batch of 14 people to devmap

archive/issues_005227.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nAs the subject says. This is a joint project with Michael Abshoff and Harald Schilly. The devmap in question is the site\n\n\n\nhttp://www.sagemath.org/development-map.html\n\n\nCC:  @haraldschilly @sagetrac-gmoroz\n\nComponent: **website/wiki**\n\nKeywords: **devmap**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5227_\n\n",
    "closed_at": "2009-02-17T13:32:14Z",
    "created_at": "2009-02-10T07:56:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20website%20/%20wiki",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add next batch of 14 people to devmap",
    "type": "issue",
    "updated_at": "2009-02-17T13:38:51Z",
    "url": "https://github.com/sagemath/sage/issues/5227",
    "user": "https://github.com/sagetrac-mvngu"
}
```
<div id="comment:0"></div>

As the subject says. This is a joint project with Michael Abshoff and Harald Schilly. The devmap in question is the site



http://www.sagemath.org/development-map.html


CC:  @haraldschilly @sagetrac-gmoroz

Component: **website/wiki**

Keywords: **devmap**

_Issue created by migration from https://trac.sagemath.org/ticket/5227_





---

archive/issue_events_060625.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-02-10T07:56:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60625"
}
```



---

archive/issue_events_060626.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-02-10T07:56:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20website%20/%20wiki",
    "label_color": "0075ca",
    "label_name": "c: website / wiki",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60626"
}
```



---

archive/issue_events_060627.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-02-10T07:56:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60627"
}
```



---

archive/issue_events_060628.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-02-10T07:56:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60628"
}
```



---

archive/issue_comments_032012.json:
```json
{
    "body": "Attachment: **[trac_5227-alphabetize.patch.gz](https://github.com/sagemath/sage/files/ticket5227/trac_5227-alphabetize.patch.gz)**\n\nalphabetize the current devmap",
    "created_at": "2009-02-10T08:25:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32012",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[trac_5227-alphabetize.patch.gz](https://github.com/sagemath/sage/files/ticket5227/trac_5227-alphabetize.patch.gz)**

alphabetize the current devmap



---

archive/issue_comments_032013.json:
```json
{
    "body": "Attachment: **[trac_5227-14-new-people.patch.gz](https://github.com/sagemath/sage/files/ticket5227/trac_5227-14-new-people.patch.gz)**\n\nadd 14 people to devmap",
    "created_at": "2009-02-10T08:26:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32013",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[trac_5227-14-new-people.patch.gz](https://github.com/sagemath/sage/files/ticket5227/trac_5227-14-new-people.patch.gz)**

add 14 people to devmap



---

archive/issue_comments_032014.json:
```json
{
    "body": "Attachment: **[trac_5227-update-info-1.patch.gz](https://github.com/sagemath/sage/files/ticket5227/trac_5227-update-info-1.patch.gz)**\n\nupdate info for Simon King and Georg S. Weber",
    "created_at": "2009-02-10T08:27:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32014",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[trac_5227-update-info-1.patch.gz](https://github.com/sagemath/sage/files/ticket5227/trac_5227-update-info-1.patch.gz)**

update info for Simon King and Georg S. Weber



---

archive/issue_comments_032015.json:
```json
{
    "body": "Attachment: **[trac_5227-update-info-2.patch.gz](https://github.com/sagemath/sage/files/ticket5227/trac_5227-update-info-2.patch.gz)**\n\nupdate info for Arnaud Bergeron, David Loeffler and Jason Merrill",
    "created_at": "2009-02-10T08:28:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32015",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[trac_5227-update-info-2.patch.gz](https://github.com/sagemath/sage/files/ticket5227/trac_5227-update-info-2.patch.gz)**

update info for Arnaud Bergeron, David Loeffler and Jason Merrill



---

archive/issue_events_060629.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-02-10T08:38:02Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "title_is": "[need review] add next batch of 14 people to devmap",
    "title_was": "add next batch of 14 people to devmap",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60629"
}
```



---

archive/issue_comments_032016.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThe attached file `contributors.xml` is an updated XML file for the development map. For those folks who like to view diffs, I've attached the relevant diffs produced using Mercurial. The patch `trac_5227-alphabetize.patch` re-organizes the current devmap XML file, in particular, alphabetizing the names list except for William Stein. The patch `trac_5227-14-new-people.patch` adds the following people to the devmap:\n1. Maite Aranes\n2. Tom Denton\n3. Amy Glen\n4. Daniel Gordon\n5. Alexander Hupfer\n6. Yann Laigle-Chapuy\n7. Matthias Meulien\n8. Ronan Paix\u00e3o\n9. David Perkinson\n10. Pearu Peterson\n11. Blair Sutton\n12. Griffen Thoma\n13. Georg S. Weber\n14. Bin Zhang\n\nThe patches `trac_5227-update-info-1.patch` and `trac_5227-update-info-2.patch` update information for the following people:\n1. Arnaud Bergeron\n2. Simon King\n3. David Loeffler\n4. Jason Merrill",
    "created_at": "2009-02-10T08:38:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32016",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:1" align="right">comment:1</div>

The attached file `contributors.xml` is an updated XML file for the development map. For those folks who like to view diffs, I've attached the relevant diffs produced using Mercurial. The patch `trac_5227-alphabetize.patch` re-organizes the current devmap XML file, in particular, alphabetizing the names list except for William Stein. The patch `trac_5227-14-new-people.patch` adds the following people to the devmap:
1. Maite Aranes
2. Tom Denton
3. Amy Glen
4. Daniel Gordon
5. Alexander Hupfer
6. Yann Laigle-Chapuy
7. Matthias Meulien
8. Ronan Paixão
9. David Perkinson
10. Pearu Peterson
11. Blair Sutton
12. Griffen Thoma
13. Georg S. Weber
14. Bin Zhang

The patches `trac_5227-update-info-1.patch` and `trac_5227-update-info-2.patch` update information for the following people:
1. Arnaud Bergeron
2. Simon King
3. David Loeffler
4. Jason Merrill



---

archive/issue_comments_032017.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nIf my counting is correct, the updated XML file shows information for **141** Sage developers around the world.",
    "created_at": "2009-02-10T08:52:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32017",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:2" align="right">comment:2</div>

If my counting is correct, the updated XML file shows information for **141** Sage developers around the world.



---

archive/issue_comments_032018.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nto my best knowledge, the location of Guillaume Moroz is wrong: instead of being in the middle\nof France, it should be in Paris (LIP6 is in Paris). The best is to check with him of course.",
    "created_at": "2009-02-12T11:20:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32018",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:3" align="right">comment:3</div>

to my best knowledge, the location of Guillaume Moroz is wrong: instead of being in the middle
of France, it should be in Paris (LIP6 is in Paris). The best is to check with him of course.



---

archive/issue_comments_032019.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nWell, \n\nthe flag being in the middle of a country usually indicates that the address in the XML file is not resolvable by Google.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-12T11:34:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32019",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Well, 

the flag being in the middle of a country usually indicates that the address in the XML file is not resolvable by Google.

Cheers,

Michael



---

archive/issue_comments_032020.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nReplying to [@zimmermann6](#comment%3A3):\n> to my best knowledge, the location of Guillaume Moroz is wrong: instead of being in the middle\n> of France, it should be in Paris (LIP6 is in Paris). The best is to check with him of course.\n\nthe location is feed into maps.google.com. if you go there, enter the location string 1:1 and hit search and the first hit is wrong, it will be also wrong on the devmap! from my experience, changing a word or some details is enough to get it right.\n\nbehind the scenes, offline: all location strings are extrcted and a script creates a mapping from each uniqe string to the lat/long coordinates that are then used online by the map-javascript when loading the page and placing the markers.",
    "created_at": "2009-02-12T11:48:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32020",
    "user": "https://github.com/haraldschilly"
}
```

<div id="comment:6" align="right">comment:6</div>

Replying to [@zimmermann6](#comment%3A3):
> to my best knowledge, the location of Guillaume Moroz is wrong: instead of being in the middle
> of France, it should be in Paris (LIP6 is in Paris). The best is to check with him of course.

the location is feed into maps.google.com. if you go there, enter the location string 1:1 and hit search and the first hit is wrong, it will be also wrong on the devmap! from my experience, changing a word or some details is enough to get it right.

behind the scenes, offline: all location strings are extrcted and a script creates a mapping from each uniqe string to the lat/long coordinates that are then used online by the map-javascript when loading the page and placing the markers.



---

archive/issue_comments_032021.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\n\"Guillaume Moroz LIP6\" on maps.google.com finds only one location, which is the correct one.\nHowever I don't know how to give that hint to the devmap.",
    "created_at": "2009-02-12T11:57:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32021",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:7" align="right">comment:7</div>

"Guillaume Moroz LIP6" on maps.google.com finds only one location, which is the correct one.
However I don't know how to give that hint to the devmap.



---

archive/issue_comments_032022.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nReplying to [@zimmermann6](#comment%3A7):\n> \"Guillaume Moroz LIP6\" on maps.google.com finds only one location...\n\nwell, i wasn't honest, google maps already narrows down the possible hits, if you have zoomed into a location. for this location, i think \"Guillaume Moroz LIP6, Paris, Fr\" is ok.",
    "created_at": "2009-02-12T12:02:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32022",
    "user": "https://github.com/haraldschilly"
}
```

<div id="comment:8" align="right">comment:8</div>

Replying to [@zimmermann6](#comment%3A7):
> "Guillaume Moroz LIP6" on maps.google.com finds only one location...

well, i wasn't honest, google maps already narrows down the possible hits, if you have zoomed into a location. for this location, i think "Guillaume Moroz LIP6, Paris, Fr" is ok.



---

archive/issue_comments_032023.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\n(Try to adjust the Summary so that this ticket shows up on the radar, especially View tickets \"10\")",
    "created_at": "2009-02-15T20:39:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32023",
    "user": "https://github.com/sagetrac-GeorgSWeber"
}
```

<div id="comment:9" align="right">comment:9</div>

(Try to adjust the Summary so that this ticket shows up on the radar, especially View tickets "10")



---

archive/issue_events_060630.json:
```json
{
    "actor": "https://github.com/sagetrac-GeorgSWeber",
    "created_at": "2009-02-15T20:39:45Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "title_is": "add next batch of 14 people to devmap",
    "title_was": "[need review] add next batch of 14 people to devmap",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60630"
}
```



---

archive/issue_events_060631.json:
```json
{
    "actor": "https://github.com/sagetrac-GeorgSWeber",
    "created_at": "2009-02-15T20:39:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60631"
}
```



---

archive/issue_comments_032024.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nYep, it's now (and only now) visible under \"http://trac.sagemath.org/sage_trac/report/10\"",
    "created_at": "2009-02-15T20:40:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32024",
    "user": "https://github.com/sagetrac-GeorgSWeber"
}
```

<div id="comment:10" align="right">comment:10</div>

Yep, it's now (and only now) visible under "http://trac.sagemath.org/sage_trac/report/10"



---

archive/issue_events_060632.json:
```json
{
    "actor": "https://github.com/sagetrac-GeorgSWeber",
    "created_at": "2009-02-15T20:51:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60632"
}
```



---

archive/issue_events_060633.json:
```json
{
    "actor": "https://github.com/sagetrac-GeorgSWeber",
    "created_at": "2009-02-15T20:51:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60633"
}
```



---

archive/issue_comments_032025.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nHi Minh,\n\ncould you please change line 148 (my own personal entry) to:\n\n```\n<contributor name=\"Georg S. Weber\" location=\"N\u00c3\u0152rnberg, Germany\" description=\"reviews, bug fixing, documentation\"/>\n```\nsince I would like to ask first at my institute before it is published here (more a formality than anything else, but nevertheless); I updated also the description.\n\nThanks for your work!!\n\nCheers, gsw",
    "created_at": "2009-02-15T20:51:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32025",
    "user": "https://github.com/sagetrac-GeorgSWeber"
}
```

<div id="comment:11" align="right">comment:11</div>

Hi Minh,

could you please change line 148 (my own personal entry) to:

```
<contributor name="Georg S. Weber" location="NÃŒrnberg, Germany" description="reviews, bug fixing, documentation"/>
```
since I would like to ask first at my institute before it is published here (more a formality than anything else, but nevertheless); I updated also the description.

Thanks for your work!!

Cheers, gsw



---

archive/issue_comments_032026.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\njust a short note, drop a message when this ready for upload to the website. so far the xml file looks good...",
    "created_at": "2009-02-15T20:54:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32026",
    "user": "https://github.com/haraldschilly"
}
```

<div id="comment:12" align="right">comment:12</div>

just a short note, drop a message when this ready for upload to the website. so far the xml file looks good...



---

archive/issue_events_060634.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-16T05:01:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60634"
}
```



---

archive/issue_events_060635.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-16T05:01:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60635"
}
```



---

archive/issue_comments_032027.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nI am cleaning up the 3.3 milestone. If a patch with positive review is put up to this ticket on time it might make it into 3.3.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-16T05:01:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32027",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:13" align="right">comment:13</div>

I am cleaning up the 3.3 milestone. If a patch with positive review is put up to this ticket on time it might make it into 3.3.

Cheers,

Michael



---

archive/issue_comments_032028.json:
```json
{
    "body": "update information for Georg S. Weber",
    "created_at": "2009-02-17T03:33:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32028",
    "user": "https://github.com/sagetrac-mvngu"
}
```

update information for Georg S. Weber



---

archive/issue_comments_032029.json:
```json
{
    "body": "Attachment: **[trac_5227-update-info-3.patch.gz](https://github.com/sagemath/sage/files/ticket5227/trac_5227-update-info-3.patch.gz)**\n\nAttachment: **[contributors.xml.gz](https://github.com/sagemath/sage/files/ticket5227/contributors.xml.gz)**\n\nthe updated XML file after applying the attached 5 patches",
    "created_at": "2009-02-17T03:34:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32029",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[trac_5227-update-info-3.patch.gz](https://github.com/sagemath/sage/files/ticket5227/trac_5227-update-info-3.patch.gz)**

Attachment: **[contributors.xml.gz](https://github.com/sagemath/sage/files/ticket5227/contributors.xml.gz)**

the updated XML file after applying the attached 5 patches



---

archive/issue_events_060636.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-02-17T03:41:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60636"
}
```



---

archive/issue_events_060637.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-02-17T03:41:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60637"
}
```



---

archive/issue_comments_032030.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nThe patch `trac_5227-update-info-3.patch` updates information for Georg S. Weber as he has requested. This is handy for folks who want to view Mercurial diffs. I've updated an updated `contributors.xml` file containing changes in the attached 5 patches. I'm sorry, folks, for taking so long to fix this issue.\n\n\n\n`@`Harald\n\n\nJust a friendly reminder that you should use `contributors.xml` for the devmap, instead of the Mercurial patches.",
    "created_at": "2009-02-17T03:41:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32030",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:14" align="right">comment:14</div>

The patch `trac_5227-update-info-3.patch` updates information for Georg S. Weber as he has requested. This is handy for folks who want to view Mercurial diffs. I've updated an updated `contributors.xml` file containing changes in the attached 5 patches. I'm sorry, folks, for taking so long to fix this issue.



`@`Harald


Just a friendly reminder that you should use `contributors.xml` for the devmap, instead of the Mercurial patches.



---

archive/issue_comments_032031.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nHi, i've uploaded the `contributors.xml` file. There were some issues:\n* the geocode api doesn't understand non-ascii characters for locations, that's easy \n* i had problems with the location of Yann Laigle-Chapuy ... it wasn't found, mabye new adress or some contradiction.",
    "created_at": "2009-02-17T13:28:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32031",
    "user": "https://github.com/haraldschilly"
}
```

<div id="comment:15" align="right">comment:15</div>

Hi, i've uploaded the `contributors.xml` file. There were some issues:
* the geocode api doesn't understand non-ascii characters for locations, that's easy 
* i had problems with the location of Yann Laigle-Chapuy ... it wasn't found, mabye new adress or some contradiction.



---

archive/issue_events_060638.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-17T13:32:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60638"
}
```



---

archive/issue_events_060639.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-17T13:32:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60639"
}
```



---

archive/issue_events_060640.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-17T13:32:14Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60640"
}
```



---

archive/issue_events_060641.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-17T13:32:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60641"
}
```



---

archive/issue_comments_032032.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">comment:16</div>\n\nOk, looks good to me. We can fix the remaining issues via a followup ticket.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-17T13:32:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32032",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:16" align="right">comment:16</div>

Ok, looks good to me. We can fix the remaining issues via a followup ticket.

Cheers,

Michael



---

archive/issue_events_060642.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-17T13:32:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60642"
}
```



---

archive/issue_events_060643.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-17T13:32:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5227#event-60643"
}
```



---

archive/issue_comments_032033.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">comment:17</div>\n\nAnd yes, i'm also for closing this ticket.\n\nThe `contributors.xml` that's now online is here:\nhttp://www.sagemath.org/res/contributors.xml\n\nUse this as a base for future updates, as usual,. and thanks for collecting all those details! :)",
    "created_at": "2009-02-17T13:34:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32033",
    "user": "https://github.com/haraldschilly"
}
```

<div id="comment:17" align="right">comment:17</div>

And yes, i'm also for closing this ticket.

The `contributors.xml` that's now online is here:
http://www.sagemath.org/res/contributors.xml

Use this as a base for future updates, as usual,. and thanks for collecting all those details! :)



---

archive/issue_comments_032034.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">comment:18</div>\n\nOk, I have updated the wiki page at \n\n  http://wiki.sagemath.org/DevMapNewPeople\n\nwith the info merged here today.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-17T13:38:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5227#issuecomment-32034",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:18" align="right">comment:18</div>

Ok, I have updated the wiki page at 

  http://wiki.sagemath.org/DevMapNewPeople

with the info merged here today.

Cheers,

Michael

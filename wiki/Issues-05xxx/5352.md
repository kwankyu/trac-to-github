# Issue 5352: the valgrind log files in sage-doctest are written to $HOME/.sage instead of $DOT_SAGE

archive/issues_005352.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nFix it so that the logs end up in DOT_SAGE since that does not have to be $HOME/.sage as hard coded for the log files.\n\nCheers,\n\nMichael\n\nCC:  @orlitzky\n\nComponent: **misc**\n\nAuthor: **Andr\u00e9 Apitzsch**\n\nReviewer: **Burcin Erocal, Michael Orlitzky**\n\nMerged: **sage-4.8.alpha4**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5352_\n\n",
    "closed_at": "2011-12-09T10:21:45Z",
    "created_at": "2009-02-23T22:37:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "the valgrind log files in sage-doctest are written to $HOME/.sage instead of $DOT_SAGE",
    "type": "issue",
    "updated_at": "2011-12-09T10:21:45Z",
    "url": "https://github.com/sagemath/sage/issues/5352",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
<div id="comment:0"></div>

Fix it so that the logs end up in DOT_SAGE since that does not have to be $HOME/.sage as hard coded for the log files.

Cheers,

Michael

CC:  @orlitzky

Component: **misc**

Author: **André Apitzsch**

Reviewer: **Burcin Erocal, Michael Orlitzky**

Merged: **sage-4.8.alpha4**

_Issue created by migration from https://trac.sagemath.org/ticket/5352_





---

archive/issue_events_062375.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-23T22:37:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62375"
}
```



---

archive/issue_events_062376.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-23T22:37:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62376"
}
```



---

archive/issue_events_062377.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-23T22:37:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62377"
}
```



---

archive/issue_events_062378.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2009-02-23T22:37:41Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62378"
}
```



---

archive/issue_events_062379.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2009-02-23T22:37:48Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62379"
}
```



---

archive/issue_events_062380.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-23T22:37:48Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62380"
}
```



---

archive/issue_events_062381.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-01T02:27:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62381"
}
```



---

archive/issue_events_062382.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-01T02:27:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62382"
}
```



---

archive/issue_comments_033011.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nBetter luck in 3.4.1.\n\nCheers,\n\nMichael",
    "created_at": "2009-03-01T02:27:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5352#issuecomment-33011",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Better luck in 3.4.1.

Cheers,

Michael



---

archive/issue_events_062383.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2011-01-05T17:03:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62383"
}
```



---

archive/issue_comments_033012.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nPatch tested successfully on 4.6.1, however (beginner question) what is the correct procedure\nfor reviewing such an out of sage-library/ issue ?",
    "created_at": "2011-02-08T10:33:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5352#issuecomment-33012",
    "user": "https://github.com/sagetrac-rochelol2"
}
```

<div id="comment:4" align="right">comment:4</div>

Patch tested successfully on 4.6.1, however (beginner question) what is the correct procedure
for reviewing such an out of sage-library/ issue ?



---

archive/issue_comments_033013.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nReplying to [@sagetrac-rochelol2](#comment%3A4):\n> Patch tested successfully on 4.6.1, however (beginner question) what is the correct procedure\n> for reviewing such an out of sage-library/ issue ?\n\nThere is a repository, referred to as the script repository, in `$SAGE_LOCAL/bin`. The patch should apply cleanly to this repo, have proper mercurial headers, etc.\n\nAttached patch does not fix all the problem places. The script `sage-valgrind` still refers to `$HOME/.sage`. Could you fix that and use \"hg export\" to create the patch?",
    "created_at": "2011-05-31T13:43:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5352#issuecomment-33013",
    "user": "https://github.com/burcin"
}
```

<div id="comment:5" align="right">comment:5</div>

Replying to [@sagetrac-rochelol2](#comment%3A4):
> Patch tested successfully on 4.6.1, however (beginner question) what is the correct procedure
> for reviewing such an out of sage-library/ issue ?

There is a repository, referred to as the script repository, in `$SAGE_LOCAL/bin`. The patch should apply cleanly to this repo, have proper mercurial headers, etc.

Attached patch does not fix all the problem places. The script `sage-valgrind` still refers to `$HOME/.sage`. Could you fix that and use "hg export" to create the patch?



---

archive/issue_events_062384.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2011-05-31T13:43:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62384"
}
```



---

archive/issue_events_062385.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2011-05-31T13:43:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62385"
}
```



---

archive/issue_comments_033014.json:
```json
{
    "body": "Reviewer: **Burcin Erocal**",
    "created_at": "2011-05-31T13:43:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5352#issuecomment-33014",
    "user": "https://github.com/burcin"
}
```

Reviewer: **Burcin Erocal**



---

archive/issue_comments_033015.json:
```json
{
    "body": "Attachment: **[trac_5352_valgrind_log.patch.gz](https://github.com/sagemath/sage/files/ticket5352/trac_5352_valgrind_log.patch.gz)**",
    "created_at": "2011-06-02T15:34:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5352#issuecomment-33015",
    "user": "https://github.com/a-andre"
}
```

Attachment: **[trac_5352_valgrind_log.patch.gz](https://github.com/sagemath/sage/files/ticket5352/trac_5352_valgrind_log.patch.gz)**



---

archive/issue_events_062386.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2011-06-02T15:36:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62386"
}
```



---

archive/issue_events_062387.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2011-06-02T15:36:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62387"
}
```



---

archive/issue_comments_033016.json:
```json
{
    "body": "Author: **Andr\u00e9 Apitzsch**",
    "created_at": "2011-06-02T15:36:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5352#issuecomment-33016",
    "user": "https://github.com/a-andre"
}
```

Author: **André Apitzsch**



---

archive/issue_comments_033017.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nI think all of the affected scripts need to source sage-env for the default value of `$DOT_SAGE`. As is,\n\n```\n$ ./local/bin/sage-valgrind \n/local/bin/sage-ipython\nmkdir: cannot create directory `/valgrind': Permission denied\n```",
    "created_at": "2011-12-04T03:25:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5352#issuecomment-33017",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:7" align="right">comment:7</div>

I think all of the affected scripts need to source sage-env for the default value of `$DOT_SAGE`. As is,

```
$ ./local/bin/sage-valgrind 
/local/bin/sage-ipython
mkdir: cannot create directory `/valgrind': Permission denied
```



---

archive/issue_events_062388.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2011-12-04T03:25:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62388"
}
```



---

archive/issue_events_062389.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2011-12-04T03:25:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62389"
}
```



---

archive/issue_events_062390.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2011-12-04T17:47:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62390"
}
```



---

archive/issue_events_062391.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2011-12-04T17:47:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62391"
}
```



---

archive/issue_comments_033018.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nReplying to [@orlitzky](#comment%3A7):\n> I think all of the affected scripts need to source sage-env for the default value of `$DOT_SAGE`.\n\nAfter RTFMing, I see that I shouldn't be running the script from bash anyway, so this criticism is invalid.",
    "created_at": "2011-12-04T17:47:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5352#issuecomment-33018",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:9" align="right">comment:9</div>

Replying to [@orlitzky](#comment%3A7):
> I think all of the affected scripts need to source sage-env for the default value of `$DOT_SAGE`.

After RTFMing, I see that I shouldn't be running the script from bash anyway, so this criticism is invalid.



---

archive/issue_comments_033019.json:
```json
{
    "body": "Changed reviewer from **Burcin Erocal** to **Burcin Erocal, Michael Orlitzky**",
    "created_at": "2011-12-05T04:58:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5352#issuecomment-33019",
    "user": "https://github.com/orlitzky"
}
```

Changed reviewer from **Burcin Erocal** to **Burcin Erocal, Michael Orlitzky**



---

archive/issue_comments_033020.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nI think this looks good. Here's how I tested:\n\n1. Install valgrind.\n2. Rebuild sage with SAGE_VALGRIND=\"yes\".\n3. Create an empty suppressions file (Trac #11918)\n4. Remove my ~/.sage/valgrind\n5. Create and export DOT_SAGE=~/grind\n6. Execute,\n   * sage -valgrind\n   * sage -cachegrind\n   * sage -callgrind\n   * sage -massif\n   * sage -tp 4 -long -valgrind devel/sage/sage (Killed this one prematurely since it was going to take a month)\n7. Tried to run sage -omega, but the exp-omega tool has been removed from recent versions of valgrind.\n8. Checked my ~/.sage and ~/grind directories to make sure all of the log files wound up in the right places.\n9. make ptestlong, no failures\n10. Grep for leftover '$HOME/.sage' instances in local/bin",
    "created_at": "2011-12-05T04:58:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5352#issuecomment-33020",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:10" align="right">comment:10</div>

I think this looks good. Here's how I tested:

1. Install valgrind.
2. Rebuild sage with SAGE_VALGRIND="yes".
3. Create an empty suppressions file (Trac #11918)
4. Remove my ~/.sage/valgrind
5. Create and export DOT_SAGE=~/grind
6. Execute,
   * sage -valgrind
   * sage -cachegrind
   * sage -callgrind
   * sage -massif
   * sage -tp 4 -long -valgrind devel/sage/sage (Killed this one prematurely since it was going to take a month)
7. Tried to run sage -omega, but the exp-omega tool has been removed from recent versions of valgrind.
8. Checked my ~/.sage and ~/grind directories to make sure all of the log files wound up in the right places.
9. make ptestlong, no failures
10. Grep for leftover '$HOME/.sage' instances in local/bin



---

archive/issue_events_062392.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2011-12-05T04:58:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62392"
}
```



---

archive/issue_events_062393.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2011-12-05T04:58:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62393"
}
```



---

archive/issue_events_062394.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-09T10:21:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62394"
}
```



---

archive/issue_events_062395.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-09T10:21:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5352#event-62395"
}
```



---

archive/issue_comments_033021.json:
```json
{
    "body": "Merged: **sage-4.8.alpha4**",
    "created_at": "2011-12-09T10:21:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5352#issuecomment-33021",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.8.alpha4**

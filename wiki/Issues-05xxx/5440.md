# Issue 5440: Failure to restart %magma in notebook.

archive/issues_005440.json:
```json
{
    "assignees": [
        "https://github.com/boothby"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nIn the notebook, if one does:\n\n  %magma \n  quit;\n\nthen\n\n  %magma\n  1+1\n\none gets an error (the terminated magma process \napparently does not get restarted).\n\nIt works fine in the shell.\n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5440_\n\n",
    "closed_at": "2020-03-29T02:12:30Z",
    "created_at": "2009-03-05T16:26:39Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Failure to restart %magma in notebook.",
    "type": "issue",
    "updated_at": "2020-03-29T02:12:30Z",
    "url": "https://github.com/sagemath/sage/issues/5440",
    "user": "https://github.com/sagetrac-kohel"
}
```
<div id="comment:0"></div>

In the notebook, if one does:

  %magma 
  quit;

then

  %magma
  1+1

one gets an error (the terminated magma process 
apparently does not get restarted).

It works fine in the shell.

Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/5440_





---

archive/issue_events_063558.json:
```json
{
    "actor": "https://github.com/sagetrac-kohel",
    "created_at": "2009-03-05T16:26:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5440",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5440#event-63558"
}
```



---

archive/issue_events_063559.json:
```json
{
    "actor": "https://github.com/sagetrac-kohel",
    "created_at": "2009-03-05T16:26:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5440",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5440#event-63559"
}
```



---

archive/issue_events_063560.json:
```json
{
    "actor": "https://github.com/sagetrac-kohel",
    "created_at": "2009-03-05T16:26:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5440",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5440#event-63560"
}
```



---

archive/issue_events_063561.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-03-05T16:26:39Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5440",
    "subject": "https://github.com/sagetrac-kohel",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5440#event-63561"
}
```



---

archive/issue_events_063562.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-15T16:27:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5440",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5440#event-63562"
}
```



---

archive/issue_events_063563.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5440",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5440#event-63563"
}
```



---

archive/issue_events_063564.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5440",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5440#event-63564"
}
```



---

archive/issue_events_063565.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5440",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5440#event-63565"
}
```



---

archive/issue_events_063566.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5440",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5440#event-63566"
}
```



---

archive/issue_events_063567.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5440",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5440#event-63567"
}
```



---

archive/issue_events_063568.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5440",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5440#event-63568"
}
```



---

archive/issue_events_063569.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5440",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5440#event-63569"
}
```



---

archive/issue_events_063570.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5440",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5440#event-63570"
}
```



---

archive/issue_events_063571.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2020-03-29T02:12:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5440",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5440#event-63571"
}
```



---

archive/issue_comments_033746.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nClosing deprecated notebook tickets",
    "created_at": "2020-03-29T02:12:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5440#issuecomment-33746",
    "user": "https://github.com/boothby"
}
```

<div id="comment:6" align="right">comment:6</div>

Closing deprecated notebook tickets

# Issue 5189: notebook -- now possible to delete all computation cells

archive/issues_005189.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis is a bug in counting the number of cells to make sure the number is >= 2. The counter should only count computation cells.\n\nDeleting all computation cells makes it impossible to create new ones.\n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5189_\n\n",
    "closed_at": "2009-02-06T21:53:40Z",
    "created_at": "2009-02-05T22:42:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "notebook -- now possible to delete all computation cells",
    "type": "issue",
    "updated_at": "2009-02-06T21:53:40Z",
    "url": "https://github.com/sagemath/sage/issues/5189",
    "user": "https://github.com/TimothyClemans"
}
```
<div id="comment:0"></div>

This is a bug in counting the number of cells to make sure the number is >= 2. The counter should only count computation cells.

Deleting all computation cells makes it impossible to create new ones.

Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/5189_





---

archive/issue_events_060131.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2009-02-05T22:42:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5189",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5189#event-60131"
}
```



---

archive/issue_events_060132.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2009-02-05T22:42:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5189",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5189#event-60132"
}
```



---

archive/issue_events_060133.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2009-02-05T22:42:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5189",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5189#event-60133"
}
```



---

archive/issue_events_060134.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2009-02-05T22:42:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5189",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5189#event-60134"
}
```



---

archive/issue_events_060135.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-02-05T22:42:23Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5189",
    "subject": "https://github.com/TimothyClemans",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5189#event-60135"
}
```



---

archive/issue_comments_031815.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThis is definitely a blocker.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-05T22:44:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5189#issuecomment-31815",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

This is definitely a blocker.

Cheers,

Michael



---

archive/issue_events_060136.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-05T22:44:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5189",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5189#event-60136"
}
```



---

archive/issue_events_060137.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-05T22:44:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5189",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5189#event-60137"
}
```



---

archive/issue_comments_031816.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAttachment: **[delete-all-cells.patch.gz](https://github.com/sagemath/sage/files/ticket5189/delete-all-cells.patch.gz)**",
    "created_at": "2009-02-06T08:18:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5189#issuecomment-31816",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:2" align="right">comment:2</div>

Attachment: **[delete-all-cells.patch.gz](https://github.com/sagemath/sage/files/ticket5189/delete-all-cells.patch.gz)**



---

archive/issue_events_060138.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-06T08:18:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5189",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5189#event-60138"
}
```



---

archive/issue_events_060139.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2009-02-06T08:22:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5189",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5189#event-60139"
}
```



---

archive/issue_events_060140.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2009-02-06T08:22:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5189",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5189#event-60140"
}
```



---

archive/issue_events_060141.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-06T08:24:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5189",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5189#event-60141"
}
```



---

archive/issue_events_060142.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-02-06T08:24:04Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/5189",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5189#event-60142"
}
```



---

archive/issue_events_060143.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-06T08:24:04Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5189",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5189#event-60143"
}
```



---

archive/issue_comments_031817.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nMerged in Sage 3.3.alpha6.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-06T21:53:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5189#issuecomment-31817",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Merged in Sage 3.3.alpha6.

Cheers,

Michael



---

archive/issue_events_060144.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-06T21:53:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5189",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5189#event-60144"
}
```

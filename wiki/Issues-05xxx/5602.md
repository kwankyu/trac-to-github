# Issue 5602: make .lighter() and .darker() methods for Sage Color objects

archive/issues_005602.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nSee the end of http://groups.google.com/group/sage-support/browse_thread/thread/44971aa416574675\n\nFor reference, here is what MMA does: \n\nhttp://reference.wolfram.com/mathematica/ref/Darker.html\n\nhttp://reference.wolfram.com/mathematica/ref/Lighter.html\n\nRelated tickets:\n\n* #5601 - predefine colors in Sage.\n* #5602 - make .lighter() and .darker() methods for Sage Color objects.\n* #5603 - make a .mix() method for Sage color objects.\n* #5604 - average Color objects when adding them together.\n* #5605 - Construct Color objects using hsl and hsv values.\n\n\nCC:  @sagetrac-mvngu\n\nComponent: **graphics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5602_\n\n",
    "closed_at": "2010-05-11T20:35:37Z",
    "created_at": "2009-03-24T21:28:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graphics",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "make .lighter() and .darker() methods for Sage Color objects",
    "type": "issue",
    "updated_at": "2010-05-11T20:35:37Z",
    "url": "https://github.com/sagemath/sage/issues/5602",
    "user": "https://github.com/jasongrout"
}
```
<div id="comment:0"></div>

See the end of http://groups.google.com/group/sage-support/browse_thread/thread/44971aa416574675

For reference, here is what MMA does: 

http://reference.wolfram.com/mathematica/ref/Darker.html

http://reference.wolfram.com/mathematica/ref/Lighter.html

Related tickets:

* #5601 - predefine colors in Sage.
* #5602 - make .lighter() and .darker() methods for Sage Color objects.
* #5603 - make a .mix() method for Sage color objects.
* #5604 - average Color objects when adding them together.
* #5605 - Construct Color objects using hsl and hsv values.


CC:  @sagetrac-mvngu

Component: **graphics**

_Issue created by migration from https://trac.sagemath.org/ticket/5602_





---

archive/issue_events_065866.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-03-24T21:28:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5602",
    "milestone_number": null,
    "milestone_title": "sage-4.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5602#event-65866"
}
```



---

archive/issue_events_065867.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-03-24T21:28:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5602",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "c: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5602#event-65867"
}
```



---

archive/issue_events_065868.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-03-24T21:28:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5602",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5602#event-65868"
}
```



---

archive/issue_events_065869.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-03-24T21:28:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5602",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5602#event-65869"
}
```



---

archive/issue_events_065870.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-03-24T21:28:09Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5602",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5602#event-65870"
}
```



---

archive/issue_events_065871.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-03-24T21:28:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5602",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5602#event-65871"
}
```



---

archive/issue_events_065872.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-03-24T21:28:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5602",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5602#event-65872"
}
```



---

archive/issue_comments_035341.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,3 +5,12 @@\n http://reference.wolfram.com/mathematica/ref/Darker.html\n \n http://reference.wolfram.com/mathematica/ref/Lighter.html\n+\n+Related tickets:\n+\n+* #5601 - predefine colors in Sage.\n+* #5602 - make .lighter() and .darker() methods for Sage Color objects.\n+* #5603 - make a .mix() method for Sage color objects.\n+* #5604 - average Color objects when adding them together.\n+* #5605 - Construct Color objects using hsl and hsv values.\n+\n``````\n",
    "created_at": "2009-11-14T18:42:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5602",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5602#issuecomment-35341",
    "user": "https://github.com/qed777"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,3 +5,12 @@
 http://reference.wolfram.com/mathematica/ref/Darker.html
 
 http://reference.wolfram.com/mathematica/ref/Lighter.html
+
+Related tickets:
+
+* #5601 - predefine colors in Sage.
+* #5602 - make .lighter() and .darker() methods for Sage Color objects.
+* #5603 - make a .mix() method for Sage color objects.
+* #5604 - average Color objects when adding them together.
+* #5605 - Construct Color objects using hsl and hsv values.
+
``````




---

archive/issue_comments_035342.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nSee the patch at #5601.",
    "created_at": "2009-11-18T04:42:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5602",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5602#issuecomment-35342",
    "user": "https://github.com/qed777"
}
```

<div id="comment:3" align="right">comment:3</div>

See the patch at #5601.



---

archive/issue_comments_035343.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThis works now:\n\n```\nsage: sage.plot.colors.red.lighter()\nRGB color (1.0, 0.33333333333333331, 0.33333333333333331)\nsage: sage.plot.colors.red.darker()\nRGB color (0.66666666666666674, 0.0, 0.0)\n```\n\nSo this ticket should be closed.",
    "created_at": "2010-05-11T20:31:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5602",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5602#issuecomment-35343",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:4" align="right">comment:4</div>

This works now:

```
sage: sage.plot.colors.red.lighter()
RGB color (1.0, 0.33333333333333331, 0.33333333333333331)
sage: sage.plot.colors.red.darker()
RGB color (0.66666666666666674, 0.0, 0.0)
```

So this ticket should be closed.



---

archive/issue_events_065873.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-05-11T20:35:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5602",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5602#event-65873"
}
```



---

archive/issue_comments_035344.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nClose as fixed:\n\n```sh\n[mvngu@sage ~]$ sage\n----------------------------------------------------------------------\n| Sage Version 4.4.1, Release Date: 2010-05-02                       |\n| Type notebook() for the GUI, and license() for information.        |\n----------------------------------------------------------------------\nsage: r = sage.plot.colors.red\nsage: r\nRGB color (1.0, 0.0, 0.0)\nsage: r.darker()\nRGB color (0.66666666666666674, 0.0, 0.0)\nsage: r.lighter()\nRGB color (1.0, 0.33333333333333331, 0.33333333333333331)\n```",
    "created_at": "2010-05-11T20:35:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5602",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5602#issuecomment-35344",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:5" align="right">comment:5</div>

Close as fixed:

```sh
[mvngu@sage ~]$ sage
----------------------------------------------------------------------
| Sage Version 4.4.1, Release Date: 2010-05-02                       |
| Type notebook() for the GUI, and license() for information.        |
----------------------------------------------------------------------
sage: r = sage.plot.colors.red
sage: r
RGB color (1.0, 0.0, 0.0)
sage: r.darker()
RGB color (0.66666666666666674, 0.0, 0.0)
sage: r.lighter()
RGB color (1.0, 0.33333333333333331, 0.33333333333333331)
```

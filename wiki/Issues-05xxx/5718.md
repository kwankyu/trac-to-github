# Issue 5718: notebook -- %hide works fine in the worksheet but shows up when printing

archive/issues_005718.json:
```json
{
    "assignees": [
        "https://github.com/boothby"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nI was able to easily reproduce the following bug in sage-3.4.\n\n```\nHello,\n\n%hide works fine in the worksheet but shows up when printing.  Any\nadvice is appreciated.\n\n - Sage3.4 VMWare Image\n - Dell Vostro 200 desktop\n\nThanks,\nlmc\n```\n\nComponent: **notebook**\n\nAuthor: **Mitesh Patel**\n\nReviewer: **Mike Hansen**\n\nMerged: **4.0.1.rc1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5718_\n\n",
    "closed_at": "2009-06-04T18:31:09Z",
    "created_at": "2009-04-08T19:40:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "notebook -- %hide works fine in the worksheet but shows up when printing",
    "type": "issue",
    "updated_at": "2009-06-06T21:55:01Z",
    "url": "https://github.com/sagemath/sage/issues/5718",
    "user": "https://github.com/williamstein"
}
```
<div id="comment:0"></div>

I was able to easily reproduce the following bug in sage-3.4.

```
Hello,

%hide works fine in the worksheet but shows up when printing.  Any
advice is appreciated.

 - Sage3.4 VMWare Image
 - Dell Vostro 200 desktop

Thanks,
lmc
```

Component: **notebook**

Author: **Mitesh Patel**

Reviewer: **Mike Hansen**

Merged: **4.0.1.rc1**

_Issue created by migration from https://trac.sagemath.org/ticket/5718_





---

archive/issue_events_067317.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-08T19:40:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5718#event-67317"
}
```



---

archive/issue_events_067318.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-08T19:40:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5718#event-67318"
}
```



---

archive/issue_events_067319.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-08T19:40:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5718#event-67319"
}
```



---

archive/issue_events_067320.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-04-08T19:40:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5718#event-67320"
}
```



---

archive/issue_events_067321.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-04-08T19:40:13Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5718#event-67321"
}
```



---

archive/issue_events_067322.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-09T19:34:07Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5718#event-67322"
}
```



---

archive/issue_events_067323.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-09T19:34:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5718#event-67323"
}
```



---

archive/issue_comments_036208.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nUnless there is a patch for this issue it will not be considered for Sage 3.4.1, hence bumped.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-09T19:34:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5718#issuecomment-36208",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

Unless there is a patch for this issue it will not be considered for Sage 3.4.1, hence bumped.

Cheers,

Michael



---

archive/issue_comments_036209.json:
```json
{
    "body": "Attachment: **[hide_print_5718.patch.gz](https://github.com/sagemath/sage/files/ticket5718/hide_print_5718.patch.gz)**",
    "created_at": "2009-04-14T09:15:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5718#issuecomment-36209",
    "user": "https://github.com/qed777"
}
```

Attachment: **[hide_print_5718.patch.gz](https://github.com/sagemath/sage/files/ticket5718/hide_print_5718.patch.gz)**



---

archive/issue_events_067324.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-04-14T09:30:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5718#event-67324"
}
```



---

archive/issue_events_067325.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-04T18:31:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5718#event-67325"
}
```



---

archive/issue_events_067326.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-04T18:31:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5718#event-67326"
}
```



---

archive/issue_comments_036210.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nLooks good to me.  Merged in 4.0.1.rc1.",
    "created_at": "2009-06-04T18:31:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5718#issuecomment-36210",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">comment:3</div>

Looks good to me.  Merged in 4.0.1.rc1.



---

archive/issue_events_067327.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-04T18:31:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5718#event-67327"
}
```



---

archive/issue_events_067328.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-04T18:31:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5718#event-67328"
}
```



---

archive/issue_comments_036211.json:
```json
{
    "body": "Author: **Mitesh Patel**",
    "created_at": "2009-06-06T21:55:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5718#issuecomment-36211",
    "user": "https://github.com/mwhansen"
}
```

Author: **Mitesh Patel**



---

archive/issue_comments_036212.json:
```json
{
    "body": "Reviewer: **Mike Hansen**",
    "created_at": "2009-06-06T21:55:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5718#issuecomment-36212",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **Mike Hansen**



---

archive/issue_comments_036213.json:
```json
{
    "body": "Merged: **4.0.1.rc1**",
    "created_at": "2009-06-06T21:55:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5718",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5718#issuecomment-36213",
    "user": "https://github.com/mwhansen"
}
```

Merged: **4.0.1.rc1**

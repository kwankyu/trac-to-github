# Issue 5092: Primes()?? gets hung in len call; also bring coverage to 100% for primes.py

archive/issues_005092.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\n\n\nComponent: **number theory**\n\nAuthor: **William Stein**\n\nReviewer: **Mike Hansen, Karl-Dieter Crisman**\n\nMerged: **sage-4.2.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5092_\n\n",
    "closed_at": "2009-10-21T04:00:22Z",
    "created_at": "2009-01-25T02:11:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Primes()?? gets hung in len call; also bring coverage to 100% for primes.py",
    "type": "issue",
    "updated_at": "2009-10-21T04:04:52Z",
    "url": "https://github.com/sagemath/sage/issues/5092",
    "user": "https://github.com/williamstein"
}
```
<div id="comment:0"></div>



Component: **number theory**

Author: **William Stein**

Reviewer: **Mike Hansen, Karl-Dieter Crisman**

Merged: **sage-4.2.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/5092_





---

archive/issue_events_058726.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-01-25T02:11:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58726"
}
```



---

archive/issue_events_058727.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-01-25T02:11:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "c: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58727"
}
```



---

archive/issue_events_058728.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-01-25T02:11:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58728"
}
```



---

archive/issue_events_058729.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-01-25T02:11:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58729"
}
```



---

archive/issue_events_058730.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-01-25T02:11:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58730"
}
```



---

archive/issue_events_058731.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-01-25T02:11:19Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58731"
}
```



---

archive/issue_events_058732.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-01-25T02:19:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58732"
}
```



---

archive/issue_events_058733.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-01-25T02:19:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58733"
}
```



---

archive/issue_events_058734.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-01-25T02:19:23Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "title_is": "Primes()?? gets hung in len call; also bring coverage to 100% for primes.py",
    "title_was": "Primes()?? gets hung in len call",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58734"
}
```



---

archive/issue_events_058735.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2009-01-25T02:29:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58735"
}
```



---

archive/issue_events_058736.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2009-01-25T02:29:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58736"
}
```



---

archive/issue_comments_030937.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nYou don't need the first two lines below in `__cmp__` anymore.\n\n```\n        if isinstance(right, Primes_class):\n            return 0\n        return cmp(type(self), type(right))\n```\n\nOtherwise, it's a positive review.",
    "created_at": "2009-01-25T02:29:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5092#issuecomment-30937",
    "user": "https://github.com/saliola"
}
```

<div id="comment:3" align="right">comment:3</div>

You don't need the first two lines below in `__cmp__` anymore.

```
        if isinstance(right, Primes_class):
            return 0
        return cmp(type(self), type(right))
```

Otherwise, it's a positive review.



---

archive/issue_events_058737.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2009-02-02T10:12:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58737"
}
```



---

archive/issue_events_058738.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2009-02-02T10:12:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58738"
}
```



---

archive/issue_comments_030938.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nThis patch causes one doctest failure:\n\n```\nsage -t -long \"devel/sage/sage/sets/set.py\"                 \n**********************************************************************\nFile \"/scratch/mabshoff/sage-3.3.alpha4/devel/sage/sage/sets/set.py\", line 278:\n    sage: Primes() < Set(QQ)\nExpected:\n    True\nGot:\n    False\n**********************************************************************\n```\n\nCheers,\n\nMichael",
    "created_at": "2009-02-02T18:19:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5092#issuecomment-30938",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

This patch causes one doctest failure:

```
sage -t -long "devel/sage/sage/sets/set.py"                 
**********************************************************************
File "/scratch/mabshoff/sage-3.3.alpha4/devel/sage/sage/sets/set.py", line 278:
    sage: Primes() < Set(QQ)
Expected:
    True
Got:
    False
**********************************************************************
```

Cheers,

Michael



---

archive/issue_events_058739.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-02T18:19:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58739"
}
```



---

archive/issue_events_058740.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-02T18:19:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58740"
}
```



---

archive/issue_comments_030939.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nReplying to [@sagetrac-mabshoff](#comment%3A5):\n> This patch causes one doctest failure:\n\nThis is a weird test. I'm not even sure that it says anything meaningful. In fact, according to the documentation of `__cmp__` for Set, it doesn't:\n\n```\n        \\note{If $X < Y$ is true this does \\emph{not} necessarily mean\n        that $X$ is a subset of $Y$.  Also, any two sets can be\n        compared, which is a general Python philosophy.}\n```",
    "created_at": "2009-02-02T21:45:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5092#issuecomment-30939",
    "user": "https://github.com/saliola"
}
```

<div id="comment:6" align="right">comment:6</div>

Replying to [@sagetrac-mabshoff](#comment%3A5):
> This patch causes one doctest failure:

This is a weird test. I'm not even sure that it says anything meaningful. In fact, according to the documentation of `__cmp__` for Set, it doesn't:

```
        \note{If $X < Y$ is true this does \emph{not} necessarily mean
        that $X$ is a subset of $Y$.  Also, any two sets can be
        compared, which is a general Python philosophy.}
```



---

archive/issue_comments_030940.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nSee #5933, which brings the coverage to 100% and was merged in 3.4.2.rc0.",
    "created_at": "2009-04-30T12:42:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5092#issuecomment-30940",
    "user": "https://github.com/aghitza"
}
```

<div id="comment:7" align="right">comment:7</div>

See #5933, which brings the coverage to 100% and was merged in 3.4.2.rc0.



---

archive/issue_comments_030941.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nWell, there is still a potential bug fix in here, so can someone take a look and extra a potential fix?\n\nCheers,\n\nMichael",
    "created_at": "2009-05-12T07:01:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5092#issuecomment-30941",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:8" align="right">comment:8</div>

Well, there is still a potential bug fix in here, so can someone take a look and extra a potential fix?

Cheers,

Michael



---

archive/issue_comments_030942.json:
```json
{
    "body": "Attachment: **[trac_5092.patch.gz](https://github.com/sagemath/sage/files/ticket5092/trac_5092.patch.gz)**",
    "created_at": "2009-10-19T19:25:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5092#issuecomment-30942",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[trac_5092.patch.gz](https://github.com/sagemath/sage/files/ticket5092/trac_5092.patch.gz)**



---

archive/issue_comments_030943.json:
```json
{
    "body": "Author: **William Stein**",
    "created_at": "2009-10-19T19:26:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5092#issuecomment-30943",
    "user": "https://github.com/mwhansen"
}
```

Author: **William Stein**



---

archive/issue_events_058741.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-19T19:26:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58741"
}
```



---

archive/issue_events_058742.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-19T19:26:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58742"
}
```



---

archive/issue_comments_030944.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nI've rebased the patch of #5933.",
    "created_at": "2009-10-19T19:26:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5092#issuecomment-30944",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:9" align="right">comment:9</div>

I've rebased the patch of #5933.



---

archive/issue_comments_030945.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nerr, on top of #5933",
    "created_at": "2009-10-19T19:46:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5092#issuecomment-30945",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:10" align="right">comment:10</div>

err, on top of #5933



---

archive/issue_events_058743.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-10-20T07:02:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58743"
}
```



---

archive/issue_events_058744.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-10-20T07:02:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58744"
}
```



---

archive/issue_comments_030946.json:
```json
{
    "body": "Reviewer: **Mike Hansen, Karl-Dieter Crisman**",
    "created_at": "2009-10-20T07:02:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5092#issuecomment-30946",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **Mike Hansen, Karl-Dieter Crisman**



---

archive/issue_comments_030947.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nPositive review. Unless you can provide an easily accessible example of where Primes(False) isn't Primes(True), but perhaps the first place that happens is waaay down the road...",
    "created_at": "2009-10-20T07:02:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5092#issuecomment-30947",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:11" align="right">comment:11</div>

Positive review. Unless you can provide an easily accessible example of where Primes(False) isn't Primes(True), but perhaps the first place that happens is waaay down the road...



---

archive/issue_comments_030948.json:
```json
{
    "body": "Merged: **sage-4.2.alpha1**",
    "created_at": "2009-10-21T04:00:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5092#issuecomment-30948",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.2.alpha1**



---

archive/issue_events_058745.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-21T04:00:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58745"
}
```



---

archive/issue_events_058746.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-21T04:00:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5092",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5092#event-58746"
}
```

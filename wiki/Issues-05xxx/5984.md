# Issue 5984: cmp related doctest failure in sage/modular/arithgroup/arithgroup_perm.py

archive/issues_005984.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "<div id=\"comment:0\"></div>\n\n\n```\nsage -t -long devel/sage/sage/modular/arithgroup/arithgroup_perm.py\n**********************************************************************\nFile \"/home/mabshoff/build-3.4.2/sage-3.4.2-eno-gcc-4.3.3/devel/sage-main/sage/modular/arithgroup/arithgroup_perm.py\", line 204:\n    sage: cmp(G, Gamma0(8))\nExpected:\n    1\nGot:\n    -1\n**********************************************************************\n```\n\nComponent: **doctest coverage**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5984_\n\n",
    "closed_at": "2009-05-05T05:38:51Z",
    "created_at": "2009-05-05T04:33:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "cmp related doctest failure in sage/modular/arithgroup/arithgroup_perm.py",
    "type": "issue",
    "updated_at": "2009-05-05T05:38:51Z",
    "url": "https://github.com/sagemath/sage/issues/5984",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
<div id="comment:0"></div>


```
sage -t -long devel/sage/sage/modular/arithgroup/arithgroup_perm.py
**********************************************************************
File "/home/mabshoff/build-3.4.2/sage-3.4.2-eno-gcc-4.3.3/devel/sage-main/sage/modular/arithgroup/arithgroup_perm.py", line 204:
    sage: cmp(G, Gamma0(8))
Expected:
    1
Got:
    -1
**********************************************************************
```

Component: **doctest coverage**

_Issue created by migration from https://trac.sagemath.org/ticket/5984_





---

archive/issue_events_071100.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-05T04:33:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5984",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5984#event-71100"
}
```



---

archive/issue_events_071101.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-05T04:33:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5984",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5984#event-71101"
}
```



---

archive/issue_events_071102.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-05T04:33:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5984",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5984#event-71102"
}
```



---

archive/issue_events_071103.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-05T04:33:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5984",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5984#event-71103"
}
```



---

archive/issue_events_071104.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-05T04:33:53Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5984",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5984#event-71104"
}
```



---

archive/issue_events_071105.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-05T04:34:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5984",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5984#event-71105"
}
```



---

archive/issue_events_071106.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-05T04:34:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/5984",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5984#event-71106"
}
```



---

archive/issue_events_071107.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-05T04:34:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5984",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5984#event-71107"
}
```



---

archive/issue_comments_038967.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nPatch coming up.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-05T04:34:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5984",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5984#issuecomment-38967",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

Patch coming up.

Cheers,

Michael



---

archive/issue_events_071108.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-05T04:34:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5984",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5984#event-71108"
}
```



---

archive/issue_comments_038968.json:
```json
{
    "body": "Attachment: **[trac_5984.patch.gz](https://github.com/sagemath/sage/files/ticket5984/trac_5984.patch.gz)**",
    "created_at": "2009-05-05T05:27:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5984",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5984#issuecomment-38968",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Attachment: **[trac_5984.patch.gz](https://github.com/sagemath/sage/files/ticket5984/trac_5984.patch.gz)**



---

archive/issue_events_071109.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-05T05:33:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5984",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5984#event-71109"
}
```



---

archive/issue_comments_038969.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.4.2.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-05T05:38:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5984",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5984#issuecomment-38969",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.4.2.

Cheers,

Michael



---

archive/issue_events_071110.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-05T05:38:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5984",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5984#event-71110"
}
```



---

archive/issue_events_071111.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-05T05:38:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5984",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5984#event-71111"
}
```

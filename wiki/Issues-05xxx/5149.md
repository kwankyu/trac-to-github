# Issue 5149: Cremona database -- fix a bug in handling of 990h

archive/issues_005149.json:
```json
{
    "body": "John mentioned that 990h3 is a special case in ticket #5138 -- I looked at the Cremona database code and found that there was one function where this special case isn't treated correctly.  I fixed that and added some doctests.  \n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/5149\n\n",
    "closed_at": "2009-02-02T02:56:56Z",
    "created_at": "2009-02-01T08:34:22Z",
    "labels": [
        "component: number theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "title": "Cremona database -- fix a bug in handling of 990h",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/5149",
    "user": "https://github.com/williamstein"
}
```
John mentioned that 990h3 is a special case in ticket #5138 -- I looked at the Cremona database code and found that there was one function where this special case isn't treated correctly.  I fixed that and added some doctests.  

**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/5149





---

archive/attachments_006004.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_5149.patch",
    "asset_url": "tarball://root/attachments/ticket5149/trac_5149.patch",
    "created_at": "2009-02-01T10:59:51Z",
    "issue": "https://github.com/sagemath/sage/issues/5149",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket5149/trac_5149.patch",
    "user": "https://github.com/JohnCremona"
}
```



---

archive/issue_comments_032661.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_5149.patch](https://github.com/sagemath/sage/files/ticket5149/trac_5149.patch)\n\nPatch applies cleanly to 3.33.alpha2 and all tests pass.  Code looks good, so pass!",
    "created_at": "2009-02-01T10:59:51Z",
    "issue": "https://github.com/sagemath/sage/issues/5149",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5149#issuecomment-32661",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_5149.patch](https://github.com/sagemath/sage/files/ticket5149/trac_5149.patch)

Patch applies cleanly to 3.33.alpha2 and all tests pass.  Code looks good, so pass!



---

archive/issue_events_031923.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-02-01T11:20:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5149",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5149#event-31923"
}
```



---

archive/issue_events_031924.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-02T02:56:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5149",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5149#event-31924"
}
```



---

archive/issue_events_031925.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-02T02:56:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5149",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5149#event-31925"
}
```



---

archive/issue_comments_032662.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.3.alpha4.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-02T02:56:56Z",
    "issue": "https://github.com/sagemath/sage/issues/5149",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5149#issuecomment-32662",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.3.alpha4.

Cheers,

Michael

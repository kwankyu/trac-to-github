# Issue 5407: fractional gens?  not good -- (QQ^3).gen(4/3) gives (0,1,0)

archive/issues_005407.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis patch fixes the bug. \n\nComponent: **linear algebra**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/5407_\n\n",
    "closed_at": "2009-03-02T04:31:50Z",
    "created_at": "2009-03-01T05:59:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fractional gens?  not good -- (QQ^3).gen(4/3) gives (0,1,0)",
    "type": "issue",
    "updated_at": "2009-03-02T04:31:50Z",
    "url": "https://github.com/sagemath/sage/issues/5407",
    "user": "https://github.com/williamstein"
}
```
<div id="comment:0"></div>

This patch fixes the bug. 

Component: **linear algebra**

_Issue created by migration from https://trac.sagemath.org/ticket/5407_





---

archive/issue_events_063087.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-03-01T05:59:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5407#event-63087"
}
```



---

archive/issue_events_063088.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-03-01T05:59:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "c: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5407#event-63088"
}
```



---

archive/issue_events_063089.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-03-01T05:59:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5407#event-63089"
}
```



---

archive/issue_events_063090.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-03-01T05:59:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5407#event-63090"
}
```



---

archive/issue_events_063091.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-03-01T05:59:34Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5407#event-63091"
}
```



---

archive/issue_events_063092.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-03-01T06:02:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5407#event-63092"
}
```



---

archive/issue_events_063093.json:
```json
{
    "actor": "https://github.com/tornaria",
    "created_at": "2009-03-01T06:06:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5407#event-63093"
}
```



---

archive/issue_events_063094.json:
```json
{
    "actor": "https://github.com/tornaria",
    "created_at": "2009-03-01T06:06:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5407#event-63094"
}
```



---

archive/issue_comments_033482.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nNice try wstein :)\n\nCheers,\n\nMichael",
    "created_at": "2009-03-01T06:11:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5407#issuecomment-33482",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Nice try wstein :)

Cheers,

Michael



---

archive/issue_events_063095.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-01T06:20:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5407#event-63095"
}
```



---

archive/issue_events_063096.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-01T06:20:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5407#event-63096"
}
```



---

archive/issue_comments_033483.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nOne doctest failure:\n\n```\nmabshoff@sage:/scratch/mabshoff/sage-3.4.alpha1$ ./sage -t -long devel/sage/sage/modules/free_module.py\nsage -t -long \"devel/sage/sage/modules/free_module.py\"      \n**********************************************************************\nFile \"/scratch/mabshoff/sage-3.4.alpha1/devel/sage/sage/modules/free_module.py\", line 1438:\n    sage: (QQ^3).gen(4/3)\nException raised:\n    Traceback (most recent call last):\n      File \"/scratch/mabshoff/sage-3.4.alpha1/local/bin/ncadoctest.py\", line 1231, in run_one_test\n        self.run_one_example(test, example, filename, compileflags)\n      File \"/scratch/mabshoff/sage-3.4.alpha1/local/bin/sagedoctest.py\", line 38, in run_one_example\n        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)\n      File \"/scratch/mabshoff/sage-3.4.alpha1/local/bin/ncadoctest.py\", line 1172, in run_one_example\n        compileflags, 1) in test.globs\n      File \"<doctest __main__.example_37[7]>\", line 1, in <module>\n        (QQ**Integer(3)).gen(Integer(4)/Integer(3))###line 1438:\n    sage: (QQ^3).gen(4/3)\n      File \"/scratch/mabshoff/sage-3.4.alpha1/local/lib/python2.5/site-packages/sage/modules/free_module.py\", line 1445, in gen\n        return self.basis()[i]\n      File \"rational.pyx\", line 236, in sage.rings.rational.Rational.__index__ (sage/rings/rational.c:4371)\n    TypeError: rational is not an integer\n**********************************************************************\n```\n\nCheers,\n\nMichael",
    "created_at": "2009-03-01T06:20:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5407#issuecomment-33483",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

One doctest failure:

```
mabshoff@sage:/scratch/mabshoff/sage-3.4.alpha1$ ./sage -t -long devel/sage/sage/modules/free_module.py
sage -t -long "devel/sage/sage/modules/free_module.py"      
**********************************************************************
File "/scratch/mabshoff/sage-3.4.alpha1/devel/sage/sage/modules/free_module.py", line 1438:
    sage: (QQ^3).gen(4/3)
Exception raised:
    Traceback (most recent call last):
      File "/scratch/mabshoff/sage-3.4.alpha1/local/bin/ncadoctest.py", line 1231, in run_one_test
        self.run_one_example(test, example, filename, compileflags)
      File "/scratch/mabshoff/sage-3.4.alpha1/local/bin/sagedoctest.py", line 38, in run_one_example
        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)
      File "/scratch/mabshoff/sage-3.4.alpha1/local/bin/ncadoctest.py", line 1172, in run_one_example
        compileflags, 1) in test.globs
      File "<doctest __main__.example_37[7]>", line 1, in <module>
        (QQ**Integer(3)).gen(Integer(4)/Integer(3))###line 1438:
    sage: (QQ^3).gen(4/3)
      File "/scratch/mabshoff/sage-3.4.alpha1/local/lib/python2.5/site-packages/sage/modules/free_module.py", line 1445, in gen
        return self.basis()[i]
      File "rational.pyx", line 236, in sage.rings.rational.Rational.__index__ (sage/rings/rational.c:4371)
    TypeError: rational is not an integer
**********************************************************************
```

Cheers,

Michael



---

archive/issue_comments_033484.json:
```json
{
    "body": "This is an updated Version of Wiliam's patch",
    "created_at": "2009-03-02T04:30:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5407#issuecomment-33484",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

This is an updated Version of Wiliam's patch



---

archive/issue_comments_033485.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nAttachment: **[trac_5407.patch.gz](https://github.com/sagemath/sage/files/ticket5407/trac_5407.patch.gz)**\n\nOk, the problem has been fixed.\n\nCheers,\n\nMichael",
    "created_at": "2009-03-02T04:31:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5407#issuecomment-33485",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Attachment: **[trac_5407.patch.gz](https://github.com/sagemath/sage/files/ticket5407/trac_5407.patch.gz)**

Ok, the problem has been fixed.

Cheers,

Michael



---

archive/issue_events_063097.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-02T04:31:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5407#event-63097"
}
```



---

archive/issue_events_063098.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-02T04:31:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5407#event-63098"
}
```



---

archive/issue_events_063099.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-02T04:31:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5407#event-63099"
}
```



---

archive/issue_events_063100.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-03-02T04:31:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/5407#event-63100"
}
```



---

archive/issue_comments_033486.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nMerged in Sage 3.4.rc0.\n\nCheers,\n\nMichael",
    "created_at": "2009-03-02T04:31:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/5407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/5407#issuecomment-33486",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Merged in Sage 3.4.rc0.

Cheers,

Michael

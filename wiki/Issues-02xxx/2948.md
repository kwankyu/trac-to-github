# Issue 2948: (Additional) Docstrings and Doctests for rings/quotient_ring.py

archive/issues_002948.json:
```json
{
    "assignees": [
        "https://github.com/cswiercz"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nProvide missing docstrings and doctests for rings/quotient_ring.py. Also provide additional doctests for several functions i.e. imporve the quality of the doctests in this file.\n\n* SCORE quotient_ring.py: 54% (12 of 22)\n\nComponent: **doctest coverage**\n\nKeywords: **quotient_ring.py**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2948_\n\n",
    "closed_at": "2008-09-27T00:53:34Z",
    "created_at": "2008-04-17T22:36:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/c%3A%20documentation"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "(Additional) Docstrings and Doctests for rings/quotient_ring.py",
    "type": "issue",
    "updated_at": "2008-09-27T00:53:34Z",
    "url": "https://github.com/sagemath/sage/issues/2948",
    "user": "https://github.com/cswiercz"
}
```
<div id="comment:0"></div>

Provide missing docstrings and doctests for rings/quotient_ring.py. Also provide additional doctests for several functions i.e. imporve the quality of the doctests in this file.

* SCORE quotient_ring.py: 54% (12 of 22)

Component: **doctest coverage**

Keywords: **quotient_ring.py**

_Issue created by migration from https://trac.sagemath.org/ticket/2948_





---

archive/issue_events_031127.json:
```json
{
    "actor": "https://github.com/cswiercz",
    "created_at": "2008-04-17T22:36:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2948",
    "milestone_number": null,
    "milestone_title": "sage-3.1.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2948#event-31127"
}
```



---

archive/issue_events_031128.json:
```json
{
    "actor": "https://github.com/cswiercz",
    "created_at": "2008-04-17T22:36:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2948",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2948#event-31128"
}
```



---

archive/issue_events_031129.json:
```json
{
    "actor": "https://github.com/cswiercz",
    "created_at": "2008-04-17T22:36:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2948",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2948#event-31129"
}
```



---

archive/issue_events_031130.json:
```json
{
    "actor": "https://github.com/cswiercz",
    "created_at": "2008-04-17T22:36:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2948",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2948#event-31130"
}
```



---

archive/issue_events_031131.json:
```json
{
    "actor": "https://github.com/cswiercz",
    "created_at": "2008-04-17T22:36:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2948",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2948#event-31131"
}
```



---

archive/issue_events_031132.json:
```json
{
    "actor": "https://github.com/cswiercz",
    "created_at": "2008-04-17T22:36:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2948",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2948#event-31132"
}
```



---

archive/issue_events_031133.json:
```json
{
    "actor": "https://github.com/cswiercz",
    "created_at": "2008-04-17T22:36:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2948",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2948#event-31133"
}
```



---

archive/issue_events_031134.json:
```json
{
    "actor": "https://github.com/cswiercz",
    "created_at": "2008-04-17T22:36:52Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2948",
    "subject": "https://github.com/cswiercz",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2948#event-31134"
}
```



---

archive/issue_comments_015433.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nChris,\n\nIs this on top of #2507? If so you should clearly indicate that.\n\nCheers,\n\nMichael",
    "created_at": "2008-04-17T23:42:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2948",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2948#issuecomment-15433",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

Chris,

Is this on top of #2507? If so you should clearly indicate that.

Cheers,

Michael



---

archive/issue_events_031135.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2008-09-26T19:09:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2948",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2948#event-31135"
}
```



---

archive/issue_comments_015434.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAttachment: **[2948.patch.gz](https://github.com/sagemath/sage/files/ticket2948/2948.patch.gz)**\n\nI'm providing a patch. The doctesting that I'm providing is pretty superficial -- it doesn't show any of the limitations of the code, for example. If someone who knows the stuff better wants to add more, please go ahead.",
    "created_at": "2008-09-26T19:09:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2948",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2948#issuecomment-15434",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:2" align="right">comment:2</div>

Attachment: **[2948.patch.gz](https://github.com/sagemath/sage/files/ticket2948/2948.patch.gz)**

I'm providing a patch. The doctesting that I'm providing is pretty superficial -- it doesn't show any of the limitations of the code, for example. If someone who knows the stuff better wants to add more, please go ahead.



---

archive/issue_events_031136.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-09-26T22:30:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2948",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2948#event-31136"
}
```



---

archive/issue_events_031137.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-09-26T22:30:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2948",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2948#event-31137"
}
```



---

archive/issue_comments_015435.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in Sage 3.1.3.alpha2",
    "created_at": "2008-09-27T00:53:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2948",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2948#issuecomment-15435",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in Sage 3.1.3.alpha2



---

archive/issue_events_031138.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-27T00:53:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2948",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2948#event-31138"
}
```



---

archive/issue_events_031139.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-27T00:53:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2948",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2948#event-31139"
}
```

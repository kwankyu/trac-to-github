# Issue 2881: numerical_integral doesn't use fast_float

archive/issues_002881.json:
```json
{
    "body": "This makes things dreadfully slow.\n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/2881\n\n",
    "closed_at": "2008-08-31T18:45:20Z",
    "created_at": "2008-04-11T22:40:23Z",
    "labels": [
        "component: calculus",
        "bug",
        "invalid"
    ],
    "title": "numerical_integral doesn't use fast_float",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2881",
    "user": "https://github.com/mwhansen"
}
```
This makes things dreadfully slow.

**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/2881





---

archive/issue_events_015121.json:
```json
{
    "actor": "https://github.com/jicama",
    "created_at": "2008-08-31T14:41:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2881",
    "label": "component: calculus",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2881#event-15121"
}
```



---

archive/issue_events_015122.json:
```json
{
    "actor": "https://github.com/jicama",
    "created_at": "2008-08-31T14:41:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2881",
    "label": "component: numerical",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2881#event-15122"
}
```



---

archive/issue_comments_015790.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis is the same issue as #3622, which now has a patch.",
    "created_at": "2008-08-31T14:52:40Z",
    "issue": "https://github.com/sagemath/sage/issues/2881",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2881#issuecomment-15790",
    "user": "https://github.com/jicama"
}
```

<a id='comment:2'></a>
This is the same issue as #3622, which now has a patch.



---

archive/issue_events_015123.json:
```json
{
    "actor": "https://github.com/jicama",
    "created_at": "2008-08-31T15:09:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2881",
    "label": "component: numerical",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2881#event-15123"
}
```



---

archive/issue_events_015124.json:
```json
{
    "actor": "https://github.com/jicama",
    "created_at": "2008-08-31T15:09:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2881",
    "label": "component: calculus",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2881#event-15124"
}
```



---

archive/issue_comments_015791.json:
```json
{
    "body": "<a id='comment:4'></a>\nThis is actually considered \"calculus\" and not \"numerical\". Closed as a duplicate of #3622.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-31T18:45:20Z",
    "issue": "https://github.com/sagemath/sage/issues/2881",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2881#issuecomment-15791",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
This is actually considered "calculus" and not "numerical". Closed as a duplicate of #3622.

Cheers,

Michael



---

archive/issue_events_015125.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-31T18:45:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2881",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2881#event-15125"
}
```



---

archive/issue_events_015126.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-31T18:45:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2881",
    "milestone": "sage-3.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2881#event-15126"
}
```

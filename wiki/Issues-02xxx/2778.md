# Issue 2778: SymbolicFormula class

archive/issues_002778.json:
```json
{
    "body": "Create a class that lets us combine SymbolicEquations using logical operators (and, or, not, xor, etc.)\n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/2778\n\n",
    "closed_at": "2009-04-16T11:25:46Z",
    "created_at": "2008-04-02T18:48:30Z",
    "labels": [
        "component: calculus",
        "enhancement",
        "duplicate"
    ],
    "title": "SymbolicFormula class",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2778",
    "user": "https://github.com/jasongrout"
}
```
Create a class that lets us combine SymbolicEquations using logical operators (and, or, not, xor, etc.)

**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/2778





---

archive/issue_events_014504.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-04-16T11:25:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2778",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2778#event-14504"
}
```



---

archive/issue_events_014505.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-04-16T11:25:46Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2778",
    "milestone": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2778#event-14505"
}
```



---

archive/issue_comments_015186.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis is a duplicate of #2744.\n\n```\n<burcin> #2778 seems to be a duplicate of #2744\n<mabs> ok\n<mabs> I agree. Close #2778 as a dupe, move the other one to the wishlist milestone.\n<mabs> Unles you want to implement #2744 obviously :)\n```",
    "created_at": "2009-04-16T11:25:46Z",
    "issue": "https://github.com/sagemath/sage/issues/2778",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2778#issuecomment-15186",
    "user": "https://github.com/burcin"
}
```

<a id='comment:1'></a>
This is a duplicate of #2744.

```
<burcin> #2778 seems to be a duplicate of #2744
<mabs> ok
<mabs> I agree. Close #2778 as a dupe, move the other one to the wishlist milestone.
<mabs> Unles you want to implement #2744 obviously :)
```



---

archive/issue_events_014506.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-04-16T11:25:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2778",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2778#event-14506"
}
```

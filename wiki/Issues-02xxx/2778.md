# Issue 2778: SymbolicFormula class

archive/issues_002778.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nCreate a class that lets us combine SymbolicEquations using logical operators (and, or, not, xor, etc.)\n\nComponent: **calculus**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2778_\n\n",
    "closed_at": "2009-04-16T11:25:46Z",
    "created_at": "2008-04-02T18:48:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20calculus",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "SymbolicFormula class",
    "type": "issue",
    "updated_at": "2009-04-16T11:25:46Z",
    "url": "https://github.com/sagemath/sage/issues/2778",
    "user": "https://github.com/jasongrout"
}
```
<div id="comment:0"></div>

Create a class that lets us combine SymbolicEquations using logical operators (and, or, not, xor, etc.)

Component: **calculus**

_Issue created by migration from https://trac.sagemath.org/ticket/2778_





---

archive/issue_events_029154.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-02T18:48:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2778",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2778#event-29154"
}
```



---

archive/issue_events_029155.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-02T18:48:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2778",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "c: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2778#event-29155"
}
```



---

archive/issue_events_029156.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-02T18:48:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2778",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2778#event-29156"
}
```



---

archive/issue_events_029157.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-02T18:48:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2778",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2778#event-29157"
}
```



---

archive/issue_events_029158.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-04-02T18:48:30Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2778",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2778#event-29158"
}
```



---

archive/issue_events_029159.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-04-16T11:25:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2778",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2778#event-29159"
}
```



---

archive/issue_events_029160.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-04-16T11:25:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2778",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "c6c6c6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2778#event-29160"
}
```



---

archive/issue_events_029161.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-04-16T11:25:46Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2778",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2778#event-29161"
}
```



---

archive/issue_comments_014442.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThis is a duplicate of #2744.\n\n```\n<burcin> #2778 seems to be a duplicate of #2744\n<mabs> ok\n<mabs> I agree. Close #2778 as a dupe, move the other one to the wishlist milestone.\n<mabs> Unles you want to implement #2744 obviously :)\n```",
    "created_at": "2009-04-16T11:25:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2778",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2778#issuecomment-14442",
    "user": "https://github.com/burcin"
}
```

<div id="comment:1" align="right">comment:1</div>

This is a duplicate of #2744.

```
<burcin> #2778 seems to be a duplicate of #2744
<mabs> ok
<mabs> I agree. Close #2778 as a dupe, move the other one to the wishlist milestone.
<mabs> Unles you want to implement #2744 obviously :)
```



---

archive/issue_events_029162.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-04-16T11:25:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2778",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2778#event-29162"
}
```

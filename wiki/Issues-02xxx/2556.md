# Issue 2556: dsage unit tests fail on linux

archive/issues_002556.json:
```json
{
    "body": "Various users have reported that the dsage unit tests fail on linux. This is a known issue and a fix is being worked on by me. \n\n**Assignee:** @yqiang\n\nIssue created by migration from https://trac.sagemath.org/ticket/2556\n\n",
    "closed_at": "2008-03-16T21:53:39Z",
    "created_at": "2008-03-16T21:41:31Z",
    "labels": [
        "component: dsage",
        "bug",
        "duplicate"
    ],
    "title": "dsage unit tests fail on linux",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2556",
    "user": "https://github.com/yqiang"
}
```
Various users have reported that the dsage unit tests fail on linux. This is a known issue and a fix is being worked on by me. 

**Assignee:** @yqiang

Issue created by migration from https://trac.sagemath.org/ticket/2556





---

archive/issue_events_013164.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-16T21:53:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2556",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2556#event-13164"
}
```



---

archive/issue_events_013165.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-16T21:53:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2556",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2556#event-13165"
}
```



---

archive/issue_comments_013815.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis is a dupe of #2553",
    "created_at": "2008-03-16T21:53:39Z",
    "issue": "https://github.com/sagemath/sage/issues/2556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2556#issuecomment-13815",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
This is a dupe of #2553



---

archive/issue_events_013166.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-16T21:53:39Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2556",
    "milestone": "sage-2.10.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2556#event-13166"
}
```

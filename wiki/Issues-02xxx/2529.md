# Issue 2529: slightly more preparation for backend switch in graphs

archive/issues_002529.json:
```json
{
    "body": "This is of a similar vein to #2433, but much smaller. I've rewritten a few more functions to not depend on NX since they definitely don't need to.\n\n**Assignee:** @rlmill\n\n**CC:**  @mwhansen\n\nIssue created by migration from https://trac.sagemath.org/ticket/2529\n\n",
    "closed_at": "2008-03-15T21:54:23Z",
    "created_at": "2008-03-15T06:16:12Z",
    "labels": [
        "component: graph theory",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.4",
    "title": "slightly more preparation for backend switch in graphs",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2529",
    "user": "https://github.com/rlmill"
}
```
This is of a similar vein to #2433, but much smaller. I've rewritten a few more functions to not depend on NX since they definitely don't need to.

**Assignee:** @rlmill

**CC:**  @mwhansen

Issue created by migration from https://trac.sagemath.org/ticket/2529





---

archive/attachments_002378.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "2529-reduce-nx-dependence.patch",
    "asset_url": "tarball://root/attachments/ticket2529/2529-reduce-nx-dependence.patch",
    "created_at": "2008-03-15T21:04:36Z",
    "issue": "https://github.com/sagemath/sage/issues/2529",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket2529/2529-reduce-nx-dependence.patch",
    "user": "https://github.com/mwhansen"
}
```



---

archive/issue_comments_013656.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [2529-reduce-nx-dependence.patch](https://github.com/sagemath/sage/files/ticket2529/2529-reduce-nx-dependence.patch)\n\nApplies to 2.10.4.alpha0 and passes tests for me.",
    "created_at": "2008-03-15T21:04:36Z",
    "issue": "https://github.com/sagemath/sage/issues/2529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2529#issuecomment-13656",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
**Attachment:** [2529-reduce-nx-dependence.patch](https://github.com/sagemath/sage/files/ticket2529/2529-reduce-nx-dependence.patch)

Applies to 2.10.4.alpha0 and passes tests for me.



---

archive/issue_events_012997.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-03-15T21:04:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2529",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2529#event-12997"
}
```



---

archive/issue_events_012998.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-15T21:54:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2529",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2529#event-12998"
}
```



---

archive/issue_events_012999.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-15T21:54:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2529",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2529#event-12999"
}
```



---

archive/issue_comments_013657.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 2.10.4.rc0",
    "created_at": "2008-03-15T21:54:23Z",
    "issue": "https://github.com/sagemath/sage/issues/2529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2529#issuecomment-13657",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 2.10.4.rc0

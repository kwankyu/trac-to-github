# Issue 2661: sage cannot compute floor / ceil of log(8)/log(2)

archive/issues_002661.json:
```json
{
    "body": "This happens because the interval remains centered around an integer.\n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/2661\n\n",
    "closed_at": "2008-03-25T04:48:35Z",
    "created_at": "2008-03-24T20:13:01Z",
    "labels": [
        "component: calculus",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.11",
    "title": "sage cannot compute floor / ceil of log(8)/log(2)",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2661",
    "user": "https://github.com/mwhansen"
}
```
This happens because the interval remains centered around an integer.

**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/2661





---

archive/issue_events_013898.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-03-24T20:14:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2661",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2661#event-13898"
}
```



---

archive/issue_comments_014548.json:
```json
{
    "body": "<a id='comment:2'></a>\nSome concerns about this were raised on IRC.  It seems that the real problem is telling if log(8)/log(2) is an integer.  The (log(8)/log(2)).simplify_log() function tells us that this is 3.",
    "created_at": "2008-03-24T21:12:16Z",
    "issue": "https://github.com/sagemath/sage/issues/2661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2661#issuecomment-14548",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:2'></a>
Some concerns about this were raised on IRC.  It seems that the real problem is telling if log(8)/log(2) is an integer.  The (log(8)/log(2)).simplify_log() function tells us that this is 3.



---

archive/issue_events_013899.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-03-24T21:26:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2661",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2661#event-13899"
}
```



---

archive/issue_events_013900.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-03-24T21:26:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2661",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2661#event-13900"
}
```



---

archive/attachments_002558.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "2661.patch",
    "asset_url": "tarball://root/attachments/ticket2661/2661.patch",
    "created_at": "2008-03-24T21:26:49Z",
    "issue": "https://github.com/sagemath/sage/issues/2661",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket2661/2661.patch",
    "user": "https://github.com/jasongrout"
}
```



---

archive/issue_comments_014549.json:
```json
{
    "body": "<a id='comment:3'></a>\n**Attachment:** [2661.patch](https://github.com/sagemath/sage/files/ticket2661/2661.patch)\n\nlooks good to me.",
    "created_at": "2008-03-24T21:26:49Z",
    "issue": "https://github.com/sagemath/sage/issues/2661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2661#issuecomment-14549",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:3'></a>
**Attachment:** [2661.patch](https://github.com/sagemath/sage/files/ticket2661/2661.patch)

looks good to me.



---

archive/issue_comments_014550.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 2.11.alpha2",
    "created_at": "2008-03-25T04:48:35Z",
    "issue": "https://github.com/sagemath/sage/issues/2661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2661#issuecomment-14550",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 2.11.alpha2



---

archive/issue_events_013901.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-25T04:48:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2661",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2661#event-13901"
}
```



---

archive/issue_events_013902.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-25T04:48:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2661",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2661#event-13902"
}
```

# Issue 2996: lite webpage broken under Konqueror

archive/issues_002996.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nWith KDE 3.5.9, the background color of the window defaults to white, not purple.  \n\nComponent: **website/wiki**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2996_\n\n",
    "closed_at": "2008-08-31T05:19:27Z",
    "created_at": "2008-04-22T05:49:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20website%20/%20wiki",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "lite webpage broken under Konqueror",
    "type": "issue",
    "updated_at": "2008-08-31T05:22:36Z",
    "url": "https://github.com/sagemath/sage/issues/2996",
    "user": "https://github.com/garyfurnish"
}
```
<div id="comment:0"></div>

With KDE 3.5.9, the background color of the window defaults to white, not purple.  

Component: **website/wiki**

_Issue created by migration from https://trac.sagemath.org/ticket/2996_





---

archive/issue_events_031652.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-04-22T05:49:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2996",
    "milestone_number": null,
    "milestone_title": "sage-3.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2996#event-31652"
}
```



---

archive/issue_events_031653.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-04-22T05:49:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2996",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20website%20/%20wiki",
    "label_color": "0075ca",
    "label_name": "c: website / wiki",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2996#event-31653"
}
```



---

archive/issue_events_031654.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-04-22T05:49:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2996",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2996#event-31654"
}
```



---

archive/issue_events_031655.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-04-22T05:49:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2996",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2996#event-31655"
}
```



---

archive/issue_events_031656.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-04-22T05:49:13Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2996",
    "subject": "https://github.com/garyfurnish",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2996#event-31656"
}
```



---

archive/issue_comments_015640.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nLooks identical to Firefox to me. I vote for invalid or fixed.",
    "created_at": "2008-08-18T17:30:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2996",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2996#issuecomment-15640",
    "user": "https://github.com/malb"
}
```

<div id="comment:1" align="right">comment:1</div>

Looks identical to Firefox to me. I vote for invalid or fixed.



---

archive/issue_comments_015641.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nFixed it is. If someone can reproduce this please let us know which konqueror version is broken.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-31T05:19:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2996",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2996#issuecomment-15641",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Fixed it is. If someone can reproduce this please let us know which konqueror version is broken.

Cheers,

Michael



---

archive/issue_events_031657.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-31T05:19:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2996",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2996#event-31657"
}
```



---

archive/issue_comments_015642.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMike Hansen just confirmed that the color issue is fixed with Konqueror 3.5.9.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-31T05:22:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2996",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2996#issuecomment-15642",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Mike Hansen just confirmed that the color issue is fixed with Konqueror 3.5.9.

Cheers,

Michael

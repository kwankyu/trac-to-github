# Issue 2643: Fix Debian Sections

archive/issues_002643.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nI failed to correctly setup the Section fields of some of the Debian control files.  Attached are a series of patches to fix these problems.\n\nComponent: **debian-package**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2643_\n\n",
    "closed_at": "2008-04-12T17:29:11Z",
    "created_at": "2008-03-22T03:40:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20distribution",
        "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix Debian Sections",
    "type": "issue",
    "updated_at": "2008-04-12T17:29:11Z",
    "url": "https://github.com/sagemath/sage/issues/2643",
    "user": "https://github.com/timabbott"
}
```
<div id="comment:0"></div>

I failed to correctly setup the Section fields of some of the Debian control files.  Attached are a series of patches to fix these problems.

Component: **debian-package**

_Issue created by migration from https://trac.sagemath.org/ticket/2643_





---

archive/issue_events_027704.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-03-22T03:40:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2643#event-27704"
}
```



---

archive/issue_events_027705.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-03-22T03:40:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20distribution",
    "label_color": "0000b0",
    "label_name": "c: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2643#event-27705"
}
```



---

archive/issue_events_027706.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-03-22T03:40:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2643#event-27706"
}
```



---

archive/issue_events_027707.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-03-22T03:40:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2643#event-27707"
}
```



---

archive/issue_events_027708.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-03-22T03:40:43Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "subject": "https://github.com/timabbott",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2643#event-27708"
}
```



---

archive/issue_events_027709.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-03-22T03:40:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2643#event-27709"
}
```



---

archive/issue_comments_013701.json:
```json
{
    "body": "Attachment: **[eclib.patch.gz](https://github.com/sagemath/sage/files/ticket2643/eclib.patch.gz)**",
    "created_at": "2008-03-22T03:47:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2643#issuecomment-13701",
    "user": "https://github.com/timabbott"
}
```

Attachment: **[eclib.patch.gz](https://github.com/sagemath/sage/files/ticket2643/eclib.patch.gz)**



---

archive/issue_comments_013702.json:
```json
{
    "body": "Attachment: **[flint.patch.gz](https://github.com/sagemath/sage/files/ticket2643/flint.patch.gz)**",
    "created_at": "2008-03-22T03:49:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2643#issuecomment-13702",
    "user": "https://github.com/timabbott"
}
```

Attachment: **[flint.patch.gz](https://github.com/sagemath/sage/files/ticket2643/flint.patch.gz)**



---

archive/issue_comments_013703.json:
```json
{
    "body": "Attachment: **[givaro.patch.gz](https://github.com/sagemath/sage/files/ticket2643/givaro.patch.gz)**",
    "created_at": "2008-03-22T03:49:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2643#issuecomment-13703",
    "user": "https://github.com/timabbott"
}
```

Attachment: **[givaro.patch.gz](https://github.com/sagemath/sage/files/ticket2643/givaro.patch.gz)**



---

archive/issue_comments_013704.json:
```json
{
    "body": "Attachment: **[libm4ri.patch.gz](https://github.com/sagemath/sage/files/ticket2643/libm4ri.patch.gz)**",
    "created_at": "2008-03-22T03:49:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2643#issuecomment-13704",
    "user": "https://github.com/timabbott"
}
```

Attachment: **[libm4ri.patch.gz](https://github.com/sagemath/sage/files/ticket2643/libm4ri.patch.gz)**



---

archive/issue_events_027710.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-01T21:20:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2643#event-27710"
}
```



---

archive/issue_comments_013705.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[singular.patch.gz](https://github.com/sagemath/sage/files/ticket2643/singular.patch.gz)**\n\nMerged eclib.patch in eclib-20080310.p1.",
    "created_at": "2008-04-01T21:20:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2643#issuecomment-13705",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[singular.patch.gz](https://github.com/sagemath/sage/files/ticket2643/singular.patch.gz)**

Merged eclib.patch in eclib-20080310.p1.



---

archive/issue_comments_013706.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI should note that missing sections cause failures when trying to upload to a repository, so we should try to be sure these get merged sometime before the 3.0 release.",
    "created_at": "2008-04-10T16:13:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2643#issuecomment-13706",
    "user": "https://github.com/timabbott"
}
```

<div id="comment:2" align="right">comment:2</div>

I should note that missing sections cause failures when trying to upload to a repository, so we should try to be sure these get merged sometime before the 3.0 release.



---

archive/issue_events_027711.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-11T22:12:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2643#event-27711"
}
```



---

archive/issue_events_027712.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-11T22:12:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2643#event-27712"
}
```



---

archive/issue_events_027713.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-11T22:12:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2643#event-27713"
}
```



---

archive/issue_comments_013707.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAll these will go in before 3.0.\n\nCheers,\n\nMichael",
    "created_at": "2008-04-11T22:12:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2643#issuecomment-13707",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

All these will go in before 3.0.

Cheers,

Michael



---

archive/issue_events_027714.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-11T22:12:18Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2643#event-27714"
}
```



---

archive/issue_events_027715.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-11T22:12:18Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2643#event-27715"
}
```



---

archive/issue_comments_013708.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged flint.patch, givaro.patch, libm4ri.patch and singular.patch into Sage 3.0.alpha4. I did not increment the patch level of the spkgs to avoid unneeded rebuilds on upgrade.\n\nCheers,\n\nMichael",
    "created_at": "2008-04-12T17:29:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2643#issuecomment-13708",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged flint.patch, givaro.patch, libm4ri.patch and singular.patch into Sage 3.0.alpha4. I did not increment the patch level of the spkgs to avoid unneeded rebuilds on upgrade.

Cheers,

Michael



---

archive/issue_events_027716.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-12T17:29:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2643",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2643#event-27716"
}
```

# Issue 2987: Debian build support for zn_poly

archive/issues_002987.json:
```json
{
    "assignees": [
        "https://github.com/timabbott"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nI've made a Debian package for zn_poly.  Patch is attached.  \n\nComponent: **debian-package**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2987_\n\n",
    "closed_at": "2008-04-22T03:43:38Z",
    "created_at": "2008-04-21T06:32:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20distribution",
        "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Debian build support for zn_poly",
    "type": "issue",
    "updated_at": "2008-04-22T03:43:38Z",
    "url": "https://github.com/sagemath/sage/issues/2987",
    "user": "https://github.com/timabbott"
}
```
<div id="comment:0"></div>

I've made a Debian package for zn_poly.  Patch is attached.  

Component: **debian-package**

_Issue created by migration from https://trac.sagemath.org/ticket/2987_





---

archive/issue_events_031572.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-21T06:32:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2987",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2987#event-31572"
}
```



---

archive/issue_events_031573.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-21T06:32:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2987",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20distribution",
    "label_color": "0000b0",
    "label_name": "c: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2987#event-31573"
}
```



---

archive/issue_events_031574.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-21T06:32:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2987",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2987#event-31574"
}
```



---

archive/issue_events_031575.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-21T06:32:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2987",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2987#event-31575"
}
```



---

archive/issue_events_031576.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-21T06:32:09Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2987",
    "subject": "https://github.com/timabbott",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2987#event-31576"
}
```



---

archive/issue_events_031577.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-21T06:32:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2987",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2987#event-31577"
}
```



---

archive/issue_comments_015609.json:
```json
{
    "body": "Attachment: **[zn_poly-debian.patch.gz](https://github.com/sagemath/sage/files/ticket2987/zn_poly-debian.patch.gz)**",
    "created_at": "2008-04-21T06:32:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2987#issuecomment-15609",
    "user": "https://github.com/timabbott"
}
```

Attachment: **[zn_poly-debian.patch.gz](https://github.com/sagemath/sage/files/ticket2987/zn_poly-debian.patch.gz)**



---

archive/issue_comments_015610.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nTim, ther is some predecessor patch missing. Neither the 0.4.1.p0 nor the 0.8.p0 spkg has any dist/debian directory. So, is there another patch?\n\nCheers,\n\nMichael",
    "created_at": "2008-04-21T07:03:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2987#issuecomment-15610",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

Tim, ther is some predecessor patch missing. Neither the 0.4.1.p0 nor the 0.8.p0 spkg has any dist/debian directory. So, is there another patch?

Cheers,

Michael



---

archive/issue_events_031578.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-21T07:03:38Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2987",
    "title_is": "[negative review, missing patches] Debian build support for zn_poly",
    "title_was": "Debian build support for zn_poly",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2987#event-31578"
}
```



---

archive/issue_comments_015611.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAhh, I did 3 commits and \"hg export\"ed all 3 to make this patch file.  Lower down in the patch file it creates the directories.  Maybe this doesn't actually work.",
    "created_at": "2008-04-21T15:55:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2987#issuecomment-15611",
    "user": "https://github.com/timabbott"
}
```

<div id="comment:2" align="right">comment:2</div>

Ahh, I did 3 commits and "hg export"ed all 3 to make this patch file.  Lower down in the patch file it creates the directories.  Maybe this doesn't actually work.



---

archive/issue_events_031579.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-22T03:43:19Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2987",
    "title_is": "Debian build support for zn_poly",
    "title_was": "[negative review, missing patches] Debian build support for zn_poly",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2987#event-31579"
}
```



---

archive/issue_events_031580.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-22T03:43:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2987",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2987#event-31580"
}
```



---

archive/issue_comments_015612.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nTim,\n\nfor mercurial you cannot concatenate patches - at least not in the order you did with this patch. I cut the three patches apart and applied them to \n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0/final/zn_poly-0.8.p0.spkg\n\nCheers,\n\nMichael",
    "created_at": "2008-04-22T03:43:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2987#issuecomment-15612",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Tim,

for mercurial you cannot concatenate patches - at least not in the order you did with this patch. I cut the three patches apart and applied them to 

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0/final/zn_poly-0.8.p0.spkg

Cheers,

Michael



---

archive/issue_events_031581.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-22T03:43:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2987",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2987#event-31581"
}
```



---

archive/issue_events_031582.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-22T03:43:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2987",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2987#event-31582"
}
```



---

archive/issue_comments_015613.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in Sage 3.0.final",
    "created_at": "2008-04-22T03:43:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2987#issuecomment-15613",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in Sage 3.0.final

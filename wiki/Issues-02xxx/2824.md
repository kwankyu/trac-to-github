# Issue 2824: sturm_bound incorrect for GammaH

archive/issues_002824.json:
```json
{
    "body": "The Sturm bound is being calculated incorrectly for GammaH (we're just returning the bound for Gamma0, which is wrong). This fixes it.\n\nWe're actually producing wrong answers, so this is getting listed as critical.\n\n**Assignee:** @craigcitro\n\nIssue created by migration from https://trac.sagemath.org/ticket/2824\n\n",
    "closed_at": "2008-04-06T14:16:22Z",
    "created_at": "2008-04-06T07:19:22Z",
    "labels": [
        "component: modular forms",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "title": "sturm_bound incorrect for GammaH",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2824",
    "user": "https://github.com/craigcitro"
}
```
The Sturm bound is being calculated incorrectly for GammaH (we're just returning the bound for Gamma0, which is wrong). This fixes it.

We're actually producing wrong answers, so this is getting listed as critical.

**Assignee:** @craigcitro

Issue created by migration from https://trac.sagemath.org/ticket/2824





---

archive/issue_events_014768.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2008-04-06T08:03:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2824",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2824#event-14768"
}
```



---

archive/attachments_002749.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac-2824.patch",
    "asset_url": "tarball://root/attachments/ticket2824/trac-2824.patch",
    "created_at": "2008-04-06T08:03:29Z",
    "issue": "https://github.com/sagemath/sage/issues/2824",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket2824/trac-2824.patch",
    "user": "https://github.com/robertwb"
}
```



---

archive/issue_comments_015433.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac-2824.patch](https://github.com/sagemath/sage/files/ticket2824/trac-2824.patch)\n\nVery good.",
    "created_at": "2008-04-06T08:03:29Z",
    "issue": "https://github.com/sagemath/sage/issues/2824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2824#issuecomment-15433",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:1'></a>
**Attachment:** [trac-2824.patch](https://github.com/sagemath/sage/files/ticket2824/trac-2824.patch)

Very good.



---

archive/issue_comments_015434.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.0.alpha2",
    "created_at": "2008-04-06T14:16:22Z",
    "issue": "https://github.com/sagemath/sage/issues/2824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2824#issuecomment-15434",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.0.alpha2



---

archive/issue_events_014769.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-06T14:16:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2824",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2824#event-14769"
}
```



---

archive/issue_events_014770.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-06T14:16:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2824",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2824#event-14770"
}
```

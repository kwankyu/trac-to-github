# Issue 2756: Debianize GAP spkg

archive/issues_002756.json:
```json
{
    "assignees": [
        "https://github.com/timabbott"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nI've attached a patch to add Debian build support to the GAP spkg (it only builds the Guava GAP package).\n\nComponent: **debian-package**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2756_\n\n",
    "closed_at": "2008-04-01T23:06:58Z",
    "created_at": "2008-04-01T18:41:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20distribution",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Debianize GAP spkg",
    "type": "issue",
    "updated_at": "2008-04-01T23:06:58Z",
    "url": "https://github.com/sagemath/sage/issues/2756",
    "user": "https://github.com/timabbott"
}
```
<div id="comment:0"></div>

I've attached a patch to add Debian build support to the GAP spkg (it only builds the Guava GAP package).

Component: **debian-package**

_Issue created by migration from https://trac.sagemath.org/ticket/2756_





---

archive/issue_events_028941.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-01T18:41:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2756",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2756#event-28941"
}
```



---

archive/issue_events_028942.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-01T18:41:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2756",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20distribution",
    "label_color": "0000b0",
    "label_name": "c: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2756#event-28942"
}
```



---

archive/issue_events_028943.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-01T18:41:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2756",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2756#event-28943"
}
```



---

archive/issue_events_028944.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-01T18:41:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2756",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2756#event-28944"
}
```



---

archive/issue_events_028945.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-01T18:41:37Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2756",
    "subject": "https://github.com/timabbott",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2756#event-28945"
}
```



---

archive/issue_events_028946.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-01T18:41:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2756",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2756#event-28946"
}
```



---

archive/issue_comments_014340.json:
```json
{
    "body": "Attachment: **[guava.patch.gz](https://github.com/sagemath/sage/files/ticket2756/guava.patch.gz)**",
    "created_at": "2008-04-01T18:41:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2756",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2756#issuecomment-14340",
    "user": "https://github.com/timabbott"
}
```

Attachment: **[guava.patch.gz](https://github.com/sagemath/sage/files/ticket2756/guava.patch.gz)**



---

archive/issue_comments_014341.json:
```json
{
    "body": "Attachment: **[gap-spkg-debian.patch.gz](https://github.com/sagemath/sage/files/ticket2756/gap-spkg-debian.patch.gz)**",
    "created_at": "2008-04-01T18:42:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2756",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2756#issuecomment-14341",
    "user": "https://github.com/timabbott"
}
```

Attachment: **[gap-spkg-debian.patch.gz](https://github.com/sagemath/sage/files/ticket2756/gap-spkg-debian.patch.gz)**



---

archive/issue_comments_014342.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nPatch looks good to me. One slight issue is that Guava has been updated to Guava 3.4. So let's merge this in alpha0 and if it doesn't work we can fix it later.\n\nCheers,\n\nMichael",
    "created_at": "2008-04-01T22:54:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2756",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2756#issuecomment-14342",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

Patch looks good to me. One slight issue is that Guava has been updated to Guava 3.4. So let's merge this in alpha0 and if it doesn't work we can fix it later.

Cheers,

Michael



---

archive/issue_events_028947.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-01T22:54:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2756",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2756#event-28947"
}
```



---

archive/issue_comments_014343.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged in gap-4.4.10.p5 in Sage 3.0.alpha0",
    "created_at": "2008-04-01T23:06:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2756",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2756#issuecomment-14343",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged in gap-4.4.10.p5 in Sage 3.0.alpha0



---

archive/issue_events_028948.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-01T23:06:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2756",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2756#event-28948"
}
```



---

archive/issue_events_028949.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-01T23:06:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2756",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2756#event-28949"
}
```

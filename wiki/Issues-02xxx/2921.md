# Issue 2921: gcc 4.3: Fix flintqs.spkg build

archive/issues_002921.json:
```json
{
    "body": "Flintqs currently doesn't build with gcc 4.3. The following simple patch fixes that.\n\nCheers,\n\nMichael\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/2921\n\n",
    "closed_at": "2008-04-14T21:56:45Z",
    "created_at": "2008-04-14T21:27:46Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "title": "gcc 4.3: Fix flintqs.spkg build",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2921",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
Flintqs currently doesn't build with gcc 4.3. The following simple patch fixes that.

Cheers,

Michael

**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/2921





---

archive/issue_events_015394.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-14T21:30:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2921",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2921#event-15394"
}
```



---

archive/attachments_002878.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "flintqs-gcc-4.3-fix.patch",
    "asset_url": "tarball://root/attachments/ticket2921/flintqs-gcc-4.3-fix.patch",
    "created_at": "2008-04-14T21:30:22Z",
    "issue": "https://github.com/sagemath/sage/issues/2921",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket2921/flintqs-gcc-4.3-fix.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```



---

archive/issue_comments_016027.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [flintqs-gcc-4.3-fix.patch](https://github.com/sagemath/sage/files/ticket2921/flintqs-gcc-4.3-fix.patch)\n\nThe updated spkg is at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0/alpha5/flintqs-20070817.p3.spkg\n\nCheers,\n\nMichael",
    "created_at": "2008-04-14T21:30:22Z",
    "issue": "https://github.com/sagemath/sage/issues/2921",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2921#issuecomment-16027",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
**Attachment:** [flintqs-gcc-4.3-fix.patch](https://github.com/sagemath/sage/files/ticket2921/flintqs-gcc-4.3-fix.patch)

The updated spkg is at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0/alpha5/flintqs-20070817.p3.spkg

Cheers,

Michael



---

archive/issue_comments_016028.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good to me. Also builds fine on gcc 4.1.2.",
    "created_at": "2008-04-14T21:48:42Z",
    "issue": "https://github.com/sagemath/sage/issues/2921",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2921#issuecomment-16028",
    "user": "https://github.com/wjp"
}
```

<a id='comment:2'></a>
Looks good to me. Also builds fine on gcc 4.1.2.



---

archive/issue_events_015395.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2008-04-14T21:48:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2921",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2921#event-15395"
}
```



---

archive/issue_events_015396.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2008-04-14T21:48:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2921",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2921#event-15396"
}
```



---

archive/issue_comments_016029.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.alpha5",
    "created_at": "2008-04-14T21:56:45Z",
    "issue": "https://github.com/sagemath/sage/issues/2921",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2921#issuecomment-16029",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.alpha5



---

archive/issue_events_015397.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-14T21:56:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2921",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2921#event-15397"
}
```



---

archive/issue_events_015398.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-14T21:56:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2921",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2921#event-15398"
}
```

# Issue 2500: Deprecate "sage -upgrade"; replace by "sage -expert_upgrade"; likewise for sage: upgrade()

archive/issues_002500.json:
```json
{
    "body": "Unless somebody thinks they can make \"sage -upgrade\" bulletproof, which I have no clue how to do, this gets deprecated. \n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/2500\n\n",
    "closed_at": "2008-08-24T17:09:52Z",
    "created_at": "2008-03-12T16:56:27Z",
    "labels": [
        "component: build",
        "bug",
        "duplicate"
    ],
    "title": "Deprecate \"sage -upgrade\"; replace by \"sage -expert_upgrade\"; likewise for sage: upgrade()",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2500",
    "user": "https://github.com/williamstein"
}
```
Unless somebody thinks they can make "sage -upgrade" bulletproof, which I have no clue how to do, this gets deprecated. 

**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/2500





---

archive/issue_comments_013363.json:
```json
{
    "body": "<a id='comment:1'></a>\nAFAIK this was shot down several times, shall we close the ticket or do you want another vote on [sage-devel]?",
    "created_at": "2008-08-18T17:19:27Z",
    "issue": "https://github.com/sagemath/sage/issues/2500",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2500#issuecomment-13363",
    "user": "https://github.com/malb"
}
```

<a id='comment:1'></a>
AFAIK this was shot down several times, shall we close the ticket or do you want another vote on [sage-devel]?



---

archive/issue_comments_013364.json:
```json
{
    "body": "<a id='comment:2'></a>\nThe proposal on sage-devel was to remove `-upgrade`, but I think renaming it would be fine.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-18T17:46:33Z",
    "issue": "https://github.com/sagemath/sage/issues/2500",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2500#issuecomment-13364",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
The proposal on sage-devel was to remove `-upgrade`, but I think renaming it would be fine.

Cheers,

Michael



---

archive/issue_events_012793.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-24T17:09:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2500",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2500#event-12793"
}
```



---

archive/issue_events_012794.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-24T17:09:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2500",
    "milestone": "sage-3.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2500#event-12794"
}
```



---

archive/issue_comments_013365.json:
```json
{
    "body": "<a id='comment:3'></a>\nWilliam opened #3944 with a patch, so I am closing this as a dupe.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-24T17:09:52Z",
    "issue": "https://github.com/sagemath/sage/issues/2500",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2500#issuecomment-13365",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
William opened #3944 with a patch, so I am closing this as a dupe.

Cheers,

Michael



---

archive/issue_events_012795.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-24T17:09:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2500",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2500#event-12795"
}
```

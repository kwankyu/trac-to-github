# Issue 2671: implement matrix/vector function similar to numpy's where or matlab's find

archive/issues_002671.json:
```json
{
    "body": "See http://www.scipy.org/NumPy_for_Matlab_Users\n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/2671\n\n",
    "closed_at": "2008-04-14T16:59:31Z",
    "created_at": "2008-03-25T22:36:22Z",
    "labels": [
        "component: linear algebra",
        "enhancement",
        "duplicate"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "title": "implement matrix/vector function similar to numpy's where or matlab's find",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2671",
    "user": "https://github.com/jasongrout"
}
```
See http://www.scipy.org/NumPy_for_Matlab_Users

**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/2671





---

archive/issue_events_013950.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-27T09:42:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2671",
    "label": "component: porting: cygwin",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2671#event-13950"
}
```



---

archive/issue_events_013951.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-27T09:42:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2671",
    "label": "component: linear algebra",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2671#event-13951"
}
```



---

archive/issue_comments_014603.json:
```json
{
    "body": "**Changing assignee** from mabshoff to @williamstein.",
    "created_at": "2008-03-27T09:42:48Z",
    "issue": "https://github.com/sagemath/sage/issues/2671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2671#issuecomment-14603",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

**Changing assignee** from mabshoff to @williamstein.



---

archive/issue_events_013952.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-27T09:42:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2671",
    "milestone": "sage-2.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2671#event-13952"
}
```



---

archive/issue_comments_014604.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis looks like a duplicate to #2670 to me...",
    "created_at": "2008-04-14T16:25:41Z",
    "issue": "https://github.com/sagemath/sage/issues/2671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2671#issuecomment-14604",
    "user": "https://github.com/dfdeshom"
}
```

<a id='comment:2'></a>
This looks like a duplicate to #2670 to me...



---

archive/issue_comments_014605.json:
```json
{
    "body": "<a id='comment:3'></a>\nIt looks like a dup to me too, so I'm closing it.\n\nThanks for fixing the other ticket by writing the function.",
    "created_at": "2008-04-14T16:59:31Z",
    "issue": "https://github.com/sagemath/sage/issues/2671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2671#issuecomment-14605",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:3'></a>
It looks like a dup to me too, so I'm closing it.

Thanks for fixing the other ticket by writing the function.



---

archive/issue_events_013953.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-14T16:59:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2671",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2671#event-13953"
}
```



---

archive/issue_events_013954.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-14T16:59:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2671",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2671#event-13954"
}
```

# Issue 2557: dsage unit tests fail on linux

archive/issues_002557.json:
```json
{
    "body": "Various users have reported that the dsage unit tests fail on linux. This is a known issue and a fix is being worked on by me. \n\n**Assignee:** @yqiang\n\nIssue created by migration from https://trac.sagemath.org/ticket/2557\n\n",
    "closed_at": "2008-03-16T21:53:53Z",
    "created_at": "2008-03-16T21:41:38Z",
    "labels": [
        "component: dsage",
        "bug",
        "duplicate"
    ],
    "title": "dsage unit tests fail on linux",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2557",
    "user": "https://github.com/yqiang"
}
```
Various users have reported that the dsage unit tests fail on linux. This is a known issue and a fix is being worked on by me. 

**Assignee:** @yqiang

Issue created by migration from https://trac.sagemath.org/ticket/2557





---

archive/issue_events_013167.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-16T21:53:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2557",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2557#event-13167"
}
```



---

archive/issue_events_013168.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-16T21:53:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2557",
    "milestone": "sage-2.10.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2557#event-13168"
}
```



---

archive/issue_comments_013816.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis is a dupe of #2553",
    "created_at": "2008-03-16T21:53:53Z",
    "issue": "https://github.com/sagemath/sage/issues/2557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2557#issuecomment-13816",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
This is a dupe of #2553



---

archive/issue_events_013169.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-16T21:53:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2557",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2557#event-13169"
}
```

# Issue 2821: get rid of anything "about this document" sections of any sage docs that say "send email to stein"

archive/issues_002821.json:
```json
{
    "body": "I'm tired of getting personal emails that are much better put on list.\n\nIssue created by migration from https://trac.sagemath.org/ticket/2821\n\n",
    "closed_at": "2008-04-25T04:14:23Z",
    "created_at": "2008-04-06T04:04:03Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.1",
    "title": "get rid of anything \"about this document\" sections of any sage docs that say \"send email to stein\"",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2821",
    "user": "https://github.com/williamstein"
}
```
I'm tired of getting personal emails that are much better put on list.

Issue created by migration from https://trac.sagemath.org/ticket/2821





---

archive/attachments_002744.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "2821-stop_bugging_wstein.patch",
    "asset_url": "tarball://root/attachments/ticket2821/2821-stop_bugging_wstein.patch",
    "created_at": "2008-04-25T00:36:07Z",
    "issue": "https://github.com/sagemath/sage/issues/2821",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket2821/2821-stop_bugging_wstein.patch",
    "user": "https://github.com/aghitza"
}
```



---

archive/issue_comments_015407.json:
```json
{
    "body": "**Attachment:** [2821-stop_bugging_wstein.patch](https://github.com/sagemath/sage/files/ticket2821/2821-stop_bugging_wstein.patch)\n\ndoc patch",
    "created_at": "2008-04-25T00:36:07Z",
    "issue": "https://github.com/sagemath/sage/issues/2821",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2821#issuecomment-15407",
    "user": "https://github.com/aghitza"
}
```

**Attachment:** [2821-stop_bugging_wstein.patch](https://github.com/sagemath/sage/files/ticket2821/2821-stop_bugging_wstein.patch)

doc patch



---

archive/issue_events_014753.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-04-25T00:36:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2821",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2821#event-14753"
}
```



---

archive/issue_comments_015408.json:
```json
{
    "body": "<a id='comment:1'></a>\nSee the attached patch.",
    "created_at": "2008-04-25T00:36:50Z",
    "issue": "https://github.com/sagemath/sage/issues/2821",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2821#issuecomment-15408",
    "user": "https://github.com/aghitza"
}
```

<a id='comment:1'></a>
See the attached patch.



---

archive/issue_comments_015409.json:
```json
{
    "body": "<a id='comment:2'></a>\nApplies cleanly using hg_doc.apply and all the latex docs pass sage -t. Also, every change in the diff file looks reasonable. Looks like a good doc patch to me.",
    "created_at": "2008-04-25T03:41:55Z",
    "issue": "https://github.com/sagemath/sage/issues/2821",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2821#issuecomment-15409",
    "user": "https://github.com/wdjoyner"
}
```

<a id='comment:2'></a>
Applies cleanly using hg_doc.apply and all the latex docs pass sage -t. Also, every change in the diff file looks reasonable. Looks like a good doc patch to me.



---

archive/issue_events_014754.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2008-04-25T03:41:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2821",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2821#event-14754"
}
```



---

archive/issue_events_014755.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2008-04-25T03:41:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2821",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2821#event-14755"
}
```



---

archive/issue_events_014756.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-25T04:14:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2821",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2821#event-14756"
}
```



---

archive/issue_events_014757.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-25T04:14:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2821",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2821#event-14757"
}
```



---

archive/issue_comments_015410.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.1.alpha0",
    "created_at": "2008-04-25T04:14:23Z",
    "issue": "https://github.com/sagemath/sage/issues/2821",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2821#issuecomment-15410",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.1.alpha0

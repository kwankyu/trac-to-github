# Issue 2219: docs for root_field should say the polynomial needs to be irreducible

archive/issues_002219.json:
```json
{
    "body": "\n```\nroot_field()\n-- whose docstring does not say that self should be irreducible,\nthough in fact it must. \n```\n\nSee http://groups.google.com/group/sage-devel/browse_thread/thread/32fe12de12d5f6a5/c91753b5e65fe7b9#c91753b5e65fe7b9\n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/2219\n\n",
    "closed_at": "2008-04-15T00:31:55Z",
    "created_at": "2008-02-20T03:51:37Z",
    "labels": [
        "component: number theory",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "title": "docs for root_field should say the polynomial needs to be irreducible",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2219",
    "user": "https://github.com/jasongrout"
}
```

```
root_field()
-- whose docstring does not say that self should be irreducible,
though in fact it must. 
```

See http://groups.google.com/group/sage-devel/browse_thread/thread/32fe12de12d5f6a5/c91753b5e65fe7b9#c91753b5e65fe7b9

**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/2219





---

archive/attachments_001982.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "2219-root_field.patch",
    "asset_url": "tarball://root/attachments/ticket2219/2219-root_field.patch",
    "created_at": "2008-04-13T03:03:53Z",
    "issue": "https://github.com/sagemath/sage/issues/2219",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket2219/2219-root_field.patch",
    "user": "https://github.com/aghitza"
}
```



---

archive/issue_comments_011516.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [2219-root_field.patch](https://github.com/sagemath/sage/files/ticket2219/2219-root_field.patch)\n\nSee the attached patch.",
    "created_at": "2008-04-13T03:03:53Z",
    "issue": "https://github.com/sagemath/sage/issues/2219",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2219#issuecomment-11516",
    "user": "https://github.com/aghitza"
}
```

<a id='comment:1'></a>
**Attachment:** [2219-root_field.patch](https://github.com/sagemath/sage/files/ticket2219/2219-root_field.patch)

See the attached patch.



---

archive/issue_events_010989.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-04-13T03:03:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2219",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2219#event-10989"
}
```



---

archive/issue_comments_011517.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good to me.",
    "created_at": "2008-04-14T23:08:24Z",
    "issue": "https://github.com/sagemath/sage/issues/2219",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2219#issuecomment-11517",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
Looks good to me.



---

archive/issue_events_010990.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-04-14T23:08:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2219",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2219#event-10990"
}
```



---

archive/issue_events_010991.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-04-14T23:08:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2219",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2219#event-10991"
}
```



---

archive/issue_comments_011518.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.alpha5",
    "created_at": "2008-04-15T00:31:55Z",
    "issue": "https://github.com/sagemath/sage/issues/2219",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2219#issuecomment-11518",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.alpha5



---

archive/issue_events_010992.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-15T00:31:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2219",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2219#event-10992"
}
```



---

archive/issue_events_010993.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-15T00:31:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2219",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2219#event-10993"
}
```

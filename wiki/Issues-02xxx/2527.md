# Issue 2527: get doctests for congroup.py and congroup_element.py up to 100%

archive/issues_002527.json:
```json
{
    "body": "Start:\n\n```\nteragon:modular was$ sage -coverage congroup.py congroup_element.py \nSCORE congroup.py: 20% (14 of 68)\nSCORE congroup_element.py: 0% (0 of 10)\n```\n\n\n**Assignee:** @craigcitro\n\nIssue created by migration from https://trac.sagemath.org/ticket/2527\n\n",
    "closed_at": "2008-08-12T23:46:47Z",
    "created_at": "2008-03-15T02:40:26Z",
    "labels": [
        "component: modular forms",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1",
    "title": "get doctests for congroup.py and congroup_element.py up to 100%",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2527",
    "user": "https://github.com/williamstein"
}
```
Start:

```
teragon:modular was$ sage -coverage congroup.py congroup_element.py 
SCORE congroup.py: 20% (14 of 68)
SCORE congroup_element.py: 0% (0 of 10)
```


**Assignee:** @craigcitro

Issue created by migration from https://trac.sagemath.org/ticket/2527





---

archive/issue_events_012987.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-04-23T02:36:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2527",
    "label": "component: number theory",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2527#event-12987"
}
```



---

archive/issue_events_012988.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-04-23T02:36:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2527",
    "label": "component: modular forms",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2527#event-12988"
}
```



---

archive/issue_comments_013647.json:
```json
{
    "body": "**Changing assignee** from @williamstein to @craigcitro.",
    "created_at": "2008-04-23T02:36:19Z",
    "issue": "https://github.com/sagemath/sage/issues/2527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2527#issuecomment-13647",
    "user": "https://github.com/aghitza"
}
```

**Changing assignee** from @williamstein to @craigcitro.



---

archive/issue_events_012989.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-12T23:46:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2527",
    "milestone": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2527#event-12989"
}
```



---

archive/issue_events_012990.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-12T23:46:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2527",
    "milestone": "sage-3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2527#event-12990"
}
```



---

archive/issue_comments_013648.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis was achieved in Sage 3.1.alpha2 via #1220.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-12T23:46:47Z",
    "issue": "https://github.com/sagemath/sage/issues/2527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2527#issuecomment-13648",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
This was achieved in Sage 3.1.alpha2 via #1220.

Cheers,

Michael



---

archive/issue_events_012991.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-12T23:46:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2527",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2527#event-12991"
}
```

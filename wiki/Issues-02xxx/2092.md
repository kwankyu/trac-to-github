# Issue 2092: Add jpeg support to gd

archive/issues_002092.json:
```json
{
    "body": "jpeg is a common format that most people keep their pictures in -- let's support it!\n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/2092\n\n",
    "closed_at": "2008-02-08T01:29:01Z",
    "created_at": "2008-02-08T00:49:58Z",
    "labels": [
        "component: graphics",
        "enhancement",
        "duplicate"
    ],
    "title": "Add jpeg support to gd",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2092",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```
jpeg is a common format that most people keep their pictures in -- let's support it!

**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/2092





---

archive/issue_comments_010517.json:
```json
{
    "body": "<a id='comment:1'></a>\nDuplicate of #2094",
    "created_at": "2008-02-08T01:29:01Z",
    "issue": "https://github.com/sagemath/sage/issues/2092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2092#issuecomment-10517",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Duplicate of #2094



---

archive/issue_events_010110.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-02-08T01:29:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2092",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2092#event-10110"
}
```



---

archive/issue_events_010111.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-02-08T01:29:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2092",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2092#event-10111"
}
```

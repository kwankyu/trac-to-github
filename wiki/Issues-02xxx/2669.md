# Issue 2669: letting matrix __setindex__ function allow setting multiple entries

archive/issues_002669.json:
```json
{
    "body": "It would be nice if we could set multiple elements using indexing notation, ala matlab or numpy (see, for example, http://www.scipy.org/NumPy_for_Matlab_Users\n\n\n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/2669\n\n",
    "closed_at": "2008-03-25T22:48:59Z",
    "created_at": "2008-03-25T22:24:46Z",
    "labels": [
        "component: linear algebra",
        "enhancement",
        "duplicate"
    ],
    "title": "letting matrix __setindex__ function allow setting multiple entries",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2669",
    "user": "https://github.com/jasongrout"
}
```
It would be nice if we could set multiple elements using indexing notation, ala matlab or numpy (see, for example, http://www.scipy.org/NumPy_for_Matlab_Users



**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/2669





---

archive/issue_events_013938.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-03-25T22:48:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2669",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2669#event-13938"
}
```



---

archive/issue_events_013939.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-03-25T22:48:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2669",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2669#event-13939"
}
```



---

archive/issue_comments_014595.json:
```json
{
    "body": "<a id='comment:1'></a>\nSee #2396.",
    "created_at": "2008-03-25T22:48:59Z",
    "issue": "https://github.com/sagemath/sage/issues/2669",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2669#issuecomment-14595",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:1'></a>
See #2396.



---

archive/issue_events_013940.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-25T23:37:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2669",
    "milestone": "sage-2.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2669#event-13940"
}
```

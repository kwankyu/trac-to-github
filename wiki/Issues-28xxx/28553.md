# Issue 28553: libsemigroups does not pass self-tests on 32bit system

archive/issues_028316.json:
```json
{
    "body": "Recently added `libsemigroups` (#27396) does not pass self-tests on 32bit system, see the attached log.\n\n\n**CC:**  @dimpase @nthiery @isuruf @tscrim @slel\n\n**Keywords:** sdl\n\n**Reviewer:** Matthias Koeppe\n\nIssue created by migration from https://trac.sagemath.org/ticket/28553\n\n",
    "closed_at": "2021-06-27T19:19:27Z",
    "created_at": "2019-10-03T13:56:58Z",
    "labels": [
        "component: packages: optional",
        "bug",
        "invalid"
    ],
    "title": "libsemigroups does not pass self-tests on 32bit system",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/28553",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```
Recently added `libsemigroups` (#27396) does not pass self-tests on 32bit system, see the attached log.


**CC:**  @dimpase @nthiery @isuruf @tscrim @slel

**Keywords:** sdl

**Reviewer:** Matthias Koeppe

Issue created by migration from https://trac.sagemath.org/ticket/28553





---

archive/attachments_021992.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "libsemigroups-0.6.7.log",
    "asset_url": "tarball://root/attachments/ticket28553/libsemigroups-0.6.7.log",
    "created_at": "2019-10-03T14:03:17Z",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket28553/libsemigroups-0.6.7.log",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```



---

archive/issue_comments_447956.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [libsemigroups-0.6.7.log](https://github.com/sagemath/sage/files/ticket28553/libsemigroups-0.6.7.log)",
    "created_at": "2019-10-03T14:03:17Z",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28553#issuecomment-447956",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

<a id='comment:1'></a>
**Attachment:** [libsemigroups-0.6.7.log](https://github.com/sagemath/sage/files/ticket28553/libsemigroups-0.6.7.log)



---

archive/issue_comments_447957.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,2 +1,2 @@\n-Recently added `libsemigroups` does not pass self-tests on 32bit system, see the attached log.\n+Recently added `libsemigroups` (#27396) does not pass self-tests on 32bit system, see the attached log.\n \n``````\n",
    "created_at": "2019-10-03T14:03:17Z",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28553#issuecomment-447957",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,2 +1,2 @@
-Recently added `libsemigroups` does not pass self-tests on 32bit system, see the attached log.
+Recently added `libsemigroups` (#27396) does not pass self-tests on 32bit system, see the attached log.
 
``````




---

archive/issue_events_255162.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2020-01-06T14:10:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "milestone": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28553#event-255162"
}
```



---

archive/issue_events_255163.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2020-01-06T14:10:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28553#event-255163"
}
```



---

archive/issue_comments_447958.json:
```json
{
    "body": "<a id='comment:2'></a>\nTicket retargeted after milestone closed",
    "created_at": "2020-01-06T14:10:03Z",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28553#issuecomment-447958",
    "user": "https://github.com/embray"
}
```

<a id='comment:2'></a>
Ticket retargeted after milestone closed



---

archive/issue_events_255164.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-04T20:06:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28553#event-255164"
}
```



---

archive/issue_events_255165.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-04T20:06:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28553#event-255165"
}
```



---

archive/issue_events_255166.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28553#event-255166"
}
```



---

archive/issue_events_255167.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28553#event-255167"
}
```



---

archive/issue_comments_447959.json:
```json
{
    "body": "<a id='comment:5'></a>\nThis may have been fixed in libsemigroups 1.0.3:\n\n- [libsemigroups 1.0.3 release notes](https://libsemigroups.readthedocs.io/en/latest/changelog.html?highlight=32#v1-0-3-released-07-12-2019)",
    "created_at": "2021-01-09T11:07:12Z",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28553#issuecomment-447959",
    "user": "https://github.com/slel"
}
```

<a id='comment:5'></a>
This may have been fixed in libsemigroups 1.0.3:

- [libsemigroups 1.0.3 release notes](https://libsemigroups.readthedocs.io/en/latest/changelog.html?highlight=32#v1-0-3-released-07-12-2019)



---

archive/issue_events_255168.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-01-09T11:07:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28553#event-255168"
}
```



---

archive/issue_events_255169.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-01-09T11:07:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28553#event-255169"
}
```



---

archive/issue_comments_447960.json:
```json
{
    "body": "<a id='comment:6'></a>\nCan someone with a 32-bit system please confirm?",
    "created_at": "2021-01-09T14:21:01Z",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28553#issuecomment-447960",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'></a>
Can someone with a 32-bit system please confirm?



---

archive/issue_comments_447961.json:
```json
{
    "body": "**Reviewer:** Matthias Koeppe",
    "created_at": "2021-01-09T18:40:13Z",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28553#issuecomment-447961",
    "user": "https://github.com/mkoeppe"
}
```

**Reviewer:** Matthias Koeppe



---

archive/issue_comments_447962.json:
```json
{
    "body": "<a id='comment:7'></a>\nGH Actions tests a few 32-bit systems.\n\nOn `ubuntu-bionic-i386` (https://github.com/sagemath/sage/runs/1615246784?check_suite_focus=true), `libsemigroups` is OK.\n\nOn `centos-7-i386` (https://github.com/sagemath/sage/runs/1615246935?check_suite_focus=true), compilation of `libsemigroups` fails, but this is the same error as on 64 bit `centos-7` (https://github.com/sagemath/sage/runs/1615246513?check_suite_focus=true) and is caused by an outdated compiler.",
    "created_at": "2021-01-09T18:40:13Z",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28553#issuecomment-447962",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:7'></a>
GH Actions tests a few 32-bit systems.

On `ubuntu-bionic-i386` (https://github.com/sagemath/sage/runs/1615246784?check_suite_focus=true), `libsemigroups` is OK.

On `centos-7-i386` (https://github.com/sagemath/sage/runs/1615246935?check_suite_focus=true), compilation of `libsemigroups` fails, but this is the same error as on 64 bit `centos-7` (https://github.com/sagemath/sage/runs/1615246513?check_suite_focus=true) and is caused by an outdated compiler.



---

archive/issue_events_255170.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-01-09T18:40:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28553#event-255170"
}
```



---

archive/issue_events_255171.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-01-09T18:40:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28553#event-255171"
}
```



---

archive/issue_events_255172.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-06-27T19:19:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28553#event-255172"
}
```



---

archive/issue_events_255173.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-06-27T19:19:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28553",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28553#event-255173"
}
```

# Issue 28796: Claim "sage" package on PyPI, replace unexplained package "sage 0.0.0"

archive/issues_028559.json:
```json
{
    "body": "Someone with the user name `dhagrow` uploaded a package `sage`, version 0.0.0, to PyPI in 2018. The package provides no description nor contact information. This is harmless, but could cause confusion. \n\nPython packages may want to declare `sage` as a dependency in their `setup.py` and `requirements.txt` - as this is the name of the Python package for the Sage library defined in `SAGEROOT/src/setup.py`.\n\nhttps://pypi.org/project/sage/\n\n\n**CC:**  @jdemeyer @vbraun @dimpase @isuruf @embray\n\n**Upstream:** Reported upstream. No feedback yet.\n\nIssue created by migration from https://trac.sagemath.org/ticket/28796\n\n",
    "created_at": "2019-11-24T16:13:37Z",
    "labels": [
        "component: distribution"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "title": "Claim \"sage\" package on PyPI, replace unexplained package \"sage 0.0.0\"",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/28796",
    "user": "https://github.com/mkoeppe"
}
```
Someone with the user name `dhagrow` uploaded a package `sage`, version 0.0.0, to PyPI in 2018. The package provides no description nor contact information. This is harmless, but could cause confusion. 

Python packages may want to declare `sage` as a dependency in their `setup.py` and `requirements.txt` - as this is the name of the Python package for the Sage library defined in `SAGEROOT/src/setup.py`.

https://pypi.org/project/sage/


**CC:**  @jdemeyer @vbraun @dimpase @isuruf @embray

**Upstream:** Reported upstream. No feedback yet.

Issue created by migration from https://trac.sagemath.org/ticket/28796





---

archive/issue_comments_452059.json:
```json
{
    "body": "<a id='comment:1'></a>\nlinks on their other submissions to pypi lead to https://github.com/dhagrow",
    "created_at": "2019-11-24T17:12:34Z",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28796#issuecomment-452059",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:1'></a>
links on their other submissions to pypi lead to https://github.com/dhagrow



---

archive/issue_events_257814.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-13T12:55:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "label": "bug",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257814"
}
```



---

archive/issue_comments_452060.json:
```json
{
    "body": "<a id='comment:3'></a>\nThe package itself is also empty.  It just contains a `sage/__init__.py` containing `__version__ = '0.0.0'`.\n\n[PEP 541](https://www.python.org/dev/peps/pep-0541/) includes procedures and conditions for requesting an ownership transfer on PyPI.",
    "created_at": "2019-12-13T12:59:08Z",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28796#issuecomment-452060",
    "user": "https://github.com/embray"
}
```

<a id='comment:3'></a>
The package itself is also empty.  It just contains a `sage/__init__.py` containing `__version__ = '0.0.0'`.

[PEP 541](https://www.python.org/dev/peps/pep-0541/) includes procedures and conditions for requesting an ownership transfer on PyPI.



---

archive/issue_events_257815.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2020-01-06T14:10:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "milestone": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257815"
}
```



---

archive/issue_events_257816.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2020-01-06T14:10:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257816"
}
```



---

archive/issue_comments_452061.json:
```json
{
    "body": "<a id='comment:4'></a>\nTicket retargeted after milestone closed",
    "created_at": "2020-01-06T14:10:03Z",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28796#issuecomment-452061",
    "user": "https://github.com/embray"
}
```

<a id='comment:4'></a>
Ticket retargeted after milestone closed



---

archive/issue_events_257817.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-09T23:09:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257817"
}
```



---

archive/issue_events_257818.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-09T23:09:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257818"
}
```



---

archive/issue_events_257819.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257819"
}
```



---

archive/issue_events_257820.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257820"
}
```



---

archive/issue_comments_452062.json:
```json
{
    "body": "**Upstream:** Reported upstream. No feedback yet.",
    "created_at": "2020-11-14T08:46:56Z",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28796#issuecomment-452062",
    "user": "https://github.com/dimpase"
}
```

**Upstream:** Reported upstream. No feedback yet.



---

archive/issue_comments_452063.json:
```json
{
    "body": "<a id='comment:7'></a>\nI've written to the author of \"sage\" here: https://github.com/dhagrow/profig/issues/20",
    "created_at": "2020-11-14T08:46:56Z",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28796#issuecomment-452063",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:7'></a>
I've written to the author of "sage" here: https://github.com/dhagrow/profig/issues/20



---

archive/issue_events_257821.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-11-14T11:39:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "label": "component: distribution",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257821"
}
```



---

archive/issue_events_257822.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257822"
}
```



---

archive/issue_events_257823.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257823"
}
```



---

archive/issue_comments_452064.json:
```json
{
    "body": "<a id='comment:9'></a>\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28796#issuecomment-452064",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:9'></a>
Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_257824.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257824"
}
```



---

archive/issue_events_257825.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257825"
}
```



---

archive/issue_events_257826.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257826"
}
```



---

archive/issue_events_257827.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257827"
}
```



---

archive/issue_events_257828.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257828"
}
```



---

archive/issue_events_257829.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257829"
}
```



---

archive/issue_events_257830.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257830"
}
```



---

archive/issue_events_257831.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28796",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28796#event-257831"
}
```

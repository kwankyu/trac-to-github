# Issue 28939: Fix failure in src/sage/structure/coerce_dict.pyx

archive/issues_028702.json:
```json
{
    "body": "Testing sage-9.0 on a Raspberry Pi 4B with Raspbian Buster\n\n\n```\nsage -t --long src/sage/structure/coerce_dict.pyx\n**********************************************************************\nFile \"src/sage/structure/coerce_dict.pyx\", line 49, in sage.structure.coerce_dict\nFailed example:\n    len(LE)\nExpected:\n    1\nGot:\n    50\n**********************************************************************\n1 item had failures:\n   1 of   9 in sage.structure.coerce_dict\n    [296 tests, 1 failure, 91.00 s]\n----------------------------------------------------------------------\nsage -t --long src/sage/structure/coerce_dict.pyx  # 1 doctest failed\n----------------------------------------------------------------------\nTotal time for all tests: 91.3 seconds\n    cpu time: 90.7 seconds\n    cumulative wall time: 91.0 seconds\n\n```\n\nDepends on #28941\n\n**Keywords:** raspberry\n\nIssue created by migration from https://trac.sagemath.org/ticket/28939\n\n",
    "created_at": "2020-01-02T12:04:36Z",
    "labels": [
        "component: porting",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "title": "Fix failure in src/sage/structure/coerce_dict.pyx",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/28939",
    "user": "https://github.com/jaapspies"
}
```
Testing sage-9.0 on a Raspberry Pi 4B with Raspbian Buster


```
sage -t --long src/sage/structure/coerce_dict.pyx
**********************************************************************
File "src/sage/structure/coerce_dict.pyx", line 49, in sage.structure.coerce_dict
Failed example:
    len(LE)
Expected:
    1
Got:
    50
**********************************************************************
1 item had failures:
   1 of   9 in sage.structure.coerce_dict
    [296 tests, 1 failure, 91.00 s]
----------------------------------------------------------------------
sage -t --long src/sage/structure/coerce_dict.pyx  # 1 doctest failed
----------------------------------------------------------------------
Total time for all tests: 91.3 seconds
    cpu time: 90.7 seconds
    cumulative wall time: 91.0 seconds

```

Depends on #28941

**Keywords:** raspberry

Issue created by migration from https://trac.sagemath.org/ticket/28939





---

archive/issue_comments_454788.json:
```json
{
    "body": "<a id='comment:1'></a>\nMoving tickets to milestone sage-9.2 based on a review of last modification date, branch status, and severity.",
    "created_at": "2020-05-01T04:28:42Z",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28939#issuecomment-454788",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:1'></a>
Moving tickets to milestone sage-9.2 based on a review of last modification date, branch status, and severity.



---

archive/issue_events_259282.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-01T04:28:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28939#event-259282"
}
```



---

archive/issue_events_259283.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-01T04:28:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28939#event-259283"
}
```



---

archive/issue_comments_454789.json:
```json
{
    "body": "**Changing keywords** from \"\" to \"raspberry\".",
    "created_at": "2020-05-13T11:21:27Z",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28939#issuecomment-454789",
    "user": "https://github.com/fchapoton"
}
```

**Changing keywords** from "" to "raspberry".



---

archive/issue_comments_454790.json:
```json
{
    "body": "**Dependencies:** #28941",
    "created_at": "2020-08-31T22:05:37Z",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28939#issuecomment-454790",
    "user": "https://github.com/dimpase"
}
```

**Dependencies:** #28941



---

archive/issue_comments_454791.json:
```json
{
    "body": "**Changing author** from \"jsp\" to \"\".",
    "created_at": "2020-08-31T22:05:37Z",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28939#issuecomment-454791",
    "user": "https://github.com/dimpase"
}
```

**Changing author** from "jsp" to "".



---

archive/issue_comments_454792.json:
```json
{
    "body": "<a id='comment:3'></a>\nThe reason for this error is handled on #28941",
    "created_at": "2020-08-31T22:05:37Z",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28939#issuecomment-454792",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:3'></a>
The reason for this error is handled on #28941



---

archive/issue_events_259284.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28939#event-259284"
}
```



---

archive/issue_events_259285.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28939#event-259285"
}
```



---

archive/issue_comments_454793.json:
```json
{
    "body": "<a id='comment:5'></a>\nMoving to 9.4, as 9.3 has been released.",
    "created_at": "2021-05-10T17:42:09Z",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28939#issuecomment-454793",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:5'></a>
Moving to 9.4, as 9.3 has been released.



---

archive/issue_events_259286.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-10T17:42:09Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28939#event-259286"
}
```



---

archive/issue_events_259287.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-10T17:42:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28939#event-259287"
}
```



---

archive/issue_events_259288.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28939#event-259288"
}
```



---

archive/issue_events_259289.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28939#event-259289"
}
```



---

archive/issue_events_259290.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T20:12:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28939#event-259290"
}
```



---

archive/issue_events_259291.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T20:12:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28939#event-259291"
}
```



---

archive/issue_events_259292.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28939#event-259292"
}
```



---

archive/issue_events_259293.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28939#event-259293"
}
```



---

archive/issue_events_259294.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28939#event-259294"
}
```



---

archive/issue_events_259295.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28939",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28939#event-259295"
}
```

# Issue 28097: Allow quotients of modabvar to return just the quotient

archive/issues_027860.json:
```json
{
    "body": "Let\n\n```\nA = J0(23)\nC = A.cuspidal_subgroup()\n```\n\nI want to compute the quotient of `A` by `C`. Running `A / C` or `A.quotient(C)` returns `(B, phi)`, where `B` is the quotient and `phi` is a map to the quotient. This ticket adds a `map_to_quotient` flag and make these equivalent\n\n```\nB = A.quotient(C)[0]\nB, _ = A.quotient(C)\nB = A.quotient(C, map_to_quotient=False)\n```\n\nI like the last one the most which is the reason for this ticket.\n\n**Keywords:** modabvar, quotient\n\n**Branch:** [u/klui/allow_quotients_in_modabvar_to_return_just_the_quotient](https://github.com/sagemath/sagetrac-mirror/tree/u/klui/allow_quotients_in_modabvar_to_return_just_the_quotient)\n\n**Commit:** [b94827ff21c5ae78a1b0c00fbdcaf63c0a3ff097](https://github.com/sagemath/sagetrac-mirror/commit/b94827ff21c5ae78a1b0c00fbdcaf63c0a3ff097)\n\n**Author:** Kevin Lui\n\nIssue created by migration from https://trac.sagemath.org/ticket/28097\n\n",
    "created_at": "2019-07-02T17:52:11Z",
    "labels": [
        "component: modular forms",
        "minor",
        "enhancement",
        "needs work"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "title": "Allow quotients of modabvar to return just the quotient",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/28097",
    "user": "https://github.com/kevinywlui"
}
```
Let

```
A = J0(23)
C = A.cuspidal_subgroup()
```

I want to compute the quotient of `A` by `C`. Running `A / C` or `A.quotient(C)` returns `(B, phi)`, where `B` is the quotient and `phi` is a map to the quotient. This ticket adds a `map_to_quotient` flag and make these equivalent

```
B = A.quotient(C)[0]
B, _ = A.quotient(C)
B = A.quotient(C, map_to_quotient=False)
```

I like the last one the most which is the reason for this ticket.

**Keywords:** modabvar, quotient

**Branch:** [u/klui/allow_quotients_in_modabvar_to_return_just_the_quotient](https://github.com/sagemath/sagetrac-mirror/tree/u/klui/allow_quotients_in_modabvar_to_return_just_the_quotient)

**Commit:** [b94827ff21c5ae78a1b0c00fbdcaf63c0a3ff097](https://github.com/sagemath/sagetrac-mirror/commit/b94827ff21c5ae78a1b0c00fbdcaf63c0a3ff097)

**Author:** Kevin Lui

Issue created by migration from https://trac.sagemath.org/ticket/28097





---

archive/issue_comments_439947.json:
```json
{
    "body": "**Branch:** [u/klui/allow_quotients_in_modabvar_to_return_just_the_quotient](https://github.com/sagemath/sagetrac-mirror/tree/u/klui/allow_quotients_in_modabvar_to_return_just_the_quotient)",
    "created_at": "2019-07-02T17:57:17Z",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28097#issuecomment-439947",
    "user": "https://github.com/kevinywlui"
}
```

**Branch:** [u/klui/allow_quotients_in_modabvar_to_return_just_the_quotient](https://github.com/sagemath/sagetrac-mirror/tree/u/klui/allow_quotients_in_modabvar_to_return_just_the_quotient)



---

archive/issue_comments_439948.json:
```json
{
    "body": "**Commit:** [b94827ff21c5ae78a1b0c00fbdcaf63c0a3ff097](https://github.com/sagemath/sagetrac-mirror/commit/b94827ff21c5ae78a1b0c00fbdcaf63c0a3ff097)",
    "created_at": "2019-07-06T17:10:11Z",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28097#issuecomment-439948",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

**Commit:** [b94827ff21c5ae78a1b0c00fbdcaf63c0a3ff097](https://github.com/sagemath/sagetrac-mirror/commit/b94827ff21c5ae78a1b0c00fbdcaf63c0a3ff097)



---

archive/issue_comments_439949.json:
```json
{
    "body": "<a id='comment:2'></a>\n**Branch pushed to git repo; I updated commit sha1. This was a forced push.** **New commits:**\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b94827ff21c5ae78a1b0c00fbdcaf63c0a3ff097\">b94827f</a></td><td><code>Allow just the quotient to be returned</code></td></tr></table>\n",
    "created_at": "2019-07-06T17:10:11Z",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28097#issuecomment-439949",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:2'></a>
**Branch pushed to git repo; I updated commit sha1. This was a forced push.** **New commits:**
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b94827ff21c5ae78a1b0c00fbdcaf63c0a3ff097">b94827f</a></td><td><code>Allow just the quotient to be returned</code></td></tr></table>




---

archive/issue_events_250362.json:
```json
{
    "actor": "https://github.com/kevinywlui",
    "created_at": "2019-07-06T17:23:56Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "rename": {
        "from": "Allow quotients in modabvar to return just the quotient",
        "to": "Allow quotients of modabvar to return just the quotient"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250362"
}
```



---

archive/issue_events_250363.json:
```json
{
    "actor": "https://github.com/kevinywlui",
    "created_at": "2019-07-06T17:23:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250363"
}
```



---

archive/issue_comments_439950.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1,16 @@\n+Let\n \n+```\n+A = J0(23)\n+C = A.cuspidal_subgroup()\n+```\n+\n+I want to compute the quotient of `A` by `C`. Running `A / C` or `A.quotient(C)` returns `(B, phi)`, where `B` is the quotient and `phi` is a map to the quotient. This ticket adds a `map_to_quotient` flag and make these equivalent\n+\n+```\n+B = A.quotient(C)[0]\n+B, _ = A.quotient(C)\n+B = A.quotient(C, map_to_quotient=False)\n+```\n+\n+I like the last one the most which is the reason for this ticket.\n``````\n",
    "created_at": "2019-07-06T17:23:56Z",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28097#issuecomment-439950",
    "user": "https://github.com/kevinywlui"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1,16 @@
+Let
 
+```
+A = J0(23)
+C = A.cuspidal_subgroup()
+```
+
+I want to compute the quotient of `A` by `C`. Running `A / C` or `A.quotient(C)` returns `(B, phi)`, where `B` is the quotient and `phi` is a map to the quotient. This ticket adds a `map_to_quotient` flag and make these equivalent
+
+```
+B = A.quotient(C)[0]
+B, _ = A.quotient(C)
+B = A.quotient(C, map_to_quotient=False)
+```
+
+I like the last one the most which is the reason for this ticket.
``````




---

archive/issue_comments_439951.json:
```json
{
    "body": "**Author:** Kevin Lui",
    "created_at": "2019-07-06T21:14:25Z",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28097#issuecomment-439951",
    "user": "https://github.com/kevinywlui"
}
```

**Author:** Kevin Lui



---

archive/issue_comments_439952.json:
```json
{
    "body": "<a id='comment:5'></a>\nTicket retargeted after milestone closed",
    "created_at": "2019-12-30T14:48:17Z",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28097#issuecomment-439952",
    "user": "https://github.com/embray"
}
```

<a id='comment:5'></a>
Ticket retargeted after milestone closed



---

archive/issue_events_250364.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "milestone": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250364"
}
```



---

archive/issue_events_250365.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250365"
}
```



---

archive/issue_events_250366.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250366"
}
```



---

archive/issue_events_250367.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250367"
}
```



---

archive/issue_comments_439953.json:
```json
{
    "body": "<a id='comment:6'></a>\nBatch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.",
    "created_at": "2020-04-14T19:41:51Z",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28097#issuecomment-439953",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:6'></a>
Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.



---

archive/issue_events_250368.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250368"
}
```



---

archive/issue_events_250369.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250369"
}
```



---

archive/issue_events_250370.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-01-31T08:34:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250370"
}
```



---

archive/issue_events_250371.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-01-31T08:34:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250371"
}
```



---

archive/issue_comments_439954.json:
```json
{
    "body": "<a id='comment:8'></a>\nred branch => needs work",
    "created_at": "2021-01-31T08:34:22Z",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28097#issuecomment-439954",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:8'></a>
red branch => needs work



---

archive/issue_comments_439955.json:
```json
{
    "body": "<a id='comment:9'></a>\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-03-15T22:07:04Z",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28097#issuecomment-439955",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:9'></a>
Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_250372.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-15T22:07:04Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250372"
}
```



---

archive/issue_events_250373.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-15T22:07:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250373"
}
```



---

archive/issue_comments_439956.json:
```json
{
    "body": "<a id='comment:10'></a>\nSetting a new milestone for this ticket based on a cursory review.",
    "created_at": "2021-07-19T00:44:56Z",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28097#issuecomment-439956",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:10'></a>
Setting a new milestone for this ticket based on a cursory review.



---

archive/issue_events_250374.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250374"
}
```



---

archive/issue_events_250375.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250375"
}
```



---

archive/issue_events_250376.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:24:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250376"
}
```



---

archive/issue_events_250377.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:24:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250377"
}
```



---

archive/issue_events_250378.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:27:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250378"
}
```



---

archive/issue_events_250379.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:27:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250379"
}
```



---

archive/issue_events_250380.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250380"
}
```



---

archive/issue_events_250381.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28097",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28097#event-250381"
}
```

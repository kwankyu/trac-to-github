# Issue 19942: "abelian" vs. "Abelian"

archive/issues_019705.json:
```json
{
    "body": "It needs a little work to change \"abelian\" to \"Abelian\", as some of the strings are generated. This is kind of spin-off from #19753.\n\n\nIssue created by migration from https://trac.sagemath.org/ticket/19942\n\n",
    "closed_at": "2016-02-23T23:00:14Z",
    "created_at": "2016-01-22T16:39:06Z",
    "labels": [
        "component: misc",
        "minor",
        "bug",
        "wontfix"
    ],
    "title": "\"abelian\" vs. \"Abelian\"",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/19942",
    "user": "https://github.com/jm58660"
}
```
It needs a little work to change "abelian" to "Abelian", as some of the strings are generated. This is kind of spin-off from #19753.


Issue created by migration from https://trac.sagemath.org/ticket/19942





---

archive/issue_comments_288028.json:
```json
{
    "body": "<a id='comment:1'></a>\nThere is no compelling reason to change \"abelian\" to \"Abelian\". I think this should be closed.",
    "created_at": "2016-01-22T17:10:51Z",
    "issue": "https://github.com/sagemath/sage/issues/19942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19942#issuecomment-288028",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:1'></a>
There is no compelling reason to change "abelian" to "Abelian". I think this should be closed.



---

archive/issue_comments_288029.json:
```json
{
    "body": "<a id='comment:2'></a>\nReplying to [@jhpalmieri](#comment%3A1):\n> There is no compelling reason to change \"abelian\" to \"Abelian\". I think this should be closed.\n\nNot a big thing, but this would be consistent with for example #19788. At least Wikipedia uses always caps for Abelian, Hamiltonian etc.\n\n(But of course english is not my own language, so I let others to decide this.)",
    "created_at": "2016-01-22T17:15:41Z",
    "issue": "https://github.com/sagemath/sage/issues/19942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19942#issuecomment-288029",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:2'></a>
Replying to [@jhpalmieri](#comment%3A1):
> There is no compelling reason to change "abelian" to "Abelian". I think this should be closed.

Not a big thing, but this would be consistent with for example #19788. At least Wikipedia uses always caps for Abelian, Hamiltonian etc.

(But of course english is not my own language, so I let others to decide this.)



---

archive/issue_comments_288030.json:
```json
{
    "body": "<a id='comment:3'></a>\nReplying to [@jm58660](#comment%3A2):\n> Replying to [@jhpalmieri](#comment%3A1):\n> > There is no compelling reason to change \"abelian\" to \"Abelian\". I think this should be closed.\n\n> \n> Not a big thing, but this would be consistent with for example #19788. At least Wikipedia uses always caps for Abelian, Hamiltonian etc.\n\nSee https://en.wikipedia.org/wiki/Abelian_group#A_note_on_the_typography.\n\nThat is, I don't think there is consistency in the mathematical world for the capitalization, or not, of \"abelian\".",
    "created_at": "2016-01-22T18:13:54Z",
    "issue": "https://github.com/sagemath/sage/issues/19942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19942#issuecomment-288030",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:3'></a>
Replying to [@jm58660](#comment%3A2):
> Replying to [@jhpalmieri](#comment%3A1):
> > There is no compelling reason to change "abelian" to "Abelian". I think this should be closed.

> 
> Not a big thing, but this would be consistent with for example #19788. At least Wikipedia uses always caps for Abelian, Hamiltonian etc.

See https://en.wikipedia.org/wiki/Abelian_group#A_note_on_the_typography.

That is, I don't think there is consistency in the mathematical world for the capitalization, or not, of "abelian".



---

archive/issue_comments_288031.json:
```json
{
    "body": "<a id='comment:4'></a>\nSomeone once joked to me that Abel has received the greatest prize in mathematics, a lowercase letter. It's fairly ubiquitous to use \"abelian\", so I would suggest closing this ticket.",
    "created_at": "2016-01-22T19:22:10Z",
    "issue": "https://github.com/sagemath/sage/issues/19942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19942#issuecomment-288031",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'></a>
Someone once joked to me that Abel has received the greatest prize in mathematics, a lowercase letter. It's fairly ubiquitous to use "abelian", so I would suggest closing this ticket.



---

archive/issue_events_180624.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-01-22T20:46:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19942",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19942#event-180624"
}
```



---

archive/issue_events_180625.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-01-22T20:46:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/19942",
    "milestone": "sage-7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19942#event-180625"
}
```



---

archive/issue_comments_288032.json:
```json
{
    "body": "<a id='comment:5'></a>\nReplying to [@tscrim](#comment%3A4):\n> Someone once joked to me that Abel has received the greatest prize in mathematics, a lowercase letter. It's fairly ubiquitous to use \"abelian\", so I would suggest closing this ticket.\n\nOK, please mark positive review. (Or actually, Volker will decide when closing this.)",
    "created_at": "2016-01-22T20:46:38Z",
    "issue": "https://github.com/sagemath/sage/issues/19942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19942#issuecomment-288032",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:5'></a>
Replying to [@tscrim](#comment%3A4):
> Someone once joked to me that Abel has received the greatest prize in mathematics, a lowercase letter. It's fairly ubiquitous to use "abelian", so I would suggest closing this ticket.

OK, please mark positive review. (Or actually, Volker will decide when closing this.)



---

archive/issue_events_180626.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2016-01-23T16:19:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19942",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19942#event-180626"
}
```



---

archive/issue_events_180627.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2016-01-23T16:19:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19942",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19942#event-180627"
}
```



---

archive/issue_events_180628.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-02-23T23:00:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19942",
    "label": "wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19942#event-180628"
}
```



---

archive/issue_events_180629.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-02-23T23:00:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19942",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19942#event-180629"
}
```



---

archive/issue_events_180630.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-02-23T23:00:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19942",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19942#event-180630"
}
```

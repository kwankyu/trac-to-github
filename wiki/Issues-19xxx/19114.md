# Issue 19114: still Mercurial

archive/issues_018877.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nThere is still the word \"Mercurial\" in the webpage\n\nhttp://www.sagemath.org/development.html\n\nComponent: **website/wiki**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19114_\n\n",
    "closed_at": "2015-09-12T14:05:15Z",
    "created_at": "2015-08-31T09:05:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20website%20/%20wiki",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "still Mercurial",
    "type": "issue",
    "updated_at": "2015-09-12T14:05:15Z",
    "url": "https://github.com/sagemath/sage/issues/19114",
    "user": "https://github.com/fchapoton"
}
```
<div id="comment:0"></div>

There is still the word "Mercurial" in the webpage

http://www.sagemath.org/development.html

Component: **website/wiki**

_Issue created by migration from https://trac.sagemath.org/ticket/19114_





---

archive/issue_events_269003.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-08-31T09:05:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19114",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19114#event-269003"
}
```



---

archive/issue_events_269004.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-08-31T09:05:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19114",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20website%20/%20wiki",
    "label_color": "0075ca",
    "label_name": "c: website / wiki",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19114#event-269004"
}
```



---

archive/issue_events_269005.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-08-31T09:05:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19114",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19114#event-269005"
}
```



---

archive/issue_events_269006.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-08-31T09:05:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19114",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19114#event-269006"
}
```



---

archive/issue_events_269007.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-08-31T09:42:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19114#event-269007"
}
```



---

archive/issue_comments_270007.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nI created a pull request on the github website repo.\n\nhttps://github.com/sagemath/website/pulls",
    "created_at": "2015-08-31T09:42:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19114#issuecomment-270007",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">comment:1</div>

I created a pull request on the github website repo.

https://github.com/sagemath/website/pulls



---

archive/issue_events_269008.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-08-31T09:42:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/19114",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19114#event-269008"
}
```



---

archive/issue_events_269009.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-08-31T09:43:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19114#event-269009"
}
```



---

archive/issue_events_269010.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-08-31T09:43:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19114",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19114#event-269010"
}
```



---

archive/issue_events_269011.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-12T14:05:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19114",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19114#event-269011"
}
```



---

archive/issue_events_269012.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-12T14:05:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19114",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19114#event-269012"
}
```

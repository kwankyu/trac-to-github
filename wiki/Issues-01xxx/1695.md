# Issue 1695: on osx 10.4 missing libintl.3.dylib in 2.9.2 dmg

archive/issues_001695.json:
```json
{
    "body": "The sage-2.9.2 in the osx 10.4 dmg failed to work because of missing libintl.\nThis failure occurred when starting the notebook.\nPutting  http://sagemath.org/SAGEbin/apple_osx/intel/10.4-extra_files/libintl.3.dylib\nin local/lib fixed it\n\n\n\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/1695\n\n",
    "closed_at": "2008-03-16T20:33:51Z",
    "created_at": "2008-01-05T23:21:03Z",
    "labels": [
        "component: distribution",
        "bug",
        "invalid"
    ],
    "title": "on osx 10.4 missing libintl.3.dylib in 2.9.2 dmg",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/1695",
    "user": "https://trac.sagemath.org/admin/accounts/users/jkantor"
}
```
The sage-2.9.2 in the osx 10.4 dmg failed to work because of missing libintl.
This failure occurred when starting the notebook.
Putting  http://sagemath.org/SAGEbin/apple_osx/intel/10.4-extra_files/libintl.3.dylib
in local/lib fixed it




**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/1695





---

archive/issue_events_007839.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/boothby",
    "created_at": "2008-03-16T19:07:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1695",
    "label": "component: notebook",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1695#event-7839"
}
```



---

archive/issue_events_007840.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/boothby",
    "created_at": "2008-03-16T19:07:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1695",
    "label": "component: distribution",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1695#event-7840"
}
```



---

archive/issue_comments_008285.json:
```json
{
    "body": "**Changing assignee** from boothby to mabshoff.",
    "created_at": "2008-03-16T19:07:31Z",
    "issue": "https://github.com/sagemath/sage/issues/1695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1695#issuecomment-8285",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```

**Changing assignee** from boothby to mabshoff.



---

archive/issue_comments_008286.json:
```json
{
    "body": "<a id='comment:2'></a>\nClosed because I don't build sage on the machine that led to this problem anymore.  In fact that machine physically no longer exists.",
    "created_at": "2008-03-16T20:33:51Z",
    "issue": "https://github.com/sagemath/sage/issues/1695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1695#issuecomment-8286",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>
Closed because I don't build sage on the machine that led to this problem anymore.  In fact that machine physically no longer exists.



---

archive/issue_events_007841.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-03-16T20:33:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1695",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1695#event-7841"
}
```



---

archive/issue_events_007842.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-16T20:52:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/1695",
    "milestone": "sage-2.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1695#event-7842"
}
```

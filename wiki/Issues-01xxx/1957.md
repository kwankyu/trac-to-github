# Issue 1957: Solaris 9 fixes for libfplll

archive/issues_001957.json:
```json
{
    "body": "The spkg at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-2.10.1/rc2/libfplll-2.1.6-20071129.p0.spkg\n\nbuilds fine on Solaris 9, OSX and Linux.\n\nCheers,\n\nMichael\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/1957\n\n",
    "closed_at": "2008-01-28T05:06:02Z",
    "created_at": "2008-01-28T03:20:09Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.1",
    "title": "Solaris 9 fixes for libfplll",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/1957",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
The spkg at

http://sage.math.washington.edu/home/mabshoff/release-cycles-2.10.1/rc2/libfplll-2.1.6-20071129.p0.spkg

builds fine on Solaris 9, OSX and Linux.

Cheers,

Michael

**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/1957





---

archive/issue_comments_009809.json:
```json
{
    "body": "**Changing assignee** from @williamstein to mabshoff.",
    "created_at": "2008-01-28T03:22:50Z",
    "issue": "https://github.com/sagemath/sage/issues/1957",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1957#issuecomment-9809",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

**Changing assignee** from @williamstein to mabshoff.



---

archive/issue_events_009379.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-28T03:22:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1957",
    "label": "component: algebraic geometry",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1957#event-9379"
}
```



---

archive/issue_events_009380.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-28T03:22:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1957",
    "label": "component: packages",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1957#event-9380"
}
```



---

archive/issue_events_009381.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-28T03:22:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1957",
    "milestone": "sage-2.10.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1957#event-9381"
}
```



---

archive/issue_events_009382.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/jkantor",
    "created_at": "2008-01-28T04:56:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1957",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1957#event-9382"
}
```



---

archive/issue_comments_009810.json:
```json
{
    "body": "<a id='comment:2'></a>\nSpkg compiles fine.",
    "created_at": "2008-01-28T04:56:37Z",
    "issue": "https://github.com/sagemath/sage/issues/1957",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1957#issuecomment-9810",
    "user": "https://trac.sagemath.org/admin/accounts/users/jkantor"
}
```

<a id='comment:2'></a>
Spkg compiles fine.



---

archive/issue_events_009383.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-28T05:06:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1957",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1957#event-9383"
}
```



---

archive/issue_events_009384.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-28T05:06:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1957",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1957#event-9384"
}
```



---

archive/issue_comments_009811.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.1.rc2",
    "created_at": "2008-01-28T05:06:02Z",
    "issue": "https://github.com/sagemath/sage/issues/1957",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1957#issuecomment-9811",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.1.rc2

# Issue 1640: missing documentation Elliptic Curve - heegner_discriminants

archive/issues_001640.json:
```json
{
    "body": "Documentation missing:\n\n```\nE = EllipticCurve('5077a')\nE.heegner_discriminants?\n```\n\n\nsays\n\n\n```\nFile:        /opt/sage/local/lib/python2.5/site-packages/sage/schemes/elliptic_curves/ell_rational_field.py\nType:        <type 'instancemethod'>\nDefinition:  E.heegner_discriminants(bound)\nDocstring: \nx.__init__(...) initializes x; see x.__class__.__doc__ for signature\n```\n\n\nbut\n\n\n```\nE.heegner_index?\n```\n\n\nis ok.\n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/1640\n\n",
    "closed_at": "2008-01-16T17:05:16Z",
    "created_at": "2007-12-30T14:27:11Z",
    "labels": [
        "component: algebraic geometry",
        "trivial",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10",
    "title": "missing documentation Elliptic Curve - heegner_discriminants",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/1640",
    "user": "https://github.com/haraldschilly"
}
```
Documentation missing:

```
E = EllipticCurve('5077a')
E.heegner_discriminants?
```


says


```
File:        /opt/sage/local/lib/python2.5/site-packages/sage/schemes/elliptic_curves/ell_rational_field.py
Type:        <type 'instancemethod'>
Definition:  E.heegner_discriminants(bound)
Docstring: 
x.__init__(...) initializes x; see x.__class__.__doc__ for signature
```


but


```
E.heegner_index?
```


is ok.

**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/1640





---

archive/issue_events_007646.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-12-30T15:57:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1640",
    "milestone": "sage-2.9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1640#event-7646"
}
```



---

archive/attachments_001304.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "1640.patch",
    "asset_url": "tarball://root/attachments/ticket1640/1640.patch",
    "created_at": "2008-01-08T11:28:10Z",
    "issue": "https://github.com/sagemath/sage/issues/1640",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket1640/1640.patch",
    "user": "https://github.com/aghitza"
}
```



---

archive/issue_comments_008026.json:
```json
{
    "body": "<a id='comment:2'></a>\n**Attachment:** [1640.patch](https://github.com/sagemath/sage/files/ticket1640/1640.patch)",
    "created_at": "2008-01-08T11:28:10Z",
    "issue": "https://github.com/sagemath/sage/issues/1640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1640#issuecomment-8026",
    "user": "https://github.com/aghitza"
}
```

<a id='comment:2'></a>
**Attachment:** [1640.patch](https://github.com/sagemath/sage/files/ticket1640/1640.patch)



---

archive/issue_events_007647.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-01-08T11:28:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1640",
    "label": "component: documentation",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1640#event-7647"
}
```



---

archive/issue_events_007648.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-01-08T11:28:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1640",
    "label": "component: algebraic geometry",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1640#event-7648"
}
```



---

archive/issue_comments_008027.json:
```json
{
    "body": "**Assignee:** @williamstein",
    "created_at": "2008-01-08T11:28:10Z",
    "issue": "https://github.com/sagemath/sage/issues/1640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1640#issuecomment-8027",
    "user": "https://github.com/aghitza"
}
```

**Assignee:** @williamstein



---

archive/issue_events_007649.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-01-16T17:02:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1640",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1640#event-7649"
}
```



---

archive/issue_comments_008028.json:
```json
{
    "body": "<a id='comment:3'></a>\nLooks very good!",
    "created_at": "2008-01-16T17:02:01Z",
    "issue": "https://github.com/sagemath/sage/issues/1640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1640#issuecomment-8028",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
Looks very good!



---

archive/issue_events_007650.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-16T17:05:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1640",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1640#event-7650"
}
```



---

archive/issue_events_007651.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-16T17:05:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1640",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1640#event-7651"
}
```



---

archive/issue_comments_008029.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 2.10.alpha4",
    "created_at": "2008-01-16T17:05:16Z",
    "issue": "https://github.com/sagemath/sage/issues/1640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1640#issuecomment-8029",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 2.10.alpha4

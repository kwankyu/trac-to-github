# Issue 1794: Gram-Schmidt typo

archive/issues_001794.json:
```json
{
    "body": "Sage calls Gram-Schmidt Gramm-Schmidt in some places (including function names). The attached patch fixes the ones I found.\n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/1794\n\n",
    "closed_at": "2008-01-21T06:08:06Z",
    "created_at": "2008-01-16T17:59:01Z",
    "labels": [
        "component: linear algebra",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.1",
    "title": "Gram-Schmidt typo",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/1794",
    "user": "https://github.com/wjp"
}
```
Sage calls Gram-Schmidt Gramm-Schmidt in some places (including function names). The attached patch fixes the ones I found.

**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/1794





---

archive/attachments_001444.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "8014.patch",
    "asset_url": "tarball://root/attachments/ticket1794/8014.patch",
    "created_at": "2008-01-16T18:00:29Z",
    "issue": "https://github.com/sagemath/sage/issues/1794",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket1794/8014.patch",
    "user": "https://github.com/wjp"
}
```



---

archive/issue_comments_008743.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [8014.patch](https://github.com/sagemath/sage/files/ticket1794/8014.patch)",
    "created_at": "2008-01-16T18:00:29Z",
    "issue": "https://github.com/sagemath/sage/issues/1794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1794#issuecomment-8743",
    "user": "https://github.com/wjp"
}
```

<a id='comment:1'></a>
**Attachment:** [8014.patch](https://github.com/sagemath/sage/files/ticket1794/8014.patch)



---

archive/issue_events_008358.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2008-01-20T06:55:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1794",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1794#event-8358"
}
```



---

archive/issue_comments_008744.json:
```json
{
    "body": "<a id='comment:2'></a>\nApply this, but there may be other instances in libs/ntl/ntl_mat_ZZ.pyx.",
    "created_at": "2008-01-20T06:55:09Z",
    "issue": "https://github.com/sagemath/sage/issues/1794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1794#issuecomment-8744",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:2'></a>
Apply this, but there may be other instances in libs/ntl/ntl_mat_ZZ.pyx.



---

archive/issue_comments_008745.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.1.alpha1",
    "created_at": "2008-01-21T06:08:06Z",
    "issue": "https://github.com/sagemath/sage/issues/1794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1794#issuecomment-8745",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.1.alpha1



---

archive/issue_events_008359.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-21T06:08:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1794",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1794#event-8359"
}
```



---

archive/issue_events_008360.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-21T06:08:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1794",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1794#event-8360"
}
```

# Issue 1699: Fix matplotlib compilation on Linux/Itanium

archive/issues_001699.json:
```json
{
    "body": "William did fix the build of matplotlib on Linux/Itanium. The spkg can be found at:\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-2.9.3/rc0/matplotlib-0.91.1.p1.spkg\n\nCheers,\n\nMichael\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/1699\n\n",
    "closed_at": "2008-01-06T13:13:57Z",
    "created_at": "2008-01-06T13:12:51Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.9.3",
    "title": "Fix matplotlib compilation on Linux/Itanium",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/1699",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
William did fix the build of matplotlib on Linux/Itanium. The spkg can be found at:

http://sage.math.washington.edu/home/mabshoff/release-cycles-2.9.3/rc0/matplotlib-0.91.1.p1.spkg

Cheers,

Michael

**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/1699





---

archive/issue_events_007855.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-06T13:13:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/1699",
    "milestone": "sage-2.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1699#event-7855"
}
```



---

archive/issue_events_007856.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-06T13:13:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1699",
    "milestone": "sage-2.9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1699#event-7856"
}
```



---

archive/issue_comments_008293.json:
```json
{
    "body": "<a id='comment:1'></a>\nWorks for me. Reassigned against 2.9.3\n\nThis is a post release ticket for the record.\n\nCheers,\n\nMichael",
    "created_at": "2008-01-06T13:13:38Z",
    "issue": "https://github.com/sagemath/sage/issues/1699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1699#issuecomment-8293",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Works for me. Reassigned against 2.9.3

This is a post release ticket for the record.

Cheers,

Michael



---

archive/issue_events_007857.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-06T13:13:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1699",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1699#event-7857"
}
```



---

archive/issue_events_007858.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-06T13:13:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1699",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1699#event-7858"
}
```



---

archive/issue_events_007859.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-06T13:13:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1699",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1699#event-7859"
}
```



---

archive/issue_comments_008294.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in 2.9.3",
    "created_at": "2008-01-06T13:13:57Z",
    "issue": "https://github.com/sagemath/sage/issues/1699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1699#issuecomment-8294",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in 2.9.3

# Issue 1709: Make experimental jmol graph plotting work

archive/issues_001709.json:
```json
{
    "body": "For example, you can do\n\n```\nsage: G = graphs.DodecahedralGraph()\nsage: G.plot3d_new()\n```\n\nThis still needs to be cleaned up, but at least now it works instead of giving confusing errors.\n\n**Assignee:** @rlmill\n\nIssue created by migration from https://trac.sagemath.org/ticket/1709\n\n",
    "closed_at": "2008-01-08T23:20:58Z",
    "created_at": "2008-01-07T09:09:18Z",
    "labels": [
        "component: graph theory",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10",
    "title": "Make experimental jmol graph plotting work",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/1709",
    "user": "https://github.com/rlmill"
}
```
For example, you can do

```
sage: G = graphs.DodecahedralGraph()
sage: G.plot3d_new()
```

This still needs to be cleaned up, but at least now it works instead of giving confusing errors.

**Assignee:** @rlmill

Issue created by migration from https://trac.sagemath.org/ticket/1709





---

archive/attachments_001363.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "jmol_graphs.patch",
    "asset_url": "tarball://root/attachments/ticket1709/jmol_graphs.patch",
    "created_at": "2008-01-08T23:05:26Z",
    "issue": "https://github.com/sagemath/sage/issues/1709",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket1709/jmol_graphs.patch",
    "user": "https://github.com/robertwb"
}
```



---

archive/issue_comments_008342.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [jmol_graphs.patch](https://github.com/sagemath/sage/files/ticket1709/jmol_graphs.patch)\n\nYep, I've been using this and it works great.",
    "created_at": "2008-01-08T23:05:26Z",
    "issue": "https://github.com/sagemath/sage/issues/1709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1709#issuecomment-8342",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:1'></a>
**Attachment:** [jmol_graphs.patch](https://github.com/sagemath/sage/files/ticket1709/jmol_graphs.patch)

Yep, I've been using this and it works great.



---

archive/issue_events_007905.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2008-01-08T23:05:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1709",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1709#event-7905"
}
```



---

archive/issue_comments_008343.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.alpha1.",
    "created_at": "2008-01-08T23:20:58Z",
    "issue": "https://github.com/sagemath/sage/issues/1709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1709#issuecomment-8343",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.alpha1.



---

archive/issue_events_007906.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-08T23:20:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1709",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1709#event-7906"
}
```



---

archive/issue_events_007907.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-08T23:20:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1709",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1709#event-7907"
}
```

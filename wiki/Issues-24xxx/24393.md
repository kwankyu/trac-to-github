# Issue 24393: WeylGroup.subgroup doesn't create a subset

archive/issues_024156.json:
```json
{
    "body": "The following code\n\n```\nW = WeylGroup(\"A2\", prefix=\"s\")\ns = W.simple_reflections()\nH = W.subgroup([s[2]])\ns[2] in list(H)\n```\n\nreturns False. One workaround is `s[2] in list(W(x) for x in H)`.\n\n**CC:**  sage-combinat\n\n**Reviewer:** Travis Scrimshaw, Simon Brandhorst\n\nIssue created by migration from https://trac.sagemath.org/ticket/24393\n\n",
    "closed_at": "2019-02-26T13:58:00Z",
    "created_at": "2017-12-16T14:24:05Z",
    "labels": [
        "component: combinatorics",
        "bug",
        "invalid"
    ],
    "title": "WeylGroup.subgroup doesn't create a subset",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/24393",
    "user": "https://github.com/vit-tucek"
}
```
The following code

```
W = WeylGroup("A2", prefix="s")
s = W.simple_reflections()
H = W.subgroup([s[2]])
s[2] in list(H)
```

returns False. One workaround is `s[2] in list(W(x) for x in H)`.

**CC:**  sage-combinat

**Reviewer:** Travis Scrimshaw, Simon Brandhorst

Issue created by migration from https://trac.sagemath.org/ticket/24393





---

archive/issue_events_218508.json:
```json
{
    "actor": "https://github.com/vit-tucek",
    "created_at": "2017-12-16T14:24:50Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/24393",
    "rename": {
        "from": "WeylGroup.subgroup is not a subset",
        "to": "WeylGroup.subgroup doesn't create a subset"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24393#event-218508"
}
```



---

archive/issue_comments_372028.json:
```json
{
    "body": "<a id='comment:2'></a>\nSeems like a duplicate of #19010",
    "created_at": "2017-12-16T15:05:11Z",
    "issue": "https://github.com/sagemath/sage/issues/24393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24393#issuecomment-372028",
    "user": "https://github.com/vit-tucek"
}
```

<a id='comment:2'></a>
Seems like a duplicate of #19010



---

archive/issue_comments_372029.json:
```json
{
    "body": "**Reviewer:** Travis Scrimshaw",
    "created_at": "2017-12-18T02:37:01Z",
    "issue": "https://github.com/sagemath/sage/issues/24393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24393#issuecomment-372029",
    "user": "https://github.com/tscrim"
}
```

**Reviewer:** Travis Scrimshaw



---

archive/issue_comments_372030.json:
```json
{
    "body": "<a id='comment:3'></a>\nI agree that this is essentially a duplicate of #19010 as the root cause is the same:\n\n```\nsage: W.has_coerce_map_from(H)\nFalse\n```\nEvery subgroup should have a coercion into its ambient group.",
    "created_at": "2017-12-18T02:37:01Z",
    "issue": "https://github.com/sagemath/sage/issues/24393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24393#issuecomment-372030",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'></a>
I agree that this is essentially a duplicate of #19010 as the root cause is the same:

```
sage: W.has_coerce_map_from(H)
False
```
Every subgroup should have a coercion into its ambient group.



---

archive/issue_events_218509.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-12-18T02:37:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24393",
    "milestone": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24393#event-218509"
}
```



---

archive/issue_events_218510.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-12-18T02:37:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24393",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24393#event-218510"
}
```



---

archive/issue_events_218511.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2018-07-03T19:55:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24393",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24393#event-218511"
}
```



---

archive/issue_events_218512.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2018-07-03T19:55:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24393",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24393#event-218512"
}
```



---

archive/issue_comments_372031.json:
```json
{
    "body": "<a id='comment:4'></a>\nindeed.",
    "created_at": "2018-07-03T19:55:59Z",
    "issue": "https://github.com/sagemath/sage/issues/24393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24393#issuecomment-372031",
    "user": "https://github.com/simonbrandhorst"
}
```

<a id='comment:4'></a>
indeed.



---

archive/issue_comments_372032.json:
```json
{
    "body": "**Changing reviewer** from \"Travis Scrimshaw\" to \"Travis Scrimshaw, Simon Brandhorst\".",
    "created_at": "2018-07-03T19:56:25Z",
    "issue": "https://github.com/sagemath/sage/issues/24393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24393#issuecomment-372032",
    "user": "https://github.com/simonbrandhorst"
}
```

**Changing reviewer** from "Travis Scrimshaw" to "Travis Scrimshaw, Simon Brandhorst".



---

archive/issue_events_218513.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-02-26T13:58:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/24393",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24393#event-218513"
}
```



---

archive/issue_events_218514.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-02-26T13:58:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/24393",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24393#event-218514"
}
```



---

archive/issue_comments_372033.json:
```json
{
    "body": "<a id='comment:6'></a>\nPresuming these are all correctly reviewed as either duplicate, invalid, or wontfix.",
    "created_at": "2019-02-26T13:58:00Z",
    "issue": "https://github.com/sagemath/sage/issues/24393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24393#issuecomment-372033",
    "user": "https://github.com/embray"
}
```

<a id='comment:6'></a>
Presuming these are all correctly reviewed as either duplicate, invalid, or wontfix.

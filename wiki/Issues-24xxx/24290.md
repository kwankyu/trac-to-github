# Issue 24290: Make simplifications in SymPy calculus on manifolds rely on SymPy only

archive/issues_024053.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\n#22801 introduced SymPy as an optional engine for symbolic calculus on manifolds. However, not all simplifications are performed by SymPy; in particular those depending on assumptions are still treated via Maxima. \n\nDepends on #22801\n\nCC:  @man74cio @rll2021 @rwst\n\nComponent: **geometry**\n\nKeywords: **manifolds calculus sympy**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/24290_\n\n",
    "created_at": "2017-11-28T08:40:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20manifolds",
        "https://github.com/sagemath/sage/labels/c%3A%20calculus"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make simplifications in SymPy calculus on manifolds rely on SymPy only",
    "type": "issue",
    "updated_at": "2022-12-29T01:40:48Z",
    "url": "https://github.com/sagemath/sage/issues/24290",
    "user": "https://github.com/egourgoulhon"
}
```
<div id="comment:0"></div>

#22801 introduced SymPy as an optional engine for symbolic calculus on manifolds. However, not all simplifications are performed by SymPy; in particular those depending on assumptions are still treated via Maxima. 

Depends on #22801

CC:  @man74cio @rll2021 @rwst

Component: **geometry**

Keywords: **manifolds calculus sympy**

_Issue created by migration from https://trac.sagemath.org/ticket/24290_





---

archive/issue_events_334216.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2017-11-28T08:40:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24290#event-334216"
}
```



---

archive/issue_events_334217.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2017-11-28T08:40:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "c: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24290#event-334217"
}
```



---

archive/issue_events_334218.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2017-11-28T08:40:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24290#event-334218"
}
```



---

archive/issue_events_334219.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2017-11-28T08:40:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24290#event-334219"
}
```



---

archive/issue_events_334220.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2017-11-28T08:40:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20manifolds",
    "label_color": "0000ff",
    "label_name": "c: manifolds",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24290#event-334220"
}
```



---

archive/issue_events_334221.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2017-11-28T08:40:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "c: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24290#event-334221"
}
```



---

archive/issue_comments_367730.json:
```json
{
    "body": "Dependencies: **#22801**",
    "created_at": "2017-11-28T08:44:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24290#issuecomment-367730",
    "user": "https://github.com/egourgoulhon"
}
```

Dependencies: **#22801**



---

archive/issue_events_334222.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2017-11-28T09:25:53Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "title_is": "Make simplifications in SymPy calculus on manifolds rely on SymPy only",
    "title_was": "Make simplifications in SymPy calculus on manifolds depend on SymPy only",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24290#event-334222"
}
```



---

archive/issue_comments_367731.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nBasically, one needs to pass Sage's `assumptions()` to SymPy and use them via `refine` (cf.  http://docs.sympy.org/latest/modules/assumptions/refine.html) or maybe via the context manager `assuming` (cf. http://docs.sympy.org/latest/modules/assumptions/assume.html).",
    "created_at": "2017-11-28T09:25:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/24290#issuecomment-367731",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:2" align="right">comment:2</div>

Basically, one needs to pass Sage's `assumptions()` to SymPy and use them via `refine` (cf.  http://docs.sympy.org/latest/modules/assumptions/refine.html) or maybe via the context manager `assuming` (cf. http://docs.sympy.org/latest/modules/assumptions/assume.html).



---

archive/issue_events_334223.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:40:48Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/24290",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/24290#event-334223"
}
```

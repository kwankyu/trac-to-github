# Issue 7556: change default rounding behavior for QQ to 'away'

archive/issues_007556.json:
```json
{
    "body": "This is inconsistent:\n\n```\nsage: (9/2).round()\n4\nsage: RDF('4.5').round()\n5\nsage: import __builtin__\nsage: __builtin__.round(float('4.5'))\n5.0\nsage: RR('4.5').round()\n5\n```\n\nIt's also inconsistent for negatives.  The simple fix is to make the default rounding direction 'away' in rational.pyx. \n\n**Assignee:** @aghitza\n\n**Author:** William Stein\n\n**Reviewer:** Mike Hansen\n\n**Merged:** sage-4.3.alpha1\n\nIssue created by migration from https://trac.sagemath.org/ticket/7556\n\n",
    "closed_at": "2009-12-01T05:09:52Z",
    "created_at": "2009-11-29T23:34:16Z",
    "labels": [
        "component: basic arithmetic",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3",
    "title": "change default rounding behavior for QQ to 'away'",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7556",
    "user": "https://github.com/williamstein"
}
```
This is inconsistent:

```
sage: (9/2).round()
4
sage: RDF('4.5').round()
5
sage: import __builtin__
sage: __builtin__.round(float('4.5'))
5.0
sage: RR('4.5').round()
5
```

It's also inconsistent for negatives.  The simple fix is to make the default rounding direction 'away' in rational.pyx. 

**Assignee:** @aghitza

**Author:** William Stein

**Reviewer:** Mike Hansen

**Merged:** sage-4.3.alpha1

Issue created by migration from https://trac.sagemath.org/ticket/7556





---

archive/issue_events_051665.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-29T23:40:47Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/7556",
    "rename": {
        "from": "change default rounding behavior for QQ to",
        "to": "change default rounding behavior for QQ to 'away'"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7556#event-51665"
}
```



---

archive/issue_events_051666.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-29T23:40:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7556",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7556#event-51666"
}
```



---

archive/attachments_009510.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_7556.patch",
    "asset_url": "tarball://root/attachments/ticket7556/trac_7556.patch",
    "created_at": "2009-11-29T23:41:25Z",
    "issue": "https://github.com/sagemath/sage/issues/7556",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7556/trac_7556.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_057657.json:
```json
{
    "body": "**Attachment:** [trac_7556.patch](https://github.com/sagemath/sage/files/ticket7556/trac_7556.patch)",
    "created_at": "2009-11-29T23:41:25Z",
    "issue": "https://github.com/sagemath/sage/issues/7556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7556#issuecomment-57657",
    "user": "https://github.com/williamstein"
}
```

**Attachment:** [trac_7556.patch](https://github.com/sagemath/sage/files/ticket7556/trac_7556.patch)



---

archive/issue_comments_057658.json:
```json
{
    "body": "**Reviewer:** Mike Hansen",
    "created_at": "2009-11-30T04:01:39Z",
    "issue": "https://github.com/sagemath/sage/issues/7556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7556#issuecomment-57658",
    "user": "https://github.com/mwhansen"
}
```

**Reviewer:** Mike Hansen



---

archive/issue_events_051667.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-30T04:01:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7556",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7556#event-51667"
}
```



---

archive/issue_events_051668.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-30T04:01:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7556",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7556#event-51668"
}
```



---

archive/issue_comments_057659.json:
```json
{
    "body": "**Author:** William Stein",
    "created_at": "2009-11-30T04:01:39Z",
    "issue": "https://github.com/sagemath/sage/issues/7556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7556#issuecomment-57659",
    "user": "https://github.com/mwhansen"
}
```

**Author:** William Stein



---

archive/issue_comments_057660.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good to me.",
    "created_at": "2009-11-30T04:01:39Z",
    "issue": "https://github.com/sagemath/sage/issues/7556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7556#issuecomment-57660",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
Looks good to me.



---

archive/issue_events_051669.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-01T05:09:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7556",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7556#event-51669"
}
```



---

archive/issue_events_051670.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-01T05:09:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7556",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7556#event-51670"
}
```



---

archive/issue_comments_057661.json:
```json
{
    "body": "**Merged:** sage-4.3.alpha1",
    "created_at": "2009-12-01T05:09:52Z",
    "issue": "https://github.com/sagemath/sage/issues/7556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7556#issuecomment-57661",
    "user": "https://github.com/mwhansen"
}
```

**Merged:** sage-4.3.alpha1

# Issue 7755: Auto-detect SageNB install path when building documentation

archive/issues_007755.json:
```json
{
    "body": "We should update the \"docbuild\" configuration so that Sphinx can locate jsMath.\n\nSee [sage-notebook](http://groups.google.com/group/sage-notebook/browse_thread/thread/9db6f5df45bc05cc), #7467.\n\n**Assignee:** mvngu\n\n**CC:**  @jhpalmieri\n\n**Author:** Mitesh Patel\n\n**Reviewer:** John Palmieri\n\n**Merged:** sage-4.3.rc2\n\nIssue created by migration from https://trac.sagemath.org/ticket/7755\n\n",
    "closed_at": "2009-12-24T07:11:50Z",
    "created_at": "2009-12-24T05:15:55Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3",
    "title": "Auto-detect SageNB install path when building documentation",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7755",
    "user": "https://github.com/qed777"
}
```
We should update the "docbuild" configuration so that Sphinx can locate jsMath.

See [sage-notebook](http://groups.google.com/group/sage-notebook/browse_thread/thread/9db6f5df45bc05cc), #7467.

**Assignee:** mvngu

**CC:**  @jhpalmieri

**Author:** Mitesh Patel

**Reviewer:** John Palmieri

**Merged:** sage-4.3.rc2

Issue created by migration from https://trac.sagemath.org/ticket/7755





---

archive/issue_comments_060044.json:
```json
{
    "body": "Auto-detect jsMath path.  Based on timdumol's comment at #7467.  sage repo.",
    "created_at": "2009-12-24T05:24:53Z",
    "issue": "https://github.com/sagemath/sage/issues/7755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7755#issuecomment-60044",
    "user": "https://github.com/qed777"
}
```

Auto-detect jsMath path.  Based on timdumol's comment at #7467.  sage repo.



---

archive/issue_events_053667.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-12-24T06:24:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7755",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7755#event-53667"
}
```



---

archive/attachments_009798.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_7755-docbuild_jsmath_path.patch",
    "asset_url": "tarball://root/attachments/ticket7755/trac_7755-docbuild_jsmath_path.patch",
    "created_at": "2009-12-24T06:24:13Z",
    "issue": "https://github.com/sagemath/sage/issues/7755",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7755/trac_7755-docbuild_jsmath_path.patch",
    "user": "https://github.com/jhpalmieri"
}
```



---

archive/issue_comments_060045.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_7755-docbuild_jsmath_path.patch](https://github.com/sagemath/sage/files/ticket7755/trac_7755-docbuild_jsmath_path.patch)\n\nLooks good.  Here's a slightly different patch to do the same thing, just making os.path.join do more of the work.  I'm committing it in your name (I just edited the patch file) and I'm giving it a positive review.",
    "created_at": "2009-12-24T06:24:13Z",
    "issue": "https://github.com/sagemath/sage/issues/7755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7755#issuecomment-60045",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_7755-docbuild_jsmath_path.patch](https://github.com/sagemath/sage/files/ticket7755/trac_7755-docbuild_jsmath_path.patch)

Looks good.  Here's a slightly different patch to do the same thing, just making os.path.join do more of the work.  I'm committing it in your name (I just edited the patch file) and I'm giving it a positive review.



---

archive/issue_comments_060046.json:
```json
{
    "body": "**Reviewer:** John Palmieri",
    "created_at": "2009-12-24T06:24:27Z",
    "issue": "https://github.com/sagemath/sage/issues/7755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7755#issuecomment-60046",
    "user": "https://github.com/jhpalmieri"
}
```

**Reviewer:** John Palmieri



---

archive/issue_events_053668.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-12-24T06:24:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7755",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7755#event-53668"
}
```



---

archive/issue_events_053669.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-12-24T06:24:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7755",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7755#event-53669"
}
```



---

archive/issue_comments_060047.json:
```json
{
    "body": "use this one instead",
    "created_at": "2009-12-24T06:24:51Z",
    "issue": "https://github.com/sagemath/sage/issues/7755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7755#issuecomment-60047",
    "user": "https://github.com/jhpalmieri"
}
```

use this one instead



---

archive/issue_events_053670.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-24T07:11:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7755",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7755#event-53670"
}
```



---

archive/issue_events_053671.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-24T07:11:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7755",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7755#event-53671"
}
```



---

archive/attachments_009799.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_7755-docbuild_jsmath_path-v2.patch",
    "asset_url": "tarball://root/attachments/ticket7755/trac_7755-docbuild_jsmath_path-v2.patch",
    "created_at": "2009-12-24T07:11:50Z",
    "issue": "https://github.com/sagemath/sage/issues/7755",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7755/trac_7755-docbuild_jsmath_path-v2.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_060048.json:
```json
{
    "body": "<a id='comment:3'></a>\n**Attachment:** [trac_7755-docbuild_jsmath_path-v2.patch](https://github.com/sagemath/sage/files/ticket7755/trac_7755-docbuild_jsmath_path-v2.patch)\n\nMerged into sage-4.3.rc2.",
    "created_at": "2009-12-24T07:11:50Z",
    "issue": "https://github.com/sagemath/sage/issues/7755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7755#issuecomment-60048",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
**Attachment:** [trac_7755-docbuild_jsmath_path-v2.patch](https://github.com/sagemath/sage/files/ticket7755/trac_7755-docbuild_jsmath_path-v2.patch)

Merged into sage-4.3.rc2.



---

archive/issue_comments_060049.json:
```json
{
    "body": "**Merged:** sage-4.3.rc2",
    "created_at": "2009-12-26T21:19:16Z",
    "issue": "https://github.com/sagemath/sage/issues/7755",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7755#issuecomment-60049",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Merged:** sage-4.3.rc2

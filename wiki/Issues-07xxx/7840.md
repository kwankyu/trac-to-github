# Issue 7840: include sagenb-0.4.9 in sage-4.3.1.alpha0

archive/issues_007840.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\n\n\nCC:  @mwhansen\n\nComponent: **notebook**\n\nAuthor: **William Stein**\n\nReviewer: **Mike Hansen**\n\nMerged: **sage-4.3.1.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7840_\n\n",
    "closed_at": "2010-01-04T07:14:50Z",
    "created_at": "2010-01-04T07:11:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "include sagenb-0.4.9 in sage-4.3.1.alpha0",
    "type": "issue",
    "updated_at": "2010-01-04T07:14:50Z",
    "url": "https://github.com/sagemath/sage/issues/7840",
    "user": "https://github.com/williamstein"
}
```
<div id="comment:0"></div>



CC:  @mwhansen

Component: **notebook**

Author: **William Stein**

Reviewer: **Mike Hansen**

Merged: **sage-4.3.1.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/7840_





---

archive/issue_events_096293.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-01-04T07:11:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7840",
    "milestone_number": null,
    "milestone_title": "sage-4.3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7840#event-96293"
}
```



---

archive/issue_events_096294.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-01-04T07:11:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7840",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7840#event-96294"
}
```



---

archive/issue_events_096295.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-01-04T07:11:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7840",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7840#event-96295"
}
```



---

archive/issue_events_096296.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-01-04T07:11:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7840",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7840#event-96296"
}
```



---

archive/issue_events_096297.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-01-04T07:11:16Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7840",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7840#event-96297"
}
```



---

archive/issue_comments_059440.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThe new spkg is here:\n\n   http://boxen.math.washington.edu/home/wstein/patches/sagenb/sagenb-0.4.9.spkg",
    "created_at": "2010-01-04T07:13:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7840",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7840#issuecomment-59440",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>

The new spkg is here:

   http://boxen.math.washington.edu/home/wstein/patches/sagenb/sagenb-0.4.9.spkg



---

archive/issue_comments_059441.json:
```json
{
    "body": "Reviewer: **Mike Hansen**",
    "created_at": "2010-01-04T07:14:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7840",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7840#issuecomment-59441",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **Mike Hansen**



---

archive/issue_events_096298.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-01-04T07:14:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7840",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7840#event-96298"
}
```



---

archive/issue_comments_059442.json:
```json
{
    "body": "Author: **William Stein**",
    "created_at": "2010-01-04T07:14:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7840",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7840#issuecomment-59442",
    "user": "https://github.com/mwhansen"
}
```

Author: **William Stein**



---

archive/issue_comments_059443.json:
```json
{
    "body": "Merged: **sage-4.3.1.alpha0**",
    "created_at": "2010-01-04T07:14:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7840",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7840#issuecomment-59443",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.3.1.alpha0**

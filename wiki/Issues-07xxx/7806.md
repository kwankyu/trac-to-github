# Issue 7806: Clarify what a MIP variable is

archive/issues_007806.json:
```json
{
    "body": "A MIP variable is more a dictionnary of variables than a \"variable\". This should be clarified both in the LP Constructions document and in the functions' docstrings.\n\nNathann\n\n**Assignee:** mvngu\n\nIssue created by migration from https://trac.sagemath.org/ticket/7806\n\n",
    "closed_at": "2010-10-09T08:28:55Z",
    "created_at": "2010-01-01T14:39:34Z",
    "labels": [
        "component: documentation",
        "bug",
        "invalid"
    ],
    "title": "Clarify what a MIP variable is",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7806",
    "user": "https://github.com/nathanncohen"
}
```
A MIP variable is more a dictionnary of variables than a "variable". This should be clarified both in the LP Constructions document and in the functions' docstrings.

Nathann

**Assignee:** mvngu

Issue created by migration from https://trac.sagemath.org/ticket/7806





---

archive/issue_comments_060754.json:
```json
{
    "body": "<a id='comment:1'></a>\nShould be closed as invalid. We have a brand-new LP thematic tutorial now !\n\nNathann",
    "created_at": "2010-10-09T08:24:02Z",
    "issue": "https://github.com/sagemath/sage/issues/7806",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7806#issuecomment-60754",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:1'></a>
Should be closed as invalid. We have a brand-new LP thematic tutorial now !

Nathann



---

archive/issue_events_054182.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-10-09T08:28:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7806",
    "milestone": "sage-4.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7806#event-54182"
}
```



---

archive/issue_events_054183.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-10-09T08:28:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7806",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7806#event-54183"
}
```



---

archive/issue_comments_060755.json:
```json
{
    "body": "<a id='comment:2'></a>\nReplying to [@nathanncohen](#comment%3A1):\n> Should be closed as invalid. We have a brand-new LP thematic tutorial now !\n\nAs you wish.",
    "created_at": "2010-10-09T08:28:55Z",
    "issue": "https://github.com/sagemath/sage/issues/7806",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7806#issuecomment-60755",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:2'></a>
Replying to [@nathanncohen](#comment%3A1):
> Should be closed as invalid. We have a brand-new LP thematic tutorial now !

As you wish.

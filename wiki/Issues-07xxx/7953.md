# Issue 7953: Curve printing only shows first defining equation

archive/issues_007953.json:
```json
{
    "body": "Reported by Ronald van Luijk:\n\nThe `print C` below only prints the first defining equation.\n\n```\nsage: # problem printing\nsage: A.<x,y,z>=AffineSpace(QQ,3)\nsage: C=Curve([x-y,2-z])\nsage: print C\nAffine Space Curve over Rational Field defined by x - y\nsage: print C.defining_ideal()\nIdeal (x - y, -z + 2) of Multivariate Polynomial Ring in x, y, z over Rational Field\n```\n\n**Assignee:** @aghitza\n\n**Author:** Willem Jan Palenstijn\n\n**Reviewer:** Alex Ghitza\n\n**Merged:** sage-4.6.1.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/7953\n\n",
    "closed_at": "2010-11-01T10:05:20Z",
    "created_at": "2010-01-16T18:10:54Z",
    "labels": [
        "component: algebraic geometry",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.1",
    "title": "Curve printing only shows first defining equation",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7953",
    "user": "https://github.com/wjp"
}
```
Reported by Ronald van Luijk:

The `print C` below only prints the first defining equation.

```
sage: # problem printing
sage: A.<x,y,z>=AffineSpace(QQ,3)
sage: C=Curve([x-y,2-z])
sage: print C
Affine Space Curve over Rational Field defined by x - y
sage: print C.defining_ideal()
Ideal (x - y, -z + 2) of Multivariate Polynomial Ring in x, y, z over Rational Field
```

**Assignee:** @aghitza

**Author:** Willem Jan Palenstijn

**Reviewer:** Alex Ghitza

**Merged:** sage-4.6.1.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/7953





---

archive/issue_events_055465.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-01-16T23:47:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7953#event-55465"
}
```



---

archive/attachments_010137.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "7953_curve_printing.patch",
    "asset_url": "tarball://root/attachments/ticket7953/7953_curve_printing.patch",
    "created_at": "2010-01-17T03:50:50Z",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7953/7953_curve_printing.patch",
    "user": "https://github.com/wjp"
}
```



---

archive/issue_comments_062492.json:
```json
{
    "body": "**Attachment:** [7953_curve_printing.patch](https://github.com/sagemath/sage/files/ticket7953/7953_curve_printing.patch)",
    "created_at": "2010-01-17T03:50:50Z",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7953#issuecomment-62492",
    "user": "https://github.com/wjp"
}
```

**Attachment:** [7953_curve_printing.patch](https://github.com/sagemath/sage/files/ticket7953/7953_curve_printing.patch)



---

archive/issue_comments_062493.json:
```json
{
    "body": "**Author:** Willem Jan Palenstijn",
    "created_at": "2010-01-17T22:39:29Z",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7953#issuecomment-62493",
    "user": "https://github.com/aghitza"
}
```

**Author:** Willem Jan Palenstijn



---

archive/issue_comments_062494.json:
```json
{
    "body": "**Reviewer:** Alex Ghitza",
    "created_at": "2010-01-17T22:39:29Z",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7953#issuecomment-62494",
    "user": "https://github.com/aghitza"
}
```

**Reviewer:** Alex Ghitza



---

archive/issue_events_055466.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-01-17T22:39:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7953#event-55466"
}
```



---

archive/issue_events_055467.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-01-17T22:39:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7953#event-55467"
}
```



---

archive/issue_comments_062495.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good.  Thanks, Willem and Ronald!",
    "created_at": "2010-01-17T22:39:29Z",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7953#issuecomment-62495",
    "user": "https://github.com/aghitza"
}
```

<a id='comment:2'></a>
Looks good.  Thanks, Willem and Ronald!



---

archive/issue_events_055468.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-01-18T01:36:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7953#event-55468"
}
```



---

archive/issue_events_055469.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-01-18T01:36:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7953#event-55469"
}
```



---

archive/issue_comments_062496.json:
```json
{
    "body": "<a id='comment:3'></a>\nAfter discussing with William (see also #7954), I'll move the non-plane curve code out of the plane_curves directory first, and rebase this patch on top of that afterwards.",
    "created_at": "2010-01-18T01:36:59Z",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7953#issuecomment-62496",
    "user": "https://github.com/wjp"
}
```

<a id='comment:3'></a>
After discussing with William (see also #7954), I'll move the non-plane curve code out of the plane_curves directory first, and rebase this patch on top of that afterwards.



---

archive/issue_comments_062497.json:
```json
{
    "body": "TABs replaced with spaces",
    "created_at": "2010-10-23T21:56:15Z",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7953#issuecomment-62497",
    "user": "https://github.com/novoselt"
}
```

TABs replaced with spaces



---

archive/issue_events_055470.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2010-10-23T21:59:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7953#event-55470"
}
```



---

archive/issue_events_055471.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2010-10-23T21:59:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7953#event-55471"
}
```



---

archive/attachments_010138.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "7953_curve_printing.2.patch",
    "asset_url": "tarball://root/attachments/ticket7953/7953_curve_printing.2.patch",
    "created_at": "2010-10-23T21:59:33Z",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7953/7953_curve_printing.2.patch",
    "user": "https://github.com/novoselt"
}
```



---

archive/issue_comments_062498.json:
```json
{
    "body": "<a id='comment:4'></a>\n**Attachment:** [7953_curve_printing.2.patch](https://github.com/sagemath/sage/files/ticket7953/7953_curve_printing.2.patch)\n\nI'll take the liberty to switch this patch back to positive review, since it does solve the problem described in the ticket (which is bad - Sage gives wrong answers) and there was no work done in a \"better direction\" for 9 month neither here nor on #7954.\n\nThe original patch uses TABs, so I changed them to spaces leaving the rest the same. Applies cleanly and passes all tests on sage-4.6.rc0.",
    "created_at": "2010-10-23T21:59:33Z",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7953#issuecomment-62498",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:4'></a>
**Attachment:** [7953_curve_printing.2.patch](https://github.com/sagemath/sage/files/ticket7953/7953_curve_printing.2.patch)

I'll take the liberty to switch this patch back to positive review, since it does solve the problem described in the ticket (which is bad - Sage gives wrong answers) and there was no work done in a "better direction" for 9 month neither here nor on #7954.

The original patch uses TABs, so I changed them to spaces leaving the rest the same. Applies cleanly and passes all tests on sage-4.6.rc0.



---

archive/issue_events_055472.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-10-24T08:40:59Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "milestone": "sage-4.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7953#event-55472"
}
```



---

archive/issue_events_055473.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-10-24T08:40:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "milestone": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7953#event-55473"
}
```



---

archive/issue_events_055474.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-01T10:05:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7953#event-55474"
}
```



---

archive/issue_events_055475.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-01T10:05:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7953#event-55475"
}
```



---

archive/issue_comments_062499.json:
```json
{
    "body": "**Merged:** sage-4.6.1.alpha0",
    "created_at": "2010-11-01T10:05:20Z",
    "issue": "https://github.com/sagemath/sage/issues/7953",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7953#issuecomment-62499",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-4.6.1.alpha0

# Issue 7456: developer guide: discuss proper ReST markup for citations

archive/issues_007456.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nAs Martin Albrecht pointed out at #7450, there is a proper way of marking up references and citations, see \nhttp://sphinx.pocoo.org/rest.html#citations\n\n\nCC:  @jhpalmieri\n\nComponent: **documentation**\n\nKeywords: **rest citation**\n\nAuthor: **Alex Ghitza**\n\nReviewer: **Nathann Cohen**\n\nMerged: **sage-4.3.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7456_\n\n",
    "closed_at": "2009-12-09T02:52:54Z",
    "created_at": "2009-11-13T22:17:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "developer guide: discuss proper ReST markup for citations",
    "type": "issue",
    "updated_at": "2009-12-09T02:52:54Z",
    "url": "https://github.com/sagemath/sage/issues/7456",
    "user": "https://github.com/aghitza"
}
```
<div id="comment:0"></div>

As Martin Albrecht pointed out at #7450, there is a proper way of marking up references and citations, see 
http://sphinx.pocoo.org/rest.html#citations


CC:  @jhpalmieri

Component: **documentation**

Keywords: **rest citation**

Author: **Alex Ghitza**

Reviewer: **Nathann Cohen**

Merged: **sage-4.3.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/7456_





---

archive/issue_events_090615.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-11-13T22:17:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7456",
    "milestone_number": null,
    "milestone_title": "sage-4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7456#event-90615"
}
```



---

archive/issue_events_090616.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-11-13T22:17:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7456",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7456#event-90616"
}
```



---

archive/issue_events_090617.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-11-13T22:17:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7456",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7456#event-90617"
}
```



---

archive/issue_events_090618.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-11-13T22:17:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7456",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7456#event-90618"
}
```



---

archive/issue_events_090619.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-11-13T22:17:55Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7456",
    "subject": "https://github.com/aghitza",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7456#event-90619"
}
```



---

archive/issue_comments_054618.json:
```json
{
    "body": "Attachment: **[trac_7456.patch.gz](https://github.com/sagemath/sage/files/ticket7456/trac_7456.patch.gz)**",
    "created_at": "2009-11-13T22:34:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7456#issuecomment-54618",
    "user": "https://github.com/aghitza"
}
```

Attachment: **[trac_7456.patch.gz](https://github.com/sagemath/sage/files/ticket7456/trac_7456.patch.gz)**



---

archive/issue_comments_054619.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nSee attached patch.",
    "created_at": "2009-11-13T22:35:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7456#issuecomment-54619",
    "user": "https://github.com/aghitza"
}
```

<div id="comment:1" align="right">comment:1</div>

See attached patch.



---

archive/issue_events_090620.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-11-13T22:37:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7456",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7456#event-90620"
}
```



---

archive/issue_comments_054620.json:
```json
{
    "body": "Author: **Alex Ghitza**",
    "created_at": "2009-11-13T22:37:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7456#issuecomment-54620",
    "user": "https://github.com/aghitza"
}
```

Author: **Alex Ghitza**



---

archive/issue_events_090621.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-08T12:33:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7456",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7456#event-90621"
}
```



---

archive/issue_events_090622.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-08T12:33:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7456",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7456#event-90622"
}
```



---

archive/issue_comments_054621.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nPositive review for this one !",
    "created_at": "2009-12-08T12:33:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7456#issuecomment-54621",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">comment:4</div>

Positive review for this one !



---

archive/issue_comments_054622.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nSee also [this thread](http://groups.google.com/group/sage-devel/browse_thread/thread/dba6129183b3b6eb?tvc=2) for some related discussion.",
    "created_at": "2009-12-08T22:11:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7456#issuecomment-54622",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:5" align="right">comment:5</div>

See also [this thread](http://groups.google.com/group/sage-devel/browse_thread/thread/dba6129183b3b6eb?tvc=2) for some related discussion.



---

archive/issue_comments_054623.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2009-12-09T02:52:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7456#issuecomment-54623",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_events_090623.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-09T02:52:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7456",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7456#event-90623"
}
```



---

archive/issue_events_090624.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-09T02:52:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7456",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7456#event-90624"
}
```



---

archive/issue_comments_054624.json:
```json
{
    "body": "Merged: **sage-4.3.rc0**",
    "created_at": "2009-12-09T02:52:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7456#issuecomment-54624",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.3.rc0**

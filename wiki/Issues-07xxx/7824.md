# Issue 7824: cliquer-1.2.p2 - add FreeBSD support

archive/issues_007824.json:
```json
{
    "body": "cliquer aborts with the error\n\n```\nCannot determine your platform or it is not supported... exiting\n```\nunless the platform is explicitly listed as supported.  This patch adds FreeBSD support - which is the same as Linux.\n\n**Assignee:** @peterjeremy\n\n**CC:**  nathann.cohen@gmail.com\n\n**Author:** Peter Jeremy\n\n**Reviewer:** David Kirkby\n\n**Merged:** sage-4.3.2.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/7824\n\n",
    "closed_at": "2010-01-24T14:38:15Z",
    "created_at": "2010-01-03T02:10:25Z",
    "labels": [
        "component: porting: bsd",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.2",
    "title": "cliquer-1.2.p2 - add FreeBSD support",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7824",
    "user": "https://github.com/peterjeremy"
}
```
cliquer aborts with the error

```
Cannot determine your platform or it is not supported... exiting
```
unless the platform is explicitly listed as supported.  This patch adds FreeBSD support - which is the same as Linux.

**Assignee:** @peterjeremy

**CC:**  nathann.cohen@gmail.com

**Author:** Peter Jeremy

**Reviewer:** David Kirkby

**Merged:** sage-4.3.2.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/7824





---

archive/attachments_009906.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "7824.cliquer.patch",
    "asset_url": "tarball://root/attachments/ticket7824/7824.cliquer.patch",
    "created_at": "2010-01-03T02:17:31Z",
    "issue": "https://github.com/sagemath/sage/issues/7824",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7824/7824.cliquer.patch",
    "user": "https://github.com/peterjeremy"
}
```



---

archive/issue_comments_060939.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [7824.cliquer.patch](https://github.com/sagemath/sage/files/ticket7824/7824.cliquer.patch)",
    "created_at": "2010-01-03T02:17:31Z",
    "issue": "https://github.com/sagemath/sage/issues/7824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7824#issuecomment-60939",
    "user": "https://github.com/peterjeremy"
}
```

<a id='comment:1'></a>
**Attachment:** [7824.cliquer.patch](https://github.com/sagemath/sage/files/ticket7824/7824.cliquer.patch)



---

archive/issue_events_054295.json:
```json
{
    "actor": "https://github.com/peterjeremy",
    "created_at": "2010-01-03T02:17:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7824",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7824#event-54295"
}
```



---

archive/issue_comments_060940.json:
```json
{
    "body": "**Author:** Peter Jeremy",
    "created_at": "2010-01-03T03:54:28Z",
    "issue": "https://github.com/sagemath/sage/issues/7824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7824#issuecomment-60940",
    "user": "https://github.com/peterjeremy"
}
```

**Author:** Peter Jeremy



---

archive/issue_events_054296.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/drkirkby",
    "created_at": "2010-01-12T17:44:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7824",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7824#event-54296"
}
```



---

archive/issue_events_054297.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/drkirkby",
    "created_at": "2010-01-12T17:44:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7824",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7824#event-54297"
}
```



---

archive/issue_comments_060941.json:
```json
{
    "body": "<a id='comment:3'></a>\nI can't check it, not having FreeBSD installed, but it is clearly 100% safe as it only affects FreeBSD.",
    "created_at": "2010-01-12T17:44:07Z",
    "issue": "https://github.com/sagemath/sage/issues/7824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7824#issuecomment-60941",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```

<a id='comment:3'></a>
I can't check it, not having FreeBSD installed, but it is clearly 100% safe as it only affects FreeBSD.



---

archive/issue_comments_060942.json:
```json
{
    "body": "**Reviewer:** David Kirkby",
    "created_at": "2010-01-12T17:46:20Z",
    "issue": "https://github.com/sagemath/sage/issues/7824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7824#issuecomment-60942",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```

**Reviewer:** David Kirkby



---

archive/issue_comments_060943.json:
```json
{
    "body": "<a id='comment:5'></a>\nAn updated spkg is available at\n\nhttp://sage.math.washington.edu/home/mvngu/spkg/standard/cliquer/cliquer-1.2.p3.spkg\n\nAll changes are committed in pjeremy's name.",
    "created_at": "2010-01-24T14:38:15Z",
    "issue": "https://github.com/sagemath/sage/issues/7824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7824#issuecomment-60943",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:5'></a>
An updated spkg is available at

http://sage.math.washington.edu/home/mvngu/spkg/standard/cliquer/cliquer-1.2.p3.spkg

All changes are committed in pjeremy's name.



---

archive/issue_comments_060944.json:
```json
{
    "body": "**Merged:** sage-4.3.2.alpha0",
    "created_at": "2010-01-24T14:38:15Z",
    "issue": "https://github.com/sagemath/sage/issues/7824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7824#issuecomment-60944",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Merged:** sage-4.3.2.alpha0



---

archive/issue_events_054298.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-01-24T14:38:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7824",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7824#event-54298"
}
```



---

archive/issue_events_054299.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-01-24T14:38:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7824",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7824#event-54299"
}
```

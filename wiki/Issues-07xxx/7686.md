# Issue 7686: Remove "AppleDouble encoded Macintosh files" from SPKGs

archive/issues_007686.json:
```json
{
    "body": "The spkg's with ._ file crap all over the place are:\n\n* `flintqs-20070817.p8` (standard)\n* `gap_packages-4.4.12.p1` (optional)\n* `boost_1_34_1` (experimental)\n* `quantlib-0.9.6` (experimental)\n* `quantlib_swig-0.9.6` (experimental)\n\nThis can be fixed by extracting the spkg, deleting the crap, and remaking it with \"sage -pkg\".  I think \"sage -pkg\" works correctly on OS X now-a-days, but certainly does on Linux.\n\n**Reviewer:** Jeroen Demeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/7686\n\n",
    "closed_at": "2016-06-12T12:02:30Z",
    "created_at": "2009-12-15T19:14:07Z",
    "labels": [
        "component: distribution",
        "bug",
        "invalid"
    ],
    "title": "Remove \"AppleDouble encoded Macintosh files\" from SPKGs",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7686",
    "user": "https://github.com/williamstein"
}
```
The spkg's with ._ file crap all over the place are:

* `flintqs-20070817.p8` (standard)
* `gap_packages-4.4.12.p1` (optional)
* `boost_1_34_1` (experimental)
* `quantlib-0.9.6` (experimental)
* `quantlib_swig-0.9.6` (experimental)

This can be fixed by extracting the spkg, deleting the crap, and remaking it with "sage -pkg".  I think "sage -pkg" works correctly on OS X now-a-days, but certainly does on Linux.

**Reviewer:** Jeroen Demeyer

Issue created by migration from https://trac.sagemath.org/ticket/7686





---

archive/issue_comments_059250.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,8 +1,9 @@\n The spkg's with ._ file crap all over the place are:\n \n-* f2c-20070816.p1\n-* flintqs-20070817.p4\n-* ghmm-20080813.p0\n-* lcalc-20080205.p3\n+* `flintqs-20070817.p4` (standard)\n+* `gap_packages-4.4.12.p1` (optional)\n+* `boost_1_34_1` (experimental)\n+* `quantlib-0.9.6` (experimental)\n+* `quantlib_swig-0.9.6` (experimental)\n \n-This can be fixed by extracting the spkg, deleting the crap, and remaking it with \"sage -pkg\".  I think \"sage -pkg\" works correctly on OS X now-a-days, but certainly does on Linux. \n+This can be fixed by extracting the spkg, deleting the crap, and remaking it with \"sage -pkg\".  I think \"sage -pkg\" works correctly on OS X now-a-days, but certainly does on Linux.\n``````\n",
    "created_at": "2012-10-05T00:39:32Z",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7686#issuecomment-59250",
    "user": "https://github.com/kini"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,8 +1,9 @@
 The spkg's with ._ file crap all over the place are:
 
-* f2c-20070816.p1
-* flintqs-20070817.p4
-* ghmm-20080813.p0
-* lcalc-20080205.p3
+* `flintqs-20070817.p4` (standard)
+* `gap_packages-4.4.12.p1` (optional)
+* `boost_1_34_1` (experimental)
+* `quantlib-0.9.6` (experimental)
+* `quantlib_swig-0.9.6` (experimental)
 
-This can be fixed by extracting the spkg, deleting the crap, and remaking it with "sage -pkg".  I think "sage -pkg" works correctly on OS X now-a-days, but certainly does on Linux. 
+This can be fixed by extracting the spkg, deleting the crap, and remaking it with "sage -pkg".  I think "sage -pkg" works correctly on OS X now-a-days, but certainly does on Linux.
``````




---

archive/issue_events_053001.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-10-05T00:40:13Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "rename": {
        "from": "Remove all \"AppleDouble encoded Macintosh files\" from the Sage source distribution",
        "to": "Remove \"AppleDouble encoded Macintosh files\" from SPKGs"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7686#event-53001"
}
```



---

archive/issue_comments_059251.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,6 +1,6 @@\n The spkg's with ._ file crap all over the place are:\n \n-* `flintqs-20070817.p4` (standard)\n+* `flintqs-20070817.p8` (standard)\n * `gap_packages-4.4.12.p1` (optional)\n * `boost_1_34_1` (experimental)\n * `quantlib-0.9.6` (experimental)\n``````\n",
    "created_at": "2012-10-05T01:13:49Z",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7686#issuecomment-59251",
    "user": "https://github.com/kini"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,6 +1,6 @@
 The spkg's with ._ file crap all over the place are:
 
-* `flintqs-20070817.p4` (standard)
+* `flintqs-20070817.p8` (standard)
 * `gap_packages-4.4.12.p1` (optional)
 * `boost_1_34_1` (experimental)
 * `quantlib-0.9.6` (experimental)
``````




---

archive/issue_events_053002.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7686#event-53002"
}
```



---

archive/issue_events_053003.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7686#event-53003"
}
```



---

archive/issue_events_053004.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7686#event-53004"
}
```



---

archive/issue_events_053005.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7686#event-53005"
}
```



---

archive/issue_events_053006.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7686#event-53006"
}
```



---

archive/issue_events_053007.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7686#event-53007"
}
```



---

archive/issue_events_053008.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7686#event-53008"
}
```



---

archive/issue_events_053009.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7686#event-53009"
}
```



---

archive/issue_events_053010.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-11T09:52:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7686#event-53010"
}
```



---

archive/issue_comments_059252.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2016-04-11T09:52:17Z",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7686#issuecomment-59252",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_comments_059253.json:
```json
{
    "body": "<a id='comment:8'></a>\nObsolete, we no longer use SPKG files.",
    "created_at": "2016-04-11T09:52:17Z",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7686#issuecomment-59253",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'></a>
Obsolete, we no longer use SPKG files.



---

archive/issue_events_053011.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-11T09:52:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7686#event-53011"
}
```



---

archive/issue_events_053012.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-11T09:52:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7686#event-53012"
}
```



---

archive/issue_events_053013.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-11T09:52:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7686#event-53013"
}
```



---

archive/issue_events_053014.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-12T12:02:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7686#event-53014"
}
```



---

archive/issue_events_053015.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-12T12:02:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7686",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7686#event-53015"
}
```

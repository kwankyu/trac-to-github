# Issue 7200: Fixing longest increasing subsequence (permutation method)

archive/issues_007200.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-vferay"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe method \"longest_increasing_subsequence\" was computing the longest increasing factors of the permutation!\n\nThis patch fixes the problem.\n\nCC:  @sagetrac-sage-combinat\n\nComponent: **combinatorics**\n\nKeywords: **permutation**\n\nAuthor: **Valentin F\u00e9ray**\n\nReviewer: **Florent Hivert**\n\nMerged: **sage-4.2.1.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7200_\n\n",
    "closed_at": "2009-11-05T01:47:57Z",
    "created_at": "2009-10-13T14:04:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.2.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fixing longest increasing subsequence (permutation method)",
    "type": "issue",
    "updated_at": "2009-11-05T01:47:57Z",
    "url": "https://github.com/sagemath/sage/issues/7200",
    "user": "https://github.com/sagetrac-vferay"
}
```
<div id="comment:0"></div>

The method "longest_increasing_subsequence" was computing the longest increasing factors of the permutation!

This patch fixes the problem.

CC:  @sagetrac-sage-combinat

Component: **combinatorics**

Keywords: **permutation**

Author: **Valentin Féray**

Reviewer: **Florent Hivert**

Merged: **sage-4.2.1.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/7200_





---

archive/issue_events_087138.json:
```json
{
    "actor": "https://github.com/sagetrac-vferay",
    "created_at": "2009-10-13T14:04:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "milestone_number": null,
    "milestone_title": "sage-4.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7200#event-87138"
}
```



---

archive/issue_events_087139.json:
```json
{
    "actor": "https://github.com/sagetrac-vferay",
    "created_at": "2009-10-13T14:04:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7200#event-87139"
}
```



---

archive/issue_events_087140.json:
```json
{
    "actor": "https://github.com/sagetrac-vferay",
    "created_at": "2009-10-13T14:04:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
    "label_color": "ff7700",
    "label_name": "p: critical / 2",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7200#event-87140"
}
```



---

archive/issue_events_087141.json:
```json
{
    "actor": "https://github.com/sagetrac-vferay",
    "created_at": "2009-10-13T14:04:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7200#event-87141"
}
```



---

archive/issue_events_087142.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-13T14:04:38Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "subject": "https://github.com/sagetrac-vferay",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7200#event-87142"
}
```



---

archive/issue_events_087143.json:
```json
{
    "actor": "https://github.com/sagetrac-vferay",
    "created_at": "2009-10-13T14:16:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7200#event-87143"
}
```



---

archive/issue_comments_051825.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[permutations_fix_longest_increasing_subsequence_vf.patch.gz](https://github.com/sagemath/sage/files/ticket7200/permutations_fix_longest_increasing_subsequence_vf.patch.gz)**",
    "created_at": "2009-10-13T14:16:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7200#issuecomment-51825",
    "user": "https://github.com/sagetrac-vferay"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[permutations_fix_longest_increasing_subsequence_vf.patch.gz](https://github.com/sagemath/sage/files/ticket7200/permutations_fix_longest_increasing_subsequence_vf.patch.gz)**



---

archive/issue_events_087144.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-14T08:40:24Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "subject": "https://github.com/sagetrac-vferay",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7200#event-87144"
}
```



---

archive/issue_events_087145.json:
```json
{
    "actor": "https://github.com/sagetrac-vferay",
    "created_at": "2009-10-14T08:40:24Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "subject": "https://github.com/sagetrac-vferay",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7200#event-87145"
}
```



---

archive/issue_events_087146.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-10-17T20:55:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7200#event-87146"
}
```



---

archive/issue_events_087147.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-10-17T20:55:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7200#event-87147"
}
```



---

archive/issue_comments_051826.json:
```json
{
    "body": "Reviewer: **Florent Hivert**",
    "created_at": "2009-10-17T20:55:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7200#issuecomment-51826",
    "user": "https://github.com/hivert"
}
```

Reviewer: **Florent Hivert**



---

archive/issue_comments_051827.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI dislike the name `length_of_lis` ! I whould never had gessed the meaning of `lis`. Is it so painful to type `length_of_longest_increasing_subsequence` or `longest_incressing_subsequence_length` ? \n\nThe latter is my favourite... \n\nI cc this to sage-combinat to have a vote. Add set the status to \"needs info\". \n\nCheers,\n\nFlorent",
    "created_at": "2009-10-17T20:55:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7200#issuecomment-51827",
    "user": "https://github.com/hivert"
}
```

<div id="comment:3" align="right">comment:3</div>

I dislike the name `length_of_lis` ! I whould never had gessed the meaning of `lis`. Is it so painful to type `length_of_longest_increasing_subsequence` or `longest_incressing_subsequence_length` ? 

The latter is my favourite... 

I cc this to sage-combinat to have a vote. Add set the status to "needs info". 

Cheers,

Florent



---

archive/issue_comments_051828.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nHi Florent,\n\nI agree... I think \"longest_increasing_subsequence_length\" is also better because it will be near \"longest_increasing_subsequences\" in the method list...\n\nBest,\nValentin",
    "created_at": "2009-10-18T09:05:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7200#issuecomment-51828",
    "user": "https://github.com/sagetrac-vferay"
}
```

<div id="comment:4" align="right">comment:4</div>

Hi Florent,

I agree... I think "longest_increasing_subsequence_length" is also better because it will be near "longest_increasing_subsequences" in the method list...

Best,
Valentin



---

archive/issue_events_087148.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-10-23T16:44:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7200#event-87148"
}
```



---

archive/issue_events_087149.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-10-23T16:44:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7200#event-87149"
}
```



---

archive/issue_comments_051829.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nHi !\n\n> I agree... I think \"longest_increasing_subsequence_length\" is also better because it will be near \"longest_increasing_subsequences\" in the method list...\n\nIn case you are just waiting for more vote, I think you can go for it ...\n\nFlorent",
    "created_at": "2009-10-23T16:44:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7200#issuecomment-51829",
    "user": "https://github.com/hivert"
}
```

<div id="comment:5" align="right">comment:5</div>

Hi !

> I agree... I think "longest_increasing_subsequence_length" is also better because it will be near "longest_increasing_subsequences" in the method list...

In case you are just waiting for more vote, I think you can go for it ...

Florent



---

archive/issue_comments_051830.json:
```json
{
    "body": "Attachment: **[permutations_fix_longest_increasing_subsequence_7200_vf.patch.gz](https://github.com/sagemath/sage/files/ticket7200/permutations_fix_longest_increasing_subsequence_7200_vf.patch.gz)**",
    "created_at": "2009-11-04T17:27:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7200#issuecomment-51830",
    "user": "https://github.com/sagetrac-vferay"
}
```

Attachment: **[permutations_fix_longest_increasing_subsequence_7200_vf.patch.gz](https://github.com/sagemath/sage/files/ticket7200/permutations_fix_longest_increasing_subsequence_7200_vf.patch.gz)**



---

archive/issue_comments_051831.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nok I changed the name. What shall I do now?",
    "created_at": "2009-11-04T17:28:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7200#issuecomment-51831",
    "user": "https://github.com/sagetrac-vferay"
}
```

<div id="comment:6" align="right">comment:6</div>

ok I changed the name. What shall I do now?



---

archive/issue_comments_051832.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nReplying to [@sagetrac-vferay](#comment%3A6):\n> ok I changed the name. What shall I do now?\n\nFirst you should say something like:\n\"I just submitted a new version of the patch which is ready for review.\"\n\nThen after that you should change the status which is currently \"needs info\" to \"needs review\" (see the frame at the bottom of the page) and then wait if someone react by putting a positive review or another request...\n\nAnd finally for the release manager you should tell that only `permutations_fix_longest_increasing_subsequence_7200_vf.patch`\nshould be applied.\n\nAs you can see, I've done this for you :-). So everything is ok... \n\nI'm reviewing the patch...\n\nFlorent",
    "created_at": "2009-11-04T21:30:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7200#issuecomment-51832",
    "user": "https://github.com/hivert"
}
```

<div id="comment:7" align="right">comment:7</div>

Replying to [@sagetrac-vferay](#comment%3A6):
> ok I changed the name. What shall I do now?

First you should say something like:
"I just submitted a new version of the patch which is ready for review."

Then after that you should change the status which is currently "needs info" to "needs review" (see the frame at the bottom of the page) and then wait if someone react by putting a positive review or another request...

And finally for the release manager you should tell that only `permutations_fix_longest_increasing_subsequence_7200_vf.patch`
should be applied.

As you can see, I've done this for you :-). So everything is ok... 

I'm reviewing the patch...

Florent



---

archive/issue_events_087150.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-11-04T21:46:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7200#event-87150"
}
```



---

archive/issue_events_087151.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-11-04T21:46:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7200#event-87151"
}
```



---

archive/issue_comments_051833.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nPatch looks good, all tests ok. Positive review.\n\nCheers,\n\nFlorent",
    "created_at": "2009-11-04T21:46:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7200#issuecomment-51833",
    "user": "https://github.com/hivert"
}
```

<div id="comment:8" align="right">comment:8</div>

Patch looks good, all tests ok. Positive review.

Cheers,

Florent



---

archive/issue_comments_051834.json:
```json
{
    "body": "Attachment: **[trac_7200.patch.gz](https://github.com/sagemath/sage/files/ticket7200/trac_7200.patch.gz)**",
    "created_at": "2009-11-05T01:43:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7200#issuecomment-51834",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[trac_7200.patch.gz](https://github.com/sagemath/sage/files/ticket7200/trac_7200.patch.gz)**



---

archive/issue_comments_051835.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nI uploaded a patch which makes a small change to the documentation markup.  I've also updated the patch on the patch server.",
    "created_at": "2009-11-05T01:47:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7200#issuecomment-51835",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:10" align="right">comment:10</div>

I uploaded a patch which makes a small change to the documentation markup.  I've also updated the patch on the patch server.



---

archive/issue_events_087152.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-05T01:47:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7200#event-87152"
}
```



---

archive/issue_events_087153.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-05T01:47:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7200#event-87153"
}
```



---

archive/issue_comments_051836.json:
```json
{
    "body": "Merged: **sage-4.2.1.alpha0**",
    "created_at": "2009-11-05T01:47:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7200",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7200#issuecomment-51836",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.2.1.alpha0**

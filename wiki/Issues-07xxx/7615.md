# Issue 7615: Fix arctan2 documentation

archive/issues_007615.json:
```json
{
    "body": "From [this thread](http://groups.google.com/group/sage-support/browse_thread/thread/9fba0850f4211fd9):\n\n```\nAs you mentioned, the documentation instructed \nme that arctan2(y,x)=arctan(y/x) and so I thought that arctan2(0,x) \nwould always equal 0. \n```\n\n\n**Assignee:** mvngu\n\n**Author:** Karl-Dieter Crisman\n\n**Reviewer:** Nathann Cohen\n\n**Merged:** sage-4.3.rc0\n\nIssue created by migration from https://trac.sagemath.org/ticket/7615\n\n",
    "closed_at": "2009-12-09T02:47:40Z",
    "created_at": "2009-12-07T12:07:00Z",
    "labels": [
        "component: documentation",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3",
    "title": "Fix arctan2 documentation",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7615",
    "user": "https://github.com/kcrisman"
}
```
From [this thread](http://groups.google.com/group/sage-support/browse_thread/thread/9fba0850f4211fd9):

```
As you mentioned, the documentation instructed 
me that arctan2(y,x)=arctan(y/x) and so I thought that arctan2(0,x) 
would always equal 0. 
```


**Assignee:** mvngu

**Author:** Karl-Dieter Crisman

**Reviewer:** Nathann Cohen

**Merged:** sage-4.3.rc0

Issue created by migration from https://trac.sagemath.org/ticket/7615





---

archive/issue_events_052290.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-12-07T14:50:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-52290"
}
```



---

archive/issue_comments_058346.json:
```json
{
    "body": "**Description changed:**\n``````diff\n\n``````\n",
    "created_at": "2009-12-07T14:55:06Z",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7615#issuecomment-58346",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Description changed:**
``````diff

``````




---

archive/issue_events_052291.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-08T12:39:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-52291"
}
```



---

archive/issue_events_052292.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-08T12:39:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-52292"
}
```



---

archive/issue_comments_058347.json:
```json
{
    "body": "<a id='comment:3'></a>\nCould you add backslashes to \"pi\" so that it prints well in LaTeX ? :-)\n\nOnce this is done, this patch can be set to \"positive review\"\n\nThank you !!!\n\nNathann",
    "created_at": "2009-12-08T12:39:10Z",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7615#issuecomment-58347",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:3'></a>
Could you add backslashes to "pi" so that it prints well in LaTeX ? :-)

Once this is done, this patch can be set to "positive review"

Thank you !!!

Nathann



---

archive/attachments_009604.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_7615.patch",
    "asset_url": "tarball://root/attachments/ticket7615/trac_7615.patch",
    "created_at": "2009-12-08T15:46:04Z",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7615/trac_7615.patch",
    "user": "https://github.com/kcrisman"
}
```



---

archive/issue_comments_058348.json:
```json
{
    "body": "**Attachment:** [trac_7615.patch](https://github.com/sagemath/sage/files/ticket7615/trac_7615.patch)\n\nBased on 4.3.alpha1",
    "created_at": "2009-12-08T15:46:04Z",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7615#issuecomment-58348",
    "user": "https://github.com/kcrisman"
}
```

**Attachment:** [trac_7615.patch](https://github.com/sagemath/sage/files/ticket7615/trac_7615.patch)

Based on 4.3.alpha1



---

archive/issue_comments_058349.json:
```json
{
    "body": "<a id='comment:4'></a>\nOf course, that was quite silly.  Thanks!",
    "created_at": "2009-12-08T15:46:33Z",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7615#issuecomment-58349",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:4'></a>
Of course, that was quite silly.  Thanks!



---

archive/issue_comments_058350.json:
```json
{
    "body": "**Reviewer:** Nathann Cohen",
    "created_at": "2009-12-08T15:46:33Z",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7615#issuecomment-58350",
    "user": "https://github.com/kcrisman"
}
```

**Reviewer:** Nathann Cohen



---

archive/issue_events_052293.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-12-08T15:46:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-52293"
}
```



---

archive/issue_events_052294.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-12-08T15:46:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-52294"
}
```



---

archive/issue_comments_058351.json:
```json
{
    "body": "**Author:** Karl-Dieter Crisman",
    "created_at": "2009-12-08T15:46:33Z",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7615#issuecomment-58351",
    "user": "https://github.com/kcrisman"
}
```

**Author:** Karl-Dieter Crisman



---

archive/issue_events_052295.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-08T15:56:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-52295"
}
```



---

archive/issue_events_052296.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-08T15:56:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-52296"
}
```



---

archive/issue_events_052297.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-09T02:47:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-52297"
}
```



---

archive/issue_events_052298.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-09T02:47:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-52298"
}
```



---

archive/issue_comments_058352.json:
```json
{
    "body": "**Merged:** sage-4.3.rc0",
    "created_at": "2009-12-09T02:47:40Z",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7615#issuecomment-58352",
    "user": "https://github.com/mwhansen"
}
```

**Merged:** sage-4.3.rc0

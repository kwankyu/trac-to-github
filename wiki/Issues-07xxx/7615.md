# Issue 7615: Fix arctan2 documentation

archive/issues_007615.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nFrom [this thread](http://groups.google.com/group/sage-support/browse_thread/thread/9fba0850f4211fd9):\n\n```\nAs you mentioned, the documentation instructed \nme that arctan2(y,x)=arctan(y/x) and so I thought that arctan2(0,x) \nwould always equal 0. \n```\n\n\nComponent: **documentation**\n\nAuthor: **Karl-Dieter Crisman**\n\nReviewer: **Nathann Cohen**\n\nMerged: **sage-4.3.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7615_\n\n",
    "closed_at": "2009-12-09T02:47:40Z",
    "created_at": "2009-12-07T12:07:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix arctan2 documentation",
    "type": "issue",
    "updated_at": "2009-12-09T02:47:40Z",
    "url": "https://github.com/sagemath/sage/issues/7615",
    "user": "https://github.com/kcrisman"
}
```
<div id="comment:0"></div>

From [this thread](http://groups.google.com/group/sage-support/browse_thread/thread/9fba0850f4211fd9):

```
As you mentioned, the documentation instructed 
me that arctan2(y,x)=arctan(y/x) and so I thought that arctan2(0,x) 
would always equal 0. 
```


Component: **documentation**

Author: **Karl-Dieter Crisman**

Reviewer: **Nathann Cohen**

Merged: **sage-4.3.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/7615_





---

archive/issue_events_092992.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-12-07T12:07:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "milestone_number": null,
    "milestone_title": "sage-4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-92992"
}
```



---

archive/issue_events_092993.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-12-07T12:07:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-92993"
}
```



---

archive/issue_events_092994.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-12-07T12:07:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-92994"
}
```



---

archive/issue_events_092995.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-12-07T12:07:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-92995"
}
```



---

archive/issue_events_092996.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-12-07T12:07:00Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "subject": "https://github.com/kcrisman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-92996"
}
```



---

archive/issue_events_092997.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-12-07T14:50:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-92997"
}
```



---

archive/issue_comments_056696.json:
```json
{
    "body": "Description changed:\n``````diff\n\n``````\n",
    "created_at": "2009-12-07T14:55:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7615#issuecomment-56696",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Description changed:
``````diff

``````




---

archive/issue_events_092998.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-08T12:39:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-92998"
}
```



---

archive/issue_events_092999.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-08T12:39:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-92999"
}
```



---

archive/issue_comments_056697.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nCould you add backslashes to \"pi\" so that it prints well in LaTeX ? :-)\n\nOnce this is done, this patch can be set to \"positive review\"\n\nThank you !!!\n\nNathann",
    "created_at": "2009-12-08T12:39:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7615#issuecomment-56697",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:3" align="right">comment:3</div>

Could you add backslashes to "pi" so that it prints well in LaTeX ? :-)

Once this is done, this patch can be set to "positive review"

Thank you !!!

Nathann



---

archive/issue_comments_056698.json:
```json
{
    "body": "Attachment: **[trac_7615.patch.gz](https://github.com/sagemath/sage/files/ticket7615/trac_7615.patch.gz)**\n\nBased on 4.3.alpha1",
    "created_at": "2009-12-08T15:46:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7615#issuecomment-56698",
    "user": "https://github.com/kcrisman"
}
```

Attachment: **[trac_7615.patch.gz](https://github.com/sagemath/sage/files/ticket7615/trac_7615.patch.gz)**

Based on 4.3.alpha1



---

archive/issue_comments_056699.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nOf course, that was quite silly.  Thanks!",
    "created_at": "2009-12-08T15:46:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7615#issuecomment-56699",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:4" align="right">comment:4</div>

Of course, that was quite silly.  Thanks!



---

archive/issue_comments_056700.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2009-12-08T15:46:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7615#issuecomment-56700",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_events_093000.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-12-08T15:46:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-93000"
}
```



---

archive/issue_events_093001.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-12-08T15:46:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-93001"
}
```



---

archive/issue_comments_056701.json:
```json
{
    "body": "Author: **Karl-Dieter Crisman**",
    "created_at": "2009-12-08T15:46:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7615#issuecomment-56701",
    "user": "https://github.com/kcrisman"
}
```

Author: **Karl-Dieter Crisman**



---

archive/issue_events_093002.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-08T15:56:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-93002"
}
```



---

archive/issue_events_093003.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-08T15:56:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-93003"
}
```



---

archive/issue_events_093004.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-09T02:47:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-93004"
}
```



---

archive/issue_events_093005.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-09T02:47:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7615#event-93005"
}
```



---

archive/issue_comments_056702.json:
```json
{
    "body": "Merged: **sage-4.3.rc0**",
    "created_at": "2009-12-09T02:47:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7615",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7615#issuecomment-56702",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.3.rc0**

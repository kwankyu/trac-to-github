# Issue 7672: topological_sort in c_graphs

archive/issues_007672.json:
```json
{
    "body": "The function topological_sort uses Networkx for the moment. As c_graphs are to become the standard implementation of graphs in Sage, this function should be rewritten in Cython.\n\n**Assignee:** @rlmill\n\nIssue created by migration from https://trac.sagemath.org/ticket/7672\n\n",
    "closed_at": "2011-01-01T13:25:38Z",
    "created_at": "2009-12-12T18:14:14Z",
    "labels": [
        "component: graph theory",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.1",
    "title": "topological_sort in c_graphs",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7672",
    "user": "https://github.com/nathanncohen"
}
```
The function topological_sort uses Networkx for the moment. As c_graphs are to become the standard implementation of graphs in Sage, this function should be rewritten in Cython.

**Assignee:** @rlmill

Issue created by migration from https://trac.sagemath.org/ticket/7672





---

archive/issue_events_052842.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-06-06T11:00:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7672",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7672#event-52842"
}
```



---

archive/issue_comments_059052.json:
```json
{
    "body": "<a id='comment:2'></a>\nDone at #10432",
    "created_at": "2011-01-01T13:25:38Z",
    "issue": "https://github.com/sagemath/sage/issues/7672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7672#issuecomment-59052",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:2'></a>
Done at #10432



---

archive/issue_events_052843.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-01-01T13:25:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7672",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7672#event-52843"
}
```



---

archive/issue_events_052844.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-01-01T13:25:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7672",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7672#event-52844"
}
```

# Issue 7726: Upgrade biopython package to 1.53

archive/issues_007726.json:
```json
{
    "body": "Biopython was released December 15th, 2009.  No major changes were made, so the new package just updates the source and SPKG.txt.\n\n**CC:**  @maxthemouse\n\n**Keywords:** biopython\n\n**Reviewer:** Adam Webb\n\n**Author:** Marshall Hampton\n\nIssue created by migration from https://trac.sagemath.org/ticket/7726\n\n",
    "closed_at": "2009-12-20T07:29:47Z",
    "created_at": "2009-12-17T19:10:15Z",
    "labels": [
        "component: packages: optional",
        "minor",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3",
    "title": "Upgrade biopython package to 1.53",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7726",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```
Biopython was released December 15th, 2009.  No major changes were made, so the new package just updates the source and SPKG.txt.

**CC:**  @maxthemouse

**Keywords:** biopython

**Reviewer:** Adam Webb

**Author:** Marshall Hampton

Issue created by migration from https://trac.sagemath.org/ticket/7726





---

archive/issue_comments_059641.json:
```json
{
    "body": "<a id='comment:1'></a>\nNew package available at:\n[http://sage.math.washington.edu/home/mhampton/biopython-1.53.p0.spkg](http://sage.math.washington.edu/home/mhampton/biopython-1.53.p0.spkg)",
    "created_at": "2009-12-17T19:11:15Z",
    "issue": "https://github.com/sagemath/sage/issues/7726",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7726#issuecomment-59641",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

<a id='comment:1'></a>
New package available at:
[http://sage.math.washington.edu/home/mhampton/biopython-1.53.p0.spkg](http://sage.math.washington.edu/home/mhampton/biopython-1.53.p0.spkg)



---

archive/issue_events_053368.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mhampton",
    "created_at": "2009-12-17T19:11:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7726",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7726#event-53368"
}
```



---

archive/issue_comments_059642.json:
```json
{
    "body": "<a id='comment:2'></a>\nThe package installs and the check runs fine. I got a couple of warnings from the BioSQL test but that seems to be expected if I understand the code. \n\nOne small thing: The changes haven't been committed to the hg depository in the package. Otherwise, it looks good to me.\n\nAdam",
    "created_at": "2009-12-17T21:24:38Z",
    "issue": "https://github.com/sagemath/sage/issues/7726",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7726#issuecomment-59642",
    "user": "https://github.com/maxthemouse"
}
```

<a id='comment:2'></a>
The package installs and the check runs fine. I got a couple of warnings from the BioSQL test but that seems to be expected if I understand the code. 

One small thing: The changes haven't been committed to the hg depository in the package. Otherwise, it looks good to me.

Adam



---

archive/issue_events_053369.json:
```json
{
    "actor": "https://github.com/maxthemouse",
    "created_at": "2009-12-17T21:24:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7726",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7726#event-53369"
}
```



---

archive/issue_events_053370.json:
```json
{
    "actor": "https://github.com/maxthemouse",
    "created_at": "2009-12-17T21:24:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7726",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7726#event-53370"
}
```



---

archive/issue_comments_059643.json:
```json
{
    "body": "<a id='comment:3'></a>\nThanks for the quick review.  I commited the changes in hg, and removed a couple of .DS_Store files that my mac had added.  This new package is up at the same link above.",
    "created_at": "2009-12-17T22:52:00Z",
    "issue": "https://github.com/sagemath/sage/issues/7726",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7726#issuecomment-59643",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

<a id='comment:3'></a>
Thanks for the quick review.  I commited the changes in hg, and removed a couple of .DS_Store files that my mac had added.  This new package is up at the same link above.



---

archive/issue_comments_059644.json:
```json
{
    "body": "**Author:** Marshall Hamption",
    "created_at": "2009-12-20T07:29:47Z",
    "issue": "https://github.com/sagemath/sage/issues/7726",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7726#issuecomment-59644",
    "user": "https://github.com/mwhansen"
}
```

**Author:** Marshall Hamption



---

archive/issue_events_053371.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-20T07:29:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7726",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7726#event-53371"
}
```



---

archive/issue_events_053372.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-20T07:29:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7726",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7726#event-53372"
}
```



---

archive/issue_comments_059645.json:
```json
{
    "body": "**Changing reviewer** from \"awebb\" to \"Adam Webb\".",
    "created_at": "2009-12-20T07:29:47Z",
    "issue": "https://github.com/sagemath/sage/issues/7726",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7726#issuecomment-59645",
    "user": "https://github.com/mwhansen"
}
```

**Changing reviewer** from "awebb" to "Adam Webb".



---

archive/issue_comments_059646.json:
```json
{
    "body": "<a id='comment:5'></a>\nwrong name",
    "created_at": "2017-07-19T08:44:30Z",
    "issue": "https://github.com/sagemath/sage/issues/7726",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7726#issuecomment-59646",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:5'></a>
wrong name



---

archive/issue_comments_059647.json:
```json
{
    "body": "**Changing author** from \"Marshall Hamption\" to \"Marshall Hampton\".",
    "created_at": "2017-07-19T08:44:30Z",
    "issue": "https://github.com/sagemath/sage/issues/7726",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7726#issuecomment-59647",
    "user": "https://github.com/fchapoton"
}
```

**Changing author** from "Marshall Hamption" to "Marshall Hampton".

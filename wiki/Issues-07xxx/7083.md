# Issue 7083: [positive review review] Improve a few special functions

archive/issues_007083.json:
```json
{
    "assignees": [
        "https://github.com/burcin"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nA few functions in functions/special.py need a little help to actually accept valid input.  Also, exp_int is duplicated in its functionality.\n\nComponent: **calculus**\n\nAuthor: **Karl-Dieter Crisman**\n\nReviewer: **David Joyner**\n\nMerged: **sage-4.2.1.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7083_\n\n",
    "closed_at": "2009-10-31T15:31:28Z",
    "created_at": "2009-09-30T15:03:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20calculus",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.2.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "[positive review review] Improve a few special functions",
    "type": "issue",
    "updated_at": "2009-11-23T14:25:38Z",
    "url": "https://github.com/sagemath/sage/issues/7083",
    "user": "https://github.com/kcrisman"
}
```
<div id="comment:0"></div>

A few functions in functions/special.py need a little help to actually accept valid input.  Also, exp_int is duplicated in its functionality.

Component: **calculus**

Author: **Karl-Dieter Crisman**

Reviewer: **David Joyner**

Merged: **sage-4.2.1.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/7083_





---

archive/issue_events_085568.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-30T15:03:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "milestone_number": null,
    "milestone_title": "sage-4.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7083#event-85568"
}
```



---

archive/issue_events_085569.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-30T15:03:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "c: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7083#event-85569"
}
```



---

archive/issue_events_085570.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-30T15:03:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7083#event-85570"
}
```



---

archive/issue_events_085571.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-30T15:03:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7083#event-85571"
}
```



---

archive/issue_events_085572.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-30T15:03:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7083#event-85572"
}
```



---

archive/issue_events_085573.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-09-30T15:03:10Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "subject": "https://github.com/kcrisman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7083#event-85573"
}
```



---

archive/issue_comments_050763.json:
```json
{
    "body": "Attachment: **[trac_7083-special-funcs.patch.gz](https://github.com/sagemath/sage/files/ticket7083/trac_7083-special-funcs.patch.gz)**\n\nBased on 4.1.2.alpha4",
    "created_at": "2009-09-30T15:04:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7083#issuecomment-50763",
    "user": "https://github.com/kcrisman"
}
```

Attachment: **[trac_7083-special-funcs.patch.gz](https://github.com/sagemath/sage/files/ticket7083/trac_7083-special-funcs.patch.gz)**

Based on 4.1.2.alpha4



---

archive/issue_comments_050764.json:
```json
{
    "body": "Author: **Karl-Dieter Crisman**",
    "created_at": "2009-09-30T15:05:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7083#issuecomment-50764",
    "user": "https://github.com/kcrisman"
}
```

Author: **Karl-Dieter Crisman**



---

archive/issue_comments_050765.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAnother option to deprecating exp_int() is to just make it call exponential_integral_1, and I would be happy to implement either way as a reviewer indicates is useful.",
    "created_at": "2009-09-30T15:05:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7083#issuecomment-50765",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:1" align="right">comment:1</div>

Another option to deprecating exp_int() is to just make it call exponential_integral_1, and I would be happy to implement either way as a reviewer indicates is useful.



---

archive/issue_events_085574.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2009-10-25T13:41:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7083#event-85574"
}
```



---

archive/issue_events_085575.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2009-10-25T13:41:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7083#event-85575"
}
```



---

archive/issue_events_085576.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2009-10-25T13:41:10Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "title_is": "[positive review review] Improve a few special functions",
    "title_was": "Improve a few special functions",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7083#event-85576"
}
```



---

archive/issue_comments_050766.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nPasses sage -testall on ubuntu 9.04 32 bit. It seems to pass on an imac running 10.6 but there are so many other failures, I'm not sure. Does what it claims to do and adds nice functionality.",
    "created_at": "2009-10-25T13:41:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7083#issuecomment-50766",
    "user": "https://github.com/wdjoyner"
}
```

<div id="comment:2" align="right">comment:2</div>

Passes sage -testall on ubuntu 9.04 32 bit. It seems to pass on an imac running 10.6 but there are so many other failures, I'm not sure. Does what it claims to do and adds nice functionality.



---

archive/issue_events_085577.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2009-10-25T13:41:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "milestone_number": null,
    "milestone_title": "sage-4.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7083#event-85577"
}
```



---

archive/issue_events_085578.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2009-10-25T13:41:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7083#event-85578"
}
```



---

archive/issue_events_085579.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-10-29T17:25:54Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7083#event-85579"
}
```



---

archive/issue_events_085580.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-10-29T17:25:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "milestone_number": null,
    "milestone_title": "sage-4.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7083#event-85580"
}
```



---

archive/issue_events_085581.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-31T15:31:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7083#event-85581"
}
```



---

archive/issue_events_085582.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-31T15:31:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7083#event-85582"
}
```



---

archive/issue_comments_050767.json:
```json
{
    "body": "Merged: **sage-4.2.1.alpha0**",
    "created_at": "2009-10-31T15:31:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7083#issuecomment-50767",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.2.1.alpha0**



---

archive/issue_comments_050768.json:
```json
{
    "body": "Reviewer: **David Joyner**",
    "created_at": "2009-10-31T15:31:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7083",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7083#issuecomment-50768",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **David Joyner**

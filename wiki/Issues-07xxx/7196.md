# Issue 7196: SageNB: Reorganize the JS/Java/CSS/HTML "data" directory

archive/issues_007196.json:
```json
{
    "assignees": [
        "https://github.com/boothby"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nReorganize `sagenb/data` around packages.\n\nCC:  @TimDumol @williamstein\n\nComponent: **notebook**\n\nAuthor: **Mitesh Patel**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7196_\n\n",
    "closed_at": "2009-10-15T01:24:39Z",
    "created_at": "2009-10-12T17:16:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "SageNB: Reorganize the JS/Java/CSS/HTML \"data\" directory",
    "type": "issue",
    "updated_at": "2009-10-15T01:24:39Z",
    "url": "https://github.com/sagemath/sage/issues/7196",
    "user": "https://github.com/qed777"
}
```
<div id="comment:0"></div>

Reorganize `sagenb/data` around packages.

CC:  @TimDumol @williamstein

Component: **notebook**

Author: **Mitesh Patel**

_Issue created by migration from https://trac.sagemath.org/ticket/7196_





---

archive/issue_events_087077.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-10-12T17:16:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7196",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7196#event-87077"
}
```



---

archive/issue_events_087078.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-10-12T17:16:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7196",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7196#event-87078"
}
```



---

archive/issue_events_087079.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-10-12T17:16:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7196",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7196#event-87079"
}
```



---

archive/issue_events_087080.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-10-12T17:16:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7196",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7196#event-87080"
}
```



---

archive/issue_events_087081.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-10-12T17:16:48Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7196",
    "subject": "https://github.com/qed777",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7196#event-87081"
}
```



---

archive/issue_comments_051779.json:
```json
{
    "body": "Attachment: **[trac_7196-sagenb_data_reorg_part_A.patch.gz](https://github.com/sagemath/sage/files/ticket7196/trac_7196-sagenb_data_reorg_part_A.patch.gz)**\n\nSageNB data/ reorg part A: Shuffle files and directories.  Apply this patch first.",
    "created_at": "2009-10-12T17:38:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7196#issuecomment-51779",
    "user": "https://github.com/qed777"
}
```

Attachment: **[trac_7196-sagenb_data_reorg_part_A.patch.gz](https://github.com/sagemath/sage/files/ticket7196/trac_7196-sagenb_data_reorg_part_A.patch.gz)**

SageNB data/ reorg part A: Shuffle files and directories.  Apply this patch first.



---

archive/issue_comments_051780.json:
```json
{
    "body": "SageNB data/ reorg part B: Fix broken paths. Apply this patch second.",
    "created_at": "2009-10-12T17:38:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7196#issuecomment-51780",
    "user": "https://github.com/qed777"
}
```

SageNB data/ reorg part B: Fix broken paths. Apply this patch second.



---

archive/issue_events_087082.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-10-12T17:40:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7196",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7196#event-87082"
}
```



---

archive/issue_comments_051781.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_7196-sagenb_data_reorg_part_B.patch.gz](https://github.com/sagemath/sage/files/ticket7196/trac_7196-sagenb_data_reorg_part_B.patch.gz)**\n\nI made the change in two parts, since the shuffle slowed Mercurial significantly.",
    "created_at": "2009-10-12T17:40:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7196#issuecomment-51781",
    "user": "https://github.com/qed777"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_7196-sagenb_data_reorg_part_B.patch.gz](https://github.com/sagemath/sage/files/ticket7196/trac_7196-sagenb_data_reorg_part_B.patch.gz)**

I made the change in two parts, since the shuffle slowed Mercurial significantly.



---

archive/issue_comments_051782.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nYou didn't mention it, but one must *MANUALLY* move data/java/jmol to data/jmol after applying this patch, since jmol isn't under revision control, and the path to jmol was changed in twist.py.",
    "created_at": "2009-10-15T01:14:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7196#issuecomment-51782",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">comment:2</div>

You didn't mention it, but one must *MANUALLY* move data/java/jmol to data/jmol after applying this patch, since jmol isn't under revision control, and the path to jmol was changed in twist.py.



---

archive/issue_comments_051783.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\npositive review, and merged into sagenb-0.3.1, which has been posted here:\n\n http://sage.math.washington.edu/home/wstein/patches/sagenb/",
    "created_at": "2009-10-15T01:24:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7196#issuecomment-51783",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:3" align="right">comment:3</div>

positive review, and merged into sagenb-0.3.1, which has been posted here:

 http://sage.math.washington.edu/home/wstein/patches/sagenb/



---

archive/issue_events_087083.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-10-15T01:24:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7196",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7196#event-87083"
}
```



---

archive/issue_events_087084.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-10-15T01:24:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7196",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7196#event-87084"
}
```



---

archive/issue_comments_051784.json:
```json
{
    "body": "Attachment: **[trac_7196-manifest.patch.gz](https://github.com/sagemath/sage/files/ticket7196/trac_7196-manifest.patch.gz)**\n\nUpdate MANIFEST.in.",
    "created_at": "2009-10-19T14:52:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7196#issuecomment-51784",
    "user": "https://github.com/qed777"
}
```

Attachment: **[trac_7196-manifest.patch.gz](https://github.com/sagemath/sage/files/ticket7196/trac_7196-manifest.patch.gz)**

Update MANIFEST.in.

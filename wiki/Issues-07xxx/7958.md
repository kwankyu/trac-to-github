# Issue 7958: Conversion of rationals into the fraction field of integer polynomials

archive/issues_007958.json:
```json
{
    "assignees": [
        "https://github.com/robertwb"
    ],
    "body": "<div id=\"comment:0\"></div>\n\n\n```\nsage: F = Frac(PolynomialRing(ZZ, 't'))\nsage: F(1/2)\n...\nTypeError: no conversion of this rational to integer\n```\n\nComponent: **coercion**\n\nAuthor: **Sebastian Pancratz, Mike Hansen**\n\nReviewer: **Mike Hansen, Sebastian Pancratz**\n\nMerged: **sage-4.3.2.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7958_\n\n",
    "closed_at": "2010-01-23T07:47:55Z",
    "created_at": "2010-01-16T19:28:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20coercion",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Conversion of rationals into the fraction field of integer polynomials",
    "type": "issue",
    "updated_at": "2010-01-23T07:47:55Z",
    "url": "https://github.com/sagemath/sage/issues/7958",
    "user": "https://github.com/sagetrac-spancratz"
}
```
<div id="comment:0"></div>


```
sage: F = Frac(PolynomialRing(ZZ, 't'))
sage: F(1/2)
...
TypeError: no conversion of this rational to integer
```

Component: **coercion**

Author: **Sebastian Pancratz, Mike Hansen**

Reviewer: **Mike Hansen, Sebastian Pancratz**

Merged: **sage-4.3.2.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/7958_





---

archive/issue_events_097978.json:
```json
{
    "actor": "https://github.com/sagetrac-spancratz",
    "created_at": "2010-01-16T19:28:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "milestone_number": null,
    "milestone_title": "sage-4.3.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97978"
}
```



---

archive/issue_events_097979.json:
```json
{
    "actor": "https://github.com/sagetrac-spancratz",
    "created_at": "2010-01-16T19:28:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97979"
}
```



---

archive/issue_events_097980.json:
```json
{
    "actor": "https://github.com/sagetrac-spancratz",
    "created_at": "2010-01-16T19:28:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97980"
}
```



---

archive/issue_events_097981.json:
```json
{
    "actor": "https://github.com/sagetrac-spancratz",
    "created_at": "2010-01-16T19:28:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97981"
}
```



---

archive/issue_events_097982.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-01-16T19:28:41Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "subject": "https://github.com/sagetrac-spancratz",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97982"
}
```



---

archive/issue_events_097983.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-01-16T19:29:45Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "subject": "https://github.com/sagetrac-spancratz",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97983"
}
```



---

archive/issue_events_097984.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-01-16T19:29:45Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "subject": "https://github.com/sagetrac-spancratz",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97984"
}
```



---

archive/issue_events_097985.json:
```json
{
    "actor": "https://github.com/sagetrac-spancratz",
    "created_at": "2010-01-16T19:29:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97985"
}
```



---

archive/issue_events_097986.json:
```json
{
    "actor": "https://github.com/sagetrac-spancratz",
    "created_at": "2010-01-16T19:29:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20coercion",
    "label_color": "0000ff",
    "label_name": "c: coercion",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97986"
}
```



---

archive/issue_comments_060850.json:
```json
{
    "body": "Attachment: **[trac7958.patch.gz](https://github.com/sagemath/sage/files/ticket7958/trac7958.patch.gz)**",
    "created_at": "2010-01-16T21:00:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7958#issuecomment-60850",
    "user": "https://github.com/sagetrac-spancratz"
}
```

Attachment: **[trac7958.patch.gz](https://github.com/sagemath/sage/files/ticket7958/trac7958.patch.gz)**



---

archive/issue_events_097987.json:
```json
{
    "actor": "https://github.com/sagetrac-spancratz",
    "created_at": "2010-01-16T21:30:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97987"
}
```



---

archive/issue_comments_060851.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nYour fix does work great for QQ, but this is actually a more general issue than just QQ:\n\n```\nsage: _.<x> = ZZ[]\nsage: K.<a> = NumberField(x^2-2)\nsage: R.<b> = K.ring_of_integers()\nsage: S.<y> = R[]\nsage: F = FractionField(S)\nsage: F(1)/F(a)\n1/a\nsage: F(1/a)\n*boom*\n```\n\nAnd a minor issue: I think the comment about QQ should be a code comment rather than in the doctest, since it might now confuse users (who might think they need to handle QQ specially themselves).",
    "created_at": "2010-01-17T17:50:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7958#issuecomment-60851",
    "user": "https://github.com/wjp"
}
```

<div id="comment:3" align="right">comment:3</div>

Your fix does work great for QQ, but this is actually a more general issue than just QQ:

```
sage: _.<x> = ZZ[]
sage: K.<a> = NumberField(x^2-2)
sage: R.<b> = K.ring_of_integers()
sage: S.<y> = R[]
sage: F = FractionField(S)
sage: F(1)/F(a)
1/a
sage: F(1/a)
*boom*
```

And a minor issue: I think the comment about QQ should be a code comment rather than in the doctest, since it might now confuse users (who might think they need to handle QQ specially themselves).



---

archive/issue_events_097988.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-01-17T17:50:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97988"
}
```



---

archive/issue_events_097989.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-01-17T17:50:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97989"
}
```



---

archive/issue_comments_060852.json:
```json
{
    "body": "Attachment: **[trac7958_b.patch.gz](https://github.com/sagemath/sage/files/ticket7958/trac7958_b.patch.gz)**\n\nAttachment: **[trac7958_c.patch.gz](https://github.com/sagemath/sage/files/ticket7958/trac7958_c.patch.gz)**",
    "created_at": "2010-01-17T21:29:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7958#issuecomment-60852",
    "user": "https://github.com/sagetrac-spancratz"
}
```

Attachment: **[trac7958_b.patch.gz](https://github.com/sagemath/sage/files/ticket7958/trac7958_b.patch.gz)**

Attachment: **[trac7958_c.patch.gz](https://github.com/sagemath/sage/files/ticket7958/trac7958_c.patch.gz)**



---

archive/issue_comments_060853.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nTo see that this issue is now resolved (for rationals and number fields), consider\n{{{sage: _.<x> = ZZ[]\nsage: K.<a> = NumberField(x<sup>5-3*x</sup>4+2424*x^3+2*x-232)\nsage: R.<b> = K.ring_of_integers()\nsage: S.<y> = R[]\nsage: F = Frac(S)\nsage: F(1/a)\na^4 - 3*a^3 + 2424*a^2 + 2/232\nsage: F(1/a).numerator()\na^4 - 3*a^3 + 2424*a^2 + 2\nsage: F(1/a).denominator()\n232\n}}}\n\nBut the last three lines highlight a bug in the printing routines.",
    "created_at": "2010-01-17T21:45:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7958#issuecomment-60853",
    "user": "https://github.com/sagetrac-spancratz"
}
```

<div id="comment:4" align="right">comment:4</div>

To see that this issue is now resolved (for rationals and number fields), consider
{{{sage: _.<x> = ZZ[]
sage: K.<a> = NumberField(x<sup>5-3*x</sup>4+2424*x^3+2*x-232)
sage: R.<b> = K.ring_of_integers()
sage: S.<y> = R[]
sage: F = Frac(S)
sage: F(1/a)
a^4 - 3*a^3 + 2424*a^2 + 2/232
sage: F(1/a).numerator()
a^4 - 3*a^3 + 2424*a^2 + 2
sage: F(1/a).denominator()
232
}}}

But the last three lines highlight a bug in the printing routines.



---

archive/issue_events_097990.json:
```json
{
    "actor": "https://github.com/sagetrac-spancratz",
    "created_at": "2010-01-17T21:45:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97990"
}
```



---

archive/issue_events_097991.json:
```json
{
    "actor": "https://github.com/sagetrac-spancratz",
    "created_at": "2010-01-17T21:45:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97991"
}
```



---

archive/issue_events_097992.json:
```json
{
    "actor": "https://github.com/sagetrac-spancratz",
    "created_at": "2010-01-17T21:45:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97992"
}
```



---

archive/issue_events_097993.json:
```json
{
    "actor": "https://github.com/sagetrac-spancratz",
    "created_at": "2010-01-17T21:45:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97993"
}
```



---

archive/issue_comments_060854.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nAttachment: **[trac7958_d.patch.gz](https://github.com/sagemath/sage/files/ticket7958/trac7958_d.patch.gz)**\n\n\n```\n\nsage: _.<x> = ZZ[]\nsage: K.<a> = NumberField(x^5-3*x^4+2424*x^3+2*x-232)\nsage: R.<b> = K.ring_of_integers()\nsage: S.<y> = R[]\nsage: F = Frac(S)\nsage: F(1/a)\na^4 - 3*a^3 + 2424*a^2 + 2/232\nsage: F(1/a).numerator()\na^4 - 3*a^3 + 2424*a^2 + 2\nsage: F(1/a).denominator()\n232\n\n```",
    "created_at": "2010-01-20T04:16:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7958#issuecomment-60854",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:5" align="right">comment:5</div>

Attachment: **[trac7958_d.patch.gz](https://github.com/sagemath/sage/files/ticket7958/trac7958_d.patch.gz)**


```

sage: _.<x> = ZZ[]
sage: K.<a> = NumberField(x^5-3*x^4+2424*x^3+2*x-232)
sage: R.<b> = K.ring_of_integers()
sage: S.<y> = R[]
sage: F = Frac(S)
sage: F(1/a)
a^4 - 3*a^3 + 2424*a^2 + 2/232
sage: F(1/a).numerator()
a^4 - 3*a^3 + 2424*a^2 + 2
sage: F(1/a).denominator()
232

```



---

archive/issue_comments_060855.json:
```json
{
    "body": "Combined version of the above patches.",
    "created_at": "2010-01-20T04:28:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7958#issuecomment-60855",
    "user": "https://github.com/mwhansen"
}
```

Combined version of the above patches.



---

archive/issue_comments_060856.json:
```json
{
    "body": "Attachment: **[trac7958.2.patch.gz](https://github.com/sagemath/sage/files/ticket7958/trac7958.2.patch.gz)**\n\nAttachment: **[trac_7958-atomic.patch.gz](https://github.com/sagemath/sage/files/ticket7958/trac_7958-atomic.patch.gz)**",
    "created_at": "2010-01-20T05:50:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7958#issuecomment-60856",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[trac7958.2.patch.gz](https://github.com/sagemath/sage/files/ticket7958/trac7958.2.patch.gz)**

Attachment: **[trac_7958-atomic.patch.gz](https://github.com/sagemath/sage/files/ticket7958/trac_7958-atomic.patch.gz)**



---

archive/issue_comments_060857.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nApplying the two patches\n\n- trac7958.2.patch\n- trac_7958-atomic.patch\n\nthis applies cleanly and passes all doctests.",
    "created_at": "2010-01-20T07:19:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7958#issuecomment-60857",
    "user": "https://github.com/sagetrac-spancratz"
}
```

<div id="comment:6" align="right">comment:6</div>

Applying the two patches

- trac7958.2.patch
- trac_7958-atomic.patch

this applies cleanly and passes all doctests.



---

archive/issue_comments_060858.json:
```json
{
    "body": "Changed author from **spancratz** to **Sebastian Pancratz, Mike Hansen**",
    "created_at": "2010-01-20T07:19:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7958#issuecomment-60858",
    "user": "https://github.com/sagetrac-spancratz"
}
```

Changed author from **spancratz** to **Sebastian Pancratz, Mike Hansen**



---

archive/issue_events_097994.json:
```json
{
    "actor": "https://github.com/sagetrac-spancratz",
    "created_at": "2010-01-20T07:19:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97994"
}
```



---

archive/issue_events_097995.json:
```json
{
    "actor": "https://github.com/sagetrac-spancratz",
    "created_at": "2010-01-20T07:19:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97995"
}
```



---

archive/issue_comments_060859.json:
```json
{
    "body": "Reviewer: **Mike Hansen, Sebastian Pancratz**",
    "created_at": "2010-01-20T07:19:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7958#issuecomment-60859",
    "user": "https://github.com/sagetrac-spancratz"
}
```

Reviewer: **Mike Hansen, Sebastian Pancratz**



---

archive/issue_comments_060860.json:
```json
{
    "body": "Merged: **sage-4.3.2.alpha0**",
    "created_at": "2010-01-23T07:47:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7958#issuecomment-60860",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **sage-4.3.2.alpha0**



---

archive/issue_events_097996.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-01-23T07:47:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97996"
}
```



---

archive/issue_events_097997.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-01-23T07:47:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7958#event-97997"
}
```



---

archive/issue_comments_060861.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nMerged patches in this order:\n\n1. [trac7958.2.patch](https://github.com/sagemath/sage/files/ticket7958/trac7958.2.patch.gz)\n2. [trac_7958-atomic.patch](https://github.com/sagemath/sage/files/ticket7958/trac_7958-atomic.patch.gz)",
    "created_at": "2010-01-23T07:47:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7958#issuecomment-60861",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:7" align="right">comment:7</div>

Merged patches in this order:

1. [trac7958.2.patch](https://github.com/sagemath/sage/files/ticket7958/trac7958.2.patch.gz)
2. [trac_7958-atomic.patch](https://github.com/sagemath/sage/files/ticket7958/trac_7958-atomic.patch.gz)

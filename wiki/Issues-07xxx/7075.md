# Issue 7075: Two last figure saves to wrong place

archive/issues_007075.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThere are two more instances in plot/plot of saving figures to the current directory as opposed to SAGE_TMP.\n\nCC:  @jhpalmieri @sagetrac-mvngu\n\nComponent: **graphics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7075_\n\n",
    "closed_at": "2009-09-30T15:01:06Z",
    "created_at": "2009-09-29T13:57:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20graphics",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Two last figure saves to wrong place",
    "type": "issue",
    "updated_at": "2009-09-30T15:01:06Z",
    "url": "https://github.com/sagemath/sage/issues/7075",
    "user": "https://github.com/kcrisman"
}
```
<div id="comment:0"></div>

There are two more instances in plot/plot of saving figures to the current directory as opposed to SAGE_TMP.

CC:  @jhpalmieri @sagetrac-mvngu

Component: **graphics**

_Issue created by migration from https://trac.sagemath.org/ticket/7075_





---

archive/issue_events_085486.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-29T13:57:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7075#event-85486"
}
```



---

archive/issue_events_085487.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-29T13:57:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "c: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7075#event-85487"
}
```



---

archive/issue_events_085488.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-29T13:57:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20trivial%20/%205",
    "label_color": "fff9e5",
    "label_name": "p: trivial / 5",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7075#event-85488"
}
```



---

archive/issue_events_085489.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-29T13:57:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7075#event-85489"
}
```



---

archive/issue_events_085490.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-29T13:57:29Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "subject": "https://github.com/kcrisman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7075#event-85490"
}
```



---

archive/issue_comments_050727.json:
```json
{
    "body": "Based on 4.1.2.alpha4",
    "created_at": "2009-09-29T14:02:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7075#issuecomment-50727",
    "user": "https://github.com/kcrisman"
}
```

Based on 4.1.2.alpha4



---

archive/issue_comments_050728.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_7075-save-fig-smart.patch.gz](https://github.com/sagemath/sage/files/ticket7075/trac_7075-save-fig-smart.patch.gz)**\n\nTo release manager: this is a duplicate of #7059, please close.",
    "created_at": "2009-09-29T14:05:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7075#issuecomment-50728",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_7075-save-fig-smart.patch.gz](https://github.com/sagemath/sage/files/ticket7075/trac_7075-save-fig-smart.patch.gz)**

To release manager: this is a duplicate of #7059, please close.



---

archive/issue_comments_050729.json:
```json
{
    "body": "Author: **Karl-Dieter Crisman**",
    "created_at": "2009-09-29T14:05:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7075#issuecomment-50729",
    "user": "https://github.com/kcrisman"
}
```

Author: **Karl-Dieter Crisman**



---

archive/issue_comments_050730.json:
```json
{
    "body": "Changed author from **Karl-Dieter Crisman** to none",
    "created_at": "2009-09-29T14:05:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7075#issuecomment-50730",
    "user": "https://github.com/kcrisman"
}
```

Changed author from **Karl-Dieter Crisman** to none



---

archive/issue_comments_050731.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nTo Minh (if it's not October 1 where you are): please close.",
    "created_at": "2009-09-30T14:58:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7075#issuecomment-50731",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:3" align="right">comment:3</div>

To Minh (if it's not October 1 where you are): please close.



---

archive/issue_events_085491.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-30T15:01:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7075#event-85491"
}
```



---

archive/issue_events_085492.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-30T15:01:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7075#event-85492"
}
```



---

archive/issue_events_085493.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-30T15:01:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20trivial%20/%205",
    "label_color": "fff9e5",
    "label_name": "p: trivial / 5",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7075#event-85493"
}
```



---

archive/issue_events_085494.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-30T15:01:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "c6c6c6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7075#event-85494"
}
```



---

archive/issue_comments_050732.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nClosing this ticket as a duplicate of #7059.",
    "created_at": "2009-09-30T15:01:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7075#issuecomment-50732",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:4" align="right">comment:4</div>

Closing this ticket as a duplicate of #7059.

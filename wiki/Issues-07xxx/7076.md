# Issue 7076: SageNB -- Do ReST introspection on a `worksheet_process`

archive/issues_007076.json:
```json
{
    "assignees": [
        "https://github.com/boothby"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis increases security (prevents a DoS by introspecting constantly), and ensures that it is done in a separate process (which may be in an entirely different server)\n\nComponent: **notebook**\n\nKeywords: **sagenb notebook**\n\nAuthor: **Tim Dumol**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7076_\n\n",
    "closed_at": "2009-09-29T20:14:06Z",
    "created_at": "2009-09-29T19:48:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "SageNB -- Do ReST introspection on a `worksheet_process`",
    "type": "issue",
    "updated_at": "2009-09-29T20:14:06Z",
    "url": "https://github.com/sagemath/sage/issues/7076",
    "user": "https://github.com/TimDumol"
}
```
<div id="comment:0"></div>

This increases security (prevents a DoS by introspecting constantly), and ensures that it is done in a separate process (which may be in an entirely different server)

Component: **notebook**

Keywords: **sagenb notebook**

Author: **Tim Dumol**

_Issue created by migration from https://trac.sagemath.org/ticket/7076_





---

archive/issue_events_085495.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-09-29T19:48:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7076",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7076#event-85495"
}
```



---

archive/issue_events_085496.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-09-29T19:48:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7076",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7076#event-85496"
}
```



---

archive/issue_events_085497.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-09-29T19:48:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7076",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7076#event-85497"
}
```



---

archive/issue_events_085498.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-09-29T19:48:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7076",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7076#event-85498"
}
```



---

archive/issue_events_085499.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-09-29T19:48:30Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7076",
    "subject": "https://github.com/TimDumol",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7076#event-85499"
}
```



---

archive/issue_comments_050733.json:
```json
{
    "body": "Attachment: **[trac_7076-introspection-reimplementation.patch.gz](https://github.com/sagemath/sage/files/ticket7076/trac_7076-introspection-reimplementation.patch.gz)**\n\nReimplements introspection.",
    "created_at": "2009-09-29T19:49:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7076",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7076#issuecomment-50733",
    "user": "https://github.com/TimDumol"
}
```

Attachment: **[trac_7076-introspection-reimplementation.patch.gz](https://github.com/sagemath/sage/files/ticket7076/trac_7076-introspection-reimplementation.patch.gz)**

Reimplements introspection.



---

archive/issue_events_085500.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-29T20:14:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7076",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7076#event-85500"
}
```



---

archive/issue_events_085501.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-29T20:14:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7076",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7076#event-85501"
}
```



---

archive/issue_events_085502.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-29T20:14:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7076",
    "milestone_number": null,
    "milestone_title": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7076#event-85502"
}
```



---

archive/issue_comments_050734.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nI've applied this to the notebook.",
    "created_at": "2009-09-29T20:14:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7076",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7076#issuecomment-50734",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>

I've applied this to the notebook.

# Issue 7076: SageNB -- Do ReST introspection on a `worksheet_process`

archive/issues_007076.json:
```json
{
    "body": "This increases security (prevents a DoS by introspecting constantly), and ensures that it is done in a separate process (which may be in an entirely different server)\n\n**Assignee:** boothby\n\n**Keywords:** sagenb notebook\n\n**Author:** Tim Dumol\n\nIssue created by migration from https://trac.sagemath.org/ticket/7076\n\n",
    "closed_at": "2009-09-29T20:14:06Z",
    "created_at": "2009-09-29T19:48:30Z",
    "labels": [
        "component: notebook",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "title": "SageNB -- Do ReST introspection on a `worksheet_process`",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7076",
    "user": "https://github.com/TimDumol"
}
```
This increases security (prevents a DoS by introspecting constantly), and ensures that it is done in a separate process (which may be in an entirely different server)

**Assignee:** boothby

**Keywords:** sagenb notebook

**Author:** Tim Dumol

Issue created by migration from https://trac.sagemath.org/ticket/7076





---

archive/attachments_008767.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_7076-introspection-reimplementation.patch",
    "asset_url": "tarball://root/attachments/ticket7076/trac_7076-introspection-reimplementation.patch",
    "created_at": "2009-09-29T19:49:59Z",
    "issue": "https://github.com/sagemath/sage/issues/7076",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7076/trac_7076-introspection-reimplementation.patch",
    "user": "https://github.com/TimDumol"
}
```



---

archive/issue_comments_052334.json:
```json
{
    "body": "**Attachment:** [trac_7076-introspection-reimplementation.patch](https://github.com/sagemath/sage/files/ticket7076/trac_7076-introspection-reimplementation.patch)\n\nReimplements introspection.",
    "created_at": "2009-09-29T19:49:59Z",
    "issue": "https://github.com/sagemath/sage/issues/7076",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7076#issuecomment-52334",
    "user": "https://github.com/TimDumol"
}
```

**Attachment:** [trac_7076-introspection-reimplementation.patch](https://github.com/sagemath/sage/files/ticket7076/trac_7076-introspection-reimplementation.patch)

Reimplements introspection.



---

archive/issue_events_047489.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-29T20:14:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7076",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7076#event-47489"
}
```



---

archive/issue_events_047490.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-29T20:14:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7076",
    "milestone": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7076#event-47490"
}
```



---

archive/issue_events_047491.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-29T20:14:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7076",
    "milestone": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7076#event-47491"
}
```



---

archive/issue_comments_052335.json:
```json
{
    "body": "<a id='comment:1'></a>\nI've applied this to the notebook.",
    "created_at": "2009-09-29T20:14:06Z",
    "issue": "https://github.com/sagemath/sage/issues/7076",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7076#issuecomment-52335",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
I've applied this to the notebook.

# Issue 7954: Defining affine curves in 3D space

archive/issues_007954.json:
```json
{
    "body": "Reported by Ronald van Luijk:\n\nbecause the Curve constructor automatically interprets a homogeneous polynomial in 3 variables as a projective curve, the following doesn't work:\n\n```\nA.<x,y,z>=AffineSpace(QQ,3)\nC=Curve([x-y,x-z])\n```\n\n**Assignee:** @aghitza\n\n**CC:**  @kwankyu\n\nIssue created by migration from https://trac.sagemath.org/ticket/7954\n\n",
    "closed_at": "2020-08-22T07:20:12Z",
    "created_at": "2010-01-16T18:15:19Z",
    "labels": [
        "component: algebraic geometry",
        "bug",
        "invalid"
    ],
    "title": "Defining affine curves in 3D space",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7954",
    "user": "https://github.com/wjp"
}
```
Reported by Ronald van Luijk:

because the Curve constructor automatically interprets a homogeneous polynomial in 3 variables as a projective curve, the following doesn't work:

```
A.<x,y,z>=AffineSpace(QQ,3)
C=Curve([x-y,x-z])
```

**Assignee:** @aghitza

**CC:**  @kwankyu

Issue created by migration from https://trac.sagemath.org/ticket/7954





---

archive/attachments_010139.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "7954_curve_constructor.patch",
    "asset_url": "tarball://root/attachments/ticket7954/7954_curve_constructor.patch",
    "created_at": "2010-01-17T03:52:22Z",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7954/7954_curve_constructor.patch",
    "user": "https://github.com/wjp"
}
```



---

archive/issue_comments_062500.json:
```json
{
    "body": "**Attachment:** [7954_curve_constructor.patch](https://github.com/sagemath/sage/files/ticket7954/7954_curve_constructor.patch)",
    "created_at": "2010-01-17T03:52:22Z",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7954#issuecomment-62500",
    "user": "https://github.com/wjp"
}
```

**Attachment:** [7954_curve_constructor.patch](https://github.com/sagemath/sage/files/ticket7954/7954_curve_constructor.patch)



---

archive/issue_comments_062501.json:
```json
{
    "body": "<a id='comment:1'></a>\nThe attached patch adds an optional `space='affine'/'projective'` keyword argument to the Curve constructor.",
    "created_at": "2010-01-17T03:53:24Z",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7954#issuecomment-62501",
    "user": "https://github.com/wjp"
}
```

<a id='comment:1'></a>
The attached patch adds an optional `space='affine'/'projective'` keyword argument to the Curve constructor.



---

archive/issue_events_055476.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-01-17T03:53:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55476"
}
```



---

archive/attachments_010140.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_7954-part2.patch",
    "asset_url": "tarball://root/attachments/ticket7954/trac_7954-part2.patch",
    "created_at": "2010-01-17T09:38:23Z",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7954/trac_7954-part2.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_062502.json:
```json
{
    "body": "**Attachment:** [trac_7954-part2.patch](https://github.com/sagemath/sage/files/ticket7954/trac_7954-part2.patch)\n\nthe previous patch was part 1.  apply this after that one.",
    "created_at": "2010-01-17T09:38:23Z",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7954#issuecomment-62502",
    "user": "https://github.com/williamstein"
}
```

**Attachment:** [trac_7954-part2.patch](https://github.com/sagemath/sage/files/ticket7954/trac_7954-part2.patch)

the previous patch was part 1.  apply this after that one.



---

archive/issue_comments_062503.json:
```json
{
    "body": "<a id='comment:2'></a>\nHi,\n\nI made some important improvements to the patch while refereeing it.  However, it is clear after looking at this code for a bit that there is a *MAJOR* design flaw (which Willem pointed out to me in person).  The flaw is this in plane_curve/affine_curve.py:\n\n```\nclass AffineSpaceCurve_generic(Curve_generic, AlgebraicScheme_subscheme_affine):\n    def _repr_type(self):\n        return \"Affine Space\"\n```\nHowever, Curve_generic is very much a *plane* curve:\n\n```\nclass Curve_generic(AlgebraicScheme_subscheme):\n...\n    def defining_polynomial(self):\n        return self.defining_polynomials()[0]\n```\n\nThus the *isa* relationship that *must* be satisfied by derivation of objects is broken.\n\nThe space curve code must be completely moved out of this directory to the appropriate place.",
    "created_at": "2010-01-17T09:41:20Z",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7954#issuecomment-62503",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>
Hi,

I made some important improvements to the patch while refereeing it.  However, it is clear after looking at this code for a bit that there is a *MAJOR* design flaw (which Willem pointed out to me in person).  The flaw is this in plane_curve/affine_curve.py:

```
class AffineSpaceCurve_generic(Curve_generic, AlgebraicScheme_subscheme_affine):
    def _repr_type(self):
        return "Affine Space"
```
However, Curve_generic is very much a *plane* curve:

```
class Curve_generic(AlgebraicScheme_subscheme):
...
    def defining_polynomial(self):
        return self.defining_polynomials()[0]
```

Thus the *isa* relationship that *must* be satisfied by derivation of objects is broken.

The space curve code must be completely moved out of this directory to the appropriate place.



---

archive/issue_events_055477.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-01-17T09:41:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55477"
}
```



---

archive/issue_events_055478.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-01-17T09:41:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55478"
}
```



---

archive/issue_comments_062504.json:
```json
{
    "body": "<a id='comment:3'></a>\nI'm going to wait to talk to willem about this instead of just trying to do it myself.",
    "created_at": "2010-01-17T09:42:43Z",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7954#issuecomment-62504",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
I'm going to wait to talk to willem about this instead of just trying to do it myself.



---

archive/issue_events_055479.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55479"
}
```



---

archive/issue_events_055480.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55480"
}
```



---

archive/issue_events_055481.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55481"
}
```



---

archive/issue_events_055482.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55482"
}
```



---

archive/issue_events_055483.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55483"
}
```



---

archive/issue_events_055484.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55484"
}
```



---

archive/issue_events_055485.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55485"
}
```



---

archive/issue_events_055486.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55486"
}
```



---

archive/issue_events_055487.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2020-07-06T09:54:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55487"
}
```



---

archive/issue_comments_062505.json:
```json
{
    "body": "<a id='comment:9'></a>\nThe issue seems to have been solved now by adding `A`mbient space argument to the `Curve` constructor.\n\n```\nsage: A.<x,y,z>=AffineSpace(QQ,3)\nsage: C = Curve([x-y,y-z],A)\nsage: C\nAffine Curve over Rational Field defined by x - y, y - z\n```",
    "created_at": "2020-07-06T09:54:53Z",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7954#issuecomment-62505",
    "user": "https://github.com/kwankyu"
}
```

<a id='comment:9'></a>
The issue seems to have been solved now by adding `A`mbient space argument to the `Curve` constructor.

```
sage: A.<x,y,z>=AffineSpace(QQ,3)
sage: C = Curve([x-y,y-z],A)
sage: C
Affine Curve over Rational Field defined by x - y, y - z
```



---

archive/issue_events_055488.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2020-07-06T09:57:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55488"
}
```



---

archive/issue_events_055489.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2020-07-06T09:57:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55489"
}
```



---

archive/issue_events_055490.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-06T12:11:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55490"
}
```



---

archive/issue_events_055491.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-06T12:11:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55491"
}
```



---

archive/issue_events_055492.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-08-22T07:20:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55492"
}
```



---

archive/issue_events_055493.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-08-22T07:20:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7954",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7954#event-55493"
}
```

# Issue 7699: Delete OS X metadata in lcalc spkg

archive/issues_007699.json:
```json
{
    "body": "\n```\n      sage-4.3.rc0/spkg/standard/lcalc-20080205.p3/src/src/.DS_Store\n      sage-4.3.rc0/spkg/standard/lcalc-20080205.p3/src/include/.DS_Store\n```\n\n**Author:** William Stein\n\n**Reviewer:** David Kirkby\n\n**Merged:** sage-4.3.1.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/7699\n\n",
    "closed_at": "2010-01-03T22:21:09Z",
    "created_at": "2009-12-16T01:26:02Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.1",
    "title": "Delete OS X metadata in lcalc spkg",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7699",
    "user": "https://github.com/williamstein"
}
```

```
      sage-4.3.rc0/spkg/standard/lcalc-20080205.p3/src/src/.DS_Store
      sage-4.3.rc0/spkg/standard/lcalc-20080205.p3/src/include/.DS_Store
```

**Author:** William Stein

**Reviewer:** David Kirkby

**Merged:** sage-4.3.1.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/7699





---

archive/issue_events_053099.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-18T06:31:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7699",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7699#event-53099"
}
```



---

archive/issue_comments_059340.json:
```json
{
    "body": "<a id='comment:1'></a>\nSee http://wstein.org/home/wstein/patches/lcalc-20080205.p4.spkg for the new spkg.",
    "created_at": "2009-12-18T06:31:41Z",
    "issue": "https://github.com/sagemath/sage/issues/7699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7699#issuecomment-59340",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
See http://wstein.org/home/wstein/patches/lcalc-20080205.p4.spkg for the new spkg.



---

archive/issue_events_053100.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/drkirkby",
    "created_at": "2009-12-24T01:32:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7699",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7699#event-53100"
}
```



---

archive/issue_events_053101.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/drkirkby",
    "created_at": "2009-12-24T01:32:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7699",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7699#event-53101"
}
```



---

archive/issue_comments_059341.json:
```json
{
    "body": "<a id='comment:2'></a>\nI'll give it a positive, as its clear you have just deleted some useless code. It builds on Solaris at least. \n\nI believe there was a comment on sage-devel about a lot of work being done on this package recently. Hopefully there will be another release, as this one generates a lot of warnings from g++. Sun's compiler will not compile it.",
    "created_at": "2009-12-24T01:32:38Z",
    "issue": "https://github.com/sagemath/sage/issues/7699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7699#issuecomment-59341",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```

<a id='comment:2'></a>
I'll give it a positive, as its clear you have just deleted some useless code. It builds on Solaris at least. 

I believe there was a comment on sage-devel about a lot of work being done on this package recently. Hopefully there will be another release, as this one generates a lot of warnings from g++. Sun's compiler will not compile it.



---

archive/issue_comments_059342.json:
```json
{
    "body": "**Author:** William Stein",
    "created_at": "2010-01-03T22:21:09Z",
    "issue": "https://github.com/sagemath/sage/issues/7699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7699#issuecomment-59342",
    "user": "https://github.com/mwhansen"
}
```

**Author:** William Stein



---

archive/issue_comments_059343.json:
```json
{
    "body": "**Merged:** sage-4.3.1.alpha0",
    "created_at": "2010-01-03T22:21:09Z",
    "issue": "https://github.com/sagemath/sage/issues/7699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7699#issuecomment-59343",
    "user": "https://github.com/mwhansen"
}
```

**Merged:** sage-4.3.1.alpha0



---

archive/issue_events_053102.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-01-03T22:21:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7699",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7699#event-53102"
}
```



---

archive/issue_events_053103.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-01-03T22:21:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7699",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7699#event-53103"
}
```



---

archive/issue_comments_059344.json:
```json
{
    "body": "**Reviewer:** David Kirkby",
    "created_at": "2010-01-03T22:21:09Z",
    "issue": "https://github.com/sagemath/sage/issues/7699",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7699#issuecomment-59344",
    "user": "https://github.com/mwhansen"
}
```

**Reviewer:** David Kirkby

# Issue 7411: improve the speed of inverse RSK

archive/issues_007411.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe main improvement comes from the use of a binary search, just as in #7408.\n\nCC:  @hivert @mwhansen @sagetrac-sage-combinat\n\nComponent: **combinatorics**\n\nKeywords: **Robinson-Schensted**\n\nAuthor: **Yann Laigle-Chapuy**\n\nReviewer: **Florent Hivert**\n\nMerged: **sage-4.2.1.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7411_\n\n",
    "closed_at": "2009-11-12T06:32:55Z",
    "created_at": "2009-11-08T14:01:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.2.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "improve the speed of inverse RSK",
    "type": "issue",
    "updated_at": "2009-11-13T05:08:53Z",
    "url": "https://github.com/sagemath/sage/issues/7411",
    "user": "https://github.com/sagetrac-ylchapuy"
}
```
<div id="comment:0"></div>

The main improvement comes from the use of a binary search, just as in #7408.

CC:  @hivert @mwhansen @sagetrac-sage-combinat

Component: **combinatorics**

Keywords: **Robinson-Schensted**

Author: **Yann Laigle-Chapuy**

Reviewer: **Florent Hivert**

Merged: **sage-4.2.1.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/7411_





---

archive/issue_events_089968.json:
```json
{
    "actor": "https://github.com/sagetrac-ylchapuy",
    "created_at": "2009-11-08T14:01:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7411",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7411#event-89968"
}
```



---

archive/issue_events_089969.json:
```json
{
    "actor": "https://github.com/sagetrac-ylchapuy",
    "created_at": "2009-11-08T14:01:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7411",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7411#event-89969"
}
```



---

archive/issue_events_089970.json:
```json
{
    "actor": "https://github.com/sagetrac-ylchapuy",
    "created_at": "2009-11-08T14:01:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7411",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7411#event-89970"
}
```



---

archive/issue_events_089971.json:
```json
{
    "actor": "https://github.com/sagetrac-ylchapuy",
    "created_at": "2009-11-08T14:01:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7411",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7411#event-89971"
}
```



---

archive/issue_events_089972.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-08T14:01:29Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7411",
    "subject": "https://github.com/sagetrac-ylchapuy",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7411#event-89972"
}
```



---

archive/issue_comments_054247.json:
```json
{
    "body": "needs #7408",
    "created_at": "2009-11-08T14:04:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7411#issuecomment-54247",
    "user": "https://github.com/sagetrac-ylchapuy"
}
```

needs #7408



---

archive/issue_comments_054248.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_7411-inverse_rsk.patch.gz](https://github.com/sagemath/sage/files/ticket7411/trac_7411-inverse_rsk.patch.gz)**",
    "created_at": "2009-11-08T14:04:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7411#issuecomment-54248",
    "user": "https://github.com/sagetrac-ylchapuy"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_7411-inverse_rsk.patch.gz](https://github.com/sagemath/sage/files/ticket7411/trac_7411-inverse_rsk.patch.gz)**



---

archive/issue_events_089973.json:
```json
{
    "actor": "https://github.com/sagetrac-ylchapuy",
    "created_at": "2009-11-08T14:04:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7411",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7411#event-89973"
}
```



---

archive/issue_comments_054249.json:
```json
{
    "body": "Reviewer: **Florent Hivert**",
    "created_at": "2009-11-08T16:46:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7411#issuecomment-54249",
    "user": "https://github.com/hivert"
}
```

Reviewer: **Florent Hivert**



---

archive/issue_comments_054250.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis in indeed a very good idea ! I'm frustrated I didn't get it myself :-) \n\nPatch is good and ready to go. \n\nCheers,\n\nFlorent",
    "created_at": "2009-11-08T16:46:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7411#issuecomment-54250",
    "user": "https://github.com/hivert"
}
```

<div id="comment:2" align="right">comment:2</div>

This in indeed a very good idea ! I'm frustrated I didn't get it myself :-) 

Patch is good and ready to go. 

Cheers,

Florent



---

archive/issue_events_089974.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-11-08T16:46:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7411",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7411#event-89974"
}
```



---

archive/issue_events_089975.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-11-08T16:46:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7411",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7411#event-89975"
}
```



---

archive/issue_events_089976.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-12T06:32:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7411",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7411#event-89976"
}
```



---

archive/issue_events_089977.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-12T06:32:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7411",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7411#event-89977"
}
```



---

archive/issue_comments_054251.json:
```json
{
    "body": "Merged: **sage-4.2.1.rc0**",
    "created_at": "2009-11-12T06:32:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7411#issuecomment-54251",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.2.1.rc0**



---

archive/issue_events_089978.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-11-13T05:08:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7411",
    "milestone_number": null,
    "milestone_title": "sage-4.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7411#event-89978"
}
```

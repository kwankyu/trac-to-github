# Issue 7764: mhansen's 4.3.rc1 merge tree may have got ahead of wstein's

archive/issues_007764.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nA block to take care of when the 4.3.1 merge cycle starts is that mhansen did some merging in I think\n\n   /scratch/mhansen/release/4.3/4.3.rc1\n\nwhich might not have been reflected in the final 4.3 release.  This could have resulted in tickets marked as closed when they weren't closed. \n\nComponent: **misc**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7764_\n\n",
    "closed_at": "2010-01-18T00:36:19Z",
    "created_at": "2009-12-24T20:19:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "mhansen's 4.3.rc1 merge tree may have got ahead of wstein's",
    "type": "issue",
    "updated_at": "2010-01-18T00:36:19Z",
    "url": "https://github.com/sagemath/sage/issues/7764",
    "user": "https://github.com/williamstein"
}
```
<div id="comment:0"></div>

A block to take care of when the 4.3.1 merge cycle starts is that mhansen did some merging in I think

   /scratch/mhansen/release/4.3/4.3.rc1

which might not have been reflected in the final 4.3 release.  This could have resulted in tickets marked as closed when they weren't closed. 

Component: **misc**

_Issue created by migration from https://trac.sagemath.org/ticket/7764_





---

archive/issue_events_095202.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-24T20:19:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7764",
    "milestone_number": null,
    "milestone_title": "sage-4.3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7764#event-95202"
}
```



---

archive/issue_events_095203.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-24T20:19:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7764",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7764#event-95203"
}
```



---

archive/issue_events_095204.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-24T20:19:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7764",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7764#event-95204"
}
```



---

archive/issue_events_095205.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-24T20:19:11Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7764",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7764#event-95205"
}
```



---

archive/issue_comments_058479.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nSee #7769 for one possible such issue.",
    "created_at": "2009-12-26T16:13:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7764#issuecomment-58479",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:1" align="right">comment:1</div>

See #7769 for one possible such issue.



---

archive/issue_events_095206.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-01-18T00:36:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7764",
    "milestone_number": null,
    "milestone_title": "sage-4.3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7764#event-95206"
}
```



---

archive/issue_events_095207.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-01-18T00:36:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7764",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7764#event-95207"
}
```

# Issue 7312: Add a function .st (meaning "such that" )which is an alias to .add_constraint

archive/issues_007312.json:
```json
{
    "body": "Add a function .st (meaning \"such that\" )which is an alias to .add_constraint. This would shorten the code, as the add_constraint option is the most used, and copy the syntax used in Pymprog\n\n**Assignee:** jkantor\n\nIssue created by migration from https://trac.sagemath.org/ticket/7312\n\n",
    "closed_at": "2010-01-14T05:54:56Z",
    "created_at": "2009-10-26T13:35:41Z",
    "labels": [
        "component: numerical",
        "bug",
        "wontfix"
    ],
    "title": "Add a function .st (meaning \"such that\" )which is an alias to .add_constraint",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7312",
    "user": "https://github.com/nathanncohen"
}
```
Add a function .st (meaning "such that" )which is an alias to .add_constraint. This would shorten the code, as the add_constraint option is the most used, and copy the syntax used in Pymprog

**Assignee:** jkantor

Issue created by migration from https://trac.sagemath.org/ticket/7312





---

archive/issue_comments_054728.json:
```json
{
    "body": "<a id='comment:1'></a>\nI'm not convinced that this is a good idea.  I think that the benefit of shorter code is outweighed by making the code more cryptic.",
    "created_at": "2009-10-26T17:18:42Z",
    "issue": "https://github.com/sagemath/sage/issues/7312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7312#issuecomment-54728",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
I'm not convinced that this is a good idea.  I think that the benefit of shorter code is outweighed by making the code more cryptic.



---

archive/issue_comments_054729.json:
```json
{
    "body": "<a id='comment:2'></a>\nWhat would you think of a .such_that method then ?",
    "created_at": "2009-10-26T18:44:48Z",
    "issue": "https://github.com/sagemath/sage/issues/7312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7312#issuecomment-54729",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:2'></a>
What would you think of a .such_that method then ?



---

archive/issue_events_049438.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-10-26T18:44:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7312",
    "label": "needs info",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7312#event-49438"
}
```



---

archive/issue_comments_054730.json:
```json
{
    "body": "<a id='comment:3'></a>\nI don't see how that that is much better than add_constraint.",
    "created_at": "2009-11-02T04:55:45Z",
    "issue": "https://github.com/sagemath/sage/issues/7312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7312#issuecomment-54730",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'></a>
I don't see how that that is much better than add_constraint.



---

archive/issue_comments_054731.json:
```json
{
    "body": "<a id='comment:4'></a>\nTrue enough....\n\nThen this ticket should be closed as \"stupid idea\" :-)\n\nNathann",
    "created_at": "2009-11-02T06:52:33Z",
    "issue": "https://github.com/sagemath/sage/issues/7312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7312#issuecomment-54731",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:4'></a>
True enough....

Then this ticket should be closed as "stupid idea" :-)

Nathann



---

archive/issue_comments_054732.json:
```json
{
    "body": "<a id='comment:5'></a>\nNo need to keep the ticket open then...",
    "created_at": "2010-01-14T05:54:56Z",
    "issue": "https://github.com/sagemath/sage/issues/7312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7312#issuecomment-54732",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:5'></a>
No need to keep the ticket open then...



---

archive/issue_events_049439.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-01-14T05:54:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7312",
    "milestone": "sage-4.3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7312#event-49439"
}
```



---

archive/issue_events_049440.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-01-14T05:54:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7312",
    "label": "wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7312#event-49440"
}
```



---

archive/issue_events_049441.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-01-14T05:54:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7312",
    "label": "needs info",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7312#event-49441"
}
```



---

archive/issue_events_049442.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-01-14T05:54:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7312",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7312#event-49442"
}
```

# Issue 7212: steenrod algebra multiplication bug

archive/issues_007212.json:
```json
{
    "body": "From sage-support:\n\n```\nI have Sage 4.1.1 install on a server, and I run the following code, \nwhich outputs the following error: \nsage: A3=SteenrodAlgebra(3) \nsage: A3.P(36,6)*A3.P(27,9,81) \n--------------------------------------------------------------------------- \nKeyError                                  Traceback (most recent call \nlast) \n/home/user_bob/<ipython console> in <module>() \n/usr/local/sage-4.1.1/local/lib/python2.6/site-packages/sage/structure/ \nelement.so in sage.structure.element.RingElement.__mul__ (sage/ \nstructure/element.c:9956)() \n/usr/local/sage-4.1.1/local/lib/python2.6/site-packages/sage/structure/ \nelement.so in sage.structure.element.RingElement._mul_ (sage/structure/ \nelement.c:10021)() \n/usr/local/sage-4.1.1/local/lib/python2.6/site-packages/sage/algebras/ \nsteenrod_algebra_element.pyc in _mul_(self, other) \n    925                         new_dict = milnor_multiplication \n(mono1, mono2) \n    926                     else: \n--> 927                         new_dict = milnor_multiplication_odd \n(mono1, mono2, p=p) \n    928                     for new_mono in new_dict: \n    929                         if result.has_key(new_mono): \n/usr/local/sage-4.1.1/local/lib/python2.6/site-packages/sage/algebras/ \nsteenrod_milnor_multiplication_odd.pyc in milnor_multiplication_odd \n(m1, m2, p) \n    225                     t = tuple(diagonal[:i+1]) \n    226                     if result.has_key((e,t)): \n--> 227                         result[(e,t)] = F(coeff + result[t]) \n    228                     else: \n    229                         result[(e,t)] = F(coeff) \nKeyError: (26, 8, 86) \n```\nThis is because of a simple bug: instead of `result[t]`, it should be `result[(e,t)]`.\n\n\n**Author:** John Palmieri\n\n**Reviewer:** William Stein\n\n**Merged:** sage-4.2.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/7212\n\n",
    "closed_at": "2009-10-15T08:52:13Z",
    "created_at": "2009-10-14T19:12:43Z",
    "labels": [
        "component: algebra",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.2",
    "title": "steenrod algebra multiplication bug",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7212",
    "user": "https://github.com/jhpalmieri"
}
```
From sage-support:

```
I have Sage 4.1.1 install on a server, and I run the following code, 
which outputs the following error: 
sage: A3=SteenrodAlgebra(3) 
sage: A3.P(36,6)*A3.P(27,9,81) 
--------------------------------------------------------------------------- 
KeyError                                  Traceback (most recent call 
last) 
/home/user_bob/<ipython console> in <module>() 
/usr/local/sage-4.1.1/local/lib/python2.6/site-packages/sage/structure/ 
element.so in sage.structure.element.RingElement.__mul__ (sage/ 
structure/element.c:9956)() 
/usr/local/sage-4.1.1/local/lib/python2.6/site-packages/sage/structure/ 
element.so in sage.structure.element.RingElement._mul_ (sage/structure/ 
element.c:10021)() 
/usr/local/sage-4.1.1/local/lib/python2.6/site-packages/sage/algebras/ 
steenrod_algebra_element.pyc in _mul_(self, other) 
    925                         new_dict = milnor_multiplication 
(mono1, mono2) 
    926                     else: 
--> 927                         new_dict = milnor_multiplication_odd 
(mono1, mono2, p=p) 
    928                     for new_mono in new_dict: 
    929                         if result.has_key(new_mono): 
/usr/local/sage-4.1.1/local/lib/python2.6/site-packages/sage/algebras/ 
steenrod_milnor_multiplication_odd.pyc in milnor_multiplication_odd 
(m1, m2, p) 
    225                     t = tuple(diagonal[:i+1]) 
    226                     if result.has_key((e,t)): 
--> 227                         result[(e,t)] = F(coeff + result[t]) 
    228                     else: 
    229                         result[(e,t)] = F(coeff) 
KeyError: (26, 8, 86) 
```
This is because of a simple bug: instead of `result[t]`, it should be `result[(e,t)]`.


**Author:** John Palmieri

**Reviewer:** William Stein

**Merged:** sage-4.2.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/7212





---

archive/issue_events_048595.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-10-14T21:20:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7212",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7212#event-48595"
}
```



---

archive/attachments_008943.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_7212-steenrod.patch",
    "asset_url": "tarball://root/attachments/ticket7212/trac_7212-steenrod.patch",
    "created_at": "2009-10-14T21:20:35Z",
    "issue": "https://github.com/sagemath/sage/issues/7212",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7212/trac_7212-steenrod.patch",
    "user": "https://github.com/jasongrout"
}
```



---

archive/issue_comments_053524.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_7212-steenrod.patch](https://github.com/sagemath/sage/files/ticket7212/trac_7212-steenrod.patch)",
    "created_at": "2009-10-14T21:20:35Z",
    "issue": "https://github.com/sagemath/sage/issues/7212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7212#issuecomment-53524",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_7212-steenrod.patch](https://github.com/sagemath/sage/files/ticket7212/trac_7212-steenrod.patch)



---

archive/issue_events_048596.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-10-14T21:50:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7212",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7212#event-48596"
}
```



---

archive/issue_events_048597.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-10-14T21:50:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7212",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7212#event-48597"
}
```



---

archive/issue_comments_053525.json:
```json
{
    "body": "**Reviewer:** William Stein",
    "created_at": "2009-10-15T08:52:13Z",
    "issue": "https://github.com/sagemath/sage/issues/7212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7212#issuecomment-53525",
    "user": "https://github.com/mwhansen"
}
```

**Reviewer:** William Stein



---

archive/issue_comments_053526.json:
```json
{
    "body": "**Changing author** from \"jhpalmieri\" to \"John Palmieri\".",
    "created_at": "2009-10-15T08:52:13Z",
    "issue": "https://github.com/sagemath/sage/issues/7212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7212#issuecomment-53526",
    "user": "https://github.com/mwhansen"
}
```

**Changing author** from "jhpalmieri" to "John Palmieri".



---

archive/issue_comments_053527.json:
```json
{
    "body": "**Merged:** sage-4.2.alpha0",
    "created_at": "2009-10-15T08:52:13Z",
    "issue": "https://github.com/sagemath/sage/issues/7212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7212#issuecomment-53527",
    "user": "https://github.com/mwhansen"
}
```

**Merged:** sage-4.2.alpha0



---

archive/issue_events_048598.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-15T08:52:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7212",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7212#event-48598"
}
```



---

archive/issue_events_048599.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-15T08:52:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7212",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7212#event-48599"
}
```

# Issue 7697: Delete OS X meta-crap from gfan-0.3.p4

archive/issues_007697.json:
```json
{
    "body": "\n```\n      sage-4.3.rc0/spkg/standard/gfan-0.3.p4/src/.DS_Store\n```\n\n**Author:** William Stein\n\n**Reviewer:** Karl-Dieter Crisman\n\n**Merged:** sage-4.3.1.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/7697\n\n",
    "closed_at": "2010-01-03T22:24:03Z",
    "created_at": "2009-12-16T01:13:31Z",
    "labels": [
        "component: distribution",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.1",
    "title": "Delete OS X meta-crap from gfan-0.3.p4",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7697",
    "user": "https://github.com/williamstein"
}
```

```
      sage-4.3.rc0/spkg/standard/gfan-0.3.p4/src/.DS_Store
```

**Author:** William Stein

**Reviewer:** Karl-Dieter Crisman

**Merged:** sage-4.3.1.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/7697





---

archive/issue_comments_059333.json:
```json
{
    "body": "<a id='comment:1'></a>\nHere is the spkg:  http://wstein.org/home/wstein/patches/gfan-0.3.p5.spkg",
    "created_at": "2009-12-18T06:26:39Z",
    "issue": "https://github.com/sagemath/sage/issues/7697",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7697#issuecomment-59333",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
Here is the spkg:  http://wstein.org/home/wstein/patches/gfan-0.3.p5.spkg



---

archive/issue_events_053092.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-18T06:26:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7697",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7697#event-53092"
}
```



---

archive/issue_events_053093.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-12-30T03:54:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7697",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7697#event-53093"
}
```



---

archive/issue_events_053094.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-12-30T03:54:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7697",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7697#event-53094"
}
```



---

archive/issue_comments_059334.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis seems fine, and still installs well - SPKG updated, hg fine, passes the (two!) relevant doctest files.  I sometimes have to watch for this as well, which is particularly annoying since Finder doesn't show the dot files.",
    "created_at": "2009-12-30T03:54:07Z",
    "issue": "https://github.com/sagemath/sage/issues/7697",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7697#issuecomment-59334",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'></a>
This seems fine, and still installs well - SPKG updated, hg fine, passes the (two!) relevant doctest files.  I sometimes have to watch for this as well, which is particularly annoying since Finder doesn't show the dot files.



---

archive/issue_comments_059335.json:
```json
{
    "body": "**Author:** William Stein",
    "created_at": "2009-12-30T03:54:07Z",
    "issue": "https://github.com/sagemath/sage/issues/7697",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7697#issuecomment-59335",
    "user": "https://github.com/kcrisman"
}
```

**Author:** William Stein



---

archive/issue_comments_059336.json:
```json
{
    "body": "**Reviewer:** Karl-Dieter Crisman",
    "created_at": "2009-12-30T03:54:07Z",
    "issue": "https://github.com/sagemath/sage/issues/7697",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7697#issuecomment-59336",
    "user": "https://github.com/kcrisman"
}
```

**Reviewer:** Karl-Dieter Crisman



---

archive/issue_events_053095.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-01-03T22:24:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7697",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7697#event-53095"
}
```



---

archive/issue_events_053096.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-01-03T22:24:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7697",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7697#event-53096"
}
```



---

archive/issue_comments_059337.json:
```json
{
    "body": "**Merged:** sage-4.3.1.alpha0",
    "created_at": "2010-01-03T22:24:03Z",
    "issue": "https://github.com/sagemath/sage/issues/7697",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7697#issuecomment-59337",
    "user": "https://github.com/mwhansen"
}
```

**Merged:** sage-4.3.1.alpha0

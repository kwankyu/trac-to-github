# Issue 7525: developer's guide: add information about editing python files in the sage library

archive/issues_007525.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThere have been a few questions on sage-support and sage-devel recently about which files to edit in the Sage library.  The attached patch tries to address this.  (It also deletes a paragraph which got included twice.)\n\n\nComponent: **documentation**\n\nAuthor: **John Palmieri**\n\nReviewer: **Minh Van Nguyen**\n\nMerged: **sage-4.3.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7525_\n\n",
    "closed_at": "2009-11-29T05:46:17Z",
    "created_at": "2009-11-24T18:11:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "developer's guide: add information about editing python files in the sage library",
    "type": "issue",
    "updated_at": "2017-07-19T08:52:40Z",
    "url": "https://github.com/sagemath/sage/issues/7525",
    "user": "https://github.com/jhpalmieri"
}
```
<div id="comment:0"></div>

There have been a few questions on sage-support and sage-devel recently about which files to edit in the Sage library.  The attached patch tries to address this.  (It also deletes a paragraph which got included twice.)


Component: **documentation**

Author: **John Palmieri**

Reviewer: **Minh Van Nguyen**

Merged: **sage-4.3.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/7525_





---

archive/issue_events_091619.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-11-24T18:11:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-91619"
}
```



---

archive/issue_events_091620.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-11-24T18:11:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-91620"
}
```



---

archive/issue_events_091621.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-11-24T18:11:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-91621"
}
```



---

archive/issue_events_091622.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-11-24T18:11:04Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "subject": "https://github.com/jhpalmieri",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-91622"
}
```



---

archive/issue_comments_055555.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_7525-developer.patch.gz](https://github.com/sagemath/sage/files/ticket7525/trac_7525-developer.patch.gz)**",
    "created_at": "2009-11-24T18:15:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7525#issuecomment-55555",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_7525-developer.patch.gz](https://github.com/sagemath/sage/files/ticket7525/trac_7525-developer.patch.gz)**



---

archive/issue_events_091623.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-11-24T18:15:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-91623"
}
```



---

archive/issue_events_091624.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-11-25T05:30:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "milestone_number": null,
    "milestone_title": "sage-4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-91624"
}
```



---

archive/issue_events_091625.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-11-25T05:30:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-91625"
}
```



---

archive/issue_events_091626.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-11-25T05:30:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-91626"
}
```



---

archive/issue_comments_055556.json:
```json
{
    "body": "Reviewer: **Minh Van Nguyen**",
    "created_at": "2009-11-25T05:30:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7525#issuecomment-55556",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **Minh Van Nguyen**



---

archive/issue_comments_055557.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nApplied OK against Sage 4.3.alpha0 and rebuilding of the Developers' Guide went fine. The expository content of the patch `trac_7525-developer.patch` is reasonable. I can confirm that that patch also removes a duplicate section.",
    "created_at": "2009-11-25T05:30:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7525#issuecomment-55557",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:2" align="right">comment:2</div>

Applied OK against Sage 4.3.alpha0 and rebuilding of the Developers' Guide went fine. The expository content of the patch `trac_7525-developer.patch` is reasonable. I can confirm that that patch also removes a duplicate section.



---

archive/issue_events_091627.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-29T05:46:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-91627"
}
```



---

archive/issue_events_091628.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-29T05:46:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7525#event-91628"
}
```



---

archive/issue_comments_055558.json:
```json
{
    "body": "Merged: **sage-4.3.alpha1**",
    "created_at": "2009-11-29T05:46:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7525#issuecomment-55558",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.3.alpha1**



---

archive/issue_comments_055559.json:
```json
{
    "body": "Changed author from **John H. Palmieri** to **John Palmieri**",
    "created_at": "2017-07-19T08:52:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7525#issuecomment-55559",
    "user": "https://github.com/fchapoton"
}
```

Changed author from **John H. Palmieri** to **John Palmieri**



---

archive/issue_comments_055560.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nunique name please",
    "created_at": "2017-07-19T08:52:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7525#issuecomment-55560",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:4" align="right">comment:4</div>

unique name please

# Issue 7210: Make revolution plot 3d

archive/issues_007210.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nMathematica has something called RevolutionPlot3D.  Maybe we should too.\n\n[http://reference.wolfram.com/mathematica/ref/RevolutionPlot3D.html](http://reference.wolfram.com/mathematica/ref/RevolutionPlot3D.html)\n\nThis is now done in #7889\n\nComponent: **graphics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7210_\n\n",
    "closed_at": "2010-02-01T04:11:34Z",
    "created_at": "2009-10-14T16:57:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graphics",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make revolution plot 3d",
    "type": "issue",
    "updated_at": "2010-02-01T04:11:34Z",
    "url": "https://github.com/sagemath/sage/issues/7210",
    "user": "https://github.com/kcrisman"
}
```
<div id="comment:0"></div>

Mathematica has something called RevolutionPlot3D.  Maybe we should too.

[http://reference.wolfram.com/mathematica/ref/RevolutionPlot3D.html](http://reference.wolfram.com/mathematica/ref/RevolutionPlot3D.html)

This is now done in #7889

Component: **graphics**

_Issue created by migration from https://trac.sagemath.org/ticket/7210_





---

archive/issue_events_087260.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-10-14T16:57:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "milestone_number": null,
    "milestone_title": "sage-4.3.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7210#event-87260"
}
```



---

archive/issue_events_087261.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-10-14T16:57:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "c: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7210#event-87261"
}
```



---

archive/issue_events_087262.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-10-14T16:57:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7210#event-87262"
}
```



---

archive/issue_events_087263.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-10-14T16:57:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7210#event-87263"
}
```



---

archive/issue_events_087264.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-10-14T16:57:23Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "subject": "https://github.com/kcrisman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7210#event-87264"
}
```



---

archive/issue_comments_051905.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nI've already adressed this at #7889",
    "created_at": "2010-01-10T08:25:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7210#issuecomment-51905",
    "user": "https://github.com/sagetrac-olazo"
}
```

<div id="comment:1" align="right">comment:1</div>

I've already adressed this at #7889



---

archive/issue_events_087265.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2010-02-01T02:21:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7210#event-87265"
}
```



---

archive/issue_events_087266.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2010-02-01T02:21:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7210#event-87266"
}
```



---

archive/issue_events_087267.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2010-02-01T02:21:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "c6c6c6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7210#event-87267"
}
```



---

archive/issue_comments_051906.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n Mathematica has something called RevolutionPlot3D.  Maybe we should too.\n \n [http://reference.wolfram.com/mathematica/ref/RevolutionPlot3D.html](http://reference.wolfram.com/mathematica/ref/RevolutionPlot3D.html)\n+\n+This is now done in #7889\n``````\n",
    "created_at": "2010-02-01T02:21:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7210#issuecomment-51906",
    "user": "https://github.com/sagetrac-olazo"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 Mathematica has something called RevolutionPlot3D.  Maybe we should too.
 
 [http://reference.wolfram.com/mathematica/ref/RevolutionPlot3D.html](http://reference.wolfram.com/mathematica/ref/RevolutionPlot3D.html)
+
+This is now done in #7889
``````




---

archive/issue_comments_051907.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMake sure you understand the guidelines on closing tickets. See [this section](http://www.sagemath.org/doc/developer/trac.html#closing-tickets) of the Developers' Guide for more information.",
    "created_at": "2010-02-01T03:31:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7210#issuecomment-51907",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:3" align="right">comment:3</div>

Make sure you understand the guidelines on closing tickets. See [this section](http://www.sagemath.org/doc/developer/trac.html#closing-tickets) of the Developers' Guide for more information.



---

archive/issue_events_087268.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-01T03:32:21Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "milestone_number": null,
    "milestone_title": "sage-4.3.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7210#event-87268"
}
```



---

archive/issue_comments_051908.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nWhoops sorry!",
    "created_at": "2010-02-01T03:54:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7210#issuecomment-51908",
    "user": "https://github.com/sagetrac-olazo"
}
```

<div id="comment:5" align="right">comment:5</div>

Whoops sorry!



---

archive/issue_events_087269.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2010-02-01T03:56:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "c6c6c6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7210#event-87269"
}
```



---

archive/issue_events_087270.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2010-02-01T03:56:06Z",
    "event": "reopened",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7210#event-87270"
}
```



---

archive/issue_events_087271.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-01T04:11:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7210#event-87271"
}
```



---

archive/issue_comments_051909.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nBe careful not to reopen tickets. I'm closing this as a duplicate of #7889.",
    "created_at": "2010-02-01T04:11:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7210#issuecomment-51909",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:7" align="right">comment:7</div>

Be careful not to reopen tickets. I'm closing this as a duplicate of #7889.



---

archive/issue_events_087272.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-01T04:11:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7210",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "c6c6c6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7210#event-87272"
}
```

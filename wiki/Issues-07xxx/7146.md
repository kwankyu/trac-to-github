# Issue 7146: MAKE and sqlite on Solaris: bomb!

archive/issues_007146.json:
```json
{
    "body": "I set MAKE to \"make -j10\" on solaris x86 (disk.math) and while building sqlite got quite a surprise:\n\n```\nwstein@disk:~$ ps -u wstein  |grep make|wc -l\n    5915\n```\n\n**Reviewer:** Jeroen Demeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/7146\n\n",
    "closed_at": "2013-05-27T11:24:59Z",
    "created_at": "2009-10-07T15:33:27Z",
    "labels": [
        "component: build",
        "minor",
        "bug",
        "invalid"
    ],
    "title": "MAKE and sqlite on Solaris: bomb!",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7146",
    "user": "https://github.com/williamstein"
}
```
I set MAKE to "make -j10" on solaris x86 (disk.math) and while building sqlite got quite a surprise:

```
wstein@disk:~$ ps -u wstein  |grep make|wc -l
    5915
```

**Reviewer:** Jeroen Demeyer

Issue created by migration from https://trac.sagemath.org/ticket/7146





---

archive/issue_events_048027.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-23T14:03:39Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7146",
    "milestone": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7146#event-48027"
}
```



---

archive/issue_comments_052981.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2013-05-23T14:03:39Z",
    "issue": "https://github.com/sagemath/sage/issues/7146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7146#issuecomment-52981",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_comments_052982.json:
```json
{
    "body": "<a id='comment:1'></a>\nI assume this refers to an old version of SQLite which is no longer relevant.",
    "created_at": "2013-05-23T14:03:39Z",
    "issue": "https://github.com/sagemath/sage/issues/7146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7146#issuecomment-52982",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'></a>
I assume this refers to an old version of SQLite which is no longer relevant.



---

archive/issue_events_048028.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-23T14:03:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7146",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7146#event-48028"
}
```



---

archive/issue_events_048029.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-23T14:03:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7146",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7146#event-48029"
}
```



---

archive/issue_events_048030.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-23T14:03:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7146",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7146#event-48030"
}
```



---

archive/issue_events_048031.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-27T11:24:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7146",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7146#event-48031"
}
```



---

archive/issue_events_048032.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-27T11:24:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7146",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7146#event-48032"
}
```

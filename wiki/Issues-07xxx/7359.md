# Issue 7359: Methods for degenerated graphs

archive/issues_007359.json:
```json
{
    "body": "A graph is said to be k-degenerated if it can be totally decomposed by successively removing vertices of degree <= k.\n\nThere should be in Sage a function answering if a graph is k-degenerated, and in this case giving the order in which the vertices should be deleted.\n\n**Assignee:** @rlmill\n\nIssue created by migration from https://trac.sagemath.org/ticket/7359\n\n",
    "closed_at": "2010-10-09T08:43:39Z",
    "created_at": "2009-10-31T08:53:48Z",
    "labels": [
        "component: graph theory",
        "enhancement",
        "duplicate"
    ],
    "title": "Methods for degenerated graphs",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7359",
    "user": "https://github.com/nathanncohen"
}
```
A graph is said to be k-degenerated if it can be totally decomposed by successively removing vertices of degree <= k.

There should be in Sage a function answering if a graph is k-degenerated, and in this case giving the order in which the vertices should be deleted.

**Assignee:** @rlmill

Issue created by migration from https://trac.sagemath.org/ticket/7359





---

archive/issue_events_049851.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-06-06T11:00:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7359",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7359#event-49851"
}
```



---

archive/issue_comments_055221.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis is exactly what the recently-reviewed #9058 does.\n\nNathann",
    "created_at": "2010-10-09T08:33:05Z",
    "issue": "https://github.com/sagemath/sage/issues/7359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7359#issuecomment-55221",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:2'></a>
This is exactly what the recently-reviewed #9058 does.

Nathann



---

archive/issue_events_049852.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-10-09T08:43:39Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7359",
    "milestone": "sage-4.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7359#event-49852"
}
```



---

archive/issue_events_049853.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-10-09T08:43:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7359",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7359#event-49853"
}
```



---

archive/issue_events_049854.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-10-09T08:43:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7359",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7359#event-49854"
}
```



---

archive/issue_events_049855.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-10-09T08:43:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7359",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7359#event-49855"
}
```

# Issue 7510: is_finite method in categories (was Primes is missing is_finite)

archive/issues_007510.json:
```json
{
    "assignees": [
        "https://github.com/hivert"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nI found that Primes has no methods `is_finite`. This is actually not a problem of `Prime` but should be dealt in categories. I put a small patch here though i'm not sure we want to do this. Isn't this redundent with categories ? \n\nCheers,\n\nFlorent\n \n\nComponent: **categories**\n\nKeywords: **Primes, is_finite**\n\nAuthor: **Florent Hivert**\n\nReviewer: **Mike Hansen**\n\nMerged: **sage-4.3.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7510_\n\n",
    "closed_at": "2009-12-01T05:30:54Z",
    "created_at": "2009-11-21T15:20:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20categories",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "is_finite method in categories (was Primes is missing is_finite)",
    "type": "issue",
    "updated_at": "2009-12-26T18:11:24Z",
    "url": "https://github.com/sagemath/sage/issues/7510",
    "user": "https://github.com/hivert"
}
```
<div id="comment:0"></div>

I found that Primes has no methods `is_finite`. This is actually not a problem of `Prime` but should be dealt in categories. I put a small patch here though i'm not sure we want to do this. Isn't this redundent with categories ? 

Cheers,

Florent
 

Component: **categories**

Keywords: **Primes, is_finite**

Author: **Florent Hivert**

Reviewer: **Mike Hansen**

Merged: **sage-4.3.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/7510_





---

archive/issue_events_091385.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-11-21T15:20:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "milestone_number": null,
    "milestone_title": "sage-4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7510#event-91385"
}
```



---

archive/issue_events_091386.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-11-21T15:20:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "c: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7510#event-91386"
}
```



---

archive/issue_events_091387.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-11-21T15:20:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7510#event-91387"
}
```



---

archive/issue_events_091388.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-11-21T15:20:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7510#event-91388"
}
```



---

archive/issue_events_091389.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-21T15:20:15Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "subject": "https://github.com/hivert",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7510#event-91389"
}
```



---

archive/issue_events_091390.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-21T15:20:27Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "subject": "https://github.com/hivert",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7510#event-91390"
}
```



---

archive/issue_events_091391.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-11-21T15:20:27Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "subject": "https://github.com/hivert",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7510#event-91391"
}
```



---

archive/issue_events_091392.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-11-21T15:44:39Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "title_is": "is_finite method in categories (was Primes is missing is_finite).",
    "title_was": "Primes is missing is_finite.",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7510#event-91392"
}
```



---

archive/issue_events_091393.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-11-21T15:44:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "c: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7510#event-91393"
}
```



---

archive/issue_events_091394.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-11-21T15:44:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20categories",
    "label_color": "0000ff",
    "label_name": "c: categories",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7510#event-91394"
}
```



---

archive/issue_comments_055339.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,15 +1,6 @@\n-Primes has no methods `is_finite`. This breaks several thing including: \n+I found that Primes has no methods `is_finite`. This is actually not a problem of `Prime` but should be dealt in categories. I put a small patch here though i'm not sure we want to do this. Isn't this redundent with categories ? \n \n-```\n-sage: contre_exemples = (p for p in Primes() and not is_prime(mersenne(p)))\n+Cheers,\n \n----------------------------------------------------------------------------\n-AttributeError                            Traceback (most recent call last)\n-\n-/home/averell/.sage/temp/tomahawk/25868/_home_averell__sage_init_sage_0.py in <module>()\n-\n-/usr/local/sage/sage-4.2/local/lib/python2.6/site-packages/sage/structure/parent.so in sage.structure.parent.Set_generic.__nonzero__ (sage/structure/parent.c:14641)()\n-\n-AttributeError: 'Primes_with_category' object has no attribute 'is_finite'\n-```\n-\n+Florent\n+ \n``````\n",
    "created_at": "2009-11-21T15:44:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7510#issuecomment-55339",
    "user": "https://github.com/hivert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,15 +1,6 @@
-Primes has no methods `is_finite`. This breaks several thing including: 
+I found that Primes has no methods `is_finite`. This is actually not a problem of `Prime` but should be dealt in categories. I put a small patch here though i'm not sure we want to do this. Isn't this redundent with categories ? 
 
-```
-sage: contre_exemples = (p for p in Primes() and not is_prime(mersenne(p)))
+Cheers,
 
----------------------------------------------------------------------------
-AttributeError                            Traceback (most recent call last)
-
-/home/averell/.sage/temp/tomahawk/25868/_home_averell__sage_init_sage_0.py in <module>()
-
-/usr/local/sage/sage-4.2/local/lib/python2.6/site-packages/sage/structure/parent.so in sage.structure.parent.Set_generic.__nonzero__ (sage/structure/parent.c:14641)()
-
-AttributeError: 'Primes_with_category' object has no attribute 'is_finite'
-```
-
+Florent
+ 
``````




---

archive/issue_events_091395.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2009-11-21T15:45:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7510#event-91395"
}
```



---

archive/issue_comments_055340.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nFor the second example, don't you want an EnumeratedSet and not a FiniteEnumeratedSet?",
    "created_at": "2009-11-21T19:18:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7510#issuecomment-55340",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:4" align="right">comment:4</div>

For the second example, don't you want an EnumeratedSet and not a FiniteEnumeratedSet?



---

archive/issue_comments_055341.json:
```json
{
    "body": "Attachment: **[trac_7510-is_finite_categories.patch.gz](https://github.com/sagemath/sage/files/ticket7510/trac_7510-is_finite_categories.patch.gz)**",
    "created_at": "2009-11-21T22:43:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7510#issuecomment-55341",
    "user": "https://github.com/hivert"
}
```

Attachment: **[trac_7510-is_finite_categories.patch.gz](https://github.com/sagemath/sage/files/ticket7510/trac_7510-is_finite_categories.patch.gz)**



---

archive/issue_comments_055342.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nReplying to [@mwhansen](#comment%3A4):\n> For the second example, don't you want an EnumeratedSet and not a FiniteEnumeratedSet?\n\nSure ! The example didn't even pass the test... I forgot to re-export before uploading the patch... Thank for pointing this out and sorry for the trouble...",
    "created_at": "2009-11-21T22:45:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7510#issuecomment-55342",
    "user": "https://github.com/hivert"
}
```

<div id="comment:5" align="right">comment:5</div>

Replying to [@mwhansen](#comment%3A4):
> For the second example, don't you want an EnumeratedSet and not a FiniteEnumeratedSet?

Sure ! The example didn't even pass the test... I forgot to re-export before uploading the patch... Thank for pointing this out and sorry for the trouble...



---

archive/issue_comments_055343.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nLooks good.",
    "created_at": "2009-12-01T05:30:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7510#issuecomment-55343",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:6" align="right">comment:6</div>

Looks good.



---

archive/issue_comments_055344.json:
```json
{
    "body": "Reviewer: **Mike Hansen**",
    "created_at": "2009-12-01T05:30:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7510#issuecomment-55344",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **Mike Hansen**



---

archive/issue_events_091396.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-01T05:30:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7510#event-91396"
}
```



---

archive/issue_events_091397.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-01T05:30:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7510#event-91397"
}
```



---

archive/issue_comments_055345.json:
```json
{
    "body": "Merged: **sage-4.3.alpha1**",
    "created_at": "2009-12-01T05:30:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7510#issuecomment-55345",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.3.alpha1**



---

archive/issue_events_091398.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-12-26T18:11:24Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/7510",
    "title_is": "is_finite method in categories (was Primes is missing is_finite)",
    "title_was": "is_finite method in categories (was Primes is missing is_finite).",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7510#event-91398"
}
```

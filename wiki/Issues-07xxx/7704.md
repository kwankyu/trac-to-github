# Issue 7704: bug in sparse matrix det

archive/issues_007704.json:
```json
{
    "body": "\n```\nsage: matrix(ZZ,4,sparse=True).det()\n...\nTypeError: charpoly() takes at most 1 positional argument (2 given)\n```\n\n**Assignee:** @williamstein\n\n**CC:**  spancratz\n\n**Author:** William Stein\n\n**Reviewer:** Sebastian Pancratz\n\n**Merged:** sage-4.3.1.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/7704\n\n",
    "closed_at": "2010-01-03T21:32:11Z",
    "created_at": "2009-12-16T08:13:27Z",
    "labels": [
        "component: linear algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.1",
    "title": "bug in sparse matrix det",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7704",
    "user": "https://github.com/williamstein"
}
```

```
sage: matrix(ZZ,4,sparse=True).det()
...
TypeError: charpoly() takes at most 1 positional argument (2 given)
```

**Assignee:** @williamstein

**CC:**  spancratz

**Author:** William Stein

**Reviewer:** Sebastian Pancratz

**Merged:** sage-4.3.1.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/7704





---

archive/attachments_009718.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_7704.patch",
    "asset_url": "tarball://root/attachments/ticket7704/trac_7704.patch",
    "created_at": "2009-12-16T08:34:24Z",
    "issue": "https://github.com/sagemath/sage/issues/7704",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7704/trac_7704.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_059403.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_7704.patch](https://github.com/sagemath/sage/files/ticket7704/trac_7704.patch)",
    "created_at": "2009-12-16T08:34:24Z",
    "issue": "https://github.com/sagemath/sage/issues/7704",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7704#issuecomment-59403",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_7704.patch](https://github.com/sagemath/sage/files/ticket7704/trac_7704.patch)



---

archive/issue_events_053140.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-16T08:34:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7704",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7704#event-53140"
}
```



---

archive/issue_comments_059404.json:
```json
{
    "body": "Very minor cosmetic change to the method",
    "created_at": "2009-12-19T01:08:52Z",
    "issue": "https://github.com/sagemath/sage/issues/7704",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7704#issuecomment-59404",
    "user": "https://trac.sagemath.org/admin/accounts/users/spancratz"
}
```

Very minor cosmetic change to the method



---

archive/attachments_009719.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_7704b.patch",
    "asset_url": "tarball://root/attachments/ticket7704/trac_7704b.patch",
    "created_at": "2009-12-19T01:13:59Z",
    "issue": "https://github.com/sagemath/sage/issues/7704",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7704/trac_7704b.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/spancratz"
}
```



---

archive/issue_comments_059405.json:
```json
{
    "body": "<a id='comment:2'></a>\n**Attachment:** [trac_7704b.patch](https://github.com/sagemath/sage/files/ticket7704/trac_7704b.patch)\n\nThe patch looks fine, and ``make test`` on my installation of SAGE 4.2.1 returns only one unrelated error, which I include for completeness:\n\n```\nsage -t  \"devel/sage/sage/plot/plot3d/tachyon.py\"\n```\n\n```\nFile \"/scratch/pancratz/sage-4.2.1/devel/sage/sage/plot/plot3d/tachyon.py\", line 297:\n    sage: os.system('rm ' + tempname)\nExpected:\n    0\nGot:\n    256\n```\n\nI have attached an additional patch which removes one unused local variable.\n\nSebastian",
    "created_at": "2009-12-19T01:13:59Z",
    "issue": "https://github.com/sagemath/sage/issues/7704",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7704#issuecomment-59405",
    "user": "https://trac.sagemath.org/admin/accounts/users/spancratz"
}
```

<a id='comment:2'></a>
**Attachment:** [trac_7704b.patch](https://github.com/sagemath/sage/files/ticket7704/trac_7704b.patch)

The patch looks fine, and ``make test`` on my installation of SAGE 4.2.1 returns only one unrelated error, which I include for completeness:

```
sage -t  "devel/sage/sage/plot/plot3d/tachyon.py"
```

```
File "/scratch/pancratz/sage-4.2.1/devel/sage/sage/plot/plot3d/tachyon.py", line 297:
    sage: os.system('rm ' + tempname)
Expected:
    0
Got:
    256
```

I have attached an additional patch which removes one unused local variable.

Sebastian



---

archive/issue_events_053141.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-29T08:34:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7704",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7704#event-53141"
}
```



---

archive/issue_events_053142.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-12-29T08:34:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7704",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7704#event-53142"
}
```



---

archive/issue_comments_059406.json:
```json
{
    "body": "**Merged:** sage-4.3.1.alpha0",
    "created_at": "2010-01-03T21:32:11Z",
    "issue": "https://github.com/sagemath/sage/issues/7704",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7704#issuecomment-59406",
    "user": "https://github.com/mwhansen"
}
```

**Merged:** sage-4.3.1.alpha0



---

archive/issue_events_053143.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-01-03T21:32:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7704",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7704#event-53143"
}
```



---

archive/issue_events_053144.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-01-03T21:32:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7704",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7704#event-53144"
}
```



---

archive/issue_comments_059407.json:
```json
{
    "body": "**Author:** William Stein",
    "created_at": "2010-01-03T21:32:11Z",
    "issue": "https://github.com/sagemath/sage/issues/7704",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7704#issuecomment-59407",
    "user": "https://github.com/mwhansen"
}
```

**Author:** William Stein



---

archive/issue_comments_059408.json:
```json
{
    "body": "**Reviewer:** Sebastian Pancratz",
    "created_at": "2010-01-03T21:32:11Z",
    "issue": "https://github.com/sagemath/sage/issues/7704",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7704#issuecomment-59408",
    "user": "https://github.com/mwhansen"
}
```

**Reviewer:** Sebastian Pancratz

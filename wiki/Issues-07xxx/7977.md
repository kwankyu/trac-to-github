# Issue 7977: explain how to use the MATH block and automatic numbering with hash-dot

archive/issues_007977.json:
```json
{
    "body": "See this [sage-combinat-devel](http://groups.google.com/group/sage-combinat-devel/browse_thread/thread/3a846ce7ba9fdb1f) thread:\n\n```\nIs there anywhere strict specification/possibilities for the docstring\nof functions ? My only source was the devel guide but it does not\nmention the \".. math:\" ReST tag and contains two different conventions\nfor the OUTPUT part of the docstrings \n```\n\n**Assignee:** mvngu\n\n**Author:** Minh Van Nguyen\n\n**Reviewer:** John Palmieri\n\n**Merged:** sage-4.3.1.rc1\n\nIssue created by migration from https://trac.sagemath.org/ticket/7977\n\n",
    "closed_at": "2010-01-19T00:07:50Z",
    "created_at": "2010-01-18T14:18:22Z",
    "labels": [
        "component: documentation",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.1",
    "title": "explain how to use the MATH block and automatic numbering with hash-dot",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7977",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```
See this [sage-combinat-devel](http://groups.google.com/group/sage-combinat-devel/browse_thread/thread/3a846ce7ba9fdb1f) thread:

```
Is there anywhere strict specification/possibilities for the docstring
of functions ? My only source was the devel guide but it does not
mention the ".. math:" ReST tag and contains two different conventions
for the OUTPUT part of the docstrings 
```

**Assignee:** mvngu

**Author:** Minh Van Nguyen

**Reviewer:** John Palmieri

**Merged:** sage-4.3.1.rc1

Issue created by migration from https://trac.sagemath.org/ticket/7977





---

archive/issue_comments_062669.json:
```json
{
    "body": "based on Sage 4.3.1.rc0",
    "created_at": "2010-01-18T14:20:35Z",
    "issue": "https://github.com/sagemath/sage/issues/7977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7977#issuecomment-62669",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

based on Sage 4.3.1.rc0



---

archive/attachments_010177.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_7977-math-block.patch",
    "asset_url": "tarball://root/attachments/ticket7977/trac_7977-math-block.patch",
    "created_at": "2010-01-18T14:21:11Z",
    "issue": "https://github.com/sagemath/sage/issues/7977",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7977/trac_7977-math-block.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```



---

archive/issue_comments_062670.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_7977-math-block.patch](https://github.com/sagemath/sage/files/ticket7977/trac_7977-math-block.patch)",
    "created_at": "2010-01-18T14:21:11Z",
    "issue": "https://github.com/sagemath/sage/issues/7977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7977#issuecomment-62670",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_7977-math-block.patch](https://github.com/sagemath/sage/files/ticket7977/trac_7977-math-block.patch)



---

archive/issue_comments_062671.json:
```json
{
    "body": "**Author:** Minh Van Nguyen",
    "created_at": "2010-01-18T14:21:11Z",
    "issue": "https://github.com/sagemath/sage/issues/7977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7977#issuecomment-62671",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Author:** Minh Van Nguyen



---

archive/issue_events_055659.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-01-18T14:21:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7977",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7977#event-55659"
}
```



---

archive/issue_comments_062672.json:
```json
{
    "body": "apply on top of the other patch",
    "created_at": "2010-01-18T16:46:18Z",
    "issue": "https://github.com/sagemath/sage/issues/7977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7977#issuecomment-62672",
    "user": "https://github.com/jhpalmieri"
}
```

apply on top of the other patch



---

archive/issue_comments_062673.json:
```json
{
    "body": "**Reviewer:** John Palmieri",
    "created_at": "2010-01-18T16:48:14Z",
    "issue": "https://github.com/sagemath/sage/issues/7977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7977#issuecomment-62673",
    "user": "https://github.com/jhpalmieri"
}
```

**Reviewer:** John Palmieri



---

archive/attachments_010178.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_7977-extra.patch",
    "asset_url": "tarball://root/attachments/ticket7977/trac_7977-extra.patch",
    "created_at": "2010-01-18T16:48:14Z",
    "issue": "https://github.com/sagemath/sage/issues/7977",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7977/trac_7977-extra.patch",
    "user": "https://github.com/jhpalmieri"
}
```



---

archive/issue_comments_062674.json:
```json
{
    "body": "<a id='comment:2'></a>\n**Attachment:** [trac_7977-extra.patch](https://github.com/sagemath/sage/files/ticket7977/trac_7977-extra.patch)\n\nLooks good to me.  I've attached a tiny patch fixing two typos unrelated to mvngu's patch.",
    "created_at": "2010-01-18T16:48:14Z",
    "issue": "https://github.com/sagemath/sage/issues/7977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7977#issuecomment-62674",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'></a>
**Attachment:** [trac_7977-extra.patch](https://github.com/sagemath/sage/files/ticket7977/trac_7977-extra.patch)

Looks good to me.  I've attached a tiny patch fixing two typos unrelated to mvngu's patch.



---

archive/issue_events_055660.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-01-18T16:48:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7977",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7977#event-55660"
}
```



---

archive/issue_events_055661.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-01-18T16:48:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7977",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7977#event-55661"
}
```



---

archive/issue_events_055662.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-01-19T00:07:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7977",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7977#event-55662"
}
```



---

archive/issue_events_055663.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-01-19T00:07:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7977",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7977#event-55663"
}
```



---

archive/issue_comments_062675.json:
```json
{
    "body": "**Merged:** sage-4.3.1.rc1",
    "created_at": "2010-01-19T00:07:50Z",
    "issue": "https://github.com/sagemath/sage/issues/7977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7977#issuecomment-62675",
    "user": "https://github.com/rlmill"
}
```

**Merged:** sage-4.3.1.rc1

# Issue 7369: Split graph.py into several files

archive/issues_007369.json:
```json
{
    "assignees": [
        "https://github.com/rlmill"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe file graph.py should be split into several files in next release. I am thinking about creating 3 files :\n* generic_graph.py\n* graph.py\n* digraph.py\nBut it is likely many of you will have better ideas. Regardless of what is chosen, this is getting urgent as the best moment to do it is just before releasing a new version, just after all the patches for graph.py have been merged.\n\nComponent: **graph theory**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7369_\n\n",
    "closed_at": "2009-12-17T16:24:51Z",
    "created_at": "2009-11-01T15:34:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Split graph.py into several files",
    "type": "issue",
    "updated_at": "2009-12-17T16:24:51Z",
    "url": "https://github.com/sagemath/sage/issues/7369",
    "user": "https://github.com/nathanncohen"
}
```
<div id="comment:0"></div>

The file graph.py should be split into several files in next release. I am thinking about creating 3 files :
* generic_graph.py
* graph.py
* digraph.py
But it is likely many of you will have better ideas. Regardless of what is chosen, this is getting urgent as the best moment to do it is just before releasing a new version, just after all the patches for graph.py have been merged.

Component: **graph theory**

_Issue created by migration from https://trac.sagemath.org/ticket/7369_





---

archive/issue_events_089368.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-11-01T15:34:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7369",
    "milestone_number": null,
    "milestone_title": "sage-4.3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7369#event-89368"
}
```



---

archive/issue_events_089369.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-11-01T15:34:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7369",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7369#event-89369"
}
```



---

archive/issue_events_089370.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-11-01T15:34:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7369",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7369#event-89370"
}
```



---

archive/issue_events_089371.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-11-01T15:34:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7369",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7369#event-89371"
}
```



---

archive/issue_events_089372.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-11-01T15:34:19Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7369",
    "subject": "https://github.com/nathanncohen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7369#event-89372"
}
```



---

archive/issue_comments_053670.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nWhy was this made a defect?  a blocker??  I'm changing it to not be a blocker or defect.",
    "created_at": "2009-11-11T17:45:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7369#issuecomment-53670",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>

Why was this made a defect?  a blocker??  I'm changing it to not be a blocker or defect.



---

archive/issue_events_089373.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-11T17:45:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7369",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7369#event-89373"
}
```



---

archive/issue_events_089374.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-11T17:45:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7369",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7369#event-89374"
}
```



---

archive/issue_events_089375.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-11T17:45:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7369",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7369#event-89375"
}
```



---

archive/issue_events_089376.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-11T17:45:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7369",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7369#event-89376"
}
```



---

archive/issue_comments_053671.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI set it to \"blocker\" because it had to be done just before releasing. It is not easy to split a file in two if there are patches to be applied on this very file, so I thought the best way would be to take care of this just before releasing, thus setting it as a blocker to avoid it being forgotten, as it will most probably be done by the release manager ! :-)\n\nWhat's your advice ???\n\nNathann",
    "created_at": "2009-11-11T19:16:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7369#issuecomment-53671",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:2" align="right">comment:2</div>

I set it to "blocker" because it had to be done just before releasing. It is not easy to split a file in two if there are patches to be applied on this very file, so I thought the best way would be to take care of this just before releasing, thus setting it as a blocker to avoid it being forgotten, as it will most probably be done by the release manager ! :-)

What's your advice ???

Nathann



---

archive/issue_comments_053672.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nTo me, blocker = high priority ticket that we *have* to close before making the next release of Sage.     I don't think refactoring code ever satisfies that criterion. \n\nI think you should set this as an enhancement then work with the release manager (and post to sage-release) to do the refactoring once you're sure about what you want to do.  \n\n -- william",
    "created_at": "2009-11-12T00:04:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7369#issuecomment-53672",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:3" align="right">comment:3</div>

To me, blocker = high priority ticket that we *have* to close before making the next release of Sage.     I don't think refactoring code ever satisfies that criterion. 

I think you should set this as an enhancement then work with the release manager (and post to sage-release) to do the refactoring once you're sure about what you want to do.  

 -- william



---

archive/issue_events_089377.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-17T16:24:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7369",
    "milestone_number": null,
    "milestone_title": "sage-4.3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7369#event-89377"
}
```



---

archive/issue_comments_053673.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nCopy of #7634",
    "created_at": "2009-12-17T16:24:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7369",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7369#issuecomment-53673",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">comment:4</div>

Copy of #7634



---

archive/issue_events_089378.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-17T16:24:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7369",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7369#event-89378"
}
```



---

archive/issue_events_089379.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-17T16:24:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7369",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "c6c6c6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7369#event-89379"
}
```



---

archive/issue_events_089380.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-12-17T16:24:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7369",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7369#event-89380"
}
```

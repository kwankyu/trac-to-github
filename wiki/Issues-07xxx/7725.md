# Issue 7725: Segmentation fault in coerce_actions.c

archive/issues_007725.json:
```json
{
    "body": "In Sage 4.3.rc0 it is possible to cause a segmentation fault as described in this message:\n\nhttp://groups.google.com/group/sage-devel/browse_thread/thread/35fd7617cf3c89a5?hl=en\n\n**Assignee:** @dwbump\n\n**CC:**  @roed314\n\nIssue created by migration from https://trac.sagemath.org/ticket/7725\n\n",
    "closed_at": "2009-12-20T19:01:58Z",
    "created_at": "2009-12-17T17:54:29Z",
    "labels": [
        "component: coercion",
        "bug",
        "duplicate"
    ],
    "title": "Segmentation fault in coerce_actions.c",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7725",
    "user": "https://github.com/dwbump"
}
```
In Sage 4.3.rc0 it is possible to cause a segmentation fault as described in this message:

http://groups.google.com/group/sage-devel/browse_thread/thread/35fd7617cf3c89a5?hl=en

**Assignee:** @dwbump

**CC:**  @roed314

Issue created by migration from https://trac.sagemath.org/ticket/7725





---

archive/issue_comments_059636.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis issue is fixed by #7718.",
    "created_at": "2009-12-17T19:20:17Z",
    "issue": "https://github.com/sagemath/sage/issues/7725",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7725#issuecomment-59636",
    "user": "https://github.com/roed314"
}
```

<a id='comment:1'></a>
This issue is fixed by #7718.



---

archive/issue_comments_059637.json:
```json
{
    "body": "<a id='comment:2'></a>\nSince the issue is fixed by #7718 I am changing the status to fixed.",
    "created_at": "2009-12-18T16:07:09Z",
    "issue": "https://github.com/sagemath/sage/issues/7725",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7725#issuecomment-59637",
    "user": "https://github.com/dwbump"
}
```

<a id='comment:2'></a>
Since the issue is fixed by #7718 I am changing the status to fixed.



---

archive/issue_comments_059638.json:
```json
{
    "body": "**Changing assignee** from @robertwb to @dwbump.",
    "created_at": "2009-12-18T16:07:09Z",
    "issue": "https://github.com/sagemath/sage/issues/7725",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7725#issuecomment-59638",
    "user": "https://github.com/dwbump"
}
```

**Changing assignee** from @robertwb to @dwbump.



---

archive/issue_events_053365.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2009-12-20T19:01:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7725",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7725#event-53365"
}
```



---

archive/issue_comments_059639.json:
```json
{
    "body": "<a id='comment:3'></a>\nSince #7718 is merged this patch can be closed.",
    "created_at": "2009-12-20T19:01:58Z",
    "issue": "https://github.com/sagemath/sage/issues/7725",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7725#issuecomment-59639",
    "user": "https://github.com/dwbump"
}
```

<a id='comment:3'></a>
Since #7718 is merged this patch can be closed.



---

archive/issue_comments_059640.json:
```json
{
    "body": "<a id='comment:4'></a>\nClosing as a duplicate of #7718.",
    "created_at": "2009-12-21T04:26:51Z",
    "issue": "https://github.com/sagemath/sage/issues/7725",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7725#issuecomment-59640",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:4'></a>
Closing as a duplicate of #7718.



---

archive/issue_events_053366.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-12-21T04:26:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7725",
    "milestone": "sage-4.3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7725#event-53366"
}
```



---

archive/issue_events_053367.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-12-21T04:26:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7725",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7725#event-53367"
}
```

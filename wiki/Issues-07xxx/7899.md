# Issue 7899: Remove variable names like $LN, $MKDIR etc in ntl

archive/issues_007899.json:
```json
{
    "body": "It was agreed recently that variables would not be used for very common commands like MV, MKDIR etc.\n\n http://groups.google.com/group/sage-devel/browse_thread/thread/bd7ae07a1157bead/970aa0dc8fa56ab7?lnk=raot\n\n#7818 usets these, so this package will break. The fix is to simply replace things like\n\n$LN with 'ln'\n\nAn updated .spkg can be found at \n\nhttp://boxen.math.washington.edu/home/kirkby/portability/ntl-5.4.2.p10/ntl-5.4.2.p10.spkg\n\n\n\n**Assignee:** GeorgSWeber\n\n**CC:**  @jaapspies @jhpalmieri\n\n**Author:** David Kirkby\n\n**Reviewer:** Jaap Spies\n\n**Merged:** sage-4.3.1.rc0\n\nIssue created by migration from https://trac.sagemath.org/ticket/7899\n\n",
    "closed_at": "2010-01-14T02:57:30Z",
    "created_at": "2010-01-12T03:55:50Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.1",
    "title": "Remove variable names like $LN, $MKDIR etc in ntl",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/7899",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```
It was agreed recently that variables would not be used for very common commands like MV, MKDIR etc.

 http://groups.google.com/group/sage-devel/browse_thread/thread/bd7ae07a1157bead/970aa0dc8fa56ab7?lnk=raot

#7818 usets these, so this package will break. The fix is to simply replace things like

$LN with 'ln'

An updated .spkg can be found at 

http://boxen.math.washington.edu/home/kirkby/portability/ntl-5.4.2.p10/ntl-5.4.2.p10.spkg



**Assignee:** GeorgSWeber

**CC:**  @jaapspies @jhpalmieri

**Author:** David Kirkby

**Reviewer:** Jaap Spies

**Merged:** sage-4.3.1.rc0

Issue created by migration from https://trac.sagemath.org/ticket/7899





---

archive/issue_events_055008.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/drkirkby",
    "created_at": "2010-01-12T03:57:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7899",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7899#event-55008"
}
```



---

archive/attachments_010036.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "ntl-variables-to-commands.patch",
    "asset_url": "tarball://root/attachments/ticket7899/ntl-variables-to-commands.patch",
    "created_at": "2010-01-12T03:57:09Z",
    "issue": "https://github.com/sagemath/sage/issues/7899",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket7899/ntl-variables-to-commands.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```



---

archive/issue_comments_061831.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [ntl-variables-to-commands.patch](https://github.com/sagemath/sage/files/ticket7899/ntl-variables-to-commands.patch)",
    "created_at": "2010-01-12T03:57:09Z",
    "issue": "https://github.com/sagemath/sage/issues/7899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7899#issuecomment-61831",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```

<a id='comment:1'></a>
**Attachment:** [ntl-variables-to-commands.patch](https://github.com/sagemath/sage/files/ticket7899/ntl-variables-to-commands.patch)



---

archive/issue_events_055009.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-12T11:11:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7899",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7899#event-55009"
}
```



---

archive/issue_events_055010.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-12T11:11:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7899",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7899#event-55010"
}
```



---

archive/issue_comments_061832.json:
```json
{
    "body": "<a id='comment:3'></a>\nThe new spkg looks good. Positive review.\n\nJaap",
    "created_at": "2010-01-12T11:11:34Z",
    "issue": "https://github.com/sagemath/sage/issues/7899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7899#issuecomment-61832",
    "user": "https://github.com/jaapspies"
}
```

<a id='comment:3'></a>
The new spkg looks good. Positive review.

Jaap



---

archive/issue_comments_061833.json:
```json
{
    "body": "**Reviewer:** Jaap Spies",
    "created_at": "2010-01-12T11:11:34Z",
    "issue": "https://github.com/sagemath/sage/issues/7899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7899#issuecomment-61833",
    "user": "https://github.com/jaapspies"
}
```

**Reviewer:** Jaap Spies



---

archive/issue_comments_061834.json:
```json
{
    "body": "**Merged:** sage-4.3.1.rc0",
    "created_at": "2010-01-14T02:57:30Z",
    "issue": "https://github.com/sagemath/sage/issues/7899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7899#issuecomment-61834",
    "user": "https://github.com/rlmill"
}
```

**Merged:** sage-4.3.1.rc0



---

archive/issue_events_055011.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-01-14T02:57:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7899",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7899#event-55011"
}
```



---

archive/issue_events_055012.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-01-14T02:57:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7899",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7899#event-55012"
}
```

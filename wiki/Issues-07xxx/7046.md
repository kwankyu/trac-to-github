# Issue 7046: [with spkg and patch] Singular fails to build on OS X 10.6 with case sensitive file system

archive/issues_007046.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThere is a typo in spkg-install so that a patched file doesn't get copied and building fails.\n\nComponent: **packages: standard**\n\nAuthor: **Ivan Andrus**\n\nReviewer: **William Stein, Minh Van Nguyen**\n\nMerged: **Sage 4.1.2.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7046_\n\n",
    "closed_at": "2009-09-30T12:32:53Z",
    "created_at": "2009-09-28T08:34:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "[with spkg and patch] Singular fails to build on OS X 10.6 with case sensitive file system",
    "type": "issue",
    "updated_at": "2009-09-30T12:32:53Z",
    "url": "https://github.com/sagemath/sage/issues/7046",
    "user": "https://github.com/gvol"
}
```
<div id="comment:0"></div>

There is a typo in spkg-install so that a patched file doesn't get copied and building fails.

Component: **packages: standard**

Author: **Ivan Andrus**

Reviewer: **William Stein, Minh Van Nguyen**

Merged: **Sage 4.1.2.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/7046_





---

archive/issue_events_085087.json:
```json
{
    "actor": "https://github.com/gvol",
    "created_at": "2009-09-28T08:34:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7046#event-85087"
}
```



---

archive/issue_events_085088.json:
```json
{
    "actor": "https://github.com/gvol",
    "created_at": "2009-09-28T08:34:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20build",
    "label_color": "0000b0",
    "label_name": "c: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7046#event-85088"
}
```



---

archive/issue_events_085089.json:
```json
{
    "actor": "https://github.com/gvol",
    "created_at": "2009-09-28T08:34:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7046#event-85089"
}
```



---

archive/issue_events_085090.json:
```json
{
    "actor": "https://github.com/gvol",
    "created_at": "2009-09-28T08:34:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7046#event-85090"
}
```



---

archive/issue_events_085091.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-28T08:38:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7046#event-85091"
}
```



---

archive/issue_events_085092.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-28T08:38:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7046#event-85092"
}
```



---

archive/issue_events_085093.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-28T08:38:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7046#event-85093"
}
```



---

archive/issue_events_085094.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-28T08:39:22Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7046#event-85094"
}
```



---

archive/issue_events_085095.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-28T08:39:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "milestone_number": null,
    "milestone_title": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7046#event-85095"
}
```



---

archive/issue_comments_050552.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nShould be patched against the `spkg-install` of the Singular spkg.",
    "created_at": "2009-09-28T08:41:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7046#issuecomment-50552",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:4" align="right">comment:4</div>

Should be patched against the `spkg-install` of the Singular spkg.



---

archive/issue_events_085096.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-09-28T08:42:52Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "subject": "https://github.com/sagetrac-mvngu",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7046#event-85096"
}
```



---

archive/issue_events_085097.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-28T08:42:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20build",
    "label_color": "0000b0",
    "label_name": "c: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7046#event-85097"
}
```



---

archive/issue_events_085098.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-28T08:42:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "0000b0",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7046#event-85098"
}
```



---

archive/issue_comments_050553.json:
```json
{
    "body": "based on Sage 4.1.2.alpha4",
    "created_at": "2009-09-28T08:52:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7046#issuecomment-50553",
    "user": "https://github.com/sagetrac-mvngu"
}
```

based on Sage 4.1.2.alpha4



---

archive/issue_comments_050554.json:
```json
{
    "body": "Author: **Ivan Andrus**",
    "created_at": "2009-09-28T08:57:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7046#issuecomment-50554",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Ivan Andrus**



---

archive/issue_comments_050555.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nAttachment: **[trac_7046-singular_build.patch.gz](https://github.com/sagemath/sage/files/ticket7046/trac_7046-singular_build.patch.gz)**\n\nUpdated Singular package at\n\nhttp://sage.math.washington.edu/home/mvngu/release/spkg/standard/singular-3-1-0-4-20090818.p1.spkg",
    "created_at": "2009-09-28T08:57:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7046#issuecomment-50555",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:6" align="right">comment:6</div>

Attachment: **[trac_7046-singular_build.patch.gz](https://github.com/sagemath/sage/files/ticket7046/trac_7046-singular_build.patch.gz)**

Updated Singular package at

http://sage.math.washington.edu/home/mvngu/release/spkg/standard/singular-3-1-0-4-20090818.p1.spkg



---

archive/issue_comments_050556.json:
```json
{
    "body": "Reviewer: **William Stein, Minh Van Nguyen**",
    "created_at": "2009-09-28T08:57:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7046#issuecomment-50556",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **William Stein, Minh Van Nguyen**



---

archive/issue_events_085099.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-28T08:57:07Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "title_is": "[with spkg and patch] Singular fails to build on OS X 10.6 with case sensitive file system",
    "title_was": "Singular fails to build on OS X 10.6 with case sensitive file system",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7046#event-85099"
}
```



---

archive/issue_comments_050557.json:
```json
{
    "body": "Attachment: **[doctest-4.1.2.alpha4-bsd.math-7046.log.bz2.gz](https://github.com/sagemath/sage/files/ticket7046/doctest-4.1.2.alpha4-bsd.math-7046.log.bz2.gz)**\n\ndoctest log for bsd.math with Sage 4.1.2.alpha4 and updated Singular spkg",
    "created_at": "2009-09-30T04:13:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7046#issuecomment-50557",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[doctest-4.1.2.alpha4-bsd.math-7046.log.bz2.gz](https://github.com/sagemath/sage/files/ticket7046/doctest-4.1.2.alpha4-bsd.math-7046.log.bz2.gz)**

doctest log for bsd.math with Sage 4.1.2.alpha4 and updated Singular spkg



---

archive/issue_comments_050558.json:
```json
{
    "body": "doctest log for cicero with Sage 4.1.2.alpha4 and updated Singular spkg",
    "created_at": "2009-09-30T04:26:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7046#issuecomment-50558",
    "user": "https://github.com/sagetrac-mvngu"
}
```

doctest log for cicero with Sage 4.1.2.alpha4 and updated Singular spkg



---

archive/issue_comments_050559.json:
```json
{
    "body": "Attachment: **[doctest-4.1.2.alpha4-cicero-7046.log.bz2.gz](https://github.com/sagemath/sage/files/ticket7046/doctest-4.1.2.alpha4-cicero-7046.log.bz2.gz)**\n\ninstall log for t2.math with Sage 4.1.2.alpha4 and updated Singular spkg",
    "created_at": "2009-09-30T12:00:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7046#issuecomment-50559",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[doctest-4.1.2.alpha4-cicero-7046.log.bz2.gz](https://github.com/sagemath/sage/files/ticket7046/doctest-4.1.2.alpha4-cicero-7046.log.bz2.gz)**

install log for t2.math with Sage 4.1.2.alpha4 and updated Singular spkg



---

archive/issue_comments_050560.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nAttachment: **[install-4.1.2.alpha4-t2.math-7046.log.bz2.gz](https://github.com/sagemath/sage/files/ticket7046/install-4.1.2.alpha4-t2.math-7046.log.bz2.gz)**\n\nReport of my testing on various platforms with the updated Singular package and the cliquer spkg at #6681:\n\n* sage.math: 64-bit Ubuntu 8.04.3 LTS --- compile OK; all doctests pass.\n* bsd.math: 64-bit Mac OS X 10.6 --- compile OK; many doctest failures, all of which have been reported to [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/cdcf10c5c730f5b5). The full doctest log is attached.\n* rosemary.math: 64-bit RHEL 5.4 --- compile OK; all doctests pass.\n* cicero on SkyNet: 32-bit Fedora 9 --- compile OK; some doctest failures, all of which have been reported to [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/6abc4e530a6cd626). The full doctest log is attached.\n* eno on SkyNet: 64-bit Fedora 9 --- compile OK; all doctests pass.\n* lena on SkyNet: 64-bit RHEL 5.3 --- compile OK; all doctests pass.\n* menas on SkyNet: 64-bit openSUSE 11.1 --- compile OK; all doctests pass.\n* t2.math: SPARC Solaris with GCC 4.4.1 --- compilation abort when trying to install sage-4.1.2.alpha4.spkg; got past installing Singular, though. The full install log is attached.",
    "created_at": "2009-09-30T12:02:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7046#issuecomment-50560",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:7" align="right">comment:7</div>

Attachment: **[install-4.1.2.alpha4-t2.math-7046.log.bz2.gz](https://github.com/sagemath/sage/files/ticket7046/install-4.1.2.alpha4-t2.math-7046.log.bz2.gz)**

Report of my testing on various platforms with the updated Singular package and the cliquer spkg at #6681:

* sage.math: 64-bit Ubuntu 8.04.3 LTS --- compile OK; all doctests pass.
* bsd.math: 64-bit Mac OS X 10.6 --- compile OK; many doctest failures, all of which have been reported to [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/cdcf10c5c730f5b5). The full doctest log is attached.
* rosemary.math: 64-bit RHEL 5.4 --- compile OK; all doctests pass.
* cicero on SkyNet: 32-bit Fedora 9 --- compile OK; some doctest failures, all of which have been reported to [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/6abc4e530a6cd626). The full doctest log is attached.
* eno on SkyNet: 64-bit Fedora 9 --- compile OK; all doctests pass.
* lena on SkyNet: 64-bit RHEL 5.3 --- compile OK; all doctests pass.
* menas on SkyNet: 64-bit openSUSE 11.1 --- compile OK; all doctests pass.
* t2.math: SPARC Solaris with GCC 4.4.1 --- compilation abort when trying to install sage-4.1.2.alpha4.spkg; got past installing Singular, though. The full install log is attached.



---

archive/issue_events_085100.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-30T12:32:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7046#event-85100"
}
```



---

archive/issue_events_085101.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-30T12:32:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7046#event-85101"
}
```



---

archive/issue_comments_050561.json:
```json
{
    "body": "Merged: **Sage 4.1.2.rc0**",
    "created_at": "2009-09-30T12:32:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7046#issuecomment-50561",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **Sage 4.1.2.rc0**



---

archive/issue_comments_050562.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nMerged `singular-3-1-0-4-20090818.p1.spkg` in the standard packages repository.",
    "created_at": "2009-09-30T12:32:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7046#issuecomment-50562",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:8" align="right">comment:8</div>

Merged `singular-3-1-0-4-20090818.p1.spkg` in the standard packages repository.

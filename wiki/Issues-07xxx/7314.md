# Issue 7314: Average distance, Wiener Index, Szeged index

archive/issues_007314.json:
```json
{
    "assignees": [
        "https://github.com/rlmill"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nHello !!\n\nThis patch defines :\n* The average distance between vertices : Graph.average_distance\n* The Szeged Index of a graph : Graph.szeged_index\n* The Wiener Index of a graph : Graph.wiener_index\n\nEverything this patch adds (except the average distance) is documented in :\nhttp://www.sciencedirect.com/science?_ob=ArticleURL&_udi=B6TY9-3VVCHY8-9&_user=10&_rdoc=1&_fmt=&_orig=search&_sort=d&_docanchor=&view=c&_acct=C000050221&_version=1&_urlVersion=0&_userid=10&md5=6d22be39b064af51023439c3bb59c459\n\nThis reference is mentioned in the docstrings.\n\nNathann\n\n\nComponent: **graph theory**\n\nAuthor: **Nathann Cohen**\n\nReviewer: **Robert Miller**\n\nMerged: **sage-4.3.rc1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/7314_\n\n",
    "closed_at": "2009-12-15T17:29:02Z",
    "created_at": "2009-10-26T16:09:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Average distance, Wiener Index, Szeged index",
    "type": "issue",
    "updated_at": "2009-12-15T17:29:02Z",
    "url": "https://github.com/sagemath/sage/issues/7314",
    "user": "https://github.com/nathanncohen"
}
```
<div id="comment:0"></div>

Hello !!

This patch defines :
* The average distance between vertices : Graph.average_distance
* The Szeged Index of a graph : Graph.szeged_index
* The Wiener Index of a graph : Graph.wiener_index

Everything this patch adds (except the average distance) is documented in :
http://www.sciencedirect.com/science?_ob=ArticleURL&_udi=B6TY9-3VVCHY8-9&_user=10&_rdoc=1&_fmt=&_orig=search&_sort=d&_docanchor=&view=c&_acct=C000050221&_version=1&_urlVersion=0&_userid=10&md5=6d22be39b064af51023439c3bb59c459

This reference is mentioned in the docstrings.

Nathann


Component: **graph theory**

Author: **Nathann Cohen**

Reviewer: **Robert Miller**

Merged: **sage-4.3.rc1**

_Issue created by migration from https://trac.sagemath.org/ticket/7314_





---

archive/issue_events_088655.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-10-26T16:09:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "milestone_number": null,
    "milestone_title": "sage-4.3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7314#event-88655"
}
```



---

archive/issue_events_088656.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-10-26T16:09:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7314#event-88656"
}
```



---

archive/issue_events_088657.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-10-26T16:09:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7314#event-88657"
}
```



---

archive/issue_events_088658.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-10-26T16:09:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7314#event-88658"
}
```



---

archive/issue_events_088659.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-10-26T16:09:59Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "subject": "https://github.com/nathanncohen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7314#event-88659"
}
```



---

archive/issue_events_088660.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-10-26T16:12:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7314#event-88660"
}
```



---

archive/issue_comments_053121.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nIn the docstrings, you should not put comments on the sage: line like in \n\n```\nEXAMPLE:: \n\n    sage: # From [2], cited in [1] \n    sage: g=graphs.PathGraph(10) \n    sage: w=lambda x: (x*(x*x -1)/6) \n    sage: g.wiener_index()==w(10) \n    True \n```\n\nInstead, it should be\n\n```\nEXAMPLE:\n\nFrom [2], cited in [1]::\n\n    sage: g=graphs.PathGraph(10) \n    sage: w=lambda x: (x*(x*x -1)/6) \n    sage: g.wiener_index()==w(10) \n    True \n```\n\nAlso, you should see the way that references are handled elsewhere in the Sage library.",
    "created_at": "2009-10-26T16:59:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7314#issuecomment-53121",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:2" align="right">comment:2</div>

In the docstrings, you should not put comments on the sage: line like in 

```
EXAMPLE:: 

    sage: # From [2], cited in [1] 
    sage: g=graphs.PathGraph(10) 
    sage: w=lambda x: (x*(x*x -1)/6) 
    sage: g.wiener_index()==w(10) 
    True 
```

Instead, it should be

```
EXAMPLE:

From [2], cited in [1]::

    sage: g=graphs.PathGraph(10) 
    sage: w=lambda x: (x*(x*x -1)/6) 
    sage: g.wiener_index()==w(10) 
    True 
```

Also, you should see the way that references are handled elsewhere in the Sage library.



---

archive/issue_comments_053122.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI hope you will prefer this one ! I had taken as examples other docstrings which you may find badly formatted... This time, I took as an example Minh's code from Cliques functiosn, knowing that never I witnessed Minh making the slightest error :-)\n\nNathann",
    "created_at": "2009-10-26T19:03:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7314#issuecomment-53122",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:3" align="right">comment:3</div>

I hope you will prefer this one ! I had taken as examples other docstrings which you may find badly formatted... This time, I took as an example Minh's code from Cliques functiosn, knowing that never I witnessed Minh making the slightest error :-)

Nathann



---

archive/issue_events_088661.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-12-15T17:17:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7314#event-88661"
}
```



---

archive/issue_events_088662.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-12-15T17:17:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7314#event-88662"
}
```



---

archive/issue_comments_053123.json:
```json
{
    "body": "Reviewer: **Robert Miller**",
    "created_at": "2009-12-15T17:17:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7314#issuecomment-53123",
    "user": "https://github.com/rlmill"
}
```

Reviewer: **Robert Miller**



---

archive/issue_comments_053124.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nAttachment: **[trac_7314.patch.gz](https://github.com/sagemath/sage/files/ticket7314/trac_7314.patch.gz)**",
    "created_at": "2009-12-15T17:17:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7314#issuecomment-53124",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:4" align="right">comment:4</div>

Attachment: **[trac_7314.patch.gz](https://github.com/sagemath/sage/files/ticket7314/trac_7314.patch.gz)**



---

archive/issue_comments_053125.json:
```json
{
    "body": "Author: **Nathann Cohen**",
    "created_at": "2009-12-15T17:17:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7314#issuecomment-53125",
    "user": "https://github.com/rlmill"
}
```

Author: **Nathann Cohen**



---

archive/issue_events_088663.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-15T17:29:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "milestone_number": null,
    "milestone_title": "sage-4.3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7314#event-88663"
}
```



---

archive/issue_events_088664.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-15T17:29:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "milestone_number": null,
    "milestone_title": "sage-4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7314#event-88664"
}
```



---

archive/issue_events_088665.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-15T17:29:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7314#event-88665"
}
```



---

archive/issue_events_088666.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-12-15T17:29:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/7314#event-88666"
}
```



---

archive/issue_comments_053126.json:
```json
{
    "body": "Merged: **sage-4.3.rc1**",
    "created_at": "2009-12-15T17:29:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/7314",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/7314#issuecomment-53126",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.3.rc1**

# Issue 17710: Context to add signal handler

archive/issues_017473.json:
```json
{
    "body": "Implement\n\n```\nwith SignalHandler(signal.SIGCHILD, handler):\n    # ...\n```\nto temporarily *add* an additional signal handler.\n\n**Reviewer:** Jeroen Demeyer\n\n**Author:** Jeroen Demeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/17710\n\n",
    "closed_at": "2017-06-09T12:14:06Z",
    "created_at": "2015-02-01T11:24:36Z",
    "labels": [
        "component: cython",
        "enhancement",
        "duplicate"
    ],
    "title": "Context to add signal handler",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/17710",
    "user": "https://github.com/jdemeyer"
}
```
Implement

```
with SignalHandler(signal.SIGCHILD, handler):
    # ...
```
to temporarily *add* an additional signal handler.

**Reviewer:** Jeroen Demeyer

**Author:** Jeroen Demeyer

Issue created by migration from https://trac.sagemath.org/ticket/17710





---

archive/issue_events_160224.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-02-01T11:25:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17710",
    "label": "component: misc",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17710#event-160224"
}
```



---

archive/issue_events_160225.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-02-01T11:25:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17710",
    "label": "component: cython",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17710#event-160225"
}
```



---

archive/issue_comments_241632.json:
```json
{
    "body": "<a id='comment:2'></a>\nSuperseded by http://cysignals.readthedocs.io/en/latest/pysignals.html#cysignals.pysignals.changesignal",
    "created_at": "2017-06-09T12:14:06Z",
    "issue": "https://github.com/sagemath/sage/issues/17710",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17710#issuecomment-241632",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'></a>
Superseded by http://cysignals.readthedocs.io/en/latest/pysignals.html#cysignals.pysignals.changesignal



---

archive/issue_events_160226.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-09T12:14:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17710",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17710#event-160226"
}
```



---

archive/issue_events_160227.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-09T12:14:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17710",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17710#event-160227"
}
```



---

archive/issue_comments_241633.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2017-06-09T12:14:06Z",
    "issue": "https://github.com/sagemath/sage/issues/17710",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17710#issuecomment-241633",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_events_160228.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-06-09T12:14:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17710",
    "milestone": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17710#event-160228"
}
```

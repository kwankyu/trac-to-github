# Issue 17795: Configure the gp interface in a single place

archive/issues_017558.json:
```json
{
    "body": "\n\n**Reviewer:** Julien Puydt, Karl-Dieter Crisman\n\nIssue created by migration from https://trac.sagemath.org/ticket/17795\n\n",
    "closed_at": "2015-02-17T19:30:10Z",
    "created_at": "2015-02-17T15:59:25Z",
    "labels": [
        "invalid"
    ],
    "title": "Configure the gp interface in a single place",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/17795",
    "user": "https://trac.sagemath.org/admin/accounts/users/Snark"
}
```


**Reviewer:** Julien Puydt, Karl-Dieter Crisman

Issue created by migration from https://trac.sagemath.org/ticket/17795





---

archive/issue_comments_242811.json:
```json
{
    "body": "<a id='comment:1'></a>\ngit-trac got stuck on that one, I had to kill it... but I apparently can't kill a ticket myself! Setting to needs_review so someone can close it...",
    "created_at": "2015-02-17T16:14:50Z",
    "issue": "https://github.com/sagemath/sage/issues/17795",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17795#issuecomment-242811",
    "user": "https://trac.sagemath.org/admin/accounts/users/Snark"
}
```

<a id='comment:1'></a>
git-trac got stuck on that one, I had to kill it... but I apparently can't kill a ticket myself! Setting to needs_review so someone can close it...



---

archive/issue_events_160900.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/Snark",
    "created_at": "2015-02-17T16:14:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17795",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17795#event-160900"
}
```



---

archive/issue_comments_242812.json:
```json
{
    "body": "<a id='comment:2'></a>\nIn this kind of a case, you can't kill it, but you can definitely set it to positive review as it's so obvious this was an accidental dup.",
    "created_at": "2015-02-17T18:40:50Z",
    "issue": "https://github.com/sagemath/sage/issues/17795",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17795#issuecomment-242812",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'></a>
In this kind of a case, you can't kill it, but you can definitely set it to positive review as it's so obvious this was an accidental dup.



---

archive/issue_events_160901.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-02-17T18:40:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17795",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17795#event-160901"
}
```



---

archive/issue_events_160902.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-02-17T18:40:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/17795",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17795#event-160902"
}
```



---

archive/issue_comments_242813.json:
```json
{
    "body": "**Reviewer:** Julien Puydt, Karl-Dieter Crisman",
    "created_at": "2015-02-17T18:40:50Z",
    "issue": "https://github.com/sagemath/sage/issues/17795",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17795#issuecomment-242813",
    "user": "https://github.com/kcrisman"
}
```

**Reviewer:** Julien Puydt, Karl-Dieter Crisman



---

archive/issue_events_160903.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-02-17T18:40:50Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/17795",
    "milestone": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17795#event-160903"
}
```



---

archive/issue_comments_242814.json:
```json
{
    "body": "<a id='comment:3'></a>\n> In this kind of a case, you can't kill it\n\n(Unless you had some super-admin privileges, I guess.)",
    "created_at": "2015-02-17T18:41:12Z",
    "issue": "https://github.com/sagemath/sage/issues/17795",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/17795#issuecomment-242814",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'></a>
> In this kind of a case, you can't kill it

(Unless you had some super-admin privileges, I guess.)



---

archive/issue_events_160904.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-17T19:30:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/17795",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17795#event-160904"
}
```



---

archive/issue_events_160905.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-17T19:30:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/17795",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/17795#event-160905"
}
```

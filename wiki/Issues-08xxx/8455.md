# Issue 8455: Update MPIR to 1.3.1

archive/issues_008455.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\n\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8455_\n\n",
    "closed_at": "2010-08-17T18:32:53Z",
    "created_at": "2010-03-06T08:35:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update MPIR to 1.3.1",
    "type": "issue",
    "updated_at": "2010-08-17T18:45:54Z",
    "url": "https://github.com/sagemath/sage/issues/8455",
    "user": "https://github.com/mwhansen"
}
```
<div id="comment:0"></div>



Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/8455_





---

archive/issue_events_105095.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-03-06T08:35:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8455",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "0000b0",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8455#event-105095"
}
```



---

archive/issue_events_105096.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-03-06T08:35:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8455",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8455#event-105096"
}
```



---

archive/issue_events_105097.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-03-06T08:35:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8455",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8455#event-105097"
}
```



---

archive/issue_events_105098.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-03-06T08:35:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8455",
    "milestone_number": null,
    "milestone_title": "sage-4.3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8455#event-105098"
}
```



---

archive/issue_comments_067120.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\n\n```\n2) I think those options are relevant to building the optional sage spkg that depends on mpi, namely mpi4py.   If somebody updates openmpi, they better make sure mpi4py also works with the new version.\n```",
    "created_at": "2010-03-14T23:13:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8455#issuecomment-67120",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">comment:2</div>


```
2) I think those options are relevant to building the optional sage spkg that depends on mpi, namely mpi4py.   If somebody updates openmpi, they better make sure mpi4py also works with the new version.
```



---

archive/issue_comments_067121.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI think the builds options are relevant to building the optional sage spkg that depends on mpi, namely mpi4py.   If somebody updates openmpi, they might make sure mpi4py also works with the new version.",
    "created_at": "2010-03-14T23:14:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8455#issuecomment-67121",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:3" align="right">comment:3</div>

I think the builds options are relevant to building the optional sage spkg that depends on mpi, namely mpi4py.   If somebody updates openmpi, they might make sure mpi4py also works with the new version.



---

archive/issue_events_105099.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-08-17T18:32:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8455",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8455#event-105099"
}
```



---

archive/issue_comments_067122.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nInvalid due to #8664.",
    "created_at": "2010-08-17T18:32:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8455#issuecomment-67122",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:4" align="right">comment:4</div>

Invalid due to #8664.



---

archive/issue_events_105100.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-08-17T18:45:54Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8455",
    "milestone_number": null,
    "milestone_title": "sage-4.5.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8455#event-105100"
}
```

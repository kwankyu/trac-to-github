# Issue 8544: Factor  polynomials over QQbar and AA

archive/issues_008544.json:
```json
{
    "assignees": [
        "https://github.com/aghitza"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nQQbar and AA have root-finding but not polynomial factorization.\n\nSee the discussion at #8344, where a special case is dealt with and a possible solution outlined.\n\nCC:  @zimmermann6\n\nComponent: **algebra**\n\nKeywords: **factor polynomial QQbar AA**\n\nAuthor: **John Cremona**\n\nReviewer: **Paul Zimmermann**\n\nMerged: **sage-4.4.4.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8544_\n\n",
    "closed_at": "2010-06-06T01:08:01Z",
    "created_at": "2010-03-15T20:44:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Factor  polynomials over QQbar and AA",
    "type": "issue",
    "updated_at": "2010-06-06T12:34:03Z",
    "url": "https://github.com/sagemath/sage/issues/8544",
    "user": "https://github.com/JohnCremona"
}
```
<div id="comment:0"></div>

QQbar and AA have root-finding but not polynomial factorization.

See the discussion at #8344, where a special case is dealt with and a possible solution outlined.

CC:  @zimmermann6

Component: **algebra**

Keywords: **factor polynomial QQbar AA**

Author: **John Cremona**

Reviewer: **Paul Zimmermann**

Merged: **sage-4.4.4.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/8544_





---

archive/issue_events_106496.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-03-15T20:44:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8544",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8544#event-106496"
}
```



---

archive/issue_events_106497.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-03-15T20:44:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8544",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8544#event-106497"
}
```



---

archive/issue_events_106498.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-03-15T20:44:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8544",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8544#event-106498"
}
```



---

archive/issue_events_106499.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-03-15T20:44:31Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8544",
    "subject": "https://github.com/JohnCremona",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8544#event-106499"
}
```



---

archive/issue_comments_068196.json:
```json
{
    "body": "Attachment: **[trac_8544-polyfactor.patch.gz](https://github.com/sagemath/sage/files/ticket8544/trac_8544-polyfactor.patch.gz)**\n\nApplies to 4.4.2",
    "created_at": "2010-05-26T20:01:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8544#issuecomment-68196",
    "user": "https://github.com/JohnCremona"
}
```

Attachment: **[trac_8544-polyfactor.patch.gz](https://github.com/sagemath/sage/files/ticket8544/trac_8544-polyfactor.patch.gz)**

Applies to 4.4.2



---

archive/issue_events_106500.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-05-26T20:02:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8544",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8544#event-106500"
}
```



---

archive/issue_comments_068197.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThe patch implements this, with tests.",
    "created_at": "2010-05-26T20:02:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8544#issuecomment-68197",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:1" align="right">comment:1</div>

The patch implements this, with tests.



---

archive/issue_events_106501.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-05-28T20:51:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8544",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8544#event-106501"
}
```



---

archive/issue_events_106502.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-05-28T20:51:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8544",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8544#event-106502"
}
```



---

archive/issue_comments_068198.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nJohn, this is nice work. All doctests pass, and I've played with the few examples you've added.\nPaul",
    "created_at": "2010-05-28T20:51:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8544#issuecomment-68198",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:2" align="right">comment:2</div>

John, this is nice work. All doctests pass, and I've played with the few examples you've added.
Paul



---

archive/issue_comments_068199.json:
```json
{
    "body": "Reviewer: **Paul Zimmermann**",
    "created_at": "2010-05-28T20:52:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8544#issuecomment-68199",
    "user": "https://github.com/zimmermann6"
}
```

Reviewer: **Paul Zimmermann**



---

archive/issue_comments_068200.json:
```json
{
    "body": "Author: **John Cremona**",
    "created_at": "2010-05-28T20:52:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8544#issuecomment-68200",
    "user": "https://github.com/zimmermann6"
}
```

Author: **John Cremona**



---

archive/issue_comments_068201.json:
```json
{
    "body": "Changed keywords from **factor polynomial QQbar** to **factor polynomial QQbar AA**",
    "created_at": "2010-05-28T20:52:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8544#issuecomment-68201",
    "user": "https://github.com/zimmermann6"
}
```

Changed keywords from **factor polynomial QQbar** to **factor polynomial QQbar AA**



---

archive/issue_events_106503.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-06-06T01:08:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8544",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8544#event-106503"
}
```



---

archive/issue_events_106504.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-06-06T01:08:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8544",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8544#event-106504"
}
```



---

archive/issue_comments_068202.json:
```json
{
    "body": "Merged: **sage-4.4.4.alpha0**",
    "created_at": "2010-06-06T01:08:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8544",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8544#issuecomment-68202",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.4.4.alpha0**



---

archive/issue_events_106505.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-06-06T12:34:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8544",
    "milestone_number": null,
    "milestone_title": "sage-4.4.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8544#event-106505"
}
```

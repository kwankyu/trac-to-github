# Issue 8057: New boehm-gc-7.1.p3.spkg works with Open Solaris x64 as 64-bit

archive/issues_008057.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-drkirkby"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nI made spkg-install work with SunOS with SAGE64=\"yes\"\n\nThe new spkg can be found here:\n[http://boxen.math.washington.edu/home/jsp/ports/boehm_gc-7.1.p3.spkg](http://boxen.math.washington.edu/home/jsp/ports/boehm_gc-7.1.p3.spkg)\n\nJaap\n\n\n\nComponent: **porting**\n\nAuthor: **Jaap Spies**\n\nReviewer: **David Kirkby**\n\nMerged: **sage-4.3.2.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8057_\n\n",
    "closed_at": "2010-02-01T00:41:22Z",
    "created_at": "2010-01-25T18:19:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20porting",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "New boehm-gc-7.1.p3.spkg works with Open Solaris x64 as 64-bit",
    "type": "issue",
    "updated_at": "2010-02-01T00:41:22Z",
    "url": "https://github.com/sagemath/sage/issues/8057",
    "user": "https://github.com/jaapspies"
}
```
<div id="comment:0"></div>

I made spkg-install work with SunOS with SAGE64="yes"

The new spkg can be found here:
[http://boxen.math.washington.edu/home/jsp/ports/boehm_gc-7.1.p3.spkg](http://boxen.math.washington.edu/home/jsp/ports/boehm_gc-7.1.p3.spkg)

Jaap



Component: **porting**

Author: **Jaap Spies**

Reviewer: **David Kirkby**

Merged: **sage-4.3.2.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/8057_





---

archive/issue_events_099268.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T18:19:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20porting",
    "label_color": "0000b0",
    "label_name": "c: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-99268"
}
```



---

archive/issue_events_099269.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T18:19:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-99269"
}
```



---

archive/issue_events_099270.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T18:19:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-99270"
}
```



---

archive/issue_events_099271.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-01-25T18:19:56Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "subject": "https://github.com/jaapspies",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-99271"
}
```



---

archive/issue_events_099272.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T18:20:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-99272"
}
```



---

archive/issue_comments_061846.json:
```json
{
    "body": "Attachment: **[boehm_gc-7.1.p3.patch.gz](https://github.com/sagemath/sage/files/ticket8057/boehm_gc-7.1.p3.patch.gz)**",
    "created_at": "2010-01-26T17:24:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8057#issuecomment-61846",
    "user": "https://github.com/jaapspies"
}
```

Attachment: **[boehm_gc-7.1.p3.patch.gz](https://github.com/sagemath/sage/files/ticket8057/boehm_gc-7.1.p3.patch.gz)**



---

archive/issue_events_099273.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-01-27T14:01:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-99273"
}
```



---

archive/issue_events_099274.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-01-27T14:01:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-99274"
}
```



---

archive/issue_comments_061847.json:
```json
{
    "body": "Reviewer: **David Kirkby**",
    "created_at": "2010-01-27T14:01:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8057#issuecomment-61847",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

Reviewer: **David Kirkby**



---

archive/issue_comments_061848.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThat's fine. My usual comment it would be better to create a trac ticket for the defect first, then the ticket number can be put in SPKG.txt.",
    "created_at": "2010-01-27T14:01:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8057#issuecomment-61848",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:2" align="right">comment:2</div>

That's fine. My usual comment it would be better to create a trac ticket for the defect first, then the ticket number can be put in SPKG.txt.



---

archive/issue_events_099275.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-01-27T14:01:36Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "title_is": "New boehm-gc-7.1.p3.spkg works with Open Solaris x64 as 64-bit",
    "title_was": "New boehm-gc-7.1.p3.spkg works with Open Solaris 64 bit",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-99275"
}
```



---

archive/issue_events_099276.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-01T00:41:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "milestone_number": null,
    "milestone_title": "sage-4.3.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-99276"
}
```



---

archive/issue_comments_061849.json:
```json
{
    "body": "Merged: **sage-4.3.2.alpha1**",
    "created_at": "2010-02-01T00:41:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8057#issuecomment-61849",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **sage-4.3.2.alpha1**



---

archive/issue_comments_061850.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged [boehm_gc-7.1.p3.spkg](http://boxen.math.washington.edu/home/jsp/ports/boehm_gc-7.1.p3.spkg) in the standard spkg repository.",
    "created_at": "2010-02-01T00:41:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8057#issuecomment-61850",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged [boehm_gc-7.1.p3.spkg](http://boxen.math.washington.edu/home/jsp/ports/boehm_gc-7.1.p3.spkg) in the standard spkg repository.



---

archive/issue_events_099277.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-01T00:41:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-99277"
}
```



---

archive/issue_events_099278.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-01T00:41:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-99278"
}
```

# Issue 8725: New SageNB release 0.8

archive/issues_008725.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout",
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis package includes the ff. patches:\n\n#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #8369, #8637, #8724\n\nFind it at http://sage.math.washington.edu/home/timdumol/sagenb-0.8.spkg.\n\nComponent: **notebook**\n\nAuthor: **Tim Dumol**\n\nReviewer: **William Stein**\n\nMerged: **sage-4.4.1.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8725_\n\n",
    "closed_at": "2010-04-29T05:40:48Z",
    "created_at": "2010-04-20T14:33:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "New SageNB release 0.8",
    "type": "issue",
    "updated_at": "2010-05-03T01:09:35Z",
    "url": "https://github.com/sagemath/sage/issues/8725",
    "user": "https://github.com/TimDumol"
}
```
<div id="comment:0"></div>

This package includes the ff. patches:

#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #8369, #8637, #8724

Find it at http://sage.math.washington.edu/home/timdumol/sagenb-0.8.spkg.

Component: **notebook**

Author: **Tim Dumol**

Reviewer: **William Stein**

Merged: **sage-4.4.1.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/8725_





---

archive/issue_events_109209.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-04-20T14:33:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "milestone_number": null,
    "milestone_title": "sage-4.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8725#event-109209"
}
```



---

archive/issue_events_109210.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-04-20T14:33:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8725#event-109210"
}
```



---

archive/issue_events_109211.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-04-20T14:33:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8725#event-109211"
}
```



---

archive/issue_events_109212.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-04-20T14:33:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8725#event-109212"
}
```



---

archive/issue_events_109213.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-04-20T14:33:02Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "subject": "https://github.com/TimDumol",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8725#event-109213"
}
```



---

archive/issue_events_109214.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-04-20T14:33:02Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "subject": "https://github.com/TimDumol",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8725#event-109214"
}
```



---

archive/issue_comments_070651.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,7 @@\n This package includes the ff. patches:\n \n-#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #693, #7997, #7633, #8369, #8637, #7665\n+#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #8369, #8637\n+\n+#8724 still needs review, but this should not be released until it has been.\n \n Find it at http://sage.math.washington.edu/home/timdumol/sagenb-0.8.spkg.\n``````\n",
    "created_at": "2010-04-20T14:49:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8725#issuecomment-70651",
    "user": "https://github.com/TimDumol"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,7 @@
 This package includes the ff. patches:
 
-#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #693, #7997, #7633, #8369, #8637, #7665
+#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #8369, #8637
+
+#8724 still needs review, but this should not be released until it has been.
 
 Find it at http://sage.math.washington.edu/home/timdumol/sagenb-0.8.spkg.
``````




---

archive/issue_events_109215.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-04-20T16:31:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8725#event-109215"
}
```



---

archive/issue_comments_070652.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,6 @@\n This package includes the ff. patches:\n \n-#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #8369, #8637\n+#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #8369, #8637, #8222\n \n #8724 still needs review, but this should not be released until it has been.\n \n``````\n",
    "created_at": "2010-04-21T11:06:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8725#issuecomment-70652",
    "user": "https://github.com/TimDumol"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,6 @@
 This package includes the ff. patches:
 
-#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #8369, #8637
+#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #8369, #8637, #8222
 
 #8724 still needs review, but this should not be released until it has been.
 
``````




---

archive/issue_comments_070653.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,4 +4,6 @@\n \n #8724 still needs review, but this should not be released until it has been.\n \n+There is a Sage main library patch at #8222 that must be included when this is.\n+\n Find it at http://sage.math.washington.edu/home/timdumol/sagenb-0.8.spkg.\n``````\n",
    "created_at": "2010-04-21T11:07:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8725#issuecomment-70653",
    "user": "https://github.com/TimDumol"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,4 +4,6 @@
 
 #8724 still needs review, but this should not be released until it has been.
 
+There is a Sage main library patch at #8222 that must be included when this is.
+
 Find it at http://sage.math.washington.edu/home/timdumol/sagenb-0.8.spkg.
``````




---

archive/issue_comments_070654.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,9 +1,7 @@\n This package includes the ff. patches:\n \n-#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #8369, #8637, #8222\n+#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #8369, #8637\n \n #8724 still needs review, but this should not be released until it has been.\n \n-There is a Sage main library patch at #8222 that must be included when this is.\n-\n Find it at http://sage.math.washington.edu/home/timdumol/sagenb-0.8.spkg.\n``````\n",
    "created_at": "2010-04-24T19:27:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8725#issuecomment-70654",
    "user": "https://github.com/TimDumol"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,9 +1,7 @@
 This package includes the ff. patches:
 
-#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #8369, #8637, #8222
+#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #8369, #8637
 
 #8724 still needs review, but this should not be released until it has been.
 
-There is a Sage main library patch at #8222 that must be included when this is.
-
 Find it at http://sage.math.washington.edu/home/timdumol/sagenb-0.8.spkg.
``````




---

archive/issue_comments_070655.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nI've reviewed this and everything is good (assuming #8222 is *not* in sagenb-0.8).    There is this issue #8755, but that can wait.",
    "created_at": "2010-04-24T22:20:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8725#issuecomment-70655",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:6" align="right">comment:6</div>

I've reviewed this and everything is good (assuming #8222 is *not* in sagenb-0.8).    There is this issue #8755, but that can wait.



---

archive/issue_comments_070656.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,7 +1,5 @@\n This package includes the ff. patches:\n \n-#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #8369, #8637\n-\n-#8724 still needs review, but this should not be released until it has been.\n+#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #8369, #8637, #8724\n \n Find it at http://sage.math.washington.edu/home/timdumol/sagenb-0.8.spkg.\n``````\n",
    "created_at": "2010-04-24T22:20:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8725#issuecomment-70656",
    "user": "https://github.com/williamstein"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,7 +1,5 @@
 This package includes the ff. patches:
 
-#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #8369, #8637
-
-#8724 still needs review, but this should not be released until it has been.
+#8724 (blocker), #5712, #7501, #6069, #3154, #8249, #8436, #8092, #8038, #8369, #8637, #8724
 
 Find it at http://sage.math.washington.edu/home/timdumol/sagenb-0.8.spkg.
``````




---

archive/issue_events_109216.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-04-24T22:20:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8725#event-109216"
}
```



---

archive/issue_events_109217.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-04-24T22:20:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8725#event-109217"
}
```



---

archive/issue_comments_070657.json:
```json
{
    "body": "Merged: **4.4.1.alpha2**",
    "created_at": "2010-04-29T05:40:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8725#issuecomment-70657",
    "user": "https://github.com/williamstein"
}
```

Merged: **4.4.1.alpha2**



---

archive/issue_events_109218.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-04-29T05:40:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8725#event-109218"
}
```



---

archive/issue_events_109219.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-04-29T05:40:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8725#event-109219"
}
```



---

archive/issue_comments_070658.json:
```json
{
    "body": "Reviewer: **William Stein**",
    "created_at": "2010-04-29T15:40:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8725#issuecomment-70658",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **William Stein**



---

archive/issue_comments_070659.json:
```json
{
    "body": "Author: **Tim Dumol**",
    "created_at": "2010-04-29T15:40:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8725#issuecomment-70659",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Tim Dumol**



---

archive/issue_comments_070660.json:
```json
{
    "body": "Changed merged from **4.4.1.alpha2** to **sage-4.4.1.alpha2**",
    "created_at": "2010-05-03T01:09:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8725",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8725#issuecomment-70660",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed merged from **4.4.1.alpha2** to **sage-4.4.1.alpha2**

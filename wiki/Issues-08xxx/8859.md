# Issue 8859: bidirectional_dijkstra does not handle vertex labels properly

archive/issues_008859.json:
```json
{
    "body": "Reported on sage-support:\n\nhttp://groups.google.com/group/sage-support/msg/a64eed13004aaa42\n\n**Assignee:** @jasongrout, @nathanncohen, @rlmill\n\n**Author:** Robert Miller\n\n**Reviewer:** Nathann Cohen\n\n**Merged:** sage-4.4.2.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/8859\n\n",
    "closed_at": "2010-05-08T22:00:30Z",
    "created_at": "2010-05-03T17:53:12Z",
    "labels": [
        "component: graph theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4.2",
    "title": "bidirectional_dijkstra does not handle vertex labels properly",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/8859",
    "user": "https://github.com/rlmill"
}
```
Reported on sage-support:

http://groups.google.com/group/sage-support/msg/a64eed13004aaa42

**Assignee:** @jasongrout, @nathanncohen, @rlmill

**Author:** Robert Miller

**Reviewer:** Nathann Cohen

**Merged:** sage-4.4.2.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/8859





---

archive/attachments_011493.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8859.patch",
    "asset_url": "tarball://root/attachments/ticket8859/trac_8859.patch",
    "created_at": "2010-05-03T17:54:22Z",
    "issue": "https://github.com/sagemath/sage/issues/8859",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8859/trac_8859.patch",
    "user": "https://github.com/rlmill"
}
```



---

archive/issue_comments_074243.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_8859.patch](https://github.com/sagemath/sage/files/ticket8859/trac_8859.patch)",
    "created_at": "2010-05-03T17:54:22Z",
    "issue": "https://github.com/sagemath/sage/issues/8859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8859#issuecomment-74243",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_8859.patch](https://github.com/sagemath/sage/files/ticket8859/trac_8859.patch)



---

archive/issue_events_063994.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-05-03T17:54:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8859",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8859#event-63994"
}
```



---

archive/issue_comments_074244.json:
```json
{
    "body": "<a id='comment:2'></a>\nApplies nicely, does its job.... And besides, it was my fault !! ;-)\n\nThank you !!!!!!!\n\nNathann",
    "created_at": "2010-05-04T17:19:58Z",
    "issue": "https://github.com/sagemath/sage/issues/8859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8859#issuecomment-74244",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:2'></a>
Applies nicely, does its job.... And besides, it was my fault !! ;-)

Thank you !!!!!!!

Nathann



---

archive/issue_events_063995.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-05-04T17:19:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8859",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8859#event-63995"
}
```



---

archive/issue_events_063996.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-05-04T17:19:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8859",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8859#event-63996"
}
```



---

archive/issue_comments_074245.json:
```json
{
    "body": "**Reviewer:** Nathann Cohen",
    "created_at": "2010-05-08T22:00:30Z",
    "issue": "https://github.com/sagemath/sage/issues/8859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8859#issuecomment-74245",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Reviewer:** Nathann Cohen



---

archive/issue_comments_074246.json:
```json
{
    "body": "**Merged:** sage-4.4.2.alpha0",
    "created_at": "2010-05-08T22:00:30Z",
    "issue": "https://github.com/sagemath/sage/issues/8859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8859#issuecomment-74246",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Merged:** sage-4.4.2.alpha0



---

archive/issue_events_063997.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-05-08T22:00:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8859",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8859#event-63997"
}
```



---

archive/issue_events_063998.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-05-08T22:00:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8859",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8859#event-63998"
}
```

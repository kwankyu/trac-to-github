# Issue 8206: Add information on hg qnew to Developer Walk-through

archive/issues_008206.json:
```json
{
    "body": "The command  hg qnew -f would be good to include in the discussion on Mercurial queues (and replace some uncertainty).\n\nAs well as a mention of the record extensions.\n\n**Assignee:** mvngu\n\n**CC:**  mvngu @roed314\n\n**Author:** Rob Beezer\n\n**Reviewer:** Marshall Hampton\n\n**Merged:** sage-4.3.3.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/8206\n\n",
    "closed_at": "2010-02-11T14:43:14Z",
    "created_at": "2010-02-07T06:25:18Z",
    "labels": [
        "component: documentation",
        "minor",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.3",
    "title": "Add information on hg qnew to Developer Walk-through",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/8206",
    "user": "https://github.com/rbeezer"
}
```
The command  hg qnew -f would be good to include in the discussion on Mercurial queues (and replace some uncertainty).

As well as a mention of the record extensions.

**Assignee:** mvngu

**CC:**  mvngu @roed314

**Author:** Rob Beezer

**Reviewer:** Marshall Hampton

**Merged:** sage-4.3.3.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/8206





---

archive/attachments_010508.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8206_developer-doc.patch",
    "asset_url": "tarball://root/attachments/ticket8206/trac_8206_developer-doc.patch",
    "created_at": "2010-02-07T06:35:47Z",
    "issue": "https://github.com/sagemath/sage/issues/8206",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8206/trac_8206_developer-doc.patch",
    "user": "https://github.com/rbeezer"
}
```



---

archive/issue_comments_065290.json:
```json
{
    "body": "**Attachment:** [trac_8206_developer-doc.patch](https://github.com/sagemath/sage/files/ticket8206/trac_8206_developer-doc.patch)",
    "created_at": "2010-02-07T06:35:47Z",
    "issue": "https://github.com/sagemath/sage/issues/8206",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8206#issuecomment-65290",
    "user": "https://github.com/rbeezer"
}
```

**Attachment:** [trac_8206_developer-doc.patch](https://github.com/sagemath/sage/files/ticket8206/trac_8206_developer-doc.patch)



---

archive/issue_events_057600.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-02-07T06:37:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8206",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8206#event-57600"
}
```



---

archive/issue_events_057601.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mhampton",
    "created_at": "2010-02-07T23:29:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8206",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8206#event-57601"
}
```



---

archive/issue_events_057602.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mhampton",
    "created_at": "2010-02-07T23:29:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8206",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8206#event-57602"
}
```



---

archive/issue_comments_065291.json:
```json
{
    "body": "<a id='comment:2'></a>\nI don't know mercurial queues well (although I know them a little better after looking at this!) but this seems like a simple patch.  Docs build OK, changes make sense, so positive review.",
    "created_at": "2010-02-07T23:29:39Z",
    "issue": "https://github.com/sagemath/sage/issues/8206",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8206#issuecomment-65291",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

<a id='comment:2'></a>
I don't know mercurial queues well (although I know them a little better after looking at this!) but this seems like a simple patch.  Docs build OK, changes make sense, so positive review.



---

archive/issue_comments_065292.json:
```json
{
    "body": "<a id='comment:3'></a>\nSee also #1610 for more stuff on \"hg record\" and cherry picking.",
    "created_at": "2010-02-09T03:16:09Z",
    "issue": "https://github.com/sagemath/sage/issues/8206",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8206#issuecomment-65292",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:3'></a>
See also #1610 for more stuff on "hg record" and cherry picking.



---

archive/issue_events_057603.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-02-11T14:43:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8206",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8206#event-57603"
}
```



---

archive/issue_events_057604.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-02-11T14:43:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8206",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8206#event-57604"
}
```



---

archive/issue_comments_065293.json:
```json
{
    "body": "**Reviewer:** Marshall Hampton",
    "created_at": "2010-02-11T14:43:14Z",
    "issue": "https://github.com/sagemath/sage/issues/8206",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8206#issuecomment-65293",
    "user": "https://github.com/qed777"
}
```

**Reviewer:** Marshall Hampton



---

archive/issue_comments_065294.json:
```json
{
    "body": "**Merged:** sage-4.3.3.alpha0",
    "created_at": "2010-02-11T14:43:14Z",
    "issue": "https://github.com/sagemath/sage/issues/8206",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8206#issuecomment-65294",
    "user": "https://github.com/qed777"
}
```

**Merged:** sage-4.3.3.alpha0



---

archive/issue_comments_065295.json:
```json
{
    "body": "**Author:** Rob Beezer",
    "created_at": "2010-02-11T14:43:14Z",
    "issue": "https://github.com/sagemath/sage/issues/8206",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8206#issuecomment-65295",
    "user": "https://github.com/qed777"
}
```

**Author:** Rob Beezer

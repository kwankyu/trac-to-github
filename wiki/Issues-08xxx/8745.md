# Issue 8745: Steiner Triple Systems

archive/issues_008745.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-sage-combinat"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis patch adds a function to block_design returning a Steiner Triple System on any number of vertices n congruent to 1 or 3 modulo 6. There is none otherwise :-)\n\nThe function is pretty short, a satisfiable decomposition being explicitely given in the reference : \nhttp://www.utu.fi/~honkala/designs.ps\n\nNathann\n\nCC:  @wdjoyner\n\nComponent: **combinatorics**\n\nAuthor: **Nathann Cohen**\n\nReviewer: **David Joyner**\n\nMerged: **sage-4.4.1.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8745_\n\n",
    "closed_at": "2010-04-29T05:08:16Z",
    "created_at": "2010-04-22T12:31:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Steiner Triple Systems",
    "type": "issue",
    "updated_at": "2010-05-03T01:03:32Z",
    "url": "https://github.com/sagemath/sage/issues/8745",
    "user": "https://github.com/nathanncohen"
}
```
<div id="comment:0"></div>

This patch adds a function to block_design returning a Steiner Triple System on any number of vertices n congruent to 1 or 3 modulo 6. There is none otherwise :-)

The function is pretty short, a satisfiable decomposition being explicitely given in the reference : 
http://www.utu.fi/~honkala/designs.ps

Nathann

CC:  @wdjoyner

Component: **combinatorics**

Author: **Nathann Cohen**

Reviewer: **David Joyner**

Merged: **sage-4.4.1.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/8745_





---

archive/issue_events_109541.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-22T12:31:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8745",
    "milestone_number": null,
    "milestone_title": "sage-4.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8745#event-109541"
}
```



---

archive/issue_events_109542.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-22T12:31:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8745",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8745#event-109542"
}
```



---

archive/issue_events_109543.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-22T12:31:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8745",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8745#event-109543"
}
```



---

archive/issue_events_109544.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-22T12:31:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8745",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8745#event-109544"
}
```



---

archive/issue_events_109545.json:
```json
{
    "actor": "https://github.com/sagetrac-sage-combinat",
    "created_at": "2010-04-22T12:31:11Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8745",
    "subject": "https://github.com/nathanncohen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8745#event-109545"
}
```



---

archive/issue_events_109546.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-22T12:34:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8745",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8745#event-109546"
}
```



---

archive/issue_comments_071006.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_8745.patch.gz](https://github.com/sagemath/sage/files/ticket8745/trac_8745.patch.gz)**",
    "created_at": "2010-04-22T12:34:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8745",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8745#issuecomment-71006",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_8745.patch.gz](https://github.com/sagemath/sage/files/ticket8745/trac_8745.patch.gz)**



---

archive/issue_comments_071007.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nApplies fine to 4.4.a1, passes sage -testall on a 10.6.2 mac (except for the r.py failure, which is unrelated), and looks good to me in terms of examples and references.\n\nThanks Nathann!",
    "created_at": "2010-04-22T22:53:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8745",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8745#issuecomment-71007",
    "user": "https://github.com/wdjoyner"
}
```

<div id="comment:2" align="right">comment:2</div>

Applies fine to 4.4.a1, passes sage -testall on a 10.6.2 mac (except for the r.py failure, which is unrelated), and looks good to me in terms of examples and references.

Thanks Nathann!



---

archive/issue_events_109547.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2010-04-22T22:53:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8745",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8745#event-109547"
}
```



---

archive/issue_events_109548.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2010-04-22T22:53:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8745",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8745#event-109548"
}
```



---

archive/issue_comments_071008.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nWhaouuuuuuu !!! Such a quick review :-)\n\nThank you very much too !! :-)\n\nNathann",
    "created_at": "2010-04-23T06:42:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8745",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8745#issuecomment-71008",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:3" align="right">comment:3</div>

Whaouuuuuuu !!! Such a quick review :-)

Thank you very much too !! :-)

Nathann



---

archive/issue_events_109549.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-04-29T05:08:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8745",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8745#event-109549"
}
```



---

archive/issue_events_109550.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-04-29T05:08:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8745",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8745#event-109550"
}
```



---

archive/issue_comments_071009.json:
```json
{
    "body": "Merged: **4.4.1.alpha2**",
    "created_at": "2010-04-29T05:08:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8745",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8745#issuecomment-71009",
    "user": "https://github.com/williamstein"
}
```

Merged: **4.4.1.alpha2**



---

archive/issue_comments_071010.json:
```json
{
    "body": "Author: **Nathann Cohen**",
    "created_at": "2010-04-30T00:37:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8745",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8745#issuecomment-71010",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Nathann Cohen**



---

archive/issue_comments_071011.json:
```json
{
    "body": "Reviewer: **David Joyner**",
    "created_at": "2010-04-30T00:37:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8745",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8745#issuecomment-71011",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **David Joyner**



---

archive/issue_comments_071012.json:
```json
{
    "body": "Changed merged from **4.4.1.alpha2** to **sage-4.4.1.alpha2**",
    "created_at": "2010-05-03T01:03:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8745",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8745#issuecomment-71012",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed merged from **4.4.1.alpha2** to **sage-4.4.1.alpha2**

# Issue 8134: Escape $s in notebook keybindings docstring, config.py

archive/issues_008134.json:
```json
{
    "body": "Sphinx complains\n\n```\n[...]/sagenb/notebook/config.py:docstring of sagenb.notebook.config:26: (WARNING/2) Inline interpreted text or phrase reference start-string without end-string.\n```\nwhen building the reference manual.\n\n\n**Assignee:** mvngu\n\n**Author:** Mitesh Patel\n\n**Reviewer:** John Palmieri\n\n**Merged:** sagenb-0.7.5\n\nIssue created by migration from https://trac.sagemath.org/ticket/8134\n\n",
    "closed_at": "2010-02-10T18:31:56Z",
    "created_at": "2010-01-31T02:28:33Z",
    "labels": [
        "component: notebook",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.3",
    "title": "Escape $s in notebook keybindings docstring, config.py",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/8134",
    "user": "https://github.com/qed777"
}
```
Sphinx complains

```
[...]/sagenb/notebook/config.py:docstring of sagenb.notebook.config:26: (WARNING/2) Inline interpreted text or phrase reference start-string without end-string.
```
when building the reference manual.


**Assignee:** mvngu

**Author:** Mitesh Patel

**Reviewer:** John Palmieri

**Merged:** sagenb-0.7.5

Issue created by migration from https://trac.sagemath.org/ticket/8134





---

archive/issue_comments_064494.json:
```json
{
    "body": "Escape `$`s in `config.py`.  *sagenb* repo.",
    "created_at": "2010-01-31T02:36:13Z",
    "issue": "https://github.com/sagemath/sage/issues/8134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8134#issuecomment-64494",
    "user": "https://github.com/qed777"
}
```

Escape `$`s in `config.py`.  *sagenb* repo.



---

archive/issue_events_056991.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-01-31T02:36:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8134",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8134#event-56991"
}
```



---

archive/attachments_010405.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8134-escape_dollars.patch",
    "asset_url": "tarball://root/attachments/ticket8134/trac_8134-escape_dollars.patch",
    "created_at": "2010-01-31T02:36:47Z",
    "issue": "https://github.com/sagemath/sage/issues/8134",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8134/trac_8134-escape_dollars.patch",
    "user": "https://github.com/qed777"
}
```



---

archive/issue_comments_064495.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_8134-escape_dollars.patch](https://github.com/sagemath/sage/files/ticket8134/trac_8134-escape_dollars.patch)\n\nI'm not sure why `sagenb.notebook.config?` doesn't render nicely in the notebook.",
    "created_at": "2010-01-31T02:36:47Z",
    "issue": "https://github.com/sagemath/sage/issues/8134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8134#issuecomment-64495",
    "user": "https://github.com/qed777"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_8134-escape_dollars.patch](https://github.com/sagemath/sage/files/ticket8134/trac_8134-escape_dollars.patch)

I'm not sure why `sagenb.notebook.config?` doesn't render nicely in the notebook.



---

archive/issue_comments_064496.json:
```json
{
    "body": "<a id='comment:2'></a>\nThe patch should apply cleanly to #8051's SageNB 0.7.2.",
    "created_at": "2010-01-31T02:44:27Z",
    "issue": "https://github.com/sagemath/sage/issues/8134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8134#issuecomment-64496",
    "user": "https://github.com/qed777"
}
```

<a id='comment:2'></a>
The patch should apply cleanly to #8051's SageNB 0.7.2.



---

archive/issue_events_056992.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-02-05T07:10:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8134",
    "label": "component: documentation",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8134#event-56992"
}
```



---

archive/issue_events_056993.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-02-05T07:10:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8134",
    "label": "component: notebook",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8134#event-56993"
}
```



---

archive/issue_comments_064497.json:
```json
{
    "body": "**Reviewer:** John Palmieri",
    "created_at": "2010-02-05T20:38:54Z",
    "issue": "https://github.com/sagemath/sage/issues/8134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8134#issuecomment-64497",
    "user": "https://github.com/jhpalmieri"
}
```

**Reviewer:** John Palmieri



---

archive/issue_comments_064498.json:
```json
{
    "body": "<a id='comment:4'></a>\nIt fixes the error messages and the output looks good.  Should there be an 'r' before the triple quotes at the top of the file?  I'll give it a positive review either way, so if you want to add that, go ahead.",
    "created_at": "2010-02-05T20:38:54Z",
    "issue": "https://github.com/sagemath/sage/issues/8134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8134#issuecomment-64498",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:4'></a>
It fixes the error messages and the output looks good.  Should there be an 'r' before the triple quotes at the top of the file?  I'll give it a positive review either way, so if you want to add that, go ahead.



---

archive/issue_events_056994.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-02-05T20:38:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8134",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8134#event-56994"
}
```



---

archive/issue_events_056995.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-02-05T20:38:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8134",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8134#event-56995"
}
```



---

archive/issue_comments_064499.json:
```json
{
    "body": "Make top docstring raw.  Replaces previous.",
    "created_at": "2010-02-06T16:24:54Z",
    "issue": "https://github.com/sagemath/sage/issues/8134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8134#issuecomment-64499",
    "user": "https://github.com/qed777"
}
```

Make top docstring raw.  Replaces previous.



---

archive/attachments_010406.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8134-escape_dollars.2.patch",
    "asset_url": "tarball://root/attachments/ticket8134/trac_8134-escape_dollars.2.patch",
    "created_at": "2010-02-06T16:26:52Z",
    "issue": "https://github.com/sagemath/sage/issues/8134",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8134/trac_8134-escape_dollars.2.patch",
    "user": "https://github.com/qed777"
}
```



---

archive/issue_comments_064500.json:
```json
{
    "body": "<a id='comment:5'></a>\n**Attachment:** [trac_8134-escape_dollars.2.patch](https://github.com/sagemath/sage/files/ticket8134/trac_8134-escape_dollars.2.patch)\n\nReplying to [@jhpalmieri](#comment%3A4):\n> It fixes the error messages and the output looks good.  Should there be an 'r' before the triple quotes at the top of the file?  I'll give it a positive review either way, so if you want to add that, go ahead.\n\nDone!",
    "created_at": "2010-02-06T16:26:52Z",
    "issue": "https://github.com/sagemath/sage/issues/8134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8134#issuecomment-64500",
    "user": "https://github.com/qed777"
}
```

<a id='comment:5'></a>
**Attachment:** [trac_8134-escape_dollars.2.patch](https://github.com/sagemath/sage/files/ticket8134/trac_8134-escape_dollars.2.patch)

Replying to [@jhpalmieri](#comment%3A4):
> It fixes the error messages and the output looks good.  Should there be an 'r' before the triple quotes at the top of the file?  I'll give it a positive review either way, so if you want to add that, go ahead.

Done!



---

archive/issue_events_056996.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-02-10T18:31:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8134",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8134#event-56996"
}
```



---

archive/issue_events_056997.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-02-10T18:31:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8134",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8134#event-56997"
}
```



---

archive/issue_comments_064501.json:
```json
{
    "body": "**Merged:** sagenb-0.7.5",
    "created_at": "2010-02-10T18:31:56Z",
    "issue": "https://github.com/sagemath/sage/issues/8134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8134#issuecomment-64501",
    "user": "https://github.com/qed777"
}
```

**Merged:** sagenb-0.7.5

# Issue 8188: additional functions for linear codes

archive/issues_008188.json:
```json
{
    "assignees": [
        "https://github.com/kwankyu"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nA few enhancements of linear codes functionality.\n\nComponent: **coding theory**\n\nAuthor: **Kwankyu Lee**\n\nReviewer: **David Joyner**\n\nMerged: **sage-4.3.3.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8188_\n\n",
    "closed_at": "2010-02-11T14:53:27Z",
    "created_at": "2010-02-05T04:01:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20coding%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "additional functions for linear codes",
    "type": "issue",
    "updated_at": "2010-02-11T14:53:27Z",
    "url": "https://github.com/sagemath/sage/issues/8188",
    "user": "https://github.com/kwankyu"
}
```
<div id="comment:0"></div>

A few enhancements of linear codes functionality.

Component: **coding theory**

Author: **Kwankyu Lee**

Reviewer: **David Joyner**

Merged: **sage-4.3.3.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/8188_





---

archive/issue_events_101073.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2010-02-05T04:01:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "milestone_number": null,
    "milestone_title": "sage-4.3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8188#event-101073"
}
```



---

archive/issue_events_101074.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2010-02-05T04:01:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20coding%20theory",
    "label_color": "0000ff",
    "label_name": "c: coding theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8188#event-101074"
}
```



---

archive/issue_events_101075.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2010-02-05T04:01:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8188#event-101075"
}
```



---

archive/issue_events_101076.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2010-02-05T04:01:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8188#event-101076"
}
```



---

archive/issue_comments_063392.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nIs this ready for review?",
    "created_at": "2010-02-05T04:20:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8188#issuecomment-63392",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:1" align="right">comment:1</div>

Is this ready for review?



---

archive/issue_events_101077.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2010-02-05T04:22:18Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "subject": "https://github.com/kwankyu",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8188#event-101077"
}
```



---

archive/issue_events_101078.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2010-02-05T04:22:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8188#event-101078"
}
```



---

archive/issue_comments_063393.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThis looks good. Thank you. I'll test it and give a formal review when I get home.\n\nCan you please add a small patch which adds your name to the AUTHOR section and a brief description of what you added?",
    "created_at": "2010-02-05T12:46:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8188#issuecomment-63393",
    "user": "https://github.com/wdjoyner"
}
```

<div id="comment:4" align="right">comment:4</div>

This looks good. Thank you. I'll test it and give a formal review when I get home.

Can you please add a small patch which adds your name to the AUTHOR section and a brief description of what you added?



---

archive/issue_events_101079.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2010-02-05T18:39:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8188#event-101079"
}
```



---

archive/issue_events_101080.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2010-02-05T18:39:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8188#event-101080"
}
```



---

archive/issue_comments_063394.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nThis patch fails to apply to 4.3.2.rc0.\n\n```\n----------------------------------------------------------------------\n| Sage Version 4.3.2.rc0, Release Date: 2010-02-03                   |\n| Type notebook() for the GUI, and license() for information.        |\n----------------------------------------------------------------------\n**********************************************************************\n*                                                                    *\n* Warning: this is a prerelease version, and it may be unstable.     *\n*                                                                    *\n**********************************************************************\nLoading Sage library. Current Mercurial branch is: 8188-coding\nsage: hg_sage.apply(\"/Users/wdj/sagefiles/trac_8188.patch\")\ncd \"/Users/wdj/sagefiles/sage-4.3.2.rc0/devel/sage\" && hg status\ncd \"/Users/wdj/sagefiles/sage-4.3.2.rc0/devel/sage\" && hg status\ncd \"/Users/wdj/sagefiles/sage-4.3.2.rc0/devel/sage\" && hg import   \"/Users/wdj/sagefiles/trac_8188.patch\"\napplying /Users/wdj/sagefiles/trac_8188.patch\npatching file sage/coding/linear_code.py\nHunk #1 FAILED at 1406\nHunk #2 succeeded at 1416 with fuzz 2 (offset -11 lines).\nHunk #3 succeeded at 1468 with fuzz 1 (offset -2 lines).\nHunk #4 succeeded at 1684 with fuzz 1 (offset 4 lines).\n1 out of 4 hunks FAILED -- saving rejects to file sage/coding/linear_code.py.rej\nabort: patch failed to apply\n```\n\nPlease make the following changes\n\n(1) fix the patch so that it applies correctly\n(2) Add you name to AUTHOR\n(3) Add a short definition of systematic and information set to the docstrings.\n\nIf you do all these, and the patch applies and passes sage -testall then I'll give this a positive review.\n\nThanks for working on this!",
    "created_at": "2010-02-05T18:39:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8188#issuecomment-63394",
    "user": "https://github.com/wdjoyner"
}
```

<div id="comment:5" align="right">comment:5</div>

This patch fails to apply to 4.3.2.rc0.

```
----------------------------------------------------------------------
| Sage Version 4.3.2.rc0, Release Date: 2010-02-03                   |
| Type notebook() for the GUI, and license() for information.        |
----------------------------------------------------------------------
**********************************************************************
*                                                                    *
* Warning: this is a prerelease version, and it may be unstable.     *
*                                                                    *
**********************************************************************
Loading Sage library. Current Mercurial branch is: 8188-coding
sage: hg_sage.apply("/Users/wdj/sagefiles/trac_8188.patch")
cd "/Users/wdj/sagefiles/sage-4.3.2.rc0/devel/sage" && hg status
cd "/Users/wdj/sagefiles/sage-4.3.2.rc0/devel/sage" && hg status
cd "/Users/wdj/sagefiles/sage-4.3.2.rc0/devel/sage" && hg import   "/Users/wdj/sagefiles/trac_8188.patch"
applying /Users/wdj/sagefiles/trac_8188.patch
patching file sage/coding/linear_code.py
Hunk #1 FAILED at 1406
Hunk #2 succeeded at 1416 with fuzz 2 (offset -11 lines).
Hunk #3 succeeded at 1468 with fuzz 1 (offset -2 lines).
Hunk #4 succeeded at 1684 with fuzz 1 (offset 4 lines).
1 out of 4 hunks FAILED -- saving rejects to file sage/coding/linear_code.py.rej
abort: patch failed to apply
```

Please make the following changes

(1) fix the patch so that it applies correctly
(2) Add you name to AUTHOR
(3) Add a short definition of systematic and information set to the docstrings.

If you do all these, and the patch applies and passes sage -testall then I'll give this a positive review.

Thanks for working on this!



---

archive/issue_comments_063395.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nThe patch applied to 4.3.2 passed doctest.",
    "created_at": "2010-02-08T06:23:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8188#issuecomment-63395",
    "user": "https://github.com/kwankyu"
}
```

<div id="comment:6" align="right">comment:6</div>

The patch applied to 4.3.2 passed doctest.



---

archive/issue_events_101081.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2010-02-08T06:23:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8188#event-101081"
}
```



---

archive/issue_events_101082.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2010-02-08T06:23:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8188#event-101082"
}
```



---

archive/issue_comments_063396.json:
```json
{
    "body": "rebased on Sage 4.3.2; added more docstrings as requested",
    "created_at": "2010-02-08T07:13:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8188#issuecomment-63396",
    "user": "https://github.com/kwankyu"
}
```

rebased on Sage 4.3.2; added more docstrings as requested



---

archive/issue_events_101083.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2010-02-08T13:50:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8188#event-101083"
}
```



---

archive/issue_events_101084.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2010-02-08T13:50:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8188#event-101084"
}
```



---

archive/issue_comments_063397.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nAttachment: **[trac_8188.patch.gz](https://github.com/sagemath/sage/files/ticket8188/trac_8188.patch.gz)**\n\nLooks good, applies fine to 4.3.2 and passes sage -testal.\n\nThank you for adding this!\n\nPositive review.",
    "created_at": "2010-02-08T13:50:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8188#issuecomment-63397",
    "user": "https://github.com/wdjoyner"
}
```

<div id="comment:7" align="right">comment:7</div>

Attachment: **[trac_8188.patch.gz](https://github.com/sagemath/sage/files/ticket8188/trac_8188.patch.gz)**

Looks good, applies fine to 4.3.2 and passes sage -testal.

Thank you for adding this!

Positive review.



---

archive/issue_comments_063398.json:
```json
{
    "body": "Merged: **sage-4.3.3.alpha0**",
    "created_at": "2010-02-11T14:53:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8188#issuecomment-63398",
    "user": "https://github.com/qed777"
}
```

Merged: **sage-4.3.3.alpha0**



---

archive/issue_events_101085.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-02-11T14:53:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8188#event-101085"
}
```



---

archive/issue_events_101086.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-02-11T14:53:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8188#event-101086"
}
```



---

archive/issue_comments_063399.json:
```json
{
    "body": "Reviewer: **David Joyner**",
    "created_at": "2010-02-11T14:53:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8188#issuecomment-63399",
    "user": "https://github.com/qed777"
}
```

Reviewer: **David Joyner**

# Issue 8716: Modular forms of level GammaH

archive/issues_008716.json:
```json
{
    "assignees": [
        "https://github.com/craigcitro"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nWe have code for modular symbols on GammaH groups but the code for modular forms is little more than stubs. Here's a patch that should fix that.\n\nComponent: **modular forms**\n\nAuthor: **David Loeffler**\n\nReviewer: **Robert Miller**\n\nMerged: **sage-4.6.2.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8716_\n\n",
    "closed_at": "2011-01-25T08:14:18Z",
    "created_at": "2010-04-19T16:53:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Modular forms of level GammaH",
    "type": "issue",
    "updated_at": "2011-01-25T08:14:18Z",
    "url": "https://github.com/sagemath/sage/issues/8716",
    "user": "https://github.com/loefflerd"
}
```
<div id="comment:0"></div>

We have code for modular symbols on GammaH groups but the code for modular forms is little more than stubs. Here's a patch that should fix that.

Component: **modular forms**

Author: **David Loeffler**

Reviewer: **Robert Miller**

Merged: **sage-4.6.2.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/8716_





---

archive/issue_events_109059.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-04-19T16:53:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109059"
}
```



---

archive/issue_events_109060.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-04-19T16:53:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
    "label_color": "0000ff",
    "label_name": "c: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109060"
}
```



---

archive/issue_events_109061.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-04-19T16:53:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109061"
}
```



---

archive/issue_events_109062.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-04-19T16:53:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109062"
}
```



---

archive/issue_events_109063.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2010-04-19T16:53:25Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "subject": "https://github.com/loefflerd",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109063"
}
```



---

archive/issue_comments_070497.json:
```json
{
    "body": "Attachment: **[trac_8716-gamma_h_modforms.patch.gz](https://github.com/sagemath/sage/files/ticket8716/trac_8716-gamma_h_modforms.patch.gz)**\n\npatch against 4.4.alpha0",
    "created_at": "2010-04-19T16:56:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8716#issuecomment-70497",
    "user": "https://github.com/loefflerd"
}
```

Attachment: **[trac_8716-gamma_h_modforms.patch.gz](https://github.com/sagemath/sage/files/ticket8716/trac_8716-gamma_h_modforms.patch.gz)**

patch against 4.4.alpha0



---

archive/issue_events_109064.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-04-19T20:44:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109064"
}
```



---

archive/issue_comments_070498.json:
```json
{
    "body": "Reviewer: **Robert Miller**",
    "created_at": "2010-12-06T10:46:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8716#issuecomment-70498",
    "user": "https://github.com/rlmill"
}
```

Reviewer: **Robert Miller**



---

archive/issue_events_109065.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-12-06T10:46:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109065"
}
```



---

archive/issue_events_109066.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-12-06T10:46:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109066"
}
```



---

archive/issue_events_109067.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-12-06T10:46:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109067"
}
```



---

archive/issue_events_109068.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-12-06T10:46:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109068"
}
```



---

archive/issue_comments_070499.json:
```json
{
    "body": "Author: **David Loeffler**",
    "created_at": "2010-12-06T10:46:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8716#issuecomment-70499",
    "user": "https://github.com/rlmill"
}
```

Author: **David Loeffler**



---

archive/issue_comments_070500.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis patch applies fine to sage-4.6.1.alpha2 and passes all long doctests. The code looks good as well, and I think this should be merged.",
    "created_at": "2010-12-06T10:46:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8716#issuecomment-70500",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:2" align="right">comment:2</div>

This patch applies fine to sage-4.6.1.alpha2 and passes all long doctests. The code looks good as well, and I think this should be merged.



---

archive/issue_comments_070501.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nTested also against sage-4.6.1.alpha3 on sage.math.washington.edu",
    "created_at": "2010-12-06T13:15:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8716#issuecomment-70501",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">comment:3</div>

Tested also against sage-4.6.1.alpha3 on sage.math.washington.edu



---

archive/issue_comments_070502.json:
```json
{
    "body": "apply over previous patch",
    "created_at": "2010-12-12T14:34:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8716#issuecomment-70502",
    "user": "https://github.com/loefflerd"
}
```

apply over previous patch



---

archive/issue_comments_070503.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nAttachment: **[trac_8716-docfix.patch.gz](https://github.com/sagemath/sage/files/ticket8716/trac_8716-docfix.patch.gz)**\n\nOops, my bad: this patch causes an error when building the documentation, due to a silly latex mistake in one of the docstrings. Here's a tiny patch which fixes that. Robert, would you mind taking a quick look to OK it?",
    "created_at": "2010-12-12T14:36:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8716#issuecomment-70503",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:4" align="right">comment:4</div>

Attachment: **[trac_8716-docfix.patch.gz](https://github.com/sagemath/sage/files/ticket8716/trac_8716-docfix.patch.gz)**

Oops, my bad: this patch causes an error when building the documentation, due to a silly latex mistake in one of the docstrings. Here's a tiny patch which fixes that. Robert, would you mind taking a quick look to OK it?



---

archive/issue_events_109069.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-12T14:36:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109069"
}
```



---

archive/issue_events_109070.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-12T14:36:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109070"
}
```



---

archive/issue_events_109071.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-12T14:36:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109071"
}
```



---

archive/issue_events_109072.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-12T14:36:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109072"
}
```



---

archive/issue_events_109073.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-12-13T11:28:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109073"
}
```



---

archive/issue_events_109074.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-12-13T11:28:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109074"
}
```



---

archive/issue_comments_070504.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nOops!",
    "created_at": "2010-12-13T11:28:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8716#issuecomment-70504",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:6" align="right">comment:6</div>

Oops!



---

archive/issue_comments_070505.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\n\n```\nApply trac_8716-gamma_h_modforms.patch, trac_8716-docfix.patch\n```\n\n(FAO PatchBot)",
    "created_at": "2010-12-31T14:29:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8716#issuecomment-70505",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:7" align="right">comment:7</div>


```
Apply trac_8716-gamma_h_modforms.patch, trac_8716-docfix.patch
```

(FAO PatchBot)



---

archive/issue_comments_070506.json:
```json
{
    "body": "Merged: **sage-4.6.2.alpha2**",
    "created_at": "2011-01-25T08:14:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8716#issuecomment-70506",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.6.2.alpha2**



---

archive/issue_events_109075.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-25T08:14:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109075"
}
```



---

archive/issue_events_109076.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-25T08:14:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8716",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8716#event-109076"
}
```

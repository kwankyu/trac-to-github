# Issue 8305: Improve documentation of Monsky-Washnitzer code

archive/issues_008305.json:
```json
{
    "assignees": [
        "https://github.com/kedlaya"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe code in schemes/elliptic_curves/monsky_washnitzer.py largely dates from a time (early 2007) before Sage documentation and doctesting standards had been codified. As a result, its coverage is terrible (26 of 107).\n\nIt may also be worth a mild refactor: since it now applies more generally to hyperelliptic curves, it probably should be under schemes/hyperelliptic_curves.\n\nCC:  @jpflori\n\nComponent: **elliptic curves**\n\nKeywords: **Monsky-Washnitzer, hyperelliptic curves**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\nBranch/Commit: **[u/chapoton/8305](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/8305) @ [`18c386a`](https://github.com/sagemath/sagetrac-mirror/commit/18c386a894b4e508ef551e5833e858ea2a5d16dd)**\n\nReviewer: **Kiran Kedlaya**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8305_\n\n",
    "closed_at": "2014-01-10T07:29:50Z",
    "created_at": "2010-02-19T03:34:22Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20elliptic%20curves",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20elliptic%20curves"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Improve documentation of Monsky-Washnitzer code",
    "type": "issue",
    "updated_at": "2014-01-10T07:29:50Z",
    "url": "https://github.com/sagemath/sage/issues/8305",
    "user": "https://github.com/kedlaya"
}
```
<div id="comment:0"></div>

The code in schemes/elliptic_curves/monsky_washnitzer.py largely dates from a time (early 2007) before Sage documentation and doctesting standards had been codified. As a result, its coverage is terrible (26 of 107).

It may also be worth a mild refactor: since it now applies more generally to hyperelliptic curves, it probably should be under schemes/hyperelliptic_curves.

CC:  @jpflori

Component: **elliptic curves**

Keywords: **Monsky-Washnitzer, hyperelliptic curves**

Author: **Frédéric Chapoton**

Branch/Commit: **[u/chapoton/8305](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/8305) @ [`18c386a`](https://github.com/sagemath/sagetrac-mirror/commit/18c386a894b4e508ef551e5833e858ea2a5d16dd)**

Reviewer: **Kiran Kedlaya**

_Issue created by migration from https://trac.sagemath.org/ticket/8305_





---

archive/issue_events_102689.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2010-02-19T03:34:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8305#event-102689"
}
```



---

archive/issue_events_102690.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2010-02-19T03:34:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20elliptic%20curves",
    "label_color": "0000ff",
    "label_name": "c: elliptic curves",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8305#event-102690"
}
```



---

archive/issue_events_102691.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2010-02-19T03:34:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8305#event-102691"
}
```



---

archive/issue_events_102692.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2010-02-19T03:34:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8305#event-102692"
}
```



---

archive/issue_events_102693.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2010-02-19T03:34:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20elliptic%20curves",
    "label_color": "0000ff",
    "label_name": "c: elliptic curves",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8305#event-102693"
}
```



---

archive/issue_events_102694.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-02-19T03:34:22Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "subject": "https://github.com/kedlaya",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8305#event-102694"
}
```



---

archive/issue_comments_064695.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThe patch at #7926 brings coverage up to 50% (though I didn't make it to documenting the really interesting stuff).",
    "created_at": "2010-02-20T08:54:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8305#issuecomment-64695",
    "user": "https://github.com/robertwb"
}
```

<div id="comment:1" align="right">comment:1</div>

The patch at #7926 brings coverage up to 50% (though I didn't make it to documenting the really interesting stuff).



---

archive/issue_comments_064696.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nOK, so this ticket should stay on ice until someone (e.g., me) has a chance to review #7926. Besides the documentation, there is also the issue of moving the MW code from elliptic to hyperelliptic where it belongs.",
    "created_at": "2010-02-20T14:57:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8305#issuecomment-64696",
    "user": "https://github.com/kedlaya"
}
```

<div id="comment:2" align="right">comment:2</div>

OK, so this ticket should stay on ice until someone (e.g., me) has a chance to review #7926. Besides the documentation, there is also the issue of moving the MW code from elliptic to hyperelliptic where it belongs.



---

archive/issue_events_102695.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-02-20T14:57:44Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "subject": "https://github.com/kedlaya",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8305#event-102695"
}
```



---

archive/issue_events_102696.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2010-02-20T14:57:44Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "subject": "https://github.com/kedlaya",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8305#event-102696"
}
```



---

archive/issue_comments_064697.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAttachment: **[trac_8305_monsky_doc.patch.gz](https://github.com/sagemath/sage/files/ticket8305/trac_8305_monsky_doc.patch.gz)**\n\nfirst patch, with coverage reaching 67%",
    "created_at": "2013-07-09T09:35:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8305#issuecomment-64697",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">comment:3</div>

Attachment: **[trac_8305_monsky_doc.patch.gz](https://github.com/sagemath/sage/files/ticket8305/trac_8305_monsky_doc.patch.gz)**

first patch, with coverage reaching 67%



---

archive/issue_events_102697.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8305#event-102697"
}
```



---

archive/issue_events_102698.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8305#event-102698"
}
```



---

archive/issue_comments_064698.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nafter some more work, coverage is almost 77%",
    "created_at": "2013-08-23T16:40:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8305#issuecomment-64698",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:5" align="right">comment:5</div>

after some more work, coverage is almost 77%



---

archive/issue_comments_064699.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nafter some more work, coverage is now 83%",
    "created_at": "2013-10-01T19:37:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8305#issuecomment-64699",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:7" align="right">comment:7</div>

after some more work, coverage is now 83%



---

archive/issue_comments_064700.json:
```json
{
    "body": "Attachment: **[trac_8305_monsky_doc_step2.patch.gz](https://github.com/sagemath/sage/files/ticket8305/trac_8305_monsky_doc_step2.patch.gz)**",
    "created_at": "2013-10-14T19:33:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8305#issuecomment-64700",
    "user": "https://github.com/fchapoton"
}
```

Attachment: **[trac_8305_monsky_doc_step2.patch.gz](https://github.com/sagemath/sage/files/ticket8305/trac_8305_monsky_doc_step2.patch.gz)**



---

archive/issue_comments_064701.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nafter some more work, coverage is now 85%",
    "created_at": "2013-10-14T19:33:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8305#issuecomment-64701",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:8" align="right">comment:8</div>

after some more work, coverage is now 85%



---

archive/issue_events_102699.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-01-05T13:00:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8305#event-102699"
}
```



---

archive/issue_comments_064702.json:
```json
{
    "body": "Commit: **[`18c386a`](https://github.com/sagemath/sagetrac-mirror/commit/18c386a894b4e508ef551e5833e858ea2a5d16dd)**",
    "created_at": "2014-01-05T13:00:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8305#issuecomment-64702",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[`18c386a`](https://github.com/sagemath/sagetrac-mirror/commit/18c386a894b4e508ef551e5833e858ea2a5d16dd)**



---

archive/issue_comments_064703.json:
```json
{
    "body": "Branch: **[u/chapoton/8305](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/8305)**",
    "created_at": "2014-01-05T13:00:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8305#issuecomment-64703",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/8305](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/8305)**



---

archive/issue_comments_064704.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nmaybe this can be considered as a good first step towards 100% ?\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/18c386a894b4e508ef551e5833e858ea2a5d16dd\"><code>18c386a</code></a></td><td><code>trac #8305 clean monsky-w doc step 2</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/71cab69ff6a6ca8e256d475da03d4aef9d26789f\"><code>71cab69</code></a></td><td><code>trac 8305 doc of monsky waschnitzer</code></td></tr></table>\n",
    "created_at": "2014-01-05T13:00:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8305#issuecomment-64704",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:9" align="right">comment:9</div>

maybe this can be considered as a good first step towards 100% ?

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/18c386a894b4e508ef551e5833e858ea2a5d16dd"><code>18c386a</code></a></td><td><code>trac #8305 clean monsky-w doc step 2</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/71cab69ff6a6ca8e256d475da03d4aef9d26789f"><code>71cab69</code></a></td><td><code>trac 8305 doc of monsky waschnitzer</code></td></tr></table>




---

archive/issue_comments_064705.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nAn excellent step, yes! Good work.\n\nI've spun off tickets #15645 and #15646 to track the remaining issues on this ticket (100% coverage and refactoring).",
    "created_at": "2014-01-07T23:08:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8305#issuecomment-64705",
    "user": "https://github.com/kedlaya"
}
```

<div id="comment:10" align="right">comment:10</div>

An excellent step, yes! Good work.

I've spun off tickets #15645 and #15646 to track the remaining issues on this ticket (100% coverage and refactoring).



---

archive/issue_events_102700.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2014-01-07T23:08:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8305#event-102700"
}
```



---

archive/issue_events_102701.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2014-01-07T23:08:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8305#event-102701"
}
```



---

archive/issue_comments_064706.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2014-01-08T08:50:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8305#issuecomment-64706",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_064707.json:
```json
{
    "body": "Reviewer: **Kiran Kedlaya**",
    "created_at": "2014-01-09T06:43:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8305#issuecomment-64707",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Kiran Kedlaya**



---

archive/issue_comments_064708.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nFill in the reviewer name...",
    "created_at": "2014-01-09T06:43:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8305#issuecomment-64708",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:12" align="right">comment:12</div>

Fill in the reviewer name...



---

archive/issue_events_102702.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-01-10T07:29:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8305#event-102702"
}
```



---

archive/issue_events_102703.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "980c7377a9053d0046990a1441b8862e14265106",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-01-10T07:29:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8305",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8305#event-102703"
}
```

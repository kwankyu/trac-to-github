# Issue 8050: fix SPKG.txt in cddlib

archive/issues_008050.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nThe file SPKG.txt in the cddlib spkg does not respect the rules at\n\nhttp://www.sagemath.org/doc/developer/producing_spkgs.html\n\n\n\nCC:  @vbraun @aghitza @sagetrac-mhampton\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8050_\n\n",
    "closed_at": "2010-02-20T17:31:46Z",
    "created_at": "2010-01-24T03:23:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix SPKG.txt in cddlib",
    "type": "issue",
    "updated_at": "2010-02-20T17:31:46Z",
    "url": "https://github.com/sagemath/sage/issues/8050",
    "user": "https://github.com/aghitza"
}
```
<div id="comment:0"></div>

The file SPKG.txt in the cddlib spkg does not respect the rules at

http://www.sagemath.org/doc/developer/producing_spkgs.html



CC:  @vbraun @aghitza @sagetrac-mhampton

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/8050_





---

archive/issue_events_099174.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-01-24T03:23:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8050",
    "milestone_number": null,
    "milestone_title": "sage-4.3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8050#event-99174"
}
```



---

archive/issue_events_099175.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-01-24T03:23:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8050",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "0000b0",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8050#event-99175"
}
```



---

archive/issue_events_099176.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-01-24T03:23:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8050",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8050#event-99176"
}
```



---

archive/issue_events_099177.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-01-24T03:23:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8050",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8050#event-99177"
}
```



---

archive/issue_comments_061740.json:
```json
{
    "body": "Updated SPKG.txt",
    "created_at": "2010-01-27T17:07:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8050#issuecomment-61740",
    "user": "https://github.com/vbraun"
}
```

Updated SPKG.txt



---

archive/issue_comments_061741.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[SPKG.txt](https://github.com/sagemath/sage/files/ticket8050/SPKG.txt)**\n\nI've attached my current version, and I think it conforms to the specs. Let me know if there is anything missing.",
    "created_at": "2010-01-27T17:09:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8050#issuecomment-61741",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[SPKG.txt](https://github.com/sagemath/sage/files/ticket8050/SPKG.txt)**

I've attached my current version, and I think it conforms to the specs. Let me know if there is anything missing.



---

archive/issue_events_099178.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2010-01-27T17:09:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8050",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8050#event-99178"
}
```



---

archive/issue_comments_061742.json:
```json
{
    "body": "Reviewer: **AlexGhitza, mhampton**",
    "created_at": "2010-01-27T17:13:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8050#issuecomment-61742",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **AlexGhitza, mhampton**



---

archive/issue_comments_061743.json:
```json
{
    "body": "Changed reviewer from **AlexGhitza, mhampton** to none",
    "created_at": "2010-01-27T17:25:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8050#issuecomment-61743",
    "user": "https://github.com/vbraun"
}
```

Changed reviewer from **AlexGhitza, mhampton** to none



---

archive/issue_comments_061744.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nCould be patch be rebased against cddlib-094f.p4.spkg?",
    "created_at": "2010-02-20T16:56:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8050#issuecomment-61744",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:4" align="right">comment:4</div>

Could be patch be rebased against cddlib-094f.p4.spkg?



---

archive/issue_comments_061745.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nThe SPKG.txt attached to this ticket is an older version, in particular the sections \"Special Update/Build Instructions\" are wrong. \n\nAll outstanding issues are already fixed in cddlib-094f.p4.spkg, see #8115. I recommend to close this ticket.",
    "created_at": "2010-02-20T17:30:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8050#issuecomment-61745",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:5" align="right">comment:5</div>

The SPKG.txt attached to this ticket is an older version, in particular the sections "Special Update/Build Instructions" are wrong. 

All outstanding issues are already fixed in cddlib-094f.p4.spkg, see #8115. I recommend to close this ticket.



---

archive/issue_comments_061746.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nClose as fixed by #8115.",
    "created_at": "2010-02-20T17:31:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8050#issuecomment-61746",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:6" align="right">comment:6</div>

Close as fixed by #8115.



---

archive/issue_events_099179.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-20T17:31:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8050",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8050#event-99179"
}
```



---

archive/issue_events_099180.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-20T17:31:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8050",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8050#event-99180"
}
```

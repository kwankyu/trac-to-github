# Issue 8907: build Python with --enabled-shared

archive/issues_008907.json:
```json
{
    "assignees": [
        "https://github.com/aghitza"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis is needed by #8542 .  There is an spkg at http://sage.math.washington.edu/home/mhansen/python-2.6.4.p8.spkg but it requires lots of testing.  Also, the changes are not committed to the repository.\n\nComponent: **porting: Cygwin**\n\nAuthor: **Mike Hansen**\n\nReviewer: **William Stein**\n\nMerged: **sage-4.4.3.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8907_\n\n",
    "closed_at": "2010-05-26T00:44:22Z",
    "created_at": "2010-05-06T17:10:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "build Python with --enabled-shared",
    "type": "issue",
    "updated_at": "2010-05-27T04:34:48Z",
    "url": "https://github.com/sagemath/sage/issues/8907",
    "user": "https://github.com/mwhansen"
}
```
<div id="comment:0"></div>

This is needed by #8542 .  There is an spkg at http://sage.math.washington.edu/home/mhansen/python-2.6.4.p8.spkg but it requires lots of testing.  Also, the changes are not committed to the repository.

Component: **porting: Cygwin**

Author: **Mike Hansen**

Reviewer: **William Stein**

Merged: **sage-4.4.3.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/8907_





---

archive/issue_events_111907.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-06T17:10:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-111907"
}
```



---

archive/issue_events_111908.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-06T17:10:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-111908"
}
```



---

archive/issue_events_111909.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-06T17:10:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-111909"
}
```



---

archive/issue_events_111910.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-05-06T17:10:55Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-111910"
}
```



---

archive/issue_events_111911.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-06T17:11:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "milestone_number": null,
    "milestone_title": "sage-4.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-111911"
}
```



---

archive/issue_comments_073021.json:
```json
{
    "body": "Author: **Mike Hansen**",
    "created_at": "2010-05-06T17:11:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8907#issuecomment-73021",
    "user": "https://github.com/mwhansen"
}
```

Author: **Mike Hansen**



---

archive/issue_events_111912.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-06T17:11:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-111912"
}
```



---

archive/issue_events_111913.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-06T17:11:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20cygwin",
    "label_color": "0000b0",
    "label_name": "c: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-111913"
}
```



---

archive/issue_events_111914.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-06T17:11:26Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "title_is": "build Python with --enabled-shared",
    "title_was": "build Python as a shared library",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-111914"
}
```



---

archive/issue_events_111915.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-06T17:11:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-111915"
}
```



---

archive/issue_comments_073022.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nCheck on OS X, we don't need --enable-shared.  I'm check on t2 now.  The spkg will have to be updated accordingly.",
    "created_at": "2010-05-06T17:57:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8907#issuecomment-73022",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:2" align="right">comment:2</div>

Check on OS X, we don't need --enable-shared.  I'm check on t2 now.  The spkg will have to be updated accordingly.



---

archive/issue_comments_073023.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\n--enabled-shared fails on t2 because of http://bugs.python.org/issue1628484 .  We need to apply the patch there.",
    "created_at": "2010-05-25T05:47:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8907#issuecomment-73023",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">comment:3</div>

--enabled-shared fails on t2 because of http://bugs.python.org/issue1628484 .  We need to apply the patch there.



---

archive/issue_events_111916.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-05-26T00:44:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-111916"
}
```



---

archive/issue_events_111917.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-05-26T00:44:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-111917"
}
```



---

archive/issue_comments_073024.json:
```json
{
    "body": "Merged: **4.4.3.alpha0**",
    "created_at": "2010-05-26T00:44:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8907#issuecomment-73024",
    "user": "https://github.com/williamstein"
}
```

Merged: **4.4.3.alpha0**



---

archive/issue_comments_073025.json:
```json
{
    "body": "Reviewer: **wstein**",
    "created_at": "2010-05-26T00:44:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8907#issuecomment-73025",
    "user": "https://github.com/williamstein"
}
```

Reviewer: **wstein**



---

archive/issue_comments_073026.json:
```json
{
    "body": "Changed merged from **4.4.3.alpha0** to **sage-4.4.3.alpha0**",
    "created_at": "2010-05-27T04:34:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8907#issuecomment-73026",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed merged from **4.4.3.alpha0** to **sage-4.4.3.alpha0**



---

archive/issue_comments_073027.json:
```json
{
    "body": "Changed reviewer from **wstein** to **William Stein**",
    "created_at": "2010-05-27T04:34:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8907#issuecomment-73027",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed reviewer from **wstein** to **William Stein**

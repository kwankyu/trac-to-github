# Issue 8002: remove dead code from sage-ptest

archive/issues_008002.json:
```json
{
    "body": "The `sage-ptest` parallel doctesting script has some dead code in it: a function `run` that's unused, and a `-sage` flag that triggers a call to a no longer existing `sage-doctest_tex` script.\n\nI'm attaching a patch that cleans this up, and as a side effect allows `sage-ptest` to test files with a `.sage` extension.\n\n**Author:** Willem Jan Palenstijn\n\n**Reviewer:** Rob Beezer\n\n**Merged:** sage-4.3.2.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/8002\n\n",
    "closed_at": "2010-01-23T10:23:46Z",
    "created_at": "2010-01-19T19:13:52Z",
    "labels": [
        "component: doctest coverage",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.2",
    "title": "remove dead code from sage-ptest",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/8002",
    "user": "https://github.com/wjp"
}
```
The `sage-ptest` parallel doctesting script has some dead code in it: a function `run` that's unused, and a `-sage` flag that triggers a call to a no longer existing `sage-doctest_tex` script.

I'm attaching a patch that cleans this up, and as a side effect allows `sage-ptest` to test files with a `.sage` extension.

**Author:** Willem Jan Palenstijn

**Reviewer:** Rob Beezer

**Merged:** sage-4.3.2.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/8002





---

archive/attachments_010218.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "scripts_8002_ptest_cleanup.patch",
    "asset_url": "tarball://root/attachments/ticket8002/scripts_8002_ptest_cleanup.patch",
    "created_at": "2010-01-19T19:26:13Z",
    "issue": "https://github.com/sagemath/sage/issues/8002",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8002/scripts_8002_ptest_cleanup.patch",
    "user": "https://github.com/wjp"
}
```



---

archive/issue_comments_062998.json:
```json
{
    "body": "**Attachment:** [scripts_8002_ptest_cleanup.patch](https://github.com/sagemath/sage/files/ticket8002/scripts_8002_ptest_cleanup.patch)",
    "created_at": "2010-01-19T19:26:13Z",
    "issue": "https://github.com/sagemath/sage/issues/8002",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8002#issuecomment-62998",
    "user": "https://github.com/wjp"
}
```

**Attachment:** [scripts_8002_ptest_cleanup.patch](https://github.com/sagemath/sage/files/ticket8002/scripts_8002_ptest_cleanup.patch)



---

archive/issue_events_055925.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-01-19T19:26:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8002",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8002#event-55925"
}
```



---

archive/issue_comments_062999.json:
```json
{
    "body": "**Author:** Willem Jan Palenstijn",
    "created_at": "2010-01-21T01:09:57Z",
    "issue": "https://github.com/sagemath/sage/issues/8002",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8002#issuecomment-62999",
    "user": "https://github.com/rbeezer"
}
```

**Author:** Willem Jan Palenstijn



---

archive/issue_events_055926.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-01-21T01:09:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8002",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8002#event-55926"
}
```



---

archive/issue_events_055927.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-01-21T01:09:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8002",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8002#event-55927"
}
```



---

archive/issue_comments_063000.json:
```json
{
    "body": "<a id='comment:2'></a>\nWritten as advertised, `run()` routine is never called, `doctest_tex` is no longer shipped as part of Sage.\n\nApplies cleanly and works fine in limited testing.\n\nPositive review.",
    "created_at": "2010-01-21T01:09:57Z",
    "issue": "https://github.com/sagemath/sage/issues/8002",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8002#issuecomment-63000",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:2'></a>
Written as advertised, `run()` routine is never called, `doctest_tex` is no longer shipped as part of Sage.

Applies cleanly and works fine in limited testing.

Positive review.



---

archive/issue_comments_063001.json:
```json
{
    "body": "**Reviewer:** Rob Beezer",
    "created_at": "2010-01-21T01:09:57Z",
    "issue": "https://github.com/sagemath/sage/issues/8002",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8002#issuecomment-63001",
    "user": "https://github.com/rbeezer"
}
```

**Reviewer:** Rob Beezer



---

archive/issue_comments_063002.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in script repository.",
    "created_at": "2010-01-23T10:23:46Z",
    "issue": "https://github.com/sagemath/sage/issues/8002",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8002#issuecomment-63002",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:3'></a>
Merged in script repository.



---

archive/issue_comments_063003.json:
```json
{
    "body": "**Merged:** sage-4.3.2.alpha0",
    "created_at": "2010-01-23T10:23:46Z",
    "issue": "https://github.com/sagemath/sage/issues/8002",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8002#issuecomment-63003",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Merged:** sage-4.3.2.alpha0



---

archive/issue_events_055928.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-01-23T10:23:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8002",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8002#event-55928"
}
```



---

archive/issue_events_055929.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-01-23T10:23:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8002",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8002#event-55929"
}
```

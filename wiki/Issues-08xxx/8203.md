# Issue 8203: misc doc fixes

archive/issues_008203.json:
```json
{
    "body": "The attached patch fixes several warnings when building the documentation.\n\n\n**Assignee:** mvngu\n\n**Author:** John Palmieri\n\n**Reviewer:** Marshall Hampton\n\n**Merged:** sage-4.3.3.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/8203\n\n",
    "closed_at": "2010-02-11T14:41:28Z",
    "created_at": "2010-02-07T03:48:02Z",
    "labels": [
        "component: documentation",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.3",
    "title": "misc doc fixes",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/8203",
    "user": "https://github.com/jhpalmieri"
}
```
The attached patch fixes several warnings when building the documentation.


**Assignee:** mvngu

**Author:** John Palmieri

**Reviewer:** Marshall Hampton

**Merged:** sage-4.3.3.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/8203





---

archive/issue_comments_065274.json:
```json
{
    "body": "<a id='comment:1'></a>\nThe attachment [trac_8203-doc.patch](https://github.com/sagemath/sage/files/ticket8203/trac_8203-doc.patch) conflicts with the patch at #8190: \n\n```\n[mvngu@sage sage-main]$ pwd\n/dev/shm/mvngu/sage-4.3.2-sage.math/devel/sage-main\n[mvngu@sage sage-main]$ hg qimport https://github.com/sagemath/sage/files/ticket8190/trac_8190-docbuild.patch && hg qpush\nadding trac_8190-docbuild.patch to series file\napplying trac_8190-docbuild.patch\nnow at: trac_8190-docbuild.patch\n[mvngu@sage sage-main]$ hg qimport https://github.com/sagemath/sage/files/ticket8203/trac_8203-doc.patch && hg qpush\nadding trac_8203-doc.patch to series file\napplying trac_8203-doc.patch\npatching file sage/gsl/ode.pyx\nHunk #1 FAILED at 205\n1 out of 3 hunks FAILED -- saving rejects to file sage/gsl/ode.pyx.rej\npatching file sage/rings/polynomial/multi_polynomial_ideal.py\nHunk #1 FAILED at 644\n1 out of 1 hunks FAILED -- saving rejects to file sage/rings/polynomial/multi_polynomial_ideal.py.rej\npatching file sage/rings/quotient_ring.py\nHunk #1 FAILED at 527\n1 out of 1 hunks FAILED -- saving rejects to file sage/rings/quotient_ring.py.rej\npatching file sage/schemes/elliptic_curves/ell_generic.py\nHunk #1 FAILED at 2155\n1 out of 1 hunks FAILED -- saving rejects to file sage/schemes/elliptic_curves/ell_generic.py.rej\npatching file sage/symbolic/expression.pyx\nHunk #1 succeeded at 4978 with fuzz 2 (offset -957 lines).\npatching file sage/symbolic/relation.py\nHunk #1 FAILED at 860\nHunk #2 FAILED at 925\nHunk #3 FAILED at 985\nHunk #4 FAILED at 1014\n4 out of 4 hunks FAILED -- saving rejects to file sage/symbolic/relation.py.rej\npatch failed, unable to continue (try -v)\npatch failed, rejects left in working dir\nerrors during apply, please fix and refresh trac_8203-doc.patch\n```\nDo you want to rebase [trac_8203-doc.patch](https://github.com/sagemath/sage/files/ticket8203/trac_8203-doc.patch) on top of #8190?",
    "created_at": "2010-02-07T04:06:50Z",
    "issue": "https://github.com/sagemath/sage/issues/8203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8203#issuecomment-65274",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:1'></a>
The attachment [trac_8203-doc.patch](https://github.com/sagemath/sage/files/ticket8203/trac_8203-doc.patch) conflicts with the patch at #8190: 

```
[mvngu@sage sage-main]$ pwd
/dev/shm/mvngu/sage-4.3.2-sage.math/devel/sage-main
[mvngu@sage sage-main]$ hg qimport https://github.com/sagemath/sage/files/ticket8190/trac_8190-docbuild.patch && hg qpush
adding trac_8190-docbuild.patch to series file
applying trac_8190-docbuild.patch
now at: trac_8190-docbuild.patch
[mvngu@sage sage-main]$ hg qimport https://github.com/sagemath/sage/files/ticket8203/trac_8203-doc.patch && hg qpush
adding trac_8203-doc.patch to series file
applying trac_8203-doc.patch
patching file sage/gsl/ode.pyx
Hunk #1 FAILED at 205
1 out of 3 hunks FAILED -- saving rejects to file sage/gsl/ode.pyx.rej
patching file sage/rings/polynomial/multi_polynomial_ideal.py
Hunk #1 FAILED at 644
1 out of 1 hunks FAILED -- saving rejects to file sage/rings/polynomial/multi_polynomial_ideal.py.rej
patching file sage/rings/quotient_ring.py
Hunk #1 FAILED at 527
1 out of 1 hunks FAILED -- saving rejects to file sage/rings/quotient_ring.py.rej
patching file sage/schemes/elliptic_curves/ell_generic.py
Hunk #1 FAILED at 2155
1 out of 1 hunks FAILED -- saving rejects to file sage/schemes/elliptic_curves/ell_generic.py.rej
patching file sage/symbolic/expression.pyx
Hunk #1 succeeded at 4978 with fuzz 2 (offset -957 lines).
patching file sage/symbolic/relation.py
Hunk #1 FAILED at 860
Hunk #2 FAILED at 925
Hunk #3 FAILED at 985
Hunk #4 FAILED at 1014
4 out of 4 hunks FAILED -- saving rejects to file sage/symbolic/relation.py.rej
patch failed, unable to continue (try -v)
patch failed, rejects left in working dir
errors during apply, please fix and refresh trac_8203-doc.patch
```
Do you want to rebase [trac_8203-doc.patch](https://github.com/sagemath/sage/files/ticket8203/trac_8203-doc.patch) on top of #8190?



---

archive/issue_events_057573.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-02-07T04:06:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8203",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8203#event-57573"
}
```



---

archive/issue_comments_065275.json:
```json
{
    "body": "**Work Issues:** rebase on top of #8190",
    "created_at": "2010-02-07T04:06:50Z",
    "issue": "https://github.com/sagemath/sage/issues/8203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8203#issuecomment-65275",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Work Issues:** rebase on top of #8190



---

archive/issue_comments_065276.json:
```json
{
    "body": "<a id='comment:2'></a>\nReplying to [mvngu](#comment%3A1):\n> Do you want to rebase [trac_8203-doc.patch](https://github.com/sagemath/sage/files/ticket8203/trac_8203-doc.patch) on top of #8190?\n\nDo I *want* to?  Not really.  But here's a new version of the patch.\n\nBy the way, can I add \"delete `SAGE_ROOT/devel/sage/doc/en/reference/sage/misc/attach.rst`\" to this ticket, since this didn't get done as part of #8022?",
    "created_at": "2010-02-07T05:37:23Z",
    "issue": "https://github.com/sagemath/sage/issues/8203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8203#issuecomment-65276",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'></a>
Replying to [mvngu](#comment%3A1):
> Do you want to rebase [trac_8203-doc.patch](https://github.com/sagemath/sage/files/ticket8203/trac_8203-doc.patch) on top of #8190?

Do I *want* to?  Not really.  But here's a new version of the patch.

By the way, can I add "delete `SAGE_ROOT/devel/sage/doc/en/reference/sage/misc/attach.rst`" to this ticket, since this didn't get done as part of #8022?



---

archive/issue_events_057574.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-02-07T05:37:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8203",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8203#event-57574"
}
```



---

archive/issue_events_057575.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-02-07T05:37:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8203",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8203#event-57575"
}
```



---

archive/attachments_010507.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8203-doc.patch",
    "asset_url": "tarball://root/attachments/ticket8203/trac_8203-doc.patch",
    "created_at": "2010-02-07T05:37:52Z",
    "issue": "https://github.com/sagemath/sage/issues/8203",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8203/trac_8203-doc.patch",
    "user": "https://github.com/jhpalmieri"
}
```



---

archive/issue_comments_065277.json:
```json
{
    "body": "**Attachment:** [trac_8203-doc.patch](https://github.com/sagemath/sage/files/ticket8203/trac_8203-doc.patch)",
    "created_at": "2010-02-07T05:37:52Z",
    "issue": "https://github.com/sagemath/sage/issues/8203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8203#issuecomment-65277",
    "user": "https://github.com/jhpalmieri"
}
```

**Attachment:** [trac_8203-doc.patch](https://github.com/sagemath/sage/files/ticket8203/trac_8203-doc.patch)



---

archive/issue_comments_065278.json:
```json
{
    "body": "<a id='comment:3'></a>\nLooks good to me, seems pretty simple and docs build OK.  Positive review.",
    "created_at": "2010-02-07T23:27:56Z",
    "issue": "https://github.com/sagemath/sage/issues/8203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8203#issuecomment-65278",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

<a id='comment:3'></a>
Looks good to me, seems pretty simple and docs build OK.  Positive review.



---

archive/issue_events_057576.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mhampton",
    "created_at": "2010-02-07T23:27:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8203",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8203#event-57576"
}
```



---

archive/issue_events_057577.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mhampton",
    "created_at": "2010-02-07T23:27:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8203",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8203#event-57577"
}
```



---

archive/issue_comments_065279.json:
```json
{
    "body": "**Merged:** sage-4.3.3.alpha0",
    "created_at": "2010-02-11T14:41:28Z",
    "issue": "https://github.com/sagemath/sage/issues/8203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8203#issuecomment-65279",
    "user": "https://github.com/qed777"
}
```

**Merged:** sage-4.3.3.alpha0



---

archive/issue_comments_065280.json:
```json
{
    "body": "**Changing work issues** from \"rebase on top of #8190\" to \"\".",
    "created_at": "2010-02-11T14:41:28Z",
    "issue": "https://github.com/sagemath/sage/issues/8203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8203#issuecomment-65280",
    "user": "https://github.com/qed777"
}
```

**Changing work issues** from "rebase on top of #8190" to "".



---

archive/issue_events_057578.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-02-11T14:41:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8203",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8203#event-57578"
}
```



---

archive/issue_events_057579.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-02-11T14:41:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8203",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8203#event-57579"
}
```



---

archive/issue_comments_065281.json:
```json
{
    "body": "**Reviewer:** Marshall Hampton",
    "created_at": "2010-02-11T14:41:28Z",
    "issue": "https://github.com/sagemath/sage/issues/8203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8203#issuecomment-65281",
    "user": "https://github.com/qed777"
}
```

**Reviewer:** Marshall Hampton

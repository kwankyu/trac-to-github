# Issue 8405: Linear Arboricity, Acyclic edge coloring

archive/issues_008405.json:
```json
{
    "body": "This patch implements LP formulations of Linear Arboricity and Acyclic edge coloring\n\nNathann\n\nThis has spam content and the spammer (mike3050) closed the ticket. Rather than reopen the ticket and result in confusion, I have moved the ticket content to #8748. To see the spam content, view the diff of the ticket description.\n\n**Assignee:** @rlmill\n\nIssue created by migration from https://trac.sagemath.org/ticket/8405\n\n",
    "closed_at": "2010-04-22T22:16:43Z",
    "created_at": "2010-02-28T23:46:35Z",
    "labels": [
        "component: graph theory",
        "enhancement",
        "invalid"
    ],
    "title": "Linear Arboricity, Acyclic edge coloring",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/8405",
    "user": "https://github.com/nathanncohen"
}
```
This patch implements LP formulations of Linear Arboricity and Acyclic edge coloring

Nathann

This has spam content and the spammer (mike3050) closed the ticket. Rather than reopen the ticket and result in confusion, I have moved the ticket content to #8748. To see the spam content, view the diff of the ticket description.

**Assignee:** @rlmill

Issue created by migration from https://trac.sagemath.org/ticket/8405





---

archive/issue_events_059506.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-02-28T23:47:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8405",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8405#event-59506"
}
```



---

archive/attachments_010840.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8405.patch",
    "asset_url": "tarball://root/attachments/ticket8405/trac_8405.patch",
    "created_at": "2010-03-01T00:11:12Z",
    "issue": "https://github.com/sagemath/sage/issues/8405",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8405/trac_8405.patch",
    "user": "https://github.com/nathanncohen"
}
```



---

archive/issue_comments_068141.json:
```json
{
    "body": "**Attachment:** [trac_8405.patch](https://github.com/sagemath/sage/files/ticket8405/trac_8405.patch)",
    "created_at": "2010-03-01T00:11:12Z",
    "issue": "https://github.com/sagemath/sage/issues/8405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8405#issuecomment-68141",
    "user": "https://github.com/nathanncohen"
}
```

**Attachment:** [trac_8405.patch](https://github.com/sagemath/sage/files/ticket8405/trac_8405.patch)



---

archive/issue_comments_068142.json:
```json
{
    "body": "<a id='comment:2'></a>\nFor an explanation of the Linear Program used to solve this problem, see the LP chapter from : http://code.google.com/p/graph-theory-algorithms-book/\n\nNathann",
    "created_at": "2010-04-08T21:21:12Z",
    "issue": "https://github.com/sagemath/sage/issues/8405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8405#issuecomment-68142",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:2'></a>
For an explanation of the Linear Program used to solve this problem, see the LP chapter from : http://code.google.com/p/graph-theory-algorithms-book/

Nathann



---

archive/issue_events_059507.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mike3050",
    "created_at": "2010-04-22T22:16:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8405",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8405#event-59507"
}
```



---

archive/issue_events_059508.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mike3050",
    "created_at": "2010-04-22T22:16:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8405",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8405#event-59508"
}
```



---

archive/issue_comments_068143.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,3 +1,4 @@\n-This patch implements LP formulations of Linear Arboricity and Acyclic edge coloring\n+This patch implements LP formulations of [auto insurance quotes](http://zolpo.com/auto-insurance/) Linear Arboricity and Acyclic edge coloring\n \n Nathann\n+Thank you.I got it.\n``````\n",
    "created_at": "2010-04-22T22:16:43Z",
    "issue": "https://github.com/sagemath/sage/issues/8405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8405#issuecomment-68143",
    "user": "https://trac.sagemath.org/admin/accounts/users/mike3050"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,3 +1,4 @@
-This patch implements LP formulations of Linear Arboricity and Acyclic edge coloring
+This patch implements LP formulations of [auto insurance quotes](http://zolpo.com/auto-insurance/) Linear Arboricity and Acyclic edge coloring
 
 Nathann
+Thank you.I got it.
``````




---

archive/issue_comments_068144.json:
```json
{
    "body": "<a id='comment:3'></a>\nReplying to [@nathanncohen](#comment%3A2):\n> For an explanation of the Linear Program used to solve this problem, see the LP chapter from : http://code.google.com/p/graph-theory-algorithms-book/\n> \n> Nathann",
    "created_at": "2010-04-22T22:16:43Z",
    "issue": "https://github.com/sagemath/sage/issues/8405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8405#issuecomment-68144",
    "user": "https://trac.sagemath.org/admin/accounts/users/mike3050"
}
```

<a id='comment:3'></a>
Replying to [@nathanncohen](#comment%3A2):
> For an explanation of the Linear Program used to solve this problem, see the LP chapter from : http://code.google.com/p/graph-theory-algorithms-book/
> 
> Nathann



---

archive/issue_comments_068145.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,4 +1,5 @@\n-This patch implements LP formulations of [auto insurance quotes](http://zolpo.com/auto-insurance/) Linear Arboricity and Acyclic edge coloring\n+This patch implements LP formulations of Linear Arboricity and Acyclic edge coloring\n \n Nathann\n-Thank you.I got it.\n+\n+This has spam content and the spammer (mike3050) closed the ticket. Rather than reopen the ticket and result in confusion, I have moved the ticket content to #8748. To see the spam content, view the diff of the ticket description.\n``````\n",
    "created_at": "2010-04-23T01:09:24Z",
    "issue": "https://github.com/sagemath/sage/issues/8405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8405#issuecomment-68145",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,4 +1,5 @@
-This patch implements LP formulations of [auto insurance quotes](http://zolpo.com/auto-insurance/) Linear Arboricity and Acyclic edge coloring
+This patch implements LP formulations of Linear Arboricity and Acyclic edge coloring
 
 Nathann
-Thank you.I got it.
+
+This has spam content and the spammer (mike3050) closed the ticket. Rather than reopen the ticket and result in confusion, I have moved the ticket content to #8748. To see the spam content, view the diff of the ticket description.
``````




---

archive/issue_events_059509.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-04-23T01:09:24Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8405",
    "milestone": "sage-4.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8405#event-59509"
}
```

# Issue 8083: fix accents in LaTeX output

archive/issues_008083.json:
```json
{
    "body": "Accented letters produced in notebook by command like this\n\n```\n%latex\n\u011b\u0161\u010d\u0159\u017e\u00fd\u00e1\u00ed\u00e9\u010f\u010e\n```\nproduce letters composed from two objects - letter and accent - and this does not look good in some cases, especially the letter \u010f. The solution is to use correct fonts in LaTeX.\n\n**Assignee:** @williamstein\n\n**Keywords:** latex\n\n**Author:** Robert Ma\u0159\u00edk\n\n**Reviewer:** John Palmieri\n\n**Merged:** sage-4.3.2.alpha1\n\nIssue created by migration from https://trac.sagemath.org/ticket/8083\n\n",
    "closed_at": "2010-01-30T23:50:32Z",
    "created_at": "2010-01-26T20:53:14Z",
    "labels": [
        "component: notebook",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.2",
    "title": "fix accents in LaTeX output",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/8083",
    "user": "https://github.com/robert-marik"
}
```
Accented letters produced in notebook by command like this

```
%latex
ěščřžýáíéďĎ
```
produce letters composed from two objects - letter and accent - and this does not look good in some cases, especially the letter ď. The solution is to use correct fonts in LaTeX.

**Assignee:** @williamstein

**Keywords:** latex

**Author:** Robert Mařík

**Reviewer:** John Palmieri

**Merged:** sage-4.3.2.alpha1

Issue created by migration from https://trac.sagemath.org/ticket/8083





---

archive/attachments_010322.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "latex_T1_fonts.patch",
    "asset_url": "tarball://root/attachments/ticket8083/latex_T1_fonts.patch",
    "created_at": "2010-01-26T20:53:34Z",
    "issue": "https://github.com/sagemath/sage/issues/8083",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8083/latex_T1_fonts.patch",
    "user": "https://github.com/robert-marik"
}
```



---

archive/issue_comments_063848.json:
```json
{
    "body": "**Attachment:** [latex_T1_fonts.patch](https://github.com/sagemath/sage/files/ticket8083/latex_T1_fonts.patch)",
    "created_at": "2010-01-26T20:53:34Z",
    "issue": "https://github.com/sagemath/sage/issues/8083",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8083#issuecomment-63848",
    "user": "https://github.com/robert-marik"
}
```

**Attachment:** [latex_T1_fonts.patch](https://github.com/sagemath/sage/files/ticket8083/latex_T1_fonts.patch)



---

archive/issue_events_056526.json:
```json
{
    "actor": "https://github.com/robert-marik",
    "created_at": "2010-01-26T20:57:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8083",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8083#event-56526"
}
```



---

archive/issue_comments_063849.json:
```json
{
    "body": "**Reviewer:** John Palmieri",
    "created_at": "2010-01-28T22:10:43Z",
    "issue": "https://github.com/sagemath/sage/issues/8083",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8083#issuecomment-63849",
    "user": "https://github.com/jhpalmieri"
}
```

**Reviewer:** John Palmieri



---

archive/issue_comments_063850.json:
```json
{
    "body": "<a id='comment:2'></a>\nWorks as advertised in the notebook.  Note that executing\n\n```\nsage: view(s = u\"\u011b\u0161\u010d\u0159\u017e\u00fd\u00e1\u00ed\u00e9\u010f\u010e\")\n```\nfrom the command line doesn't work at all, and I can't figure out how to fix it.  I can get it not to throw an error -- see the experimental patch.  I think that the command to write the LaTeX file (in the definition of the `view` function) should be something like\n\n```\ncodecs.open(tex_file, mode='w', encoding='utf_8').write(s)\n```\nbut this garbles the string s.  Anyway, this belongs on another ticket.",
    "created_at": "2010-01-28T22:10:43Z",
    "issue": "https://github.com/sagemath/sage/issues/8083",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8083#issuecomment-63850",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'></a>
Works as advertised in the notebook.  Note that executing

```
sage: view(s = u"ěščřžýáíéďĎ")
```
from the command line doesn't work at all, and I can't figure out how to fix it.  I can get it not to throw an error -- see the experimental patch.  I think that the command to write the LaTeX file (in the definition of the `view` function) should be something like

```
codecs.open(tex_file, mode='w', encoding='utf_8').write(s)
```
but this garbles the string s.  Anyway, this belongs on another ticket.



---

archive/issue_events_056527.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-01-28T22:10:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8083",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8083#event-56527"
}
```



---

archive/issue_events_056528.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-01-28T22:10:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8083",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8083#event-56528"
}
```



---

archive/issue_comments_063851.json:
```json
{
    "body": "do not merge: for illustration and testing only",
    "created_at": "2010-01-28T22:11:07Z",
    "issue": "https://github.com/sagemath/sage/issues/8083",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8083#issuecomment-63851",
    "user": "https://github.com/jhpalmieri"
}
```

do not merge: for illustration and testing only



---

archive/attachments_010323.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8083-experimental.patch",
    "asset_url": "tarball://root/attachments/ticket8083/trac_8083-experimental.patch",
    "created_at": "2010-01-30T11:36:34Z",
    "issue": "https://github.com/sagemath/sage/issues/8083",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8083/trac_8083-experimental.patch",
    "user": "https://github.com/qed777"
}
```



---

archive/issue_comments_063852.json:
```json
{
    "body": "<a id='comment:3'></a>\n**Attachment:** [trac_8083-experimental.patch](https://github.com/sagemath/sage/files/ticket8083/trac_8083-experimental.patch)\n\nOut of curiosity: Do other font encodings ever conflict with `T1`?  Other alphabets may require a different one, e.g., for\n\n```\n%latex\n\u0422\u0435\u043e\u0440\u0438\u044f \u0447\u0438\u0441\u0435\u043b\n```\none can use\n\n```python\nsage.misc.latex.latex.extra_preamble('\\\\usepackage[T2A]{fontenc}')\n```",
    "created_at": "2010-01-30T11:36:34Z",
    "issue": "https://github.com/sagemath/sage/issues/8083",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8083#issuecomment-63852",
    "user": "https://github.com/qed777"
}
```

<a id='comment:3'></a>
**Attachment:** [trac_8083-experimental.patch](https://github.com/sagemath/sage/files/ticket8083/trac_8083-experimental.patch)

Out of curiosity: Do other font encodings ever conflict with `T1`?  Other alphabets may require a different one, e.g., for

```
%latex
Теория чисел
```
one can use

```python
sage.misc.latex.latex.extra_preamble('\\usepackage[T2A]{fontenc}')
```



---

archive/issue_comments_063853.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged [latex_T1_fonts.patch](https://github.com/sagemath/sage/files/ticket8083/latex_T1_fonts.patch).",
    "created_at": "2010-01-30T23:50:32Z",
    "issue": "https://github.com/sagemath/sage/issues/8083",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8083#issuecomment-63853",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:4'></a>
Merged [latex_T1_fonts.patch](https://github.com/sagemath/sage/files/ticket8083/latex_T1_fonts.patch).



---

archive/issue_comments_063854.json:
```json
{
    "body": "**Merged:** sage-4.3.2.alpha1",
    "created_at": "2010-01-30T23:50:32Z",
    "issue": "https://github.com/sagemath/sage/issues/8083",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8083#issuecomment-63854",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Merged:** sage-4.3.2.alpha1



---

archive/issue_events_056529.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-01-30T23:50:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8083",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8083#event-56529"
}
```



---

archive/issue_events_056530.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-01-30T23:50:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8083",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8083#event-56530"
}
```

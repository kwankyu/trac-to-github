# Issue 8043: #7544 breaks #7355: "sage -i foo" does not automatically find the version number

archive/issues_008043.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nAs detailed at [#7544 comment:4](https://github.com/sagemath/sage/issues/7544#comment:4), the functionality introduced in #7355 was broken by #7544.\n\nComponent: **misc**\n\nAuthor: **John Palmieri**\n\nReviewer: **Dan Drake**\n\nMerged: **sage-4.4.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8043_\n\n",
    "closed_at": "2010-04-23T19:10:20Z",
    "created_at": "2010-01-23T05:54:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "#7544 breaks #7355: \"sage -i foo\" does not automatically find the version number",
    "type": "issue",
    "updated_at": "2010-04-23T19:10:20Z",
    "url": "https://github.com/sagemath/sage/issues/8043",
    "user": "https://github.com/dandrake"
}
```
<div id="comment:0"></div>

As detailed at [#7544 comment:4](https://github.com/sagemath/sage/issues/7544#comment:4), the functionality introduced in #7355 was broken by #7544.

Component: **misc**

Author: **John Palmieri**

Reviewer: **Dan Drake**

Merged: **sage-4.4.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/8043_





---

archive/issue_events_099105.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-01-23T05:54:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "milestone_number": null,
    "milestone_title": "sage-4.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8043#event-99105"
}
```



---

archive/issue_events_099106.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-01-23T05:54:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8043#event-99106"
}
```



---

archive/issue_events_099107.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-01-23T05:54:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8043#event-99107"
}
```



---

archive/issue_events_099108.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-10T18:58:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8043#event-99108"
}
```



---

archive/issue_comments_061636.json:
```json
{
    "body": "Author: **John Palmieri**",
    "created_at": "2010-04-10T18:58:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8043#issuecomment-61636",
    "user": "https://github.com/jhpalmieri"
}
```

Author: **John Palmieri**



---

archive/issue_comments_061637.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nHere's a patch.  See #8669 for a somewhat related ticket.",
    "created_at": "2010-04-10T18:58:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8043#issuecomment-61637",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:1" align="right">comment:1</div>

Here's a patch.  See #8669 for a somewhat related ticket.



---

archive/issue_comments_061638.json:
```json
{
    "body": "Attachment: **[trac_8043-spkg-no-version.patch.gz](https://github.com/sagemath/sage/files/ticket8043/trac_8043-spkg-no-version.patch.gz)**\n\nscripts repo",
    "created_at": "2010-04-10T18:58:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8043#issuecomment-61638",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment: **[trac_8043-spkg-no-version.patch.gz](https://github.com/sagemath/sage/files/ticket8043/trac_8043-spkg-no-version.patch.gz)**

scripts repo



---

archive/issue_events_099109.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-04-12T06:28:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8043#event-99109"
}
```



---

archive/issue_events_099110.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-04-12T06:28:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8043#event-99110"
}
```



---

archive/issue_comments_061639.json:
```json
{
    "body": "Reviewer: **Dan Drake**",
    "created_at": "2010-04-12T06:28:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8043#issuecomment-61639",
    "user": "https://github.com/dandrake"
}
```

Reviewer: **Dan Drake**



---

archive/issue_comments_061640.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis fixes the problem, and does so in a simple, POSIX-compliant way. Positive review.",
    "created_at": "2010-04-12T06:28:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8043#issuecomment-61640",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:2" align="right">comment:2</div>

This fixes the problem, and does so in a simple, POSIX-compliant way. Positive review.



---

archive/issue_comments_061641.json:
```json
{
    "body": "Merged: **sage-4.4.alpha2**",
    "created_at": "2010-04-23T19:10:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8043#issuecomment-61641",
    "user": "https://github.com/jhpalmieri"
}
```

Merged: **sage-4.4.alpha2**



---

archive/issue_comments_061642.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in 4.4.alpha2.",
    "created_at": "2010-04-23T19:10:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8043#issuecomment-61642",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in 4.4.alpha2.



---

archive/issue_events_099111.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-23T19:10:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8043#event-99111"
}
```



---

archive/issue_events_099112.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-23T19:10:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8043#event-99112"
}
```

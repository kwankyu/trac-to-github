# Issue 8277: Using matrix() to convert between sparse and dense.

archive/issues_008277.json:
```json
{
    "body": "A call to `matrix` does not change the sparsity:\n\n```\n    sage: mat = matrix(ZZ, [[1,1],[1,1]], sparse=False)\n    sage: type(mat)\n    <type 'sage.matrix.matrix_integer_dense.Matrix_integer_dense'>\n    sage: mat2 = matrix(ZZ, mat, sparse=True)\n    sage: type(mat2)\n    <type 'sage.matrix.matrix_integer_dense.Matrix_integer_dense'>\n```\n\n**Assignee:** @hivert\n\n**Keywords:** sparse matrix conversion\n\nIssue created by migration from https://trac.sagemath.org/ticket/8277\n\n",
    "closed_at": "2018-03-01T15:00:46Z",
    "created_at": "2010-02-15T22:01:08Z",
    "labels": [
        "component: linear algebra",
        "bug",
        "duplicate"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "title": "Using matrix() to convert between sparse and dense.",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/8277",
    "user": "https://github.com/hivert"
}
```
A call to `matrix` does not change the sparsity:

```
    sage: mat = matrix(ZZ, [[1,1],[1,1]], sparse=False)
    sage: type(mat)
    <type 'sage.matrix.matrix_integer_dense.Matrix_integer_dense'>
    sage: mat2 = matrix(ZZ, mat, sparse=True)
    sage: type(mat2)
    <type 'sage.matrix.matrix_integer_dense.Matrix_integer_dense'>
```

**Assignee:** @hivert

**Keywords:** sparse matrix conversion

Issue created by migration from https://trac.sagemath.org/ticket/8277





---

archive/issue_events_058218.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-09-02T10:50:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8277",
    "label": "component: algebra",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8277#event-58218"
}
```



---

archive/issue_events_058219.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-09-02T10:50:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8277",
    "label": "component: linear algebra",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8277#event-58219"
}
```



---

archive/issue_events_058220.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8277",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8277#event-58220"
}
```



---

archive/issue_events_058221.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8277",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8277#event-58221"
}
```



---

archive/issue_events_058222.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8277",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8277#event-58222"
}
```



---

archive/issue_events_058223.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8277",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8277#event-58223"
}
```



---

archive/issue_events_058224.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8277",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8277#event-58224"
}
```



---

archive/issue_events_058225.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8277",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8277#event-58225"
}
```



---

archive/issue_events_058226.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8277",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8277#event-58226"
}
```



---

archive/issue_events_058227.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8277",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8277#event-58227"
}
```



---

archive/issue_events_058228.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-01T15:00:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8277",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8277#event-58228"
}
```



---

archive/issue_comments_066172.json:
```json
{
    "body": "**Changing author** from \"Florent Hivert\" to \"\".",
    "created_at": "2018-03-01T15:00:46Z",
    "issue": "https://github.com/sagemath/sage/issues/8277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8277#issuecomment-66172",
    "user": "https://github.com/jdemeyer"
}
```

**Changing author** from "Florent Hivert" to "".



---

archive/issue_events_058229.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-01T15:00:46Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8277",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8277#event-58229"
}
```



---

archive/issue_events_058230.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-01T15:00:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8277",
    "milestone": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8277#event-58230"
}
```



---

archive/issue_events_058231.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-03-01T15:00:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8277",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8277#event-58231"
}
```



---

archive/issue_comments_066173.json:
```json
{
    "body": "<a id='comment:6'></a>\nFixed in #24742.",
    "created_at": "2018-03-01T15:00:46Z",
    "issue": "https://github.com/sagemath/sage/issues/8277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8277#issuecomment-66173",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'></a>
Fixed in #24742.

# Issue 8748: Linear Arboricity, Acyclic edge coloring

archive/issues_008748.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout",
        "https://github.com/nathanncohen",
        "https://github.com/rlmill"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis patch implements LP formulations of Linear Arboricity and Acyclic edge coloring\n\nNathann Thank you.I got it.\n\nThis ticket is the same as #8405. The latter got spam content and the spammer closed the ticket. It would be more trouble and result in confusion to reopen the ticket. So I have moved the ticket over to this one.\n\nComponent: **graph theory**\n\nAuthor: **Nathann Cohen**\n\nReviewer: **Robert Miller**\n\nMerged: **sage-4.5.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8748_\n\n",
    "closed_at": "2010-06-29T16:44:16Z",
    "created_at": "2010-04-23T01:04:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Linear Arboricity, Acyclic edge coloring",
    "type": "issue",
    "updated_at": "2010-06-29T16:44:16Z",
    "url": "https://github.com/sagemath/sage/issues/8748",
    "user": "https://github.com/nathanncohen"
}
```
<div id="comment:0"></div>

This patch implements LP formulations of Linear Arboricity and Acyclic edge coloring

Nathann Thank you.I got it.

This ticket is the same as #8405. The latter got spam content and the spammer closed the ticket. It would be more trouble and result in confusion to reopen the ticket. So I have moved the ticket over to this one.

Component: **graph theory**

Author: **Nathann Cohen**

Reviewer: **Robert Miller**

Merged: **sage-4.5.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/8748_





---

archive/issue_events_109574.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-23T01:04:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "milestone_number": null,
    "milestone_title": "sage-4.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8748#event-109574"
}
```



---

archive/issue_events_109575.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-23T01:04:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8748#event-109575"
}
```



---

archive/issue_events_109576.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-23T01:04:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8748#event-109576"
}
```



---

archive/issue_events_109577.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-23T01:04:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8748#event-109577"
}
```



---

archive/issue_events_109578.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-04-23T01:04:32Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "subject": "https://github.com/nathanncohen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8748#event-109578"
}
```



---

archive/issue_events_109579.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-23T01:04:32Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "subject": "https://github.com/nathanncohen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8748#event-109579"
}
```



---

archive/issue_events_109580.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-04-23T01:04:32Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "subject": "https://github.com/nathanncohen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8748#event-109580"
}
```



---

archive/issue_events_109581.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-04-23T01:06:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8748#event-109581"
}
```



---

archive/issue_comments_071025.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nFor an explanation of the Linear Program used to solve this problem, see the LP chapter from :  http://code.google.com/p/graph-theory-algorithms-book/\n\nNathann",
    "created_at": "2010-04-23T01:06:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8748#issuecomment-71025",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:1" align="right">comment:1</div>

For an explanation of the Linear Program used to solve this problem, see the LP chapter from :  http://code.google.com/p/graph-theory-algorithms-book/

Nathann



---

archive/issue_events_109582.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-06-21T21:00:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8748#event-109582"
}
```



---

archive/issue_events_109583.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-06-21T21:00:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8748#event-109583"
}
```



---

archive/issue_comments_071026.json:
```json
{
    "body": "Reviewer: **Robert Miller**",
    "created_at": "2010-06-21T21:00:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8748#issuecomment-71026",
    "user": "https://github.com/rlmill"
}
```

Reviewer: **Robert Miller**



---

archive/issue_comments_071027.json:
```json
{
    "body": "Author: **Nathann Cohen**",
    "created_at": "2010-06-21T21:00:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8748#issuecomment-71027",
    "user": "https://github.com/rlmill"
}
```

Author: **Nathann Cohen**



---

archive/issue_comments_071028.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nFailures:\n\n```\nsage -t -only-optional=glpk,cbc \"devel/sage-main/sage/graphs/graph_coloring.py\"\n**********************************************************************\nFile \"/Users/rlmill/sage-4.4.4.alpha0-cbc/devel/sage-main/sage/graphs/graph_coloring.py\", line 749:\n    sage: all([g1.has_edge(e) or g2.has_edge(e) for e in g.edges()])  # optional - GLPK, CBC\nExpected:\n    True\nGot:\n    False\n**********************************************************************\nFile \"/Users/rlmill/sage-4.4.4.alpha0-cbc/devel/sage-main/sage/graphs/graph_coloring.py\", line 922:\n    sage: all([ any([gg.has_edge(e) for gg in colors]) for e in g.edges()])     # optional - GLPK, CBC\nExpected:\n    True\nGot:\n    False\n**********************************************************************\n```",
    "created_at": "2010-06-21T21:00:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8748#issuecomment-71028",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:2" align="right">comment:2</div>

Failures:

```
sage -t -only-optional=glpk,cbc "devel/sage-main/sage/graphs/graph_coloring.py"
**********************************************************************
File "/Users/rlmill/sage-4.4.4.alpha0-cbc/devel/sage-main/sage/graphs/graph_coloring.py", line 749:
    sage: all([g1.has_edge(e) or g2.has_edge(e) for e in g.edges()])  # optional - GLPK, CBC
Expected:
    True
Got:
    False
**********************************************************************
File "/Users/rlmill/sage-4.4.4.alpha0-cbc/devel/sage-main/sage/graphs/graph_coloring.py", line 922:
    sage: all([ any([gg.has_edge(e) for gg in colors]) for e in g.edges()])     # optional - GLPK, CBC
Expected:
    True
Got:
    False
**********************************************************************
```



---

archive/issue_comments_071029.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nYet another graph constructor from networkx, with {} instead of None as labels. A g.edges(labels = False) did the trick :-)\n\nSorry abou that !\n\nNathann",
    "created_at": "2010-06-21T21:14:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8748#issuecomment-71029",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:3" align="right">comment:3</div>

Yet another graph constructor from networkx, with {} instead of None as labels. A g.edges(labels = False) did the trick :-)

Sorry abou that !

Nathann



---

archive/issue_events_109584.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-06-21T21:14:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8748#event-109584"
}
```



---

archive/issue_events_109585.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-06-21T21:14:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8748#event-109585"
}
```



---

archive/issue_events_109586.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-06-21T21:57:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8748#event-109586"
}
```



---

archive/issue_events_109587.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-06-21T21:57:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8748#event-109587"
}
```



---

archive/issue_comments_071030.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nAttachment: **[trac_8748.2.patch.gz](https://github.com/sagemath/sage/files/ticket8748/trac_8748.2.patch.gz)**",
    "created_at": "2010-06-21T21:57:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8748#issuecomment-71030",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:4" align="right">comment:4</div>

Attachment: **[trac_8748.2.patch.gz](https://github.com/sagemath/sage/files/ticket8748/trac_8748.2.patch.gz)**



---

archive/issue_events_109588.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-06-29T16:44:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8748#event-109588"
}
```



---

archive/issue_events_109589.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-06-29T16:44:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8748#event-109589"
}
```



---

archive/issue_comments_071031.json:
```json
{
    "body": "Merged: **sage-4.5.alpha1**",
    "created_at": "2010-06-29T16:44:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8748#issuecomment-71031",
    "user": "https://github.com/rlmill"
}
```

Merged: **sage-4.5.alpha1**

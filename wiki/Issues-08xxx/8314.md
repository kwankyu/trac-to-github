# Issue 8314: numerical noise in sage/misc/functional.py

archive/issues_008314.json:
```json
{
    "body": "From [sage-devel](http://groups.google.com/group/sage-devel/msg/ba90601a2c25291d):\n\n```\nOn 32-bit Suse I get this fuzz:\n\nFile \"/local/jec/sage-4.3.3.alpha1/devel/sage/sage/misc/functional.py\",\nline 705:\n    sage: h.n()\nExpected:\n    0.33944794097891573\nGot:\n    0.33944794097891567 \n```\n\n**Author:** Minh Van Nguyen\n\n**Reviewer:** John Cremona\n\n**Merged:** sage-4.3.3.final\n\nIssue created by migration from https://trac.sagemath.org/ticket/8314\n\n",
    "closed_at": "2010-02-22T03:57:49Z",
    "created_at": "2010-02-20T16:26:06Z",
    "labels": [
        "component: doctest coverage",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.3",
    "title": "numerical noise in sage/misc/functional.py",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/8314",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```
From [sage-devel](http://groups.google.com/group/sage-devel/msg/ba90601a2c25291d):

```
On 32-bit Suse I get this fuzz:

File "/local/jec/sage-4.3.3.alpha1/devel/sage/sage/misc/functional.py",
line 705:
    sage: h.n()
Expected:
    0.33944794097891573
Got:
    0.33944794097891567 
```

**Author:** Minh Van Nguyen

**Reviewer:** John Cremona

**Merged:** sage-4.3.3.final

Issue created by migration from https://trac.sagemath.org/ticket/8314





---

archive/issue_comments_066587.json:
```json
{
    "body": "based on Sage 4.3.3.alpha1",
    "created_at": "2010-02-20T16:28:08Z",
    "issue": "https://github.com/sagemath/sage/issues/8314",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8314#issuecomment-66587",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

based on Sage 4.3.3.alpha1



---

archive/issue_events_058530.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-02-20T16:28:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8314",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8314#event-58530"
}
```



---

archive/attachments_010668.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8314-doctest.patch",
    "asset_url": "tarball://root/attachments/ticket8314/trac_8314-doctest.patch",
    "created_at": "2010-02-20T16:28:28Z",
    "issue": "https://github.com/sagemath/sage/issues/8314",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8314/trac_8314-doctest.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```



---

archive/issue_comments_066588.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_8314-doctest.patch](https://github.com/sagemath/sage/files/ticket8314/trac_8314-doctest.patch)",
    "created_at": "2010-02-20T16:28:28Z",
    "issue": "https://github.com/sagemath/sage/issues/8314",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8314#issuecomment-66588",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_8314-doctest.patch](https://github.com/sagemath/sage/files/ticket8314/trac_8314-doctest.patch)



---

archive/issue_comments_066589.json:
```json
{
    "body": "<a id='comment:2'></a>\npositive review -- the patch replaces two digits by ... so the tests pass.  It would have been nice to know why those digits are platform-dependent though...",
    "created_at": "2010-02-20T17:15:27Z",
    "issue": "https://github.com/sagemath/sage/issues/8314",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8314#issuecomment-66589",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:2'></a>
positive review -- the patch replaces two digits by ... so the tests pass.  It would have been nice to know why those digits are platform-dependent though...



---

archive/issue_events_058531.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-02-20T17:15:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8314",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8314#event-58531"
}
```



---

archive/issue_events_058532.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-02-20T17:15:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8314",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8314#event-58532"
}
```



---

archive/issue_events_058533.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-02-21T19:46:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8314",
    "label": "blocker",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8314#event-58533"
}
```



---

archive/issue_comments_066590.json:
```json
{
    "body": "**Merged:** sage-4.3.3.final",
    "created_at": "2010-02-22T03:57:49Z",
    "issue": "https://github.com/sagemath/sage/issues/8314",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8314#issuecomment-66590",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Merged:** sage-4.3.3.final



---

archive/issue_events_058534.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-02-22T03:57:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8314",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8314#event-58534"
}
```



---

archive/issue_events_058535.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-02-22T03:57:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8314",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8314#event-58535"
}
```



---

archive/issue_comments_066591.json:
```json
{
    "body": "**Reviewer:** John Cremona",
    "created_at": "2010-02-22T03:57:49Z",
    "issue": "https://github.com/sagemath/sage/issues/8314",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8314#issuecomment-66591",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Reviewer:** John Cremona



---

archive/issue_comments_066592.json:
```json
{
    "body": "**Author:** Minh Van Nguyen",
    "created_at": "2010-02-22T03:57:49Z",
    "issue": "https://github.com/sagemath/sage/issues/8314",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8314#issuecomment-66592",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Author:** Minh Van Nguyen

# Issue 8510: conjugacy_class(es)_representatives

archive/issues_008510.json:
```json
{
    "assignees": [
        "https://github.com/aghitza"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe group SL(2,3) has a method conjugacy_class_representatives()\nwhile the group DiCyclicGroup(3) has a method\nconjugacy_classes_representatives(). These names are similar but not\nidentical. Presumably one should be changed.\n\nSee: \n\nhttp://groups.google.com/group/sage-devel/browse_thread/thread/e65176f82a47e06d\n\nComponent: **algebra**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8510_\n\n",
    "closed_at": "2014-02-11T21:23:28Z",
    "created_at": "2010-03-12T19:05:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "conjugacy_class(es)_representatives",
    "type": "issue",
    "updated_at": "2014-02-11T21:23:28Z",
    "url": "https://github.com/sagemath/sage/issues/8510",
    "user": "https://github.com/dwbump"
}
```
<div id="comment:0"></div>

The group SL(2,3) has a method conjugacy_class_representatives()
while the group DiCyclicGroup(3) has a method
conjugacy_classes_representatives(). These names are similar but not
identical. Presumably one should be changed.

See: 

http://groups.google.com/group/sage-devel/browse_thread/thread/e65176f82a47e06d

Component: **algebra**

_Issue created by migration from https://trac.sagemath.org/ticket/8510_





---

archive/issue_events_105955.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-03-12T19:05:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8510",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8510#event-105955"
}
```



---

archive/issue_events_105956.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-03-12T19:05:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8510",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8510#event-105956"
}
```



---

archive/issue_events_105957.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-03-12T19:05:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8510",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8510#event-105957"
}
```



---

archive/issue_events_105958.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-03-12T19:05:37Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8510",
    "subject": "https://github.com/dwbump",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8510#event-105958"
}
```



---

archive/issue_comments_067842.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\n`SL(2,3)` now has both.",
    "created_at": "2014-02-02T11:29:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8510",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8510#issuecomment-67842",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:1" align="right">comment:1</div>

`SL(2,3)` now has both.



---

archive/issue_events_105959.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2014-02-02T11:29:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8510",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8510#event-105959"
}
```



---

archive/issue_events_105960.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-02-09T21:18:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8510",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8510#event-105960"
}
```



---

archive/issue_events_105961.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-02-09T21:18:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8510",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8510#event-105961"
}
```



---

archive/issue_events_105962.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-11T21:23:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8510",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8510#event-105962"
}
```



---

archive/issue_events_105963.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-11T21:23:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8510",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8510#event-105963"
}
```

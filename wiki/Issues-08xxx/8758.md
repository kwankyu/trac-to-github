# Issue 8758: Add a top-level /data with static.File(DATA) that serves files from the data directory

archive/issues_008758.json:
```json
{
    "body": "Instead of having /js, /css, etc, we should serve all static data files out of /data.  For now this patch just adds /data using `child_data = static.File(DATA)`, where `static.File(path)` is defined in twisted.  It will serve files with the correct MIME type based on extension.\n\nEventually we should find all the /js and /css paths and change them to /data\n\nAt some point in the future we could even implement caching of all the static files easily by subclassing static.File(path), and making everything in /data cached.\n\n**Assignee:** acleone\n\n**CC:**  acleone @williamstein @TimDumol\n\n**Author:** Alex Leone\n\n**Reviewer:** William Stein\n\n**Merged:** SageNB 0.8.1\n\nIssue created by migration from https://trac.sagemath.org/ticket/8758\n\n",
    "closed_at": "2010-07-11T05:58:13Z",
    "created_at": "2010-04-24T22:58:06Z",
    "labels": [
        "component: notebook",
        "trivial",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.5",
    "title": "Add a top-level /data with static.File(DATA) that serves files from the data directory",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/8758",
    "user": "https://trac.sagemath.org/admin/accounts/users/acleone"
}
```
Instead of having /js, /css, etc, we should serve all static data files out of /data.  For now this patch just adds /data using `child_data = static.File(DATA)`, where `static.File(path)` is defined in twisted.  It will serve files with the correct MIME type based on extension.

Eventually we should find all the /js and /css paths and change them to /data

At some point in the future we could even implement caching of all the static files easily by subclassing static.File(path), and making everything in /data cached.

**Assignee:** acleone

**CC:**  acleone @williamstein @TimDumol

**Author:** Alex Leone

**Reviewer:** William Stein

**Merged:** SageNB 0.8.1

Issue created by migration from https://trac.sagemath.org/ticket/8758





---

archive/issue_comments_072920.json:
```json
{
    "body": "Adds the two static.File(DATA) calls to twist.py",
    "created_at": "2010-04-24T23:01:42Z",
    "issue": "https://github.com/sagemath/sage/issues/8758",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8758#issuecomment-72920",
    "user": "https://trac.sagemath.org/admin/accounts/users/acleone"
}
```

Adds the two static.File(DATA) calls to twist.py



---

archive/attachments_011367.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8758-data-toplevel-with-static-file.patch",
    "asset_url": "tarball://root/attachments/ticket8758/trac_8758-data-toplevel-with-static-file.patch",
    "created_at": "2010-04-24T23:02:01Z",
    "issue": "https://github.com/sagemath/sage/issues/8758",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8758/trac_8758-data-toplevel-with-static-file.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/acleone"
}
```



---

archive/issue_comments_072921.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_8758-data-toplevel-with-static-file.patch](https://github.com/sagemath/sage/files/ticket8758/trac_8758-data-toplevel-with-static-file.patch)",
    "created_at": "2010-04-24T23:02:01Z",
    "issue": "https://github.com/sagemath/sage/issues/8758",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8758#issuecomment-72921",
    "user": "https://trac.sagemath.org/admin/accounts/users/acleone"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_8758-data-toplevel-with-static-file.patch](https://github.com/sagemath/sage/files/ticket8758/trac_8758-data-toplevel-with-static-file.patch)



---

archive/issue_events_063080.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/acleone",
    "created_at": "2010-04-24T23:02:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8758",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8758#event-63080"
}
```



---

archive/issue_events_063081.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-04-25T00:28:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8758",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8758#event-63081"
}
```



---

archive/issue_events_063082.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-04-25T00:28:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8758",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8758#event-63082"
}
```



---

archive/issue_comments_072922.json:
```json
{
    "body": "<a id='comment:2'></a>\nNot a patch bomb.",
    "created_at": "2010-04-25T00:28:08Z",
    "issue": "https://github.com/sagemath/sage/issues/8758",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8758#issuecomment-72922",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>
Not a patch bomb.



---

archive/issue_comments_072923.json:
```json
{
    "body": "Changes the url to /static/.  Replaces previous.",
    "created_at": "2010-05-29T22:38:07Z",
    "issue": "https://github.com/sagemath/sage/issues/8758",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8758#issuecomment-72923",
    "user": "https://trac.sagemath.org/admin/accounts/users/acleone"
}
```

Changes the url to /static/.  Replaces previous.



---

archive/attachments_011368.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8758-toplevel-static-url.patch",
    "asset_url": "tarball://root/attachments/ticket8758/trac_8758-toplevel-static-url.patch",
    "created_at": "2010-05-29T22:43:37Z",
    "issue": "https://github.com/sagemath/sage/issues/8758",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8758/trac_8758-toplevel-static-url.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/acleone"
}
```



---

archive/issue_comments_072924.json:
```json
{
    "body": "<a id='comment:3'></a>\n**Attachment:** [trac_8758-toplevel-static-url.patch](https://github.com/sagemath/sage/files/ticket8758/trac_8758-toplevel-static-url.patch)\n\nThe latest patch changes the url to /static/.\n\nI think we should also rename the \"data\" directory in the sagenb source to \"static\", and rename the global DATA to STATIC.\n\nDATA is slightly confusing because uploading files to a worksheet also uses a DATA global, eg DATA+'mydatafile.txt'.",
    "created_at": "2010-05-29T22:43:37Z",
    "issue": "https://github.com/sagemath/sage/issues/8758",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8758#issuecomment-72924",
    "user": "https://trac.sagemath.org/admin/accounts/users/acleone"
}
```

<a id='comment:3'></a>
**Attachment:** [trac_8758-toplevel-static-url.patch](https://github.com/sagemath/sage/files/ticket8758/trac_8758-toplevel-static-url.patch)

The latest patch changes the url to /static/.

I think we should also rename the "data" directory in the sagenb source to "static", and rename the global DATA to STATIC.

DATA is slightly confusing because uploading files to a worksheet also uses a DATA global, eg DATA+'mydatafile.txt'.



---

archive/issue_comments_072925.json:
```json
{
    "body": "<a id='comment:4'></a>\nOk - ignore the new patch.  Let's leave the url as /data.",
    "created_at": "2010-06-02T23:14:25Z",
    "issue": "https://github.com/sagemath/sage/issues/8758",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8758#issuecomment-72925",
    "user": "https://trac.sagemath.org/admin/accounts/users/acleone"
}
```

<a id='comment:4'></a>
Ok - ignore the new patch.  Let's leave the url as /data.



---

archive/issue_comments_072926.json:
```json
{
    "body": "**Reviewer:** William Stein",
    "created_at": "2010-06-29T08:01:36Z",
    "issue": "https://github.com/sagemath/sage/issues/8758",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8758#issuecomment-72926",
    "user": "https://github.com/loefflerd"
}
```

**Reviewer:** William Stein



---

archive/issue_events_063083.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-06-29T08:01:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8758",
    "milestone": "sage-4.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8758#event-63083"
}
```



---

archive/issue_events_063084.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-07-11T05:58:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8758",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8758#event-63084"
}
```



---

archive/issue_events_063085.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-07-11T05:58:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8758",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8758#event-63085"
}
```



---

archive/issue_comments_072927.json:
```json
{
    "body": "**Merged:** SageNB 0.8.1",
    "created_at": "2010-07-11T05:58:13Z",
    "issue": "https://github.com/sagemath/sage/issues/8758",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8758#issuecomment-72927",
    "user": "https://github.com/TimDumol"
}
```

**Merged:** SageNB 0.8.1

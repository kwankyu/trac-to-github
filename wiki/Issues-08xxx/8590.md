# Issue 8590: `A` still using old coercion framework

archive/issues_008590.json:
```json
{
    "assignees": [
        "https://github.com/aghitza"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThere is a failure in the test of the file\n\nsage.categories.hopf_algebras_with_basis\n\nThe problem occurs during the test suite of an hopf algebra and returns the error\n\n```\n   RuntimeError: `A` still using old coercion framework\n```\n\nI am unfortunately not able to solve really this problem, but, if the competent people have no time right now, I can write a small patch predicting the error in the test so that the tests pass\n\nComponent: **algebra**\n\nReviewer: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8590_\n\n",
    "closed_at": "2014-02-02T20:43:01Z",
    "created_at": "2010-03-23T17:40:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "`A` still using old coercion framework",
    "type": "issue",
    "updated_at": "2014-02-02T20:43:01Z",
    "url": "https://github.com/sagemath/sage/issues/8590",
    "user": "https://github.com/sagetrac-vferay"
}
```
<div id="comment:0"></div>

There is a failure in the test of the file

sage.categories.hopf_algebras_with_basis

The problem occurs during the test suite of an hopf algebra and returns the error

```
   RuntimeError: `A` still using old coercion framework
```

I am unfortunately not able to solve really this problem, but, if the competent people have no time right now, I can write a small patch predicting the error in the test so that the tests pass

Component: **algebra**

Reviewer: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/8590_





---

archive/issue_events_107186.json:
```json
{
    "actor": "https://github.com/sagetrac-vferay",
    "created_at": "2010-03-23T17:40:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8590",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8590#event-107186"
}
```



---

archive/issue_events_107187.json:
```json
{
    "actor": "https://github.com/sagetrac-vferay",
    "created_at": "2010-03-23T17:40:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8590",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8590#event-107187"
}
```



---

archive/issue_events_107188.json:
```json
{
    "actor": "https://github.com/sagetrac-vferay",
    "created_at": "2010-03-23T17:40:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8590",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8590#event-107188"
}
```



---

archive/issue_events_107189.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-03-23T17:40:36Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8590",
    "subject": "https://github.com/sagetrac-vferay",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8590#event-107189"
}
```



---

archive/issue_events_107190.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2014-02-02T11:20:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8590",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8590#event-107190"
}
```



---

archive/issue_events_107191.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-02-02T15:55:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8590",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8590#event-107191"
}
```



---

archive/issue_events_107192.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-02-02T15:55:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8590",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8590#event-107192"
}
```



---

archive/issue_comments_068738.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2014-02-02T15:55:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8590#issuecomment-68738",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_107193.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-02T20:43:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8590",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8590#event-107193"
}
```



---

archive/issue_events_107194.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-02T20:43:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8590",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8590#event-107194"
}
```

# Issue 8243: Annoying warnings when building the HTML reference manual

archive/issues_008243.json:
```json
{
    "body": "Let's fix or suppress these, so that it's easier to identify new problems.\n\nSee the attachment.\n\n**Assignee:** mvngu\n\n**CC:**  @jhpalmieri mvngu\n\nIssue created by migration from https://trac.sagemath.org/ticket/8243\n\n",
    "closed_at": "2010-02-12T03:44:44Z",
    "created_at": "2010-02-11T21:43:21Z",
    "labels": [
        "component: documentation",
        "bug",
        "duplicate"
    ],
    "title": "Annoying warnings when building the HTML reference manual",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/8243",
    "user": "https://github.com/qed777"
}
```
Let's fix or suppress these, so that it's easier to identify new problems.

See the attachment.

**Assignee:** mvngu

**CC:**  @jhpalmieri mvngu

Issue created by migration from https://trac.sagemath.org/ticket/8243





---

archive/issue_comments_065788.json:
```json
{
    "body": "<a id='comment:1'></a>\nIs this a duplicate of #8244?",
    "created_at": "2010-02-11T22:41:17Z",
    "issue": "https://github.com/sagemath/sage/issues/8243",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8243#issuecomment-65788",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:1'></a>
Is this a duplicate of #8244?



---

archive/issue_events_057957.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-02-12T03:44:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8243",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8243#event-57957"
}
```



---

archive/issue_events_057958.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-02-12T03:44:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8243",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8243#event-57958"
}
```



---

archive/issue_comments_065789.json:
```json
{
    "body": "<a id='comment:2'></a>\nClosing this ticket as a duplicate of #8244.",
    "created_at": "2010-02-12T03:44:44Z",
    "issue": "https://github.com/sagemath/sage/issues/8243",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8243#issuecomment-65789",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:2'></a>
Closing this ticket as a duplicate of #8244.



---

archive/issue_events_057959.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-02-12T03:44:44Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8243",
    "milestone": "sage-4.3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8243#event-57959"
}
```

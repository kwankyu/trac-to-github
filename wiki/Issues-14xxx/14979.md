# Issue 14979: GUAVA failures for linear codes

archive/issues_014742.json:
```json
{
    "body": "Working on #14909, we discovered some failing doctests for the GAP package GUAVA.  For example:\n\n```\nsage: F.<a> = GF(4)\nsage: C = HammingCode(2,F)\nsage: v = vector(F, [1,0,0,a,1])\nsage: C.decode(v)\n(a + 1, 0, 0, a, 1)\nsage: C.decode(v, algorithm=\"nearest neighbor\")\n(a + 1, 0, 0, a, 1)\nsage: C.decode(v, algorithm=\"guava\")  # optional - gap_packages (Guava package)\nBOOM!\n```\n\nThere are other failures for `permutation_automorphism_group` too.\n\n**CC:**  @wdjoyner\n\n**Reviewer:** Jeroen Demeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/14979\n\n",
    "closed_at": "2015-04-02T22:27:07Z",
    "created_at": "2013-07-26T12:16:46Z",
    "labels": [
        "component: coding theory",
        "bug",
        "duplicate"
    ],
    "title": "GUAVA failures for linear codes",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/14979",
    "user": "https://github.com/nilesjohnson"
}
```
Working on #14909, we discovered some failing doctests for the GAP package GUAVA.  For example:

```
sage: F.<a> = GF(4)
sage: C = HammingCode(2,F)
sage: v = vector(F, [1,0,0,a,1])
sage: C.decode(v)
(a + 1, 0, 0, a, 1)
sage: C.decode(v, algorithm="nearest neighbor")
(a + 1, 0, 0, a, 1)
sage: C.decode(v, algorithm="guava")  # optional - gap_packages (Guava package)
BOOM!
```

There are other failures for `permutation_automorphism_group` too.

**CC:**  @wdjoyner

**Reviewer:** Jeroen Demeyer

Issue created by migration from https://trac.sagemath.org/ticket/14979





---

archive/issue_events_131986.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14979",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14979#event-131986"
}
```



---

archive/issue_events_131987.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14979",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14979#event-131987"
}
```



---

archive/issue_events_131988.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14979",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14979#event-131988"
}
```



---

archive/issue_events_131989.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14979",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14979#event-131989"
}
```



---

archive/issue_comments_186042.json:
```json
{
    "body": "<a id='comment:3'></a>\nsigning in to keep track of this ticket.",
    "created_at": "2014-06-07T15:56:23Z",
    "issue": "https://github.com/sagemath/sage/issues/14979",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14979#issuecomment-186042",
    "user": "https://github.com/ppurka"
}
```

<a id='comment:3'></a>
signing in to keep track of this ticket.



---

archive/issue_events_131990.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14979",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14979#event-131990"
}
```



---

archive/issue_events_131991.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14979",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14979#event-131991"
}
```



---

archive/issue_comments_186043.json:
```json
{
    "body": "<a id='comment:5'></a>\nCould you please tell us what is \"BOOM\" ?  I got two different BOOMs, depending on the installation of guava. Daniel",
    "created_at": "2015-04-02T14:33:39Z",
    "issue": "https://github.com/sagemath/sage/issues/14979",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14979#issuecomment-186043",
    "user": "https://trac.sagemath.org/admin/accounts/users/danielaugot"
}
```

<a id='comment:5'></a>
Could you please tell us what is "BOOM" ?  I got two different BOOMs, depending on the installation of guava. Daniel



---

archive/issue_events_131992.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-02T14:53:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14979",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14979#event-131992"
}
```



---

archive/issue_comments_186044.json:
```json
{
    "body": "<a id='comment:6'></a>\nDuplicate of #16660.",
    "created_at": "2015-04-02T14:53:15Z",
    "issue": "https://github.com/sagemath/sage/issues/14979",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14979#issuecomment-186044",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'></a>
Duplicate of #16660.



---

archive/issue_events_131993.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-02T14:53:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14979",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14979#event-131993"
}
```



---

archive/issue_comments_186045.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2015-04-02T14:53:15Z",
    "issue": "https://github.com/sagemath/sage/issues/14979",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14979#issuecomment-186045",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_events_131994.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-02T14:53:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14979",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14979#event-131994"
}
```



---

archive/issue_events_131995.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-02T14:53:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14979",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14979#event-131995"
}
```



---

archive/issue_events_131996.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-02T22:27:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14979",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14979#event-131996"
}
```



---

archive/issue_events_131997.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-02T22:27:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14979",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14979#event-131997"
}
```



---

archive/issue_events_131998.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-02T22:27:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14979",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14979#event-131998"
}
```

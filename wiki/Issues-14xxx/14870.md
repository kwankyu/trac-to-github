# Issue 14870: Failure with Python int modulo a rational

archive/issues_014666.json:
```json
{
    "body": "Some fun I came across:\n\n```\nsage: int(5) % QQ(2)\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n<ipython-input-4-8ec0f9994ded> in <module>()\n----> 1 int(Integer(5)) % QQ(Integer(2))\n\nTypeError: Argument 'self' has incorrect type (expected sage.rings.rational.Rational, got int)\n```\n\nNote that this works:\n\n```\nsage: 5 % QQ(2)\n1\n```\n\n**Assignee:** @tscrim\n\n**CC:**  @vbraun @roed314\n\n**Reviewer:** Beth Malmskog\n\n**Author:** Travis Scrimshaw\n\n**Merged:** sage-5.12.beta0\n\nIssue created by migration from https://trac.sagemath.org/ticket/14870\n\n",
    "closed_at": "2013-08-02T14:18:15Z",
    "created_at": "2013-07-09T10:21:07Z",
    "labels": [
        "component: basic arithmetic",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.12",
    "title": "Failure with Python int modulo a rational",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/14870",
    "user": "https://github.com/tscrim"
}
```
Some fun I came across:

```
sage: int(5) % QQ(2)
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
<ipython-input-4-8ec0f9994ded> in <module>()
----> 1 int(Integer(5)) % QQ(Integer(2))

TypeError: Argument 'self' has incorrect type (expected sage.rings.rational.Rational, got int)
```

Note that this works:

```
sage: 5 % QQ(2)
1
```

**Assignee:** @tscrim

**CC:**  @vbraun @roed314

**Reviewer:** Beth Malmskog

**Author:** Travis Scrimshaw

**Merged:** sage-5.12.beta0

Issue created by migration from https://trac.sagemath.org/ticket/14870





---

archive/issue_comments_184684.json:
```json
{
    "body": "**Author:** Travis Scrimshaw",
    "created_at": "2013-07-11T08:29:52Z",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14870#issuecomment-184684",
    "user": "https://github.com/tscrim"
}
```

**Author:** Travis Scrimshaw



---

archive/issue_comments_184685.json:
```json
{
    "body": "<a id='comment:1'></a>\nFixed. Thanks Volker and David for your insight.",
    "created_at": "2013-07-11T08:29:52Z",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14870#issuecomment-184685",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'></a>
Fixed. Thanks Volker and David for your insight.



---

archive/issue_events_131197.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-07-11T08:29:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14870#event-131197"
}
```



---

archive/issue_comments_184686.json:
```json
{
    "body": "<a id='comment:3'></a>\nYou shouldn't check the type exactly, derived classes of rational should be treated like a rational.\n\nAlso, the whole `PY_*` macro usage that you see in the sources is mostly historical, nowadays Cython understands `isinstance(x, Rational)` just fine and this is the preferred form of a type check.",
    "created_at": "2013-07-11T16:39:29Z",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14870#issuecomment-184686",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:3'></a>
You shouldn't check the type exactly, derived classes of rational should be treated like a rational.

Also, the whole `PY_*` macro usage that you see in the sources is mostly historical, nowadays Cython understands `isinstance(x, Rational)` just fine and this is the preferred form of a type check.



---

archive/issue_events_131198.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/malmskog",
    "created_at": "2013-07-11T22:19:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14870#event-131198"
}
```



---

archive/issue_events_131199.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/malmskog",
    "created_at": "2013-07-11T22:19:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14870#event-131199"
}
```



---

archive/issue_comments_184687.json:
```json
{
    "body": "<a id='comment:4'></a>\nWe tried using decimals like 3.0, coerced rationals like QQ(3), non-reduced fractions like 4/2, complex-like values like 3+0*I, and absolute values of integers as moduli (second argument in function)--all of these were correctly interpreted as integers.  We also found correct error messages for moduli that didn't make sense, for example mod(31/10, 10) produces a correct error message.  Looks good!",
    "created_at": "2013-07-11T22:19:32Z",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14870#issuecomment-184687",
    "user": "https://trac.sagemath.org/admin/accounts/users/malmskog"
}
```

<a id='comment:4'></a>
We tried using decimals like 3.0, coerced rationals like QQ(3), non-reduced fractions like 4/2, complex-like values like 3+0*I, and absolute values of integers as moduli (second argument in function)--all of these were correctly interpreted as integers.  We also found correct error messages for moduli that didn't make sense, for example mod(31/10, 10) produces a correct error message.  Looks good!



---

archive/attachments_020674.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_14870-fix_int_mod_QQ-ts.patch",
    "asset_url": "tarball://root/attachments/ticket14870/trac_14870-fix_int_mod_QQ-ts.patch",
    "created_at": "2013-07-12T04:51:59Z",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket14870/trac_14870-fix_int_mod_QQ-ts.patch",
    "user": "https://github.com/tscrim"
}
```



---

archive/issue_comments_184688.json:
```json
{
    "body": "**Attachment:** [trac_14870-fix_int_mod_QQ-ts.patch](https://github.com/sagemath/sage/files/ticket14870/trac_14870-fix_int_mod_QQ-ts.patch)",
    "created_at": "2013-07-12T04:51:59Z",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14870#issuecomment-184688",
    "user": "https://github.com/tscrim"
}
```

**Attachment:** [trac_14870-fix_int_mod_QQ-ts.patch](https://github.com/sagemath/sage/files/ticket14870/trac_14870-fix_int_mod_QQ-ts.patch)



---

archive/issue_comments_184689.json:
```json
{
    "body": "<a id='comment:5'></a>\nNew version as per Volker's comment.",
    "created_at": "2013-07-12T04:52:22Z",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14870#issuecomment-184689",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'></a>
New version as per Volker's comment.



---

archive/issue_comments_184690.json:
```json
{
    "body": "<a id='comment:6'></a>\n`@`malmskog, you'll need to write your real name as the reviewer.",
    "created_at": "2013-07-12T04:52:49Z",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14870#issuecomment-184690",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'></a>
`@`malmskog, you'll need to write your real name as the reviewer.



---

archive/issue_comments_184691.json:
```json
{
    "body": "**Reviewer:** Beth Malmskog",
    "created_at": "2013-07-15T17:36:37Z",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14870#issuecomment-184691",
    "user": "https://trac.sagemath.org/admin/accounts/users/malmskog"
}
```

**Reviewer:** Beth Malmskog



---

archive/issue_events_131200.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-21T21:43:28Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14870#event-131200"
}
```



---

archive/issue_events_131201.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-21T21:43:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14870#event-131201"
}
```



---

archive/issue_events_131202.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-24T06:46:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14870#event-131202"
}
```



---

archive/issue_events_131203.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-24T06:46:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14870#event-131203"
}
```



---

archive/issue_events_131204.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-02T14:18:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14870#event-131204"
}
```



---

archive/issue_events_131205.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-02T14:18:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14870#event-131205"
}
```



---

archive/issue_comments_184692.json:
```json
{
    "body": "**Merged:** sage-5.12.beta0",
    "created_at": "2013-08-02T14:18:15Z",
    "issue": "https://github.com/sagemath/sage/issues/14870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14870#issuecomment-184692",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.12.beta0

# Issue 14561: cleanup of doc in sha_tate.py

archive/issues_014357.json:
```json
{
    "assignees": [
        "https://github.com/JohnCremona"
    ],
    "body": "<div id=\"comment:0\"></div>\n\njust a small patch to clean minor things in this file on tate-shafarevich\n\n- one unused variable found by pyflakes\n- minor details in the documentation\n- raise converted into python3 syntax\n\nDepends on #14571\n\nComponent: **elliptic curves**\n\nKeywords: **tate shafarevich**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\nReviewer: **Volker Braun**\n\nMerged: **sage-5.10.rc1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14561_\n\n",
    "closed_at": "2013-06-05T19:51:34Z",
    "created_at": "2013-05-09T20:07:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20elliptic%20curves",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/c%3A%20documentation"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "cleanup of doc in sha_tate.py",
    "type": "issue",
    "updated_at": "2013-06-06T12:51:42Z",
    "url": "https://github.com/sagemath/sage/issues/14561",
    "user": "https://github.com/fchapoton"
}
```
<div id="comment:0"></div>

just a small patch to clean minor things in this file on tate-shafarevich

- one unused variable found by pyflakes
- minor details in the documentation
- raise converted into python3 syntax

Depends on #14571

Component: **elliptic curves**

Keywords: **tate shafarevich**

Author: **Frédéric Chapoton**

Reviewer: **Volker Braun**

Merged: **sage-5.10.rc1**

_Issue created by migration from https://trac.sagemath.org/ticket/14561_





---

archive/issue_events_204244.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-09T20:07:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14561#event-204244"
}
```



---

archive/issue_events_204245.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-09T20:07:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20elliptic%20curves",
    "label_color": "0000ff",
    "label_name": "c: elliptic curves",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14561#event-204245"
}
```



---

archive/issue_events_204246.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-09T20:07:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14561#event-204246"
}
```



---

archive/issue_events_204247.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-09T20:07:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14561#event-204247"
}
```



---

archive/issue_events_204248.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-09T20:07:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14561#event-204248"
}
```



---

archive/issue_events_204249.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2013-05-09T20:07:31Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "subject": "https://github.com/fchapoton",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14561#event-204249"
}
```



---

archive/issue_comments_176371.json:
```json
{
    "body": "Attachment: **[trac_14561_cleanup_of_sha-fc.patch.gz](https://github.com/sagemath/sage/files/ticket14561/trac_14561_cleanup_of_sha-fc.patch.gz)**",
    "created_at": "2013-05-09T20:10:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14561#issuecomment-176371",
    "user": "https://github.com/fchapoton"
}
```

Attachment: **[trac_14561_cleanup_of_sha-fc.patch.gz](https://github.com/sagemath/sage/files/ticket14561/trac_14561_cleanup_of_sha-fc.patch.gz)**



---

archive/issue_comments_176372.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,3 +2,4 @@\n \n - one unused variable found by pyflakes\n - minor details in the documentation\n+- raise converted into python3 syntax\n``````\n",
    "created_at": "2013-05-09T20:11:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14561#issuecomment-176372",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,3 +2,4 @@
 
 - one unused variable found by pyflakes
 - minor details in the documentation
+- raise converted into python3 syntax
``````




---

archive/issue_events_204250.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-09T20:11:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14561#event-204250"
}
```



---

archive/issue_comments_176373.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nYou are duplicating the citation:\n\n```\n[plane_cur] /home/vbraun/opt/sage-5.10.beta2/local/lib/python2.7/site-packages/sage/schemes/elliptic_curves/sha_tate.py:docstring of sage.schemes.elliptic_curves.sha_tate.Sha.an_padic:8: WARNING: duplicate citation MTT, other instance in /home/vbraun/opt/sage-5.10.beta2/devel/sage/doc/en/reference/plane_curves/../plane_curves/sage/schemes/elliptic_curves/sha_tate.rst\n```",
    "created_at": "2013-05-10T09:18:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14561#issuecomment-176373",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:2" align="right">comment:2</div>

You are duplicating the citation:

```
[plane_cur] /home/vbraun/opt/sage-5.10.beta2/local/lib/python2.7/site-packages/sage/schemes/elliptic_curves/sha_tate.py:docstring of sage.schemes.elliptic_curves.sha_tate.Sha.an_padic:8: WARNING: duplicate citation MTT, other instance in /home/vbraun/opt/sage-5.10.beta2/devel/sage/doc/en/reference/plane_curves/../plane_curves/sage/schemes/elliptic_curves/sha_tate.rst
```



---

archive/issue_comments_176374.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI do not have this problem on 5.10.beta1.\n\nThe reference is not duplicated in the file itself.\n\nI will try with beta2 when I got it.",
    "created_at": "2013-05-10T10:27:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14561#issuecomment-176374",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">comment:3</div>

I do not have this problem on 5.10.beta1.

The reference is not duplicated in the file itself.

I will try with beta2 when I got it.



---

archive/issue_comments_176375.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nno problem either with 5.10.beta2. The doc builds without any warnings.",
    "created_at": "2013-05-10T14:24:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14561#issuecomment-176375",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:4" align="right">comment:4</div>

no problem either with 5.10.beta2. The doc builds without any warnings.



---

archive/issue_comments_176376.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2013-05-10T16:40:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14561#issuecomment-176376",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_176377.json:
```json
{
    "body": "Author: **Frederic Chapoton**",
    "created_at": "2013-05-10T16:40:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14561#issuecomment-176377",
    "user": "https://github.com/vbraun"
}
```

Author: **Frederic Chapoton**



---

archive/issue_comments_176378.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nSorry for the confusion, stale citation pickle I think...",
    "created_at": "2013-05-10T16:40:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14561#issuecomment-176378",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:5" align="right">comment:5</div>

Sorry for the confusion, stale citation pickle I think...



---

archive/issue_events_204251.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-05-10T16:40:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14561#event-204251"
}
```



---

archive/issue_events_204252.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-05-10T16:40:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14561#event-204252"
}
```



---

archive/issue_comments_176379.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nWell, thanks for the review !",
    "created_at": "2013-05-10T17:05:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14561#issuecomment-176379",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:6" align="right">comment:6</div>

Well, thanks for the review !



---

archive/issue_comments_176380.json:
```json
{
    "body": "Dependencies: **#14571**",
    "created_at": "2013-05-12T09:41:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14561#issuecomment-176380",
    "user": "https://github.com/jdemeyer"
}
```

Dependencies: **#14571**



---

archive/issue_events_204253.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-12T09:41:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14561#event-204253"
}
```



---

archive/issue_events_204254.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-12T09:41:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14561#event-204254"
}
```



---

archive/issue_comments_176381.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nMaybe I should rather remove the offending Unicode sha ?",
    "created_at": "2013-05-14T18:58:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14561#issuecomment-176381",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:8" align="right">comment:8</div>

Maybe I should rather remove the offending Unicode sha ?



---

archive/issue_comments_176382.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nNo, its a good testcase. Eventually we should just extend the pdf doc build by all unicode code points that we need, and the patch at #14571 is a step in that direction.",
    "created_at": "2013-05-14T19:18:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14561#issuecomment-176382",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:9" align="right">comment:9</div>

No, its a good testcase. Eventually we should just extend the pdf doc build by all unicode code points that we need, and the patch at #14571 is a step in that direction.



---

archive/issue_events_204255.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-05T19:51:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14561#event-204255"
}
```



---

archive/issue_events_204256.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-05T19:51:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14561#event-204256"
}
```



---

archive/issue_comments_176383.json:
```json
{
    "body": "Merged: **sage-5.10.rc1**",
    "created_at": "2013-06-05T19:51:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14561#issuecomment-176383",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.10.rc1**



---

archive/issue_events_204257.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-05T19:51:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14561#event-204257"
}
```



---

archive/issue_events_204258.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-05T19:51:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14561#event-204258"
}
```



---

archive/issue_comments_176384.json:
```json
{
    "body": "Changed author from **Frederic Chapoton** to **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2013-06-06T12:51:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14561#issuecomment-176384",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Frederic Chapoton** to **Frédéric Chapoton**

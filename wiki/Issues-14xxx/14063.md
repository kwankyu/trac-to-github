# Issue 14063: Remove CombinatorialClass from Compositions

archive/issues_013859.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-sage-combinat"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nPart of #12913.\n\nDepends on #14065\n\nCC:  @sagetrac-sage-combinat @nthiery\n\nComponent: **combinatorics**\n\nKeywords: **deprecation**\n\nAuthor: **Travis Scrimshaw**\n\nReviewer: **Vincent Delecroix**\n\nMerged: **sage-5.8.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14063_\n\n",
    "closed_at": "2013-02-28T10:34:16Z",
    "created_at": "2013-02-05T23:19:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove CombinatorialClass from Compositions",
    "type": "issue",
    "updated_at": "2013-02-28T10:34:16Z",
    "url": "https://github.com/sagemath/sage/issues/14063",
    "user": "https://github.com/tscrim"
}
```
<div id="comment:0"></div>

Part of #12913.

Depends on #14065

CC:  @sagetrac-sage-combinat @nthiery

Component: **combinatorics**

Keywords: **deprecation**

Author: **Travis Scrimshaw**

Reviewer: **Vincent Delecroix**

Merged: **sage-5.8.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/14063_





---

archive/issue_events_195890.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-05T23:19:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195890"
}
```



---

archive/issue_events_195891.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-05T23:19:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195891"
}
```



---

archive/issue_events_195892.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-05T23:19:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195892"
}
```



---

archive/issue_events_195893.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-05T23:19:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195893"
}
```



---

archive/issue_events_195894.json:
```json
{
    "actor": "https://github.com/sagetrac-sage-combinat",
    "created_at": "2013-02-05T23:19:13Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "subject": "https://github.com/tscrim",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195894"
}
```



---

archive/issue_comments_167166.json:
```json
{
    "body": "Dependencies: **#14065**",
    "created_at": "2013-02-06T04:09:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-167166",
    "user": "https://github.com/tscrim"
}
```

Dependencies: **#14065**



---

archive/issue_comments_167167.json:
```json
{
    "body": "Author: **Travis Scrimshaw**",
    "created_at": "2013-02-06T04:09:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-167167",
    "user": "https://github.com/tscrim"
}
```

Author: **Travis Scrimshaw**



---

archive/issue_events_195895.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-06T04:09:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195895"
}
```



---

archive/issue_comments_167168.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nHi,\n\n* composition_from_subset must be renamed to from_subset\n* You may remove the few trailing whitespaces that remain.\n\nThe rest looks good.\n\nVincent",
    "created_at": "2013-02-15T17:50:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-167168",
    "user": "https://github.com/videlec"
}
```

<div id="comment:2" align="right">comment:2</div>

Hi,

* composition_from_subset must be renamed to from_subset
* You may remove the few trailing whitespaces that remain.

The rest looks good.

Vincent



---

archive/issue_comments_167169.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nComments on the doc in composition.py\n* line 786: list(Compositions(4)) may be replaced with Compositions(4).list() (idem lines 877, 882, 927)\n* line 844: there is a strange sharp at the end of the line\n* line 859: a space is missing",
    "created_at": "2013-02-15T20:04:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-167169",
    "user": "https://github.com/videlec"
}
```

<div id="comment:3" align="right">comment:3</div>

Comments on the doc in composition.py
* line 786: list(Compositions(4)) may be replaced with Compositions(4).list() (idem lines 877, 882, 927)
* line 844: there is a strange sharp at the end of the line
* line 859: a space is missing



---

archive/issue_comments_167170.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMade fixes based on your comments.\n\nThanks,\n\nTravis",
    "created_at": "2013-02-15T21:17:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-167170",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:4" align="right">comment:4</div>

Made fixes based on your comments.

Thanks,

Travis



---

archive/issue_events_195896.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2013-02-15T21:49:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195896"
}
```



---

archive/issue_events_195897.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2013-02-15T21:49:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195897"
}
```



---

archive/issue_comments_167171.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nGreat job !\n\nVincent",
    "created_at": "2013-02-15T21:49:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-167171",
    "user": "https://github.com/videlec"
}
```

<div id="comment:5" align="right">comment:5</div>

Great job !

Vincent



---

archive/issue_comments_167172.json:
```json
{
    "body": "Reviewer: **Vincent Delecroix**",
    "created_at": "2013-02-16T11:30:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-167172",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Vincent Delecroix**



---

archive/issue_events_195898.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-16T11:30:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195898"
}
```



---

archive/issue_events_195899.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-16T11:30:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "milestone_number": null,
    "milestone_title": "sage-5.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195899"
}
```



---

archive/issue_comments_167173.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nHow was this patch file created, as some headers are missing? You should use `hg export [tip]`.",
    "created_at": "2013-02-19T21:52:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-167173",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:7" align="right">comment:7</div>

How was this patch file created, as some headers are missing? You should use `hg export [tip]`.



---

archive/issue_comments_167174.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nForgot to export, sorry about that. Fixed.",
    "created_at": "2013-02-19T22:16:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-167174",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:8" align="right">comment:8</div>

Forgot to export, sorry about that. Fixed.



---

archive/issue_comments_167175.json:
```json
{
    "body": "Merged: **sage-5.8.beta1**",
    "created_at": "2013-02-22T19:12:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-167175",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.8.beta1**



---

archive/issue_events_195900.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-22T19:12:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195900"
}
```



---

archive/issue_events_195901.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-22T19:12:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195901"
}
```



---

archive/issue_comments_167176.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nThe LaTeX documentation doesn't build because you use backslashes inside the\n\n```\ndef cardinality(self):\n\"\"\"\n\"\"\"\n```\ndocstring instead of\n\n```\ndef cardinality(self):\nr\"\"\"\n\"\"\"\n```",
    "created_at": "2013-02-23T00:11:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-167176",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:10" align="right">comment:10</div>

The LaTeX documentation doesn't build because you use backslashes inside the

```
def cardinality(self):
"""
"""
```
docstring instead of

```
def cardinality(self):
r"""
"""
```



---

archive/issue_comments_167177.json:
```json
{
    "body": "Changed merged from **sage-5.8.beta1** to none",
    "created_at": "2013-02-23T00:11:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-167177",
    "user": "https://github.com/jdemeyer"
}
```

Changed merged from **sage-5.8.beta1** to none



---

archive/issue_events_195902.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-23T00:11:09Z",
    "event": "reopened",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195902"
}
```



---

archive/issue_events_195903.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-23T00:58:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195903"
}
```



---

archive/issue_comments_167178.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nMade the fixes (I put it in a few other places as well). I'll double/triple-check over the compiled docbuild once it finishes the rebuild.",
    "created_at": "2013-02-23T00:58:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-167178",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:11" align="right">comment:11</div>

Made the fixes (I put it in a few other places as well). I'll double/triple-check over the compiled docbuild once it finishes the rebuild.



---

archive/issue_comments_167179.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nI also fixed a broken link. Jeroen, would it be okay if I set this back to positive review?\n\nSorry about this,\n\nTravis",
    "created_at": "2013-02-23T01:19:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-167179",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:12" align="right">comment:12</div>

I also fixed a broken link. Jeroen, would it be okay if I set this back to positive review?

Sorry about this,

Travis



---

archive/issue_comments_167180.json:
```json
{
    "body": "Fixed docstrings",
    "created_at": "2013-02-23T02:59:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-167180",
    "user": "https://github.com/tscrim"
}
```

Fixed docstrings



---

archive/issue_comments_167181.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nAttachment: **[trac_14063-remove_cc_compositions-ts.patch.gz](https://github.com/sagemath/sage/files/ticket14063/trac_14063-remove_cc_compositions-ts.patch.gz)**\n\nI'm setting this back to positive review...I hope that's okay...",
    "created_at": "2013-02-23T03:00:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-167181",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:13" align="right">comment:13</div>

Attachment: **[trac_14063-remove_cc_compositions-ts.patch.gz](https://github.com/sagemath/sage/files/ticket14063/trac_14063-remove_cc_compositions-ts.patch.gz)**

I'm setting this back to positive review...I hope that's okay...



---

archive/issue_events_195904.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-23T03:00:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195904"
}
```



---

archive/issue_events_195905.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-23T03:00:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195905"
}
```



---

archive/issue_events_195906.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-28T10:34:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195906"
}
```



---

archive/issue_events_195907.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-28T10:34:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-195907"
}
```



---

archive/issue_comments_167182.json:
```json
{
    "body": "Merged: **sage-5.8.beta2**",
    "created_at": "2013-02-28T10:34:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-167182",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.8.beta2**

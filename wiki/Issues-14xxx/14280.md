# Issue 14280: The pexpect spkg looks strange

archive/issues_014076.json:
```json
{
    "body": "First of all, it's stuck to 2.0 while upstream is up to something like 2.4 -- though it is explained in SPKG.txt that upgrading to 2.1 was out of the question because it was shockingly slow (sic). I don't know if it's the case and how to check it, so I would like to see that documented more properly.\n\nSecond, the spkg has the sources in two directories, src/ and src.del/, which contain mostly the same thing (there's a small diff).\n\nThird, the spkg comes with patches, and those patches do not say whether they have been forwarded upstream, which as far as I know means they didn't.\n\n**Assignee:** @williamstein\n\n**Reviewer:** Julien Puydt\n\nIssue created by migration from https://trac.sagemath.org/ticket/14280\n\n",
    "closed_at": "2015-05-19T06:43:45Z",
    "created_at": "2013-03-15T21:47:00Z",
    "labels": [
        "component: interfaces",
        "bug",
        "invalid"
    ],
    "title": "The pexpect spkg looks strange",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/14280",
    "user": "https://trac.sagemath.org/admin/accounts/users/Snark"
}
```
First of all, it's stuck to 2.0 while upstream is up to something like 2.4 -- though it is explained in SPKG.txt that upgrading to 2.1 was out of the question because it was shockingly slow (sic). I don't know if it's the case and how to check it, so I would like to see that documented more properly.

Second, the spkg has the sources in two directories, src/ and src.del/, which contain mostly the same thing (there's a small diff).

Third, the spkg comes with patches, and those patches do not say whether they have been forwarded upstream, which as far as I know means they didn't.

**Assignee:** @williamstein

**Reviewer:** Julien Puydt

Issue created by migration from https://trac.sagemath.org/ticket/14280





---

archive/issue_events_124311.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14280",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14280#event-124311"
}
```



---

archive/issue_events_124312.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14280",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14280#event-124312"
}
```



---

archive/issue_events_124313.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14280",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14280#event-124313"
}
```



---

archive/issue_events_124314.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14280",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14280#event-124314"
}
```



---

archive/issue_events_124315.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14280",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14280#event-124315"
}
```



---

archive/issue_events_124316.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14280",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14280#event-124316"
}
```



---

archive/issue_events_124317.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14280",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14280#event-124317"
}
```



---

archive/issue_events_124318.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14280",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14280#event-124318"
}
```



---

archive/issue_events_124319.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2015-05-07T02:55:14Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14280",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14280#event-124319"
}
```



---

archive/issue_events_124320.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2015-05-07T02:55:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14280",
    "label": "needs info",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14280#event-124320"
}
```



---

archive/issue_comments_173413.json:
```json
{
    "body": "<a id='comment:5'></a>\nI guess this no longer applies.",
    "created_at": "2015-05-07T02:55:14Z",
    "issue": "https://github.com/sagemath/sage/issues/14280",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14280#issuecomment-173413",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:5'></a>
I guess this no longer applies.



---

archive/issue_events_124321.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/Snark",
    "created_at": "2015-05-07T04:38:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14280",
    "label": "needs info",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14280#event-124321"
}
```



---

archive/issue_events_124322.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/Snark",
    "created_at": "2015-05-07T04:38:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14280",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14280#event-124322"
}
```



---

archive/issue_comments_173414.json:
```json
{
    "body": "<a id='comment:6'></a>\nYes, please just close!",
    "created_at": "2015-05-07T04:38:18Z",
    "issue": "https://github.com/sagemath/sage/issues/14280",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14280#issuecomment-173414",
    "user": "https://trac.sagemath.org/admin/accounts/users/Snark"
}
```

<a id='comment:6'></a>
Yes, please just close!



---

archive/issue_comments_173415.json:
```json
{
    "body": "**Reviewer:** Julien Puydt",
    "created_at": "2015-05-07T04:47:39Z",
    "issue": "https://github.com/sagemath/sage/issues/14280",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14280#issuecomment-173415",
    "user": "https://github.com/nexttime"
}
```

**Reviewer:** Julien Puydt



---

archive/issue_events_124323.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-19T06:43:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14280",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14280#event-124323"
}
```



---

archive/issue_events_124324.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-19T06:43:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14280",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14280#event-124324"
}
```

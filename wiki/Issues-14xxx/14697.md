# Issue 14697: System-wide GCC picks Sage's libraries on Cygwin

archive/issues_014493.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nOn Cygwin, libraries are stored in \"bin\" together with executables.\nAs we add Sage's bin directory to PATH (to be able to use the executables we generated!), the system-wide GCC may pick up some Sage's libraries (e.g. gmp, mpfr or mpc) rather than the system-wide ones it is supposed to if versions are identical.\nUnfortunately, this may raise incompatibilities: I experienced a bunch of segfaults when building different spkgs.\n\nCC:  @nexttime @kcrisman @dimpase\n\nComponent: **porting: Cygwin**\n\nKeywords: **cygwin gcc path**\n\nReviewer: **Jean-Pierre Flori**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14697_\n\n",
    "closed_at": "2014-11-28T18:39:46Z",
    "created_at": "2013-06-06T13:44:21Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20cygwin"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "System-wide GCC picks Sage's libraries on Cygwin",
    "type": "issue",
    "updated_at": "2014-11-28T18:39:46Z",
    "url": "https://github.com/sagemath/sage/issues/14697",
    "user": "https://github.com/jpflori"
}
```
<div id="comment:0"></div>

On Cygwin, libraries are stored in "bin" together with executables.
As we add Sage's bin directory to PATH (to be able to use the executables we generated!), the system-wide GCC may pick up some Sage's libraries (e.g. gmp, mpfr or mpc) rather than the system-wide ones it is supposed to if versions are identical.
Unfortunately, this may raise incompatibilities: I experienced a bunch of segfaults when building different spkgs.

CC:  @nexttime @kcrisman @dimpase

Component: **porting: Cygwin**

Keywords: **cygwin gcc path**

Reviewer: **Jean-Pierre Flori**

_Issue created by migration from https://trac.sagemath.org/ticket/14697_





---

archive/issue_events_206615.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-06T13:44:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206615"
}
```



---

archive/issue_events_206616.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-06T13:44:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20cygwin",
    "label_color": "0000b0",
    "label_name": "c: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206616"
}
```



---

archive/issue_events_206617.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-06T13:44:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206617"
}
```



---

archive/issue_events_206618.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-06T13:44:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206618"
}
```



---

archive/issue_events_206619.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-06T13:44:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20cygwin",
    "label_color": "0000b0",
    "label_name": "c: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206619"
}
```



---

archive/issue_events_206620.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206620"
}
```



---

archive/issue_events_206621.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206621"
}
```



---

archive/issue_events_206622.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206622"
}
```



---

archive/issue_events_206623.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206623"
}
```



---

archive/issue_events_206624.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206624"
}
```



---

archive/issue_events_206625.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206625"
}
```



---

archive/issue_events_206626.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206626"
}
```



---

archive/issue_events_206627.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206627"
}
```



---

archive/issue_events_206628.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-11-25T14:37:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206628"
}
```



---

archive/issue_comments_178596.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nFixed by #10572.",
    "created_at": "2014-11-25T14:37:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14697#issuecomment-178596",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:5" align="right">comment:5</div>

Fixed by #10572.



---

archive/issue_events_206629.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-11-25T14:37:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206629"
}
```



---

archive/issue_events_206630.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-11-25T14:37:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206630"
}
```



---

archive/issue_events_206631.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-11-25T14:37:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206631"
}
```



---

archive/issue_comments_178597.json:
```json
{
    "body": "Reviewer: **Jean-Pierre Flori**",
    "created_at": "2014-11-25T14:38:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14697#issuecomment-178597",
    "user": "https://github.com/jpflori"
}
```

Reviewer: **Jean-Pierre Flori**



---

archive/issue_events_206632.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-28T18:39:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206632"
}
```



---

archive/issue_events_206633.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-28T18:39:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-206633"
}
```

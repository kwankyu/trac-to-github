# Issue 14093: Failing Doctest in sage.quadratic_form.quadratic_form__equivalence_testing.py

archive/issues_013889.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThere is an easy to fix failing doctest in `sage/quadratic_form/quadratic_form__equivalence_testing.py`. Let's fix it!  \n\nComponent: **number theory**\n\nKeywords: **quadratic_form**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14093_\n\n",
    "closed_at": "2013-02-28T11:04:08Z",
    "created_at": "2013-02-10T09:30:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/good%20first%20issue",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Failing Doctest in sage.quadratic_form.quadratic_form__equivalence_testing.py",
    "type": "issue",
    "updated_at": "2013-02-28T11:04:08Z",
    "url": "https://github.com/sagemath/sage/issues/14093",
    "user": "https://github.com/KPanComputes"
}
```
<div id="comment:0"></div>

There is an easy to fix failing doctest in `sage/quadratic_form/quadratic_form__equivalence_testing.py`. Let's fix it!  

Component: **number theory**

Keywords: **quadratic_form**

_Issue created by migration from https://trac.sagemath.org/ticket/14093_





---

archive/issue_events_196356.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-02-10T09:30:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14093",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14093#event-196356"
}
```



---

archive/issue_events_196357.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-02-10T09:30:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14093",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "c: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14093#event-196357"
}
```



---

archive/issue_events_196358.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-02-10T09:30:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14093",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14093#event-196358"
}
```



---

archive/issue_events_196359.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-02-10T09:30:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14093",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14093#event-196359"
}
```



---

archive/issue_events_196360.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-02-10T09:30:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14093",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14093#event-196360"
}
```



---

archive/issue_events_196361.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2013-02-10T09:30:37Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/14093",
    "subject": "https://github.com/KPanComputes",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14093#event-196361"
}
```



---

archive/issue_comments_167554.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-There is an easy to fix failing doctest in sage/quadratic_form/quadratic_form__equivalence_testing.py. Let's fix it!  \n+There is an easy to fix failing doctest in `sage/quadratic_form/quadratic_form__equivalence_testing.py`. Let's fix it!  \n``````\n",
    "created_at": "2013-02-10T09:31:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14093",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14093#issuecomment-167554",
    "user": "https://github.com/KPanComputes"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-There is an easy to fix failing doctest in sage/quadratic_form/quadratic_form__equivalence_testing.py. Let's fix it!  
+There is an easy to fix failing doctest in `sage/quadratic_form/quadratic_form__equivalence_testing.py`. Let's fix it!  
``````




---

archive/issue_events_196362.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-02-10T09:31:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14093",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14093#event-196362"
}
```



---

archive/issue_comments_167555.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_14093_importerror_fix.patch.gz](https://github.com/sagemath/sage/files/ticket14093/trac_14093_importerror_fix.patch.gz)**",
    "created_at": "2013-02-10T09:31:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14093",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14093#issuecomment-167555",
    "user": "https://github.com/KPanComputes"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_14093_importerror_fix.patch.gz](https://github.com/sagemath/sage/files/ticket14093/trac_14093_importerror_fix.patch.gz)**



---

archive/issue_events_196363.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-02-16T15:22:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14093",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14093#event-196363"
}
```



---

archive/issue_comments_167556.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nOkay! My fault! Just discovered how to do things the right way! Sorry for the noise.",
    "created_at": "2013-02-16T15:23:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14093",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14093#issuecomment-167556",
    "user": "https://github.com/KPanComputes"
}
```

<div id="comment:3" align="right">comment:3</div>

Okay! My fault! Just discovered how to do things the right way! Sorry for the noise.



---

archive/issue_comments_167557.json:
```json
{
    "body": "Changed author from **Kannappan Sampath** to none",
    "created_at": "2013-02-25T04:19:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14093",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14093#issuecomment-167557",
    "user": "https://github.com/tscrim"
}
```

Changed author from **Kannappan Sampath** to none



---

archive/issue_events_196364.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-25T04:19:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14093",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14093#event-196364"
}
```



---

archive/issue_events_196365.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-25T04:19:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14093",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14093#event-196365"
}
```



---

archive/issue_events_196366.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-28T11:04:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14093",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14093#event-196366"
}
```



---

archive/issue_events_196367.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-28T11:04:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14093",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14093#event-196367"
}
```

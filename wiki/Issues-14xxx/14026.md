# Issue 14026: PolyBoRi should depend on IPython

archive/issues_013822.json:
```json
{
    "body": "`PolyBoRi` contains an `IPython`-based shell (`ipbori`).  `spkg/standard/deps` should reflect this.\n\n**Assignee:** GeorgSWeber\n\n**CC:**  @vbraun\n\n**Reviewer:** Alexander Dreyer\n\n**Author:** Jeroen Demeyer\n\n**Merged:** sage-5.9.beta1\n\nIssue created by migration from https://trac.sagemath.org/ticket/14026\n\n",
    "closed_at": "2013-03-18T09:18:39Z",
    "created_at": "2013-01-28T13:10:20Z",
    "labels": [
        "component: build",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.9",
    "title": "PolyBoRi should depend on IPython",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/14026",
    "user": "https://github.com/jdemeyer"
}
```
`PolyBoRi` contains an `IPython`-based shell (`ipbori`).  `spkg/standard/deps` should reflect this.

**Assignee:** GeorgSWeber

**CC:**  @vbraun

**Reviewer:** Alexander Dreyer

**Author:** Jeroen Demeyer

**Merged:** sage-5.9.beta1

Issue created by migration from https://trac.sagemath.org/ticket/14026





---

archive/issue_comments_168786.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-`PolyBoRi` contains an `IPython`-based shell (`ipbori`).  `spkg/standard/deps` should reflect this.  This might be the cause of the upgrade failure of `IPython` (mentioned at #14024), where `local/bin/ipython` was not installed correctly.\n+`PolyBoRi` contains an `IPython`-based shell (`ipbori`).  `spkg/standard/deps` should reflect this.\n``````\n",
    "created_at": "2013-01-28T13:16:57Z",
    "issue": "https://github.com/sagemath/sage/issues/14026",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14026#issuecomment-168786",
    "user": "https://github.com/jdemeyer"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1 @@
-`PolyBoRi` contains an `IPython`-based shell (`ipbori`).  `spkg/standard/deps` should reflect this.  This might be the cause of the upgrade failure of `IPython` (mentioned at #14024), where `local/bin/ipython` was not installed correctly.
+`PolyBoRi` contains an `IPython`-based shell (`ipbori`).  `spkg/standard/deps` should reflect this.
``````




---

archive/attachments_019588.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "14026_polybori_deps.patch",
    "asset_url": "tarball://root/attachments/ticket14026/14026_polybori_deps.patch",
    "created_at": "2013-01-28T13:40:11Z",
    "issue": "https://github.com/sagemath/sage/issues/14026",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket14026/14026_polybori_deps.patch",
    "user": "https://github.com/jdemeyer"
}
```



---

archive/issue_comments_168787.json:
```json
{
    "body": "<a id='comment:2'></a>\n**Attachment:** [14026_polybori_deps.patch](https://github.com/sagemath/sage/files/ticket14026/14026_polybori_deps.patch)",
    "created_at": "2013-01-28T13:40:11Z",
    "issue": "https://github.com/sagemath/sage/issues/14026",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14026#issuecomment-168787",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'></a>
**Attachment:** [14026_polybori_deps.patch](https://github.com/sagemath/sage/files/ticket14026/14026_polybori_deps.patch)



---

archive/issue_events_121419.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-28T13:40:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14026",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14026#event-121419"
}
```



---

archive/issue_comments_168788.json:
```json
{
    "body": "<a id='comment:3'></a>\nI'm not entirely sure this patch is really needed, but it can't possibly hurt.",
    "created_at": "2013-01-29T07:49:26Z",
    "issue": "https://github.com/sagemath/sage/issues/14026",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14026#issuecomment-168788",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'></a>
I'm not entirely sure this patch is really needed, but it can't possibly hurt.



---

archive/issue_events_121420.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-29T07:49:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14026",
    "label": "blocker",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14026#event-121420"
}
```



---

archive/issue_events_121421.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-29T07:49:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14026",
    "label": "critical",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14026#event-121421"
}
```



---

archive/issue_events_121422.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-08T14:31:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14026",
    "label": "critical",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14026#event-121422"
}
```



---

archive/issue_events_121423.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-08T14:31:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14026",
    "label": "minor",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14026#event-121423"
}
```



---

archive/issue_comments_168789.json:
```json
{
    "body": "<a id='comment:5'></a>\nIndeed, it's not needed, since ipbori can fall back to a plain python shell at runtime. But it won't hurt and ensures that the best shell is used. So positively reviewed.",
    "created_at": "2013-03-15T22:04:01Z",
    "issue": "https://github.com/sagemath/sage/issues/14026",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14026#issuecomment-168789",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:5'></a>
Indeed, it's not needed, since ipbori can fall back to a plain python shell at runtime. But it won't hurt and ensures that the best shell is used. So positively reviewed.



---

archive/issue_events_121424.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2013-03-15T22:04:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14026",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14026#event-121424"
}
```



---

archive/issue_events_121425.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2013-03-15T22:04:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14026",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14026#event-121425"
}
```



---

archive/issue_comments_168790.json:
```json
{
    "body": "**Reviewer:** Alexander Dreyer",
    "created_at": "2013-03-18T09:18:39Z",
    "issue": "https://github.com/sagemath/sage/issues/14026",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14026#issuecomment-168790",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Alexander Dreyer



---

archive/issue_comments_168791.json:
```json
{
    "body": "**Merged:** sage-5.9.beta1",
    "created_at": "2013-03-18T09:18:39Z",
    "issue": "https://github.com/sagemath/sage/issues/14026",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14026#issuecomment-168791",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.9.beta1



---

archive/issue_events_121426.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-18T09:18:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14026",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14026#event-121426"
}
```



---

archive/issue_events_121427.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-18T09:18:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14026",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14026#event-121427"
}
```

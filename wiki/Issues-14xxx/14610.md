# Issue 14610: Energy function for level zero LS paths for all types

archive/issues_014406.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mshimo"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nExtend the energy function for level zero LS paths to work for dual untwisted affine root systems. Currently it only works for untwisted affine root systems.\nThis patch also implements is_short_root for finite irreducible types.\n\nCC:  @sagetrac-sage-combinat @bsalisbury1\n\nComponent: **combinatorics**\n\nKeywords: **crystal graph, days49**\n\nAuthor: **Mark Shimozono, Anne Schilling**\n\nReviewer: **Anne Schilling, Mark Shimozono, Ben Salisbury**\n\nMerged: **sage-5.12.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14610_\n\n",
    "closed_at": "2013-08-16T21:15:48Z",
    "created_at": "2013-05-18T15:40:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.12",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Energy function for level zero LS paths for all types",
    "type": "issue",
    "updated_at": "2013-08-16T21:15:48Z",
    "url": "https://github.com/sagemath/sage/issues/14610",
    "user": "https://github.com/sagetrac-mshimo"
}
```
<div id="comment:0"></div>

Extend the energy function for level zero LS paths to work for dual untwisted affine root systems. Currently it only works for untwisted affine root systems.
This patch also implements is_short_root for finite irreducible types.

CC:  @sagetrac-sage-combinat @bsalisbury1

Component: **combinatorics**

Keywords: **crystal graph, days49**

Author: **Mark Shimozono, Anne Schilling**

Reviewer: **Anne Schilling, Mark Shimozono, Ben Salisbury**

Merged: **sage-5.12.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/14610_





---

archive/issue_events_205035.json:
```json
{
    "actor": "https://github.com/sagetrac-mshimo",
    "created_at": "2013-05-18T15:40:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205035"
}
```



---

archive/issue_events_205036.json:
```json
{
    "actor": "https://github.com/sagetrac-mshimo",
    "created_at": "2013-05-18T15:40:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205036"
}
```



---

archive/issue_events_205037.json:
```json
{
    "actor": "https://github.com/sagetrac-mshimo",
    "created_at": "2013-05-18T15:40:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205037"
}
```



---

archive/issue_events_205038.json:
```json
{
    "actor": "https://github.com/sagetrac-mshimo",
    "created_at": "2013-05-18T15:40:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205038"
}
```



---

archive/issue_events_205039.json:
```json
{
    "actor": "https://github.com/sagetrac-mshimo",
    "created_at": "2013-05-18T15:40:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205039"
}
```



---

archive/issue_events_205040.json:
```json
{
    "actor": "https://github.com/sagetrac-mshimo",
    "created_at": "2013-05-18T15:40:23Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "subject": "https://github.com/sagetrac-mshimo",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205040"
}
```



---

archive/issue_events_205041.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2013-05-21T01:59:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205041"
}
```



---

archive/issue_comments_177207.json:
```json
{
    "body": "Reviewer: **Anne Schilling, Mark Shimozono**",
    "created_at": "2013-05-21T01:59:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14610#issuecomment-177207",
    "user": "https://github.com/anneschilling"
}
```

Reviewer: **Anne Schilling, Mark Shimozono**



---

archive/issue_comments_177208.json:
```json
{
    "body": "Author: **Mark Shimozono, Anne Schilling**",
    "created_at": "2013-05-21T01:59:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14610#issuecomment-177208",
    "user": "https://github.com/anneschilling"
}
```

Author: **Mark Shimozono, Anne Schilling**



---

archive/issue_events_205042.json:
```json
{
    "actor": "https://github.com/sagetrac-mshimo",
    "created_at": "2013-05-21T04:08:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205042"
}
```



---

archive/issue_events_205043.json:
```json
{
    "actor": "https://github.com/sagetrac-mshimo",
    "created_at": "2013-05-21T04:08:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205043"
}
```



---

archive/issue_events_205044.json:
```json
{
    "actor": "https://github.com/sagetrac-mshimo",
    "created_at": "2013-05-21T14:18:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205044"
}
```



---

archive/issue_events_205045.json:
```json
{
    "actor": "https://github.com/sagetrac-mshimo",
    "created_at": "2013-05-21T14:18:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205045"
}
```



---

archive/issue_events_205046.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2013-06-21T10:38:29Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "title_is": "Energy function for level zero LS paths for all types",
    "title_was": "energy function for level zero LS paths for dual untwisted affine root systems",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205046"
}
```



---

archive/issue_events_205047.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2013-06-21T10:38:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205047"
}
```



---

archive/issue_events_205048.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2013-06-21T10:38:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205048"
}
```



---

archive/issue_comments_177209.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,2 @@\n-Extend the energy function for level zero LS paths to work for dual untwisted affine root systems.\n-Currently it works for untwisted affine root systems.\n+Extend the energy function for level zero LS paths to work for dual untwisted affine root systems. Currently it only works for untwisted affine root systems.\n+This patch also implements is_short_root for finite irreducible types.\n``````\n",
    "created_at": "2013-06-21T10:38:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14610#issuecomment-177209",
    "user": "https://github.com/anneschilling"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,2 @@
-Extend the energy function for level zero LS paths to work for dual untwisted affine root systems.
-Currently it works for untwisted affine root systems.
+Extend the energy function for level zero LS paths to work for dual untwisted affine root systems. Currently it only works for untwisted affine root systems.
+This patch also implements is_short_root for finite irreducible types.
``````




---

archive/issue_comments_177210.json:
```json
{
    "body": "Changed keywords from **crystal graph** to **crystal graph, days49**",
    "created_at": "2013-07-02T00:11:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14610#issuecomment-177210",
    "user": "https://github.com/anneschilling"
}
```

Changed keywords from **crystal graph** to **crystal graph, days49**



---

archive/issue_events_205049.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2013-07-02T00:11:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205049"
}
```



---

archive/issue_comments_177211.json:
```json
{
    "body": "Changed author from **Mark Shimozono, Anne Schilling** to **Mark Shimozono, Anne Schilling, Ben Salisbury**",
    "created_at": "2013-07-02T17:08:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14610#issuecomment-177211",
    "user": "https://github.com/anneschilling"
}
```

Changed author from **Mark Shimozono, Anne Schilling** to **Mark Shimozono, Anne Schilling, Ben Salisbury**



---

archive/issue_comments_177212.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nHi Ben,\n\nThe patch is now ready for the final review.\n\nThanks,\n\nAnne",
    "created_at": "2013-07-02T17:11:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14610#issuecomment-177212",
    "user": "https://github.com/anneschilling"
}
```

<div id="comment:8" align="right">comment:8</div>

Hi Ben,

The patch is now ready for the final review.

Thanks,

Anne



---

archive/issue_comments_177213.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nHi Anne and Mark,\n\nThank you for your work on this!  Everything looks good!\n\n```\n----------------------------------------------------------------------\nAll tests passed!\n----------------------------------------------------------------------\nTotal time for all tests: 1692.1 seconds\n    cpu time: 5689.0 seconds\n    cumulative wall time: 9948.0 seconds\n```\n\n~Ben",
    "created_at": "2013-07-02T18:54:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14610#issuecomment-177213",
    "user": "https://github.com/bsalisbury1"
}
```

<div id="comment:9" align="right">comment:9</div>

Hi Anne and Mark,

Thank you for your work on this!  Everything looks good!

```
----------------------------------------------------------------------
All tests passed!
----------------------------------------------------------------------
Total time for all tests: 1692.1 seconds
    cpu time: 5689.0 seconds
    cumulative wall time: 9948.0 seconds
```

~Ben



---

archive/issue_comments_177214.json:
```json
{
    "body": "Changed author from **Mark Shimozono, Anne Schilling, Ben Salisbury** to **Mark Shimozono, Anne Schilling**",
    "created_at": "2013-07-02T18:54:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14610#issuecomment-177214",
    "user": "https://github.com/bsalisbury1"
}
```

Changed author from **Mark Shimozono, Anne Schilling, Ben Salisbury** to **Mark Shimozono, Anne Schilling**



---

archive/issue_comments_177215.json:
```json
{
    "body": "Changed reviewer from **Anne Schilling, Mark Shimozono** to **Anne Schilling, Mark Shimozono, Ben Salisbury**",
    "created_at": "2013-07-02T18:54:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14610#issuecomment-177215",
    "user": "https://github.com/bsalisbury1"
}
```

Changed reviewer from **Anne Schilling, Mark Shimozono** to **Anne Schilling, Mark Shimozono, Ben Salisbury**



---

archive/issue_events_205050.json:
```json
{
    "actor": "https://github.com/bsalisbury1",
    "created_at": "2013-07-02T18:54:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205050"
}
```



---

archive/issue_events_205051.json:
```json
{
    "actor": "https://github.com/bsalisbury1",
    "created_at": "2013-07-02T18:54:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205051"
}
```



---

archive/issue_comments_177216.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nBen, thank you for the review!",
    "created_at": "2013-07-02T19:11:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14610#issuecomment-177216",
    "user": "https://github.com/anneschilling"
}
```

<div id="comment:10" align="right">comment:10</div>

Ben, thank you for the review!



---

archive/issue_events_205052.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-21T21:45:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205052"
}
```



---

archive/issue_events_205053.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-21T21:45:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205053"
}
```



---

archive/issue_comments_177217.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nThis `AssertionError` should be changed into a `ValueError` or `TypeError`:\n\n```\nsage: RootSystem(['D',2]).root_lattice().simple_root(1).is_short_root() \n---------------------------------------------------------------------------\nAssertionError                            Traceback (most recent call last)\n<ipython-input-25-e961de76571c> in <module>()\n----> 1 RootSystem(['D',Integer(2)]).root_lattice().simple_root(Integer(1)).is_short_root()\n\n/mazur/release/merger/sage-5.12.beta1/local/lib/python2.7/site-packages/sage/combinat/root_system/root_lattice_realizations.pyc in is_short_root(self)\n   3290             \"\"\"\n   3291             ct = self.parent().cartan_type()\n-> 3292             assert ct.is_irreducible()\n   3293             if not ct.is_finite():\n   3294                 raise NotImplementedError, \"Implemented only for irreducible finite root systems\"\n\nAssertionError: \n```",
    "created_at": "2013-08-15T15:23:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14610#issuecomment-177217",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:12" align="right">comment:12</div>

This `AssertionError` should be changed into a `ValueError` or `TypeError`:

```
sage: RootSystem(['D',2]).root_lattice().simple_root(1).is_short_root() 
---------------------------------------------------------------------------
AssertionError                            Traceback (most recent call last)
<ipython-input-25-e961de76571c> in <module>()
----> 1 RootSystem(['D',Integer(2)]).root_lattice().simple_root(Integer(1)).is_short_root()

/mazur/release/merger/sage-5.12.beta1/local/lib/python2.7/site-packages/sage/combinat/root_system/root_lattice_realizations.pyc in is_short_root(self)
   3290             """
   3291             ct = self.parent().cartan_type()
-> 3292             assert ct.is_irreducible()
   3293             if not ct.is_finite():
   3294                 raise NotImplementedError, "Implemented only for irreducible finite root systems"

AssertionError: 
```



---

archive/issue_events_205054.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-15T15:23:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205054"
}
```



---

archive/issue_events_205055.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-15T15:23:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205055"
}
```



---

archive/issue_comments_177218.json:
```json
{
    "body": "Attachment: **[trac_14610-LSenergy-ms.patch.gz](https://github.com/sagemath/sage/files/ticket14610/trac_14610-LSenergy-ms.patch.gz)**",
    "created_at": "2013-08-15T16:14:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14610#issuecomment-177218",
    "user": "https://github.com/anneschilling"
}
```

Attachment: **[trac_14610-LSenergy-ms.patch.gz](https://github.com/sagemath/sage/files/ticket14610/trac_14610-LSenergy-ms.patch.gz)**



---

archive/issue_comments_177219.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nFixed!",
    "created_at": "2013-08-15T16:15:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14610#issuecomment-177219",
    "user": "https://github.com/anneschilling"
}
```

<div id="comment:13" align="right">comment:13</div>

Fixed!



---

archive/issue_events_205056.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T09:46:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205056"
}
```



---

archive/issue_events_205057.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T09:46:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205057"
}
```



---

archive/issue_comments_177220.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nHere is the new output for the above situation:\n\n```\nsage: RootSystem(['D',2]).root_lattice().simple_root(1).is_short_root()\n---------------------------------------------------------------------------\nValueError                                Traceback (most recent call last)\n<ipython-input-1-31128c2ac44d> in <module>()\n----> 1 RootSystem(['D',Integer(2)]).root_lattice().simple_root(Integer(1)).is_short_root()\n\n/Users/salis1bt/sage-5.12.beta0/local/lib/python2.7/site-packages/sage/combinat/root_system/root_lattice_realizations.pyc in is_short_root(self)\n   3291             ct = self.parent().cartan_type()\n   3292             if not ct.is_irreducible():\n-> 3293                 raise ValueError, \"Cartan type needs to be irreducible!\"\n   3294             if not ct.is_finite():\n   3295                 raise NotImplementedError, \"Implemented only for irreducible finite root systems\"\n\nValueError: Cartan type needs to be irreducible!\n```\nAll looks good to me.\n\n```\n----------------------------------------------------------------------\nAll tests passed!\n----------------------------------------------------------------------\nTotal time for all tests: 1835.0 seconds\n    cpu time: 5844.4 seconds\n    cumulative wall time: 7022.8 seconds\n```",
    "created_at": "2013-08-16T16:46:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14610#issuecomment-177220",
    "user": "https://github.com/bsalisbury1"
}
```

<div id="comment:15" align="right">comment:15</div>

Here is the new output for the above situation:

```
sage: RootSystem(['D',2]).root_lattice().simple_root(1).is_short_root()
---------------------------------------------------------------------------
ValueError                                Traceback (most recent call last)
<ipython-input-1-31128c2ac44d> in <module>()
----> 1 RootSystem(['D',Integer(2)]).root_lattice().simple_root(Integer(1)).is_short_root()

/Users/salis1bt/sage-5.12.beta0/local/lib/python2.7/site-packages/sage/combinat/root_system/root_lattice_realizations.pyc in is_short_root(self)
   3291             ct = self.parent().cartan_type()
   3292             if not ct.is_irreducible():
-> 3293                 raise ValueError, "Cartan type needs to be irreducible!"
   3294             if not ct.is_finite():
   3295                 raise NotImplementedError, "Implemented only for irreducible finite root systems"

ValueError: Cartan type needs to be irreducible!
```
All looks good to me.

```
----------------------------------------------------------------------
All tests passed!
----------------------------------------------------------------------
Total time for all tests: 1835.0 seconds
    cpu time: 5844.4 seconds
    cumulative wall time: 7022.8 seconds
```



---

archive/issue_events_205058.json:
```json
{
    "actor": "https://github.com/bsalisbury1",
    "created_at": "2013-08-16T16:46:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205058"
}
```



---

archive/issue_events_205059.json:
```json
{
    "actor": "https://github.com/bsalisbury1",
    "created_at": "2013-08-16T16:46:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205059"
}
```



---

archive/issue_comments_177221.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">comment:16</div>\n\nThank you!\n\nAnne",
    "created_at": "2013-08-16T18:58:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14610#issuecomment-177221",
    "user": "https://github.com/anneschilling"
}
```

<div id="comment:16" align="right">comment:16</div>

Thank you!

Anne



---

archive/issue_events_205060.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T21:15:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205060"
}
```



---

archive/issue_events_205061.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T21:15:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14610#event-205061"
}
```



---

archive/issue_comments_177222.json:
```json
{
    "body": "Merged: **sage-5.12.beta1**",
    "created_at": "2013-08-16T21:15:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14610",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14610#issuecomment-177222",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.12.beta1**

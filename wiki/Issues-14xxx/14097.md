# Issue 14097: Fix broken 'sage -valgrind' interaction with ipython

archive/issues_013893.json:
```json
{
    "body": "In 5.6 Sage fails to start under valgrind due to interaction with ipython. The attached patch fixes the problem.\n\n**Assignee:** @jasongrout\n\n**CC:**  @vbraun\n\n**Keywords:** valgrind, ipython\n\n**Author:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/14097\n\n",
    "closed_at": "2014-10-13T15:49:27Z",
    "created_at": "2013-02-11T20:02:38Z",
    "labels": [
        "component: misc",
        "bug",
        "duplicate"
    ],
    "title": "Fix broken 'sage -valgrind' interaction with ipython",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/14097",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
In 5.6 Sage fails to start under valgrind due to interaction with ipython. The attached patch fixes the problem.

**Assignee:** @jasongrout

**CC:**  @vbraun

**Keywords:** valgrind, ipython

**Author:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/14097





---

archive/attachments_019672.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "sage-5.6-fix_sage-valgrind_vs_ipython_issue.patch",
    "asset_url": "tarball://root/attachments/ticket14097/sage-5.6-fix_sage-valgrind_vs_ipython_issue.patch",
    "created_at": "2013-08-13T15:35:53Z",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket14097/sage-5.6-fix_sage-valgrind_vs_ipython_issue.patch",
    "user": "https://github.com/jdemeyer"
}
```



---

archive/issue_comments_169994.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [sage-5.6-fix_sage-valgrind_vs_ipython_issue.patch](https://github.com/sagemath/sage/files/ticket14097/sage-5.6-fix_sage-valgrind_vs_ipython_issue.patch)",
    "created_at": "2013-08-13T15:35:53Z",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14097#issuecomment-169994",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'></a>
**Attachment:** [sage-5.6-fix_sage-valgrind_vs_ipython_issue.patch](https://github.com/sagemath/sage/files/ticket14097/sage-5.6-fix_sage-valgrind_vs_ipython_issue.patch)



---

archive/issue_events_122145.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14097#event-122145"
}
```



---

archive/issue_events_122146.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14097#event-122146"
}
```



---

archive/issue_comments_169995.json:
```json
{
    "body": "<a id='comment:2'></a>\nI tried the above fix and it did not change anything for me on top of Sage 5.11 unfortunately.\n\nRunning a command equivalent to what sage-valgrind does (without the patch here) from a sage shell (./sage -sh) *just works* so something really fishy is going on here...",
    "created_at": "2013-10-07T22:30:41Z",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14097#issuecomment-169995",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpflori"
}
```

<a id='comment:2'></a>
I tried the above fix and it did not change anything for me on top of Sage 5.11 unfortunately.

Running a command equivalent to what sage-valgrind does (without the patch here) from a sage shell (./sage -sh) *just works* so something really fishy is going on here...



---

archive/issue_comments_169996.json:
```json
{
    "body": "<a id='comment:3'></a>\nNote that valgrind-3.9.0 is out.",
    "created_at": "2013-12-23T19:25:09Z",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14097#issuecomment-169996",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpflori"
}
```

<a id='comment:3'></a>
Note that valgrind-3.9.0 is out.



---

archive/issue_events_122147.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14097#event-122147"
}
```



---

archive/issue_events_122148.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14097#event-122148"
}
```



---

archive/issue_events_122149.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14097#event-122149"
}
```



---

archive/issue_events_122150.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14097#event-122150"
}
```



---

archive/issue_events_122151.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14097#event-122151"
}
```



---

archive/issue_events_122152.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14097#event-122152"
}
```



---

archive/issue_events_122153.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-02T19:17:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14097#event-122153"
}
```



---

archive/issue_comments_169997.json:
```json
{
    "body": "<a id='comment:7'></a>\nI've added this to #15586, propose to close as duplicate",
    "created_at": "2014-10-02T19:17:19Z",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14097#issuecomment-169997",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:7'></a>
I've added this to #15586, propose to close as duplicate



---

archive/issue_events_122154.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/jpflori",
    "created_at": "2014-10-08T05:18:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14097#event-122154"
}
```



---

archive/issue_comments_169998.json:
```json
{
    "body": "<a id='comment:8'></a>\nI agree, let's close this one.",
    "created_at": "2014-10-08T05:18:01Z",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14097#issuecomment-169998",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpflori"
}
```

<a id='comment:8'></a>
I agree, let's close this one.



---

archive/issue_events_122155.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/jpflori",
    "created_at": "2014-10-08T05:18:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14097#event-122155"
}
```



---

archive/issue_events_122156.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/jpflori",
    "created_at": "2014-10-08T05:18:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14097#event-122156"
}
```



---

archive/issue_events_122157.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-13T15:49:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14097#event-122157"
}
```



---

archive/issue_events_122158.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-13T15:49:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14097#event-122158"
}
```



---

archive/issue_events_122159.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-13T15:49:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14097",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14097#event-122159"
}
```

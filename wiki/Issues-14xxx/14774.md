# Issue 14774: Simion-Schmidt map for permutations

archive/issues_014570.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-sage-combinat"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nAdds the Simion-Schmidt map to the permutation class.\n\nThis map implements the Simion-Schmidt map which sends an arbitrary permutation to a pattern avoiding permutation, where the permutation pattern is one of four length-three patterns.  This method also implements the bijection between (for example) ``[1,2,3]``- and ``[1,3,2]``-avoiding permutations.\n\nCC:  @tscrim\n\nComponent: **combinatorics**\n\nKeywords: **patterns days49**\n\nAuthor: **Tom Denton**\n\nReviewer: **Chris Berg**\n\nMerged: **sage-5.11.beta3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14774_\n\n",
    "closed_at": "2013-06-20T21:37:40Z",
    "created_at": "2013-06-19T10:37:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.11",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Simion-Schmidt map for permutations",
    "type": "issue",
    "updated_at": "2013-06-20T21:37:40Z",
    "url": "https://github.com/sagemath/sage/issues/14774",
    "user": "https://github.com/sdenton4"
}
```
<div id="comment:0"></div>

Adds the Simion-Schmidt map to the permutation class.

This map implements the Simion-Schmidt map which sends an arbitrary permutation to a pattern avoiding permutation, where the permutation pattern is one of four length-three patterns.  This method also implements the bijection between (for example) ``[1,2,3]``- and ``[1,3,2]``-avoiding permutations.

CC:  @tscrim

Component: **combinatorics**

Keywords: **patterns days49**

Author: **Tom Denton**

Reviewer: **Chris Berg**

Merged: **sage-5.11.beta3**

_Issue created by migration from https://trac.sagemath.org/ticket/14774_





---

archive/issue_events_207904.json:
```json
{
    "actor": "https://github.com/sdenton4",
    "created_at": "2013-06-19T10:37:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14774#event-207904"
}
```



---

archive/issue_events_207905.json:
```json
{
    "actor": "https://github.com/sdenton4",
    "created_at": "2013-06-19T10:37:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14774#event-207905"
}
```



---

archive/issue_events_207906.json:
```json
{
    "actor": "https://github.com/sdenton4",
    "created_at": "2013-06-19T10:37:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14774#event-207906"
}
```



---

archive/issue_events_207907.json:
```json
{
    "actor": "https://github.com/sdenton4",
    "created_at": "2013-06-19T10:37:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14774#event-207907"
}
```



---

archive/issue_events_207908.json:
```json
{
    "actor": "https://github.com/sagetrac-sage-combinat",
    "created_at": "2013-06-19T10:37:14Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "subject": "https://github.com/sdenton4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14774#event-207908"
}
```



---

archive/issue_events_207909.json:
```json
{
    "actor": "https://github.com/sdenton4",
    "created_at": "2013-06-19T10:50:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14774#event-207909"
}
```



---

archive/issue_events_207910.json:
```json
{
    "actor": "https://github.com/sdenton4",
    "created_at": "2013-06-19T12:10:11Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "title_is": "Simion-Schmidt map for permutations",
    "title_was": "Simion-Schmidt map fro permutations",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14774#event-207910"
}
```



---

archive/issue_comments_180255.json:
```json
{
    "body": "Reviewer: **tscrim**",
    "created_at": "2013-06-19T12:10:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14774#issuecomment-180255",
    "user": "https://github.com/sdenton4"
}
```

Reviewer: **tscrim**



---

archive/issue_comments_180256.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThe patch is blank.",
    "created_at": "2013-06-19T12:56:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14774#issuecomment-180256",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:4" align="right">comment:4</div>

The patch is blank.



---

archive/issue_comments_180257.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nWell that was dumb; I thought the size of the file was kinda small.  The patch should now be non-empty, all other issues aside....",
    "created_at": "2013-06-19T16:31:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14774#issuecomment-180257",
    "user": "https://github.com/sdenton4"
}
```

<div id="comment:5" align="right">comment:5</div>

Well that was dumb; I thought the size of the file was kinda small.  The patch should now be non-empty, all other issues aside....



---

archive/issue_comments_180258.json:
```json
{
    "body": "Changed reviewer from **tscrim** to **cberg**",
    "created_at": "2013-06-19T21:09:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14774#issuecomment-180258",
    "user": "https://github.com/sdenton4"
}
```

Changed reviewer from **tscrim** to **cberg**



---

archive/issue_comments_180259.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nApply: trac_14774-simion-schmidt-td.patch",
    "created_at": "2013-06-19T21:11:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14774#issuecomment-180259",
    "user": "https://github.com/sagetrac-chrisjamesberg"
}
```

<div id="comment:7" align="right">comment:7</div>

Apply: trac_14774-simion-schmidt-td.patch



---

archive/issue_comments_180260.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n Adds the Simion-Schmidt map to the permutation class.\n \n-This map implements the Simion-Schmidt map which sends an arbitrary permutation to a pattern avoiding permutation, where the permutation pattern is one of four length-three patterns.  This method also implements the bijection between (for example) ``[1,2,3]``- and ``[2,3,1]``-avoiding permutations.\n+This map implements the Simion-Schmidt map which sends an arbitrary permutation to a pattern avoiding permutation, where the permutation pattern is one of four length-three patterns.  This method also implements the bijection between (for example) ``[1,2,3]``- and ``[1,3,2]``-avoiding permutations.\n``````\n",
    "created_at": "2013-06-19T21:13:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14774#issuecomment-180260",
    "user": "https://github.com/sdenton4"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 Adds the Simion-Schmidt map to the permutation class.
 
-This map implements the Simion-Schmidt map which sends an arbitrary permutation to a pattern avoiding permutation, where the permutation pattern is one of four length-three patterns.  This method also implements the bijection between (for example) ``[1,2,3]``- and ``[2,3,1]``-avoiding permutations.
+This map implements the Simion-Schmidt map which sends an arbitrary permutation to a pattern avoiding permutation, where the permutation pattern is one of four length-three patterns.  This method also implements the bijection between (for example) ``[1,2,3]``- and ``[1,3,2]``-avoiding permutations.
``````




---

archive/issue_comments_180261.json:
```json
{
    "body": "Changed author from **sdenton** to **tom denton**",
    "created_at": "2013-06-19T21:14:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14774#issuecomment-180261",
    "user": "https://github.com/sdenton4"
}
```

Changed author from **sdenton** to **tom denton**



---

archive/issue_events_207911.json:
```json
{
    "actor": "https://github.com/sagetrac-chrisjamesberg",
    "created_at": "2013-06-19T21:15:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14774#event-207911"
}
```



---

archive/issue_events_207912.json:
```json
{
    "actor": "https://github.com/sagetrac-chrisjamesberg",
    "created_at": "2013-06-19T21:15:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14774#event-207912"
}
```



---

archive/issue_comments_180262.json:
```json
{
    "body": "Changed author from **tom denton** to **Tom Denton**",
    "created_at": "2013-06-20T06:53:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14774#issuecomment-180262",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **tom denton** to **Tom Denton**



---

archive/issue_comments_180263.json:
```json
{
    "body": "Changed reviewer from **cberg** to **Chris Berg**",
    "created_at": "2013-06-20T06:53:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14774#issuecomment-180263",
    "user": "https://github.com/jdemeyer"
}
```

Changed reviewer from **cberg** to **Chris Berg**



---

archive/issue_comments_180264.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nThe patch needs a proper commit message (use `hg qrefresh -e` to change the message).",
    "created_at": "2013-06-20T07:15:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14774#issuecomment-180264",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:12" align="right">comment:12</div>

The patch needs a proper commit message (use `hg qrefresh -e` to change the message).



---

archive/issue_events_207913.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-20T07:17:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14774#event-207913"
}
```



---

archive/issue_events_207914.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-20T07:17:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14774#event-207914"
}
```



---

archive/issue_comments_180265.json:
```json
{
    "body": "Simion Schmidt map",
    "created_at": "2013-06-20T08:53:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14774#issuecomment-180265",
    "user": "https://github.com/sdenton4"
}
```

Simion Schmidt map



---

archive/issue_comments_180266.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nAttachment: **[trac_14774-simion-schmidt-td.patch.gz](https://github.com/sagemath/sage/files/ticket14774/trac_14774-simion-schmidt-td.patch.gz)**\n\nAdded a commit message to the patch.",
    "created_at": "2013-06-20T08:53:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14774#issuecomment-180266",
    "user": "https://github.com/sdenton4"
}
```

<div id="comment:14" align="right">comment:14</div>

Attachment: **[trac_14774-simion-schmidt-td.patch.gz](https://github.com/sagemath/sage/files/ticket14774/trac_14774-simion-schmidt-td.patch.gz)**

Added a commit message to the patch.



---

archive/issue_events_207915.json:
```json
{
    "actor": "https://github.com/sagetrac-chrisjamesberg",
    "created_at": "2013-06-20T10:03:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14774#event-207915"
}
```



---

archive/issue_events_207916.json:
```json
{
    "actor": "https://github.com/sagetrac-chrisjamesberg",
    "created_at": "2013-06-20T10:03:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14774#event-207916"
}
```



---

archive/issue_events_207917.json:
```json
{
    "actor": "https://github.com/sagetrac-chrisjamesberg",
    "created_at": "2013-06-20T10:03:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14774#event-207917"
}
```



---

archive/issue_events_207918.json:
```json
{
    "actor": "https://github.com/sagetrac-chrisjamesberg",
    "created_at": "2013-06-20T10:03:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14774#event-207918"
}
```



---

archive/issue_comments_180267.json:
```json
{
    "body": "Merged: **sage-5.11.beta3**",
    "created_at": "2013-06-20T21:37:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14774#issuecomment-180267",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.11.beta3**



---

archive/issue_events_207919.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-20T21:37:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14774#event-207919"
}
```



---

archive/issue_events_207920.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-20T21:37:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14774",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14774#event-207920"
}
```

# Issue 14309: IPython breaks a couple of doctests if Python didn't build `_ctypes`

archive/issues_014105.json:
```json
{
    "body": "See [comment:ticket:14265:26] ff.\n\n(Previously, e.g. in Sage 5.2, and probably later releases as well, this didn't cause any problems.)\n\nI've so far only seen this on Solaris SPARC (with GCC 4.7.0), presumably just because it's the only configuration where Python currently fails to build `_ctypes` (which in my case is a Python bug, to be fixed on another ticket).\n\n\n**Assignee:** mvngu\n\n**Keywords:** Got WARNING %gui event loop SunOS Solaris SPARC GCC 4.7.x\n\n**Reviewer:** Michael Orlitzky\n\nIssue created by migration from https://trac.sagemath.org/ticket/14309\n\n",
    "closed_at": "2021-10-04T23:44:13Z",
    "created_at": "2013-03-19T10:49:54Z",
    "labels": [
        "component: doctest coverage",
        "bug",
        "invalid"
    ],
    "title": "IPython breaks a couple of doctests if Python didn't build `_ctypes`",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/14309",
    "user": "https://github.com/nexttime"
}
```
See [comment:ticket:14265:26] ff.

(Previously, e.g. in Sage 5.2, and probably later releases as well, this didn't cause any problems.)

I've so far only seen this on Solaris SPARC (with GCC 4.7.0), presumably just because it's the only configuration where Python currently fails to build `_ctypes` (which in my case is a Python bug, to be fixed on another ticket).


**Assignee:** mvngu

**Keywords:** Got WARNING %gui event loop SunOS Solaris SPARC GCC 4.7.x

**Reviewer:** Michael Orlitzky

Issue created by migration from https://trac.sagemath.org/ticket/14309





---

archive/issue_comments_174069.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis is an IPython bug imho. It has been introduced in https://github.com/ipython/ipython/issues/1394/",
    "created_at": "2013-03-24T19:00:58Z",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14309#issuecomment-174069",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:1'></a>
This is an IPython bug imho. It has been introduced in https://github.com/ipython/ipython/issues/1394/



---

archive/issue_comments_174070.json:
```json
{
    "body": "<a id='comment:2'></a>\nAlso, why is it a problem? We now check in the Python spkg that `_ctypes` can be imported.",
    "created_at": "2013-04-11T13:12:48Z",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14309#issuecomment-174070",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'></a>
Also, why is it a problem? We now check in the Python spkg that `_ctypes` can be imported.



---

archive/issue_events_124717.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14309#event-124717"
}
```



---

archive/issue_events_124718.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14309#event-124718"
}
```



---

archive/issue_events_124719.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14309#event-124719"
}
```



---

archive/issue_events_124720.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14309#event-124720"
}
```



---

archive/issue_events_124721.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14309#event-124721"
}
```



---

archive/issue_events_124722.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14309#event-124722"
}
```



---

archive/issue_events_124723.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14309#event-124723"
}
```



---

archive/issue_events_124724.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14309#event-124724"
}
```



---

archive/issue_events_124725.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-07T08:03:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14309#event-124725"
}
```



---

archive/issue_events_124726.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-07T08:03:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14309#event-124726"
}
```



---

archive/issue_comments_174071.json:
```json
{
    "body": "<a id='comment:7'></a>\noutdated, should close",
    "created_at": "2021-09-07T08:03:56Z",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14309#issuecomment-174071",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:7'></a>
outdated, should close



---

archive/issue_events_124727.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2021-10-04T23:15:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14309#event-124727"
}
```



---

archive/issue_events_124728.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2021-10-04T23:15:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14309#event-124728"
}
```



---

archive/issue_comments_174072.json:
```json
{
    "body": "<a id='comment:8'></a>\npython3's `spkg-configure.m4` checks for ctypes.",
    "created_at": "2021-10-04T23:15:30Z",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14309#issuecomment-174072",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:8'></a>
python3's `spkg-configure.m4` checks for ctypes.



---

archive/issue_comments_174073.json:
```json
{
    "body": "**Reviewer:** Michael Orlitzky",
    "created_at": "2021-10-04T23:15:30Z",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14309#issuecomment-174073",
    "user": "https://github.com/orlitzky"
}
```

**Reviewer:** Michael Orlitzky



---

archive/issue_events_124729.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T23:44:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14309#event-124729"
}
```



---

archive/issue_events_124730.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T23:44:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14309",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14309#event-124730"
}
```

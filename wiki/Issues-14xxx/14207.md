# Issue 14207: Document usage of peflags on Cygwin

archive/issues_014003.json:
```json
{
    "body": "By default the max heap memory available to processes is quite low on Cygwin (about 500MB) so that doctests requiring more memory than that will fail (typically doctests using PARI which doubles its memory when its not sufficient anymore, so you basically jump from 256MB to 512MB and boom).\n\n**CC:**  @dimpase @kcrisman\n\n**Keywords:** cygwin, peflags, cygwin32, days101\n\nIssue created by migration from https://trac.sagemath.org/ticket/14207\n\n",
    "closed_at": "2019-06-15T21:40:49Z",
    "created_at": "2013-03-01T10:33:47Z",
    "labels": [
        "component: porting: cygwin",
        "minor",
        "enhancement",
        "worksforme",
        "invalid"
    ],
    "title": "Document usage of peflags on Cygwin",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/14207",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpflori"
}
```
By default the max heap memory available to processes is quite low on Cygwin (about 500MB) so that doctests requiring more memory than that will fail (typically doctests using PARI which doubles its memory when its not sufficient anymore, so you basically jump from 256MB to 512MB and boom).

**CC:**  @dimpase @kcrisman

**Keywords:** cygwin, peflags, cygwin32, days101

Issue created by migration from https://trac.sagemath.org/ticket/14207





---

archive/issue_comments_171945.json:
```json
{
    "body": "<a id='comment:1'></a>\nWhy \"document\" and not \"fix\"?",
    "created_at": "2013-03-06T07:44:57Z",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14207#issuecomment-171945",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'></a>
Why "document" and not "fix"?



---

archive/issue_events_123377.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14207#event-123377"
}
```



---

archive/issue_events_123378.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14207#event-123378"
}
```



---

archive/issue_events_123379.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14207#event-123379"
}
```



---

archive/issue_events_123380.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14207#event-123380"
}
```



---

archive/issue_events_123381.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14207#event-123381"
}
```



---

archive/issue_events_123382.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14207#event-123382"
}
```



---

archive/issue_events_123383.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14207#event-123383"
}
```



---

archive/issue_events_123384.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14207#event-123384"
}
```



---

archive/issue_comments_171946.json:
```json
{
    "body": "<a id='comment:6'></a>\nThis isn't really an issue on Cygwin 64.  It is more of an issue on 32-bit Windows where addressing is much more constrained.",
    "created_at": "2017-04-13T10:27:52Z",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14207#issuecomment-171946",
    "user": "https://github.com/embray"
}
```

<a id='comment:6'></a>
This isn't really an issue on Cygwin 64.  It is more of an issue on 32-bit Windows where addressing is much more constrained.



---

archive/issue_events_123385.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-04-13T10:27:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14207#event-123385"
}
```



---

archive/issue_events_123386.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-04-13T10:27:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "label": "wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14207#event-123386"
}
```



---

archive/issue_comments_171947.json:
```json
{
    "body": "**Changing keywords** from \"cygwin peflags\" to \"cygwin, peflags, cygwin32\".",
    "created_at": "2017-04-18T09:00:28Z",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14207#issuecomment-171947",
    "user": "https://github.com/embray"
}
```

**Changing keywords** from "cygwin peflags" to "cygwin, peflags, cygwin32".



---

archive/issue_events_123387.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-06-15T21:40:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14207#event-123387"
}
```



---

archive/issue_events_123388.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-06-15T21:40:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "label": "worksforme",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14207#event-123388"
}
```



---

archive/issue_comments_171948.json:
```json
{
    "body": "<a id='comment:8'></a>\nI think issues like this with PARI have been mostly fixed, or at least worked around (e.g. #20463).  It might still be a problem on 32-bit Cygwin but that is not likely to ever work.",
    "created_at": "2019-06-15T21:40:49Z",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14207#issuecomment-171948",
    "user": "https://github.com/embray"
}
```

<a id='comment:8'></a>
I think issues like this with PARI have been mostly fixed, or at least worked around (e.g. #20463).  It might still be a problem on 32-bit Cygwin but that is not likely to ever work.



---

archive/issue_events_123389.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-06-15T21:40:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "label": "wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14207#event-123389"
}
```



---

archive/issue_comments_171949.json:
```json
{
    "body": "**Changing keywords** from \"cygwin, peflags, cygwin32\" to \"cygwin, peflags, cygwin32, days101\".",
    "created_at": "2019-06-16T06:39:58Z",
    "issue": "https://github.com/sagemath/sage/issues/14207",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14207#issuecomment-171949",
    "user": "https://github.com/slel"
}
```

**Changing keywords** from "cygwin, peflags, cygwin32" to "cygwin, peflags, cygwin32, days101".

# Issue 14381: Split m4 spkg from (big) autotools one

archive/issues_014177.json:
```json
{
    "body": "Even though we don't make m4 spkg standard yet, I think it's a good idea to split it from the big autotools one.\n\nAnd as it's a Sage prereq on all platform and it may happen that the usually installed system-wide one can be broken, it will ease things to have it as a separate spkg for people who want to install a recent m4 through m4, or for us if one day we decide to make it standard.\n\n\n**CC:**  jdmeyer\n\n**Keywords:** m4 spkg\n\n**Reviewer:** Michael Orlitzky\n\nIssue created by migration from https://trac.sagemath.org/ticket/14381\n\n",
    "closed_at": "2021-10-04T23:44:13Z",
    "created_at": "2013-03-29T10:42:51Z",
    "labels": [
        "component: packages: optional",
        "minor",
        "enhancement",
        "invalid"
    ],
    "title": "Split m4 spkg from (big) autotools one",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/14381",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpflori"
}
```
Even though we don't make m4 spkg standard yet, I think it's a good idea to split it from the big autotools one.

And as it's a Sage prereq on all platform and it may happen that the usually installed system-wide one can be broken, it will ease things to have it as a separate spkg for people who want to install a recent m4 through m4, or for us if one day we decide to make it standard.


**CC:**  jdmeyer

**Keywords:** m4 spkg

**Reviewer:** Michael Orlitzky

Issue created by migration from https://trac.sagemath.org/ticket/14381





---

archive/issue_comments_175393.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis could be helpful on Cygwin?",
    "created_at": "2013-03-29T15:43:33Z",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14381#issuecomment-175393",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:1'></a>
This could be helpful on Cygwin?



---

archive/issue_comments_175394.json:
```json
{
    "body": "<a id='comment:2'></a>\nReplying to [@kcrisman](#comment%3A1):\n> This could be helpful on Cygwin?\n\nI don't think so.\nNote that m4 is a prereq on every platform anyway.",
    "created_at": "2013-03-29T15:46:23Z",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14381#issuecomment-175394",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpflori"
}
```

<a id='comment:2'></a>
Replying to [@kcrisman](#comment%3A1):
> This could be helpful on Cygwin?

I don't think so.
Note that m4 is a prereq on every platform anyway.



---

archive/issue_events_125615.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125615"
}
```



---

archive/issue_events_125616.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125616"
}
```



---

archive/issue_events_125617.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125617"
}
```



---

archive/issue_events_125618.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125618"
}
```



---

archive/issue_events_125619.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125619"
}
```



---

archive/issue_events_125620.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125620"
}
```



---

archive/issue_events_125621.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125621"
}
```



---

archive/issue_events_125622.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125622"
}
```



---

archive/issue_comments_175395.json:
```json
{
    "body": "<a id='comment:7'></a>\nSetting spkg proposals that have not seen recent activity to \"sage-wishlist\".",
    "created_at": "2020-06-19T18:16:00Z",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14381#issuecomment-175395",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:7'></a>
Setting spkg proposals that have not seen recent activity to "sage-wishlist".



---

archive/issue_events_125623.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-19T18:16:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "needs info",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125623"
}
```



---

archive/issue_events_125624.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-19T18:16:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125624"
}
```



---

archive/issue_events_125625.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-19T18:16:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125625"
}
```



---

archive/issue_events_125626.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T04:05:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "needs info",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125626"
}
```



---

archive/issue_events_125627.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T04:05:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125627"
}
```



---

archive/issue_events_125628.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T04:05:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125628"
}
```



---

archive/issue_comments_175396.json:
```json
{
    "body": "<a id='comment:8'></a>\noutdated, should close",
    "created_at": "2021-08-26T04:05:49Z",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14381#issuecomment-175396",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:8'></a>
outdated, should close



---

archive/issue_events_125629.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2021-10-04T23:08:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125629"
}
```



---

archive/issue_events_125630.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2021-10-04T23:08:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125630"
}
```



---

archive/issue_comments_175397.json:
```json
{
    "body": "**Reviewer:** Michael Orlitzky",
    "created_at": "2021-10-04T23:08:29Z",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14381#issuecomment-175397",
    "user": "https://github.com/orlitzky"
}
```

**Reviewer:** Michael Orlitzky



---

archive/issue_comments_175398.json:
```json
{
    "body": "<a id='comment:9'></a>\nThere is no more autotools SPKG; it's a `_prereq`, and there are better ways to install this stuff besides.",
    "created_at": "2021-10-04T23:08:29Z",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14381#issuecomment-175398",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:9'></a>
There is no more autotools SPKG; it's a `_prereq`, and there are better ways to install this stuff besides.



---

archive/issue_comments_175399.json:
```json
{
    "body": "<a id='comment:10'></a>\nActually it's a `_bootstrap`, but yes",
    "created_at": "2021-10-04T23:16:06Z",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14381#issuecomment-175399",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:10'></a>
Actually it's a `_bootstrap`, but yes



---

archive/issue_events_125631.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T23:44:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125631"
}
```



---

archive/issue_events_125632.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T23:44:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-125632"
}
```

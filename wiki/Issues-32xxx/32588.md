# Issue 32588: giac 1.6.0.47p3 does not pass self-tests

archive/issues_032351.json:
```json
{
    "body": "giac 1.6.0.47p3 does not pass self-tests `chk_fhan2` and `chk_fhan9` on both 32bit and 64bit VM, see the various logs in attachment.\n\n\nDepends on #31563\n\n**CC:**  @frederichan-IMJPRG parisse\n\n**Keywords:** sdl\n\nIssue created by migration from https://trac.sagemath.org/ticket/32588\n\n",
    "created_at": "2021-09-29T22:43:34Z",
    "labels": [
        "component: packages: standard",
        "bug",
        "needs review",
        "invalid"
    ],
    "title": "giac 1.6.0.47p3 does not pass self-tests",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/32588",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```
giac 1.6.0.47p3 does not pass self-tests `chk_fhan2` and `chk_fhan9` on both 32bit and 64bit VM, see the various logs in attachment.


Depends on #31563

**CC:**  @frederichan-IMJPRG parisse

**Keywords:** sdl

Issue created by migration from https://trac.sagemath.org/ticket/32588





---

archive/attachments_022286.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "giac-1.6.0.47p3.log",
    "asset_url": "tarball://root/attachments/ticket32588/giac-1.6.0.47p3.log",
    "created_at": "2021-09-29T22:44:05Z",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket32588/giac-1.6.0.47p3.log",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```



---

archive/issue_comments_528249.json:
```json
{
    "body": "**Attachment:** [giac-1.6.0.47p3.log](https://github.com/sagemath/sage/files/ticket32588/giac-1.6.0.47p3.log)",
    "created_at": "2021-09-29T22:44:05Z",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32588#issuecomment-528249",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

**Attachment:** [giac-1.6.0.47p3.log](https://github.com/sagemath/sage/files/ticket32588/giac-1.6.0.47p3.log)



---

archive/attachments_022287.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "chk_fhan2.log",
    "asset_url": "tarball://root/attachments/ticket32588/chk_fhan2.log",
    "created_at": "2021-09-29T22:44:14Z",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket32588/chk_fhan2.log",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```



---

archive/issue_comments_528250.json:
```json
{
    "body": "**Attachment:** [chk_fhan2.log](https://github.com/sagemath/sage/files/ticket32588/chk_fhan2.log)",
    "created_at": "2021-09-29T22:44:14Z",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32588#issuecomment-528250",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

**Attachment:** [chk_fhan2.log](https://github.com/sagemath/sage/files/ticket32588/chk_fhan2.log)



---

archive/attachments_022288.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "chk_fhan9.log",
    "asset_url": "tarball://root/attachments/ticket32588/chk_fhan9.log",
    "created_at": "2021-09-29T22:46:45Z",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket32588/chk_fhan9.log",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```



---

archive/issue_comments_528251.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [chk_fhan9.log](https://github.com/sagemath/sage/files/ticket32588/chk_fhan9.log)",
    "created_at": "2021-09-29T22:46:45Z",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32588#issuecomment-528251",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

<a id='comment:1'></a>
**Attachment:** [chk_fhan9.log](https://github.com/sagemath/sage/files/ticket32588/chk_fhan9.log)



---

archive/issue_comments_528252.json:
```json
{
    "body": "**Changing keywords** from \"\" to \"sdl\".",
    "created_at": "2021-09-29T23:53:17Z",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32588#issuecomment-528252",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

**Changing keywords** from "" to "sdl".



---

archive/issue_events_294952.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T20:12:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32588#event-294952"
}
```



---

archive/issue_events_294953.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T20:12:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32588#event-294953"
}
```



---

archive/issue_comments_528253.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,2 +1,2 @@\n-giac 1.6.0.47p3 does not pass self-tests `chk_fhan2` and `chk_fhan9` on my 32bit VM, see the various logs in attachment.\n+giac 1.6.0.47p3 does not pass self-tests `chk_fhan2` and `chk_fhan9` on both 32bit and 64bit VM, see the various logs in attachment.\n \n``````\n",
    "created_at": "2022-01-02T09:56:18Z",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32588#issuecomment-528253",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,2 +1,2 @@
-giac 1.6.0.47p3 does not pass self-tests `chk_fhan2` and `chk_fhan9` on my 32bit VM, see the various logs in attachment.
+giac 1.6.0.47p3 does not pass self-tests `chk_fhan2` and `chk_fhan9` on both 32bit and 64bit VM, see the various logs in attachment.
 
``````




---

archive/issue_events_294954.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/tmonteil",
    "created_at": "2022-01-02T09:56:18Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "rename": {
        "from": "giac 1.6.0.47p3 does not pass self-tests on 32bit systems",
        "to": "giac 1.6.0.47p3 does not pass self-tests"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32588#event-294954"
}
```



---

archive/issue_comments_528254.json:
```json
{
    "body": "<a id='comment:5'></a>\nHave you tried the pending upgrade to giac 1.7.x? #31563",
    "created_at": "2022-01-02T20:48:28Z",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32588#issuecomment-528254",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:5'></a>
Have you tried the pending upgrade to giac 1.7.x? #31563



---

archive/issue_events_294955.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-02-06T20:23:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32588#event-294955"
}
```



---

archive/issue_comments_528255.json:
```json
{
    "body": "**Dependencies:** #31563",
    "created_at": "2022-02-06T20:23:15Z",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32588#issuecomment-528255",
    "user": "https://github.com/mkoeppe"
}
```

**Dependencies:** #31563



---

archive/issue_events_294956.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-02-06T20:23:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32588#event-294956"
}
```

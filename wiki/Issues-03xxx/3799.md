# Issue 3799: degree sequence should not be a single integer in graph_database

archive/issues_003799.json:
```json
{
    "assignees": [
        "https://github.com/rlmill"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis is part of Jason Grout's formatting of his database, but it should be translated to and from a sequence of integers for the user.\n\nCC:  @rlmill\n\nComponent: **graph theory**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3799_\n\n",
    "closed_at": "2008-09-29T23:29:05Z",
    "created_at": "2008-08-10T10:04:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "degree sequence should not be a single integer in graph_database",
    "type": "issue",
    "updated_at": "2008-09-29T23:29:05Z",
    "url": "https://github.com/sagemath/sage/issues/3799",
    "user": "https://github.com/rlmill"
}
```
<div id="comment:0"></div>

This is part of Jason Grout's formatting of his database, but it should be translated to and from a sequence of integers for the user.

CC:  @rlmill

Component: **graph theory**

_Issue created by migration from https://trac.sagemath.org/ticket/3799_





---

archive/issue_events_041691.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-08-10T10:04:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "milestone_number": null,
    "milestone_title": "sage-3.1.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3799#event-41691"
}
```



---

archive/issue_events_041692.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-08-10T10:04:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3799#event-41692"
}
```



---

archive/issue_events_041693.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-08-10T10:04:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3799#event-41693"
}
```



---

archive/issue_events_041694.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-08-10T10:04:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3799#event-41694"
}
```



---

archive/issue_events_041695.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-08-10T10:04:30Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "subject": "https://github.com/rlmill",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3799#event-41695"
}
```



---

archive/issue_comments_020943.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nthis patch inspired a full graph database re-write.  Coming soon!",
    "created_at": "2008-08-14T00:46:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3799#issuecomment-20943",
    "user": "https://github.com/sagetrac-ekirkman"
}
```

<div id="comment:1" align="right">comment:1</div>

this patch inspired a full graph database re-write.  Coming soon!



---

archive/issue_comments_020944.json:
```json
{
    "body": "Attachment: **[3799-databases.patch.gz](https://github.com/sagemath/sage/files/ticket3799/3799-databases.patch.gz)**",
    "created_at": "2008-09-22T10:39:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3799#issuecomment-20944",
    "user": "https://github.com/sagetrac-ekirkman"
}
```

Attachment: **[3799-databases.patch.gz](https://github.com/sagemath/sage/files/ticket3799/3799-databases.patch.gz)**



---

archive/issue_comments_020945.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis patch should be stable for review, and it includes some definite improvements.  There will be more database enhancements coming, but they'll deserve their own tickets.  Oops -- one more patch coming with doctest fixes...",
    "created_at": "2008-09-22T11:16:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3799#issuecomment-20945",
    "user": "https://github.com/sagetrac-ekirkman"
}
```

<div id="comment:2" align="right">comment:2</div>

This patch should be stable for review, and it includes some definite improvements.  There will be more database enhancements coming, but they'll deserve their own tickets.  Oops -- one more patch coming with doctest fixes...



---

archive/issue_comments_020946.json:
```json
{
    "body": "Attachment: **[3799-doctest.patch.gz](https://github.com/sagemath/sage/files/ticket3799/3799-doctest.patch.gz)**",
    "created_at": "2008-09-22T11:28:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3799#issuecomment-20946",
    "user": "https://github.com/sagetrac-ekirkman"
}
```

Attachment: **[3799-doctest.patch.gz](https://github.com/sagemath/sage/files/ticket3799/3799-doctest.patch.gz)**



---

archive/issue_events_041696.json:
```json
{
    "actor": "https://github.com/sagetrac-ekirkman",
    "created_at": "2008-09-22T11:28:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3799#event-41696"
}
```



---

archive/issue_comments_020947.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nDue to the new restrictions on incoming patches, this can't be merged until all of the functions added/modified have doctests. This includes `verify_type, verify_operator, construct_skeleton, _apply_format, _apply_plot,` and maybe others. For discussion, if nothing else, I challenge `No doctest is intentional.` in `database.py`.\n\nPerhaps this could be reworded/more specific (and you don't need to shout, I can hear you ;-) ):\n\n```\n# WORD ON THE STREET IS THAT SQLITE IS RETARDED ABOUT\n# *ALTER TABLE* COMMANDS... SO MEANWHILE WE ACCOMPLISH THIS\n# BY CREATING A TEMPORARY TABLE.  SUGGESTIONS FOR SPEEDUP ARE\n# WELCOME.  (OR JUST SEND A PATCH...)\n```\n\nWhy was the copyright statement removed from `graph_database.py`? Remember that these statements protect our code!",
    "created_at": "2008-09-22T15:45:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3799#issuecomment-20947",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:4" align="right">comment:4</div>

Due to the new restrictions on incoming patches, this can't be merged until all of the functions added/modified have doctests. This includes `verify_type, verify_operator, construct_skeleton, _apply_format, _apply_plot,` and maybe others. For discussion, if nothing else, I challenge `No doctest is intentional.` in `database.py`.

Perhaps this could be reworded/more specific (and you don't need to shout, I can hear you ;-) ):

```
# WORD ON THE STREET IS THAT SQLITE IS RETARDED ABOUT
# *ALTER TABLE* COMMANDS... SO MEANWHILE WE ACCOMPLISH THIS
# BY CREATING A TEMPORARY TABLE.  SUGGESTIONS FOR SPEEDUP ARE
# WELCOME.  (OR JUST SEND A PATCH...)
```

Why was the copyright statement removed from `graph_database.py`? Remember that these statements protect our code!



---

archive/issue_events_041697.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-09-22T15:45:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3799#event-41697"
}
```



---

archive/issue_events_041698.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-09-22T15:45:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3799#event-41698"
}
```



---

archive/issue_comments_020948.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nAttachment: **[3799-coverage.patch.gz](https://github.com/sagemath/sage/files/ticket3799/3799-coverage.patch.gz)**\n\nAttached another patch to address the problems mentioned.",
    "created_at": "2008-09-29T21:50:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3799#issuecomment-20948",
    "user": "https://github.com/sagetrac-ekirkman"
}
```

<div id="comment:5" align="right">comment:5</div>

Attachment: **[3799-coverage.patch.gz](https://github.com/sagemath/sage/files/ticket3799/3799-coverage.patch.gz)**

Attached another patch to address the problems mentioned.



---

archive/issue_events_041699.json:
```json
{
    "actor": "https://github.com/sagetrac-ekirkman",
    "created_at": "2008-09-29T21:50:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3799#event-41699"
}
```



---

archive/issue_events_041700.json:
```json
{
    "actor": "https://github.com/sagetrac-ekirkman",
    "created_at": "2008-09-29T21:50:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3799#event-41700"
}
```



---

archive/issue_comments_020949.json:
```json
{
    "body": "Attachment: **[3799-oops.patch.gz](https://github.com/sagemath/sage/files/ticket3799/3799-oops.patch.gz)**",
    "created_at": "2008-09-29T22:05:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3799#issuecomment-20949",
    "user": "https://github.com/sagetrac-ekirkman"
}
```

Attachment: **[3799-oops.patch.gz](https://github.com/sagemath/sage/files/ticket3799/3799-oops.patch.gz)**



---

archive/issue_events_041701.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-09-29T22:06:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3799#event-41701"
}
```



---

archive/issue_events_041702.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-09-29T22:06:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3799#event-41702"
}
```



---

archive/issue_comments_020950.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nLooks good to me.",
    "created_at": "2008-09-29T22:06:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3799#issuecomment-20950",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:6" align="right">comment:6</div>

Looks good to me.



---

archive/issue_events_041703.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-29T23:29:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3799#event-41703"
}
```



---

archive/issue_events_041704.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-29T23:29:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3799#event-41704"
}
```



---

archive/issue_comments_020951.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nMerged all four patches in Sage 3.1.3.alpha2",
    "created_at": "2008-09-29T23:29:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3799#issuecomment-20951",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:7" align="right">comment:7</div>

Merged all four patches in Sage 3.1.3.alpha2

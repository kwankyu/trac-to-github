# Issue 3795: increase coverage of sage/server/notebook/cell.py

archive/issues_003795.json:
```json
{
    "assignees": [
        "https://github.com/boothby"
    ],
    "body": "<div id=\"comment:0\"></div>\n\n\n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3795_\n\n",
    "closed_at": "2008-08-11T08:19:25Z",
    "created_at": "2008-08-09T15:13:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "increase coverage of sage/server/notebook/cell.py",
    "type": "issue",
    "updated_at": "2008-08-11T08:19:25Z",
    "url": "https://github.com/sagemath/sage/issues/3795",
    "user": "https://github.com/mwhansen"
}
```
<div id="comment:0"></div>



Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/3795_





---

archive/issue_events_041645.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-08-09T15:13:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3795",
    "milestone_number": null,
    "milestone_title": "sage-3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3795#event-41645"
}
```



---

archive/issue_events_041646.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-08-09T15:13:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3795",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3795#event-41646"
}
```



---

archive/issue_events_041647.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-08-09T15:13:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3795",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3795#event-41647"
}
```



---

archive/issue_events_041648.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-08-09T15:13:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3795",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3795#event-41648"
}
```



---

archive/issue_events_041649.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-08-09T15:13:45Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3795",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3795#event-41649"
}
```



---

archive/issue_comments_020926.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_3795.patch.gz](https://github.com/sagemath/sage/files/ticket3795/trac_3795.patch.gz)**",
    "created_at": "2008-08-09T15:16:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3795",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3795#issuecomment-20926",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_3795.patch.gz](https://github.com/sagemath/sage/files/ticket3795/trac_3795.patch.gz)**



---

archive/issue_events_041650.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-08-09T15:16:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3795",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3795#event-41650"
}
```



---

archive/issue_comments_020927.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nBefore:\n\n```\nSCORE cell.py: 8% (7 of 82)\n```\n\nAfter:\n\n```\nSCORE cell.py: 79% (67 of 84)\n```",
    "created_at": "2008-08-09T15:18:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3795",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3795#issuecomment-20927",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:2" align="right">comment:2</div>

Before:

```
SCORE cell.py: 8% (7 of 82)
```

After:

```
SCORE cell.py: 79% (67 of 84)
```



---

archive/issue_comments_020928.json:
```json
{
    "body": "Attachment: **[3795-mhansen-doctest-cell.patch.gz](https://github.com/sagemath/sage/files/ticket3795/3795-mhansen-doctest-cell.patch.gz)**",
    "created_at": "2008-08-10T23:15:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3795",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3795#issuecomment-20928",
    "user": "https://github.com/ncalexan"
}
```

Attachment: **[3795-mhansen-doctest-cell.patch.gz](https://github.com/sagemath/sage/files/ticket3795/3795-mhansen-doctest-cell.patch.gz)**



---

archive/issue_comments_020929.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI think mhansen's patch is excellent, but it broke if applied after #3568.  My updated patch depends on #3568 and makes the \"get a test notebook\" interface uniform using #3568.\n\nAll credit to mhansen.\n\nAttach only `3795-mhansen-doctest-cell.patch`, *after* #3568.",
    "created_at": "2008-08-10T23:19:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3795",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3795#issuecomment-20929",
    "user": "https://github.com/ncalexan"
}
```

<div id="comment:3" align="right">comment:3</div>

I think mhansen's patch is excellent, but it broke if applied after #3568.  My updated patch depends on #3568 and makes the "get a test notebook" interface uniform using #3568.

All credit to mhansen.

Attach only `3795-mhansen-doctest-cell.patch`, *after* #3568.



---

archive/issue_comments_020930.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nI think this can get \"positive review\" if it doctests.",
    "created_at": "2008-08-10T23:20:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3795",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3795#issuecomment-20930",
    "user": "https://github.com/ncalexan"
}
```

<div id="comment:4" align="right">comment:4</div>

I think this can get "positive review" if it doctests.



---

archive/issue_events_041651.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2008-08-10T23:20:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3795",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3795#event-41651"
}
```



---

archive/issue_events_041652.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2008-08-10T23:20:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3795",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3795#event-41652"
}
```



---

archive/issue_events_041653.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-11T08:19:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3795",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3795#event-41653"
}
```



---

archive/issue_events_041654.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-11T08:19:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3795",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3795#event-41654"
}
```



---

archive/issue_comments_020931.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nMerged trac_3795.patch in Sage 3.1.alpha1. Sorry Nick, due to #3568 causing massive doctest failures William will redo it for 3.1.alpha2. Hopefully someone will take your patch and rebase it on the new release.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-11T08:19:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3795",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3795#issuecomment-20931",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Merged trac_3795.patch in Sage 3.1.alpha1. Sorry Nick, due to #3568 causing massive doctest failures William will redo it for 3.1.alpha2. Hopefully someone will take your patch and rebase it on the new release.

Cheers,

Michael

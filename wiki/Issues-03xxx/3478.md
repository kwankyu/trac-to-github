# Issue 3478: clisp picks up wrong dvipdf

archive/issues_003478.json:
```json
{
    "body": "on my OSX 10.4.11 intel,\n\nthe clisp build was picking up dvipdf from the wrong place (fink or darwinports or something) and failed while trying to build its docs.\n\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/3478\n\n",
    "closed_at": "2009-02-16T01:53:37Z",
    "created_at": "2008-06-19T22:26:24Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "title": "clisp picks up wrong dvipdf",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3478",
    "user": "https://trac.sagemath.org/admin/accounts/users/dmharvey"
}
```
on my OSX 10.4.11 intel,

the clisp build was picking up dvipdf from the wrong place (fink or darwinports or something) and failed while trying to build its docs.


**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/3478





---

archive/issue_events_019159.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-26T06:37:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3478",
    "milestone": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3478#event-19159"
}
```



---

archive/issue_events_019160.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-16T01:53:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3478",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3478#event-19160"
}
```



---

archive/issue_comments_019717.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis is no longer an issue since Sage with MacPorts or Fink in $PATH will refuse to build. Also note that the libpng update at #5217 will fix the problem in a different way.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-16T01:53:37Z",
    "issue": "https://github.com/sagemath/sage/issues/3478",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3478#issuecomment-19717",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
This is no longer an issue since Sage with MacPorts or Fink in $PATH will refuse to build. Also note that the libpng update at #5217 will fix the problem in a different way.

Cheers,

Michael



---

archive/issue_events_019161.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-16T01:53:37Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3478",
    "milestone": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3478#event-19161"
}
```



---

archive/issue_events_019162.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-16T01:53:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3478",
    "milestone": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3478#event-19162"
}
```

# Issue 3344: small changes to documentation for erf, n

archive/issues_003344.json:
```json
{
    "body": "The attached patch should be pretty self-explanatory.  When I view the documention for erf, I see 'strong{only}' instead of the intended 'only'.  I also added a space to the math expression, to try to make it more readable.  Finally, in the documentation for n(-), I've emphasized that various of the arguments are optional.\n\n\n\n**Keywords:** erf, n, documentation\n\nIssue created by migration from https://trac.sagemath.org/ticket/3344\n\n",
    "closed_at": "2008-06-04T21:15:48Z",
    "created_at": "2008-05-31T21:52:40Z",
    "labels": [
        "component: misc",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.3",
    "title": "small changes to documentation for erf, n",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3344",
    "user": "https://github.com/jhpalmieri"
}
```
The attached patch should be pretty self-explanatory.  When I view the documention for erf, I see 'strong{only}' instead of the intended 'only'.  I also added a space to the math expression, to try to make it more readable.  Finally, in the documentation for n(-), I've emphasized that various of the arguments are optional.



**Keywords:** erf, n, documentation

Issue created by migration from https://trac.sagemath.org/ticket/3344





---

archive/issue_events_018015.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-31T21:53:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3344",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3344#event-18015"
}
```



---

archive/attachments_003404.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "3344.patch",
    "asset_url": "tarball://root/attachments/ticket3344/3344.patch",
    "created_at": "2008-05-31T21:53:42Z",
    "issue": "https://github.com/sagemath/sage/issues/3344",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket3344/3344.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```



---

archive/issue_comments_018610.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [3344.patch](https://github.com/sagemath/sage/files/ticket3344/3344.patch)",
    "created_at": "2008-05-31T21:53:42Z",
    "issue": "https://github.com/sagemath/sage/issues/3344",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3344#issuecomment-18610",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
**Attachment:** [3344.patch](https://github.com/sagemath/sage/files/ticket3344/3344.patch)



---

archive/issue_comments_018611.json:
```json
{
    "body": "<a id='comment:2'></a>\nstraight forward patch, positive review.",
    "created_at": "2008-06-03T14:31:05Z",
    "issue": "https://github.com/sagemath/sage/issues/3344",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3344#issuecomment-18611",
    "user": "https://github.com/malb"
}
```

<a id='comment:2'></a>
straight forward patch, positive review.



---

archive/issue_events_018016.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-06-03T14:31:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3344",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3344#event-18016"
}
```



---

archive/issue_events_018017.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-06-03T14:31:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3344",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3344#event-18017"
}
```



---

archive/issue_events_018018.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-04T21:15:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3344",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3344#event-18018"
}
```



---

archive/issue_events_018019.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-04T21:15:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3344",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3344#event-18019"
}
```



---

archive/issue_comments_018612.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.3.alpha1",
    "created_at": "2008-06-04T21:15:48Z",
    "issue": "https://github.com/sagemath/sage/issues/3344",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3344#issuecomment-18612",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.3.alpha1

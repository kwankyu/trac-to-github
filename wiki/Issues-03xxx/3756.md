# Issue 3756: finance -- fix a bug in fractional brownian motion random walks

archive/issues_003756.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nI stupidly forgot to sum the terms in the random walk.  This patch fixes that bug, updates the doctests, etc.   \n\nThis patch depends on #3755!\n\nComponent: **finance**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3756_\n\n",
    "closed_at": "2008-08-06T01:23:28Z",
    "created_at": "2008-08-02T01:17:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20finance",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "finance -- fix a bug in fractional brownian motion random walks",
    "type": "issue",
    "updated_at": "2008-08-06T01:23:28Z",
    "url": "https://github.com/sagemath/sage/issues/3756",
    "user": "https://github.com/williamstein"
}
```
<div id="comment:0"></div>

I stupidly forgot to sum the terms in the random walk.  This patch fixes that bug, updates the doctests, etc.   

This patch depends on #3755!

Component: **finance**

_Issue created by migration from https://trac.sagemath.org/ticket/3756_





---

archive/issue_events_041145.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-02T01:17:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3756",
    "milestone_number": null,
    "milestone_title": "sage-3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3756#event-41145"
}
```



---

archive/issue_events_041146.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-02T01:17:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3756",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20finance",
    "label_color": "0000ff",
    "label_name": "c: finance",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3756#event-41146"
}
```



---

archive/issue_events_041147.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-02T01:17:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3756",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3756#event-41147"
}
```



---

archive/issue_events_041148.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-02T01:17:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3756",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3756#event-41148"
}
```



---

archive/issue_events_041149.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-02T01:17:43Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3756",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3756#event-41149"
}
```



---

archive/issue_events_041150.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-02T01:17:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3756",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3756#event-41150"
}
```



---

archive/issue_events_041151.json:
```json
{
    "actor": "https://github.com/itolkov",
    "created_at": "2008-08-02T02:25:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3756",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3756#event-41151"
}
```



---

archive/issue_comments_020670.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[sage-3756.patch.gz](https://github.com/sagemath/sage/files/ticket3756/sage-3756.patch.gz)**\n\nLooks good. +1",
    "created_at": "2008-08-02T02:25:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3756",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3756#issuecomment-20670",
    "user": "https://github.com/itolkov"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[sage-3756.patch.gz](https://github.com/sagemath/sage/files/ticket3756/sage-3756.patch.gz)**

Looks good. +1



---

archive/issue_events_041152.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-06T01:23:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3756",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3756#event-41152"
}
```



---

archive/issue_events_041153.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-06T01:23:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3756",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3756#event-41153"
}
```



---

archive/issue_comments_020671.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged in Sage 3.1.alpha1",
    "created_at": "2008-08-06T01:23:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3756",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3756#issuecomment-20671",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged in Sage 3.1.alpha1

# Issue 3417: added new combinatorial functions for tableaux and crystals

archive/issues_003417.json:
```json
{
    "body": "I added a two new functions \npromotion and promotion_inverse\nfor rectangular tableaux.\n\nI also added a reflection operator\nfor the crystal library.\n\n**Assignee:** Mike Hansen\n\n**CC:**  sage-combinat\n\n**Keywords:** promotion, reflection\n\nIssue created by migration from https://trac.sagemath.org/ticket/3417\n\n",
    "closed_at": "2008-06-13T18:33:29Z",
    "created_at": "2008-06-13T18:29:47Z",
    "labels": [
        "component: combinatorics",
        "minor",
        "bug",
        "duplicate"
    ],
    "title": "added new combinatorial functions for tableaux and crystals",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3417",
    "user": "https://github.com/anneschilling"
}
```
I added a two new functions 
promotion and promotion_inverse
for rectangular tableaux.

I also added a reflection operator
for the crystal library.

**Assignee:** Mike Hansen

**CC:**  sage-combinat

**Keywords:** promotion, reflection

Issue created by migration from https://trac.sagemath.org/ticket/3417





---

archive/issue_events_018641.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-13T18:33:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3417",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3417#event-18641"
}
```



---

archive/issue_comments_019360.json:
```json
{
    "body": "<a id='comment:1'></a>\nHi Anne,\n\ntrac is a little tricky when pressing the submit button, so you did create a dupe. Don't worry about that, everybody did at some point.\n\nThis is a dupe of #3418.",
    "created_at": "2008-06-13T18:33:29Z",
    "issue": "https://github.com/sagemath/sage/issues/3417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3417#issuecomment-19360",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Hi Anne,

trac is a little tricky when pressing the submit button, so you did create a dupe. Don't worry about that, everybody did at some point.

This is a dupe of #3418.



---

archive/issue_events_018642.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-13T18:33:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3417",
    "label": "component: combinatorics",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3417#event-18642"
}
```



---

archive/issue_events_018643.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-13T18:33:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3417",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3417#event-18643"
}
```

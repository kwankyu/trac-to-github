# Issue 3396: new function in misc/latex.py: print_or_typeset

archive/issues_003396.json:
```json
{
    "body": "This patch defines a function in sage.misc.latex, print_or_typeset, which runs 'view' if in notebook mode with the typeset box, and runs 'print' otherwise.  See the discussion toward the end of this thread:\n\n[http://groups.google.com/group/sage-support/browse_frm/thread/9698e83a1d1b22ac](http://groups.google.com/group/sage-support/browse_frm/thread/9698e83a1d1b22ac)\n\n\n\n**Keywords:** latex, view, print\n\nIssue created by migration from https://trac.sagemath.org/ticket/3396\n\n",
    "closed_at": "2008-06-15T20:07:54Z",
    "created_at": "2008-06-11T04:31:55Z",
    "labels": [
        "component: misc",
        "minor",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.3",
    "title": "new function in misc/latex.py: print_or_typeset",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3396",
    "user": "https://github.com/jhpalmieri"
}
```
This patch defines a function in sage.misc.latex, print_or_typeset, which runs 'view' if in notebook mode with the typeset box, and runs 'print' otherwise.  See the discussion toward the end of this thread:

[http://groups.google.com/group/sage-support/browse_frm/thread/9698e83a1d1b22ac](http://groups.google.com/group/sage-support/browse_frm/thread/9698e83a1d1b22ac)



**Keywords:** latex, view, print

Issue created by migration from https://trac.sagemath.org/ticket/3396





---

archive/issue_comments_019105.json:
```json
{
    "body": "new function, print_or_typeset",
    "created_at": "2008-06-11T04:32:32Z",
    "issue": "https://github.com/sagemath/sage/issues/3396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3396#issuecomment-19105",
    "user": "https://github.com/jhpalmieri"
}
```

new function, print_or_typeset



---

archive/attachments_003472.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "3396.patch",
    "asset_url": "tarball://root/attachments/ticket3396/3396.patch",
    "created_at": "2008-06-11T15:05:15Z",
    "issue": "https://github.com/sagemath/sage/issues/3396",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket3396/3396.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/attachments_003473.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "sage-3396-doctests.patch",
    "asset_url": "tarball://root/attachments/ticket3396/sage-3396-doctests.patch",
    "created_at": "2008-06-11T15:05:15Z",
    "issue": "https://github.com/sagemath/sage/issues/3396",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket3396/sage-3396-doctests.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_019106.json:
```json
{
    "body": "**Attachment:** [3396.patch](https://github.com/sagemath/sage/files/ticket3396/3396.patch)\n\n**Attachment:** [sage-3396-doctests.patch](https://github.com/sagemath/sage/files/ticket3396/sage-3396-doctests.patch)\n\napply this after the other patch",
    "created_at": "2008-06-11T15:05:15Z",
    "issue": "https://github.com/sagemath/sage/issues/3396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3396#issuecomment-19106",
    "user": "https://github.com/williamstein"
}
```

**Attachment:** [3396.patch](https://github.com/sagemath/sage/files/ticket3396/3396.patch)

**Attachment:** [sage-3396-doctests.patch](https://github.com/sagemath/sage/files/ticket3396/sage-3396-doctests.patch)

apply this after the other patch



---

archive/issue_events_018457.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-06-11T15:05:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3396",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3396#event-18457"
}
```



---

archive/issue_events_018458.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-15T20:07:54Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3396",
    "milestone": "sage-3.0.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3396#event-18458"
}
```



---

archive/issue_events_018459.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-15T20:07:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3396",
    "milestone": "sage-3.0.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3396#event-18459"
}
```



---

archive/issue_events_018460.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-15T20:07:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3396",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3396#event-18460"
}
```



---

archive/issue_events_018461.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-15T20:07:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3396",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3396#event-18461"
}
```



---

archive/issue_comments_019107.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged both patch in Sage 3.0.3.rc0",
    "created_at": "2008-06-15T20:07:54Z",
    "issue": "https://github.com/sagemath/sage/issues/3396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3396#issuecomment-19107",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged both patch in Sage 3.0.3.rc0

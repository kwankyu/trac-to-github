# Issue 3352: slight inconsistency in the dsage documentation

archive/issues_003352.json:
```json
{
    "body": "In Section 16.2 of the Reference Manual, the command DSage for Distributed Sage is out of date; the new command seems to be dsage (all lower case).\n\nIssue created by migration from https://trac.sagemath.org/ticket/3352\n\n",
    "closed_at": "2010-02-02T06:21:08Z",
    "created_at": "2008-06-02T21:57:44Z",
    "labels": [
        "component: documentation",
        "minor",
        "bug",
        "invalid"
    ],
    "title": "slight inconsistency in the dsage documentation",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3352",
    "user": "https://trac.sagemath.org/admin/accounts/users/ljpk"
}
```
In Section 16.2 of the Reference Manual, the command DSage for Distributed Sage is out of date; the new command seems to be dsage (all lower case).

Issue created by migration from https://trac.sagemath.org/ticket/3352





---

archive/issue_events_018081.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-26T06:37:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3352",
    "milestone": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3352#event-18081"
}
```



---

archive/issue_events_018082.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-02-02T06:21:08Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3352",
    "milestone": "sage-4.3.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3352#event-18082"
}
```



---

archive/issue_comments_018676.json:
```json
{
    "body": "<a id='comment:2'></a>\nAs of Sage 4.3.1, ticket #7975 removed dsage from the standard spkg repository. I'm closing this ticket as invalid.",
    "created_at": "2010-02-02T06:21:08Z",
    "issue": "https://github.com/sagemath/sage/issues/3352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3352#issuecomment-18676",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:2'></a>
As of Sage 4.3.1, ticket #7975 removed dsage from the standard spkg repository. I'm closing this ticket as invalid.



---

archive/issue_events_018083.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-02-02T06:21:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3352",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3352#event-18083"
}
```

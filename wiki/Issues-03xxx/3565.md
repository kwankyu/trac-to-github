# Issue 3565: worksheet.py -- increase doctest coverage to 50%.

archive/issues_003565.json:
```json
{
    "body": "Before:\n\n```\nteragon-2:notebook was$ sage -coverage worksheet.py\n----------------------------------------------------------------------\nworksheet.py\nSCORE worksheet.py: 35% (68 of 193)\n```\n\n**Assignee:** failure\n\n**CC:**  @TimDumol @qed777\n\n**Reviewer:** David Roe\n\nIssue created by migration from https://trac.sagemath.org/ticket/3565\n\n",
    "closed_at": "2012-06-02T12:39:19Z",
    "created_at": "2008-07-06T19:05:49Z",
    "labels": [
        "component: doctest coverage",
        "enhancement",
        "invalid"
    ],
    "title": "worksheet.py -- increase doctest coverage to 50%.",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3565",
    "user": "https://github.com/williamstein"
}
```
Before:

```
teragon-2:notebook was$ sage -coverage worksheet.py
----------------------------------------------------------------------
worksheet.py
SCORE worksheet.py: 35% (68 of 193)
```

**Assignee:** failure

**CC:**  @TimDumol @qed777

**Reviewer:** David Roe

Issue created by migration from https://trac.sagemath.org/ticket/3565





---

archive/issue_comments_020293.json:
```json
{
    "body": "<a id='comment:1'></a>\nSee #3552. This continues that.",
    "created_at": "2008-07-06T19:07:22Z",
    "issue": "https://github.com/sagemath/sage/issues/3565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3565#issuecomment-20293",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
See #3552. This continues that.



---

archive/issue_comments_020294.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1,8 @@\n+Before:\n \n+```\n+teragon-2:notebook was$ sage -coverage worksheet.py\n+----------------------------------------------------------------------\n+worksheet.py\n+SCORE worksheet.py: 35% (68 of 193)\n+```\n``````\n",
    "created_at": "2008-07-06T19:08:01Z",
    "issue": "https://github.com/sagemath/sage/issues/3565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3565#issuecomment-20294",
    "user": "https://github.com/williamstein"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1,8 @@
+Before:
 
+```
+teragon-2:notebook was$ sage -coverage worksheet.py
+----------------------------------------------------------------------
+worksheet.py
+SCORE worksheet.py: 35% (68 of 193)
+```
``````




---

archive/issue_comments_020295.json:
```json
{
    "body": "<a id='comment:3'></a>\nIs this still valid after the sagenb change?",
    "created_at": "2010-05-26T20:31:57Z",
    "issue": "https://github.com/sagemath/sage/issues/3565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3565#issuecomment-20295",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'></a>
Is this still valid after the sagenb change?



---

archive/issue_comments_020296.json:
```json
{
    "body": "<a id='comment:4'></a>\nWe probably shouldn't be focusing on adding doctests to this, but shouldn't delete it until we can safely do so without breaking old notebook pickles... #9237.",
    "created_at": "2010-06-21T20:30:29Z",
    "issue": "https://github.com/sagemath/sage/issues/3565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3565#issuecomment-20296",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:4'></a>
We probably shouldn't be focusing on adding doctests to this, but shouldn't delete it until we can safely do so without breaking old notebook pickles... #9237.



---

archive/issue_events_019800.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-05-29T00:39:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3565",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3565#event-19800"
}
```



---

archive/issue_events_019801.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-05-29T00:39:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3565",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3565#event-19801"
}
```



---

archive/issue_events_019802.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-05-29T00:39:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3565",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3565#event-19802"
}
```



---

archive/issue_events_019803.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-05-29T00:39:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3565",
    "milestone": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3565#event-19803"
}
```



---

archive/issue_comments_020297.json:
```json
{
    "body": "<a id='comment:6'></a>\nNo longer valid since sagenb is a different project.",
    "created_at": "2012-05-29T00:39:33Z",
    "issue": "https://github.com/sagemath/sage/issues/3565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3565#issuecomment-20297",
    "user": "https://github.com/roed314"
}
```

<a id='comment:6'></a>
No longer valid since sagenb is a different project.



---

archive/issue_events_019804.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T12:39:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3565",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3565#event-19804"
}
```



---

archive/issue_events_019805.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T12:39:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3565",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3565#event-19805"
}
```



---

archive/issue_comments_020298.json:
```json
{
    "body": "**Reviewer:** David Roe",
    "created_at": "2012-06-02T12:39:19Z",
    "issue": "https://github.com/sagemath/sage/issues/3565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3565#issuecomment-20298",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** David Roe

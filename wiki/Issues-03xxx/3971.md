# Issue 3971: hidden markov models -- implement nsteps and log_likelihood_cutoff

archive/issues_003971.json:
```json
{
    "body": "I forgot to implement the nsteps and log_likelihood_cuttoff parameters for discrete hidden markov models, despite documenting them as implemented. The attached short patch fixes this oversight. \n\n**Assignee:** cwitty\n\nIssue created by migration from https://trac.sagemath.org/ticket/3971\n\n",
    "closed_at": "2008-08-28T12:00:19Z",
    "created_at": "2008-08-27T23:37:19Z",
    "labels": [
        "component: misc",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.2",
    "title": "hidden markov models -- implement nsteps and log_likelihood_cutoff",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3971",
    "user": "https://github.com/williamstein"
}
```
I forgot to implement the nsteps and log_likelihood_cuttoff parameters for discrete hidden markov models, despite documenting them as implemented. The attached short patch fixes this oversight. 

**Assignee:** cwitty

Issue created by migration from https://trac.sagemath.org/ticket/3971





---

archive/attachments_004336.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "sage-3971.patch",
    "asset_url": "tarball://root/attachments/ticket3971/sage-3971.patch",
    "created_at": "2008-08-27T23:40:00Z",
    "issue": "https://github.com/sagemath/sage/issues/3971",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket3971/sage-3971.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_023199.json:
```json
{
    "body": "**Attachment:** [sage-3971.patch](https://github.com/sagemath/sage/files/ticket3971/sage-3971.patch)",
    "created_at": "2008-08-27T23:40:00Z",
    "issue": "https://github.com/sagemath/sage/issues/3971",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3971#issuecomment-23199",
    "user": "https://github.com/williamstein"
}
```

**Attachment:** [sage-3971.patch](https://github.com/sagemath/sage/files/ticket3971/sage-3971.patch)



---

archive/issue_comments_023200.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis looks good to me. No real comments but positive review",
    "created_at": "2008-08-28T06:11:09Z",
    "issue": "https://github.com/sagemath/sage/issues/3971",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3971#issuecomment-23200",
    "user": "https://trac.sagemath.org/admin/accounts/users/jkantor"
}
```

<a id='comment:1'></a>
This looks good to me. No real comments but positive review



---

archive/issue_events_022809.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/jkantor",
    "created_at": "2008-08-28T06:11:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3971",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3971#event-22809"
}
```



---

archive/issue_comments_023201.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.1.2.alpha2",
    "created_at": "2008-08-28T12:00:19Z",
    "issue": "https://github.com/sagemath/sage/issues/3971",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3971#issuecomment-23201",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.1.2.alpha2



---

archive/issue_events_022810.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-28T12:00:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3971",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3971#event-22810"
}
```



---

archive/issue_events_022811.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-28T12:00:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3971",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3971#event-22811"
}
```

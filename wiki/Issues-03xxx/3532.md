# Issue 3532: gdbm spkg

archive/issues_003532.json:
```json
{
    "body": "GDBM is a prerequisite for M2 #10.  It is an optional/experimental spkg and may be downloaded at http://sage.math.washington.edu/home/gfurnish/spkg/gdbm-1.8.3.spkg\n\n**Assignee:** @garyfurnish\n\n**Keywords:** editor_mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/3532\n\n",
    "closed_at": "2008-07-16T18:22:58Z",
    "created_at": "2008-06-29T02:59:20Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.6",
    "title": "gdbm spkg",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3532",
    "user": "https://github.com/garyfurnish"
}
```
GDBM is a prerequisite for M2 #10.  It is an optional/experimental spkg and may be downloaded at http://sage.math.washington.edu/home/gfurnish/spkg/gdbm-1.8.3.spkg

**Assignee:** @garyfurnish

**Keywords:** editor_mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/3532





---

archive/issue_comments_020056.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis builds fine for me on Ubuntu 8.04 on a Core2Duo.",
    "created_at": "2008-06-29T16:48:18Z",
    "issue": "https://github.com/sagemath/sage/issues/3532",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3532#issuecomment-20056",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
This builds fine for me on Ubuntu 8.04 on a Core2Duo.



---

archive/issue_comments_020057.json:
```json
{
    "body": "**Changing keywords** from \"\" to \"editor_mabshoff\".",
    "created_at": "2008-07-02T20:01:58Z",
    "issue": "https://github.com/sagemath/sage/issues/3532",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3532#issuecomment-20057",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

**Changing keywords** from "" to "editor_mabshoff".



---

archive/issue_comments_020058.json:
```json
{
    "body": "<a id='comment:3'></a>\nNew spkg has been uploaded to same location.",
    "created_at": "2008-07-16T17:46:42Z",
    "issue": "https://github.com/sagemath/sage/issues/3532",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3532#issuecomment-20058",
    "user": "https://github.com/garyfurnish"
}
```

<a id='comment:3'></a>
New spkg has been uploaded to same location.



---

archive/issue_events_019566.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-16T18:20:44Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3532",
    "rename": {
        "from": "[With SPKG] gdbm spkg",
        "to": "gdbm spkg"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3532#event-19566"
}
```



---

archive/issue_events_019567.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-16T18:20:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3532",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3532#event-19567"
}
```



---

archive/issue_comments_020059.json:
```json
{
    "body": "<a id='comment:4'></a>\nThe spkg at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.6/alpha0/gdbm-1.8.3.spkg\n\nhas minimal fixes to SPKG.txt over Gary's last version. Positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-07-16T18:20:44Z",
    "issue": "https://github.com/sagemath/sage/issues/3532",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3532#issuecomment-20059",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
The spkg at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.6/alpha0/gdbm-1.8.3.spkg

has minimal fixes to SPKG.txt over Gary's last version. Positive review.

Cheers,

Michael



---

archive/issue_events_019568.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-16T18:22:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3532",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3532#event-19568"
}
```



---

archive/issue_events_019569.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-16T18:22:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3532",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3532#event-19569"
}
```



---

archive/issue_comments_020060.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged in Sage 3.0.6.alpha0",
    "created_at": "2008-07-16T18:22:58Z",
    "issue": "https://github.com/sagemath/sage/issues/3532",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3532#issuecomment-20060",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged in Sage 3.0.6.alpha0

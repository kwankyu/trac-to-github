# Issue 3951: Make a "Tickets in which I have participated" report

archive/issues_003951.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nIt would be great to have a Trac report that listed all the tickets in which the logged in user has participated (i.e., created, commented on, in the CC list, in the editor list, etc.)  I think this basically would be a report of all tickets that could possibly email you updates.\n\nFor me, it would serve as a \"Tickets that I should follow up on\" report :)\n\n\nComponent: **website/wiki**\n\nAuthor: **Jason Grout**\n\nReviewer: **Minh Van Nguyen**\n\nMerged: **Sage 4.1.2.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3951_\n\n",
    "closed_at": "2009-09-29T15:10:32Z",
    "created_at": "2008-08-25T19:52:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20website%20/%20wiki",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make a \"Tickets in which I have participated\" report",
    "type": "issue",
    "updated_at": "2009-09-29T15:10:32Z",
    "url": "https://github.com/sagemath/sage/issues/3951",
    "user": "https://github.com/jasongrout"
}
```
<div id="comment:0"></div>

It would be great to have a Trac report that listed all the tickets in which the logged in user has participated (i.e., created, commented on, in the CC list, in the editor list, etc.)  I think this basically would be a report of all tickets that could possibly email you updates.

For me, it would serve as a "Tickets that I should follow up on" report :)


Component: **website/wiki**

Author: **Jason Grout**

Reviewer: **Minh Van Nguyen**

Merged: **Sage 4.1.2.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/3951_





---

archive/issue_events_043679.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-08-25T19:52:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3951",
    "milestone_number": null,
    "milestone_title": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3951#event-43679"
}
```



---

archive/issue_events_043680.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-08-25T19:52:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3951",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20website%20/%20wiki",
    "label_color": "0075ca",
    "label_name": "c: website / wiki",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3951#event-43680"
}
```



---

archive/issue_events_043681.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-08-25T19:52:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3951",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3951#event-43681"
}
```



---

archive/issue_events_043682.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-08-25T19:52:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3951",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3951#event-43682"
}
```



---

archive/issue_events_043683.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-25T19:52:25Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3951",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3951#event-43683"
}
```



---

archive/issue_comments_022103.json:
```json
{
    "body": "Reviewer: **Minh Van Nguyen**",
    "created_at": "2009-09-29T15:10:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3951",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3951#issuecomment-22103",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **Minh Van Nguyen**



---

archive/issue_comments_022104.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nOne needs to login to trac first before getting results for this trac report.",
    "created_at": "2009-09-29T15:10:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3951",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3951#issuecomment-22104",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:1" align="right">comment:1</div>

One needs to login to trac first before getting results for this trac report.



---

archive/issue_events_043684.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-29T15:10:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3951",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3951#event-43684"
}
```



---

archive/issue_comments_022105.json:
```json
{
    "body": "Author: **Jason Grout**",
    "created_at": "2009-09-29T15:10:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3951",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3951#issuecomment-22105",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Jason Grout**



---

archive/issue_comments_022106.json:
```json
{
    "body": "Merged: **Sage 4.1.2.rc0**",
    "created_at": "2009-09-29T15:10:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3951",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3951#issuecomment-22106",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **Sage 4.1.2.rc0**

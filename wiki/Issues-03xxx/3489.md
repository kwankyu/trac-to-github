# Issue 3489: DsageNg

archive/issues_003489.json:
```json
{
    "body": "Implement general Distributed Object Computing (DOC) in Sage.  This includes parallel computation, exogenous event handling, and general distributed services (e.g. persistence).\n\nhttp://wiki.sagemath.org/CategoryDsageNg\n\n\n**Assignee:** ghtdak\n\nIssue created by migration from https://trac.sagemath.org/ticket/3489\n\n",
    "closed_at": "2008-07-10T19:44:03Z",
    "created_at": "2008-06-20T23:55:23Z",
    "labels": [
        "component: dsage",
        "enhancement",
        "invalid"
    ],
    "title": "DsageNg",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3489",
    "user": "https://trac.sagemath.org/admin/accounts/users/ghtdak"
}
```
Implement general Distributed Object Computing (DOC) in Sage.  This includes parallel computation, exogenous event handling, and general distributed services (e.g. persistence).

http://wiki.sagemath.org/CategoryDsageNg


**Assignee:** ghtdak

Issue created by migration from https://trac.sagemath.org/ticket/3489





---

archive/issue_events_019245.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-21T01:17:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3489",
    "label": "feature",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3489#event-19245"
}
```



---

archive/issue_events_019246.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/ghtdak",
    "created_at": "2008-07-10T19:44:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3489",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3489#event-19246"
}
```



---

archive/issue_comments_019773.json:
```json
{
    "body": "<a id='comment:2'></a>\nneeds a better name",
    "created_at": "2008-07-10T19:44:03Z",
    "issue": "https://github.com/sagemath/sage/issues/3489",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3489#issuecomment-19773",
    "user": "https://trac.sagemath.org/admin/accounts/users/ghtdak"
}
```

<a id='comment:2'></a>
needs a better name



---

archive/issue_events_019247.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-11T04:09:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3489",
    "label": "feature",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3489#event-19247"
}
```

# Issue 3235: cygwin -- mpfi; get it to work with Cygwin by fixing configure.ac

archive/issues_003235.json:
```json
{
    "body": "See also #3188.\n\nThe spkg:\n\n  http://sage.math.washington.edu/home/was/cygwin/mpfi-1.3.4-cvs20071125.p6.spkg\n\nSEE THE NOTES in the log.  Note that src/auto-conf-stuff is also changed.\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/3235\n\n",
    "closed_at": "2008-05-18T17:06:04Z",
    "created_at": "2008-05-17T03:55:33Z",
    "labels": [
        "component: porting: cygwin",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.2",
    "title": "cygwin -- mpfi; get it to work with Cygwin by fixing configure.ac",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3235",
    "user": "https://github.com/williamstein"
}
```
See also #3188.

The spkg:

  http://sage.math.washington.edu/home/was/cygwin/mpfi-1.3.4-cvs20071125.p6.spkg

SEE THE NOTES in the log.  Note that src/auto-conf-stuff is also changed.

**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/3235





---

archive/issue_events_017336.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-17T20:40:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3235",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3235#event-17336"
}
```



---

archive/issue_comments_017916.json:
```json
{
    "body": "<a id='comment:2'></a>\nSpkg looks good to me. I added a diff of configure.ac.orig and configure.ac so that we can send the patch upstream. Positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-05-18T17:05:51Z",
    "issue": "https://github.com/sagemath/sage/issues/3235",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3235#issuecomment-17916",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Spkg looks good to me. I added a diff of configure.ac.orig and configure.ac so that we can send the patch upstream. Positive review.

Cheers,

Michael



---

archive/issue_events_017337.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-18T17:05:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3235",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3235#event-17337"
}
```



---

archive/issue_events_017338.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-18T17:05:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3235",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3235#event-17338"
}
```



---

archive/issue_events_017339.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-18T17:06:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3235",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3235#event-17339"
}
```



---

archive/issue_events_017340.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-05-18T17:06:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3235",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3235#event-17340"
}
```



---

archive/issue_comments_017917.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.2.alpha1",
    "created_at": "2008-05-18T17:06:04Z",
    "issue": "https://github.com/sagemath/sage/issues/3235",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3235#issuecomment-17917",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.2.alpha1

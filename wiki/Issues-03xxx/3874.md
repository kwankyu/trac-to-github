# Issue 3874: Moebius plot bug

archive/issues_003874.json:
```json
{
    "assignees": [
        "https://github.com/kcrisman"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nplot(moebius) plots the Moebius function nicely from 0 to 50.\nplot(moebius,50,100) plots the y-values of the Moebius function nicely from 50 to 100. Unfortunately, the x-values are still from 0 to 50!\n\nComponent: **number theory**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3874_\n\n",
    "closed_at": "2008-08-18T23:15:42Z",
    "created_at": "2008-08-15T13:20:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Moebius plot bug",
    "type": "issue",
    "updated_at": "2008-08-18T23:15:42Z",
    "url": "https://github.com/sagemath/sage/issues/3874",
    "user": "https://github.com/kcrisman"
}
```
<div id="comment:0"></div>

plot(moebius) plots the Moebius function nicely from 0 to 50.
plot(moebius,50,100) plots the y-values of the Moebius function nicely from 50 to 100. Unfortunately, the x-values are still from 0 to 50!

Component: **number theory**

_Issue created by migration from https://trac.sagemath.org/ticket/3874_





---

archive/issue_events_042676.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2008-08-15T13:20:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "milestone_number": null,
    "milestone_title": "sage-3.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3874#event-42676"
}
```



---

archive/issue_events_042677.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2008-08-15T13:20:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "c: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3874#event-42677"
}
```



---

archive/issue_events_042678.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2008-08-15T13:20:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3874#event-42678"
}
```



---

archive/issue_events_042679.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2008-08-15T13:20:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3874#event-42679"
}
```



---

archive/issue_events_042680.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-15T13:20:01Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "subject": "https://github.com/kcrisman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3874#event-42680"
}
```



---

archive/issue_comments_021482.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[moebius.patch.gz](https://github.com/sagemath/sage/files/ticket3874/moebius.patch.gz)**",
    "created_at": "2008-08-15T13:22:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3874#issuecomment-21482",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[moebius.patch.gz](https://github.com/sagemath/sage/files/ticket3874/moebius.patch.gz)**



---

archive/issue_events_042681.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2008-08-15T13:22:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3874#event-42681"
}
```



---

archive/issue_events_042682.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-15T13:22:46Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "subject": "https://github.com/kcrisman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3874#event-42682"
}
```



---

archive/issue_events_042683.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2008-08-15T13:22:46Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "subject": "https://github.com/kcrisman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3874#event-42683"
}
```



---

archive/issue_comments_021483.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\n\n```\n\nCan you throw in a call to\n\n  p.xmin(xmin)\n\nbefore returning p?  The plot of  plot(moebius, 500,550) would look\nmuch nicer as a result. \n```",
    "created_at": "2008-08-15T16:41:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3874#issuecomment-21483",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">comment:2</div>


```

Can you throw in a call to

  p.xmin(xmin)

before returning p?  The plot of  plot(moebius, 500,550) would look
much nicer as a result. 
```



---

archive/issue_events_042684.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-15T16:41:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3874#event-42684"
}
```



---

archive/issue_events_042685.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-15T16:41:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3874#event-42685"
}
```



---

archive/issue_events_042686.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-15T17:08:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3874#event-42686"
}
```



---

archive/issue_events_042687.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-15T17:08:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3874#event-42687"
}
```



---

archive/issue_comments_021484.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\n\n```\n> William,\n>\n> If you are referring to the viewing window starting around x=-1, my\n> understanding was that this is taken care of by the patches to\n> https://github.com/sagemath/sage/issues/3806, which will be in 3.1.\n> I attach the output of\n> sage: plot(moebius,500,550)\n> on my system, with those patches included.\n>\n> If that's not what you mean, let me know what the problem with the\n> plot is and I will be happy to have the code reset the xmin. Thanks\n> for the feedback!\n>\n> - kcrisman\n\nWith the excellent work at #3806, I immediately change my opinion to\npositive review for your patch as is. \n\n```",
    "created_at": "2008-08-15T17:08:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3874#issuecomment-21484",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:3" align="right">comment:3</div>


```
> William,
>
> If you are referring to the viewing window starting around x=-1, my
> understanding was that this is taken care of by the patches to
> https://github.com/sagemath/sage/issues/3806, which will be in 3.1.
> I attach the output of
> sage: plot(moebius,500,550)
> on my system, with those patches included.
>
> If that's not what you mean, let me know what the problem with the
> plot is and I will be happy to have the code reset the xmin. Thanks
> for the feedback!
>
> - kcrisman

With the excellent work at #3806, I immediately change my opinion to
positive review for your patch as is. 

```



---

archive/issue_events_042688.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-18T23:15:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3874#event-42688"
}
```



---

archive/issue_events_042689.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-18T23:15:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3874#event-42689"
}
```



---

archive/issue_comments_021485.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in Sage 3.1.2.alpha0",
    "created_at": "2008-08-18T23:15:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3874",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3874#issuecomment-21485",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in Sage 3.1.2.alpha0

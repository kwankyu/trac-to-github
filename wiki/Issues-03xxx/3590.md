# Issue 3590: dage_interfaces -- port detection code hangs solid

archive/issues_003590.json:
```json
{
    "body": "\n```\nsage -t  devel/sage/sage/dsage/interface/dsage_interface.py *** ***\nError: TIMED OUT! *** ***\n[DSage] Closed connection to localhost\n[DSage] Closed connection to localhost\n[DSage] Closed connection to localhost\n[DSage] Closed connection to localhost\n*** *** Error: TIMED OUT! *** ***\n        [2697.3 s]\n```\n\nThis is at\n\n```\nport = find_open_port().next()\n```\n\nThis happens on *some machines*, e.g., fermat.math.harvard.edu, but not others.\n\n\n\n**Assignee:** @yqiang\n\nIssue created by migration from https://trac.sagemath.org/ticket/3590\n\n",
    "closed_at": "2008-07-08T00:20:31Z",
    "created_at": "2008-07-07T20:42:55Z",
    "labels": [
        "component: dsage",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.4",
    "title": "dage_interfaces -- port detection code hangs solid",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3590",
    "user": "https://github.com/williamstein"
}
```

```
sage -t  devel/sage/sage/dsage/interface/dsage_interface.py *** ***
Error: TIMED OUT! *** ***
[DSage] Closed connection to localhost
[DSage] Closed connection to localhost
[DSage] Closed connection to localhost
[DSage] Closed connection to localhost
*** *** Error: TIMED OUT! *** ***
        [2697.3 s]
```

This is at

```
port = find_open_port().next()
```

This happens on *some machines*, e.g., fermat.math.harvard.edu, but not others.



**Assignee:** @yqiang

Issue created by migration from https://trac.sagemath.org/ticket/3590





---

archive/attachments_003746.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "10033.patch",
    "asset_url": "tarball://root/attachments/ticket3590/10033.patch",
    "created_at": "2008-07-07T23:20:42Z",
    "issue": "https://github.com/sagemath/sage/issues/3590",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket3590/10033.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_020440.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [10033.patch](https://github.com/sagemath/sage/files/ticket3590/10033.patch)",
    "created_at": "2008-07-07T23:20:42Z",
    "issue": "https://github.com/sagemath/sage/issues/3590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3590#issuecomment-20440",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
**Attachment:** [10033.patch](https://github.com/sagemath/sage/files/ticket3590/10033.patch)



---

archive/issue_events_019993.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-07-07T23:20:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3590",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3590#event-19993"
}
```



---

archive/attachments_003747.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "10034.patch",
    "asset_url": "tarball://root/attachments/ticket3590/10034.patch",
    "created_at": "2008-07-08T00:10:15Z",
    "issue": "https://github.com/sagemath/sage/issues/3590",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket3590/10034.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_020441.json:
```json
{
    "body": "**Attachment:** [10034.patch](https://github.com/sagemath/sage/files/ticket3590/10034.patch)",
    "created_at": "2008-07-08T00:10:15Z",
    "issue": "https://github.com/sagemath/sage/issues/3590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3590#issuecomment-20441",
    "user": "https://github.com/williamstein"
}
```

**Attachment:** [10034.patch](https://github.com/sagemath/sage/files/ticket3590/10034.patch)



---

archive/issue_comments_020442.json:
```json
{
    "body": "<a id='comment:2'></a>\nIt's better than the existing code :)",
    "created_at": "2008-07-08T00:19:47Z",
    "issue": "https://github.com/sagemath/sage/issues/3590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3590#issuecomment-20442",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:2'></a>
It's better than the existing code :)



---

archive/issue_events_019994.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2008-07-08T00:19:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3590",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3590#event-19994"
}
```



---

archive/issue_events_019995.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2008-07-08T00:19:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3590",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3590#event-19995"
}
```



---

archive/issue_events_019996.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-07-08T00:20:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3590",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3590#event-19996"
}
```



---

archive/issue_events_019997.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-07-08T00:20:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3590",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3590#event-19997"
}
```

# Issue 3301: Fix POSIX issues in gmp's spkg-install

archive/issues_003301.json:
```json
{
    "body": "\n```\nUbuntu 8.04, 64-bit.\n\nFails to build GMP.  All before this seems to work ok.\n\nFirst message that looks suspect in \"install.log\" is:\n\nPatching gmp-h.in (fixes OSX 10.5 issues and gcc 4.3 problems)\nDo we have a Core2 CPU?... No\n[: 220: ==: unexpected operator\n```\n\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/3301\n\n",
    "closed_at": "2008-07-31T01:13:45Z",
    "created_at": "2008-05-25T20:26:39Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1",
    "title": "Fix POSIX issues in gmp's spkg-install",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3301",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

```
Ubuntu 8.04, 64-bit.

Fails to build GMP.  All before this seems to work ok.

First message that looks suspect in "install.log" is:

Patching gmp-h.in (fixes OSX 10.5 issues and gcc 4.3 problems)
Do we have a Core2 CPU?... No
[: 220: ==: unexpected operator
```


**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/3301





---

archive/issue_events_017732.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-31T01:13:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3301",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3301#event-17732"
}
```



---

archive/issue_comments_018268.json:
```json
{
    "body": "<a id='comment:1'></a>\nThe issue has been resolved in Sage 3.0.6.\n\nCheers,\n\nMichael",
    "created_at": "2008-07-31T01:13:45Z",
    "issue": "https://github.com/sagemath/sage/issues/3301",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3301#issuecomment-18268",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
The issue has been resolved in Sage 3.0.6.

Cheers,

Michael



---

archive/issue_events_017733.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-31T01:13:45Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3301",
    "milestone": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3301#event-17733"
}
```



---

archive/issue_events_017734.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-31T01:13:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3301",
    "milestone": "sage-3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3301#event-17734"
}
```

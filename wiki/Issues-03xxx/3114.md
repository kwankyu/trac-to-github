# Issue 3114: blacklist  gcc version 4.0.0 on OSX

archive/issues_003114.json:
```json
{
    "body": "Any gcc 4.0.0 provided by Apple is buggier than a Florida swamp. Refuse to build Sage with it at start up!\n\nCheers,\n\nMichael\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/3114\n\n",
    "closed_at": "2010-01-17T06:32:54Z",
    "created_at": "2008-05-06T22:06:10Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.1",
    "title": "blacklist  gcc version 4.0.0 on OSX",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3114",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
Any gcc 4.0.0 provided by Apple is buggier than a Florida swamp. Refuse to build Sage with it at start up!

Cheers,

Michael

**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/3114





---

archive/issue_events_016543.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-15T23:19:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3114",
    "label": "blocker",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3114#event-16543"
}
```



---

archive/issue_events_016544.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-01-17T06:32:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3114",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3114#event-16544"
}
```



---

archive/issue_comments_017241.json:
```json
{
    "body": "<a id='comment:2'></a>\n`prereq` currently checks for gcc >= 4.0.1, so this has already been taken care of.",
    "created_at": "2010-01-17T06:32:54Z",
    "issue": "https://github.com/sagemath/sage/issues/3114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3114#issuecomment-17241",
    "user": "https://github.com/wjp"
}
```

<a id='comment:2'></a>
`prereq` currently checks for gcc >= 4.0.1, so this has already been taken care of.

# Issue 3050: notebook -- add a "remember me" checkbox to the login page.

archive/issues_003050.json:
```json
{
    "assignees": [
        "https://github.com/TimothyClemans"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nMany webpages have a little \"remember me\" checkbox to determine whether you have to enter login credentials every time you visit a web page.  Possibly implement something similar in Sage. \n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3050_\n\n",
    "closed_at": "2008-05-19T06:07:52Z",
    "created_at": "2008-04-28T16:48:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "notebook -- add a \"remember me\" checkbox to the login page.",
    "type": "issue",
    "updated_at": "2008-05-19T06:07:52Z",
    "url": "https://github.com/sagemath/sage/issues/3050",
    "user": "https://github.com/TimothyClemans"
}
```
<div id="comment:0"></div>

Many webpages have a little "remember me" checkbox to determine whether you have to enter login credentials every time you visit a web page.  Possibly implement something similar in Sage. 

Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/3050_





---

archive/issue_events_032245.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-04-28T16:48:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "milestone_number": null,
    "milestone_title": "sage-3.0.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32245"
}
```



---

archive/issue_events_032246.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-04-28T16:48:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32246"
}
```



---

archive/issue_events_032247.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-04-28T16:48:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32247"
}
```



---

archive/issue_events_032248.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-04-28T16:48:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32248"
}
```



---

archive/issue_events_032249.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-04-28T16:48:26Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "subject": "https://github.com/TimothyClemans",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32249"
}
```



---

archive/issue_comments_015984.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-Port the attached SVN patch from Knoboo to the Sage Notebook.\n+Many webpages have a little \"remember me\" checkbox to determine whether you have to enter login credentials every time you visit a web page.  Possibly implement something similar in Sage. \n``````\n",
    "created_at": "2008-04-29T16:20:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3050#issuecomment-15984",
    "user": "https://github.com/williamstein"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-Port the attached SVN patch from Knoboo to the Sage Notebook.
+Many webpages have a little "remember me" checkbox to determine whether you have to enter login credentials every time you visit a web page.  Possibly implement something similar in Sage. 
``````




---

archive/issue_comments_015985.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThe Knoboo people have this feature.  Timothy attached a patch that is relevant to their implementation, which might be of some use to whoever implements this for sage.",
    "created_at": "2008-04-29T16:20:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3050#issuecomment-15985",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>

The Knoboo people have this feature.  Timothy attached a patch that is relevant to their implementation, which might be of some use to whoever implements this for sage.



---

archive/issue_events_032250.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-04-29T16:20:36Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "title_is": "notebook -- add a \"remember me\" checkbox to the login page.",
    "title_was": "notebook -- add \"remember me\" checkbox",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32250"
}
```



---

archive/issue_comments_015986.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI want to make it clear that this functionality is not in Knoboo currently. From Alex Clemesha, \"We will get this into knoboo as soon as the \"settings\" functionality\nstarts to settle into knoboo, because, (as you would probably agree), this only thing missing with the patch would be a way for the Admin user to set the \"expires time\" (instead of hard-coded).\"",
    "created_at": "2008-04-29T20:06:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3050#issuecomment-15986",
    "user": "https://github.com/TimothyClemans"
}
```

<div id="comment:2" align="right">comment:2</div>

I want to make it clear that this functionality is not in Knoboo currently. From Alex Clemesha, "We will get this into knoboo as soon as the "settings" functionality
starts to settle into knoboo, because, (as you would probably agree), this only thing missing with the patch would be a way for the Admin user to set the "expires time" (instead of hard-coded)."



---

archive/issue_comments_015987.json:
```json
{
    "body": "Attachment: **[extcode-3050.patch.gz](https://github.com/sagemath/sage/files/ticket3050/extcode-3050.patch.gz)**",
    "created_at": "2008-05-17T15:42:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3050#issuecomment-15987",
    "user": "https://github.com/TimothyClemans"
}
```

Attachment: **[extcode-3050.patch.gz](https://github.com/sagemath/sage/files/ticket3050/extcode-3050.patch.gz)**



---

archive/issue_comments_015988.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nFixes #3155 also.",
    "created_at": "2008-05-17T15:42:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3050#issuecomment-15988",
    "user": "https://github.com/TimothyClemans"
}
```

<div id="comment:3" align="right">comment:3</div>

Fixes #3155 also.



---

archive/issue_events_032251.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-05-17T15:42:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32251"
}
```



---

archive/issue_comments_015989.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nWarning: Depends on #3213",
    "created_at": "2008-05-17T15:46:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3050#issuecomment-15989",
    "user": "https://github.com/TimothyClemans"
}
```

<div id="comment:4" align="right">comment:4</div>

Warning: Depends on #3213



---

archive/issue_comments_015990.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nREVIEW:\n\n1. It would be nice if you added a comment about what is going on in the modifications to twist.py.\n\n2. I don't actually understand how to test that this patch is actually doing something!  Could you give me simple step-by-step instructions to test out a situation where the behavior of something is different whether or not remember me is checked?   Does this feature only do anything when there are multiple accounts?  I tried what I thought was the obvious test, and it seems like Remember Me just doesn't work.  We can figure this out at the cafe today.",
    "created_at": "2008-05-17T16:14:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3050#issuecomment-15990",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:5" align="right">comment:5</div>

REVIEW:

1. It would be nice if you added a comment about what is going on in the modifications to twist.py.

2. I don't actually understand how to test that this patch is actually doing something!  Could you give me simple step-by-step instructions to test out a situation where the behavior of something is different whether or not remember me is checked?   Does this feature only do anything when there are multiple accounts?  I tried what I thought was the obvious test, and it seems like Remember Me just doesn't work.  We can figure this out at the cafe today.



---

archive/issue_events_032252.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-05-17T17:43:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32252"
}
```



---

archive/issue_events_032253.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-05-17T17:43:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32253"
}
```



---

archive/issue_comments_015991.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nMerged all three patches in Sage 3.0.2.alpha1",
    "created_at": "2008-05-17T20:29:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3050#issuecomment-15991",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:7" align="right">comment:7</div>

Merged all three patches in Sage 3.0.2.alpha1



---

archive/issue_events_032254.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-17T20:29:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32254"
}
```



---

archive/issue_events_032255.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-17T20:29:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32255"
}
```



---

archive/issue_comments_015992.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nHi,\n\nI was just using Sage on my computer with this patch applied and having a lot of problems if I open and close my browser.  I get into a state where I absolutely can't login, etc.\n\nI.e., this patch is definitely not ready for prime time.  It will break the notebook for a lot of people in confusing ways. \n\nWe'll get it fixed though.",
    "created_at": "2008-05-18T16:32:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3050#issuecomment-15992",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:8" align="right">comment:8</div>

Hi,

I was just using Sage on my computer with this patch applied and having a lot of problems if I open and close my browser.  I get into a state where I absolutely can't login, etc.

I.e., this patch is definitely not ready for prime time.  It will break the notebook for a lot of people in confusing ways. 

We'll get it fixed though.



---

archive/issue_events_032256.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-05-18T16:32:31Z",
    "event": "reopened",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32256"
}
```



---

archive/issue_events_032257.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-18T17:14:13Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "title_is": "[reverted] notebook -- add a \"remember me\" checkbox to the login page.",
    "title_was": "notebook -- add a \"remember me\" checkbox to the login page.",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32257"
}
```



---

archive/issue_comments_015993.json:
```json
{
    "body": "new and includes sage-3050.patch and sage-3050_2.patch",
    "created_at": "2008-05-19T03:35:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3050#issuecomment-15993",
    "user": "https://github.com/TimothyClemans"
}
```

new and includes sage-3050.patch and sage-3050_2.patch



---

archive/issue_events_032258.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-05-19T03:37:53Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "title_is": "notebook -- add a \"remember me\" checkbox to the login page.",
    "title_was": "[reverted] notebook -- add a \"remember me\" checkbox to the login page.",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32258"
}
```



---

archive/issue_events_032259.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-05-19T03:37:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32259"
}
```



---

archive/issue_comments_015994.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nAttachment: **[trac_3050-revised-sage-3050.patch.gz](https://github.com/sagemath/sage/files/ticket3050/trac_3050-revised-sage-3050.patch.gz)**\n\nI changed the cookie name to the static name \"nb_session\" in both the render for UserToplevel and in the function get_our_cookie in guard.py. I don't know if doing this fixes the problem.",
    "created_at": "2008-05-19T03:37:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3050#issuecomment-15994",
    "user": "https://github.com/TimothyClemans"
}
```

<div id="comment:10" align="right">comment:10</div>

Attachment: **[trac_3050-revised-sage-3050.patch.gz](https://github.com/sagemath/sage/files/ticket3050/trac_3050-revised-sage-3050.patch.gz)**

I changed the cookie name to the static name "nb_session" in both the render for UserToplevel and in the function get_our_cookie in guard.py. I don't know if doing this fixes the problem.



---

archive/issue_comments_015995.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nThe new patch works.",
    "created_at": "2008-05-19T03:45:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3050#issuecomment-15995",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:11" align="right">comment:11</div>

The new patch works.



---

archive/issue_events_032260.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-05-19T03:45:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32260"
}
```



---

archive/issue_events_032261.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-05-19T03:45:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32261"
}
```



---

archive/issue_comments_015996.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nThe new patch still causes problems, where notebooks just \"don't work\".  This is very serious and would cause mass problems by users.",
    "created_at": "2008-05-19T05:35:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3050#issuecomment-15996",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:12" align="right">comment:12</div>

The new patch still causes problems, where notebooks just "don't work".  This is very serious and would cause mass problems by users.



---

archive/issue_events_032262.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-05-19T05:35:55Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "title_is": "[negative review] notebook -- add a \"remember me\" checkbox to the login page.",
    "title_was": "notebook -- add a \"remember me\" checkbox to the login page.",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32262"
}
```



---

archive/issue_events_032263.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-05-19T05:41:14Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "title_is": "notebook -- add a \"remember me\" checkbox to the login page.",
    "title_was": "[negative review] notebook -- add a \"remember me\" checkbox to the login page.",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32263"
}
```



---

archive/issue_comments_015997.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nWait, I spoke too soon.  I was confused by another separate problem.",
    "created_at": "2008-05-19T05:41:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3050#issuecomment-15997",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:13" align="right">comment:13</div>

Wait, I spoke too soon.  I was confused by another separate problem.



---

archive/issue_comments_015998.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nMerged extcode-3050.patch and trac_3050-revised-sage-3050.patch in Sage 3.0.2.alpha1",
    "created_at": "2008-05-19T06:07:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3050#issuecomment-15998",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:14" align="right">comment:14</div>

Merged extcode-3050.patch and trac_3050-revised-sage-3050.patch in Sage 3.0.2.alpha1



---

archive/issue_events_032264.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-19T06:07:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32264"
}
```



---

archive/issue_events_032265.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-05-19T06:07:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3050",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3050#event-32265"
}
```

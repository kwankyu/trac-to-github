# Issue 3012: animate.py: convert not being found is an annoying debug message - fix that

archive/issues_003012.json:
```json
{
    "body": "Bjake Hammersholt Roune reported:\n\n```\n> [...]\n sage -t  devel/sage/sage/plot/animate.py                    sh:\n convert: command\n  not found\n```\nHide it or test if it exists before running it. This should be easy to fix.\n\nCheers,\n\nMichael\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/3012\n\n",
    "closed_at": "2008-09-29T22:26:54Z",
    "created_at": "2008-04-23T22:07:43Z",
    "labels": [
        "component: misc",
        "bug",
        "duplicate"
    ],
    "title": "animate.py: convert not being found is an annoying debug message - fix that",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3012",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
Bjake Hammersholt Roune reported:

```
> [...]
 sage -t  devel/sage/sage/plot/animate.py                    sh:
 convert: command
  not found
```
Hide it or test if it exists before running it. This should be easy to fix.

Cheers,

Michael

**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/3012





---

archive/issue_events_015898.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-29T22:26:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3012",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3012#event-15898"
}
```



---

archive/issue_events_015899.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-29T22:26:54Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/3012",
    "milestone": "sage-3.1.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3012#event-15899"
}
```



---

archive/issue_comments_016514.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis is a duplicate of #767. Closed.\n\nCheers,\n\nMichael",
    "created_at": "2008-09-29T22:26:54Z",
    "issue": "https://github.com/sagemath/sage/issues/3012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3012#issuecomment-16514",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
This is a duplicate of #767. Closed.

Cheers,

Michael



---

archive/issue_events_015900.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-29T22:26:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3012",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3012#event-15900"
}
```

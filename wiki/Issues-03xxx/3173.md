# Issue 3173: add 64 bit OSX build support to eclib

archive/issues_003173.json:
```json
{
    "body": "spkg coming up\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/3173\n\n",
    "closed_at": "2008-06-08T22:27:26Z",
    "created_at": "2008-05-13T00:22:47Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0.3",
    "title": "add 64 bit OSX build support to eclib",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/3173",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
spkg coming up

**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/3173





---

archive/issue_events_016933.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-08T22:08:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3173",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3173#event-16933"
}
```



---

archive/issue_comments_017587.json:
```json
{
    "body": "<a id='comment:2'></a>\nThe updated spkg is at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.3/alpha2/eclib-20080310.p2.spkg\n\nI did test build it on 32 and 64 bit OSX, Linux.\n\nCheers,\n\nMichael",
    "created_at": "2008-06-08T22:08:33Z",
    "issue": "https://github.com/sagemath/sage/issues/3173",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3173#issuecomment-17587",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
The updated spkg is at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0.3/alpha2/eclib-20080310.p2.spkg

I did test build it on 32 and 64 bit OSX, Linux.

Cheers,

Michael



---

archive/issue_events_016934.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-06-08T22:16:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3173",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3173#event-16934"
}
```



---

archive/issue_events_016935.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-06-08T22:16:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3173",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3173#event-16935"
}
```



---

archive/issue_comments_017588.json:
```json
{
    "body": "<a id='comment:3'></a>\nworks on 32 bit linux.",
    "created_at": "2008-06-08T22:16:09Z",
    "issue": "https://github.com/sagemath/sage/issues/3173",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3173#issuecomment-17588",
    "user": "https://github.com/garyfurnish"
}
```

<a id='comment:3'></a>
works on 32 bit linux.



---

archive/issue_events_016936.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-08T22:27:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3173",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3173#event-16936"
}
```



---

archive/issue_events_016937.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-06-08T22:27:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3173",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3173#event-16937"
}
```



---

archive/issue_comments_017589.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.0.3.alpha2",
    "created_at": "2008-06-08T22:27:26Z",
    "issue": "https://github.com/sagemath/sage/issues/3173",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3173#issuecomment-17589",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.0.3.alpha2

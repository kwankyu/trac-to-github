# Issue 3918: notebook -- MAJOR BUG involving uploading file from URL

archive/issues_003918.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "<div id=\"comment:0\"></div>\n\n1. Go to Data --> Upload File\n2. Select a URL that takes \"forever\" to download.\n3. Now the ENTIRE SERVER hangs \"forever\".  \n\nThis is clearly very much not good. \n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/3918_\n\n",
    "closed_at": "2008-09-09T00:40:30Z",
    "created_at": "2008-08-20T23:17:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "notebook -- MAJOR BUG involving uploading file from URL",
    "type": "issue",
    "updated_at": "2008-09-09T00:40:30Z",
    "url": "https://github.com/sagemath/sage/issues/3918",
    "user": "https://github.com/williamstein"
}
```
<div id="comment:0"></div>

1. Go to Data --> Upload File
2. Select a URL that takes "forever" to download.
3. Now the ENTIRE SERVER hangs "forever".  

This is clearly very much not good. 

Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/3918_





---

archive/issue_events_043207.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-20T23:17:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "milestone_number": null,
    "milestone_title": "sage-3.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-43207"
}
```



---

archive/issue_events_043208.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-20T23:17:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-43208"
}
```



---

archive/issue_events_043209.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-20T23:17:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
    "label_color": "ff7700",
    "label_name": "p: critical / 2",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-43209"
}
```



---

archive/issue_events_043210.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-08-20T23:17:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-43210"
}
```



---

archive/issue_events_043211.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-08-20T23:17:14Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-43211"
}
```



---

archive/issue_events_043212.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-09-08T10:25:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-43212"
}
```



---

archive/issue_events_043213.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-09-08T10:25:03Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-43213"
}
```



---

archive/issue_events_043214.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-09-08T10:25:03Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-43214"
}
```



---

archive/issue_events_043215.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-09-08T10:25:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-43215"
}
```



---

archive/issue_comments_021825.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_3918.patch.gz](https://github.com/sagemath/sage/files/ticket3918/trac_3918.patch.gz)**",
    "created_at": "2008-09-08T10:25:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3918#issuecomment-21825",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_3918.patch.gz](https://github.com/sagemath/sage/files/ticket3918/trac_3918.patch.gz)**



---

archive/issue_comments_021826.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nFor review, I first reproduced the bug using sage-3.1.1 and then tried to reproduce it with the patch applied. The bug seems to be fixed with the patch applied. I also made sure that uploading a file worked in general.",
    "created_at": "2008-09-08T11:12:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3918#issuecomment-21826",
    "user": "https://github.com/TimothyClemans"
}
```

<div id="comment:2" align="right">comment:2</div>

For review, I first reproduced the bug using sage-3.1.1 and then tried to reproduce it with the patch applied. The bug seems to be fixed with the patch applied. I also made sure that uploading a file worked in general.



---

archive/issue_events_043216.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-09-08T11:12:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-43216"
}
```



---

archive/issue_comments_021827.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.1.2.rc1",
    "created_at": "2008-09-09T00:40:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/3918#issuecomment-21827",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.1.2.rc1



---

archive/issue_events_043217.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-09T00:40:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-43217"
}
```



---

archive/issue_events_043218.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-09T00:40:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/3918",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/3918#event-43218"
}
```

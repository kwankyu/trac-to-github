# Issue 34872: AdditiveAbelianGroup_class.exponent() returns a Python int for the trivial group

archive/issues_034635.json:
```json
{
    "body": "\n```sage\nsage: type(AdditiveAbelianGroup([]).exponent())\n<class 'int'>\n```\n\n\n**Branch:** [public/34872](https://github.com/sagemath/sagetrac-mirror/tree/public/34872)\n\n**Commit:** [6d6e02089ccf56760cb6f63f27f7ad4ccbb2d405](https://github.com/sagemath/sagetrac-mirror/commit/6d6e02089ccf56760cb6f63f27f7ad4ccbb2d405)\n\n**Author:** Lorenz Panny\n\nIssue created by migration from https://trac.sagemath.org/ticket/34872\n\n",
    "created_at": "2022-12-21T15:40:37Z",
    "labels": [
        "component: group theory",
        "minor",
        "bug",
        "needs review"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "title": "AdditiveAbelianGroup_class.exponent() returns a Python int for the trivial group",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/34872",
    "user": "https://github.com/yyyyx4"
}
```

```sage
sage: type(AdditiveAbelianGroup([]).exponent())
<class 'int'>
```


**Branch:** [public/34872](https://github.com/sagemath/sagetrac-mirror/tree/public/34872)

**Commit:** [6d6e02089ccf56760cb6f63f27f7ad4ccbb2d405](https://github.com/sagemath/sagetrac-mirror/commit/6d6e02089ccf56760cb6f63f27f7ad4ccbb2d405)

**Author:** Lorenz Panny

Issue created by migration from https://trac.sagemath.org/ticket/34872





---

archive/issue_comments_564354.json:
```json
{
    "body": "**Branch:** [public/34872](https://github.com/sagemath/sagetrac-mirror/tree/public/34872)",
    "created_at": "2022-12-21T15:42:01Z",
    "issue": "https://github.com/sagemath/sage/issues/34872",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34872#issuecomment-564354",
    "user": "https://github.com/yyyyx4"
}
```

**Branch:** [public/34872](https://github.com/sagemath/sagetrac-mirror/tree/public/34872)



---

archive/issue_comments_564355.json:
```json
{
    "body": "**Commit:** [6d6e02089ccf56760cb6f63f27f7ad4ccbb2d405](https://github.com/sagemath/sagetrac-mirror/commit/6d6e02089ccf56760cb6f63f27f7ad4ccbb2d405)",
    "created_at": "2022-12-21T15:42:01Z",
    "issue": "https://github.com/sagemath/sage/issues/34872",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34872#issuecomment-564355",
    "user": "https://github.com/yyyyx4"
}
```

**Commit:** [6d6e02089ccf56760cb6f63f27f7ad4ccbb2d405](https://github.com/sagemath/sagetrac-mirror/commit/6d6e02089ccf56760cb6f63f27f7ad4ccbb2d405)



---

archive/issue_comments_564356.json:
```json
{
    "body": "**Author:** Lorenz Panny",
    "created_at": "2022-12-21T15:42:01Z",
    "issue": "https://github.com/sagemath/sage/issues/34872",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34872#issuecomment-564356",
    "user": "https://github.com/yyyyx4"
}
```

**Author:** Lorenz Panny



---

archive/issue_events_310503.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2022-12-21T15:42:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34872",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34872#event-310503"
}
```

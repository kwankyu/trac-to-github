# Issue 9932: Make PyWavelets an experimental package

archive/issues_009931.json:
```json
{
    "body": "PyWavelets is a Python wavelet transforms module that can do:\n\n* 1D and 2D Forward and Inverse Discrete Wavelet Transform (DWT and IDWT)\n* 1D and 2D Stationary Wavelet Transform (Undecimated Wavelet Transform)\n* 1D and 2D Wavelet Packet decomposition and reconstruction\n* Computing Approximations of wavelet and scaling functions\n* Over seventy built-in wavelet filters and support for custom wavelets\n* Single and double precision calculations\n* Results compatibility with Matlab Wavelet Toolbox (tm)\n\nDocumentation and links to more resources is available online\nat http://www.pybytes.com/pywavelets/.\n\n**Reviewer:** Jeroen Demeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/9932\n\n",
    "closed_at": "2016-02-23T22:59:46Z",
    "created_at": "2010-09-17T10:48:37Z",
    "labels": [
        "component: packages: experimental",
        "minor",
        "enhancement",
        "wontfix"
    ],
    "title": "Make PyWavelets an experimental package",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/9932",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```
PyWavelets is a Python wavelet transforms module that can do:

* 1D and 2D Forward and Inverse Discrete Wavelet Transform (DWT and IDWT)
* 1D and 2D Stationary Wavelet Transform (Undecimated Wavelet Transform)
* 1D and 2D Wavelet Packet decomposition and reconstruction
* Computing Approximations of wavelet and scaling functions
* Over seventy built-in wavelet filters and support for custom wavelets
* Single and double precision calculations
* Results compatibility with Matlab Wavelet Toolbox (tm)

Documentation and links to more resources is available online
at http://www.pybytes.com/pywavelets/.

**Reviewer:** Jeroen Demeyer

Issue created by migration from https://trac.sagemath.org/ticket/9932





---

archive/issue_comments_091685.json:
```json
{
    "body": "<a id='comment:1'></a>\nFirst attempt is up at:\n\n[http://sage.math.washington.edu/home/mhampton/PyWavelets-0.2.0.p0.spkg](http://sage.math.washington.edu/home/mhampton/PyWavelets-0.2.0.p0.spkg)",
    "created_at": "2010-09-17T10:49:15Z",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9932#issuecomment-91685",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

<a id='comment:1'></a>
First attempt is up at:

[http://sage.math.washington.edu/home/mhampton/PyWavelets-0.2.0.p0.spkg](http://sage.math.washington.edu/home/mhampton/PyWavelets-0.2.0.p0.spkg)



---

archive/issue_comments_091686.json:
```json
{
    "body": "<a id='comment:2'></a>\nOld-style packages are obsolete.",
    "created_at": "2016-01-22T10:52:58Z",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9932#issuecomment-91686",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'></a>
Old-style packages are obsolete.



---

archive/issue_comments_091687.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2016-01-22T10:52:58Z",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9932#issuecomment-91687",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_events_074904.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-22T10:52:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9932#event-74904"
}
```



---

archive/issue_events_074905.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-22T10:53:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9932#event-74905"
}
```



---

archive/issue_events_074906.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-22T10:53:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9932#event-74906"
}
```



---

archive/issue_events_074907.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-02-23T22:59:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9932#event-74907"
}
```



---

archive/issue_events_074908.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-02-23T22:59:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9932#event-74908"
}
```



---

archive/issue_events_074909.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-02-23T22:59:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9932",
    "label": "wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9932#event-74909"
}
```

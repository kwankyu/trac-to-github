# Issue 9838: Bugfix in WeylCharacterRing __call__ method

archive/issues_009837.json:
```json
{
    "body": "This addresses a bug that was reported here:\n\nhttp://groups.google.com/group/sage-combinat-devel/msg/252fd7fa0e297214\n\nThe `__call__` method of a Weyl Character ring, when `style=\"coroots\"` is specified, tries to interpret the arguments as the coroots of a weight; that weight\nis then the actual argument. However this is not appropriate if the argument is\nnot a tuple. Therefore this should be tested.\n\nThe patch implements the test.\n\n**Assignee:** @dwbump\n\n**CC:**  sage-combinat\n\n**Author:** Daniel Bump\n\n**Reviewer:** Anne Schilling\n\n**Merged:** sage-4.6.1.alpha1\n\nIssue created by migration from https://trac.sagemath.org/ticket/9838\n\n",
    "closed_at": "2010-11-10T22:20:13Z",
    "created_at": "2010-08-29T19:03:03Z",
    "labels": [
        "component: group theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.1",
    "title": "Bugfix in WeylCharacterRing __call__ method",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/9838",
    "user": "https://github.com/dwbump"
}
```
This addresses a bug that was reported here:

http://groups.google.com/group/sage-combinat-devel/msg/252fd7fa0e297214

The `__call__` method of a Weyl Character ring, when `style="coroots"` is specified, tries to interpret the arguments as the coroots of a weight; that weight
is then the actual argument. However this is not appropriate if the argument is
not a tuple. Therefore this should be tested.

The patch implements the test.

**Assignee:** @dwbump

**CC:**  sage-combinat

**Author:** Daniel Bump

**Reviewer:** Anne Schilling

**Merged:** sage-4.6.1.alpha1

Issue created by migration from https://trac.sagemath.org/ticket/9838





---

archive/issue_comments_089816.json:
```json
{
    "body": "**Changing assignee** from @aghitza to @dwbump.",
    "created_at": "2010-08-29T19:08:17Z",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9838#issuecomment-89816",
    "user": "https://github.com/dwbump"
}
```

**Changing assignee** from @aghitza to @dwbump.



---

archive/issue_events_073968.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-08-29T19:08:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73968"
}
```



---

archive/issue_events_073969.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-08-29T19:10:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "component: algebra",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73969"
}
```



---

archive/issue_events_073970.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-08-29T19:10:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "component: group theory",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73970"
}
```



---

archive/issue_comments_089817.json:
```json
{
    "body": "<a id='comment:4'></a>\nReplying to [@dwbump](#comment%3A3):\n\nThis is a bug fix. All tests pass!",
    "created_at": "2010-10-19T06:31:31Z",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9838#issuecomment-89817",
    "user": "https://github.com/anneschilling"
}
```

<a id='comment:4'></a>
Replying to [@dwbump](#comment%3A3):

This is a bug fix. All tests pass!



---

archive/issue_events_073971.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2010-10-19T06:31:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73971"
}
```



---

archive/issue_events_073972.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2010-10-19T06:31:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73972"
}
```



---

archive/issue_comments_089818.json:
```json
{
    "body": "**Changing author** from \"bump\" to \"Daniel Bump\".",
    "created_at": "2010-10-26T08:32:36Z",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9838#issuecomment-89818",
    "user": "https://github.com/jdemeyer"
}
```

**Changing author** from "bump" to "Daniel Bump".



---

archive/issue_comments_089819.json:
```json
{
    "body": "**Reviewer:** Anne Schilling",
    "created_at": "2010-10-26T08:32:36Z",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9838#issuecomment-89819",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Anne Schilling



---

archive/issue_events_073973.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-10-26T08:32:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "milestone": "sage-4.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73973"
}
```



---

archive/issue_events_073974.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-10-26T08:32:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "milestone": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73974"
}
```



---

archive/issue_comments_089820.json:
```json
{
    "body": "<a id='comment:6'></a>\nPlease change the commit message of the patch trac_9838.patch (use `hg qrefresh -e` for that).",
    "created_at": "2010-10-26T13:34:43Z",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9838#issuecomment-89820",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'></a>
Please change the commit message of the patch trac_9838.patch (use `hg qrefresh -e` for that).



---

archive/issue_events_073975.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-10-26T13:34:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73975"
}
```



---

archive/issue_events_073976.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-10-26T13:34:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73976"
}
```



---

archive/issue_comments_089821.json:
```json
{
    "body": "<a id='comment:7'></a>\n> Please change the commit message of the patch trac_9838.patch (use hg qrefresh -e for that).\n\nDone. -Dan",
    "created_at": "2010-10-26T15:19:11Z",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9838#issuecomment-89821",
    "user": "https://github.com/dwbump"
}
```

<a id='comment:7'></a>
> Please change the commit message of the patch trac_9838.patch (use hg qrefresh -e for that).

Done. -Dan



---

archive/issue_events_073977.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-10-26T15:19:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73977"
}
```



---

archive/issue_events_073978.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-10-26T15:19:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73978"
}
```



---

archive/issue_comments_089822.json:
```json
{
    "body": "<a id='comment:8'></a>\nReplying to [@dwbump](#comment%3A7):\n> Done. -Dan\n\nSorry, the ticket number should also be in the first line of the commit message.",
    "created_at": "2010-10-27T08:52:20Z",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9838#issuecomment-89822",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'></a>
Replying to [@dwbump](#comment%3A7):
> Done. -Dan

Sorry, the ticket number should also be in the first line of the commit message.



---

archive/issue_events_073979.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-10-27T08:52:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73979"
}
```



---

archive/issue_events_073980.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-10-27T08:52:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73980"
}
```



---

archive/attachments_013191.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_9838.patch",
    "asset_url": "tarball://root/attachments/ticket9838/trac_9838.patch",
    "created_at": "2010-10-27T16:25:02Z",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket9838/trac_9838.patch",
    "user": "https://github.com/dwbump"
}
```



---

archive/issue_comments_089823.json:
```json
{
    "body": "**Attachment:** [trac_9838.patch](https://github.com/sagemath/sage/files/ticket9838/trac_9838.patch)\n\n#9838: bugfix in WeylCharac terRing call method",
    "created_at": "2010-10-27T16:25:02Z",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9838#issuecomment-89823",
    "user": "https://github.com/dwbump"
}
```

**Attachment:** [trac_9838.patch](https://github.com/sagemath/sage/files/ticket9838/trac_9838.patch)

#9838: bugfix in WeylCharac terRing call method



---

archive/issue_events_073981.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-10-27T16:26:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73981"
}
```



---

archive/issue_events_073982.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-10-27T16:26:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73982"
}
```



---

archive/issue_comments_089824.json:
```json
{
    "body": "<a id='comment:9'></a>\n> Sorry, the ticket number should also be in the first line of the commit message. \n\nDone.",
    "created_at": "2010-10-27T16:26:11Z",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9838#issuecomment-89824",
    "user": "https://github.com/dwbump"
}
```

<a id='comment:9'></a>
> Sorry, the ticket number should also be in the first line of the commit message. 

Done.



---

archive/issue_comments_089825.json:
```json
{
    "body": "**Merged:** sage-4.6.1.alpha0",
    "created_at": "2010-11-01T10:11:20Z",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9838#issuecomment-89825",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-4.6.1.alpha0



---

archive/issue_events_073983.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-01T10:11:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73983"
}
```



---

archive/issue_events_073984.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-01T10:11:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73984"
}
```



---

archive/issue_events_073985.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-02T15:34:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73985"
}
```



---

archive/issue_comments_089826.json:
```json
{
    "body": "**Changing merged** from \"sage-4.6.1.alpha0\" to \"\".",
    "created_at": "2010-11-02T15:34:34Z",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9838#issuecomment-89826",
    "user": "https://github.com/jdemeyer"
}
```

**Changing merged** from "sage-4.6.1.alpha0" to "".



---

archive/issue_comments_089827.json:
```json
{
    "body": "<a id='comment:12'></a>\nDue to a mistake by me (confusing #9838 with #9893), this ticket did not get merged in sage-4.6.1.alpha0.",
    "created_at": "2010-11-02T15:35:17Z",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9838#issuecomment-89827",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:12'></a>
Due to a mistake by me (confusing #9838 with #9893), this ticket did not get merged in sage-4.6.1.alpha0.



---

archive/issue_events_073986.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-02T15:35:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73986"
}
```



---

archive/issue_events_073987.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-02T15:35:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73987"
}
```



---

archive/issue_comments_089828.json:
```json
{
    "body": "<a id='comment:13'></a>\nThe ticket is still described as resolved:fixed.\n\nI don't think I can revert the fixed status: trac admin has to do that.\n(It doesn't matter if this won't cause the release manager to\nforget the patch.)",
    "created_at": "2010-11-02T23:36:18Z",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9838#issuecomment-89828",
    "user": "https://github.com/dwbump"
}
```

<a id='comment:13'></a>
The ticket is still described as resolved:fixed.

I don't think I can revert the fixed status: trac admin has to do that.
(It doesn't matter if this won't cause the release manager to
forget the patch.)



---

archive/issue_events_073988.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-11-04T11:43:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73988"
}
```



---

archive/issue_events_073989.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-11-04T11:43:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73989"
}
```



---

archive/issue_events_073990.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-11-04T11:43:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73990"
}
```



---

archive/issue_events_073991.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-11-04T11:44:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73991"
}
```



---

archive/issue_events_073992.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-11-04T11:44:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73992"
}
```



---

archive/issue_comments_089829.json:
```json
{
    "body": "<a id='comment:18'></a>\nReplying to [@dwbump](#comment%3A13):\n> I don't think I can revert the fixed status: trac admin has to do that.\n\nDone.",
    "created_at": "2010-11-04T11:45:07Z",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9838#issuecomment-89829",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:18'></a>
Replying to [@dwbump](#comment%3A13):
> I don't think I can revert the fixed status: trac admin has to do that.

Done.



---

archive/issue_events_073993.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-10T22:20:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73993"
}
```



---

archive/issue_events_073994.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-10T22:20:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9838#event-73994"
}
```



---

archive/issue_comments_089830.json:
```json
{
    "body": "**Merged:** sage-4.6.1.alpha1",
    "created_at": "2010-11-10T22:20:13Z",
    "issue": "https://github.com/sagemath/sage/issues/9838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9838#issuecomment-89830",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-4.6.1.alpha1

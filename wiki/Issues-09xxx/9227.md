# Issue 9227: units.length.millimeter missing

archive/issues_009227.json:
```json
{
    "assignees": [
        "https://github.com/aghitza"
    ],
    "body": "<div id=\"comment:0\"></div>\n\na millimeter [mm] is 1/1000 of a meter and very often used by engineers.\n\nCC:  asjarman@xtra.co.nz\n\nComponent: **misc**\n\nAuthor: **Martin Allen**\n\nReviewer: **Simon Spicer**\n\nMerged: **sage-4.6.2.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9227_\n\n",
    "closed_at": "2011-01-25T08:14:40Z",
    "created_at": "2010-06-12T12:55:42Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p%3A%20trivial%20/%205",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "units.length.millimeter missing",
    "type": "issue",
    "updated_at": "2011-01-25T08:14:40Z",
    "url": "https://github.com/sagemath/sage/issues/9227",
    "user": "https://github.com/haraldschilly"
}
```
<div id="comment:0"></div>

a millimeter [mm] is 1/1000 of a meter and very often used by engineers.

CC:  asjarman@xtra.co.nz

Component: **misc**

Author: **Martin Allen**

Reviewer: **Simon Spicer**

Merged: **sage-4.6.2.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/9227_





---

archive/issue_events_116903.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-06-12T12:55:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9227#event-116903"
}
```



---

archive/issue_events_116904.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-06-12T12:55:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9227#event-116904"
}
```



---

archive/issue_events_116905.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-06-12T12:55:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20trivial%20/%205",
    "label_color": "fff9e5",
    "label_name": "p: trivial / 5",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9227#event-116905"
}
```



---

archive/issue_events_116906.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-06-12T12:55:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9227#event-116906"
}
```



---

archive/issue_events_116907.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-06-12T12:55:42Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "subject": "https://github.com/haraldschilly",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9227#event-116907"
}
```



---

archive/issue_comments_077370.json:
```json
{
    "body": "Attachment: **[trac_9227_mm_and_km_units.patch.gz](https://github.com/sagemath/sage/files/ticket9227/trac_9227_mm_and_km_units.patch.gz)**\n\npatch to add mm and km",
    "created_at": "2010-08-27T23:01:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9227#issuecomment-77370",
    "user": "https://github.com/sagetrac-allmar"
}
```

Attachment: **[trac_9227_mm_and_km_units.patch.gz](https://github.com/sagemath/sage/files/ticket9227/trac_9227_mm_and_km_units.patch.gz)**

patch to add mm and km



---

archive/issue_events_116908.json:
```json
{
    "actor": "https://github.com/sagetrac-allmar",
    "created_at": "2010-08-29T06:07:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9227#event-116908"
}
```



---

archive/issue_events_116909.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-09-02T11:09:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9227#event-116909"
}
```



---

archive/issue_comments_077371.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nYou mentioned\n\n```\nEqual to 1000 meters.\n```\ntwice. You should replace one by a comparison to yards, feet or something else.",
    "created_at": "2011-01-07T09:02:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9227#issuecomment-77371",
    "user": "https://github.com/a-andre"
}
```

<div id="comment:3" align="right">comment:3</div>

You mentioned

```
Equal to 1000 meters.
```
twice. You should replace one by a comparison to yards, feet or something else.



---

archive/issue_comments_077372.json:
```json
{
    "body": "Attachment: **[trac_9227_mm_and_km_units.2.patch.gz](https://github.com/sagemath/sage/files/ticket9227/trac_9227_mm_and_km_units.2.patch.gz)**",
    "created_at": "2011-01-07T22:56:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9227#issuecomment-77372",
    "user": "https://github.com/haikona"
}
```

Attachment: **[trac_9227_mm_and_km_units.2.patch.gz](https://github.com/sagemath/sage/files/ticket9227/trac_9227_mm_and_km_units.2.patch.gz)**



---

archive/issue_comments_077373.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nDoctests fail due to there being TAB characters in this patch.\n\nI've added an updated patch, which replaces the TAB characters in the patch with spaces, as well as changing line 650:\n`'kilometer':'Equal to 1000 meters.\\nEqual to 1000 meters.',`\nto\n`'kilometer':'Equal to 1000 meters.\\nEqual to 3280.8399 feet.',`",
    "created_at": "2011-01-07T23:04:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9227#issuecomment-77373",
    "user": "https://github.com/haikona"
}
```

<div id="comment:4" align="right">comment:4</div>

Doctests fail due to there being TAB characters in this patch.

I've added an updated patch, which replaces the TAB characters in the patch with spaces, as well as changing line 650:
`'kilometer':'Equal to 1000 meters.\nEqual to 1000 meters.',`
to
`'kilometer':'Equal to 1000 meters.\nEqual to 3280.8399 feet.',`



---

archive/issue_comments_077374.json:
```json
{
    "body": "Author: **Martin Allen**",
    "created_at": "2011-01-07T23:04:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9227#issuecomment-77374",
    "user": "https://github.com/haikona"
}
```

Author: **Martin Allen**



---

archive/issue_comments_077375.json:
```json
{
    "body": "Reviewer: **Simon Spicer**",
    "created_at": "2011-01-07T23:04:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9227#issuecomment-77375",
    "user": "https://github.com/haikona"
}
```

Reviewer: **Simon Spicer**



---

archive/issue_comments_077376.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nLooks good to me.",
    "created_at": "2011-01-10T01:12:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9227#issuecomment-77376",
    "user": "https://github.com/adeines"
}
```

<div id="comment:5" align="right">comment:5</div>

Looks good to me.



---

archive/issue_events_116910.json:
```json
{
    "actor": "https://github.com/adeines",
    "created_at": "2011-01-10T01:12:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9227#event-116910"
}
```



---

archive/issue_events_116911.json:
```json
{
    "actor": "https://github.com/adeines",
    "created_at": "2011-01-10T01:12:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9227#event-116911"
}
```



---

archive/issue_events_116912.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-10T02:18:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9227#event-116912"
}
```



---

archive/issue_events_116913.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-10T02:18:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9227#event-116913"
}
```



---

archive/issue_comments_077377.json:
```json
{
    "body": "Merged: **sage-4.6.2.alpha2**",
    "created_at": "2011-01-25T08:14:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9227#issuecomment-77377",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.6.2.alpha2**



---

archive/issue_events_116914.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-25T08:14:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9227#event-116914"
}
```



---

archive/issue_events_116915.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-25T08:14:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9227",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9227#event-116915"
}
```

# Issue 9669: Create contour_plot3d

archive/issues_009669.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-olazo"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThere is an excelent 2d contour_plot. We should also have 3d one.\n\nCC:  @kcrisman\n\nComponent: **graphics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9669_\n\n",
    "created_at": "2010-08-02T16:09:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graphics",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Create contour_plot3d",
    "type": "issue",
    "updated_at": "2011-04-20T02:53:15Z",
    "url": "https://github.com/sagemath/sage/issues/9669",
    "user": "https://github.com/sagetrac-olazo"
}
```
<div id="comment:0"></div>

There is an excelent 2d contour_plot. We should also have 3d one.

CC:  @kcrisman

Component: **graphics**

_Issue created by migration from https://trac.sagemath.org/ticket/9669_





---

archive/issue_events_123685.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2010-08-02T16:09:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9669",
    "milestone_number": null,
    "milestone_title": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9669#event-123685"
}
```



---

archive/issue_events_123686.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2010-08-02T16:09:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9669",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "c: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9669#event-123686"
}
```



---

archive/issue_events_123687.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2010-08-02T16:09:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9669",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9669#event-123687"
}
```



---

archive/issue_events_123688.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2010-08-02T16:09:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9669",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9669#event-123688"
}
```



---

archive/issue_events_123689.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2010-08-02T16:09:00Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/9669",
    "subject": "https://github.com/sagetrac-olazo",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9669#event-123689"
}
```



---

archive/issue_comments_084756.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nCan you attach a picture or two of exactly what you would have in mind for this?",
    "created_at": "2011-04-20T02:52:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9669",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9669#issuecomment-84756",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:1" align="right">comment:1</div>

Can you attach a picture or two of exactly what you would have in mind for this?



---

archive/issue_events_123690.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-04-20T02:53:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9669",
    "milestone_number": null,
    "milestone_title": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9669#event-123690"
}
```



---

archive/issue_events_123691.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-04-20T02:53:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9669",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9669#event-123691"
}
```



---

archive/issue_events_123692.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-04-20T02:53:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9669",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9669#event-123692"
}
```

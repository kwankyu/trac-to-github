# Issue 9392: Broken docstring in binpacking method

archive/issues_009392.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout",
        "https://github.com/sagetrac-jkantor"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nNot really broken, but subject to change of behaviours depending on the solver used.... ;-)\n\nNathann\n\nCC:  @rlmill @nexttime\n\nComponent: **numerical**\n\nAuthor: **Nathann Cohen**\n\nReviewer: **Leif Leonhardy, Robert Miller**\n\nMerged: **sage-4.5.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9392_\n\n",
    "closed_at": "2010-07-05T22:33:49Z",
    "created_at": "2010-06-30T05:46:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20numerical",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Broken docstring in binpacking method",
    "type": "issue",
    "updated_at": "2010-07-06T04:03:16Z",
    "url": "https://github.com/sagemath/sage/issues/9392",
    "user": "https://github.com/nathanncohen"
}
```
<div id="comment:0"></div>

Not really broken, but subject to change of behaviours depending on the solver used.... ;-)

Nathann

CC:  @rlmill @nexttime

Component: **numerical**

Author: **Nathann Cohen**

Reviewer: **Leif Leonhardy, Robert Miller**

Merged: **sage-4.5.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/9392_





---

archive/issue_events_119274.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-06-30T05:46:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "milestone_number": null,
    "milestone_title": "sage-4.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9392#event-119274"
}
```



---

archive/issue_events_119275.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-06-30T05:46:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "c: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9392#event-119275"
}
```



---

archive/issue_events_119276.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-06-30T05:46:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9392#event-119276"
}
```



---

archive/issue_events_119277.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-06-30T05:46:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9392#event-119277"
}
```



---

archive/issue_events_119278.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-06-30T05:46:10Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "subject": "https://github.com/nathanncohen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9392#event-119278"
}
```



---

archive/issue_events_119279.json:
```json
{
    "actor": "https://github.com/sagetrac-jkantor",
    "created_at": "2010-06-30T05:46:10Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "subject": "https://github.com/nathanncohen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9392#event-119279"
}
```



---

archive/issue_events_119280.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-06-30T05:47:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9392#event-119280"
}
```



---

archive/issue_comments_080198.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_9392.patch.gz](https://github.com/sagemath/sage/files/ticket9392/trac_9392.patch.gz)**\n\nHere it is !\n\nNathann",
    "created_at": "2010-06-30T05:47:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9392#issuecomment-80198",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_9392.patch.gz](https://github.com/sagemath/sage/files/ticket9392/trac_9392.patch.gz)**

Here it is !

Nathann



---

archive/issue_comments_080199.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nNathann's patch fails on normal (non-optional) doctesting because of undefined variables/some sections *not* marked \"optional\".",
    "created_at": "2010-07-04T20:15:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9392#issuecomment-80199",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:3" align="right">comment:3</div>

Nathann's patch fails on normal (non-optional) doctesting because of undefined variables/some sections *not* marked "optional".



---

archive/issue_events_119281.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2010-07-04T20:15:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9392#event-119281"
}
```



---

archive/issue_events_119282.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2010-07-04T20:15:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9392#event-119282"
}
```



---

archive/issue_comments_080200.json:
```json
{
    "body": "Attachment: **[trac_9392-first_reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket9392/trac_9392-first_reviewer.patch.gz)**\n\nFixes non-optional doctesting. Apply on top of Nathann's patch.",
    "created_at": "2010-07-04T20:16:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9392#issuecomment-80200",
    "user": "https://github.com/nexttime"
}
```

Attachment: **[trac_9392-first_reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket9392/trac_9392-first_reviewer.patch.gz)**

Fixes non-optional doctesting. Apply on top of Nathann's patch.



---

archive/issue_events_119283.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2010-07-04T20:25:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9392#event-119283"
}
```



---

archive/issue_events_119284.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2010-07-04T20:25:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9392#event-119284"
}
```



---

archive/issue_comments_080201.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nWith my reviewer patch applied, at least passes reasonable tests (`-long`, `-long` with `-only-optional=cbc,glpk` in `sage/numerical`) on a 32-bit system where the doctest previously did *not* fail...\n\nLeaving as \"needs review\" for further testing.",
    "created_at": "2010-07-04T20:25:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9392#issuecomment-80201",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:4" align="right">comment:4</div>

With my reviewer patch applied, at least passes reasonable tests (`-long`, `-long` with `-only-optional=cbc,glpk` in `sage/numerical`) on a 32-bit system where the doctest previously did *not* fail...

Leaving as "needs review" for further testing.



---

archive/issue_comments_080202.json:
```json
{
    "body": "Author: **Nathann Cohen**",
    "created_at": "2010-07-05T21:27:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9392#issuecomment-80202",
    "user": "https://github.com/rlmill"
}
```

Author: **Nathann Cohen**



---

archive/issue_comments_080203.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nDoesn't this depend on #9312?",
    "created_at": "2010-07-05T21:27:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9392#issuecomment-80203",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:5" align="right">comment:5</div>

Doesn't this depend on #9312?



---

archive/issue_comments_080204.json:
```json
{
    "body": "Reviewer: **Leif Leonhardy, Robert Miller**",
    "created_at": "2010-07-05T21:27:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9392#issuecomment-80204",
    "user": "https://github.com/rlmill"
}
```

Reviewer: **Leif Leonhardy, Robert Miller**



---

archive/issue_comments_080205.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nNot really... Though if we say it depends on #9312, then we do not need to add these \"optional\" flags anymore :-D\n\nNathann",
    "created_at": "2010-07-05T21:33:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9392#issuecomment-80205",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:6" align="right">comment:6</div>

Not really... Though if we say it depends on #9312, then we do not need to add these "optional" flags anymore :-D

Nathann



---

archive/issue_events_119285.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-07-05T22:32:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9392#event-119285"
}
```



---

archive/issue_events_119286.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-07-05T22:32:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9392#event-119286"
}
```



---

archive/issue_comments_080206.json:
```json
{
    "body": "Merged: **sage-4.5.alpha3**",
    "created_at": "2010-07-05T22:33:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9392#issuecomment-80206",
    "user": "https://github.com/rlmill"
}
```

Merged: **sage-4.5.alpha3**



---

archive/issue_events_119287.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-07-05T22:33:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9392#event-119287"
}
```



---

archive/issue_events_119288.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-07-05T22:33:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9392#event-119288"
}
```



---

archive/issue_comments_080207.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nWell, if GLPK gets a standard package, we should **definitely** remove the `optional` tags, since otherwise these tests are omitted in the usual test process.\n\n(We could just substitute `optional` by `standard` to make life easier in case the package is made optional again for some reason. Same for Nathann's patch at #9312.)",
    "created_at": "2010-07-05T23:11:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9392#issuecomment-80207",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:9" align="right">comment:9</div>

Well, if GLPK gets a standard package, we should **definitely** remove the `optional` tags, since otherwise these tests are omitted in the usual test process.

(We could just substitute `optional` by `standard` to make life easier in case the package is made optional again for some reason. Same for Nathann's patch at #9312.)



---

archive/issue_comments_080208.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nReplying to [@nexttime](#comment%3A9):\n> Well, if GLPK gets a standard package, we should **definitely** remove the `optional` tags, since otherwise these tests are omitted in the usual test process.\n\nThis is addressed at #9432.",
    "created_at": "2010-07-06T04:03:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9392#issuecomment-80208",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:10" align="right">comment:10</div>

Replying to [@nexttime](#comment%3A9):
> Well, if GLPK gets a standard package, we should **definitely** remove the `optional` tags, since otherwise these tests are omitted in the usual test process.

This is addressed at #9432.

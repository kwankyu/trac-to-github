# Issue 9629: sage -merge should look for sensible-editor before pico

archive/issues_009629.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nI don't have pico installed, and hadn't set $EDITOR, so \"sage -merge\" bombed out; but on Debian-like systems, there's always sensible-editor, which is pretty much what you'd expect. The sage -merge script should look for that before looking for pico.\n\n(Also, uppity free software types would argue that you should look for nano, not pico...)\n\nComponent: **misc**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9629_\n\n",
    "closed_at": "2014-06-23T19:15:58Z",
    "created_at": "2010-07-29T00:32:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage -merge should look for sensible-editor before pico",
    "type": "issue",
    "updated_at": "2014-06-23T19:15:58Z",
    "url": "https://github.com/sagemath/sage/issues/9629",
    "user": "https://github.com/dandrake"
}
```
<div id="comment:0"></div>

I don't have pico installed, and hadn't set $EDITOR, so "sage -merge" bombed out; but on Debian-like systems, there's always sensible-editor, which is pretty much what you'd expect. The sage -merge script should look for that before looking for pico.

(Also, uppity free software types would argue that you should look for nano, not pico...)

Component: **misc**

_Issue created by migration from https://trac.sagemath.org/ticket/9629_





---

archive/issue_events_123068.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-07-29T00:32:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9629#event-123068"
}
```



---

archive/issue_events_123069.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-07-29T00:32:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9629#event-123069"
}
```



---

archive/issue_events_123070.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-07-29T00:32:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9629#event-123070"
}
```



---

archive/issue_events_123071.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-07-29T00:32:23Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "subject": "https://github.com/dandrake",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9629#event-123071"
}
```



---

archive/issue_events_123072.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9629#event-123072"
}
```



---

archive/issue_events_123073.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9629#event-123073"
}
```



---

archive/issue_events_123074.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9629#event-123074"
}
```



---

archive/issue_events_123075.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9629#event-123075"
}
```



---

archive/issue_events_123076.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9629#event-123076"
}
```



---

archive/issue_events_123077.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9629#event-123077"
}
```



---

archive/issue_events_123078.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-06-22T16:01:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9629#event-123078"
}
```



---

archive/issue_comments_084159.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\n`-merge` option is gone.",
    "created_at": "2014-06-22T16:01:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9629#issuecomment-84159",
    "user": "https://github.com/a-andre"
}
```

<div id="comment:4" align="right">comment:4</div>

`-merge` option is gone.



---

archive/issue_events_123079.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-06-22T16:01:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9629#event-123079"
}
```



---

archive/issue_events_123080.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-06-22T16:16:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9629#event-123080"
}
```



---

archive/issue_events_123081.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-06-22T16:16:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9629#event-123081"
}
```



---

archive/issue_comments_084160.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nReplying to [@a-andre](#comment%3A4):\n> `-merge` option is gone.\n\nGreat, one ticket less!",
    "created_at": "2014-06-22T16:16:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9629#issuecomment-84160",
    "user": "https://github.com/videlec"
}
```

<div id="comment:5" align="right">comment:5</div>

Replying to [@a-andre](#comment%3A4):
> `-merge` option is gone.

Great, one ticket less!



---

archive/issue_events_123082.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-06-23T19:15:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9629#event-123082"
}
```



---

archive/issue_events_123083.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-06-23T19:15:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9629",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9629#event-123083"
}
```

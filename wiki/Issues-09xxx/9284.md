# Issue 9284: Locales switching in web UI

archive/issues_009284.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nCurrently on SAGE online (at least on http://www.sagenb.org/) account user settings, the user cannot choose what language to have one's SAGE session in.  If this were enabled, then more users might be attracted to SAGE.  Having the user specify his preferred SAGE session language as the default when setting up a new account will attract more users worldwide.\n\nCC:  @mkoeppe\n\nComponent: **user interface**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9284_\n\n",
    "closed_at": "2021-08-20T13:58:41Z",
    "created_at": "2010-06-20T19:46:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Locales switching in web UI",
    "type": "issue",
    "updated_at": "2021-08-20T13:58:41Z",
    "url": "https://github.com/sagemath/sage/issues/9284",
    "user": "https://github.com/sagetrac-q10"
}
```
<div id="comment:0"></div>

Currently on SAGE online (at least on http://www.sagenb.org/) account user settings, the user cannot choose what language to have one's SAGE session in.  If this were enabled, then more users might be attracted to SAGE.  Having the user specify his preferred SAGE session language as the default when setting up a new account will attract more users worldwide.

CC:  @mkoeppe

Component: **user interface**

_Issue created by migration from https://trac.sagemath.org/ticket/9284_





---

archive/issue_events_117709.json:
```json
{
    "actor": "https://github.com/sagetrac-q10",
    "created_at": "2010-06-20T19:46:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9284",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9284#event-117709"
}
```



---

archive/issue_events_117710.json:
```json
{
    "actor": "https://github.com/sagetrac-q10",
    "created_at": "2010-06-20T19:46:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9284",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9284#event-117710"
}
```



---

archive/issue_events_117711.json:
```json
{
    "actor": "https://github.com/sagetrac-q10",
    "created_at": "2010-06-20T19:46:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9284",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9284#event-117711"
}
```



---

archive/issue_events_117712.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-06-20T19:46:48Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/9284",
    "subject": "https://github.com/sagetrac-q10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9284#event-117712"
}
```



---

archive/issue_comments_078261.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nobsolete ?",
    "created_at": "2020-06-29T07:40:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9284",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9284#issuecomment-78261",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">comment:1</div>

obsolete ?



---

archive/issue_events_117713.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-06-29T07:40:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9284",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9284#event-117713"
}
```



---

archive/issue_comments_078262.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThe Jupyter Project supports interface\ninternationalisation and localisation.\n\nSee for instance:\n\n- https://jupyterlab.readthedocs.io/en/latest/user/language.html",
    "created_at": "2021-08-20T08:34:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9284",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9284#issuecomment-78262",
    "user": "https://github.com/slel"
}
```

<div id="comment:2" align="right">comment:2</div>

The Jupyter Project supports interface
internationalisation and localisation.

See for instance:

- https://jupyterlab.readthedocs.io/en/latest/user/language.html



---

archive/issue_comments_078263.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nso can we close ?",
    "created_at": "2021-08-20T11:55:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9284",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9284#issuecomment-78263",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">comment:3</div>

so can we close ?



---

archive/issue_events_117714.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-08-20T13:26:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9284",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9284#event-117714"
}
```



---

archive/issue_events_117715.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-08-20T13:26:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9284",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9284#event-117715"
}
```



---

archive/issue_comments_078264.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThis was about the sagenb.org server, so let's close.",
    "created_at": "2021-08-20T13:26:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9284",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9284#issuecomment-78264",
    "user": "https://github.com/slel"
}
```

<div id="comment:4" align="right">comment:4</div>

This was about the sagenb.org server, so let's close.



---

archive/issue_events_117716.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-08-20T13:58:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9284",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9284#event-117716"
}
```



---

archive/issue_events_117717.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-08-20T13:58:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9284",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9284#event-117717"
}
```

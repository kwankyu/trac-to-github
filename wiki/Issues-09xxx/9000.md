# Issue 9000: sage notebook: change default interact color selector

archive/issues_009000.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout",
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nPeople voted yes to the following:\n\n```\nWould anybody be opposed to me changing the default for \ncolor_select from 'farbtastic' to 'colorpicker'.  The \nfarbtastic color picker is *HUGE*, whereas the 'colorpicker' \none is much smaller and more usable.\n```\n\nComponent: **notebook**\n\nAuthor: **William Stein**\n\nReviewer: **Tim Dumol**\n\nMerged: **sagenb-0.8.3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9000_\n\n",
    "closed_at": "2010-10-04T01:34:43Z",
    "created_at": "2010-05-20T19:37:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage notebook: change default interact color selector",
    "type": "issue",
    "updated_at": "2014-10-01T15:54:26Z",
    "url": "https://github.com/sagemath/sage/issues/9000",
    "user": "https://github.com/williamstein"
}
```
<div id="comment:0"></div>

People voted yes to the following:

```
Would anybody be opposed to me changing the default for 
color_select from 'farbtastic' to 'colorpicker'.  The 
farbtastic color picker is *HUGE*, whereas the 'colorpicker' 
one is much smaller and more usable.
```

Component: **notebook**

Author: **William Stein**

Reviewer: **Tim Dumol**

Merged: **sagenb-0.8.3**

_Issue created by migration from https://trac.sagemath.org/ticket/9000_





---

archive/issue_events_113350.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-05-20T19:37:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "milestone_number": null,
    "milestone_title": "sage-4.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9000#event-113350"
}
```



---

archive/issue_events_113351.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-05-20T19:37:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9000#event-113351"
}
```



---

archive/issue_events_113352.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-05-20T19:37:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9000#event-113352"
}
```



---

archive/issue_events_113353.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-05-20T19:37:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9000#event-113353"
}
```



---

archive/issue_events_113354.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-05-20T19:37:34Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9000#event-113354"
}
```



---

archive/issue_events_113355.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-05-20T19:37:34Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9000#event-113355"
}
```



---

archive/issue_comments_074111.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nI ended up changing it to jpicker instead of colorpicker, since after trying it out, frankly colorpicker sucks, whereas jpicker is very nice.",
    "created_at": "2010-08-11T18:58:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9000#issuecomment-74111",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>

I ended up changing it to jpicker instead of colorpicker, since after trying it out, frankly colorpicker sucks, whereas jpicker is very nice.



---

archive/issue_events_113356.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-08-11T18:58:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9000#event-113356"
}
```



---

archive/issue_events_113357.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-08-12T06:01:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9000#event-113357"
}
```



---

archive/issue_events_113358.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-08-12T06:01:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9000#event-113358"
}
```



---

archive/issue_comments_074112.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI think you may have accidentally changed the wrong function (`html_color_selector` instead of `color_selector`).",
    "created_at": "2010-08-12T06:01:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9000#issuecomment-74112",
    "user": "https://github.com/TimDumol"
}
```

<div id="comment:2" align="right">comment:2</div>

I think you may have accidentally changed the wrong function (`html_color_selector` instead of `color_selector`).



---

archive/issue_comments_074113.json:
```json
{
    "body": "Attachment: **[trac_9000-interact_color.patch.gz](https://github.com/sagemath/sage/files/ticket9000/trac_9000-interact_color.patch.gz)**\n\nnew version that fixes the (valid) issue that tim pointed out.",
    "created_at": "2010-08-14T02:22:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9000#issuecomment-74113",
    "user": "https://github.com/williamstein"
}
```

Attachment: **[trac_9000-interact_color.patch.gz](https://github.com/sagemath/sage/files/ticket9000/trac_9000-interact_color.patch.gz)**

new version that fixes the (valid) issue that tim pointed out.



---

archive/issue_events_113359.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-08-14T02:23:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9000#event-113359"
}
```



---

archive/issue_events_113360.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-08-14T02:23:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9000#event-113360"
}
```



---

archive/issue_events_113361.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-08-14T03:19:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9000#event-113361"
}
```



---

archive/issue_events_113362.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-08-14T03:19:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9000#event-113362"
}
```



---

archive/issue_comments_074114.json:
```json
{
    "body": "Author: **William Stein**",
    "created_at": "2010-08-14T03:19:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9000#issuecomment-74114",
    "user": "https://github.com/TimDumol"
}
```

Author: **William Stein**



---

archive/issue_comments_074115.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nLooks good to me. Doctests pass. Positive review.",
    "created_at": "2010-08-14T03:19:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9000#issuecomment-74115",
    "user": "https://github.com/TimDumol"
}
```

<div id="comment:4" align="right">comment:4</div>

Looks good to me. Doctests pass. Positive review.



---

archive/issue_comments_074116.json:
```json
{
    "body": "Reviewer: **Tim Dumol**",
    "created_at": "2010-08-14T03:19:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9000#issuecomment-74116",
    "user": "https://github.com/TimDumol"
}
```

Reviewer: **Tim Dumol**



---

archive/issue_events_113363.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-10-04T01:34:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9000#event-113363"
}
```



---

archive/issue_events_113364.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-10-04T01:34:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9000#event-113364"
}
```



---

archive/issue_comments_074117.json:
```json
{
    "body": "Merged: **sagenb-0.8.3**",
    "created_at": "2010-10-04T01:34:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9000#issuecomment-74117",
    "user": "https://github.com/qed777"
}
```

Merged: **sagenb-0.8.3**



---

archive/issue_comments_074118.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nApparently this never quite got changed, or maybe it just was changed back or something.  See [this commit](https://github.com/sagemath/sagenb/commit/ece3ebf2c5a5e4389013988182609501d9332121), which I accidentally put in without review, sigh...",
    "created_at": "2014-10-01T15:54:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9000",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9000#issuecomment-74118",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:6" align="right">comment:6</div>

Apparently this never quite got changed, or maybe it just was changed back or something.  See [this commit](https://github.com/sagemath/sagenb/commit/ece3ebf2c5a5e4389013988182609501d9332121), which I accidentally put in without review, sigh...

# Issue 9929: polynomial ring over pAdics doesn't respect the sparse argument

archive/issues_009928.json:
```json
{
    "body": "\n```\nsage: R.<x> = PolynomialRing(Zp(17), sparse=True)\nsage: x**(10**10)\nTraceback (most recent call last)\n...\nMemoryError:\n```\n\nThis should work (and be fast). The solution is probably in the file `polynomial_ring_constructor.py` in the function `single_variate`.\n\n**Assignee:** @aghitza\n\n**CC:**  @roed314 @xcaruso\n\n**Keywords:** padicIMA\n\n**Reviewer:** Julian R\u00fcth\n\n**Author:** Julian R\u00fcth\n\nIssue created by migration from https://trac.sagemath.org/ticket/9929\n\n",
    "closed_at": "2019-02-26T13:58:00Z",
    "created_at": "2010-09-17T07:27:14Z",
    "labels": [
        "component: algebra",
        "bug",
        "invalid"
    ],
    "title": "polynomial ring over pAdics doesn't respect the sparse argument",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/9929",
    "user": "https://trac.sagemath.org/admin/accounts/users/ylchapuy"
}
```

```
sage: R.<x> = PolynomialRing(Zp(17), sparse=True)
sage: x**(10**10)
Traceback (most recent call last)
...
MemoryError:
```

This should work (and be fast). The solution is probably in the file `polynomial_ring_constructor.py` in the function `single_variate`.

**Assignee:** @aghitza

**CC:**  @roed314 @xcaruso

**Keywords:** padicIMA

**Reviewer:** Julian Rüth

**Author:** Julian Rüth

Issue created by migration from https://trac.sagemath.org/ticket/9929





---

archive/issue_comments_091662.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1,10 @@\n-the answer is probably in the file `polynomial_ring_constructor.py` in the function `single_variate`\n+\n+```\n+sage: R.<x> = PolynomialRing(Zp(17), sparse=True)\n+sage: x**(10**10)\n+Traceback (most recent call last)\n+...\n+MemoryError:\n+```\n+\n+This should work (and be fast). The solution is probably in the file `polynomial_ring_constructor.py` in the function `single_variate`.\n``````\n",
    "created_at": "2010-10-04T14:24:48Z",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9929#issuecomment-91662",
    "user": "https://trac.sagemath.org/admin/accounts/users/ylchapuy"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1,10 @@
-the answer is probably in the file `polynomial_ring_constructor.py` in the function `single_variate`
+
+```
+sage: R.<x> = PolynomialRing(Zp(17), sparse=True)
+sage: x**(10**10)
+Traceback (most recent call last)
+...
+MemoryError:
+```
+
+This should work (and be fast). The solution is probably in the file `polynomial_ring_constructor.py` in the function `single_variate`.
``````




---

archive/issue_events_074876.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9929#event-74876"
}
```



---

archive/issue_events_074877.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9929#event-74877"
}
```



---

archive/issue_events_074878.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9929#event-74878"
}
```



---

archive/issue_events_074879.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9929#event-74879"
}
```



---

archive/issue_events_074880.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9929#event-74880"
}
```



---

archive/issue_events_074881.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9929#event-74881"
}
```



---

archive/issue_events_074882.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9929#event-74882"
}
```



---

archive/issue_events_074883.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9929#event-74883"
}
```



---

archive/issue_events_074884.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-07-26T22:23:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9929#event-74884"
}
```



---

archive/issue_events_074885.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-07-26T22:23:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9929#event-74885"
}
```



---

archive/issue_comments_091663.json:
```json
{
    "body": "**Changing keywords** from \"\" to \"padicIMA\".",
    "created_at": "2018-07-26T22:23:51Z",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9929#issuecomment-91663",
    "user": "https://github.com/saraedum"
}
```

**Changing keywords** from "" to "padicIMA".



---

archive/issue_comments_091664.json:
```json
{
    "body": "**Reviewer:** Julian R\u00fcth",
    "created_at": "2018-07-26T22:23:51Z",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9929#issuecomment-91664",
    "user": "https://github.com/saraedum"
}
```

**Reviewer:** Julian Rüth



---

archive/issue_comments_091665.json:
```json
{
    "body": "<a id='comment:6'></a>\nworks for me.",
    "created_at": "2018-07-26T22:23:51Z",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9929#issuecomment-91665",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:6'></a>
works for me.



---

archive/issue_comments_091666.json:
```json
{
    "body": "**Author:** Julian R\u00fcth",
    "created_at": "2018-07-26T22:23:51Z",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9929#issuecomment-91666",
    "user": "https://github.com/saraedum"
}
```

**Author:** Julian Rüth



---

archive/issue_events_074886.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-07-26T22:23:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9929#event-74886"
}
```



---

archive/issue_events_074887.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-07-26T22:23:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9929#event-74887"
}
```



---

archive/issue_events_074888.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-02-26T13:58:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9929#event-74888"
}
```



---

archive/issue_events_074889.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-02-26T13:58:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9929#event-74889"
}
```



---

archive/issue_comments_091667.json:
```json
{
    "body": "<a id='comment:8'></a>\nPresuming these are all correctly reviewed as either duplicate, invalid, or wontfix.",
    "created_at": "2019-02-26T13:58:00Z",
    "issue": "https://github.com/sagemath/sage/issues/9929",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9929#issuecomment-91667",
    "user": "https://github.com/embray"
}
```

<a id='comment:8'></a>
Presuming these are all correctly reviewed as either duplicate, invalid, or wontfix.

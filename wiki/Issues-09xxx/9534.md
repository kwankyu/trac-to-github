# Issue 9534: add base method for permutation groups

archive/issues_009534.json:
```json
{
    "body": "Patch to add a (working) base method for permutation groups.\n\n---\n\nApply [attachment:trac_9534-permgroup_base.patch](https://github.com/sagemath/sage/files/ticket9534/trac_9534-permgroup_base.patch) to the Sage library.\n\nDepends on #10334\n\nDepends on #10335\n\n**Assignee:** jasonbhill\n\n**Keywords:** base\n\n**Author:** Jason Hill\n\n**Reviewer:** Mike Hansen\n\n**Merged:** sage-4.7.2.alpha4\n\nIssue created by migration from https://trac.sagemath.org/ticket/9534\n\n",
    "closed_at": "2011-10-07T19:11:06Z",
    "created_at": "2010-07-17T23:33:27Z",
    "labels": [
        "component: group theory",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.2",
    "title": "add base method for permutation groups",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/9534",
    "user": "https://trac.sagemath.org/admin/accounts/users/jasonbhill"
}
```
Patch to add a (working) base method for permutation groups.

---

Apply [attachment:trac_9534-permgroup_base.patch](https://github.com/sagemath/sage/files/ticket9534/trac_9534-permgroup_base.patch) to the Sage library.

Depends on #10334

Depends on #10335

**Assignee:** jasonbhill

**Keywords:** base

**Author:** Jason Hill

**Reviewer:** Mike Hansen

**Merged:** sage-4.7.2.alpha4

Issue created by migration from https://trac.sagemath.org/ticket/9534





---

archive/issue_comments_084484.json:
```json
{
    "body": "<a id='comment:1'></a>\nThe existing base method returned \"Integer Ring\" for all permutation groups as it was inherited from ParentWithBase. This new base method uses GAP's base from stabilizer chain method to return an actual base, with an optional seed.\n\nThe patch was placed in the combinat queue as it depends on Mike Hansen's domain modifications.",
    "created_at": "2010-07-18T22:03:28Z",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9534#issuecomment-84484",
    "user": "https://trac.sagemath.org/admin/accounts/users/jasonbhill"
}
```

<a id='comment:1'></a>
The existing base method returned "Integer Ring" for all permutation groups as it was inherited from ParentWithBase. This new base method uses GAP's base from stabilizer chain method to return an actual base, with an optional seed.

The patch was placed in the combinat queue as it depends on Mike Hansen's domain modifications.



---

archive/issue_comments_084485.json:
```json
{
    "body": "**Changing author** from \"jasonbhill\" to \"Jason Hill\".",
    "created_at": "2010-11-26T06:05:27Z",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9534#issuecomment-84485",
    "user": "https://github.com/mwhansen"
}
```

**Changing author** from "jasonbhill" to "Jason Hill".



---

archive/issue_comments_084486.json:
```json
{
    "body": "**Reviewer:** Mike Hansen",
    "created_at": "2010-11-26T06:05:27Z",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9534#issuecomment-84486",
    "user": "https://github.com/mwhansen"
}
```

**Reviewer:** Mike Hansen



---

archive/issue_comments_084487.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n Patch to add a (working) base method for permutation groups.\n+\n+This depends on #10334 and #10335\n``````\n",
    "created_at": "2010-11-26T06:05:27Z",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9534#issuecomment-84487",
    "user": "https://github.com/mwhansen"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 Patch to add a (working) base method for permutation groups.
+
+This depends on #10334 and #10335
``````




---

archive/issue_comments_084488.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis is actually Jason's code, and it looks good to me.",
    "created_at": "2010-11-26T06:05:27Z",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9534#issuecomment-84488",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
This is actually Jason's code, and it looks good to me.



---

archive/issue_events_070756.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-11-26T06:05:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70756"
}
```



---

archive/issue_events_070757.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-11-26T06:05:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70757"
}
```



---

archive/issue_events_070758.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-11-26T06:05:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70758"
}
```



---

archive/issue_comments_084489.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,3 +1 @@\n Patch to add a (working) base method for permutation groups.\n-\n-This depends on #10334 and #10335\n``````\n",
    "created_at": "2011-06-01T12:12:30Z",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9534#issuecomment-84489",
    "user": "https://github.com/jdemeyer"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,3 +1 @@
 Patch to add a (working) base method for permutation groups.
-
-This depends on #10334 and #10335
``````




---

archive/attachments_012644.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_9534-permgroup_base.patch",
    "asset_url": "tarball://root/attachments/ticket9534/trac_9534-permgroup_base.patch",
    "created_at": "2011-06-01T12:12:30Z",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket9534/trac_9534-permgroup_base.patch",
    "user": "https://github.com/jdemeyer"
}
```



---

archive/issue_comments_084490.json:
```json
{
    "body": "<a id='comment:4'></a>\n**Attachment:** [trac_9534-permgroup_base.patch](https://github.com/sagemath/sage/files/ticket9534/trac_9534-permgroup_base.patch)",
    "created_at": "2011-06-01T12:12:30Z",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9534#issuecomment-84490",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'></a>
**Attachment:** [trac_9534-permgroup_base.patch](https://github.com/sagemath/sage/files/ticket9534/trac_9534-permgroup_base.patch)



---

archive/issue_comments_084491.json:
```json
{
    "body": "**Dependencies:** #10334, #10335",
    "created_at": "2011-06-01T12:12:30Z",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9534#issuecomment-84491",
    "user": "https://github.com/jdemeyer"
}
```

**Dependencies:** #10334, #10335



---

archive/issue_events_070759.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-10T08:44:05Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "milestone": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70759"
}
```



---

archive/issue_events_070760.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-10T08:44:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "milestone": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70760"
}
```



---

archive/issue_events_070761.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-16T21:33:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "milestone": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70761"
}
```



---

archive/issue_events_070762.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-16T21:33:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "milestone": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70762"
}
```



---

archive/issue_events_070763.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-22T12:49:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70763"
}
```



---

archive/issue_events_070764.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-22T12:49:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70764"
}
```



---

archive/issue_comments_084492.json:
```json
{
    "body": "**Merged:** sage-4.7.2.alpha0",
    "created_at": "2011-07-22T12:49:09Z",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9534#issuecomment-84492",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-4.7.2.alpha0



---

archive/issue_comments_084493.json:
```json
{
    "body": "<a id='comment:8'></a>\nUnmerged because of an issue with #10335.",
    "created_at": "2011-08-01T11:37:36Z",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9534#issuecomment-84493",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'></a>
Unmerged because of an issue with #10335.



---

archive/issue_comments_084494.json:
```json
{
    "body": "**Changing merged** from \"sage-4.7.2.alpha0\" to \"\".",
    "created_at": "2011-08-01T11:37:36Z",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9534#issuecomment-84494",
    "user": "https://github.com/jdemeyer"
}
```

**Changing merged** from "sage-4.7.2.alpha0" to "".



---

archive/issue_events_070765.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-08-01T11:37:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70765"
}
```



---

archive/issue_events_070766.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-08-01T11:37:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70766"
}
```



---

archive/issue_events_070767.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-08-01T11:37:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70767"
}
```



---

archive/issue_events_070768.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-08-01T11:37:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "milestone": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70768"
}
```



---

archive/issue_events_070769.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-08-01T11:37:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "label": "pending",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70769"
}
```



---

archive/issue_comments_084495.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n Patch to add a (working) base method for permutation groups.\n+\n+---\n+\n+Apply [attachment:trac_9534-permgroup_base.patch](https://github.com/sagemath/sage/files/ticket9534/trac_9534-permgroup_base.patch) to the Sage library.\n``````\n",
    "created_at": "2011-09-23T10:48:37Z",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9534#issuecomment-84495",
    "user": "https://github.com/nexttime"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1,5 @@
 Patch to add a (working) base method for permutation groups.
+
+---
+
+Apply [attachment:trac_9534-permgroup_base.patch](https://github.com/sagemath/sage/files/ticket9534/trac_9534-permgroup_base.patch) to the Sage library.
``````




---

archive/issue_comments_084496.json:
```json
{
    "body": "<a id='comment:11'></a>\nDoes this now have to be rebased on (the rebased) #10335?\n\nIn case it does, one should set it to \"needs work\", otherwise the milestone should be changed to Sage 4.7.2 again.",
    "created_at": "2011-09-23T10:48:37Z",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9534#issuecomment-84496",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:11'></a>
Does this now have to be rebased on (the rebased) #10335?

In case it does, one should set it to "needs work", otherwise the milestone should be changed to Sage 4.7.2 again.



---

archive/issue_events_070770.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-07T19:11:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "milestone": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70770"
}
```



---

archive/issue_events_070771.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-07T19:11:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "label": "pending",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70771"
}
```



---

archive/issue_events_070772.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-07T19:11:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70772"
}
```



---

archive/issue_events_070773.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-07T19:11:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9534#event-70773"
}
```



---

archive/issue_comments_084497.json:
```json
{
    "body": "**Merged:** sage-4.7.2.alpha4",
    "created_at": "2011-10-07T19:11:06Z",
    "issue": "https://github.com/sagemath/sage/issues/9534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9534#issuecomment-84497",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-4.7.2.alpha4

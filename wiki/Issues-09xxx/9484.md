# Issue 9484: add demos for implicit_plot3d that show how to do CSG (constructive solid geometry)

archive/issues_009484.json:
```json
{
    "body": "See http://groups.google.com/group/sage-support/browse_thread/thread/e05229d90733c78d for an example of how to do CSG (intersections and unions of solid objects) with implicit_plot3d; I think the given example:\n\n```\nsage: var('x,y,z')\n(x, y, z)\nsage: implicit_plot3d(max_symbolic(min_symbolic(x*x+y*y-1, x*x+z*z-2), x-1.8, y-1.8, z-1.8, -x-1.8, -y-1.8, -z-1.8), (x, -2, 2), (y, -2, 2), (z, -2, 2), smooth=False)\n```\n(along with some explanation) should be added to the implicit_plot3d docstring.\n\n**Assignee:** cwitty\n\n**Author:** Carl Witty\n\n**Reviewer:** Karl-Dieter Crisman\n\n**Work Issues:** Very minor doc formatting\n\nIssue created by migration from https://trac.sagemath.org/ticket/9484\n\n",
    "created_at": "2010-07-12T17:34:19Z",
    "labels": [
        "component: graphics",
        "minor",
        "enhancement",
        "needs work"
    ],
    "title": "add demos for implicit_plot3d that show how to do CSG (constructive solid geometry)",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/9484",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```
See http://groups.google.com/group/sage-support/browse_thread/thread/e05229d90733c78d for an example of how to do CSG (intersections and unions of solid objects) with implicit_plot3d; I think the given example:

```
sage: var('x,y,z')
(x, y, z)
sage: implicit_plot3d(max_symbolic(min_symbolic(x*x+y*y-1, x*x+z*z-2), x-1.8, y-1.8, z-1.8, -x-1.8, -y-1.8, -z-1.8), (x, -2, 2), (y, -2, 2), (z, -2, 2), smooth=False)
```
(along with some explanation) should be added to the implicit_plot3d docstring.

**Assignee:** cwitty

**Author:** Carl Witty

**Reviewer:** Karl-Dieter Crisman

**Work Issues:** Very minor doc formatting

Issue created by migration from https://trac.sagemath.org/ticket/9484





---

archive/issue_comments_083732.json:
```json
{
    "body": "**Author:** Carl Witty",
    "created_at": "2010-07-18T02:54:05Z",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9484#issuecomment-83732",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

**Author:** Carl Witty



---

archive/attachments_012570.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_9484-implicit-plot3d-csg-example.patch",
    "asset_url": "tarball://root/attachments/ticket9484/trac_9484-implicit-plot3d-csg-example.patch",
    "created_at": "2010-07-18T02:54:05Z",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket9484/trac_9484-implicit-plot3d-csg-example.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```



---

archive/issue_comments_083733.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_9484-implicit-plot3d-csg-example.patch](https://github.com/sagemath/sage/files/ticket9484/trac_9484-implicit-plot3d-csg-example.patch)\n\nI picked a slightly different example.",
    "created_at": "2010-07-18T02:54:05Z",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9484#issuecomment-83733",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_9484-implicit-plot3d-csg-example.patch](https://github.com/sagemath/sage/files/ticket9484/trac_9484-implicit-plot3d-csg-example.patch)

I picked a slightly different example.



---

archive/issue_events_070257.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/cwitty",
    "created_at": "2010-07-18T02:54:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9484#event-70257"
}
```



---

archive/issue_events_070258.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/cwitty",
    "created_at": "2010-07-18T02:54:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "milestone": "sage-4.5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9484#event-70258"
}
```



---

archive/issue_comments_083734.json:
```json
{
    "body": "<a id='comment:2'></a>\nProbably depends on #9482 and #9483.",
    "created_at": "2010-08-05T20:46:24Z",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9484#issuecomment-83734",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'></a>
Probably depends on #9482 and #9483.



---

archive/issue_comments_083735.json:
```json
{
    "body": "<a id='comment:3'></a>\nAfter #9483, I go from error message to no error message with this example as expected.  Can't test the actual view for some reason :(",
    "created_at": "2010-08-06T01:27:37Z",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9484#issuecomment-83735",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'></a>
After #9483, I go from error message to no error message with this example as expected.  Can't test the actual view for some reason :(



---

archive/issue_comments_083736.json:
```json
{
    "body": "<a id='comment:4'></a>\nI apologize for not marking the dependency in my comment.\n\nBy the way, you could see a version of the plot by adding a viewer='tachyon' keyword argument to the implicit_plot3d call.",
    "created_at": "2010-08-06T02:45:40Z",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9484#issuecomment-83736",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

<a id='comment:4'></a>
I apologize for not marking the dependency in my comment.

By the way, you could see a version of the plot by adding a viewer='tachyon' keyword argument to the implicit_plot3d call.



---

archive/issue_comments_083737.json:
```json
{
    "body": "<a id='comment:5'></a>\nThis looks fine, except probably one should put commands like `implicit_plot3d` in double ticks like <code>\\`\\`implicit_plot3d\\`\\`</code>, or in that case you might even be able to use `:meth:` or something like that.  Very interesting example!",
    "created_at": "2010-08-06T14:18:32Z",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9484#issuecomment-83737",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:5'></a>
This looks fine, except probably one should put commands like `implicit_plot3d` in double ticks like <code>\`\`implicit_plot3d\`\`</code>, or in that case you might even be able to use `:meth:` or something like that.  Very interesting example!



---

archive/issue_comments_083738.json:
```json
{
    "body": "**Reviewer:** Karl-Dieter Crisman",
    "created_at": "2010-08-06T14:18:32Z",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9484#issuecomment-83738",
    "user": "https://github.com/kcrisman"
}
```

**Reviewer:** Karl-Dieter Crisman



---

archive/issue_events_070259.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2010-08-06T14:18:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9484#event-70259"
}
```



---

archive/issue_events_070260.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2010-08-06T14:18:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9484#event-70260"
}
```



---

archive/issue_comments_083739.json:
```json
{
    "body": "**Work Issues:** Very minor doc formatting",
    "created_at": "2010-08-06T14:18:32Z",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9484#issuecomment-83739",
    "user": "https://github.com/kcrisman"
}
```

**Work Issues:** Very minor doc formatting



---

archive/issue_events_070261.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9484#event-70261"
}
```



---

archive/issue_events_070262.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9484#event-70262"
}
```



---

archive/issue_events_070263.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9484#event-70263"
}
```



---

archive/issue_events_070264.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9484#event-70264"
}
```



---

archive/issue_events_070265.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9484#event-70265"
}
```



---

archive/issue_events_070266.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9484#event-70266"
}
```



---

archive/issue_events_070267.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9484#event-70267"
}
```



---

archive/issue_events_070268.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9484#event-70268"
}
```



---

archive/issue_events_070269.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9484",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9484#event-70269"
}
```

# Issue 9068: remove redundant sgn function from quadratic_forms/extras

archive/issues_009068.json:
```json
{
    "body": "There is a sgn() function defined in sage/quadratic_forms/extras.py which just duplicates the one in sage/functions/generalized.py, so I have removed it and adjusted imports accordingly.\n\nMay depend on #7828.\n\n**Assignee:** @aghitza\n\n**CC:**  @kcrisman\n\n**Keywords:** sgn sign\n\n**Author:** John Cremona\n\n**Reviewer:** Alex Ghitza\n\n**Merged:** sage-4.4.4.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/9068\n\n",
    "closed_at": "2010-06-06T01:16:46Z",
    "created_at": "2010-05-27T21:00:41Z",
    "labels": [
        "component: algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4.4",
    "title": "remove redundant sgn function from quadratic_forms/extras",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/9068",
    "user": "https://github.com/JohnCremona"
}
```
There is a sgn() function defined in sage/quadratic_forms/extras.py which just duplicates the one in sage/functions/generalized.py, so I have removed it and adjusted imports accordingly.

May depend on #7828.

**Assignee:** @aghitza

**CC:**  @kcrisman

**Keywords:** sgn sign

**Author:** John Cremona

**Reviewer:** Alex Ghitza

**Merged:** sage-4.4.4.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/9068





---

archive/attachments_011836.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_9068-sgn.patch",
    "asset_url": "tarball://root/attachments/ticket9068/trac_9068-sgn.patch",
    "created_at": "2010-05-27T21:13:17Z",
    "issue": "https://github.com/sagemath/sage/issues/9068",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket9068/trac_9068-sgn.patch",
    "user": "https://github.com/JohnCremona"
}
```



---

archive/issue_comments_076872.json:
```json
{
    "body": "**Attachment:** [trac_9068-sgn.patch](https://github.com/sagemath/sage/files/ticket9068/trac_9068-sgn.patch)\n\nApplies to 4.4.3.alpha0",
    "created_at": "2010-05-27T21:13:17Z",
    "issue": "https://github.com/sagemath/sage/issues/9068",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9068#issuecomment-76872",
    "user": "https://github.com/JohnCremona"
}
```

**Attachment:** [trac_9068-sgn.patch](https://github.com/sagemath/sage/files/ticket9068/trac_9068-sgn.patch)

Applies to 4.4.3.alpha0



---

archive/issue_events_066079.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-05-27T21:14:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9068",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9068#event-66079"
}
```



---

archive/issue_events_066080.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-06-05T01:32:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9068",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9068#event-66080"
}
```



---

archive/issue_events_066081.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-06-05T01:32:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9068",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9068#event-66081"
}
```



---

archive/issue_events_066082.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-06-05T01:32:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9068",
    "milestone": "sage-4.4.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9068#event-66082"
}
```



---

archive/issue_comments_076873.json:
```json
{
    "body": "**Reviewer:** Alex Ghitza",
    "created_at": "2010-06-05T01:32:17Z",
    "issue": "https://github.com/sagemath/sage/issues/9068",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9068#issuecomment-76873",
    "user": "https://github.com/aghitza"
}
```

**Reviewer:** Alex Ghitza



---

archive/issue_comments_076874.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good.",
    "created_at": "2010-06-05T01:32:17Z",
    "issue": "https://github.com/sagemath/sage/issues/9068",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9068#issuecomment-76874",
    "user": "https://github.com/aghitza"
}
```

<a id='comment:2'></a>
Looks good.



---

archive/issue_comments_076875.json:
```json
{
    "body": "**Merged:** sage-4.4.4.alpha0",
    "created_at": "2010-06-06T01:16:46Z",
    "issue": "https://github.com/sagemath/sage/issues/9068",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9068#issuecomment-76875",
    "user": "https://github.com/mwhansen"
}
```

**Merged:** sage-4.4.4.alpha0



---

archive/issue_events_066083.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-06-06T01:16:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9068",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9068#event-66083"
}
```



---

archive/issue_events_066084.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-06-06T01:16:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9068",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9068#event-66084"
}
```

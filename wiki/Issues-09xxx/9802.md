# Issue 9802: Add random diagonalizable matrix to matrix/constructor.py

archive/issues_009801.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout",
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis depends on #9720, so first apply the v3 patch from there.  This method generates random diagonalizable matrices whose eigenvectors, if computed by hand, have only integer values. This routine is designed as educational tool, generating exam and homework problems, and problem generating interacts.\n\nCC:  @rbeezer @wdjoyner\n\nComponent: **linear algebra**\n\nAuthor: **Billy Wonderly**\n\nReviewer: **David Joyner**\n\nMerged: **sage-4.6.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9802_\n\n",
    "closed_at": "2010-09-15T09:53:42Z",
    "created_at": "2010-08-25T18:45:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add random diagonalizable matrix to matrix/constructor.py",
    "type": "issue",
    "updated_at": "2010-09-15T09:53:42Z",
    "url": "https://github.com/sagemath/sage/issues/9802",
    "user": "https://github.com/sagetrac-bwonderly"
}
```
<div id="comment:0"></div>

This depends on #9720, so first apply the v3 patch from there.  This method generates random diagonalizable matrices whose eigenvectors, if computed by hand, have only integer values. This routine is designed as educational tool, generating exam and homework problems, and problem generating interacts.

CC:  @rbeezer @wdjoyner

Component: **linear algebra**

Author: **Billy Wonderly**

Reviewer: **David Joyner**

Merged: **sage-4.6.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/9802_





---

archive/issue_events_125905.json:
```json
{
    "actor": "https://github.com/sagetrac-bwonderly",
    "created_at": "2010-08-25T18:45:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "milestone_number": null,
    "milestone_title": "sage-4.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9802#event-125905"
}
```



---

archive/issue_events_125906.json:
```json
{
    "actor": "https://github.com/sagetrac-bwonderly",
    "created_at": "2010-08-25T18:45:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "c: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9802#event-125906"
}
```



---

archive/issue_events_125907.json:
```json
{
    "actor": "https://github.com/sagetrac-bwonderly",
    "created_at": "2010-08-25T18:45:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9802#event-125907"
}
```



---

archive/issue_events_125908.json:
```json
{
    "actor": "https://github.com/sagetrac-bwonderly",
    "created_at": "2010-08-25T18:45:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9802#event-125908"
}
```



---

archive/issue_events_125909.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-08-25T18:45:05Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "subject": "https://github.com/sagetrac-bwonderly",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9802#event-125909"
}
```



---

archive/issue_events_125910.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-08-25T18:45:05Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "subject": "https://github.com/sagetrac-bwonderly",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9802#event-125910"
}
```



---

archive/issue_comments_087037.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_9802-random-diagonalizable-matrix.patch.gz](https://github.com/sagemath/sage/files/ticket9802/trac_9802-random-diagonalizable-matrix.patch.gz)**",
    "created_at": "2010-08-25T18:47:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9802#issuecomment-87037",
    "user": "https://github.com/sagetrac-bwonderly"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_9802-random-diagonalizable-matrix.patch.gz](https://github.com/sagemath/sage/files/ticket9802/trac_9802-random-diagonalizable-matrix.patch.gz)**



---

archive/issue_events_125911.json:
```json
{
    "actor": "https://github.com/sagetrac-bwonderly",
    "created_at": "2010-08-25T18:47:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9802#event-125911"
}
```



---

archive/issue_comments_087038.json:
```json
{
    "body": "Attachment: **[trac_9802-random-diagonalizable-matrix-v2.patch.gz](https://github.com/sagemath/sage/files/ticket9802/trac_9802-random-diagonalizable-matrix-v2.patch.gz)**\n\nrevised to fit generalization of random_matrix constructor. Apply v4 from #9720 and v2 from #9803 and go from there. This patch is independent from #9754, but will be rebased as soon as either one gets a positive review",
    "created_at": "2010-08-28T02:39:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9802#issuecomment-87038",
    "user": "https://github.com/sagetrac-bwonderly"
}
```

Attachment: **[trac_9802-random-diagonalizable-matrix-v2.patch.gz](https://github.com/sagemath/sage/files/ticket9802/trac_9802-random-diagonalizable-matrix-v2.patch.gz)**

revised to fit generalization of random_matrix constructor. Apply v4 from #9720 and v2 from #9803 and go from there. This patch is independent from #9754, but will be rebased as soon as either one gets a positive review



---

archive/issue_comments_087039.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThe v2 patch is independent of #9754. There are revisions to the documentation of the random_diagonalizable_matrix routine, as well as to the random_matrix routine.  The code is revised to fit with the generalization of the random_matrix constructor.  First apply v4 from #9720, and then v2 from #9803 and go from there.",
    "created_at": "2010-08-28T17:39:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9802#issuecomment-87039",
    "user": "https://github.com/sagetrac-bwonderly"
}
```

<div id="comment:2" align="right">comment:2</div>

The v2 patch is independent of #9754. There are revisions to the documentation of the random_diagonalizable_matrix routine, as well as to the random_matrix routine.  The code is revised to fit with the generalization of the random_matrix constructor.  First apply v4 from #9720, and then v2 from #9803 and go from there.



---

archive/issue_comments_087040.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nThis (with the other patches, as indicated above) applied fine to 4.5.1 and passed sage -testall.\n\nPositive review, as far as I am concerned (and will be useful for me teaching linear algebra later in the semester!). Perhaps Mike Hansen should have the final say?\n\nThanks Rob and bwonderly!",
    "created_at": "2010-08-28T21:58:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9802#issuecomment-87040",
    "user": "https://github.com/wdjoyner"
}
```

<div id="comment:3" align="right">comment:3</div>

This (with the other patches, as indicated above) applied fine to 4.5.1 and passed sage -testall.

Positive review, as far as I am concerned (and will be useful for me teaching linear algebra later in the semester!). Perhaps Mike Hansen should have the final say?

Thanks Rob and bwonderly!



---

archive/issue_comments_087041.json:
```json
{
    "body": "Reviewer: **David Joyner**",
    "created_at": "2010-09-02T02:20:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9802#issuecomment-87041",
    "user": "https://github.com/rbeezer"
}
```

Reviewer: **David Joyner**



---

archive/issue_events_125912.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-09-02T02:20:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9802#event-125912"
}
```



---

archive/issue_events_125913.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-09-02T02:20:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9802#event-125913"
}
```



---

archive/issue_comments_087042.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMike Hansen looked in on #9803, so I've marked this as ready to go based on comments above.  Thanks, David.",
    "created_at": "2010-09-02T02:20:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9802#issuecomment-87042",
    "user": "https://github.com/rbeezer"
}
```

<div id="comment:4" align="right">comment:4</div>

Mike Hansen looked in on #9803, so I've marked this as ready to go based on comments above.  Thanks, David.



---

archive/issue_comments_087043.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\n## Release Manager\n\n#9720, #9803, #9802, #9754 is each dependent on the predecessor, merge in this\norder.",
    "created_at": "2010-09-03T06:28:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9802#issuecomment-87043",
    "user": "https://github.com/sagetrac-bwonderly"
}
```

<div id="comment:5" align="right">comment:5</div>

## Release Manager

#9720, #9803, #9802, #9754 is each dependent on the predecessor, merge in this
order.



---

archive/issue_events_125914.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-09-15T09:53:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9802#event-125914"
}
```



---

archive/issue_events_125915.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-09-15T09:53:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9802#event-125915"
}
```



---

archive/issue_comments_087044.json:
```json
{
    "body": "Merged: **sage-4.6.alpha1**",
    "created_at": "2010-09-15T09:53:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9802",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9802#issuecomment-87044",
    "user": "https://github.com/qed777"
}
```

Merged: **sage-4.6.alpha1**

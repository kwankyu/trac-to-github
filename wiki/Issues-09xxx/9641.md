# Issue 9641: Race condition with sage -tp

archive/issues_009641.json:
```json
{
    "body": "All files are named .sage/tmp/filename. At high parallelism, there are often conflicts (e.g. all.py, proof.py, ...) that result in untested code and spurious errors. \n\n**Assignee:** mvngu\n\n**CC:**  @craigcitro\n\n**Reviewer:** Willem Jan Palenstijn\n\nIssue created by migration from https://trac.sagemath.org/ticket/9641\n\n",
    "closed_at": "2011-01-11T06:17:41Z",
    "created_at": "2010-07-29T21:34:32Z",
    "labels": [
        "component: doctest coverage",
        "bug",
        "duplicate"
    ],
    "title": "Race condition with sage -tp",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/9641",
    "user": "https://github.com/robertwb"
}
```
All files are named .sage/tmp/filename. At high parallelism, there are often conflicts (e.g. all.py, proof.py, ...) that result in untested code and spurious errors. 

**Assignee:** mvngu

**CC:**  @craigcitro

**Reviewer:** Willem Jan Palenstijn

Issue created by migration from https://trac.sagemath.org/ticket/9641





---

archive/issue_events_071811.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-09-02T11:06:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9641",
    "label": "component: algebra",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9641#event-71811"
}
```



---

archive/issue_events_071812.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2010-09-02T11:06:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9641",
    "label": "component: doctest coverage",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9641#event-71812"
}
```



---

archive/issue_comments_086231.json:
```json
{
    "body": "**Changing assignee** from @aghitza to mvngu.",
    "created_at": "2010-09-02T11:06:30Z",
    "issue": "https://github.com/sagemath/sage/issues/9641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9641#issuecomment-86231",
    "user": "https://github.com/aghitza"
}
```

**Changing assignee** from @aghitza to mvngu.



---

archive/issue_comments_086232.json:
```json
{
    "body": "<a id='comment:2'></a>\nIs this a duplicate of #9739 ? (This one is older, but #9739 has work in progress.)",
    "created_at": "2011-01-11T01:21:20Z",
    "issue": "https://github.com/sagemath/sage/issues/9641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9641#issuecomment-86232",
    "user": "https://github.com/wjp"
}
```

<a id='comment:2'></a>
Is this a duplicate of #9739 ? (This one is older, but #9739 has work in progress.)



---

archive/issue_events_071813.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-11T06:17:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9641",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9641#event-71813"
}
```



---

archive/issue_events_071814.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-11T06:17:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9641",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9641#event-71814"
}
```



---

archive/issue_comments_086233.json:
```json
{
    "body": "**Reviewer:** Willem Jan Palenstijn",
    "created_at": "2011-01-11T06:17:41Z",
    "issue": "https://github.com/sagemath/sage/issues/9641",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9641#issuecomment-86233",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Willem Jan Palenstijn



---

archive/issue_events_071815.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2011-01-11T09:22:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/9641",
    "milestone": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9641#event-71815"
}
```

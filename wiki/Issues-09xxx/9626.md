# Issue 9626: Update and extend "SPKG Tracking" Wiki page

archive/issues_009626.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu",
        "https://github.com/haraldschilly"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe information given at http://wiki.sagemath.org/Sage_Spkg_Tracking is fairly out of date (and incomplete).\n\nPerhaps we could add updating this page to the [release manager's task/check list](http://wiki.sagemath.org/devel/ReleaseManagement) (in case a new release includes updated spkgs).\n\nI also consider adding a column \"Upcoming\" which people creating/updating spkgs can fill with the next version and, if appropriate, a link to the corresponding ticket. An alternative is to add a second *row* for each upcoming spkg, just below the main row for that package, i.e. its current version within Sage.\n\nAnother useful column would be something like \"Valid of\", containing the date and/or Sage version each entry (row) has been updated last.\n\nThe column \"Latest stable (upstream) release\" is subject to change more or less frequently, independent of Sage releases; so if we keep it, it should perhaps continually be updated by the spkg maintainers, or at least carry a date, indicating when this information was entered.\n\nCC:  @slel\n\nComponent: **packages: standard**\n\nKeywords: **SPKG.txt spkgs documentation version release**\n\nReviewer: **Markus Wageringel**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/9626_\n\n",
    "closed_at": "2020-08-14T00:54:07Z",
    "created_at": "2010-07-28T17:38:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update and extend \"SPKG Tracking\" Wiki page",
    "type": "issue",
    "updated_at": "2020-08-14T00:54:07Z",
    "url": "https://github.com/sagemath/sage/issues/9626",
    "user": "https://github.com/nexttime"
}
```
<div id="comment:0"></div>

The information given at http://wiki.sagemath.org/Sage_Spkg_Tracking is fairly out of date (and incomplete).

Perhaps we could add updating this page to the [release manager's task/check list](http://wiki.sagemath.org/devel/ReleaseManagement) (in case a new release includes updated spkgs).

I also consider adding a column "Upcoming" which people creating/updating spkgs can fill with the next version and, if appropriate, a link to the corresponding ticket. An alternative is to add a second *row* for each upcoming spkg, just below the main row for that package, i.e. its current version within Sage.

Another useful column would be something like "Valid of", containing the date and/or Sage version each entry (row) has been updated last.

The column "Latest stable (upstream) release" is subject to change more or less frequently, independent of Sage releases; so if we keep it, it should perhaps continually be updated by the spkg maintainers, or at least carry a date, indicating when this information was entered.

CC:  @slel

Component: **packages: standard**

Keywords: **SPKG.txt spkgs documentation version release**

Reviewer: **Markus Wageringel**

_Issue created by migration from https://trac.sagemath.org/ticket/9626_





---

archive/issue_events_123030.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2010-07-28T17:38:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20website%20/%20wiki",
    "label_color": "0075ca",
    "label_name": "c: website / wiki",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9626#event-123030"
}
```



---

archive/issue_events_123031.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2010-07-28T17:38:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9626#event-123031"
}
```



---

archive/issue_events_123032.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2010-07-28T17:38:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9626#event-123032"
}
```



---

archive/issue_events_123033.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-07-28T17:38:06Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "subject": "https://github.com/nexttime",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9626#event-123033"
}
```



---

archive/issue_events_123034.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-07-28T17:38:06Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "subject": "https://github.com/nexttime",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9626#event-123034"
}
```



---

archive/issue_events_123035.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2010-07-28T17:39:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20website%20/%20wiki",
    "label_color": "0075ca",
    "label_name": "c: website / wiki",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9626#event-123035"
}
```



---

archive/issue_events_123036.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2010-07-28T17:39:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "0000b0",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9626#event-123036"
}
```



---

archive/issue_events_123037.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2010-07-28T17:39:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9626#event-123037"
}
```



---

archive/issue_comments_084149.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nOutdated, should be closed",
    "created_at": "2020-07-08T16:35:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9626#issuecomment-84149",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">comment:2</div>

Outdated, should be closed



---

archive/issue_events_123038.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-08T16:35:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9626#event-123038"
}
```



---

archive/issue_events_123039.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-08T16:35:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9626#event-123039"
}
```



---

archive/issue_comments_084150.json:
```json
{
    "body": "Reviewer: **Markus Wageringel**",
    "created_at": "2020-08-13T20:25:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9626#issuecomment-84150",
    "user": "https://github.com/mwageringel"
}
```

Reviewer: **Markus Wageringel**



---

archive/issue_events_123040.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-08-13T20:25:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9626#event-123040"
}
```



---

archive/issue_events_123041.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-08-13T20:25:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9626#event-123041"
}
```



---

archive/issue_events_123042.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-08-14T00:54:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9626#event-123042"
}
```



---

archive/issue_events_123043.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-08-14T00:54:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "c6c6c6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9626#event-123043"
}
```



---

archive/issue_comments_084151.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThe page http://wiki.sagemath.org/Sage_Spkg_Tracking\nnow redirects to http://wiki.sagemath.org/spkg\nso this can indeed be closed.",
    "created_at": "2020-08-14T00:54:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/9626#issuecomment-84151",
    "user": "https://github.com/slel"
}
```

<div id="comment:4" align="right">comment:4</div>

The page http://wiki.sagemath.org/Sage_Spkg_Tracking
now redirects to http://wiki.sagemath.org/spkg
so this can indeed be closed.



---

archive/issue_events_123044.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-08-14T00:54:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9626#event-123044"
}
```



---

archive/issue_events_123045.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-08-14T00:54:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/9626",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/9626#event-123045"
}
```

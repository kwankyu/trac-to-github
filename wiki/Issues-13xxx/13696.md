# Issue 13696: process chained equalities with different signs correctly

archive/issues_013492.json:
```json
{
    "body": "we have to deal with what remained from #12091.  At least, report errors for the inputs like the following:\n\n```\nsage: p = MixedIntegerLinearProgram()\nsage: b = p.new_variable()\nsage: b[0] <= 555*b[1] >= 2\nsage: b[0] <= 555*b[1] == 2\n```\nwhich even with #12091 are incorrectly processed\n\n```\nsage: b[0] <= 555*b[1] >= 2\n2 <= x_0 <= 555*x_1\nsage: b[0] <= 555*b[1] == 2\n555*x_1 == 2\n```\n\n\nDepends on #14540\n\n**Assignee:** @nathanncohen\n\n**CC:**  @vbraun @ppurka @nathanncohen\n\n**Reviewer:** Jeroen Demeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/13696\n\n",
    "closed_at": "2016-06-12T12:02:30Z",
    "created_at": "2012-11-10T11:30:36Z",
    "labels": [
        "component: linear programming",
        "bug",
        "invalid"
    ],
    "title": "process chained equalities with different signs correctly",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/13696",
    "user": "https://github.com/dimpase"
}
```
we have to deal with what remained from #12091.  At least, report errors for the inputs like the following:

```
sage: p = MixedIntegerLinearProgram()
sage: b = p.new_variable()
sage: b[0] <= 555*b[1] >= 2
sage: b[0] <= 555*b[1] == 2
```
which even with #12091 are incorrectly processed

```
sage: b[0] <= 555*b[1] >= 2
2 <= x_0 <= 555*x_1
sage: b[0] <= 555*b[1] == 2
555*x_1 == 2
```


Depends on #14540

**Assignee:** @nathanncohen

**CC:**  @vbraun @ppurka @nathanncohen

**Reviewer:** Jeroen Demeyer

Issue created by migration from https://trac.sagemath.org/ticket/13696





---

archive/issue_events_117873.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2012-11-10T11:32:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "label": "bug",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117873"
}
```



---

archive/issue_events_117874.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117874"
}
```



---

archive/issue_events_117875.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117875"
}
```



---

archive/issue_events_117876.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117876"
}
```



---

archive/issue_events_117877.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117877"
}
```



---

archive/issue_events_117878.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117878"
}
```



---

archive/issue_events_117879.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117879"
}
```



---

archive/issue_events_117880.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117880"
}
```



---

archive/issue_events_117881.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117881"
}
```



---

archive/issue_comments_163150.json:
```json
{
    "body": "<a id='comment:6'></a>\nDescription modified to make the example self-contained.",
    "created_at": "2016-04-03T20:02:41Z",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13696#issuecomment-163150",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:6'></a>
Description modified to make the example self-contained.



---

archive/issue_comments_163151.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,6 +1,8 @@\n we have to deal with what remained from #12091.  At least, report errors for the inputs like the following:\n \n ```\n+sage: p = MixedIntegerLinearProgram()\n+sage: b = p.new_variable()\n sage: b[0] <= 555*b[1] >= 2\n sage: b[0] <= 555*b[1] == 2\n ```\n``````\n",
    "created_at": "2016-04-03T20:02:41Z",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13696#issuecomment-163151",
    "user": "https://github.com/mkoeppe"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,6 +1,8 @@
 we have to deal with what remained from #12091.  At least, report errors for the inputs like the following:
 
 ```
+sage: p = MixedIntegerLinearProgram()
+sage: b = p.new_variable()
 sage: b[0] <= 555*b[1] >= 2
 sage: b[0] <= 555*b[1] == 2
 ```
``````




---

archive/issue_events_117882.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2016-04-05T12:21:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117882"
}
```



---

archive/issue_events_117883.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2016-04-05T12:21:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "label": "wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117883"
}
```



---

archive/issue_comments_163152.json:
```json
{
    "body": "**Changing dependencies** from \"#12091\" to \"#14540\".",
    "created_at": "2016-04-19T09:32:15Z",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13696#issuecomment-163152",
    "user": "https://github.com/jdemeyer"
}
```

**Changing dependencies** from "#12091" to "#14540".



---

archive/issue_events_117884.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-19T09:58:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "milestone": "sage-7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117884"
}
```



---

archive/issue_events_117885.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-19T09:58:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "label": "wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117885"
}
```



---

archive/issue_comments_163153.json:
```json
{
    "body": "<a id='comment:10'></a>\nAnd how about:\n\n```\nsage: b[0] <= 3 <= 4 <= b[1]\n4 <= x_1\n```",
    "created_at": "2016-04-20T21:49:08Z",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13696#issuecomment-163153",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:10'></a>
And how about:

```
sage: b[0] <= 3 <= 4 <= b[1]
4 <= x_1
```



---

archive/issue_comments_163154.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2016-04-21T08:50:08Z",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13696#issuecomment-163154",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_events_117886.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-21T08:50:08Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "milestone": "sage-7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117886"
}
```



---

archive/issue_events_117887.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-21T08:50:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117887"
}
```



---

archive/issue_events_117888.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-21T08:50:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117888"
}
```



---

archive/issue_events_117889.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-21T08:50:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117889"
}
```



---

archive/issue_comments_163155.json:
```json
{
    "body": "<a id='comment:13'></a>\nThe reviewer of #14540 was not happy with a partial solution, so it's probably better to thoroughly fix the problem on #14540.",
    "created_at": "2016-04-21T08:58:12Z",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13696#issuecomment-163155",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:13'></a>
The reviewer of #14540 was not happy with a partial solution, so it's probably better to thoroughly fix the problem on #14540.



---

archive/issue_events_117890.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-12T12:02:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117890"
}
```



---

archive/issue_events_117891.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-12T12:02:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13696",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13696#event-117891"
}
```

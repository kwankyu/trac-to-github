# Issue 13091: Bug in graph tensor_product

archive/issues_012919.json:
```json
{
    "body": "#12791 introduced a bug in the graph tensor_product if the graph is an undirected graph (only half the edges get added).  The bad thing was there was a test that would have uncovered this, but the test output was wrong, so apparently wasn't checked by hand!\n\nI have a patch, but I'm waiting to hear from the person that mentioned this bug to me about who I should credit.\n\n**Assignee:** @jasongrout, @nathanncohen, @rlmill\n\n**CC:**  @nathanncohen @dcoudert\n\n**Reviewer:** David Coudert\n\n**Author:** Jason Grout\n\n**Merged:** sage-5.1.beta5\n\nIssue created by migration from https://trac.sagemath.org/ticket/13091\n\n",
    "closed_at": "2012-06-18T10:26:29Z",
    "created_at": "2012-06-06T14:14:07Z",
    "labels": [
        "component: graph theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.1",
    "title": "Bug in graph tensor_product",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/13091",
    "user": "https://github.com/jasongrout"
}
```
#12791 introduced a bug in the graph tensor_product if the graph is an undirected graph (only half the edges get added).  The bad thing was there was a test that would have uncovered this, but the test output was wrong, so apparently wasn't checked by hand!

I have a patch, but I'm waiting to hear from the person that mentioned this bug to me about who I should credit.

**Assignee:** @jasongrout, @nathanncohen, @rlmill

**CC:**  @nathanncohen @dcoudert

**Reviewer:** David Coudert

**Author:** Jason Grout

**Merged:** sage-5.1.beta5

Issue created by migration from https://trac.sagemath.org/ticket/13091





---

archive/issue_events_110777.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2012-06-06T14:17:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13091",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13091#event-110777"
}
```



---

archive/issue_comments_151624.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis needs review, but I might change the patch message if the credit line needs to be changed.",
    "created_at": "2012-06-06T14:17:42Z",
    "issue": "https://github.com/sagemath/sage/issues/13091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13091#issuecomment-151624",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:1'></a>
This needs review, but I might change the patch message if the credit line needs to be changed.



---

archive/attachments_018305.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac-13091-tensor_product.patch",
    "asset_url": "tarball://root/attachments/ticket13091/trac-13091-tensor_product.patch",
    "created_at": "2012-06-06T15:24:52Z",
    "issue": "https://github.com/sagemath/sage/issues/13091",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket13091/trac-13091-tensor_product.patch",
    "user": "https://github.com/jasongrout"
}
```



---

archive/issue_comments_151625.json:
```json
{
    "body": "<a id='comment:2'></a>\n**Attachment:** [trac-13091-tensor_product.patch](https://github.com/sagemath/sage/files/ticket13091/trac-13091-tensor_product.patch)\n\nOkay, I fixed the credits in the commit message.  This is ready for review.",
    "created_at": "2012-06-06T15:24:52Z",
    "issue": "https://github.com/sagemath/sage/issues/13091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13091#issuecomment-151625",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:2'></a>
**Attachment:** [trac-13091-tensor_product.patch](https://github.com/sagemath/sage/files/ticket13091/trac-13091-tensor_product.patch)

Okay, I fixed the credits in the commit message.  This is ready for review.



---

archive/issue_comments_151626.json:
```json
{
    "body": "**Reviewer:** David Coudert",
    "created_at": "2012-06-07T09:40:35Z",
    "issue": "https://github.com/sagemath/sage/issues/13091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13091#issuecomment-151626",
    "user": "https://github.com/dcoudert"
}
```

**Reviewer:** David Coudert



---

archive/issue_comments_151627.json:
```json
{
    "body": "<a id='comment:3'></a>\nOups, sorry for the mistake.\n\nThe patch is working correctly and passes all tests. I give positive review.",
    "created_at": "2012-06-07T09:40:35Z",
    "issue": "https://github.com/sagemath/sage/issues/13091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13091#issuecomment-151627",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:3'></a>
Oups, sorry for the mistake.

The patch is working correctly and passes all tests. I give positive review.



---

archive/issue_events_110778.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2012-06-07T09:40:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13091",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13091#event-110778"
}
```



---

archive/issue_events_110779.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2012-06-07T09:40:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13091",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13091#event-110779"
}
```



---

archive/issue_events_110780.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-18T10:26:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13091",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13091#event-110780"
}
```



---

archive/issue_events_110781.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-18T10:26:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13091",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13091#event-110781"
}
```



---

archive/issue_comments_151628.json:
```json
{
    "body": "**Merged:** sage-5.1.beta5",
    "created_at": "2012-06-18T10:26:29Z",
    "issue": "https://github.com/sagemath/sage/issues/13091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13091#issuecomment-151628",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.1.beta5

# Issue 13051: Faceted point colours

archive/issues_012879.json:
```json
{
    "body": "When a 2D point is plotted with the `faceted argument, there is no way to change the colour of the border from black. This makes it look inconsistent with other graphical objects, such as function plots, if they are of another colour (see attached image). These faceted points are useful for showing discontinuities in functions, so it'd be nice to be able to customize.\n\nAs well, the default border colour should probably be blue in accordance with the default for normal points.\n\n**Assignee:** @jasongrout, @williamstein\n\n**Reviewer:** Karl-Dieter Crisman, Eviatar Bach\n\nIssue created by migration from https://trac.sagemath.org/ticket/13051\n\n",
    "closed_at": "2013-08-13T08:37:36Z",
    "created_at": "2012-05-28T17:47:38Z",
    "labels": [
        "component: graphics",
        "minor",
        "enhancement",
        "duplicate"
    ],
    "title": "Faceted point colours",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/13051",
    "user": "https://github.com/eviatarbach"
}
```
When a 2D point is plotted with the `faceted argument, there is no way to change the colour of the border from black. This makes it look inconsistent with other graphical objects, such as function plots, if they are of another colour (see attached image). These faceted points are useful for showing discontinuities in functions, so it'd be nice to be able to customize.

As well, the default border colour should probably be blue in accordance with the default for normal points.

**Assignee:** @jasongrout, @williamstein

**Reviewer:** Karl-Dieter Crisman, Eviatar Bach

Issue created by migration from https://trac.sagemath.org/ticket/13051





---

archive/attachments_018245.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "tmp_8.png",
    "asset_url": "tarball://root/attachments/ticket13051/tmp_8.png",
    "created_at": "2012-05-28T17:48:24Z",
    "issue": "https://github.com/sagemath/sage/issues/13051",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket13051/tmp_8.png",
    "user": "https://github.com/eviatarbach"
}
```



---

archive/issue_comments_150934.json:
```json
{
    "body": "**Attachment:** [tmp_8.png](https://github.com/sagemath/sage/files/ticket13051/tmp_8.png)",
    "created_at": "2012-05-28T17:48:24Z",
    "issue": "https://github.com/sagemath/sage/issues/13051",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13051#issuecomment-150934",
    "user": "https://github.com/eviatarbach"
}
```

**Attachment:** [tmp_8.png](https://github.com/sagemath/sage/files/ticket13051/tmp_8.png)



---

archive/issue_comments_150935.json:
```json
{
    "body": "<a id='comment:1'></a>\nPresumably this would be fixed by #13576?",
    "created_at": "2013-07-10T20:57:26Z",
    "issue": "https://github.com/sagemath/sage/issues/13051",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13051#issuecomment-150935",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:1'></a>
Presumably this would be fixed by #13576?



---

archive/issue_comments_150936.json:
```json
{
    "body": "<a id='comment:2'></a>\nYes, it would.",
    "created_at": "2013-07-11T02:29:44Z",
    "issue": "https://github.com/sagemath/sage/issues/13051",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13051#issuecomment-150936",
    "user": "https://github.com/eviatarbach"
}
```

<a id='comment:2'></a>
Yes, it would.



---

archive/issue_comments_150937.json:
```json
{
    "body": "**Reviewer:** Karl-Dieter Crisman, Eviatar Bach",
    "created_at": "2013-07-11T04:47:22Z",
    "issue": "https://github.com/sagemath/sage/issues/13051",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13051#issuecomment-150937",
    "user": "https://github.com/kcrisman"
}
```

**Reviewer:** Karl-Dieter Crisman, Eviatar Bach



---

archive/issue_events_110256.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-07-11T04:47:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13051",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13051#event-110256"
}
```



---

archive/issue_events_110257.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-07-11T04:47:22Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13051",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13051#event-110257"
}
```



---

archive/issue_events_110258.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-07-11T04:47:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13051",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13051#event-110258"
}
```



---

archive/issue_events_110259.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-07-11T04:47:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13051",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13051#event-110259"
}
```



---

archive/issue_events_110260.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T08:37:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13051",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13051#event-110260"
}
```



---

archive/issue_events_110261.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T08:37:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13051",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13051#event-110261"
}
```



---

archive/issue_events_110262.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T08:37:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13051",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13051#event-110262"
}
```

# Issue 13553: Bessel class plot method does not support any standard plot options

archive/issues_013349.json:
```json
{
    "body": "The `Bessel` class defined its own plot method which is called e.g. when you run\n\n```\nsage: plot(Bessel(0,'J'), 0, 10)\n```\n\nbut it doesn't pass along extra keyword parameters to the top-level plot function:\n\n```\nsage: plot(Bessel(0,'J'),0,10,color='red')\nTypeError: plot() got an unexpected keyword argument 'rgbcolor'\n```\n\n---\n\nThis issue was originally reported here: http://ask.sagemath.org/question/1819/plotting-bessel-function\n\n**Assignee:** @jasongrout, @williamstein\n\n**CC:**  @kcrisman\n\n**Keywords:** plot bessel\n\n**Reviewer:** Vincent Delecroix\n\nIssue created by migration from https://trac.sagemath.org/ticket/13553\n\n",
    "closed_at": "2018-05-18T17:16:26Z",
    "created_at": "2012-09-30T21:27:17Z",
    "labels": [
        "component: graphics",
        "bug",
        "wontfix"
    ],
    "title": "Bessel class plot method does not support any standard plot options",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/13553",
    "user": "https://github.com/benjaminfjones"
}
```
The `Bessel` class defined its own plot method which is called e.g. when you run

```
sage: plot(Bessel(0,'J'), 0, 10)
```

but it doesn't pass along extra keyword parameters to the top-level plot function:

```
sage: plot(Bessel(0,'J'),0,10,color='red')
TypeError: plot() got an unexpected keyword argument 'rgbcolor'
```

---

This issue was originally reported here: http://ask.sagemath.org/question/1819/plotting-bessel-function

**Assignee:** @jasongrout, @williamstein

**CC:**  @kcrisman

**Keywords:** plot bessel

**Reviewer:** Vincent Delecroix

Issue created by migration from https://trac.sagemath.org/ticket/13553





---

archive/issue_events_116078.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13553#event-116078"
}
```



---

archive/issue_events_116079.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13553#event-116079"
}
```



---

archive/issue_events_116080.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13553#event-116080"
}
```



---

archive/issue_events_116081.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13553#event-116081"
}
```



---

archive/issue_events_116082.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13553#event-116082"
}
```



---

archive/issue_events_116083.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13553#event-116083"
}
```



---

archive/issue_events_116084.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13553#event-116084"
}
```



---

archive/issue_events_116085.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13553#event-116085"
}
```



---

archive/issue_comments_160569.json:
```json
{
    "body": "<a id='comment:6'></a>\nno longer an issue, let us close that one",
    "created_at": "2018-04-30T08:53:45Z",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13553#issuecomment-160569",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:6'></a>
no longer an issue, let us close that one



---

archive/issue_events_116086.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-04-30T08:53:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13553#event-116086"
}
```



---

archive/issue_events_116087.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-04-30T08:53:45Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13553#event-116087"
}
```



---

archive/issue_events_116088.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-01T12:30:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13553#event-116088"
}
```



---

archive/issue_events_116089.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-01T12:30:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13553#event-116089"
}
```



---

archive/issue_comments_160570.json:
```json
{
    "body": "**Reviewer:** Vincent Delecroix",
    "created_at": "2018-05-01T12:30:30Z",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13553#issuecomment-160570",
    "user": "https://github.com/videlec"
}
```

**Reviewer:** Vincent Delecroix



---

archive/issue_events_116090.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "label": "wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13553#event-116090"
}
```



---

archive/issue_events_116091.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13553#event-116091"
}
```



---

archive/issue_events_116092.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13553#event-116092"
}
```



---

archive/issue_comments_160571.json:
```json
{
    "body": "<a id='comment:8'></a>\nclosing positively reviewed duplicates",
    "created_at": "2018-05-18T17:16:26Z",
    "issue": "https://github.com/sagemath/sage/issues/13553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13553#issuecomment-160571",
    "user": "https://github.com/videlec"
}
```

<a id='comment:8'></a>
closing positively reviewed duplicates

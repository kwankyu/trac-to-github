# Issue 13967: Redesign Mac app loading page to match redesigned notebook.

archive/issues_013763.json:
```json
{
    "assignees": [
        "https://github.com/gvol"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe notebook is being redesigned and it would be nice to have the loading page match the new design.\n\nCC:  @kcrisman\n\nComponent: **user interface**\n\nKeywords: **mac app, mac**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13967_\n\n",
    "closed_at": "2016-01-10T10:24:06Z",
    "created_at": "2013-01-18T10:12:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Redesign Mac app loading page to match redesigned notebook.",
    "type": "issue",
    "updated_at": "2016-01-10T10:24:06Z",
    "url": "https://github.com/sagemath/sage/issues/13967",
    "user": "https://github.com/gvol"
}
```
<div id="comment:0"></div>

The notebook is being redesigned and it would be nice to have the loading page match the new design.

CC:  @kcrisman

Component: **user interface**

Keywords: **mac app, mac**

_Issue created by migration from https://trac.sagemath.org/ticket/13967_





---

archive/issue_events_194339.json:
```json
{
    "actor": "https://github.com/gvol",
    "created_at": "2013-01-18T10:12:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13967",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13967#event-194339"
}
```



---

archive/issue_events_194340.json:
```json
{
    "actor": "https://github.com/gvol",
    "created_at": "2013-01-18T10:12:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13967",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13967#event-194340"
}
```



---

archive/issue_events_194341.json:
```json
{
    "actor": "https://github.com/gvol",
    "created_at": "2013-01-18T10:12:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13967",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13967#event-194341"
}
```



---

archive/issue_events_194342.json:
```json
{
    "actor": "https://github.com/gvol",
    "created_at": "2013-01-18T10:12:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13967",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13967#event-194342"
}
```



---

archive/issue_events_194343.json:
```json
{
    "actor": "https://github.com/gvol",
    "created_at": "2013-01-18T10:12:03Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/13967",
    "subject": "https://github.com/gvol",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13967#event-194343"
}
```



---

archive/issue_comments_165321.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nsage-really-pending... who knows when that will happen.",
    "created_at": "2013-01-20T02:21:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13967#issuecomment-165321",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:1" align="right">comment:1</div>

sage-really-pending... who knows when that will happen.



---

archive/issue_events_194344.json:
```json
{
    "actor": "https://github.com/gvol",
    "created_at": "2016-01-08T02:44:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13967",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13967#event-194344"
}
```



---

archive/issue_comments_165322.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThe new notebook never got finished, and never will at this point.",
    "created_at": "2016-01-08T02:44:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13967#issuecomment-165322",
    "user": "https://github.com/gvol"
}
```

<div id="comment:2" align="right">comment:2</div>

The new notebook never got finished, and never will at this point.



---

archive/issue_events_194345.json:
```json
{
    "actor": "https://github.com/gvol",
    "created_at": "2016-01-08T02:44:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13967",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13967#event-194345"
}
```



---

archive/issue_events_194346.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-01-10T10:24:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13967",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13967#event-194346"
}
```



---

archive/issue_events_194347.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-01-10T10:24:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13967",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13967#event-194347"
}
```

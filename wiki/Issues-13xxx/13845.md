# Issue 13845: BooleanPolynomialRing.random_element chokes on terms parameter of type rational

archive/issues_013641.json:
```json
{
    "body": "This shouldn't produce an error:\n\n```\nsage: n = 10\nsage: B = BooleanPolynomialRing(n, 'x')\nsage: B.random_element(terms=(n/2)**2))\n```\n\n**Assignee:** @aghitza\n\n**CC:**  @alexanderdreyer PolyBoRi\n\n**Upstream:** None of the above - read trac for reasoning.\n\n**Reviewer:** Alexander Dreyer\n\n**Author:** Martin Albrecht\n\n**Merged:** sage-5.6.beta2\n\nIssue created by migration from https://trac.sagemath.org/ticket/13845\n\n",
    "closed_at": "2012-12-29T19:31:46Z",
    "created_at": "2012-12-19T12:16:44Z",
    "labels": [
        "component: basic arithmetic",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.6",
    "title": "BooleanPolynomialRing.random_element chokes on terms parameter of type rational",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/13845",
    "user": "https://github.com/malb"
}
```
This shouldn't produce an error:

```
sage: n = 10
sage: B = BooleanPolynomialRing(n, 'x')
sage: B.random_element(terms=(n/2)**2))
```

**Assignee:** @aghitza

**CC:**  @alexanderdreyer PolyBoRi

**Upstream:** None of the above - read trac for reasoning.

**Reviewer:** Alexander Dreyer

**Author:** Martin Albrecht

**Merged:** sage-5.6.beta2

Issue created by migration from https://trac.sagemath.org/ticket/13845





---

archive/issue_events_119541.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2012-12-19T12:26:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13845",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13845#event-119541"
}
```



---

archive/attachments_019352.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_13845.patch",
    "asset_url": "tarball://root/attachments/ticket13845/trac_13845.patch",
    "created_at": "2012-12-19T12:26:54Z",
    "issue": "https://github.com/sagemath/sage/issues/13845",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket13845/trac_13845.patch",
    "user": "https://github.com/malb"
}
```



---

archive/issue_comments_165578.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_13845.patch](https://github.com/sagemath/sage/files/ticket13845/trac_13845.patch)",
    "created_at": "2012-12-19T12:26:54Z",
    "issue": "https://github.com/sagemath/sage/issues/13845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13845#issuecomment-165578",
    "user": "https://github.com/malb"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_13845.patch](https://github.com/sagemath/sage/files/ticket13845/trac_13845.patch)



---

archive/issue_comments_165579.json:
```json
{
    "body": "**Author:** Martin Albrecht",
    "created_at": "2012-12-19T12:26:54Z",
    "issue": "https://github.com/sagemath/sage/issues/13845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13845#issuecomment-165579",
    "user": "https://github.com/malb"
}
```

**Author:** Martin Albrecht



---

archive/issue_comments_165580.json:
```json
{
    "body": "**Reviewer:** Alexander Dreyer",
    "created_at": "2012-12-20T12:40:37Z",
    "issue": "https://github.com/sagemath/sage/issues/13845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13845#issuecomment-165580",
    "user": "https://github.com/alexanderdreyer"
}
```

**Reviewer:** Alexander Dreyer



---

archive/issue_events_119542.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2012-12-20T12:40:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13845",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13845#event-119542"
}
```



---

archive/issue_events_119543.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2012-12-20T12:40:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13845",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13845#event-119543"
}
```



---

archive/issue_comments_165581.json:
```json
{
    "body": "<a id='comment:2'></a>\nI can give positive review for that patch.",
    "created_at": "2012-12-20T12:40:37Z",
    "issue": "https://github.com/sagemath/sage/issues/13845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13845#issuecomment-165581",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:2'></a>
I can give positive review for that patch.



---

archive/issue_events_119544.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-29T19:31:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13845",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13845#event-119544"
}
```



---

archive/issue_events_119545.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-29T19:31:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13845",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13845#event-119545"
}
```



---

archive/issue_comments_165582.json:
```json
{
    "body": "**Merged:** sage-5.6.beta2",
    "created_at": "2012-12-29T19:31:46Z",
    "issue": "https://github.com/sagemath/sage/issues/13845",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13845#issuecomment-165582",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.6.beta2

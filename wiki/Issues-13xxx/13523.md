# Issue 13523: Implement sage -O which passes the -O option to python to disable assertions

archive/issues_013319.json:
```json
{
    "body": "In the meantime, one can export `PYTHONOPTIMIZE=yes` to get the same effect.\n\n**Assignee:** @nexttime\n\n**Keywords:** days85\n\nIssue created by migration from https://trac.sagemath.org/ticket/13523\n\n",
    "created_at": "2012-09-24T08:40:43Z",
    "labels": [
        "component: scripts",
        "enhancement",
        "needs info"
    ],
    "title": "Implement sage -O which passes the -O option to python to disable assertions",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/13523",
    "user": "https://github.com/nthiery"
}
```
In the meantime, one can export `PYTHONOPTIMIZE=yes` to get the same effect.

**Assignee:** @nexttime

**Keywords:** days85

Issue created by migration from https://trac.sagemath.org/ticket/13523





---

archive/issue_events_115734.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-115734"
}
```



---

archive/issue_events_115735.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-115735"
}
```



---

archive/issue_events_115736.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-115736"
}
```



---

archive/issue_events_115737.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-115737"
}
```



---

archive/issue_events_115738.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-115738"
}
```



---

archive/issue_events_115739.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-115739"
}
```



---

archive/issue_events_115740.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-115740"
}
```



---

archive/issue_events_115741.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-115741"
}
```



---

archive/issue_comments_160152.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+In the meantime, one can export `PYTHONOPTIMIZE=yes` to get the same effect.\n``````\n",
    "created_at": "2016-12-07T18:27:18Z",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13523#issuecomment-160152",
    "user": "https://github.com/saraedum"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+In the meantime, one can export `PYTHONOPTIMIZE=yes` to get the same effect.
``````




---

archive/issue_comments_160153.json:
```json
{
    "body": "<a id='comment:6'></a>\nSince the command line interface is hacky enough already maybe it would be better to document the `PYTHONOPTIMIZE=yes` somewhere instead?",
    "created_at": "2017-03-14T12:23:15Z",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13523#issuecomment-160153",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:6'></a>
Since the command line interface is hacky enough already maybe it would be better to document the `PYTHONOPTIMIZE=yes` somewhere instead?



---

archive/issue_events_115742.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-03-14T12:23:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "label": "needs info",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-115742"
}
```



---

archive/issue_comments_160154.json:
```json
{
    "body": "**Changing keywords** from \"\" to \"days85\".",
    "created_at": "2017-03-14T16:12:12Z",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13523#issuecomment-160154",
    "user": "https://github.com/saraedum"
}
```

**Changing keywords** from "" to "days85".



---

archive/issue_events_115743.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-115743"
}
```

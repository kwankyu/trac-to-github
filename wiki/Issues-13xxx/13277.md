# Issue 13277: Add _factor_univariate_polynomial() for RealField and ComplexField

archive/issues_013105.json:
```json
{
    "body": "As a part of #11731, transfer the code which does the factorization over `RealField` and `ComplexField` from `sage.rings.polynomial.polynomial_element.Polynomial.factor()` to the corresponding classes.\n\nDepends on #13276\n\n**Assignee:** @aghitza\n\n**Keywords:** sd59\n\n**Branch/Commit:** [601121d0b891041e0ffe9dc12edbb87f053e870c](https://github.com/sagemath/sagetrac-mirror/commit/601121d0b891041e0ffe9dc12edbb87f053e870c)\n\n**Reviewer:** Martin Albrecht\n\n**Author:** Julian Rueth\n\nIssue created by migration from https://trac.sagemath.org/ticket/13277\n\n",
    "closed_at": "2014-07-01T12:54:21Z",
    "created_at": "2012-07-19T22:49:17Z",
    "labels": [
        "component: basic arithmetic",
        "trivial"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "title": "Add _factor_univariate_polynomial() for RealField and ComplexField",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/13277",
    "user": "https://github.com/saraedum"
}
```
As a part of #11731, transfer the code which does the factorization over `RealField` and `ComplexField` from `sage.rings.polynomial.polynomial_element.Polynomial.factor()` to the corresponding classes.

Depends on #13276

**Assignee:** @aghitza

**Keywords:** sd59

**Branch/Commit:** [601121d0b891041e0ffe9dc12edbb87f053e870c](https://github.com/sagemath/sagetrac-mirror/commit/601121d0b891041e0ffe9dc12edbb87f053e870c)

**Reviewer:** Martin Albrecht

**Author:** Julian Rueth

Issue created by migration from https://trac.sagemath.org/ticket/13277





---

archive/issue_comments_155685.json:
```json
{
    "body": "<a id='comment:1'></a>\nThe new code does not call `pari.set_real_precision()` anymore. According to the docstring it does not have an effect on the computation anyway.",
    "created_at": "2012-07-19T22:51:09Z",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13277#issuecomment-155685",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:1'></a>
The new code does not call `pari.set_real_precision()` anymore. According to the docstring it does not have an effect on the computation anyway.



---

archive/issue_events_113218.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/asalerno",
    "created_at": "2013-07-11T22:23:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13277#event-113218"
}
```



---

archive/attachments_018608.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_13277.patch",
    "asset_url": "tarball://root/attachments/ticket13277/trac_13277.patch",
    "created_at": "2013-07-11T22:23:44Z",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket13277/trac_13277.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/asalerno"
}
```



---

archive/issue_comments_155686.json:
```json
{
    "body": "<a id='comment:2'></a>\n**Attachment:** [trac_13277.patch](https://github.com/sagemath/sage/files/ticket13277/trac_13277.patch)",
    "created_at": "2013-07-11T22:23:44Z",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13277#issuecomment-155686",
    "user": "https://trac.sagemath.org/admin/accounts/users/asalerno"
}
```

<a id='comment:2'></a>
**Attachment:** [trac_13277.patch](https://github.com/sagemath/sage/files/ticket13277/trac_13277.patch)



---

archive/issue_comments_155687.json:
```json
{
    "body": "<a id='comment:3'></a>\nThis patch depends on #13274, which no longer applies to Sage 5.10.",
    "created_at": "2013-07-11T22:25:02Z",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13277#issuecomment-155687",
    "user": "https://trac.sagemath.org/admin/accounts/users/asalerno"
}
```

<a id='comment:3'></a>
This patch depends on #13274, which no longer applies to Sage 5.10.



---

archive/issue_events_113219.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/asalerno",
    "created_at": "2013-07-11T22:25:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13277#event-113219"
}
```



---

archive/issue_events_113220.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/asalerno",
    "created_at": "2013-07-11T22:25:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13277#event-113220"
}
```



---

archive/issue_events_113221.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13277#event-113221"
}
```



---

archive/issue_events_113222.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13277#event-113222"
}
```



---

archive/issue_events_113223.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13277#event-113223"
}
```



---

archive/issue_events_113224.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13277#event-113224"
}
```



---

archive/issue_events_113225.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13277#event-113225"
}
```



---

archive/issue_events_113226.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13277#event-113226"
}
```



---

archive/issue_comments_155688.json:
```json
{
    "body": "**Branch:** [u/saraedum/ticket/13277](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/13277)",
    "created_at": "2014-06-25T16:50:25Z",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13277#issuecomment-155688",
    "user": "https://github.com/saraedum"
}
```

**Branch:** [u/saraedum/ticket/13277](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/13277)



---

archive/issue_comments_155689.json:
```json
{
    "body": "**Changing keywords** from \"\" to \"sd59\".",
    "created_at": "2014-06-25T16:51:37Z",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13277#issuecomment-155689",
    "user": "https://github.com/saraedum"
}
```

**Changing keywords** from "" to "sd59".



---

archive/issue_events_113227.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-06-25T16:51:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13277#event-113227"
}
```



---

archive/issue_events_113228.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-06-25T16:51:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13277#event-113228"
}
```



---

archive/issue_comments_155690.json:
```json
{
    "body": "**Commit:** [601121d0b891041e0ffe9dc12edbb87f053e870c](https://github.com/sagemath/sagetrac-mirror/commit/601121d0b891041e0ffe9dc12edbb87f053e870c)",
    "created_at": "2014-06-30T18:11:16Z",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13277#issuecomment-155690",
    "user": "https://github.com/malb"
}
```

**Commit:** [601121d0b891041e0ffe9dc12edbb87f053e870c](https://github.com/sagemath/sagetrac-mirror/commit/601121d0b891041e0ffe9dc12edbb87f053e870c)



---

archive/issue_comments_155691.json:
```json
{
    "body": "**Reviewer:** Martin Albrecht",
    "created_at": "2014-06-30T18:11:16Z",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13277#issuecomment-155691",
    "user": "https://github.com/malb"
}
```

**Reviewer:** Martin Albrecht



---

archive/issue_comments_155692.json:
```json
{
    "body": "<a id='comment:9'></a>\nLooks good.\n\n---\n**New commits:**\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/601121d0b891041e0ffe9dc12edbb87f053e870c\">601121d</a></td><td><code>Trac 13277: Added _factor_univariate_polynomial() for ComplexField and RealField</code></td></tr></table>\n",
    "created_at": "2014-06-30T18:11:16Z",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13277#issuecomment-155692",
    "user": "https://github.com/malb"
}
```

<a id='comment:9'></a>
Looks good.

---
**New commits:**
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/601121d0b891041e0ffe9dc12edbb87f053e870c">601121d</a></td><td><code>Trac 13277: Added _factor_univariate_polynomial() for ComplexField and RealField</code></td></tr></table>




---

archive/issue_events_113229.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2014-06-30T18:11:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13277#event-113229"
}
```



---

archive/issue_events_113230.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2014-06-30T18:11:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13277#event-113230"
}
```



---

archive/issue_events_113231.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-01T12:54:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13277#event-113231"
}
```



---

archive/issue_events_113232.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9b8648d7ed56bbb18a855aa8edc1bb6aabab36cc",
    "created_at": "2014-07-01T12:54:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13277#event-113232"
}
```



---

archive/issue_comments_155693.json:
```json
{
    "body": "**Changing branch** from \"[u/saraedum/ticket/13277](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/13277)\" to \"[601121d0b891041e0ffe9dc12edbb87f053e870c](https://github.com/sagemath/sagetrac-mirror/commit/601121d0b891041e0ffe9dc12edbb87f053e870c)\".",
    "created_at": "2014-07-01T12:54:21Z",
    "issue": "https://github.com/sagemath/sage/issues/13277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13277#issuecomment-155693",
    "user": "https://github.com/vbraun"
}
```

**Changing branch** from "[u/saraedum/ticket/13277](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/13277)" to "[601121d0b891041e0ffe9dc12edbb87f053e870c](https://github.com/sagemath/sagetrac-mirror/commit/601121d0b891041e0ffe9dc12edbb87f053e870c)".

# Issue 13139: Bug in IntegerFactorization

archive/issues_012967.json:
```json
{
    "body": "\n```\nsage: from sage.structure.factorization_integer import IntegerFactorization\nsage: IntegerFactorization([(3,1)],unsafe=True) \n---------------------------------------------------------------------------\nNameError                                 Traceback (most recent call last)\n\n../local/lib/python2.7/site-packages/sage/structure/factorization_integer.py in __init__(self, x, unit, cr, sort, simplify, unsafe)\n     58         if unsafe:\n     59             if unit is None:\n---> 60                 self._Factorization__unit = sage.rings.integer.ONE\n     61             else:\n     62                 self._Factorization__unit = unit\n\nNameError: global name 'sage' is not defined\n```\n\n**Reviewer:** R. Andrew Ohana\n\n**Author:** Andr\u00e9 Apitzsch\n\n**Merged:** sage-5.2.beta0\n\nIssue created by migration from https://trac.sagemath.org/ticket/13139\n\n",
    "closed_at": "2012-07-02T15:24:44Z",
    "created_at": "2012-06-19T22:00:46Z",
    "labels": [
        "component: factorization",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.2",
    "title": "Bug in IntegerFactorization",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/13139",
    "user": "https://github.com/a-andre"
}
```

```
sage: from sage.structure.factorization_integer import IntegerFactorization
sage: IntegerFactorization([(3,1)],unsafe=True) 
---------------------------------------------------------------------------
NameError                                 Traceback (most recent call last)

../local/lib/python2.7/site-packages/sage/structure/factorization_integer.py in __init__(self, x, unit, cr, sort, simplify, unsafe)
     58         if unsafe:
     59             if unit is None:
---> 60                 self._Factorization__unit = sage.rings.integer.ONE
     61             else:
     62                 self._Factorization__unit = unit

NameError: global name 'sage' is not defined
```

**Reviewer:** R. Andrew Ohana

**Author:** André Apitzsch

**Merged:** sage-5.2.beta0

Issue created by migration from https://trac.sagemath.org/ticket/13139





---

archive/issue_events_111420.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2012-06-19T22:04:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13139",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13139#event-111420"
}
```



---

archive/issue_comments_152785.json:
```json
{
    "body": "**Reviewer:** R. Andrew Ohana",
    "created_at": "2012-06-19T23:08:11Z",
    "issue": "https://github.com/sagemath/sage/issues/13139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13139#issuecomment-152785",
    "user": "https://github.com/ohanar"
}
```

**Reviewer:** R. Andrew Ohana



---

archive/issue_comments_152786.json:
```json
{
    "body": "<a id='comment:2'></a>\nPlease add a doctest to verify that this bug is fixed.",
    "created_at": "2012-06-19T23:08:11Z",
    "issue": "https://github.com/sagemath/sage/issues/13139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13139#issuecomment-152786",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:2'></a>
Please add a doctest to verify that this bug is fixed.



---

archive/issue_events_111421.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2012-06-19T23:08:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13139",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13139#event-111421"
}
```



---

archive/issue_events_111422.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2012-06-19T23:08:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13139",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13139#event-111422"
}
```



---

archive/attachments_018385.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_13139.patch",
    "asset_url": "tarball://root/attachments/ticket13139/trac_13139.patch",
    "created_at": "2012-06-20T08:29:49Z",
    "issue": "https://github.com/sagemath/sage/issues/13139",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket13139/trac_13139.patch",
    "user": "https://github.com/a-andre"
}
```



---

archive/issue_comments_152787.json:
```json
{
    "body": "<a id='comment:3'></a>\n**Attachment:** [trac_13139.patch](https://github.com/sagemath/sage/files/ticket13139/trac_13139.patch)",
    "created_at": "2012-06-20T08:29:49Z",
    "issue": "https://github.com/sagemath/sage/issues/13139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13139#issuecomment-152787",
    "user": "https://github.com/a-andre"
}
```

<a id='comment:3'></a>
**Attachment:** [trac_13139.patch](https://github.com/sagemath/sage/files/ticket13139/trac_13139.patch)



---

archive/issue_events_111423.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2012-06-20T08:29:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13139",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13139#event-111423"
}
```



---

archive/issue_events_111424.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2012-06-20T08:29:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13139",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13139#event-111424"
}
```



---

archive/issue_comments_152788.json:
```json
{
    "body": "<a id='comment:4'></a>\nOk, looks good to me.",
    "created_at": "2012-06-20T09:32:10Z",
    "issue": "https://github.com/sagemath/sage/issues/13139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13139#issuecomment-152788",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:4'></a>
Ok, looks good to me.



---

archive/issue_events_111425.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2012-06-20T09:32:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13139",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13139#event-111425"
}
```



---

archive/issue_events_111426.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2012-06-20T09:32:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13139",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13139#event-111426"
}
```



---

archive/issue_comments_152789.json:
```json
{
    "body": "**Merged:** sage-5.2.beta0",
    "created_at": "2012-07-02T15:24:44Z",
    "issue": "https://github.com/sagemath/sage/issues/13139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13139#issuecomment-152789",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.2.beta0



---

archive/issue_events_111427.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-02T15:24:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13139",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13139#event-111427"
}
```



---

archive/issue_events_111428.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-02T15:24:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13139",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13139#event-111428"
}
```

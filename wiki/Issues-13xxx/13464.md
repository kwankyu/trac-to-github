# Issue 13464: list good mirrors as fallback in sage-list-packages

archive/issues_013292.json:
```json
{
    "body": "This just replaces the dead scipy mirror with some reliable ones. Also, the directory must be spkg, not packages (just a matter of symlinking, but it's the other way round right now).\n\nNote: this applies inside $SAGE_ROOT/local/bin/\n\n**Assignee:** @jasongrout\n\n**Author:** Harald Schilly\n\nIssue created by migration from https://trac.sagemath.org/ticket/13464\n\n",
    "created_at": "2012-09-14T14:57:14Z",
    "labels": [
        "component: misc",
        "enhancement",
        "needs work"
    ],
    "title": "list good mirrors as fallback in sage-list-packages",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/13464",
    "user": "https://github.com/haraldschilly"
}
```
This just replaces the dead scipy mirror with some reliable ones. Also, the directory must be spkg, not packages (just a matter of symlinking, but it's the other way round right now).

Note: this applies inside $SAGE_ROOT/local/bin/

**Assignee:** @jasongrout

**Author:** Harald Schilly

Issue created by migration from https://trac.sagemath.org/ticket/13464





---

archive/attachments_018915.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "working_mirrors_in_sage-list-packages.patch",
    "asset_url": "tarball://root/attachments/ticket13464/working_mirrors_in_sage-list-packages.patch",
    "created_at": "2012-09-15T09:12:22Z",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket13464/working_mirrors_in_sage-list-packages.patch",
    "user": "https://github.com/haraldschilly"
}
```



---

archive/issue_comments_159696.json:
```json
{
    "body": "**Attachment:** [working_mirrors_in_sage-list-packages.patch](https://github.com/sagemath/sage/files/ticket13464/working_mirrors_in_sage-list-packages.patch)",
    "created_at": "2012-09-15T09:12:22Z",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13464#issuecomment-159696",
    "user": "https://github.com/haraldschilly"
}
```

**Attachment:** [working_mirrors_in_sage-list-packages.patch](https://github.com/sagemath/sage/files/ticket13464/working_mirrors_in_sage-list-packages.patch)



---

archive/issue_comments_159697.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n-This just replaces the dead scipy mirror with some reliable ones.\n+This just replaces the dead scipy mirror with some reliable ones. Also, the directory must be spkg, not packages (just a matter of symlinking, but it's the other way round right now).\n+\n+Note: this applies inside $SAGE_ROOT/local/bin/\n``````\n",
    "created_at": "2012-09-15T09:13:45Z",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13464#issuecomment-159697",
    "user": "https://github.com/haraldschilly"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1,3 @@
-This just replaces the dead scipy mirror with some reliable ones.
+This just replaces the dead scipy mirror with some reliable ones. Also, the directory must be spkg, not packages (just a matter of symlinking, but it's the other way round right now).
+
+Note: this applies inside $SAGE_ROOT/local/bin/
``````




---

archive/issue_events_115399.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2012-09-15T09:13:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13464#event-115399"
}
```



---

archive/issue_comments_159698.json:
```json
{
    "body": "**Author:** Harald Schilly",
    "created_at": "2012-10-05T19:22:42Z",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13464#issuecomment-159698",
    "user": "https://github.com/jdemeyer"
}
```

**Author:** Harald Schilly



---

archive/issue_events_115400.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-10-05T19:22:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13464#event-115400"
}
```



---

archive/issue_events_115401.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-10-05T19:22:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13464#event-115401"
}
```



---

archive/issue_comments_159699.json:
```json
{
    "body": "<a id='comment:2'></a>\nWhen googling for \"Sage optional packages\", I get this link:\n\n```\nhttp://www.sagemath.org/packages/optional/\n```\n\nNote that it uses \"packages\", not \"spkg\".  So I don't think it should be changed in the script.  Or you should at least say why it should be changed.",
    "created_at": "2012-10-05T19:22:42Z",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13464#issuecomment-159699",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'></a>
When googling for "Sage optional packages", I get this link:

```
http://www.sagemath.org/packages/optional/
```

Note that it uses "packages", not "spkg".  So I don't think it should be changed in the script.  Or you should at least say why it should be changed.



---

archive/issue_comments_159700.json:
```json
{
    "body": "<a id='comment:3'></a>\njdemeyer: i haven't thoroughly  checked this, but what I observed is, that on the mirrors it's spkg. This is probably due to some symlinks&rsync server stuff and so on.\n\nE.g.\n\n```\nhttp://ftp.sh.cvut.cz/MIRRORS/sagemath/packages/\nvs.\nhttp://ftp.sh.cvut.cz/MIRRORS/sagemath/spkg/\n```\n\nObviously, therefore downloading from another mirror fails. Plan B would be to figure out how to fix this for all mirrors.",
    "created_at": "2012-10-09T11:12:51Z",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13464#issuecomment-159700",
    "user": "https://github.com/haraldschilly"
}
```

<a id='comment:3'></a>
jdemeyer: i haven't thoroughly  checked this, but what I observed is, that on the mirrors it's spkg. This is probably due to some symlinks&rsync server stuff and so on.

E.g.

```
http://ftp.sh.cvut.cz/MIRRORS/sagemath/packages/
vs.
http://ftp.sh.cvut.cz/MIRRORS/sagemath/spkg/
```

Obviously, therefore downloading from another mirror fails. Plan B would be to figure out how to fix this for all mirrors.



---

archive/issue_events_115402.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13464#event-115402"
}
```



---

archive/issue_events_115403.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13464#event-115403"
}
```



---

archive/issue_events_115404.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13464#event-115404"
}
```



---

archive/issue_events_115405.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13464#event-115405"
}
```



---

archive/issue_comments_159701.json:
```json
{
    "body": "<a id='comment:6'></a>\nAdditionally, `ftp.sh.cvut.cz` looks down, and `http://http://mirrors.xmission.com/sage/` should be `http://mirrors.xmission.com/sage/`.",
    "created_at": "2014-03-14T14:24:50Z",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13464#issuecomment-159701",
    "user": "https://github.com/mezzarobba"
}
```

<a id='comment:6'></a>
Additionally, `ftp.sh.cvut.cz` looks down, and `http://http://mirrors.xmission.com/sage/` should be `http://mirrors.xmission.com/sage/`.



---

archive/issue_events_115406.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13464#event-115406"
}
```



---

archive/issue_events_115407.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13464#event-115407"
}
```



---

archive/issue_events_115408.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13464#event-115408"
}
```



---

archive/issue_events_115409.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13464#event-115409"
}
```



---

archive/issue_events_115410.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13464",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13464#event-115410"
}
```

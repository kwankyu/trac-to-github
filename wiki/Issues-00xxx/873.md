# Issue 873: sage -clone is rebuilding everything !

archive/issues_000873.json:
```json
{
    "body": "Doing \"sage -clone foo\" rebuild all the pyx files, which is terrible. \n\n**Assignee:** @williamstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/873\n\n",
    "closed_at": "2007-10-13T07:14:33Z",
    "created_at": "2007-10-13T05:49:12Z",
    "labels": [
        "component: user interface",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.7",
    "title": "sage -clone is rebuilding everything !",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/873",
    "user": "https://github.com/williamstein"
}
```
Doing "sage -clone foo" rebuild all the pyx files, which is terrible. 

**Assignee:** @williamstein

Issue created by migration from https://trac.sagemath.org/ticket/873





---

archive/issue_events_004128.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-13T05:49:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/873",
    "label": "component: algebraic geometry",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/873#event-4128"
}
```



---

archive/issue_events_004129.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-13T05:49:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/873",
    "label": "component: user interface",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/873#event-4129"
}
```



---

archive/issue_comments_004104.json:
```json
{
    "body": "fixes slow \"-clone\" (look for Cython-generated files, not Pyrex)",
    "created_at": "2007-10-13T07:02:26Z",
    "issue": "https://github.com/sagemath/sage/issues/873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/873#issuecomment-4104",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

fixes slow "-clone" (look for Cython-generated files, not Pyrex)



---

archive/attachments_000559.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "466.patch",
    "asset_url": "tarball://root/attachments/ticket873/466.patch",
    "created_at": "2007-10-13T07:02:41Z",
    "issue": "https://github.com/sagemath/sage/issues/873",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket873/466.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```



---

archive/issue_comments_004105.json:
```json
{
    "body": "<a id='comment:3'></a>\n**Attachment:** [466.patch](https://github.com/sagemath/sage/files/ticket873/466.patch)",
    "created_at": "2007-10-13T07:02:41Z",
    "issue": "https://github.com/sagemath/sage/issues/873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/873#issuecomment-4105",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

<a id='comment:3'></a>
**Attachment:** [466.patch](https://github.com/sagemath/sage/files/ticket873/466.patch)



---

archive/issue_events_004130.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-13T07:14:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/873",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/873#event-4130"
}
```

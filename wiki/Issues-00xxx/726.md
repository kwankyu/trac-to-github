# Issue 726: notebook worksheet area has horizontal scrollbar

archive/issues_000726.json:
```json
{
    "body": "This shouldn't be needed unless the output from a cell needs it.\n\n**Assignee:** boothby\n\nIssue created by migration from https://trac.sagemath.org/ticket/726\n\n",
    "closed_at": "2007-10-20T18:45:37Z",
    "created_at": "2007-09-21T03:14:28Z",
    "labels": [
        "component: notebook",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.9",
    "title": "notebook worksheet area has horizontal scrollbar",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/726",
    "user": "https://github.com/mwhansen"
}
```
This shouldn't be needed unless the output from a cell needs it.

**Assignee:** boothby

Issue created by migration from https://trac.sagemath.org/ticket/726





---

archive/issue_events_003282.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans",
    "created_at": "2007-09-21T03:17:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/726",
    "label": "minor",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/726#event-3282"
}
```



---

archive/issue_events_003283.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans",
    "created_at": "2007-10-20T18:45:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/726",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/726#event-3283"
}
```



---

archive/issue_comments_003175.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis was fixed by mhansen",
    "created_at": "2007-10-20T18:45:37Z",
    "issue": "https://github.com/sagemath/sage/issues/726",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/726#issuecomment-3175",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```

<a id='comment:2'></a>
This was fixed by mhansen



---

archive/issue_events_003284.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-10-24T17:12:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/726",
    "milestone": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/726#event-3284"
}
```



---

archive/issue_events_003285.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-10-24T17:12:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/726",
    "milestone": "sage-2.8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/726#event-3285"
}
```

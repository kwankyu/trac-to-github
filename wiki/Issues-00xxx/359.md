# Issue 359: Consider adding dhtml javascript drawing functionality to the SAGE notebook

archive/issues_000359.json:
```json
{
    "body": "See, e.g., \n  http://www.walterzorn.com/jsgraphics/jsgraphics_e.htm\n\n**Assignee:** Robert Miller\n\n**CC:**  acleone\n\nIssue created by migration from https://trac.sagemath.org/ticket/359\n\n",
    "closed_at": "2010-03-29T21:14:20Z",
    "created_at": "2007-04-26T05:15:16Z",
    "labels": [
        "component: notebook",
        "minor",
        "enhancement",
        "duplicate"
    ],
    "title": "Consider adding dhtml javascript drawing functionality to the SAGE notebook",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/359",
    "user": "https://github.com/williamstein"
}
```
See, e.g., 
  http://www.walterzorn.com/jsgraphics/jsgraphics_e.htm

**Assignee:** Robert Miller

**CC:**  acleone

Issue created by migration from https://trac.sagemath.org/ticket/359





---

archive/issue_events_001403.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-09-10T02:20:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/359",
    "milestone": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/359#event-1403"
}
```



---

archive/issue_comments_001301.json:
```json
{
    "body": "<a id='comment:2'></a>\nSee #6973 (use SVG or canvas).",
    "created_at": "2010-03-29T21:14:20Z",
    "issue": "https://github.com/sagemath/sage/issues/359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/359#issuecomment-1301",
    "user": "https://trac.sagemath.org/admin/accounts/users/acleone"
}
```

<a id='comment:2'></a>
See #6973 (use SVG or canvas).



---

archive/issue_events_001404.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/acleone",
    "created_at": "2010-03-29T21:14:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/359",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/359#event-1404"
}
```



---

archive/issue_events_001405.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/acleone",
    "created_at": "2010-03-29T21:14:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/359",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/359#event-1405"
}
```

# Issue 588: Symmetrica spkg for SAGE

archive/issues_000588.json:
```json
{
    "body": "\n\n**Assignee:** @mwhansen\n\n**CC:**  sage-combinat\n\nIssue created by migration from https://trac.sagemath.org/ticket/588\n\n",
    "closed_at": "2007-09-05T05:04:57Z",
    "created_at": "2007-09-04T19:44:52Z",
    "labels": [
        "component: combinatorics",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.4",
    "title": "Symmetrica spkg for SAGE",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/588",
    "user": "https://github.com/mwhansen"
}
```


**Assignee:** @mwhansen

**CC:**  sage-combinat

Issue created by migration from https://trac.sagemath.org/ticket/588





---

archive/issue_comments_002199.json:
```json
{
    "body": "<a id='comment:1'></a>\nInitial spkg can be found here: http://www.mwhansen.org/symmetrica-0.1.spkg",
    "created_at": "2007-09-04T19:49:56Z",
    "issue": "https://github.com/sagemath/sage/issues/588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/588#issuecomment-2199",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Initial spkg can be found here: http://www.mwhansen.org/symmetrica-0.1.spkg



---

archive/issue_comments_002200.json:
```json
{
    "body": "<a id='comment:2'></a>\nUpdated version: http://www.mwhansen.org/symmetrica-0.2.spkg",
    "created_at": "2007-09-04T20:02:42Z",
    "issue": "https://github.com/sagemath/sage/issues/588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/588#issuecomment-2200",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
Updated version: http://www.mwhansen.org/symmetrica-0.2.spkg



---

archive/issue_events_002551.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2007-09-04T21:35:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/588",
    "label": "bug",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/588#event-2551"
}
```



---

archive/issue_events_002552.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2007-09-04T21:35:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/588",
    "label": "enhancement",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/588#event-2552"
}
```



---

archive/issue_comments_002201.json:
```json
{
    "body": "**Changing assignee** from @williamstein to @mwhansen.",
    "created_at": "2007-09-04T21:36:00Z",
    "issue": "https://github.com/sagemath/sage/issues/588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/588#issuecomment-2201",
    "user": "https://github.com/mwhansen"
}
```

**Changing assignee** from @williamstein to @mwhansen.



---

archive/issue_comments_002202.json:
```json
{
    "body": "<a id='comment:5'></a>\nI've made this an official optional spkg for now.  It will likely move to standard at some point.",
    "created_at": "2007-09-05T05:04:57Z",
    "issue": "https://github.com/sagemath/sage/issues/588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/588#issuecomment-2202",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:5'></a>
I've made this an official optional spkg for now.  It will likely move to standard at some point.



---

archive/issue_events_002553.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-05T05:04:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/588",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/588#event-2553"
}
```



---

archive/issue_events_002554.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-05T05:05:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/588",
    "milestone": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/588#event-2554"
}
```



---

archive/issue_events_002555.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-09-05T05:05:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/588",
    "milestone": "sage-2.8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/588#event-2555"
}
```

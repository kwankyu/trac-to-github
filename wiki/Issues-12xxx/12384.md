# Issue 12384: sage.combinat.e_one_star uses MS-DOS style line breaks

archive/issues_012212.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-sage-combinat"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe carriage returns are causing problems with the new doctest framework.\n\nDepends on #9958\n\nComponent: **combinatorics**\n\nAuthor: **David Roe**\n\nReviewer: **David Loeffler**\n\nMerged: **sage-5.0.beta9**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12384_\n\n",
    "closed_at": "2012-03-21T22:08:23Z",
    "created_at": "2012-01-30T01:04:39Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage.combinat.e_one_star uses MS-DOS style line breaks",
    "type": "issue",
    "updated_at": "2012-03-21T22:08:23Z",
    "url": "https://github.com/sagemath/sage/issues/12384",
    "user": "https://github.com/roed314"
}
```
<div id="comment:0"></div>

The carriage returns are causing problems with the new doctest framework.

Depends on #9958

Component: **combinatorics**

Author: **David Roe**

Reviewer: **David Loeffler**

Merged: **sage-5.0.beta9**

_Issue created by migration from https://trac.sagemath.org/ticket/12384_





---

archive/issue_events_167383.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-01-30T01:04:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12384",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12384#event-167383"
}
```



---

archive/issue_events_167384.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-01-30T01:04:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12384",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12384#event-167384"
}
```



---

archive/issue_events_167385.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-01-30T01:04:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12384",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12384#event-167385"
}
```



---

archive/issue_events_167386.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-01-30T01:04:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12384",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12384#event-167386"
}
```



---

archive/issue_events_167387.json:
```json
{
    "actor": "https://github.com/sagetrac-sage-combinat",
    "created_at": "2012-01-30T01:04:39Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12384",
    "subject": "https://github.com/roed314",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12384#event-167387"
}
```



---

archive/issue_events_167388.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-01-30T01:05:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12384",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12384#event-167388"
}
```



---

archive/issue_comments_135313.json:
```json
{
    "body": "Apply only this patch",
    "created_at": "2012-03-13T00:44:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12384",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12384#issuecomment-135313",
    "user": "https://github.com/loefflerd"
}
```

Apply only this patch



---

archive/issue_events_167389.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-03-13T00:46:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12384",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12384#event-167389"
}
```



---

archive/issue_events_167390.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-03-13T00:46:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12384",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12384#event-167390"
}
```



---

archive/issue_comments_135314.json:
```json
{
    "body": "Reviewer: **David Loeffler**",
    "created_at": "2012-03-13T00:46:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12384",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12384#issuecomment-135314",
    "user": "https://github.com/loefflerd"
}
```

Reviewer: **David Loeffler**



---

archive/issue_comments_135315.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAttachment: **[12384v2.patch.gz](https://github.com/sagemath/sage/files/ticket12384/12384v2.patch.gz)**\n\nWe may as well keep the patchbot quiet by killing trailing whitespace at the same time. Since it's just a two-character change, I guess we don't need another reviewing cycle.",
    "created_at": "2012-03-13T00:46:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12384",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12384#issuecomment-135315",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:2" align="right">comment:2</div>

Attachment: **[12384v2.patch.gz](https://github.com/sagemath/sage/files/ticket12384/12384v2.patch.gz)**

We may as well keep the patchbot quiet by killing trailing whitespace at the same time. Since it's just a two-character change, I guess we don't need another reviewing cycle.



---

archive/issue_comments_135316.json:
```json
{
    "body": "Dependencies: **#9958**",
    "created_at": "2012-03-13T00:47:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12384",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12384#issuecomment-135316",
    "user": "https://github.com/loefflerd"
}
```

Dependencies: **#9958**



---

archive/issue_comments_135317.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nI'm happy to sign off on whitespace changes anyway.",
    "created_at": "2012-03-13T01:14:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12384",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12384#issuecomment-135317",
    "user": "https://github.com/roed314"
}
```

<div id="comment:4" align="right">comment:4</div>

I'm happy to sign off on whitespace changes anyway.



---

archive/issue_events_167391.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-21T22:08:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12384",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12384#event-167391"
}
```



---

archive/issue_events_167392.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-21T22:08:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12384",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12384#event-167392"
}
```



---

archive/issue_comments_135318.json:
```json
{
    "body": "Merged: **sage-5.0.beta9**",
    "created_at": "2012-03-21T22:08:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12384",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12384#issuecomment-135318",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta9**

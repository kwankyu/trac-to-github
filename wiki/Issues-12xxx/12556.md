# Issue 12556: optional package database_cremona_ellcurve-20120113.spkg causes test failure

archive/issues_012384.json:
```json
{
    "body": "sage-4.8 without any optional packages passes all tests.\n\nsage-4.8 with the optional package database_cremona_ellcurve-20120113.spkg fails the following test:\n\n```\neno% ./sage -t  --long -force_lib \"devel/sage/doc/en/bordeaux_2008/elliptic_curves.rst\"\nsage -t --long -force_lib \"devel/sage/doc/en/bordeaux_2008/elliptic_curves.rst\"\n  ***   Warning: new stack size = 1098912 (1.048 Mbytes).\n**********************************************************************\nFile \"/home/mariah/sage/sage-4.8-x86_64-Linux-core2-fc-optional/devel/sage/doc/en/bordeaux_2008/elliptic_curves.rst\", line 257:\n    sage: E.gens()            # a few seconds\nExpected:\n    [(1 : 3 : 1), (67/4 : 507/8 : 1)]\nGot:\n    [(-11/9 : 47/27 : 1), (1 : 3 : 1)]\n**********************************************************************\n1 items had failures:\n   1 of   6 in __main__.example_8\n***Test Failed*** 1 failures.\nFor whitespace errors, see the file /home/mariah/.sage//tmp/elliptic_curves_10000.py\n         [18.6 s]\n \n----------------------------------------------------------------------\nThe following tests failed:\n\n\n        sage -t --long -force_lib \"devel/sage/doc/en/bordeaux_2008/elliptic_curves.rst\"\nTotal time for all tests: 18.7 seconds\neno%\n```\n\n\n**Reviewer:** Mariah Lenox\n\nIssue created by migration from https://trac.sagemath.org/ticket/12556\n\n",
    "closed_at": "2012-06-02T13:25:09Z",
    "created_at": "2012-02-21T20:06:19Z",
    "labels": [
        "component: packages: optional",
        "bug",
        "worksforme",
        "invalid"
    ],
    "title": "optional package database_cremona_ellcurve-20120113.spkg causes test failure",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/12556",
    "user": "https://trac.sagemath.org/admin/accounts/users/mariah"
}
```
sage-4.8 without any optional packages passes all tests.

sage-4.8 with the optional package database_cremona_ellcurve-20120113.spkg fails the following test:

```
eno% ./sage -t  --long -force_lib "devel/sage/doc/en/bordeaux_2008/elliptic_curves.rst"
sage -t --long -force_lib "devel/sage/doc/en/bordeaux_2008/elliptic_curves.rst"
  ***   Warning: new stack size = 1098912 (1.048 Mbytes).
**********************************************************************
File "/home/mariah/sage/sage-4.8-x86_64-Linux-core2-fc-optional/devel/sage/doc/en/bordeaux_2008/elliptic_curves.rst", line 257:
    sage: E.gens()            # a few seconds
Expected:
    [(1 : 3 : 1), (67/4 : 507/8 : 1)]
Got:
    [(-11/9 : 47/27 : 1), (1 : 3 : 1)]
**********************************************************************
1 items had failures:
   1 of   6 in __main__.example_8
***Test Failed*** 1 failures.
For whitespace errors, see the file /home/mariah/.sage//tmp/elliptic_curves_10000.py
         [18.6 s]
 
----------------------------------------------------------------------
The following tests failed:


        sage -t --long -force_lib "devel/sage/doc/en/bordeaux_2008/elliptic_curves.rst"
Total time for all tests: 18.7 seconds
eno%
```


**Reviewer:** Mariah Lenox

Issue created by migration from https://trac.sagemath.org/ticket/12556





---

archive/issue_events_104266.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mariah",
    "created_at": "2012-05-23T18:46:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12556",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12556#event-104266"
}
```



---

archive/issue_comments_141387.json:
```json
{
    "body": "<a id='comment:1'></a>\nsage-5.0 and the updated optional package database_cremona_ellcurve-20120302.spkg does not\nhave the reported test failure.\n\nI believe this ticket can be closed.",
    "created_at": "2012-05-23T18:46:15Z",
    "issue": "https://github.com/sagemath/sage/issues/12556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12556#issuecomment-141387",
    "user": "https://trac.sagemath.org/admin/accounts/users/mariah"
}
```

<a id='comment:1'></a>
sage-5.0 and the updated optional package database_cremona_ellcurve-20120302.spkg does not
have the reported test failure.

I believe this ticket can be closed.



---

archive/issue_comments_141388.json:
```json
{
    "body": "**Reviewer:** mariah",
    "created_at": "2012-05-23T18:46:15Z",
    "issue": "https://github.com/sagemath/sage/issues/12556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12556#issuecomment-141388",
    "user": "https://trac.sagemath.org/admin/accounts/users/mariah"
}
```

**Reviewer:** mariah



---

archive/issue_comments_141389.json:
```json
{
    "body": "**Author:** mariah",
    "created_at": "2012-05-23T18:47:06Z",
    "issue": "https://github.com/sagemath/sage/issues/12556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12556#issuecomment-141389",
    "user": "https://trac.sagemath.org/admin/accounts/users/mariah"
}
```

**Author:** mariah



---

archive/issue_events_104267.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mariah",
    "created_at": "2012-05-23T18:47:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12556",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12556#event-104267"
}
```



---

archive/issue_events_104268.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mariah",
    "created_at": "2012-05-23T18:47:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12556",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12556#event-104268"
}
```



---

archive/issue_comments_141390.json:
```json
{
    "body": "**Changing author** from \"mariah\" to \"\".",
    "created_at": "2012-05-23T21:41:27Z",
    "issue": "https://github.com/sagemath/sage/issues/12556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12556#issuecomment-141390",
    "user": "https://github.com/jdemeyer"
}
```

**Changing author** from "mariah" to "".



---

archive/issue_events_104269.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-23T21:41:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12556",
    "milestone": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12556#event-104269"
}
```



---

archive/issue_comments_141391.json:
```json
{
    "body": "**Changing reviewer** from \"mariah\" to \"Mariah Lenox\".",
    "created_at": "2012-05-23T21:41:27Z",
    "issue": "https://github.com/sagemath/sage/issues/12556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12556#issuecomment-141391",
    "user": "https://github.com/jdemeyer"
}
```

**Changing reviewer** from "mariah" to "Mariah Lenox".



---

archive/issue_events_104270.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T13:25:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12556",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12556#event-104270"
}
```



---

archive/issue_events_104271.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T13:25:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12556",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12556#event-104271"
}
```



---

archive/issue_events_104272.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T13:25:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12556",
    "label": "worksforme",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12556#event-104272"
}
```

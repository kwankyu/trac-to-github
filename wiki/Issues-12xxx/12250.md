# Issue 12250: Implementation of weak k-tableaux

archive/issues_012078.json:
```json
{
    "body": "This patch implements a new class of weak k-tableaux. It also removes white spaces from core.py.\n\nThe methods on k-charge were contributed by Nate Gallup and Avi Dalal during Sage Days 49 in Orsay.\n\nDepends on #14519\n\nDepends on #14101\n\nDepends on #7983\n\nDepends on #14772\n\nDepends on #13589\n\nDepends on #14907\n\nDepends on #10630\n\nDepends on #14143\n\nDepends on #14015\n\nDepends on #14516\n\n**Assignee:** sage-combinat\n\n**CC:**  sage-combinat npgallup\n\n**Keywords:** tableaux, days49\n\n**Reviewer:** Mike Zabrocki, Travis Scrimshaw\n\n**Author:** Anne Schilling\n\n**Merged:** sage-5.12.beta5\n\nIssue created by migration from https://trac.sagemath.org/ticket/12250\n\n",
    "closed_at": "2013-09-02T10:19:26Z",
    "created_at": "2012-01-02T23:36:48Z",
    "labels": [
        "component: combinatorics",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.12",
    "title": "Implementation of weak k-tableaux",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/12250",
    "user": "https://github.com/anneschilling"
}
```
This patch implements a new class of weak k-tableaux. It also removes white spaces from core.py.

The methods on k-charge were contributed by Nate Gallup and Avi Dalal during Sage Days 49 in Orsay.

Depends on #14519

Depends on #14101

Depends on #7983

Depends on #14772

Depends on #13589

Depends on #14907

Depends on #10630

Depends on #14143

Depends on #14015

Depends on #14516

**Assignee:** sage-combinat

**CC:**  sage-combinat npgallup

**Keywords:** tableaux, days49

**Reviewer:** Mike Zabrocki, Travis Scrimshaw

**Author:** Anne Schilling

**Merged:** sage-5.12.beta5

Issue created by migration from https://trac.sagemath.org/ticket/12250





---

archive/issue_comments_134801.json:
```json
{
    "body": "**Assignee:** sage-combinat",
    "created_at": "2012-01-02T23:50:18Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134801",
    "user": "https://github.com/anneschilling"
}
```

**Assignee:** sage-combinat



---

archive/issue_events_100515.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2012-01-02T23:50:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "label": "component: combinatorics",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12250#event-100515"
}
```



---

archive/issue_comments_134802.json:
```json
{
    "body": "**Changing keywords** from \"tableaux\" to \"tableaux, days49\".",
    "created_at": "2013-06-19T12:23:20Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134802",
    "user": "https://github.com/anneschilling"
}
```

**Changing keywords** from "tableaux" to "tableaux, days49".



---

archive/issue_comments_134803.json:
```json
{
    "body": "**Changing dependencies** from \"#11742\" to \"\".",
    "created_at": "2013-06-19T12:23:20Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134803",
    "user": "https://github.com/anneschilling"
}
```

**Changing dependencies** from "#11742" to "".



---

archive/issue_comments_134804.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-A new class of weak and strong k-tableaux\n+A new class of weak k-tableaux\n``````\n",
    "created_at": "2013-06-19T12:23:20Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134804",
    "user": "https://github.com/anneschilling"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1 @@
-A new class of weak and strong k-tableaux
+A new class of weak k-tableaux
``````




---

archive/issue_comments_134805.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-A new class of weak k-tableaux\n+This patch implements a new class of weak k-tableaux. It also removes white spaces from core.py.\n``````\n",
    "created_at": "2013-06-19T12:33:31Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134805",
    "user": "https://github.com/anneschilling"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1 @@
-A new class of weak k-tableaux
+This patch implements a new class of weak k-tableaux. It also removes white spaces from core.py.
``````




---

archive/issue_events_100516.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2013-06-19T12:33:31Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "rename": {
        "from": "Implementation of weak and strong k-tableaux",
        "to": "Implementation of weak k-tableaux"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12250#event-100516"
}
```



---

archive/issue_comments_134806.json:
```json
{
    "body": "<a id='comment:4'></a>\nStrong tableaux was moved into ticket #14776",
    "created_at": "2013-06-19T12:36:08Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134806",
    "user": "https://github.com/zabrocki"
}
```

<a id='comment:4'></a>
Strong tableaux was moved into ticket #14776



---

archive/issue_comments_134807.json:
```json
{
    "body": "**Reviewer:** Mike Zabrocki, Anne Schilling",
    "created_at": "2013-07-13T06:52:43Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134807",
    "user": "https://github.com/anneschilling"
}
```

**Reviewer:** Mike Zabrocki, Anne Schilling



---

archive/issue_events_100517.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2013-07-13T06:52:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12250#event-100517"
}
```



---

archive/issue_comments_134808.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n This patch implements a new class of weak k-tableaux. It also removes white spaces from core.py.\n+\n+The methods on k-charge were contributed by Nate Gallup and Avi Dalal during Sage Days 49 in Orsay.\n``````\n",
    "created_at": "2013-07-13T06:54:20Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134808",
    "user": "https://github.com/anneschilling"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 This patch implements a new class of weak k-tableaux. It also removes white spaces from core.py.
+
+The methods on k-charge were contributed by Nate Gallup and Avi Dalal during Sage Days 49 in Orsay.
``````




---

archive/issue_comments_134809.json:
```json
{
    "body": "**Dependencies:** #14519",
    "created_at": "2013-07-13T06:56:22Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134809",
    "user": "https://github.com/anneschilling"
}
```

**Dependencies:** #14519



---

archive/issue_comments_134810.json:
```json
{
    "body": "<a id='comment:9'></a>\nHey Anne and Mike,\n\nThank you both for your work on this. However there are a few things I've noticed while glancing over the patch:\n\n- The inner bullet list on the `INPUT:` block for `WeakTableau` has one too many spaces, so it is overly indented. It needs to line up as follows:\n\n```\n- item 1\n\n  - inner item a\n  - inner item b\n\n- item 2\n```\n- `k_charge_I()` and `k_charge_J()` have the same docstring. In fact, it might be worthwhile to actually describe the algorithm.\n- I think it is worthwhile to have one abstract base class for all `WeakTableau` elements which has it's abstract methods defined. For example, you can also consolidate all of the more detailed explanations of the methods in there and link to them in their concrete classes. In particular, it because an easier/more straightforward to check to see if it's a `WeakTableau` and allow you an easy way to compare `__eq__` between different representations of a weak tableau.\n- In the same vein, could we make it so that we can convert between different representations with parents? I.e. I'd like to do the following:\n\n```\nsage: T = WeakTableaux(3, [5,2,1], [1,1,1,1,1,1])\nsage: T2 = WeakTableaux(3, [3,2,1], [1,1,1,1,1,1], representation='bounded')\nsage: T2(T[0])\n```\n- Could we have a method like `T.representation('bounded')` which returns the corresponding parent using a different representation. Same on the elements (which of course would call the respective `to_*` method, but it would make it consistent IMO)?\n- I don't see an easy way to go to a factorized permutation from the other 2 representations. Right now it seems like I have to call\n\n```\nWeakTableau_factorized_permutation.from_core_tableau().\n```\n- I think the `from_*` (and quite possibly the `to_*`) methods maybe should be available in the parents rather than as classmethods for the element classes.\n- The `.. SEEALSO::` block in `k_charge()` is not formatting correctly, so it doesn't link the methods.\n- Could you use the <code>:arxiv:\\`1234.5678\\`</code> autolink?\n- Remove ``self`` as an input (ex. in `list_of_standard_cells()`)\n\nAck, that ended up being a longer list than I expected...\n\nBest,\n\nTravis",
    "created_at": "2013-07-13T16:14:28Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134810",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:9'></a>
Hey Anne and Mike,

Thank you both for your work on this. However there are a few things I've noticed while glancing over the patch:

- The inner bullet list on the `INPUT:` block for `WeakTableau` has one too many spaces, so it is overly indented. It needs to line up as follows:

```
- item 1

  - inner item a
  - inner item b

- item 2
```
- `k_charge_I()` and `k_charge_J()` have the same docstring. In fact, it might be worthwhile to actually describe the algorithm.
- I think it is worthwhile to have one abstract base class for all `WeakTableau` elements which has it's abstract methods defined. For example, you can also consolidate all of the more detailed explanations of the methods in there and link to them in their concrete classes. In particular, it because an easier/more straightforward to check to see if it's a `WeakTableau` and allow you an easy way to compare `__eq__` between different representations of a weak tableau.
- In the same vein, could we make it so that we can convert between different representations with parents? I.e. I'd like to do the following:

```
sage: T = WeakTableaux(3, [5,2,1], [1,1,1,1,1,1])
sage: T2 = WeakTableaux(3, [3,2,1], [1,1,1,1,1,1], representation='bounded')
sage: T2(T[0])
```
- Could we have a method like `T.representation('bounded')` which returns the corresponding parent using a different representation. Same on the elements (which of course would call the respective `to_*` method, but it would make it consistent IMO)?
- I don't see an easy way to go to a factorized permutation from the other 2 representations. Right now it seems like I have to call

```
WeakTableau_factorized_permutation.from_core_tableau().
```
- I think the `from_*` (and quite possibly the `to_*`) methods maybe should be available in the parents rather than as classmethods for the element classes.
- The `.. SEEALSO::` block in `k_charge()` is not formatting correctly, so it doesn't link the methods.
- Could you use the <code>:arxiv:\`1234.5678\`</code> autolink?
- Remove ``self`` as an input (ex. in `list_of_standard_cells()`)

Ack, that ended up being a longer list than I expected...

Best,

Travis



---

archive/issue_comments_134811.json:
```json
{
    "body": "**Changing author** from \"Anne Schilling, Mike Zabrocki\" to \"Anne Schilling\".",
    "created_at": "2013-07-13T17:06:48Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134811",
    "user": "https://github.com/zabrocki"
}
```

**Changing author** from "Anne Schilling, Mike Zabrocki" to "Anne Schilling".



---

archive/issue_comments_134812.json:
```json
{
    "body": "**Changing reviewer** from \"Mike Zabrocki, Anne Schilling\" to \"Mike Zabrocki\".",
    "created_at": "2013-07-13T17:06:48Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134812",
    "user": "https://github.com/zabrocki"
}
```

**Changing reviewer** from "Mike Zabrocki, Anne Schilling" to "Mike Zabrocki".



---

archive/issue_events_100518.json:
```json
{
    "actor": "https://github.com/zabrocki",
    "created_at": "2013-07-13T17:06:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12250#event-100518"
}
```



---

archive/issue_events_100519.json:
```json
{
    "actor": "https://github.com/zabrocki",
    "created_at": "2013-07-13T17:06:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12250#event-100519"
}
```



---

archive/issue_comments_134813.json:
```json
{
    "body": "<a id='comment:11'></a>\nHi Travis and Mike,\n\nThank you for your review comments. I incorporated them all, except for the input of the inner shape yet and the comments below:\n\n> - In the same vein, could we make it so that we can convert between different representations with parents? I.e. I'd like to do the following:\n> \n> ```\n> sage: T = WeakTableaux(3, [5,2,1], [1,1,1,1,1,1])\n> sage: T2 = WeakTableaux(3, [3,2,1], [1,1,1,1,1,1], representation='bounded')\n> sage: T2(T[0])\n> ```\n\nWe discussed coercing during design discussions at Sage Days 49 and currently decided against it. For now, it is easy to go between the various representations with the representation method. If we really need this feature, we can always add coercion later.\n\n> - I think the `from_*` (and quite possibly the `to_*`) methods maybe should be available in the parents rather than as classmethods for the element classes.\n\nThis is what Nicolas suggested to me, so I will leave it as is.\n\nBest,\n\nAnne",
    "created_at": "2013-07-14T17:58:48Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134813",
    "user": "https://github.com/anneschilling"
}
```

<a id='comment:11'></a>
Hi Travis and Mike,

Thank you for your review comments. I incorporated them all, except for the input of the inner shape yet and the comments below:

> - In the same vein, could we make it so that we can convert between different representations with parents? I.e. I'd like to do the following:
> 
> ```
> sage: T = WeakTableaux(3, [5,2,1], [1,1,1,1,1,1])
> sage: T2 = WeakTableaux(3, [3,2,1], [1,1,1,1,1,1], representation='bounded')
> sage: T2(T[0])
> ```

We discussed coercing during design discussions at Sage Days 49 and currently decided against it. For now, it is easy to go between the various representations with the representation method. If we really need this feature, we can always add coercion later.

> - I think the `from_*` (and quite possibly the `to_*`) methods maybe should be available in the parents rather than as classmethods for the element classes.

This is what Nicolas suggested to me, so I will leave it as is.

Best,

Anne



---

archive/issue_comments_134814.json:
```json
{
    "body": "**Changing reviewer** from \"Mike Zabrocki\" to \"Mike Zabrocki, Travis Scrimshaw\".",
    "created_at": "2013-07-15T16:41:26Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134814",
    "user": "https://github.com/anneschilling"
}
```

**Changing reviewer** from "Mike Zabrocki" to "Mike Zabrocki, Travis Scrimshaw".



---

archive/issue_events_100520.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2013-07-15T16:41:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12250#event-100520"
}
```



---

archive/issue_events_100521.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2013-07-15T16:41:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12250#event-100521"
}
```



---

archive/issue_comments_134815.json:
```json
{
    "body": "<a id='comment:13'></a>\nHi Mike,\n\nThanks for your review patch! I incorporated the changes. The new version now take as input\n\n```\nWeakTableaux(k, shape, weight, representation = 'core')\n```\nwhere shape is either a shape or a tuple of shapes in the skew case.\n\nI hope everything looks ok now!\n\nAnne",
    "created_at": "2013-07-15T16:44:02Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134815",
    "user": "https://github.com/anneschilling"
}
```

<a id='comment:13'></a>
Hi Mike,

Thanks for your review patch! I incorporated the changes. The new version now take as input

```
WeakTableaux(k, shape, weight, representation = 'core')
```
where shape is either a shape or a tuple of shapes in the skew case.

I hope everything looks ok now!

Anne



---

archive/issue_comments_134816.json:
```json
{
    "body": "**Changing dependencies** from \"#14519\" to \"#14519, #14101\".",
    "created_at": "2013-07-16T19:24:36Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134816",
    "user": "https://github.com/anneschilling"
}
```

**Changing dependencies** from "#14519" to "#14519, #14101".



---

archive/issue_comments_134817.json:
```json
{
    "body": "<a id='comment:15'></a>\nHi Mike,\n\nThe updated patch incorporated all changes we discussed (by e-mail). It\n\n- checks that the weight, shape, etc of the element agrees with its parent\n- checks for semistandardness\n- implements latex and pretty printing methods\n- makes some changes Nicolas suggested\n\nThe semistandard checks and pretty printing rely on #14101, so I put this as a dependency.\n\nBest,\n\nAnne",
    "created_at": "2013-07-16T21:39:46Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134817",
    "user": "https://github.com/anneschilling"
}
```

<a id='comment:15'></a>
Hi Mike,

The updated patch incorporated all changes we discussed (by e-mail). It

- checks that the weight, shape, etc of the element agrees with its parent
- checks for semistandardness
- implements latex and pretty printing methods
- makes some changes Nicolas suggested

The semistandard checks and pretty printing rely on #14101, so I put this as a dependency.

Best,

Anne



---

archive/issue_comments_134818.json:
```json
{
    "body": "<a id='comment:16'></a>\nHi Mike,\n\nI just posted a new version which incorporates your two doc review patches and fixes the bug in weights.\n\nAnne",
    "created_at": "2013-07-19T03:27:04Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134818",
    "user": "https://github.com/anneschilling"
}
```

<a id='comment:16'></a>
Hi Mike,

I just posted a new version which incorporates your two doc review patches and fixes the bug in weights.

Anne



---

archive/issue_comments_134819.json:
```json
{
    "body": "<a id='comment:17'></a>\nFixed two more little issues that Mike found (spurious import of Tableau and innter_shape -> inner_shape).\n\nAnne",
    "created_at": "2013-07-20T17:28:27Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134819",
    "user": "https://github.com/anneschilling"
}
```

<a id='comment:17'></a>
Fixed two more little issues that Mike found (spurious import of Tableau and innter_shape -> inner_shape).

Anne



---

archive/issue_events_100522.json:
```json
{
    "actor": "https://github.com/zabrocki",
    "created_at": "2013-07-21T02:37:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12250#event-100522"
}
```



---

archive/issue_events_100523.json:
```json
{
    "actor": "https://github.com/zabrocki",
    "created_at": "2013-07-21T02:37:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12250#event-100523"
}
```



---

archive/issue_comments_134820.json:
```json
{
    "body": "<a id='comment:19'></a>\nI'm happy and all tests pass.",
    "created_at": "2013-07-21T02:38:24Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134820",
    "user": "https://github.com/zabrocki"
}
```

<a id='comment:19'></a>
I'm happy and all tests pass.



---

archive/issue_comments_134821.json:
```json
{
    "body": "<a id='comment:20'></a>\nReplying to [@zabrocki](#comment%3A19):\n> I'm happy and all tests pass.\n\nGreat, thank you for the review! I just made height_of_restriced_subword a private method. Nothing else changed.\n\nAnne",
    "created_at": "2013-07-21T06:04:26Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134821",
    "user": "https://github.com/anneschilling"
}
```

<a id='comment:20'></a>
Replying to [@zabrocki](#comment%3A19):
> I'm happy and all tests pass.

Great, thank you for the review! I just made height_of_restriced_subword a private method. Nothing else changed.

Anne



---

archive/issue_comments_134822.json:
```json
{
    "body": "<a id='comment:21'></a>\nI reaffirm positive review.\n\n-Mike",
    "created_at": "2013-07-21T10:50:09Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134822",
    "user": "https://github.com/zabrocki"
}
```

<a id='comment:21'></a>
I reaffirm positive review.

-Mike



---

archive/issue_events_100524.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-21T21:10:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12250#event-100524"
}
```



---

archive/issue_events_100525.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-21T21:10:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12250#event-100525"
}
```



---

archive/attachments_017006.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_12250-ktableaux-as.patch",
    "asset_url": "tarball://root/attachments/ticket12250/trac_12250-ktableaux-as.patch",
    "created_at": "2013-07-30T21:16:00Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket12250/trac_12250-ktableaux-as.patch",
    "user": "https://github.com/anneschilling"
}
```



---

archive/issue_comments_134823.json:
```json
{
    "body": "**Attachment:** [trac_12250-ktableaux-as.patch](https://github.com/sagemath/sage/files/ticket12250/trac_12250-ktableaux-as.patch)",
    "created_at": "2013-07-30T21:16:00Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134823",
    "user": "https://github.com/anneschilling"
}
```

**Attachment:** [trac_12250-ktableaux-as.patch](https://github.com/sagemath/sage/files/ticket12250/trac_12250-ktableaux-as.patch)



---

archive/issue_comments_134824.json:
```json
{
    "body": "**Changing dependencies** from \"#14519, #14101\" to \"#14519, #14101, #7983, #14772, #13589, #14907, #10630, #14143, #14015, #14516\".",
    "created_at": "2013-08-02T14:27:29Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134824",
    "user": "https://github.com/jdemeyer"
}
```

**Changing dependencies** from "#14519, #14101" to "#14519, #14101, #7983, #14772, #13589, #14907, #10630, #14143, #14015, #14516".



---

archive/issue_events_100526.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-02T14:47:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12250#event-100526"
}
```



---

archive/issue_events_100527.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-02T14:47:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "label": "pending",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12250#event-100527"
}
```



---

archive/issue_events_100528.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2013-08-03T17:06:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12250#event-100528"
}
```



---

archive/issue_events_100529.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2013-08-03T17:06:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "label": "pending",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12250#event-100529"
}
```



---

archive/issue_events_100530.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-09-02T10:19:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12250#event-100530"
}
```



---

archive/issue_events_100531.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-09-02T10:19:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12250#event-100531"
}
```



---

archive/issue_comments_134825.json:
```json
{
    "body": "**Merged:** sage-5.12.beta5",
    "created_at": "2013-09-02T10:19:26Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134825",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.12.beta5



---

archive/issue_comments_134826.json:
```json
{
    "body": "<a id='comment:27'></a>\nSee http://trac.sagemath.org/ticket/15444 for a follow-up on the k-charge implementation.",
    "created_at": "2013-11-24T07:22:00Z",
    "issue": "https://github.com/sagemath/sage/issues/12250",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12250#issuecomment-134826",
    "user": "https://github.com/anneschilling"
}
```

<a id='comment:27'></a>
See http://trac.sagemath.org/ticket/15444 for a follow-up on the k-charge implementation.

# Issue 12582: Implement the coxeter_diagram method for all CartanType's

archive/issues_012410.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-sage-combinat"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nFor crystallographic CartanTypes, this will be done in #6588\n\nComponent: **combinatorics**\n\nKeywords: **coxeter**\n\nReviewer: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12582_\n\n",
    "closed_at": "2014-03-11T14:04:45Z",
    "created_at": "2012-02-24T23:14:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement the coxeter_diagram method for all CartanType's",
    "type": "issue",
    "updated_at": "2014-03-11T14:04:45Z",
    "url": "https://github.com/sagemath/sage/issues/12582",
    "user": "https://github.com/nthiery"
}
```
<div id="comment:0"></div>

For crystallographic CartanTypes, this will be done in #6588

Component: **combinatorics**

Keywords: **coxeter**

Reviewer: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/12582_





---

archive/issue_events_170989.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2012-02-24T23:14:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12582#event-170989"
}
```



---

archive/issue_events_170990.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2012-02-24T23:14:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12582#event-170990"
}
```



---

archive/issue_events_170991.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2012-02-24T23:14:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12582#event-170991"
}
```



---

archive/issue_events_170992.json:
```json
{
    "actor": "https://github.com/sagetrac-sage-combinat",
    "created_at": "2012-02-24T23:14:32Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "subject": "https://github.com/nthiery",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12582#event-170992"
}
```



---

archive/issue_comments_139676.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+For crystallographic CartanTypes, this will be done in #6588\n``````\n",
    "created_at": "2012-02-25T00:36:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12582#issuecomment-139676",
    "user": "https://github.com/nthiery"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+For crystallographic CartanTypes, this will be done in #6588
``````




---

archive/issue_events_170993.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2012-02-25T00:36:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12582#event-170993"
}
```



---

archive/issue_events_170994.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2012-02-25T00:36:57Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "title_is": "Implement the coxeter_diagram method for all CartanType's",
    "title_was": "Implement the coxeter_diagram method, at least for crystallographic cartan types.",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12582#event-170994"
}
```



---

archive/issue_comments_139677.json:
```json
{
    "body": "Changed keywords from none to **coxeter**",
    "created_at": "2012-11-19T17:00:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12582#issuecomment-139677",
    "user": "https://github.com/fchapoton"
}
```

Changed keywords from none to **coxeter**



---

archive/issue_comments_139678.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nDoes this just mean defining coxeter_diagram in the files type_H.py and friends ?",
    "created_at": "2012-11-20T08:32:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12582#issuecomment-139678",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">comment:3</div>

Does this just mean defining coxeter_diagram in the files type_H.py and friends ?



---

archive/issue_comments_139679.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nReplying to [@fchapoton](#comment%3A3):\n> Does this just mean defining coxeter_diagram in the files type_H.py and friends ?\n\nI forgot actually, but yes, I guess so!\n\nWe will also want a CoxeterDiagram to be a valid cartan type as well (I don't know what's the status there), but that might be for another ticket.",
    "created_at": "2012-11-22T08:33:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12582#issuecomment-139679",
    "user": "https://github.com/nthiery"
}
```

<div id="comment:4" align="right">comment:4</div>

Replying to [@fchapoton](#comment%3A3):
> Does this just mean defining coxeter_diagram in the files type_H.py and friends ?

I forgot actually, but yes, I guess so!

We will also want a CoxeterDiagram to be a valid cartan type as well (I don't know what's the status there), but that might be for another ticket.



---

archive/issue_comments_139680.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nIn sage 6.2.beta3:\n\n```\nsage: H3 = CartanType(['H',3])\nsage: H3.coxeter_diagram()\nGraph on 3 vertices\nsage: I7 = CartanType(['I',7])\nsage: I7.coxeter_diagram()\nGraph on 2 vertices\n```\nSo maybe this ticket could be closed ?",
    "created_at": "2014-03-06T15:01:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12582#issuecomment-139680",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:5" align="right">comment:5</div>

In sage 6.2.beta3:

```
sage: H3 = CartanType(['H',3])
sage: H3.coxeter_diagram()
Graph on 3 vertices
sage: I7 = CartanType(['I',7])
sage: I7.coxeter_diagram()
Graph on 2 vertices
```
So maybe this ticket could be closed ?



---

archive/issue_events_170995.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-03-06T15:01:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12582#event-170995"
}
```



---

archive/issue_events_170996.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-03-06T15:01:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12582#event-170996"
}
```



---

archive/issue_events_170997.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-03-06T18:03:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12582#event-170997"
}
```



---

archive/issue_events_170998.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-03-06T18:03:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12582#event-170998"
}
```



---

archive/issue_comments_139681.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2014-03-06T18:03:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12582#issuecomment-139681",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_170999.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-11T14:04:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12582#event-170999"
}
```



---

archive/issue_events_171000.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-11T14:04:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12582",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12582#event-171000"
}
```

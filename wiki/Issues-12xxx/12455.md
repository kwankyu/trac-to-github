# Issue 12455: Make Airy functions symbolic

archive/issues_012283.json:
```json
{
    "assignees": [
        "https://github.com/rwst"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nAs discussed in [sage-support](http://groups.google.com/group/sage-support/browse_thread/thread/f458d0f9cfd89c9a).\n\nCurrently sage can evaluate airy functions numerically:\n\n```\nsage: airy_ai(1.4)\n0.0820380498076\n```\n\nbut it doesn't work symbolically\n\n```\nsage: airy_ai(x)\n...\nTypeError: Cannot evaluate symbolic expression to a numeric value.\n```\n\nWe should make it symbolical for both airy_ai and airy_bi, as well as their derivatives.\n\nDepends on #12289\nDepends on #17130\n\nCC:  @kcrisman @burcin @benjaminfjones @fredrik-johansson @eviatarbach\n\nComponent: **symbolics**\n\nKeywords: **Airy functions sd40.5 sd48**\n\nAuthor: **Oscar Gerardo Lazo Arjona, Benjamin Jones, Douglas McNeil, Eviatar Bach, Ralf Stephan**\n\nBranch: **[`2f6945a`](https://github.com/sagemath/sagetrac-mirror/commit/2f6945af73219458fb0a2745e2df9e2eea79b055)**\n\nReviewer: **Eviatar Bach, Karl-Dieter Crisman, Burcin Erocal, Ralf Stephan, Jeroen Demeyer, Marc Mezzarobba**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12455_\n\n",
    "closed_at": "2015-03-12T23:06:43Z",
    "created_at": "2012-02-07T04:47:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make Airy functions symbolic",
    "type": "issue",
    "updated_at": "2015-05-27T12:06:06Z",
    "url": "https://github.com/sagemath/sage/issues/12455",
    "user": "https://github.com/sagetrac-olazo"
}
```
<div id="comment:0"></div>

As discussed in [sage-support](http://groups.google.com/group/sage-support/browse_thread/thread/f458d0f9cfd89c9a).

Currently sage can evaluate airy functions numerically:

```
sage: airy_ai(1.4)
0.0820380498076
```

but it doesn't work symbolically

```
sage: airy_ai(x)
...
TypeError: Cannot evaluate symbolic expression to a numeric value.
```

We should make it symbolical for both airy_ai and airy_bi, as well as their derivatives.

Depends on #12289
Depends on #17130

CC:  @kcrisman @burcin @benjaminfjones @fredrik-johansson @eviatarbach

Component: **symbolics**

Keywords: **Airy functions sd40.5 sd48**

Author: **Oscar Gerardo Lazo Arjona, Benjamin Jones, Douglas McNeil, Eviatar Bach, Ralf Stephan**

Branch: **[`2f6945a`](https://github.com/sagemath/sagetrac-mirror/commit/2f6945af73219458fb0a2745e2df9e2eea79b055)**

Reviewer: **Eviatar Bach, Karl-Dieter Crisman, Burcin Erocal, Ralf Stephan, Jeroen Demeyer, Marc Mezzarobba**

_Issue created by migration from https://trac.sagemath.org/ticket/12455_





---

archive/issue_events_168754.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2012-02-07T04:47:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168754"
}
```



---

archive/issue_events_168755.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2012-02-07T04:47:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168755"
}
```



---

archive/issue_events_168756.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2012-02-07T04:47:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168756"
}
```



---

archive/issue_events_168757.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-02-07T04:47:41Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "subject": "https://github.com/sagetrac-olazo",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168757"
}
```



---

archive/issue_events_168758.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2012-02-07T04:47:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "c: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168758"
}
```



---

archive/issue_events_168759.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-02-07T04:48:52Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "subject": "https://github.com/sagetrac-olazo",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168759"
}
```



---

archive/issue_events_168760.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2012-02-07T04:48:52Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "subject": "https://github.com/sagetrac-olazo",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168760"
}
```



---

archive/issue_comments_137445.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n+As discussed in [sage-support](http://groups.google.com/group/sage-support/browse_thread/thread/f458d0f9cfd89c9a).\n+\n Currently sage can evaluate airy functions numerically:\n \n ```\n``````\n",
    "created_at": "2012-02-07T04:52:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137445",
    "user": "https://github.com/sagetrac-olazo"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
+As discussed in [sage-support](http://groups.google.com/group/sage-support/browse_thread/thread/f458d0f9cfd89c9a).
+
 Currently sage can evaluate airy functions numerically:
 
 ```
``````




---

archive/issue_comments_137446.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nAttachment: **[trac_12455-symbolic_airy.patch.gz](https://github.com/sagemath/sage/files/ticket12455/trac_12455-symbolic_airy.patch.gz)**\n\nI've added a patch, which should do the job, but it has a few shortcomings:\n\n1.-The resulting symbolic functions seem to remain on hold:\n\n```\nsage: airy_ai(1.0)\nairy_ai(1.00000000000000)\n```\nYou need to force it to evaluate:\n\n```\nsage: airy_ai(1.0).n()\n0.135292416313\n```\n\n2.- This doesn't work:\n\n```\nsage: airy_ai(2.0).n(digits=100)\n0.0349241304233\n```\n3.- There is no evaluation for airy_ai_prime or airy_bi_prime\n\n4.- I'm not sure about how should the functions be called, some possible schemes are\n\n{ai,bi,aip,bip}\n{ai,bai,aip,baip}\n{airy_ai,airy_bi,airy_ai_prime,airy_bi_prime}\n\nAnd also whether the latex representation should be capitalized or not. I chose the third scheme, and capitalized typesetting.",
    "created_at": "2012-02-07T08:05:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137446",
    "user": "https://github.com/sagetrac-olazo"
}
```

<div id="comment:4" align="right">comment:4</div>

Attachment: **[trac_12455-symbolic_airy.patch.gz](https://github.com/sagemath/sage/files/ticket12455/trac_12455-symbolic_airy.patch.gz)**

I've added a patch, which should do the job, but it has a few shortcomings:

1.-The resulting symbolic functions seem to remain on hold:

```
sage: airy_ai(1.0)
airy_ai(1.00000000000000)
```
You need to force it to evaluate:

```
sage: airy_ai(1.0).n()
0.135292416313
```

2.- This doesn't work:

```
sage: airy_ai(2.0).n(digits=100)
0.0349241304233
```
3.- There is no evaluation for airy_ai_prime or airy_bi_prime

4.- I'm not sure about how should the functions be called, some possible schemes are

{ai,bi,aip,bip}
{ai,bai,aip,baip}
{airy_ai,airy_bi,airy_ai_prime,airy_bi_prime}

And also whether the latex representation should be capitalized or not. I chose the third scheme, and capitalized typesetting.



---

archive/issue_events_168761.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2012-02-07T08:06:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168761"
}
```



---

archive/issue_comments_137447.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nGood start.  A few responses.\n* You'll need to have more methods.  We should probably use [mpmath](http://mpmath.googlecode.com/svn/trunk/doc/build/functions/bessel.html#airy-and-scorer-functions) to evaluate these functions in general, though using [the SciPy version](http://docs.scipy.org/doc/scipy/reference/generated/scipy.special.airy.html) could be useful over RDF.  See #11888 for a very recent example of this kind of thing and how to implement it.\n* That should also fix precision issues, if properly done, I think.\n* SciPy has evaluation for the airy primes.  I think that Maxima has these too.  (Cc:ing FJ in case he has plans to add these to mpmath.)\n* We should have translations of these to Maxima and friends.  That will probably help us name them.   What do Mathematica and Maple call them?\n* All methods, including initialization methods, need doctests.  Many of the new symbolic functions added at [symbolics](../wiki/symbolics)/functions will have nice models to follow.\n\nThanks for getting us a start on this!   That's great.",
    "created_at": "2012-02-07T13:59:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137447",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:6" align="right">comment:6</div>

Good start.  A few responses.
* You'll need to have more methods.  We should probably use [mpmath](http://mpmath.googlecode.com/svn/trunk/doc/build/functions/bessel.html#airy-and-scorer-functions) to evaluate these functions in general, though using [the SciPy version](http://docs.scipy.org/doc/scipy/reference/generated/scipy.special.airy.html) could be useful over RDF.  See #11888 for a very recent example of this kind of thing and how to implement it.
* That should also fix precision issues, if properly done, I think.
* SciPy has evaluation for the airy primes.  I think that Maxima has these too.  (Cc:ing FJ in case he has plans to add these to mpmath.)
* We should have translations of these to Maxima and friends.  That will probably help us name them.   What do Mathematica and Maple call them?
* All methods, including initialization methods, need doctests.  Many of the new symbolic functions added at [symbolics](../wiki/symbolics)/functions will have nice models to follow.

Thanks for getting us a start on this!   That's great.



---

archive/issue_events_168762.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-02-07T13:59:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168762"
}
```



---

archive/issue_events_168763.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-02-07T13:59:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168763"
}
```



---

archive/issue_events_168764.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-02-07T13:59:30Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "title_is": "Make Airy functions symbolic",
    "title_was": "Make Airy functions symbolical",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168764"
}
```



---

archive/issue_comments_137448.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nmpmath has the derivatives (as well as integrals) -- just use the optional 'derivative' parameter.",
    "created_at": "2012-02-07T14:03:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137448",
    "user": "https://github.com/fredrik-johansson"
}
```

<div id="comment:7" align="right">comment:7</div>

mpmath has the derivatives (as well as integrals) -- just use the optional 'derivative' parameter.



---

archive/issue_comments_137449.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\n> mpmath has the derivatives (as well as integrals) -- just use the optional 'derivative' parameter.\n\nOk, I didn't realize that was what that parameter was about.  Though in retrospect it seems obvious!",
    "created_at": "2012-02-07T14:05:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137449",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:8" align="right">comment:8</div>

> mpmath has the derivatives (as well as integrals) -- just use the optional 'derivative' parameter.

Ok, I didn't realize that was what that parameter was about.  Though in retrospect it seems obvious!



---

archive/issue_comments_137450.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nOn second thought, I think it would be better to use the airy equation to calculate derivatives or order higher than 1. Like\n\n```\nsage: airy_ai(2,x)\nx*airy_ai(x)\nsage: airy_ai(3,x)\nairy_ai(x)+x*airy_ai_prime(x)\nsage: diff(airy_ai(x),x,2)\nx*airy_ai(x)\nsage: diff(airy_ai(x),x,3)\nairy_ai(x)+x*airy_ai_prime(x)\n```\n\nwhich is very likey to be the way mpmath calculates higher order derivatives. Integrals however, would be returned as:\n\n```\nsage: airy_ai(-1,x)\nairy_ai(-1,x)\nsage: integral(airy_ai(x),x)\nairy_ai(-1,x)\n```\n\nwhat do you think?",
    "created_at": "2012-02-07T22:14:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137450",
    "user": "https://github.com/sagetrac-olazo"
}
```

<div id="comment:9" align="right">comment:9</div>

On second thought, I think it would be better to use the airy equation to calculate derivatives or order higher than 1. Like

```
sage: airy_ai(2,x)
x*airy_ai(x)
sage: airy_ai(3,x)
airy_ai(x)+x*airy_ai_prime(x)
sage: diff(airy_ai(x),x,2)
x*airy_ai(x)
sage: diff(airy_ai(x),x,3)
airy_ai(x)+x*airy_ai_prime(x)
```

which is very likey to be the way mpmath calculates higher order derivatives. Integrals however, would be returned as:

```
sage: airy_ai(-1,x)
airy_ai(-1,x)
sage: integral(airy_ai(x),x)
airy_ai(-1,x)
```

what do you think?



---

archive/issue_comments_137451.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nAttachment: **[trac_12455-symbolic_airy2.patch.gz](https://github.com/sagemath/sage/files/ticket12455/trac_12455-symbolic_airy2.patch.gz)**\n\nI just added a new patch. The new version includes generalized derivatives, evaluation with mpmath, and special values of the functions and their derivatives. Just a one doubt, the coverage is:\n\n```\noscar@oscar-netbook:~$ sage -coverage airy.py\nSCORE /home/oscar/sage/my_patches/airy.py: 76% (20 of 26)\n\nMissing documentation:\n\t * __init__(self):\n\t * __init__(self):\n\t * __init__(self):\n\t * __init__(self):\n\t * __init__(self):\n\t * __init__(self):\n\n\nPossibly wrong (function name doesn't occur in doctests):\n\t * _derivative_(self, x, diff_param=None):\n\t * _eval_(self, x):\n\t * _evalf_(self, x, parent=None):\n\t * _derivative_(self, x, diff_param=None):\n\t * _eval_(self, x):\n\t * _evalf_(self, x, parent=None):\n\t * _derivative_(self, alpha, *args, **kwds):\n\t * _eval_(self, alpha, *args):\n\t * _evalf_(self, alpha, x, parent=None):\n\t * _derivative_(self, x, diff_param=None):\n\t * _eval_(self, x):\n\t * _evalf_(self, x, parent=None):\n\t * _derivative_(self, x, diff_param=None):\n\t * _eval_(self, x):\n\t * _evalf_(self, x, parent=None):\n\t * _derivative_(self, alpha, *args, **kwds):\n\t * _eval_(self, alpha, *args):\n\t * _evalf_(self, alpha, x, parent=None):\n```\n\nIs that important?",
    "created_at": "2012-02-19T01:01:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137451",
    "user": "https://github.com/sagetrac-olazo"
}
```

<div id="comment:10" align="right">comment:10</div>

Attachment: **[trac_12455-symbolic_airy2.patch.gz](https://github.com/sagemath/sage/files/ticket12455/trac_12455-symbolic_airy2.patch.gz)**

I just added a new patch. The new version includes generalized derivatives, evaluation with mpmath, and special values of the functions and their derivatives. Just a one doubt, the coverage is:

```
oscar@oscar-netbook:~$ sage -coverage airy.py
SCORE /home/oscar/sage/my_patches/airy.py: 76% (20 of 26)

Missing documentation:
	 * __init__(self):
	 * __init__(self):
	 * __init__(self):
	 * __init__(self):
	 * __init__(self):
	 * __init__(self):


Possibly wrong (function name doesn't occur in doctests):
	 * _derivative_(self, x, diff_param=None):
	 * _eval_(self, x):
	 * _evalf_(self, x, parent=None):
	 * _derivative_(self, x, diff_param=None):
	 * _eval_(self, x):
	 * _evalf_(self, x, parent=None):
	 * _derivative_(self, alpha, *args, **kwds):
	 * _eval_(self, alpha, *args):
	 * _evalf_(self, alpha, x, parent=None):
	 * _derivative_(self, x, diff_param=None):
	 * _eval_(self, x):
	 * _evalf_(self, x, parent=None):
	 * _derivative_(self, x, diff_param=None):
	 * _eval_(self, x):
	 * _evalf_(self, x, parent=None):
	 * _derivative_(self, alpha, *args, **kwds):
	 * _eval_(self, alpha, *args):
	 * _evalf_(self, alpha, x, parent=None):
```

Is that important?



---

archive/issue_events_168765.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2012-02-19T01:01:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168765"
}
```



---

archive/issue_events_168766.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2012-02-19T01:01:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168766"
}
```



---

archive/issue_comments_137452.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\n> Is that important?\n\nThe second part isn't, as you are implicitly testing these 'hidden' functions.  By the way,\n\n```\nExamples::\n```\nshould be \n\n```\nEXAMPLES::\n```\nHowever, the first part (the initialization methods) is important for our coverage.   You can just do a couple from the big examples that you have.\n\nDo these pass doctests?  I'm surprised that \n\n```\nsage: airy_ai_simple= FunctionAiryAiSimple()\n```\nwould work when you run tests, assuming you didn't import `FunctionAiryAiSimple` into the global namespace.\n\nAnyway, overall on a quick glance this looks great; unfortunately, I won't have time to review it properly soon - hopefully someone else will, because you put a lot of great work into it and we should totally have these symbolic.  Thanks!",
    "created_at": "2012-02-19T01:12:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137452",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:11" align="right">comment:11</div>

> Is that important?

The second part isn't, as you are implicitly testing these 'hidden' functions.  By the way,

```
Examples::
```
should be 

```
EXAMPLES::
```
However, the first part (the initialization methods) is important for our coverage.   You can just do a couple from the big examples that you have.

Do these pass doctests?  I'm surprised that 

```
sage: airy_ai_simple= FunctionAiryAiSimple()
```
would work when you run tests, assuming you didn't import `FunctionAiryAiSimple` into the global namespace.

Anyway, overall on a quick glance this looks great; unfortunately, I won't have time to review it properly soon - hopefully someone else will, because you put a lot of great work into it and we should totally have these symbolic.  Thanks!



---

archive/issue_comments_137453.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\n> Do these pass doctests?  I'm surprised that \n> \n> ```\n> sage: airy_ai_simple= FunctionAiryAiSimple()\n> ```\n> would work when you run tests, assuming you didn't import `FunctionAiryAiSimple` into the global namespace.\n\nIt looks like they do:\n\n```\noscar@oscar-netbook:~$ sage -coverage airy.py\nsage -t  \"/home/oscar/sage/my_patches/airy.py\"              \n\t [51.5 s]\n \n----------------------------------------------------------------------\nAll tests passed!\nTotal time for all tests: 51.7 seconds\n```\nWhy shouldn't they? I'll take care of the `__init__` functions.",
    "created_at": "2012-02-19T02:46:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137453",
    "user": "https://github.com/sagetrac-olazo"
}
```

<div id="comment:12" align="right">comment:12</div>

> Do these pass doctests?  I'm surprised that 
> 
> ```
> sage: airy_ai_simple= FunctionAiryAiSimple()
> ```
> would work when you run tests, assuming you didn't import `FunctionAiryAiSimple` into the global namespace.

It looks like they do:

```
oscar@oscar-netbook:~$ sage -coverage airy.py
sage -t  "/home/oscar/sage/my_patches/airy.py"              
	 [51.5 s]
 
----------------------------------------------------------------------
All tests passed!
Total time for all tests: 51.7 seconds
```
Why shouldn't they? I'll take care of the `__init__` functions.



---

archive/issue_comments_137454.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nI'm sorry, that should be:\n\n```\noscar@oscar-netbook:~$ sage -t airy.py\nsage -t  \"/home/oscar/sage/my_patches/airy.py\"              \n\t [51.5 s]\n \n----------------------------------------------------------------------\nAll tests passed!\nTotal time for all tests: 51.7 seconds\n```\n(the command was sage -t)",
    "created_at": "2012-02-19T02:49:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137454",
    "user": "https://github.com/sagetrac-olazo"
}
```

<div id="comment:13" align="right">comment:13</div>

I'm sorry, that should be:

```
oscar@oscar-netbook:~$ sage -t airy.py
sage -t  "/home/oscar/sage/my_patches/airy.py"              
	 [51.5 s]
 
----------------------------------------------------------------------
All tests passed!
Total time for all tests: 51.7 seconds
```
(the command was sage -t)



---

archive/issue_events_168767.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-02-19T03:19:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168767"
}
```



---

archive/issue_events_168768.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-02-19T03:19:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168768"
}
```



---

archive/issue_comments_137455.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\n\n```\nsage: airy_ai_simple= FunctionAiryAiSimple()\n```\n> would work when you run tests, assuming you didn't import `FunctionAiryAiSimple` into the global namespace.\n\nI'm just surprised this doesn't cause trouble.  \n\nOn another note, this apparently does something weird when applied to 5.0.beta4.   Namely, Sage won't start:\n\n```\n---> 65 from sage.functions.all import sin, cos\n\nImportError: cannot import name sin\nError importing ipy_profile_sage - perhaps you should run %upgrade?\nWARNING: Loading of ipy_profile_sage failed.\n```\nI'll spare you the rest of the traceback.  I think there is some kind of circular import thing going on here, but I don't understand imports that well.  Perhaps moving the import of airy to further down (after trig functions) would help - that's a totally uninformed guess, of course.",
    "created_at": "2012-02-19T03:19:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137455",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:14" align="right">comment:14</div>


```
sage: airy_ai_simple= FunctionAiryAiSimple()
```
> would work when you run tests, assuming you didn't import `FunctionAiryAiSimple` into the global namespace.

I'm just surprised this doesn't cause trouble.  

On another note, this apparently does something weird when applied to 5.0.beta4.   Namely, Sage won't start:

```
---> 65 from sage.functions.all import sin, cos

ImportError: cannot import name sin
Error importing ipy_profile_sage - perhaps you should run %upgrade?
WARNING: Loading of ipy_profile_sage failed.
```
I'll spare you the rest of the traceback.  I think there is some kind of circular import thing going on here, but I don't understand imports that well.  Perhaps moving the import of airy to further down (after trig functions) would help - that's a totally uninformed guess, of course.



---

archive/issue_comments_137456.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nAttachment: **[trac_12455-symbolic_airy3.patch.gz](https://github.com/sagemath/sage/files/ticket12455/trac_12455-symbolic_airy3.patch.gz)**\n\nI just added a new patch, now with 100 % test coverage, but I still get this message:\n\n```\noscar@oscar-netbook:~$ sage -coverage /home/oscar/sage/my_patches/airy.py ----------------------------------------------------------------------\n/home/oscar/sage/my_patches/airy.py\nERROR: Please add a `TestSuite(s).run()` doctest.\nSCORE /home/oscar/sage/my_patches/airy.py: 100% (26 of 26)\n----------------------------------------------------------------------\n```\n\nWhat is this TestSuite thing? I also changed the order in which functions are initialized, so that airy_ai_general is initialized first. Doing it last made some symbolic operations, such as simplify not work (It said something about airy_ai requiring 2 arguments).",
    "created_at": "2012-02-21T18:44:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137456",
    "user": "https://github.com/sagetrac-olazo"
}
```

<div id="comment:15" align="right">comment:15</div>

Attachment: **[trac_12455-symbolic_airy3.patch.gz](https://github.com/sagemath/sage/files/ticket12455/trac_12455-symbolic_airy3.patch.gz)**

I just added a new patch, now with 100 % test coverage, but I still get this message:

```
oscar@oscar-netbook:~$ sage -coverage /home/oscar/sage/my_patches/airy.py ----------------------------------------------------------------------
/home/oscar/sage/my_patches/airy.py
ERROR: Please add a `TestSuite(s).run()` doctest.
SCORE /home/oscar/sage/my_patches/airy.py: 100% (26 of 26)
----------------------------------------------------------------------
```

What is this TestSuite thing? I also changed the order in which functions are initialized, so that airy_ai_general is initialized first. Doing it last made some symbolic operations, such as simplify not work (It said something about airy_ai requiring 2 arguments).



---

archive/issue_comments_137457.json:
```json
{
    "body": "Author: **Oscar Gerardo Lazo Arjona**",
    "created_at": "2012-02-21T18:44:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137457",
    "user": "https://github.com/sagetrac-olazo"
}
```

Author: **Oscar Gerardo Lazo Arjona**



---

archive/issue_events_168769.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2012-02-21T18:44:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168769"
}
```



---

archive/issue_events_168770.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2012-02-21T18:44:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168770"
}
```



---

archive/issue_comments_137458.json:
```json
{
    "body": "Reviewer: **Benjamin Jones**",
    "created_at": "2012-02-21T19:03:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137458",
    "user": "https://github.com/benjaminfjones"
}
```

Reviewer: **Benjamin Jones**



---

archive/issue_comments_137459.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">comment:16</div>\n\nHi Oscar,\nI haven't looked over your patch very closely yet, but I plan to. One comment on first glance: in several places you call\n\n```\nreturn mpmath_utils.call(airy_bi_mpmath, x, derivative=1, parent=RR) \n```\n\nbut I think we want to preserve the parent of `x` instead of forcing it to be `RR`. Importing the `parent` function from `sage.structure.coerce` using a different name like `sage_parent` (because as burcin pointed out in one of my tickets, parent is the name of the parameter in `_evalf_`) and doing something like\n\n```\nfrom sage.structure.coerce import parent as sage_parent\nR = parent or sage_parent(x)\nreturn mpmath_utils.call(airy_bi_mpmath, x, derivative=1, parent=R)\n```\n\nwould do the trick I think. \n\nOther comment: can we add conversions to Maxima along with the Mma conversion you included? Here is a link to the appropriate chapter in the manual: http://maxima.sourceforge.net/docs/manual/en/maxima_15.html#SEC80",
    "created_at": "2012-02-21T19:03:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137459",
    "user": "https://github.com/benjaminfjones"
}
```

<div id="comment:16" align="right">comment:16</div>

Hi Oscar,
I haven't looked over your patch very closely yet, but I plan to. One comment on first glance: in several places you call

```
return mpmath_utils.call(airy_bi_mpmath, x, derivative=1, parent=RR) 
```

but I think we want to preserve the parent of `x` instead of forcing it to be `RR`. Importing the `parent` function from `sage.structure.coerce` using a different name like `sage_parent` (because as burcin pointed out in one of my tickets, parent is the name of the parameter in `_evalf_`) and doing something like

```
from sage.structure.coerce import parent as sage_parent
R = parent or sage_parent(x)
return mpmath_utils.call(airy_bi_mpmath, x, derivative=1, parent=R)
```

would do the trick I think. 

Other comment: can we add conversions to Maxima along with the Mma conversion you included? Here is a link to the appropriate chapter in the manual: http://maxima.sourceforge.net/docs/manual/en/maxima_15.html#SEC80



---

archive/issue_comments_137460.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">comment:17</div>\n\nThe patch applies to 5.0.beta6 cleanly, but upon `sage -br` I get an import error ending with:\n\n```\n/home/jonesbe/sage/sage-5.0.beta6/local/lib/python2.7/site-packages/sage/gsl/dft.py in <module>()\n     63 from sage.rings.real_mpfr import RR\n     64 from sage.rings.all import I\n---> 65 from sage.functions.all import sin, cos\n     66 from sage.gsl.fft import FastFourierTransform\n     67 from sage.gsl.dwt import WaveletTransform\n\nImportError: cannot import name sin\nError importing ipy_profile_sage - perhaps you should run %upgrade?\nWARNING: Loading of ipy_profile_sage failed.\n```\n\nSeems like the same thing kcrisman ran into with the previous patch applied to 5.0.beta4.",
    "created_at": "2012-02-29T02:33:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137460",
    "user": "https://github.com/benjaminfjones"
}
```

<div id="comment:17" align="right">comment:17</div>

The patch applies to 5.0.beta6 cleanly, but upon `sage -br` I get an import error ending with:

```
/home/jonesbe/sage/sage-5.0.beta6/local/lib/python2.7/site-packages/sage/gsl/dft.py in <module>()
     63 from sage.rings.real_mpfr import RR
     64 from sage.rings.all import I
---> 65 from sage.functions.all import sin, cos
     66 from sage.gsl.fft import FastFourierTransform
     67 from sage.gsl.dwt import WaveletTransform

ImportError: cannot import name sin
Error importing ipy_profile_sage - perhaps you should run %upgrade?
WARNING: Loading of ipy_profile_sage failed.
```

Seems like the same thing kcrisman ran into with the previous patch applied to 5.0.beta4.



---

archive/issue_events_168771.json:
```json
{
    "actor": "https://github.com/benjaminfjones",
    "created_at": "2012-02-29T02:33:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168771"
}
```



---

archive/issue_events_168772.json:
```json
{
    "actor": "https://github.com/benjaminfjones",
    "created_at": "2012-02-29T02:33:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168772"
}
```



---

archive/issue_comments_137461.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">comment:18</div>\n\nI found the cause of the problem in comment [comment:17]. There is a circular import going on because\n\n```\nfrom airy import airy_ai, airy_bi\n```\noccurs at the top of `sage/functions/all.py`. If you move it to the bottom of the file, sage starts just fine.\n\nThere a several doctests that fail, however. Oscar, I think that running `sage -coverage` only checks to see if there are doctest for every function, it doesn't actually run the tests. Use\n\n```\nsage -t devel/sage/sage/functions/airy.py\n```\n\nto run the tests and get feedback.",
    "created_at": "2012-03-24T16:38:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137461",
    "user": "https://github.com/benjaminfjones"
}
```

<div id="comment:18" align="right">comment:18</div>

I found the cause of the problem in comment [comment:17]. There is a circular import going on because

```
from airy import airy_ai, airy_bi
```
occurs at the top of `sage/functions/all.py`. If you move it to the bottom of the file, sage starts just fine.

There a several doctests that fail, however. Oscar, I think that running `sage -coverage` only checks to see if there are doctest for every function, it doesn't actually run the tests. Use

```
sage -t devel/sage/sage/functions/airy.py
```

to run the tests and get feedback.



---

archive/issue_comments_137462.json:
```json
{
    "body": "Work Issues: **circular import, doctest failures**",
    "created_at": "2012-03-24T16:38:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137462",
    "user": "https://github.com/benjaminfjones"
}
```

Work Issues: **circular import, doctest failures**



---

archive/issue_comments_137463.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">comment:19</div>\n\nHere at SD 40.5, we're going to pick this ticket up, do the work, and get it into sage.",
    "created_at": "2012-05-26T17:45:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137463",
    "user": "https://github.com/benjaminfjones"
}
```

<div id="comment:19" align="right">comment:19</div>

Here at SD 40.5, we're going to pick this ticket up, do the work, and get it into sage.



---

archive/issue_comments_137464.json:
```json
{
    "body": "Changed reviewer from **Benjamin Jones** to none",
    "created_at": "2012-05-26T17:45:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137464",
    "user": "https://github.com/benjaminfjones"
}
```

Changed reviewer from **Benjamin Jones** to none



---

archive/issue_comments_137465.json:
```json
{
    "body": "Changed author from **Oscar Gerardo Lazo Arjona** to **Oscar Gerardo Lazo Arjona, Benjamin Jones**",
    "created_at": "2012-05-26T17:45:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137465",
    "user": "https://github.com/benjaminfjones"
}
```

Changed author from **Oscar Gerardo Lazo Arjona** to **Oscar Gerardo Lazo Arjona, Benjamin Jones**



---

archive/issue_comments_137466.json:
```json
{
    "body": "Changed keywords from **Airy functions** to **Airy functions sd40.5**",
    "created_at": "2012-05-26T17:57:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137466",
    "user": "https://github.com/benjaminfjones"
}
```

Changed keywords from **Airy functions** to **Airy functions sd40.5**



---

archive/issue_comments_137467.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">comment:21</div>\n\nOkay, I'm posting this for future reference.  I've attempted to preserve as much as possible of the original code while switching to the \"new-style\" dictionary-argument .n() approach we're working on in #12289, but I'm setting it to \"needs work\" myself while we think through some of the consequences of the switch.  Some of the `_evalf_` stuff violates DRY and I'm not yet sure of the best way to deal with it.\n\nAs well, the patch currently segfaults in testing when it computes beta(0.5, 0.5) due to some problem with the current #12289 package, which I suspect is unrelated.  But I think the new approach is beginning to take shape.",
    "created_at": "2012-05-29T05:11:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137467",
    "user": "https://github.com/sagetrac-dsm"
}
```

<div id="comment:21" align="right">comment:21</div>

Okay, I'm posting this for future reference.  I've attempted to preserve as much as possible of the original code while switching to the "new-style" dictionary-argument .n() approach we're working on in #12289, but I'm setting it to "needs work" myself while we think through some of the consequences of the switch.  Some of the `_evalf_` stuff violates DRY and I'm not yet sure of the best way to deal with it.

As well, the patch currently segfaults in testing when it computes beta(0.5, 0.5) due to some problem with the current #12289 package, which I suspect is unrelated.  But I think the new approach is beginning to take shape.



---

archive/issue_comments_137468.json:
```json
{
    "body": "Attachment: **[trac_12455_newstyle_airy.patch.gz](https://github.com/sagemath/sage/files/ticket12455/trac_12455_newstyle_airy.patch.gz)**\n\nnewstyle rewrite, version 1",
    "created_at": "2012-05-29T05:12:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137468",
    "user": "https://github.com/sagetrac-dsm"
}
```

Attachment: **[trac_12455_newstyle_airy.patch.gz](https://github.com/sagemath/sage/files/ticket12455/trac_12455_newstyle_airy.patch.gz)**

newstyle rewrite, version 1



---

archive/issue_comments_137469.json:
```json
{
    "body": "Dependencies: **#12289**",
    "created_at": "2012-05-29T05:13:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137469",
    "user": "https://github.com/sagetrac-dsm"
}
```

Dependencies: **#12289**



---

archive/issue_comments_137470.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">comment:23</div>\n\nOh, yeah: and #13028 was a significant nuisance while working on this.",
    "created_at": "2012-05-29T05:15:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137470",
    "user": "https://github.com/sagetrac-dsm"
}
```

<div id="comment:23" align="right">comment:23</div>

Oh, yeah: and #13028 was a significant nuisance while working on this.



---

archive/issue_comments_137471.json:
```json
{
    "body": "Attachment: **[trac_12455_newstyle_airy2.patch.gz](https://github.com/sagemath/sage/files/ticket12455/trac_12455_newstyle_airy2.patch.gz)**",
    "created_at": "2013-06-19T00:03:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137471",
    "user": "https://github.com/eviatarbach"
}
```

Attachment: **[trac_12455_newstyle_airy2.patch.gz](https://github.com/sagemath/sage/files/ticket12455/trac_12455_newstyle_airy2.patch.gz)**



---

archive/issue_comments_137472.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">comment:25</div>\n\nNew patch, which should pass all doctests once #12289 is merged.\n\nIt makes the following changes:\n* Changing an incorrect `airy_bi` identity\n* Adding `airy.py` to the documentation\n* Checking whether `n` is an integer (for some reason mpmath is able to evaluate numerically with non-integral `n`)\n* Formatting changes\n* Removing the parent's precision check because it wasn't used\n* Removing the check for the `parent` keyword because Burcin told me to\n\nPatchbot apply trac_12455_newstyle_airy.patch trac_12455_newstyle_airy2.patch",
    "created_at": "2013-06-19T00:09:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137472",
    "user": "https://github.com/eviatarbach"
}
```

<div id="comment:25" align="right">comment:25</div>

New patch, which should pass all doctests once #12289 is merged.

It makes the following changes:
* Changing an incorrect `airy_bi` identity
* Adding `airy.py` to the documentation
* Checking whether `n` is an integer (for some reason mpmath is able to evaluate numerically with non-integral `n`)
* Formatting changes
* Removing the parent's precision check because it wasn't used
* Removing the check for the `parent` keyword because Burcin told me to

Patchbot apply trac_12455_newstyle_airy.patch trac_12455_newstyle_airy2.patch



---

archive/issue_comments_137473.json:
```json
{
    "body": "Changed keywords from **Airy functions sd40.5** to **Airy functions sd40.5 sd48**",
    "created_at": "2013-06-19T00:19:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137473",
    "user": "https://github.com/eviatarbach"
}
```

Changed keywords from **Airy functions sd40.5** to **Airy functions sd40.5 sd48**



---

archive/issue_comments_137474.json:
```json
{
    "body": "<div id=\"comment:27\" align=\"right\">comment:27</div>\n\nNeeds review, then?  Note that I had to rebase #12289.",
    "created_at": "2013-06-19T01:48:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137474",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:27" align="right">comment:27</div>

Needs review, then?  Note that I had to rebase #12289.



---

archive/issue_events_168773.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-19T01:48:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168773"
}
```



---

archive/issue_events_168774.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-19T01:48:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168774"
}
```



---

archive/issue_comments_137475.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -35,3 +35,7 @@\n ```\n \n We should make it symbolical for both airy_ai and airy_bi, as well as their derivatives.\n+\n+---\n+\n+Apply [attachment: trac_12455_newstyle_airy.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455_newstyle_airy.patch.gz) and [attachment: trac_12455_newstyle_airy2.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455_newstyle_airy2.patch.gz).\n``````\n",
    "created_at": "2013-06-19T01:48:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137475",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -35,3 +35,7 @@
 ```
 
 We should make it symbolical for both airy_ai and airy_bi, as well as their derivatives.
+
+---
+
+Apply [attachment: trac_12455_newstyle_airy.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455_newstyle_airy.patch.gz) and [attachment: trac_12455_newstyle_airy2.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455_newstyle_airy2.patch.gz).
``````




---

archive/issue_comments_137476.json:
```json
{
    "body": "Changed author from **Oscar Gerardo Lazo Arjona, Benjamin Jones** to **Oscar Gerardo Lazo Arjona, Benjamin Jones, Eviatar Bach**",
    "created_at": "2013-06-19T01:48:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137476",
    "user": "https://github.com/kcrisman"
}
```

Changed author from **Oscar Gerardo Lazo Arjona, Benjamin Jones** to **Oscar Gerardo Lazo Arjona, Benjamin Jones, Eviatar Bach**



---

archive/issue_comments_137477.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -38,4 +38,4 @@\n \n ---\n \n-Apply [attachment: trac_12455_newstyle_airy.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455_newstyle_airy.patch.gz) and [attachment: trac_12455_newstyle_airy2.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455_newstyle_airy2.patch.gz).\n+Apply [attachment: trac_12455-newstyle-airy-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy-rebase.patch.gz) and [attachment: trac_12455_newstyle_airy2.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455_newstyle_airy2.patch.gz).\n``````\n",
    "created_at": "2013-06-19T20:12:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137477",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -38,4 +38,4 @@
 
 ---
 
-Apply [attachment: trac_12455_newstyle_airy.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455_newstyle_airy.patch.gz) and [attachment: trac_12455_newstyle_airy2.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455_newstyle_airy2.patch.gz).
+Apply [attachment: trac_12455-newstyle-airy-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy-rebase.patch.gz) and [attachment: trac_12455_newstyle_airy2.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455_newstyle_airy2.patch.gz).
``````




---

archive/issue_comments_137478.json:
```json
{
    "body": "<div id=\"comment:28\" align=\"right\">comment:28</div>\n\nPatchbot apply trac_12455-newstyle-airy-rebase.patch and trac_12455_newstyle_airy2.patch\n\nI don't know that either of the needs work issues are still there...",
    "created_at": "2013-06-19T20:12:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137478",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:28" align="right">comment:28</div>

Patchbot apply trac_12455-newstyle-airy-rebase.patch and trac_12455_newstyle_airy2.patch

I don't know that either of the needs work issues are still there...



---

archive/issue_comments_137479.json:
```json
{
    "body": "Reviewer: **Eviatar Bach, Karl-Dieter Crisman**",
    "created_at": "2013-06-19T20:12:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137479",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **Eviatar Bach, Karl-Dieter Crisman**



---

archive/issue_comments_137480.json:
```json
{
    "body": "<div id=\"comment:29\" align=\"right\">comment:29</div>\n\nThis needs a trivial extra patch to fix\n\n```\n\nsage -t sage/functions/special.py\n**********************************************************************\nFile \"sage/functions/special.py\", line 389, in sage.functions.special._init\nFailed example:\n    spherical_harmonic(3,2,1,2)\nExpected:\n    15/4*sqrt(7/30)*e^(4*I)*sin(1)^2*cos(1)/sqrt(pi)\nGot:\n    15/4*sqrt(7/30)*cos(1)*e^(4*I)*sin(1)^2/sqrt(pi)\n**********************************************************************\n```\nwhich is really a rebase issue off of #9880, no worries.",
    "created_at": "2013-06-19T20:19:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137480",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:29" align="right">comment:29</div>

This needs a trivial extra patch to fix

```

sage -t sage/functions/special.py
**********************************************************************
File "sage/functions/special.py", line 389, in sage.functions.special._init
Failed example:
    spherical_harmonic(3,2,1,2)
Expected:
    15/4*sqrt(7/30)*e^(4*I)*sin(1)^2*cos(1)/sqrt(pi)
Got:
    15/4*sqrt(7/30)*cos(1)*e^(4*I)*sin(1)^2/sqrt(pi)
**********************************************************************
```
which is really a rebase issue off of #9880, no worries.



---

archive/issue_comments_137481.json:
```json
{
    "body": "<div id=\"comment:30\" align=\"right\">comment:30</div>\n\nOkay, a little more rebasing for #12289.  Patches coming up.",
    "created_at": "2013-06-19T21:18:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137481",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:30" align="right">comment:30</div>

Okay, a little more rebasing for #12289.  Patches coming up.



---

archive/issue_comments_137482.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -38,4 +38,4 @@\n \n ---\n \n-Apply [attachment: trac_12455-newstyle-airy-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy-rebase.patch.gz) and [attachment: trac_12455_newstyle_airy2.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455_newstyle_airy2.patch.gz).\n+Apply [attachment: trac_12455-newstyle-airy-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy-rebase.patch.gz) and [attachment: trac_12455-newstyle-airy2-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy2-rebase.patch.gz).\n``````\n",
    "created_at": "2013-06-19T21:27:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137482",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -38,4 +38,4 @@
 
 ---
 
-Apply [attachment: trac_12455-newstyle-airy-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy-rebase.patch.gz) and [attachment: trac_12455_newstyle_airy2.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455_newstyle_airy2.patch.gz).
+Apply [attachment: trac_12455-newstyle-airy-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy-rebase.patch.gz) and [attachment: trac_12455-newstyle-airy2-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy2-rebase.patch.gz).
``````




---

archive/issue_comments_137483.json:
```json
{
    "body": "<div id=\"comment:31\" align=\"right\">comment:31</div>\n\nApply trac_12455-newstyle-airy-rebase.patch and trac_12455-newstyle-airy2-rebase.patch",
    "created_at": "2013-06-19T21:27:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137483",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:31" align="right">comment:31</div>

Apply trac_12455-newstyle-airy-rebase.patch and trac_12455-newstyle-airy2-rebase.patch



---

archive/issue_comments_137484.json:
```json
{
    "body": "<div id=\"comment:32\" align=\"right\">comment:32</div>\n\nEviatar, how much would you say still needs to be reviewed?  (In the sense that you have not yet given it positive review.)  Just your patch?\n\n(I'm running doctests now.)",
    "created_at": "2013-06-19T21:28:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137484",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:32" align="right">comment:32</div>

Eviatar, how much would you say still needs to be reviewed?  (In the sense that you have not yet given it positive review.)  Just your patch?

(I'm running doctests now.)



---

archive/issue_comments_137485.json:
```json
{
    "body": "<div id=\"comment:33\" align=\"right\">comment:33</div>\n\nI made some mistake while rebasing - one moment.",
    "created_at": "2013-06-19T21:32:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137485",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:33" align="right">comment:33</div>

I made some mistake while rebasing - one moment.



---

archive/issue_comments_137486.json:
```json
{
    "body": "<div id=\"comment:34\" align=\"right\">comment:34</div>\n\nI think I was fairly thorough, but I haven't positive-reviewed since my patch hasn't been looked at by someone else.",
    "created_at": "2013-06-19T21:35:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137486",
    "user": "https://github.com/eviatarbach"
}
```

<div id="comment:34" align="right">comment:34</div>

I think I was fairly thorough, but I haven't positive-reviewed since my patch hasn't been looked at by someone else.



---

archive/issue_comments_137487.json:
```json
{
    "body": "<div id=\"comment:35\" align=\"right\">comment:35</div>\n\nAttachment: **[trac_12455-newstyle-airy-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy-rebase.patch.gz)**\n\nOkay, now I have to fix something in the other one... sigh.  Final version coming soon, it *does* pass tests!",
    "created_at": "2013-06-19T21:55:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137487",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:35" align="right">comment:35</div>

Attachment: **[trac_12455-newstyle-airy-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy-rebase.patch.gz)**

Okay, now I have to fix something in the other one... sigh.  Final version coming soon, it *does* pass tests!



---

archive/issue_comments_137488.json:
```json
{
    "body": "Attachment: **[trac_12455-newstyle-airy2-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy2-rebase.patch.gz)**",
    "created_at": "2013-06-19T21:55:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137488",
    "user": "https://github.com/kcrisman"
}
```

Attachment: **[trac_12455-newstyle-airy2-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy2-rebase.patch.gz)**



---

archive/issue_comments_137489.json:
```json
{
    "body": "<div id=\"comment:36\" align=\"right\">comment:36</div>\n\nPatchbot, apply trac_12455-newstyle-airy-rebase.patch and trac_12455-newstyle-airy2-rebase.patch",
    "created_at": "2013-06-19T21:56:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137489",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:36" align="right">comment:36</div>

Patchbot, apply trac_12455-newstyle-airy-rebase.patch and trac_12455-newstyle-airy2-rebase.patch



---

archive/issue_comments_137490.json:
```json
{
    "body": "<div id=\"comment:37\" align=\"right\">comment:37</div>\n\n> (for some reason mpmath is able to evaluate numerically with non-integral n)\n\nBut this is even in the documentation you wrote!\n\n```\nReturn the `\\alpha`-th order fractional derivative with respect to `z`.\n```\n\"Fractional\", right?  So ... ?  Plus, you do *not* make this change in airy_bi, only airy_ai, for some reason.\n\nAnother question:\n\n```\nsage: (plot(airy_bi(x), (x, -10, 5)) +\\ \n```\nAre we allowing this kind of continuation in doctests still?  I can't remember but seem to recall something about this being a problem.\n\nOtherwise this all seems fine.",
    "created_at": "2013-06-19T22:54:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137490",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:37" align="right">comment:37</div>

> (for some reason mpmath is able to evaluate numerically with non-integral n)

But this is even in the documentation you wrote!

```
Return the `\alpha`-th order fractional derivative with respect to `z`.
```
"Fractional", right?  So ... ?  Plus, you do *not* make this change in airy_bi, only airy_ai, for some reason.

Another question:

```
sage: (plot(airy_bi(x), (x, -10, 5)) +\ 
```
Are we allowing this kind of continuation in doctests still?  I can't remember but seem to recall something about this being a problem.

Otherwise this all seems fine.



---

archive/issue_comments_137491.json:
```json
{
    "body": "<div id=\"comment:38\" align=\"right\">comment:38</div>\n\nI didn't write it. But looking at the mpmath documentation, it should work. It didn't work for most values before my patch anyway, but I'll see what I can do.",
    "created_at": "2013-06-19T23:07:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137491",
    "user": "https://github.com/eviatarbach"
}
```

<div id="comment:38" align="right">comment:38</div>

I didn't write it. But looking at the mpmath documentation, it should work. It didn't work for most values before my patch anyway, but I'll see what I can do.



---

archive/issue_events_168775.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2013-06-19T23:07:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168775"
}
```



---

archive/issue_events_168776.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2013-06-19T23:07:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168776"
}
```



---

archive/issue_comments_137492.json:
```json
{
    "body": "<div id=\"comment:39\" align=\"right\">comment:39</div>\n\n> I didn't write it. \n\nGood point, sorry!  In fact, it's just copied from [the mpmath doc](http://mpmath.googlecode.com/svn/tags/0.17/doc/build/functions/bessel.html#airyai).  But now you have taken charge :-)  Unfortunately, I can't find a lot of independent implementations of this...",
    "created_at": "2013-06-19T23:29:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137492",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:39" align="right">comment:39</div>

> I didn't write it. 

Good point, sorry!  In fact, it's just copied from [the mpmath doc](http://mpmath.googlecode.com/svn/tags/0.17/doc/build/functions/bessel.html#airyai).  But now you have taken charge :-)  Unfortunately, I can't find a lot of independent implementations of this...



---

archive/issue_comments_137493.json:
```json
{
    "body": "Attachment: **[trac_12455-airy_review.patch.gz](https://github.com/sagemath/sage/files/ticket12455/trac_12455-airy_review.patch.gz)**",
    "created_at": "2013-06-20T01:33:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137493",
    "user": "https://github.com/burcin"
}
```

Attachment: **[trac_12455-airy_review.patch.gz](https://github.com/sagemath/sage/files/ticket12455/trac_12455-airy_review.patch.gz)**



---

archive/issue_comments_137494.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -38,4 +38,8 @@\n \n ---\n \n-Apply [attachment: trac_12455-newstyle-airy-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy-rebase.patch.gz) and [attachment: trac_12455-newstyle-airy2-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy2-rebase.patch.gz).\n+Apply \n+\n+* [attachment: trac_12455-newstyle-airy-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy-rebase.patch.gz)\n+* [attachment: trac_12455-newstyle-airy2-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy2-rebase.patch.gz)\n+* [attachment: trac_12455-airy_review.patch]\n``````\n",
    "created_at": "2013-06-20T01:43:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137494",
    "user": "https://github.com/burcin"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -38,4 +38,8 @@
 
 ---
 
-Apply [attachment: trac_12455-newstyle-airy-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy-rebase.patch.gz) and [attachment: trac_12455-newstyle-airy2-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy2-rebase.patch.gz).
+Apply 
+
+* [attachment: trac_12455-newstyle-airy-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy-rebase.patch.gz)
+* [attachment: trac_12455-newstyle-airy2-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy2-rebase.patch.gz)
+* [attachment: trac_12455-airy_review.patch]
``````




---

archive/issue_comments_137495.json:
```json
{
    "body": "Changed reviewer from **Eviatar Bach, Karl-Dieter Crisman** to **Eviatar Bach, Karl-Dieter Crisman, Burcin Erocal**",
    "created_at": "2013-06-20T01:43:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137495",
    "user": "https://github.com/burcin"
}
```

Changed reviewer from **Eviatar Bach, Karl-Dieter Crisman** to **Eviatar Bach, Karl-Dieter Crisman, Burcin Erocal**



---

archive/issue_comments_137496.json:
```json
{
    "body": "<div id=\"comment:40\" align=\"right\">comment:40</div>\n\n[attachment: trac_12455-airy_review.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-airy_review.patch.gz) fixes a couple of problems:\n\n* `airy_{a,b}i_general` should evaluate to `airy_{a,b}i_simple`, `airy_{a,b}i_prime`, etc. depending on the first parameter\n\n* differentiating `airy_{a,b}i_general` in the first parameter is not allowed\n\n* `_evalf_` should not raise errors for unrecognized algorithm argument\n\n\nThis is still needs work because we use indirect doctests everywhere. There is no reason to go through the `airy_{a,b}i` wrapper functions. We should call the symbolic functions directly in the doctests. There are also a lot of code paths that are not tested. I changed the fractional order behavior, but didn't need to change any doctests.\n\nIt would be great if someone else can fix/add doctests. I will move on to something else now.",
    "created_at": "2013-06-20T01:43:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137496",
    "user": "https://github.com/burcin"
}
```

<div id="comment:40" align="right">comment:40</div>

[attachment: trac_12455-airy_review.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-airy_review.patch.gz) fixes a couple of problems:

* `airy_{a,b}i_general` should evaluate to `airy_{a,b}i_simple`, `airy_{a,b}i_prime`, etc. depending on the first parameter

* differentiating `airy_{a,b}i_general` in the first parameter is not allowed

* `_evalf_` should not raise errors for unrecognized algorithm argument


This is still needs work because we use indirect doctests everywhere. There is no reason to go through the `airy_{a,b}i` wrapper functions. We should call the symbolic functions directly in the doctests. There are also a lot of code paths that are not tested. I changed the fractional order behavior, but didn't need to change any doctests.

It would be great if someone else can fix/add doctests. I will move on to something else now.



---

archive/issue_comments_137497.json:
```json
{
    "body": "Changed author from **Oscar Gerardo Lazo Arjona, Benjamin Jones, Eviatar Bach** to **Oscar Gerardo Lazo Arjona, Benjamin Jones, D. S. McNeil, Eviatar Bach**",
    "created_at": "2013-06-20T01:43:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137497",
    "user": "https://github.com/burcin"
}
```

Changed author from **Oscar Gerardo Lazo Arjona, Benjamin Jones, Eviatar Bach** to **Oscar Gerardo Lazo Arjona, Benjamin Jones, D. S. McNeil, Eviatar Bach**



---

archive/issue_events_168777.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168777"
}
```



---

archive/issue_events_168778.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168778"
}
```



---

archive/issue_comments_137498.json:
```json
{
    "body": "<div id=\"comment:42\" align=\"right\">comment:42</div>\n\nThis patch depends on a new release of Pynac and other fixes to make the `algorithm` keyword work sensibly. Also, I don't see much point in having the Maxima implementation as an option, considering the fact that it doesn't accept complex input, doesn't have arbitrary precision, and is slower than mpmath (doesn't appear to scale better either).\n\nI'd hate to delay this patch longer, especially considering that the Airy functions are probably among the most-used special functions. Could we remove the Maxima option for now and perhaps add it in the future, maybe along with a patch that adds a SciPy option as suggested in one of the comments?",
    "created_at": "2013-09-10T03:48:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137498",
    "user": "https://github.com/eviatarbach"
}
```

<div id="comment:42" align="right">comment:42</div>

This patch depends on a new release of Pynac and other fixes to make the `algorithm` keyword work sensibly. Also, I don't see much point in having the Maxima implementation as an option, considering the fact that it doesn't accept complex input, doesn't have arbitrary precision, and is slower than mpmath (doesn't appear to scale better either).

I'd hate to delay this patch longer, especially considering that the Airy functions are probably among the most-used special functions. Could we remove the Maxima option for now and perhaps add it in the future, maybe along with a patch that adds a SciPy option as suggested in one of the comments?



---

archive/issue_comments_137499.json:
```json
{
    "body": "<div id=\"comment:43\" align=\"right\">comment:43</div>\n\nI'm fine with removing Maxima from the options for numerical evaluation (numerical is not generally where Maxima shines). I agree with @burcin, though, that indirect doctests should be fixed wherever possible and become direct tests and that the branch coverage should be higher.\n\nWhat exactly is the issue with having the Maxima option? Is it the limited precision and domain? We could always check that inputs are okay whenever Maxima is selected and raise an exception otherwise.",
    "created_at": "2013-09-10T06:03:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137499",
    "user": "https://github.com/benjaminfjones"
}
```

<div id="comment:43" align="right">comment:43</div>

I'm fine with removing Maxima from the options for numerical evaluation (numerical is not generally where Maxima shines). I agree with @burcin, though, that indirect doctests should be fixed wherever possible and become direct tests and that the branch coverage should be higher.

What exactly is the issue with having the Maxima option? Is it the limited precision and domain? We could always check that inputs are okay whenever Maxima is selected and raise an exception otherwise.



---

archive/issue_events_168779.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168779"
}
```



---

archive/issue_events_168780.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168780"
}
```



---

archive/issue_events_168781.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168781"
}
```



---

archive/issue_events_168782.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168782"
}
```



---

archive/issue_comments_137500.json:
```json
{
    "body": "Branch: **[u/rws/make_airy_functions_symbolic](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/make_airy_functions_symbolic)**",
    "created_at": "2014-07-19T10:19:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137500",
    "user": "https://github.com/rwst"
}
```

Branch: **[u/rws/make_airy_functions_symbolic](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/make_airy_functions_symbolic)**



---

archive/issue_comments_137501.json:
```json
{
    "body": "Commit: **[`22731c9`](https://github.com/sagemath/sagetrac-mirror/commit/22731c9d5b976301d282e3b48e259f5f13d4b8bd)**",
    "created_at": "2014-07-24T08:16:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137501",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[`22731c9`](https://github.com/sagemath/sagetrac-mirror/commit/22731c9d5b976301d282e3b48e259f5f13d4b8bd)**



---

archive/issue_comments_137502.json:
```json
{
    "body": "<div id=\"comment:47\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/aeb08003cec50604bb727bd9a5b36993708a6ca7\"><code>aeb0800</code></a></td><td><code>12455: remove algorithm=maxima</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/22731c9d5b976301d282e3b48e259f5f13d4b8bd\"><code>22731c9</code></a></td><td><code>12455: implement algorithm='scipy'</code></td></tr></table>\n",
    "created_at": "2014-07-24T08:16:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137502",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:47"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/aeb08003cec50604bb727bd9a5b36993708a6ca7"><code>aeb0800</code></a></td><td><code>12455: remove algorithm=maxima</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/22731c9d5b976301d282e3b48e259f5f13d4b8bd"><code>22731c9</code></a></td><td><code>12455: implement algorithm='scipy'</code></td></tr></table>




---

archive/issue_comments_137503.json:
```json
{
    "body": "<div id=\"comment:48\" align=\"right\">comment:48</div>\n\nReplying to [@benjaminfjones](#comment%3A43):\n> What exactly is the issue with having the Maxima option? Is it the limited precision and domain? We could always check that inputs are okay whenever Maxima is selected and raise an exception otherwise.\n\nIt's useless, especially when scipy is implemented as algorithm. OTOH, it introduces more code paths to test and deepens a dependency that is unwanted.\n\nOh, and scipy is a bit faster:\n\n```\nsage: timeit(\"airy_bi(2).n(algorithm='maxima')\")\n625 loops, best of 3: 685 \u00b5s per loop\nsage: timeit(\"airy_bi(2).n(algorithm='scipy')\")\n625 loops, best of 3: 124 \u00b5s per loop\n```",
    "created_at": "2014-07-24T08:16:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137503",
    "user": "https://github.com/rwst"
}
```

<div id="comment:48" align="right">comment:48</div>

Replying to [@benjaminfjones](#comment%3A43):
> What exactly is the issue with having the Maxima option? Is it the limited precision and domain? We could always check that inputs are okay whenever Maxima is selected and raise an exception otherwise.

It's useless, especially when scipy is implemented as algorithm. OTOH, it introduces more code paths to test and deepens a dependency that is unwanted.

Oh, and scipy is a bit faster:

```
sage: timeit("airy_bi(2).n(algorithm='maxima')")
625 loops, best of 3: 685 µs per loop
sage: timeit("airy_bi(2).n(algorithm='scipy')")
625 loops, best of 3: 124 µs per loop
```



---

archive/issue_comments_137504.json:
```json
{
    "body": "Changed commit from **[`22731c9`](https://github.com/sagemath/sagetrac-mirror/commit/22731c9d5b976301d282e3b48e259f5f13d4b8bd)** to **[`515e343`](https://github.com/sagemath/sagetrac-mirror/commit/515e343290448d2bbe5e75f88accea814be9215f)**",
    "created_at": "2014-07-24T15:05:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137504",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`22731c9`](https://github.com/sagemath/sagetrac-mirror/commit/22731c9d5b976301d282e3b48e259f5f13d4b8bd)** to **[`515e343`](https://github.com/sagemath/sagetrac-mirror/commit/515e343290448d2bbe5e75f88accea814be9215f)**



---

archive/issue_comments_137505.json:
```json
{
    "body": "<div id=\"comment:49\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a88d0353f7519a75023e784b189c693b43e4b86b\"><code>a88d035</code></a></td><td><code>12455: fix indirect and plot doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f91a3c2ee9e75aa291278536ac8093f8aecc682b\"><code>f91a3c2</code></a></td><td><code>12455: hold_derivate only implemented in wrapper functions</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/515e343290448d2bbe5e75f88accea814be9215f\"><code>515e343</code></a></td><td><code>12455: add small index to documentation head</code></td></tr></table>\n",
    "created_at": "2014-07-24T15:05:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137505",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:49"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a88d0353f7519a75023e784b189c693b43e4b86b"><code>a88d035</code></a></td><td><code>12455: fix indirect and plot doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f91a3c2ee9e75aa291278536ac8093f8aecc682b"><code>f91a3c2</code></a></td><td><code>12455: hold_derivate only implemented in wrapper functions</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/515e343290448d2bbe5e75f88accea814be9215f"><code>515e343</code></a></td><td><code>12455: add small index to documentation head</code></td></tr></table>




---

archive/issue_events_168783.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-07-24T15:15:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168783"
}
```



---

archive/issue_events_168784.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-07-24T15:15:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168784"
}
```



---

archive/issue_comments_137506.json:
```json
{
    "body": "Changed author from **Oscar Gerardo Lazo Arjona, Benjamin Jones, D. S. McNeil, Eviatar Bach** to **Oscar Gerardo Lazo Arjona, Benjamin Jones, D. S. McNeil, Eviatar Bach, Ralf Stephan**",
    "created_at": "2014-07-24T15:15:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137506",
    "user": "https://github.com/rwst"
}
```

Changed author from **Oscar Gerardo Lazo Arjona, Benjamin Jones, D. S. McNeil, Eviatar Bach** to **Oscar Gerardo Lazo Arjona, Benjamin Jones, D. S. McNeil, Eviatar Bach, Ralf Stephan**



---

archive/issue_comments_137507.json:
```json
{
    "body": "<div id=\"comment:50\" align=\"right\">comment:50</div>\n\nEverything was reviewed up to my recent changes:\n\n* merged develop\n* removal of maxima and inclusion of scipy for numerics\n* replacement of indirect with direct doctests (still 100% coverage)\n* removal of hold_derivate keyword from `airy_ai/bi_general()` where it wasn't implemented (it is still in `airy_ai/bi()` wrapper functions\n* small index at the top of documentation for orientation, because of confusing presentation order of functions, which cannot be controlled, sadly\n\nSo please review.",
    "created_at": "2014-07-24T15:15:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137507",
    "user": "https://github.com/rwst"
}
```

<div id="comment:50" align="right">comment:50</div>

Everything was reviewed up to my recent changes:

* merged develop
* removal of maxima and inclusion of scipy for numerics
* replacement of indirect with direct doctests (still 100% coverage)
* removal of hold_derivate keyword from `airy_ai/bi_general()` where it wasn't implemented (it is still in `airy_ai/bi()` wrapper functions
* small index at the top of documentation for orientation, because of confusing presentation order of functions, which cannot be controlled, sadly

So please review.



---

archive/issue_comments_137508.json:
```json
{
    "body": "Changed work issues from **circular import, doctest failures** to none",
    "created_at": "2014-07-24T15:15:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137508",
    "user": "https://github.com/rwst"
}
```

Changed work issues from **circular import, doctest failures** to none



---

archive/issue_events_168785.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168785"
}
```



---

archive/issue_events_168786.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168786"
}
```



---

archive/issue_comments_137509.json:
```json
{
    "body": "<div id=\"comment:52\" align=\"right\">comment:52</div>\n\none failing doctest, see patchbot report",
    "created_at": "2014-08-23T12:20:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137509",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:52" align="right">comment:52</div>

one failing doctest, see patchbot report



---

archive/issue_events_168787.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-08-23T12:20:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168787"
}
```



---

archive/issue_events_168788.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-08-23T12:20:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168788"
}
```



---

archive/issue_comments_137510.json:
```json
{
    "body": "Changed commit from **[`515e343`](https://github.com/sagemath/sagetrac-mirror/commit/515e343290448d2bbe5e75f88accea814be9215f)** to **[`8e76828`](https://github.com/sagemath/sagetrac-mirror/commit/8e76828f9f5c988b0429230cd1050c6086fb730b)**",
    "created_at": "2014-08-26T11:19:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137510",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`515e343`](https://github.com/sagemath/sagetrac-mirror/commit/515e343290448d2bbe5e75f88accea814be9215f)** to **[`8e76828`](https://github.com/sagemath/sagetrac-mirror/commit/8e76828f9f5c988b0429230cd1050c6086fb730b)**



---

archive/issue_comments_137511.json:
```json
{
    "body": "<div id=\"comment:53\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/17158927b386e755f897acacabcd1c5b816df557\"><code>1715892</code></a></td><td><code>Merge branch 'develop' into t/12455/make_airy_functions_symbolic</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8e76828f9f5c988b0429230cd1050c6086fb730b\"><code>8e76828</code></a></td><td><code>12455: trigger doctest with MaximaFunction as intended</code></td></tr></table>\n",
    "created_at": "2014-08-26T11:19:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137511",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:53"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/17158927b386e755f897acacabcd1c5b816df557"><code>1715892</code></a></td><td><code>Merge branch 'develop' into t/12455/make_airy_functions_symbolic</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8e76828f9f5c988b0429230cd1050c6086fb730b"><code>8e76828</code></a></td><td><code>12455: trigger doctest with MaximaFunction as intended</code></td></tr></table>




---

archive/issue_events_168789.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2014-08-26T11:20:54Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "subject": "https://github.com/rwst",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168789"
}
```



---

archive/issue_events_168790.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-08-26T11:20:54Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "subject": "https://github.com/rwst",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168790"
}
```



---

archive/issue_events_168791.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-08-26T11:22:05Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "subject": "https://github.com/rwst",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168791"
}
```



---

archive/issue_events_168792.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2014-08-26T11:22:05Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "subject": "https://github.com/rwst",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168792"
}
```



---

archive/issue_events_168793.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-08-26T11:22:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168793"
}
```



---

archive/issue_events_168794.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-08-26T11:22:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168794"
}
```



---

archive/issue_comments_137512.json:
```json
{
    "body": "Changed branch from **[u/rws/make_airy_functions_symbolic](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/make_airy_functions_symbolic)** to **[public/ticket/12455](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/12455)**",
    "created_at": "2014-09-25T19:59:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137512",
    "user": "https://github.com/fchapoton"
}
```

Changed branch from **[u/rws/make_airy_functions_symbolic](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/make_airy_functions_symbolic)** to **[public/ticket/12455](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/12455)**



---

archive/issue_comments_137513.json:
```json
{
    "body": "<div id=\"comment:57\" align=\"right\">comment:57</div>\n\nRebased on 6.4.beta3, and add a few `# rel tol` to the scipy results.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c3449e86ac942de214c90b3aa61665f6b5b13575\"><code>c3449e8</code></a></td><td><code>Merge branch 'u/rws/make_airy_functions_symbolic' and 6.4.beta3</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/95526065b2703e42378838747055bb6bbc409402\"><code>9552606</code></a></td><td><code>trac #12455 fixing doctests by adding rel tol and changing imports</code></td></tr></table>\n",
    "created_at": "2014-09-25T19:59:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137513",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:57" align="right">comment:57</div>

Rebased on 6.4.beta3, and add a few `# rel tol` to the scipy results.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c3449e86ac942de214c90b3aa61665f6b5b13575"><code>c3449e8</code></a></td><td><code>Merge branch 'u/rws/make_airy_functions_symbolic' and 6.4.beta3</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/95526065b2703e42378838747055bb6bbc409402"><code>9552606</code></a></td><td><code>trac #12455 fixing doctests by adding rel tol and changing imports</code></td></tr></table>




---

archive/issue_comments_137514.json:
```json
{
    "body": "Changed commit from **[`8e76828`](https://github.com/sagemath/sagetrac-mirror/commit/8e76828f9f5c988b0429230cd1050c6086fb730b)** to **[`9552606`](https://github.com/sagemath/sagetrac-mirror/commit/95526065b2703e42378838747055bb6bbc409402)**",
    "created_at": "2014-09-25T19:59:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137514",
    "user": "https://github.com/fchapoton"
}
```

Changed commit from **[`8e76828`](https://github.com/sagemath/sagetrac-mirror/commit/8e76828f9f5c988b0429230cd1050c6086fb730b)** to **[`9552606`](https://github.com/sagemath/sagetrac-mirror/commit/95526065b2703e42378838747055bb6bbc409402)**



---

archive/issue_comments_137515.json:
```json
{
    "body": "Changed commit from **[`9552606`](https://github.com/sagemath/sagetrac-mirror/commit/95526065b2703e42378838747055bb6bbc409402)** to **[`7102227`](https://github.com/sagemath/sagetrac-mirror/commit/7102227617bf289b2620636e397b92c081f9715a)**",
    "created_at": "2014-09-28T07:29:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137515",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`9552606`](https://github.com/sagemath/sagetrac-mirror/commit/95526065b2703e42378838747055bb6bbc409402)** to **[`7102227`](https://github.com/sagemath/sagetrac-mirror/commit/7102227617bf289b2620636e397b92c081f9715a)**



---

archive/issue_comments_137516.json:
```json
{
    "body": "<div id=\"comment:58\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7102227617bf289b2620636e397b92c081f9715a\"><code>7102227</code></a></td><td><code>Merge with 6.4.beta4</code></td></tr></table>\n",
    "created_at": "2014-09-28T07:29:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137516",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:58"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7102227617bf289b2620636e397b92c081f9715a"><code>7102227</code></a></td><td><code>Merge with 6.4.beta4</code></td></tr></table>




---

archive/issue_comments_137517.json:
```json
{
    "body": "<div id=\"comment:59\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5b731d32d7aec6bdb88acf777253c777ccb6f509\"><code>5b731d3</code></a></td><td><code>trac #12455 change use of Airy as example of maxima function</code></td></tr></table>\n",
    "created_at": "2014-09-30T14:07:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137517",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:59"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5b731d32d7aec6bdb88acf777253c777ccb6f509"><code>5b731d3</code></a></td><td><code>trac #12455 change use of Airy as example of maxima function</code></td></tr></table>




---

archive/issue_comments_137518.json:
```json
{
    "body": "Changed commit from **[`7102227`](https://github.com/sagemath/sagetrac-mirror/commit/7102227617bf289b2620636e397b92c081f9715a)** to **[`5b731d3`](https://github.com/sagemath/sagetrac-mirror/commit/5b731d32d7aec6bdb88acf777253c777ccb6f509)**",
    "created_at": "2014-09-30T14:07:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137518",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`7102227`](https://github.com/sagemath/sagetrac-mirror/commit/7102227617bf289b2620636e397b92c081f9715a)** to **[`5b731d3`](https://github.com/sagemath/sagetrac-mirror/commit/5b731d32d7aec6bdb88acf777253c777ccb6f509)**



---

archive/issue_comments_137519.json:
```json
{
    "body": "<div id=\"comment:60\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/39deab4d13c17120737a875a4b2744a67ce61227\"><code>39deab4</code></a></td><td><code>trac #12455 formatting doc and code</code></td></tr></table>\n",
    "created_at": "2014-09-30T14:13:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137519",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:60"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/39deab4d13c17120737a875a4b2744a67ce61227"><code>39deab4</code></a></td><td><code>trac #12455 formatting doc and code</code></td></tr></table>




---

archive/issue_comments_137520.json:
```json
{
    "body": "Changed commit from **[`5b731d3`](https://github.com/sagemath/sagetrac-mirror/commit/5b731d32d7aec6bdb88acf777253c777ccb6f509)** to **[`39deab4`](https://github.com/sagemath/sagetrac-mirror/commit/39deab4d13c17120737a875a4b2744a67ce61227)**",
    "created_at": "2014-09-30T14:13:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137520",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`5b731d3`](https://github.com/sagemath/sagetrac-mirror/commit/5b731d32d7aec6bdb88acf777253c777ccb6f509)** to **[`39deab4`](https://github.com/sagemath/sagetrac-mirror/commit/39deab4d13c17120737a875a4b2744a67ce61227)**



---

archive/issue_comments_137521.json:
```json
{
    "body": "Changed reviewer from **Eviatar Bach, Karl-Dieter Crisman, Burcin Erocal** to **Eviatar Bach, Karl-Dieter Crisman, Burcin Erocal, Ralf Stephan**",
    "created_at": "2014-10-02T09:21:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137521",
    "user": "https://github.com/rwst"
}
```

Changed reviewer from **Eviatar Bach, Karl-Dieter Crisman, Burcin Erocal** to **Eviatar Bach, Karl-Dieter Crisman, Burcin Erocal, Ralf Stephan**



---

archive/issue_events_168795.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-10-02T09:21:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168795"
}
```



---

archive/issue_events_168796.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-10-02T09:21:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168796"
}
```



---

archive/issue_comments_137522.json:
```json
{
    "body": "<div id=\"comment:61\" align=\"right\">comment:61</div>\n\n\n```\nsage -t --long src/sage/functions/airy.py\n**********************************************************************\nFile \"src/sage/functions/airy.py\", line 404, in sage.functions.airy.airy_ai\nFailed example:\n    plot(airy_ai(x), (x, -10, 5)) + plot(airy_ai_prime(x),\n     (x, -10, 5), color='red')\nExpected nothing\nGot:\n    Graphics object consisting of 2 graphics primitives\n**********************************************************************\nFile \"src/sage/functions/airy.py\", line 799, in sage.functions.airy.airy_bi\nFailed example:\n    plot(airy_bi(x), (x, -10, 5)) + plot(airy_bi_prime(x),\n     (x, -10, 5), color='red')\nExpected nothing\nGot:\n    Graphics object consisting of 2 graphics primitives\n```",
    "created_at": "2014-10-02T09:21:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137522",
    "user": "https://github.com/rwst"
}
```

<div id="comment:61" align="right">comment:61</div>


```
sage -t --long src/sage/functions/airy.py
**********************************************************************
File "src/sage/functions/airy.py", line 404, in sage.functions.airy.airy_ai
Failed example:
    plot(airy_ai(x), (x, -10, 5)) + plot(airy_ai_prime(x),
     (x, -10, 5), color='red')
Expected nothing
Got:
    Graphics object consisting of 2 graphics primitives
**********************************************************************
File "src/sage/functions/airy.py", line 799, in sage.functions.airy.airy_bi
Failed example:
    plot(airy_bi(x), (x, -10, 5)) + plot(airy_bi_prime(x),
     (x, -10, 5), color='red')
Expected nothing
Got:
    Graphics object consisting of 2 graphics primitives
```



---

archive/issue_comments_137523.json:
```json
{
    "body": "<div id=\"comment:62\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fbd6c357dfdba94d7630d6f2becb411f6169f004\"><code>fbd6c35</code></a></td><td><code>trac #12455 plot output</code></td></tr></table>\n",
    "created_at": "2014-10-03T14:48:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137523",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:62"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fbd6c357dfdba94d7630d6f2becb411f6169f004"><code>fbd6c35</code></a></td><td><code>trac #12455 plot output</code></td></tr></table>




---

archive/issue_comments_137524.json:
```json
{
    "body": "Changed commit from **[`39deab4`](https://github.com/sagemath/sagetrac-mirror/commit/39deab4d13c17120737a875a4b2744a67ce61227)** to **[`fbd6c35`](https://github.com/sagemath/sagetrac-mirror/commit/fbd6c357dfdba94d7630d6f2becb411f6169f004)**",
    "created_at": "2014-10-03T14:48:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137524",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`39deab4`](https://github.com/sagemath/sagetrac-mirror/commit/39deab4d13c17120737a875a4b2744a67ce61227)** to **[`fbd6c35`](https://github.com/sagemath/sagetrac-mirror/commit/fbd6c357dfdba94d7630d6f2becb411f6169f004)**



---

archive/issue_events_168797.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-10-03T14:49:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168797"
}
```



---

archive/issue_events_168798.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-10-03T14:49:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168798"
}
```



---

archive/issue_comments_137525.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -35,11 +35,3 @@\n ```\n \n We should make it symbolical for both airy_ai and airy_bi, as well as their derivatives.\n-\n----\n-\n-Apply \n-\n-* [attachment: trac_12455-newstyle-airy-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy-rebase.patch.gz)\n-* [attachment: trac_12455-newstyle-airy2-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy2-rebase.patch.gz)\n-* [attachment: trac_12455-airy_review.patch]\n``````\n",
    "created_at": "2014-10-11T18:55:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137525",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -35,11 +35,3 @@
 ```
 
 We should make it symbolical for both airy_ai and airy_bi, as well as their derivatives.
-
----
-
-Apply 
-
-* [attachment: trac_12455-newstyle-airy-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy-rebase.patch.gz)
-* [attachment: trac_12455-newstyle-airy2-rebase.patch](https://github.com/sagemath/sage/files/ticket12455/trac_12455-newstyle-airy2-rebase.patch.gz)
-* [attachment: trac_12455-airy_review.patch]
``````




---

archive/issue_comments_137526.json:
```json
{
    "body": "<div id=\"comment:65\" align=\"right\">comment:65</div>\n\nJust to make you aware: this conflicts with #17130. When either of these tickets gets positive_review, the other should be rebased.",
    "created_at": "2014-10-11T18:57:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137526",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:65" align="right">comment:65</div>

Just to make you aware: this conflicts with #17130. When either of these tickets gets positive_review, the other should be rebased.



---

archive/issue_events_168799.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-11T18:59:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168799"
}
```



---

archive/issue_events_168800.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-11T18:59:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168800"
}
```



---

archive/issue_comments_137527.json:
```json
{
    "body": "<div id=\"comment:66\" align=\"right\">comment:66</div>\n\nAnd why the change to `def hypergeometric_U`? The new code seems quite hackish, why is it needed and how does it relate to this ticket?",
    "created_at": "2014-10-11T18:59:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137527",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:66" align="right">comment:66</div>

And why the change to `def hypergeometric_U`? The new code seems quite hackish, why is it needed and how does it relate to this ticket?



---

archive/issue_comments_137528.json:
```json
{
    "body": "<div id=\"comment:67\" align=\"right\">comment:67</div>\n\nThese should be an error:\n\n```\nsage: airy_ai(3).n(algorithm='scipy', prec=200)\n0.006591139357460719\nsage: airy_ai(3).n(algorithm='whatever')\n0.00659113935746072\n```\n\nAnd I don't like this either (parent should be `RR`):\n\n```\nsage: parent(airy_ai(3).n(algorithm='scipy'))\nReal Double Field\n```",
    "created_at": "2014-10-11T19:12:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137528",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:67" align="right">comment:67</div>

These should be an error:

```
sage: airy_ai(3).n(algorithm='scipy', prec=200)
0.006591139357460719
sage: airy_ai(3).n(algorithm='whatever')
0.00659113935746072
```

And I don't like this either (parent should be `RR`):

```
sage: parent(airy_ai(3).n(algorithm='scipy'))
Real Double Field
```



---

archive/issue_events_168801.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-11T19:12:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168801"
}
```



---

archive/issue_events_168802.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-11T19:12:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168802"
}
```



---

archive/issue_comments_137529.json:
```json
{
    "body": "<div id=\"comment:68\" align=\"right\">comment:68</div>\n\nWhat's the point of\n\n```\nif len(args) > 0:\n   raise TypeError((\"symbolic function airy_ai takes at most 3 arguments \"\n      \"({} given)\").format(len(args) + 3))\n```\nwhy not simply remove `*args` completely then?",
    "created_at": "2014-10-11T19:22:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137529",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:68" align="right">comment:68</div>

What's the point of

```
if len(args) > 0:
   raise TypeError(("symbolic function airy_ai takes at most 3 arguments "
      "({} given)").format(len(args) + 3))
```
why not simply remove `*args` completely then?



---

archive/issue_comments_137530.json:
```json
{
    "body": "<div id=\"comment:69\" align=\"right\">comment:69</div>\n\nReplying to [@jdemeyer](#comment%3A66):\n> And why the change to `def hypergeometric_U`? The new code seems quite hackish, why is it needed and how does it relate to this ticket?\n\nI think that unrelated patch can be safely removed because scipy evaluation is replaced by mpmath in #14896 (of course hoping that that gets reviewed at some time).",
    "created_at": "2014-10-12T16:47:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137530",
    "user": "https://github.com/rwst"
}
```

<div id="comment:69" align="right">comment:69</div>

Replying to [@jdemeyer](#comment%3A66):
> And why the change to `def hypergeometric_U`? The new code seems quite hackish, why is it needed and how does it relate to this ticket?

I think that unrelated patch can be safely removed because scipy evaluation is replaced by mpmath in #14896 (of course hoping that that gets reviewed at some time).



---

archive/issue_comments_137531.json:
```json
{
    "body": "<div id=\"comment:70\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/15e7e34d6ec0854e2bb642f9b2df6fe5d870aadd\"><code>15e7e34</code></a></td><td><code>Merge branch 'develop' into t/12455/public/ticket/12455</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2c0931c0c9d0f259968d65aab9e1eb4c2d332ac2\"><code>2c0931c</code></a></td><td><code>12455: remove fishy code in hypergeometric_U()</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b4213d373b99194e3ad8eecefee0dbb7b382ddd8\"><code>b4213d3</code></a></td><td><code>12455: with algorithm=scipy return reals for real argument</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b0841c906bae1f282bedea5fb53b200b0cebe6b4\"><code>b0841c9</code></a></td><td><code>12455: catch attempts to exceed precision with algorithm=scipy</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/484a61559726129a63d53beb9da8a700a69db418\"><code>484a615</code></a></td><td><code>12455: catch unknown evalf algorithms</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/58e006c7bbc717fcb70911e8786fc5d94bf2ff1a\"><code>58e006c</code></a></td><td><code>12455: remove superfluous args and check</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/870e003d7f190ac4edd3fce77a584e657e447c57\"><code>870e003</code></a></td><td><code>12455: fix plot doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/61847c43253572e53c6ed25fe208f4eb19fd1b87\"><code>61847c4</code></a></td><td><code>Merge branch 'public/ticket/12455' of trac.sagemath.org:sage into t/12455/public/ticket/12455</code></td></tr></table>\n",
    "created_at": "2014-10-13T14:39:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137531",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:70"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/15e7e34d6ec0854e2bb642f9b2df6fe5d870aadd"><code>15e7e34</code></a></td><td><code>Merge branch 'develop' into t/12455/public/ticket/12455</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2c0931c0c9d0f259968d65aab9e1eb4c2d332ac2"><code>2c0931c</code></a></td><td><code>12455: remove fishy code in hypergeometric_U()</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b4213d373b99194e3ad8eecefee0dbb7b382ddd8"><code>b4213d3</code></a></td><td><code>12455: with algorithm=scipy return reals for real argument</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b0841c906bae1f282bedea5fb53b200b0cebe6b4"><code>b0841c9</code></a></td><td><code>12455: catch attempts to exceed precision with algorithm=scipy</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/484a61559726129a63d53beb9da8a700a69db418"><code>484a615</code></a></td><td><code>12455: catch unknown evalf algorithms</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/58e006c7bbc717fcb70911e8786fc5d94bf2ff1a"><code>58e006c</code></a></td><td><code>12455: remove superfluous args and check</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/870e003d7f190ac4edd3fce77a584e657e447c57"><code>870e003</code></a></td><td><code>12455: fix plot doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/61847c43253572e53c6ed25fe208f4eb19fd1b87"><code>61847c4</code></a></td><td><code>Merge branch 'public/ticket/12455' of trac.sagemath.org:sage into t/12455/public/ticket/12455</code></td></tr></table>




---

archive/issue_comments_137532.json:
```json
{
    "body": "Changed commit from **[`fbd6c35`](https://github.com/sagemath/sagetrac-mirror/commit/fbd6c357dfdba94d7630d6f2becb411f6169f004)** to **[`61847c4`](https://github.com/sagemath/sagetrac-mirror/commit/61847c43253572e53c6ed25fe208f4eb19fd1b87)**",
    "created_at": "2014-10-13T14:39:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137532",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`fbd6c35`](https://github.com/sagemath/sagetrac-mirror/commit/fbd6c357dfdba94d7630d6f2becb411f6169f004)** to **[`61847c4`](https://github.com/sagemath/sagetrac-mirror/commit/61847c43253572e53c6ed25fe208f4eb19fd1b87)**



---

archive/issue_events_168803.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-10-13T14:40:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168803"
}
```



---

archive/issue_events_168804.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-10-13T14:40:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168804"
}
```



---

archive/issue_comments_137533.json:
```json
{
    "body": "<div id=\"comment:72\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6d107297a5d8867f1a1e9996678ffa441adc8a2d\"><code>6d10729</code></a></td><td><code>Simplify numerical evaluation of BuiltinFunctions</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b6e1ed44a663f7410fddb2e3e4c134aa3a0ce8cf\"><code>b6e1ed4</code></a></td><td><code>Merge remote-tracking branches 'origin/u/jdemeyer/ticket/17131' and 'origin/u/jdemeyer/ticket/17133' into ticket/17130</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/382f97a71989fb04131c0720918e9b66b19c125b\"><code>382f97a</code></a></td><td><code>Merge branch 'u/jdemeyer/ticket/17130' of trac.sagemath.org:sage into 6.5beta1</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/72659895c652ce20ed1373f95bfea378d9cbe546\"><code>7265989</code></a></td><td><code>17130: reviewer's patch: fix typo</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c47dbd49b0e5ebb89b3d7afa694478c3d572866f\"><code>c47dbd4</code></a></td><td><code>Fix Trac #17328 again in a better way</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a486db2b801f528a1a00b0dadb4c4c522f21654a\"><code>a486db2</code></a></td><td><code>Call the factorial() method of an Integer</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9d3cbbdbafb12d1da899abafe0137b4a5639de30\"><code>9d3cbbd</code></a></td><td><code>Fix numerical noise</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/abab222e67040518acee766f747c161a7445bc0a\"><code>abab222</code></a></td><td><code>Fix more numerical noise</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/01614f7524f952bece01ef02f68eb28c4a293e24\"><code>01614f7</code></a></td><td><code>Merge branch 'u/jdemeyer/ticket/17130' of trac.sagemath.org:sage into t/12455/public/ticket/12455</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/aac86a20bbb84df905e4000dfb79f125247cf85b\"><code>aac86a2</code></a></td><td><code>12455: simplifications due to trac 17130</code></td></tr></table>\n",
    "created_at": "2014-11-29T15:34:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137533",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:72"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6d107297a5d8867f1a1e9996678ffa441adc8a2d"><code>6d10729</code></a></td><td><code>Simplify numerical evaluation of BuiltinFunctions</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b6e1ed44a663f7410fddb2e3e4c134aa3a0ce8cf"><code>b6e1ed4</code></a></td><td><code>Merge remote-tracking branches 'origin/u/jdemeyer/ticket/17131' and 'origin/u/jdemeyer/ticket/17133' into ticket/17130</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/382f97a71989fb04131c0720918e9b66b19c125b"><code>382f97a</code></a></td><td><code>Merge branch 'u/jdemeyer/ticket/17130' of trac.sagemath.org:sage into 6.5beta1</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/72659895c652ce20ed1373f95bfea378d9cbe546"><code>7265989</code></a></td><td><code>17130: reviewer's patch: fix typo</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c47dbd49b0e5ebb89b3d7afa694478c3d572866f"><code>c47dbd4</code></a></td><td><code>Fix Trac #17328 again in a better way</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a486db2b801f528a1a00b0dadb4c4c522f21654a"><code>a486db2</code></a></td><td><code>Call the factorial() method of an Integer</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9d3cbbdbafb12d1da899abafe0137b4a5639de30"><code>9d3cbbd</code></a></td><td><code>Fix numerical noise</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/abab222e67040518acee766f747c161a7445bc0a"><code>abab222</code></a></td><td><code>Fix more numerical noise</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/01614f7524f952bece01ef02f68eb28c4a293e24"><code>01614f7</code></a></td><td><code>Merge branch 'u/jdemeyer/ticket/17130' of trac.sagemath.org:sage into t/12455/public/ticket/12455</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/aac86a20bbb84df905e4000dfb79f125247cf85b"><code>aac86a2</code></a></td><td><code>12455: simplifications due to trac 17130</code></td></tr></table>




---

archive/issue_comments_137534.json:
```json
{
    "body": "Changed commit from **[`61847c4`](https://github.com/sagemath/sagetrac-mirror/commit/61847c43253572e53c6ed25fe208f4eb19fd1b87)** to **[`aac86a2`](https://github.com/sagemath/sagetrac-mirror/commit/aac86a20bbb84df905e4000dfb79f125247cf85b)**",
    "created_at": "2014-11-29T15:34:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137534",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`61847c4`](https://github.com/sagemath/sagetrac-mirror/commit/61847c43253572e53c6ed25fe208f4eb19fd1b87)** to **[`aac86a2`](https://github.com/sagemath/sagetrac-mirror/commit/aac86a20bbb84df905e4000dfb79f125247cf85b)**



---

archive/issue_comments_137535.json:
```json
{
    "body": "Changed dependencies from **#12289** to **#12289, #17130**",
    "created_at": "2014-11-29T15:35:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137535",
    "user": "https://github.com/rwst"
}
```

Changed dependencies from **#12289** to **#12289, #17130**



---

archive/issue_comments_137536.json:
```json
{
    "body": "<div id=\"comment:74\" align=\"right\">comment:74</div>\n\nFr\u00e9d\u00e9ric, how much of this did you positively review up to now?  In principle you and rws can review each other's contributions here.  It would take me a while to get up to speed here.",
    "created_at": "2015-02-03T02:47:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137536",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:74" align="right">comment:74</div>

Frédéric, how much of this did you positively review up to now?  In principle you and rws can review each other's contributions here.  It would take me a while to get up to speed here.



---

archive/issue_comments_137537.json:
```json
{
    "body": "<div id=\"comment:75\" align=\"right\">comment:75</div>\n\na typo in line 5 of the doc:\n\n```\nAiry functions are solutions to the differential equation `f''(z) +f(z)x=0`.\n```\nThe differential equation is wrong, should be `f'' - x f = 0`",
    "created_at": "2015-03-10T20:13:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137537",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:75" align="right">comment:75</div>

a typo in line 5 of the doc:

```
Airy functions are solutions to the differential equation `f''(z) +f(z)x=0`.
```
The differential equation is wrong, should be `f'' - x f = 0`



---

archive/issue_comments_137538.json:
```json
{
    "body": "Changed commit from **[`aac86a2`](https://github.com/sagemath/sagetrac-mirror/commit/aac86a20bbb84df905e4000dfb79f125247cf85b)** to **[`070e728`](https://github.com/sagemath/sagetrac-mirror/commit/070e72855b15c138e9d22ae8325633d84ef49f69)**",
    "created_at": "2015-03-11T13:58:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137538",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`aac86a2`](https://github.com/sagemath/sagetrac-mirror/commit/aac86a20bbb84df905e4000dfb79f125247cf85b)** to **[`070e728`](https://github.com/sagemath/sagetrac-mirror/commit/070e72855b15c138e9d22ae8325633d84ef49f69)**



---

archive/issue_comments_137539.json:
```json
{
    "body": "<div id=\"comment:76\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3cac77521299220649d5c4b530bf72463761033f\"><code>3cac775</code></a></td><td><code>Merge branch 'develop' into t/12455/public/ticket/12455</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/070e72855b15c138e9d22ae8325633d84ef49f69\"><code>070e728</code></a></td><td><code>12455: doc typos</code></td></tr></table>\n",
    "created_at": "2015-03-11T13:58:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137539",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:76"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3cac77521299220649d5c4b530bf72463761033f"><code>3cac775</code></a></td><td><code>Merge branch 'develop' into t/12455/public/ticket/12455</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/070e72855b15c138e9d22ae8325633d84ef49f69"><code>070e728</code></a></td><td><code>12455: doc typos</code></td></tr></table>




---

archive/issue_events_168805.json:
```json
{
    "actor": "https://github.com/sagetrac-olazo",
    "created_at": "2015-03-11T13:59:20Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "subject": "https://github.com/rwst",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168805"
}
```



---

archive/issue_events_168806.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-03-11T13:59:20Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "subject": "https://github.com/rwst",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168806"
}
```



---

archive/issue_events_168807.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-03-11T13:59:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168807"
}
```



---

archive/issue_events_168808.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-03-11T13:59:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168808"
}
```



---

archive/issue_comments_137540.json:
```json
{
    "body": "<div id=\"comment:78\" align=\"right\">comment:78</div>\n\nx should be z in the differential equation",
    "created_at": "2015-03-12T10:11:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137540",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:78" align="right">comment:78</div>

x should be z in the differential equation



---

archive/issue_comments_137541.json:
```json
{
    "body": "<div id=\"comment:79\" align=\"right\">comment:79</div>\n\nWhat exactly still needs review here?\n\nThe code looks pretty good to me overall, but there are lots of things I don't understand in detail. (For a simple example, what is the motivation for splitting the implementation in so many unrelated classes?) And yet I'm tempted to give this ticket positive review (leaving possible remaining issues for later), since many other people have looked at it and it is clearly an improvement over what sage currently has.",
    "created_at": "2015-03-12T10:45:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137541",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:79" align="right">comment:79</div>

What exactly still needs review here?

The code looks pretty good to me overall, but there are lots of things I don't understand in detail. (For a simple example, what is the motivation for splitting the implementation in so many unrelated classes?) And yet I'm tempted to give this ticket positive review (leaving possible remaining issues for later), since many other people have looked at it and it is clearly an improvement over what sage currently has.



---

archive/issue_comments_137542.json:
```json
{
    "body": "<div id=\"comment:80\" align=\"right\">comment:80</div>\n\nMore nitpicking, not necessarily for this ticket:\n* Raising a `ValueError` in `FunctionAiryAiGeneral._derivative_` when `diff_param == 0` may not be the most appropriate, since (if I'm not mistaken) the partial derivative would make sense mathematically.\n* Is it necessary to special-case `alpha == 0` etc. in `FunctionAiryAiGeneral._evalf_`?",
    "created_at": "2015-03-12T11:12:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137542",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:80" align="right">comment:80</div>

More nitpicking, not necessarily for this ticket:
* Raising a `ValueError` in `FunctionAiryAiGeneral._derivative_` when `diff_param == 0` may not be the most appropriate, since (if I'm not mistaken) the partial derivative would make sense mathematically.
* Is it necessary to special-case `alpha == 0` etc. in `FunctionAiryAiGeneral._evalf_`?



---

archive/issue_comments_137543.json:
```json
{
    "body": "<div id=\"comment:81\" align=\"right\">comment:81</div>\n\nReplying to [@mezzarobba](#comment%3A80):\n> * Is it necessary to special-case `alpha == 0` etc. in `FunctionAiryAiGeneral._evalf_`?\n\nScratch that, I misread the code.",
    "created_at": "2015-03-12T12:26:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137543",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:81" align="right">comment:81</div>

Replying to [@mezzarobba](#comment%3A80):
> * Is it necessary to special-case `alpha == 0` etc. in `FunctionAiryAiGeneral._evalf_`?

Scratch that, I misread the code.



---

archive/issue_comments_137544.json:
```json
{
    "body": "Changed commit from **[`070e728`](https://github.com/sagemath/sagetrac-mirror/commit/070e72855b15c138e9d22ae8325633d84ef49f69)** to **[`91530d0`](https://github.com/sagemath/sagetrac-mirror/commit/91530d08a7a37bc03876962c1a1ae0bca10bfd70)**",
    "created_at": "2015-03-12T12:42:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137544",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`070e728`](https://github.com/sagemath/sagetrac-mirror/commit/070e72855b15c138e9d22ae8325633d84ef49f69)** to **[`91530d0`](https://github.com/sagemath/sagetrac-mirror/commit/91530d08a7a37bc03876962c1a1ae0bca10bfd70)**



---

archive/issue_comments_137545.json:
```json
{
    "body": "<div id=\"comment:82\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/91530d08a7a37bc03876962c1a1ae0bca10bfd70\"><code>91530d0</code></a></td><td><code>Ai/Bi: doc fixes and clarifications</code></td></tr></table>\n",
    "created_at": "2015-03-12T12:42:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137545",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:82"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/91530d08a7a37bc03876962c1a1ae0bca10bfd70"><code>91530d0</code></a></td><td><code>Ai/Bi: doc fixes and clarifications</code></td></tr></table>




---

archive/issue_comments_137546.json:
```json
{
    "body": "<div id=\"comment:83\" align=\"right\">comment:83</div>\n\nOne last thing: IMO `hold_derivative` should be `True` by default in `airy_ai` and `airy_bi`, except perhaps when the differentiation order is one. But I don't know if that's consistent with the way things are done in other parts of sage symbolics. Thoughts?",
    "created_at": "2015-03-12T13:22:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137546",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:83" align="right">comment:83</div>

One last thing: IMO `hold_derivative` should be `True` by default in `airy_ai` and `airy_bi`, except perhaps when the differentiation order is one. But I don't know if that's consistent with the way things are done in other parts of sage symbolics. Thoughts?



---

archive/issue_comments_137547.json:
```json
{
    "body": "<div id=\"comment:84\" align=\"right\">comment:84</div>\n\nReplying to [@mezzarobba](#comment%3A80):\n> x should be z in the differential equation\n\nStrange typo.\n> * Raising a `ValueError` in `FunctionAiryAiGeneral._derivative_` when `diff_param == 0` may not be the most appropriate, since (if I'm not mistaken) the partial derivative would make sense mathematically.\n\nA quick look at old and new code shows 1. `ValueError`, 2. `NotImplementedError`, 3. `assert`. So, this needs to be unified to `NotImplementedError`.\n> One last thing: IMO `hold_derivative` should be `True` by default in `airy_ai` and `airy_bi`, except perhaps when the differentiation order is one. But I don't know if that's consistent with the way things are done in other parts of sage symbolics. Thoughts?\n\nI believe the code of this ticket is the only one that has such a parameter, so please feel free to improve it.\n\nI have looked at your patch and it's fine, so if you think the rest is OK please set positive.",
    "created_at": "2015-03-12T14:20:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137547",
    "user": "https://github.com/rwst"
}
```

<div id="comment:84" align="right">comment:84</div>

Replying to [@mezzarobba](#comment%3A80):
> x should be z in the differential equation

Strange typo.
> * Raising a `ValueError` in `FunctionAiryAiGeneral._derivative_` when `diff_param == 0` may not be the most appropriate, since (if I'm not mistaken) the partial derivative would make sense mathematically.

A quick look at old and new code shows 1. `ValueError`, 2. `NotImplementedError`, 3. `assert`. So, this needs to be unified to `NotImplementedError`.
> One last thing: IMO `hold_derivative` should be `True` by default in `airy_ai` and `airy_bi`, except perhaps when the differentiation order is one. But I don't know if that's consistent with the way things are done in other parts of sage symbolics. Thoughts?

I believe the code of this ticket is the only one that has such a parameter, so please feel free to improve it.

I have looked at your patch and it's fine, so if you think the rest is OK please set positive.



---

archive/issue_comments_137548.json:
```json
{
    "body": "Changed commit from **[`91530d0`](https://github.com/sagemath/sagetrac-mirror/commit/91530d08a7a37bc03876962c1a1ae0bca10bfd70)** to **[`2f6945a`](https://github.com/sagemath/sagetrac-mirror/commit/2f6945af73219458fb0a2745e2df9e2eea79b055)**",
    "created_at": "2015-03-12T14:57:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137548",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`91530d0`](https://github.com/sagemath/sagetrac-mirror/commit/91530d08a7a37bc03876962c1a1ae0bca10bfd70)** to **[`2f6945a`](https://github.com/sagemath/sagetrac-mirror/commit/2f6945af73219458fb0a2745e2df9e2eea79b055)**



---

archive/issue_comments_137549.json:
```json
{
    "body": "<div id=\"comment:85\"></div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/347cd082fd914f8171ec0fa3eae005e1647cea8d\"><code>347cd08</code></a></td><td><code>#12455 Airy functions: doc fixes and clarifications</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6aaf2da50ac32e193178e6670ab4ed73dbeba46a\"><code>6aaf2da</code></a></td><td><code>#12455 Airy funs: raise NotImplementedError on diff(airy_?i(\u03b1,x),\u03b1)</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c80be63e16da620c6ab51d2153adb89d2b7a4973\"><code>c80be63</code></a></td><td><code>#12455 Airy funs: hold_derivative by default</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2f6945af73219458fb0a2745e2df9e2eea79b055\"><code>2f6945a</code></a></td><td><code>#12455 Airy functions: avoid calling deprecated function</code></td></tr></table>\n",
    "created_at": "2015-03-12T14:57:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137549",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:85"></div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/347cd082fd914f8171ec0fa3eae005e1647cea8d"><code>347cd08</code></a></td><td><code>#12455 Airy functions: doc fixes and clarifications</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6aaf2da50ac32e193178e6670ab4ed73dbeba46a"><code>6aaf2da</code></a></td><td><code>#12455 Airy funs: raise NotImplementedError on diff(airy_?i(α,x),α)</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c80be63e16da620c6ab51d2153adb89d2b7a4973"><code>c80be63</code></a></td><td><code>#12455 Airy funs: hold_derivative by default</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2f6945af73219458fb0a2745e2df9e2eea79b055"><code>2f6945a</code></a></td><td><code>#12455 Airy functions: avoid calling deprecated function</code></td></tr></table>




---

archive/issue_comments_137550.json:
```json
{
    "body": "<div id=\"comment:86\" align=\"right\">comment:86</div>\n\nReplying to [@rwst](#comment%3A84):\n> > * Raising a `ValueError` in `FunctionAiryAiGeneral._derivative_` when `diff_param == 0` may not be the most appropriate, since (if I'm not mistaken) the partial derivative would make sense mathematically.\n> \n> A quick look at old and new code shows 1. `ValueError`, 2. `NotImplementedError`, 3. `assert`. So, this needs to be unified to `NotImplementedError`.\n> > One last thing: IMO `hold_derivative` should be `True` by default in `airy_ai` and `airy_bi`, except perhaps when the differentiation order is one. But I don't know if that's consistent with the way things are done in other parts of sage symbolics. Thoughts?\n> \n> I believe the code of this ticket is the only one that has such a parameter, so please feel free to improve it.\n\nBoth done (now rather than later to avoid breaking the interface).\n\n> I have looked at your patch and it's fine, so if you think the rest is OK please set positive.\n\nI'm okay with everything prior to my commits. Could you (or someone else) have a quick look at my last changes and set the ticket to positive review? Thanks!",
    "created_at": "2015-03-12T15:01:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137550",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:86" align="right">comment:86</div>

Replying to [@rwst](#comment%3A84):
> > * Raising a `ValueError` in `FunctionAiryAiGeneral._derivative_` when `diff_param == 0` may not be the most appropriate, since (if I'm not mistaken) the partial derivative would make sense mathematically.
> 
> A quick look at old and new code shows 1. `ValueError`, 2. `NotImplementedError`, 3. `assert`. So, this needs to be unified to `NotImplementedError`.
> > One last thing: IMO `hold_derivative` should be `True` by default in `airy_ai` and `airy_bi`, except perhaps when the differentiation order is one. But I don't know if that's consistent with the way things are done in other parts of sage symbolics. Thoughts?
> 
> I believe the code of this ticket is the only one that has such a parameter, so please feel free to improve it.

Both done (now rather than later to avoid breaking the interface).

> I have looked at your patch and it's fine, so if you think the rest is OK please set positive.

I'm okay with everything prior to my commits. Could you (or someone else) have a quick look at my last changes and set the ticket to positive review? Thanks!



---

archive/issue_comments_137551.json:
```json
{
    "body": "Changed reviewer from **Eviatar Bach, Karl-Dieter Crisman, Burcin Erocal, Ralf Stephan** to **Eviatar Bach, Karl-Dieter Crisman, Burcin Erocal, Ralf Stephan, Jeroen Demeyer, Marc Mezzarobba**",
    "created_at": "2015-03-12T15:01:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137551",
    "user": "https://github.com/mezzarobba"
}
```

Changed reviewer from **Eviatar Bach, Karl-Dieter Crisman, Burcin Erocal, Ralf Stephan** to **Eviatar Bach, Karl-Dieter Crisman, Burcin Erocal, Ralf Stephan, Jeroen Demeyer, Marc Mezzarobba**



---

archive/issue_comments_137552.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -11,26 +11,7 @@\n \n ```\n sage: airy_ai(x)\n----------------------------------------------------------------------------\n-TypeError                                 Traceback (most recent call last)\n-\n-/home/oscar/Escritorio/tesis/calculos/<ipython console> in <module>()\n-\n-/home/oscar/sage/sage-4.7.1/local/lib/python2.6/site-packages/sage/functions/special.pyc in airy_ai(x)\n-    621    \"\"\"\n-    622    _init()\n---> 623    return RDF(meval(\"airy_ai(%s)\"%RDF(x)))\n-    624 \n-    625 def airy_bi(x):\n-\n-/home/oscar/sage/sage-4.7.1/local/lib/python2.6/site-packages/sage/structure/parent.so in sage.structure.parent.Parent.__call__ (sage/structure/parent.c:7102)()\n-\n-/home/oscar/sage/sage-4.7.1/local/lib/python2.6/site-packages/sage/structure/coerce_maps.so in sage.structure.coerce_maps.NamedConvertMap._call_ (sage/structure/coerce_maps.c:4221)()\n-\n-/home/oscar/sage/sage-4.7.1/local/lib/python2.6/site-packages/sage/symbolic/expression.so in sage.symbolic.expression.Expression._real_double_ (sage/symbolic/expression.cpp:5391)()\n-\n-/home/oscar/sage/sage-4.7.1/local/lib/python2.6/site-packages/sage/symbolic/expression.so in sage.symbolic.expression.Expression._eval_self (sage/symbolic/expression.cpp:4898)()\n-\n+...\n TypeError: Cannot evaluate symbolic expression to a numeric value.\n ```\n \n``````\n",
    "created_at": "2015-03-12T15:01:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137552",
    "user": "https://github.com/mezzarobba"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -11,26 +11,7 @@
 
 ```
 sage: airy_ai(x)
----------------------------------------------------------------------------
-TypeError                                 Traceback (most recent call last)
-
-/home/oscar/Escritorio/tesis/calculos/<ipython console> in <module>()
-
-/home/oscar/sage/sage-4.7.1/local/lib/python2.6/site-packages/sage/functions/special.pyc in airy_ai(x)
-    621    """
-    622    _init()
---> 623    return RDF(meval("airy_ai(%s)"%RDF(x)))
-    624 
-    625 def airy_bi(x):
-
-/home/oscar/sage/sage-4.7.1/local/lib/python2.6/site-packages/sage/structure/parent.so in sage.structure.parent.Parent.__call__ (sage/structure/parent.c:7102)()
-
-/home/oscar/sage/sage-4.7.1/local/lib/python2.6/site-packages/sage/structure/coerce_maps.so in sage.structure.coerce_maps.NamedConvertMap._call_ (sage/structure/coerce_maps.c:4221)()
-
-/home/oscar/sage/sage-4.7.1/local/lib/python2.6/site-packages/sage/symbolic/expression.so in sage.symbolic.expression.Expression._real_double_ (sage/symbolic/expression.cpp:5391)()
-
-/home/oscar/sage/sage-4.7.1/local/lib/python2.6/site-packages/sage/symbolic/expression.so in sage.symbolic.expression.Expression._eval_self (sage/symbolic/expression.cpp:4898)()
-
+...
 TypeError: Cannot evaluate symbolic expression to a numeric value.
 ```
 
``````




---

archive/issue_events_168809.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-03-12T15:14:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168809"
}
```



---

archive/issue_events_168810.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-03-12T15:14:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168810"
}
```



---

archive/issue_comments_137553.json:
```json
{
    "body": "<div id=\"comment:87\" align=\"right\">comment:87</div>\n\nIs fine and passes tests in `functions/`. Thank you too.",
    "created_at": "2015-03-12T15:14:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137553",
    "user": "https://github.com/rwst"
}
```

<div id="comment:87" align="right">comment:87</div>

Is fine and passes tests in `functions/`. Thank you too.



---

archive/issue_events_168811.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-03-12T23:06:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168811"
}
```



---

archive/issue_events_168812.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "34e190b5dfde4b124775424254b7b272c255d6a5",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-03-12T23:06:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12455#event-168812"
}
```



---

archive/issue_comments_137554.json:
```json
{
    "body": "Changed branch from **[public/ticket/12455](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/12455)** to **[`2f6945a`](https://github.com/sagemath/sagetrac-mirror/commit/2f6945af73219458fb0a2745e2df9e2eea79b055)**",
    "created_at": "2015-03-12T23:06:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137554",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/ticket/12455](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/12455)** to **[`2f6945a`](https://github.com/sagemath/sagetrac-mirror/commit/2f6945af73219458fb0a2745e2df9e2eea79b055)**



---

archive/issue_comments_137555.json:
```json
{
    "body": "Changed author from **Oscar Gerardo Lazo Arjona, Benjamin Jones, D. S. McNeil, Eviatar Bach, Ralf Stephan** to **Oscar Gerardo Lazo Arjona, Benjamin Jones, Douglas McNeil, Eviatar Bach, Ralf Stephan**",
    "created_at": "2015-05-27T12:06:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137555",
    "user": "https://github.com/kcrisman"
}
```

Changed author from **Oscar Gerardo Lazo Arjona, Benjamin Jones, D. S. McNeil, Eviatar Bach, Ralf Stephan** to **Oscar Gerardo Lazo Arjona, Benjamin Jones, Douglas McNeil, Eviatar Bach, Ralf Stephan**



---

archive/issue_comments_137556.json:
```json
{
    "body": "Changed commit from **[`2f6945a`](https://github.com/sagemath/sagetrac-mirror/commit/2f6945af73219458fb0a2745e2df9e2eea79b055)** to none",
    "created_at": "2015-05-27T12:06:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12455#issuecomment-137556",
    "user": "https://github.com/kcrisman"
}
```

Changed commit from **[`2f6945a`](https://github.com/sagemath/sagetrac-mirror/commit/2f6945af73219458fb0a2745e2df9e2eea79b055)** to none

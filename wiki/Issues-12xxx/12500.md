# Issue 12500: sagetex: fix self-tests on OpenSolaris

archive/issues_012328.json:
```json
{
    "body": "On OpenSolaris, sagetex's self-tests fail.  New spkg:\n\n[http://sage.math.washington.edu/home/palmieri/SPKG/sagetex-2.3.3.p2.spkg](http://sage.math.washington.edu/home/palmieri/SPKG/sagetex-2.3.3.p2.spkg)\n\n\n**CC:**  @dandrake\n\n**Author:** John Palmieri\n\n**Reviewer:** Jeroen Demeyer\n\n**Merged:** sage-5.0.beta4\n\nIssue created by migration from https://trac.sagemath.org/ticket/12500\n\n",
    "closed_at": "2012-02-12T09:33:31Z",
    "created_at": "2012-02-11T23:54:21Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "title": "sagetex: fix self-tests on OpenSolaris",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/12500",
    "user": "https://github.com/jhpalmieri"
}
```
On OpenSolaris, sagetex's self-tests fail.  New spkg:

[http://sage.math.washington.edu/home/palmieri/SPKG/sagetex-2.3.3.p2.spkg](http://sage.math.washington.edu/home/palmieri/SPKG/sagetex-2.3.3.p2.spkg)


**CC:**  @dandrake

**Author:** John Palmieri

**Reviewer:** Jeroen Demeyer

**Merged:** sage-5.0.beta4

Issue created by migration from https://trac.sagemath.org/ticket/12500





---

archive/issue_comments_140436.json:
```json
{
    "body": "patch for sagetex spkg; for reference only",
    "created_at": "2012-02-11T23:56:54Z",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12500#issuecomment-140436",
    "user": "https://github.com/jhpalmieri"
}
```

patch for sagetex spkg; for reference only



---

archive/attachments_017400.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_12500-sagetex.patch",
    "asset_url": "tarball://root/attachments/ticket12500/trac_12500-sagetex.patch",
    "created_at": "2012-02-11T23:57:17Z",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket12500/trac_12500-sagetex.patch",
    "user": "https://github.com/jhpalmieri"
}
```



---

archive/issue_comments_140437.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_12500-sagetex.patch](https://github.com/sagemath/sage/files/ticket12500/trac_12500-sagetex.patch)",
    "created_at": "2012-02-11T23:57:17Z",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12500#issuecomment-140437",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_12500-sagetex.patch](https://github.com/sagemath/sage/files/ticket12500/trac_12500-sagetex.patch)



---

archive/issue_events_103510.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-11T23:57:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12500#event-103510"
}
```



---

archive/issue_comments_140438.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2012-02-12T09:33:31Z",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12500#issuecomment-140438",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_events_103511.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-12T09:33:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12500#event-103511"
}
```



---

archive/issue_events_103512.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-12T09:33:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12500#event-103512"
}
```



---

archive/issue_comments_140439.json:
```json
{
    "body": "**Merged:** sage-5.0.beta4",
    "created_at": "2012-02-12T09:33:31Z",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12500#issuecomment-140439",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.0.beta4



---

archive/issue_comments_140440.json:
```json
{
    "body": "<a id='comment:3'></a>\nThanks for noticing this. I've pulled your changes into my spkg repo.",
    "created_at": "2012-02-13T02:41:30Z",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12500#issuecomment-140440",
    "user": "https://github.com/dandrake"
}
```

<a id='comment:3'></a>
Thanks for noticing this. I've pulled your changes into my spkg repo.

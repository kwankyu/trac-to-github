# Issue 12829: Remove <object> cast in real_mpfi.pyx

archive/issues_012657.json:
```json
{
    "body": "The following is very wrong code (from `sage/rings/real_mpfi.pyx`)\n\n```\n# todo: The <object> coerce is evidently to get around a weird bug in SageX (?)   \nmpfi_interv_q(self.value, <object>rat.value, <object>rat1.value) \n```\n\nSurprisingly, it only breaks on PPC64.  See #11705 for more context.\n\n**Assignee:** drkirkby\n\n**Keywords:** ppc64 big endian mpfi\n\n**Reviewer:** Fran\u00e7ois Bissey\n\n**Author:** Paul Zimmermann, Jeroen Demeyer\n\n**Merged:** sage-5.0.beta14\n\nIssue created by migration from https://trac.sagemath.org/ticket/12829\n\n",
    "closed_at": "2012-04-19T06:39:50Z",
    "created_at": "2012-04-11T08:20:38Z",
    "labels": [
        "component: porting",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "title": "Remove <object> cast in real_mpfi.pyx",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/12829",
    "user": "https://github.com/jdemeyer"
}
```
The following is very wrong code (from `sage/rings/real_mpfi.pyx`)

```
# todo: The <object> coerce is evidently to get around a weird bug in SageX (?)   
mpfi_interv_q(self.value, <object>rat.value, <object>rat1.value) 
```

Surprisingly, it only breaks on PPC64.  See #11705 for more context.

**Assignee:** drkirkby

**Keywords:** ppc64 big endian mpfi

**Reviewer:** François Bissey

**Author:** Paul Zimmermann, Jeroen Demeyer

**Merged:** sage-5.0.beta14

Issue created by migration from https://trac.sagemath.org/ticket/12829





---

archive/attachments_017896.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "12829_mpfi.patch",
    "asset_url": "tarball://root/attachments/ticket12829/12829_mpfi.patch",
    "created_at": "2012-04-11T08:22:47Z",
    "issue": "https://github.com/sagemath/sage/issues/12829",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket12829/12829_mpfi.patch",
    "user": "https://github.com/jdemeyer"
}
```



---

archive/issue_comments_146644.json:
```json
{
    "body": "**Attachment:** [12829_mpfi.patch](https://github.com/sagemath/sage/files/ticket12829/12829_mpfi.patch)",
    "created_at": "2012-04-11T08:22:47Z",
    "issue": "https://github.com/sagemath/sage/issues/12829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12829#issuecomment-146644",
    "user": "https://github.com/jdemeyer"
}
```

**Attachment:** [12829_mpfi.patch](https://github.com/sagemath/sage/files/ticket12829/12829_mpfi.patch)



---

archive/issue_events_107571.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-11T08:23:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12829",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12829#event-107571"
}
```



---

archive/issue_comments_146645.json:
```json
{
    "body": "<a id='comment:2'></a>\nWorks for me on the power7 box I have access to.",
    "created_at": "2012-04-11T22:18:06Z",
    "issue": "https://github.com/sagemath/sage/issues/12829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12829#issuecomment-146645",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:2'></a>
Works for me on the power7 box I have access to.



---

archive/issue_events_107572.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2012-04-11T22:18:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12829",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12829#event-107572"
}
```



---

archive/issue_events_107573.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2012-04-11T22:18:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12829",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12829#event-107573"
}
```



---

archive/issue_comments_146646.json:
```json
{
    "body": "**Reviewer:** Fran\u00e7ois Bissey",
    "created_at": "2012-04-13T08:04:10Z",
    "issue": "https://github.com/sagemath/sage/issues/12829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12829#issuecomment-146646",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** François Bissey



---

archive/issue_comments_146647.json:
```json
{
    "body": "**Merged:** sage-5.0.beta14",
    "created_at": "2012-04-19T06:39:50Z",
    "issue": "https://github.com/sagemath/sage/issues/12829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12829#issuecomment-146647",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.0.beta14



---

archive/issue_events_107574.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-19T06:39:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12829",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12829#event-107574"
}
```



---

archive/issue_events_107575.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-19T06:39:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12829",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12829#event-107575"
}
```

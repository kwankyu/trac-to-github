# Issue 12352: Add some tests for the Qhull optional package

archive/issues_012180.json:
```json
{
    "body": "At our dear leader's request I am adding a test file for the Qhull optional package.  \n\nThis will be quite minimal, since what I'd really like is to add a Qhull backend for the Polyhedron class, which would come with its own tests.  \n\nIf anyone is interested in adding more elaborate tests it wouldn't be very hard since the documentation at [the Qhull site](http://www.qhull.org/) is pretty good.\n\n**CC:**  @videlec\n\n**Keywords:** Qhull, polyhedra, polyhedron\n\n**Branch:** [public/ticket/12352](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/12352)\n\n**Commit:** [547c5084461b0aad197a7f4316a39efe37398601](https://github.com/sagemath/sagetrac-mirror/commit/547c5084461b0aad197a7f4316a39efe37398601)\n\n**Reviewer:** Fr\u00e9d\u00e9ric Chapoton\n\n**Author:** Fr\u00e9d\u00e9ric Chapoton\n\nIssue created by migration from https://trac.sagemath.org/ticket/12352\n\n",
    "closed_at": "2016-08-30T13:32:25Z",
    "created_at": "2012-01-25T00:51:52Z",
    "labels": [
        "component: packages: optional",
        "minor",
        "enhancement",
        "wontfix"
    ],
    "title": "Add some tests for the Qhull optional package",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/12352",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```
At our dear leader's request I am adding a test file for the Qhull optional package.  

This will be quite minimal, since what I'd really like is to add a Qhull backend for the Polyhedron class, which would come with its own tests.  

If anyone is interested in adding more elaborate tests it wouldn't be very hard since the documentation at [the Qhull site](http://www.qhull.org/) is pretty good.

**CC:**  @videlec

**Keywords:** Qhull, polyhedra, polyhedron

**Branch:** [public/ticket/12352](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/12352)

**Commit:** [547c5084461b0aad197a7f4316a39efe37398601](https://github.com/sagemath/sagetrac-mirror/commit/547c5084461b0aad197a7f4316a39efe37398601)

**Reviewer:** Frédéric Chapoton

**Author:** Frédéric Chapoton

Issue created by migration from https://trac.sagemath.org/ticket/12352





---

archive/attachments_017172.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_12352_Qhull_optional_package_testfile.patch",
    "asset_url": "tarball://root/attachments/ticket12352/trac_12352_Qhull_optional_package_testfile.patch",
    "created_at": "2012-01-25T00:55:01Z",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket12352/trac_12352_Qhull_optional_package_testfile.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```



---

archive/issue_comments_136710.json:
```json
{
    "body": "**Attachment:** [trac_12352_Qhull_optional_package_testfile.patch](https://github.com/sagemath/sage/files/ticket12352/trac_12352_Qhull_optional_package_testfile.patch)\n\nadds a minimal test for the Qhull optional package",
    "created_at": "2012-01-25T00:55:01Z",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12352#issuecomment-136710",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

**Attachment:** [trac_12352_Qhull_optional_package_testfile.patch](https://github.com/sagemath/sage/files/ticket12352/trac_12352_Qhull_optional_package_testfile.patch)

adds a minimal test for the Qhull optional package



---

archive/issue_events_101646.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mhampton",
    "created_at": "2012-01-25T00:55:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101646"
}
```



---

archive/issue_events_101647.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-03-12T16:08:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101647"
}
```



---

archive/issue_events_101648.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-03-12T16:08:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101648"
}
```



---

archive/issue_comments_136711.json:
```json
{
    "body": "<a id='comment:2'></a>\nThe tests need to be marked with \"# optional qhull\" or something, since otherwise they fail when this optional package isn't installed.",
    "created_at": "2012-03-12T16:08:07Z",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12352#issuecomment-136711",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:2'></a>
The tests need to be marked with "# optional qhull" or something, since otherwise they fail when this optional package isn't installed.



---

archive/issue_events_101649.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101649"
}
```



---

archive/issue_events_101650.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101650"
}
```



---

archive/issue_events_101651.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101651"
}
```



---

archive/issue_events_101652.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101652"
}
```



---

archive/issue_events_101653.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101653"
}
```



---

archive/issue_events_101654.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101654"
}
```



---

archive/issue_events_101655.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101655"
}
```



---

archive/issue_events_101656.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101656"
}
```



---

archive/issue_comments_136712.json:
```json
{
    "body": "**Branch:** [public/ticket/12352](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/12352)",
    "created_at": "2014-12-29T07:59:22Z",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12352#issuecomment-136712",
    "user": "https://github.com/fchapoton"
}
```

**Branch:** [public/ticket/12352](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/12352)



---

archive/issue_comments_136713.json:
```json
{
    "body": "<a id='comment:7'></a>\n**New commits:**\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/24a3059ff5c043399954b127692655bb2361a3c6\">24a3059</a></td><td><code>trac #12352: adding some tests for the Qhull optional package</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/547c5084461b0aad197a7f4316a39efe37398601\">547c508</a></td><td><code>trac #12352 refreshed</code></td></tr></table>\n",
    "created_at": "2014-12-29T07:59:22Z",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12352#issuecomment-136713",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:7'></a>
**New commits:**
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/24a3059ff5c043399954b127692655bb2361a3c6">24a3059</a></td><td><code>trac #12352: adding some tests for the Qhull optional package</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/547c5084461b0aad197a7f4316a39efe37398601">547c508</a></td><td><code>trac #12352 refreshed</code></td></tr></table>




---

archive/issue_events_101657.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-12-29T07:59:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101657"
}
```



---

archive/issue_events_101658.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-12-29T07:59:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101658"
}
```



---

archive/issue_comments_136714.json:
```json
{
    "body": "**Commit:** [547c5084461b0aad197a7f4316a39efe37398601](https://github.com/sagemath/sagetrac-mirror/commit/547c5084461b0aad197a7f4316a39efe37398601)",
    "created_at": "2014-12-29T07:59:22Z",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12352#issuecomment-136714",
    "user": "https://github.com/fchapoton"
}
```

**Commit:** [547c5084461b0aad197a7f4316a39efe37398601](https://github.com/sagemath/sagetrac-mirror/commit/547c5084461b0aad197a7f4316a39efe37398601)



---

archive/issue_comments_136715.json:
```json
{
    "body": "<a id='comment:8'></a>\nSince this is just tests and nothing else, why not add this to a `spkg-check` file inside the `qhull` package?",
    "created_at": "2015-04-09T08:23:17Z",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12352#issuecomment-136715",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'></a>
Since this is just tests and nothing else, why not add this to a `spkg-check` file inside the `qhull` package?



---

archive/issue_events_101659.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-08-16T10:54:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101659"
}
```



---

archive/issue_events_101660.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-08-16T10:54:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "label": "needs info",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101660"
}
```



---

archive/issue_comments_136716.json:
```json
{
    "body": "<a id='comment:9'></a>\nReplying to [@jdemeyer](#comment%3A8):\n> Since this is just tests and nothing else, why not add this to a `spkg-check` file inside the `qhull` package?\n\n+1. There is no interface being tested or so.\n\nDoesn't `qhull` have a testsuite?",
    "created_at": "2015-08-16T10:54:01Z",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12352#issuecomment-136716",
    "user": "https://github.com/videlec"
}
```

<a id='comment:9'></a>
Replying to [@jdemeyer](#comment%3A8):
> Since this is just tests and nothing else, why not add this to a `spkg-check` file inside the `qhull` package?

+1. There is no interface being tested or so.

Doesn't `qhull` have a testsuite?



---

archive/issue_comments_136717.json:
```json
{
    "body": "<a id='comment:10'></a>\nMarking this as \"wontfix\". The spkg-check of the qhull package (#21143) seems enough.",
    "created_at": "2016-08-03T15:51:16Z",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12352#issuecomment-136717",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:10'></a>
Marking this as "wontfix". The spkg-check of the qhull package (#21143) seems enough.



---

archive/issue_events_101661.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-08-03T15:51:16Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101661"
}
```



---

archive/issue_events_101662.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-08-03T15:51:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "label": "needs info",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101662"
}
```



---

archive/issue_events_101663.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-08-03T15:51:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101663"
}
```



---

archive/issue_comments_136718.json:
```json
{
    "body": "**Changing author** from \"Marshall Hampton\" to \"Fr\u00e9d\u00e9ric Chapoton\".",
    "created_at": "2016-08-08T13:35:16Z",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12352#issuecomment-136718",
    "user": "https://github.com/fchapoton"
}
```

**Changing author** from "Marshall Hampton" to "Frédéric Chapoton".



---

archive/issue_comments_136719.json:
```json
{
    "body": "**Reviewer:** Fr\u00e9d\u00e9ric Chapoton",
    "created_at": "2016-08-08T13:35:16Z",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12352#issuecomment-136719",
    "user": "https://github.com/fchapoton"
}
```

**Reviewer:** Frédéric Chapoton



---

archive/issue_events_101664.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-08T13:35:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101664"
}
```



---

archive/issue_events_101665.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-08T13:35:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101665"
}
```



---

archive/issue_comments_136720.json:
```json
{
    "body": "<a id='comment:12'></a>\nDetermined to be invalid/duplicate/wontfix (closing as \"wontfix\" as a catch-all resolution).",
    "created_at": "2016-08-30T13:32:25Z",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12352#issuecomment-136720",
    "user": "https://github.com/embray"
}
```

<a id='comment:12'></a>
Determined to be invalid/duplicate/wontfix (closing as "wontfix" as a catch-all resolution).



---

archive/issue_events_101666.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101666"
}
```



---

archive/issue_events_101667.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101667"
}
```



---

archive/issue_events_101668.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12352",
    "label": "wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12352#event-101668"
}
```

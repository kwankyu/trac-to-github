# Issue 12451: Syntax highlighting for notebook debugger

archive/issues_012279.json:
```json
{
    "body": "This adds syntax highlighting for the notebook debugger of ticket #7509.\n\nDepends on patch [attachment:trac_7509.5.patch:ticket:7509.](https://github.com/sagemath/sage/files/ticket12451/c86dfb5234ffb9c97dede7c4ca837f00)\n\nFor reasons of speed only a small part of the source code around the current position is loaded and highlighted. We detect when the\ncode fragment starts in the middle of a docstring to ensure correct\nhighlighting in this case.\n\n**Assignee:** @jasongrout, @qed777, @williamstein\n\n**CC:**  @fchapoton\n\n**Author:** Wilfried Huss\n\nIssue created by migration from https://trac.sagemath.org/ticket/12451\n\n",
    "closed_at": "2020-09-02T07:48:41Z",
    "created_at": "2012-02-06T11:17:12Z",
    "labels": [
        "component: notebook",
        "minor",
        "enhancement",
        "invalid"
    ],
    "title": "Syntax highlighting for notebook debugger",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/12451",
    "user": "https://trac.sagemath.org/admin/accounts/users/whuss"
}
```
This adds syntax highlighting for the notebook debugger of ticket #7509.

Depends on patch [attachment:trac_7509.5.patch:ticket:7509.](https://github.com/sagemath/sage/files/ticket12451/c86dfb5234ffb9c97dede7c4ca837f00)

For reasons of speed only a small part of the source code around the current position is loaded and highlighted. We detect when the
code fragment starts in the middle of a docstring to ensure correct
highlighting in this case.

**Assignee:** @jasongrout, @qed777, @williamstein

**CC:**  @fchapoton

**Author:** Wilfried Huss

Issue created by migration from https://trac.sagemath.org/ticket/12451





---

archive/attachments_017327.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_12451.patch",
    "asset_url": "tarball://root/attachments/ticket12451/trac_12451.patch",
    "created_at": "2012-02-06T11:18:39Z",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket12451/trac_12451.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/whuss"
}
```



---

archive/issue_comments_139580.json:
```json
{
    "body": "**Attachment:** [trac_12451.patch](https://github.com/sagemath/sage/files/ticket12451/trac_12451.patch)",
    "created_at": "2012-02-06T11:18:39Z",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12451#issuecomment-139580",
    "user": "https://trac.sagemath.org/admin/accounts/users/whuss"
}
```

**Attachment:** [trac_12451.patch](https://github.com/sagemath/sage/files/ticket12451/trac_12451.patch)



---

archive/issue_events_102980.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/whuss",
    "created_at": "2012-02-06T11:28:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-102980"
}
```



---

archive/issue_comments_139581.json:
```json
{
    "body": "<a id='comment:2'></a>\nPlease fill in your real name as Author.",
    "created_at": "2012-07-27T20:41:11Z",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12451#issuecomment-139581",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'></a>
Please fill in your real name as Author.



---

archive/issue_comments_139582.json:
```json
{
    "body": "**Author:** Wilfried Huss",
    "created_at": "2012-07-30T08:57:03Z",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12451#issuecomment-139582",
    "user": "https://trac.sagemath.org/admin/accounts/users/whuss"
}
```

**Author:** Wilfried Huss



---

archive/issue_comments_139583.json:
```json
{
    "body": "<a id='comment:4'></a>\nNeeds rebase, and probably an upstream request...",
    "created_at": "2013-06-18T20:06:25Z",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12451#issuecomment-139583",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:4'></a>
Needs rebase, and probably an upstream request...



---

archive/issue_events_102981.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-18T20:06:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-102981"
}
```



---

archive/issue_events_102982.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-18T20:06:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-102982"
}
```



---

archive/issue_events_102983.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-102983"
}
```



---

archive/issue_events_102984.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-102984"
}
```



---

archive/issue_events_102985.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-102985"
}
```



---

archive/issue_events_102986.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-102986"
}
```



---

archive/issue_events_102987.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-102987"
}
```



---

archive/issue_events_102988.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-102988"
}
```



---

archive/issue_events_102989.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-102989"
}
```



---

archive/issue_events_102990.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-102990"
}
```



---

archive/issue_events_102991.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-18T00:36:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-102991"
}
```



---

archive/issue_events_102992.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-18T00:36:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-102992"
}
```



---

archive/issue_comments_139584.json:
```json
{
    "body": "<a id='comment:9'></a>\nProposing to close all sagenb tickets as outdated, so that all remaining open tickets in the notebook component are about the Jupyter notebook.",
    "created_at": "2020-08-18T00:36:52Z",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12451#issuecomment-139584",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:9'></a>
Proposing to close all sagenb tickets as outdated, so that all remaining open tickets in the notebook component are about the Jupyter notebook.



---

archive/issue_events_102993.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-18T00:36:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-102993"
}
```



---

archive/issue_events_102994.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-09-02T07:48:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-102994"
}
```



---

archive/issue_events_102995.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-09-02T07:48:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-102995"
}
```

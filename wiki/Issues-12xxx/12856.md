# Issue 12856: multiline doctests starting with '....:'

archive/issues_012684.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nSince the Sage prompt for multiline commands is `....:`, this should also work for doctests. At the moment you have to use the python `...` prompt for doctests.\n\nApply [attachment: trac_12856_multiline_doctests.patch](https://github.com/sagemath/sage/files/ticket12856/trac_12856_multiline_doctests.patch.gz) to the scripts repo.\n\nComponent: **doctest coverage**\n\nKeywords: **multiline doctest**\n\nReviewer: **Daniel Krenn**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12856_\n\n",
    "closed_at": "2013-02-08T13:24:11Z",
    "created_at": "2012-04-18T21:42:42Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "multiline doctests starting with '....:'",
    "type": "issue",
    "updated_at": "2013-02-08T13:24:11Z",
    "url": "https://github.com/sagemath/sage/issues/12856",
    "user": "https://github.com/dkrenn"
}
```
<div id="comment:0"></div>

Since the Sage prompt for multiline commands is `....:`, this should also work for doctests. At the moment you have to use the python `...` prompt for doctests.

Apply [attachment: trac_12856_multiline_doctests.patch](https://github.com/sagemath/sage/files/ticket12856/trac_12856_multiline_doctests.patch.gz) to the scripts repo.

Component: **doctest coverage**

Keywords: **multiline doctest**

Reviewer: **Daniel Krenn**

_Issue created by migration from https://trac.sagemath.org/ticket/12856_





---

archive/issue_events_175753.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2012-04-18T21:42:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12856#event-175753"
}
```



---

archive/issue_events_175754.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2012-04-18T21:42:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12856#event-175754"
}
```



---

archive/issue_events_175755.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2012-04-18T21:42:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12856#event-175755"
}
```



---

archive/issue_events_175756.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2012-04-18T21:42:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12856#event-175756"
}
```



---

archive/issue_events_175757.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2012-04-18T21:42:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12856#event-175757"
}
```



---

archive/issue_events_175758.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2012-04-18T21:42:42Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "subject": "https://github.com/dkrenn",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12856#event-175758"
}
```



---

archive/issue_comments_144942.json:
```json
{
    "body": "Attachment: **[trac_12856_multiline_doctests.patch.gz](https://github.com/sagemath/sage/files/ticket12856/trac_12856_multiline_doctests.patch.gz)**",
    "created_at": "2012-04-18T21:44:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12856#issuecomment-144942",
    "user": "https://github.com/dkrenn"
}
```

Attachment: **[trac_12856_multiline_doctests.patch.gz](https://github.com/sagemath/sage/files/ticket12856/trac_12856_multiline_doctests.patch.gz)**



---

archive/issue_comments_144943.json:
```json
{
    "body": "Author: **Daniel Krenn**",
    "created_at": "2012-04-18T21:46:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12856#issuecomment-144943",
    "user": "https://github.com/dkrenn"
}
```

Author: **Daniel Krenn**



---

archive/issue_events_175759.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2012-04-18T21:46:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12856#event-175759"
}
```



---

archive/issue_comments_144944.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n Since the Sage prompt for multiline commands is `....:`, this should also work for doctests. At the moment you have to use the python `...` prompt for doctests.\n+\n+Apply [attachment: trac_12856_multiline_doctests.patch](https://github.com/sagemath/sage/files/ticket12856/trac_12856_multiline_doctests.patch.gz) to the scripts repo.\n``````\n",
    "created_at": "2012-04-18T21:46:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12856#issuecomment-144944",
    "user": "https://github.com/dkrenn"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 Since the Sage prompt for multiline commands is `....:`, this should also work for doctests. At the moment you have to use the python `...` prompt for doctests.
+
+Apply [attachment: trac_12856_multiline_doctests.patch](https://github.com/sagemath/sage/files/ticket12856/trac_12856_multiline_doctests.patch.gz) to the scripts repo.
``````




---

archive/issue_comments_144945.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nPatch [attachment: trac_12856_multiline_doctests.patch](https://github.com/sagemath/sage/files/ticket12856/trac_12856_multiline_doctests.patch.gz) is built on 5.0.beta13.",
    "created_at": "2012-04-18T21:50:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12856#issuecomment-144945",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:2" align="right">comment:2</div>

Patch [attachment: trac_12856_multiline_doctests.patch](https://github.com/sagemath/sage/files/ticket12856/trac_12856_multiline_doctests.patch.gz) is built on 5.0.beta13.



---

archive/issue_comments_144946.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nSee also #10458 and #12415. (I suspect this and/or #10458 would need to be rebased on #12415 since the latter rewrites much of the doctest framework.)",
    "created_at": "2012-05-14T13:09:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12856#issuecomment-144946",
    "user": "https://github.com/kini"
}
```

<div id="comment:3" align="right">comment:3</div>

See also #10458 and #12415. (I suspect this and/or #10458 would need to be rebased on #12415 since the latter rewrites much of the doctest framework.)



---

archive/issue_comments_144947.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nActually I guess this ticket is pretty much a duplicate of #10458...",
    "created_at": "2012-05-14T13:10:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12856#issuecomment-144947",
    "user": "https://github.com/kini"
}
```

<div id="comment:4" align="right">comment:4</div>

Actually I guess this ticket is pretty much a duplicate of #10458...



---

archive/issue_events_175760.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-05T19:29:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12856#event-175760"
}
```



---

archive/issue_events_175761.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-05T19:29:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12856#event-175761"
}
```



---

archive/issue_comments_144948.json:
```json
{
    "body": "Reviewer: **Daniel Krenn**",
    "created_at": "2013-02-05T19:29:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12856#issuecomment-144948",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Daniel Krenn**



---

archive/issue_events_175762.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-05T19:29:50Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12856#event-175762"
}
```



---

archive/issue_comments_144949.json:
```json
{
    "body": "Changed author from **Daniel Krenn** to none",
    "created_at": "2013-02-05T19:29:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12856#issuecomment-144949",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Daniel Krenn** to none



---

archive/issue_events_175763.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-08T13:24:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12856#event-175763"
}
```



---

archive/issue_events_175764.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-08T13:24:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12856#event-175764"
}
```



---

archive/issue_events_175765.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-08T13:24:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12856#event-175765"
}
```



---

archive/issue_events_175766.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-08T13:24:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12856",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "c6c6c6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12856#event-175766"
}
```

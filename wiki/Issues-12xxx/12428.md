# Issue 12428: make flintqs respect global CXX flag

archive/issues_012256.json:
```json
{
    "body": "The flintqs spkg currently does not respect the global CXX flag, we should fix this (for instance for #12426).\n\nI've posted an spkg at [http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/flintqs-20070817.p7.spkg](http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/flintqs-20070817.p7.spkg).\n\nSee also #11792.\n\n**Assignee:** GeorgSWeber\n\n**Reviewer:** Julien Puydt, Leif Leonhardy, Fran\u00e7ois Bissey\n\n**Author:** R. Andrew Ohana\n\nIssue created by migration from https://trac.sagemath.org/ticket/12428\n\n",
    "closed_at": "2017-01-21T18:03:11Z",
    "created_at": "2012-02-04T17:20:08Z",
    "labels": [
        "component: packages: standard",
        "bug",
        "invalid"
    ],
    "title": "make flintqs respect global CXX flag",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/12428",
    "user": "https://github.com/ohanar"
}
```
The flintqs spkg currently does not respect the global CXX flag, we should fix this (for instance for #12426).

I've posted an spkg at [http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/flintqs-20070817.p7.spkg](http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/flintqs-20070817.p7.spkg).

See also #11792.

**Assignee:** GeorgSWeber

**Reviewer:** Julien Puydt, Leif Leonhardy, François Bissey

**Author:** R. Andrew Ohana

Issue created by migration from https://trac.sagemath.org/ticket/12428





---

archive/issue_comments_139104.json:
```json
{
    "body": "for review purposes",
    "created_at": "2012-02-04T17:20:23Z",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12428#issuecomment-139104",
    "user": "https://github.com/ohanar"
}
```

for review purposes



---

archive/issue_events_102589.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2012-02-04T17:20:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102589"
}
```



---

archive/attachments_017292.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "flintqs-20070817.p7.patch",
    "asset_url": "tarball://root/attachments/ticket12428/flintqs-20070817.p7.patch",
    "created_at": "2012-02-04T17:20:31Z",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket12428/flintqs-20070817.p7.patch",
    "user": "https://github.com/ohanar"
}
```



---

archive/issue_comments_139105.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [flintqs-20070817.p7.patch](https://github.com/sagemath/sage/files/ticket12428/flintqs-20070817.p7.patch)",
    "created_at": "2012-02-04T17:20:31Z",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12428#issuecomment-139105",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:1'></a>
**Attachment:** [flintqs-20070817.p7.patch](https://github.com/sagemath/sage/files/ticket12428/flintqs-20070817.p7.patch)



---

archive/issue_comments_139106.json:
```json
{
    "body": "<a id='comment:2'></a>\nIsn't CXX for the C++ compiler and CPP for the C preprocessor?",
    "created_at": "2012-02-04T20:06:18Z",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12428#issuecomment-139106",
    "user": "https://trac.sagemath.org/admin/accounts/users/Snark"
}
```

<a id='comment:2'></a>
Isn't CXX for the C++ compiler and CPP for the C preprocessor?



---

archive/issue_comments_139107.json:
```json
{
    "body": "<a id='comment:3'></a>\nReplying to [Snark](#comment%3A2):\n> Isn't CXX for the C++ compiler and CPP for the C preprocessor?\n\nYes, but flintqs disagrees (look at the makefile).",
    "created_at": "2012-02-04T20:08:03Z",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12428#issuecomment-139107",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:3'></a>
Replying to [Snark](#comment%3A2):
> Isn't CXX for the C++ compiler and CPP for the C preprocessor?

Yes, but flintqs disagrees (look at the makefile).



---

archive/issue_events_102590.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2012-03-17T05:00:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102590"
}
```



---

archive/issue_events_102591.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2012-03-17T05:00:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102591"
}
```



---

archive/issue_comments_139108.json:
```json
{
    "body": "**Author:** R. Andrew Ohana",
    "created_at": "2012-03-17T05:00:38Z",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12428#issuecomment-139108",
    "user": "https://github.com/nexttime"
}
```

**Author:** R. Andrew Ohana



---

archive/issue_comments_139109.json:
```json
{
    "body": "<a id='comment:4'></a>\nI don't understand why we still have this separate spkg; as mentioned on previous tickets, a more recent version of FLINT QS is included in FLINT 1.x (and hence also in our FLINT spkg), which we just don't build and/or install there, for whatever reason... :-/\n\n---\n\nAgain, `$CXX` should be quoted.",
    "created_at": "2012-03-17T05:00:38Z",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12428#issuecomment-139109",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:4'></a>
I don't understand why we still have this separate spkg; as mentioned on previous tickets, a more recent version of FLINT QS is included in FLINT 1.x (and hence also in our FLINT spkg), which we just don't build and/or install there, for whatever reason... :-/

---

Again, `$CXX` should be quoted.



---

archive/issue_comments_139110.json:
```json
{
    "body": "**Reviewer:** Julien Puydt, Leif Leonhardy",
    "created_at": "2012-03-17T05:00:38Z",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12428#issuecomment-139110",
    "user": "https://github.com/nexttime"
}
```

**Reviewer:** Julien Puydt, Leif Leonhardy



---

archive/issue_comments_139111.json:
```json
{
    "body": "<a id='comment:5'></a>\nReplying to [@nexttime](#comment%3A4):\n> as mentioned on previous tickets, a more recent version of FLINT QS is included in FLINT 1.x\n\nApparently, this rumour is false.\n\n> Again, `$CXX` should be quoted.\n\nIndeed. Or quote the whole argument `\"CPP=$CXX\"`.",
    "created_at": "2013-06-21T09:38:22Z",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12428#issuecomment-139111",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:5'></a>
Replying to [@nexttime](#comment%3A4):
> as mentioned on previous tickets, a more recent version of FLINT QS is included in FLINT 1.x

Apparently, this rumour is false.

> Again, `$CXX` should be quoted.

Indeed. Or quote the whole argument `"CPP=$CXX"`.



---

archive/issue_comments_139112.json:
```json
{
    "body": "<a id='comment:6'></a>\nAbout the flintqs vs flint rumor, see [Bill Hart's mail](https://groups.google.com/forum/#!msg/flint-devel/lmRdEPV6el8/JNbZ4eTy_n0J) when I asked about it last year.",
    "created_at": "2013-06-21T13:30:05Z",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12428#issuecomment-139112",
    "user": "https://trac.sagemath.org/admin/accounts/users/Snark"
}
```

<a id='comment:6'></a>
About the flintqs vs flint rumor, see [Bill Hart's mail](https://groups.google.com/forum/#!msg/flint-devel/lmRdEPV6el8/JNbZ4eTy_n0J) when I asked about it last year.



---

archive/issue_events_102592.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102592"
}
```



---

archive/issue_events_102593.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102593"
}
```



---

archive/issue_events_102594.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102594"
}
```



---

archive/issue_events_102595.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102595"
}
```



---

archive/issue_events_102596.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-02-04T08:00:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "label": "component: build",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102596"
}
```



---

archive/issue_events_102597.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-02-04T08:00:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "label": "component: packages: standard",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102597"
}
```



---

archive/issue_events_102598.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102598"
}
```



---

archive/issue_events_102599.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102599"
}
```



---

archive/issue_events_102600.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102600"
}
```



---

archive/issue_events_102601.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102601"
}
```



---

archive/issue_comments_139113.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n The flintqs spkg currently does not respect the global CXX flag, we should fix this (for instance for #12426).\n \n I've posted an spkg at [http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/flintqs-20070817.p7.spkg](http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/flintqs-20070817.p7.spkg).\n+\n+See also #11792.\n``````\n",
    "created_at": "2014-11-13T14:12:24Z",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12428#issuecomment-139113",
    "user": "https://github.com/jdemeyer"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 The flintqs spkg currently does not respect the global CXX flag, we should fix this (for instance for #12426).
 
 I've posted an spkg at [http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/flintqs-20070817.p7.spkg](http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/flintqs-20070817.p7.spkg).
+
+See also #11792.
``````




---

archive/issue_events_102602.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-09-17T07:28:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102602"
}
```



---

archive/issue_events_102603.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-09-17T07:28:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102603"
}
```



---

archive/issue_events_102604.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-09-17T07:28:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102604"
}
```



---

archive/issue_comments_139114.json:
```json
{
    "body": "<a id='comment:13'></a>\nObsoleted by new autotooled `flintQS`.",
    "created_at": "2016-09-17T07:28:01Z",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12428#issuecomment-139114",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:13'></a>
Obsoleted by new autotooled `flintQS`.



---

archive/issue_comments_139115.json:
```json
{
    "body": "**Changing reviewer** from \"Julien Puydt, Leif Leonhardy\" to \"Julien Puydt, Leif Leonhardy, Fran\u00e7ois Bissey\".",
    "created_at": "2016-09-17T07:28:01Z",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12428#issuecomment-139115",
    "user": "https://github.com/kiwifb"
}
```

**Changing reviewer** from "Julien Puydt, Leif Leonhardy" to "Julien Puydt, Leif Leonhardy, François Bissey".



---

archive/issue_events_102605.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-01-21T18:03:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102605"
}
```



---

archive/issue_events_102606.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-01-21T18:03:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12428",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12428#event-102606"
}
```

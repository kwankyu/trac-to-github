# Issue 12420: sage-banner needs fixing for next version

archive/issues_012248.json:
```json
{
    "assignees": [
        "https://github.com/ppurka"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nRunning sage-4.8 shows the banner as sage-4.7.2.\n\nOne option is to furtively change the tarball on the website with the correct banner file :)\n\nThe other option is to make the jump to 5.0 when the time comes.\n\nComponent: **scripts**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12420_\n\n",
    "closed_at": "2012-02-27T11:27:33Z",
    "created_at": "2012-02-03T19:25:57Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20scripts",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage-banner needs fixing for next version",
    "type": "issue",
    "updated_at": "2012-02-27T11:30:10Z",
    "url": "https://github.com/sagemath/sage/issues/12420",
    "user": "https://github.com/ppurka"
}
```
<div id="comment:0"></div>

Running sage-4.8 shows the banner as sage-4.7.2.

One option is to furtively change the tarball on the website with the correct banner file :)

The other option is to make the jump to 5.0 when the time comes.

Component: **scripts**

_Issue created by migration from https://trac.sagemath.org/ticket/12420_





---

archive/issue_events_167988.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-02-03T19:25:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12420",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12420#event-167988"
}
```



---

archive/issue_events_167989.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-02-03T19:25:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12420",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20scripts",
    "label_color": "0000b0",
    "label_name": "c: scripts",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12420#event-167989"
}
```



---

archive/issue_events_167990.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-02-03T19:25:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12420",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20trivial%20/%205",
    "label_color": "fff9e5",
    "label_name": "p: trivial / 5",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12420#event-167990"
}
```



---

archive/issue_events_167991.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-02-03T19:25:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12420",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12420#event-167991"
}
```



---

archive/issue_events_167992.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2012-02-03T19:25:57Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12420",
    "subject": "https://github.com/ppurka",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12420#event-167992"
}
```



---

archive/issue_comments_136211.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\ni think this is a frivolous ticket. I downloaded another sage-4.8 some other time and it shows up the version correctly. Not sure how the first download turned out to have the wrong banner.",
    "created_at": "2012-02-27T09:55:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12420",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12420#issuecomment-136211",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:1" align="right">comment:1</div>

i think this is a frivolous ticket. I downloaded another sage-4.8 some other time and it shows up the version correctly. Not sure how the first download turned out to have the wrong banner.



---

archive/issue_events_167993.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-02-27T09:55:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12420",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12420#event-167993"
}
```



---

archive/issue_events_167994.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-02-27T09:55:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12420",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12420#event-167994"
}
```



---

archive/issue_events_167995.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-02-27T09:55:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12420",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12420#event-167995"
}
```



---

archive/issue_events_167996.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-02-27T09:55:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12420",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12420#event-167996"
}
```



---

archive/issue_events_167997.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2012-02-27T09:55:50Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/12420",
    "subject": "https://github.com/ppurka",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12420#event-167997"
}
```



---

archive/issue_events_167998.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-02-27T09:55:50Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12420",
    "subject": "https://github.com/ppurka",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12420#event-167998"
}
```



---

archive/issue_comments_136212.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nAre you sure the problem was with the banner?  I guess that you simply ran a different version of Sage on your computer, or somehow mixed up different Sage versions on your machine.",
    "created_at": "2012-02-27T11:27:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12420",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12420#issuecomment-136212",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:5" align="right">comment:5</div>

Are you sure the problem was with the banner?  I guess that you simply ran a different version of Sage on your computer, or somehow mixed up different Sage versions on your machine.



---

archive/issue_events_167999.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-27T11:27:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12420",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12420#event-167999"
}
```



---

archive/issue_events_168000.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-27T11:27:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12420",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12420#event-168000"
}
```



---

archive/issue_comments_136213.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nYes. I had double checked. Not sure what really happened and I am not really interested in digging into something that seems fixed on another machine.\n\nThanks for closing the ticket.",
    "created_at": "2012-02-27T11:30:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12420",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12420#issuecomment-136213",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:6" align="right">comment:6</div>

Yes. I had double checked. Not sure what really happened and I am not really interested in digging into something that seems fixed on another machine.

Thanks for closing the ticket.

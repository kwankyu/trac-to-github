# Issue 12484: CombinatorialFreeModule's: customizing the order of terms in the output of elements

archive/issues_012312.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-sage-combinat"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nAs the title says.\n\nBy the way, this (partially) refactors sage.misc.misc.repr_lincomb and its use in FreeAlgebra and friends. It also discards some now useless code in symmetric functions.\n\n\nDepends on #12490\n\nCC:  @sagetrac-sage-combinat\n\nComponent: **combinatorics**\n\nKeywords: **Cernay2012**\n\nAuthor: **Nicolas M. Thi\u00e9ry**\n\nReviewer: **Florent Hivert**\n\nMerged: **sage-5.0.beta5**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12484_\n\n",
    "closed_at": "2012-02-22T10:46:44Z",
    "created_at": "2012-02-09T17:11:22Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "CombinatorialFreeModule's: customizing the order of terms in the output of elements",
    "type": "issue",
    "updated_at": "2012-02-22T10:46:44Z",
    "url": "https://github.com/sagemath/sage/issues/12484",
    "user": "https://github.com/nthiery"
}
```
<div id="comment:0"></div>

As the title says.

By the way, this (partially) refactors sage.misc.misc.repr_lincomb and its use in FreeAlgebra and friends. It also discards some now useless code in symmetric functions.


Depends on #12490

CC:  @sagetrac-sage-combinat

Component: **combinatorics**

Keywords: **Cernay2012**

Author: **Nicolas M. Thiéry**

Reviewer: **Florent Hivert**

Merged: **sage-5.0.beta5**

_Issue created by migration from https://trac.sagemath.org/ticket/12484_





---

archive/issue_events_169213.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2012-02-09T17:11:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12484#event-169213"
}
```



---

archive/issue_events_169214.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2012-02-09T17:11:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12484#event-169214"
}
```



---

archive/issue_events_169215.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2012-02-09T17:11:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12484#event-169215"
}
```



---

archive/issue_events_169216.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2012-02-09T17:11:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12484#event-169216"
}
```



---

archive/issue_events_169217.json:
```json
{
    "actor": "https://github.com/sagetrac-sage-combinat",
    "created_at": "2012-02-09T17:11:22Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "subject": "https://github.com/nthiery",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12484#event-169217"
}
```



---

archive/issue_events_169218.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2012-02-10T12:55:32Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "title_is": "CombinatorialFreeModule's: customizing the order of terms in the output of elements",
    "title_was": "Implements CombinatorialFreeModule(...).print_options(monomials_cmp = ...)",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12484#event-169218"
}
```



---

archive/issue_comments_137856.json:
```json
{
    "body": "Changed keywords from none to **Cernay2012**",
    "created_at": "2012-02-11T00:38:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12484#issuecomment-137856",
    "user": "https://github.com/hivert"
}
```

Changed keywords from none to **Cernay2012**



---

archive/issue_comments_137857.json:
```json
{
    "body": "Dependencies: **#12490**",
    "created_at": "2012-02-13T17:12:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12484#issuecomment-137857",
    "user": "https://github.com/nthiery"
}
```

Dependencies: **#12490**



---

archive/issue_comments_137858.json:
```json
{
    "body": "Reviewer: **Florent Hivert**",
    "created_at": "2012-02-13T17:12:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12484#issuecomment-137858",
    "user": "https://github.com/nthiery"
}
```

Reviewer: **Florent Hivert**



---

archive/issue_comments_137859.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nAll test passed (except for the usal timeout on sandpiles) on Sage.5.0.beta3 on Debian x86 64 bits.",
    "created_at": "2012-02-13T17:18:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12484#issuecomment-137859",
    "user": "https://github.com/nthiery"
}
```

<div id="comment:4" align="right">comment:4</div>

All test passed (except for the usal timeout on sandpiles) on Sage.5.0.beta3 on Debian x86 64 bits.



---

archive/issue_comments_137860.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,4 @@\n+As the title says.\n \n+By the way, this (partially) refactors sage.misc.misc.repr_lincomb and its use in FreeAlgebra and friends. It also discards some now useless code in symmetric functions.\n+\n``````\n",
    "created_at": "2012-02-13T17:18:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12484#issuecomment-137860",
    "user": "https://github.com/nthiery"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,4 @@
+As the title says.
 
+By the way, this (partially) refactors sage.misc.misc.repr_lincomb and its use in FreeAlgebra and friends. It also discards some now useless code in symmetric functions.
+
``````




---

archive/issue_events_169219.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2012-02-13T17:18:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12484#event-169219"
}
```



---

archive/issue_comments_137861.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nFor the record: I had a review patch fixing the doc and checking the input of\nCombinatorialFreeModule on sage-combinat queue. Nicolas reviewed it\npositively. I'm uploading here everything folded.",
    "created_at": "2012-02-14T16:22:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12484#issuecomment-137861",
    "user": "https://github.com/hivert"
}
```

<div id="comment:5" align="right">comment:5</div>

For the record: I had a review patch fixing the doc and checking the input of
CombinatorialFreeModule on sage-combinat queue. Nicolas reviewed it
positively. I'm uploading here everything folded.



---

archive/issue_events_169220.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2012-02-14T16:24:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12484#event-169220"
}
```



---

archive/issue_events_169221.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2012-02-14T16:24:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12484#event-169221"
}
```



---

archive/issue_comments_137862.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nReplying to [@hivert](#comment%3A5):\n> For the record: I had a review patch fixing the doc and checking the input of\n> CombinatorialFreeModule on sage-combinat queue. Nicolas reviewed it\n> positively. I'm uploading here everything folded.\n\nActually, Nicolas is uploading it...",
    "created_at": "2012-02-14T16:24:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12484#issuecomment-137862",
    "user": "https://github.com/hivert"
}
```

<div id="comment:6" align="right">comment:6</div>

Replying to [@hivert](#comment%3A5):
> For the record: I had a review patch fixing the doc and checking the input of
> CombinatorialFreeModule on sage-combinat queue. Nicolas reviewed it
> positively. I'm uploading here everything folded.

Actually, Nicolas is uploading it...



---

archive/issue_comments_137863.json:
```json
{
    "body": "Attachment: **[trac_12484-free_module-monomials_cmp-nt.patch.gz](https://github.com/sagemath/sage/files/ticket12484/trac_12484-free_module-monomials_cmp-nt.patch.gz)**",
    "created_at": "2012-02-14T16:35:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12484#issuecomment-137863",
    "user": "https://github.com/nthiery"
}
```

Attachment: **[trac_12484-free_module-monomials_cmp-nt.patch.gz](https://github.com/sagemath/sage/files/ticket12484/trac_12484-free_module-monomials_cmp-nt.patch.gz)**



---

archive/issue_comments_137864.json:
```json
{
    "body": "Merged: **sage-5.0.beta5**",
    "created_at": "2012-02-22T10:46:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12484#issuecomment-137864",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta5**



---

archive/issue_events_169222.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-22T10:46:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12484#event-169222"
}
```



---

archive/issue_events_169223.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-22T10:46:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12484",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12484#event-169223"
}
```

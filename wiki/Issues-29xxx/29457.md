# Issue 29457: Upgrading Sage issues

archive/issues_029220.json:
```json
{
    "body": "Somehow I can seem to upgrade sage with the sage -upgrade command it gives me this error: error: cannot open .git/FETCH_HEAD: Permission denied\n\n**Keywords:** upgrade\n\n**Branch:** [master](https://github.com/sagemath/sagetrac-mirror/tree/master)\n\n**Commit:** [31cb54b77951c888af513734ca36ac045fa9ca86](https://github.com/sagemath/sagetrac-mirror/commit/31cb54b77951c888af513734ca36ac045fa9ca86)\n\n**Reviewer:** Matthias Koeppe\n\nIssue created by migration from https://trac.sagemath.org/ticket/29457\n\n",
    "closed_at": "2020-05-06T16:50:39Z",
    "created_at": "2020-04-03T16:06:00Z",
    "labels": [
        "component: build",
        "bug",
        "invalid"
    ],
    "title": "Upgrading Sage issues",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/29457",
    "user": "https://github.com/WindowsSystemAdmin"
}
```
Somehow I can seem to upgrade sage with the sage -upgrade command it gives me this error: error: cannot open .git/FETCH_HEAD: Permission denied

**Keywords:** upgrade

**Branch:** [master](https://github.com/sagemath/sagetrac-mirror/tree/master)

**Commit:** [31cb54b77951c888af513734ca36ac045fa9ca86](https://github.com/sagemath/sagetrac-mirror/commit/31cb54b77951c888af513734ca36ac045fa9ca86)

**Reviewer:** Matthias Koeppe

Issue created by migration from https://trac.sagemath.org/ticket/29457





---

archive/issue_comments_465869.json:
```json
{
    "body": "**Branch:** [master](https://github.com/sagemath/sagetrac-mirror/tree/master)",
    "created_at": "2020-04-03T16:21:50Z",
    "issue": "https://github.com/sagemath/sage/issues/29457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29457#issuecomment-465869",
    "user": "https://github.com/WindowsSystemAdmin"
}
```

**Branch:** [master](https://github.com/sagemath/sagetrac-mirror/tree/master)



---

archive/issue_comments_465870.json:
```json
{
    "body": "**Commit:** [31cb54b77951c888af513734ca36ac045fa9ca86](https://github.com/sagemath/sagetrac-mirror/commit/31cb54b77951c888af513734ca36ac045fa9ca86)",
    "created_at": "2020-04-03T16:21:50Z",
    "issue": "https://github.com/sagemath/sage/issues/29457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29457#issuecomment-465870",
    "user": "https://github.com/WindowsSystemAdmin"
}
```

**Commit:** [31cb54b77951c888af513734ca36ac045fa9ca86](https://github.com/sagemath/sagetrac-mirror/commit/31cb54b77951c888af513734ca36ac045fa9ca86)



---

archive/issue_comments_465871.json:
```json
{
    "body": "<a id='comment:2'></a>\nHave you followed the steps [here](http://doc.sagemath.org/html/en/developer/trac.html#reporting-bugs)? For example, have you reported this problem on sage-devel?",
    "created_at": "2020-04-03T16:37:30Z",
    "issue": "https://github.com/sagemath/sage/issues/29457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29457#issuecomment-465871",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'></a>
Have you followed the steps [here](http://doc.sagemath.org/html/en/developer/trac.html#reporting-bugs)? For example, have you reported this problem on sage-devel?



---

archive/issue_comments_465872.json:
```json
{
    "body": "<a id='comment:3'></a>\nReplying to [@jhpalmieri](#comment%3A2):\n> Have you followed the steps [here](http://doc.sagemath.org/html/en/developer/trac.html#reporting-bugs)? For example, have you reported this problem on sage-devel? \n\nnow I did here is the site https://groups.google.com/forum/#!topic/sage-devel/xQZRmDEYcyY",
    "created_at": "2020-04-03T16:52:00Z",
    "issue": "https://github.com/sagemath/sage/issues/29457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29457#issuecomment-465872",
    "user": "https://github.com/WindowsSystemAdmin"
}
```

<a id='comment:3'></a>
Replying to [@jhpalmieri](#comment%3A2):
> Have you followed the steps [here](http://doc.sagemath.org/html/en/developer/trac.html#reporting-bugs)? For example, have you reported this problem on sage-devel? 

now I did here is the site https://groups.google.com/forum/#!topic/sage-devel/xQZRmDEYcyY



---

archive/issue_events_264609.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2020-04-18T22:01:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29457",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29457#event-264609"
}
```



---

archive/issue_comments_465873.json:
```json
{
    "body": "<a id='comment:4'></a>\nI think this ticket should be closed. If a precise problem with upgrading is identified, we can open a new ticket with that specific target.",
    "created_at": "2020-04-18T22:01:59Z",
    "issue": "https://github.com/sagemath/sage/issues/29457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29457#issuecomment-465873",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:4'></a>
I think this ticket should be closed. If a precise problem with upgrading is identified, we can open a new ticket with that specific target.



---

archive/issue_events_264610.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2020-04-18T22:01:59Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29457",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29457#event-264610"
}
```



---

archive/issue_events_264611.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-18T22:52:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29457",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29457#event-264611"
}
```



---

archive/issue_events_264612.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-18T22:52:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29457",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29457#event-264612"
}
```



---

archive/issue_comments_465874.json:
```json
{
    "body": "**Reviewer:** Matthias Koeppe",
    "created_at": "2020-04-18T22:52:07Z",
    "issue": "https://github.com/sagemath/sage/issues/29457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29457#issuecomment-465874",
    "user": "https://github.com/mkoeppe"
}
```

**Reviewer:** Matthias Koeppe



---

archive/issue_events_264613.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-05-06T16:50:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29457",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29457#event-264613"
}
```



---

archive/issue_events_264614.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-05-06T16:50:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29457",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29457#event-264614"
}
```

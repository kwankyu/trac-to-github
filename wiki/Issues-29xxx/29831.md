# Issue 29831: SageMath crashes upon launch

archive/issues_029594.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nWhen [SageMath](../wiki/SageMath) is launched, it immediately crashes. I'm running Arch Linux. Logs attached.\n\nComponent: **PLEASE CHANGE**\n\nReviewer: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29831_\n\n",
    "closed_at": "2020-06-14T13:24:20Z",
    "created_at": "2020-06-09T11:10:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "SageMath crashes upon launch",
    "type": "issue",
    "updated_at": "2020-06-14T13:24:20Z",
    "url": "https://github.com/sagemath/sage/issues/29831",
    "user": "https://github.com/marmistrz"
}
```
<div id="comment:0"></div>

When [SageMath](../wiki/SageMath) is launched, it immediately crashes. I'm running Arch Linux. Logs attached.

Component: **PLEASE CHANGE**

Reviewer: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/29831_





---

archive/issue_events_407993.json:
```json
{
    "actor": "https://github.com/marmistrz",
    "created_at": "2020-06-09T11:10:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29831",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29831#event-407993"
}
```



---

archive/issue_events_407994.json:
```json
{
    "actor": "https://github.com/marmistrz",
    "created_at": "2020-06-09T11:10:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29831",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29831#event-407994"
}
```



---

archive/issue_events_407995.json:
```json
{
    "actor": "https://github.com/marmistrz",
    "created_at": "2020-06-09T11:10:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29831",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29831#event-407995"
}
```



---

archive/issue_comments_470959.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[sage.log](https://github.com/sagemath/sage/files/ticket29831/sage.log)**\n\nYou are mixing system python packages and user-installed python modules. Don't do that.",
    "created_at": "2020-06-09T14:11:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29831",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29831#issuecomment-470959",
    "user": "https://github.com/antonio-rojas"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[sage.log](https://github.com/sagemath/sage/files/ticket29831/sage.log)**

You are mixing system python packages and user-installed python modules. Don't do that.



---

archive/issue_events_407996.json:
```json
{
    "actor": "https://github.com/antonio-rojas",
    "created_at": "2020-06-09T14:11:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29831",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29831#event-407996"
}
```



---

archive/issue_events_407997.json:
```json
{
    "actor": "https://github.com/antonio-rojas",
    "created_at": "2020-06-09T14:11:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29831",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29831#event-407997"
}
```



---

archive/issue_comments_470960.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI didn't notice that, sorry!",
    "created_at": "2020-06-10T17:46:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29831",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29831#issuecomment-470960",
    "user": "https://github.com/marmistrz"
}
```

<div id="comment:3" align="right">comment:3</div>

I didn't notice that, sorry!



---

archive/issue_events_407998.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-10T17:57:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29831",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29831#event-407998"
}
```



---

archive/issue_events_407999.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-10T17:57:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29831",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29831#event-407999"
}
```



---

archive/issue_comments_470961.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2020-06-10T18:53:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29831",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29831#issuecomment-470961",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_events_408000.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-06-14T13:24:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29831",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29831#event-408000"
}
```



---

archive/issue_events_408001.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-06-14T13:24:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29831",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29831#event-408001"
}
```

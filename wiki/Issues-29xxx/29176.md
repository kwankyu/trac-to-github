# Issue 29176: Bug in Voronoi Diagram

archive/issues_028939.json:
```json
{
    "body": "We fix a bug that was exposed computing an inexact Voronoi Diagram (see below).\n\n`cdd` already computes the incidence matrix, we expose it in this ticket.\n\nWhen deciding whether or not a vertex lies on a hyperplane for inexact polyhedra, we allow numerical noise up to absolute value of `1e-6`. This might be a bad choice depending on the size of the polyhedron as witnessed in the following example:\n\n```\n\nsage: e = [[11582947.657000002, 5374.38, 4177.06, 1.0], [11562795.9322, 5373.62, 4168.38, 1.0]]\nsage: p = Polyhedron(ieqs=e); p\nA 3-dimensional polyhedron in RDF^3 defined as the convex hull of 1 vertex, 2 rays, 1 line\nsage: p.incidence_matrix()\n[0 0]\n[0 0]\n[0 1]\n[0 0]\n```\n\nSetting the cache of incidence matrix to the matrix that `cdd` already computed for us produces the correct output:\n\n```\nsage: p.incidence_matrix()\n[1 1]\n[1 0]\n[0 1]\n[1 1]\n```\n\nThis fixes the error reported here:\n\nhttps://ask.sagemath.org/question/49749/voronoidiagram-returns-empty-regions/\n\nThe following should give two non-empty regions\n\n```\nsage: P = [[-2687.19, -2088.53], [-2686.81, -2084.19]]\n....: V = VoronoiDiagram(P)\n....: R = V.regions()\n....: \nsage: R\n{P(-2687.19000000000, -2088.53000000000): The empty polyhedron in RDF^0,\n P(-2686.81000000000, -2084.19000000000): A 1-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex and 1 ray\n```\n\nWith this ticket we obtain:\n\n```\nsage: R\n{P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,\n P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}\n```\n\n**CC:**  @jplab @LaisRast\n\n**Keywords:** cdd, incidence matrix\n\n**Branch/Commit:** [fb2d66a0e71f164e9bef37922c8d544b9b270c9c](https://github.com/sagemath/sagetrac-mirror/commit/fb2d66a0e71f164e9bef37922c8d544b9b270c9c)\n\n**Reviewer:** Jean-Philippe Labb\u00e9\n\n**Author:** Jonathan Kliem\n\nIssue created by migration from https://trac.sagemath.org/ticket/29176\n\n",
    "closed_at": "2020-05-02T21:58:26Z",
    "created_at": "2020-02-10T12:53:15Z",
    "labels": [
        "component: geometry",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "title": "Bug in Voronoi Diagram",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/29176",
    "user": "https://github.com/jplab"
}
```
We fix a bug that was exposed computing an inexact Voronoi Diagram (see below).

`cdd` already computes the incidence matrix, we expose it in this ticket.

When deciding whether or not a vertex lies on a hyperplane for inexact polyhedra, we allow numerical noise up to absolute value of `1e-6`. This might be a bad choice depending on the size of the polyhedron as witnessed in the following example:

```

sage: e = [[11582947.657000002, 5374.38, 4177.06, 1.0], [11562795.9322, 5373.62, 4168.38, 1.0]]
sage: p = Polyhedron(ieqs=e); p
A 3-dimensional polyhedron in RDF^3 defined as the convex hull of 1 vertex, 2 rays, 1 line
sage: p.incidence_matrix()
[0 0]
[0 0]
[0 1]
[0 0]
```

Setting the cache of incidence matrix to the matrix that `cdd` already computed for us produces the correct output:

```
sage: p.incidence_matrix()
[1 1]
[1 0]
[0 1]
[1 1]
```

This fixes the error reported here:

https://ask.sagemath.org/question/49749/voronoidiagram-returns-empty-regions/

The following should give two non-empty regions

```
sage: P = [[-2687.19, -2088.53], [-2686.81, -2084.19]]
....: V = VoronoiDiagram(P)
....: R = V.regions()
....: 
sage: R
{P(-2687.19000000000, -2088.53000000000): The empty polyhedron in RDF^0,
 P(-2686.81000000000, -2084.19000000000): A 1-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex and 1 ray
```

With this ticket we obtain:

```
sage: R
{P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,
 P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}
```

**CC:**  @jplab @LaisRast

**Keywords:** cdd, incidence matrix

**Branch/Commit:** [fb2d66a0e71f164e9bef37922c8d544b9b270c9c](https://github.com/sagemath/sagetrac-mirror/commit/fb2d66a0e71f164e9bef37922c8d544b9b270c9c)

**Reviewer:** Jean-Philippe Labbé

**Author:** Jonathan Kliem

Issue created by migration from https://trac.sagemath.org/ticket/29176





---

archive/issue_comments_460332.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis can and should be solved by fetching the incidences from cdd.\n\nI will try to do so. This might get rid of a lot of numerical inconsistencies.",
    "created_at": "2020-03-19T11:12:56Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460332",
    "user": "https://github.com/kliem"
}
```

<a id='comment:1'></a>
This can and should be solved by fetching the incidences from cdd.

I will try to do so. This might get rid of a lot of numerical inconsistencies.



---

archive/issue_comments_460333.json:
```json
{
    "body": "<a id='comment:2'></a>\n**New commits:**\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c03aee4b4445c7954c667adcf233fa8ba1612d62\">c03aee4</a></td><td><code>expose incidence matrix computed by backend cdd</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/33b59112324e37dcbdab7706047e47cf9dbf5568\">33b5911</a></td><td><code>add doctests that 29176 is fixed</code></td></tr></table>\n",
    "created_at": "2020-03-19T14:55:32Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460333",
    "user": "https://github.com/kliem"
}
```

<a id='comment:2'></a>
**New commits:**
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c03aee4b4445c7954c667adcf233fa8ba1612d62">c03aee4</a></td><td><code>expose incidence matrix computed by backend cdd</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/33b59112324e37dcbdab7706047e47cf9dbf5568">33b5911</a></td><td><code>add doctests that 29176 is fixed</code></td></tr></table>




---

archive/issue_comments_460334.json:
```json
{
    "body": "**Branch:** [public/29176](https://github.com/sagemath/sagetrac-mirror/tree/public/29176)",
    "created_at": "2020-03-19T14:55:32Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460334",
    "user": "https://github.com/kliem"
}
```

**Branch:** [public/29176](https://github.com/sagemath/sagetrac-mirror/tree/public/29176)



---

archive/issue_comments_460335.json:
```json
{
    "body": "**Changing keywords** from \"\" to \"cdd, incidence matrix\".",
    "created_at": "2020-03-19T14:55:32Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460335",
    "user": "https://github.com/kliem"
}
```

**Changing keywords** from "" to "cdd, incidence matrix".



---

archive/issue_comments_460336.json:
```json
{
    "body": "**Commit:** [33b59112324e37dcbdab7706047e47cf9dbf5568](https://github.com/sagemath/sagetrac-mirror/commit/33b59112324e37dcbdab7706047e47cf9dbf5568)",
    "created_at": "2020-03-19T14:55:32Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460336",
    "user": "https://github.com/kliem"
}
```

**Commit:** [33b59112324e37dcbdab7706047e47cf9dbf5568](https://github.com/sagemath/sagetrac-mirror/commit/33b59112324e37dcbdab7706047e47cf9dbf5568)



---

archive/issue_comments_460337.json:
```json
{
    "body": "**Author:** Jonathan Kliem",
    "created_at": "2020-03-19T14:55:32Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460337",
    "user": "https://github.com/kliem"
}
```

**Author:** Jonathan Kliem



---

archive/issue_events_261760.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-03-19T14:55:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-261760"
}
```



---

archive/issue_comments_460338.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,4 +1,36 @@\n-The following should give two non-empty regions:\n+We fix a bug that was exposed computing an inexact Voronoi Diagram (see below).\n+\n+`cdd` already computes the incidence matrix, we expose it in this ticket.\n+\n+When deciding whether or not a vertex lies on a hyperplane for inexact polyhedra, we allow numerical noise up to absolute value of `1e-6`. This might be a bad choice depending on the size of the polyhedron as witnessed in the following example:\n+\n+```\n+\n+sage: e = [[11582947.657000002, 5374.38, 4177.06, 1.0], [11562795.9322, 5373.62, 4168.38, 1.0]]\n+sage: p = Polyhedron(ieqs=e); p\n+A 3-dimensional polyhedron in RDF^3 defined as the convex hull of 1 vertex, 2 rays, 1 line\n+sage: p.incidence_matrix()\n+[0 0]\n+[0 0]\n+[0 1]\n+[0 0]\n+```\n+\n+Setting the cache of incidence matrix to the matrix that `cdd` already computed for us produces the correct output:\n+\n+```\n+sage: p.incidence_matrix()\n+[1 1]\n+[1 0]\n+[0 1]\n+[1 1]\n+```\n+\n+This fixes the error reported here:\n+\n+https://ask.sagemath.org/question/49749/voronoidiagram-returns-empty-regions/\n+\n+The following should give two non-empty regions\n \n ```\n sage: P = [[-2687.19, -2088.53], [-2686.81, -2084.19]]\n@@ -10,6 +42,10 @@\n  P(-2686.81000000000, -2084.19000000000): A 1-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex and 1 ray\n ```\n \n-This way reported here:\n+With this ticket we obtain:\n \n-https://ask.sagemath.org/question/49749/voronoidiagram-returns-empty-regions/\n+```\n+sage: R\n+{P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,\n+ P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}\n+```\n``````\n",
    "created_at": "2020-03-19T14:55:32Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460338",
    "user": "https://github.com/kliem"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,4 +1,36 @@
-The following should give two non-empty regions:
+We fix a bug that was exposed computing an inexact Voronoi Diagram (see below).
+
+`cdd` already computes the incidence matrix, we expose it in this ticket.
+
+When deciding whether or not a vertex lies on a hyperplane for inexact polyhedra, we allow numerical noise up to absolute value of `1e-6`. This might be a bad choice depending on the size of the polyhedron as witnessed in the following example:
+
+```
+
+sage: e = [[11582947.657000002, 5374.38, 4177.06, 1.0], [11562795.9322, 5373.62, 4168.38, 1.0]]
+sage: p = Polyhedron(ieqs=e); p
+A 3-dimensional polyhedron in RDF^3 defined as the convex hull of 1 vertex, 2 rays, 1 line
+sage: p.incidence_matrix()
+[0 0]
+[0 0]
+[0 1]
+[0 0]
+```
+
+Setting the cache of incidence matrix to the matrix that `cdd` already computed for us produces the correct output:
+
+```
+sage: p.incidence_matrix()
+[1 1]
+[1 0]
+[0 1]
+[1 1]
+```
+
+This fixes the error reported here:
+
+https://ask.sagemath.org/question/49749/voronoidiagram-returns-empty-regions/
+
+The following should give two non-empty regions
 
 ```
 sage: P = [[-2687.19, -2088.53], [-2686.81, -2084.19]]
@@ -10,6 +42,10 @@
  P(-2686.81000000000, -2084.19000000000): A 1-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex and 1 ray
 ```
 
-This way reported here:
+With this ticket we obtain:
 
-https://ask.sagemath.org/question/49749/voronoidiagram-returns-empty-regions/
+```
+sage: R
+{P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,
+ P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}
+```
``````




---

archive/issue_comments_460339.json:
```json
{
    "body": "<a id='comment:3'></a>\nThanks! Looks good to me.",
    "created_at": "2020-04-19T13:33:56Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460339",
    "user": "https://github.com/jplab"
}
```

<a id='comment:3'></a>
Thanks! Looks good to me.



---

archive/issue_events_261761.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-04-19T13:33:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-261761"
}
```



---

archive/issue_events_261762.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-04-19T13:33:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-261762"
}
```



---

archive/issue_comments_460340.json:
```json
{
    "body": "**Reviewer:** Jean-Philippe Labb\u00e9",
    "created_at": "2020-04-19T13:33:56Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460340",
    "user": "https://github.com/jplab"
}
```

**Reviewer:** Jean-Philippe Labbé



---

archive/issue_comments_460341.json:
```json
{
    "body": "<a id='comment:4'></a>\nThank you.",
    "created_at": "2020-04-19T13:34:34Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460341",
    "user": "https://github.com/kliem"
}
```

<a id='comment:4'></a>
Thank you.



---

archive/issue_events_261763.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-21T22:42:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-261763"
}
```



---

archive/issue_events_261764.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-21T22:42:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-261764"
}
```



---

archive/issue_comments_460342.json:
```json
{
    "body": "<a id='comment:5'></a>\nOn Python 2 Debian 9 x86_64:\n\n```\n**********************************************************************\nFile \"src/sage/geometry/polyhedron/backend_cdd.py\", line 231, in sage.geometry.polyhedron.backend_cdd.Polyhedron_cdd._init_from_cdd_output\nFailed example:\n    R\nExpected:\n    {P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,\n     P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}\nGot:\n    {P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,\n     P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}\n**********************************************************************\n1 item had failures:\n```",
    "created_at": "2020-04-21T22:42:38Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460342",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:5'></a>
On Python 2 Debian 9 x86_64:

```
**********************************************************************
File "src/sage/geometry/polyhedron/backend_cdd.py", line 231, in sage.geometry.polyhedron.backend_cdd.Polyhedron_cdd._init_from_cdd_output
Failed example:
    R
Expected:
    {P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,
     P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}
Got:
    {P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,
     P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}
**********************************************************************
1 item had failures:
```



---

archive/issue_comments_460343.json:
```json
{
    "body": "**Changing commit** from \"[33b59112324e37dcbdab7706047e47cf9dbf5568](https://github.com/sagemath/sagetrac-mirror/commit/33b59112324e37dcbdab7706047e47cf9dbf5568)\" to \"[9352525b50851921f000335615e75e0f1bdd5f2e](https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e)\".",
    "created_at": "2020-04-22T05:40:02Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460343",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

**Changing commit** from "[33b59112324e37dcbdab7706047e47cf9dbf5568](https://github.com/sagemath/sagetrac-mirror/commit/33b59112324e37dcbdab7706047e47cf9dbf5568)" to "[9352525b50851921f000335615e75e0f1bdd5f2e](https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e)".



---

archive/issue_comments_460344.json:
```json
{
    "body": "<a id='comment:6'></a>\n**Branch pushed to git repo; I updated commit sha1.** **New commits:**\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e\">9352525</a></td><td><code>fixed dictionary in python2</code></td></tr></table>\n",
    "created_at": "2020-04-22T05:40:02Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460344",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:6'></a>
**Branch pushed to git repo; I updated commit sha1.** **New commits:**
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e">9352525</a></td><td><code>fixed dictionary in python2</code></td></tr></table>




---

archive/issue_events_261765.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-04-22T05:41:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-261765"
}
```



---

archive/issue_events_261766.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-04-22T05:41:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-261766"
}
```



---

archive/issue_comments_460345.json:
```json
{
    "body": "<a id='comment:7'></a>\nI tested this in python2:\n\n```\n>>> {(-2686.81000000000, -2084.19000000000): 0, (-2687.19000000000, -2088.53000000000): 0}\n{(-2687.19, -2088.53): 0, (-2686.81, -2084.19): 0}\n```\n\nSo the fix should work fine.",
    "created_at": "2020-04-22T05:41:12Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460345",
    "user": "https://github.com/kliem"
}
```

<a id='comment:7'></a>
I tested this in python2:

```
>>> {(-2686.81000000000, -2084.19000000000): 0, (-2687.19000000000, -2088.53000000000): 0}
{(-2687.19, -2088.53): 0, (-2686.81, -2084.19): 0}
```

So the fix should work fine.



---

archive/issue_events_261767.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-04-22T12:34:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-261767"
}
```



---

archive/issue_events_261768.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-04-22T12:34:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-261768"
}
```



---

archive/issue_comments_460346.json:
```json
{
    "body": "<a id='comment:8'></a>\nThanks for the quick fix.",
    "created_at": "2020-04-22T12:34:45Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460346",
    "user": "https://github.com/jplab"
}
```

<a id='comment:8'></a>
Thanks for the quick fix.



---

archive/issue_comments_460347.json:
```json
{
    "body": "**Changing branch** from \"[public/29176](https://github.com/sagemath/sagetrac-mirror/tree/public/29176)\" to \"[9352525b50851921f000335615e75e0f1bdd5f2e](https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e)\".",
    "created_at": "2020-04-23T22:32:59Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460347",
    "user": "https://github.com/vbraun"
}
```

**Changing branch** from "[public/29176](https://github.com/sagemath/sagetrac-mirror/tree/public/29176)" to "[9352525b50851921f000335615e75e0f1bdd5f2e](https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e)".



---

archive/issue_events_261769.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-23T22:32:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-261769"
}
```



---

archive/issue_events_261770.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-23T22:32:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-261770"
}
```



---

archive/issue_comments_460348.json:
```json
{
    "body": "<a id='comment:10'></a>\n\n```\n**********************************************************************\nFile \"src/sage/geometry/polyhedron/backend_cdd.py\", line 234, in sage.geometry.polyhedron.backend_cdd.Polyhedron_cdd._init_from_cdd_output\nFailed example:\n    R  # py2\nExpected:\n    {P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,\n     P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}\nGot:\n    {P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,\n     P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}\n**********************************************************************\n1 item had failures:\n   1 of  10 in sage.geometry.polyhedron.backend_cdd.Polyhedron_cdd._init_from_cdd_output\n    [46 tests, 1 failure, 0.59 s]\n**********************************************************************\n```",
    "created_at": "2020-04-25T08:52:42Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460348",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:10'></a>

```
**********************************************************************
File "src/sage/geometry/polyhedron/backend_cdd.py", line 234, in sage.geometry.polyhedron.backend_cdd.Polyhedron_cdd._init_from_cdd_output
Failed example:
    R  # py2
Expected:
    {P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,
     P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}
Got:
    {P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,
     P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}
**********************************************************************
1 item had failures:
   1 of  10 in sage.geometry.polyhedron.backend_cdd.Polyhedron_cdd._init_from_cdd_output
    [46 tests, 1 failure, 0.59 s]
**********************************************************************
```



---

archive/issue_comments_460349.json:
```json
{
    "body": "**Changing commit** from \"[9352525b50851921f000335615e75e0f1bdd5f2e](https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e)\" to \"\".",
    "created_at": "2020-04-25T08:52:42Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460349",
    "user": "https://github.com/vbraun"
}
```

**Changing commit** from "[9352525b50851921f000335615e75e0f1bdd5f2e](https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e)" to "".



---

archive/issue_comments_460350.json:
```json
{
    "body": "<a id='comment:11'></a>\nGot rid of the dictionary printing in the tests.\n\n---\n**New commits:**\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2860b021050540ec678324ada4393e9ea78f603c\">2860b02</a></td><td><code>expose incidence matrix computed by backend cdd</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b13fb31d2cdb711bc234868a260d37fcf7d1e3f2\">b13fb31</a></td><td><code>add doctests that 29176 is fixed</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/581f5547b85ee1971c24f3c8580c2f04950b6b32\">581f554</a></td><td><code>fixed dictionary in python2</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fb2d66a0e71f164e9bef37922c8d544b9b270c9c\">fb2d66a</a></td><td><code>fix test by avoiding printing the dictionary at all</code></td></tr></table>\n",
    "created_at": "2020-04-25T22:12:49Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460350",
    "user": "https://github.com/kliem"
}
```

<a id='comment:11'></a>
Got rid of the dictionary printing in the tests.

---
**New commits:**
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2860b021050540ec678324ada4393e9ea78f603c">2860b02</a></td><td><code>expose incidence matrix computed by backend cdd</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b13fb31d2cdb711bc234868a260d37fcf7d1e3f2">b13fb31</a></td><td><code>add doctests that 29176 is fixed</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/581f5547b85ee1971c24f3c8580c2f04950b6b32">581f554</a></td><td><code>fixed dictionary in python2</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fb2d66a0e71f164e9bef37922c8d544b9b270c9c">fb2d66a</a></td><td><code>fix test by avoiding printing the dictionary at all</code></td></tr></table>




---

archive/issue_comments_460351.json:
```json
{
    "body": "**Commit:** [fb2d66a0e71f164e9bef37922c8d544b9b270c9c](https://github.com/sagemath/sagetrac-mirror/commit/fb2d66a0e71f164e9bef37922c8d544b9b270c9c)",
    "created_at": "2020-04-25T22:12:49Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460351",
    "user": "https://github.com/kliem"
}
```

**Commit:** [fb2d66a0e71f164e9bef37922c8d544b9b270c9c](https://github.com/sagemath/sagetrac-mirror/commit/fb2d66a0e71f164e9bef37922c8d544b9b270c9c)



---

archive/issue_events_261771.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-04-25T22:12:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-261771"
}
```



---

archive/issue_comments_460352.json:
```json
{
    "body": "**Changing branch** from \"[9352525b50851921f000335615e75e0f1bdd5f2e](https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e)\" to \"[public/29176-reb](https://github.com/sagemath/sagetrac-mirror/tree/public/29176-reb)\".",
    "created_at": "2020-04-25T22:12:49Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460352",
    "user": "https://github.com/kliem"
}
```

**Changing branch** from "[9352525b50851921f000335615e75e0f1bdd5f2e](https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e)" to "[public/29176-reb](https://github.com/sagemath/sagetrac-mirror/tree/public/29176-reb)".



---

archive/issue_events_261772.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-04-27T07:24:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-261772"
}
```



---

archive/issue_events_261773.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-04-27T07:24:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-261773"
}
```



---

archive/issue_comments_460353.json:
```json
{
    "body": "<a id='comment:13'></a>\nThank you.",
    "created_at": "2020-04-27T07:24:46Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460353",
    "user": "https://github.com/kliem"
}
```

<a id='comment:13'></a>
Thank you.



---

archive/issue_comments_460354.json:
```json
{
    "body": "**Changing branch** from \"[public/29176-reb](https://github.com/sagemath/sagetrac-mirror/tree/public/29176-reb)\" to \"[fb2d66a0e71f164e9bef37922c8d544b9b270c9c](https://github.com/sagemath/sagetrac-mirror/commit/fb2d66a0e71f164e9bef37922c8d544b9b270c9c)\".",
    "created_at": "2020-05-02T21:58:26Z",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460354",
    "user": "https://github.com/vbraun"
}
```

**Changing branch** from "[public/29176-reb](https://github.com/sagemath/sagetrac-mirror/tree/public/29176-reb)" to "[fb2d66a0e71f164e9bef37922c8d544b9b270c9c](https://github.com/sagemath/sagetrac-mirror/commit/fb2d66a0e71f164e9bef37922c8d544b9b270c9c)".



---

archive/issue_events_261774.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-05-02T21:58:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-261774"
}
```



---

archive/issue_events_261775.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "cbcab1ad339232b14004cb08956f6e9c8b19ee49",
    "created_at": "2020-05-02T21:58:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-261775"
}
```

# Issue 29810: Add package poetry

archive/issues_029573.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nThis may be required by some newer Python packages (see #29803). Unfortunately it has LOTS of dependencies.\n\n\nComponent: **build**\n\nBranch: **[u/mkoeppe/add_package_poetry](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/add_package_poetry)**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29810_\n\n",
    "created_at": "2020-06-05T22:22:58Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20build",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wishlist%20item"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add package poetry",
    "type": "issue",
    "updated_at": "2020-06-05T22:23:40Z",
    "url": "https://github.com/sagemath/sage/issues/29810",
    "user": "https://github.com/mkoeppe"
}
```
<div id="comment:0"></div>

This may be required by some newer Python packages (see #29803). Unfortunately it has LOTS of dependencies.


Component: **build**

Branch: **[u/mkoeppe/add_package_poetry](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/add_package_poetry)**

_Issue created by migration from https://trac.sagemath.org/ticket/29810_





---

archive/issue_events_407740.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-05T22:22:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29810",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20build",
    "label_color": "0000b0",
    "label_name": "c: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29810#event-407740"
}
```



---

archive/issue_events_407741.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-05T22:22:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29810",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29810#event-407741"
}
```



---

archive/issue_events_407742.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-05T22:22:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29810",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29810#event-407742"
}
```



---

archive/issue_events_407743.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-05T22:22:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29810",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29810#event-407743"
}
```



---

archive/issue_comments_470689.json:
```json
{
    "body": "Branch: **[u/mkoeppe/add_package_poetry](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/add_package_poetry)**",
    "created_at": "2020-06-05T22:23:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29810",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29810#issuecomment-470689",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/add_package_poetry](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/add_package_poetry)**

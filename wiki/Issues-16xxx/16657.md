# Issue 16657: Fix containment test for bounded integer sequences

archive/issues_016420.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\n\n\nComponent: **PLEASE CHANGE**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16657_\n\n",
    "closed_at": "2014-10-30T10:12:37Z",
    "created_at": "2014-07-15T12:48:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix containment test for bounded integer sequences",
    "type": "issue",
    "updated_at": "2014-10-30T10:12:37Z",
    "url": "https://github.com/sagemath/sage/issues/16657",
    "user": "https://github.com/simon-king-jena"
}
```
<div id="comment:0"></div>



Component: **PLEASE CHANGE**

_Issue created by migration from https://trac.sagemath.org/ticket/16657_





---

archive/issue_events_236046.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2014-07-15T12:48:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16657",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16657#event-236046"
}
```



---

archive/issue_events_236047.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2014-07-15T12:48:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16657",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16657#event-236047"
}
```



---

archive/issue_events_236048.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16657",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16657#event-236048"
}
```



---

archive/issue_events_236049.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16657",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16657#event-236049"
}
```



---

archive/issue_comments_217276.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nWhat's the purpose of this ticket?",
    "created_at": "2014-10-29T08:06:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16657#issuecomment-217276",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">comment:2</div>

What's the purpose of this ticket?



---

archive/issue_comments_217277.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI didn't open it. The only strange thing is that the title refers to my work, and actually I think it is identical with some commit message I did on #15820. Either someone has hacked my account, or the git trac scripts have a bug (say, `git trac push 15820` creating a new ticket).",
    "created_at": "2014-10-29T10:43:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16657#issuecomment-217277",
    "user": "https://github.com/simon-king-jena"
}
```

<div id="comment:3" align="right">comment:3</div>

I didn't open it. The only strange thing is that the title refers to my work, and actually I think it is identical with some commit message I did on #15820. Either someone has hacked my account, or the git trac scripts have a bug (say, `git trac push 15820` creating a new ticket).



---

archive/issue_comments_217278.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nNote also that this ticket immediately follows the \"help\" ticket #16656.",
    "created_at": "2014-10-29T10:46:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16657#issuecomment-217278",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">comment:4</div>

Note also that this ticket immediately follows the "help" ticket #16656.



---

archive/issue_events_236050.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-30T10:12:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16657",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16657#event-236050"
}
```



---

archive/issue_events_236051.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-30T10:12:37Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16657",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16657#event-236051"
}
```

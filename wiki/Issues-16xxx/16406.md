# Issue 16406: Make NN a semiring by default.

archive/issues_016169.json:
```json
{
    "body": "Currently we need to ask explicitly to have NN be a semiring:\n\n```\nsage: NN = NonNegativeIntegers(category=Semirings() & InfiniteEnumeratedSets())\nsage: NN in Semirings()\nTrue\n```\n\nThis would be a natural default.\n\n**CC:**  sage-combinat @tscrim @simon-king-jena\n\n**Reviewer:** Matthias Koeppe\n\nIssue created by migration from https://trac.sagemath.org/ticket/16406\n\n",
    "closed_at": "2021-08-03T15:30:52Z",
    "created_at": "2014-05-28T07:28:32Z",
    "labels": [
        "component: algebra",
        "enhancement",
        "invalid"
    ],
    "title": "Make NN a semiring by default.",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/16406",
    "user": "https://github.com/nthiery"
}
```
Currently we need to ask explicitly to have NN be a semiring:

```
sage: NN = NonNegativeIntegers(category=Semirings() & InfiniteEnumeratedSets())
sage: NN in Semirings()
True
```

This would be a natural default.

**CC:**  sage-combinat @tscrim @simon-king-jena

**Reviewer:** Matthias Koeppe

Issue created by migration from https://trac.sagemath.org/ticket/16406





---

archive/issue_comments_214326.json:
```json
{
    "body": "<a id='comment:1'></a>\nThere's also the `NonNegativeIntegerSemiring()` that could likely go/be merged too.",
    "created_at": "2014-05-28T14:38:46Z",
    "issue": "https://github.com/sagemath/sage/issues/16406",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16406#issuecomment-214326",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'></a>
There's also the `NonNegativeIntegerSemiring()` that could likely go/be merged too.



---

archive/issue_events_147631.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16406",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16406#event-147631"
}
```



---

archive/issue_events_147632.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16406",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16406#event-147632"
}
```



---

archive/issue_comments_214327.json:
```json
{
    "body": "<a id='comment:3'></a>\nThis seem now to be the case by default\n\n```\nsage: NN.category()                                                             \nCategory of facade infinite enumerated commutative semirings\nsage: NN in Semirings()                                                         \nTrue\n```\nso we should probably close as invalid ?",
    "created_at": "2021-08-02T12:31:39Z",
    "issue": "https://github.com/sagemath/sage/issues/16406",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16406#issuecomment-214327",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'></a>
This seem now to be the case by default

```
sage: NN.category()                                                             
Category of facade infinite enumerated commutative semirings
sage: NN in Semirings()                                                         
True
```
so we should probably close as invalid ?



---

archive/issue_events_147633.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-08-02T12:31:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16406",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16406#event-147633"
}
```



---

archive/issue_events_147634.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-08-02T12:31:39Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16406",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16406#event-147634"
}
```



---

archive/issue_comments_214328.json:
```json
{
    "body": "**Reviewer:** Matthias Koeppe",
    "created_at": "2021-08-02T17:39:54Z",
    "issue": "https://github.com/sagemath/sage/issues/16406",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16406#issuecomment-214328",
    "user": "https://github.com/mkoeppe"
}
```

**Reviewer:** Matthias Koeppe



---

archive/issue_events_147635.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-02T17:39:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16406",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16406#event-147635"
}
```



---

archive/issue_events_147636.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-02T17:39:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16406",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16406#event-147636"
}
```



---

archive/issue_events_147637.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-08-03T15:30:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16406",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16406#event-147637"
}
```



---

archive/issue_events_147638.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-08-03T15:30:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16406",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16406#event-147638"
}
```

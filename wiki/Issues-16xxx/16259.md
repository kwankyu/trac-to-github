# Issue 16259: allow pkg installation via sage -i http://github.com/...tarball.tar.gz

archive/issues_016022.json:
```json
{
    "body": "Github projects, when tagged via `git tag`, get automatically packaged (see project page / releases), and the URL has a standardized form, as well as the tarball. So, it would be most convenient if a project like patchbot that adheres to the SPKG directory structure could be installed by just saying\n\n```\nsage -i https://github.com/robertwb/sage-patchbot/archive/2.1.1.tar.gz\n```\nSage would then take the `PACKAGE_NAME` from `SPKG.txt`, or it could be given as optional argument to `sage -i`.\n\n**CC:**  @nexttime\n\n**Keywords:** spkg, installation, http\n\n**Reviewer:** Jeroen Demeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/16259\n\n",
    "closed_at": "2015-04-13T17:46:51Z",
    "created_at": "2014-04-28T14:37:27Z",
    "labels": [
        "component: scripts",
        "enhancement",
        "duplicate"
    ],
    "title": "allow pkg installation via sage -i http://github.com/...tarball.tar.gz",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/16259",
    "user": "https://github.com/rwst"
}
```
Github projects, when tagged via `git tag`, get automatically packaged (see project page / releases), and the URL has a standardized form, as well as the tarball. So, it would be most convenient if a project like patchbot that adheres to the SPKG directory structure could be installed by just saying

```
sage -i https://github.com/robertwb/sage-patchbot/archive/2.1.1.tar.gz
```
Sage would then take the `PACKAGE_NAME` from `SPKG.txt`, or it could be given as optional argument to `sage -i`.

**CC:**  @nexttime

**Keywords:** spkg, installation, http

**Reviewer:** Jeroen Demeyer

Issue created by migration from https://trac.sagemath.org/ticket/16259





---

archive/issue_events_146105.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16259",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16259#event-146105"
}
```



---

archive/issue_events_146106.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16259",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16259#event-146106"
}
```



---

archive/issue_events_146107.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16259",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16259#event-146107"
}
```



---

archive/issue_events_146108.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16259",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16259#event-146108"
}
```



---

archive/issue_events_146109.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-02-17T13:53:05Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16259",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16259#event-146109"
}
```



---

archive/issue_events_146110.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-02-17T13:53:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16259",
    "label": "wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16259#event-146110"
}
```



---

archive/issue_events_146111.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-13T07:21:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16259",
    "label": "wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16259#event-146111"
}
```



---

archive/issue_comments_210732.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2015-04-13T07:21:42Z",
    "issue": "https://github.com/sagemath/sage/issues/16259",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16259#issuecomment-210732",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_comments_210733.json:
```json
{
    "body": "<a id='comment:5'></a>\nDuplicate of #16501.",
    "created_at": "2015-04-13T07:21:42Z",
    "issue": "https://github.com/sagemath/sage/issues/16259",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16259#issuecomment-210733",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:5'></a>
Duplicate of #16501.



---

archive/issue_events_146112.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-13T07:21:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16259",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16259#event-146112"
}
```



---

archive/issue_events_146113.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-13T07:21:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16259",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16259#event-146113"
}
```



---

archive/issue_events_146114.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-13T07:21:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16259",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16259#event-146114"
}
```



---

archive/issue_events_146115.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-13T17:46:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16259",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16259#event-146115"
}
```



---

archive/issue_events_146116.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-13T17:46:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16259",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16259#event-146116"
}
```



---

archive/issue_events_146117.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-13T17:46:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16259",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16259#event-146117"
}
```

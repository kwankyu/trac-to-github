# Issue 16046: parameter allow_subfield of absolute_order_from_module_generators has no effect

archive/issues_015809.json:
```json
{
    "body": "The following doctest in `sage.rings.number_field.order.absolute_order_from_module_generators` is incorrect:\n\n```\nsage: F.<alpha> = NumberField(x**4+3)\nsage: F.order([alpha**2], allow_subfield=True)\nOrder in Number Field in alpha with defining polynomial x^4 + 3\n```\n\nThis should be a order in a smaller field. The code for `allow_subfield` seems incomplete.\n\n**CC:**  jakobkroeker\n\n**Stopgaps:** wrongAnswerMarker\n\n**Reviewer:** Julian R\u00fcth\n\nIssue created by migration from https://trac.sagemath.org/ticket/16046\n\n",
    "closed_at": "2018-05-18T17:16:26Z",
    "created_at": "2014-04-02T11:53:51Z",
    "labels": [
        "component: number theory",
        "minor",
        "bug",
        "wontfix"
    ],
    "title": "parameter allow_subfield of absolute_order_from_module_generators has no effect",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/16046",
    "user": "https://github.com/saraedum"
}
```
The following doctest in `sage.rings.number_field.order.absolute_order_from_module_generators` is incorrect:

```
sage: F.<alpha> = NumberField(x**4+3)
sage: F.order([alpha**2], allow_subfield=True)
Order in Number Field in alpha with defining polynomial x^4 + 3
```

This should be a order in a smaller field. The code for `allow_subfield` seems incomplete.

**CC:**  jakobkroeker

**Stopgaps:** wrongAnswerMarker

**Reviewer:** Julian Rüth

Issue created by migration from https://trac.sagemath.org/ticket/16046





---

archive/issue_events_143792.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16046",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16046#event-143792"
}
```



---

archive/issue_events_143793.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16046",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16046#event-143793"
}
```



---

archive/issue_events_143794.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16046",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16046#event-143794"
}
```



---

archive/issue_events_143795.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16046",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16046#event-143795"
}
```



---

archive/issue_comments_206725.json:
```json
{
    "body": "**Stopgaps:** wrongAnswerMarker",
    "created_at": "2017-03-04T00:24:21Z",
    "issue": "https://github.com/sagemath/sage/issues/16046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16046#issuecomment-206725",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

**Stopgaps:** wrongAnswerMarker



---

archive/issue_comments_206726.json:
```json
{
    "body": "<a id='comment:4'></a>\nInteresting. I just ran into this problem again. We never added the stopgap actually.",
    "created_at": "2018-03-09T11:01:04Z",
    "issue": "https://github.com/sagemath/sage/issues/16046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16046#issuecomment-206726",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:4'></a>
Interesting. I just ran into this problem again. We never added the stopgap actually.



---

archive/issue_comments_206727.json:
```json
{
    "body": "<a id='comment:5'></a>\nFixed in #24934.",
    "created_at": "2018-03-09T12:49:56Z",
    "issue": "https://github.com/sagemath/sage/issues/16046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16046#issuecomment-206727",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:5'></a>
Fixed in #24934.



---

archive/issue_events_143796.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-03-09T12:49:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16046",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16046#event-143796"
}
```



---

archive/issue_events_143797.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-03-09T12:49:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16046",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16046#event-143797"
}
```



---

archive/issue_events_143798.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-03-09T12:50:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16046",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16046#event-143798"
}
```



---

archive/issue_events_143799.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-03-09T12:50:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16046",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16046#event-143799"
}
```



---

archive/issue_comments_206728.json:
```json
{
    "body": "**Reviewer:** Julian R\u00fcth",
    "created_at": "2018-03-09T12:50:07Z",
    "issue": "https://github.com/sagemath/sage/issues/16046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16046#issuecomment-206728",
    "user": "https://github.com/saraedum"
}
```

**Reviewer:** Julian Rüth



---

archive/issue_events_143800.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16046",
    "label": "wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16046#event-143800"
}
```



---

archive/issue_comments_206729.json:
```json
{
    "body": "<a id='comment:7'></a>\nclosing positively reviewed duplicates",
    "created_at": "2018-05-18T17:16:26Z",
    "issue": "https://github.com/sagemath/sage/issues/16046",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16046#issuecomment-206729",
    "user": "https://github.com/videlec"
}
```

<a id='comment:7'></a>
closing positively reviewed duplicates



---

archive/issue_events_143801.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16046",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16046#event-143801"
}
```



---

archive/issue_events_143802.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16046",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16046#event-143802"
}
```

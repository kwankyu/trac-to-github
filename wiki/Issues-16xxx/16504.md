# Issue 16504: A mandatory 'nonnegative' argument for MixedIntegerLinearProgram.new_variable() until the standard changes

archive/issues_016267.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nThis should create an unbounded real variable:\n\n```\nsage: p = MixedIntegerLinearProgram()\nsage: x = p.new_variable(nonnegative=False) \n```\n\nWe should see `nonnegative` as a toggle, not as a type. The following should also work as expected intuitively:\n\n```\nsage: x = p.new_variable(integer=True, nonnegative=False)\nsage: x = p.new_variable(integer=True, nonnegative=True)\n```\n\nCC:  @nathanncohen @dimpase @vbraun @dcoudert @sagetrac-hartke @wdjoyner @johnperry-math @benjaminfjones @ingolfured\n\nComponent: **linear programming**\n\nAuthor: **Nathann Cohen**\n\nBranch/Commit: **[`45bc937`](https://github.com/sagemath/sagetrac-mirror/commit/45bc9377d525b94d7f9ddfbba15b9f074a324937)**\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16504_\n\n",
    "closed_at": "2014-07-06T17:55:54Z",
    "created_at": "2014-06-20T15:11:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20linear%20programming",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "A mandatory 'nonnegative' argument for MixedIntegerLinearProgram.new_variable() until the standard changes",
    "type": "issue",
    "updated_at": "2014-07-06T17:55:54Z",
    "url": "https://github.com/sagemath/sage/issues/16504",
    "user": "https://github.com/jdemeyer"
}
```
<div id="comment:0"></div>

This should create an unbounded real variable:

```
sage: p = MixedIntegerLinearProgram()
sage: x = p.new_variable(nonnegative=False) 
```

We should see `nonnegative` as a toggle, not as a type. The following should also work as expected intuitively:

```
sage: x = p.new_variable(integer=True, nonnegative=False)
sage: x = p.new_variable(integer=True, nonnegative=True)
```

CC:  @nathanncohen @dimpase @vbraun @dcoudert @sagetrac-hartke @wdjoyner @johnperry-math @benjaminfjones @ingolfured

Component: **linear programming**

Author: **Nathann Cohen**

Branch/Commit: **[`45bc937`](https://github.com/sagemath/sagetrac-mirror/commit/45bc9377d525b94d7f9ddfbba15b9f074a324937)**

Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/16504_





---

archive/issue_events_233634.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-06-20T15:11:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233634"
}
```



---

archive/issue_events_233635.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-06-20T15:11:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20linear%20programming",
    "label_color": "0000ff",
    "label_name": "c: linear programming",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233635"
}
```



---

archive/issue_events_233636.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-06-20T15:11:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233636"
}
```



---

archive/issue_events_233637.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-06-20T15:11:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233637"
}
```



---

archive/issue_comments_213786.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nI personally have no idea on the implementation of all this LP stuff. Would it be easy to implement this?",
    "created_at": "2014-06-20T15:18:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213786",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:1" align="right">comment:1</div>

I personally have no idea on the implementation of all this LP stuff. Would it be easy to implement this?



---

archive/issue_comments_213787.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nYes, it would be easy. The main problem is that if you want to change the current behaviour of the function (i.e. if you want nonnegative=False to be the default) then you will have to \"make nonnegative mandatory\" for a while and raise a warning when it is not set. Otherwise when you will change it, guys who currently use \"integer=True\" to mean \"nonnegative integer\" will find themselves with LP returning weird answers.\n\nThat was the point of #15521.\n\nAlso, it may seem weird to see all these variables be nonnegative by default but that's really the standard for LP solvers. The LP backends really assume non negativity unless asked otherwise, and changing the standard in Sage is really something that we do \"for the general mathematicians knowing that it could surprise guys used to LP\".\n\nNathann",
    "created_at": "2014-06-20T20:24:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213787",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:2" align="right">comment:2</div>

Yes, it would be easy. The main problem is that if you want to change the current behaviour of the function (i.e. if you want nonnegative=False to be the default) then you will have to "make nonnegative mandatory" for a while and raise a warning when it is not set. Otherwise when you will change it, guys who currently use "integer=True" to mean "nonnegative integer" will find themselves with LP returning weird answers.

That was the point of #15521.

Also, it may seem weird to see all these variables be nonnegative by default but that's really the standard for LP solvers. The LP backends really assume non negativity unless asked otherwise, and changing the standard in Sage is really something that we do "for the general mathematicians knowing that it could surprise guys used to LP".

Nathann



---

archive/issue_comments_213788.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nReplying to [@nathanncohen](#comment%3A2):\n> That was the point of #15521.\n\nI don't think that #15521 made things more clear...\n\nHow about this:\nwe require at most one of `integer`, `binary`, `real` to be given, with `real` the default if none is given.\n\n`nonnegative` is a boolean determining whether or not the variable is assumed to be nonnegative. In the function, we have `nonnegative=None` by default which is currently interpreted as `nonnegative=True` but with the warning from #15521.\n\nI think this proposal is backwards- and forwards-compatible (unlike #15521, which is backwards- but not forwards-compatible).",
    "created_at": "2014-06-20T20:49:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213788",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">comment:3</div>

Replying to [@nathanncohen](#comment%3A2):
> That was the point of #15521.

I don't think that #15521 made things more clear...

How about this:
we require at most one of `integer`, `binary`, `real` to be given, with `real` the default if none is given.

`nonnegative` is a boolean determining whether or not the variable is assumed to be nonnegative. In the function, we have `nonnegative=None` by default which is currently interpreted as `nonnegative=True` but with the warning from #15521.

I think this proposal is backwards- and forwards-compatible (unlike #15521, which is backwards- but not forwards-compatible).



---

archive/issue_comments_213789.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nReplying to [@nathanncohen](#comment%3A2):\n> The main problem is that if you want to change the current behaviour of the function (i.e. if you want nonnegative=False to be the default) then you will have to \"make nonnegative mandatory\" for a while and raise a warning when it is not set.\n\nI agree with this, but I don't see the problem. The deprecation warning from #15521 isn't a problem either...",
    "created_at": "2014-06-20T20:51:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213789",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">comment:4</div>

Replying to [@nathanncohen](#comment%3A2):
> The main problem is that if you want to change the current behaviour of the function (i.e. if you want nonnegative=False to be the default) then you will have to "make nonnegative mandatory" for a while and raise a warning when it is not set.

I agree with this, but I don't see the problem. The deprecation warning from #15521 isn't a problem either...



---

archive/issue_comments_213790.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nEssentially, the code should look like\n\n```python\ndef new_variable(self, real=False, binary=False, integer=False, nonnegative=None):\n    if sum([real, binary, integer]) >= 2:\n        raise ValueError(\"exactly one of the available types has to be True\")\n    \n    if nonnegative is None:\n        deprecation(15521, \"The default value of 'nonnegative' will change, to \"+\n                    \"False instead of True. \" +\n                    \"You should add the explicit 'nonnegative=True'.\")\n        nonnegative = True\n\n    if binary:\n        vtype = self.__BINARY\n    elif integer:\n        vtype = self.__INTEGER\n    else:  # real is the default if no type is explicitly given\n        vtype = self.__REAL\n        \n    # Set minimum value to 0 or -oo depending on \"nonnegative\"\n    ...\n```",
    "created_at": "2014-06-20T21:01:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213790",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:5" align="right">comment:5</div>

Essentially, the code should look like

```python
def new_variable(self, real=False, binary=False, integer=False, nonnegative=None):
    if sum([real, binary, integer]) >= 2:
        raise ValueError("exactly one of the available types has to be True")
    
    if nonnegative is None:
        deprecation(15521, "The default value of 'nonnegative' will change, to "+
                    "False instead of True. " +
                    "You should add the explicit 'nonnegative=True'.")
        nonnegative = True

    if binary:
        vtype = self.__BINARY
    elif integer:
        vtype = self.__INTEGER
    else:  # real is the default if no type is explicitly given
        vtype = self.__REAL
        
    # Set minimum value to 0 or -oo depending on "nonnegative"
    ...
```



---

archive/issue_comments_213791.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\n> I agree with this, but I don't see the problem. The deprecation warning from #15521 isn't a problem either...\n\nIt is not a problem, it just means that \"doing the job\" takes one year :-P\n\nNathann",
    "created_at": "2014-06-21T08:20:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213791",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:6" align="right">comment:6</div>

> I agree with this, but I don't see the problem. The deprecation warning from #15521 isn't a problem either...

It is not a problem, it just means that "doing the job" takes one year :-P

Nathann



---

archive/issue_comments_213792.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\n> I don't think that #15521 made things more clear...\n\nWell, then improve it if you do not like it :-P\n\n> How about this:\n> we require at most one of `integer`, `binary`, `real` to be given, with `real` the default if none is given.\n\nMakes sense. Even though, with the turn that events take, I wonder if it would not be better to require one of them to be explicitly given : no default. Again, nonnegative variables are the default in this field, and we switched to another. Requiring a variable would require the users to read the doc, which may avoid misunderstandings.\n\n> `nonnegative` is a boolean determining whether or not the variable is assumed to be nonnegative. In the function, we have `nonnegative=None` by default which is currently interpreted as `nonnegative=True` but with the warning from #15521.\n> \n> I think this proposal is backwards- and forwards-compatible (unlike #15521, which is backwards- but not forwards-compatible).\n\nIt does make sense.\n\nNathann",
    "created_at": "2014-06-21T08:23:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213792",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:7" align="right">comment:7</div>

> I don't think that #15521 made things more clear...

Well, then improve it if you do not like it :-P

> How about this:
> we require at most one of `integer`, `binary`, `real` to be given, with `real` the default if none is given.

Makes sense. Even though, with the turn that events take, I wonder if it would not be better to require one of them to be explicitly given : no default. Again, nonnegative variables are the default in this field, and we switched to another. Requiring a variable would require the users to read the doc, which may avoid misunderstandings.

> `nonnegative` is a boolean determining whether or not the variable is assumed to be nonnegative. In the function, we have `nonnegative=None` by default which is currently interpreted as `nonnegative=True` but with the warning from #15521.
> 
> I think this proposal is backwards- and forwards-compatible (unlike #15521, which is backwards- but not forwards-compatible).

It does make sense.

Nathann



---

archive/issue_comments_213793.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nYo !\n\n> Essentially, the code should look like\n\nLooks good.\n\nNathann",
    "created_at": "2014-06-21T08:24:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213793",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:8" align="right">comment:8</div>

Yo !

> Essentially, the code should look like

Looks good.

Nathann



---

archive/issue_comments_213794.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nSince `integer, binary, real` are mutually exclusive, why not simply have a `type` variable that takes one on the three values ?",
    "created_at": "2014-06-21T10:05:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213794",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<div id="comment:9" align="right">comment:9</div>

Since `integer, binary, real` are mutually exclusive, why not simply have a `type` variable that takes one on the three values ?



---

archive/issue_comments_213795.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nReplying to [@sagetrac-tmonteil](#comment%3A9):\n> Since `integer, binary, real` are mutually exclusive, why not simply have a `type` variable that takes one on the three values ?\n\nBackwards compatibility. With my proposal, we would have full backwards compatibility.",
    "created_at": "2014-06-22T18:22:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213795",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:10" align="right">comment:10</div>

Replying to [@sagetrac-tmonteil](#comment%3A9):
> Since `integer, binary, real` are mutually exclusive, why not simply have a `type` variable that takes one on the three values ?

Backwards compatibility. With my proposal, we would have full backwards compatibility.



---

archive/issue_comments_213796.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nReplying to [@nathanncohen](#comment%3A7):\n> Even though, with the turn that events take, I wonder if it would not be better to require one of them to be explicitly given : no default.\n\nAgain, for backwards compatibility, we cannot just do that.",
    "created_at": "2014-06-22T18:23:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213796",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:11" align="right">comment:11</div>

Replying to [@nathanncohen](#comment%3A7):
> Even though, with the turn that events take, I wonder if it would not be better to require one of them to be explicitly given : no default.

Again, for backwards compatibility, we cannot just do that.



---

archive/issue_comments_213797.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nReplying to [@nathanncohen](#comment%3A7):\n> nonnegative variables are the default in this field, and we switched to another.\n\nNo, we didn't switch, we just added a deprecation warning. This ticket doesn't change that.",
    "created_at": "2014-06-22T18:23:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213797",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:12" align="right">comment:12</div>

Replying to [@nathanncohen](#comment%3A7):
> nonnegative variables are the default in this field, and we switched to another.

No, we didn't switch, we just added a deprecation warning. This ticket doesn't change that.



---

archive/issue_comments_213798.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nReplying to [@nathanncohen](#comment%3A8):\n> Looks good.\n\nHi Nathann,\n\nGiven that you know this code and I don't, could you make the final implementation based on my pseudo-code?",
    "created_at": "2014-06-22T18:24:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213798",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:13" align="right">comment:13</div>

Replying to [@nathanncohen](#comment%3A8):
> Looks good.

Hi Nathann,

Given that you know this code and I don't, could you make the final implementation based on my pseudo-code?



---

archive/issue_comments_213799.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nYo !\n\n> Given that you know this code and I don't, could you make the final implementation\n\nHmmmm.... You are hardly a beginner with Sage, but let's make a deal : I implement this ticket and you finish #16308 using #16374 that has been merged in the meantime ? I need this for my combinatorial design stuff `:-P`\n\nNathann",
    "created_at": "2014-06-23T08:46:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213799",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:14" align="right">comment:14</div>

Yo !

> Given that you know this code and I don't, could you make the final implementation

Hmmmm.... You are hardly a beginner with Sage, but let's make a deal : I implement this ticket and you finish #16308 using #16374 that has been merged in the meantime ? I need this for my combinatorial design stuff `:-P`

Nathann



---

archive/issue_events_233638.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-06-23T10:55:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233638"
}
```



---

archive/issue_comments_213800.json:
```json
{
    "body": "Branch: **[u/ncohen/16504](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/16504)**",
    "created_at": "2014-06-23T10:55:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213800",
    "user": "https://github.com/nathanncohen"
}
```

Branch: **[u/ncohen/16504](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/16504)**



---

archive/issue_comments_213801.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nHello !\n\nHere is the branch. Took much longer than I thought.\n\nWhat it does :\n- Removes the `is_nonnegative`, `set_nonnegative` functions that were introduced in #15521 as 'nonnegative' is not meant to be a variable type but a  'property' of any type. Those functions are renamed to `is_real` and `set_real`, i.e. their status before #16504\n\n- It was formerly possible to do things like that \n\n```\nsage: p = MixedIntegerLinearProgram()\nsage: v = p.new_variable()\nsage: p.set_min(v[0],15)\nsage: p.get_min(v[0])\n15.0\n```\n  but it was NOT possible to do things like that\n\n```\nsage: p.set_min(v,17)\nsage: p.get_min(v[18])\n17.0\n```\ni.e. it was possible to define upper/lower bound on a specific variable, but not to do the same on a dictionary of variables in such a way that the new variables created fro the dictionary will inherit the bounds. But that's exactly what we wanted to do here.\n\n  Thus I needed to add arguments lower/upper bound to the constructor of `MIPVariable`, plus set functions where it was needed.\n\nWell... `needs_review` !\n\nNathann",
    "created_at": "2014-06-23T10:55:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213801",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:15" align="right">comment:15</div>

Hello !

Here is the branch. Took much longer than I thought.

What it does :
- Removes the `is_nonnegative`, `set_nonnegative` functions that were introduced in #15521 as 'nonnegative' is not meant to be a variable type but a  'property' of any type. Those functions are renamed to `is_real` and `set_real`, i.e. their status before #16504

- It was formerly possible to do things like that 

```
sage: p = MixedIntegerLinearProgram()
sage: v = p.new_variable()
sage: p.set_min(v[0],15)
sage: p.get_min(v[0])
15.0
```
  but it was NOT possible to do things like that

```
sage: p.set_min(v,17)
sage: p.get_min(v[18])
17.0
```
i.e. it was possible to define upper/lower bound on a specific variable, but not to do the same on a dictionary of variables in such a way that the new variables created fro the dictionary will inherit the bounds. But that's exactly what we wanted to do here.

  Thus I needed to add arguments lower/upper bound to the constructor of `MIPVariable`, plus set functions where it was needed.

Well... `needs_review` !

Nathann



---

archive/issue_events_233639.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-06-23T10:56:11Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "title_is": "A mandatory 'nonnegative' argument for MixedIntegerLinearProgram.new_variable() until the standard changes",
    "title_was": "Fix the confusion in MILP.new_variable()",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233639"
}
```



---

archive/issue_comments_213802.json:
```json
{
    "body": "Commit: **[`b1f43af`](https://github.com/sagemath/sagetrac-mirror/commit/b1f43af754d4ab7e136015519c7fc0cdbe32dd36)**",
    "created_at": "2014-06-23T10:56:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213802",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[`b1f43af`](https://github.com/sagemath/sagetrac-mirror/commit/b1f43af754d4ab7e136015519c7fc0cdbe32dd36)**



---

archive/issue_comments_213803.json:
```json
{
    "body": "<div id=\"comment:17\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b1f43af754d4ab7e136015519c7fc0cdbe32dd36\"><code>b1f43af</code></a></td><td><code>trac #16504: A mandatory 'nonnegative' argument for MixedIntegerLinearProgram.new_variable() until the standard changes</code></td></tr></table>\n",
    "created_at": "2014-06-23T10:56:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213803",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:17"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b1f43af754d4ab7e136015519c7fc0cdbe32dd36"><code>b1f43af</code></a></td><td><code>trac #16504: A mandatory 'nonnegative' argument for MixedIntegerLinearProgram.new_variable() until the standard changes</code></td></tr></table>




---

archive/issue_comments_213804.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">comment:18</div>\n\nAs now `p.set_min(v,17)` becomes possible, why not have optional `max` and `min` in `new_variable()` (many solvers have it, at least per variable) ? E.g. `v=p.new_variable(min=17, max=18)`. A usual application is relaxation of binary IPs, so one would have real variables `p.new_variable(min=0, max=1)`.",
    "created_at": "2014-06-23T11:27:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213804",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:18" align="right">comment:18</div>

As now `p.set_min(v,17)` becomes possible, why not have optional `max` and `min` in `new_variable()` (many solvers have it, at least per variable) ? E.g. `v=p.new_variable(min=17, max=18)`. A usual application is relaxation of binary IPs, so one would have real variables `p.new_variable(min=0, max=1)`.



---

archive/issue_comments_213805.json:
```json
{
    "body": "Author: **Nathann Cohen**",
    "created_at": "2014-06-23T11:30:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213805",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Nathann Cohen**



---

archive/issue_comments_213806.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">comment:19</div>\n\nReplying to [@dimpase](#comment%3A18):\n> As now `p.set_min(v,17)` becomes possible, why not have optional `max` and `min` in `new_variable()` (many solvers have it, at least per variable) ? E.g. `v=p.new_variable(min=17, max=18)`. A usual application is relaxation of binary IPs, so one would have real variables `p.new_variable(min=0, max=1)`.\n\nI agree, but perhaps not on this ticket. Let's try not to do too many things at the same time.",
    "created_at": "2014-06-23T11:30:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213806",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:19" align="right">comment:19</div>

Replying to [@dimpase](#comment%3A18):
> As now `p.set_min(v,17)` becomes possible, why not have optional `max` and `min` in `new_variable()` (many solvers have it, at least per variable) ? E.g. `v=p.new_variable(min=17, max=18)`. A usual application is relaxation of binary IPs, so one would have real variables `p.new_variable(min=0, max=1)`.

I agree, but perhaps not on this ticket. Let's try not to do too many things at the same time.



---

archive/issue_comments_213807.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">comment:21</div>\n\nCan you undo this change:\n\n```diff\n@@ -241,11 +239,10 @@ cdef class MixedIntegerLinearProgram(SageObject):\nsage: g = graphs.PetersenGraph()\nsage: p = MixedIntegerLinearProgram(maximization=True)\n- sage: b = p.new_variable()\n+ sage: b = p.new_variable(binary=True)\nsage: p.set_objective(sum([b[v] for v in g]))\nsage: for (u,v) in g.edges(labels=None):\n... p.add_constraint(b[u] + b[v], max=1)\n- sage: p.set_binary(b)\nsage: p.solve(objective_only=True)\n4.0\n\"\"\"\n```",
    "created_at": "2014-06-23T12:25:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213807",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:21" align="right">comment:21</div>

Can you undo this change:

```diff
@@ -241,11 +239,10 @@ cdef class MixedIntegerLinearProgram(SageObject):
sage: g = graphs.PetersenGraph()
sage: p = MixedIntegerLinearProgram(maximization=True)
- sage: b = p.new_variable()
+ sage: b = p.new_variable(binary=True)
sage: p.set_objective(sum([b[v] for v in g]))
sage: for (u,v) in g.edges(labels=None):
... p.add_constraint(b[u] + b[v], max=1)
- sage: p.set_binary(b)
sage: p.solve(objective_only=True)
4.0
"""
```



---

archive/issue_comments_213808.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">comment:22</div>\n\nAnd why remove this warning?\n\n```diff\n.. WARNING::\n-\n- By default, all ``x[i]`` are assumed to be non-negative. See\n- :meth:`set_min` to set a different lower bound.\n```",
    "created_at": "2014-06-23T12:26:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213808",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:22" align="right">comment:22</div>

And why remove this warning?

```diff
.. WARNING::
-
- By default, all ``x[i]`` are assumed to be non-negative. See
- :meth:`set_min` to set a different lower bound.
```



---

archive/issue_comments_213809.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2014-06-23T12:26:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213809",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_213810.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">comment:23</div>\n\nYou should add more doctests for `nonnegative=False`. As far as I can see, there is no doctest for an actual MILP problem with `nonnegative=False` variables. You should doctest this both for `integer` and `real` types.",
    "created_at": "2014-06-23T12:28:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213810",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:23" align="right">comment:23</div>

You should add more doctests for `nonnegative=False`. As far as I can see, there is no doctest for an actual MILP problem with `nonnegative=False` variables. You should doctest this both for `integer` and `real` types.



---

archive/issue_events_233640.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-06-23T12:28:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233640"
}
```



---

archive/issue_events_233641.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-06-23T12:28:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233641"
}
```



---

archive/issue_comments_213811.json:
```json
{
    "body": "Work Issues: **nonnegative=False doctests**",
    "created_at": "2014-06-23T12:28:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213811",
    "user": "https://github.com/jdemeyer"
}
```

Work Issues: **nonnegative=False doctests**



---

archive/issue_comments_213812.json:
```json
{
    "body": "Changed reviewer from **Jeroen Demeyer** to none",
    "created_at": "2014-06-23T12:34:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213812",
    "user": "https://github.com/dimpase"
}
```

Changed reviewer from **Jeroen Demeyer** to none



---

archive/issue_comments_213813.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">comment:24</div>\n\nAm I correct that this is a safe change in the sense that the existing code using `nonnegative=True` as default would either continue to work, or will refuse to work? I.e. people won't possibly start getting wrong answers?",
    "created_at": "2014-06-23T12:34:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213813",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:24" align="right">comment:24</div>

Am I correct that this is a safe change in the sense that the existing code using `nonnegative=True` as default would either continue to work, or will refuse to work? I.e. people won't possibly start getting wrong answers?



---

archive/issue_comments_213814.json:
```json
{
    "body": "Changed work issues from **nonnegative=False doctests** to none",
    "created_at": "2014-06-23T12:34:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213814",
    "user": "https://github.com/dimpase"
}
```

Changed work issues from **nonnegative=False doctests** to none



---

archive/issue_comments_213815.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">comment:25</div>\n\nReplying to [@dimpase](#comment%3A24):\n> Am I correct that this is a safe change in the sense that the existing code using `nonnegative=True` as default would either continue to work, or will refuse to work? I.e. people won't possibly start getting wrong answers?\n\nOld code should work exactly as before, possibly with a deprecation warning.",
    "created_at": "2014-06-23T12:38:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213815",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:25" align="right">comment:25</div>

Replying to [@dimpase](#comment%3A24):
> Am I correct that this is a safe change in the sense that the existing code using `nonnegative=True` as default would either continue to work, or will refuse to work? I.e. people won't possibly start getting wrong answers?

Old code should work exactly as before, possibly with a deprecation warning.



---

archive/issue_comments_213816.json:
```json
{
    "body": "Work Issues: **nonnegative=False doctests**",
    "created_at": "2014-06-23T12:39:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213816",
    "user": "https://github.com/jdemeyer"
}
```

Work Issues: **nonnegative=False doctests**



---

archive/issue_comments_213817.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2014-06-23T12:39:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213817",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_213818.json:
```json
{
    "body": "<div id=\"comment:27\" align=\"right\">comment:27</div>\n\nCould you replace\n\n```\nif isinstance(v, MIPVariable):\n    v.set_min(min)\nelse:\n    self._backend.variable_lower_bound(self._variables[v], min)\n```\nby the more Pythonic\n\n```\ntry:\n    v.set_min(min)\nexcept AttributeError:\n    self._backend.variable_lower_bound(self._variables[v], min)\n```\n\nMy personal opinion is that `v.set_min(min)` should actually work in all cases, but that's perhaps for a new ticket.",
    "created_at": "2014-06-23T12:48:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213818",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:27" align="right">comment:27</div>

Could you replace

```
if isinstance(v, MIPVariable):
    v.set_min(min)
else:
    self._backend.variable_lower_bound(self._variables[v], min)
```
by the more Pythonic

```
try:
    v.set_min(min)
except AttributeError:
    self._backend.variable_lower_bound(self._variables[v], min)
```

My personal opinion is that `v.set_min(min)` should actually work in all cases, but that's perhaps for a new ticket.



---

archive/issue_comments_213819.json:
```json
{
    "body": "Changed work issues from **nonnegative=False doctests** to **nonnegative=False doctests, avoid isinstance()**",
    "created_at": "2014-06-23T12:48:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213819",
    "user": "https://github.com/jdemeyer"
}
```

Changed work issues from **nonnegative=False doctests** to **nonnegative=False doctests, avoid isinstance()**



---

archive/issue_comments_213820.json:
```json
{
    "body": "<div id=\"comment:29\" align=\"right\">comment:29</div>\n\n> Can you undo this change:\n\nWhy ? `O_o`\n\nIt's better in the new version, isn't it ? `O_o`\n\nNathann",
    "created_at": "2014-06-23T14:13:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213820",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:29" align="right">comment:29</div>

> Can you undo this change:

Why ? `O_o`

It's better in the new version, isn't it ? `O_o`

Nathann



---

archive/issue_comments_213821.json:
```json
{
    "body": "<div id=\"comment:30\" align=\"right\">comment:30</div>\n\n> And why remove this warning?\n> \n> ```diff\n> .. WARNING::\n> -\n> - By default, all ``x[i]`` are assumed to be non-negative. See\n> - :meth:`set_min` to set a different lower bound.\n> ```\n\nBecause there is no need for a warning anymore : now the user explicitly says whether he wants the variables to be nonnegative or not, or he gets a warning.\n\nPlus it will be the other way around next year.\n\nNathann",
    "created_at": "2014-06-23T14:15:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213821",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:30" align="right">comment:30</div>

> And why remove this warning?
> 
> ```diff
> .. WARNING::
> -
> - By default, all ``x[i]`` are assumed to be non-negative. See
> - :meth:`set_min` to set a different lower bound.
> ```

Because there is no need for a warning anymore : now the user explicitly says whether he wants the variables to be nonnegative or not, or he gets a warning.

Plus it will be the other way around next year.

Nathann



---

archive/issue_comments_213822.json:
```json
{
    "body": "<div id=\"comment:31\" align=\"right\">comment:31</div>\n\n> Am I correct that this is a safe change in the sense that the existing code using `nonnegative=True` as default would either continue to work, or will refuse to work? I.e. people won't possibly start getting wrong answers?\n\nIt will work, and there should be no wrong answer (unless there is a bug somewhere). The default behaviour is now \"nonnegative + warning\", and the correct way to use this class is to specify a value for nonnegative.\n\nNathann",
    "created_at": "2014-06-23T14:16:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213822",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:31" align="right">comment:31</div>

> Am I correct that this is a safe change in the sense that the existing code using `nonnegative=True` as default would either continue to work, or will refuse to work? I.e. people won't possibly start getting wrong answers?

It will work, and there should be no wrong answer (unless there is a bug somewhere). The default behaviour is now "nonnegative + warning", and the correct way to use this class is to specify a value for nonnegative.

Nathann



---

archive/issue_events_233642.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-06-23T14:25:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233642"
}
```



---

archive/issue_events_233643.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-06-23T14:25:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233643"
}
```



---

archive/issue_comments_213823.json:
```json
{
    "body": "Changed commit from **[`b1f43af`](https://github.com/sagemath/sagetrac-mirror/commit/b1f43af754d4ab7e136015519c7fc0cdbe32dd36)** to **[`97b2f1f`](https://github.com/sagemath/sagetrac-mirror/commit/97b2f1fa06456de3e54da1787ce20915aceb8ad9)**",
    "created_at": "2014-06-23T14:25:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213823",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`b1f43af`](https://github.com/sagemath/sagetrac-mirror/commit/b1f43af754d4ab7e136015519c7fc0cdbe32dd36)** to **[`97b2f1f`](https://github.com/sagemath/sagetrac-mirror/commit/97b2f1fa06456de3e54da1787ce20915aceb8ad9)**



---

archive/issue_comments_213824.json:
```json
{
    "body": "<div id=\"comment:33\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5f81c2d22d339dac4464ed666f6ec34b15f20a9a\"><code>5f81c2d</code></a></td><td><code>trac #16504: Tastes and colors ................</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/97b2f1fa06456de3e54da1787ce20915aceb8ad9\"><code>97b2f1f</code></a></td><td><code>trac #16504: Broken doctest</code></td></tr></table>\n",
    "created_at": "2014-06-23T14:25:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213824",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:33"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5f81c2d22d339dac4464ed666f6ec34b15f20a9a"><code>5f81c2d</code></a></td><td><code>trac #16504: Tastes and colors ................</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/97b2f1fa06456de3e54da1787ce20915aceb8ad9"><code>97b2f1f</code></a></td><td><code>trac #16504: Broken doctest</code></td></tr></table>




---

archive/issue_comments_213825.json:
```json
{
    "body": "<div id=\"comment:34\"></div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e9ac71d0d8bd2334b7f13ddf7d974deed35906b1\"><code>e9ac71d</code></a></td><td><code>trac #16504: Tastes and colors ................</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d437be4a258a648e86f8cc1845ade75467609601\"><code>d437be4</code></a></td><td><code>trac #16504: Broken doctest</code></td></tr></table>\n",
    "created_at": "2014-06-23T14:28:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213825",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:34"></div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e9ac71d0d8bd2334b7f13ddf7d974deed35906b1"><code>e9ac71d</code></a></td><td><code>trac #16504: Tastes and colors ................</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d437be4a258a648e86f8cc1845ade75467609601"><code>d437be4</code></a></td><td><code>trac #16504: Broken doctest</code></td></tr></table>




---

archive/issue_comments_213826.json:
```json
{
    "body": "Changed commit from **[`97b2f1f`](https://github.com/sagemath/sagetrac-mirror/commit/97b2f1fa06456de3e54da1787ce20915aceb8ad9)** to **[`d437be4`](https://github.com/sagemath/sagetrac-mirror/commit/d437be4a258a648e86f8cc1845ade75467609601)**",
    "created_at": "2014-06-23T14:28:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213826",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`97b2f1f`](https://github.com/sagemath/sagetrac-mirror/commit/97b2f1fa06456de3e54da1787ce20915aceb8ad9)** to **[`d437be4`](https://github.com/sagemath/sagetrac-mirror/commit/d437be4a258a648e86f8cc1845ade75467609601)**



---

archive/issue_comments_213827.json:
```json
{
    "body": "Changed work issues from **nonnegative=False doctests, avoid isinstance()** to **nonnegative=False doctests**",
    "created_at": "2014-06-23T17:02:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213827",
    "user": "https://github.com/jdemeyer"
}
```

Changed work issues from **nonnegative=False doctests, avoid isinstance()** to **nonnegative=False doctests**



---

archive/issue_comments_213828.json:
```json
{
    "body": "<div id=\"comment:35\" align=\"right\">comment:35</div>\n\nStill needs `nonnegative=False` doctests.",
    "created_at": "2014-06-23T17:02:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213828",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:35" align="right">comment:35</div>

Still needs `nonnegative=False` doctests.



---

archive/issue_events_233644.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-06-23T17:02:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233644"
}
```



---

archive/issue_events_233645.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-06-23T17:02:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233645"
}
```



---

archive/issue_comments_213829.json:
```json
{
    "body": "<div id=\"comment:36\" align=\"right\">comment:36</div>\n\nHere are the 10 lines you requested. \n\nNathann",
    "created_at": "2014-06-24T07:31:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213829",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:36" align="right">comment:36</div>

Here are the 10 lines you requested. 

Nathann



---

archive/issue_events_233646.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-06-24T07:31:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233646"
}
```



---

archive/issue_events_233647.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-06-24T07:31:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233647"
}
```



---

archive/issue_comments_213830.json:
```json
{
    "body": "Changed commit from **[`d437be4`](https://github.com/sagemath/sagetrac-mirror/commit/d437be4a258a648e86f8cc1845ade75467609601)** to **[`ebc3316`](https://github.com/sagemath/sagetrac-mirror/commit/ebc3316525036bcc2a94edcddaa81bca26f3ea9c)**",
    "created_at": "2014-06-24T07:31:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213830",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`d437be4`](https://github.com/sagemath/sagetrac-mirror/commit/d437be4a258a648e86f8cc1845ade75467609601)** to **[`ebc3316`](https://github.com/sagemath/sagetrac-mirror/commit/ebc3316525036bcc2a94edcddaa81bca26f3ea9c)**



---

archive/issue_comments_213831.json:
```json
{
    "body": "<div id=\"comment:38\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ebc3316525036bcc2a94edcddaa81bca26f3ea9c\"><code>ebc3316</code></a></td><td><code>trac #16504: additional doctest for nonnegative=False</code></td></tr></table>\n",
    "created_at": "2014-06-24T07:31:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213831",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:38"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ebc3316525036bcc2a94edcddaa81bca26f3ea9c"><code>ebc3316</code></a></td><td><code>trac #16504: additional doctest for nonnegative=False</code></td></tr></table>




---

archive/issue_comments_213832.json:
```json
{
    "body": "Changed branch from **[u/ncohen/16504](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/16504)** to **[u/jdemeyer/ticket/16504](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/16504)**",
    "created_at": "2014-06-24T13:23:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213832",
    "user": "https://github.com/jdemeyer"
}
```

Changed branch from **[u/ncohen/16504](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/16504)** to **[u/jdemeyer/ticket/16504](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/16504)**



---

archive/issue_comments_213833.json:
```json
{
    "body": "Changed commit from **[`ebc3316`](https://github.com/sagemath/sagetrac-mirror/commit/ebc3316525036bcc2a94edcddaa81bca26f3ea9c)** to **[`e6ff553`](https://github.com/sagemath/sagetrac-mirror/commit/e6ff553f8280f25854b2192cf3a4fa5dcd923a0b)**",
    "created_at": "2014-06-24T13:24:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213833",
    "user": "https://github.com/jdemeyer"
}
```

Changed commit from **[`ebc3316`](https://github.com/sagemath/sagetrac-mirror/commit/ebc3316525036bcc2a94edcddaa81bca26f3ea9c)** to **[`e6ff553`](https://github.com/sagemath/sagetrac-mirror/commit/e6ff553f8280f25854b2192cf3a4fa5dcd923a0b)**



---

archive/issue_comments_213834.json:
```json
{
    "body": "Changed work issues from **nonnegative=False doctests** to none",
    "created_at": "2014-06-24T13:24:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213834",
    "user": "https://github.com/jdemeyer"
}
```

Changed work issues from **nonnegative=False doctests** to none



---

archive/issue_comments_213835.json:
```json
{
    "body": "<div id=\"comment:40\"></div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e6ff553f8280f25854b2192cf3a4fa5dcd923a0b\"><code>e6ff553</code></a></td><td><code>Cosmetic changes</code></td></tr></table>\n",
    "created_at": "2014-06-24T13:24:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213835",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:40"></div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e6ff553f8280f25854b2192cf3a4fa5dcd923a0b"><code>e6ff553</code></a></td><td><code>Cosmetic changes</code></td></tr></table>




---

archive/issue_comments_213836.json:
```json
{
    "body": "<div id=\"comment:41\" align=\"right\">comment:41</div>\n\nPlease review this commit.",
    "created_at": "2014-06-24T13:25:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213836",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:41" align="right">comment:41</div>

Please review this commit.



---

archive/issue_comments_213837.json:
```json
{
    "body": "<div id=\"comment:42\" align=\"right\">comment:42</div>\n\nI see nothing wrong with it !\n\nNathann",
    "created_at": "2014-06-24T13:28:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213837",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:42" align="right">comment:42</div>

I see nothing wrong with it !

Nathann



---

archive/issue_events_233648.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-06-24T13:35:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233648"
}
```



---

archive/issue_events_233649.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-06-24T13:35:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233649"
}
```



---

archive/issue_comments_213838.json:
```json
{
    "body": "<div id=\"comment:44\" align=\"right\">comment:44</div>\n\nHey man... What about #16308 ?\n\nNathann",
    "created_at": "2014-06-24T13:36:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213838",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:44" align="right">comment:44</div>

Hey man... What about #16308 ?

Nathann



---

archive/issue_comments_213839.json:
```json
{
    "body": "<div id=\"comment:45\" align=\"right\">comment:45</div>\n\nReplying to [@nathanncohen](#comment%3A44):\n> Hey man... What about #16308 ?\n\nEasy, I will look at it...",
    "created_at": "2014-06-24T14:37:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213839",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:45" align="right">comment:45</div>

Replying to [@nathanncohen](#comment%3A44):
> Hey man... What about #16308 ?

Easy, I will look at it...



---

archive/issue_comments_213840.json:
```json
{
    "body": "<div id=\"comment:46\" align=\"right\">comment:46</div>\n\ndoctest failure src/doc/en/thematic_tutorials/linear_programming.rst",
    "created_at": "2014-06-25T13:49:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213840",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:46" align="right">comment:46</div>

doctest failure src/doc/en/thematic_tutorials/linear_programming.rst



---

archive/issue_events_233650.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-06-25T13:49:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233650"
}
```



---

archive/issue_events_233651.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-06-25T13:49:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233651"
}
```



---

archive/issue_events_233652.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-06-25T14:12:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233652"
}
```



---

archive/issue_events_233653.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-06-25T14:12:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233653"
}
```



---

archive/issue_comments_213841.json:
```json
{
    "body": "Changed commit from **[`e6ff553`](https://github.com/sagemath/sagetrac-mirror/commit/e6ff553f8280f25854b2192cf3a4fa5dcd923a0b)** to **[`1903937`](https://github.com/sagemath/sagetrac-mirror/commit/190393753fff48a6e210066fe0f0fff931ef404c)**",
    "created_at": "2014-06-25T14:12:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213841",
    "user": "https://github.com/nathanncohen"
}
```

Changed commit from **[`e6ff553`](https://github.com/sagemath/sagetrac-mirror/commit/e6ff553f8280f25854b2192cf3a4fa5dcd923a0b)** to **[`1903937`](https://github.com/sagemath/sagetrac-mirror/commit/190393753fff48a6e210066fe0f0fff931ef404c)**



---

archive/issue_comments_213842.json:
```json
{
    "body": "<div id=\"comment:47\" align=\"right\">comment:47</div>\n\nCursed tutorial. Sorry 'bout that `:-/`\n\nI rewrote the necessary part in an additional commit.\n\nNathann\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/190393753fff48a6e210066fe0f0fff931ef404c\"><code>1903937</code></a></td><td><code>trac #16504: Updates the tutorial</code></td></tr></table>\n",
    "created_at": "2014-06-25T14:12:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213842",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:47" align="right">comment:47</div>

Cursed tutorial. Sorry 'bout that `:-/`

I rewrote the necessary part in an additional commit.

Nathann

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/190393753fff48a6e210066fe0f0fff931ef404c"><code>1903937</code></a></td><td><code>trac #16504: Updates the tutorial</code></td></tr></table>




---

archive/issue_comments_213843.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/ticket/16504](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/16504)** to **[public/16504](https://github.com/sagemath/sagetrac-mirror/tree/public/16504)**",
    "created_at": "2014-06-25T14:12:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213843",
    "user": "https://github.com/nathanncohen"
}
```

Changed branch from **[u/jdemeyer/ticket/16504](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/16504)** to **[public/16504](https://github.com/sagemath/sagetrac-mirror/tree/public/16504)**



---

archive/issue_comments_213844.json:
```json
{
    "body": "<div id=\"comment:48\" align=\"right\">comment:48</div>\n\nYo!\nYLC LGTM.",
    "created_at": "2014-06-26T14:11:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213844",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:48" align="right">comment:48</div>

Yo!
YLC LGTM.



---

archive/issue_events_233654.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2014-06-26T14:11:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233654"
}
```



---

archive/issue_events_233655.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2014-06-26T14:11:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233655"
}
```



---

archive/issue_comments_213845.json:
```json
{
    "body": "<div id=\"comment:49\" align=\"right\">comment:49</div>\n\nYour Loving Cousin ? `O_o`",
    "created_at": "2014-06-26T14:12:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213845",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:49" align="right">comment:49</div>

Your Loving Cousin ? `O_o`



---

archive/issue_comments_213846.json:
```json
{
    "body": "<div id=\"comment:50\" align=\"right\">comment:50</div>\n\nYOUR LAST CHANGES ????\n\n(I think I got it `:-P`)",
    "created_at": "2014-06-26T14:13:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213846",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:50" align="right">comment:50</div>

YOUR LAST CHANGES ????

(I think I got it `:-P`)



---

archive/issue_comments_213847.json:
```json
{
    "body": "<div id=\"comment:51\" align=\"right\">comment:51</div>\n\nYour Last Commit ;-)",
    "created_at": "2014-06-26T14:15:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213847",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:51" align="right">comment:51</div>

Your Last Commit ;-)



---

archive/issue_comments_213848.json:
```json
{
    "body": "<div id=\"comment:52\" align=\"right\">comment:52</div>\n\nDoctests fail in src/sage/numerical/mip.pyx",
    "created_at": "2014-06-27T02:49:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213848",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:52" align="right">comment:52</div>

Doctests fail in src/sage/numerical/mip.pyx



---

archive/issue_events_233656.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-06-27T02:49:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233656"
}
```



---

archive/issue_events_233657.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-06-27T02:49:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233657"
}
```



---

archive/issue_comments_213849.json:
```json
{
    "body": "<div id=\"comment:53\" align=\"right\">comment:53</div>\n\nNot on my computer. Did you merge anything in between ?\n\nNathann",
    "created_at": "2014-06-27T07:24:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213849",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:53" align="right">comment:53</div>

Not on my computer. Did you merge anything in between ?

Nathann



---

archive/issue_comments_213850.json:
```json
{
    "body": "<div id=\"comment:54\" align=\"right\">comment:54</div>\n\nReplying to [@nathanncohen](#comment%3A53):\n> Not on my computer. Did you merge anything in between ?\n\nsame here, works for me too, so that must be cosmic rays (possibly emitted by git) affecting Volker's machine...",
    "created_at": "2014-06-27T07:30:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213850",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:54" align="right">comment:54</div>

Replying to [@nathanncohen](#comment%3A53):
> Not on my computer. Did you merge anything in between ?

same here, works for me too, so that must be cosmic rays (possibly emitted by git) affecting Volker's machine...



---

archive/issue_comments_213851.json:
```json
{
    "body": "<div id=\"comment:55\" align=\"right\">comment:55</div>\n\njust to make sure I merged public/16504 over the latest 6.3.beta5, and I get 2 failures:\n\n```\nsage -t src/sage/numerical/mip.pyx\n**********************************************************************\nFile \"src/sage/numerical/mip.pyx\", line 253, in sage.numerical.mip.MixedIntegerLinearProgram\nFailed example:\n    for type in [\"binary\", \"integer\"]:\n        k = 3\n        items = [1/5, 1/3, 2/3, 3/4, 5/7]\n        maximum=1\n        p=MixedIntegerLinearProgram()\n        box=p.new_variable(**{type:True})\n        for b in range(k):\n             p.add_constraint(p.sum([items[i]*box[i,b] for i in range(len(items))]) <= maximum)\n        for i in range(len(items)):\n            p.add_constraint(p.sum([box[i,b] for b in range(k)]) == 1)\n        p.set_objective(None)\n        _ = p.solve()\n        box=p.get_values(box)\n        print(all(v in ZZ for v in box.values()))\nExpected:\n    True\n    True\nGot:\n    True\n    doctest:839: DeprecationWarning: The default value of 'nonnegative' will change, to False instead of True. You should add the explicit 'nonnegative=True'.\n    See http://trac.sagemath.org/15521 for details.\n    True\n**********************************************************************\nFile \"src/sage/numerical/mip.pyx\", line 364, in sage.numerical.mip.MixedIntegerLinearProgram.?\nFailed example:\n    v = p.new_variable(real=True)\nExpected:\n    doctest:839: DeprecationWarning: The default value of 'nonnegative' will change, to False instead of True. You should add the explicit 'nonnegative=True'.\n    See http://trac.sagemath.org/15521 for details.\nGot:\n    <BLANKLINE>\n**********************************************************************\n2 items had failures:\n   1 of   9 in sage.numerical.mip.MixedIntegerLinearProgram\n   1 of  15 in sage.numerical.mip.MixedIntegerLinearProgram.?\n    [440 tests, 2 failures, 8.08 s]\n----------------------------------------------------------------------\nsage -t src/sage/numerical/mip.pyx  # 2 doctests failed\n----------------------------------------------------------------------\nTotal time for all tests: 8.7 seconds\n    cpu time: 8.1 seconds\n    cumulative wall time: 8.1 seconds\n```",
    "created_at": "2014-07-03T10:56:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213851",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:55" align="right">comment:55</div>

just to make sure I merged public/16504 over the latest 6.3.beta5, and I get 2 failures:

```
sage -t src/sage/numerical/mip.pyx
**********************************************************************
File "src/sage/numerical/mip.pyx", line 253, in sage.numerical.mip.MixedIntegerLinearProgram
Failed example:
    for type in ["binary", "integer"]:
        k = 3
        items = [1/5, 1/3, 2/3, 3/4, 5/7]
        maximum=1
        p=MixedIntegerLinearProgram()
        box=p.new_variable(**{type:True})
        for b in range(k):
             p.add_constraint(p.sum([items[i]*box[i,b] for i in range(len(items))]) <= maximum)
        for i in range(len(items)):
            p.add_constraint(p.sum([box[i,b] for b in range(k)]) == 1)
        p.set_objective(None)
        _ = p.solve()
        box=p.get_values(box)
        print(all(v in ZZ for v in box.values()))
Expected:
    True
    True
Got:
    True
    doctest:839: DeprecationWarning: The default value of 'nonnegative' will change, to False instead of True. You should add the explicit 'nonnegative=True'.
    See http://trac.sagemath.org/15521 for details.
    True
**********************************************************************
File "src/sage/numerical/mip.pyx", line 364, in sage.numerical.mip.MixedIntegerLinearProgram.?
Failed example:
    v = p.new_variable(real=True)
Expected:
    doctest:839: DeprecationWarning: The default value of 'nonnegative' will change, to False instead of True. You should add the explicit 'nonnegative=True'.
    See http://trac.sagemath.org/15521 for details.
Got:
    <BLANKLINE>
**********************************************************************
2 items had failures:
   1 of   9 in sage.numerical.mip.MixedIntegerLinearProgram
   1 of  15 in sage.numerical.mip.MixedIntegerLinearProgram.?
    [440 tests, 2 failures, 8.08 s]
----------------------------------------------------------------------
sage -t src/sage/numerical/mip.pyx  # 2 doctests failed
----------------------------------------------------------------------
Total time for all tests: 8.7 seconds
    cpu time: 8.1 seconds
    cumulative wall time: 8.1 seconds
```



---

archive/issue_comments_213852.json:
```json
{
    "body": "<div id=\"comment:56\" align=\"right\">comment:56</div>\n\nthe following obvious change fixes the things:\n\n```\ndiff --git a/src/sage/numerical/mip.pyx b/src/sage/numerical/mip.pyx\nindex 2f8d0e3..5741f68 100644\n--- a/src/sage/numerical/mip.pyx\n+++ b/src/sage/numerical/mip.pyx\n@@ -265,6 +265,8 @@ cdef class MixedIntegerLinearProgram(SageObject):\n         ....:     box=p.get_values(box)\n         ....:     print(all(v in ZZ for v in box.values()))\n         True\n+        doctest:839: DeprecationWarning: The default value of 'nonnegative' will change, to False inst\n+        See http://trac.sagemath.org/15521 for details.\n         True\n     \"\"\"\n \n@@ -362,8 +364,6 @@ cdef class MixedIntegerLinearProgram(SageObject):\n \n             sage: p = MixedIntegerLinearProgram()\n             sage: v = p.new_variable(real=True)\n-            doctest:839: DeprecationWarning: The default value of 'nonnegative' will change, to False \n-            See http://trac.sagemath.org/15521 for details.\n             sage: p.get_min(v[0])\n             0.0\n         \"\"\"\n```",
    "created_at": "2014-07-03T11:02:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213852",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:56" align="right">comment:56</div>

the following obvious change fixes the things:

```
diff --git a/src/sage/numerical/mip.pyx b/src/sage/numerical/mip.pyx
index 2f8d0e3..5741f68 100644
--- a/src/sage/numerical/mip.pyx
+++ b/src/sage/numerical/mip.pyx
@@ -265,6 +265,8 @@ cdef class MixedIntegerLinearProgram(SageObject):
         ....:     box=p.get_values(box)
         ....:     print(all(v in ZZ for v in box.values()))
         True
+        doctest:839: DeprecationWarning: The default value of 'nonnegative' will change, to False inst
+        See http://trac.sagemath.org/15521 for details.
         True
     """
 
@@ -362,8 +364,6 @@ cdef class MixedIntegerLinearProgram(SageObject):
 
             sage: p = MixedIntegerLinearProgram()
             sage: v = p.new_variable(real=True)
-            doctest:839: DeprecationWarning: The default value of 'nonnegative' will change, to False 
-            See http://trac.sagemath.org/15521 for details.
             sage: p.get_min(v[0])
             0.0
         """
```



---

archive/issue_comments_213853.json:
```json
{
    "body": "<div id=\"comment:57\" align=\"right\">comment:57</div>\n\nTo fix this properly, some commit untangling is needed, I suppose. Probably, a rebase over beta5 might be the easiest option, but I am not 100% sure.",
    "created_at": "2014-07-03T11:29:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213853",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:57" align="right">comment:57</div>

To fix this properly, some commit untangling is needed, I suppose. Probably, a rebase over beta5 might be the easiest option, but I am not 100% sure.



---

archive/issue_comments_213854.json:
```json
{
    "body": "<div id=\"comment:58\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/27a0bce21afa79837f697797bd0aa1db53b63459\"><code>27a0bce</code></a></td><td><code>trac #16504: Merged with beta5</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/45bc9377d525b94d7f9ddfbba15b9f074a324937\"><code>45bc937</code></a></td><td><code>trac #16504: Broken doctest</code></td></tr></table>\n",
    "created_at": "2014-07-03T11:43:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213854",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:58"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/27a0bce21afa79837f697797bd0aa1db53b63459"><code>27a0bce</code></a></td><td><code>trac #16504: Merged with beta5</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/45bc9377d525b94d7f9ddfbba15b9f074a324937"><code>45bc937</code></a></td><td><code>trac #16504: Broken doctest</code></td></tr></table>




---

archive/issue_comments_213855.json:
```json
{
    "body": "Changed commit from **[`1903937`](https://github.com/sagemath/sagetrac-mirror/commit/190393753fff48a6e210066fe0f0fff931ef404c)** to **[`45bc937`](https://github.com/sagemath/sagetrac-mirror/commit/45bc9377d525b94d7f9ddfbba15b9f074a324937)**",
    "created_at": "2014-07-03T11:43:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213855",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`1903937`](https://github.com/sagemath/sagetrac-mirror/commit/190393753fff48a6e210066fe0f0fff931ef404c)** to **[`45bc937`](https://github.com/sagemath/sagetrac-mirror/commit/45bc9377d525b94d7f9ddfbba15b9f074a324937)**



---

archive/issue_events_233658.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-07-03T11:43:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233658"
}
```



---

archive/issue_events_233659.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-07-03T11:43:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233659"
}
```



---

archive/issue_events_233660.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2014-07-03T21:54:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233660"
}
```



---

archive/issue_events_233661.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2014-07-03T21:54:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233661"
}
```



---

archive/issue_comments_213856.json:
```json
{
    "body": "<div id=\"comment:60\" align=\"right\">comment:60</div>\n\nLGTM",
    "created_at": "2014-07-03T21:54:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213856",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:60" align="right">comment:60</div>

LGTM



---

archive/issue_events_233662.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-06T17:55:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233662"
}
```



---

archive/issue_events_233663.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "6c29eb57cade10f30a010b9db0136640594054ab",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-07-06T17:55:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16504#event-233663"
}
```



---

archive/issue_comments_213857.json:
```json
{
    "body": "Changed branch from **[public/16504](https://github.com/sagemath/sagetrac-mirror/tree/public/16504)** to **[`45bc937`](https://github.com/sagemath/sagetrac-mirror/commit/45bc9377d525b94d7f9ddfbba15b9f074a324937)**",
    "created_at": "2014-07-06T17:55:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16504",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16504#issuecomment-213857",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/16504](https://github.com/sagemath/sagetrac-mirror/tree/public/16504)** to **[`45bc937`](https://github.com/sagemath/sagetrac-mirror/commit/45bc9377d525b94d7f9ddfbba15b9f074a324937)**

# Issue 21095: Slowness in exactification

archive/issues_020858.json:
```json
{
    "body": "On sage-7.3.beta9\n\n```\nsage: x = polygen(ZZ)\nsage: p = 67*x^4 - 33*x^3 + 94*x^2 - 30*x + 57\nsage: r = p.roots(QQbar, multiplicities=False)\nsage: r[0].abs().exactify()  # <- takes forever\n```\n\n**Reviewer:** Vincent Delecroix\n\nIssue created by migration from https://trac.sagemath.org/ticket/21095\n\n",
    "closed_at": "2019-02-26T13:58:00Z",
    "created_at": "2016-07-25T20:45:37Z",
    "labels": [
        "component: number fields",
        "bug",
        "invalid"
    ],
    "title": "Slowness in exactification",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/21095",
    "user": "https://github.com/videlec"
}
```
On sage-7.3.beta9

```
sage: x = polygen(ZZ)
sage: p = 67*x^4 - 33*x^3 + 94*x^2 - 30*x + 57
sage: r = p.roots(QQbar, multiplicities=False)
sage: r[0].abs().exactify()  # <- takes forever
```

**Reviewer:** Vincent Delecroix

Issue created by migration from https://trac.sagemath.org/ticket/21095





---

archive/issue_comments_309807.json:
```json
{
    "body": "<a id='comment:1'></a>\nduplicate of #15600",
    "created_at": "2018-12-23T08:10:10Z",
    "issue": "https://github.com/sagemath/sage/issues/21095",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21095#issuecomment-309807",
    "user": "https://github.com/mezzarobba"
}
```

<a id='comment:1'></a>
duplicate of #15600



---

archive/issue_events_190647.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-12-23T08:10:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/21095",
    "milestone": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21095#event-190647"
}
```



---

archive/issue_events_190648.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-12-23T08:10:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21095",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21095#event-190648"
}
```



---

archive/issue_comments_309808.json:
```json
{
    "body": "**Reviewer:** Vincent Delecroix",
    "created_at": "2018-12-23T09:13:37Z",
    "issue": "https://github.com/sagemath/sage/issues/21095",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21095#issuecomment-309808",
    "user": "https://github.com/videlec"
}
```

**Reviewer:** Vincent Delecroix



---

archive/issue_events_190649.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-12-23T09:13:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21095",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21095#event-190649"
}
```



---

archive/issue_events_190650.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-12-23T09:13:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/21095",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21095#event-190650"
}
```



---

archive/issue_events_190651.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-02-26T13:58:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/21095",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21095#event-190651"
}
```



---

archive/issue_events_190652.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-02-26T13:58:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/21095",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/21095#event-190652"
}
```



---

archive/issue_comments_309809.json:
```json
{
    "body": "<a id='comment:3'></a>\nPresuming these are all correctly reviewed as either duplicate, invalid, or wontfix.",
    "created_at": "2019-02-26T13:58:00Z",
    "issue": "https://github.com/sagemath/sage/issues/21095",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/21095#issuecomment-309809",
    "user": "https://github.com/embray"
}
```

<a id='comment:3'></a>
Presuming these are all correctly reviewed as either duplicate, invalid, or wontfix.

# Issue 31211: ci-cygwin*.yml: Fix packaging up of SAGE_LOCAL/var/tmp/sage/build/*

archive/issues_030974.json:
```json
{
    "body": "These files are intended to be put in the logs artifact, but the code is being run with the wrong shell, so it fails silently:\n\nhttps://github.com/kliem/sage/runs/1626910566\n\n```\n0s\nRun mkdir -p \"artifacts/$LOGS_ARTIFACT_NAME\"; for a in \"/opt/sage-255cf51a4f41b054d32a39562102877b0b18969f\"/var/tmp/sage/build/*; do if [ -d \"$a\" ]; then tar -c --remove-files -f \"artifacts/$LOGS_ARTIFACT_NAME/$(basename \"$a\").tar\" \"$a\"; fi; done; cp -r logs/*  \"artifacts/$LOGS_ARTIFACT_NAME\"\n  mkdir -p \"artifacts/$LOGS_ARTIFACT_NAME\"; for a in \"/opt/sage-255cf51a4f41b054d32a39562102877b0b18969f\"/var/tmp/sage/build/*; do if [ -d \"$a\" ]; then tar -c --remove-files -f \"artifacts/$LOGS_ARTIFACT_NAME/$(basename \"$a\").tar\" \"$a\"; fi; done; cp -r logs/*  \"artifacts/$LOGS_ARTIFACT_NAME\"\n  shell: C:\\Program Files\\Git\\bin\\bash.EXE --noprofile --norc -e -o pipefail {0}\n```\n(the correct shell to use for running tar is the cygwin shell.)\n\n\n\nDepends on #33791\n\n**CC:**  @kliem @malb\n\n**Reviewer:** Kwankyu Lee\n\nIssue created by migration from https://trac.sagemath.org/ticket/31211\n\n",
    "closed_at": "2022-08-02T06:31:03Z",
    "created_at": "2021-01-08T18:28:04Z",
    "labels": [
        "component: porting",
        "minor",
        "bug",
        "invalid"
    ],
    "title": "ci-cygwin*.yml: Fix packaging up of SAGE_LOCAL/var/tmp/sage/build/*",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/31211",
    "user": "https://github.com/mkoeppe"
}
```
These files are intended to be put in the logs artifact, but the code is being run with the wrong shell, so it fails silently:

https://github.com/kliem/sage/runs/1626910566

```
0s
Run mkdir -p "artifacts/$LOGS_ARTIFACT_NAME"; for a in "/opt/sage-255cf51a4f41b054d32a39562102877b0b18969f"/var/tmp/sage/build/*; do if [ -d "$a" ]; then tar -c --remove-files -f "artifacts/$LOGS_ARTIFACT_NAME/$(basename "$a").tar" "$a"; fi; done; cp -r logs/*  "artifacts/$LOGS_ARTIFACT_NAME"
  mkdir -p "artifacts/$LOGS_ARTIFACT_NAME"; for a in "/opt/sage-255cf51a4f41b054d32a39562102877b0b18969f"/var/tmp/sage/build/*; do if [ -d "$a" ]; then tar -c --remove-files -f "artifacts/$LOGS_ARTIFACT_NAME/$(basename "$a").tar" "$a"; fi; done; cp -r logs/*  "artifacts/$LOGS_ARTIFACT_NAME"
  shell: C:\Program Files\Git\bin\bash.EXE --noprofile --norc -e -o pipefail {0}
```
(the correct shell to use for running tar is the cygwin shell.)



Depends on #33791

**CC:**  @kliem @malb

**Reviewer:** Kwankyu Lee

Issue created by migration from https://trac.sagemath.org/ticket/31211





---

archive/issue_comments_502935.json:
```json
{
    "body": "<a id='comment:1'></a>\nSage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-03-24T02:04:25Z",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31211#issuecomment-502935",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:1'></a>
Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_282082.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31211#event-282082"
}
```



---

archive/issue_events_282083.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31211#event-282083"
}
```



---

archive/issue_events_282084.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-09T21:26:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31211#event-282084"
}
```



---

archive/issue_events_282085.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-09T21:26:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31211#event-282085"
}
```



---

archive/issue_events_282086.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-27T01:03:43Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31211#event-282086"
}
```



---

archive/issue_events_282087.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-27T01:03:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31211#event-282087"
}
```



---

archive/issue_events_282088.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-07T00:07:21Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31211#event-282088"
}
```



---

archive/issue_events_282089.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-07T00:07:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31211#event-282089"
}
```



---

archive/issue_comments_502936.json:
```json
{
    "body": "**Changing dependencies** from \"#31064\" to \"#33791\".",
    "created_at": "2022-05-03T21:48:33Z",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31211#issuecomment-502936",
    "user": "https://github.com/mkoeppe"
}
```

**Changing dependencies** from "#31064" to "#33791".



---

archive/issue_events_282090.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T21:48:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31211#event-282090"
}
```



---

archive/issue_comments_502937.json:
```json
{
    "body": "<a id='comment:5'></a>\ndone in #33791",
    "created_at": "2022-05-03T21:48:33Z",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31211#issuecomment-502937",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:5'></a>
done in #33791



---

archive/issue_events_282091.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T21:48:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31211#event-282091"
}
```



---

archive/issue_events_282092.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2022-07-29T20:53:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31211#event-282092"
}
```



---

archive/issue_events_282093.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2022-07-29T20:53:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31211#event-282093"
}
```



---

archive/issue_comments_502938.json:
```json
{
    "body": "<a id='comment:6'></a>\nOkay.",
    "created_at": "2022-07-29T20:53:11Z",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31211#issuecomment-502938",
    "user": "https://github.com/kwankyu"
}
```

<a id='comment:6'></a>
Okay.



---

archive/issue_comments_502939.json:
```json
{
    "body": "**Reviewer:** Kwankyu Lee",
    "created_at": "2022-07-29T20:53:11Z",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31211#issuecomment-502939",
    "user": "https://github.com/kwankyu"
}
```

**Reviewer:** Kwankyu Lee



---

archive/issue_events_282094.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-02T06:31:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31211#event-282094"
}
```



---

archive/issue_events_282095.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-02T06:31:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31211",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31211#event-282095"
}
```

# Issue 31440: Docker/GH Actions: Work around problems with faccessat2 in current glibc versions

archive/issues_031203.json:
```json
{
    "body": "This is blocking tests of `ubuntu-hirsute` at the moment:\nhttps://groups.google.com/g/sage-release/c/hVIsURxpdhc/m/oo4d0ODvAQAJ\n\n\n**CC:**  @dimpase @slel\n\n**Reviewer:** Samuel Leli\u00e8vre\n\nIssue created by migration from https://trac.sagemath.org/ticket/31440\n\n",
    "closed_at": "2021-08-26T02:08:43Z",
    "created_at": "2021-02-27T20:59:29Z",
    "labels": [
        "component: porting",
        "bug",
        "invalid"
    ],
    "title": "Docker/GH Actions: Work around problems with faccessat2 in current glibc versions",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/31440",
    "user": "https://github.com/mkoeppe"
}
```
This is blocking tests of `ubuntu-hirsute` at the moment:
https://groups.google.com/g/sage-release/c/hVIsURxpdhc/m/oo4d0ODvAQAJ


**CC:**  @dimpase @slel

**Reviewer:** Samuel Lelièvre

Issue created by migration from https://trac.sagemath.org/ticket/31440





---

archive/issue_comments_507592.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis may have been fixed - either in Docker or the distributions - for example `ubuntu-hirsute-standard` succeeded in https://github.com/sagemath/sage/runs/2051745189?check_suite_focus=true",
    "created_at": "2021-03-07T22:57:19Z",
    "issue": "https://github.com/sagemath/sage/issues/31440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31440#issuecomment-507592",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:1'></a>
This may have been fixed - either in Docker or the distributions - for example `ubuntu-hirsute-standard` succeeded in https://github.com/sagemath/sage/runs/2051745189?check_suite_focus=true



---

archive/issue_comments_507593.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis problem has gone away by itself on GH Actions, but I ran into it again on a local run using `tox -e docker-fedora-34-maximal`.",
    "created_at": "2021-03-24T02:06:48Z",
    "issue": "https://github.com/sagemath/sage/issues/31440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31440#issuecomment-507593",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'></a>
This problem has gone away by itself on GH Actions, but I ran into it again on a local run using `tox -e docker-fedora-34-maximal`.



---

archive/issue_events_284289.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:06:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31440",
    "label": "component: porting",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31440#event-284289"
}
```



---

archive/issue_comments_507594.json:
```json
{
    "body": "<a id='comment:3'></a>\nMoving to 9.4, as 9.3 has been released.",
    "created_at": "2021-05-10T17:42:09Z",
    "issue": "https://github.com/sagemath/sage/issues/31440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31440#issuecomment-507594",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:3'></a>
Moving to 9.4, as 9.3 has been released.



---

archive/issue_events_284290.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-10T17:42:09Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31440",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31440#event-284290"
}
```



---

archive/issue_events_284291.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-10T17:42:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/31440",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31440#event-284291"
}
```



---

archive/issue_events_284292.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T02:32:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31440",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31440#event-284292"
}
```



---

archive/issue_events_284293.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T02:32:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/31440",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31440#event-284293"
}
```



---

archive/issue_comments_507595.json:
```json
{
    "body": "**Reviewer:** Samuel Leli\u00e8vre",
    "created_at": "2021-08-19T22:02:56Z",
    "issue": "https://github.com/sagemath/sage/issues/31440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/31440#issuecomment-507595",
    "user": "https://github.com/slel"
}
```

**Reviewer:** Samuel Lelièvre



---

archive/issue_events_284294.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-08-19T22:02:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31440",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31440#event-284294"
}
```



---

archive/issue_events_284295.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-08-19T22:02:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/31440",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31440#event-284295"
}
```



---

archive/issue_events_284296.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T02:08:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/31440",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31440#event-284296"
}
```



---

archive/issue_events_284297.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T02:08:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/31440",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/31440#event-284297"
}
```

# Issue 10695: Use execv() instead of Subprocess.call() in sage-run

archive/issues_010639.json:
```json
{
    "body": "`sage-run` runs `.sage` files from the command line.  The problem is that pressing `CTRL-C` interrupts the `sage-run` process, but the child process (doing the actual computation) might still continue.  This is very annoying because after this, there is absolutely no way to interrupt the still-running `sage`.\n\n**Keywords:** sage-run interrupt execve\n\n**Author:** Jeroen Demeyer\n\n**Reviewer:** Willem Jan Palenstijn\n\n**Merged:** sage-4.6.2.alpha3\n\nIssue created by migration from https://trac.sagemath.org/ticket/10695\n\n",
    "closed_at": "2011-01-26T22:29:17Z",
    "created_at": "2011-01-26T14:38:46Z",
    "labels": [
        "component: scripts",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.2",
    "title": "Use execv() instead of Subprocess.call() in sage-run",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/10695",
    "user": "https://github.com/jdemeyer"
}
```
`sage-run` runs `.sage` files from the command line.  The problem is that pressing `CTRL-C` interrupts the `sage-run` process, but the child process (doing the actual computation) might still continue.  This is very annoying because after this, there is absolutely no way to interrupt the still-running `sage`.

**Keywords:** sage-run interrupt execve

**Author:** Jeroen Demeyer

**Reviewer:** Willem Jan Palenstijn

**Merged:** sage-4.6.2.alpha3

Issue created by migration from https://trac.sagemath.org/ticket/10695





---

archive/issue_comments_104736.json:
```json
{
    "body": "SCRIPTS patch",
    "created_at": "2011-01-26T14:40:36Z",
    "issue": "https://github.com/sagemath/sage/issues/10695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10695#issuecomment-104736",
    "user": "https://github.com/jdemeyer"
}
```

SCRIPTS patch



---

archive/attachments_014520.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "10695_execv.patch",
    "asset_url": "tarball://root/attachments/ticket10695/10695_execv.patch",
    "created_at": "2011-01-26T14:42:08Z",
    "issue": "https://github.com/sagemath/sage/issues/10695",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket10695/10695_execv.patch",
    "user": "https://github.com/jdemeyer"
}
```



---

archive/issue_comments_104737.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [10695_execv.patch](https://github.com/sagemath/sage/files/ticket10695/10695_execv.patch)",
    "created_at": "2011-01-26T14:42:08Z",
    "issue": "https://github.com/sagemath/sage/issues/10695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10695#issuecomment-104737",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'></a>
**Attachment:** [10695_execv.patch](https://github.com/sagemath/sage/files/ticket10695/10695_execv.patch)



---

archive/issue_events_082990.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-26T14:42:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10695",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10695#event-82990"
}
```



---

archive/issue_events_082991.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-26T14:42:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10695",
    "label": "bug",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10695#event-82991"
}
```



---

archive/issue_comments_104738.json:
```json
{
    "body": "**Reviewer:** Willem Jan Palenstijn",
    "created_at": "2011-01-26T15:24:38Z",
    "issue": "https://github.com/sagemath/sage/issues/10695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10695#issuecomment-104738",
    "user": "https://github.com/wjp"
}
```

**Reviewer:** Willem Jan Palenstijn



---

archive/issue_comments_104739.json:
```json
{
    "body": "<a id='comment:2'></a>\nJeroen clarified by email that this happens if the called script catches KeyboardInterrupt and doesn't quit.\n\nLooks good, and fixes the problem. Positive review.",
    "created_at": "2011-01-26T15:24:38Z",
    "issue": "https://github.com/sagemath/sage/issues/10695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10695#issuecomment-104739",
    "user": "https://github.com/wjp"
}
```

<a id='comment:2'></a>
Jeroen clarified by email that this happens if the called script catches KeyboardInterrupt and doesn't quit.

Looks good, and fixes the problem. Positive review.



---

archive/issue_events_082992.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2011-01-26T15:24:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10695",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10695#event-82992"
}
```



---

archive/issue_events_082993.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2011-01-26T15:24:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10695",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10695#event-82993"
}
```



---

archive/issue_comments_104740.json:
```json
{
    "body": "**Merged:** sage-4.6.2.alpha3",
    "created_at": "2011-01-26T22:29:17Z",
    "issue": "https://github.com/sagemath/sage/issues/10695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10695#issuecomment-104740",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-4.6.2.alpha3



---

archive/issue_events_082994.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-26T22:29:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10695",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10695#event-82994"
}
```



---

archive/issue_events_082995.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-26T22:29:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10695",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10695#event-82995"
}
```

# Issue 10438: [duplicate] Calculate the character of a newform

archive/issues_010385.json:
```json
{
    "assignees": [
        "https://github.com/craigcitro"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nNewforms of level Gamma1(N), or GammaH(N) (after #8716), are always eigenforms for the diamond operators (i.e. they have a nebentypus Dirichlet character) but we don't at present have any way of calculating it. \n\n(This ticket is subsumed in #10451).\n\nComponent: **modular forms**\n\nKeywords: **newform dirichlet character**\n\nReviewer: **David Loeffler**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10438_\n\n",
    "closed_at": "2011-02-04T15:06:15Z",
    "created_at": "2010-12-07T13:36:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "[duplicate] Calculate the character of a newform",
    "type": "issue",
    "updated_at": "2011-02-04T15:06:15Z",
    "url": "https://github.com/sagemath/sage/issues/10438",
    "user": "https://github.com/loefflerd"
}
```
<div id="comment:0"></div>

Newforms of level Gamma1(N), or GammaH(N) (after #8716), are always eigenforms for the diamond operators (i.e. they have a nebentypus Dirichlet character) but we don't at present have any way of calculating it. 

(This ticket is subsumed in #10451).

Component: **modular forms**

Keywords: **newform dirichlet character**

Reviewer: **David Loeffler**

_Issue created by migration from https://trac.sagemath.org/ticket/10438_





---

archive/issue_events_135483.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-07T13:36:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135483"
}
```



---

archive/issue_events_135484.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-07T13:36:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
    "label_color": "0000ff",
    "label_name": "c: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135484"
}
```



---

archive/issue_events_135485.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-07T13:36:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135485"
}
```



---

archive/issue_events_135486.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-07T13:36:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135486"
}
```



---

archive/issue_events_135487.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2010-12-07T13:36:23Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "subject": "https://github.com/loefflerd",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135487"
}
```



---

archive/issue_comments_097798.json:
```json
{
    "body": "Patch against 4.6.1.alpha3",
    "created_at": "2010-12-07T14:02:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10438#issuecomment-97798",
    "user": "https://github.com/loefflerd"
}
```

Patch against 4.6.1.alpha3



---

archive/issue_comments_097799.json:
```json
{
    "body": "Changed keywords from none to **newform dirichlet character**",
    "created_at": "2010-12-07T15:10:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10438#issuecomment-97799",
    "user": "https://github.com/loefflerd"
}
```

Changed keywords from none to **newform dirichlet character**



---

archive/issue_events_135488.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-07T15:10:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135488"
}
```



---

archive/issue_comments_097800.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,3 @@\n-Newforms of level Gamma1(N), or GammaH(N) (after 8716), are always eigenforms for the diamond operators (i.e. they have a character) but we don't at present have any way of calculating it. \n+Newforms of level Gamma1(N), or GammaH(N) (after #8716), are always eigenforms for the diamond operators (i.e. they have a nebentypus Dirichlet character) but we don't at present have any way of calculating it. \n \n+This will be needed for ongoing work on local components of newforms.\n``````\n",
    "created_at": "2010-12-07T15:10:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10438#issuecomment-97800",
    "user": "https://github.com/loefflerd"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,3 @@
-Newforms of level Gamma1(N), or GammaH(N) (after 8716), are always eigenforms for the diamond operators (i.e. they have a character) but we don't at present have any way of calculating it. 
+Newforms of level Gamma1(N), or GammaH(N) (after #8716), are always eigenforms for the diamond operators (i.e. they have a nebentypus Dirichlet character) but we don't at present have any way of calculating it. 
 
+This will be needed for ongoing work on local components of newforms.
``````




---

archive/issue_comments_097801.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_10438-diamond_eigenvalues.patch.gz](https://github.com/sagemath/sage/files/ticket10438/trac_10438-diamond_eigenvalues.patch.gz)**\n\nHere's a patch.",
    "created_at": "2010-12-07T15:10:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10438#issuecomment-97801",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_10438-diamond_eigenvalues.patch.gz](https://github.com/sagemath/sage/files/ticket10438/trac_10438-diamond_eigenvalues.patch.gz)**

Here's a patch.



---

archive/issue_comments_097802.json:
```json
{
    "body": "Author: **David Loeffler**",
    "created_at": "2010-12-09T22:25:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10438#issuecomment-97802",
    "user": "https://github.com/loefflerd"
}
```

Author: **David Loeffler**



---

archive/issue_events_135489.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-11T16:13:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135489"
}
```



---

archive/issue_events_135490.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-11T16:13:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135490"
}
```



---

archive/issue_comments_097803.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nJust realised this is coded in a rather stupid way (generic Hecke modules don't have a diamond_bracket_operator method, and hence shouldn't have a diamond_eigenvalue method either). I will fix this in conjunction with work on #10451.",
    "created_at": "2010-12-11T16:32:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10438#issuecomment-97803",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:4" align="right">comment:4</div>

Just realised this is coded in a rather stupid way (generic Hecke modules don't have a diamond_bracket_operator method, and hence shouldn't have a diamond_eigenvalue method either). I will fix this in conjunction with work on #10451.



---

archive/issue_events_135491.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-11T16:32:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135491"
}
```



---

archive/issue_events_135492.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-11T16:32:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135492"
}
```



---

archive/issue_events_135493.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-12T14:53:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135493"
}
```



---

archive/issue_events_135494.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-12T14:53:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135494"
}
```



---

archive/issue_events_135495.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-12T14:53:43Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135495"
}
```



---

archive/issue_comments_097804.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n Newforms of level Gamma1(N), or GammaH(N) (after #8716), are always eigenforms for the diamond operators (i.e. they have a nebentypus Dirichlet character) but we don't at present have any way of calculating it. \n \n-This will be needed for ongoing work on local components of newforms.\n+(This ticket is subsumed in #10451).\n``````\n",
    "created_at": "2010-12-12T14:53:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10438#issuecomment-97804",
    "user": "https://github.com/loefflerd"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 Newforms of level Gamma1(N), or GammaH(N) (after #8716), are always eigenforms for the diamond operators (i.e. they have a nebentypus Dirichlet character) but we don't at present have any way of calculating it. 
 
-This will be needed for ongoing work on local components of newforms.
+(This ticket is subsumed in #10451).
``````




---

archive/issue_events_135496.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-12-12T14:53:43Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "title_is": "[duplicate] Calculate the character of a newform",
    "title_was": "Calculate the character of a newform",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135496"
}
```



---

archive/issue_comments_097805.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nI propose to close this ticket as \"duplicate\", since the functionality I proposed to add is included in #10451.",
    "created_at": "2010-12-12T14:53:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10438#issuecomment-97805",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:5" align="right">comment:5</div>

I propose to close this ticket as "duplicate", since the functionality I proposed to add is included in #10451.



---

archive/issue_comments_097806.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nI'm setting this to \"positive review\" as an invitation to the release manager to close it, because all the functionality I planned to add is subsumed in #10451.\n\nRelease manager: please close as duplicate, and *do not* merge the patch.",
    "created_at": "2011-01-27T10:22:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10438#issuecomment-97806",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:6" align="right">comment:6</div>

I'm setting this to "positive review" as an invitation to the release manager to close it, because all the functionality I planned to add is subsumed in #10451.

Release manager: please close as duplicate, and *do not* merge the patch.



---

archive/issue_events_135497.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-01-27T10:22:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135497"
}
```



---

archive/issue_events_135498.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-01-27T10:22:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135498"
}
```



---

archive/issue_comments_097807.json:
```json
{
    "body": "Changed author from **David Loeffler** to none",
    "created_at": "2011-02-04T15:06:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10438#issuecomment-97807",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **David Loeffler** to none



---

archive/issue_events_135499.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-04T15:06:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135499"
}
```



---

archive/issue_events_135500.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-04T15:06:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135500"
}
```



---

archive/issue_comments_097808.json:
```json
{
    "body": "Reviewer: **David Loeffler**",
    "created_at": "2011-02-04T15:06:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10438#issuecomment-97808",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **David Loeffler**



---

archive/issue_events_135501.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-04T15:06:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135501"
}
```



---

archive/issue_events_135502.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-04T15:06:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10438",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "c6c6c6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10438#event-135502"
}
```

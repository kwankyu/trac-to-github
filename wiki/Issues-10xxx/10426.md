# Issue 10426: Add matrix row and column swapping, returning a new matrix

archive/issues_010373.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout",
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThere is a series of \"with_\" commands that do row operations on a matrix and return a new matrix.  The set looks complete, except row and column swapping seems to be missing.  Patch adds them in.\n\nCC:  @kcrisman\n\nComponent: **linear algebra**\n\nAuthor: **Rob Beezer**\n\nReviewer: **Alain Filbois**\n\nMerged: **sage-4.7.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10426_\n\n",
    "closed_at": "2011-03-18T13:40:21Z",
    "created_at": "2010-12-03T23:05:18Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add matrix row and column swapping, returning a new matrix",
    "type": "issue",
    "updated_at": "2011-03-18T13:40:21Z",
    "url": "https://github.com/sagemath/sage/issues/10426",
    "user": "https://github.com/rbeezer"
}
```
<div id="comment:0"></div>

There is a series of "with_" commands that do row operations on a matrix and return a new matrix.  The set looks complete, except row and column swapping seems to be missing.  Patch adds them in.

CC:  @kcrisman

Component: **linear algebra**

Author: **Rob Beezer**

Reviewer: **Alain Filbois**

Merged: **sage-4.7.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/10426_





---

archive/issue_events_135290.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-12-03T23:05:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10426",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10426#event-135290"
}
```



---

archive/issue_events_135291.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-12-03T23:05:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10426",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "c: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10426#event-135291"
}
```



---

archive/issue_events_135292.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-12-03T23:05:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10426",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10426#event-135292"
}
```



---

archive/issue_events_135293.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-12-03T23:05:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10426",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10426#event-135293"
}
```



---

archive/issue_events_135294.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-12-03T23:05:18Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/10426",
    "subject": "https://github.com/rbeezer",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10426#event-135294"
}
```



---

archive/issue_events_135295.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-12-03T23:05:18Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/10426",
    "subject": "https://github.com/rbeezer",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10426#event-135295"
}
```



---

archive/issue_events_135296.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-12-03T23:08:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10426",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10426#event-135296"
}
```



---

archive/issue_events_135297.json:
```json
{
    "actor": "https://github.com/sagetrac-alain054",
    "created_at": "2011-02-08T11:28:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10426",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10426#event-135297"
}
```



---

archive/issue_events_135298.json:
```json
{
    "actor": "https://github.com/sagetrac-alain054",
    "created_at": "2011-02-08T11:28:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10426",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10426#event-135298"
}
```



---

archive/issue_comments_097630.json:
```json
{
    "body": "Reviewer: **Alain Filbois**",
    "created_at": "2011-02-08T11:28:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10426#issuecomment-97630",
    "user": "https://github.com/sagetrac-alain054"
}
```

Reviewer: **Alain Filbois**



---

archive/issue_comments_097631.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAttachment: **[trac_10426-matrix-row-column-swapping.patch.gz](https://github.com/sagemath/sage/files/ticket10426/trac_10426-matrix-row-column-swapping.patch.gz)**\n\nPatch is correct.",
    "created_at": "2011-02-08T11:28:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10426#issuecomment-97631",
    "user": "https://github.com/sagetrac-alain054"
}
```

<div id="comment:2" align="right">comment:2</div>

Attachment: **[trac_10426-matrix-row-column-swapping.patch.gz](https://github.com/sagemath/sage/files/ticket10426/trac_10426-matrix-row-column-swapping.patch.gz)**

Patch is correct.



---

archive/issue_comments_097632.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nThanks, Alain!  I'm about to build a constructor to create elementary matrices, so I've been hoping this would go in soon.",
    "created_at": "2011-02-08T17:41:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10426#issuecomment-97632",
    "user": "https://github.com/rbeezer"
}
```

<div id="comment:3" align="right">comment:3</div>

Thanks, Alain!  I'm about to build a constructor to create elementary matrices, so I've been hoping this would go in soon.



---

archive/issue_events_135299.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-08T21:08:43Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10426",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10426#event-135299"
}
```



---

archive/issue_events_135300.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-08T21:08:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10426",
    "milestone_number": null,
    "milestone_title": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10426#event-135300"
}
```



---

archive/issue_comments_097633.json:
```json
{
    "body": "Merged: **sage-4.7.alpha2**",
    "created_at": "2011-03-18T13:40:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10426#issuecomment-97633",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.alpha2**



---

archive/issue_events_135301.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-18T13:40:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10426",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10426#event-135301"
}
```



---

archive/issue_events_135302.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-18T13:40:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10426",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10426#event-135302"
}
```

# Issue 10265: Add palindrome defect word to the word constructor

archive/issues_010264.json:
```json
{
    "assignees": [
        "https://github.com/seblabbe"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nAdd the finite word `w = a b^k a b^{k-1} a a b^{k-1} a b^{k} a` to the words constructor.\n\nAs described by Brlek, Hamel, Nivat and Reuteunaer in `[1]`, this\nfinite word `w` is such that the infinite periodic word `w^\\omega`\nhave palindromic defect ``k``.\n\n`[1]` S. Brlek, S. Hamel, M. Nivat and C. Reutenauer, On the palindromic complexity of in\ufb01nite words, Int. J. Found. Comput. Sci. 15: 2 (2004) 293\u2013306.\n\n\nCC:  @sagetrac-abmasse\n\nComponent: **combinatorics**\n\nAuthor: **S\u00e9bastien Labb\u00e9**\n\nReviewer: **Alexandre Blondin Mass\u00e9**\n\nMerged: **sage-4.6.2.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10265_\n\n",
    "closed_at": "2011-01-12T06:33:59Z",
    "created_at": "2010-11-14T18:05:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add palindrome defect word to the word constructor",
    "type": "issue",
    "updated_at": "2011-01-12T06:33:59Z",
    "url": "https://github.com/sagemath/sage/issues/10265",
    "user": "https://github.com/seblabbe"
}
```
<div id="comment:0"></div>

Add the finite word `w = a b^k a b^{k-1} a a b^{k-1} a b^{k} a` to the words constructor.

As described by Brlek, Hamel, Nivat and Reuteunaer in `[1]`, this
finite word `w` is such that the infinite periodic word `w^\omega`
have palindromic defect ``k``.

`[1]` S. Brlek, S. Hamel, M. Nivat and C. Reutenauer, On the palindromic complexity of inﬁnite words, Int. J. Found. Comput. Sci. 15: 2 (2004) 293–306.


CC:  @sagetrac-abmasse

Component: **combinatorics**

Author: **Sébastien Labbé**

Reviewer: **Alexandre Blondin Massé**

Merged: **sage-4.6.2.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/10265_





---

archive/issue_events_133468.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-11-14T18:05:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-133468"
}
```



---

archive/issue_events_133469.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-11-14T18:05:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-133469"
}
```



---

archive/issue_events_133470.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-11-14T18:05:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-133470"
}
```



---

archive/issue_events_133471.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-11-14T18:05:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-133471"
}
```



---

archive/issue_events_133472.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-11-14T18:05:04Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "subject": "https://github.com/seblabbe",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-133472"
}
```



---

archive/issue_comments_095717.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nNeeds review!",
    "created_at": "2010-11-14T18:09:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-95717",
    "user": "https://github.com/seblabbe"
}
```

<div id="comment:1" align="right">comment:1</div>

Needs review!



---

archive/issue_comments_095718.json:
```json
{
    "body": "Author: **S\u00e9bastien Labb\u00e9**",
    "created_at": "2010-11-14T18:09:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-95718",
    "user": "https://github.com/seblabbe"
}
```

Author: **Sébastien Labbé**



---

archive/issue_events_133473.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-11-14T18:09:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-133473"
}
```



---

archive/issue_comments_095719.json:
```json
{
    "body": "Applies over sage-4.6",
    "created_at": "2010-11-14T18:44:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-95719",
    "user": "https://github.com/seblabbe"
}
```

Applies over sage-4.6



---

archive/issue_comments_095720.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAttachment: **[trac_10265-defect-word-sl.patch.gz](https://github.com/sagemath/sage/files/ticket10265/trac_10265-defect-word-sl.patch.gz)**\n\nI just re-uploaded the patch (fixing one doctest comment).",
    "created_at": "2010-11-14T18:45:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-95720",
    "user": "https://github.com/seblabbe"
}
```

<div id="comment:2" align="right">comment:2</div>

Attachment: **[trac_10265-defect-word-sl.patch.gz](https://github.com/sagemath/sage/files/ticket10265/trac_10265-defect-word-sl.patch.gz)**

I just re-uploaded the patch (fixing one doctest comment).



---

archive/issue_events_133474.json:
```json
{
    "actor": "https://github.com/sagetrac-abmasse",
    "created_at": "2010-11-26T18:52:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-133474"
}
```



---

archive/issue_events_133475.json:
```json
{
    "actor": "https://github.com/sagetrac-abmasse",
    "created_at": "2010-11-26T18:52:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-133475"
}
```



---

archive/issue_events_133476.json:
```json
{
    "actor": "https://github.com/sagetrac-abmasse",
    "created_at": "2010-11-26T18:52:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-133476"
}
```



---

archive/issue_events_133477.json:
```json
{
    "actor": "https://github.com/sagetrac-abmasse",
    "created_at": "2010-11-26T18:52:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-133477"
}
```



---

archive/issue_events_133478.json:
```json
{
    "actor": "https://github.com/sagetrac-abmasse",
    "created_at": "2010-11-26T18:52:22Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-133478"
}
```



---

archive/issue_events_133479.json:
```json
{
    "actor": "https://github.com/sagetrac-abmasse",
    "created_at": "2010-11-26T18:52:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-133479"
}
```



---

archive/issue_comments_095721.json:
```json
{
    "body": "Reviewer: **Alexandre Blondin Mass\u00e9**",
    "created_at": "2010-11-26T18:52:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-95721",
    "user": "https://github.com/sagetrac-abmasse"
}
```

Reviewer: **Alexandre Blondin Massé**



---

archive/issue_comments_095722.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI just finished testing the patch. Everything seems good, all tests passed and the documentation builds find. Positive review.",
    "created_at": "2010-11-26T18:52:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-95722",
    "user": "https://github.com/sagetrac-abmasse"
}
```

<div id="comment:3" align="right">comment:3</div>

I just finished testing the patch. Everything seems good, all tests passed and the documentation builds find. Positive review.



---

archive/issue_comments_095723.json:
```json
{
    "body": "Merged: **sage-4.6.2.alpha0**",
    "created_at": "2011-01-12T06:33:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-95723",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.6.2.alpha0**



---

archive/issue_events_133480.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-12T06:33:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-133480"
}
```



---

archive/issue_events_133481.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-12T06:33:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-133481"
}
```

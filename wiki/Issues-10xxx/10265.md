# Issue 10265: Add palindrome defect word to the word constructor

archive/issues_010264.json:
```json
{
    "body": "Add the finite word `w = a b^k a b^{k-1} a a b^{k-1} a b^{k} a` to the words constructor.\n\nAs described by Brlek, Hamel, Nivat and Reuteunaer in `[1]`, this\nfinite word `w` is such that the infinite periodic word `w^\\omega`\nhave palindromic defect ``k``.\n\n`[1]` S. Brlek, S. Hamel, M. Nivat and C. Reutenauer, On the palindromic complexity of in\ufb01nite words, Int. J. Found. Comput. Sci. 15: 2 (2004) 293\u2013306.\n\n\n**Assignee:** @seblabbe\n\n**CC:**  abmasse\n\n**Author:** S\u00e9bastien Labb\u00e9\n\n**Reviewer:** Alexandre Blondin Mass\u00e9\n\n**Merged:** sage-4.6.2.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/10265\n\n",
    "closed_at": "2011-01-12T06:33:59Z",
    "created_at": "2010-11-14T18:05:04Z",
    "labels": [
        "component: combinatorics",
        "minor",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.2",
    "title": "Add palindrome defect word to the word constructor",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/10265",
    "user": "https://github.com/seblabbe"
}
```
Add the finite word `w = a b^k a b^{k-1} a a b^{k-1} a b^{k} a` to the words constructor.

As described by Brlek, Hamel, Nivat and Reuteunaer in `[1]`, this
finite word `w` is such that the infinite periodic word `w^\omega`
have palindromic defect ``k``.

`[1]` S. Brlek, S. Hamel, M. Nivat and C. Reutenauer, On the palindromic complexity of inﬁnite words, Int. J. Found. Comput. Sci. 15: 2 (2004) 293–306.


**Assignee:** @seblabbe

**CC:**  abmasse

**Author:** Sébastien Labbé

**Reviewer:** Alexandre Blondin Massé

**Merged:** sage-4.6.2.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/10265





---

archive/issue_comments_097715.json:
```json
{
    "body": "<a id='comment:1'></a>\nNeeds review!",
    "created_at": "2010-11-14T18:09:32Z",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-97715",
    "user": "https://github.com/seblabbe"
}
```

<a id='comment:1'></a>
Needs review!



---

archive/issue_comments_097716.json:
```json
{
    "body": "**Author:** S\u00e9bastien Labb\u00e9",
    "created_at": "2010-11-14T18:09:32Z",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-97716",
    "user": "https://github.com/seblabbe"
}
```

**Author:** Sébastien Labbé



---

archive/issue_events_078695.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-11-14T18:09:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-78695"
}
```



---

archive/issue_comments_097717.json:
```json
{
    "body": "Applies over sage-4.6",
    "created_at": "2010-11-14T18:44:51Z",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-97717",
    "user": "https://github.com/seblabbe"
}
```

Applies over sage-4.6



---

archive/attachments_013916.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_10265-defect-word-sl.patch",
    "asset_url": "tarball://root/attachments/ticket10265/trac_10265-defect-word-sl.patch",
    "created_at": "2010-11-14T18:45:53Z",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket10265/trac_10265-defect-word-sl.patch",
    "user": "https://github.com/seblabbe"
}
```



---

archive/issue_comments_097718.json:
```json
{
    "body": "<a id='comment:2'></a>\n**Attachment:** [trac_10265-defect-word-sl.patch](https://github.com/sagemath/sage/files/ticket10265/trac_10265-defect-word-sl.patch)\n\nI just re-uploaded the patch (fixing one doctest comment).",
    "created_at": "2010-11-14T18:45:53Z",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-97718",
    "user": "https://github.com/seblabbe"
}
```

<a id='comment:2'></a>
**Attachment:** [trac_10265-defect-word-sl.patch](https://github.com/sagemath/sage/files/ticket10265/trac_10265-defect-word-sl.patch)

I just re-uploaded the patch (fixing one doctest comment).



---

archive/issue_events_078696.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/abmasse",
    "created_at": "2010-11-26T18:52:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "minor",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-78696"
}
```



---

archive/issue_events_078697.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/abmasse",
    "created_at": "2010-11-26T18:52:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-78697"
}
```



---

archive/issue_events_078698.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/abmasse",
    "created_at": "2010-11-26T18:52:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-78698"
}
```



---

archive/issue_events_078699.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/abmasse",
    "created_at": "2010-11-26T18:52:22Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "milestone": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-78699"
}
```



---

archive/issue_events_078700.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/abmasse",
    "created_at": "2010-11-26T18:52:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "milestone": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-78700"
}
```



---

archive/issue_comments_097719.json:
```json
{
    "body": "**Reviewer:** Alexandre Blondin Mass\u00e9",
    "created_at": "2010-11-26T18:52:22Z",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-97719",
    "user": "https://trac.sagemath.org/admin/accounts/users/abmasse"
}
```

**Reviewer:** Alexandre Blondin Massé



---

archive/issue_comments_097720.json:
```json
{
    "body": "<a id='comment:3'></a>\nI just finished testing the patch. Everything seems good, all tests passed and the documentation builds find. Positive review.",
    "created_at": "2010-11-26T18:52:22Z",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-97720",
    "user": "https://trac.sagemath.org/admin/accounts/users/abmasse"
}
```

<a id='comment:3'></a>
I just finished testing the patch. Everything seems good, all tests passed and the documentation builds find. Positive review.



---

archive/issue_comments_097721.json:
```json
{
    "body": "**Merged:** sage-4.6.2.alpha0",
    "created_at": "2011-01-12T06:33:59Z",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10265#issuecomment-97721",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-4.6.2.alpha0



---

archive/issue_events_078701.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-12T06:33:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-78701"
}
```



---

archive/issue_events_078702.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-12T06:33:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10265",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10265#event-78702"
}
```

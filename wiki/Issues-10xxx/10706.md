# Issue 10706: A doctest of word morphism is saving an object locally

archive/issues_010646.json:
```json
{
    "body": "The file `sage/combinat/words/morphism.py` contains the following test which save an object locally :\n\n```\nsage: save(w,'test')\n```\n\nA local file is thus created each time `sage -t` is run on the file...\n\n**Assignee:** @seblabbe\n\n**CC:**  abmasse\n\n**Author:** S\u00e9bastien Labb\u00e9\n\n**Reviewer:** Benjamin Dexheimer\n\n**Merged:** sage-4.6.2.rc0\n\nIssue created by migration from https://trac.sagemath.org/ticket/10706\n\n",
    "closed_at": "2011-02-16T08:50:23Z",
    "created_at": "2011-01-28T20:59:43Z",
    "labels": [
        "component: doctest coverage",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.2",
    "title": "A doctest of word morphism is saving an object locally",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/10706",
    "user": "https://github.com/seblabbe"
}
```
The file `sage/combinat/words/morphism.py` contains the following test which save an object locally :

```
sage: save(w,'test')
```

A local file is thus created each time `sage -t` is run on the file...

**Assignee:** @seblabbe

**CC:**  abmasse

**Author:** Sébastien Labbé

**Reviewer:** Benjamin Dexheimer

**Merged:** sage-4.6.2.rc0

Issue created by migration from https://trac.sagemath.org/ticket/10706





---

archive/issue_comments_104785.json:
```json
{
    "body": "**Author:** S\u00e9bastien Labb\u00e9",
    "created_at": "2011-01-28T21:01:24Z",
    "issue": "https://github.com/sagemath/sage/issues/10706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10706#issuecomment-104785",
    "user": "https://github.com/seblabbe"
}
```

**Author:** Sébastien Labbé



---

archive/issue_events_083046.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2011-01-28T21:01:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10706",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10706#event-83046"
}
```



---

archive/attachments_014526.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_10706_word_morphism_doctest_fix-sl.patch",
    "asset_url": "tarball://root/attachments/ticket10706/trac_10706_word_morphism_doctest_fix-sl.patch",
    "created_at": "2011-01-28T21:01:24Z",
    "issue": "https://github.com/sagemath/sage/issues/10706",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket10706/trac_10706_word_morphism_doctest_fix-sl.patch",
    "user": "https://github.com/seblabbe"
}
```



---

archive/issue_comments_104786.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_10706_word_morphism_doctest_fix-sl.patch](https://github.com/sagemath/sage/files/ticket10706/trac_10706_word_morphism_doctest_fix-sl.patch)",
    "created_at": "2011-01-28T21:01:24Z",
    "issue": "https://github.com/sagemath/sage/issues/10706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10706#issuecomment-104786",
    "user": "https://github.com/seblabbe"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_10706_word_morphism_doctest_fix-sl.patch](https://github.com/sagemath/sage/files/ticket10706/trac_10706_word_morphism_doctest_fix-sl.patch)



---

archive/issue_comments_104787.json:
```json
{
    "body": "**Reviewer:** Benjamin Dexheimer",
    "created_at": "2011-02-08T10:36:21Z",
    "issue": "https://github.com/sagemath/sage/issues/10706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10706#issuecomment-104787",
    "user": "https://trac.sagemath.org/admin/accounts/users/benjamin"
}
```

**Reviewer:** Benjamin Dexheimer



---

archive/issue_events_083047.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/benjamin",
    "created_at": "2011-02-08T10:36:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10706",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10706#event-83047"
}
```



---

archive/issue_events_083048.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/benjamin",
    "created_at": "2011-02-08T10:36:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10706",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10706#event-83048"
}
```



---

archive/issue_comments_104788.json:
```json
{
    "body": "**Merged:** sage-4.6.2.rc0",
    "created_at": "2011-02-16T08:50:23Z",
    "issue": "https://github.com/sagemath/sage/issues/10706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10706#issuecomment-104788",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-4.6.2.rc0



---

archive/issue_events_083049.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-16T08:50:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10706",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10706#event-83049"
}
```



---

archive/issue_events_083050.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-16T08:50:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10706",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10706#event-83050"
}
```

# Issue 10456: Schuetzenberger Involution on Tableau

archive/issues_010403.json:
```json
{
    "body": "Duplicate ticket accidentally opened. Please delete.\n\nIssue created by migration from https://trac.sagemath.org/ticket/10456\n\n",
    "closed_at": "2010-12-10T05:11:30Z",
    "created_at": "2010-12-10T02:01:02Z",
    "labels": [
        "duplicate"
    ],
    "title": "Schuetzenberger Involution on Tableau",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/10456",
    "user": "https://trac.sagemath.org/admin/accounts/users/ebeyerstedt"
}
```
Duplicate ticket accidentally opened. Please delete.

Issue created by migration from https://trac.sagemath.org/ticket/10456





---

archive/issue_comments_100023.json:
```json
{
    "body": "**Changing keywords** from \"schuetzenberger, involution, words\" to \"\".",
    "created_at": "2010-12-10T02:04:52Z",
    "issue": "https://github.com/sagemath/sage/issues/10456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10456#issuecomment-100023",
    "user": "https://trac.sagemath.org/admin/accounts/users/ebeyerstedt"
}
```

**Changing keywords** from "schuetzenberger, involution, words" to "".



---

archive/issue_comments_100024.json:
```json
{
    "body": "**Remove assignee** sage-combinat.",
    "created_at": "2010-12-10T02:04:52Z",
    "issue": "https://github.com/sagemath/sage/issues/10456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10456#issuecomment-100024",
    "user": "https://trac.sagemath.org/admin/accounts/users/ebeyerstedt"
}
```

**Remove assignee** sage-combinat.



---

archive/issue_comments_100025.json:
```json
{
    "body": "**Changing author** from \"Erin Beyerstedt\" to \"\".",
    "created_at": "2010-12-10T02:04:52Z",
    "issue": "https://github.com/sagemath/sage/issues/10456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10456#issuecomment-100025",
    "user": "https://trac.sagemath.org/admin/accounts/users/ebeyerstedt"
}
```

**Changing author** from "Erin Beyerstedt" to "".



---

archive/issue_comments_100026.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-Implementation of the schuetzenberger involution on tableau.\n+Duplicate ticket accidentally opened. Please delete.\n``````\n",
    "created_at": "2010-12-10T02:04:52Z",
    "issue": "https://github.com/sagemath/sage/issues/10456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10456#issuecomment-100026",
    "user": "https://trac.sagemath.org/admin/accounts/users/ebeyerstedt"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1 @@
-Implementation of the schuetzenberger involution on tableau.
+Duplicate ticket accidentally opened. Please delete.
``````




---

archive/issue_events_080187.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/ebeyerstedt",
    "created_at": "2010-12-10T02:04:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10456",
    "label": "component: combinatorics",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10456#event-80187"
}
```



---

archive/issue_comments_100027.json:
```json
{
    "body": "<a id='comment:2'></a>\nDuplicate ticket accidentally opened.",
    "created_at": "2010-12-10T05:11:30Z",
    "issue": "https://github.com/sagemath/sage/issues/10456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10456#issuecomment-100027",
    "user": "https://trac.sagemath.org/admin/accounts/users/ebeyerstedt"
}
```

<a id='comment:2'></a>
Duplicate ticket accidentally opened.



---

archive/issue_events_080188.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/ebeyerstedt",
    "created_at": "2010-12-10T05:11:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10456",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10456#event-80188"
}
```



---

archive/issue_events_080189.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/ebeyerstedt",
    "created_at": "2010-12-10T05:11:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10456",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10456#event-80189"
}
```

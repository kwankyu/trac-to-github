# Issue 10863: Add check for orthogonal/unitary matrices

archive/issues_010795.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout",
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nAdds `.is_unitary()` for matrices over general rings, plus an over-riding method for matrices over RDF and CDF.\n\n**Depends on:**\n1. #10536\n\n**Apply:**\n1. [attachment: trac_10863-unitary-matrices.patch](https://github.com/sagemath/sage/files/ticket10863/trac_10863-unitary-matrices.patch.gz)\n\nComponent: **linear algebra**\n\nAuthor: **Rob Beezer**\n\nReviewer: **Martin Raum**\n\nMerged: **sage-4.7.alpha4**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10863_\n\n",
    "closed_at": "2011-04-05T12:00:37Z",
    "created_at": "2011-03-01T04:49:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add check for orthogonal/unitary matrices",
    "type": "issue",
    "updated_at": "2011-04-05T12:00:37Z",
    "url": "https://github.com/sagemath/sage/issues/10863",
    "user": "https://github.com/rbeezer"
}
```
<div id="comment:0"></div>

Adds `.is_unitary()` for matrices over general rings, plus an over-riding method for matrices over RDF and CDF.

**Depends on:**
1. #10536

**Apply:**
1. [attachment: trac_10863-unitary-matrices.patch](https://github.com/sagemath/sage/files/ticket10863/trac_10863-unitary-matrices.patch.gz)

Component: **linear algebra**

Author: **Rob Beezer**

Reviewer: **Martin Raum**

Merged: **sage-4.7.alpha4**

_Issue created by migration from https://trac.sagemath.org/ticket/10863_





---

archive/issue_events_142582.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-03-01T04:49:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "c: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10863#event-142582"
}
```



---

archive/issue_events_142583.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-03-01T04:49:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10863#event-142583"
}
```



---

archive/issue_events_142584.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-03-01T04:49:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10863#event-142584"
}
```



---

archive/issue_events_142585.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2011-03-01T04:49:38Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "subject": "https://github.com/rbeezer",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10863#event-142585"
}
```



---

archive/issue_events_142586.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-03-01T04:49:38Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "subject": "https://github.com/rbeezer",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10863#event-142586"
}
```



---

archive/issue_comments_105207.json:
```json
{
    "body": "Attachment: **[trac_10863-unitary-matrices.patch.gz](https://github.com/sagemath/sage/files/ticket10863/trac_10863-unitary-matrices.patch.gz)**",
    "created_at": "2011-03-01T04:52:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10863#issuecomment-105207",
    "user": "https://github.com/rbeezer"
}
```

Attachment: **[trac_10863-unitary-matrices.patch.gz](https://github.com/sagemath/sage/files/ticket10863/trac_10863-unitary-matrices.patch.gz)**



---

archive/issue_comments_105208.json:
```json
{
    "body": "Author: **Rob Beezer**",
    "created_at": "2011-03-01T04:54:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10863#issuecomment-105208",
    "user": "https://github.com/rbeezer"
}
```

Author: **Rob Beezer**



---

archive/issue_events_142587.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-03-01T04:54:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10863#event-142587"
}
```



---

archive/issue_comments_105209.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n Adds `.is_unitary()` for matrices over general rings, plus an over-riding method for matrices over RDF and CDF.\n+\n+Depends #10536\n``````\n",
    "created_at": "2011-03-01T04:54:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10863#issuecomment-105209",
    "user": "https://github.com/rbeezer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 Adds `.is_unitary()` for matrices over general rings, plus an over-riding method for matrices over RDF and CDF.
+
+Depends #10536
``````




---

archive/issue_comments_105210.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nSee relevant discussion on #10848.",
    "created_at": "2011-03-05T19:08:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10863#issuecomment-105210",
    "user": "https://github.com/rbeezer"
}
```

<div id="comment:2" align="right">comment:2</div>

See relevant discussion on #10848.



---

archive/issue_comments_105211.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,7 @@\n Adds `.is_unitary()` for matrices over general rings, plus an over-riding method for matrices over RDF and CDF.\n \n-Depends #10536\n+**Depends on:**\n+1. #10536\n+\n+**Apply:**\n+1. [attachment: trac_10863-unitary-matrices.patch](https://github.com/sagemath/sage/files/ticket10863/trac_10863-unitary-matrices.patch.gz)\n``````\n",
    "created_at": "2011-03-25T03:32:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10863#issuecomment-105211",
    "user": "https://github.com/sagetrac-mraum"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,7 @@
 Adds `.is_unitary()` for matrices over general rings, plus an over-riding method for matrices over RDF and CDF.
 
-Depends #10536
+**Depends on:**
+1. #10536
+
+**Apply:**
+1. [attachment: trac_10863-unitary-matrices.patch](https://github.com/sagemath/sage/files/ticket10863/trac_10863-unitary-matrices.patch.gz)
``````




---

archive/issue_comments_105212.json:
```json
{
    "body": "Reviewer: **Martin Raum**",
    "created_at": "2011-03-25T03:32:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10863#issuecomment-105212",
    "user": "https://github.com/sagetrac-mraum"
}
```

Reviewer: **Martin Raum**



---

archive/issue_comments_105213.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nLooks good to me; All tests pass.",
    "created_at": "2011-03-25T03:32:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10863#issuecomment-105213",
    "user": "https://github.com/sagetrac-mraum"
}
```

<div id="comment:3" align="right">comment:3</div>

Looks good to me; All tests pass.



---

archive/issue_events_142588.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2011-03-25T03:32:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10863#event-142588"
}
```



---

archive/issue_events_142589.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2011-03-25T03:32:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10863#event-142589"
}
```



---

archive/issue_events_142590.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-25T08:38:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "milestone_number": null,
    "milestone_title": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10863#event-142590"
}
```



---

archive/issue_comments_105214.json:
```json
{
    "body": "Merged: **sage-4.7.alpha4**",
    "created_at": "2011-04-05T12:00:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10863#issuecomment-105214",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.alpha4**



---

archive/issue_events_142591.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-04-05T12:00:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10863#event-142591"
}
```



---

archive/issue_events_142592.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-04-05T12:00:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10863",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10863#event-142592"
}
```

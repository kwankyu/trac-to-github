# Issue 10253: tutorial on base conversion

archive/issues_010252.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nAs the subject says. See [this thread](https://groups.google.com/group/sage-devel/browse_thread/thread/36f73d1e9def6683) for some background information.\n\nCC:  @KPanComputes\n\nComponent: **documentation**\n\nAuthor: **Riccardo Murri**\n\nBranch/Commit: **[u/ncohen/10253](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/10253) @ [`9e619ef`](https://github.com/sagemath/sagetrac-mirror/commit/9e619efd4728998ec8f4461ec5eac775a7a2c478)**\n\nReviewer: **Nathann Cohen**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10253_\n\n",
    "closed_at": "2013-12-17T19:59:39Z",
    "created_at": "2010-11-12T10:25:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "tutorial on base conversion",
    "type": "issue",
    "updated_at": "2013-12-17T19:59:39Z",
    "url": "https://github.com/sagemath/sage/issues/10253",
    "user": "https://github.com/sagetrac-mvngu"
}
```
<div id="comment:0"></div>

As the subject says. See [this thread](https://groups.google.com/group/sage-devel/browse_thread/thread/36f73d1e9def6683) for some background information.

CC:  @KPanComputes

Component: **documentation**

Author: **Riccardo Murri**

Branch/Commit: **[u/ncohen/10253](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/10253) @ [`9e619ef`](https://github.com/sagemath/sagetrac-mirror/commit/9e619efd4728998ec8f4461ec5eac775a7a2c478)**

Reviewer: **Nathann Cohen**

_Issue created by migration from https://trac.sagemath.org/ticket/10253_





---

archive/issue_events_133265.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-11-12T10:25:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-133265"
}
```



---

archive/issue_events_133266.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-11-12T10:25:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-133266"
}
```



---

archive/issue_events_133267.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-11-12T10:25:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-133267"
}
```



---

archive/issue_events_133268.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-11-12T10:25:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-133268"
}
```



---

archive/issue_events_133269.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-11-12T10:25:37Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "subject": "https://github.com/sagetrac-mvngu",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-133269"
}
```



---

archive/issue_comments_095520.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAdding beginner as the docs for the question at hand is fairly straight forward to write.",
    "created_at": "2013-02-10T22:31:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-95520",
    "user": "https://github.com/KPanComputes"
}
```

<div id="comment:1" align="right">comment:1</div>

Adding beginner as the docs for the question at hand is fairly straight forward to write.



---

archive/issue_events_133270.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-02-10T22:31:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-133270"
}
```



---

archive/issue_events_133271.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-133271"
}
```



---

archive/issue_events_133272.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-133272"
}
```



---

archive/issue_comments_095521.json:
```json
{
    "body": "Add pointers to base conversion features in docstring of `IntegerRing_class`",
    "created_at": "2013-08-25T10:19:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-95521",
    "user": "https://github.com/riccardomurri"
}
```

Add pointers to base conversion features in docstring of `IntegerRing_class`



---

archive/issue_events_133273.json:
```json
{
    "actor": "https://github.com/riccardomurri",
    "created_at": "2013-08-25T10:22:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-133273"
}
```



---

archive/issue_comments_095522.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAttachment: **[sage-issue-10253.diff.gz](https://github.com/sagemath/sage/files/ticket10253/sage-issue-10253.diff.gz)**\n\nI agree with what Nick Anderson said in the linked ML thread that the features are already documented, so I'm not sure what exactly should be done here.  The attached patch is my attempt at providing some pointers to the relevant functions in the main `IntegerRing_class` documentation.",
    "created_at": "2013-08-25T10:22:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-95522",
    "user": "https://github.com/riccardomurri"
}
```

<div id="comment:3" align="right">comment:3</div>

Attachment: **[sage-issue-10253.diff.gz](https://github.com/sagemath/sage/files/ticket10253/sage-issue-10253.diff.gz)**

I agree with what Nick Anderson said in the linked ML thread that the features are already documented, so I'm not sure what exactly should be done here.  The attached patch is my attempt at providing some pointers to the relevant functions in the main `IntegerRing_class` documentation.



---

archive/issue_comments_095523.json:
```json
{
    "body": "Author: **Riccardo Murri**",
    "created_at": "2013-12-04T13:17:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-95523",
    "user": "https://github.com/nathanncohen"
}
```

Author: **Riccardo Murri**



---

archive/issue_comments_095524.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nOkayokay, makes sense `:-)`\n\nI converted the .diff file to a git commit, and I set it to positive review.\n\nThanks !\n\nNathann",
    "created_at": "2013-12-04T13:17:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-95524",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">comment:4</div>

Okayokay, makes sense `:-)`

I converted the .diff file to a git commit, and I set it to positive review.

Thanks !

Nathann



---

archive/issue_comments_095525.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2013-12-04T13:17:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-95525",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_comments_095526.json:
```json
{
    "body": "Branch: **[u/ncohen/10253](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/10253)**",
    "created_at": "2013-12-04T13:17:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-95526",
    "user": "https://github.com/nathanncohen"
}
```

Branch: **[u/ncohen/10253](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/10253)**



---

archive/issue_comments_095527.json:
```json
{
    "body": "<div id=\"comment:5\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><code>[9e619ef](https://github.com/sagemath/sagetrac-mirror/commit/9e619ef)</code></td><td><code>trac #10253: additional explanation on base conversion of integers</code></td></tr></table>\n",
    "created_at": "2013-12-04T13:17:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-95527",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><code>[9e619ef](https://github.com/sagemath/sagetrac-mirror/commit/9e619ef)</code></td><td><code>trac #10253: additional explanation on base conversion of integers</code></td></tr></table>




---

archive/issue_comments_095528.json:
```json
{
    "body": "Commit: **[`9e619ef`](https://github.com/sagemath/sagetrac-mirror/commit/9e619efd4728998ec8f4461ec5eac775a7a2c478)**",
    "created_at": "2013-12-04T13:17:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-95528",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[`9e619ef`](https://github.com/sagemath/sagetrac-mirror/commit/9e619efd4728998ec8f4461ec5eac775a7a2c478)**



---

archive/issue_events_133274.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-12-04T13:18:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-133274"
}
```



---

archive/issue_events_133275.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-12-04T13:18:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-133275"
}
```



---

archive/issue_events_133276.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-04T18:15:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "milestone_number": null,
    "milestone_title": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-133276"
}
```



---

archive/issue_events_133277.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-04T18:15:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "milestone_number": null,
    "milestone_title": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-133277"
}
```



---

archive/issue_events_133278.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "milestone_number": null,
    "milestone_title": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-133278"
}
```



---

archive/issue_events_133279.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-133279"
}
```



---

archive/issue_events_133280.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-17T19:59:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-133280"
}
```



---

archive/issue_events_133281.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5920a6337a8c609cf1750313e49b622fe9bbfc21",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2013-12-17T19:59:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-133281"
}
```

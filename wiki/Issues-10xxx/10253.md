# Issue 10253: tutorial on base conversion

archive/issues_010252.json:
```json
{
    "body": "As the subject says. See [this thread](https://groups.google.com/group/sage-devel/browse_thread/thread/36f73d1e9def6683) for some background information.\n\n**Assignee:** mvngu\n\n**CC:**  @KPanComputes\n\n**Branch:** [u/ncohen/10253](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/10253)\n\n**Commit:** [9e619efd4728998ec8f4461ec5eac775a7a2c478](https://github.com/sagemath/sagetrac-mirror/commit/9e619efd4728998ec8f4461ec5eac775a7a2c478)\n\n**Reviewer:** Nathann Cohen\n\n**Author:** Riccardo Murri\n\nIssue created by migration from https://trac.sagemath.org/ticket/10253\n\n",
    "closed_at": "2013-12-17T19:59:39Z",
    "created_at": "2010-11-12T10:25:37Z",
    "labels": [
        "component: documentation",
        "enhancement",
        "good first issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.1",
    "title": "tutorial on base conversion",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/10253",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```
As the subject says. See [this thread](https://groups.google.com/group/sage-devel/browse_thread/thread/36f73d1e9def6683) for some background information.

**Assignee:** mvngu

**CC:**  @KPanComputes

**Branch:** [u/ncohen/10253](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/10253)

**Commit:** [9e619efd4728998ec8f4461ec5eac775a7a2c478](https://github.com/sagemath/sagetrac-mirror/commit/9e619efd4728998ec8f4461ec5eac775a7a2c478)

**Reviewer:** Nathann Cohen

**Author:** Riccardo Murri

Issue created by migration from https://trac.sagemath.org/ticket/10253





---

archive/issue_comments_097517.json:
```json
{
    "body": "<a id='comment:1'></a>\nAdding beginner as the docs for the question at hand is fairly straight forward to write.",
    "created_at": "2013-02-10T22:31:42Z",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-97517",
    "user": "https://github.com/KPanComputes"
}
```

<a id='comment:1'></a>
Adding beginner as the docs for the question at hand is fairly straight forward to write.



---

archive/issue_events_078558.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-02-10T22:31:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "label": "good first issue",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-78558"
}
```



---

archive/issue_events_078559.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-78559"
}
```



---

archive/issue_events_078560.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-78560"
}
```



---

archive/issue_comments_097518.json:
```json
{
    "body": "Add pointers to base conversion features in docstring of `IntegerRing_class`",
    "created_at": "2013-08-25T10:19:38Z",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-97518",
    "user": "https://github.com/riccardomurri"
}
```

Add pointers to base conversion features in docstring of `IntegerRing_class`



---

archive/issue_events_078561.json:
```json
{
    "actor": "https://github.com/riccardomurri",
    "created_at": "2013-08-25T10:22:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-78561"
}
```



---

archive/attachments_013903.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "sage-issue-10253.diff",
    "asset_url": "tarball://root/attachments/ticket10253/sage-issue-10253.diff",
    "created_at": "2013-08-25T10:22:52Z",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket10253/sage-issue-10253.diff",
    "user": "https://github.com/riccardomurri"
}
```



---

archive/issue_comments_097519.json:
```json
{
    "body": "<a id='comment:3'></a>\n**Attachment:** [sage-issue-10253.diff](https://github.com/sagemath/sage/files/ticket10253/sage-issue-10253.diff)\n\nI agree with what Nick Anderson said in the linked ML thread that the features are already documented, so I'm not sure what exactly should be done here.  The attached patch is my attempt at providing some pointers to the relevant functions in the main `IntegerRing_class` documentation.",
    "created_at": "2013-08-25T10:22:52Z",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-97519",
    "user": "https://github.com/riccardomurri"
}
```

<a id='comment:3'></a>
**Attachment:** [sage-issue-10253.diff](https://github.com/sagemath/sage/files/ticket10253/sage-issue-10253.diff)

I agree with what Nick Anderson said in the linked ML thread that the features are already documented, so I'm not sure what exactly should be done here.  The attached patch is my attempt at providing some pointers to the relevant functions in the main `IntegerRing_class` documentation.



---

archive/issue_comments_097520.json:
```json
{
    "body": "**Author:** Riccardo Murri",
    "created_at": "2013-12-04T13:17:43Z",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-97520",
    "user": "https://github.com/nathanncohen"
}
```

**Author:** Riccardo Murri



---

archive/issue_comments_097521.json:
```json
{
    "body": "<a id='comment:4'></a>\nOkayokay, makes sense `:-)`\n\nI converted the .diff file to a git commit, and I set it to positive review.\n\nThanks !\n\nNathann",
    "created_at": "2013-12-04T13:17:43Z",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-97521",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:4'></a>
Okayokay, makes sense `:-)`

I converted the .diff file to a git commit, and I set it to positive review.

Thanks !

Nathann



---

archive/issue_comments_097522.json:
```json
{
    "body": "**Reviewer:** Nathann Cohen",
    "created_at": "2013-12-04T13:17:43Z",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-97522",
    "user": "https://github.com/nathanncohen"
}
```

**Reviewer:** Nathann Cohen



---

archive/issue_comments_097523.json:
```json
{
    "body": "**Branch:** [u/ncohen/10253](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/10253)",
    "created_at": "2013-12-04T13:17:43Z",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-97523",
    "user": "https://github.com/nathanncohen"
}
```

**Branch:** [u/ncohen/10253](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/10253)



---

archive/issue_comments_097524.json:
```json
{
    "body": "<a id='comment:5'></a>\n**Branch pushed to git repo; I updated commit sha1.** **New commits:**\n<table><tr><td>[9e619ef](https://github.com/sagemath/sagetrac-mirror/commit/9e619ef)</td><td><code>trac #10253: additional explanation on base conversion of integers</code></td></tr></table>\n",
    "created_at": "2013-12-04T13:17:58Z",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-97524",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:5'></a>
**Branch pushed to git repo; I updated commit sha1.** **New commits:**
<table><tr><td>[9e619ef](https://github.com/sagemath/sagetrac-mirror/commit/9e619ef)</td><td><code>trac #10253: additional explanation on base conversion of integers</code></td></tr></table>




---

archive/issue_comments_097525.json:
```json
{
    "body": "**Commit:** [9e619efd4728998ec8f4461ec5eac775a7a2c478](https://github.com/sagemath/sagetrac-mirror/commit/9e619efd4728998ec8f4461ec5eac775a7a2c478)",
    "created_at": "2013-12-04T13:17:58Z",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10253#issuecomment-97525",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

**Commit:** [9e619efd4728998ec8f4461ec5eac775a7a2c478](https://github.com/sagemath/sagetrac-mirror/commit/9e619efd4728998ec8f4461ec5eac775a7a2c478)



---

archive/issue_events_078562.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-12-04T13:18:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-78562"
}
```



---

archive/issue_events_078563.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-12-04T13:18:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-78563"
}
```



---

archive/issue_events_078564.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-04T18:15:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "milestone": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-78564"
}
```



---

archive/issue_events_078565.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-04T18:15:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "milestone": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-78565"
}
```



---

archive/issue_events_078566.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "milestone": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-78566"
}
```



---

archive/issue_events_078567.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-78567"
}
```



---

archive/issue_events_078568.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-17T19:59:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-78568"
}
```



---

archive/issue_events_078569.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5920a6337a8c609cf1750313e49b622fe9bbfc21",
    "created_at": "2013-12-17T19:59:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10253",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10253#event-78569"
}
```

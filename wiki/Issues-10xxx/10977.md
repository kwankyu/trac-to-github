# Issue 10977: Document vector constructor behavior

archive/issues_010898.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout",
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nVector constructor accepts complex numbers and generators as input, which the `Sequence()` constructor is able to handle, thus allowing a vector to be constructed.\n\nDocumentation-only patch aims to make this clear.\n\nDepends on #10595.\n\nCC:  @jhpalmieri\n\nComponent: **linear algebra**\n\nAuthor: **Rob Beezer**\n\nReviewer: **Martin Raum**\n\nMerged: **sage-4.7.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10977_\n\n",
    "closed_at": "2011-03-28T07:18:44Z",
    "created_at": "2011-03-21T23:11:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Document vector constructor behavior",
    "type": "issue",
    "updated_at": "2011-03-28T07:18:44Z",
    "url": "https://github.com/sagemath/sage/issues/10977",
    "user": "https://github.com/rbeezer"
}
```
<div id="comment:0"></div>

Vector constructor accepts complex numbers and generators as input, which the `Sequence()` constructor is able to handle, thus allowing a vector to be constructed.

Documentation-only patch aims to make this clear.

Depends on #10595.

CC:  @jhpalmieri

Component: **linear algebra**

Author: **Rob Beezer**

Reviewer: **Martin Raum**

Merged: **sage-4.7.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/10977_





---

archive/issue_events_144335.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-03-21T23:11:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "milestone_number": null,
    "milestone_title": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10977#event-144335"
}
```



---

archive/issue_events_144336.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-03-21T23:11:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "c: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10977#event-144336"
}
```



---

archive/issue_events_144337.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-03-21T23:11:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10977#event-144337"
}
```



---

archive/issue_events_144338.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-03-21T23:11:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10977#event-144338"
}
```



---

archive/issue_events_144339.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2011-03-21T23:11:44Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "subject": "https://github.com/rbeezer",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10977#event-144339"
}
```



---

archive/issue_events_144340.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-03-21T23:11:44Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "subject": "https://github.com/rbeezer",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10977#event-144340"
}
```



---

archive/issue_comments_107899.json:
```json
{
    "body": "Attachment: **[trac_10977-vector-constructor-doc.patch.gz](https://github.com/sagemath/sage/files/ticket10977/trac_10977-vector-constructor-doc.patch.gz)**",
    "created_at": "2011-03-21T23:39:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10977#issuecomment-107899",
    "user": "https://github.com/rbeezer"
}
```

Attachment: **[trac_10977-vector-constructor-doc.patch.gz](https://github.com/sagemath/sage/files/ticket10977/trac_10977-vector-constructor-doc.patch.gz)**



---

archive/issue_events_144341.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-03-21T23:40:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10977#event-144341"
}
```



---

archive/issue_events_144342.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-03-21T23:40:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10977#event-144342"
}
```



---

archive/issue_comments_107900.json:
```json
{
    "body": "Author: **Rob Beezer**",
    "created_at": "2011-03-21T23:40:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10977#issuecomment-107900",
    "user": "https://github.com/rbeezer"
}
```

Author: **Rob Beezer**



---

archive/issue_comments_107901.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nTo the patchbot: this depends on #10595.",
    "created_at": "2011-03-23T22:45:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10977#issuecomment-107901",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:2" align="right">comment:2</div>

To the patchbot: this depends on #10595.



---

archive/issue_comments_107902.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n Vector constructor accepts complex numbers and generators as input, which the `Sequence()` constructor is able to handle, thus allowing a vector to be constructed.\n \n Documentation-only patch aims to make this clear.\n+\n+Depends on #10595.\n``````\n",
    "created_at": "2011-03-23T22:45:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10977#issuecomment-107902",
    "user": "https://github.com/jhpalmieri"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 Vector constructor accepts complex numbers and generators as input, which the `Sequence()` constructor is able to handle, thus allowing a vector to be constructed.
 
 Documentation-only patch aims to make this clear.
+
+Depends on #10595.
``````




---

archive/issue_events_144343.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2011-03-25T04:51:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10977#event-144343"
}
```



---

archive/issue_events_144344.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2011-03-25T04:51:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10977#event-144344"
}
```



---

archive/issue_comments_107903.json:
```json
{
    "body": "Reviewer: **Martin Raum**",
    "created_at": "2011-03-25T04:51:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10977#issuecomment-107903",
    "user": "https://github.com/sagetrac-mraum"
}
```

Reviewer: **Martin Raum**



---

archive/issue_comments_107904.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nThat's an useful clarification; And it looks good as is.",
    "created_at": "2011-03-25T04:51:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10977#issuecomment-107904",
    "user": "https://github.com/sagetrac-mraum"
}
```

<div id="comment:3" align="right">comment:3</div>

That's an useful clarification; And it looks good as is.



---

archive/issue_events_144345.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-28T07:18:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10977#event-144345"
}
```



---

archive/issue_events_144346.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-28T07:18:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10977#event-144346"
}
```



---

archive/issue_comments_107905.json:
```json
{
    "body": "Merged: **sage-4.7.alpha3**",
    "created_at": "2011-03-28T07:18:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10977#issuecomment-107905",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.alpha3**

# Issue 10486: Conversion List -> PARI finite field extension element disregards characteristic

archive/issues_010433.json:
```json
{
    "body": "Reported by Yannick Neyt in personal communication:\n\n```\nsage: k.<a>=GF(3^11)\nsage: x = k([1,0,1,1]); x\na^3 + a^2 + 1\nsage: x+x+x\n3*a^3 + 3*a^2\n```\n\nClearly, we should get zero in the last line, since the characteristic is 3.  The problem is visible in the PARI representation:\n\n```\nsage: pari(x)\nMod(a^3 + a^2 + Mod(1, 3), Mod(1, 3)*a^11 + Mod(2, 3)*a^2 + Mod(1, 3))\n```\n\n**Assignee:** @aghitza\n\n**Keywords:** finite field pari\n\n**Author:** Jeroen Demeyer\n\n**Reviewer:** John Cremona\n\n**Merged:** sage-4.6.2.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/10486\n\n",
    "closed_at": "2011-01-12T06:40:29Z",
    "created_at": "2010-12-16T16:28:21Z",
    "labels": [
        "component: basic arithmetic",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.2",
    "title": "Conversion List -> PARI finite field extension element disregards characteristic",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/10486",
    "user": "https://github.com/jdemeyer"
}
```
Reported by Yannick Neyt in personal communication:

```
sage: k.<a>=GF(3^11)
sage: x = k([1,0,1,1]); x
a^3 + a^2 + 1
sage: x+x+x
3*a^3 + 3*a^2
```

Clearly, we should get zero in the last line, since the characteristic is 3.  The problem is visible in the PARI representation:

```
sage: pari(x)
Mod(a^3 + a^2 + Mod(1, 3), Mod(1, 3)*a^11 + Mod(2, 3)*a^2 + Mod(1, 3))
```

**Assignee:** @aghitza

**Keywords:** finite field pari

**Author:** Jeroen Demeyer

**Reviewer:** John Cremona

**Merged:** sage-4.6.2.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/10486





---

archive/attachments_014175.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "10486.patch",
    "asset_url": "tarball://root/attachments/ticket10486/10486.patch",
    "created_at": "2010-12-16T17:16:01Z",
    "issue": "https://github.com/sagemath/sage/issues/10486",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket10486/10486.patch",
    "user": "https://github.com/jdemeyer"
}
```



---

archive/issue_comments_100601.json:
```json
{
    "body": "**Attachment:** [10486.patch](https://github.com/sagemath/sage/files/ticket10486/10486.patch)",
    "created_at": "2010-12-16T17:16:01Z",
    "issue": "https://github.com/sagemath/sage/issues/10486",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10486#issuecomment-100601",
    "user": "https://github.com/jdemeyer"
}
```

**Attachment:** [10486.patch](https://github.com/sagemath/sage/files/ticket10486/10486.patch)



---

archive/issue_comments_100602.json:
```json
{
    "body": "**Author:** Jeroen Demeyer",
    "created_at": "2010-12-16T17:21:39Z",
    "issue": "https://github.com/sagemath/sage/issues/10486",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10486#issuecomment-100602",
    "user": "https://github.com/jdemeyer"
}
```

**Author:** Jeroen Demeyer



---

archive/issue_events_080546.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-12-16T17:21:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10486",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10486#event-80546"
}
```



---

archive/issue_comments_100603.json:
```json
{
    "body": "**Reviewer:** John Cremona",
    "created_at": "2010-12-17T10:40:08Z",
    "issue": "https://github.com/sagemath/sage/issues/10486",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10486#issuecomment-100603",
    "user": "https://github.com/JohnCremona"
}
```

**Reviewer:** John Cremona



---

archive/issue_comments_100604.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good, just testing now...",
    "created_at": "2010-12-17T10:40:08Z",
    "issue": "https://github.com/sagemath/sage/issues/10486",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10486#issuecomment-100604",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:2'></a>
Looks good, just testing now...



---

archive/issue_events_080547.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-12-17T11:10:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10486",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10486#event-80547"
}
```



---

archive/issue_events_080548.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-12-17T11:10:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10486",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10486#event-80548"
}
```



---

archive/issue_events_080549.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-12T06:40:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10486",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10486#event-80549"
}
```



---

archive/issue_events_080550.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-12T06:40:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10486",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10486#event-80550"
}
```



---

archive/issue_comments_100605.json:
```json
{
    "body": "**Merged:** sage-4.6.2.alpha0",
    "created_at": "2011-01-12T06:40:29Z",
    "issue": "https://github.com/sagemath/sage/issues/10486",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10486#issuecomment-100605",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-4.6.2.alpha0

# Issue 10634: Weighted Choice

archive/issues_010581.json:
```json
{
    "body": "This adds weighted choice to the choice function in prandom.py.\n\n**Assignee:** @jasongrout\n\n**Author:** Eviatar Bach\n\nIssue created by migration from https://trac.sagemath.org/ticket/10634\n\n",
    "created_at": "2011-01-15T03:23:12Z",
    "labels": [
        "component: misc",
        "minor",
        "enhancement",
        "needs work"
    ],
    "title": "Weighted Choice",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/10634",
    "user": "https://github.com/eviatarbach"
}
```
This adds weighted choice to the choice function in prandom.py.

**Assignee:** @jasongrout

**Author:** Eviatar Bach

Issue created by migration from https://trac.sagemath.org/ticket/10634





---

archive/attachments_014426.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "15208.patch",
    "asset_url": "tarball://root/attachments/ticket10634/15208.patch",
    "created_at": "2011-01-15T03:52:54Z",
    "issue": "https://github.com/sagemath/sage/issues/10634",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket10634/15208.patch",
    "user": "https://github.com/eviatarbach"
}
```



---

archive/issue_comments_103664.json:
```json
{
    "body": "**Attachment:** [15208.patch](https://github.com/sagemath/sage/files/ticket10634/15208.patch)",
    "created_at": "2011-01-15T03:52:54Z",
    "issue": "https://github.com/sagemath/sage/issues/10634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10634#issuecomment-103664",
    "user": "https://github.com/eviatarbach"
}
```

**Attachment:** [15208.patch](https://github.com/sagemath/sage/files/ticket10634/15208.patch)



---

archive/issue_events_082311.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2011-01-15T03:53:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10634",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10634#event-82311"
}
```



---

archive/issue_comments_103665.json:
```json
{
    "body": "<a id='comment:2'></a>\nA few points:\n\n(1) There are a few typos in the doc (\"choose choose\", and \"if given a list\" reads better than \"if inputted a list\").\n\n(2) The use of factor=1/max(seq.values()) means that the code can run forever if the maximum weight is a Python integer > 1, because then factor becomes 0.\n\n```\n\nsage: choice({1:int(2)})\n[forever..]\n\n```\n\n(3) I don't know if it matters, but the while True approach can be quite inefficient for large arrays, because the odds of succeeding on any given try can be pretty low:\n\n```\nsage: d = dict((i,1) for i in range(10**4))\nsage: time z = choice(d)\nCPU times: user 0.14 s, sys: 0.00 s, total: 0.14 s\nWall time: 0.15 s\n\nsage: time z = choice(range(10**4))\nCPU times: user 0.00 s, sys: 0.00 s, total: 0.00 s\nWall time: 0.00 s\n```",
    "created_at": "2011-02-16T14:05:11Z",
    "issue": "https://github.com/sagemath/sage/issues/10634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10634#issuecomment-103665",
    "user": "https://trac.sagemath.org/admin/accounts/users/dsm"
}
```

<a id='comment:2'></a>
A few points:

(1) There are a few typos in the doc ("choose choose", and "if given a list" reads better than "if inputted a list").

(2) The use of factor=1/max(seq.values()) means that the code can run forever if the maximum weight is a Python integer > 1, because then factor becomes 0.

```

sage: choice({1:int(2)})
[forever..]

```

(3) I don't know if it matters, but the while True approach can be quite inefficient for large arrays, because the odds of succeeding on any given try can be pretty low:

```
sage: d = dict((i,1) for i in range(10**4))
sage: time z = choice(d)
CPU times: user 0.14 s, sys: 0.00 s, total: 0.14 s
Wall time: 0.15 s

sage: time z = choice(range(10**4))
CPU times: user 0.00 s, sys: 0.00 s, total: 0.00 s
Wall time: 0.00 s
```



---

archive/issue_events_082312.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-05-08T14:49:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10634",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10634#event-82312"
}
```



---

archive/issue_events_082313.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-05-08T14:49:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10634",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10634#event-82313"
}
```



---

archive/issue_comments_103666.json:
```json
{
    "body": "<a id='comment:3'></a>\nUntil the previous comments are addressed/answered... `:-)`\n\nOh, and it would be nice to keep the lines to less than 80 characters !\n\nNathann",
    "created_at": "2011-05-08T14:49:55Z",
    "issue": "https://github.com/sagemath/sage/issues/10634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10634#issuecomment-103666",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:3'></a>
Until the previous comments are addressed/answered... `:-)`

Oh, and it would be nice to keep the lines to less than 80 characters !

Nathann



---

archive/issue_events_082314.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10634",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10634#event-82314"
}
```



---

archive/issue_events_082315.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10634",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10634#event-82315"
}
```



---

archive/issue_events_082316.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10634",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10634#event-82316"
}
```



---

archive/issue_events_082317.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10634",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10634#event-82317"
}
```



---

archive/issue_events_082318.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10634",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10634#event-82318"
}
```



---

archive/issue_events_082319.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10634",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10634#event-82319"
}
```



---

archive/issue_events_082320.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10634",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10634#event-82320"
}
```



---

archive/issue_events_082321.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10634",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10634#event-82321"
}
```



---

archive/issue_events_082322.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10634",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10634#event-82322"
}
```

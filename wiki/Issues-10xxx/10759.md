# Issue 10759: remove dead code in matrix/matrix_dense.pyx

archive/issues_010696.json:
```json
{
    "body": "while trying to improve the coverage of `matrix/matrix_dense.pyx`,\nwe noticed with Marc Fuentes that the function\n`_multiply_classical` from that file is dead code.\nIndeed, all instances of that function in other files are locally\ndefined, and it seems impossible to exhibit a test which calls that\nfunction. We therefore propose to remove that dead code.\n\nIndeed, after removing that function, all doctests still pass.\n\n**Assignee:** @jasongrout, @williamstein\n\n**CC:**  Marc Fuentes\n\n**Author:** Marc Fuentes, Paul Zimmermann\n\n**Reviewer:** Nathann Cohen\n\n**Merged:** sage-4.7.1.alpha1\n\nIssue created by migration from https://trac.sagemath.org/ticket/10759\n\n",
    "closed_at": "2011-05-25T12:52:38Z",
    "created_at": "2011-02-08T22:05:58Z",
    "labels": [
        "component: linear algebra",
        "minor",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.1",
    "title": "remove dead code in matrix/matrix_dense.pyx",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/10759",
    "user": "https://github.com/zimmermann6"
}
```
while trying to improve the coverage of `matrix/matrix_dense.pyx`,
we noticed with Marc Fuentes that the function
`_multiply_classical` from that file is dead code.
Indeed, all instances of that function in other files are locally
defined, and it seems impossible to exhibit a test which calls that
function. We therefore propose to remove that dead code.

Indeed, after removing that function, all doctests still pass.

**Assignee:** @jasongrout, @williamstein

**CC:**  Marc Fuentes

**Author:** Marc Fuentes, Paul Zimmermann

**Reviewer:** Nathann Cohen

**Merged:** sage-4.7.1.alpha1

Issue created by migration from https://trac.sagemath.org/ticket/10759





---

archive/attachments_014600.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_10759.patch",
    "asset_url": "tarball://root/attachments/ticket10759/trac_10759.patch",
    "created_at": "2011-02-08T22:07:18Z",
    "issue": "https://github.com/sagemath/sage/issues/10759",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket10759/trac_10759.patch",
    "user": "https://github.com/zimmermann6"
}
```



---

archive/issue_comments_105520.json:
```json
{
    "body": "**Attachment:** [trac_10759.patch](https://github.com/sagemath/sage/files/ticket10759/trac_10759.patch)",
    "created_at": "2011-02-08T22:07:18Z",
    "issue": "https://github.com/sagemath/sage/issues/10759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10759#issuecomment-105520",
    "user": "https://github.com/zimmermann6"
}
```

**Attachment:** [trac_10759.patch](https://github.com/sagemath/sage/files/ticket10759/trac_10759.patch)



---

archive/issue_comments_105521.json:
```json
{
    "body": "**Author:** Marc Fuentes, Paul Zimmermann",
    "created_at": "2011-02-08T22:07:59Z",
    "issue": "https://github.com/sagemath/sage/issues/10759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10759#issuecomment-105521",
    "user": "https://github.com/zimmermann6"
}
```

**Author:** Marc Fuentes, Paul Zimmermann



---

archive/issue_events_083594.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-02-08T22:07:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10759",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10759#event-83594"
}
```



---

archive/issue_comments_105522.json:
```json
{
    "body": "<a id='comment:3'></a>\nPasses -testall -long, and it's a private method anyway... So I guess it's safe to let it go `:-)`\n\nNathann",
    "created_at": "2011-05-08T18:21:23Z",
    "issue": "https://github.com/sagemath/sage/issues/10759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10759#issuecomment-105522",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:3'></a>
Passes -testall -long, and it's a private method anyway... So I guess it's safe to let it go `:-)`

Nathann



---

archive/issue_events_083595.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-05-08T18:21:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10759",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10759#event-83595"
}
```



---

archive/issue_events_083596.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-05-08T18:21:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10759",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10759#event-83596"
}
```



---

archive/issue_comments_105523.json:
```json
{
    "body": "**Reviewer:** Nathann Cohen",
    "created_at": "2011-05-08T18:21:23Z",
    "issue": "https://github.com/sagemath/sage/issues/10759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10759#issuecomment-105523",
    "user": "https://github.com/nathanncohen"
}
```

**Reviewer:** Nathann Cohen



---

archive/issue_events_083597.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-10T07:31:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10759",
    "milestone": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10759#event-83597"
}
```



---

archive/issue_events_083598.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-10T07:31:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10759",
    "milestone": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10759#event-83598"
}
```



---

archive/issue_events_083599.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-25T12:52:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10759",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10759#event-83599"
}
```



---

archive/issue_events_083600.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-25T12:52:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10759",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10759#event-83600"
}
```



---

archive/issue_comments_105524.json:
```json
{
    "body": "**Merged:** sage-4.7.1.alpha1",
    "created_at": "2011-05-25T12:52:38Z",
    "issue": "https://github.com/sagemath/sage/issues/10759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10759#issuecomment-105524",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-4.7.1.alpha1

# Issue 10623: Move integer factorization functions to a separate file

archive/issues_010570.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nSee [#1145 comment:14](https://github.com/sagemath/sage/issues/1145#comment:14).\n\nDepends on #5945.\n\nCC:  @sagetrac-spancratz\n\nComponent: **factorization**\n\nAuthor: **Andr\u00e9 Apitzsch**\n\nReviewer: **David Kirkby, Mariah Lenox**\n\nMerged: **sage-4.7.1.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10623_\n\n",
    "closed_at": "2011-05-30T14:51:50Z",
    "created_at": "2011-01-13T17:05:57Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20factorization",
        "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Move integer factorization functions to a separate file",
    "type": "issue",
    "updated_at": "2011-05-30T14:51:50Z",
    "url": "https://github.com/sagemath/sage/issues/10623",
    "user": "https://github.com/a-andre"
}
```
<div id="comment:0"></div>

See [#1145 comment:14](https://github.com/sagemath/sage/issues/1145#comment:14).

Depends on #5945.

CC:  @sagetrac-spancratz

Component: **factorization**

Author: **André Apitzsch**

Reviewer: **David Kirkby, Mariah Lenox**

Merged: **sage-4.7.1.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/10623_





---

archive/issue_events_138667.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2011-01-13T17:05:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "milestone_number": null,
    "milestone_title": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10623#event-138667"
}
```



---

archive/issue_events_138668.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2011-01-13T17:05:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20factorization",
    "label_color": "0000ff",
    "label_name": "c: factorization",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10623#event-138668"
}
```



---

archive/issue_events_138669.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2011-01-13T17:05:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10623#event-138669"
}
```



---

archive/issue_events_138670.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2011-01-13T17:05:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10623#event-138670"
}
```



---

archive/issue_events_138671.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2011-01-13T17:09:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10623#event-138671"
}
```



---

archive/issue_events_138672.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2011-01-13T17:09:24Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "title_is": "Move integer factorization functions to a separate file",
    "title_was": "Move integer factorization functions to a separat file",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10623#event-138672"
}
```



---

archive/issue_comments_101496.json:
```json
{
    "body": "Attachment: **[trac_10623.patch.gz](https://github.com/sagemath/sage/files/ticket10623/trac_10623.patch.gz)**",
    "created_at": "2011-01-18T13:54:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10623#issuecomment-101496",
    "user": "https://github.com/a-andre"
}
```

Attachment: **[trac_10623.patch.gz](https://github.com/sagemath/sage/files/ticket10623/trac_10623.patch.gz)**



---

archive/issue_comments_101497.json:
```json
{
    "body": "Reviewer: **David Kirkby**",
    "created_at": "2011-04-27T20:51:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10623#issuecomment-101497",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

Reviewer: **David Kirkby**



---

archive/issue_comments_101498.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI verified the examples using Mathematica, but can't comment on the rest of the ticket. \n\nI think it's useful to indicate how the results are known to be correct. In this case I agree they are. \n\nDave",
    "created_at": "2011-04-27T20:51:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10623#issuecomment-101498",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:2" align="right">comment:2</div>

I verified the examples using Mathematica, but can't comment on the rest of the ticket. 

I think it's useful to indicate how the results are known to be correct. In this case I agree they are. 

Dave



---

archive/issue_comments_101499.json:
```json
{
    "body": "Dependencies: **#5945**",
    "created_at": "2011-05-11T07:30:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10623#issuecomment-101499",
    "user": "https://github.com/robertwb"
}
```

Dependencies: **#5945**



---

archive/issue_comments_101500.json:
```json
{
    "body": "Changed reviewer from **David Kirkby** to **David Kirkby, Mariah Lenox**",
    "created_at": "2011-05-18T14:10:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10623#issuecomment-101500",
    "user": "https://github.com/sagetrac-mariah"
}
```

Changed reviewer from **David Kirkby** to **David Kirkby, Mariah Lenox**



---

archive/issue_comments_101501.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nI applied the patch to sage-4.7.rc2, did 'sage -b', and then ran\n'make testlong'.  All tests passed.  Positive review.",
    "created_at": "2011-05-18T14:10:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10623#issuecomment-101501",
    "user": "https://github.com/sagetrac-mariah"
}
```

<div id="comment:4" align="right">comment:4</div>

I applied the patch to sage-4.7.rc2, did 'sage -b', and then ran
'make testlong'.  All tests passed.  Positive review.



---

archive/issue_events_138673.json:
```json
{
    "actor": "https://github.com/sagetrac-mariah",
    "created_at": "2011-05-18T14:10:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10623#event-138673"
}
```



---

archive/issue_events_138674.json:
```json
{
    "actor": "https://github.com/sagetrac-mariah",
    "created_at": "2011-05-18T14:10:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10623#event-138674"
}
```



---

archive/issue_comments_101502.json:
```json
{
    "body": "Changed dependencies from **#5945** to none",
    "created_at": "2011-05-19T08:38:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10623#issuecomment-101502",
    "user": "https://github.com/jdemeyer"
}
```

Changed dependencies from **#5945** to none



---

archive/issue_events_138675.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-19T08:38:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10623#event-138675"
}
```



---

archive/issue_events_138676.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-19T08:38:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20critical%20/%202",
    "label_color": "ff7700",
    "label_name": "p: critical / 2",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10623#event-138676"
}
```



---

archive/issue_events_138677.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-19T08:38:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "milestone_number": null,
    "milestone_title": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10623#event-138677"
}
```



---

archive/issue_events_138678.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-19T08:38:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10623#event-138678"
}
```



---

archive/issue_comments_101503.json:
```json
{
    "body": "Merged: **sage-4.7.1.alpha2**",
    "created_at": "2011-05-30T14:51:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10623#issuecomment-101503",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.1.alpha2**



---

archive/issue_events_138679.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-30T14:51:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10623#event-138679"
}
```



---

archive/issue_events_138680.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-30T14:51:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10623",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10623#event-138680"
}
```

# Issue 10012: Set _pari_prime when initializing NumberFieldIdeal from a PARI prime ideal

archive/issues_010011.json:
```json
{
    "body": "When a `NumberFieldIdeal` is initialized from a PARI prime ideal, it would be good to set `self._pari_prime` to that PARI prime ideal. Then `self.is_prime()` will not have to compute anything for example.\n\nDependencies: #9898, #9753, #4836\n\n**Assignee:** @jdemeyer\n\n**Keywords:** pari prime ideal\n\n**Author:** Jeroen Demeyer\n\n**Reviewer:** John Cremona\n\n**Merged:** sage-4.6.1.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/10012\n\n",
    "closed_at": "2010-11-01T10:20:00Z",
    "created_at": "2010-09-25T15:21:20Z",
    "labels": [
        "component: number fields",
        "minor",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.1",
    "title": "Set _pari_prime when initializing NumberFieldIdeal from a PARI prime ideal",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/10012",
    "user": "https://github.com/jdemeyer"
}
```
When a `NumberFieldIdeal` is initialized from a PARI prime ideal, it would be good to set `self._pari_prime` to that PARI prime ideal. Then `self.is_prime()` will not have to compute anything for example.

Dependencies: #9898, #9753, #4836

**Assignee:** @jdemeyer

**Keywords:** pari prime ideal

**Author:** Jeroen Demeyer

**Reviewer:** John Cremona

**Merged:** sage-4.6.1.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/10012





---

archive/issue_comments_093366.json:
```json
{
    "body": "**Changing assignee** from @loefflerd to @jdemeyer.",
    "created_at": "2010-09-25T15:31:10Z",
    "issue": "https://github.com/sagemath/sage/issues/10012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10012#issuecomment-93366",
    "user": "https://github.com/jdemeyer"
}
```

**Changing assignee** from @loefflerd to @jdemeyer.



---

archive/issue_comments_093367.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n When a `NumberFieldIdeal` is initialized from a PARI prime ideal, it would be good to set `self._pari_prime` to that PARI prime ideal. Then `self.is_prime()` will not have to compute anything for example.\n+\n+Dependencies: #9898, #9753, #4836\n``````\n",
    "created_at": "2010-09-25T15:31:10Z",
    "issue": "https://github.com/sagemath/sage/issues/10012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10012#issuecomment-93367",
    "user": "https://github.com/jdemeyer"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 When a `NumberFieldIdeal` is initialized from a PARI prime ideal, it would be good to set `self._pari_prime` to that PARI prime ideal. Then `self.is_prime()` will not have to compute anything for example.
+
+Dependencies: #9898, #9753, #4836
``````




---

archive/attachments_013539.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "10012_pari_prime.patch",
    "asset_url": "tarball://root/attachments/ticket10012/10012_pari_prime.patch",
    "created_at": "2010-09-25T15:31:10Z",
    "issue": "https://github.com/sagemath/sage/issues/10012",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket10012/10012_pari_prime.patch",
    "user": "https://github.com/jdemeyer"
}
```



---

archive/issue_comments_093368.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [10012_pari_prime.patch](https://github.com/sagemath/sage/files/ticket10012/10012_pari_prime.patch)",
    "created_at": "2010-09-25T15:31:10Z",
    "issue": "https://github.com/sagemath/sage/issues/10012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10012#issuecomment-93368",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'></a>
**Attachment:** [10012_pari_prime.patch](https://github.com/sagemath/sage/files/ticket10012/10012_pari_prime.patch)



---

archive/issue_events_075837.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-09-27T22:14:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10012",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10012#event-75837"
}
```



---

archive/issue_events_075838.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-10-24T08:17:57Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10012",
    "milestone": "sage-4.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10012#event-75838"
}
```



---

archive/issue_events_075839.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-10-24T08:17:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10012",
    "milestone": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10012#event-75839"
}
```



---

archive/issue_comments_093369.json:
```json
{
    "body": "<a id='comment:4'></a>\nPatch looks good and applies fine to 4.6.rc0.  I'm now doing a full test.",
    "created_at": "2010-10-31T18:18:58Z",
    "issue": "https://github.com/sagemath/sage/issues/10012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10012#issuecomment-93369",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:4'></a>
Patch looks good and applies fine to 4.6.rc0.  I'm now doing a full test.



---

archive/issue_events_075840.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-10-31T18:33:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10012",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10012#event-75840"
}
```



---

archive/issue_events_075841.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-10-31T18:33:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10012",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10012#event-75841"
}
```



---

archive/issue_comments_093370.json:
```json
{
    "body": "<a id='comment:5'></a>\nReplying to [@JohnCremona](#comment%3A4):\n> Patch looks good and applies fine to 4.6.rc0.  I'm now doing a full test.\n\nAll (long) tests pass.",
    "created_at": "2010-10-31T18:33:39Z",
    "issue": "https://github.com/sagemath/sage/issues/10012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10012#issuecomment-93370",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:5'></a>
Replying to [@JohnCremona](#comment%3A4):
> Patch looks good and applies fine to 4.6.rc0.  I'm now doing a full test.

All (long) tests pass.



---

archive/issue_comments_093371.json:
```json
{
    "body": "**Merged:** sage-4.6.1.alpha0",
    "created_at": "2010-11-01T10:20:00Z",
    "issue": "https://github.com/sagemath/sage/issues/10012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10012#issuecomment-93371",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-4.6.1.alpha0



---

archive/issue_events_075842.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-01T10:20:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10012",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10012#event-75842"
}
```



---

archive/issue_events_075843.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-11-01T10:20:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10012",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10012#event-75843"
}
```



---

archive/issue_comments_093372.json:
```json
{
    "body": "**Reviewer:** John Cremona",
    "created_at": "2010-11-01T10:30:59Z",
    "issue": "https://github.com/sagemath/sage/issues/10012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10012#issuecomment-93372",
    "user": "https://github.com/qed777"
}
```

**Reviewer:** John Cremona

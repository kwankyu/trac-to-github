# Issue 10957: Add analytic testing to riemann.pyx

archive/issues_010878.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-evanandel"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nWhile the Riemann Mapping algorithm is numerical, there are a few examples where the analytic map is known. This ticket will incorporate some of those examples into the doctests so we can know whether changes/platform dependent instability make the accuracy better or worse.\n\nCC:  @robertwb @jasongrout @kcrisman @sagetrac-mvngu @jdemeyer @wdjoyner\n\nComponent: **calculus**\n\nKeywords: **riemann map testing**\n\nReviewer: **Burcin Erocal**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/10957_\n\n",
    "closed_at": "2011-06-01T07:21:07Z",
    "created_at": "2011-03-17T18:39:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20calculus",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add analytic testing to riemann.pyx",
    "type": "issue",
    "updated_at": "2011-06-01T07:21:07Z",
    "url": "https://github.com/sagemath/sage/issues/10957",
    "user": "https://github.com/sagetrac-evanandel"
}
```
<div id="comment:0"></div>

While the Riemann Mapping algorithm is numerical, there are a few examples where the analytic map is known. This ticket will incorporate some of those examples into the doctests so we can know whether changes/platform dependent instability make the accuracy better or worse.

CC:  @robertwb @jasongrout @kcrisman @sagetrac-mvngu @jdemeyer @wdjoyner

Component: **calculus**

Keywords: **riemann map testing**

Reviewer: **Burcin Erocal**

_Issue created by migration from https://trac.sagemath.org/ticket/10957_





---

archive/issue_events_143895.json:
```json
{
    "actor": "https://github.com/sagetrac-evanandel",
    "created_at": "2011-03-17T18:39:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/10957",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10957#event-143895"
}
```



---

archive/issue_events_143896.json:
```json
{
    "actor": "https://github.com/sagetrac-evanandel",
    "created_at": "2011-03-17T18:39:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10957",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "c: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10957#event-143896"
}
```



---

archive/issue_events_143897.json:
```json
{
    "actor": "https://github.com/sagetrac-evanandel",
    "created_at": "2011-03-17T18:39:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10957",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10957#event-143897"
}
```



---

archive/issue_events_143898.json:
```json
{
    "actor": "https://github.com/sagetrac-evanandel",
    "created_at": "2011-03-17T18:39:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10957",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10957#event-143898"
}
```



---

archive/issue_events_143899.json:
```json
{
    "actor": "https://github.com/sagetrac-evanandel",
    "created_at": "2011-03-17T18:39:00Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/10957",
    "subject": "https://github.com/sagetrac-evanandel",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10957#event-143899"
}
```



---

archive/issue_events_143900.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2011-05-31T18:37:07Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/10957",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10957#event-143900"
}
```



---

archive/issue_comments_106588.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\n#11273 contains a fix for this ticket. This one should be closed as a duplicate.",
    "created_at": "2011-05-31T18:37:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10957",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10957#issuecomment-106588",
    "user": "https://github.com/burcin"
}
```

<div id="comment:1" align="right">comment:1</div>

#11273 contains a fix for this ticket. This one should be closed as a duplicate.



---

archive/issue_events_143901.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2011-05-31T18:37:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10957",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10957#event-143901"
}
```



---

archive/issue_events_143902.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2011-05-31T18:37:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10957",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10957#event-143902"
}
```



---

archive/issue_events_143903.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2011-05-31T18:37:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10957",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10957#event-143903"
}
```



---

archive/issue_comments_106589.json:
```json
{
    "body": "Reviewer: **Burcin Erocal**",
    "created_at": "2011-05-31T18:37:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/10957",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/10957#issuecomment-106589",
    "user": "https://github.com/burcin"
}
```

Reviewer: **Burcin Erocal**



---

archive/issue_events_143904.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-01T07:21:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10957",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10957#event-143904"
}
```



---

archive/issue_events_143905.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-01T07:21:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/10957",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10957#event-143905"
}
```



---

archive/issue_events_143906.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-01T07:21:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/10957",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10957#event-143906"
}
```



---

archive/issue_events_143907.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-01T07:21:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/10957",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "c6c6c6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/10957#event-143907"
}
```

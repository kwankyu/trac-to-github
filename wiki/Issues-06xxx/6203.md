# Issue 6203: numerical noise on sparc solaris (trivial to fix)

archive/issues_006203.json:
```json
{
    "body": "\n```\nsage -t  devel/sage/sage/symbolic/expression.pyx\n**********************************************************************\nFile \"/home/wstein/build-4.4.0/mark/sage-4.0.1.alpha0/devel/sage-main/sage/symbolic/expression.pyx\", line 5486:\n    sage: f.find_minimum_on_interval(1, 5, tol=1e-3)\nExpected:\n    (-3.288371361890984, 3.42575079030572)\nGot:\n    (-3.2883713618909844, 3.42575079030572)\n**********************************************************************\n1 items had failures:\n   1 of   8 in __main__.example_141\n***Test Failed*** 1 failures.\n```\n\n**Assignee:** @mwhansen\n\n**Author:** Mike Hansen\n\n**Reviewer:** William Stein\n\n**Merged:** 4.0.1.rc0\n\nIssue created by migration from https://trac.sagemath.org/ticket/6203\n\n",
    "closed_at": "2009-06-04T09:05:26Z",
    "created_at": "2009-06-03T23:11:19Z",
    "labels": [
        "component: porting: solaris",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0.1",
    "title": "numerical noise on sparc solaris (trivial to fix)",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/6203",
    "user": "https://github.com/williamstein"
}
```

```
sage -t  devel/sage/sage/symbolic/expression.pyx
**********************************************************************
File "/home/wstein/build-4.4.0/mark/sage-4.0.1.alpha0/devel/sage-main/sage/symbolic/expression.pyx", line 5486:
    sage: f.find_minimum_on_interval(1, 5, tol=1e-3)
Expected:
    (-3.288371361890984, 3.42575079030572)
Got:
    (-3.2883713618909844, 3.42575079030572)
**********************************************************************
1 items had failures:
   1 of   8 in __main__.example_141
***Test Failed*** 1 failures.
```

**Assignee:** @mwhansen

**Author:** Mike Hansen

**Reviewer:** William Stein

**Merged:** 4.0.1.rc0

Issue created by migration from https://trac.sagemath.org/ticket/6203





---

archive/issue_events_040397.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-04T06:34:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6203",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6203#event-40397"
}
```



---

archive/issue_comments_042544.json:
```json
{
    "body": "**Assignee:** @mwhansen",
    "created_at": "2009-06-04T06:34:26Z",
    "issue": "https://github.com/sagemath/sage/issues/6203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6203#issuecomment-42544",
    "user": "https://github.com/mwhansen"
}
```

**Assignee:** @mwhansen



---

archive/issue_events_040398.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-04T06:34:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6203",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6203#event-40398"
}
```



---

archive/issue_events_040399.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-04T08:24:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6203",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6203#event-40399"
}
```



---

archive/attachments_007595.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_6203.patch",
    "asset_url": "tarball://root/attachments/ticket6203/trac_6203.patch",
    "created_at": "2009-06-04T08:24:42Z",
    "issue": "https://github.com/sagemath/sage/issues/6203",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket6203/trac_6203.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_042545.json:
```json
{
    "body": "<a id='comment:2'></a>\n**Attachment:** [trac_6203.patch](https://github.com/sagemath/sage/files/ticket6203/trac_6203.patch)",
    "created_at": "2009-06-04T08:24:42Z",
    "issue": "https://github.com/sagemath/sage/issues/6203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6203#issuecomment-42545",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>
**Attachment:** [trac_6203.patch](https://github.com/sagemath/sage/files/ticket6203/trac_6203.patch)



---

archive/issue_events_040400.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-04T09:05:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6203",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6203#event-40400"
}
```



---

archive/issue_events_040401.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-04T09:05:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6203",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6203#event-40401"
}
```



---

archive/issue_comments_042546.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in 4.0.1.rc0.",
    "created_at": "2009-06-04T09:05:26Z",
    "issue": "https://github.com/sagemath/sage/issues/6203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6203#issuecomment-42546",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'></a>
Merged in 4.0.1.rc0.



---

archive/issue_comments_042547.json:
```json
{
    "body": "**Author:** Mike Hansen",
    "created_at": "2009-06-06T20:43:39Z",
    "issue": "https://github.com/sagemath/sage/issues/6203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6203#issuecomment-42547",
    "user": "https://github.com/mwhansen"
}
```

**Author:** Mike Hansen



---

archive/issue_comments_042548.json:
```json
{
    "body": "**Merged:** 4.0.1.rc0",
    "created_at": "2009-06-06T20:43:39Z",
    "issue": "https://github.com/sagemath/sage/issues/6203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6203#issuecomment-42548",
    "user": "https://github.com/mwhansen"
}
```

**Merged:** 4.0.1.rc0



---

archive/issue_comments_042549.json:
```json
{
    "body": "**Reviewer:** William Stein",
    "created_at": "2009-06-06T20:43:39Z",
    "issue": "https://github.com/sagemath/sage/issues/6203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6203#issuecomment-42549",
    "user": "https://github.com/mwhansen"
}
```

**Reviewer:** William Stein

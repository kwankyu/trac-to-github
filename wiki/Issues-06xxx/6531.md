# Issue 6531: Add generic ring classes to reference manual

archive/issues_006531.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nModule `sage.rings.ring` contains 11 base classes for various types of ring. This should be added to the reference manual. The file also needs a few more doctests to get to 100% coverage.\n\nComponent: **documentation**\n\nKeywords: **rings documentation doctest**\n\nAuthor: **David Loeffler**\n\nReviewer: **John Cremona**\n\nMerged: **Sage 4.1.2.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6531_\n\n",
    "closed_at": "2009-08-31T06:03:39Z",
    "created_at": "2009-07-14T09:59:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/doctest%20coverage"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add generic ring classes to reference manual",
    "type": "issue",
    "updated_at": "2009-08-31T06:03:39Z",
    "url": "https://github.com/sagemath/sage/issues/6531",
    "user": "https://github.com/loefflerd"
}
```
<div id="comment:0"></div>

Module `sage.rings.ring` contains 11 base classes for various types of ring. This should be added to the reference manual. The file also needs a few more doctests to get to 100% coverage.

Component: **documentation**

Keywords: **rings documentation doctest**

Author: **David Loeffler**

Reviewer: **John Cremona**

Merged: **Sage 4.1.2.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/6531_





---

archive/issue_events_078388.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-14T09:59:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "milestone_number": null,
    "milestone_title": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6531#event-78388"
}
```



---

archive/issue_events_078389.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-14T09:59:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6531#event-78389"
}
```



---

archive/issue_events_078390.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-14T09:59:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6531#event-78390"
}
```



---

archive/issue_events_078391.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-14T09:59:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6531#event-78391"
}
```



---

archive/issue_events_078392.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-14T09:59:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6531#event-78392"
}
```



---

archive/issue_events_078393.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-14T09:59:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6531#event-78393"
}
```



---

archive/issue_comments_045028.json:
```json
{
    "body": "patch against 4.1",
    "created_at": "2009-07-14T10:11:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6531#issuecomment-45028",
    "user": "https://github.com/loefflerd"
}
```

patch against 4.1



---

archive/issue_events_078394.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-14T10:16:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6531#event-78394"
}
```



---

archive/issue_comments_045029.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_6531-restify_generic_ring.patch.gz](https://github.com/sagemath/sage/files/ticket6531/trac_6531-restify_generic_ring.patch.gz)**\n\nThis patch does the ReSTifying, adds all missing doctests (although I cheated by flagging some old unpickling functions with \"not tested\"), and comments out a few methods that achieve nothing at all.",
    "created_at": "2009-07-14T10:16:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6531#issuecomment-45029",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_6531-restify_generic_ring.patch.gz](https://github.com/sagemath/sage/files/ticket6531/trac_6531-restify_generic_ring.patch.gz)**

This patch does the ReSTifying, adds all missing doctests (although I cheated by flagging some old unpickling functions with "not tested"), and comments out a few methods that achieve nothing at all.



---

archive/issue_comments_045030.json:
```json
{
    "body": "Author: **David Loeffler**",
    "created_at": "2009-07-14T10:16:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6531#issuecomment-45030",
    "user": "https://github.com/loefflerd"
}
```

Author: **David Loeffler**



---

archive/issue_comments_045031.json:
```json
{
    "body": "Attachment: **[trac_6531-restify_generic_ring-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket6531/trac_6531-restify_generic_ring-rebase.patch.gz)**\n\nReplaces previous; rebased to 4.1.1",
    "created_at": "2009-08-30T14:42:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6531#issuecomment-45031",
    "user": "https://github.com/JohnCremona"
}
```

Attachment: **[trac_6531-restify_generic_ring-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket6531/trac_6531-restify_generic_ring-rebase.patch.gz)**

Replaces previous; rebased to 4.1.1



---

archive/issue_comments_045032.json:
```json
{
    "body": "Reviewer: **John Cremona**",
    "created_at": "2009-08-30T14:43:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6531#issuecomment-45032",
    "user": "https://github.com/JohnCremona"
}
```

Reviewer: **John Cremona**



---

archive/issue_events_078395.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-08-30T14:43:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6531#event-78395"
}
```



---

archive/issue_events_078396.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-08-30T14:43:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6531#event-78396"
}
```



---

archive/issue_comments_045033.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nOn applying this to 4.1.1 there were some merge problems, which I fixed.  The second patch replaces the first and applies to 4.1.1.   I kept the author's name to David!",
    "created_at": "2009-08-30T14:43:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6531#issuecomment-45033",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:2" align="right">comment:2</div>

On applying this to 4.1.1 there were some merge problems, which I fixed.  The second patch replaces the first and applies to 4.1.1.   I kept the author's name to David!



---

archive/issue_comments_045034.json:
```json
{
    "body": "Merged: **Sage 4.1.2.alpha0**",
    "created_at": "2009-08-31T06:03:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6531#issuecomment-45034",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **Sage 4.1.2.alpha0**



---

archive/issue_comments_045035.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged `trac_6531-restify_generic_ring-rebase.patch`. See #6850 for a follow-up to this ticket.",
    "created_at": "2009-08-31T06:03:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6531#issuecomment-45035",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged `trac_6531-restify_generic_ring-rebase.patch`. See #6850 for a follow-up to this ticket.



---

archive/issue_events_078397.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-08-31T06:03:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6531#event-78397"
}
```



---

archive/issue_events_078398.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-08-31T06:03:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6531",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6531#event-78398"
}
```

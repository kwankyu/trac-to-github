# Issue 6859: Add more graph generators

archive/issues_006859.json:
```json
{
    "assignees": [
        "https://github.com/rlmill"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis patch add graph generators for the hyper star, (n,k)-star, n-star, and bubble sort graph.\n\nComponent: **graph theory**\n\nAuthor: **Michael Yurko**\n\nReviewer: **Jason Grout, Minh Van Nguyen**\n\nMerged: **Sage 4.1.2.alpha4**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6859_\n\n",
    "closed_at": "2009-09-24T10:55:52Z",
    "created_at": "2009-09-02T01:42:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add more graph generators",
    "type": "issue",
    "updated_at": "2009-09-27T10:19:53Z",
    "url": "https://github.com/sagemath/sage/issues/6859",
    "user": "https://github.com/sagetrac-myurko"
}
```
<div id="comment:0"></div>

This patch add graph generators for the hyper star, (n,k)-star, n-star, and bubble sort graph.

Component: **graph theory**

Author: **Michael Yurko**

Reviewer: **Jason Grout, Minh Van Nguyen**

Merged: **Sage 4.1.2.alpha4**

_Issue created by migration from https://trac.sagemath.org/ticket/6859_





---

archive/issue_events_082626.json:
```json
{
    "actor": "https://github.com/sagetrac-myurko",
    "created_at": "2009-09-02T01:42:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "milestone_number": null,
    "milestone_title": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6859#event-82626"
}
```



---

archive/issue_events_082627.json:
```json
{
    "actor": "https://github.com/sagetrac-myurko",
    "created_at": "2009-09-02T01:42:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6859#event-82627"
}
```



---

archive/issue_events_082628.json:
```json
{
    "actor": "https://github.com/sagetrac-myurko",
    "created_at": "2009-09-02T01:42:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6859#event-82628"
}
```



---

archive/issue_events_082629.json:
```json
{
    "actor": "https://github.com/sagetrac-myurko",
    "created_at": "2009-09-02T01:42:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6859#event-82629"
}
```



---

archive/issue_events_082630.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-09-02T01:42:16Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "subject": "https://github.com/sagetrac-myurko",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6859#event-82630"
}
```



---

archive/issue_comments_048629.json:
```json
{
    "body": "Attachment: **[trac_6859.patch.gz](https://github.com/sagemath/sage/files/ticket6859/trac_6859.patch.gz)**",
    "created_at": "2009-09-02T01:42:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48629",
    "user": "https://github.com/sagetrac-myurko"
}
```

Attachment: **[trac_6859.patch.gz](https://github.com/sagemath/sage/files/ticket6859/trac_6859.patch.gz)**



---

archive/issue_events_082631.json:
```json
{
    "actor": "https://github.com/sagetrac-myurko",
    "created_at": "2009-09-02T01:44:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6859#event-82631"
}
```



---

archive/issue_comments_048630.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nWould it be possible to add to the docstrings the definition of what these graphs should be ?\n\nNathann",
    "created_at": "2009-09-06T09:54:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48630",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:2" align="right">comment:2</div>

Would it be possible to add to the docstrings the definition of what these graphs should be ?

Nathann



---

archive/issue_comments_048631.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nSure. I'll add definitions.",
    "created_at": "2009-09-06T13:40:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48631",
    "user": "https://github.com/sagetrac-myurko"
}
```

<div id="comment:3" align="right">comment:3</div>

Sure. I'll add definitions.



---

archive/issue_comments_048632.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nI do not know if you are aware of it ( I was not until very recently ) but the docstrings are used to generate a very complete documentation accessible through there :\n\nhttp://www.sagemath.org/doc/reference/graphs.html\n\nThis also means that you can use LaTeX in your description if you deem it necessary, and that the formula will be automatically translated into beautiful equations on this page ;-)\n\nNathann",
    "created_at": "2009-09-06T14:13:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48632",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">comment:4</div>

I do not know if you are aware of it ( I was not until very recently ) but the docstrings are used to generate a very complete documentation accessible through there :

http://www.sagemath.org/doc/reference/graphs.html

This also means that you can use LaTeX in your description if you deem it necessary, and that the formula will be automatically translated into beautiful equations on this page ;-)

Nathann



---

archive/issue_events_082632.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-09-06T14:13:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6859#event-82632"
}
```



---

archive/issue_events_082633.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-09-06T14:13:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6859#event-82633"
}
```



---

archive/issue_comments_048633.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nWell, I knew that you can use Latex in docstrings, but I tried not to use it since it is hard to read when introspecting.",
    "created_at": "2009-09-06T14:32:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48633",
    "user": "https://github.com/sagetrac-myurko"
}
```

<div id="comment:5" align="right">comment:5</div>

Well, I knew that you can use Latex in docstrings, but I tried not to use it since it is hard to read when introspecting.



---

archive/issue_comments_048634.json:
```json
{
    "body": "Attachment: **[trac_6859_definitions.patch.gz](https://github.com/sagemath/sage/files/ticket6859/trac_6859_definitions.patch.gz)**\n\nAdds definitions of graphs to docstrings",
    "created_at": "2009-09-06T15:20:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48634",
    "user": "https://github.com/sagetrac-myurko"
}
```

Attachment: **[trac_6859_definitions.patch.gz](https://github.com/sagemath/sage/files/ticket6859/trac_6859_definitions.patch.gz)**

Adds definitions of graphs to docstrings



---

archive/issue_events_082634.json:
```json
{
    "actor": "https://github.com/sagetrac-myurko",
    "created_at": "2009-09-06T15:21:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6859#event-82634"
}
```



---

archive/issue_events_082635.json:
```json
{
    "actor": "https://github.com/sagetrac-myurko",
    "created_at": "2009-09-06T15:21:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6859#event-82635"
}
```



---

archive/issue_comments_048635.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nVery nice.\n\nI'm attaching a patch which optimizes some of the code to use more python things (like swapping), plus fixes a few typos.  I think someone needs to review my patch.",
    "created_at": "2009-09-22T16:45:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48635",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:7" align="right">comment:7</div>

Very nice.

I'm attaching a patch which optimizes some of the code to use more python things (like swapping), plus fixes a few typos.  I think someone needs to review my patch.



---

archive/issue_comments_048636.json:
```json
{
    "body": "Attachment: **[trac-6859-optimize.patch.gz](https://github.com/sagemath/sage/files/ticket6859/trac-6859-optimize.patch.gz)**\n\napply on top of previous patches",
    "created_at": "2009-09-22T16:46:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48636",
    "user": "https://github.com/jasongrout"
}
```

Attachment: **[trac-6859-optimize.patch.gz](https://github.com/sagemath/sage/files/ticket6859/trac-6859-optimize.patch.gz)**

apply on top of previous patches



---

archive/issue_comments_048637.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nI obviously can't review the patch, but the swapping certainly looks better. Coding too long in java has made forget some of the nice python idioms.",
    "created_at": "2009-09-22T17:54:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48637",
    "user": "https://github.com/sagetrac-myurko"
}
```

<div id="comment:8" align="right">comment:8</div>

I obviously can't review the patch, but the swapping certainly looks better. Coding too long in java has made forget some of the nice python idioms.



---

archive/issue_comments_048638.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nOkay, positive review for your patch.\n\nYou can review my changes (just make sure that you still get the same graphs).  If you okay my changes, change this ticket to \"positive review\".",
    "created_at": "2009-09-22T19:14:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48638",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:9" align="right">comment:9</div>

Okay, positive review for your patch.

You can review my changes (just make sure that you still get the same graphs).  If you okay my changes, change this ticket to "positive review".



---

archive/issue_comments_048639.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nAll the graphs except the n,k star graph worked still. However, it was just a one line fix to keep the v[0] = tmp_bit line inside the for loop (otherwise all the vertices become looped). I've uploaded a one line patch to fix it.",
    "created_at": "2009-09-22T21:37:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48639",
    "user": "https://github.com/sagetrac-myurko"
}
```

<div id="comment:10" align="right">comment:10</div>

All the graphs except the n,k star graph worked still. However, it was just a one line fix to keep the v[0] = tmp_bit line inside the for loop (otherwise all the vertices become looped). I've uploaded a one line patch to fix it.



---

archive/issue_comments_048640.json:
```json
{
    "body": "Attachment: **[trac-6859-optimize-fix.patch.gz](https://github.com/sagemath/sage/files/ticket6859/trac-6859-optimize-fix.patch.gz)**",
    "created_at": "2009-09-22T21:38:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48640",
    "user": "https://github.com/sagetrac-myurko"
}
```

Attachment: **[trac-6859-optimize-fix.patch.gz](https://github.com/sagemath/sage/files/ticket6859/trac-6859-optimize-fix.patch.gz)**



---

archive/issue_comments_048641.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nah, right.  Okay, then, positive review.",
    "created_at": "2009-09-22T22:41:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48641",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:11" align="right">comment:11</div>

ah, right.  Okay, then, positive review.



---

archive/issue_events_082636.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-09-22T22:41:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6859#event-82636"
}
```



---

archive/issue_events_082637.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-09-22T22:41:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6859#event-82637"
}
```



---

archive/issue_comments_048642.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nThe patch `trac_6859-ascii-chars.patch` uses only ASCII characters for quotation marks and long dashes. Without it, merging `trac_6859.patch` and building the reference manual would result in the following error:\n\n```\nreading sources... sage/graphs/graph_generators /scratch/mvngu/release/sage-4.1.2.alpha2/local/lib/python2.6/site-packages/Sphinx-0.5.1-py2.6.egg/sphinx/environment.py:543: DeprecationWarning: BaseException.message has been deprecated as of Python 2.6\n  raise SphinxError(err.message)\nSphinx error:\n```",
    "created_at": "2009-09-24T10:06:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48642",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:12" align="right">comment:12</div>

The patch `trac_6859-ascii-chars.patch` uses only ASCII characters for quotation marks and long dashes. Without it, merging `trac_6859.patch` and building the reference manual would result in the following error:

```
reading sources... sage/graphs/graph_generators /scratch/mvngu/release/sage-4.1.2.alpha2/local/lib/python2.6/site-packages/Sphinx-0.5.1-py2.6.egg/sphinx/environment.py:543: DeprecationWarning: BaseException.message has been deprecated as of Python 2.6
  raise SphinxError(err.message)
Sphinx error:
```



---

archive/issue_comments_048643.json:
```json
{
    "body": "use ASCII characters for quotation marks and long dashes",
    "created_at": "2009-09-24T10:24:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48643",
    "user": "https://github.com/sagetrac-mvngu"
}
```

use ASCII characters for quotation marks and long dashes



---

archive/issue_comments_048644.json:
```json
{
    "body": "Attachment: **[trac_6859-ascii-chars.patch.gz](https://github.com/sagemath/sage/files/ticket6859/trac_6859-ascii-chars.patch.gz)**\n\nAttachment: **[trac_6859-formatting-issues.patch.gz](https://github.com/sagemath/sage/files/ticket6859/trac_6859-formatting-issues.patch.gz)**\n\nproper formatting of lists",
    "created_at": "2009-09-24T10:24:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48644",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[trac_6859-ascii-chars.patch.gz](https://github.com/sagemath/sage/files/ticket6859/trac_6859-ascii-chars.patch.gz)**

Attachment: **[trac_6859-formatting-issues.patch.gz](https://github.com/sagemath/sage/files/ticket6859/trac_6859-formatting-issues.patch.gz)**

proper formatting of lists



---

archive/issue_comments_048645.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nThe patch `trac_6859-formatting-issues.patch` fixes formatting of lists. Without it, I get the following warnings when building the reference manual:\n\n```\nWARNING: /scratch/mvngu/release/sage-4.1.2.alpha2/local/lib/python2.6/site-packages/sage/graphs/graph_generators.py:docstring of sage.graphs.graph_generators.GraphGenerators.HyperStarGraph:22: (WARNING/2) Bullet list ends without a blank line; unexpected unindent.\nWARNING: /scratch/mvngu/release/sage-4.1.2.alpha2/local/lib/python2.6/site-packages/sage/graphs/graph_generators.py:docstring of sage.graphs.graph_generators.GraphGenerators.NKStarGraph:25: (WARNING/2) Bullet list ends without a blank line; unexpected unindent.\nWARNING: /scratch/mvngu/release/sage-4.1.2.alpha2/local/lib/python2.6/site-packages/sage/graphs/graph_generators.py:docstring of sage.graphs.graph_generators.GraphGenerators.NStarGraph:19: (WARNING/2) Bullet list ends without a blank line; unexpected unindent.\n```",
    "created_at": "2009-09-24T10:26:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48645",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:13" align="right">comment:13</div>

The patch `trac_6859-formatting-issues.patch` fixes formatting of lists. Without it, I get the following warnings when building the reference manual:

```
WARNING: /scratch/mvngu/release/sage-4.1.2.alpha2/local/lib/python2.6/site-packages/sage/graphs/graph_generators.py:docstring of sage.graphs.graph_generators.GraphGenerators.HyperStarGraph:22: (WARNING/2) Bullet list ends without a blank line; unexpected unindent.
WARNING: /scratch/mvngu/release/sage-4.1.2.alpha2/local/lib/python2.6/site-packages/sage/graphs/graph_generators.py:docstring of sage.graphs.graph_generators.GraphGenerators.NKStarGraph:25: (WARNING/2) Bullet list ends without a blank line; unexpected unindent.
WARNING: /scratch/mvngu/release/sage-4.1.2.alpha2/local/lib/python2.6/site-packages/sage/graphs/graph_generators.py:docstring of sage.graphs.graph_generators.GraphGenerators.NStarGraph:19: (WARNING/2) Bullet list ends without a blank line; unexpected unindent.
```



---

archive/issue_events_082638.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-24T10:55:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6859#event-82638"
}
```



---

archive/issue_events_082639.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-24T10:55:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6859#event-82639"
}
```



---

archive/issue_comments_048646.json:
```json
{
    "body": "Reviewer: **Jason Grout, Minh Van Nguyen**",
    "created_at": "2009-09-24T10:55:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48646",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **Jason Grout, Minh Van Nguyen**



---

archive/issue_comments_048647.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nMerged patches in this order:\n\n1. `trac_6859.patch`\n2. `trac_6859-ascii-chars.patch`\n3. `trac_6859_definitions.patch`\n4. `trac-6859-optimize.patch`\n5. `trac-6859-optimize-fix.patch`\n6. `trac_6859-formatting-issues.patch`",
    "created_at": "2009-09-24T10:55:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48647",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:14" align="right">comment:14</div>

Merged patches in this order:

1. `trac_6859.patch`
2. `trac_6859-ascii-chars.patch`
3. `trac_6859_definitions.patch`
4. `trac-6859-optimize.patch`
5. `trac-6859-optimize-fix.patch`
6. `trac_6859-formatting-issues.patch`



---

archive/issue_comments_048648.json:
```json
{
    "body": "Author: **Michael Yurko**",
    "created_at": "2009-09-24T10:55:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48648",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Michael Yurko**



---

archive/issue_comments_048649.json:
```json
{
    "body": "Merged: **Sage 4.1.2.alpha3**",
    "created_at": "2009-09-24T10:55:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48649",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **Sage 4.1.2.alpha3**



---

archive/issue_comments_048650.json:
```json
{
    "body": "Changed merged from **Sage 4.1.2.alpha3** to **Sage 4.1.2.alpha4**",
    "created_at": "2009-09-27T10:19:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48650",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed merged from **Sage 4.1.2.alpha3** to **Sage 4.1.2.alpha4**



---

archive/issue_comments_048651.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nThere is no 4.1.2.alpha3. Sage 4.1.2.alpha3 was William Stein's release for working on making the notebook a standalone package.",
    "created_at": "2009-09-27T10:19:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6859",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6859#issuecomment-48651",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:15" align="right">comment:15</div>

There is no 4.1.2.alpha3. Sage 4.1.2.alpha3 was William Stein's release for working on making the notebook a standalone package.

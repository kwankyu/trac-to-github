# Issue 6057: Add a file Solaris.txt / SOLARIS.txt

archive/issues_006057.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nIt would be useful if a file was added in the top-level directory with the current state of the Solaris port. i.e. what the known issues are with Solaris, any tips, workarounds etc. \n\nComponent: **porting: Solaris**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6057_\n\n",
    "closed_at": "2009-05-17T09:20:14Z",
    "created_at": "2009-05-17T08:45:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20solaris",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add a file Solaris.txt / SOLARIS.txt",
    "type": "issue",
    "updated_at": "2009-05-17T09:20:14Z",
    "url": "https://github.com/sagemath/sage/issues/6057",
    "user": "https://github.com/sagetrac-drkirkby"
}
```
<div id="comment:0"></div>

It would be useful if a file was added in the top-level directory with the current state of the Solaris port. i.e. what the known issues are with Solaris, any tips, workarounds etc. 

Component: **porting: Solaris**

_Issue created by migration from https://trac.sagemath.org/ticket/6057_





---

archive/issue_events_072082.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2009-05-17T08:45:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6057",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20solaris",
    "label_color": "0000b0",
    "label_name": "c: porting: solaris",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6057#event-72082"
}
```



---

archive/issue_events_072083.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2009-05-17T08:45:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6057",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6057#event-72083"
}
```



---

archive/issue_events_072084.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2009-05-17T08:45:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6057",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6057#event-72084"
}
```



---

archive/issue_events_072085.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-17T08:45:26Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6057",
    "subject": "https://github.com/sagetrac-drkirkby",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6057#event-72085"
}
```



---

archive/issue_events_072086.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-17T09:20:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6057",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6057#event-72086"
}
```



---

archive/issue_events_072087.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-17T09:20:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6057",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "c6c6c6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6057#event-72087"
}
```



---

archive/issue_comments_039678.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nNo, we have README.txt and the wiki, no need to add anything.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-17T09:20:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6057#issuecomment-39678",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

No, we have README.txt and the wiki, no need to add anything.

Cheers,

Michael



---

archive/issue_events_072088.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-17T09:20:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6057",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6057#event-72088"
}
```

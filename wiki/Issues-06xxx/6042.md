# Issue 6042: Bring doctests of modular/modsym/ambient.py up to 100%

archive/issues_006042.json:
```json
{
    "assignees": [
        "https://github.com/craigcitro"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis 2500-line file has a low score:\n 26% (26 of 97)\n\nI have nearly finished documenting it and will upload a patch over the weekend.\n\n\nComponent: **modular forms**\n\nKeywords: **modular symbols**\n\nAuthor: **John Cremona**\n\nReviewer: **David Loeffler, Alex Ghitza**\n\nMerged: **4.0.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6042_\n\n",
    "closed_at": "2009-05-18T18:05:27Z",
    "created_at": "2009-05-15T07:21:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20documentation"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bring doctests of modular/modsym/ambient.py up to 100%",
    "type": "issue",
    "updated_at": "2009-06-07T09:15:14Z",
    "url": "https://github.com/sagemath/sage/issues/6042",
    "user": "https://github.com/JohnCremona"
}
```
<div id="comment:0"></div>

This 2500-line file has a low score:
 26% (26 of 97)

I have nearly finished documenting it and will upload a patch over the weekend.


Component: **modular forms**

Keywords: **modular symbols**

Author: **John Cremona**

Reviewer: **David Loeffler, Alex Ghitza**

Merged: **4.0.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/6042_





---

archive/issue_events_071903.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-05-15T07:21:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6042#event-71903"
}
```



---

archive/issue_events_071904.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-05-15T07:21:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
    "label_color": "0000ff",
    "label_name": "c: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6042#event-71904"
}
```



---

archive/issue_events_071905.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-05-15T07:21:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6042#event-71905"
}
```



---

archive/issue_events_071906.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-05-15T07:21:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6042#event-71906"
}
```



---

archive/issue_events_071907.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-05-15T07:21:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6042#event-71907"
}
```



---

archive/issue_events_071908.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-05-15T07:21:34Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "subject": "https://github.com/JohnCremona",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6042#event-71908"
}
```



---

archive/issue_comments_039537.json:
```json
{
    "body": "Applies to 3.4.2",
    "created_at": "2009-05-16T15:35:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39537",
    "user": "https://github.com/JohnCremona"
}
```

Applies to 3.4.2



---

archive/issue_events_071909.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-05-16T15:39:20Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "title_is": "[needs rebasing] Bring doctests of modular/modsym/ambient.py up to 100%",
    "title_was": "Bring doctests of modular/modsym/ambient.py up to 100%",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6042#event-71909"
}
```



---

archive/issue_comments_039538.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_6042.patch.gz](https://github.com/sagemath/sage/files/ticket6042/trac_6042.patch.gz)**\n\nThis is just sickening.  I must have spent over 10 hours in the last week documenting this file, resulting in a patch over 2000 lines long, and 100% doctest coverage.  But now I cannot apply it since after the 7 patches on 3 tickets which David warned me about (#5736, #4357 and #5250 all already merged in 4.0alpha0, in that order) I get this mess:\n\n```\njohn@ubuntu%sage -hg qpush\napplying trac_6042.patch\npatching file sage/modular/modsym/ambient.py\nHunk #17 FAILED at 800\nHunk #25 FAILED at 1291\nHunk #45 FAILED at 2230\nHunk #60 FAILED at 2829\nHunk #61 FAILED at 2836\nHunk #65 FAILED at 2962\nHunk #71 FAILED at 3195\n7 out of 76 hunks FAILED -- saving rejects to file sage/modular/modsym/ambient.py.rej\npatch failed, unable to continue (try -v)\npatch failed, rejects left in working dir\nErrors during apply, please fix and refresh trac_6042.patch\n```\n\nI'm really not sure I can be bothered to mess with this any more.  Is there any system to actually help one merge conflicting patches sensibly?  I have never managed to get things like k3diff to work.\n\nI will at least upload my patch  so that it does not get lost, but I have other things to do.",
    "created_at": "2009-05-16T15:39:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39538",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_6042.patch.gz](https://github.com/sagemath/sage/files/ticket6042/trac_6042.patch.gz)**

This is just sickening.  I must have spent over 10 hours in the last week documenting this file, resulting in a patch over 2000 lines long, and 100% doctest coverage.  But now I cannot apply it since after the 7 patches on 3 tickets which David warned me about (#5736, #4357 and #5250 all already merged in 4.0alpha0, in that order) I get this mess:

```
john@ubuntu%sage -hg qpush
applying trac_6042.patch
patching file sage/modular/modsym/ambient.py
Hunk #17 FAILED at 800
Hunk #25 FAILED at 1291
Hunk #45 FAILED at 2230
Hunk #60 FAILED at 2829
Hunk #61 FAILED at 2836
Hunk #65 FAILED at 2962
Hunk #71 FAILED at 3195
7 out of 76 hunks FAILED -- saving rejects to file sage/modular/modsym/ambient.py.rej
patch failed, unable to continue (try -v)
patch failed, rejects left in working dir
Errors during apply, please fix and refresh trac_6042.patch
```

I'm really not sure I can be bothered to mess with this any more.  Is there any system to actually help one merge conflicting patches sensibly?  I have never managed to get things like k3diff to work.

I will at least upload my patch  so that it does not get lost, but I have other things to do.



---

archive/issue_comments_039539.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nReplying to [@JohnCremona](#comment%3A1):\n> This is just sickening.  I must have spent over 10 hours in the last week documenting this file, resulting in a patch over 2000 lines long, and 100% doctest coverage.  But now I cannot apply it since after the 7 patches on 3 tickets which David warned me about (#5736, #4357 and #5250 all already merged in 4.0alpha0, in that order) I get this mess:\n> \n> I will at least upload my patch  so that it does not get lost, but I have other things to do.\n\nHi John,\n\nI feel your pain.  I don't know of an automated system to get this done properly.  But I'll try to rebase it to 4.0alpha0, and review it in the process.  Hopefully I can do this in the next 36 hours or so.",
    "created_at": "2009-05-17T00:11:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39539",
    "user": "https://github.com/aghitza"
}
```

<div id="comment:2" align="right">comment:2</div>

Replying to [@JohnCremona](#comment%3A1):
> This is just sickening.  I must have spent over 10 hours in the last week documenting this file, resulting in a patch over 2000 lines long, and 100% doctest coverage.  But now I cannot apply it since after the 7 patches on 3 tickets which David warned me about (#5736, #4357 and #5250 all already merged in 4.0alpha0, in that order) I get this mess:
> 
> I will at least upload my patch  so that it does not get lost, but I have other things to do.

Hi John,

I feel your pain.  I don't know of an automated system to get this done properly.  But I'll try to rebase it to 4.0alpha0, and review it in the process.  Hopefully I can do this in the next 36 hours or so.



---

archive/issue_comments_039540.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAlex: have you already started on this? (I'm not sure what time zone you're in.) I ask because I'm probably the best placed person to sort it out, as the changes which have caused the conflicts were mine, which also puts me under a certain moral obligation as well :-). I don't want to tread on your toes if you've already done it so let me know if you have, but if not, leave it to me and I'll sort it out tomorrow morning.",
    "created_at": "2009-05-17T21:51:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39540",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:3" align="right">comment:3</div>

Alex: have you already started on this? (I'm not sure what time zone you're in.) I ask because I'm probably the best placed person to sort it out, as the changes which have caused the conflicts were mine, which also puts me under a certain moral obligation as well :-). I don't want to tread on your toes if you've already done it so let me know if you have, but if not, leave it to me and I'll sort it out tomorrow morning.



---

archive/issue_comments_039541.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nI have started but haven't gotten very far yet, so I won't be upset if you want to do it.  Most of the stuff is fairly straightforward, but there are some functions where you introduced documentation and examples, and then John's patch is doing things all over again.  So that would best be done by one of you.",
    "created_at": "2009-05-17T22:04:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39541",
    "user": "https://github.com/aghitza"
}
```

<div id="comment:4" align="right">comment:4</div>

I have started but haven't gotten very far yet, so I won't be upset if you want to do it.  Most of the stuff is fairly straightforward, but there are some functions where you introduced documentation and examples, and then John's patch is doing things all over again.  So that would best be done by one of you.



---

archive/issue_comments_039542.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nI've rebased the patch to 4.0.alpha0 and checked that it passes doctests, and had a look through the code, and it all looks fine -- let's get this in without further delay!",
    "created_at": "2009-05-18T09:08:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39542",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:5" align="right">comment:5</div>

I've rebased the patch to 4.0.alpha0 and checked that it passes doctests, and had a look through the code, and it all looks fine -- let's get this in without further delay!



---

archive/issue_events_071910.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-05-18T09:08:58Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "title_is": "Bring doctests of modular/modsym/ambient.py up to 100%",
    "title_was": "[needs rebasing] Bring doctests of modular/modsym/ambient.py up to 100%",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6042#event-71910"
}
```



---

archive/issue_events_071911.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-05-18T09:08:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6042#event-71911"
}
```



---

archive/issue_comments_039543.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nThanks David (and Alex)!",
    "created_at": "2009-05-18T09:13:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39543",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:6" align="right">comment:6</div>

Thanks David (and Alex)!



---

archive/issue_comments_039544.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nBTW, I see that \"rebasing\" has included \"reattributing credit in the patch header\"! :)",
    "created_at": "2009-05-18T15:23:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39544",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:7" align="right">comment:7</div>

BTW, I see that "rebasing" has included "reattributing credit in the patch header"! :)



---

archive/issue_events_071912.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-18T15:35:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6042#event-71912"
}
```



---

archive/issue_events_071913.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-18T15:35:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6042#event-71913"
}
```



---

archive/issue_comments_039545.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nReplying to [@JohnCremona](#comment%3A7):\n> BTW, I see that \"rebasing\" has included \"reattributing credit in the patch header\"! :)\n\nHehe, I will fix this once I import the patch. \n\nDavid: Before posting the patch you can just edit the credit in the hg header at the top of the file.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-18T15:35:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39545",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:8" align="right">comment:8</div>

Replying to [@JohnCremona](#comment%3A7):
> BTW, I see that "rebasing" has included "reattributing credit in the patch header"! :)

Hehe, I will fix this once I import the patch. 

David: Before posting the patch you can just edit the credit in the hg header at the top of the file.

Cheers,

Michael



---

archive/issue_comments_039546.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nReplying to [@sagetrac-mabshoff](#comment%3A8):\n> Replying to [@JohnCremona](#comment%3A7):\n> > BTW, I see that \"rebasing\" has included \"reattributing credit in the patch header\"! :)\n> \n> \n> Hehe, I will fix this once I import the patch. \n> \n> David: Before posting the patch you can just edit the credit in the hg header at the top of the file.\n> \n> Cheers,\n> \n> Michael\n\nOf course I would like credit to go to David too, if hg will allow.",
    "created_at": "2009-05-18T15:39:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39546",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:9" align="right">comment:9</div>

Replying to [@sagetrac-mabshoff](#comment%3A8):
> Replying to [@JohnCremona](#comment%3A7):
> > BTW, I see that "rebasing" has included "reattributing credit in the patch header"! :)
> 
> 
> Hehe, I will fix this once I import the patch. 
> 
> David: Before posting the patch you can just edit the credit in the hg header at the top of the file.
> 
> Cheers,
> 
> Michael

Of course I would like credit to go to David too, if hg will allow.



---

archive/issue_comments_039547.json:
```json
{
    "body": "Attachment: **[trac_6042-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket6042/trac_6042-rebase.patch.gz)**\n\npatch against 4.0.alpha0",
    "created_at": "2009-05-18T15:39:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39547",
    "user": "https://github.com/loefflerd"
}
```

Attachment: **[trac_6042-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket6042/trac_6042-rebase.patch.gz)**

patch against 4.0.alpha0



---

archive/issue_comments_039548.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nI just found out about \"qrefresh -u\", so I can masquerade as anybody I like. I'll now attribute any patches that don't work to some unsuspecting victim :-) I've uploaded a new version with credit correctly attributed to John.",
    "created_at": "2009-05-18T15:43:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39548",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:10" align="right">comment:10</div>

I just found out about "qrefresh -u", so I can masquerade as anybody I like. I'll now attribute any patches that don't work to some unsuspecting victim :-) I've uploaded a new version with credit correctly attributed to John.



---

archive/issue_comments_039549.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nReplying to [@loefflerd](#comment%3A10):\n> I just found out about \"qrefresh -u\", so I can masquerade as anybody I like. I'll now attribute any patches that don't work to some unsuspecting victim :-) I've uploaded a new version with credit correctly attributed to John.\n\nthanks -- I don't really mind, of course, but I was looking when I made sure that yours was a replacement patch and not a second patch.",
    "created_at": "2009-05-18T15:54:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39549",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:11" align="right">comment:11</div>

Replying to [@loefflerd](#comment%3A10):
> I just found out about "qrefresh -u", so I can masquerade as anybody I like. I'll now attribute any patches that don't work to some unsuspecting victim :-) I've uploaded a new version with credit correctly attributed to John.

thanks -- I don't really mind, of course, but I was looking when I made sure that yours was a replacement patch and not a second patch.



---

archive/issue_comments_039550.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nThe rebased patch on top of two other trivial tickets in my 4.0.rc0 merge tree:\n\n```\nOverall weighted coverage score:  75.0%\nTotal number of functions:  22100\nWe need    6 more function to get to 75% coverage.\n```\n\n:))\n\nCheers,\n\nMichael",
    "created_at": "2009-05-18T16:46:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39550",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:12" align="right">comment:12</div>

The rebased patch on top of two other trivial tickets in my 4.0.rc0 merge tree:

```
Overall weighted coverage score:  75.0%
Total number of functions:  22100
We need    6 more function to get to 75% coverage.
```

:))

Cheers,

Michael



---

archive/issue_comments_039551.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nThat makes it all worth while!",
    "created_at": "2009-05-18T17:03:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39551",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:13" align="right">comment:13</div>

That makes it all worth while!



---

archive/issue_comments_039552.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nMerged in Sage 4.0.rc0.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-18T18:05:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39552",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:14" align="right">comment:14</div>

Merged in Sage 4.0.rc0.

Cheers,

Michael



---

archive/issue_events_071914.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-18T18:05:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6042#event-71914"
}
```



---

archive/issue_events_071915.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-18T18:05:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6042#event-71915"
}
```



---

archive/issue_comments_039553.json:
```json
{
    "body": "Merged: **4.0.rc0**",
    "created_at": "2009-06-07T09:15:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39553",
    "user": "https://github.com/loefflerd"
}
```

Merged: **4.0.rc0**



---

archive/issue_comments_039554.json:
```json
{
    "body": "Reviewer: **David Loeffler, Alex Ghitza**",
    "created_at": "2009-06-07T09:15:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39554",
    "user": "https://github.com/loefflerd"
}
```

Reviewer: **David Loeffler, Alex Ghitza**



---

archive/issue_comments_039555.json:
```json
{
    "body": "Author: **John Cremona**",
    "created_at": "2009-06-07T09:15:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6042",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6042#issuecomment-39555",
    "user": "https://github.com/loefflerd"
}
```

Author: **John Cremona**

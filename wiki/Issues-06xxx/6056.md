# Issue 6056: HISTORY.txt for sage-4.0.alpha0 needs updating

archive/issues_006056.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nThe file starts:\n\n```\n#####################################################################\nSAGE-3.2.3: January 5th, 2009\n#####################################################################\n```\n\n\nCC:  @rlmill @boothby\n\nComponent: **documentation**\n\nAuthor: **Minh Van Nguyen**\n\nReviewer: **Robert Miller**\n\nMerged: **sage-4.1.rc1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6056_\n\n",
    "closed_at": "2009-07-07T03:51:41Z",
    "created_at": "2009-05-17T08:40:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "HISTORY.txt for sage-4.0.alpha0 needs updating",
    "type": "issue",
    "updated_at": "2009-07-07T03:51:41Z",
    "url": "https://github.com/sagemath/sage/issues/6056",
    "user": "https://github.com/sagetrac-drkirkby"
}
```
<div id="comment:0"></div>

The file starts:

```
#####################################################################
SAGE-3.2.3: January 5th, 2009
#####################################################################
```


CC:  @rlmill @boothby

Component: **documentation**

Author: **Minh Van Nguyen**

Reviewer: **Robert Miller**

Merged: **sage-4.1.rc1**

_Issue created by migration from https://trac.sagemath.org/ticket/6056_





---

archive/issue_events_072074.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2009-05-17T08:40:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6056",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6056#event-72074"
}
```



---

archive/issue_events_072075.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2009-05-17T08:40:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6056",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6056#event-72075"
}
```



---

archive/issue_events_072076.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-17T09:19:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6056",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6056#event-72076"
}
```



---

archive/issue_comments_039671.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,7 +1,8 @@\n The file starts:\n \n+```\n #####################################################################\n SAGE-3.2.3: January 5th, 2009\n #####################################################################\n+```\n \n-\n``````\n",
    "created_at": "2009-05-17T09:19:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6056",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6056#issuecomment-39671",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,7 +1,8 @@
 The file starts:
 
+```
 #####################################################################
 SAGE-3.2.3: January 5th, 2009
 #####################################################################
+```
 
-
``````




---

archive/issue_comments_039672.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nNew HISTORY.txt file is up at \n\n\n\nhttp://www.sagemath.org/src/announce/HISTORY.txt\n\n\n\nIt includes releases up to and including Sage 4.0.1. Since I'll be updating this file, my working draft is available at\n\n\n\nhttp://sage.math.washington.edu/home/mvngu/doc/changelog/HISTORY.txt\n\n\n\nI think this ticket can be closed now. There's no need to merge any patch. All one has to do is make sure that the above links aren't broken.",
    "created_at": "2009-06-16T16:18:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6056",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6056#issuecomment-39672",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:2" align="right">comment:2</div>

New HISTORY.txt file is up at 



http://www.sagemath.org/src/announce/HISTORY.txt



It includes releases up to and including Sage 4.0.1. Since I'll be updating this file, my working draft is available at



http://sage.math.washington.edu/home/mvngu/doc/changelog/HISTORY.txt



I think this ticket can be closed now. There's no need to merge any patch. All one has to do is make sure that the above links aren't broken.



---

archive/issue_comments_039673.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI'll be updating `HISTORY.txt` after each release. I think this ticket can be closed now, since there's no patch to apply.",
    "created_at": "2009-06-26T04:43:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6056",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6056#issuecomment-39673",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:3" align="right">comment:3</div>

I'll be updating `HISTORY.txt` after each release. I think this ticket can be closed now, since there's no patch to apply.



---

archive/issue_events_072077.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-07T03:46:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6056",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6056#event-72077"
}
```



---

archive/issue_comments_039674.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nI'm changing this to positive review so the release manager(s) would pick it up and close it as appropriate.",
    "created_at": "2009-07-07T03:46:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6056",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6056#issuecomment-39674",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:4" align="right">comment:4</div>

I'm changing this to positive review so the release manager(s) would pick it up and close it as appropriate.



---

archive/issue_comments_039675.json:
```json
{
    "body": "Author: **Minh Van Nguyen**",
    "created_at": "2009-07-07T03:51:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6056",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6056#issuecomment-39675",
    "user": "https://github.com/rlmill"
}
```

Author: **Minh Van Nguyen**



---

archive/issue_events_072078.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-07T03:51:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6056",
    "milestone_number": null,
    "milestone_title": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6056#event-72078"
}
```



---

archive/issue_events_072079.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-07T03:51:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6056",
    "milestone_number": null,
    "milestone_title": "sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6056#event-72079"
}
```



---

archive/issue_comments_039676.json:
```json
{
    "body": "Reviewer: **Robert Miller**",
    "created_at": "2009-07-07T03:51:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6056",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6056#issuecomment-39676",
    "user": "https://github.com/rlmill"
}
```

Reviewer: **Robert Miller**



---

archive/issue_events_072080.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-07T03:51:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6056",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6056#event-72080"
}
```



---

archive/issue_events_072081.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-07T03:51:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6056",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6056#event-72081"
}
```



---

archive/issue_comments_039677.json:
```json
{
    "body": "Merged: **sage-4.1.rc1**",
    "created_at": "2009-07-07T03:51:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6056",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6056#issuecomment-39677",
    "user": "https://github.com/rlmill"
}
```

Merged: **sage-4.1.rc1**

# Issue 6577: Reference manual build errors in 4.1.1.alpha0

archive/issues_006577.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nThis is in 4.1.1.alpha0: \n\n```\nWARNING: /home/david/sage-4.1/local/lib/python2.6/site-packages/sage/databases/jones.py:docstring of sage.databases.jones.JonesDatabase.get:8: (WARNING/2) Inline literal start-string without end-string.                                              \nWARNING: /home/david/sage-4.1/local/lib/python2.6/site-packages/sage/databases/jones.py:docstring of sage.databases.jones.JonesDatabase.ramified_at:11: (WARNING/2) Inline literal start-string without end-string.                                     \nWARNING: /home/david/sage-4.1/local/lib/python2.6/site-packages/sage/databases/jones.py:docstring of sage.databases.jones.JonesDatabase.unramified_outside:13: (WARNING/2) Inline literal start-string without end-string.                              \nWARNING: /home/david/sage-4.1/local/lib/python2.6/site-packages/sage/schemes/elliptic_curves/ell_rational_field.py:docstring of sage.schemes.elliptic_curves.ell_rational_field.ModularParameterization:3: (WARNING/2) Inline interpreted text or phrase reference start-string without end-string.                                                                                 \nWARNING: /home/david/sage-4.1/local/lib/python2.6/site-packages/sage/schemes/elliptic_curves/ell_rational_field.py:docstring of sage.schemes.elliptic_curves.ell_rational_field.ModularParameterization:4: (WARNING/2) Block quote ends without a blank line; unexpected unindent.\n```        \n\nComponent: **documentation**\n\nKeywords: **reference manual**\n\nAuthor: **David Loeffler**\n\nReviewer: **Minh Van Nguyen**\n\nMerged: **Sage 4.1.1.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6577_\n\n",
    "closed_at": "2009-07-23T01:58:29Z",
    "created_at": "2009-07-21T09:56:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Reference manual build errors in 4.1.1.alpha0",
    "type": "issue",
    "updated_at": "2009-07-23T01:58:29Z",
    "url": "https://github.com/sagemath/sage/issues/6577",
    "user": "https://github.com/loefflerd"
}
```
<div id="comment:0"></div>

This is in 4.1.1.alpha0: 

```
WARNING: /home/david/sage-4.1/local/lib/python2.6/site-packages/sage/databases/jones.py:docstring of sage.databases.jones.JonesDatabase.get:8: (WARNING/2) Inline literal start-string without end-string.                                              
WARNING: /home/david/sage-4.1/local/lib/python2.6/site-packages/sage/databases/jones.py:docstring of sage.databases.jones.JonesDatabase.ramified_at:11: (WARNING/2) Inline literal start-string without end-string.                                     
WARNING: /home/david/sage-4.1/local/lib/python2.6/site-packages/sage/databases/jones.py:docstring of sage.databases.jones.JonesDatabase.unramified_outside:13: (WARNING/2) Inline literal start-string without end-string.                              
WARNING: /home/david/sage-4.1/local/lib/python2.6/site-packages/sage/schemes/elliptic_curves/ell_rational_field.py:docstring of sage.schemes.elliptic_curves.ell_rational_field.ModularParameterization:3: (WARNING/2) Inline interpreted text or phrase reference start-string without end-string.                                                                                 
WARNING: /home/david/sage-4.1/local/lib/python2.6/site-packages/sage/schemes/elliptic_curves/ell_rational_field.py:docstring of sage.schemes.elliptic_curves.ell_rational_field.ModularParameterization:4: (WARNING/2) Block quote ends without a blank line; unexpected unindent.
```        

Component: **documentation**

Keywords: **reference manual**

Author: **David Loeffler**

Reviewer: **Minh Van Nguyen**

Merged: **Sage 4.1.1.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/6577_





---

archive/issue_events_079004.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-21T09:56:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6577",
    "milestone_number": null,
    "milestone_title": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6577#event-79004"
}
```



---

archive/issue_events_079005.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-21T09:56:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6577",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6577#event-79005"
}
```



---

archive/issue_events_079006.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-21T09:56:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6577",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6577#event-79006"
}
```



---

archive/issue_events_079007.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-21T09:56:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6577",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6577#event-79007"
}
```



---

archive/issue_comments_045501.json:
```json
{
    "body": "Attachment: **[trac_6577-refmanual_warnings_4_1_1_alpha0.patch.gz](https://github.com/sagemath/sage/files/ticket6577/trac_6577-refmanual_warnings_4_1_1_alpha0.patch.gz)**",
    "created_at": "2009-07-21T11:21:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6577",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6577#issuecomment-45501",
    "user": "https://github.com/loefflerd"
}
```

Attachment: **[trac_6577-refmanual_warnings_4_1_1_alpha0.patch.gz](https://github.com/sagemath/sage/files/ticket6577/trac_6577-refmanual_warnings_4_1_1_alpha0.patch.gz)**



---

archive/issue_events_079008.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-21T11:22:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6577",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6577#event-79008"
}
```



---

archive/issue_events_079009.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-21T11:22:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6577",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6577#event-79009"
}
```



---

archive/issue_comments_045502.json:
```json
{
    "body": "Changed keywords from none to **reference manual**",
    "created_at": "2009-07-21T11:22:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6577",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6577#issuecomment-45502",
    "user": "https://github.com/loefflerd"
}
```

Changed keywords from none to **reference manual**



---

archive/issue_events_079010.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-21T11:22:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6577",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6577#event-79010"
}
```



---

archive/issue_comments_045503.json:
```json
{
    "body": "Author: **David Loeffler**",
    "created_at": "2009-07-21T14:18:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6577",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6577#issuecomment-45503",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **David Loeffler**



---

archive/issue_comments_045504.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nBegone are the warnings :-)",
    "created_at": "2009-07-21T14:18:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6577",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6577#issuecomment-45504",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:2" align="right">comment:2</div>

Begone are the warnings :-)



---

archive/issue_events_079011.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-21T14:18:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6577",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6577#event-79011"
}
```



---

archive/issue_events_079012.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-21T14:18:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6577",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6577#event-79012"
}
```



---

archive/issue_comments_045505.json:
```json
{
    "body": "Reviewer: **Minh Van Nguyen**",
    "created_at": "2009-07-21T14:18:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6577",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6577#issuecomment-45505",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **Minh Van Nguyen**



---

archive/issue_comments_045506.json:
```json
{
    "body": "Merged: **Sage 4.1.1.alpha1**",
    "created_at": "2009-07-23T01:58:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6577",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6577#issuecomment-45506",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **Sage 4.1.1.alpha1**



---

archive/issue_events_079013.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-23T01:58:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6577",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6577#event-79013"
}
```



---

archive/issue_events_079014.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-23T01:58:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6577",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6577#event-79014"
}
```

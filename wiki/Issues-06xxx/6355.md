# Issue 6355: Cliquer to compute maximum cliques

archive/issues_006355.json:
```json
{
    "assignees": [
        "https://github.com/rlmill"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nHello everybody ! I hope this is the last step for this patch to compute the maximum cliques in a graph.\nHere is the SPKG file with the source code of Cliquer.\nAs for planarity or other modules, the original source code of cliquer is copied, but in this case it is copied in local/lib/cliquer-1.2, I was told planarity was to be an exception to the rule.\n\nYou can download the SPKG file at this address :\nhttp://www-sop.inria.fr/members/Nathann.Cohen/cliquer-1.2.spkg\n\nThe vote to include this SPKG can be found here:\nhttp://groups.google.com/group/sage-devel/browse_thread/thread/b24e7c1603b5f988\n\nThere is also an accompanying patch, at #5793.\n\nComponent: **graph theory**\n\nKeywords: **Clique max, Cliquer**\n\nAuthor: **Nathann Cohen**\n\nReviewer: **Robert Miller**\n\nMerged: **Sage 4.1.1.rc1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6355_\n\n",
    "closed_at": "2009-07-31T23:30:48Z",
    "created_at": "2009-06-18T12:57:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Cliquer to compute maximum cliques",
    "type": "issue",
    "updated_at": "2009-07-31T23:30:48Z",
    "url": "https://github.com/sagemath/sage/issues/6355",
    "user": "https://github.com/nathanncohen"
}
```
<div id="comment:0"></div>

Hello everybody ! I hope this is the last step for this patch to compute the maximum cliques in a graph.
Here is the SPKG file with the source code of Cliquer.
As for planarity or other modules, the original source code of cliquer is copied, but in this case it is copied in local/lib/cliquer-1.2, I was told planarity was to be an exception to the rule.

You can download the SPKG file at this address :
http://www-sop.inria.fr/members/Nathann.Cohen/cliquer-1.2.spkg

The vote to include this SPKG can be found here:
http://groups.google.com/group/sage-devel/browse_thread/thread/b24e7c1603b5f988

There is also an accompanying patch, at #5793.

Component: **graph theory**

Keywords: **Clique max, Cliquer**

Author: **Nathann Cohen**

Reviewer: **Robert Miller**

Merged: **Sage 4.1.1.rc1**

_Issue created by migration from https://trac.sagemath.org/ticket/6355_





---

archive/issue_events_075886.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-06-18T12:57:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75886"
}
```



---

archive/issue_events_075887.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-06-18T12:57:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75887"
}
```



---

archive/issue_events_075888.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-06-18T12:57:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75888"
}
```



---

archive/issue_events_075889.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-18T12:57:35Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "subject": "https://github.com/nathanncohen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75889"
}
```



---

archive/issue_events_075890.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-06-18T12:57:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75890"
}
```



---

archive/issue_events_075891.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-19T22:34:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75891"
}
```



---

archive/issue_comments_042455.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThis ticket, plus #5793 and #5669, are all a total mess, IMHO. There are two separate spkg's linked, here and on #5669, and there are two different sets of patches (`11803.patch` is separately 229.4KB and 6.0KB) at #5793 and #5669.\n\n1. Patch names need to be of the form `trac_####-description.patch`.\n2. I don't even know which spkg to review.\n3. #5793 and #5669 are total duplicates.\n\nMy recommendation is to close the other two tickets as duplicates, indicate which spkg to review here, properly format the correct patches to be applied, and post everything here. There's just too much confusion right now.\n\nNathann -- I would really like to see cliquer included in Sage, so if you need any help getting this in, please don't hesitate to contact me directly.",
    "created_at": "2009-06-19T22:34:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42455",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:1" align="right">comment:1</div>

This ticket, plus #5793 and #5669, are all a total mess, IMHO. There are two separate spkg's linked, here and on #5669, and there are two different sets of patches (`11803.patch` is separately 229.4KB and 6.0KB) at #5793 and #5669.

1. Patch names need to be of the form `trac_####-description.patch`.
2. I don't even know which spkg to review.
3. #5793 and #5669 are total duplicates.

My recommendation is to close the other two tickets as duplicates, indicate which spkg to review here, properly format the correct patches to be applied, and post everything here. There's just too much confusion right now.

Nathann -- I would really like to see cliquer included in Sage, so if you need any help getting this in, please don't hesitate to contact me directly.



---

archive/issue_comments_042456.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAs far as I could tell at Sage Days 16, the next step required for cliquer is: Someone needs to implement a build system for Cliquer, either using autotools or SCons. Given how simple cliquer is, SCons is probably the way to go, since this would be something like the one liner `Library('cliquer', ['foo.c', 'bar.c'])`.",
    "created_at": "2009-07-01T14:56:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42456",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:2" align="right">comment:2</div>

As far as I could tell at Sage Days 16, the next step required for cliquer is: Someone needs to implement a build system for Cliquer, either using autotools or SCons. Given how simple cliquer is, SCons is probably the way to go, since this would be something like the one liner `Library('cliquer', ['foo.c', 'bar.c'])`.



---

archive/issue_comments_042457.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI updated the SPKG file for cliquer.. This new one builds a shared library and I do not call any .c file in the cython code anymore !!\n\nThe address is still the same, though ;-)\n\nNathann",
    "created_at": "2009-07-07T13:16:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42457",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:3" align="right">comment:3</div>

I updated the SPKG file for cliquer.. This new one builds a shared library and I do not call any .c file in the cython code anymore !!

The address is still the same, though ;-)

Nathann



---

archive/issue_comments_042458.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nReplying to [@nathanncohen](#comment%3A3):\n> I updated the SPKG file for cliquer.. This new one builds a shared library and I do not call any .c file in the cython code anymore !!\n\nTrue, this builds a shared library, but it doesn't seem very portable. The problem is that the code you've written probably works only on systems very similar to yours. For example, when I try building on OS X (32- or 64-bit), I get:\n\n```\nld: unknown option: -soname\ncollect2: ld returned 1 exit status\ncp: cannot stat `libcliquer.so': No such file or directory\n\nreal\t0m2.000s\nuser\t0m0.465s\nsys\t0m0.190s\nsage: An error occurred while installing cliquer-1.2\n```\n\nYou need to use SCons or some form of autotools as I suggested above, for this to ever be a viable SPKG.",
    "created_at": "2009-07-13T18:56:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42458",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:4" align="right">comment:4</div>

Replying to [@nathanncohen](#comment%3A3):
> I updated the SPKG file for cliquer.. This new one builds a shared library and I do not call any .c file in the cython code anymore !!

True, this builds a shared library, but it doesn't seem very portable. The problem is that the code you've written probably works only on systems very similar to yours. For example, when I try building on OS X (32- or 64-bit), I get:

```
ld: unknown option: -soname
collect2: ld returned 1 exit status
cp: cannot stat `libcliquer.so': No such file or directory

real	0m2.000s
user	0m0.465s
sys	0m0.190s
sage: An error occurred while installing cliquer-1.2
```

You need to use SCons or some form of autotools as I suggested above, for this to ever be a viable SPKG.



---

archive/issue_events_075892.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-07-16T09:56:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75892"
}
```



---

archive/issue_events_075893.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-07-16T09:56:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75893"
}
```



---

archive/issue_comments_042459.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nI just updated the SPKG, which now uses Scons !\n\nNathann",
    "created_at": "2009-07-16T09:56:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42459",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:5" align="right">comment:5</div>

I just updated the SPKG, which now uses Scons !

Nathann



---

archive/issue_comments_042460.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nNathann,\n\nThis is great work! However, you are still hard-coding some Linux-isms in spkg-install:\n\n```\nscons && cp Build/libcliquer.so \"$SAGE_LOCAL/lib/\"\n```\n\nUnfortunately, on OS X, it is `libcliquer.dylib` that is built by SCons. Why don't you copy `Build/*` over, to include all cases? You should also delete the commented part from spkg-install. While I'm reviewing, you need a Mercurial repository in the base directory of the spkg, and the License section of SPKG.txt needs to be clarified (i.e. GPLv2+).\n\nKeep up the good work!",
    "created_at": "2009-07-16T18:45:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42460",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:6" align="right">comment:6</div>

Nathann,

This is great work! However, you are still hard-coding some Linux-isms in spkg-install:

```
scons && cp Build/libcliquer.so "$SAGE_LOCAL/lib/"
```

Unfortunately, on OS X, it is `libcliquer.dylib` that is built by SCons. Why don't you copy `Build/*` over, to include all cases? You should also delete the commented part from spkg-install. While I'm reviewing, you need a Mercurial repository in the base directory of the spkg, and the License section of SPKG.txt needs to be clarified (i.e. GPLv2+).

Keep up the good work!



---

archive/issue_events_075894.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-16T18:45:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75894"
}
```



---

archive/issue_events_075895.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-16T18:45:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75895"
}
```



---

archive/issue_comments_042461.json:
```json
{
    "body": "Changed author from **ncohen** to **Nathann Cohen**",
    "created_at": "2009-07-16T18:45:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42461",
    "user": "https://github.com/rlmill"
}
```

Changed author from **ncohen** to **Nathann Cohen**



---

archive/issue_comments_042462.json:
```json
{
    "body": "Reviewer: **Robert Miller**",
    "created_at": "2009-07-16T18:45:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42462",
    "user": "https://github.com/rlmill"
}
```

Reviewer: **Robert Miller**



---

archive/issue_comments_042463.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nAll has been done according to your wishes ;-)\n\nThe spkg has been updated !",
    "created_at": "2009-07-17T18:18:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42463",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:7" align="right">comment:7</div>

All has been done according to your wishes ;-)

The spkg has been updated !



---

archive/issue_events_075896.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-07-17T18:18:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75896"
}
```



---

archive/issue_events_075897.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-07-17T18:18:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75897"
}
```



---

archive/issue_events_075898.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-17T18:28:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75898"
}
```



---

archive/issue_events_075899.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-17T18:28:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75899"
}
```



---

archive/issue_comments_042464.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\n1. You need to check in your changes to the repo!\n\n2. You should add a line to `spkg-install` which copies header files to the right place, something like `cp src/*.h \"$SAGE_LOCAL/include/cliquer/\"`\n\n3. This is all I can think of, so after this it's a lot of testing on different platforms.",
    "created_at": "2009-07-17T18:28:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42464",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:8" align="right">comment:8</div>

1. You need to check in your changes to the repo!

2. You should add a line to `spkg-install` which copies header files to the right place, something like `cp src/*.h "$SAGE_LOCAL/include/cliquer/"`

3. This is all I can think of, so after this it's a lot of testing on different platforms.



---

archive/issue_events_075900.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-07-17T18:34:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75900"
}
```



---

archive/issue_events_075901.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-07-17T18:34:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75901"
}
```



---

archive/issue_comments_042465.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\ndone again ! ;-)",
    "created_at": "2009-07-17T18:34:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42465",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:9" align="right">comment:9</div>

done again ! ;-)



---

archive/issue_comments_042466.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nChange `cliquer-1.2` to `cliquer` (twice) in `spkg-install`, so that we don't need to keep upgrading code in the Sage library when Cliquer upgrades.\n\nAfter that, I think that this is ready to go! It builds successfully on 32-bit and 64-bit OS X and on sage.math.",
    "created_at": "2009-07-17T18:42:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42466",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:10" align="right">comment:10</div>

Change `cliquer-1.2` to `cliquer` (twice) in `spkg-install`, so that we don't need to keep upgrading code in the Sage library when Cliquer upgrades.

After that, I think that this is ready to go! It builds successfully on 32-bit and 64-bit OS X and on sage.math.



---

archive/issue_comments_042467.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nDone. I have to change the patch though, as the directory changed.",
    "created_at": "2009-07-17T18:46:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42467",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:11" align="right">comment:11</div>

Done. I have to change the patch though, as the directory changed.



---

archive/issue_comments_042468.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nThe spkg looks great! Nice work.\n\nNOTE: This will be little useful without also merging #5793, when it is ready.\n\nNathann, shall we continue at #5793?",
    "created_at": "2009-07-17T18:49:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42468",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:12" align="right">comment:12</div>

The spkg looks great! Nice work.

NOTE: This will be little useful without also merging #5793, when it is ready.

Nathann, shall we continue at #5793?



---

archive/issue_events_075902.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-17T18:49:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75902"
}
```



---

archive/issue_events_075903.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-17T18:49:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75903"
}
```



---

archive/issue_comments_042469.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nA small problem with the creation of a directory, and changes commited to the hg repository.",
    "created_at": "2009-07-17T18:59:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42469",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:13" align="right">comment:13</div>

A small problem with the creation of a directory, and changes commited to the hg repository.



---

archive/issue_comments_042470.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nAh, you beat me to posting the same fix! :-)",
    "created_at": "2009-07-17T19:01:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42470",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:14" align="right">comment:14</div>

Ah, you beat me to posting the same fix! :-)



---

archive/issue_events_075904.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-17T19:06:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75904"
}
```



---

archive/issue_events_075905.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-17T19:06:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75905"
}
```



---

archive/issue_comments_042471.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nHowever, there is now a `cliquer-1.2.spkg` inside the spkg!!!",
    "created_at": "2009-07-17T19:06:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42471",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:15" align="right">comment:15</div>

However, there is now a `cliquer-1.2.spkg` inside the spkg!!!



---

archive/issue_comments_042472.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">comment:16</div>\n\nIt comes from a tar command run in the wrong directory.... fixed ;-)",
    "created_at": "2009-07-17T19:11:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42472",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:16" align="right">comment:16</div>

It comes from a tar command run in the wrong directory.... fixed ;-)



---

archive/issue_events_075906.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-07-17T19:11:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75906"
}
```



---

archive/issue_events_075907.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-07-17T19:11:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75907"
}
```



---

archive/issue_events_075908.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-17T19:12:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75908"
}
```



---

archive/issue_events_075909.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-17T19:12:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75909"
}
```



---

archive/issue_events_075910.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-21T20:32:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "milestone_number": null,
    "milestone_title": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75910"
}
```



---

archive/issue_comments_042473.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,3 +4,6 @@\n \n You can download the SPKG file at this address :\n http://www-sop.inria.fr/members/Nathann.Cohen/cliquer-1.2.spkg\n+\n+The vote to include this SPKG can be found here:\n+http://groups.google.com/group/sage-devel/browse_thread/thread/b24e7c1603b5f988\n``````\n",
    "created_at": "2009-07-21T20:32:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42473",
    "user": "https://github.com/rlmill"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,3 +4,6 @@
 
 You can download the SPKG file at this address :
 http://www-sop.inria.fr/members/Nathann.Cohen/cliquer-1.2.spkg
+
+The vote to include this SPKG can be found here:
+http://groups.google.com/group/sage-devel/browse_thread/thread/b24e7c1603b5f988
``````




---

archive/issue_comments_042474.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,3 +7,5 @@\n \n The vote to include this SPKG can be found here:\n http://groups.google.com/group/sage-devel/browse_thread/thread/b24e7c1603b5f988\n+\n+There is also an accompanying patch, at #5793.\n``````\n",
    "created_at": "2009-07-21T20:41:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42474",
    "user": "https://github.com/rlmill"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,3 +7,5 @@
 
 The vote to include this SPKG can be found here:
 http://groups.google.com/group/sage-devel/browse_thread/thread/b24e7c1603b5f988
+
+There is also an accompanying patch, at #5793.
``````




---

archive/issue_comments_042475.json:
```json
{
    "body": "Merged: **Sage 4.1.1.alpha1**",
    "created_at": "2009-07-23T04:41:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42475",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **Sage 4.1.1.alpha1**



---

archive/issue_comments_042476.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">comment:21</div>\n\nThe SPKG at\n\nhttp://www-sop.inria.fr/members/Nathann.Cohen/cliquer-1.2.spkg\n\ndoesn't conform to the naming convention for SPKG's. I've renamed it as `cliquer-1.2.p0.spkg` and uploaded the renamed SPKG up at\n\nhttp://sage.math.washington.edu/home/mvngu/patch/cliquer-1.2.p0.spkg\n\nAs far as I understand, this new SPKG doesn't depend on #5793. So I'm merging the SPKG into the standard SPKG repository.",
    "created_at": "2009-07-23T04:41:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42476",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:21" align="right">comment:21</div>

The SPKG at

http://www-sop.inria.fr/members/Nathann.Cohen/cliquer-1.2.spkg

doesn't conform to the naming convention for SPKG's. I've renamed it as `cliquer-1.2.p0.spkg` and uploaded the renamed SPKG up at

http://sage.math.washington.edu/home/mvngu/patch/cliquer-1.2.p0.spkg

As far as I understand, this new SPKG doesn't depend on #5793. So I'm merging the SPKG into the standard SPKG repository.



---

archive/issue_events_075911.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-23T04:41:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75911"
}
```



---

archive/issue_events_075912.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-23T04:41:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75912"
}
```



---

archive/issue_comments_042477.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">comment:22</div>\n\nSee #6626 for a follow-up to this ticket.",
    "created_at": "2009-07-26T08:27:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42477",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:22" align="right">comment:22</div>

See #6626 for a follow-up to this ticket.



---

archive/issue_events_075913.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-26T08:27:34Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "title_is": "Cliquer to compute maximum cliques",
    "title_was": "[with SPKG] Cliquer to compute maximum cliques",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75913"
}
```



---

archive/issue_events_075914.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-26T08:27:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75914"
}
```



---

archive/issue_events_075915.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-26T08:27:34Z",
    "event": "reopened",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75915"
}
```



---

archive/issue_events_075916.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-26T08:30:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75916"
}
```



---

archive/issue_comments_042478.json:
```json
{
    "body": "Changed merged from **Sage 4.1.1.alpha1** to none",
    "created_at": "2009-07-26T08:30:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42478",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed merged from **Sage 4.1.1.alpha1** to none



---

archive/issue_comments_042479.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">comment:23</div>\n\nI'm reopening this ticket until #6626 is fixed.",
    "created_at": "2009-07-26T08:30:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42479",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:23" align="right">comment:23</div>

I'm reopening this ticket until #6626 is fixed.



---

archive/issue_comments_042480.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">comment:24</div>\n\nAre there any code or doctests to test the functionalities provided by this package? Any package that is merged in the standard package repository must be doctested by code in the Sage library.",
    "created_at": "2009-07-31T10:22:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42480",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:24" align="right">comment:24</div>

Are there any code or doctests to test the functionalities provided by this package? Any package that is merged in the standard package repository must be doctested by code in the Sage library.



---

archive/issue_comments_042481.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">comment:25</div>\n\nThe code/doctest related to cliquer is to be found in #5793 : almost all the functions of the Graph class related to cliques use it, and if I make no mistake I documented all of them ;-)",
    "created_at": "2009-07-31T10:25:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42481",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:25" align="right">comment:25</div>

The code/doctest related to cliquer is to be found in #5793 : almost all the functions of the Graph class related to cliques use it, and if I make no mistake I documented all of them ;-)



---

archive/issue_comments_042482.json:
```json
{
    "body": "Merged: **Sage 4.1.1.rc1**",
    "created_at": "2009-07-31T23:30:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42482",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **Sage 4.1.1.rc1**



---

archive/issue_events_075917.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-31T23:30:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6355#event-75917"
}
```



---

archive/issue_comments_042483.json:
```json
{
    "body": "<div id=\"comment:26\" align=\"right\">comment:26</div>\n\nMerged in standard package repository.",
    "created_at": "2009-07-31T23:30:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6355#issuecomment-42483",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:26" align="right">comment:26</div>

Merged in standard package repository.

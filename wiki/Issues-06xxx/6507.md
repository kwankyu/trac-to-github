# Issue 6507: doc sidebar toggle

archive/issues_006507.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nSee [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/8c8fe7c5d0c0f725/4807f5553bdbd6b0#4807f5553bdbd6b0) for an early version.\n\nComponent: **documentation**\n\nAuthor: **Mitesh Patel**\n\nReviewer: **Tim Dumol**\n\nMerged: **Sage 4.1.2.alpha4**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6507_\n\n",
    "closed_at": "2009-09-23T02:43:47Z",
    "created_at": "2009-07-10T11:05:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "doc sidebar toggle",
    "type": "issue",
    "updated_at": "2009-09-27T09:44:47Z",
    "url": "https://github.com/sagemath/sage/issues/6507",
    "user": "https://github.com/qed777"
}
```
<div id="comment:0"></div>

See [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/8c8fe7c5d0c0f725/4807f5553bdbd6b0#4807f5553bdbd6b0) for an early version.

Component: **documentation**

Author: **Mitesh Patel**

Reviewer: **Tim Dumol**

Merged: **Sage 4.1.2.alpha4**

_Issue created by migration from https://trac.sagemath.org/ticket/6507_





---

archive/issue_events_078087.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-07-10T11:05:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6507#event-78087"
}
```



---

archive/issue_events_078088.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-07-10T11:05:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6507#event-78088"
}
```



---

archive/issue_events_078089.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-07-10T11:05:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6507#event-78089"
}
```



---

archive/issue_events_078090.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-07-10T11:05:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6507#event-78090"
}
```



---

archive/issue_comments_044783.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_6507_sidebar_toggle.patch.gz](https://github.com/sagemath/sage/files/ticket6507/trac_6507_sidebar_toggle.patch.gz)**\n\nThe new toggle highlights itself on mouse-over, runs the length of the content, auto-resizes itself, uses cookies, and should work in the live, static, and offline documentation in most browsers.  This includes Firefox, Opera, and the Qt WebKit demo browser on Linux.  Chromium, it seems, does not yet let offline pages set cookies, but more polish is on order.",
    "created_at": "2009-07-10T11:21:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6507#issuecomment-44783",
    "user": "https://github.com/qed777"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_6507_sidebar_toggle.patch.gz](https://github.com/sagemath/sage/files/ticket6507/trac_6507_sidebar_toggle.patch.gz)**

The new toggle highlights itself on mouse-over, runs the length of the content, auto-resizes itself, uses cookies, and should work in the live, static, and offline documentation in most browsers.  This includes Firefox, Opera, and the Qt WebKit demo browser on Linux.  Chromium, it seems, does not yet let offline pages set cookies, but more polish is on order.



---

archive/issue_events_078091.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-07-27T09:16:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6507#event-78091"
}
```



---

archive/issue_comments_044784.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI applied the patch on r12658 (Sage 4.1). There do not appear to be any differences in the UI.",
    "created_at": "2009-07-27T09:16:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6507#issuecomment-44784",
    "user": "https://github.com/TimDumol"
}
```

<div id="comment:2" align="right">comment:2</div>

I applied the patch on r12658 (Sage 4.1). There do not appear to be any differences in the UI.



---

archive/issue_comments_044785.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nCan you be more specific, e.g., about the browser and OS?  Did you rebuild the HTML documentation?\n\nI should add that I don't have access to a machine running Mac OS.  It would be useful to know how it fares in multiple Mac OS browsers in all three scenarios (live, fast static, offline).\n\nBut first, we should try to get it working on some machine other than mine.\n\n(In applying the patch, I got this warning:\n\n```\nHunk #1 succeeded at 9 with fuzz 1 (offset -1 lines).\n```\nThis is just a consequence of #6512.)",
    "created_at": "2009-07-27T09:57:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6507#issuecomment-44785",
    "user": "https://github.com/qed777"
}
```

<div id="comment:3" align="right">comment:3</div>

Can you be more specific, e.g., about the browser and OS?  Did you rebuild the HTML documentation?

I should add that I don't have access to a machine running Mac OS.  It would be useful to know how it fares in multiple Mac OS browsers in all three scenarios (live, fast static, offline).

But first, we should try to get it working on some machine other than mine.

(In applying the patch, I got this warning:

```
Hunk #1 succeeded at 9 with fuzz 1 (offset -1 lines).
```
This is just a consequence of #6512.)



---

archive/issue_events_078092.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-09-02T10:50:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6507#event-78092"
}
```



---

archive/issue_events_078093.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-09-02T10:50:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6507#event-78093"
}
```



---

archive/issue_comments_044786.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nApart from the \"fuzz\" mentioned above, the patch still appears to work for me.  I'm changing the summary to WPNR, but please let me know...",
    "created_at": "2009-09-02T10:50:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6507#issuecomment-44786",
    "user": "https://github.com/qed777"
}
```

<div id="comment:4" align="right">comment:4</div>

Apart from the "fuzz" mentioned above, the patch still appears to work for me.  I'm changing the summary to WPNR, but please let me know...



---

archive/issue_comments_044787.json:
```json
{
    "body": "Reviewer: **Tim Dumol**",
    "created_at": "2009-09-22T15:40:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6507#issuecomment-44787",
    "user": "https://github.com/TimDumol"
}
```

Reviewer: **Tim Dumol**



---

archive/issue_events_078094.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-09-22T15:40:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6507#event-78094"
}
```



---

archive/issue_events_078095.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-09-22T15:40:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6507#event-78095"
}
```



---

archive/issue_comments_044788.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nWorks as advertised after a doc rebuild. Positive review.",
    "created_at": "2009-09-22T15:40:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6507#issuecomment-44788",
    "user": "https://github.com/TimDumol"
}
```

<div id="comment:5" align="right">comment:5</div>

Works as advertised after a doc rebuild. Positive review.



---

archive/issue_events_078096.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-23T02:43:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "milestone_number": null,
    "milestone_title": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6507#event-78096"
}
```



---

archive/issue_comments_044789.json:
```json
{
    "body": "Merged: **Sage 4.1.2.alpha3**",
    "created_at": "2009-09-23T02:43:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6507#issuecomment-44789",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **Sage 4.1.2.alpha3**



---

archive/issue_events_078097.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-23T02:43:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6507#event-78097"
}
```



---

archive/issue_events_078098.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-23T02:43:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6507#event-78098"
}
```



---

archive/issue_comments_044790.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nThere is no 4.1.2.alpha3. Sage 4.1.2.alpha3 was William Stein's release for working on making the notebook a standalone package.",
    "created_at": "2009-09-27T09:44:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6507#issuecomment-44790",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:7" align="right">comment:7</div>

There is no 4.1.2.alpha3. Sage 4.1.2.alpha3 was William Stein's release for working on making the notebook a standalone package.



---

archive/issue_comments_044791.json:
```json
{
    "body": "Changed merged from **Sage 4.1.2.alpha3** to **Sage 4.1.2.alpha4**",
    "created_at": "2009-09-27T09:44:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6507#issuecomment-44791",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed merged from **Sage 4.1.2.alpha3** to **Sage 4.1.2.alpha4**

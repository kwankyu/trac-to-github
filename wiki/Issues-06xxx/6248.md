# Issue 6248: remove executable bits from sage-README-osx.txt

archive/issues_006248.json:
```json
{
    "body": "The file sage-README-osx.txt in SAGE_ROOT has mode 755, which is silly for a text file. Someone in charge of the \"official\" tarball should run a \"chmod 644\" on that file so it's not executable.\n\n**Author:** Craig Citro\n\n**Merged:** 4.0.2.rc3\n\nIssue created by migration from https://trac.sagemath.org/ticket/6248\n\n",
    "closed_at": "2009-06-18T10:32:06Z",
    "created_at": "2009-06-08T14:41:05Z",
    "labels": [
        "component: distribution",
        "trivial"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0.2",
    "title": "remove executable bits from sage-README-osx.txt",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/6248",
    "user": "https://github.com/dandrake"
}
```
The file sage-README-osx.txt in SAGE_ROOT has mode 755, which is silly for a text file. Someone in charge of the "official" tarball should run a "chmod 644" on that file so it's not executable.

**Author:** Craig Citro

**Merged:** 4.0.2.rc3

Issue created by migration from https://trac.sagemath.org/ticket/6248





---

archive/issue_comments_042917.json:
```json
{
    "body": "**Merged:** 4.0.2.rc3",
    "created_at": "2009-06-18T10:32:06Z",
    "issue": "https://github.com/sagemath/sage/issues/6248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6248#issuecomment-42917",
    "user": "https://github.com/craigcitro"
}
```

**Merged:** 4.0.2.rc3



---

archive/issue_comments_042918.json:
```json
{
    "body": "<a id='comment:1'></a>\nDone.",
    "created_at": "2009-06-18T10:32:06Z",
    "issue": "https://github.com/sagemath/sage/issues/6248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6248#issuecomment-42918",
    "user": "https://github.com/craigcitro"
}
```

<a id='comment:1'></a>
Done.



---

archive/issue_comments_042919.json:
```json
{
    "body": "**Author:** Craig Citro",
    "created_at": "2009-06-18T10:32:06Z",
    "issue": "https://github.com/sagemath/sage/issues/6248",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6248#issuecomment-42919",
    "user": "https://github.com/craigcitro"
}
```

**Author:** Craig Citro



---

archive/issue_events_040683.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-06-18T10:32:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6248",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6248#event-40683"
}
```



---

archive/issue_events_040684.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-06-19T13:16:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6248",
    "milestone": "sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6248#event-40684"
}
```



---

archive/issue_events_040685.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-06-19T13:16:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6248",
    "milestone": "sage-4.0.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6248#event-40685"
}
```

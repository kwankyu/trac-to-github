# Issue 6402: Fix bugs + improve documentation for overconvergent modular forms

archive/issues_006402.json:
```json
{
    "assignees": [
        "https://github.com/craigcitro"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis patch fixes lots of bugs + adds a substantial amount of documentation and examples, based on my talk at Sage Days 16.\n\nCC:  @williamstein\n\nComponent: **modular forms**\n\nAuthor: **David Loeffler**\n\nReviewer: **Alex Ghitza, Minh Van Nguyen**\n\nMerged: **Sage 4.1.2.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6402_\n\n",
    "closed_at": "2009-08-24T12:34:57Z",
    "created_at": "2009-06-25T12:11:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix bugs + improve documentation for overconvergent modular forms",
    "type": "issue",
    "updated_at": "2009-08-24T12:36:33Z",
    "url": "https://github.com/sagemath/sage/issues/6402",
    "user": "https://github.com/loefflerd"
}
```
<div id="comment:0"></div>

This patch fixes lots of bugs + adds a substantial amount of documentation and examples, based on my talk at Sage Days 16.

CC:  @williamstein

Component: **modular forms**

Author: **David Loeffler**

Reviewer: **Alex Ghitza, Minh Van Nguyen**

Merged: **Sage 4.1.2.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/6402_





---

archive/issue_events_076567.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-06-25T12:11:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
    "label_color": "0000ff",
    "label_name": "c: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6402#event-76567"
}
```



---

archive/issue_events_076568.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-06-25T12:11:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6402#event-76568"
}
```



---

archive/issue_events_076569.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-06-25T12:11:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6402#event-76569"
}
```



---

archive/issue_events_076570.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-06-25T12:11:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "label": "https://github.com/sagemath/sage/labels/feature",
    "label_color": "008080",
    "label_name": "feature",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6402#event-76570"
}
```



---

archive/issue_events_076571.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-06-25T12:11:59Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "subject": "https://github.com/loefflerd",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6402#event-76571"
}
```



---

archive/issue_comments_043113.json:
```json
{
    "body": "patch against 4.0.2",
    "created_at": "2009-06-25T12:13:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6402#issuecomment-43113",
    "user": "https://github.com/loefflerd"
}
```

patch against 4.0.2



---

archive/issue_comments_043114.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_6402-overconvergent_fixes.patch.gz](https://github.com/sagemath/sage/files/ticket6402/trac_6402-overconvergent_fixes.patch.gz)**\n\nHere's a patch.",
    "created_at": "2009-06-25T12:15:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6402#issuecomment-43114",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_6402-overconvergent_fixes.patch.gz](https://github.com/sagemath/sage/files/ticket6402/trac_6402-overconvergent_fixes.patch.gz)**

Here's a patch.



---

archive/issue_events_076572.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-06-25T16:56:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6402#event-76572"
}
```



---

archive/issue_events_076573.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-14T21:51:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "milestone_number": null,
    "milestone_title": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6402#event-76573"
}
```



---

archive/issue_events_076574.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-14T21:51:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "label": "https://github.com/sagemath/sage/labels/feature",
    "label_color": "008080",
    "label_name": "feature",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6402#event-76574"
}
```



---

archive/issue_comments_043115.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nLooks good, and the added documentation is really great.\n\nI'm attaching a tiny referee patch fixing a few typos, and if you are ok with it we can give this a positive review.\n\nThere is one small issue, which I leave up to you whether you want to fix it or not: in the method `valuation_plot`, you added an optional argument `rmax`, but the docstring says nothing about its role.",
    "created_at": "2009-08-19T09:44:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6402#issuecomment-43115",
    "user": "https://github.com/aghitza"
}
```

<div id="comment:4" align="right">comment:4</div>

Looks good, and the added documentation is really great.

I'm attaching a tiny referee patch fixing a few typos, and if you are ok with it we can give this a positive review.

There is one small issue, which I leave up to you whether you want to fix it or not: in the method `valuation_plot`, you added an optional argument `rmax`, but the docstring says nothing about its role.



---

archive/issue_comments_043116.json:
```json
{
    "body": "apply after the previous patch",
    "created_at": "2009-08-19T09:45:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6402#issuecomment-43116",
    "user": "https://github.com/aghitza"
}
```

apply after the previous patch



---

archive/issue_comments_043117.json:
```json
{
    "body": "Author: **David Loeffler**",
    "created_at": "2009-08-19T09:52:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6402#issuecomment-43117",
    "user": "https://github.com/loefflerd"
}
```

Author: **David Loeffler**



---

archive/issue_comments_043118.json:
```json
{
    "body": "Reviewer: **Alex Ghitza**",
    "created_at": "2009-08-19T09:52:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6402#issuecomment-43118",
    "user": "https://github.com/loefflerd"
}
```

Reviewer: **Alex Ghitza**



---

archive/issue_comments_043119.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nAttachment: **[trac_6402-referee.patch.gz](https://github.com/sagemath/sage/files/ticket6402/trac_6402-referee.patch.gz)**\n\nFine by me.",
    "created_at": "2009-08-19T09:52:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6402#issuecomment-43119",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:5" align="right">comment:5</div>

Attachment: **[trac_6402-referee.patch.gz](https://github.com/sagemath/sage/files/ticket6402/trac_6402-referee.patch.gz)**

Fine by me.



---

archive/issue_events_076575.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-08-19T09:52:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6402#event-76575"
}
```



---

archive/issue_events_076576.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-08-19T09:52:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6402#event-76576"
}
```



---

archive/issue_comments_043120.json:
```json
{
    "body": "reviewer patch: more typo fixes",
    "created_at": "2009-08-23T13:27:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6402#issuecomment-43120",
    "user": "https://github.com/sagetrac-mvngu"
}
```

reviewer patch: more typo fixes



---

archive/issue_events_076577.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-08-23T13:29:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6402#event-76577"
}
```



---

archive/issue_events_076578.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-08-23T13:29:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6402#event-76578"
}
```



---

archive/issue_comments_043121.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nAttachment: **[trac_6402-typos.patch.gz](https://github.com/sagemath/sage/files/ticket6402/trac_6402-typos.patch.gz)**\n\nThe reviewer patch `trac_6402-typos.patch` fixes some typos left over from the previous two patches. If that is OK, then the whole ticket can be merged.",
    "created_at": "2009-08-23T13:29:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6402#issuecomment-43121",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:6" align="right">comment:6</div>

Attachment: **[trac_6402-typos.patch.gz](https://github.com/sagemath/sage/files/ticket6402/trac_6402-typos.patch.gz)**

The reviewer patch `trac_6402-typos.patch` fixes some typos left over from the previous two patches. If that is OK, then the whole ticket can be merged.



---

archive/issue_comments_043122.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nI certainly have no problems with Minh's suggestions. Can I suggest we get this in soon, rather than fixing every conceivable micro-bug, since the initial patch fixes some rather major and embarassing bugs that render the whole module basically unusable?",
    "created_at": "2009-08-24T08:30:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6402#issuecomment-43122",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:7" align="right">comment:7</div>

I certainly have no problems with Minh's suggestions. Can I suggest we get this in soon, rather than fixing every conceivable micro-bug, since the initial patch fixes some rather major and embarassing bugs that render the whole module basically unusable?



---

archive/issue_events_076579.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-08-24T08:30:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6402#event-76579"
}
```



---

archive/issue_events_076580.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-08-24T08:30:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6402#event-76580"
}
```



---

archive/issue_comments_043123.json:
```json
{
    "body": "Merged: **Sage 4.1.2.alpha0**",
    "created_at": "2009-08-24T12:34:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6402#issuecomment-43123",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **Sage 4.1.2.alpha0**



---

archive/issue_comments_043124.json:
```json
{
    "body": "Changed reviewer from **Alex Ghitza** to **Alex Ghitza, Minh Van Nguyen**",
    "created_at": "2009-08-24T12:34:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6402#issuecomment-43124",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed reviewer from **Alex Ghitza** to **Alex Ghitza, Minh Van Nguyen**



---

archive/issue_events_076581.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-08-24T12:34:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6402#event-76581"
}
```



---

archive/issue_events_076582.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-08-24T12:34:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6402#event-76582"
}
```



---

archive/issue_comments_043125.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nMerged all three patches in this order:\n\n1. `trac_6402-overconvergent_fixes.patch`\n2. `trac_6402-referee.patch`\n3. `trac_6402-typos.patch`",
    "created_at": "2009-08-24T12:36:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6402#issuecomment-43125",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:9" align="right">comment:9</div>

Merged all three patches in this order:

1. `trac_6402-overconvergent_fixes.patch`
2. `trac_6402-referee.patch`
3. `trac_6402-typos.patch`

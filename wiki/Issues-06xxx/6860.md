# Issue 6860: dimensions of modular forms spaces for Gamma(N) is slow

archive/issues_006860.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\n\n```\nsage: dimension_cusp_forms(Gamma(11))\n[hangs forever]\n```\n\nThis has now been **fixed** as a consequence of #10546. Release manager: please close as fixed.\n\nComponent: **modular forms**\n\nKeywords: **sd51**\n\nReviewer: **David Loeffler, Alex Ghitza**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6860_\n\n",
    "closed_at": "2013-08-13T08:46:45Z",
    "created_at": "2009-09-02T02:22:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "dimensions of modular forms spaces for Gamma(N) is slow",
    "type": "issue",
    "updated_at": "2013-08-13T08:46:45Z",
    "url": "https://github.com/sagemath/sage/issues/6860",
    "user": "https://github.com/williamstein"
}
```
<div id="comment:0"></div>


```
sage: dimension_cusp_forms(Gamma(11))
[hangs forever]
```

This has now been **fixed** as a consequence of #10546. Release manager: please close as fixed.

Component: **modular forms**

Keywords: **sd51**

Reviewer: **David Loeffler, Alex Ghitza**

_Issue created by migration from https://trac.sagemath.org/ticket/6860_





---

archive/issue_events_082640.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-02T02:22:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82640"
}
```



---

archive/issue_events_082641.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-02T02:22:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "c: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82641"
}
```



---

archive/issue_events_082642.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-02T02:22:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82642"
}
```



---

archive/issue_events_082643.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-02T02:22:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82643"
}
```



---

archive/issue_events_082644.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-02T02:22:05Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82644"
}
```



---

archive/issue_events_082645.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-02T13:05:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82645"
}
```



---

archive/issue_events_082646.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-02T13:05:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82646"
}
```



---

archive/issue_comments_048652.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nWait, I'm just impatient, since:\n\n```\nsage: dimension_cusp_forms(Gamma(11))\n26\n```\n\nSo I'm changing this to an enhancement, since I think this should be fast.",
    "created_at": "2009-09-02T13:05:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6860#issuecomment-48652",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>

Wait, I'm just impatient, since:

```
sage: dimension_cusp_forms(Gamma(11))
26
```

So I'm changing this to an enhancement, since I think this should be fast.



---

archive/issue_events_082647.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-02T13:05:35Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "title_is": "dimensions of modular forms spaces for Gamma(N) is slow",
    "title_was": "dimensions of modular forms spaces for Gamma(N) is busted / not implemented",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82647"
}
```



---

archive/issue_events_082648.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2013-07-22T15:22:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "c: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82648"
}
```



---

archive/issue_events_082649.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2013-07-22T15:22:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
    "label_color": "0000ff",
    "label_name": "c: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82649"
}
```



---

archive/issue_events_082650.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2013-07-22T15:22:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82650"
}
```



---

archive/issue_comments_048653.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis has now been fastified:\n\n```python\nsage: %time dimension_cusp_forms(Gamma(10^28+731))                             \nCPU times: user 0.04 s, sys: 0.02 s, total: 0.06 s\nWall time: 1.54 s\n36274370885528165103530592700784546931709800073501194539717685491542679896172544001\n```\n(and most of that is the time taken to factor the level). I propose closing this as fixed.",
    "created_at": "2013-07-22T15:22:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6860#issuecomment-48653",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:2" align="right">comment:2</div>

This has now been fastified:

```python
sage: %time dimension_cusp_forms(Gamma(10^28+731))                             
CPU times: user 0.04 s, sys: 0.02 s, total: 0.06 s
Wall time: 1.54 s
36274370885528165103530592700784546931709800073501194539717685491542679896172544001
```
(and most of that is the time taken to factor the level). I propose closing this as fixed.



---

archive/issue_events_082651.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2013-07-22T15:22:54Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82651"
}
```



---

archive/issue_comments_048654.json:
```json
{
    "body": "Author: **David Loeffler**",
    "created_at": "2013-07-22T16:41:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6860#issuecomment-48654",
    "user": "https://github.com/aghitza"
}
```

Author: **David Loeffler**



---

archive/issue_events_082652.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2013-07-22T16:41:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82652"
}
```



---

archive/issue_events_082653.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2013-07-22T16:41:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82653"
}
```



---

archive/issue_comments_048655.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAgreed.",
    "created_at": "2013-07-22T16:41:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6860#issuecomment-48655",
    "user": "https://github.com/aghitza"
}
```

<div id="comment:3" align="right">comment:3</div>

Agreed.



---

archive/issue_events_082654.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2013-07-22T16:41:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82654"
}
```



---

archive/issue_events_082655.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2013-07-22T16:41:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82655"
}
```



---

archive/issue_comments_048656.json:
```json
{
    "body": "Reviewer: **Alex Ghitza**",
    "created_at": "2013-07-22T16:41:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6860#issuecomment-48656",
    "user": "https://github.com/aghitza"
}
```

Reviewer: **Alex Ghitza**



---

archive/issue_comments_048657.json:
```json
{
    "body": "Changed keywords from none to **sd51**",
    "created_at": "2013-07-22T16:46:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6860#issuecomment-48657",
    "user": "https://github.com/aghitza"
}
```

Changed keywords from none to **sd51**



---

archive/issue_comments_048658.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,3 +3,5 @@\n sage: dimension_cusp_forms(Gamma(11))\n [hangs forever]\n ```\n+\n+This has now been **fixed** as a consequence of #10546. Release manager: please close as fixed.\n``````\n",
    "created_at": "2013-07-22T17:11:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6860#issuecomment-48658",
    "user": "https://github.com/loefflerd"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,3 +3,5 @@
 sage: dimension_cusp_forms(Gamma(11))
 [hangs forever]
 ```
+
+This has now been **fixed** as a consequence of #10546. Release manager: please close as fixed.
``````




---

archive/issue_comments_048659.json:
```json
{
    "body": "Changed reviewer from **Alex Ghitza** to **David Loeffler, Alex Ghitza**",
    "created_at": "2013-08-09T11:19:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6860#issuecomment-48659",
    "user": "https://github.com/jdemeyer"
}
```

Changed reviewer from **Alex Ghitza** to **David Loeffler, Alex Ghitza**



---

archive/issue_comments_048660.json:
```json
{
    "body": "Changed author from **David Loeffler** to none",
    "created_at": "2013-08-09T11:19:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6860#issuecomment-48660",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **David Loeffler** to none



---

archive/issue_events_082656.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T08:46:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82656"
}
```



---

archive/issue_events_082657.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T08:46:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82657"
}
```



---

archive/issue_events_082658.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T08:46:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82658"
}
```



---

archive/issue_events_082659.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T08:46:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6860",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "c6c6c6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6860#event-82659"
}
```

# Issue 6188: Add more files in sage/rings/number_field to reference manual

archive/issues_006188.json:
```json
{
    "assignees": [
        "https://github.com/loefflerd"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis patch adds the files `order.py`, `number_field_element_quadratic.pyx`, `number_field_rel.py`, `number_field_ideal_rel.py`, and `unit_group.py` to the reference manual, and makes the necessary ReST formatting fixes to get them to build correctly.\n\n\nComponent: **number theory**\n\nAuthor: **David Loeffler**\n\nReviewer: **John Cremona**\n\nMerged: **4.0.2.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6188_\n\n",
    "closed_at": "2009-06-13T19:47:35Z",
    "created_at": "2009-06-02T17:22:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20documentation"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add more files in sage/rings/number_field to reference manual",
    "type": "issue",
    "updated_at": "2009-06-13T19:47:35Z",
    "url": "https://github.com/sagemath/sage/issues/6188",
    "user": "https://github.com/loefflerd"
}
```
<div id="comment:0"></div>

This patch adds the files `order.py`, `number_field_element_quadratic.pyx`, `number_field_rel.py`, `number_field_ideal_rel.py`, and `unit_group.py` to the reference manual, and makes the necessary ReST formatting fixes to get them to build correctly.


Component: **number theory**

Author: **David Loeffler**

Reviewer: **John Cremona**

Merged: **4.0.2.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/6188_





---

archive/issue_events_073725.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-06-02T17:22:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "milestone_number": null,
    "milestone_title": "sage-4.0.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6188#event-73725"
}
```



---

archive/issue_events_073726.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-06-02T17:22:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "c: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6188#event-73726"
}
```



---

archive/issue_events_073727.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-06-02T17:22:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6188#event-73727"
}
```



---

archive/issue_events_073728.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-06-02T17:22:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6188#event-73728"
}
```



---

archive/issue_events_073729.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-06-02T17:22:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6188#event-73729"
}
```



---

archive/issue_events_073730.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-02T17:22:41Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "subject": "https://github.com/loefflerd",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6188#event-73730"
}
```



---

archive/issue_events_073731.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-06-02T17:22:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6188#event-73731"
}
```



---

archive/issue_comments_040955.json:
```json
{
    "body": "patch against 4.0.1.alpha0",
    "created_at": "2009-06-02T17:24:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6188#issuecomment-40955",
    "user": "https://github.com/loefflerd"
}
```

patch against 4.0.1.alpha0



---

archive/issue_comments_040956.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_6188.patch.gz](https://github.com/sagemath/sage/files/ticket6188/trac_6188.patch.gz)**",
    "created_at": "2009-06-02T17:25:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6188#issuecomment-40956",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_6188.patch.gz](https://github.com/sagemath/sage/files/ticket6188/trac_6188.patch.gz)**



---

archive/issue_events_073732.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-02T17:25:51Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "subject": "https://github.com/loefflerd",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6188#event-73732"
}
```



---

archive/issue_events_073733.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-06-02T17:25:51Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "subject": "https://github.com/loefflerd",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6188#event-73733"
}
```



---

archive/issue_comments_040957.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,2 @@\n-This patch adds the files {{{\n-order.py}}}, `number_field_element_quadratic.pyx`, `number_field_rel.py`, `number_field_ideal_rel.py`, and `unit_group.py` to the reference manual, and makes the necessary ReST formatting fixes to get them to build correctly.\n+This patch adds the files `order.py`, `number_field_element_quadratic.pyx`, `number_field_rel.py`, `number_field_ideal_rel.py`, and `unit_group.py` to the reference manual, and makes the necessary ReST formatting fixes to get them to build correctly.\n \n``````\n",
    "created_at": "2009-06-02T17:25:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6188#issuecomment-40957",
    "user": "https://github.com/loefflerd"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,2 @@
-This patch adds the files {{{
-order.py}}}, `number_field_element_quadratic.pyx`, `number_field_rel.py`, `number_field_ideal_rel.py`, and `unit_group.py` to the reference manual, and makes the necessary ReST formatting fixes to get them to build correctly.
+This patch adds the files `order.py`, `number_field_element_quadratic.pyx`, `number_field_rel.py`, `number_field_ideal_rel.py`, and `unit_group.py` to the reference manual, and makes the necessary ReST formatting fixes to get them to build correctly.
 
``````




---

archive/issue_comments_040958.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI am half way through reviewing this and should be able to finish tomorrow.",
    "created_at": "2009-06-03T21:29:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6188#issuecomment-40958",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:2" align="right">comment:2</div>

I am half way through reviewing this and should be able to finish tomorrow.



---

archive/issue_comments_040959.json:
```json
{
    "body": "Attachment: **[trac_6188_review.patch.gz](https://github.com/sagemath/sage/files/ticket6188/trac_6188_review.patch.gz)**\n\nApply after previous",
    "created_at": "2009-06-04T21:41:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6188#issuecomment-40959",
    "user": "https://github.com/JohnCremona"
}
```

Attachment: **[trac_6188_review.patch.gz](https://github.com/sagemath/sage/files/ticket6188/trac_6188_review.patch.gz)**

Apply after previous



---

archive/issue_events_073734.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-06-04T21:46:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6188#event-73734"
}
```



---

archive/issue_comments_040960.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI reviewed David's patch by rebuilding the reference manual and looking through the relavant sections.  I found quite a few more things needing tidying up (several in functions I wrote, so my fault), hence the second patch nearly half as large as the first.\n\nI'm happy to OK David's contribution, but someone else (David?) should run mine.  It's all docstring changes, but I did check that all tests in sage/rings/number_field still pass.",
    "created_at": "2009-06-04T21:46:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6188#issuecomment-40960",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:3" align="right">comment:3</div>

I reviewed David's patch by rebuilding the reference manual and looking through the relavant sections.  I found quite a few more things needing tidying up (several in functions I wrote, so my fault), hence the second patch nearly half as large as the first.

I'm happy to OK David's contribution, but someone else (David?) should run mine.  It's all docstring changes, but I did check that all tests in sage/rings/number_field still pass.



---

archive/issue_events_073735.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-06-05T08:28:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6188#event-73735"
}
```



---

archive/issue_events_073736.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-06-05T08:28:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6188#event-73736"
}
```



---

archive/issue_comments_040961.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nLooks fine to me.",
    "created_at": "2009-06-05T08:28:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6188#issuecomment-40961",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:4" align="right">comment:4</div>

Looks fine to me.



---

archive/issue_events_073737.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-13T19:47:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6188#event-73737"
}
```



---

archive/issue_events_073738.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-13T19:47:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6188#event-73738"
}
```



---

archive/issue_comments_040962.json:
```json
{
    "body": "Merged: **4.0.2.alpha0**",
    "created_at": "2009-06-13T19:47:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6188#issuecomment-40962",
    "user": "https://github.com/ncalexan"
}
```

Merged: **4.0.2.alpha0**



---

archive/issue_comments_040963.json:
```json
{
    "body": "Author: **David Loeffler**",
    "created_at": "2009-06-13T19:47:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6188#issuecomment-40963",
    "user": "https://github.com/ncalexan"
}
```

Author: **David Loeffler**



---

archive/issue_comments_040964.json:
```json
{
    "body": "Reviewer: **John Cremona**",
    "created_at": "2009-06-13T19:47:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6188#issuecomment-40964",
    "user": "https://github.com/ncalexan"
}
```

Reviewer: **John Cremona**

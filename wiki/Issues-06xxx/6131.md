# Issue 6131: cmp for number field elements

archive/issues_006131.json:
```json
{
    "body": "See discussion at http://groups.google.com/group/sage-nt/browse_thread/thread/422606e40805d5d0?hl=en\n\nNote that `cmp(list(a), list(b))` can be slow...\n\nIssue created by migration from https://trac.sagemath.org/ticket/6131\n\n",
    "closed_at": "2009-05-26T20:06:28Z",
    "created_at": "2009-05-26T18:12:50Z",
    "labels": [
        "component: basic arithmetic",
        "bug",
        "duplicate"
    ],
    "title": "cmp for number field elements",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/6131",
    "user": "https://github.com/robertwb"
}
```
See discussion at http://groups.google.com/group/sage-nt/browse_thread/thread/422606e40805d5d0?hl=en

Note that `cmp(list(a), list(b))` can be slow...

Issue created by migration from https://trac.sagemath.org/ticket/6131





---

archive/issue_events_039961.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-26T20:06:28Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6131",
    "milestone": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6131#event-39961"
}
```



---

archive/issue_events_039962.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-26T20:06:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6131",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6131#event-39962"
}
```



---

archive/issue_comments_041881.json:
```json
{
    "body": "<a id='comment:1'></a>\nDuplicate of #6132",
    "created_at": "2009-05-26T20:06:28Z",
    "issue": "https://github.com/sagemath/sage/issues/6131",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6131#issuecomment-41881",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Duplicate of #6132



---

archive/issue_events_039963.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-26T20:06:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6131",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6131#event-39963"
}
```

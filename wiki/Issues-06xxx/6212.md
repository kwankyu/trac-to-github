# Issue 6212: make sage -upgrade check that there are no applied hg queue patches

archive/issues_006212.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nIt would be nice if sage -upgrade aborted if are applied hg queue patches, in the same way it aborts if there are local changes.\n\nCC:  @jasongrout @sagetrac-jthurber\n\nComponent: **distribution**\n\nKeywords: **sage upgrade applied hg queue**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6212_\n\n",
    "closed_at": "2014-03-19T04:35:53Z",
    "created_at": "2009-06-04T20:40:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20distribution",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "make sage -upgrade check that there are no applied hg queue patches",
    "type": "issue",
    "updated_at": "2014-03-19T04:35:53Z",
    "url": "https://github.com/sagemath/sage/issues/6212",
    "user": "https://github.com/ncalexan"
}
```
<div id="comment:0"></div>

It would be nice if sage -upgrade aborted if are applied hg queue patches, in the same way it aborts if there are local changes.

CC:  @jasongrout @sagetrac-jthurber

Component: **distribution**

Keywords: **sage upgrade applied hg queue**

_Issue created by migration from https://trac.sagemath.org/ticket/6212_





---

archive/issue_events_074028.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-04T20:40:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-74028"
}
```



---

archive/issue_events_074029.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-04T20:40:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20distribution",
    "label_color": "0000b0",
    "label_name": "c: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-74029"
}
```



---

archive/issue_events_074030.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-04T20:40:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-74030"
}
```



---

archive/issue_events_074031.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-04T20:40:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-74031"
}
```



---

archive/issue_comments_041175.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nEven better:\n\n```\n[1:38pm] jason-: you have patches that are applied using mercurial queues.  Do you want to pop all of the patches? [y/n]\n[1:38pm] jason-: yes would pop and continue the upgrae\n[1:39pm] jason-: no would quit the upgrade\n```",
    "created_at": "2009-06-04T20:40:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6212#issuecomment-41175",
    "user": "https://github.com/ncalexan"
}
```

<div id="comment:1" align="right">comment:1</div>

Even better:

```
[1:38pm] jason-: you have patches that are applied using mercurial queues.  Do you want to pop all of the patches? [y/n]
[1:38pm] jason-: yes would pop and continue the upgrae
[1:39pm] jason-: no would quit the upgrade
```



---

archive/issue_events_074032.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-74032"
}
```



---

archive/issue_events_074033.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-74033"
}
```



---

archive/issue_events_074034.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-74034"
}
```



---

archive/issue_events_074035.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-74035"
}
```



---

archive/issue_events_074036.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-03-14T16:44:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-74036"
}
```



---

archive/issue_comments_041176.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nwe have switched to git, this is obsolete",
    "created_at": "2014-03-14T16:44:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6212#issuecomment-41176",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:5" align="right">comment:5</div>

we have switched to git, this is obsolete



---

archive/issue_events_074037.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-03-14T16:44:26Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-74037"
}
```



---

archive/issue_events_074038.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2014-03-15T13:33:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-74038"
}
```



---

archive/issue_events_074039.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2014-03-15T13:33:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-74039"
}
```



---

archive/issue_events_074040.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-19T04:35:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-74040"
}
```



---

archive/issue_events_074041.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-19T04:35:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-74041"
}
```



---

archive/issue_events_074042.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-19T04:35:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-74042"
}
```



---

archive/issue_events_074043.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-19T04:35:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "c6c6c6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-74043"
}
```

# Issue 6721: spell-check all modules under sage/media

archive/issues_006721.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nAs the subject says.\n\nComponent: **documentation**\n\nAuthor: **Minh Van Nguyen**\n\nReviewer: **Mitesh Patel**\n\nMerged: **Sage 4.1.1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6721_\n\n",
    "closed_at": "2009-08-14T07:56:51Z",
    "created_at": "2009-08-10T04:13:58Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "spell-check all modules under sage/media",
    "type": "issue",
    "updated_at": "2009-08-14T07:56:51Z",
    "url": "https://github.com/sagemath/sage/issues/6721",
    "user": "https://github.com/sagetrac-mvngu"
}
```
<div id="comment:0"></div>

As the subject says.

Component: **documentation**

Author: **Minh Van Nguyen**

Reviewer: **Mitesh Patel**

Merged: **Sage 4.1.1**

_Issue created by migration from https://trac.sagemath.org/ticket/6721_





---

archive/issue_events_080899.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-08-10T04:13:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6721",
    "milestone_number": null,
    "milestone_title": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6721#event-80899"
}
```



---

archive/issue_events_080900.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-08-10T04:13:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6721",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6721#event-80900"
}
```



---

archive/issue_events_080901.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-08-10T04:13:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6721",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6721#event-80901"
}
```



---

archive/issue_events_080902.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-08-10T04:13:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6721",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6721#event-80902"
}
```



---

archive/issue_comments_047083.json:
```json
{
    "body": "Author: **Minh Van Nguyen**",
    "created_at": "2009-08-10T04:15:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6721#issuecomment-47083",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Minh Van Nguyen**



---

archive/issue_events_080903.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-08-10T04:15:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6721",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6721#event-80903"
}
```



---

archive/issue_comments_047084.json:
```json
{
    "body": "based on Sage 4.1.1.rc2",
    "created_at": "2009-08-11T12:32:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6721#issuecomment-47084",
    "user": "https://github.com/sagetrac-mvngu"
}
```

based on Sage 4.1.1.rc2



---

archive/issue_events_080904.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-08-14T07:16:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6721",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6721#event-80904"
}
```



---

archive/issue_events_080905.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2009-08-14T07:16:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6721",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6721#event-80905"
}
```



---

archive/issue_comments_047085.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAttachment: **[trac_6721-spell-check-media.patch.gz](https://github.com/sagemath/sage/files/ticket6721/trac_6721-spell-check-media.patch.gz)**\n\nI ran `sage -merge <ticket_number> -t directory` and started the notebook without problems.",
    "created_at": "2009-08-14T07:16:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6721#issuecomment-47085",
    "user": "https://github.com/qed777"
}
```

<div id="comment:2" align="right">comment:2</div>

Attachment: **[trac_6721-spell-check-media.patch.gz](https://github.com/sagemath/sage/files/ticket6721/trac_6721-spell-check-media.patch.gz)**

I ran `sage -merge <ticket_number> -t directory` and started the notebook without problems.



---

archive/issue_comments_047086.json:
```json
{
    "body": "Reviewer: **Mitesh Patel**",
    "created_at": "2009-08-14T07:18:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6721#issuecomment-47086",
    "user": "https://github.com/qed777"
}
```

Reviewer: **Mitesh Patel**



---

archive/issue_comments_047087.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nI suppose I should run `sage -t -long -optional -randorder` later with all spelling patches applied.",
    "created_at": "2009-08-14T07:28:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6721#issuecomment-47087",
    "user": "https://github.com/qed777"
}
```

<div id="comment:4" align="right">comment:4</div>

I suppose I should run `sage -t -long -optional -randorder` later with all spelling patches applied.



---

archive/issue_comments_047088.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nI also ran `sage -docbuild reference html --jsmath` without Sphinx warnings or errors.",
    "created_at": "2009-08-14T07:35:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6721#issuecomment-47088",
    "user": "https://github.com/qed777"
}
```

<div id="comment:5" align="right">comment:5</div>

I also ran `sage -docbuild reference html --jsmath` without Sphinx warnings or errors.



---

archive/issue_events_080906.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-08-14T07:56:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6721",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6721#event-80906"
}
```



---

archive/issue_events_080907.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-08-14T07:56:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6721",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6721#event-80907"
}
```



---

archive/issue_comments_047089.json:
```json
{
    "body": "Merged: **Sage 4.1.1**",
    "created_at": "2009-08-14T07:56:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6721#issuecomment-47089",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **Sage 4.1.1**

# Issue 6738: typo in spkg-install for atlas spkg

archive/issues_006738.json:
```json
{
    "body": "I just noticed this in an error message:\n\n```\nBuilding shared ATLAS libraris failed\n```\n\nSee the typo in the word \"libraris\".\n\n**Assignee:** mabshoff\n\n**Author:** Minh Van Nguyen\n\n**Reviewer:** William Stein\n\n**Merged:** Sage 4.1.1\n\nIssue created by migration from https://trac.sagemath.org/ticket/6738\n\n",
    "closed_at": "2009-08-12T00:59:35Z",
    "created_at": "2009-08-11T22:45:21Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.1",
    "title": "typo in spkg-install for atlas spkg",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/6738",
    "user": "https://github.com/williamstein"
}
```
I just noticed this in an error message:

```
Building shared ATLAS libraris failed
```

See the typo in the word "libraris".

**Assignee:** mabshoff

**Author:** Minh Van Nguyen

**Reviewer:** William Stein

**Merged:** Sage 4.1.1

Issue created by migration from https://trac.sagemath.org/ticket/6738





---

archive/issue_events_044815.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-08-11T23:34:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6738",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6738#event-44815"
}
```



---

archive/issue_comments_048736.json:
```json
{
    "body": "**Author:** Minh Van Nguyen",
    "created_at": "2009-08-11T23:34:11Z",
    "issue": "https://github.com/sagemath/sage/issues/6738",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6738#issuecomment-48736",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Author:** Minh Van Nguyen



---

archive/issue_comments_048737.json:
```json
{
    "body": "<a id='comment:1'></a>\nThe typo lies in the file `spkg-install-script` of the ATLAS spkg. An updated spkg that fixes the reported typo is up at:\n\nhttp://sage.math.washington.edu/home/mvngu/patch/atlas-3.8.3.p7.spkg",
    "created_at": "2009-08-11T23:34:11Z",
    "issue": "https://github.com/sagemath/sage/issues/6738",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6738#issuecomment-48737",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:1'></a>
The typo lies in the file `spkg-install-script` of the ATLAS spkg. An updated spkg that fixes the reported typo is up at:

http://sage.math.washington.edu/home/mvngu/patch/atlas-3.8.3.p7.spkg



---

archive/issue_events_044816.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-08-12T00:02:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6738",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6738#event-44816"
}
```



---

archive/issue_events_044817.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-08-12T00:02:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6738",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6738#event-44817"
}
```



---

archive/issue_events_044818.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-08-12T00:59:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6738",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6738#event-44818"
}
```



---

archive/issue_events_044819.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-08-12T00:59:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6738",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6738#event-44819"
}
```



---

archive/issue_comments_048738.json:
```json
{
    "body": "**Merged:** Sage 4.1.1",
    "created_at": "2009-08-12T00:59:35Z",
    "issue": "https://github.com/sagemath/sage/issues/6738",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6738#issuecomment-48738",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Merged:** Sage 4.1.1



---

archive/issue_comments_048739.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in the standard spkg repository.",
    "created_at": "2009-08-12T00:59:35Z",
    "issue": "https://github.com/sagemath/sage/issues/6738",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6738#issuecomment-48739",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:3'></a>
Merged in the standard spkg repository.



---

archive/issue_comments_048740.json:
```json
{
    "body": "**Reviewer:** William Stein",
    "created_at": "2009-08-12T00:59:35Z",
    "issue": "https://github.com/sagemath/sage/issues/6738",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6738#issuecomment-48740",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Reviewer:** William Stein

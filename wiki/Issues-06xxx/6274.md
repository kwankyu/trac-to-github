# Issue 6274: fix formatting of files under sage/plot

archive/issues_006274.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nThis is a follow-up to #6270. Here, we expose more 2-D plotting functions in the reference manual. Patches should be applied in this order:\n1. the patch at #6257\n2. the patch at #6270\n3. and finally the patch on this ticket.\n\nCC:  @kcrisman @jhpalmieri\n\nComponent: **documentation**\n\nKeywords: **plot**\n\nAuthor: **Minh Van Nguyen**\n\nReviewer: **John Palmieri**\n\nMerged: **sage-4.1.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6274_\n\n",
    "closed_at": "2009-06-24T10:11:04Z",
    "created_at": "2009-06-13T12:23:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/doctest%20coverage"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix formatting of files under sage/plot",
    "type": "issue",
    "updated_at": "2009-06-24T10:11:04Z",
    "url": "https://github.com/sagemath/sage/issues/6274",
    "user": "https://github.com/sagetrac-mvngu"
}
```
<div id="comment:0"></div>

This is a follow-up to #6270. Here, we expose more 2-D plotting functions in the reference manual. Patches should be applied in this order:
1. the patch at #6257
2. the patch at #6270
3. and finally the patch on this ticket.

CC:  @kcrisman @jhpalmieri

Component: **documentation**

Keywords: **plot**

Author: **Minh Van Nguyen**

Reviewer: **John Palmieri**

Merged: **sage-4.1.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/6274_





---

archive/issue_events_074747.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-13T12:23:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-74747"
}
```



---

archive/issue_events_074748.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-13T12:23:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-74748"
}
```



---

archive/issue_events_074749.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-13T12:23:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-74749"
}
```



---

archive/issue_comments_041703.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_6274.patch.gz](https://github.com/sagemath/sage/files/ticket6274/trac_6274.patch.gz)**",
    "created_at": "2009-06-13T15:27:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6274#issuecomment-41703",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_6274.patch.gz](https://github.com/sagemath/sage/files/ticket6274/trac_6274.patch.gz)**



---

archive/issue_comments_041704.json:
```json
{
    "body": "Changed keywords from none to **plot**",
    "created_at": "2009-06-13T15:27:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6274#issuecomment-41704",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed keywords from none to **plot**



---

archive/issue_events_074750.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-13T15:27:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-74750"
}
```



---

archive/issue_events_074751.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-13T15:27:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "milestone_number": null,
    "milestone_title": "sage-4.0.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-74751"
}
```



---

archive/issue_comments_041705.json:
```json
{
    "body": "Author: **Minh Van Nguyen**",
    "created_at": "2009-06-13T15:27:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6274#issuecomment-41705",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Minh Van Nguyen**



---

archive/issue_events_074752.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-13T15:27:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-74752"
}
```



---

archive/issue_comments_041706.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,4 @@\n-This is a follow-up to #6270.\n+This is a follow-up to #6270. Here, we expose more 2-D plotting functions in the reference manual. Patches should be applied in this order:\n+1. the patch at #6257\n+2. the patch at #6270\n+3. and finally the patch on this ticket.\n``````\n",
    "created_at": "2009-06-13T15:27:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6274#issuecomment-41706",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,4 @@
-This is a follow-up to #6270.
+This is a follow-up to #6270. Here, we expose more 2-D plotting functions in the reference manual. Patches should be applied in this order:
+1. the patch at #6257
+2. the patch at #6270
+3. and finally the patch on this ticket.
``````




---

archive/issue_events_074753.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-06-15T23:42:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-74753"
}
```



---

archive/issue_events_074754.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-06-15T23:42:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-74754"
}
```



---

archive/issue_comments_041707.json:
```json
{
    "body": "Reviewer: **John Palmieri**",
    "created_at": "2009-06-15T23:42:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6274#issuecomment-41707",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **John Palmieri**



---

archive/issue_comments_041708.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nLooks good.  I'm attaching a referee's patch with two trivial fixes.",
    "created_at": "2009-06-15T23:42:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6274#issuecomment-41708",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:2" align="right">comment:2</div>

Looks good.  I'm attaching a referee's patch with two trivial fixes.



---

archive/issue_comments_041709.json:
```json
{
    "body": "Attachment: **[ref_6274.patch.gz](https://github.com/sagemath/sage/files/ticket6274/ref_6274.patch.gz)**",
    "created_at": "2009-06-15T23:42:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6274#issuecomment-41709",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment: **[ref_6274.patch.gz](https://github.com/sagemath/sage/files/ticket6274/ref_6274.patch.gz)**



---

archive/issue_comments_041710.json:
```json
{
    "body": "Merged: **sage-4.1.alpha0**",
    "created_at": "2009-06-24T10:11:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6274#issuecomment-41710",
    "user": "https://github.com/rlmill"
}
```

Merged: **sage-4.1.alpha0**



---

archive/issue_events_074755.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-24T10:11:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-74755"
}
```



---

archive/issue_events_074756.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-24T10:11:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-74756"
}
```

# Issue 6075: dsage is not yet entirely optional

archive/issues_006075.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nFor 4.0, dsage was made an optional spkg.  However, if I rm -rf build/ and sage -ba, the newly built tree has no dsage.\n\nCC:  @craigcitro\n\nComponent: **dsage**\n\nKeywords: **dsage optional spkg**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6075_\n\n",
    "closed_at": "2009-05-28T05:07:16Z",
    "created_at": "2009-05-18T21:02:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20dsage",
        "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "dsage is not yet entirely optional",
    "type": "issue",
    "updated_at": "2009-05-28T05:07:16Z",
    "url": "https://github.com/sagemath/sage/issues/6075",
    "user": "https://github.com/ncalexan"
}
```
<div id="comment:0"></div>

For 4.0, dsage was made an optional spkg.  However, if I rm -rf build/ and sage -ba, the newly built tree has no dsage.

CC:  @craigcitro

Component: **dsage**

Keywords: **dsage optional spkg**

_Issue created by migration from https://trac.sagemath.org/ticket/6075_





---

archive/issue_events_072304.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-05-18T21:02:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6075",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6075#event-72304"
}
```



---

archive/issue_events_072305.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-05-18T21:02:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6075",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20dsage",
    "label_color": "0000ff",
    "label_name": "c: dsage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6075#event-72305"
}
```



---

archive/issue_events_072306.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-05-18T21:02:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6075",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6075#event-72306"
}
```



---

archive/issue_events_072307.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-05-18T21:02:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6075",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6075#event-72307"
}
```



---

archive/issue_events_072308.json:
```json
{
    "actor": "https://github.com/yqiang",
    "created_at": "2009-05-18T21:02:20Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6075",
    "subject": "https://github.com/ncalexan",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6075#event-72308"
}
```



---

archive/issue_events_072309.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-26T20:49:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6075",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6075#event-72309"
}
```



---

archive/issue_events_072310.json:
```json
{
    "actor": "https://github.com/yqiang",
    "created_at": "2009-05-26T20:49:25Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/6075",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6075#event-72310"
}
```



---

archive/issue_events_072311.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-26T20:49:25Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6075",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6075#event-72311"
}
```



---

archive/issue_events_072312.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-26T20:49:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6075",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6075#event-72312"
}
```



---

archive/issue_comments_039805.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nI've added a patch that makes things continue to work if dsage is not there.  However, we really should make it so that dsage does not install into sage.dsage.  This requires quite a few more changes within dsage itself.",
    "created_at": "2009-05-26T20:49:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6075#issuecomment-39805",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

I've added a patch that makes things continue to work if dsage is not there.  However, we really should make it so that dsage does not install into sage.dsage.  This requires quite a few more changes within dsage itself.



---

archive/issue_comments_039806.json:
```json
{
    "body": "Attachment: **[trac_6075.patch.gz](https://github.com/sagemath/sage/files/ticket6075/trac_6075.patch.gz)**",
    "created_at": "2009-05-28T02:44:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6075#issuecomment-39806",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[trac_6075.patch.gz](https://github.com/sagemath/sage/files/ticket6075/trac_6075.patch.gz)**



---

archive/issue_comments_039807.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI updated the patch and made http://sage.math.washington.edu/home/mhansen/dsage-1.0.1.spkg which moves DSage from sage.dsage to just dsage.",
    "created_at": "2009-05-28T02:47:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6075#issuecomment-39807",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">comment:3</div>

I updated the patch and made http://sage.math.washington.edu/home/mhansen/dsage-1.0.1.spkg which moves DSage from sage.dsage to just dsage.



---

archive/issue_comments_039808.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nYep, this looks good. You have to manually remove `$SAGE_ROOT/devel/sage/build/sage/dsage` to see that this completely works, but then, that's exactly what the patch at #5977 does for you ...",
    "created_at": "2009-05-28T04:57:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6075#issuecomment-39808",
    "user": "https://github.com/craigcitro"
}
```

<div id="comment:4" align="right">comment:4</div>

Yep, this looks good. You have to manually remove `$SAGE_ROOT/devel/sage/build/sage/dsage` to see that this completely works, but then, that's exactly what the patch at #5977 does for you ...



---

archive/issue_events_072313.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-05-28T04:57:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6075",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6075#event-72313"
}
```



---

archive/issue_events_072314.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T05:07:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6075",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6075#event-72314"
}
```



---

archive/issue_events_072315.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T05:07:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6075",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6075#event-72315"
}
```



---

archive/issue_comments_039809.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nMerged in 4.0.rc1.",
    "created_at": "2009-05-28T05:07:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6075#issuecomment-39809",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:5" align="right">comment:5</div>

Merged in 4.0.rc1.

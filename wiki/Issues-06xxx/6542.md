# Issue 6542: tachyon ouput seems broken in sage-4.1

archive/issues_006542.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mhampton"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nAs part of tracking this down, I am planning on adding doctests to more of the tachyon related files, which currently have low coverage.\n\nCC:  @kcrisman @williamstein @boothby @sagetrac-mvngu\n\nComponent: **graphics**\n\nKeywords: **tachyon, raytracer**\n\nAuthor: **Marshall Hampton**\n\nReviewer: **Tim Dumol**\n\nMerged: **Sage 4.1.1.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6542_\n\n",
    "closed_at": "2009-07-29T11:12:52Z",
    "created_at": "2009-07-16T11:49:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graphics",
        "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20graphics"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "tachyon ouput seems broken in sage-4.1",
    "type": "issue",
    "updated_at": "2009-07-29T11:12:52Z",
    "url": "https://github.com/sagemath/sage/issues/6542",
    "user": "https://github.com/sagetrac-mhampton"
}
```
<div id="comment:0"></div>

As part of tracking this down, I am planning on adding doctests to more of the tachyon related files, which currently have low coverage.

CC:  @kcrisman @williamstein @boothby @sagetrac-mvngu

Component: **graphics**

Keywords: **tachyon, raytracer**

Author: **Marshall Hampton**

Reviewer: **Tim Dumol**

Merged: **Sage 4.1.1.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/6542_





---

archive/issue_events_078520.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-16T11:49:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "milestone_number": null,
    "milestone_title": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78520"
}
```



---

archive/issue_events_078521.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-16T11:49:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "c: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78521"
}
```



---

archive/issue_events_078522.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-16T11:49:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78522"
}
```



---

archive/issue_events_078523.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-16T11:49:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78523"
}
```



---

archive/issue_events_078524.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-16T11:49:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "c: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78524"
}
```



---

archive/issue_events_078525.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-07-16T11:49:09Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "subject": "https://github.com/sagetrac-mhampton",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78525"
}
```



---

archive/issue_comments_045122.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThis problem was raised in this [sage-support](http://groups.google.com/group/sage-support/browse_thread/thread/12104b9aec94c758) thread.",
    "created_at": "2009-07-16T11:52:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6542#issuecomment-45122",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:1" align="right">comment:1</div>

This problem was raised in this [sage-support](http://groups.google.com/group/sage-support/browse_thread/thread/12104b9aec94c758) thread.



---

archive/issue_comments_045123.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis might have been broken by 6269 or 6372.  At any rate, the tostr function in tachyon.py is now missing a crucial piece.  I am working on a patch.",
    "created_at": "2009-07-16T14:38:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6542#issuecomment-45123",
    "user": "https://github.com/sagetrac-mhampton"
}
```

<div id="comment:2" align="right">comment:2</div>

This might have been broken by 6269 or 6372.  At any rate, the tostr function in tachyon.py is now missing a crucial piece.  I am working on a patch.



---

archive/issue_comments_045124.json:
```json
{
    "body": "fixes a mistaken deletion of critical functionality in tachyon's tostr function",
    "created_at": "2009-07-16T18:19:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6542#issuecomment-45124",
    "user": "https://github.com/sagetrac-mhampton"
}
```

fixes a mistaken deletion of critical functionality in tachyon's tostr function



---

archive/issue_comments_045125.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAttachment: **[trac_6542_tachyon_tostr.patch.gz](https://github.com/sagemath/sage/files/ticket6542/trac_6542_tachyon_tostr.patch.gz)**\n\nThe patch fixes a mistaken deletion of critical functionality in tachyon's tostr function.  Looks like a piece of this function was sliced off by mistake during the colorsys refactoring.\n\nI will open a seperate ticket to improve the tachyon doctests; I think this patch should go in ASAP since tachyon is totally broken without it.",
    "created_at": "2009-07-16T18:22:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6542#issuecomment-45125",
    "user": "https://github.com/sagetrac-mhampton"
}
```

<div id="comment:3" align="right">comment:3</div>

Attachment: **[trac_6542_tachyon_tostr.patch.gz](https://github.com/sagemath/sage/files/ticket6542/trac_6542_tachyon_tostr.patch.gz)**

The patch fixes a mistaken deletion of critical functionality in tachyon's tostr function.  Looks like a piece of this function was sliced off by mistake during the colorsys refactoring.

I will open a seperate ticket to improve the tachyon doctests; I think this patch should go in ASAP since tachyon is totally broken without it.



---

archive/issue_events_078526.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-16T18:22:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78526"
}
```



---

archive/issue_events_078527.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-16T18:22:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78527"
}
```



---

archive/issue_events_078528.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-16T18:22:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78528"
}
```



---

archive/issue_events_078529.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-07-16T18:37:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78529"
}
```



---

archive/issue_events_078530.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-07-16T18:37:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78530"
}
```



---

archive/issue_comments_045126.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThis happened during the rebasing of #6372, moving the tachyon stuff.  Perhaps this is a warning against restoring positive reviews immediately upon rebase, or perhaps not.  In any case, the current patch needs a doctest to catch this - currently the function returns None every time this is called on a non-string, and that is probably why things didn't get caught before.",
    "created_at": "2009-07-16T18:37:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6542#issuecomment-45126",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:4" align="right">comment:4</div>

This happened during the rebasing of #6372, moving the tachyon stuff.  Perhaps this is a warning against restoring positive reviews immediately upon rebase, or perhaps not.  In any case, the current patch needs a doctest to catch this - currently the function returns None every time this is called on a non-string, and that is probably why things didn't get caught before.



---

archive/issue_comments_045127.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nWell, yeah - the whole damn file needs doctests, which will take a while.  But I really think this should be fixed as soon as possible.\n\nI made ticket #6543 to fix the lack of doctests; that can build on this patch.",
    "created_at": "2009-07-16T19:13:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6542#issuecomment-45127",
    "user": "https://github.com/sagetrac-mhampton"
}
```

<div id="comment:5" align="right">comment:5</div>

Well, yeah - the whole damn file needs doctests, which will take a while.  But I really think this should be fixed as soon as possible.

I made ticket #6543 to fix the lack of doctests; that can build on this patch.



---

archive/issue_comments_045128.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nReplying to [@sagetrac-mhampton](#comment%3A5):\n> Well, yeah - the whole damn file needs doctests, which will take a while.  But I really think this should be fixed as soon as possible.\n\nTrue; I just meant to add a string with doctest that \n\n```\ntostr([5,4,3])\n' 5.0 4.0 3.0 '\n```\nas per Sage convention to check that it was fixed; that shouldn't be too bad.  I'd do it myself but do not have a current Sage build available.\n> I made ticket #6543 to fix the lack of doctests; that can build on this patch.\n\nYes, I've been thinking about doing this for a while too.",
    "created_at": "2009-07-16T19:22:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6542#issuecomment-45128",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:6" align="right">comment:6</div>

Replying to [@sagetrac-mhampton](#comment%3A5):
> Well, yeah - the whole damn file needs doctests, which will take a while.  But I really think this should be fixed as soon as possible.

True; I just meant to add a string with doctest that 

```
tostr([5,4,3])
' 5.0 4.0 3.0 '
```
as per Sage convention to check that it was fixed; that shouldn't be too bad.  I'd do it myself but do not have a current Sage build available.
> I made ticket #6543 to fix the lack of doctests; that can build on this patch.

Yes, I've been thinking about doing this for a while too.



---

archive/issue_comments_045129.json:
```json
{
    "body": "Attachment: **[trac_6542_tachyon_tostr.2.patch.gz](https://github.com/sagemath/sage/files/ticket6542/trac_6542_tachyon_tostr.2.patch.gz)**\n\nnew version with doctest",
    "created_at": "2009-07-16T20:06:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6542#issuecomment-45129",
    "user": "https://github.com/sagetrac-mhampton"
}
```

Attachment: **[trac_6542_tachyon_tostr.2.patch.gz](https://github.com/sagemath/sage/files/ticket6542/trac_6542_tachyon_tostr.2.patch.gz)**

new version with doctest



---

archive/issue_comments_045130.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nOK, I added a doctest to the tostr function.",
    "created_at": "2009-07-16T20:07:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6542#issuecomment-45130",
    "user": "https://github.com/sagetrac-mhampton"
}
```

<div id="comment:7" align="right">comment:7</div>

OK, I added a doctest to the tostr function.



---

archive/issue_events_078531.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-16T20:07:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78531"
}
```



---

archive/issue_events_078532.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-16T20:07:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78532"
}
```



---

archive/issue_comments_045131.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nThe function itself (obviously, since just replaces from before) works fine for me as is, and the documentation works fine in testing.  Unfortunately, I can't currently merge it in a Sage build and do a test run, so needs further review to ensure that.  This should be REALLY EASY to do for anyone who has a current Sage 4.1.",
    "created_at": "2009-07-17T13:07:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6542#issuecomment-45131",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:8" align="right">comment:8</div>

The function itself (obviously, since just replaces from before) works fine for me as is, and the documentation works fine in testing.  Unfortunately, I can't currently merge it in a Sage build and do a test run, so needs further review to ensure that.  This should be REALLY EASY to do for anyone who has a current Sage 4.1.



---

archive/issue_events_078533.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-07-17T13:07:06Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "title_is": "[partial positive review] tachyon ouput seems broken in sage-4.1",
    "title_was": "tachyon ouput seems broken in sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78533"
}
```



---

archive/issue_events_078534.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-07-23T03:21:23Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "subject": "https://github.com/sagetrac-mhampton",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78534"
}
```



---

archive/issue_events_078535.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-23T03:21:23Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "subject": "https://github.com/sagetrac-mhampton",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78535"
}
```



---

archive/issue_comments_045132.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nWorks perfectly for me -- Sage 4.1.",
    "created_at": "2009-07-27T09:22:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6542#issuecomment-45132",
    "user": "https://github.com/TimDumol"
}
```

<div id="comment:10" align="right">comment:10</div>

Works perfectly for me -- Sage 4.1.



---

archive/issue_events_078536.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-07-27T09:22:28Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "title_is": "tachyon ouput seems broken in sage-4.1",
    "title_was": "[partial positive review] tachyon ouput seems broken in sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78536"
}
```



---

archive/issue_events_078537.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-07-27T09:22:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78537"
}
```



---

archive/issue_events_078538.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2009-07-27T09:22:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78538"
}
```



---

archive/issue_comments_045133.json:
```json
{
    "body": "Reviewer: **timdumol**",
    "created_at": "2009-07-27T09:22:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6542#issuecomment-45133",
    "user": "https://github.com/TimDumol"
}
```

Reviewer: **timdumol**



---

archive/issue_comments_045134.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nTypo in the word \"seperated\" on line 990:\n\n```\n-Converts vector information to a space-seperated string.\n+Converts vector information to a space-separated string.\n```",
    "created_at": "2009-07-27T13:19:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6542#issuecomment-45134",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:11" align="right">comment:11</div>

Typo in the word "seperated" on line 990:

```
-Converts vector information to a space-seperated string.
+Converts vector information to a space-separated string.
```



---

archive/issue_comments_045135.json:
```json
{
    "body": "Merged: **Sage 4.1.1.rc0**",
    "created_at": "2009-07-29T11:12:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6542#issuecomment-45135",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **Sage 4.1.1.rc0**



---

archive/issue_events_078539.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-29T11:12:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78539"
}
```



---

archive/issue_events_078540.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-29T11:12:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6542#event-78540"
}
```



---

archive/issue_comments_045136.json:
```json
{
    "body": "Author: **Marshall Hampton**",
    "created_at": "2009-07-29T11:12:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6542#issuecomment-45136",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Marshall Hampton**



---

archive/issue_comments_045137.json:
```json
{
    "body": "Changed reviewer from **timdumol** to **Tim Dumol**",
    "created_at": "2009-07-29T11:12:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6542#issuecomment-45137",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed reviewer from **timdumol** to **Tim Dumol**

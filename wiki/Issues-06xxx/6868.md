# Issue 6868: Coin-OR CBC ( compatible with the symbolics from MIP )

archive/issues_006868.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nHello everybody !!!\n\nAs I wrote a new numerical.MIP class using symbolics, which will be waaaaaayyyyy easier to use, here is the necessary update of CBC which can stand all those news !\n\nTo install it :\n\n```\nsage -f http://www-sop.inria.fr/members/Nathann.Cohen/cbc-2.3.p0.spkg\n```\n\nComponent: **packages: optional**\n\nAuthor: **Nathann Cohen**\n\nReviewer: **Minh Van Nguyen**\n\nMerged: **Sage 4.1.2.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6868_\n\n",
    "closed_at": "2009-09-10T12:19:51Z",
    "created_at": "2009-09-02T17:34:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Coin-OR CBC ( compatible with the symbolics from MIP )",
    "type": "issue",
    "updated_at": "2009-09-10T12:19:51Z",
    "url": "https://github.com/sagemath/sage/issues/6868",
    "user": "https://github.com/nathanncohen"
}
```
<div id="comment:0"></div>

Hello everybody !!!

As I wrote a new numerical.MIP class using symbolics, which will be waaaaaayyyyy easier to use, here is the necessary update of CBC which can stand all those news !

To install it :

```
sage -f http://www-sop.inria.fr/members/Nathann.Cohen/cbc-2.3.p0.spkg
```

Component: **packages: optional**

Author: **Nathann Cohen**

Reviewer: **Minh Van Nguyen**

Merged: **Sage 4.1.2.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/6868_





---

archive/issue_events_082765.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-09-02T17:34:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "milestone_number": null,
    "milestone_title": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6868#event-82765"
}
```



---

archive/issue_events_082766.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-09-02T17:34:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20optional",
    "label_color": "0000b0",
    "label_name": "c: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6868#event-82766"
}
```



---

archive/issue_events_082767.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-09-02T17:34:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6868#event-82767"
}
```



---

archive/issue_events_082768.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-09-02T17:34:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6868#event-82768"
}
```



---

archive/issue_events_082769.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-09-02T17:34:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6868#event-82769"
}
```



---

archive/issue_comments_048761.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,6 @@\n Hello everybody !!!\n \n-As I wrote a new numerical.MIP class using symbolics, which will be waaaaaayyyyy easier to use, here is the necessary update of GLPK which can stand all those news !\n+As I wrote a new numerical.MIP class using symbolics, which will be waaaaaayyyyy easier to use, here is the necessary update of CBC which can stand all those news !\n \n To install it :\n \n``````\n",
    "created_at": "2009-09-02T17:40:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6868#issuecomment-48761",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,6 @@
 Hello everybody !!!
 
-As I wrote a new numerical.MIP class using symbolics, which will be waaaaaayyyyy easier to use, here is the necessary update of GLPK which can stand all those news !
+As I wrote a new numerical.MIP class using symbolics, which will be waaaaaayyyyy easier to use, here is the necessary update of CBC which can stand all those news !
 
 To install it :
 
``````




---

archive/issue_comments_048762.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nncohen --- Can you also provide a local copy of the spkg under your sage.math home dir?",
    "created_at": "2009-09-02T17:49:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6868#issuecomment-48762",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:2" align="right">comment:2</div>

ncohen --- Can you also provide a local copy of the spkg under your sage.math home dir?



---

archive/issue_events_082770.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-02T17:49:00Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "title_is": "Coin-OR CBC ( compatible with the symbolics from MIP )",
    "title_was": "[with SPKG] Coin-OR CBC ( compatible with the symbolics from MIP )",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6868#event-82770"
}
```



---

archive/issue_events_082771.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-02T17:49:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6868#event-82771"
}
```



---

archive/issue_comments_048763.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nDone : /home/ncohen/cbc-2.3.p0.spkg",
    "created_at": "2009-09-02T19:37:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6868#issuecomment-48763",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:3" align="right">comment:3</div>

Done : /home/ncohen/cbc-2.3.p0.spkg



---

archive/issue_comments_048764.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nUpdated package at\n\nhttp://sage.math.washington.edu/home/mvngu/release/spkg/optional/cbc-2.3.p0.spkg\n\nIt incorporates ncohen's original spkg, but with some general clean up.",
    "created_at": "2009-09-08T14:57:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6868#issuecomment-48764",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:4" align="right">comment:4</div>

Updated package at

http://sage.math.washington.edu/home/mvngu/release/spkg/optional/cbc-2.3.p0.spkg

It incorporates ncohen's original spkg, but with some general clean up.



---

archive/issue_comments_048765.json:
```json
{
    "body": "Merged: **Sage 4.1.2.alpha2**",
    "created_at": "2009-09-10T12:19:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6868#issuecomment-48765",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **Sage 4.1.2.alpha2**



---

archive/issue_events_082772.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-10T12:19:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6868#event-82772"
}
```



---

archive/issue_events_082773.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-10T12:19:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6868#event-82773"
}
```



---

archive/issue_comments_048766.json:
```json
{
    "body": "Reviewer: **Minh Van Nguyen**",
    "created_at": "2009-09-10T12:19:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6868#issuecomment-48766",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **Minh Van Nguyen**



---

archive/issue_events_082774.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-10T12:19:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6868#event-82774"
}
```



---

archive/issue_events_082775.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-10T12:19:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6868#event-82775"
}
```



---

archive/issue_comments_048767.json:
```json
{
    "body": "Author: **Nathann Cohen**",
    "created_at": "2009-09-10T12:19:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6868#issuecomment-48767",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Nathann Cohen**



---

archive/issue_comments_048768.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nMerged in the optional packages repository at\n\nhttp://www.sagemath.org/packages/optional/\n\nYou can get the spkg at\n\nhttp://www.sagemath.org/packages/optional/cbc-2.3.p0.spkg",
    "created_at": "2009-09-10T12:19:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6868",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6868#issuecomment-48768",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:5" align="right">comment:5</div>

Merged in the optional packages repository at

http://www.sagemath.org/packages/optional/

You can get the spkg at

http://www.sagemath.org/packages/optional/cbc-2.3.p0.spkg

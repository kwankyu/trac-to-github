# Issue 6458: Inverse modulo an ideal in a relative number field

archive/issues_006458.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis adds a few lines to get `inverse_mod` working in the ring of integers of a relative field.\n\nComponent: **number theory**\n\nAuthor: **David Loeffler**\n\nReviewer: **Nick Alexander**\n\nMerged: **sage-4.1.1.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6458_\n\n",
    "closed_at": "2009-07-16T21:10:37Z",
    "created_at": "2009-07-02T13:27:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Inverse modulo an ideal in a relative number field",
    "type": "issue",
    "updated_at": "2009-07-16T21:10:37Z",
    "url": "https://github.com/sagemath/sage/issues/6458",
    "user": "https://github.com/loefflerd"
}
```
<div id="comment:0"></div>

This adds a few lines to get `inverse_mod` working in the ring of integers of a relative field.

Component: **number theory**

Author: **David Loeffler**

Reviewer: **Nick Alexander**

Merged: **sage-4.1.1.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/6458_





---

archive/issue_events_077397.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-02T13:27:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "milestone_number": null,
    "milestone_title": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6458#event-77397"
}
```



---

archive/issue_events_077398.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-02T13:27:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "c: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6458#event-77398"
}
```



---

archive/issue_events_077399.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-02T13:27:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6458#event-77399"
}
```



---

archive/issue_events_077400.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-02T13:27:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6458#event-77400"
}
```



---

archive/issue_events_077401.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-07-02T13:27:16Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "subject": "https://github.com/loefflerd",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6458#event-77401"
}
```



---

archive/issue_comments_043873.json:
```json
{
    "body": "patch against 4.1.alpha2",
    "created_at": "2009-07-02T13:28:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6458#issuecomment-43873",
    "user": "https://github.com/loefflerd"
}
```

patch against 4.1.alpha2



---

archive/issue_comments_043874.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_6458-relative_ideal_inverse_mod.patch.gz](https://github.com/sagemath/sage/files/ticket6458/trac_6458-relative_ideal_inverse_mod.patch.gz)**",
    "created_at": "2009-07-02T13:35:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6458#issuecomment-43874",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_6458-relative_ideal_inverse_mod.patch.gz](https://github.com/sagemath/sage/files/ticket6458/trac_6458-relative_ideal_inverse_mod.patch.gz)**



---

archive/issue_events_077402.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-02T13:35:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6458#event-77402"
}
```



---

archive/issue_comments_043875.json:
```json
{
    "body": "Reviewer: **Nick Alexander**",
    "created_at": "2009-07-14T21:15:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6458#issuecomment-43875",
    "user": "https://github.com/ncalexan"
}
```

Reviewer: **Nick Alexander**



---

archive/issue_comments_043876.json:
```json
{
    "body": "Author: **David Loeffler**",
    "created_at": "2009-07-14T21:15:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6458#issuecomment-43876",
    "user": "https://github.com/ncalexan"
}
```

Author: **David Loeffler**



---

archive/issue_comments_043877.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThese doctests don't actually assert that the results are correct.  Could you add the few lines verifying that you're really getting a basis and really getting an inverse?",
    "created_at": "2009-07-14T21:15:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6458#issuecomment-43877",
    "user": "https://github.com/ncalexan"
}
```

<div id="comment:2" align="right">comment:2</div>

These doctests don't actually assert that the results are correct.  Could you add the few lines verifying that you're really getting a basis and really getting an inverse?



---

archive/issue_events_077403.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-07-14T21:15:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6458#event-77403"
}
```



---

archive/issue_events_077404.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-07-14T21:15:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6458#event-77404"
}
```



---

archive/issue_comments_043878.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAlso, I get a doctest failure on sage.math.  This could be transient -- this is with a slightly out of date sage build.  But there's no way this will work on all architectures, so testing the property will be much better.\n\n```\nsage -t -long devel/sage/sage/rings/number_field/number_field_element.pyx\n**********************************************************************\nFile \"/scratch/ncalexan/sage-4.0.2.alpha1/devel/sage-main/sage/rings/number_field/number_field_element.pyx\", line 3436:\n    sage: OE(b - a).inverse_mod(17*b)\nExpected:\n    (-25*b + 26)*a + 51*b - 1\nGot:\n    (26*b - 25)*a - 51*b - 1\n```",
    "created_at": "2009-07-14T21:20:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6458#issuecomment-43878",
    "user": "https://github.com/ncalexan"
}
```

<div id="comment:3" align="right">comment:3</div>

Also, I get a doctest failure on sage.math.  This could be transient -- this is with a slightly out of date sage build.  But there's no way this will work on all architectures, so testing the property will be much better.

```
sage -t -long devel/sage/sage/rings/number_field/number_field_element.pyx
**********************************************************************
File "/scratch/ncalexan/sage-4.0.2.alpha1/devel/sage-main/sage/rings/number_field/number_field_element.pyx", line 3436:
    sage: OE(b - a).inverse_mod(17*b)
Expected:
    (-25*b + 26)*a + 51*b - 1
Got:
    (26*b - 25)*a - 51*b - 1
```



---

archive/issue_comments_043879.json:
```json
{
    "body": "Attachment: **[trac_6458-fix.patch.gz](https://github.com/sagemath/sage/files/ticket6458/trac_6458-fix.patch.gz)**\n\napply after previous patch",
    "created_at": "2009-07-14T22:04:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6458#issuecomment-43879",
    "user": "https://github.com/loefflerd"
}
```

Attachment: **[trac_6458-fix.patch.gz](https://github.com/sagemath/sage/files/ticket6458/trac_6458-fix.patch.gz)**

apply after previous patch



---

archive/issue_events_077405.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-14T22:05:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6458#event-77405"
}
```



---

archive/issue_events_077406.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-14T22:05:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6458#event-77406"
}
```



---

archive/issue_comments_043880.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nGood point; I have uploaded a second patch that adjusts the doctests as you suggest.",
    "created_at": "2009-07-14T22:05:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6458#issuecomment-43880",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:4" align="right">comment:4</div>

Good point; I have uploaded a second patch that adjusts the doctests as you suggest.



---

archive/issue_comments_043881.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nBeautiful.",
    "created_at": "2009-07-15T02:34:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6458#issuecomment-43881",
    "user": "https://github.com/ncalexan"
}
```

<div id="comment:5" align="right">comment:5</div>

Beautiful.



---

archive/issue_events_077407.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-07-15T02:34:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6458#event-77407"
}
```



---

archive/issue_events_077408.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-07-15T02:34:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6458#event-77408"
}
```



---

archive/issue_comments_043882.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nDavid, the patch `trac_6458-relative_ideal_inverse_mod.patch` doesn't have your username. So I'm committing it in your name. Merged both patches in sage-4.1.1-alpha0. I can't close this ticket because I don't have the privilege to do so. Sorry, folks :-(",
    "created_at": "2009-07-16T19:54:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6458#issuecomment-43882",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:6" align="right">comment:6</div>

David, the patch `trac_6458-relative_ideal_inverse_mod.patch` doesn't have your username. So I'm committing it in your name. Merged both patches in sage-4.1.1-alpha0. I can't close this ticket because I don't have the privilege to do so. Sorry, folks :-(



---

archive/issue_comments_043883.json:
```json
{
    "body": "Merged: **sage-4.1.1.alpha0**",
    "created_at": "2009-07-16T21:10:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6458#issuecomment-43883",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **sage-4.1.1.alpha0**



---

archive/issue_events_077409.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-16T21:10:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6458#event-77409"
}
```



---

archive/issue_events_077410.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-16T21:10:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6458",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6458#event-77410"
}
```

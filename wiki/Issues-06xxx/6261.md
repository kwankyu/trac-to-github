# Issue 6261: Add multiplicative order for matrices over finite fields

archive/issues_006261.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nFor the algorithm used, see\nFrank Celler and Charles R. Leedham-Green, \"Calculating the Order of an Invertible Matrix\".\n\nComponent: **linear algebra**\n\nAuthor: **Yann Laigle-Chapuy**\n\nReviewer: **John Cremona**\n\nMerged: **4.1.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6261_\n\n",
    "closed_at": "2009-06-24T10:10:33Z",
    "created_at": "2009-06-11T16:54:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add multiplicative order for matrices over finite fields",
    "type": "issue",
    "updated_at": "2009-06-27T08:35:24Z",
    "url": "https://github.com/sagemath/sage/issues/6261",
    "user": "https://github.com/sagetrac-ylchapuy"
}
```
<div id="comment:0"></div>

For the algorithm used, see
Frank Celler and Charles R. Leedham-Green, "Calculating the Order of an Invertible Matrix".

Component: **linear algebra**

Author: **Yann Laigle-Chapuy**

Reviewer: **John Cremona**

Merged: **4.1.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/6261_





---

archive/issue_events_074572.json:
```json
{
    "actor": "https://github.com/sagetrac-ylchapuy",
    "created_at": "2009-06-11T16:54:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "milestone_number": null,
    "milestone_title": "sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6261#event-74572"
}
```



---

archive/issue_events_074573.json:
```json
{
    "actor": "https://github.com/sagetrac-ylchapuy",
    "created_at": "2009-06-11T16:54:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "c: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6261#event-74573"
}
```



---

archive/issue_events_074574.json:
```json
{
    "actor": "https://github.com/sagetrac-ylchapuy",
    "created_at": "2009-06-11T16:54:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6261#event-74574"
}
```



---

archive/issue_events_074575.json:
```json
{
    "actor": "https://github.com/sagetrac-ylchapuy",
    "created_at": "2009-06-11T16:54:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6261#event-74575"
}
```



---

archive/issue_events_074576.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-11T16:54:34Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "subject": "https://github.com/sagetrac-ylchapuy",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6261#event-74576"
}
```



---

archive/issue_events_074577.json:
```json
{
    "actor": "https://github.com/sagetrac-ylchapuy",
    "created_at": "2009-06-11T16:57:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6261#event-74577"
}
```



---

archive/issue_comments_041565.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac-6261.patch.gz](https://github.com/sagemath/sage/files/ticket6261/trac-6261.patch.gz)**",
    "created_at": "2009-06-11T16:57:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6261#issuecomment-41565",
    "user": "https://github.com/sagetrac-ylchapuy"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac-6261.patch.gz](https://github.com/sagemath/sage/files/ticket6261/trac-6261.patch.gz)**



---

archive/issue_events_074578.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-06-14T16:33:15Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "title_is": "[with review] Add multiplicative order for matrices over finite fields",
    "title_was": "Add multiplicative order for matrices over finite fields",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6261#event-74578"
}
```



---

archive/issue_comments_041566.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis looks good to me.  One question:  in the line\n\n```\nppart = p**Integer(a).exact_log(p)\n```\ndo you really mean ppart to be the largest power of p which is <= a, rather than the pargest power which divides a?\nIf so, please change this to \"positive review\".\n\nRemark:  I have a Magma implementation of an algorithm to determine when a polynomial over ZZ is cyclotomic (algorithm of Smyth and Beukers).  That could be used to extend this function to matrices over ZZ, at least.",
    "created_at": "2009-06-14T16:33:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6261#issuecomment-41566",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:2" align="right">comment:2</div>

This looks good to me.  One question:  in the line

```
ppart = p**Integer(a).exact_log(p)
```
do you really mean ppart to be the largest power of p which is <= a, rather than the pargest power which divides a?
If so, please change this to "positive review".

Remark:  I have a Magma implementation of an algorithm to determine when a polynomial over ZZ is cyclotomic (algorithm of Smyth and Beukers).  That could be used to extend this function to matrices over ZZ, at least.



---

archive/issue_events_074579.json:
```json
{
    "actor": "https://github.com/sagetrac-ylchapuy",
    "created_at": "2009-06-16T20:07:37Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "title_is": "Add multiplicative order for matrices over finite fields",
    "title_was": "[with review] Add multiplicative order for matrices over finite fields",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6261#event-74579"
}
```



---

archive/issue_events_074580.json:
```json
{
    "actor": "https://github.com/sagetrac-ylchapuy",
    "created_at": "2009-06-16T20:07:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6261#event-74580"
}
```



---

archive/issue_events_074581.json:
```json
{
    "actor": "https://github.com/sagetrac-ylchapuy",
    "created_at": "2009-06-16T20:07:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6261#event-74581"
}
```



---

archive/issue_comments_041567.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nReplying to [@JohnCremona](#comment%3A2):\n> This looks good to me.  One question:  in the line\n> \n> ```\n> ppart = p**Integer(a).exact_log(p)\n> ```\n> do you really mean ppart to be the largest power of p which is <= a, rather than the pargest power which divides a?\n> If so, please change this to \"positive review\".\n\nYes, it's what I mean. Thanks for the review.",
    "created_at": "2009-06-16T20:07:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6261#issuecomment-41567",
    "user": "https://github.com/sagetrac-ylchapuy"
}
```

<div id="comment:3" align="right">comment:3</div>

Replying to [@JohnCremona](#comment%3A2):
> This looks good to me.  One question:  in the line
> 
> ```
> ppart = p**Integer(a).exact_log(p)
> ```
> do you really mean ppart to be the largest power of p which is <= a, rather than the pargest power which divides a?
> If so, please change this to "positive review".

Yes, it's what I mean. Thanks for the review.



---

archive/issue_comments_041568.json:
```json
{
    "body": "Author: **Yann Laigle-Chapuy**",
    "created_at": "2009-06-24T10:10:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6261#issuecomment-41568",
    "user": "https://github.com/rlmill"
}
```

Author: **Yann Laigle-Chapuy**



---

archive/issue_events_074582.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-24T10:10:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6261#event-74582"
}
```



---

archive/issue_events_074583.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-24T10:10:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6261#event-74583"
}
```



---

archive/issue_comments_041569.json:
```json
{
    "body": "Reviewer: **John Cremona**",
    "created_at": "2009-06-24T10:10:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6261#issuecomment-41569",
    "user": "https://github.com/rlmill"
}
```

Reviewer: **John Cremona**



---

archive/issue_comments_041570.json:
```json
{
    "body": "Merged: **Yann Laigle-Chapuy**",
    "created_at": "2009-06-24T10:10:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6261#issuecomment-41570",
    "user": "https://github.com/rlmill"
}
```

Merged: **Yann Laigle-Chapuy**



---

archive/issue_comments_041571.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\n\"Merged in: Yann Laigle-Chapuy\"? Hang on a minute! Which version was this actually merged in?",
    "created_at": "2009-06-27T08:32:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6261#issuecomment-41571",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:5" align="right">comment:5</div>

"Merged in: Yann Laigle-Chapuy"? Hang on a minute! Which version was this actually merged in?



---

archive/issue_comments_041572.json:
```json
{
    "body": "Changed merged from **Yann Laigle-Chapuy** to **4.1.alpha0**",
    "created_at": "2009-06-27T08:35:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6261",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6261#issuecomment-41572",
    "user": "https://github.com/loefflerd"
}
```

Changed merged from **Yann Laigle-Chapuy** to **4.1.alpha0**

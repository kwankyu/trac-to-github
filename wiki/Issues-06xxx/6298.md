# Issue 6298: create an experimental libcocoa spkg

archive/issues_006298.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe spkg is here:\n\n   http://sage.math.washington.edu/home/wstein/patches/libcocoa-0.9930.spkg\n\nComponent: **packages: standard**\n\nAuthor: **William Stein**\n\nReviewer: **John Cremona**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6298_\n\n",
    "closed_at": "2009-06-15T16:57:16Z",
    "created_at": "2009-06-15T13:01:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "create an experimental libcocoa spkg",
    "type": "issue",
    "updated_at": "2009-06-19T13:50:07Z",
    "url": "https://github.com/sagemath/sage/issues/6298",
    "user": "https://github.com/williamstein"
}
```
<div id="comment:0"></div>

The spkg is here:

   http://sage.math.washington.edu/home/wstein/patches/libcocoa-0.9930.spkg

Component: **packages: standard**

Author: **William Stein**

Reviewer: **John Cremona**

_Issue created by migration from https://trac.sagemath.org/ticket/6298_





---

archive/issue_events_074989.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-15T13:01:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6298",
    "milestone_number": null,
    "milestone_title": "sage-4.0.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6298#event-74989"
}
```



---

archive/issue_events_074990.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-15T13:01:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6298",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "0000b0",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6298#event-74990"
}
```



---

archive/issue_events_074991.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-15T13:01:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6298",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6298#event-74991"
}
```



---

archive/issue_events_074992.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-15T13:01:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6298",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6298#event-74992"
}
```



---

archive/issue_events_074993.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-06-15T13:01:04Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6298",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6298#event-74993"
}
```



---

archive/issue_comments_041849.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n+The spkg is here:\n \n+   http://sage.math.washington.edu/home/wstein/patches/libcocoa-0.9930.spkg\n``````\n",
    "created_at": "2009-06-15T13:47:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6298",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6298#issuecomment-41849",
    "user": "https://github.com/williamstein"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
+The spkg is here:
 
+   http://sage.math.washington.edu/home/wstein/patches/libcocoa-0.9930.spkg
``````




---

archive/issue_events_074994.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-15T13:47:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6298",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6298#event-74994"
}
```



---

archive/issue_comments_041850.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nI'm just asking to put this in experimental, so it doesn't need much review.  Just check that it is up to spec.   That's it.",
    "created_at": "2009-06-15T13:47:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6298",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6298#issuecomment-41850",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>

I'm just asking to put this in experimental, so it doesn't need much review.  Just check that it is up to spec.   That's it.



---

archive/issue_events_074995.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-06-15T16:50:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6298",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6298#event-74995"
}
```



---

archive/issue_events_074996.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-06-15T16:50:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6298",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6298#event-74996"
}
```



---

archive/issue_comments_041851.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI successfully built this on 4.0.1.  I'm not an expert on spkgs but there is an SPKG.txt which looks ok, the spkg-install script doesn't do anything bad that I could tell, and there is an hg repository.  So I see no reason why not to let this loose (watch those double negatives!) as an experimental spkg.",
    "created_at": "2009-06-15T16:50:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6298",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6298#issuecomment-41851",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:2" align="right">comment:2</div>

I successfully built this on 4.0.1.  I'm not an expert on spkgs but there is an SPKG.txt which looks ok, the spkg-install script doesn't do anything bad that I could tell, and there is an hg repository.  So I see no reason why not to let this loose (watch those double negatives!) as an experimental spkg.



---

archive/issue_comments_041852.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nmerged into optional repo",
    "created_at": "2009-06-15T16:57:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6298",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6298#issuecomment-41852",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:3" align="right">comment:3</div>

merged into optional repo



---

archive/issue_events_074997.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-15T16:57:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6298",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6298#event-74997"
}
```



---

archive/issue_events_074998.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-15T16:57:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6298",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6298#event-74998"
}
```



---

archive/issue_comments_041853.json:
```json
{
    "body": "Reviewer: **John Cremona**",
    "created_at": "2009-06-19T13:50:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6298",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6298#issuecomment-41853",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **John Cremona**



---

archive/issue_comments_041854.json:
```json
{
    "body": "Author: **William Stein**",
    "created_at": "2009-06-19T13:50:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6298",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6298#issuecomment-41854",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **William Stein**

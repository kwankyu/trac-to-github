# Issue 6014: hexads in S(5,6,12) and mathematical blackjack

archive/issues_006014.json:
```json
{
    "assignees": [
        "https://github.com/rlmill"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis patch implements kittens, hexads and mathematical blackjack as explained in\n\n```\n    R. Curtis, ``The Steiner system $S(5,6,12)$, the Mathieu group $M_{12}$, \n    and the kitten,'' in {\\bf Computational group theory}, ed. M. Atkinson, \n    Academic Press, 1984.\n    J. Conway, ``Hexacode and tetracode - MINIMOG and MOG,'' in {\\bf Computational \n    group theory}, ed. M. Atkinson, Academic Press, 1984.\n    J. Conway and N. Sloane, ``Lexicographic codes: error-correcting codes from \n    game theory,'' IEEE Trans. Infor. Theory32(1986)337-348.\n    J. Kahane and A. Ryba, ``The hexad game,'' Electronic Journal of Combinatorics, 8 (2001) \n    http://www.combinatorics.org/Volume_8/Abstracts/v8i2r11.html\n```\n\nIt is used in a book on coding theory I'm writing with Jon-Lark Kim on coding theory, which uses Sage throughout to illustrate error-correcting codes.\n\n\nComponent: **coding theory**\n\nKeywords: **coding theory**\n\nAuthor: **David Joyner**\n\nReviewer: **Dan Gordon**\n\nMerged: **4.0.2.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6014_\n\n",
    "closed_at": "2009-06-13T21:45:34Z",
    "created_at": "2009-05-10T12:56:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20coding%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "hexads in S(5,6,12) and mathematical blackjack",
    "type": "issue",
    "updated_at": "2009-06-13T21:45:34Z",
    "url": "https://github.com/sagemath/sage/issues/6014",
    "user": "https://github.com/wdjoyner"
}
```
<div id="comment:0"></div>

This patch implements kittens, hexads and mathematical blackjack as explained in

```
    R. Curtis, ``The Steiner system $S(5,6,12)$, the Mathieu group $M_{12}$, 
    and the kitten,'' in {\bf Computational group theory}, ed. M. Atkinson, 
    Academic Press, 1984.
    J. Conway, ``Hexacode and tetracode - MINIMOG and MOG,'' in {\bf Computational 
    group theory}, ed. M. Atkinson, Academic Press, 1984.
    J. Conway and N. Sloane, ``Lexicographic codes: error-correcting codes from 
    game theory,'' IEEE Trans. Infor. Theory32(1986)337-348.
    J. Kahane and A. Ryba, ``The hexad game,'' Electronic Journal of Combinatorics, 8 (2001) 
    http://www.combinatorics.org/Volume_8/Abstracts/v8i2r11.html
```

It is used in a book on coding theory I'm writing with Jon-Lark Kim on coding theory, which uses Sage throughout to illustrate error-correcting codes.


Component: **coding theory**

Keywords: **coding theory**

Author: **David Joyner**

Reviewer: **Dan Gordon**

Merged: **4.0.2.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/6014_





---

archive/issue_events_071527.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2009-05-10T12:56:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "milestone_number": null,
    "milestone_title": "sage-4.0.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6014#event-71527"
}
```



---

archive/issue_events_071528.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2009-05-10T12:56:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20coding%20theory",
    "label_color": "0000ff",
    "label_name": "c: coding theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6014#event-71528"
}
```



---

archive/issue_events_071529.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2009-05-10T12:56:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6014#event-71529"
}
```



---

archive/issue_events_071530.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2009-05-10T12:56:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6014#event-71530"
}
```



---

archive/issue_events_071531.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2009-05-10T12:56:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6014#event-71531"
}
```



---

archive/issue_events_071532.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-05-10T12:56:56Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "subject": "https://github.com/wdjoyner",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6014#event-71532"
}
```



---

archive/issue_comments_039272.json:
```json
{
    "body": "applies to 3.4.2 and passes sage -testall",
    "created_at": "2009-05-10T12:58:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6014#issuecomment-39272",
    "user": "https://github.com/wdjoyner"
}
```

applies to 3.4.2 and passes sage -testall



---

archive/issue_events_071533.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2009-05-12T00:33:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6014#event-71533"
}
```



---

archive/issue_comments_039273.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_6014-hexads.patch.gz](https://github.com/sagemath/sage/files/ticket6014/trac_6014-hexads.patch.gz)**",
    "created_at": "2009-05-12T00:33:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6014#issuecomment-39273",
    "user": "https://github.com/wdjoyner"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_6014-hexads.patch.gz](https://github.com/sagemath/sage/files/ticket6014/trac_6014-hexads.patch.gz)**



---

archive/issue_comments_039274.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAfter reviewing the patch I think that it looks good overall, but there is one major problem.  In setting_up() various global variables are created with information for minimog_shuffle.  The find_hexad() functions take MINIMOG as an argument, but use the minimog_shuffle global variables.  Right now this is the only minimog implemented, but if minimog_modulo11 is implemented in the future, other changes would be needed.\n\nInstead of having the global variables separate from the minimog, why not define a class minimog, which contains all the line, cross, tet, and other information.  This would take care of the above problem, and be better programming practice.  \n\nAlso, is it really that difficult to implement minimog_modulo11?  I think the only problem is that then the minimog can't be defined as a matrix over QQ.  Since there aren't any arithmetic operations being done to the matrix elements, this shouldn't be a real impediment.  If the minimogs are implemented as classes, then a function could be added to convert minimog elements to strings that handles infinity.",
    "created_at": "2009-06-01T18:46:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6014#issuecomment-39274",
    "user": "https://github.com/sagetrac-dgordon"
}
```

<div id="comment:3" align="right">comment:3</div>

After reviewing the patch I think that it looks good overall, but there is one major problem.  In setting_up() various global variables are created with information for minimog_shuffle.  The find_hexad() functions take MINIMOG as an argument, but use the minimog_shuffle global variables.  Right now this is the only minimog implemented, but if minimog_modulo11 is implemented in the future, other changes would be needed.

Instead of having the global variables separate from the minimog, why not define a class minimog, which contains all the line, cross, tet, and other information.  This would take care of the above problem, and be better programming practice.  

Also, is it really that difficult to implement minimog_modulo11?  I think the only problem is that then the minimog can't be defined as a matrix over QQ.  Since there aren't any arithmetic operations being done to the matrix elements, this shouldn't be a real impediment.  If the minimogs are implemented as classes, then a function could be added to convert minimog elements to strings that handles infinity.



---

archive/issue_events_071534.json:
```json
{
    "actor": "https://github.com/sagetrac-dgordon",
    "created_at": "2009-06-01T18:46:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6014#event-71534"
}
```



---

archive/issue_events_071535.json:
```json
{
    "actor": "https://github.com/sagetrac-dgordon",
    "created_at": "2009-06-01T18:46:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6014#event-71535"
}
```



---

archive/issue_comments_039275.json:
```json
{
    "body": "Attachment: **[trac_6014-hexads2.patch.gz](https://github.com/sagemath/sage/files/ticket6014/trac_6014-hexads2.patch.gz)**",
    "created_at": "2009-06-03T04:41:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6014#issuecomment-39275",
    "user": "https://github.com/wdjoyner"
}
```

Attachment: **[trac_6014-hexads2.patch.gz](https://github.com/sagemath/sage/files/ticket6014/trac_6014-hexads2.patch.gz)**



---

archive/issue_comments_039276.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThis implements the referee's suggestions, passes sage -testall on 4.0.1.a0 on a 64bit ubuntu 9.04 machine(except for the problem with html.py, which was already reported as a separate issue). Also, sage -docbuild reference html worked without errors.\n\nBoth patches need to be applied.",
    "created_at": "2009-06-03T12:22:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6014#issuecomment-39276",
    "user": "https://github.com/wdjoyner"
}
```

<div id="comment:4" align="right">comment:4</div>

This implements the referee's suggestions, passes sage -testall on 4.0.1.a0 on a 64bit ubuntu 9.04 machine(except for the problem with html.py, which was already reported as a separate issue). Also, sage -docbuild reference html worked without errors.

Both patches need to be applied.



---

archive/issue_events_071536.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2009-06-03T12:22:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6014#event-71536"
}
```



---

archive/issue_events_071537.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2009-06-03T12:22:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6014#event-71537"
}
```



---

archive/issue_comments_039277.json:
```json
{
    "body": "Attachment: **[trac_6014-hexads3.patch.gz](https://github.com/sagemath/sage/files/ticket6014/trac_6014-hexads3.patch.gz)**\n\nto be applied after the other two",
    "created_at": "2009-06-03T22:12:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6014#issuecomment-39277",
    "user": "https://github.com/wdjoyner"
}
```

Attachment: **[trac_6014-hexads3.patch.gz](https://github.com/sagemath/sage/files/ticket6014/trac_6014-hexads3.patch.gz)**

to be applied after the other two



---

archive/issue_comments_039278.json:
```json
{
    "body": "Attachment: **[trac_6014-hexads4.patch.gz](https://github.com/sagemath/sage/files/ticket6014/trac_6014-hexads4.patch.gz)**",
    "created_at": "2009-06-04T02:36:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6014#issuecomment-39278",
    "user": "https://github.com/wdjoyner"
}
```

Attachment: **[trac_6014-hexads4.patch.gz](https://github.com/sagemath/sage/files/ticket6014/trac_6014-hexads4.patch.gz)**



---

archive/issue_comments_039279.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nThese patches add the examples suggested by the referee.\n\nOne odd thing is that it appears that a list of elements containing infinity is sorted differently on an intel mac than on an ubuntu machine.\nBecause of this, I had to add a # random comment to a docstring.",
    "created_at": "2009-06-04T02:38:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6014#issuecomment-39279",
    "user": "https://github.com/wdjoyner"
}
```

<div id="comment:5" align="right">comment:5</div>

These patches add the examples suggested by the referee.

One odd thing is that it appears that a list of elements containing infinity is sorted differently on an intel mac than on an ubuntu machine.
Because of this, I had to add a # random comment to a docstring.



---

archive/issue_events_071538.json:
```json
{
    "actor": "https://github.com/sagetrac-dgordon",
    "created_at": "2009-06-04T14:06:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6014#event-71538"
}
```



---

archive/issue_events_071539.json:
```json
{
    "actor": "https://github.com/sagetrac-dgordon",
    "created_at": "2009-06-04T14:06:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6014#event-71539"
}
```



---

archive/issue_comments_039280.json:
```json
{
    "body": "Author: **David Joyner**",
    "created_at": "2009-06-13T21:45:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6014#issuecomment-39280",
    "user": "https://github.com/ncalexan"
}
```

Author: **David Joyner**



---

archive/issue_comments_039281.json:
```json
{
    "body": "Reviewer: **Dan Gordon**",
    "created_at": "2009-06-13T21:45:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6014#issuecomment-39281",
    "user": "https://github.com/ncalexan"
}
```

Reviewer: **Dan Gordon**



---

archive/issue_comments_039282.json:
```json
{
    "body": "Merged: **4.0.2.alpha0**",
    "created_at": "2009-06-13T21:45:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6014#issuecomment-39282",
    "user": "https://github.com/ncalexan"
}
```

Merged: **4.0.2.alpha0**



---

archive/issue_events_071540.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-13T21:45:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6014#event-71540"
}
```



---

archive/issue_events_071541.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-13T21:45:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6014",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6014#event-71541"
}
```

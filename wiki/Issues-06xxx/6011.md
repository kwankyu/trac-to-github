# Issue 6011: Bring plot/arrow.py to 100% coverage

archive/issues_006011.json:
```json
{
    "assignees": [
        "https://github.com/kcrisman"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nBring plot/arrow.py to 100% coverage\n\nComponent: **documentation**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6011_\n\n",
    "closed_at": "2009-05-11T09:02:40Z",
    "created_at": "2009-05-09T02:45:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bring plot/arrow.py to 100% coverage",
    "type": "issue",
    "updated_at": "2009-05-11T09:02:40Z",
    "url": "https://github.com/sagemath/sage/issues/6011",
    "user": "https://github.com/kcrisman"
}
```
<div id="comment:0"></div>

Bring plot/arrow.py to 100% coverage

Component: **documentation**

_Issue created by migration from https://trac.sagemath.org/ticket/6011_





---

archive/issue_events_071493.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-09T02:45:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6011#event-71493"
}
```



---

archive/issue_events_071494.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-09T02:45:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6011#event-71494"
}
```



---

archive/issue_events_071495.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-09T02:45:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6011#event-71495"
}
```



---

archive/issue_events_071496.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-09T02:45:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6011#event-71496"
}
```



---

archive/issue_events_071497.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-09T02:45:35Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "subject": "https://github.com/kcrisman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6011#event-71497"
}
```



---

archive/issue_comments_039256.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_6011.patch.gz](https://github.com/sagemath/sage/files/ticket6011/trac_6011.patch.gz)**\n\nAlso adds tiny plot3d enhancement (original doctest only tested Graphics.plot3d).\n\nPlease see [http://groups.google.com/group/sage-devel/browse_thread/thread/1adac4035031b140/c36f1f8a7c8a9b43#c36f1f8a7c8a9b43](http://groups.google.com/group/sage-devel/browse_thread/thread/1adac4035031b140/c36f1f8a7c8a9b43#c36f1f8a7c8a9b43) for why there is no loads(dumps()) test.  Yet.",
    "created_at": "2009-05-09T02:49:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6011#issuecomment-39256",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_6011.patch.gz](https://github.com/sagemath/sage/files/ticket6011/trac_6011.patch.gz)**

Also adds tiny plot3d enhancement (original doctest only tested Graphics.plot3d).

Please see [http://groups.google.com/group/sage-devel/browse_thread/thread/1adac4035031b140/c36f1f8a7c8a9b43#c36f1f8a7c8a9b43](http://groups.google.com/group/sage-devel/browse_thread/thread/1adac4035031b140/c36f1f8a7c8a9b43#c36f1f8a7c8a9b43) for why there is no loads(dumps()) test.  Yet.



---

archive/issue_events_071498.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-09T02:49:05Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "subject": "https://github.com/kcrisman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6011#event-71498"
}
```



---

archive/issue_events_071499.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-09T02:49:05Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "subject": "https://github.com/kcrisman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6011#event-71499"
}
```



---

archive/issue_events_071500.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-09T02:49:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6011#event-71500"
}
```



---

archive/issue_events_071501.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-09T02:52:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6011#event-71501"
}
```



---

archive/issue_events_071502.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-09T02:52:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6011#event-71502"
}
```



---

archive/issue_comments_039257.json:
```json
{
    "body": "Attachment: **[trac_6011-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket6011/trac_6011-reviewer.patch.gz)**\n\nreviewer patch; add consistency to the whole module",
    "created_at": "2009-05-09T04:10:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6011#issuecomment-39257",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[trac_6011-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket6011/trac_6011-reviewer.patch.gz)**

reviewer patch; add consistency to the whole module



---

archive/issue_comments_039258.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nREFEREE REPORT\n\n\n\nThe patch `trac_6011.patch` applies OK against the \"post-final\" version sage-3.4.2, all doctests pass with the options `-t -long`, and the doctest coverage is now 100% as claimed. Note that `sage/plot/arrow.py` is not included in the reference manual, so you can't search for it in order to view the documentation for `sage/plot/arrow.py`.\n\n\n\nThe patch adds some trivial inconsistencies in how \"two-dimensional\" and \"three-dimensional\" are abbreviated. These and other inconsistencies are fixed in the reviewer patch `trac_6011-reviewer.patch`. Apart from the issue of consistency throughout the whole module, positive review for kcrisman's patch. Only my patch needs to be reviewed.",
    "created_at": "2009-05-09T04:18:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6011#issuecomment-39258",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:3" align="right">comment:3</div>

REFEREE REPORT



The patch `trac_6011.patch` applies OK against the "post-final" version sage-3.4.2, all doctests pass with the options `-t -long`, and the doctest coverage is now 100% as claimed. Note that `sage/plot/arrow.py` is not included in the reference manual, so you can't search for it in order to view the documentation for `sage/plot/arrow.py`.



The patch adds some trivial inconsistencies in how "two-dimensional" and "three-dimensional" are abbreviated. These and other inconsistencies are fixed in the reviewer patch `trac_6011-reviewer.patch`. Apart from the issue of consistency throughout the whole module, positive review for kcrisman's patch. Only my patch needs to be reviewed.



---

archive/issue_events_071503.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-05-09T20:35:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6011#event-71503"
}
```



---

archive/issue_events_071504.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-05-09T20:35:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6011#event-71504"
}
```



---

archive/issue_events_071505.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-11T09:02:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6011#event-71505"
}
```



---

archive/issue_events_071506.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-11T09:02:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6011#event-71506"
}
```



---

archive/issue_comments_039259.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nMerged both patches in Sage 4.0.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-11T09:02:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6011",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6011#issuecomment-39259",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Merged both patches in Sage 4.0.alpha0.

Cheers,

Michael

# Issue 6239: fix ReST formatting for pydesign module ext_rep

archive/issues_006239.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-carlohamalainen"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis is a follow up to #6093. The patch on that ticket adds documentation to the pydesign module ext_rep. However, some of the formatting of the docstrings doesn't follow ReST formatting, hence doesn't show up nicely in the HTML and PDF versions. The module `sage/combinat/designs/ext_rep.py` should also be added to the reference manual.\n\nCC:  @sagetrac-carlohamalainen @wdjoyner\n\nComponent: **documentation**\n\nKeywords: **ReST, ext_rep.py**\n\nAuthor: **Minh Van Nguyen**\n\nReviewer: **Carlo Hamalainen**\n\nMerged: **sage-4.1.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6239_\n\n",
    "closed_at": "2009-06-24T09:58:45Z",
    "created_at": "2009-06-07T06:11:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix ReST formatting for pydesign module ext_rep",
    "type": "issue",
    "updated_at": "2009-06-24T09:58:45Z",
    "url": "https://github.com/sagemath/sage/issues/6239",
    "user": "https://github.com/sagetrac-mvngu"
}
```
<div id="comment:0"></div>

This is a follow up to #6093. The patch on that ticket adds documentation to the pydesign module ext_rep. However, some of the formatting of the docstrings doesn't follow ReST formatting, hence doesn't show up nicely in the HTML and PDF versions. The module `sage/combinat/designs/ext_rep.py` should also be added to the reference manual.

CC:  @sagetrac-carlohamalainen @wdjoyner

Component: **documentation**

Keywords: **ReST, ext_rep.py**

Author: **Minh Van Nguyen**

Reviewer: **Carlo Hamalainen**

Merged: **sage-4.1.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/6239_





---

archive/issue_events_074318.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-07T06:11:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6239",
    "milestone_number": null,
    "milestone_title": "sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6239#event-74318"
}
```



---

archive/issue_events_074319.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-07T06:11:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6239",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6239#event-74319"
}
```



---

archive/issue_events_074320.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-07T06:11:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6239",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6239#event-74320"
}
```



---

archive/issue_events_074321.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-07T06:11:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6239",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6239#event-74321"
}
```



---

archive/issue_events_074322.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-13T17:16:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6239",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6239#event-74322"
}
```



---

archive/issue_comments_041403.json:
```json
{
    "body": "Author: **Minh Van Nguyen**",
    "created_at": "2009-06-13T17:16:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6239",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6239#issuecomment-41403",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Minh Van Nguyen**



---

archive/issue_comments_041404.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_6239.patch.gz](https://github.com/sagemath/sage/files/ticket6239/trac_6239.patch.gz)**",
    "created_at": "2009-06-13T17:16:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6239",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6239#issuecomment-41404",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_6239.patch.gz](https://github.com/sagemath/sage/files/ticket6239/trac_6239.patch.gz)**



---

archive/issue_events_074323.json:
```json
{
    "actor": "https://github.com/sagetrac-carlohamalainen",
    "created_at": "2009-06-15T09:53:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6239",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6239#event-74323"
}
```



---

archive/issue_events_074324.json:
```json
{
    "actor": "https://github.com/sagetrac-carlohamalainen",
    "created_at": "2009-06-15T09:53:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6239",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6239#event-74324"
}
```



---

archive/issue_events_074325.json:
```json
{
    "actor": "https://github.com/sagetrac-carlohamalainen",
    "created_at": "2009-06-15T09:53:19Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6239",
    "subject": "https://github.com/sagetrac-carlohamalainen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6239#event-74325"
}
```



---

archive/issue_comments_041405.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThe patch applies cleanly to 4.0.1, the formatting looks good, and \"sage -docbuild reference html\" worked.",
    "created_at": "2009-06-15T09:53:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6239",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6239#issuecomment-41405",
    "user": "https://github.com/sagetrac-carlohamalainen"
}
```

<div id="comment:2" align="right">comment:2</div>

The patch applies cleanly to 4.0.1, the formatting looks good, and "sage -docbuild reference html" worked.



---

archive/issue_events_074326.json:
```json
{
    "actor": "https://github.com/sagetrac-carlohamalainen",
    "created_at": "2009-06-15T09:53:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6239",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6239#event-74326"
}
```



---

archive/issue_comments_041406.json:
```json
{
    "body": "Reviewer: **Carlo Hamalainen**",
    "created_at": "2009-06-15T09:56:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6239",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6239#issuecomment-41406",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **Carlo Hamalainen**



---

archive/issue_events_074327.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-24T09:58:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6239",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6239#event-74327"
}
```



---

archive/issue_comments_041407.json:
```json
{
    "body": "Merged: **sage-4.1.alpha0**",
    "created_at": "2009-06-24T09:58:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6239",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6239#issuecomment-41407",
    "user": "https://github.com/rlmill"
}
```

Merged: **sage-4.1.alpha0**

# Issue 6509: sum of four squares function

archive/issues_006509.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nThis can probably be improved, but its a first attempt that seems to work.\n\nComponent: **basic arithmetic**\n\nReviewer: **jsp**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6509_\n\n",
    "closed_at": "2009-07-18T18:34:01Z",
    "created_at": "2009-07-10T20:19:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sum of four squares function",
    "type": "issue",
    "updated_at": "2009-07-21T01:59:20Z",
    "url": "https://github.com/sagemath/sage/issues/6509",
    "user": "https://github.com/sagetrac-mhampton"
}
```
<div id="comment:0"></div>

This can probably be improved, but its a first attempt that seems to work.

Component: **basic arithmetic**

Reviewer: **jsp**

_Issue created by migration from https://trac.sagemath.org/ticket/6509_





---

archive/issue_events_078105.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-10T20:19:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6509",
    "milestone_number": null,
    "milestone_title": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6509#event-78105"
}
```



---

archive/issue_events_078106.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-10T20:19:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6509",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "c: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6509#event-78106"
}
```



---

archive/issue_events_078107.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-10T20:19:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6509",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6509#event-78107"
}
```



---

archive/issue_events_078108.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-10T20:19:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6509",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6509#event-78108"
}
```



---

archive/issue_events_078109.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-10T20:22:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6509",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6509#event-78109"
}
```



---

archive/issue_comments_044798.json:
```json
{
    "body": "Attachment: **[trac_6509_four_squares.patch.gz](https://github.com/sagemath/sage/files/ticket6509/trac_6509_four_squares.patch.gz)**",
    "created_at": "2009-07-10T21:08:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6509#issuecomment-44798",
    "user": "https://github.com/sagetrac-mhampton"
}
```

Attachment: **[trac_6509_four_squares.patch.gz](https://github.com/sagemath/sage/files/ticket6509/trac_6509_four_squares.patch.gz)**



---

archive/issue_comments_044799.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nOops, last one had a few mistakes.  Current version is correct up to 10000 I think with the new \"long\" doctest.",
    "created_at": "2009-07-10T21:09:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6509#issuecomment-44799",
    "user": "https://github.com/sagetrac-mhampton"
}
```

<div id="comment:2" align="right">comment:2</div>

Oops, last one had a few mistakes.  Current version is correct up to 10000 I think with the new "long" doctest.



---

archive/issue_events_078110.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2009-07-10T21:23:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6509",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6509#event-78110"
}
```



---

archive/issue_events_078111.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2009-07-10T21:23:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6509",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6509#event-78111"
}
```



---

archive/issue_comments_044800.json:
```json
{
    "body": "Changed reviewer from **wstein** to **jsp**",
    "created_at": "2009-07-10T21:23:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6509#issuecomment-44800",
    "user": "https://github.com/jaapspies"
}
```

Changed reviewer from **wstein** to **jsp**



---

archive/issue_comments_044801.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nTested this with a lot of values taken from:\n[http://www.research.att.com/~njas/sequences/A006431](http://www.research.att.com/~njas/sequences/A006431)\n\nand usage of:\n\n[http://www.alpertron.com.ar/FSQUARES.HTM](http://www.alpertron.com.ar/FSQUARES.HTM)\n\nWhen we look at [http://www.research.att.com/~njas/sequences/A002635](http://www.research.att.com/~njas/sequences/A002635)\nwe see that for instance 82 can be written in 7 ways as the sum of\n4 squares. For more results see:\n[http://www.research.att.com/~njas/sequences/b002635.txt](http://www.research.att.com/~njas/sequences/b002635.txt)\n\nSo indeed this can be improved. But for now this is ok for a first attempt.\nPositive review.\n\nJaap",
    "created_at": "2009-07-10T21:23:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6509#issuecomment-44801",
    "user": "https://github.com/jaapspies"
}
```

<div id="comment:3" align="right">comment:3</div>

Tested this with a lot of values taken from:
[http://www.research.att.com/~njas/sequences/A006431](http://www.research.att.com/~njas/sequences/A006431)

and usage of:

[http://www.alpertron.com.ar/FSQUARES.HTM](http://www.alpertron.com.ar/FSQUARES.HTM)

When we look at [http://www.research.att.com/~njas/sequences/A002635](http://www.research.att.com/~njas/sequences/A002635)
we see that for instance 82 can be written in 7 ways as the sum of
4 squares. For more results see:
[http://www.research.att.com/~njas/sequences/b002635.txt](http://www.research.att.com/~njas/sequences/b002635.txt)

So indeed this can be improved. But for now this is ok for a first attempt.
Positive review.

Jaap



---

archive/issue_comments_044802.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThis is a duplicate of ticket #6529. That ticket also incorporates the code contained in `trac_6509_four_squares.patch`.",
    "created_at": "2009-07-18T18:34:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6509#issuecomment-44802",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:4" align="right">comment:4</div>

This is a duplicate of ticket #6529. That ticket also incorporates the code contained in `trac_6509_four_squares.patch`.



---

archive/issue_events_078112.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-18T18:34:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6509",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6509#event-78112"
}
```



---

archive/issue_events_078113.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-18T18:34:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6509",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6509#event-78113"
}
```



---

archive/issue_events_078114.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-18T18:34:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6509",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6509#event-78114"
}
```



---

archive/issue_events_078115.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-18T18:34:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6509",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "c6c6c6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6509#event-78115"
}
```



---

archive/issue_events_078116.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-18T18:34:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6509",
    "milestone_number": null,
    "milestone_title": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6509#event-78116"
}
```

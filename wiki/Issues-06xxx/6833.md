# Issue 6833: use revision control for worksheet history

archive/issues_006833.json:
```json
{
    "assignees": [
        "https://github.com/boothby"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nSage ships an excellent revision control software but uses a rather simple approach to worksheet version-ing. It would be nice if a hg repository was used behind the scenes. This would enable diffs, cool sharing (just pull my worksheet) and collaboration.\n\nUpstream: **Reported upstream. Developers acknowledge bug.**\n\nCC:  @kini\n\nComponent: **notebook**\n\nKeywords: **notebook, mercurial, hg**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6833_\n\n",
    "closed_at": "2020-03-29T02:12:30Z",
    "created_at": "2009-08-27T23:21:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wishlist%20item"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "use revision control for worksheet history",
    "type": "issue",
    "updated_at": "2020-03-29T02:12:30Z",
    "url": "https://github.com/sagemath/sage/issues/6833",
    "user": "https://github.com/malb"
}
```
<div id="comment:0"></div>

Sage ships an excellent revision control software but uses a rather simple approach to worksheet version-ing. It would be nice if a hg repository was used behind the scenes. This would enable diffs, cool sharing (just pull my worksheet) and collaboration.

Upstream: **Reported upstream. Developers acknowledge bug.**

CC:  @kini

Component: **notebook**

Keywords: **notebook, mercurial, hg**

_Issue created by migration from https://trac.sagemath.org/ticket/6833_





---

archive/issue_events_082328.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-08-27T23:21:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6833#event-82328"
}
```



---

archive/issue_events_082329.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-08-27T23:21:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6833#event-82329"
}
```



---

archive/issue_events_082330.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-08-27T23:21:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6833#event-82330"
}
```



---

archive/issue_events_082331.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-08-27T23:21:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6833#event-82331"
}
```



---

archive/issue_events_082332.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-08-27T23:21:32Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "subject": "https://github.com/malb",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6833#event-82332"
}
```



---

archive/issue_comments_048389.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nLet's just close this ticket.",
    "created_at": "2014-06-25T00:56:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6833#issuecomment-48389",
    "user": "https://github.com/malb"
}
```

<div id="comment:2" align="right">comment:2</div>

Let's just close this ticket.



---

archive/issue_events_082333.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-08-18T21:59:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6833#event-82333"
}
```



---

archive/issue_events_082334.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-08-18T21:59:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6833#event-82334"
}
```



---

archive/issue_events_082335.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-08-27T15:37:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6833#event-82335"
}
```



---

archive/issue_events_082336.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-08-27T15:37:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6833#event-82336"
}
```



---

archive/issue_events_082337.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-08-27T15:37:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6833#event-82337"
}
```



---

archive/issue_comments_048390.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nhttps://github.com/sagemath/sagenb/issues/216",
    "created_at": "2014-08-27T15:37:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6833#issuecomment-48390",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:4" align="right">comment:4</div>

https://github.com/sagemath/sagenb/issues/216



---

archive/issue_comments_048391.json:
```json
{
    "body": "Upstream: **Reported upstream. Developers acknowledge bug.**",
    "created_at": "2014-08-27T15:37:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6833#issuecomment-48391",
    "user": "https://github.com/kcrisman"
}
```

Upstream: **Reported upstream. Developers acknowledge bug.**



---

archive/issue_events_082338.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-08-27T15:38:01Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "title_is": "use revision control for worksheet history",
    "title_was": "use hg for worksheet history",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6833#event-82338"
}
```



---

archive/issue_comments_048392.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nSome other ideas from #301:\n\n* Show history without code lines.\n* Show history without SAGE command line markup\n* Show history by time\n* Show activity graphs by time\n* Show history for worksheets\n* Show history for cell in worksheet\n  * Show differences between two edits in a cell\n\nNaturally not all of these would be implemented here, but any that aren't could be moved to another ticket.  Should sagenb, or the universe, last that long.",
    "created_at": "2014-11-20T14:02:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6833#issuecomment-48392",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:6" align="right">comment:6</div>

Some other ideas from #301:

* Show history without code lines.
* Show history without SAGE command line markup
* Show history by time
* Show activity graphs by time
* Show history for worksheets
* Show history for cell in worksheet
  * Show differences between two edits in a cell

Naturally not all of these would be implemented here, but any that aren't could be moved to another ticket.  Should sagenb, or the universe, last that long.



---

archive/issue_events_082339.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2020-03-29T02:12:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6833#event-82339"
}
```



---

archive/issue_events_082340.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2020-03-29T02:12:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6833#event-82340"
}
```



---

archive/issue_comments_048393.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nClosing deprecated notebook tickets",
    "created_at": "2020-03-29T02:12:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6833",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6833#issuecomment-48393",
    "user": "https://github.com/boothby"
}
```

<div id="comment:7" align="right">comment:7</div>

Closing deprecated notebook tickets

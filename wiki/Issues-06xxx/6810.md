# Issue 6810: improve doctest coverage in schemes/homset.py

archive/issues_006810.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe current coverage is 8% and should be brought up to 100%.\n\nAlso, there are small but annoying bugs such as: the methods that enumerate the points on schemes over finite fields do not sort the list before returning it (this issue is addressed by #9055).\n\n\nDepends on #11599\n\nCC:  @vbraun\n\nComponent: **algebraic geometry**\n\nKeywords: **doctest schemes homset**\n\nReviewer: **Volker Braun, Andrey Novoseltsev**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6810_\n\n",
    "closed_at": "2012-02-24T15:05:17Z",
    "created_at": "2009-08-23T01:57:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "improve doctest coverage in schemes/homset.py",
    "type": "issue",
    "updated_at": "2012-02-24T15:05:17Z",
    "url": "https://github.com/sagemath/sage/issues/6810",
    "user": "https://github.com/aghitza"
}
```
<div id="comment:0"></div>

The current coverage is 8% and should be brought up to 100%.

Also, there are small but annoying bugs such as: the methods that enumerate the points on schemes over finite fields do not sort the list before returning it (this issue is addressed by #9055).


Depends on #11599

CC:  @vbraun

Component: **algebraic geometry**

Keywords: **doctest schemes homset**

Reviewer: **Volker Braun, Andrey Novoseltsev**

_Issue created by migration from https://trac.sagemath.org/ticket/6810_





---

archive/issue_events_082004.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-08-23T01:57:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6810#event-82004"
}
```



---

archive/issue_events_082005.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-08-23T01:57:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "c: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6810#event-82005"
}
```



---

archive/issue_events_082006.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-08-23T01:57:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6810#event-82006"
}
```



---

archive/issue_events_082007.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-08-23T01:57:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6810#event-82007"
}
```



---

archive/issue_events_082008.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-08-23T01:57:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6810#event-82008"
}
```



---

archive/issue_events_082009.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-08-23T01:57:17Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "subject": "https://github.com/aghitza",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6810#event-82009"
}
```



---

archive/issue_events_082010.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2010-11-01T02:54:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6810#event-82010"
}
```



---

archive/issue_events_082011.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2010-11-01T02:54:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6810#event-82011"
}
```



---

archive/issue_comments_048064.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n The current coverage is 8% and should be brought up to 100%.\n \n-Also, there are small but annoying bugs such as: the methods that enumerate the points on schemes over finite fields do not sort the list before returning it.\n+Also, there are small but annoying bugs such as: the methods that enumerate the points on schemes over finite fields do not sort the list before returning it (this issue is addressed by #9055).\n \n``````\n",
    "created_at": "2010-11-01T02:54:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6810#issuecomment-48064",
    "user": "https://github.com/novoselt"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
 The current coverage is 8% and should be brought up to 100%.
 
-Also, there are small but annoying bugs such as: the methods that enumerate the points on schemes over finite fields do not sort the list before returning it.
+Also, there are small but annoying bugs such as: the methods that enumerate the points on schemes over finite fields do not sort the list before returning it (this issue is addressed by #9055).
 
``````




---

archive/issue_comments_048065.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nShould be fixed in #11599.",
    "created_at": "2011-12-29T18:19:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6810#issuecomment-48065",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:2" align="right">comment:2</div>

Should be fixed in #11599.



---

archive/issue_events_082012.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-02-20T00:29:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6810#event-82012"
}
```



---

archive/issue_comments_048066.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nIndeed, it is fixed there!",
    "created_at": "2012-02-20T00:29:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6810#issuecomment-48066",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:3" align="right">comment:3</div>

Indeed, it is fixed there!



---

archive/issue_comments_048067.json:
```json
{
    "body": "Dependencies: **#11599**",
    "created_at": "2012-02-20T00:29:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6810#issuecomment-48067",
    "user": "https://github.com/novoselt"
}
```

Dependencies: **#11599**



---

archive/issue_events_082013.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-02-20T00:30:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6810#event-82013"
}
```



---

archive/issue_events_082014.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-02-20T00:30:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6810#event-82014"
}
```



---

archive/issue_comments_048068.json:
```json
{
    "body": "Reviewer: **Andrey Novoseltsev**",
    "created_at": "2012-02-20T00:30:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6810#issuecomment-48068",
    "user": "https://github.com/novoselt"
}
```

Reviewer: **Andrey Novoseltsev**



---

archive/issue_comments_048069.json:
```json
{
    "body": "Author: **Volker Braun**",
    "created_at": "2012-02-20T00:30:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6810#issuecomment-48069",
    "user": "https://github.com/novoselt"
}
```

Author: **Volker Braun**



---

archive/issue_events_082015.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-24T15:05:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6810#event-82015"
}
```



---

archive/issue_events_082016.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-24T15:05:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "c6c6c6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6810#event-82016"
}
```



---

archive/issue_comments_048070.json:
```json
{
    "body": "Changed author from **Volker Braun** to none",
    "created_at": "2012-02-24T15:05:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6810#issuecomment-48070",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Volker Braun** to none



---

archive/issue_events_082017.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-24T15:05:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6810#event-82017"
}
```



---

archive/issue_events_082018.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-24T15:05:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6810#event-82018"
}
```



---

archive/issue_comments_048071.json:
```json
{
    "body": "Changed reviewer from **Andrey Novoseltsev** to **Volker Braun, Andrey Novoseltsev**",
    "created_at": "2012-02-24T15:05:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6810#issuecomment-48071",
    "user": "https://github.com/jdemeyer"
}
```

Changed reviewer from **Andrey Novoseltsev** to **Volker Braun, Andrey Novoseltsev**



---

archive/issue_events_082019.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-24T15:05:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6810",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6810#event-82019"
}
```

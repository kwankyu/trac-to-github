# Issue 6227: Remove python_* files in ext directory

archive/issues_006227.json:
```json
{
    "body": "from Robertwb:\n\n```\n> Ext\n>  - Yes, we should remove all of the python_* files now that they're\n> shipped with Cython. It just hasn't been done yet (should be easy,\n> but not sure what the fallout might be).\n```\n\n\n**Assignee:** cwitty\n\nIssue created by migration from https://trac.sagemath.org/ticket/6227\n\n",
    "closed_at": "2010-01-14T05:50:46Z",
    "created_at": "2009-06-05T17:17:03Z",
    "labels": [
        "component: misc",
        "enhancement",
        "duplicate"
    ],
    "title": "Remove python_* files in ext directory",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/6227",
    "user": "https://github.com/williamstein"
}
```
from Robertwb:

```
> Ext
>  - Yes, we should remove all of the python_* files now that they're
> shipped with Cython. It just hasn't been done yet (should be easy,
> but not sure what the fallout might be).
```


**Assignee:** cwitty

Issue created by migration from https://trac.sagemath.org/ticket/6227





---

archive/issue_events_040554.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-01-14T05:50:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6227",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6227#event-40554"
}
```



---

archive/issue_comments_042708.json:
```json
{
    "body": "<a id='comment:1'></a>\nDupe of #6223.",
    "created_at": "2010-01-14T05:50:46Z",
    "issue": "https://github.com/sagemath/sage/issues/6227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6227#issuecomment-42708",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:1'></a>
Dupe of #6223.



---

archive/issue_events_040555.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-01-14T05:50:46Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6227",
    "milestone": "sage-4.3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6227#event-40555"
}
```



---

archive/issue_events_040556.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-01-14T05:50:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6227",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6227#event-40556"
}
```

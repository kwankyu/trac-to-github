# Issue 6125: [with spkgs] disable testing in flint, mpir, etc. packages

archive/issues_006125.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nIn 4.0.rc0, at least the FLINT spkg has tests enabled. These should be disabled before the release.\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6125_\n\n",
    "closed_at": "2009-05-29T17:31:03Z",
    "created_at": "2009-05-24T15:50:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "[with spkgs] disable testing in flint, mpir, etc. packages",
    "type": "issue",
    "updated_at": "2009-05-29T17:31:03Z",
    "url": "https://github.com/sagemath/sage/issues/6125",
    "user": "https://github.com/burcin"
}
```
<div id="comment:0"></div>

In 4.0.rc0, at least the FLINT spkg has tests enabled. These should be disabled before the release.

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/6125_





---

archive/issue_events_073023.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-05-24T15:50:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6125",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6125#event-73023"
}
```



---

archive/issue_events_073024.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-05-24T15:50:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6125",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "0000b0",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6125#event-73024"
}
```



---

archive/issue_events_073025.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-05-24T15:50:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6125",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6125#event-73025"
}
```



---

archive/issue_events_073026.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-05-24T15:50:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6125",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6125#event-73026"
}
```



---

archive/issue_events_073027.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-24T15:50:12Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6125",
    "subject": "https://github.com/burcin",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6125#event-73027"
}
```



---

archive/issue_events_073028.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-29T04:26:46Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/6125",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6125#event-73028"
}
```



---

archive/issue_events_073029.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-29T04:26:46Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6125",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6125#event-73029"
}
```



---

archive/issue_events_073030.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-29T04:26:46Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/6125",
    "title_is": "[with spkgs] disable testing in flint, mpir, etc. packages",
    "title_was": "disable testing in flint, mpir, etc. packages",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6125#event-73030"
}
```



---

archive/issue_events_073031.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-29T04:26:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6125",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6125#event-73031"
}
```



---

archive/issue_comments_040435.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nI have http://sage.math.washington.edu/home/mhansen/flint-1.2.4.p3.spkg and http://sage.math.washington.edu/home/mhansen/gmp-mpir-1.1.2.spkg which disable the tests.",
    "created_at": "2009-05-29T04:26:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6125",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6125#issuecomment-40435",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

I have http://sage.math.washington.edu/home/mhansen/flint-1.2.4.p3.spkg and http://sage.math.washington.edu/home/mhansen/gmp-mpir-1.1.2.spkg which disable the tests.



---

archive/issue_events_073032.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-29T04:26:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6125",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6125#event-73032"
}
```



---

archive/issue_events_073033.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-29T13:30:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6125",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6125#event-73033"
}
```



---

archive/issue_comments_040436.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI've slightly modified the comment in spkg-install for both of the above spkg to make sense.  The logic is identical.  The modified spkgs:\n\nhttp://sage.math.washington.edu/home/wstein/tmp/gmp-mpir-1.1.2.spkg\n\nhttp://sage.math.washington.edu/home/wstein/tmp/flint-1.2.4.p4.spkg",
    "created_at": "2009-05-29T13:30:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6125",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6125#issuecomment-40436",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">comment:2</div>

I've slightly modified the comment in spkg-install for both of the above spkg to make sense.  The logic is identical.  The modified spkgs:

http://sage.math.washington.edu/home/wstein/tmp/gmp-mpir-1.1.2.spkg

http://sage.math.washington.edu/home/wstein/tmp/flint-1.2.4.p4.spkg



---

archive/issue_events_073034.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-29T17:31:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6125",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6125#event-73034"
}
```



---

archive/issue_events_073035.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-29T17:31:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6125",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6125#event-73035"
}
```



---

archive/issue_comments_040437.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in 4.0.rc2.",
    "created_at": "2009-05-29T17:31:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6125",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6125#issuecomment-40437",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in 4.0.rc2.

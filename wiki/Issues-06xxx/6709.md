# Issue 6709: There are many doctest failues on Solaris 32-bit builds.

archive/issues_006709.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-drkirkby"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nI'll post a couple of logs later, but thought I'd create a ticket for this. \n\nCC:  david.kirkby@onetel.net @dimpase\n\nComponent: **porting: Solaris**\n\nReviewer: **Michael Orlitzky**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6709_\n\n",
    "closed_at": "2020-07-15T07:18:41Z",
    "created_at": "2009-08-09T10:24:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20solaris",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "There are many doctest failues on Solaris 32-bit builds.",
    "type": "issue",
    "updated_at": "2020-07-15T07:18:41Z",
    "url": "https://github.com/sagemath/sage/issues/6709",
    "user": "https://github.com/sagetrac-drkirkby"
}
```
<div id="comment:0"></div>

I'll post a couple of logs later, but thought I'd create a ticket for this. 

CC:  david.kirkby@onetel.net @dimpase

Component: **porting: Solaris**

Reviewer: **Michael Orlitzky**

_Issue created by migration from https://trac.sagemath.org/ticket/6709_





---

archive/issue_events_080775.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2009-08-09T10:24:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80775"
}
```



---

archive/issue_events_080776.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2009-08-09T10:24:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20solaris",
    "label_color": "0000b0",
    "label_name": "c: porting: solaris",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80776"
}
```



---

archive/issue_events_080777.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2009-08-09T10:24:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80777"
}
```



---

archive/issue_events_080778.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2009-08-09T10:24:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80778"
}
```



---

archive/issue_comments_047019.json:
```json
{
    "body": "Test failues on sage-4.1.1.rc0 with Maxima 5.19.0",
    "created_at": "2009-08-09T18:43:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6709#issuecomment-47019",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

Test failues on sage-4.1.1.rc0 with Maxima 5.19.0



---

archive/issue_comments_047020.json:
```json
{
    "body": "Attachment: **[sun4u-S10u7-test.log](https://github.com/sagemath/sage/files/ticket6709/sun4u-S10u7-test.log)**\n\nAttachment: **[test.log](https://github.com/sagemath/sage/files/ticket6709/test.log)**\n\ntest.log on SPARC after bug fix to ECL 9.8.1",
    "created_at": "2009-08-11T09:56:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6709#issuecomment-47020",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

Attachment: **[sun4u-S10u7-test.log](https://github.com/sagemath/sage/files/ticket6709/sun4u-S10u7-test.log)**

Attachment: **[test.log](https://github.com/sagemath/sage/files/ticket6709/test.log)**

test.log on SPARC after bug fix to ECL 9.8.1



---

archive/issue_comments_047021.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAfter adding the bug fix to sysfun.lsp which was added by one of ECL developers on the 10th August 2009, here is the output on a SPARC box running Solaris 10 update 7.",
    "created_at": "2009-08-11T10:01:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6709#issuecomment-47021",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:1" align="right">comment:1</div>

After adding the bug fix to sysfun.lsp which was added by one of ECL developers on the 10th August 2009, here is the output on a SPARC box running Solaris 10 update 7.



---

archive/issue_comments_047022.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI added \"32-bit\" to the title, as this ticket was created when Sage was only building 64-bit. \n\nIt can be closed as fixed, as all doctests have passed on Solaris or OpenSolaris both SPARC and x86 for a long time.",
    "created_at": "2011-04-02T12:35:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6709#issuecomment-47022",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:3" align="right">comment:3</div>

I added "32-bit" to the title, as this ticket was created when Sage was only building 64-bit. 

It can be closed as fixed, as all doctests have passed on Solaris or OpenSolaris both SPARC and x86 for a long time.



---

archive/issue_events_080779.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-02T12:35:58Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "title_is": "There are many doctest failues on Solaris 32-bit builds.",
    "title_was": "There are many doctest failues on Solaris.",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80779"
}
```



---

archive/issue_events_080780.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-02T12:35:58Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "subject": "https://github.com/sagetrac-drkirkby",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80780"
}
```



---

archive/issue_comments_047023.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nReplying to [@sagetrac-drkirkby](#comment%3A3):\n> I added \"32-bit\" to the title, as this ticket was created when Sage was only building 64-bit. \n\nI mean the ticket was opened when Sage was only building 32-bit. Now it will build 64-bit, but does not work very well in 64-bit mode.",
    "created_at": "2011-04-02T12:37:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6709#issuecomment-47023",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:4" align="right">comment:4</div>

Replying to [@sagetrac-drkirkby](#comment%3A3):
> I added "32-bit" to the title, as this ticket was created when Sage was only building 64-bit. 

I mean the ticket was opened when Sage was only building 32-bit. Now it will build 64-bit, but does not work very well in 64-bit mode.



---

archive/issue_events_080781.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80781"
}
```



---

archive/issue_events_080782.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80782"
}
```



---

archive/issue_events_080783.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80783"
}
```



---

archive/issue_events_080784.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80784"
}
```



---

archive/issue_events_080785.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80785"
}
```



---

archive/issue_events_080786.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80786"
}
```



---

archive/issue_events_080787.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80787"
}
```



---

archive/issue_events_080788.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80788"
}
```



---

archive/issue_events_080789.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-08T16:51:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80789"
}
```



---

archive/issue_events_080790.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-08T16:51:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80790"
}
```



---

archive/issue_comments_047024.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nOutdated, should be closed",
    "created_at": "2020-07-08T16:51:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6709#issuecomment-47024",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:9" align="right">comment:9</div>

Outdated, should be closed



---

archive/issue_events_080791.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-07-12T19:57:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80791"
}
```



---

archive/issue_events_080792.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2020-07-12T19:57:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80792"
}
```



---

archive/issue_comments_047025.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nThe goal of these tickets is laudable, but:\n\n* We need at least one user who is able to test.\n* The package/OS information on this ticket is outdated beyond usefulness.\n* Upstream is a better place to report portability issues these days.",
    "created_at": "2020-07-12T19:57:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6709#issuecomment-47025",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:10" align="right">comment:10</div>

The goal of these tickets is laudable, but:

* We need at least one user who is able to test.
* The package/OS information on this ticket is outdated beyond usefulness.
* Upstream is a better place to report portability issues these days.



---

archive/issue_comments_047026.json:
```json
{
    "body": "Reviewer: **Michael Orlitzky**",
    "created_at": "2020-07-12T19:57:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6709#issuecomment-47026",
    "user": "https://github.com/orlitzky"
}
```

Reviewer: **Michael Orlitzky**



---

archive/issue_events_080793.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-07-15T07:18:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80793"
}
```



---

archive/issue_events_080794.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-07-15T07:18:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6709#event-80794"
}
```



---

archive/issue_comments_047027.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nClosing very old sun/solaris tickets. Any tentative for this OS should start afresh.",
    "created_at": "2020-07-15T07:18:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6709",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6709#issuecomment-47027",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:11" align="right">comment:11</div>

Closing very old sun/solaris tickets. Any tentative for this OS should start afresh.

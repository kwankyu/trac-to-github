# Issue 6024: clisp-->ecl switch

archive/issues_006024.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThis is required for gcc 4.4.0, Solaris and so on.\n\nCheers,\n\nMichael\n\nComponent: **porting**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6024_\n\n",
    "closed_at": "2009-05-16T00:27:14Z",
    "created_at": "2009-05-12T06:11:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20porting",
        "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "clisp-->ecl switch",
    "type": "issue",
    "updated_at": "2009-05-16T00:27:14Z",
    "url": "https://github.com/sagemath/sage/issues/6024",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
<div id="comment:0"></div>

This is required for gcc 4.4.0, Solaris and so on.

Cheers,

Michael

Component: **porting**

_Issue created by migration from https://trac.sagemath.org/ticket/6024_





---

archive/issue_events_071684.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-12T06:11:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6024#event-71684"
}
```



---

archive/issue_events_071685.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-12T06:11:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20porting",
    "label_color": "0000b0",
    "label_name": "c: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6024#event-71685"
}
```



---

archive/issue_events_071686.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-12T06:11:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6024#event-71686"
}
```



---

archive/issue_events_071687.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-12T06:11:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6024#event-71687"
}
```



---

archive/issue_events_071688.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-12T06:11:52Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6024#event-71688"
}
```



---

archive/issue_comments_039404.json:
```json
{
    "body": "Attachment: **[trac_6024-doctest-fix.patch.gz](https://github.com/sagemath/sage/files/ticket6024/trac_6024-doctest-fix.patch.gz)**",
    "created_at": "2009-05-12T06:14:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6024#issuecomment-39404",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Attachment: **[trac_6024-doctest-fix.patch.gz](https://github.com/sagemath/sage/files/ticket6024/trac_6024-doctest-fix.patch.gz)**



---

archive/issue_events_071689.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-12T07:40:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6024#event-71689"
}
```



---

archive/issue_events_071690.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-12T07:40:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6024#event-71690"
}
```



---

archive/issue_comments_039405.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nThe new Maxima is at \n\n  http://sage.math.washington.edu/home/mabshoff/release-cycles-4.0/alpha0/maxima-5.16.3.p2.spkg\n\nNote that the ecl.spkg is still missing and will be next.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-15T14:45:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6024#issuecomment-39405",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

The new Maxima is at 

  http://sage.math.washington.edu/home/mabshoff/release-cycles-4.0/alpha0/maxima-5.16.3.p2.spkg

Note that the ecl.spkg is still missing and will be next.

Cheers,

Michael



---

archive/issue_events_071691.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-15T14:45:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6024#event-71691"
}
```



---

archive/issue_comments_039406.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThe ecl.spkg skpg that now finally works is at \n\n   http://sage.math.washington.edu/home/mabshoff/release-cycles-4.0/alpha0/ecl-9.4.1.spkg\n\nCheers,\n\nMichael",
    "created_at": "2009-05-15T20:35:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6024#issuecomment-39406",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

The ecl.spkg skpg that now finally works is at 

   http://sage.math.washington.edu/home/mabshoff/release-cycles-4.0/alpha0/ecl-9.4.1.spkg

Cheers,

Michael



---

archive/issue_comments_039407.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nPositive review pending:\n\n1. Remove the msvc directory for now.\n\n2. Put \"unset MAKE\" at the top of spkg-install for now, since it definitely breaks if one doesn't do that.",
    "created_at": "2009-05-16T00:14:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6024#issuecomment-39407",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:5" align="right">comment:5</div>

Positive review pending:

1. Remove the msvc directory for now.

2. Put "unset MAKE" at the top of spkg-install for now, since it definitely breaks if one doesn't do that.



---

archive/issue_events_071692.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-16T00:14:40Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "title_is": "clisp-->ecl switch",
    "title_was": "ecl->clisp switch",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6024#event-71692"
}
```



---

archive/issue_events_071693.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-16T00:14:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6024#event-71693"
}
```



---

archive/issue_events_071694.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-16T00:14:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6024#event-71694"
}
```



---

archive/issue_comments_039408.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nok, full positive review.",
    "created_at": "2009-05-16T00:17:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6024#issuecomment-39408",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:6" align="right">comment:6</div>

ok, full positive review.



---

archive/issue_comments_039409.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nMerged both spkgs and the patch in Sage 4.0.alpha0. \n\nI also fixed deps and install accordingly.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-16T00:27:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6024#issuecomment-39409",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:7" align="right">comment:7</div>

Merged both spkgs and the patch in Sage 4.0.alpha0. 

I also fixed deps and install accordingly.

Cheers,

Michael



---

archive/issue_events_071695.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-16T00:27:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6024#event-71695"
}
```



---

archive/issue_events_071696.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-16T00:27:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6024",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6024#event-71696"
}
```

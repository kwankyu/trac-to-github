# Issue 6009: Bring plot/text.py to 100%

archive/issues_006009.json:
```json
{
    "assignees": [
        "https://github.com/kcrisman"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nBring plot/text.py to 100%.\n\nComponent: **doctest coverage**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6009_\n\n",
    "closed_at": "2009-05-11T08:52:14Z",
    "created_at": "2009-05-08T19:14:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bring plot/text.py to 100%",
    "type": "issue",
    "updated_at": "2009-05-11T08:52:14Z",
    "url": "https://github.com/sagemath/sage/issues/6009",
    "user": "https://github.com/kcrisman"
}
```
<div id="comment:0"></div>

Bring plot/text.py to 100%.

Component: **doctest coverage**

_Issue created by migration from https://trac.sagemath.org/ticket/6009_





---

archive/issue_events_071468.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-08T19:14:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6009#event-71468"
}
```



---

archive/issue_events_071469.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-08T19:14:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6009#event-71469"
}
```



---

archive/issue_events_071470.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-08T19:14:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6009#event-71470"
}
```



---

archive/issue_events_071471.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-08T19:14:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6009#event-71471"
}
```



---

archive/issue_events_071472.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-08T19:14:56Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "subject": "https://github.com/kcrisman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6009#event-71472"
}
```



---

archive/issue_comments_039241.json:
```json
{
    "body": "Attachment: **[trac_6009.patch.gz](https://github.com/sagemath/sage/files/ticket6009/trac_6009.patch.gz)**",
    "created_at": "2009-05-08T19:16:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6009#issuecomment-39241",
    "user": "https://github.com/kcrisman"
}
```

Attachment: **[trac_6009.patch.gz](https://github.com/sagemath/sage/files/ticket6009/trac_6009.patch.gz)**



---

archive/issue_comments_039242.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nSee [http://groups.google.com/group/sage-devel/browse_thread/thread/1adac4035031b140/c36f1f8a7c8a9b43#c36f1f8a7c8a9b43](http://groups.google.com/group/sage-devel/browse_thread/thread/1adac4035031b140/c36f1f8a7c8a9b43#c36f1f8a7c8a9b43) for why there is no loads(dumps()) test.",
    "created_at": "2009-05-08T19:25:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6009#issuecomment-39242",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:1" align="right">comment:1</div>

See [http://groups.google.com/group/sage-devel/browse_thread/thread/1adac4035031b140/c36f1f8a7c8a9b43#c36f1f8a7c8a9b43](http://groups.google.com/group/sage-devel/browse_thread/thread/1adac4035031b140/c36f1f8a7c8a9b43#c36f1f8a7c8a9b43) for why there is no loads(dumps()) test.



---

archive/issue_events_071473.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-08T19:25:51Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "subject": "https://github.com/kcrisman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6009#event-71473"
}
```



---

archive/issue_events_071474.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-08T19:25:51Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "subject": "https://github.com/kcrisman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6009#event-71474"
}
```



---

archive/issue_events_071475.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-08T19:25:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6009#event-71475"
}
```



---

archive/issue_events_071476.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-09T02:51:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6009#event-71476"
}
```



---

archive/issue_events_071477.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-05-09T02:51:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6009#event-71477"
}
```



---

archive/issue_comments_039243.json:
```json
{
    "body": "reviewer patch: fix typos, add consistency to the whole module",
    "created_at": "2009-05-09T02:54:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6009#issuecomment-39243",
    "user": "https://github.com/sagetrac-mvngu"
}
```

reviewer patch: fix typos, add consistency to the whole module



---

archive/issue_comments_039244.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAttachment: **[trac_6009-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket6009/trac_6009-reviewer.patch.gz)**\n\nREFEREE REPORT\n\n\n\nThe patch `trac_6009.patch` applies OK against the \"post-final\" sage-3.4.2, all doctests pass with the options `-t -long`. The reference manual built fine, but note that `sage/plot/text.py` is not included in the reference manual so don't be surprised when you can't search for `sage/plot/text.py` in the reference manual. The doctest coverage for `sage/plot/text.py` is 100% as claimed.\n\n\n\nHowever, I notice that the patch introduces some typos and inconsistencies into the module `sage/plot/text.py`. These are fixed in the reviewer patch `trac_6009-reviewer.patch`. Apart from these issues, positive review for kcrisman's patch. Only my patch needs to be reviewed.",
    "created_at": "2009-05-09T03:02:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6009#issuecomment-39244",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:3" align="right">comment:3</div>

Attachment: **[trac_6009-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket6009/trac_6009-reviewer.patch.gz)**

REFEREE REPORT



The patch `trac_6009.patch` applies OK against the "post-final" sage-3.4.2, all doctests pass with the options `-t -long`. The reference manual built fine, but note that `sage/plot/text.py` is not included in the reference manual so don't be surprised when you can't search for `sage/plot/text.py` in the reference manual. The doctest coverage for `sage/plot/text.py` is 100% as claimed.



However, I notice that the patch introduces some typos and inconsistencies into the module `sage/plot/text.py`. These are fixed in the reviewer patch `trac_6009-reviewer.patch`. Apart from these issues, positive review for kcrisman's patch. Only my patch needs to be reviewed.



---

archive/issue_events_071478.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-05-09T20:34:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6009#event-71478"
}
```



---

archive/issue_events_071479.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-05-09T20:34:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6009#event-71479"
}
```



---

archive/issue_comments_039245.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nReviewer patch looks good.",
    "created_at": "2009-05-09T20:34:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6009#issuecomment-39245",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:4" align="right">comment:4</div>

Reviewer patch looks good.



---

archive/issue_events_071480.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-11T08:52:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6009#event-71480"
}
```



---

archive/issue_events_071481.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-11T08:52:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6009#event-71481"
}
```



---

archive/issue_events_071482.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-11T08:52:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6009#event-71482"
}
```



---

archive/issue_events_071483.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-11T08:52:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6009#event-71483"
}
```



---

archive/issue_comments_039246.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nMerged both patches in Sage 4.0.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-11T08:52:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6009",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6009#issuecomment-39246",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Merged both patches in Sage 4.0.alpha0.

Cheers,

Michael

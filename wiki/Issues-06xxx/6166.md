# Issue 6166: strip 'nodetex' from the reference manual

archive/issues_006166.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe patch is supposed to remove 'nodetex' (and any other directives in the same line) from  the reference manual; these are already stripped from interactive docstrings by #6122.\n\n\nCC:  @rbeezer @mwhansen\n\nComponent: **documentation**\n\nAuthor: **John Palmieri, Mike Hansen**\n\nReviewer: **John Palmieri, Mike Hansen**\n\nMerged: **4.0.1.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6166_\n\n",
    "closed_at": "2009-06-01T05:28:35Z",
    "created_at": "2009-05-31T04:58:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "strip 'nodetex' from the reference manual",
    "type": "issue",
    "updated_at": "2009-06-06T22:47:36Z",
    "url": "https://github.com/sagemath/sage/issues/6166",
    "user": "https://github.com/jhpalmieri"
}
```
<div id="comment:0"></div>

The patch is supposed to remove 'nodetex' (and any other directives in the same line) from  the reference manual; these are already stripped from interactive docstrings by #6122.


CC:  @rbeezer @mwhansen

Component: **documentation**

Author: **John Palmieri, Mike Hansen**

Reviewer: **John Palmieri, Mike Hansen**

Merged: **4.0.1.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/6166_





---

archive/issue_events_073462.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-05-31T04:58:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6166#event-73462"
}
```



---

archive/issue_events_073463.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-05-31T04:58:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6166#event-73463"
}
```



---

archive/issue_events_073464.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-05-31T04:58:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6166#event-73464"
}
```



---

archive/issue_events_073465.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-05-31T04:58:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6166#event-73465"
}
```



---

archive/issue_events_073466.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-05-31T04:58:37Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "subject": "https://github.com/jhpalmieri",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6166#event-73466"
}
```



---

archive/issue_events_073467.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-05-31T04:58:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6166#event-73467"
}
```



---

archive/issue_comments_040685.json:
```json
{
    "body": "Attachment: **[no-nodetex-ref.patch.gz](https://github.com/sagemath/sage/files/ticket6166/no-nodetex-ref.patch.gz)**",
    "created_at": "2009-05-31T04:58:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6166#issuecomment-40685",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment: **[no-nodetex-ref.patch.gz](https://github.com/sagemath/sage/files/ticket6166/no-nodetex-ref.patch.gz)**



---

archive/issue_events_073468.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-06-01T05:10:55Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6166#event-73468"
}
```



---

archive/issue_events_073469.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-01T05:10:55Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6166#event-73469"
}
```



---

archive/issue_comments_040686.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_6166-2.patch.gz](https://github.com/sagemath/sage/files/ticket6166/trac_6166-2.patch.gz)**\n\nThe original patch looks good except it doesn't handle the case when there is an empty docstring.  I've added a patch on top which fixes this.  John, can you review my small patch?",
    "created_at": "2009-06-01T05:10:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6166#issuecomment-40686",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_6166-2.patch.gz](https://github.com/sagemath/sage/files/ticket6166/trac_6166-2.patch.gz)**

The original patch looks good except it doesn't handle the case when there is an empty docstring.  I've added a patch on top which fixes this.  John, can you review my small patch?



---

archive/issue_comments_040687.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nLooks good to me.  (Although we just shouldn't allow empty docstrings in the first place :)",
    "created_at": "2009-06-01T05:16:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6166#issuecomment-40687",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:2" align="right">comment:2</div>

Looks good to me.  (Although we just shouldn't allow empty docstrings in the first place :)



---

archive/issue_events_073470.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-06-01T05:16:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6166#event-73470"
}
```



---

archive/issue_events_073471.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-01T05:28:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6166#event-73471"
}
```



---

archive/issue_events_073472.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-01T05:28:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6166#event-73472"
}
```



---

archive/issue_comments_040688.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI think docstringlines is an empty list if there is no docstring as well.\n\nMerged in 4.0.1.alpha0.",
    "created_at": "2009-06-01T05:28:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6166#issuecomment-40688",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">comment:3</div>

I think docstringlines is an empty list if there is no docstring as well.

Merged in 4.0.1.alpha0.



---

archive/issue_comments_040689.json:
```json
{
    "body": "Reviewer: **John Palmieri, Mike Hansen**",
    "created_at": "2009-06-06T22:47:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6166#issuecomment-40689",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **John Palmieri, Mike Hansen**



---

archive/issue_comments_040690.json:
```json
{
    "body": "Author: **John Palmieri, Mike Hansen**",
    "created_at": "2009-06-06T22:47:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6166#issuecomment-40690",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **John Palmieri, Mike Hansen**



---

archive/issue_comments_040691.json:
```json
{
    "body": "Merged: **4.0.1.alpha0**",
    "created_at": "2009-06-06T22:47:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6166",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6166#issuecomment-40691",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **4.0.1.alpha0**

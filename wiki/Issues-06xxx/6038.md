# Issue 6038: add implicit_plot3d to the reference manual (and maybe an example in the tutorial)

archive/issues_006038.json:
```json
{
    "body": "Please add the wonderful implicit_plot3d to the reference manual, and possibly even add an example to the tutorial.\n\n**CC:**  wcauchois\n\nIssue created by migration from https://trac.sagemath.org/ticket/6038\n\n",
    "closed_at": "2009-05-26T16:49:29Z",
    "created_at": "2009-05-14T15:26:53Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0",
    "title": "add implicit_plot3d to the reference manual (and maybe an example in the tutorial)",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/6038",
    "user": "https://github.com/williamstein"
}
```
Please add the wonderful implicit_plot3d to the reference manual, and possibly even add an example to the tutorial.

**CC:**  wcauchois

Issue created by migration from https://trac.sagemath.org/ticket/6038





---

archive/issue_events_039237.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/wcauchois",
    "created_at": "2009-05-25T05:37:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6038",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6038#event-39237"
}
```



---

archive/attachments_007315.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac6038.patch",
    "asset_url": "tarball://root/attachments/ticket6038/trac6038.patch",
    "created_at": "2009-05-25T05:37:25Z",
    "issue": "https://github.com/sagemath/sage/issues/6038",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket6038/trac6038.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/wcauchois"
}
```



---

archive/issue_comments_040915.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac6038.patch](https://github.com/sagemath/sage/files/ticket6038/trac6038.patch)\n\n(This patch is based on Sage 4.0.rc0.)\n\nIn order to make the automatically generated reference documentation clearer, I moved the implicit_plot3d() function into a separate file called \"implicit_plot3d.py\". I see this has been done for some of the other plotting functions as well. I also took the liberty of adding a little bit to the meager section on \"Three-Dimensional Plots\" in the tour of Sage.",
    "created_at": "2009-05-25T05:37:25Z",
    "issue": "https://github.com/sagemath/sage/issues/6038",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6038#issuecomment-40915",
    "user": "https://trac.sagemath.org/admin/accounts/users/wcauchois"
}
```

<a id='comment:1'></a>
**Attachment:** [trac6038.patch](https://github.com/sagemath/sage/files/ticket6038/trac6038.patch)

(This patch is based on Sage 4.0.rc0.)

In order to make the automatically generated reference documentation clearer, I moved the implicit_plot3d() function into a separate file called "implicit_plot3d.py". I see this has been done for some of the other plotting functions as well. I also took the liberty of adding a little bit to the meager section on "Three-Dimensional Plots" in the tour of Sage.



---

archive/issue_comments_040916.json:
```json
{
    "body": "<a id='comment:3'></a>\nLooks good to me.\n\nMerged in Sage 4.0.rc1.",
    "created_at": "2009-05-26T16:49:29Z",
    "issue": "https://github.com/sagemath/sage/issues/6038",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6038#issuecomment-40916",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'></a>
Looks good to me.

Merged in Sage 4.0.rc1.



---

archive/issue_events_039238.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-26T16:49:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6038",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6038#event-39238"
}
```



---

archive/issue_events_039239.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-26T16:49:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6038",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6038#event-39239"
}
```



---

archive/issue_events_039240.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-26T16:49:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6038",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6038#event-39240"
}
```

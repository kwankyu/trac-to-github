# Issue 6747: Improve plotting of trees

archive/issues_006747.json:
```json
{
    "assignees": [
        "https://github.com/boothby"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nWhen one plots a tree with  `G.plot(layout='tree')`, the result is ugly.  For one thing, a tree plot should never have crossing lines.  I have code that makes a nice tree plot in nearly-linear time, which should be included in Sage.\n\nCC:  @sagetrac-sage-combinat @jasongrout @sagetrac-myurko\n\nComponent: **graph theory**\n\nAuthor: **Tom Boothby**\n\nReviewer: **Nathann Cohen, Michael Yurko**\n\nMerged: **sage-4.2.1.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6747_\n\n",
    "closed_at": "2009-10-31T16:44:14Z",
    "created_at": "2009-08-14T18:02:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.2.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Improve plotting of trees",
    "type": "issue",
    "updated_at": "2009-10-31T16:44:14Z",
    "url": "https://github.com/sagemath/sage/issues/6747",
    "user": "https://github.com/boothby"
}
```
<div id="comment:0"></div>

When one plots a tree with  `G.plot(layout='tree')`, the result is ugly.  For one thing, a tree plot should never have crossing lines.  I have code that makes a nice tree plot in nearly-linear time, which should be included in Sage.

CC:  @sagetrac-sage-combinat @jasongrout @sagetrac-myurko

Component: **graph theory**

Author: **Tom Boothby**

Reviewer: **Nathann Cohen, Michael Yurko**

Merged: **sage-4.2.1.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/6747_





---

archive/issue_events_081188.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-08-14T18:02:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6747#event-81188"
}
```



---

archive/issue_events_081189.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-08-14T18:02:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6747#event-81189"
}
```



---

archive/issue_events_081190.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-08-14T18:02:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6747#event-81190"
}
```



---

archive/issue_events_081191.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-08-14T18:02:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6747#event-81191"
}
```



---

archive/issue_events_081192.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-08-14T18:02:05Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "subject": "https://github.com/boothby",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6747#event-81192"
}
```



---

archive/issue_comments_047550.json:
```json
{
    "body": "old plot",
    "created_at": "2009-08-14T18:02:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6747#issuecomment-47550",
    "user": "https://github.com/boothby"
}
```

old plot



---

archive/issue_comments_047551.json:
```json
{
    "body": "Attachment: **[tree2.png](https://github.com/sagemath/sage/files/ticket6747/tree2.png)**\n\nAttachment: **[tree.png](https://github.com/sagemath/sage/files/ticket6747/tree.png)**\n\ndesired output",
    "created_at": "2009-08-14T18:02:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6747#issuecomment-47551",
    "user": "https://github.com/boothby"
}
```

Attachment: **[tree2.png](https://github.com/sagemath/sage/files/ticket6747/tree2.png)**

Attachment: **[tree.png](https://github.com/sagemath/sage/files/ticket6747/tree.png)**

desired output



---

archive/issue_comments_047552.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nYou code looks like it first chooses the center vertex in the graph, then converts is as a Poset by picking this special vertex as a root, and plotting this Poset... But you did not post the patch :-)",
    "created_at": "2009-08-14T19:18:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6747#issuecomment-47552",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:1" align="right">comment:1</div>

You code looks like it first chooses the center vertex in the graph, then converts is as a Poset by picking this special vertex as a root, and plotting this Poset... But you did not post the patch :-)



---

archive/issue_comments_047553.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAttachment: **[6747_tree_layout.patch.gz](https://github.com/sagemath/sage/files/ticket6747/6747_tree_layout.patch.gz)**\n\nUhm... my code does nothing like what you said.  And I didn't post the patch because I hadn't cleaned it up / incorporated it into the GraphPlot class / documented it.  But now I have!",
    "created_at": "2009-08-14T23:24:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6747#issuecomment-47553",
    "user": "https://github.com/boothby"
}
```

<div id="comment:2" align="right">comment:2</div>

Attachment: **[6747_tree_layout.patch.gz](https://github.com/sagemath/sage/files/ticket6747/6747_tree_layout.patch.gz)**

Uhm... my code does nothing like what you said.  And I didn't post the patch because I hadn't cleaned it up / incorporated it into the GraphPlot class / documented it.  But now I have!



---

archive/issue_events_081193.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-08-14T23:24:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6747#event-81193"
}
```



---

archive/issue_events_081194.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-08-14T23:24:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6747#event-81194"
}
```



---

archive/issue_events_081195.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-08-16T06:11:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "milestone_number": null,
    "milestone_title": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6747#event-81195"
}
```



---

archive/issue_events_081196.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-08-18T12:23:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6747#event-81196"
}
```



---

archive/issue_events_081197.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-08-18T12:23:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6747#event-81197"
}
```



---

archive/issue_comments_047554.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nI just tried your patch, and as I never plotted any tree in Sage, I tried to generate some for a start.... I tried your algorithm on graphs.BalancedTree(3,4) and graphs.BalancedTree(3,5). They are displayed on my machine in a very odd shape ( the picture had a large width and a very small height ). I admit these graphs contain a lot of vertices and may not be good examples, but what do you think of this result ? Do you think there would be a way to slightly change you code so that in such cases the plot could have a look closer from what one gets with the current layout=tree parameter ?\n\nI know it is very easy to criticize in such cases, and much harder to come up with a good algorithm.. Thinking about it, I thought that maybe the best way to draw a tree properly would be to begin with a cross-free embedding of the tree in the plane ( as you mentionned ), then to use the usual trick of repulsion between vertices to obtain a balanced shape ? I know it is not a good solution because of its complexity. I'll continue thinking about it, though O_o",
    "created_at": "2009-08-18T12:23:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6747#issuecomment-47554",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">comment:4</div>

I just tried your patch, and as I never plotted any tree in Sage, I tried to generate some for a start.... I tried your algorithm on graphs.BalancedTree(3,4) and graphs.BalancedTree(3,5). They are displayed on my machine in a very odd shape ( the picture had a large width and a very small height ). I admit these graphs contain a lot of vertices and may not be good examples, but what do you think of this result ? Do you think there would be a way to slightly change you code so that in such cases the plot could have a look closer from what one gets with the current layout=tree parameter ?

I know it is very easy to criticize in such cases, and much harder to come up with a good algorithm.. Thinking about it, I thought that maybe the best way to draw a tree properly would be to begin with a cross-free embedding of the tree in the plane ( as you mentionned ), then to use the usual trick of repulsion between vertices to obtain a balanced shape ? I know it is not a good solution because of its complexity. I'll continue thinking about it, though O_o



---

archive/issue_comments_047555.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nThese are extreme examples, and I'm not sure that a top-down layout will ever look good.  I believe that the solution for this particular problem would be to implement a circular tree layout algorithm.  However, if you play with the parameters, you can get a fairly nice plot.\n\n```\n    T = graphs.BalancedTree(3,5)\n    P = T.plot(layout='tree',tree_root=0)\n    P.show(aspect_ratio=.1,figsize=(15,15))\n```",
    "created_at": "2009-08-19T16:51:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6747#issuecomment-47555",
    "user": "https://github.com/boothby"
}
```

<div id="comment:5" align="right">comment:5</div>

These are extreme examples, and I'm not sure that a top-down layout will ever look good.  I believe that the solution for this particular problem would be to implement a circular tree layout algorithm.  However, if you play with the parameters, you can get a fairly nice plot.

```
    T = graphs.BalancedTree(3,5)
    P = T.plot(layout='tree',tree_root=0)
    P.show(aspect_ratio=.1,figsize=(15,15))
```



---

archive/issue_comments_047556.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nApplies fines, and I could not find any bug in it. Clearly improves the plotting for \"human\" trees. This new patch includes the previous one, plus :\n* A INPUT section\n* A new semicolumn after EXAMPLES ::\n\nIf you agree with these changes, you can set this ticket to positive review !\n\nNathann",
    "created_at": "2009-10-13T09:42:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6747#issuecomment-47556",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:7" align="right">comment:7</div>

Applies fines, and I could not find any bug in it. Clearly improves the plotting for "human" trees. This new patch includes the previous one, plus :
* A INPUT section
* A new semicolumn after EXAMPLES ::

If you agree with these changes, you can set this ticket to positive review !

Nathann



---

archive/issue_comments_047557.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nAttachment: **[trac_6747-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket6747/trac_6747-reviewer.patch.gz)**\n\nThanks much for improving the layout of trees!\n\nPlease have a look at #7004, which also started splitting the layout algorithms in different methods. So we need to decide on a naming convention. \n\nThen I guess this patch can get in, and #7400 be rebased on it.",
    "created_at": "2009-10-14T10:53:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6747#issuecomment-47557",
    "user": "https://github.com/nthiery"
}
```

<div id="comment:8" align="right">comment:8</div>

Attachment: **[trac_6747-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket6747/trac_6747-reviewer.patch.gz)**

Thanks much for improving the layout of trees!

Please have a look at #7004, which also started splitting the layout algorithms in different methods. So we need to decide on a naming convention. 

Then I guess this patch can get in, and #7400 be rebased on it.



---

archive/issue_events_081198.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-10-19T21:24:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6747#event-81198"
}
```



---

archive/issue_events_081199.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-10-19T21:24:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6747#event-81199"
}
```



---

archive/issue_events_081200.json:
```json
{
    "actor": "https://github.com/sagetrac-myurko",
    "created_at": "2009-10-30T20:56:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6747#event-81200"
}
```



---

archive/issue_events_081201.json:
```json
{
    "actor": "https://github.com/sagetrac-myurko",
    "created_at": "2009-10-30T20:56:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6747#event-81201"
}
```



---

archive/issue_comments_047558.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nReplying to [@nathanncohen](#comment%3A7):\n> Applies fines, and I could not find any bug in it. Clearly improves the plotting for \"human\" trees. This new patch includes the previous one, plus :\n> * A INPUT section\n> * A new semicolumn after EXAMPLES ::\n> \n> If you agree with these changes, you can set this ticket to positive review !\n> \n> Nathann\n\nThe change look fine.",
    "created_at": "2009-10-30T20:56:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6747#issuecomment-47558",
    "user": "https://github.com/sagetrac-myurko"
}
```

<div id="comment:12" align="right">comment:12</div>

Replying to [@nathanncohen](#comment%3A7):
> Applies fines, and I could not find any bug in it. Clearly improves the plotting for "human" trees. This new patch includes the previous one, plus :
> * A INPUT section
> * A new semicolumn after EXAMPLES ::
> 
> If you agree with these changes, you can set this ticket to positive review !
> 
> Nathann

The change look fine.



---

archive/issue_events_081202.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-31T16:44:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6747#event-81202"
}
```



---

archive/issue_events_081203.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-31T16:44:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6747#event-81203"
}
```



---

archive/issue_comments_047559.json:
```json
{
    "body": "Reviewer: **Nathann Cohen, Michael Yurko**",
    "created_at": "2009-10-31T16:44:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6747#issuecomment-47559",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **Nathann Cohen, Michael Yurko**



---

archive/issue_comments_047560.json:
```json
{
    "body": "Author: **Tom Boothby**",
    "created_at": "2009-10-31T16:44:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6747#issuecomment-47560",
    "user": "https://github.com/mwhansen"
}
```

Author: **Tom Boothby**



---

archive/issue_comments_047561.json:
```json
{
    "body": "Merged: **sage-4.2.1.alpha0**",
    "created_at": "2009-10-31T16:44:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6747#issuecomment-47561",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.2.1.alpha0**

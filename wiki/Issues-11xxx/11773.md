# Issue 11773: Elliptic curve constructor with no arguments gives an obscure error message

archive/issues_011601.json:
```json
{
    "body": "\n```\nsage: EllipticCurve()\n```\nleads to an assertion error instead of a sensible error message.\n\nThis has been fixed as part of #13100.  This ticket can be closed.\n\n**Assignee:** @JohnCremona\n\n**Keywords:** elliptic curve\n\n**Reviewer:** John Cremona\n\nIssue created by migration from https://trac.sagemath.org/ticket/11773\n\n",
    "closed_at": "2012-06-19T13:33:10Z",
    "created_at": "2011-09-01T16:34:38Z",
    "labels": [
        "component: elliptic curves",
        "minor",
        "bug",
        "duplicate"
    ],
    "title": "Elliptic curve constructor with no arguments gives an obscure error message",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/11773",
    "user": "https://github.com/JohnCremona"
}
```

```
sage: EllipticCurve()
```
leads to an assertion error instead of a sensible error message.

This has been fixed as part of #13100.  This ticket can be closed.

**Assignee:** @JohnCremona

**Keywords:** elliptic curve

**Reviewer:** John Cremona

Issue created by migration from https://trac.sagemath.org/ticket/11773





---

archive/issue_comments_125025.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis has been fixed as part of #13100.",
    "created_at": "2012-06-10T16:43:02Z",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11773#issuecomment-125025",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:1'></a>
This has been fixed as part of #13100.



---

archive/issue_comments_125026.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -3,3 +3,5 @@\n sage: EllipticCurve()\n ```\n leads to an assertion error instead of a sensible error message.\n+\n+This has been fixed as part of #13100.  This ticket can be closed.\n``````\n",
    "created_at": "2012-06-10T16:43:02Z",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11773#issuecomment-125026",
    "user": "https://github.com/JohnCremona"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -3,3 +3,5 @@
 sage: EllipticCurve()
 ```
 leads to an assertion error instead of a sensible error message.
+
+This has been fixed as part of #13100.  This ticket can be closed.
``````




---

archive/issue_events_094714.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-06-11T16:23:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-94714"
}
```



---

archive/issue_events_094715.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-06-11T16:24:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-94715"
}
```



---

archive/issue_events_094716.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-06-11T16:24:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-94716"
}
```



---

archive/issue_comments_125027.json:
```json
{
    "body": "<a id='comment:3'></a>\n#13100 has a positive review",
    "created_at": "2012-06-11T16:24:08Z",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11773#issuecomment-125027",
    "user": "https://github.com/roed314"
}
```

<a id='comment:3'></a>
#13100 has a positive review



---

archive/issue_events_094717.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-06-11T16:24:08Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "milestone": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-94717"
}
```



---

archive/issue_comments_125028.json:
```json
{
    "body": "**Reviewer:** John Cremona",
    "created_at": "2012-06-19T13:33:10Z",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11773#issuecomment-125028",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** John Cremona



---

archive/issue_events_094718.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-19T13:33:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-94718"
}
```



---

archive/issue_events_094719.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-19T13:33:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-94719"
}
```



---

archive/issue_events_094720.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-19T13:33:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-94720"
}
```

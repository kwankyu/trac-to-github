# Issue 11477: Build problems with pynifti-p0

archive/issues_011305.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\n./sage -i pynifti-p0\n\nresults in some requests to overwrite files, and also a build error.   Below are some identifying entries from the command trace:\n\n\n```\npynifti-p0\nhttp://www.sagemath.org//packages/experimental/pynifti-p0.spkg --> pynifti-p0.spkg\n-rw-r--r-- 1 root 1237 6157441 Jun 14 00:57 /qp/sage-4.7/spkg/optional/pynifti-p0.spkg\n...\n(cp znzlib/*.a lib; cp znzlib/*.h include;)\ncp: overwrite `lib/libznz.a'? cp: overwrite `include/znzlib.h'? ranlib lib/*.a\n znzlib installed.\n\n(cp niftilib/*.a lib; cp niftilib/*.h include;)\ncp: overwrite `lib/libniftiio.a'? y\ncp: overwrite `include/nifti1.h'? y\ncp: overwrite `include/nifti1_io.h'? y\nranlib lib/*.a\nranlib: lib/libfslio.a: Malformed archive\nmake: *** [nifti_install] Error 1\n```\n\n\n\nComponent: **packages: optional**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11477_\n\n",
    "closed_at": "2021-09-10T17:33:19Z",
    "created_at": "2011-06-14T12:57:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Build problems with pynifti-p0",
    "type": "issue",
    "updated_at": "2021-09-10T17:33:19Z",
    "url": "https://github.com/sagemath/sage/issues/11477",
    "user": "https://github.com/sagetrac-ijstokes"
}
```
<div id="comment:0"></div>

./sage -i pynifti-p0

results in some requests to overwrite files, and also a build error.   Below are some identifying entries from the command trace:


```
pynifti-p0
http://www.sagemath.org//packages/experimental/pynifti-p0.spkg --> pynifti-p0.spkg
-rw-r--r-- 1 root 1237 6157441 Jun 14 00:57 /qp/sage-4.7/spkg/optional/pynifti-p0.spkg
...
(cp znzlib/*.a lib; cp znzlib/*.h include;)
cp: overwrite `lib/libznz.a'? cp: overwrite `include/znzlib.h'? ranlib lib/*.a
 znzlib installed.

(cp niftilib/*.a lib; cp niftilib/*.h include;)
cp: overwrite `lib/libniftiio.a'? y
cp: overwrite `include/nifti1.h'? y
cp: overwrite `include/nifti1_io.h'? y
ranlib lib/*.a
ranlib: lib/libfslio.a: Malformed archive
make: *** [nifti_install] Error 1
```



Component: **packages: optional**

_Issue created by migration from https://trac.sagemath.org/ticket/11477_





---

archive/issue_events_151449.json:
```json
{
    "actor": "https://github.com/sagetrac-ijstokes",
    "created_at": "2011-06-14T12:57:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151449"
}
```



---

archive/issue_events_151450.json:
```json
{
    "actor": "https://github.com/sagetrac-ijstokes",
    "created_at": "2011-06-14T12:57:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20optional",
    "label_color": "0000b0",
    "label_name": "c: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151450"
}
```



---

archive/issue_events_151451.json:
```json
{
    "actor": "https://github.com/sagetrac-ijstokes",
    "created_at": "2011-06-14T12:57:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151451"
}
```



---

archive/issue_events_151452.json:
```json
{
    "actor": "https://github.com/sagetrac-ijstokes",
    "created_at": "2011-06-14T12:57:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151452"
}
```



---

archive/issue_comments_116463.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,6 +2,8 @@\n \n results in some requests to overwrite files, and also a build error.   Below are some identifying entries from the command trace:\n \n+\n+```\n pynifti-p0\n http://www.sagemath.org//packages/experimental/pynifti-p0.spkg --> pynifti-p0.spkg\n -rw-r--r-- 1 root 1237 6157441 Jun 14 00:57 /qp/sage-4.7/spkg/optional/pynifti-p0.spkg\n@@ -17,4 +19,6 @@\n ranlib lib/*.a\n ranlib: lib/libfslio.a: Malformed archive\n make: *** [nifti_install] Error 1\n+```\n \n+\n``````\n",
    "created_at": "2011-06-14T12:59:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11477#issuecomment-116463",
    "user": "https://github.com/sagetrac-ijstokes"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,6 +2,8 @@
 
 results in some requests to overwrite files, and also a build error.   Below are some identifying entries from the command trace:
 
+
+```
 pynifti-p0
 http://www.sagemath.org//packages/experimental/pynifti-p0.spkg --> pynifti-p0.spkg
 -rw-r--r-- 1 root 1237 6157441 Jun 14 00:57 /qp/sage-4.7/spkg/optional/pynifti-p0.spkg
@@ -17,4 +19,6 @@
 ranlib lib/*.a
 ranlib: lib/libfslio.a: Malformed archive
 make: *** [nifti_install] Error 1
+```
 
+
``````




---

archive/issue_events_151453.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151453"
}
```



---

archive/issue_events_151454.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151454"
}
```



---

archive/issue_events_151455.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151455"
}
```



---

archive/issue_events_151456.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151456"
}
```



---

archive/issue_events_151457.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151457"
}
```



---

archive/issue_events_151458.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151458"
}
```



---

archive/issue_events_151459.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151459"
}
```



---

archive/issue_events_151460.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151460"
}
```



---

archive/issue_comments_116464.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nSetting spkg proposals that have not seen recent activity to \"sage-wishlist\".",
    "created_at": "2020-06-19T18:16:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11477#issuecomment-116464",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">comment:6</div>

Setting spkg proposals that have not seen recent activity to "sage-wishlist".



---

archive/issue_events_151461.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-19T18:16:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151461"
}
```



---

archive/issue_events_151462.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-19T18:16:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151462"
}
```



---

archive/issue_events_151463.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-19T18:16:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151463"
}
```



---

archive/issue_events_151464.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T19:04:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151464"
}
```



---

archive/issue_comments_116465.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\noutdated, should close",
    "created_at": "2021-08-26T19:04:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11477#issuecomment-116465",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:7" align="right">comment:7</div>

outdated, should close



---

archive/issue_events_151465.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T19:04:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151465"
}
```



---

archive/issue_events_151466.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T19:04:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151466"
}
```



---

archive/issue_events_151467.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2021-09-10T11:24:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151467"
}
```



---

archive/issue_events_151468.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2021-09-10T11:24:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151468"
}
```



---

archive/issue_comments_116466.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nI agree.",
    "created_at": "2021-09-10T11:24:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11477#issuecomment-116466",
    "user": "https://github.com/kwankyu"
}
```

<div id="comment:8" align="right">comment:8</div>

I agree.



---

archive/issue_events_151469.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-10T17:33:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151469"
}
```



---

archive/issue_events_151470.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-10T17:33:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11477",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11477#event-151470"
}
```

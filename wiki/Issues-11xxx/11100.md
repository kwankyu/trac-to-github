# Issue 11100: Increase default timeout of test_executable() to 50 seconds

archive/issues_010966.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu"
    ],
    "body": "<div id=\"comment:0\"></div>\n\n\n\nComponent: **doctest coverage**\n\nAuthor: **Jeroen Demeyer**\n\nReviewer: **Volker Braun**\n\nMerged: **sage-4.7.alpha4**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11100_\n\n",
    "closed_at": "2011-04-07T08:38:13Z",
    "created_at": "2011-03-31T12:00:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Increase default timeout of test_executable() to 50 seconds",
    "type": "issue",
    "updated_at": "2011-04-07T08:38:13Z",
    "url": "https://github.com/sagemath/sage/issues/11100",
    "user": "https://github.com/jdemeyer"
}
```
<div id="comment:0"></div>



Component: **doctest coverage**

Author: **Jeroen Demeyer**

Reviewer: **Volker Braun**

Merged: **sage-4.7.alpha4**

_Issue created by migration from https://trac.sagemath.org/ticket/11100_





---

archive/issue_events_145709.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-31T12:00:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11100",
    "milestone_number": null,
    "milestone_title": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11100#event-145709"
}
```



---

archive/issue_events_145710.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-31T12:00:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11100",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11100#event-145710"
}
```



---

archive/issue_events_145711.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-31T12:00:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11100",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11100#event-145711"
}
```



---

archive/issue_events_145712.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-31T12:00:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11100",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11100#event-145712"
}
```



---

archive/issue_events_145713.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2011-03-31T12:00:44Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11100",
    "subject": "https://github.com/jdemeyer",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11100#event-145713"
}
```



---

archive/issue_events_145714.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-31T12:05:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11100",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11100#event-145714"
}
```



---

archive/issue_comments_109832.json:
```json
{
    "body": "Attachment: **[11100_test_executable.patch.gz](https://github.com/sagemath/sage/files/ticket11100/11100_test_executable.patch.gz)**",
    "created_at": "2011-03-31T12:07:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11100#issuecomment-109832",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[11100_test_executable.patch.gz](https://github.com/sagemath/sage/files/ticket11100/11100_test_executable.patch.gz)**



---

archive/issue_comments_109833.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2011-04-02T02:41:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11100#issuecomment-109833",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_145715.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2011-04-02T02:41:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11100",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11100#event-145715"
}
```



---

archive/issue_events_145716.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2011-04-02T02:41:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11100",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11100#event-145716"
}
```



---

archive/issue_comments_109834.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nIncreasing it to 50 seconds is fine with me; But I doubt it that its sufficient on some NFS-connected machines where Sage startup can take forever. The default for doctests to time out is 360 seconds according to http://www.sagemath.org/doc/installation/source.html\n\nBut either way is better than 10 seconds.",
    "created_at": "2011-04-02T02:41:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11100#issuecomment-109834",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:2" align="right">comment:2</div>

Increasing it to 50 seconds is fine with me; But I doubt it that its sufficient on some NFS-connected machines where Sage startup can take forever. The default for doctests to time out is 360 seconds according to http://www.sagemath.org/doc/installation/source.html

But either way is better than 10 seconds.



---

archive/issue_comments_109835.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\n360 is for a *whole file* I believe, while the 50 seconds is for one test.  So increasing the timeout for *test_executable()* much more will not help because you will run into the whole-file timeout anyway.",
    "created_at": "2011-04-02T08:04:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11100#issuecomment-109835",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">comment:3</div>

360 is for a *whole file* I believe, while the 50 seconds is for one test.  So increasing the timeout for *test_executable()* much more will not help because you will run into the whole-file timeout anyway.



---

archive/issue_events_145717.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-04-07T08:38:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11100",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11100#event-145717"
}
```



---

archive/issue_events_145718.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-04-07T08:38:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11100",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11100#event-145718"
}
```



---

archive/issue_comments_109836.json:
```json
{
    "body": "Merged: **sage-4.7.alpha4**",
    "created_at": "2011-04-07T08:38:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11100#issuecomment-109836",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.alpha4**

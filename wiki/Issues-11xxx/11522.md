# Issue 11522: partition centralizer_size doc is badly set up

archive/issues_011350.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nWe should use `.. math::` to display the equation. Moreover,\nsome TeX bracket are missing on the exponent. It should be `i^{m[i]}` instead of `i^m[i]`. \n\n---\n\nApply [attachment: trac_11522-jhp.patch](https://github.com/sagemath/sage/files/ticket11522/trac_11522-jhp.patch.gz).\n\n\nComponent: **documentation**\n\nAuthor: **Ivo Hedtke**\n\nReviewer: **John Palmieri**\n\nMerged: **sage-4.7.2.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11522_\n\n",
    "closed_at": "2011-07-27T13:41:02Z",
    "created_at": "2011-06-18T21:49:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20trivial%20/%205",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "partition centralizer_size doc is badly set up",
    "type": "issue",
    "updated_at": "2011-07-27T13:41:02Z",
    "url": "https://github.com/sagemath/sage/issues/11522",
    "user": "https://github.com/hivert"
}
```
<div id="comment:0"></div>

We should use `.. math::` to display the equation. Moreover,
some TeX bracket are missing on the exponent. It should be `i^{m[i]}` instead of `i^m[i]`. 

---

Apply [attachment: trac_11522-jhp.patch](https://github.com/sagemath/sage/files/ticket11522/trac_11522-jhp.patch.gz).


Component: **documentation**

Author: **Ivo Hedtke**

Reviewer: **John Palmieri**

Merged: **sage-4.7.2.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/11522_





---

archive/issue_events_152299.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-06-18T21:49:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11522#event-152299"
}
```



---

archive/issue_events_152300.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-06-18T21:49:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11522#event-152300"
}
```



---

archive/issue_events_152301.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-06-18T21:49:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20trivial%20/%205",
    "label_color": "fff9e5",
    "label_name": "p: trivial / 5",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11522#event-152301"
}
```



---

archive/issue_events_152302.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-06-18T21:49:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11522#event-152302"
}
```



---

archive/issue_events_152303.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2011-06-18T21:49:37Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "subject": "https://github.com/hivert",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11522#event-152303"
}
```



---

archive/issue_events_152304.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-06-18T21:51:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11522#event-152304"
}
```



---

archive/issue_comments_117302.json:
```json
{
    "body": "Attachment: **[trac_11522.patch.gz](https://github.com/sagemath/sage/files/ticket11522/trac_11522.patch.gz)**",
    "created_at": "2011-07-20T20:36:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11522#issuecomment-117302",
    "user": "https://github.com/hedtke"
}
```

Attachment: **[trac_11522.patch.gz](https://github.com/sagemath/sage/files/ticket11522/trac_11522.patch.gz)**



---

archive/issue_events_152305.json:
```json
{
    "actor": "https://github.com/hedtke",
    "created_at": "2011-07-20T20:36:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11522#event-152305"
}
```



---

archive/issue_comments_117303.json:
```json
{
    "body": "Attachment: **[trac_11522-jhp.patch.gz](https://github.com/sagemath/sage/files/ticket11522/trac_11522-jhp.patch.gz)**\n\napply only this patch",
    "created_at": "2011-07-23T17:16:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11522#issuecomment-117303",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment: **[trac_11522-jhp.patch.gz](https://github.com/sagemath/sage/files/ticket11522/trac_11522-jhp.patch.gz)**

apply only this patch



---

archive/issue_comments_117304.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nLooks good except for a missing \"r\" at the triple quotes starting the docstring.  Without the \"r\", the \"\\frac\" isn't rendered properly for me.  I've attached a new patch which includes all of your changes and also adds the \"r\".",
    "created_at": "2011-07-23T17:18:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11522#issuecomment-117304",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:3" align="right">comment:3</div>

Looks good except for a missing "r" at the triple quotes starting the docstring.  Without the "r", the "\frac" isn't rendered properly for me.  I've attached a new patch which includes all of your changes and also adds the "r".



---

archive/issue_comments_117305.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,7 @@\n We should use `.. math::` to display the equation. Moreover,\n some TeX bracket are missing on the exponent. It should be `i^{m[i]}` instead of `i^m[i]`. \n+\n+---\n+\n+Apply [attachment: trac_11522-jhp.patch](https://github.com/sagemath/sage/files/ticket11522/trac_11522-jhp.patch.gz).\n+\n``````\n",
    "created_at": "2011-07-23T17:18:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11522#issuecomment-117305",
    "user": "https://github.com/jhpalmieri"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,7 @@
 We should use `.. math::` to display the equation. Moreover,
 some TeX bracket are missing on the exponent. It should be `i^{m[i]}` instead of `i^m[i]`. 
+
+---
+
+Apply [attachment: trac_11522-jhp.patch](https://github.com/sagemath/sage/files/ticket11522/trac_11522-jhp.patch.gz).
+
``````




---

archive/issue_events_152306.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2011-07-23T17:18:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11522#event-152306"
}
```



---

archive/issue_events_152307.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2011-07-23T17:18:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11522#event-152307"
}
```



---

archive/issue_comments_117306.json:
```json
{
    "body": "Reviewer: **John Palmieri**",
    "created_at": "2011-07-23T17:18:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11522#issuecomment-117306",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **John Palmieri**



---

archive/issue_comments_117307.json:
```json
{
    "body": "Author: **Ivo Hedtke**",
    "created_at": "2011-07-23T17:18:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11522#issuecomment-117307",
    "user": "https://github.com/jhpalmieri"
}
```

Author: **Ivo Hedtke**



---

archive/issue_events_152308.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T11:30:04Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11522#event-152308"
}
```



---

archive/issue_events_152309.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T11:30:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11522#event-152309"
}
```



---

archive/issue_events_152310.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-27T13:41:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11522#event-152310"
}
```



---

archive/issue_events_152311.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-27T13:41:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11522#event-152311"
}
```



---

archive/issue_comments_117308.json:
```json
{
    "body": "Merged: **sage-4.7.2.alpha0**",
    "created_at": "2011-07-27T13:41:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11522",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11522#issuecomment-117308",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.2.alpha0**

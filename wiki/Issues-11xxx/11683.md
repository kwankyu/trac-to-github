# Issue 11683: ell_curve_isogeny initialization

archive/issues_011511.json:
```json
{
    "assignees": [
        "https://github.com/JohnCremona"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe import of sage/schemes/elliptic_curves/ell_curve_isogeny.py (which happens on every startup of sage) takes a relatively big amount of time\n\n```\n$ time ./sage -startuptime|grep ell_curve_isogeny:\n          ell_curve_isogeny: 0.133 (ell_field)\n\nreal    0m0.994s\nuser    0m0.810s\nsys     0m0.154s\n```\nLazily computing the constants in this file helps a lot (see the attached patch):\n\n```\n$ time ./sage -startuptime|grep ell_curve_isogeny:\n          ell_curve_isogeny: 0.001 (ell_field)\n\nreal    0m0.849s\nuser    0m0.672s\nsys     0m0.164s\n```\n**Apply:**\n\n1. [attachment: trac_11683_ell_curve_isogeny_speedup_combined.patch](https://github.com/sagemath/sage/files/ticket11683/trac_11683_ell_curve_isogeny_speedup_combined.patch.gz)\n2. [attachment: trac_11683_review.patch](https://github.com/sagemath/sage/files/ticket11683/trac_11683_review.patch.gz)\n\nto the sage repository.\n\nCC:  @sagetrac-fschulze\n\nComponent: **elliptic curves**\n\nKeywords: **startup, initialization, sd32, sd35**\n\nAuthor: **Julian Rueth, John Cremona**\n\nReviewer: **Julian Rueth, Frithjof Schulze**\n\nMerged: **sage-5.0.beta7**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11683_\n\n",
    "closed_at": "2012-03-04T21:19:33Z",
    "created_at": "2011-08-12T18:03:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20elliptic%20curves",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "ell_curve_isogeny initialization",
    "type": "issue",
    "updated_at": "2012-03-04T21:19:33Z",
    "url": "https://github.com/sagemath/sage/issues/11683",
    "user": "https://github.com/saraedum"
}
```
<div id="comment:0"></div>

The import of sage/schemes/elliptic_curves/ell_curve_isogeny.py (which happens on every startup of sage) takes a relatively big amount of time

```
$ time ./sage -startuptime|grep ell_curve_isogeny:
          ell_curve_isogeny: 0.133 (ell_field)

real    0m0.994s
user    0m0.810s
sys     0m0.154s
```
Lazily computing the constants in this file helps a lot (see the attached patch):

```
$ time ./sage -startuptime|grep ell_curve_isogeny:
          ell_curve_isogeny: 0.001 (ell_field)

real    0m0.849s
user    0m0.672s
sys     0m0.164s
```
**Apply:**

1. [attachment: trac_11683_ell_curve_isogeny_speedup_combined.patch](https://github.com/sagemath/sage/files/ticket11683/trac_11683_ell_curve_isogeny_speedup_combined.patch.gz)
2. [attachment: trac_11683_review.patch](https://github.com/sagemath/sage/files/ticket11683/trac_11683_review.patch.gz)

to the sage repository.

CC:  @sagetrac-fschulze

Component: **elliptic curves**

Keywords: **startup, initialization, sd32, sd35**

Author: **Julian Rueth, John Cremona**

Reviewer: **Julian Rueth, Frithjof Schulze**

Merged: **sage-5.0.beta7**

_Issue created by migration from https://trac.sagemath.org/ticket/11683_





---

archive/issue_events_155187.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2011-08-12T18:03:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155187"
}
```



---

archive/issue_events_155188.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2011-08-12T18:03:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20elliptic%20curves",
    "label_color": "0000ff",
    "label_name": "c: elliptic curves",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155188"
}
```



---

archive/issue_events_155189.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2011-08-12T18:03:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155189"
}
```



---

archive/issue_events_155190.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2011-08-12T18:03:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155190"
}
```



---

archive/issue_events_155191.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2011-08-12T18:03:30Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "subject": "https://github.com/saraedum",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155191"
}
```



---

archive/issue_comments_120682.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_11683_ell_curve_isogeny_speedup.patch.gz](https://github.com/sagemath/sage/files/ticket11683/trac_11683_ell_curve_isogeny_speedup.patch.gz)**\n\nI would like to see the initial comments placed in context, and perhaps a doctest for the actual computation of Psi5 (even if it takes 4s). In any case, you do need doctests.",
    "created_at": "2011-08-23T06:25:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120682",
    "user": "https://github.com/robertwb"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_11683_ell_curve_isogeny_speedup.patch.gz](https://github.com/sagemath/sage/files/ticket11683/trac_11683_ell_curve_isogeny_speedup.patch.gz)**

I would like to see the initial comments placed in context, and perhaps a doctest for the actual computation of Psi5 (even if it takes 4s). In any case, you do need doctests.



---

archive/issue_comments_120683.json:
```json
{
    "body": "Changed upstream from **Not yet reported upstream; Will do shortly.** to none",
    "created_at": "2011-08-24T18:09:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120683",
    "user": "https://github.com/saraedum"
}
```

Changed upstream from **Not yet reported upstream; Will do shortly.** to none



---

archive/issue_comments_120684.json:
```json
{
    "body": "Author: **Julian R\u00fcth**",
    "created_at": "2011-08-24T18:09:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120684",
    "user": "https://github.com/saraedum"
}
```

Author: **Julian Rüth**



---

archive/issue_comments_120685.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAttachment: **[trac_11683_ell_curve_isogeny_speedup_docstrings.patch.gz](https://github.com/sagemath/sage/files/ticket11683/trac_11683_ell_curve_isogeny_speedup_docstrings.patch.gz)**\n\nYou're right. I turned the comments describing how to compute the precomputed things into docstrings. Unfortunately they don't seem to work anymore. The line\n\n```\nsage: Psi5 = sum([X**i * f.coefficient({x:i})(X,t) for i in range(3)]).monic()\n```\nfails with\n\n```\nAttributeError: 'sage.rings.polynomial.multi_polynomial_libsingular.MPolynomial_libsingular' object has no attribute 'monic'\n```\n\nCan this be fixed easily?",
    "created_at": "2011-08-24T18:09:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120685",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:2" align="right">comment:2</div>

Attachment: **[trac_11683_ell_curve_isogeny_speedup_docstrings.patch.gz](https://github.com/sagemath/sage/files/ticket11683/trac_11683_ell_curve_isogeny_speedup_docstrings.patch.gz)**

You're right. I turned the comments describing how to compute the precomputed things into docstrings. Unfortunately they don't seem to work anymore. The line

```
sage: Psi5 = sum([X**i * f.coefficient({x:i})(X,t) for i in range(3)]).monic()
```
fails with

```
AttributeError: 'sage.rings.polynomial.multi_polynomial_libsingular.MPolynomial_libsingular' object has no attribute 'monic'
```

Can this be fixed easily?



---

archive/issue_comments_120686.json:
```json
{
    "body": "Changed author from **Julian R\u00fcth** to **Julian Rueth**",
    "created_at": "2011-08-24T18:59:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120686",
    "user": "https://github.com/saraedum"
}
```

Changed author from **Julian Rüth** to **Julian Rueth**



---

archive/issue_comments_120687.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\n**Complaint:**\n\nYou completely deleted the following comment from the code:\n\n```\n3887\t \t# Since these take some time to compute (480s for l=7, ?? for l=13) we \n3888\t \t# precompute them.  Here is how they may be computed: \n...\n[explanation of how precomputing can be done]\n```",
    "created_at": "2011-08-24T18:59:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120687",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:4" align="right">comment:4</div>

**Complaint:**

You completely deleted the following comment from the code:

```
3887	 	# Since these take some time to compute (480s for l=7, ?? for l=13) we 
3888	 	# precompute them.  Here is how they may be computed: 
...
[explanation of how precomputing can be done]
```



---

archive/issue_comments_120688.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\n(and I think a comment about how big precomputations were done is the most important thing to keep.)",
    "created_at": "2011-08-24T19:00:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120688",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:5" align="right">comment:5</div>

(and I think a comment about how big precomputations were done is the most important thing to keep.)



---

archive/issue_comments_120689.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nWow, I completely missed the boat there. Never mind regarding my above comments.  They are the same as Robert's.",
    "created_at": "2011-08-24T19:01:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120689",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:6" align="right">comment:6</div>

Wow, I completely missed the boat there. Never mind regarding my above comments.  They are the same as Robert's.



---

archive/issue_comments_120690.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nThe claimed code for pre-computing Psi does not work at all.   Even changing the monic() part to do what is intended totally fails to give the claimed result.   Some remarks:\n\n1. This code was originally introduced by John Cremona and his undergrad student in #6887, and given a positive review by Christian Wuthrich.  At that point the comment about how to compute the polys should have been changed to a doctest.  Oops.\n\n2. One could not use the examples (if they worked) to compute more values of Psi, since they depend on the Fricke module table, which is only computed up to ell=13.  \n\n3. The definition and meaning of Psi should be documented.  I spent maybe 30 minutes wondering and searching, and I still am not completely sure *precisely* what Psi even is...\n\nSo I suggest removing these broken examples from the second patch, then setting this patch as \"needs review\".  I'll ping John Cremona and request that he add an appropriate doctest illustrating how to compute Psi, and documenting what it is.  I'm betting this will be really trivial for him to do. (I'm surprised it wasn't for me, because I just did a lot with explicit isogenies for an REU this summer... but it wasn't.)",
    "created_at": "2011-08-24T19:58:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120690",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:7" align="right">comment:7</div>

The claimed code for pre-computing Psi does not work at all.   Even changing the monic() part to do what is intended totally fails to give the claimed result.   Some remarks:

1. This code was originally introduced by John Cremona and his undergrad student in #6887, and given a positive review by Christian Wuthrich.  At that point the comment about how to compute the polys should have been changed to a doctest.  Oops.

2. One could not use the examples (if they worked) to compute more values of Psi, since they depend on the Fricke module table, which is only computed up to ell=13.  

3. The definition and meaning of Psi should be documented.  I spent maybe 30 minutes wondering and searching, and I still am not completely sure *precisely* what Psi even is...

So I suggest removing these broken examples from the second patch, then setting this patch as "needs review".  I'll ping John Cremona and request that he add an appropriate doctest illustrating how to compute Psi, and documenting what it is.  I'm betting this will be really trivial for him to do. (I'm surprised it wasn't for me, because I just did a lot with explicit isogenies for an REU this summer... but it wasn't.)



---

archive/issue_comments_120691.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nI pinged John Cremona, whose computer responded \"I am away and will not have internet access until 31 August.\"",
    "created_at": "2011-08-24T20:00:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120691",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:8" align="right">comment:8</div>

I pinged John Cremona, whose computer responded "I am away and will not have internet access until 31 August."



---

archive/issue_comments_120692.json:
```json
{
    "body": "Changed keywords from **startup, initialization** to **startup, initialization, sd32**",
    "created_at": "2011-08-24T23:30:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120692",
    "user": "https://github.com/williamstein"
}
```

Changed keywords from **startup, initialization** to **startup, initialization, sd32**



---

archive/issue_comments_120693.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nSorry for the delay.  I'll document this better and make sure that the current documentation and description of the precomputation fits the existing code -- there are some choice to be made.  Meanwhile, here's a small amount of explanatory theory:\n\nFor l=2,3,5,7,13, the curve `X_0(l)` has genus zero, so its function field is pure transcendental.  The Fricke Module, denoted t, is a generator of the function field such that t=0 and t=oo lie above j=oo; the normalization here is completely classical, and the situation is fully defined by giving j as a degree l+1 rational function in t, which is what the Fricke_module dict in the code does (though not for l=2 which is simpler and handled entirely separately).\n\nNext one writes down a \"universal elliptic curve with l-isogeny\", say `E_t`, as a curve over Q(t) with j-invariant j (viewed as an element of Q(t)).  This is not unique!  I have used different choices in the past and this needs sorting out since the later formulas depend on the choice.  A good choice is one such that `E_t` is defined and with good reduction at all t except t=0 and t=oo (which are irrelevant since those values correspond to j=oo);  but one is forced to also have bad reduction at values of t above j=0 and j=1728 when l=1 (mod 3, resp. 4).  Once the model for `E_t` is chosen, Psi is the monic factor of degree (l-1)/2 of the l-division polynomial of `E_t` which defined the isogeny, and which is used in the isogeny code.\n\nI am writing this up -- there's an old preprint of me and Mark Watkins which never got finished, and now there's more delay since with my student Kimi I am extending all this to the cases where `X_0(l)` is hyperelliptic.",
    "created_at": "2011-09-03T17:22:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120693",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:10" align="right">comment:10</div>

Sorry for the delay.  I'll document this better and make sure that the current documentation and description of the precomputation fits the existing code -- there are some choice to be made.  Meanwhile, here's a small amount of explanatory theory:

For l=2,3,5,7,13, the curve `X_0(l)` has genus zero, so its function field is pure transcendental.  The Fricke Module, denoted t, is a generator of the function field such that t=0 and t=oo lie above j=oo; the normalization here is completely classical, and the situation is fully defined by giving j as a degree l+1 rational function in t, which is what the Fricke_module dict in the code does (though not for l=2 which is simpler and handled entirely separately).

Next one writes down a "universal elliptic curve with l-isogeny", say `E_t`, as a curve over Q(t) with j-invariant j (viewed as an element of Q(t)).  This is not unique!  I have used different choices in the past and this needs sorting out since the later formulas depend on the choice.  A good choice is one such that `E_t` is defined and with good reduction at all t except t=0 and t=oo (which are irrelevant since those values correspond to j=oo);  but one is forced to also have bad reduction at values of t above j=0 and j=1728 when l=1 (mod 3, resp. 4).  Once the model for `E_t` is chosen, Psi is the monic factor of degree (l-1)/2 of the l-division polynomial of `E_t` which defined the isogeny, and which is used in the isogeny code.

I am writing this up -- there's an old preprint of me and Mark Watkins which never got finished, and now there's more delay since with my student Kimi I am extending all this to the cases where `X_0(l)` is hyperelliptic.



---

archive/issue_comments_120694.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nI am currently working on this and will upload a patch when done.",
    "created_at": "2011-09-28T14:56:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120694",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:11" align="right">comment:11</div>

I am currently working on this and will upload a patch when done.



---

archive/issue_comments_120695.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nThe new patch includes the first two.  I have added doctests which show that the cached values are correctly computed.  At the same time I simplified the generic kernels a lot by using a better model for the universal elliptic curve with l-isogeny (you should be able to see that a lot more is deleted than added in the patch).\n\nI added substantial explanation in the docstrings.  So I hope this will be acceptable!  (I forgot to check rebuilding the docs, and will do that now to try to catch any ReST markup problems.)",
    "created_at": "2011-09-29T15:54:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120695",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:12" align="right">comment:12</div>

The new patch includes the first two.  I have added doctests which show that the cached values are correctly computed.  At the same time I simplified the generic kernels a lot by using a better model for the universal elliptic curve with l-isogeny (you should be able to see that a lot more is deleted than added in the patch).

I added substantial explanation in the docstrings.  So I hope this will be acceptable!  (I forgot to check rebuilding the docs, and will do that now to try to catch any ReST markup problems.)



---

archive/issue_comments_120696.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -19,3 +19,7 @@\n user    0m0.672s\n sys     0m0.164s\n ```\n+\n+**Apply:**\n+\n+1. [attachment: trac_11683_ell_curve_isogeny_speedup_combined.patch](https://github.com/sagemath/sage/files/ticket11683/trac_11683_ell_curve_isogeny_speedup_combined.patch.gz)\n``````\n",
    "created_at": "2011-09-29T15:54:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120696",
    "user": "https://github.com/JohnCremona"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -19,3 +19,7 @@
 user    0m0.672s
 sys     0m0.164s
 ```
+
+**Apply:**
+
+1. [attachment: trac_11683_ell_curve_isogeny_speedup_combined.patch](https://github.com/sagemath/sage/files/ticket11683/trac_11683_ell_curve_isogeny_speedup_combined.patch.gz)
``````




---

archive/issue_comments_120697.json:
```json
{
    "body": "Changed author from **Julian Rueth** to **Julian Rueth, John Cremona**",
    "created_at": "2011-09-29T15:54:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120697",
    "user": "https://github.com/JohnCremona"
}
```

Changed author from **Julian Rueth** to **Julian Rueth, John Cremona**



---

archive/issue_events_155192.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2011-09-29T15:54:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155192"
}
```



---

archive/issue_comments_120698.json:
```json
{
    "body": "Replaces both previous patches; applies to 4.7.2.alpha2",
    "created_at": "2011-09-29T16:19:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120698",
    "user": "https://github.com/JohnCremona"
}
```

Replaces both previous patches; applies to 4.7.2.alpha2



---

archive/issue_comments_120699.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nAttachment: **[trac_11683_ell_curve_isogeny_speedup_combined.patch.gz](https://github.com/sagemath/sage/files/ticket11683/trac_11683_ell_curve_isogeny_speedup_combined.patch.gz)**\n\nThere were some markup problems, now fixed and the patch replaced.",
    "created_at": "2011-09-29T16:20:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120699",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:13" align="right">comment:13</div>

Attachment: **[trac_11683_ell_curve_isogeny_speedup_combined.patch.gz](https://github.com/sagemath/sage/files/ticket11683/trac_11683_ell_curve_isogeny_speedup_combined.patch.gz)**

There were some markup problems, now fixed and the patch replaced.



---

archive/issue_comments_120700.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nI can now compute the generic kernels in negligible time for l=2,3,5,7 and only 18s for l=13 (compared to over 10 hours!).  If people think it is worth it I will revise the patch accordingly.  Then we can test 2,3,5,7 every time and l=13 with a # long time tag.",
    "created_at": "2011-09-30T16:00:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120700",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:14" align="right">comment:14</div>

I can now compute the generic kernels in negligible time for l=2,3,5,7 and only 18s for l=13 (compared to over 10 hours!).  If people think it is worth it I will revise the patch accordingly.  Then we can test 2,3,5,7 every time and l=13 with a # long time tag.



---

archive/issue_comments_120701.json:
```json
{
    "body": "Changed keywords from **startup, initialization, sd32** to **startup, initialization, sd32, sd35**",
    "created_at": "2011-12-21T16:49:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120701",
    "user": "https://github.com/saraedum"
}
```

Changed keywords from **startup, initialization, sd32** to **startup, initialization, sd32, sd35**



---

archive/issue_comments_120702.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nReplying to [@JohnCremona](#comment%3A14):\n> I can now compute the generic kernels in negligible time for l=2,3,5,7 and only 18s for l=13 (compared to over 10 hours!).  If people think it is worth it I will revise the patch accordingly.  Then we can test 2,3,5,7 every time and l=13 with a # long time tag.\n\nJust a remark, it seems sage improved quite a bit on the underlying computation. It is quite a bit faster than 13 hours (that is what the docstring says) already:\n\n```\nsage: time a,b=Psi(13, use_stored=True),Psi(13, use_stored=False)\nTime: CPU 178.47 s, Wall: 178.51 s\nsage: a==b\nTrue\n```",
    "created_at": "2011-12-21T16:49:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120702",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:15" align="right">comment:15</div>

Replying to [@JohnCremona](#comment%3A14):
> I can now compute the generic kernels in negligible time for l=2,3,5,7 and only 18s for l=13 (compared to over 10 hours!).  If people think it is worth it I will revise the patch accordingly.  Then we can test 2,3,5,7 every time and l=13 with a # long time tag.

Just a remark, it seems sage improved quite a bit on the underlying computation. It is quite a bit faster than 13 hours (that is what the docstring says) already:

```
sage: time a,b=Psi(13, use_stored=True),Psi(13, use_stored=False)
Time: CPU 178.47 s, Wall: 178.51 s
sage: a==b
True
```



---

archive/issue_comments_120703.json:
```json
{
    "body": "Reviewer: **Julian Rueth, Frithjof Schulze**",
    "created_at": "2011-12-21T21:27:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120703",
    "user": "https://github.com/saraedum"
}
```

Reviewer: **Julian Rueth, Frithjof Schulze**



---

archive/issue_comments_120704.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">comment:17</div>\n\n[the docstring said \"10\" hours of course]\n\nWe fixed a few typos. Looking at the changes to `isogenies_prime_degree_genus_0()` we're wondering if this actually adds any new functionality/fixes a problem? If so, shouldn't there be a doctest for it?",
    "created_at": "2011-12-21T21:42:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120704",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:17" align="right">comment:17</div>

[the docstring said "10" hours of course]

We fixed a few typos. Looking at the changes to `isogenies_prime_degree_genus_0()` we're wondering if this actually adds any new functionality/fixes a problem? If so, shouldn't there be a doctest for it?



---

archive/issue_events_155193.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2011-12-21T21:42:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155193"
}
```



---

archive/issue_events_155194.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2011-12-21T21:42:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155194"
}
```



---

archive/issue_comments_120705.json:
```json
{
    "body": "fixes some typos",
    "created_at": "2011-12-21T21:44:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120705",
    "user": "https://github.com/saraedum"
}
```

fixes some typos



---

archive/issue_comments_120706.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">comment:18</div>\n\nAttachment: **[trac_11683_review.patch.gz](https://github.com/sagemath/sage/files/ticket11683/trac_11683_review.patch.gz)**",
    "created_at": "2011-12-21T21:50:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120706",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:18" align="right">comment:18</div>

Attachment: **[trac_11683_review.patch.gz](https://github.com/sagemath/sage/files/ticket11683/trac_11683_review.patch.gz)**



---

archive/issue_comments_120707.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -23,3 +23,6 @@\n **Apply:**\n \n 1. [attachment: trac_11683_ell_curve_isogeny_speedup_combined.patch](https://github.com/sagemath/sage/files/ticket11683/trac_11683_ell_curve_isogeny_speedup_combined.patch.gz)\n+2. [attachment: trac_11683_review.patch](https://github.com/sagemath/sage/files/ticket11683/trac_11683_review.patch.gz)\n+\n+to the sage repository.\n``````\n",
    "created_at": "2011-12-21T21:50:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120707",
    "user": "https://github.com/saraedum"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -23,3 +23,6 @@
 **Apply:**
 
 1. [attachment: trac_11683_ell_curve_isogeny_speedup_combined.patch](https://github.com/sagemath/sage/files/ticket11683/trac_11683_ell_curve_isogeny_speedup_combined.patch.gz)
+2. [attachment: trac_11683_review.patch](https://github.com/sagemath/sage/files/ticket11683/trac_11683_review.patch.gz)
+
+to the sage repository.
``````




---

archive/issue_events_155195.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2011-12-22T14:55:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155195"
}
```



---

archive/issue_events_155196.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2011-12-22T14:55:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155196"
}
```



---

archive/issue_comments_120708.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">comment:19</div>\n\nReplying to [@saraedum](#comment%3A17):\n> [the docstring said \"10\" hours of course]\n> \n> We fixed a few typos. Looking at the changes to `isogenies_prime_degree_genus_0()` we're wondering if this actually adds any new functionality/fixes a problem? If so, shouldn't there be a doctest for it?\n\nThe algorithm has been slightly simplified in the following ways:  it works with the Fricke polynomials which are ZZ[t] rather than the actual Fricke modules (which are rational functions with denominator t), thus avoiding the creation of rational functions.  Secondly, it uses a different model for the universal elliptic curve defined over Q(t), which is integral over Z[t] and has good reduction at as many places as possible.  This makes the generic kernel polynomials lie in Z[t][x] rather than Q(t)[x], and also makes them simpler.\n\nThe previous version was not incorrect at all, but this seemed to be the right time to replace the older models with new ones, since the code was being refactored anyway.\n\nThanks for fixing the typos;  I have set the ticket back to \"needs review\".",
    "created_at": "2011-12-22T14:55:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120708",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:19" align="right">comment:19</div>

Replying to [@saraedum](#comment%3A17):
> [the docstring said "10" hours of course]
> 
> We fixed a few typos. Looking at the changes to `isogenies_prime_degree_genus_0()` we're wondering if this actually adds any new functionality/fixes a problem? If so, shouldn't there be a doctest for it?

The algorithm has been slightly simplified in the following ways:  it works with the Fricke polynomials which are ZZ[t] rather than the actual Fricke modules (which are rational functions with denominator t), thus avoiding the creation of rational functions.  Secondly, it uses a different model for the universal elliptic curve defined over Q(t), which is integral over Z[t] and has good reduction at as many places as possible.  This makes the generic kernel polynomials lie in Z[t][x] rather than Q(t)[x], and also makes them simpler.

The previous version was not incorrect at all, but this seemed to be the right time to replace the older models with new ones, since the code was being refactored anyway.

Thanks for fixing the typos;  I have set the ticket back to "needs review".



---

archive/issue_comments_120709.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">comment:20</div>\n\nPlease re-review this patch so that it can go into 5.0!!!!  The last review was positive apart from some trivial typos, but several months later it has not yet been merged just because it was reset to \"needs review\"",
    "created_at": "2012-02-24T10:06:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120709",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:20" align="right">comment:20</div>

Please re-review this patch so that it can go into 5.0!!!!  The last review was positive apart from some trivial typos, but several months later it has not yet been merged just because it was reset to "needs review"



---

archive/issue_comments_120710.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">comment:21</div>\n\nThe patches apply fine to 5.0.beta5 and all the tests in elliptic_curves pass, so please will someone give this a positive review!!!!",
    "created_at": "2012-02-24T10:14:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120710",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:21" align="right">comment:21</div>

The patches apply fine to 5.0.beta5 and all the tests in elliptic_curves pass, so please will someone give this a positive review!!!!



---

archive/issue_comments_120711.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">comment:22</div>\n\nReplying to [@JohnCremona](#comment%3A21):\n\nI have very little time right now, but will re-review the patch tomorrow evening.",
    "created_at": "2012-02-24T21:02:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120711",
    "user": "https://github.com/sagetrac-fschulze"
}
```

<div id="comment:22" align="right">comment:22</div>

Replying to [@JohnCremona](#comment%3A21):

I have very little time right now, but will re-review the patch tomorrow evening.



---

archive/issue_events_155197.json:
```json
{
    "actor": "https://github.com/sagetrac-fschulze",
    "created_at": "2012-02-25T21:22:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155197"
}
```



---

archive/issue_events_155198.json:
```json
{
    "actor": "https://github.com/sagetrac-fschulze",
    "created_at": "2012-02-25T21:22:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155198"
}
```



---

archive/issue_comments_120712.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">comment:23</div>\n\nThe documentation for Psi is missing some of the information from your comment above: It states the places of bad reduction of E_t but doesn't mention that E_t is a 'universal elliptic curve with l-isogeny' and that the model was specifically chosen for it's reduction behaviour. Shouldn't this information also be in the docstring?\n\nEverything else looks OK (with the doctests still running).",
    "created_at": "2012-02-25T21:22:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120712",
    "user": "https://github.com/sagetrac-fschulze"
}
```

<div id="comment:23" align="right">comment:23</div>

The documentation for Psi is missing some of the information from your comment above: It states the places of bad reduction of E_t but doesn't mention that E_t is a 'universal elliptic curve with l-isogeny' and that the model was specifically chosen for it's reduction behaviour. Shouldn't this information also be in the docstring?

Everything else looks OK (with the doctests still running).



---

archive/issue_comments_120713.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">comment:24</div>\n\nReplying to [@sagetrac-fschulze](#comment%3A23):\n> The documentation for Psi is missing some of the information from your comment above: It states the places of bad reduction of E_t but doesn't mention that E_t is a 'universal elliptic curve with l-isogeny' and that the model was specifically chosen for it's reduction behaviour. Shouldn't this information also be in the docstring?\n> \n\nYou may be right, but it's a docstring not a textbook or thesis...we are currently finishing similar (though more complicated) code for all the cases where X_0(l) is hyperelliptic, and also getting these genus 0 cases to apply in characteristics 2 and 3, so there will be a lot more to say then (at which point we'll move this code into a separate file too).\n\n> Everything else looks OK (with the doctests still running).",
    "created_at": "2012-02-25T23:01:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120713",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:24" align="right">comment:24</div>

Replying to [@sagetrac-fschulze](#comment%3A23):
> The documentation for Psi is missing some of the information from your comment above: It states the places of bad reduction of E_t but doesn't mention that E_t is a 'universal elliptic curve with l-isogeny' and that the model was specifically chosen for it's reduction behaviour. Shouldn't this information also be in the docstring?
> 

You may be right, but it's a docstring not a textbook or thesis...we are currently finishing similar (though more complicated) code for all the cases where X_0(l) is hyperelliptic, and also getting these genus 0 cases to apply in characteristics 2 and 3, so there will be a lot more to say then (at which point we'll move this code into a separate file too).

> Everything else looks OK (with the doctests still running).



---

archive/issue_comments_120714.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">comment:25</div>\n\nI think that the docstrings already are very long too.\n\nIs there maybe a reference one could add? Julian and I tried to understand this patch and didn't found many references. The preprint you mentioned above only has 'Fricke, Not sure which book'. I believe adding such a reference would be a lot of help for other people in the future.",
    "created_at": "2012-02-26T07:46:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120714",
    "user": "https://github.com/sagetrac-fschulze"
}
```

<div id="comment:25" align="right">comment:25</div>

I think that the docstrings already are very long too.

Is there maybe a reference one could add? Julian and I tried to understand this patch and didn't found many references. The preprint you mentioned above only has 'Fricke, Not sure which book'. I believe adding such a reference would be a lot of help for other people in the future.



---

archive/issue_comments_120715.json:
```json
{
    "body": "<div id=\"comment:26\" align=\"right\">comment:26</div>\n\nOne reason the docstrings are long is that I was asked (e.g. on this ticket) to give an explanation.  So please do not now ask me to shorten them.  As for references, as I have already said, there is an old unpublished preprint by me and Mark Watkins, which has now been superceded (we now use better models), and there will be a PhD thesis by Kimi Tsukazaki, but he has not written it yet.  I hope you are not suggesting that the code cannot be accepted into Sage until his thesis has been written and examined and passed?  On the contrary, I was hoping that the code based on his thesis (which includes  many more prime l as I have already said) would be accepted into Sage -- on its merits because it works -- before he submits the thesis.\n\nI feel I am going round in circles here.  This ticket was created because the old code was causing an unnecessary delay in Sage startup time, and the patch fixes that, and clarifies how the code works at the same time.",
    "created_at": "2012-02-26T12:21:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120715",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:26" align="right">comment:26</div>

One reason the docstrings are long is that I was asked (e.g. on this ticket) to give an explanation.  So please do not now ask me to shorten them.  As for references, as I have already said, there is an old unpublished preprint by me and Mark Watkins, which has now been superceded (we now use better models), and there will be a PhD thesis by Kimi Tsukazaki, but he has not written it yet.  I hope you are not suggesting that the code cannot be accepted into Sage until his thesis has been written and examined and passed?  On the contrary, I was hoping that the code based on his thesis (which includes  many more prime l as I have already said) would be accepted into Sage -- on its merits because it works -- before he submits the thesis.

I feel I am going round in circles here.  This ticket was created because the old code was causing an unnecessary delay in Sage startup time, and the patch fixes that, and clarifies how the code works at the same time.



---

archive/issue_events_155199.json:
```json
{
    "actor": "https://github.com/sagetrac-fschulze",
    "created_at": "2012-02-26T14:31:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155199"
}
```



---

archive/issue_events_155200.json:
```json
{
    "actor": "https://github.com/sagetrac-fschulze",
    "created_at": "2012-02-26T14:31:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155200"
}
```



---

archive/issue_comments_120716.json:
```json
{
    "body": "<div id=\"comment:27\" align=\"right\">comment:27</div>\n\nI didn't mean to suggest that the code can not be accepted. It obviously is an improvement and I just wanted to know if the documentation could be improved by adding a reference.\n\nI will give the patch a positive review now. Further improvements to the documentation will probably happen when your more general code arrives.",
    "created_at": "2012-02-26T14:31:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120716",
    "user": "https://github.com/sagetrac-fschulze"
}
```

<div id="comment:27" align="right">comment:27</div>

I didn't mean to suggest that the code can not be accepted. It obviously is an improvement and I just wanted to know if the documentation could be improved by adding a reference.

I will give the patch a positive review now. Further improvements to the documentation will probably happen when your more general code arrives.



---

archive/issue_comments_120717.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,7 +8,6 @@\n user    0m0.810s\n sys     0m0.154s\n ```\n-\n Lazily computing the constants in this file helps a lot (see the attached patch):\n \n ```\n@@ -19,7 +18,6 @@\n user    0m0.672s\n sys     0m0.164s\n ```\n-\n **Apply:**\n \n 1. [attachment: trac_11683_ell_curve_isogeny_speedup_combined.patch](https://github.com/sagemath/sage/files/ticket11683/trac_11683_ell_curve_isogeny_speedup_combined.patch.gz)\n``````\n",
    "created_at": "2012-02-26T14:35:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120717",
    "user": "https://github.com/sagetrac-fschulze"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,7 +8,6 @@
 user    0m0.810s
 sys     0m0.154s
 ```
-
 Lazily computing the constants in this file helps a lot (see the attached patch):
 
 ```
@@ -19,7 +18,6 @@
 user    0m0.672s
 sys     0m0.164s
 ```
-
 **Apply:**
 
 1. [attachment: trac_11683_ell_curve_isogeny_speedup_combined.patch](https://github.com/sagemath/sage/files/ticket11683/trac_11683_ell_curve_isogeny_speedup_combined.patch.gz)
``````




---

archive/issue_events_155201.json:
```json
{
    "actor": "https://github.com/sagetrac-fschulze",
    "created_at": "2012-02-26T14:35:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155201"
}
```



---

archive/issue_events_155202.json:
```json
{
    "actor": "https://github.com/sagetrac-fschulze",
    "created_at": "2012-02-26T14:35:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155202"
}
```



---

archive/issue_comments_120718.json:
```json
{
    "body": "<div id=\"comment:28\" align=\"right\">comment:28</div>\n\nTested with sage-5.0.beta5. The patch applies fine and all doctests pass.",
    "created_at": "2012-02-26T14:35:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120718",
    "user": "https://github.com/sagetrac-fschulze"
}
```

<div id="comment:28" align="right">comment:28</div>

Tested with sage-5.0.beta5. The patch applies fine and all doctests pass.



---

archive/issue_comments_120719.json:
```json
{
    "body": "<div id=\"comment:29\" align=\"right\">comment:29</div>\n\nReplying to [@sagetrac-fschulze](#comment%3A27):\n> I didn't mean to suggest that the code can not be accepted. It obviously is an improvement and I just wanted to know if the documentation could be improved by adding a reference.\n> \n> I will give the patch a positive review now. Further improvements to the documentation will probably happen when your more general code arrives.\n\nThanks.",
    "created_at": "2012-02-26T14:57:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120719",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:29" align="right">comment:29</div>

Replying to [@sagetrac-fschulze](#comment%3A27):
> I didn't mean to suggest that the code can not be accepted. It obviously is an improvement and I just wanted to know if the documentation could be improved by adding a reference.
> 
> I will give the patch a positive review now. Further improvements to the documentation will probably happen when your more general code arrives.

Thanks.



---

archive/issue_comments_120720.json:
```json
{
    "body": "Merged: **sage-5.0.beta7**",
    "created_at": "2012-03-04T21:19:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11683#issuecomment-120720",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta7**



---

archive/issue_events_155203.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-04T21:19:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155203"
}
```



---

archive/issue_events_155204.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-04T21:19:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11683",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11683#event-155204"
}
```

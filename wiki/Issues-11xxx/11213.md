# Issue 11213: ModularCurve

archive/issues_011041.json:
```json
{
    "assignees": [
        "https://github.com/craigcitro"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThere is a class ModularCurve which needs to be built. It has a spot open in SAGE_ROOT/devel/sage/sage/modular/curves/ \n\nThere is already a folder created. Notes1.txt is attached.\n\nComponent: **modular forms**\n\nKeywords: **Modular Curve, Modular Polynomials**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11213_\n\n",
    "created_at": "2011-04-18T19:26:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "ModularCurve",
    "type": "issue",
    "updated_at": "2011-04-18T19:26:49Z",
    "url": "https://github.com/sagemath/sage/issues/11213",
    "user": "https://github.com/tdupu"
}
```
<div id="comment:0"></div>

There is a class ModularCurve which needs to be built. It has a spot open in SAGE_ROOT/devel/sage/sage/modular/curves/ 

There is already a folder created. Notes1.txt is attached.

Component: **modular forms**

Keywords: **Modular Curve, Modular Polynomials**

_Issue created by migration from https://trac.sagemath.org/ticket/11213_





---

archive/issue_events_147019.json:
```json
{
    "actor": "https://github.com/tdupu",
    "created_at": "2011-04-18T19:26:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11213",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
    "label_color": "0000ff",
    "label_name": "c: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11213#event-147019"
}
```



---

archive/issue_events_147020.json:
```json
{
    "actor": "https://github.com/tdupu",
    "created_at": "2011-04-18T19:26:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11213",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11213#event-147020"
}
```



---

archive/issue_events_147021.json:
```json
{
    "actor": "https://github.com/tdupu",
    "created_at": "2011-04-18T19:26:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11213",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11213#event-147021"
}
```



---

archive/issue_events_147022.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2011-04-18T19:26:49Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11213",
    "subject": "https://github.com/tdupu",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11213#event-147022"
}
```



---

archive/issue_comments_111892.json:
```json
{
    "body": "Attachment: **[idea1.txt](https://github.com/sagemath/sage/files/ticket11213/idea1.txt)**\n\ntxt file copy of email contained sage/modular/curves/notes",
    "created_at": "2011-04-18T19:28:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11213",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11213#issuecomment-111892",
    "user": "https://github.com/tdupu"
}
```

Attachment: **[idea1.txt](https://github.com/sagemath/sage/files/ticket11213/idea1.txt)**

txt file copy of email contained sage/modular/curves/notes

# Issue 11078: jmol in the flask notebook

archive/issues_010955.json:
```json
{
    "body": "The attached patch solves an issue with jmol finding the right .jmol.zip file in the new flask notebook.  This patch should only be applied *after* the notebook is moved to a flask notebook.\n\nThe patch should replace the following line in `sage/plot/plot3d/base.pyx`\n\n```\nf.write('set defaultdirectory \"%s\"\\n' % archive_name)\n```\nwith\n\n```\nimport sagenb\nf.write('set defaultdirectory \"cells/%s/%s\"\\n' % (sagenb.notebook.interact.SAGE_CELL_ID, archive_name))\n```\n\n\nDepends on #11080\n\n**Assignee:** @jasongrout, @qed777, @williamstein\n\n**CC:**  rkirov @mwhansen @kcrisman\n\n**Keywords:** sd31 sd35.5\n\n**Reviewer:** Jason Grout\n\n**Author:** William Stein, Jonathan Gutow\n\n**Merged:** sage-5.2.beta0\n\nIssue created by migration from https://trac.sagemath.org/ticket/11078\n\n",
    "closed_at": "2012-07-25T06:40:59Z",
    "created_at": "2011-03-29T11:47:49Z",
    "labels": [
        "component: notebook",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.2",
    "title": "jmol in the flask notebook",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/11078",
    "user": "https://github.com/jasongrout"
}
```
The attached patch solves an issue with jmol finding the right .jmol.zip file in the new flask notebook.  This patch should only be applied *after* the notebook is moved to a flask notebook.

The patch should replace the following line in `sage/plot/plot3d/base.pyx`

```
f.write('set defaultdirectory "%s"\n' % archive_name)
```
with

```
import sagenb
f.write('set defaultdirectory "cells/%s/%s"\n' % (sagenb.notebook.interact.SAGE_CELL_ID, archive_name))
```


Depends on #11080

**Assignee:** @jasongrout, @qed777, @williamstein

**CC:**  rkirov @mwhansen @kcrisman

**Keywords:** sd31 sd35.5

**Reviewer:** Jason Grout

**Author:** William Stein, Jonathan Gutow

**Merged:** sage-5.2.beta0

Issue created by migration from https://trac.sagemath.org/ticket/11078





---

archive/issue_comments_111323.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -12,3 +12,5 @@\n f.write('set defaultdirectory \"cells/%s/%s\"\\n' % (sagenb.notebook.interact.SAGE_CELL_ID, archive_name))\n ```\n \n+Note: I haven't actually attached a patch since I hope that we can incorporate the new Jmol in, which would conflict with the patch (but would not conflict with the actual changes on this ticket).\n+\n``````\n",
    "created_at": "2011-03-30T04:49:21Z",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11078#issuecomment-111323",
    "user": "https://github.com/jasongrout"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -12,3 +12,5 @@
 f.write('set defaultdirectory "cells/%s/%s"\n' % (sagenb.notebook.interact.SAGE_CELL_ID, archive_name))
 ```
 
+Note: I haven't actually attached a patch since I hope that we can incorporate the new Jmol in, which would conflict with the patch (but would not conflict with the actual changes on this ticket).
+
``````




---

archive/issue_comments_111324.json:
```json
{
    "body": "**Changing keywords** from \"\" to \"sd31\".",
    "created_at": "2011-06-14T17:10:22Z",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11078#issuecomment-111324",
    "user": "https://github.com/williamstein"
}
```

**Changing keywords** from "" to "sd31".



---

archive/issue_comments_111325.json:
```json
{
    "body": "<a id='comment:4'></a>\nThis week, I compiled sage-4.7.1.rc0 and installed the new flask Sage notebook following instructions from : http://code.google.com/r/rkirov-flask/. I also followed the section `Adding stock Jmol` plus the patch `trac_11078.patch`. This made Jmol to work from the notebook : Great!\n\nBut, from the command line, with the patch `trac_11078.patch` a command like `sphere()` opens Jmol but Jmol stays all black. I had to mimic the modifications done in #11503 on the same file `sage/plot/plot3d/base.pyx` to make Jmol works properly from the command line.\n\nS\u00e9bastien",
    "created_at": "2011-07-25T19:50:58Z",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11078#issuecomment-111325",
    "user": "https://github.com/seblabbe"
}
```

<a id='comment:4'></a>
This week, I compiled sage-4.7.1.rc0 and installed the new flask Sage notebook following instructions from : http://code.google.com/r/rkirov-flask/. I also followed the section `Adding stock Jmol` plus the patch `trac_11078.patch`. This made Jmol to work from the notebook : Great!

But, from the command line, with the patch `trac_11078.patch` a command like `sphere()` opens Jmol but Jmol stays all black. I had to mimic the modifications done in #11503 on the same file `sage/plot/plot3d/base.pyx` to make Jmol works properly from the command line.

Sébastien



---

archive/issue_events_086886.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-21T09:28:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "milestone": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86886"
}
```



---

archive/issue_events_086887.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-21T09:28:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "milestone": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86887"
}
```



---

archive/issue_comments_111326.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -12,5 +12,3 @@\n f.write('set defaultdirectory \"cells/%s/%s\"\\n' % (sagenb.notebook.interact.SAGE_CELL_ID, archive_name))\n ```\n \n-Note: I haven't actually attached a patch since I hope that we can incorporate the new Jmol in, which would conflict with the patch (but would not conflict with the actual changes on this ticket).\n-\n``````\n",
    "created_at": "2011-12-31T15:45:10Z",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11078#issuecomment-111326",
    "user": "https://github.com/jasongrout"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -12,5 +12,3 @@
 f.write('set defaultdirectory "cells/%s/%s"\n' % (sagenb.notebook.interact.SAGE_CELL_ID, archive_name))
 ```
 
-Note: I haven't actually attached a patch since I hope that we can incorporate the new Jmol in, which would conflict with the patch (but would not conflict with the actual changes on this ticket).
-
``````




---

archive/issue_events_086888.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2011-12-31T16:38:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86888"
}
```



---

archive/issue_comments_111327.json:
```json
{
    "body": "**Author:** William Stein, Jonathan Gutow",
    "created_at": "2011-12-31T16:38:17Z",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11078#issuecomment-111327",
    "user": "https://github.com/jasongrout"
}
```

**Author:** William Stein, Jonathan Gutow



---

archive/attachments_015046.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11078.patch",
    "asset_url": "tarball://root/attachments/ticket11078/trac_11078.patch",
    "created_at": "2011-12-31T16:38:17Z",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11078/trac_11078.patch",
    "user": "https://github.com/jasongrout"
}
```



---

archive/issue_comments_111328.json:
```json
{
    "body": "<a id='comment:7'></a>\n**Attachment:** [trac_11078.patch](https://github.com/sagemath/sage/files/ticket11078/trac_11078.patch)\n\nI updated the patch with the changes from #11503.  With the flask notebook, sphere() works both from the command line and from the notebook.\n\nI'm setting as needs review to have one other person double-check this patch works as intended, though I guess I might qualify as the only needed reviewer.  Basically, what I did here was just rebase the patch at #11503.",
    "created_at": "2011-12-31T16:38:17Z",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11078#issuecomment-111328",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:7'></a>
**Attachment:** [trac_11078.patch](https://github.com/sagemath/sage/files/ticket11078/trac_11078.patch)

I updated the patch with the changes from #11503.  With the flask notebook, sphere() works both from the command line and from the notebook.

I'm setting as needs review to have one other person double-check this patch works as intended, though I guess I might qualify as the only needed reviewer.  Basically, what I did here was just rebase the patch at #11503.



---

archive/issue_comments_111329.json:
```json
{
    "body": "**Reviewer:** Jason Grout",
    "created_at": "2011-12-31T16:38:17Z",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11078#issuecomment-111329",
    "user": "https://github.com/jasongrout"
}
```

**Reviewer:** Jason Grout



---

archive/issue_events_086889.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-01-06T03:34:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86889"
}
```



---

archive/issue_events_086890.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-01-06T03:34:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86890"
}
```



---

archive/issue_comments_111330.json:
```json
{
    "body": "<a id='comment:8'></a>\nThis looks great to me!",
    "created_at": "2012-01-06T03:34:22Z",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11078#issuecomment-111330",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:8'></a>
This looks great to me!



---

archive/issue_comments_111331.json:
```json
{
    "body": "**Dependencies:** #11080",
    "created_at": "2012-01-09T23:03:43Z",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11078#issuecomment-111331",
    "user": "https://github.com/jdemeyer"
}
```

**Dependencies:** #11080



---

archive/issue_comments_111332.json:
```json
{
    "body": "**Changing keywords** from \"sd31\" to \"sd31, sd35.5\".",
    "created_at": "2012-01-10T15:24:07Z",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11078#issuecomment-111332",
    "user": "https://github.com/jasongrout"
}
```

**Changing keywords** from "sd31" to "sd31, sd35.5".



---

archive/issue_comments_111333.json:
```json
{
    "body": "**Changing keywords** from \"sd31, sd35.5\" to \"sd31 sd35.5\".",
    "created_at": "2012-01-10T15:24:17Z",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11078#issuecomment-111333",
    "user": "https://github.com/jasongrout"
}
```

**Changing keywords** from "sd31, sd35.5" to "sd31 sd35.5".



---

archive/issue_events_086891.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-14T14:36:39Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "milestone": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86891"
}
```



---

archive/issue_events_086892.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-14T14:36:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "label": "pending",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86892"
}
```



---

archive/issue_events_086893.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-08T08:40:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "milestone": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86893"
}
```



---

archive/issue_events_086894.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-08T08:40:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "label": "pending",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86894"
}
```



---

archive/issue_events_086895.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-23T21:39:57Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "milestone": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86895"
}
```



---

archive/issue_events_086896.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-23T21:39:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "label": "pending",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86896"
}
```



---

archive/issue_events_086897.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T14:39:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "milestone": "sage-5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86897"
}
```



---

archive/issue_events_086898.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T14:39:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "label": "pending",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86898"
}
```



---

archive/issue_events_086899.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-02T15:21:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86899"
}
```



---

archive/issue_events_086900.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-02T15:21:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86900"
}
```



---

archive/issue_comments_111334.json:
```json
{
    "body": "**Merged:** sage-5.2.beta0",
    "created_at": "2012-07-02T15:21:47Z",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11078#issuecomment-111334",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.2.beta0



---

archive/issue_comments_111335.json:
```json
{
    "body": "<a id='comment:17'></a>\nUnmerging this from sage-5.2 due to the serious security issue at #13270.",
    "created_at": "2012-07-23T06:50:43Z",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11078#issuecomment-111335",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:17'></a>
Unmerging this from sage-5.2 due to the serious security issue at #13270.



---

archive/issue_comments_111336.json:
```json
{
    "body": "**Changing merged** from \"sage-5.2.beta0\" to \"\".",
    "created_at": "2012-07-23T06:50:43Z",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11078#issuecomment-111336",
    "user": "https://github.com/jdemeyer"
}
```

**Changing merged** from "sage-5.2.beta0" to "".



---

archive/issue_events_086901.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-23T06:50:43Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "milestone": "sage-5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86901"
}
```



---

archive/issue_events_086902.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-23T06:50:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "label": "pending",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86902"
}
```



---

archive/issue_events_086903.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-23T06:51:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86903"
}
```



---

archive/issue_events_086904.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-23T06:51:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86904"
}
```



---

archive/issue_events_086905.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-23T06:51:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86905"
}
```



---

archive/issue_comments_111337.json:
```json
{
    "body": "**Merged:** sage-5.2.beta0",
    "created_at": "2012-07-25T06:40:59Z",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11078#issuecomment-111337",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.2.beta0



---

archive/issue_events_086906.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-25T06:40:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "milestone": "sage-5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86906"
}
```



---

archive/issue_events_086907.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-25T06:40:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "label": "pending",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86907"
}
```



---

archive/issue_events_086908.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-25T06:40:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86908"
}
```



---

archive/issue_events_086909.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-25T06:40:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11078",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11078#event-86909"
}
```

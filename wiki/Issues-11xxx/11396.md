# Issue 11396: Patch buildbot fails to understand "Apply" section

archive/issues_011224.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu",
        "https://github.com/haraldschilly"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe following does not work (as seen on #10140):\n\n**Apply**\n1. [attachment: trac_10140_sublattice_intersection.patch](https://github.com/sagemath/sage/files/ticket11396/trac_10140_sublattice_intersection.patch.gz)\n\nThe patch buildbot fails to understand this syntax and will try to apply all attached patches instead.\n\nCC:  @robertwb\n\nComponent: **website/wiki**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11396_\n\n",
    "closed_at": "2014-03-06T13:28:22Z",
    "created_at": "2011-05-28T15:58:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20website%20/%20wiki",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Patch buildbot fails to understand \"Apply\" section",
    "type": "issue",
    "updated_at": "2014-03-06T13:28:22Z",
    "url": "https://github.com/sagemath/sage/issues/11396",
    "user": "https://github.com/vbraun"
}
```
<div id="comment:0"></div>

The following does not work (as seen on #10140):

**Apply**
1. [attachment: trac_10140_sublattice_intersection.patch](https://github.com/sagemath/sage/files/ticket11396/trac_10140_sublattice_intersection.patch.gz)

The patch buildbot fails to understand this syntax and will try to apply all attached patches instead.

CC:  @robertwb

Component: **website/wiki**

_Issue created by migration from https://trac.sagemath.org/ticket/11396_





---

archive/issue_events_150124.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2011-05-28T15:58:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20website%20/%20wiki",
    "label_color": "0075ca",
    "label_name": "c: website / wiki",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11396#event-150124"
}
```



---

archive/issue_events_150125.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2011-05-28T15:58:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11396#event-150125"
}
```



---

archive/issue_events_150126.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2011-05-28T15:58:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11396#event-150126"
}
```



---

archive/issue_events_150127.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2011-05-28T15:58:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11396#event-150127"
}
```



---

archive/issue_events_150128.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2011-05-28T15:58:46Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "subject": "https://github.com/vbraun",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11396#event-150128"
}
```



---

archive/issue_events_150129.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2011-05-28T15:58:46Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "subject": "https://github.com/vbraun",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11396#event-150129"
}
```



---

archive/issue_comments_115225.json:
```json
{
    "body": "Attachment: **[trac_10140_base_cone_on_ppl.patch.gz](https://github.com/sagemath/sage/files/ticket11396/trac_10140_base_cone_on_ppl.patch.gz)**\n\nInitial patch",
    "created_at": "2011-05-28T15:59:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11396#issuecomment-115225",
    "user": "https://github.com/vbraun"
}
```

Attachment: **[trac_10140_base_cone_on_ppl.patch.gz](https://github.com/sagemath/sage/files/ticket11396/trac_10140_base_cone_on_ppl.patch.gz)**

Initial patch



---

archive/issue_comments_115226.json:
```json
{
    "body": "Attachment: **[trac_10140_sublattice_intersection.patch.gz](https://github.com/sagemath/sage/files/ticket11396/trac_10140_sublattice_intersection.patch.gz)**\n\nInitial patch",
    "created_at": "2011-05-28T15:59:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11396#issuecomment-115226",
    "user": "https://github.com/vbraun"
}
```

Attachment: **[trac_10140_sublattice_intersection.patch.gz](https://github.com/sagemath/sage/files/ticket11396/trac_10140_sublattice_intersection.patch.gz)**

Initial patch



---

archive/issue_comments_115227.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-The following does not work (as seen on #10140:\n+The following does not work (as seen on #10140):\n \n **Apply**\n 1. [attachment: trac_10140_sublattice_intersection.patch](https://github.com/sagemath/sage/files/ticket11396/trac_10140_sublattice_intersection.patch.gz)\n``````\n",
    "created_at": "2011-05-28T16:01:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11396#issuecomment-115227",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-The following does not work (as seen on #10140:
+The following does not work (as seen on #10140):
 
 **Apply**
 1. [attachment: trac_10140_sublattice_intersection.patch](https://github.com/sagemath/sage/files/ticket11396/trac_10140_sublattice_intersection.patch.gz)
``````




---

archive/issue_comments_115228.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nTrue. For now see http://wiki.sagemath.org/buildbot to get it to detect the correct list of patches.",
    "created_at": "2011-05-28T16:22:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11396#issuecomment-115228",
    "user": "https://github.com/robertwb"
}
```

<div id="comment:2" align="right">comment:2</div>

True. For now see http://wiki.sagemath.org/buildbot to get it to detect the correct list of patches.



---

archive/issue_comments_115229.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI've added a new comment with a new \"Apply\" line to #10140 and it did reset the patch list. But the last patch on the \"Apply\" line is not being picked up, and I can't figure out why.",
    "created_at": "2011-05-28T16:35:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11396#issuecomment-115229",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:3" align="right">comment:3</div>

I've added a new comment with a new "Apply" line to #10140 and it did reset the patch list. But the last patch on the "Apply" line is not being picked up, and I can't figure out why.



---

archive/issue_events_150130.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-03-06T12:44:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11396#event-150130"
}
```



---

archive/issue_comments_115230.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nSince we have now switched to git, this can be closed.",
    "created_at": "2014-03-06T12:44:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11396#issuecomment-115230",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:4" align="right">comment:4</div>

Since we have now switched to git, this can be closed.



---

archive/issue_events_150131.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-03-06T12:44:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11396#event-150131"
}
```



---

archive/issue_events_150132.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-06T13:28:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11396#event-150132"
}
```



---

archive/issue_events_150133.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-06T13:28:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11396#event-150133"
}
```



---

archive/issue_events_150134.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-06T13:28:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11396#event-150134"
}
```



---

archive/issue_events_150135.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-06T13:28:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11396",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "c6c6c6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11396#event-150135"
}
```

# Issue 11381: Add more simplification methods to symbolic vectors.

archive/issues_011209.json:
```json
{
    "assignees": [
        "https://github.com/burcin"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nFollowup to #11335.\n\nDepends on #11335\n\nComponent: **symbolics**\n\nAuthor: **Robert Bradshaw**\n\nReviewer: **Joris Vankerschaver**\n\nMerged: **sage-4.7.1.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11381_\n\n",
    "closed_at": "2011-06-08T07:14:23Z",
    "created_at": "2011-05-26T06:21:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add more simplification methods to symbolic vectors.",
    "type": "issue",
    "updated_at": "2011-06-08T07:14:23Z",
    "url": "https://github.com/sagemath/sage/issues/11381",
    "user": "https://github.com/robertwb"
}
```
<div id="comment:0"></div>

Followup to #11335.

Depends on #11335

Component: **symbolics**

Author: **Robert Bradshaw**

Reviewer: **Joris Vankerschaver**

Merged: **sage-4.7.1.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/11381_





---

archive/issue_events_149893.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2011-05-26T06:21:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11381#event-149893"
}
```



---

archive/issue_events_149894.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2011-05-26T06:21:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "c: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11381#event-149894"
}
```



---

archive/issue_events_149895.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2011-05-26T06:21:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11381#event-149895"
}
```



---

archive/issue_events_149896.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2011-05-26T06:21:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11381#event-149896"
}
```



---

archive/issue_events_149897.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2011-05-26T06:21:03Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "subject": "https://github.com/robertwb",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11381#event-149897"
}
```



---

archive/issue_events_149898.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2011-05-26T06:24:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11381#event-149898"
}
```



---

archive/issue_comments_114991.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[11381-vector-simplify.patch.gz](https://github.com/sagemath/sage/files/ticket11381/11381-vector-simplify.patch.gz)**",
    "created_at": "2011-05-26T06:24:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11381#issuecomment-114991",
    "user": "https://github.com/robertwb"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[11381-vector-simplify.patch.gz](https://github.com/sagemath/sage/files/ticket11381/11381-vector-simplify.patch.gz)**



---

archive/issue_comments_114992.json:
```json
{
    "body": "Dependencies: **#11335**",
    "created_at": "2011-05-26T06:24:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11381#issuecomment-114992",
    "user": "https://github.com/robertwb"
}
```

Dependencies: **#11335**



---

archive/issue_comments_114993.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nNice solution!  I'm ready to give this a positive review, but is there a reason why you didn't replace `simplify_full` by an element-wise method as well?",
    "created_at": "2011-05-29T06:47:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11381#issuecomment-114993",
    "user": "https://github.com/jvkersch"
}
```

<div id="comment:2" align="right">comment:2</div>

Nice solution!  I'm ready to give this a positive review, but is there a reason why you didn't replace `simplify_full` by an element-wise method as well?



---

archive/issue_comments_114994.json:
```json
{
    "body": "Reviewer: **Joris Vankerschaver**",
    "created_at": "2011-05-29T06:47:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11381#issuecomment-114994",
    "user": "https://github.com/jvkersch"
}
```

Reviewer: **Joris Vankerschaver**



---

archive/issue_comments_114995.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nThe only reason was that didn't want to trash the custom docstring.",
    "created_at": "2011-05-29T08:13:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11381#issuecomment-114995",
    "user": "https://github.com/robertwb"
}
```

<div id="comment:3" align="right">comment:3</div>

The only reason was that didn't want to trash the custom docstring.



---

archive/issue_comments_114996.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nAttachment: **[11381-vector-simplify-reviewer-changes.patch.gz](https://github.com/sagemath/sage/files/ticket11381/11381-vector-simplify-reviewer-changes.patch.gz)**\n\nHi Robert,\n\nGood point, but I would still advocate replacing `simplify_full` by an elementwise method: not only does it restore the symmetry between `simplify_full` and the other elementwise methods, your code is also more robust and generic.  And since the docstring for `apply` already has a test for `simplify_full`, the changes are only minimal.\n\nI've gone ahead and made this change to your patch.  If you agree with the changes, I'm ready to give this a positive review (not sure it's OK to do that, but the changes are very minimal).  If not, please delete my patch and I'll approve your original version instead.",
    "created_at": "2011-05-29T23:05:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11381#issuecomment-114996",
    "user": "https://github.com/jvkersch"
}
```

<div id="comment:4" align="right">comment:4</div>

Attachment: **[11381-vector-simplify-reviewer-changes.patch.gz](https://github.com/sagemath/sage/files/ticket11381/11381-vector-simplify-reviewer-changes.patch.gz)**

Hi Robert,

Good point, but I would still advocate replacing `simplify_full` by an elementwise method: not only does it restore the symmetry between `simplify_full` and the other elementwise methods, your code is also more robust and generic.  And since the docstring for `apply` already has a test for `simplify_full`, the changes are only minimal.

I've gone ahead and made this change to your patch.  If you agree with the changes, I'm ready to give this a positive review (not sure it's OK to do that, but the changes are very minimal).  If not, please delete my patch and I'll approve your original version instead.



---

archive/issue_comments_114997.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nYour change looks good, though I noticed that the patch didn't have a good description (which was propagated to your update). Fixed. (Yes, it's totally kosher for the original author to +1 the referee's changes.)",
    "created_at": "2011-05-30T17:35:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11381#issuecomment-114997",
    "user": "https://github.com/robertwb"
}
```

<div id="comment:5" align="right">comment:5</div>

Your change looks good, though I noticed that the patch didn't have a good description (which was propagated to your update). Fixed. (Yes, it's totally kosher for the original author to +1 the referee's changes.)



---

archive/issue_comments_114998.json:
```json
{
    "body": "Attachment: **[11381-vector-simplify-reviewer-changes.2.patch.gz](https://github.com/sagemath/sage/files/ticket11381/11381-vector-simplify-reviewer-changes.2.patch.gz)**\n\napply only this patch",
    "created_at": "2011-05-30T17:35:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11381#issuecomment-114998",
    "user": "https://github.com/robertwb"
}
```

Attachment: **[11381-vector-simplify-reviewer-changes.2.patch.gz](https://github.com/sagemath/sage/files/ticket11381/11381-vector-simplify-reviewer-changes.2.patch.gz)**

apply only this patch



---

archive/issue_comments_114999.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nGreat!  Thanks for noticing the absence of a good description, I always forget that.",
    "created_at": "2011-05-30T18:43:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11381#issuecomment-114999",
    "user": "https://github.com/jvkersch"
}
```

<div id="comment:6" align="right">comment:6</div>

Great!  Thanks for noticing the absence of a good description, I always forget that.



---

archive/issue_events_149899.json:
```json
{
    "actor": "https://github.com/jvkersch",
    "created_at": "2011-05-30T18:43:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11381#event-149899"
}
```



---

archive/issue_events_149900.json:
```json
{
    "actor": "https://github.com/jvkersch",
    "created_at": "2011-05-30T18:43:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11381#event-149900"
}
```



---

archive/issue_comments_115000.json:
```json
{
    "body": "Author: **Robert Bradshaw**",
    "created_at": "2011-06-01T07:52:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11381#issuecomment-115000",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Robert Bradshaw**



---

archive/issue_comments_115001.json:
```json
{
    "body": "Merged: **sage-4.7.1.alpha3**",
    "created_at": "2011-06-08T07:14:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11381#issuecomment-115001",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.1.alpha3**



---

archive/issue_events_149901.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-08T07:14:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11381#event-149901"
}
```



---

archive/issue_events_149902.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-08T07:14:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11381",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11381#event-149902"
}
```

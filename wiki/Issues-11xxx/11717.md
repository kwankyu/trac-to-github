# Issue 11717: sage.schemes.generic.scheme initialization

archive/issues_011545.json:
```json
{
    "body": "I do not really understand why, but the initialization of *sage.schemes.generic.scheme* takes a bit to run:\n\n```\n$ time ./sage --startuptime|grep ' scheme:'\n         scheme: 0.027 (spec)\n```\n\nThe attached patch improves this:\n\n```\n$ time ./sage --startuptime|grep ' scheme:'\n         scheme: 0.000 (spec)\n```\n\nThe overall improvement is, of course, relatively small:\n\n```\n$ time for in in `seq 100`;do ./sage --startuptime>/dev/null;done\nreal    1m28.181s\n```\nvs.\n\n```\n$ time for in in `seq 100`;do ./sage --startuptime>/dev/null;done\nreal    1m27.629s\n```\n\n**Assignee:** @jasongrout\n\n**Reviewer:** Volker Braun\n\nIssue created by migration from https://trac.sagemath.org/ticket/11717\n\n",
    "closed_at": "2012-08-15T07:55:55Z",
    "created_at": "2011-08-22T22:00:09Z",
    "labels": [
        "component: misc",
        "minor",
        "enhancement",
        "worksforme",
        "invalid"
    ],
    "title": "sage.schemes.generic.scheme initialization",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/11717",
    "user": "https://github.com/saraedum"
}
```
I do not really understand why, but the initialization of *sage.schemes.generic.scheme* takes a bit to run:

```
$ time ./sage --startuptime|grep ' scheme:'
         scheme: 0.027 (spec)
```

The attached patch improves this:

```
$ time ./sage --startuptime|grep ' scheme:'
         scheme: 0.000 (spec)
```

The overall improvement is, of course, relatively small:

```
$ time for in in `seq 100`;do ./sage --startuptime>/dev/null;done
real    1m28.181s
```
vs.

```
$ time for in in `seq 100`;do ./sage --startuptime>/dev/null;done
real    1m27.629s
```

**Assignee:** @jasongrout

**Reviewer:** Volker Braun

Issue created by migration from https://trac.sagemath.org/ticket/11717





---

archive/attachments_016060.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11717_sage_schemes_generic_scheme_speedup.patch",
    "asset_url": "tarball://root/attachments/ticket11717/trac_11717_sage_schemes_generic_scheme_speedup.patch",
    "created_at": "2011-08-24T05:18:44Z",
    "issue": "https://github.com/sagemath/sage/issues/11717",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11717/trac_11717_sage_schemes_generic_scheme_speedup.patch",
    "user": "https://github.com/saraedum"
}
```



---

archive/issue_comments_123838.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_11717_sage_schemes_generic_scheme_speedup.patch](https://github.com/sagemath/sage/files/ticket11717/trac_11717_sage_schemes_generic_scheme_speedup.patch)\n\nTicket can be closed. It is not an issue in 4.7.2.alpha2 anymore.",
    "created_at": "2011-08-24T05:18:44Z",
    "issue": "https://github.com/sagemath/sage/issues/11717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11717#issuecomment-123838",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_11717_sage_schemes_generic_scheme_speedup.patch](https://github.com/sagemath/sage/files/ticket11717/trac_11717_sage_schemes_generic_scheme_speedup.patch)

Ticket can be closed. It is not an issue in 4.7.2.alpha2 anymore.



---

archive/issue_events_094099.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2011-09-19T13:19:28Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11717",
    "milestone": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11717#event-94099"
}
```



---

archive/issue_comments_123839.json:
```json
{
    "body": "**Upstream:** Fixed upstream, but not in a stable release.",
    "created_at": "2011-09-19T13:19:28Z",
    "issue": "https://github.com/sagemath/sage/issues/11717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11717#issuecomment-123839",
    "user": "https://github.com/saraedum"
}
```

**Upstream:** Fixed upstream, but not in a stable release.



---

archive/issue_events_094100.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-08-12T17:30:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11717",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11717#event-94100"
}
```



---

archive/issue_comments_123840.json:
```json
{
    "body": "**Reviewer:** Volker Braun",
    "created_at": "2012-08-12T17:31:38Z",
    "issue": "https://github.com/sagemath/sage/issues/11717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11717#issuecomment-123840",
    "user": "https://github.com/vbraun"
}
```

**Reviewer:** Volker Braun



---

archive/issue_comments_123841.json:
```json
{
    "body": "<a id='comment:4'></a>\nClose as invalid",
    "created_at": "2012-08-12T17:31:38Z",
    "issue": "https://github.com/sagemath/sage/issues/11717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11717#issuecomment-123841",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'></a>
Close as invalid



---

archive/issue_events_094101.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-08-12T17:31:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11717",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11717#event-94101"
}
```



---

archive/issue_events_094102.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-08-12T17:31:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11717",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11717#event-94102"
}
```



---

archive/issue_events_094103.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-15T07:55:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11717",
    "label": "worksforme",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11717#event-94103"
}
```



---

archive/issue_comments_123842.json:
```json
{
    "body": "**Changing upstream** from \"Fixed upstream, but not in a stable release.\" to \"\".",
    "created_at": "2012-08-15T07:55:55Z",
    "issue": "https://github.com/sagemath/sage/issues/11717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11717#issuecomment-123842",
    "user": "https://github.com/jdemeyer"
}
```

**Changing upstream** from "Fixed upstream, but not in a stable release." to "".



---

archive/issue_events_094104.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-15T07:55:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11717",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11717#event-94104"
}
```



---

archive/issue_events_094105.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-15T07:55:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11717",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11717#event-94105"
}
```

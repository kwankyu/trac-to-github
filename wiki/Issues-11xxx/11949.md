# Issue 11949: Gurobi Support !

archive/issues_011777.json:
```json
{
    "assignees": [
        "https://github.com/nathanncohen"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nHere it is ! It cost me several nights along with my first evening in Belgium, but it works ! `;-)`\n\nThis patch implements a new interface between Sage and Gurobi (http://www.gurobi.com)\n\nYEAH !\n\n(and now I'm going out ! `:-D`)\n\nNathann\n\nAPPLY:\n* [attachment: trac_11949.patch](https://github.com/sagemath/sage/files/ticket11949/trac_11949.patch.gz)\n\nCC:  @malb @sagetrac-mvngu @dcoudert @sagetrac-lsampaio @ypfmde @novoselt @jasongrout\n\nComponent: **linear programming**\n\nAuthor: **Nathann Cohen**\n\nReviewer: **Peter M\u00fcller**\n\nMerged: **sage-4.8.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11949_\n\n",
    "closed_at": "2011-11-14T17:32:19Z",
    "created_at": "2011-10-24T19:42:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20linear%20programming",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Gurobi Support !",
    "type": "issue",
    "updated_at": "2012-01-13T16:05:24Z",
    "url": "https://github.com/sagemath/sage/issues/11949",
    "user": "https://github.com/nathanncohen"
}
```
<div id="comment:0"></div>

Here it is ! It cost me several nights along with my first evening in Belgium, but it works ! `;-)`

This patch implements a new interface between Sage and Gurobi (http://www.gurobi.com)

YEAH !

(and now I'm going out ! `:-D`)

Nathann

APPLY:
* [attachment: trac_11949.patch](https://github.com/sagemath/sage/files/ticket11949/trac_11949.patch.gz)

CC:  @malb @sagetrac-mvngu @dcoudert @sagetrac-lsampaio @ypfmde @novoselt @jasongrout

Component: **linear programming**

Author: **Nathann Cohen**

Reviewer: **Peter Müller**

Merged: **sage-4.8.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/11949_





---

archive/issue_events_159924.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-10-24T19:42:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159924"
}
```



---

archive/issue_events_159925.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-10-24T19:42:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20linear%20programming",
    "label_color": "0000ff",
    "label_name": "c: linear programming",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159925"
}
```



---

archive/issue_events_159926.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-10-24T19:42:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159926"
}
```



---

archive/issue_events_159927.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-10-24T19:42:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159927"
}
```



---

archive/issue_events_159928.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-10-24T19:42:31Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "subject": "https://github.com/nathanncohen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159928"
}
```



---

archive/issue_comments_126591.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,3 +7,6 @@\n (and now I'm going out ! `:-D`)\n \n Nathann\n+\n+APPLY:\n+* [attachment: trac_11949.patch](https://github.com/sagemath/sage/files/ticket11949/trac_11949.patch.gz)\n``````\n",
    "created_at": "2011-10-24T19:43:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11949#issuecomment-126591",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,3 +7,6 @@
 (and now I'm going out ! `:-D`)
 
 Nathann
+
+APPLY:
+* [attachment: trac_11949.patch](https://github.com/sagemath/sage/files/ticket11949/trac_11949.patch.gz)
``````




---

archive/issue_events_159929.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-10-24T19:43:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159929"
}
```



---

archive/issue_comments_126592.json:
```json
{
    "body": "Milestone sage-4.7.3 deleted",
    "created_at": "2011-11-03T16:14:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11949#issuecomment-126592",
    "user": "https://github.com/jdemeyer"
}
```

Milestone sage-4.7.3 deleted



---

archive/issue_events_159930.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-03T16:14:43Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159930"
}
```



---

archive/issue_events_159931.json:
```json
{
    "actor": "https://github.com/ypfmde",
    "created_at": "2011-11-09T10:04:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159931"
}
```



---

archive/issue_comments_126593.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nI tried this patch in 4.8.alpha0 for many examples, for which I previously used the Coin-cbc solver or the standalone versions of scip and gurobi via the mps-export of the linear program. It seems to work fine with Gurobi versions 4.5 and 4.6. The docs are updated, and the thematic tutorial on linear programming explains well how to make gurobi visible to sage.\n\nIt's great that this powerful solver (for which one can get a free academic license) can now be used from inside Sage.\n\nPeter M\u00fcller",
    "created_at": "2011-11-09T10:04:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11949#issuecomment-126593",
    "user": "https://github.com/ypfmde"
}
```

<div id="comment:5" align="right">comment:5</div>

I tried this patch in 4.8.alpha0 for many examples, for which I previously used the Coin-cbc solver or the standalone versions of scip and gurobi via the mps-export of the linear program. It seems to work fine with Gurobi versions 4.5 and 4.6. The docs are updated, and the thematic tutorial on linear programming explains well how to make gurobi visible to sage.

It's great that this powerful solver (for which one can get a free academic license) can now be used from inside Sage.

Peter Müller



---

archive/issue_events_159932.json:
```json
{
    "actor": "https://github.com/ypfmde",
    "created_at": "2011-11-09T10:04:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159932"
}
```



---

archive/issue_events_159933.json:
```json
{
    "actor": "https://github.com/ypfmde",
    "created_at": "2011-11-09T10:04:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159933"
}
```



---

archive/issue_comments_126594.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nMy GOD `O_O`\n\nGurobi in Sage. Thank you soooooooooooooo much for reviewing it `:-)`\n\nNathann",
    "created_at": "2011-11-09T10:09:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11949#issuecomment-126594",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:6" align="right">comment:6</div>

My GOD `O_O`

Gurobi in Sage. Thank you soooooooooooooo much for reviewing it `:-)`

Nathann



---

archive/issue_comments_126595.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nYou should change \"optional - required GUROBI\" to \"optional - GUROBI\"",
    "created_at": "2011-11-09T13:16:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11949#issuecomment-126595",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:7" align="right">comment:7</div>

You should change "optional - required GUROBI" to "optional - GUROBI"



---

archive/issue_events_159934.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-09T13:16:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159934"
}
```



---

archive/issue_events_159935.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-09T13:16:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159935"
}
```



---

archive/issue_events_159936.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-11-09T13:23:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159936"
}
```



---

archive/issue_events_159937.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-11-09T13:23:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159937"
}
```



---

archive/issue_comments_126596.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nOopps.... Fixed ! `:-)`\n\n\nNathann",
    "created_at": "2011-11-09T13:23:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11949#issuecomment-126596",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:8" align="right">comment:8</div>

Oopps.... Fixed ! `:-)`


Nathann



---

archive/issue_events_159938.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-12T07:19:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159938"
}
```



---

archive/issue_events_159939.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-12T07:19:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159939"
}
```



---

archive/issue_comments_126597.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nThere are some issues with the documentation:\n\n```\ndochtml.log:/mnt/usb1/scratch/jdemeyer/merger/sage-4.8.alpha2/devel/sage/doc/en/thematic_tutorials/linear_programming.rst:521: WARNING: Bullet list ends without a blank line; unexpected unindent.\ndochtml.log:/mnt/usb1/scratch/jdemeyer/merger/sage-4.8.alpha2/devel/sage/doc/en/thematic_tutorials/linear_programming.rst:532: WARNING: Inline strong start-string without end-string.\n```",
    "created_at": "2011-11-12T07:19:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11949#issuecomment-126597",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:9" align="right">comment:9</div>

There are some issues with the documentation:

```
dochtml.log:/mnt/usb1/scratch/jdemeyer/merger/sage-4.8.alpha2/devel/sage/doc/en/thematic_tutorials/linear_programming.rst:521: WARNING: Bullet list ends without a blank line; unexpected unindent.
dochtml.log:/mnt/usb1/scratch/jdemeyer/merger/sage-4.8.alpha2/devel/sage/doc/en/thematic_tutorials/linear_programming.rst:532: WARNING: Inline strong start-string without end-string.
```



---

archive/issue_events_159940.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-11-12T16:15:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159940"
}
```



---

archive/issue_events_159941.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-11-12T16:15:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159941"
}
```



---

archive/issue_comments_126598.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nAhem... Updated `^^;`\n\nNathann",
    "created_at": "2011-11-12T16:15:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11949#issuecomment-126598",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:10" align="right">comment:10</div>

Ahem... Updated `^^;`

Nathann



---

archive/issue_comments_126599.json:
```json
{
    "body": "Attachment: **[trac_11949.patch.gz](https://github.com/sagemath/sage/files/ticket11949/trac_11949.patch.gz)**",
    "created_at": "2011-11-12T16:15:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11949#issuecomment-126599",
    "user": "https://github.com/nathanncohen"
}
```

Attachment: **[trac_11949.patch.gz](https://github.com/sagemath/sage/files/ticket11949/trac_11949.patch.gz)**



---

archive/issue_comments_126600.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\npmueller: please add your real name as Reviewer on this ticket and also add yourself to [http://trac.sagemath.org/sage_trac/#AccountNamesMappedtoRealNames](http://trac.sagemath.org/sage_trac/#AccountNamesMappedtoRealNames).",
    "created_at": "2011-11-14T09:52:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11949#issuecomment-126600",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:11" align="right">comment:11</div>

pmueller: please add your real name as Reviewer on this ticket and also add yourself to [http://trac.sagemath.org/sage_trac/#AccountNamesMappedtoRealNames](http://trac.sagemath.org/sage_trac/#AccountNamesMappedtoRealNames).



---

archive/issue_events_159942.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T09:52:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159942"
}
```



---

archive/issue_events_159943.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T09:52:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159943"
}
```



---

archive/issue_comments_126601.json:
```json
{
    "body": "Reviewer: **Peter Mueller**",
    "created_at": "2011-11-14T10:17:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11949#issuecomment-126601",
    "user": "https://github.com/ypfmde"
}
```

Reviewer: **Peter Mueller**



---

archive/issue_events_159944.json:
```json
{
    "actor": "https://github.com/ypfmde",
    "created_at": "2011-11-14T10:17:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159944"
}
```



---

archive/issue_events_159945.json:
```json
{
    "actor": "https://github.com/ypfmde",
    "created_at": "2011-11-14T10:17:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159945"
}
```



---

archive/issue_events_159946.json:
```json
{
    "actor": "https://github.com/ypfmde",
    "created_at": "2011-11-14T10:18:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159946"
}
```



---

archive/issue_events_159947.json:
```json
{
    "actor": "https://github.com/ypfmde",
    "created_at": "2011-11-14T10:18:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159947"
}
```



---

archive/issue_events_159948.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T17:32:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159948"
}
```



---

archive/issue_events_159949.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T17:32:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11949#event-159949"
}
```



---

archive/issue_comments_126602.json:
```json
{
    "body": "Merged: **sage-4.8.alpha2**",
    "created_at": "2011-11-14T17:32:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11949#issuecomment-126602",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.8.alpha2**



---

archive/issue_comments_126603.json:
```json
{
    "body": "Changed reviewer from **Peter Mueller** to **Peter M\u00fcller**",
    "created_at": "2012-01-13T16:05:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11949",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11949#issuecomment-126603",
    "user": "https://github.com/jdemeyer"
}
```

Changed reviewer from **Peter Mueller** to **Peter Müller**

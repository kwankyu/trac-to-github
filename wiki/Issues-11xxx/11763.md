# Issue 11763: Parents for polyhedra

archive/issues_011591.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mhampton"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nThe Polyhedron class is, so far, free-standing with some sort of coercion for the base ring tacked manually. This ticket strives to add parents for polyhedra and make the base ring coercion work more naturally. \n\nThere will be 3 supported base rings:\n* ZZ (meaning that the polyhedron is a lattice polytope, that is, both H- and V-representation are defined over ZZ)\n* QQ\n* RDF\n\nApply:\n* [attachment: trac_11763-polyhedra_coercion_folded.3.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.3.patch.gz)\n* [attachment: trac_11763_fix_associahedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)\n* [attachment: trac_11763_lazy_import.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_lazy_import.patch.gz)\n\n\nDepends on #11634\nDepends on #13109\nDepends on #11310\n\nCC:  @robertwb\n\nComponent: **geometry**\n\nAuthor: **Volker Braun**\n\nReviewer: **Dmitrii Pasechnik**\n\nMerged: **sage-5.6.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11763_\n\n",
    "closed_at": "2012-12-21T09:30:41Z",
    "created_at": "2011-08-31T14:13:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Parents for polyhedra",
    "type": "issue",
    "updated_at": "2012-12-28T02:53:46Z",
    "url": "https://github.com/sagemath/sage/issues/11763",
    "user": "https://github.com/vbraun"
}
```
<div id="comment:0"></div>

The Polyhedron class is, so far, free-standing with some sort of coercion for the base ring tacked manually. This ticket strives to add parents for polyhedra and make the base ring coercion work more naturally. 

There will be 3 supported base rings:
* ZZ (meaning that the polyhedron is a lattice polytope, that is, both H- and V-representation are defined over ZZ)
* QQ
* RDF

Apply:
* [attachment: trac_11763-polyhedra_coercion_folded.3.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.3.patch.gz)
* [attachment: trac_11763_fix_associahedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)
* [attachment: trac_11763_lazy_import.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_lazy_import.patch.gz)


Depends on #11634
Depends on #13109
Depends on #11310

CC:  @robertwb

Component: **geometry**

Author: **Volker Braun**

Reviewer: **Dmitrii Pasechnik**

Merged: **sage-5.6.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/11763_





---

archive/issue_events_156530.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2011-08-31T14:13:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "milestone_number": null,
    "milestone_title": "sage-5.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156530"
}
```



---

archive/issue_events_156531.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2011-08-31T14:13:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "c: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156531"
}
```



---

archive/issue_events_156532.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2011-08-31T14:13:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156532"
}
```



---

archive/issue_events_156533.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2011-08-31T14:13:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156533"
}
```



---

archive/issue_events_156534.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2011-08-31T14:13:15Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "subject": "https://github.com/vbraun",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156534"
}
```



---

archive/issue_comments_122467.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nWould there be any benefit in supporting real fields of arbitrary precision?",
    "created_at": "2011-08-31T14:58:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122467",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:1" align="right">comment:1</div>

Would there be any benefit in supporting real fields of arbitrary precision?



---

archive/issue_comments_122468.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nWe don't have an implementation of the double description algorithm for other base rings, so we wouldn't be able to compute anything.",
    "created_at": "2011-08-31T15:05:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122468",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:2" align="right">comment:2</div>

We don't have an implementation of the double description algorithm for other base rings, so we wouldn't be able to compute anything.



---

archive/issue_comments_122469.json:
```json
{
    "body": "Attachment: **[trac_11763_ZZ_polyhedron.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_ZZ_polyhedron.patch.gz)**\n\nInitial patch",
    "created_at": "2011-09-08T17:00:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122469",
    "user": "https://github.com/vbraun"
}
```

Attachment: **[trac_11763_ZZ_polyhedron.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_ZZ_polyhedron.patch.gz)**

Initial patch



---

archive/issue_comments_122470.json:
```json
{
    "body": "Initial patch",
    "created_at": "2011-09-08T17:00:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122470",
    "user": "https://github.com/vbraun"
}
```

Initial patch



---

archive/issue_comments_122471.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,4 +5,8 @@\n * QQ\n * RDF\n \n+Apply:\n+* [attachment: trac_11763_ZZ_polyhedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_ZZ_polyhedron.patch.gz)\n+* [attachment: trac_11763_parents_for_polyhedron.py](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents_for_polyhedron.py.gz)\n \n+\n``````\n",
    "created_at": "2011-09-08T17:06:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122471",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,4 +5,8 @@
 * QQ
 * RDF
 
+Apply:
+* [attachment: trac_11763_ZZ_polyhedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_ZZ_polyhedron.patch.gz)
+* [attachment: trac_11763_parents_for_polyhedron.py](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents_for_polyhedron.py.gz)
 
+
``````




---

archive/issue_comments_122472.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAttachment: **[trac_11763_parents_for_polyhedron.py.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents_for_polyhedron.py.gz)**\n\nThis is now ready for inclusion. Marshall, are you interested in reviewing this patch and its dependency? ;-)",
    "created_at": "2011-09-08T17:06:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122472",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:3" align="right">comment:3</div>

Attachment: **[trac_11763_parents_for_polyhedron.py.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents_for_polyhedron.py.gz)**

This is now ready for inclusion. Marshall, are you interested in reviewing this patch and its dependency? ;-)



---

archive/issue_events_156535.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2011-09-08T17:06:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156535"
}
```



---

archive/issue_comments_122473.json:
```json
{
    "body": "Author: **Volker Braun**",
    "created_at": "2011-09-08T17:06:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122473",
    "user": "https://github.com/vbraun"
}
```

Author: **Volker Braun**



---

archive/issue_comments_122474.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,6 +7,6 @@\n \n Apply:\n * [attachment: trac_11763_ZZ_polyhedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_ZZ_polyhedron.patch.gz)\n-* [attachment: trac_11763_parents_for_polyhedron.py](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents_for_polyhedron.py.gz)\n+* [attachment: trac_11763_parents_for_polyhedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents_for_polyhedron.patch.gz)\n \n \n``````\n",
    "created_at": "2011-12-16T10:27:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122474",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,6 +7,6 @@
 
 Apply:
 * [attachment: trac_11763_ZZ_polyhedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_ZZ_polyhedron.patch.gz)
-* [attachment: trac_11763_parents_for_polyhedron.py](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents_for_polyhedron.py.gz)
+* [attachment: trac_11763_parents_for_polyhedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents_for_polyhedron.patch.gz)
 
 
``````




---

archive/issue_comments_122475.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nFix comparison of H/V-representation objects:\n\n```\nsage: triangle = Polyhedron([(0,0), (1,0), (0,1)])\nsage: ieq = triangle.inequality_generator().next()\nsage: ieq == copy(ieq)\nFalse\n```\nNow returns ``True``, as it should.",
    "created_at": "2012-02-19T02:05:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122475",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:5" align="right">comment:5</div>

Fix comparison of H/V-representation objects:

```
sage: triangle = Polyhedron([(0,0), (1,0), (0,1)])
sage: ieq = triangle.inequality_generator().next()
sage: ieq == copy(ieq)
False
```
Now returns ``True``, as it should.



---

archive/issue_comments_122476.json:
```json
{
    "body": "Updated patch",
    "created_at": "2012-02-19T02:06:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122476",
    "user": "https://github.com/vbraun"
}
```

Updated patch



---

archive/issue_comments_122477.json:
```json
{
    "body": "Attachment: **[trac_11763_parents_for_polyhedron.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents_for_polyhedron.patch.gz)**\n\nAttachment: **[trac_11763_ZZ_polyhedron-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_ZZ_polyhedron-rebase.patch.gz)**\n\nRebased for new patch at 11634",
    "created_at": "2012-03-11T14:17:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122477",
    "user": "https://github.com/loefflerd"
}
```

Attachment: **[trac_11763_parents_for_polyhedron.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents_for_polyhedron.patch.gz)**

Attachment: **[trac_11763_ZZ_polyhedron-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_ZZ_polyhedron-rebase.patch.gz)**

Rebased for new patch at 11634



---

archive/issue_comments_122478.json:
```json
{
    "body": "Attachment: **[trac_11763_parents-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents-rebase.patch.gz)**\n\nRebased for new patch at #11634",
    "created_at": "2012-03-11T14:18:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122478",
    "user": "https://github.com/loefflerd"
}
```

Attachment: **[trac_11763_parents-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents-rebase.patch.gz)**

Rebased for new patch at #11634



---

archive/issue_comments_122479.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nThe new patch I posted at #11634 broke these, so I rebased them.\n\nApply trac_11763_ZZ_polyhedron-rebase.patch, trac_11763_parents-rebase.patch",
    "created_at": "2012-03-11T14:19:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122479",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:6" align="right">comment:6</div>

The new patch I posted at #11634 broke these, so I rebased them.

Apply trac_11763_ZZ_polyhedron-rebase.patch, trac_11763_parents-rebase.patch



---

archive/issue_comments_122480.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,7 +6,7 @@\n * RDF\n \n Apply:\n-* [attachment: trac_11763_ZZ_polyhedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_ZZ_polyhedron.patch.gz)\n-* [attachment: trac_11763_parents_for_polyhedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents_for_polyhedron.patch.gz)\n+* [attachment: trac_11763_ZZ_polyhedron-rebase.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_ZZ_polyhedron-rebase.patch.gz)\n+* [attachment: trac_11763_parents-rebase.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents-rebase.patch.gz)\n \n \n``````\n",
    "created_at": "2012-03-11T14:19:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122480",
    "user": "https://github.com/loefflerd"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,7 +6,7 @@
 * RDF
 
 Apply:
-* [attachment: trac_11763_ZZ_polyhedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_ZZ_polyhedron.patch.gz)
-* [attachment: trac_11763_parents_for_polyhedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents_for_polyhedron.patch.gz)
+* [attachment: trac_11763_ZZ_polyhedron-rebase.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_ZZ_polyhedron-rebase.patch.gz)
+* [attachment: trac_11763_parents-rebase.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents-rebase.patch.gz)
 
 
``````




---

archive/issue_comments_122481.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nApply trac_11763_zz_polyhedron-rebase.patch, trac_11763_parents-rebase.patch \n\n(bloody patchbot!)",
    "created_at": "2012-03-11T14:20:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122481",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:7" align="right">comment:7</div>

Apply trac_11763_zz_polyhedron-rebase.patch, trac_11763_parents-rebase.patch 

(bloody patchbot!)



---

archive/issue_comments_122482.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nNo matter what I do, the patchbot won't pick up the two patches above, so I've done a single qfolded patch. Hopefully this will now work!",
    "created_at": "2012-03-11T14:23:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122482",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:8" align="right">comment:8</div>

No matter what I do, the patchbot won't pick up the two patches above, so I've done a single qfolded patch. Hopefully this will now work!



---

archive/issue_comments_122483.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,7 +6,6 @@\n * RDF\n \n Apply:\n-* [attachment: trac_11763_ZZ_polyhedron-rebase.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_ZZ_polyhedron-rebase.patch.gz)\n-* [attachment: trac_11763_parents-rebase.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents-rebase.patch.gz)\n+* [attachment: trac_11763-polyhedra_coercion_folded.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.patch.gz)\n \n \n``````\n",
    "created_at": "2012-03-11T14:23:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122483",
    "user": "https://github.com/loefflerd"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,7 +6,6 @@
 * RDF
 
 Apply:
-* [attachment: trac_11763_ZZ_polyhedron-rebase.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_ZZ_polyhedron-rebase.patch.gz)
-* [attachment: trac_11763_parents-rebase.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_parents-rebase.patch.gz)
+* [attachment: trac_11763-polyhedra_coercion_folded.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.patch.gz)
 
 
``````




---

archive/issue_comments_122484.json:
```json
{
    "body": "Apply only this patch. Patch against 5.0.beta7",
    "created_at": "2012-03-12T09:59:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122484",
    "user": "https://github.com/loefflerd"
}
```

Apply only this patch. Patch against 5.0.beta7



---

archive/issue_comments_122485.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nAttachment: **[trac_11763-polyhedra_coercion_folded.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.patch.gz)**\n\nSorry, I made a hash of rebasing the patch, so here's a new version.",
    "created_at": "2012-03-12T10:00:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122485",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:10" align="right">comment:10</div>

Attachment: **[trac_11763-polyhedra_coercion_folded.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.patch.gz)**

Sorry, I made a hash of rebasing the patch, so here's a new version.



---

archive/issue_comments_122486.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nThis changes the behavior of the .vertices() method, returning \"sage.geometry.polyhedron.representation.Vertex\" objects instead of simple lists of coordinates.  This breaks some code of mine, so I think there should be a deprecation warning before introducing this.  I don't really like the behavior, but I see that .vertices_list() does return a list of lists of coordinates.\n\nThis patch also seems to slow things down a bit, but I haven't carefully tested that aspect.",
    "created_at": "2012-03-12T15:29:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122486",
    "user": "https://github.com/sagetrac-mhampton"
}
```

<div id="comment:11" align="right">comment:11</div>

This changes the behavior of the .vertices() method, returning "sage.geometry.polyhedron.representation.Vertex" objects instead of simple lists of coordinates.  This breaks some code of mine, so I think there should be a deprecation warning before introducing this.  I don't really like the behavior, but I see that .vertices_list() does return a list of lists of coordinates.

This patch also seems to slow things down a bit, but I haven't carefully tested that aspect.



---

archive/issue_comments_122487.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nThe `Vertex` object implements the list interface, so any code that doesn't just test `isinstance(-,list)` should work fine. Plus, you can't really deprecate something unless you want to remove it later. I added the 'vertices_list` method so people can keep their broken code with minimal patching if they want.",
    "created_at": "2012-03-12T18:27:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122487",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:12" align="right">comment:12</div>

The `Vertex` object implements the list interface, so any code that doesn't just test `isinstance(-,list)` should work fine. Plus, you can't really deprecate something unless you want to remove it later. I added the 'vertices_list` method so people can keep their broken code with minimal patching if they want.



---

archive/issue_comments_122488.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nWell, no, there is quite a bit of my code that does not work fine, and I am not talking about artificial test cases.  Here is a shortened example of something this patch breaks:\n\n```\nc = polytopes.n_cube(3)\nv = c.vertices()[0]\npoint(v)\n```\nThis now gives a \"TypeError: 'sage.rings.integer.Integer' object does not support indexing\".",
    "created_at": "2012-03-12T21:07:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122488",
    "user": "https://github.com/sagetrac-mhampton"
}
```

<div id="comment:13" align="right">comment:13</div>

Well, no, there is quite a bit of my code that does not work fine, and I am not talking about artificial test cases.  Here is a shortened example of something this patch breaks:

```
c = polytopes.n_cube(3)
v = c.vertices()[0]
point(v)
```
This now gives a "TypeError: 'sage.rings.integer.Integer' object does not support indexing".



---

archive/issue_comments_122489.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nFor the record: #12541 and #12544 have a similar situation. I replaced tuples with tuple-like containers and some stuff got broken. As it turned out, it was not too difficult to fix it by eliminating some \"hard typechecks\", although they are still waiting for an approval. If this is the case here, perhaps it is also not too difficult to fix.\n\nAs I understand it, direct typechecking is not welcomed in either Python or Sage, so if the new class implements list interface, it should be OK to use in place of the old one. If it does break some code, it is a bug and it should be fixed before merging, but not a reason for avoiding the change entirely or inventing elaborate deprecation scheme...",
    "created_at": "2012-03-12T21:19:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122489",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:14" align="right">comment:14</div>

For the record: #12541 and #12544 have a similar situation. I replaced tuples with tuple-like containers and some stuff got broken. As it turned out, it was not too difficult to fix it by eliminating some "hard typechecks", although they are still waiting for an approval. If this is the case here, perhaps it is also not too difficult to fix.

As I understand it, direct typechecking is not welcomed in either Python or Sage, so if the new class implements list interface, it should be OK to use in place of the old one. If it does break some code, it is a bug and it should be fixed before merging, but not a reason for avoiding the change entirely or inventing elaborate deprecation scheme...



---

archive/issue_events_156536.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-03-26T18:11:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156536"
}
```



---

archive/issue_events_156537.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-03-26T18:11:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156537"
}
```



---

archive/issue_comments_122490.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nThere is a doctest failing on 5.0.beta10, according to the patchbot (something to do with the new associahedron class from #10817)",
    "created_at": "2012-03-26T18:11:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122490",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:15" align="right">comment:15</div>

There is a doctest failing on 5.0.beta10, according to the patchbot (something to do with the new associahedron class from #10817)



---

archive/issue_comments_122491.json:
```json
{
    "body": "Work Issues: **doctest failure**",
    "created_at": "2012-03-26T18:11:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122491",
    "user": "https://github.com/loefflerd"
}
```

Work Issues: **doctest failure**



---

archive/issue_events_156538.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-03-29T18:43:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156538"
}
```



---

archive/issue_events_156539.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-03-29T18:43:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156539"
}
```



---

archive/issue_comments_122492.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">comment:16</div>\n\nThe associahedron needed to be translated to the parent/element framework as well. I've fixed this. Also, I rediffed trac_11763-polyhedra_coercion_folded.2.patch because it only applied with some fuzz on sage-5.0.beta11",
    "created_at": "2012-03-29T18:43:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122492",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:16" align="right">comment:16</div>

The associahedron needed to be translated to the parent/element framework as well. I've fixed this. Also, I rediffed trac_11763-polyhedra_coercion_folded.2.patch because it only applied with some fuzz on sage-5.0.beta11



---

archive/issue_comments_122493.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,6 +6,7 @@\n * RDF\n \n Apply:\n-* [attachment: trac_11763-polyhedra_coercion_folded.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.patch.gz)\n+* [attachment: trac_11763-polyhedra_coercion_folded.2.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.patch.gz)\n+* [attachment: trac_11763_fix_associahedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)\n \n \n``````\n",
    "created_at": "2012-03-29T18:43:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122493",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,6 +6,7 @@
 * RDF
 
 Apply:
-* [attachment: trac_11763-polyhedra_coercion_folded.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.patch.gz)
+* [attachment: trac_11763-polyhedra_coercion_folded.2.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.patch.gz)
+* [attachment: trac_11763_fix_associahedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)
 
 
``````




---

archive/issue_comments_122494.json:
```json
{
    "body": "Attachment: **[trac_11763_relax_typechecks.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_relax_typechecks.patch.gz)**\n\nUpdated patch",
    "created_at": "2012-03-29T19:00:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122494",
    "user": "https://github.com/vbraun"
}
```

Attachment: **[trac_11763_relax_typechecks.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_relax_typechecks.patch.gz)**

Updated patch



---

archive/issue_comments_122495.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,5 +8,5 @@\n Apply:\n * [attachment: trac_11763-polyhedra_coercion_folded.2.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.patch.gz)\n * [attachment: trac_11763_fix_associahedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)\n+* [attachment: trac_11763_relax_typechecks.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_relax_typechecks.patch.gz)\n \n-\n``````\n",
    "created_at": "2012-03-29T19:01:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122495",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,5 +8,5 @@
 Apply:
 * [attachment: trac_11763-polyhedra_coercion_folded.2.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.patch.gz)
 * [attachment: trac_11763_fix_associahedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)
+* [attachment: trac_11763_relax_typechecks.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_relax_typechecks.patch.gz)
 
-
``````




---

archive/issue_comments_122496.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">comment:17</div>\n\nThe final patch fixes the `point()` command to not make dumb type checks.",
    "created_at": "2012-03-29T19:01:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122496",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:17" align="right">comment:17</div>

The final patch fixes the `point()` command to not make dumb type checks.



---

archive/issue_comments_122497.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">comment:18</div>\n\nI noted a problem in the documentatation build, the updated patch fixes just that.",
    "created_at": "2012-03-29T20:04:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122497",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:18" align="right">comment:18</div>

I noted a problem in the documentatation build, the updated patch fixes just that.



---

archive/issue_comments_122498.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">comment:19</div>\n\nApply trac_11763-polyhedra_coercion_folded.2.patch, trac_11763_fix_associahedron.patch, trac_11763_relax_typechecks.patch\n\n(for patchbot)",
    "created_at": "2012-03-30T12:40:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122498",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:19" align="right">comment:19</div>

Apply trac_11763-polyhedra_coercion_folded.2.patch, trac_11763_fix_associahedron.patch, trac_11763_relax_typechecks.patch

(for patchbot)



---

archive/issue_comments_122499.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">comment:20</div>\n\nI thought the point of having \"Apply\" section in the description was to make the patchbot happy, why does it need to be duplicated?..",
    "created_at": "2012-03-30T15:33:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122499",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:20" align="right">comment:20</div>

I thought the point of having "Apply" section in the description was to make the patchbot happy, why does it need to be duplicated?..



---

archive/issue_comments_122500.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">comment:21</div>\n\nIt was previously applying the patches in an order other than that listed on the ticket description (with coercion_folded.2.patch last rather than first). I didn't think to check whether the patches were applying happily anyway; as it happens they were. Sorry for the noise!",
    "created_at": "2012-03-30T15:50:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122500",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:21" align="right">comment:21</div>

It was previously applying the patches in an order other than that listed on the ticket description (with coercion_folded.2.patch last rather than first). I didn't think to check whether the patches were applying happily anyway; as it happens they were. Sorry for the noise!



---

archive/issue_comments_122501.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">comment:22</div>\n\nOh, I don't complain about your post - I complain about patchbot not figuring out what is the correct order despite a clear description ;-)",
    "created_at": "2012-03-30T19:35:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122501",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:22" align="right">comment:22</div>

Oh, I don't complain about your post - I complain about patchbot not figuring out what is the correct order despite a clear description ;-)



---

archive/issue_events_156540.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-04-26T15:10:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156540"
}
```



---

archive/issue_events_156541.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-04-26T15:10:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156541"
}
```



---

archive/issue_comments_122502.json:
```json
{
    "body": "Changed work issues from **doctest failure** to **remove trailing whitespaces**",
    "created_at": "2012-04-26T15:10:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122502",
    "user": "https://github.com/fchapoton"
}
```

Changed work issues from **doctest failure** to **remove trailing whitespaces**



---

archive/issue_events_156542.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-04-30T21:30:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156542"
}
```



---

archive/issue_events_156543.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-04-30T21:30:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156543"
}
```



---

archive/issue_comments_122503.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">comment:24</div>\n\nWe don't have a policy on trailing white space, so its completely useless bikeshedding to require it one way or another. But since this ticket is the special case where we create a new subdirectory layout, I used a simple sed script to remove trailing whitespace.",
    "created_at": "2012-04-30T21:30:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122503",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:24" align="right">comment:24</div>

We don't have a policy on trailing white space, so its completely useless bikeshedding to require it one way or another. But since this ticket is the special case where we create a new subdirectory layout, I used a simple sed script to remove trailing whitespace.



---

archive/issue_comments_122504.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -9,4 +9,6 @@\n * [attachment: trac_11763-polyhedra_coercion_folded.2.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.patch.gz)\n * [attachment: trac_11763_fix_associahedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)\n * [attachment: trac_11763_relax_typechecks.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_relax_typechecks.patch.gz)\n+* [attachment: trac_11763_trac_11763_remove_trailing_whitespace.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_trac_11763_remove_trailing_whitespace.patch.gz)\n \n+\n``````\n",
    "created_at": "2012-04-30T21:30:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122504",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -9,4 +9,6 @@
 * [attachment: trac_11763-polyhedra_coercion_folded.2.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.patch.gz)
 * [attachment: trac_11763_fix_associahedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)
 * [attachment: trac_11763_relax_typechecks.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_relax_typechecks.patch.gz)
+* [attachment: trac_11763_trac_11763_remove_trailing_whitespace.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_trac_11763_remove_trailing_whitespace.patch.gz)
 
+
``````




---

archive/issue_comments_122505.json:
```json
{
    "body": "Changed work issues from **remove trailing whitespaces** to none",
    "created_at": "2012-04-30T21:30:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122505",
    "user": "https://github.com/vbraun"
}
```

Changed work issues from **remove trailing whitespaces** to none



---

archive/issue_comments_122506.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -9,6 +9,6 @@\n * [attachment: trac_11763-polyhedra_coercion_folded.2.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.patch.gz)\n * [attachment: trac_11763_fix_associahedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)\n * [attachment: trac_11763_relax_typechecks.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_relax_typechecks.patch.gz)\n-* [attachment: trac_11763_trac_11763_remove_trailing_whitespace.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_trac_11763_remove_trailing_whitespace.patch.gz)\n+* [attachment: trac_11763_remove_trailing_whitespace.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_remove_trailing_whitespace.patch.gz)\n \n \n``````\n",
    "created_at": "2012-04-30T21:31:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122506",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -9,6 +9,6 @@
 * [attachment: trac_11763-polyhedra_coercion_folded.2.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.patch.gz)
 * [attachment: trac_11763_fix_associahedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)
 * [attachment: trac_11763_relax_typechecks.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_relax_typechecks.patch.gz)
-* [attachment: trac_11763_trac_11763_remove_trailing_whitespace.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_trac_11763_remove_trailing_whitespace.patch.gz)
+* [attachment: trac_11763_remove_trailing_whitespace.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_remove_trailing_whitespace.patch.gz)
 
 
``````




---

archive/issue_comments_122507.json:
```json
{
    "body": "<div id=\"comment:26\" align=\"right\">comment:26</div>\n\n* There is now an error to be corrected, see bot report for 5.0rc0. This comes from a strange comparison with 'anything', which looks weird.\n\n* Maybe fold all the patches into one ?\n\n* In the part about associahedra, you replaced the sentence \"Returns the Cartan type of self.\" by the sentence \"Returns the Cartan type\". This seems to be less informative.",
    "created_at": "2012-05-04T17:39:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122507",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:26" align="right">comment:26</div>

* There is now an error to be corrected, see bot report for 5.0rc0. This comes from a strange comparison with 'anything', which looks weird.

* Maybe fold all the patches into one ?

* In the part about associahedra, you replaced the sentence "Returns the Cartan type of self." by the sentence "Returns the Cartan type". This seems to be less informative.



---

archive/issue_events_156544.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-05-04T17:39:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156544"
}
```



---

archive/issue_events_156545.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-05-04T17:39:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156545"
}
```



---

archive/issue_comments_122508.json:
```json
{
    "body": "Updated patch",
    "created_at": "2012-05-04T17:58:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122508",
    "user": "https://github.com/vbraun"
}
```

Updated patch



---

archive/issue_comments_122509.json:
```json
{
    "body": "<div id=\"comment:27\" align=\"right\">comment:27</div>\n\nAttachment: **[trac_11763-polyhedra_coercion_folded.2.2.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.2.patch.gz)**\n\nI've changed the `cmp()` doctest to use `abs(cmp())` to avoid dependence on arbitrary memory positions.\n\nFolding patches is considered harmful since it destroys history without any benefit.\n\nA method always returns information about `self`, you could add \"... of self\" to every docstring.",
    "created_at": "2012-05-04T18:02:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122509",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:27" align="right">comment:27</div>

Attachment: **[trac_11763-polyhedra_coercion_folded.2.2.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.2.patch.gz)**

I've changed the `cmp()` doctest to use `abs(cmp())` to avoid dependence on arbitrary memory positions.

Folding patches is considered harmful since it destroys history without any benefit.

A method always returns information about `self`, you could add "... of self" to every docstring.



---

archive/issue_events_156546.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-05-04T18:07:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156546"
}
```



---

archive/issue_events_156547.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-05-04T18:07:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156547"
}
```



---

archive/issue_comments_122510.json:
```json
{
    "body": "<div id=\"comment:29\" align=\"right\">comment:29</div>\n\nRediffed for sage-5.0.beta3 because #12340 introduced a newline in one of the docstrings. No actual code changed.",
    "created_at": "2012-06-11T15:19:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122510",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:29" align="right">comment:29</div>

Rediffed for sage-5.0.beta3 because #12340 introduced a newline in one of the docstrings. No actual code changed.



---

archive/issue_comments_122511.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -11,4 +11,6 @@\n * [attachment: trac_11763_relax_typechecks.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_relax_typechecks.patch.gz)\n * [attachment: trac_11763_remove_trailing_whitespace.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_remove_trailing_whitespace.patch.gz)\n \n+For the patchbot:\n+Apply trac_11763-polyhedra_coercion_folded.2.patch, trac_11763_fix_associahedron.patch, trac_11763_relax_typechecks.patch, trac_11763_remove_trailing_whitespace.patch\n \n``````\n",
    "created_at": "2012-06-11T15:27:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122511",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -11,4 +11,6 @@
 * [attachment: trac_11763_relax_typechecks.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_relax_typechecks.patch.gz)
 * [attachment: trac_11763_remove_trailing_whitespace.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_remove_trailing_whitespace.patch.gz)
 
+For the patchbot:
+Apply trac_11763-polyhedra_coercion_folded.2.patch, trac_11763_fix_associahedron.patch, trac_11763_relax_typechecks.patch, trac_11763_remove_trailing_whitespace.patch
 
``````




---

archive/issue_comments_122512.json:
```json
{
    "body": "<div id=\"comment:31\" align=\"right\">comment:31</div>\n\nApply trac_11763-polyhedra_coercion_folded.2.patch, trac_11763_fix_associahedron.patch, trac_11763_relax_typechecks.patch, trac_11763_remove_trailing_whitespace.patch",
    "created_at": "2012-06-11T15:31:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122512",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:31" align="right">comment:31</div>

Apply trac_11763-polyhedra_coercion_folded.2.patch, trac_11763_fix_associahedron.patch, trac_11763_relax_typechecks.patch, trac_11763_remove_trailing_whitespace.patch



---

archive/issue_comments_122513.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -11,6 +11,3 @@\n * [attachment: trac_11763_relax_typechecks.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_relax_typechecks.patch.gz)\n * [attachment: trac_11763_remove_trailing_whitespace.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_remove_trailing_whitespace.patch.gz)\n \n-For the patchbot:\n-Apply trac_11763-polyhedra_coercion_folded.2.patch, trac_11763_fix_associahedron.patch, trac_11763_relax_typechecks.patch, trac_11763_remove_trailing_whitespace.patch\n-\n``````\n",
    "created_at": "2012-06-11T15:31:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122513",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -11,6 +11,3 @@
 * [attachment: trac_11763_relax_typechecks.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_relax_typechecks.patch.gz)
 * [attachment: trac_11763_remove_trailing_whitespace.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_remove_trailing_whitespace.patch.gz)
 
-For the patchbot:
-Apply trac_11763-polyhedra_coercion_folded.2.patch, trac_11763_fix_associahedron.patch, trac_11763_relax_typechecks.patch, trac_11763_remove_trailing_whitespace.patch
-
``````




---

archive/issue_comments_122514.json:
```json
{
    "body": "Changed dependencies from **#11634** to **#11634, #13109**",
    "created_at": "2012-06-29T15:49:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122514",
    "user": "https://github.com/vbraun"
}
```

Changed dependencies from **#11634** to **#11634, #13109**



---

archive/issue_comments_122515.json:
```json
{
    "body": "<div id=\"comment:33\" align=\"right\">comment:33</div>\n\n#11310 fixes all the catch-all `except:` clauses, breaking this patch.",
    "created_at": "2012-07-01T16:13:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122515",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:33" align="right">comment:33</div>

#11310 fixes all the catch-all `except:` clauses, breaking this patch.



---

archive/issue_comments_122516.json:
```json
{
    "body": "Changed dependencies from **#11634, #13109** to **#11634, #13109, #11310**",
    "created_at": "2012-07-01T16:13:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122516",
    "user": "https://github.com/vbraun"
}
```

Changed dependencies from **#11634, #13109** to **#11634, #13109, #11310**



---

archive/issue_comments_122517.json:
```json
{
    "body": "<div id=\"comment:34\" align=\"right\">comment:34</div>\n\nI'm getting one error when testing:\n\n```\nFile \"/home/novoselt/sage-5.2.rc1/devel/sage-main/sage/libs/ppl.pyx\", line 119:\n    sage: Polyhedron(vertices=gs, backend='cddr')  # long time (3s on sage.math, 2011)\nException raised:\n    Traceback (most recent call last):\n      File \"/home/novoselt/sage-5.2.rc1/local/bin/ncadoctest.py\", line 1231, in run_one_test\n        self.run_one_example(test, example, filename, compileflags)\n      File \"/home/novoselt/sage-5.2.rc1/local/bin/sagedoctest.py\", line 38, in run_one_example\n        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)\n      File \"/home/novoselt/sage-5.2.rc1/local/bin/ncadoctest.py\", line 1172, in run_one_example\n        compileflags, 1) in test.globs\n      File \"<doctest __main__.example_0[37]>\", line 1, in <module>\n        Polyhedron(vertices=gs, backend='cddr')  # long time (3s on sage.math, 2011)###line 119:\n    sage: Polyhedron(vertices=gs, backend='cddr')  # long time (3s on sage.math, 2011)\n      File \"/home/novoselt/sage-5.2.rc1/local/lib/python/site-packages/sage/misc/decorators.py\", line 691, in wrapper\n        return func(*args, **kwds)\n      File \"/home/novoselt/sage-5.2.rc1/local/lib/python/site-packages/sage/geometry/polyhedron/constructor.py\", line 325, in Polyhedron\n        parent = Polyhedra(base_ring, ambient_dim, backend=backend)\n      File \"/home/novoselt/sage-5.2.rc1/local/lib/python/site-packages/sage/geometry/polyhedron/parent.py\", line 56, in Polyhedra\n        ') implemented for given basering (='+str(base_ring)+').')\n    ValueError: No such backend (=cddr) implemented for given basering (=Integer Ring).\n```",
    "created_at": "2012-07-25T19:50:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122517",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:34" align="right">comment:34</div>

I'm getting one error when testing:

```
File "/home/novoselt/sage-5.2.rc1/devel/sage-main/sage/libs/ppl.pyx", line 119:
    sage: Polyhedron(vertices=gs, backend='cddr')  # long time (3s on sage.math, 2011)
Exception raised:
    Traceback (most recent call last):
      File "/home/novoselt/sage-5.2.rc1/local/bin/ncadoctest.py", line 1231, in run_one_test
        self.run_one_example(test, example, filename, compileflags)
      File "/home/novoselt/sage-5.2.rc1/local/bin/sagedoctest.py", line 38, in run_one_example
        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)
      File "/home/novoselt/sage-5.2.rc1/local/bin/ncadoctest.py", line 1172, in run_one_example
        compileflags, 1) in test.globs
      File "<doctest __main__.example_0[37]>", line 1, in <module>
        Polyhedron(vertices=gs, backend='cddr')  # long time (3s on sage.math, 2011)###line 119:
    sage: Polyhedron(vertices=gs, backend='cddr')  # long time (3s on sage.math, 2011)
      File "/home/novoselt/sage-5.2.rc1/local/lib/python/site-packages/sage/misc/decorators.py", line 691, in wrapper
        return func(*args, **kwds)
      File "/home/novoselt/sage-5.2.rc1/local/lib/python/site-packages/sage/geometry/polyhedron/constructor.py", line 325, in Polyhedron
        parent = Polyhedra(base_ring, ambient_dim, backend=backend)
      File "/home/novoselt/sage-5.2.rc1/local/lib/python/site-packages/sage/geometry/polyhedron/parent.py", line 56, in Polyhedra
        ') implemented for given basering (='+str(base_ring)+').')
    ValueError: No such backend (=cddr) implemented for given basering (=Integer Ring).
```



---

archive/issue_comments_122518.json:
```json
{
    "body": "<div id=\"comment:35\" align=\"right\">comment:35</div>\n\nI fixed the doctest (its `backend=cdd` not `cddr`) and added documentation to the `sage.geometry.polyhedron.parent.Polyhedra` constructor function.",
    "created_at": "2012-08-04T18:12:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122518",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:35" align="right">comment:35</div>

I fixed the doctest (its `backend=cdd` not `cddr`) and added documentation to the `sage.geometry.polyhedron.parent.Polyhedra` constructor function.



---

archive/issue_comments_122519.json:
```json
{
    "body": "<div id=\"comment:36\" align=\"right\">comment:36</div>\n\nUpdated patch fixes one important issue. The inherited comparison for `PolyhedronFace_base` was too lax, which resulted in identical (and not just isomorphic) face lattices in some cases. Fixed now.",
    "created_at": "2012-08-04T22:47:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122519",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:36" align="right">comment:36</div>

Updated patch fixes one important issue. The inherited comparison for `PolyhedronFace_base` was too lax, which resulted in identical (and not just isomorphic) face lattices in some cases. Fixed now.



---

archive/issue_comments_122520.json:
```json
{
    "body": "Changed dependencies from **#11634, #13109, #11310** to **#11634, #13109, #11310, #13638**",
    "created_at": "2012-10-22T11:49:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122520",
    "user": "https://github.com/vbraun"
}
```

Changed dependencies from **#11634, #13109, #11310** to **#11634, #13109, #11310, #13638**



---

archive/issue_comments_122521.json:
```json
{
    "body": "<div id=\"comment:37\" align=\"right\">comment:37</div>\n\nRebased for #13638",
    "created_at": "2012-10-22T11:49:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122521",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:37" align="right">comment:37</div>

Rebased for #13638



---

archive/issue_comments_122522.json:
```json
{
    "body": "Attachment: **[trac_11763_remove_trailing_whitespace.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_remove_trailing_whitespace.patch.gz)**\n\nUpdated patch",
    "created_at": "2012-10-22T11:50:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122522",
    "user": "https://github.com/vbraun"
}
```

Attachment: **[trac_11763_remove_trailing_whitespace.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_remove_trailing_whitespace.patch.gz)**

Updated patch



---

archive/issue_comments_122523.json:
```json
{
    "body": "Updated patch",
    "created_at": "2012-10-23T21:45:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122523",
    "user": "https://github.com/vbraun"
}
```

Updated patch



---

archive/issue_comments_122524.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,6 +8,4 @@\n Apply:\n * [attachment: trac_11763-polyhedra_coercion_folded.2.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.patch.gz)\n * [attachment: trac_11763_fix_associahedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)\n-* [attachment: trac_11763_relax_typechecks.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_relax_typechecks.patch.gz)\n-* [attachment: trac_11763_remove_trailing_whitespace.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_remove_trailing_whitespace.patch.gz)\n \n``````\n",
    "created_at": "2012-10-23T21:47:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122524",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,6 +8,4 @@
 Apply:
 * [attachment: trac_11763-polyhedra_coercion_folded.2.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.patch.gz)
 * [attachment: trac_11763_fix_associahedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)
-* [attachment: trac_11763_relax_typechecks.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_relax_typechecks.patch.gz)
-* [attachment: trac_11763_remove_trailing_whitespace.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_remove_trailing_whitespace.patch.gz)
 
``````




---

archive/issue_comments_122525.json:
```json
{
    "body": "<div id=\"comment:38\" align=\"right\">comment:38</div>\n\nAttachment: **[trac_11763_fix_associahedron.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)**\n\nI've changed the `PolyhedronFace` class to just derive from `SageObject` this should be clearer. Also, the more trivial patches are folded into the first.\n\nIt would be great if this ticket could be reviewed before the heat death of the universe.",
    "created_at": "2012-10-23T21:47:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122525",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:38" align="right">comment:38</div>

Attachment: **[trac_11763_fix_associahedron.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)**

I've changed the `PolyhedronFace` class to just derive from `SageObject` this should be clearer. Also, the more trivial patches are folded into the first.

It would be great if this ticket could be reviewed before the heat death of the universe.



---

archive/issue_comments_122526.json:
```json
{
    "body": "<div id=\"comment:39\" align=\"right\">comment:39</div>\n\nRebased for changes in #13638",
    "created_at": "2012-10-24T09:35:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122526",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:39" align="right">comment:39</div>

Rebased for changes in #13638



---

archive/issue_comments_122527.json:
```json
{
    "body": "<div id=\"comment:40\" align=\"right\">comment:40</div>\n\nApply only trac_11763-polyhedra_coercion_folded.2.patch trac_11763_fix_associahedron.patch",
    "created_at": "2012-10-26T13:34:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122527",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:40" align="right">comment:40</div>

Apply only trac_11763-polyhedra_coercion_folded.2.patch trac_11763_fix_associahedron.patch



---

archive/issue_comments_122528.json:
```json
{
    "body": "<div id=\"comment:41\" align=\"right\">comment:41</div>\n\nOk, let us forget about trailing whitespaces.\n\nBut what about the startup plugin which is complaining here ?",
    "created_at": "2012-10-26T14:38:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122528",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:41" align="right">comment:41</div>

Ok, let us forget about trailing whitespaces.

But what about the startup plugin which is complaining here ?



---

archive/issue_comments_122529.json:
```json
{
    "body": "<div id=\"comment:42\" align=\"right\">comment:42</div>\n\nI agree that using lazy imports would be good. I'll give it a whirl.",
    "created_at": "2012-10-26T14:46:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122529",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:42" align="right">comment:42</div>

I agree that using lazy imports would be good. I'll give it a whirl.



---

archive/issue_comments_122530.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,4 +8,5 @@\n Apply:\n * [attachment: trac_11763-polyhedra_coercion_folded.2.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.patch.gz)\n * [attachment: trac_11763_fix_associahedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)\n+* [attachment: trac_11763_lazy_import.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_lazy_import.patch.gz)\n \n``````\n",
    "created_at": "2012-10-26T14:55:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122530",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,4 +8,5 @@
 Apply:
 * [attachment: trac_11763-polyhedra_coercion_folded.2.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.patch.gz)
 * [attachment: trac_11763_fix_associahedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)
+* [attachment: trac_11763_lazy_import.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_lazy_import.patch.gz)
 
``````




---

archive/issue_comments_122531.json:
```json
{
    "body": "<div id=\"comment:43\" align=\"right\">comment:43</div>\n\nPatch loads Polyhedron lazily, so we don't import anything on startup.",
    "created_at": "2012-10-26T14:55:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122531",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:43" align="right">comment:43</div>

Patch loads Polyhedron lazily, so we don't import anything on startup.



---

archive/issue_comments_122532.json:
```json
{
    "body": "<div id=\"comment:44\" align=\"right\">comment:44</div>\n\nAlso lazily import the parent `PolyhedralSets`, plus one typo fix.",
    "created_at": "2012-10-26T16:32:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122532",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:44" align="right">comment:44</div>

Also lazily import the parent `PolyhedralSets`, plus one typo fix.



---

archive/issue_comments_122533.json:
```json
{
    "body": "<div id=\"comment:45\" align=\"right\">comment:45</div>\n\nApply trac_11763-polyhedra_coercion_folded.2.patch trac_11763_fix_associahedron.patch trac_11763_lazy_import.patch",
    "created_at": "2012-10-26T18:26:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122533",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:45" align="right">comment:45</div>

Apply trac_11763-polyhedra_coercion_folded.2.patch trac_11763_fix_associahedron.patch trac_11763_lazy_import.patch



---

archive/issue_comments_122534.json:
```json
{
    "body": "Improved patch",
    "created_at": "2012-11-06T04:02:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122534",
    "user": "https://github.com/vbraun"
}
```

Improved patch



---

archive/issue_comments_122535.json:
```json
{
    "body": "Changed dependencies from **#11634, #13109, #11310, #13638** to **#11634, #13109, #11310**",
    "created_at": "2012-11-26T13:09:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122535",
    "user": "https://github.com/vbraun"
}
```

Changed dependencies from **#11634, #13109, #11310, #13638** to **#11634, #13109, #11310**



---

archive/issue_comments_122536.json:
```json
{
    "body": "<div id=\"comment:46\" align=\"right\">comment:46</div>\n\nAttachment: **[trac_11763_lazy_import.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_lazy_import.patch.gz)**\n\nI switched the order this patch and #13638.",
    "created_at": "2012-11-26T13:09:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122536",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:46" align="right">comment:46</div>

Attachment: **[trac_11763_lazy_import.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763_lazy_import.patch.gz)**

I switched the order this patch and #13638.



---

archive/issue_events_156548.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2012-11-27T04:46:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156548"
}
```



---

archive/issue_events_156549.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2012-11-27T04:46:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156549"
}
```



---

archive/issue_comments_122537.json:
```json
{
    "body": "<div id=\"comment:48\" align=\"right\">comment:48</div>\n\nThe patchbot seems to follow the patch order in the Attachments list, rather than in the ticket description Apply list.",
    "created_at": "2012-11-27T12:37:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122537",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:48" align="right">comment:48</div>

The patchbot seems to follow the patch order in the Attachments list, rather than in the ticket description Apply list.



---

archive/issue_comments_122538.json:
```json
{
    "body": "<div id=\"comment:49\" align=\"right\">comment:49</div>\n\nReplying to [@dimpase](#comment%3A48):\n> The patchbot seems to follow the patch order in the Attachments list, rather than in the ticket description Apply list.\n\nIf I am reading the [right patchbot source](https://github.com/robertwb/sage-patchbot/blob/master/src/trac.py), it seems that it looks for `apply` in the whole rss feed...",
    "created_at": "2012-11-27T12:49:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122538",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:49" align="right">comment:49</div>

Replying to [@dimpase](#comment%3A48):
> The patchbot seems to follow the patch order in the Attachments list, rather than in the ticket description Apply list.

If I am reading the [right patchbot source](https://github.com/robertwb/sage-patchbot/blob/master/src/trac.py), it seems that it looks for `apply` in the whole rss feed...



---

archive/issue_comments_122539.json:
```json
{
    "body": "<div id=\"comment:50\" align=\"right\">comment:50</div>\n\nFor the patchbot:\n\nApply trac_11763-polyhedra_coercion_folded.2.patch, trac_11763_fix_associahedron.patch, trac_11763_lazy_import.patch",
    "created_at": "2012-11-27T16:52:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122539",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:50" align="right">comment:50</div>

For the patchbot:

Apply trac_11763-polyhedra_coercion_folded.2.patch, trac_11763_fix_associahedron.patch, trac_11763_lazy_import.patch



---

archive/issue_comments_122540.json:
```json
{
    "body": "Reviewer: **Dmitrii Pasechnik**",
    "created_at": "2012-11-28T15:35:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122540",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Dmitrii Pasechnik**



---

archive/issue_comments_122541.json:
```json
{
    "body": "<div id=\"comment:52\" align=\"right\">comment:52</div>\n\nOn some systems, such as OS X >= 10.6 and Itanium:\n\n```\nsage -t  --long -force_lib devel/sage/sage/geometry/polyhedron/base.py\n**********************************************************************\nFile \"/Users/dehayebuildbot/build/sage/dehaye/dehaye_full/build/sage-5.6.beta0/devel/sage-main/sage/geometry/polyhedron/base.py\", line 385:\n    sage: cmp(P, 'string')\nExpected:\n    -1\nGot:\n    1\n**********************************************************************\n```",
    "created_at": "2012-12-18T07:36:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122541",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:52" align="right">comment:52</div>

On some systems, such as OS X >= 10.6 and Itanium:

```
sage -t  --long -force_lib devel/sage/sage/geometry/polyhedron/base.py
**********************************************************************
File "/Users/dehayebuildbot/build/sage/dehaye/dehaye_full/build/sage-5.6.beta0/devel/sage-main/sage/geometry/polyhedron/base.py", line 385:
    sage: cmp(P, 'string')
Expected:
    -1
Got:
    1
**********************************************************************
```



---

archive/issue_events_156550.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-18T08:05:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156550"
}
```



---

archive/issue_events_156551.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-18T08:05:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156551"
}
```



---

archive/issue_comments_122542.json:
```json
{
    "body": "Attachment: **[trac_11763-polyhedra_coercion_folded.2.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.patch.gz)**\n\nUpdated patch",
    "created_at": "2012-12-18T10:06:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122542",
    "user": "https://github.com/vbraun"
}
```

Attachment: **[trac_11763-polyhedra_coercion_folded.2.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.patch.gz)**

Updated patch



---

archive/issue_events_156552.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-12-18T10:06:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156552"
}
```



---

archive/issue_events_156553.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-12-18T10:06:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156553"
}
```



---

archive/issue_comments_122543.json:
```json
{
    "body": "<div id=\"comment:54\" align=\"right\">comment:54</div>\n\nI've added the trivial fix.",
    "created_at": "2012-12-18T10:06:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122543",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:54" align="right">comment:54</div>

I've added the trivial fix.



---

archive/issue_comments_122544.json:
```json
{
    "body": "<div id=\"comment:55\" align=\"right\">comment:55</div>\n\nAnd now I noticed that this was already fixed in #12193.",
    "created_at": "2012-12-18T10:43:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122544",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:55" align="right">comment:55</div>

And now I noticed that this was already fixed in #12193.



---

archive/issue_comments_122545.json:
```json
{
    "body": "Merged: **sage-5.6.beta1**",
    "created_at": "2012-12-21T09:30:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122545",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.6.beta1**



---

archive/issue_events_156554.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-21T09:30:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156554"
}
```



---

archive/issue_events_156555.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-21T09:30:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11763#event-156555"
}
```



---

archive/issue_comments_122546.json:
```json
{
    "body": "Attachment: **[trac_11763-polyhedra_coercion_folded.3.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.3.patch.gz)**",
    "created_at": "2012-12-21T09:41:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122546",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[trac_11763-polyhedra_coercion_folded.3.patch.gz](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.3.patch.gz)**



---

archive/issue_comments_122547.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,7 +6,7 @@\n * RDF\n \n Apply:\n-* [attachment: trac_11763-polyhedra_coercion_folded.2.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.patch.gz)\n+* [attachment: trac_11763-polyhedra_coercion_folded.3.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.3.patch.gz)\n * [attachment: trac_11763_fix_associahedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)\n * [attachment: trac_11763_lazy_import.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_lazy_import.patch.gz)\n \n``````\n",
    "created_at": "2012-12-21T09:41:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122547",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,7 +6,7 @@
 * RDF
 
 Apply:
-* [attachment: trac_11763-polyhedra_coercion_folded.2.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.2.patch.gz)
+* [attachment: trac_11763-polyhedra_coercion_folded.3.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763-polyhedra_coercion_folded.3.patch.gz)
 * [attachment: trac_11763_fix_associahedron.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_fix_associahedron.patch.gz)
 * [attachment: trac_11763_lazy_import.patch](https://github.com/sagemath/sage/files/ticket11763/trac_11763_lazy_import.patch.gz)
 
``````




---

archive/issue_comments_122548.json:
```json
{
    "body": "<div id=\"comment:57\" align=\"right\">comment:57</div>\n\nRebased to sage-5.6.beta0.",
    "created_at": "2012-12-21T09:41:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122548",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:57" align="right">comment:57</div>

Rebased to sage-5.6.beta0.



---

archive/issue_comments_122549.json:
```json
{
    "body": "<div id=\"comment:58\" align=\"right\">comment:58</div>\n\nHey Volker,\n\nDo you remember what was the reason for this change in `normalize_rays`?\n\n```diff\ndiff --git a/sage/geometry/cone.py b/sage/geometry/cone.py\n\nIndex: sage/geometry/cone.py\n===================================================================\n--- a/sage/geometry/cone.py\n+++ b/sage/geometry/cone.py\n@@ -568,7 +568,10 @@\n         V = lattice.base_extend(QQ)\n         for n, ray in enumerate(rays):\n             try:\n-                ray = V(ray)\n+                if isinstance(ray, (list, tuple, V._element_class)):\n+                    ray = V(ray)\n+                else:\n+                    ray = V(list(ray))\n             except TypeError:\n                 raise TypeError(\"cannot convert %s to %s!\" % (ray, V))\n             if ray.is_zero():\n```\nIt seems that if `V` does not want to take a `ray`, we should not force it.",
    "created_at": "2012-12-24T20:26:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122549",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:58" align="right">comment:58</div>

Hey Volker,

Do you remember what was the reason for this change in `normalize_rays`?

```diff
diff --git a/sage/geometry/cone.py b/sage/geometry/cone.py

Index: sage/geometry/cone.py
===================================================================
--- a/sage/geometry/cone.py
+++ b/sage/geometry/cone.py
@@ -568,7 +568,10 @@
         V = lattice.base_extend(QQ)
         for n, ray in enumerate(rays):
             try:
-                ray = V(ray)
+                if isinstance(ray, (list, tuple, V._element_class)):
+                    ray = V(ray)
+                else:
+                    ray = V(list(ray))
             except TypeError:
                 raise TypeError("cannot convert %s to %s!" % (ray, V))
             if ray.is_zero():
```
It seems that if `V` does not want to take a `ray`, we should not force it.



---

archive/issue_comments_122550.json:
```json
{
    "body": "<div id=\"comment:59\" align=\"right\">comment:59</div>\n\nI think the idea is to allow arbitrary iterables in `normalize_rays`. Why not? Its an auxiliary function, and the more general it is the better.",
    "created_at": "2012-12-24T20:34:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122550",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:59" align="right">comment:59</div>

I think the idea is to allow arbitrary iterables in `normalize_rays`. Why not? Its an auxiliary function, and the more general it is the better.



---

archive/issue_comments_122551.json:
```json
{
    "body": "<div id=\"comment:60\" align=\"right\">comment:60</div>\n\nI just was wondering what was the particular use case - found this bit because of the fuzz with another patch. I don't mind it, although changing what `V.__call__` accepts seems a more pleasant solution ;-)",
    "created_at": "2012-12-25T02:42:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122551",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:60" align="right">comment:60</div>

I just was wondering what was the particular use case - found this bit because of the fuzz with another patch. I don't mind it, although changing what `V.__call__` accepts seems a more pleasant solution ;-)



---

archive/issue_comments_122552.json:
```json
{
    "body": "<div id=\"comment:61\" align=\"right\">comment:61</div>\n\nReplying to [@novoselt](#comment%3A60):\n> I don't mind it, although changing what `V.__call__` accepts seems a more pleasant solution ;-)\n\nWasn't that one of our design decisions, that the ToricLattice should forbid conversion between the lattice and its dual?",
    "created_at": "2012-12-25T13:24:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122552",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:61" align="right">comment:61</div>

Replying to [@novoselt](#comment%3A60):
> I don't mind it, although changing what `V.__call__` accepts seems a more pleasant solution ;-)

Wasn't that one of our design decisions, that the ToricLattice should forbid conversion between the lattice and its dual?



---

archive/issue_comments_122553.json:
```json
{
    "body": "<div id=\"comment:62\" align=\"right\">comment:62</div>\n\nReplying to [@vbraun](#comment%3A61):\n> Wasn't that one of our design decisions, that the ToricLattice should forbid conversion between the lattice and its dual? \n\nWe prohibit coercion, conversion is certainly possible, and here `V` is the spanned vector space anyway. But I've also hit many times a situation when a standard constructor in Sage would not take generators as input, which is probably what has happened here.",
    "created_at": "2012-12-28T02:53:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11763#issuecomment-122553",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:62" align="right">comment:62</div>

Replying to [@vbraun](#comment%3A61):
> Wasn't that one of our design decisions, that the ToricLattice should forbid conversion between the lattice and its dual? 

We prohibit coercion, conversion is certainly possible, and here `V` is the spanned vector space anyway. But I've also hit many times a situation when a standard constructor in Sage would not take generators as input, which is probably what has happened here.

# Issue 11867: Singular Spielwiese

archive/issues_011695.json:
```json
{
    "body": "The next version of Singular does some major refactoring, in particular splits off polynomial handling into libpolys and introduces the coeffs struct that holds the coefficient ring (base ring) information just like the ring struct holds the information about polys.\n\nThe upstream source is developed here: https://github.com/Singular/Sources\n\nUpdated spkg: http://www.stp.dias.ie/~vbraun/Sage/spkg/singular-3-9-9.spkg (from September 2011)\n\nWARNING: This is not working yet, major breakage in the Sage library needs to be fixed.\n\n**Assignee:** @aghitza\n\n**CC:**  @burcin @kiwifb\n\n**Keywords:** sd34\n\n**Work Issues:** update Sage library\n\n**Reviewer:** Fran\u00e7ois Bissey\n\nIssue created by migration from https://trac.sagemath.org/ticket/11867\n\n",
    "closed_at": "2018-05-18T17:16:26Z",
    "created_at": "2011-09-29T08:31:59Z",
    "labels": [
        "component: algebra",
        "wontfix"
    ],
    "title": "Singular Spielwiese",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/11867",
    "user": "https://github.com/vbraun"
}
```
The next version of Singular does some major refactoring, in particular splits off polynomial handling into libpolys and introduces the coeffs struct that holds the coefficient ring (base ring) information just like the ring struct holds the information about polys.

The upstream source is developed here: https://github.com/Singular/Sources

Updated spkg: http://www.stp.dias.ie/~vbraun/Sage/spkg/singular-3-9-9.spkg (from September 2011)

WARNING: This is not working yet, major breakage in the Sage library needs to be fixed.

**Assignee:** @aghitza

**CC:**  @burcin @kiwifb

**Keywords:** sd34

**Work Issues:** update Sage library

**Reviewer:** François Bissey

Issue created by migration from https://trac.sagemath.org/ticket/11867





---

archive/attachments_016307.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11867_Singular-spielwiese.patch",
    "asset_url": "tarball://root/attachments/ticket11867/trac_11867_Singular-spielwiese.patch",
    "created_at": "2011-09-29T08:34:52Z",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11867/trac_11867_Singular-spielwiese.patch",
    "user": "https://github.com/vbraun"
}
```



---

archive/issue_comments_126346.json:
```json
{
    "body": "**Attachment:** [trac_11867_Singular-spielwiese.patch](https://github.com/sagemath/sage/files/ticket11867/trac_11867_Singular-spielwiese.patch)\n\nfixes for module_list.py since libraries and headers move around",
    "created_at": "2011-09-29T08:34:52Z",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11867#issuecomment-126346",
    "user": "https://github.com/vbraun"
}
```

**Attachment:** [trac_11867_Singular-spielwiese.patch](https://github.com/sagemath/sage/files/ticket11867/trac_11867_Singular-spielwiese.patch)

fixes for module_list.py since libraries and headers move around



---

archive/issue_comments_126347.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,4 +1,6 @@\n The next version of Singular does some major refactoring, in particular splits off polynomial handling into libpolys and introduces the coeffs struct that holds the coefficient ring (base ring) information just like the ring struct holds the information about polys.\n+\n+The upstream source is developed here: https://github.com/Singular2/Sources/commits/spielwiese\n \n Updated spkg: http://www.stp.dias.ie/~vbraun/Sage/spkg/singular-3-9-9.spkg\n \n``````\n",
    "created_at": "2011-09-29T08:36:58Z",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11867#issuecomment-126347",
    "user": "https://github.com/vbraun"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,4 +1,6 @@
 The next version of Singular does some major refactoring, in particular splits off polynomial handling into libpolys and introduces the coeffs struct that holds the coefficient ring (base ring) information just like the ring struct holds the information about polys.
+
+The upstream source is developed here: https://github.com/Singular2/Sources/commits/spielwiese
 
 Updated spkg: http://www.stp.dias.ie/~vbraun/Sage/spkg/singular-3-9-9.spkg
 
``````




---

archive/issue_comments_126348.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,7 +1,7 @@\n The next version of Singular does some major refactoring, in particular splits off polynomial handling into libpolys and introduces the coeffs struct that holds the coefficient ring (base ring) information just like the ring struct holds the information about polys.\n \n-The upstream source is developed here: https://github.com/Singular2/Sources/commits/spielwiese\n+The upstream source is developed here: https://github.com/Singular/Sources\n \n-Updated spkg: http://www.stp.dias.ie/~vbraun/Sage/spkg/singular-3-9-9.spkg\n+Updated spkg: http://www.stp.dias.ie/~vbraun/Sage/spkg/singular-3-9-9.spkg (from September 2011)\n \n WARNING: This is not working yet, major breakage in the Sage library needs to be fixed.\n``````\n",
    "created_at": "2012-02-06T08:55:53Z",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11867#issuecomment-126348",
    "user": "https://github.com/burcin"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,7 +1,7 @@
 The next version of Singular does some major refactoring, in particular splits off polynomial handling into libpolys and introduces the coeffs struct that holds the coefficient ring (base ring) information just like the ring struct holds the information about polys.
 
-The upstream source is developed here: https://github.com/Singular2/Sources/commits/spielwiese
+The upstream source is developed here: https://github.com/Singular/Sources
 
-Updated spkg: http://www.stp.dias.ie/~vbraun/Sage/spkg/singular-3-9-9.spkg
+Updated spkg: http://www.stp.dias.ie/~vbraun/Sage/spkg/singular-3-9-9.spkg (from September 2011)
 
 WARNING: This is not working yet, major breakage in the Sage library needs to be fixed.
``````




---

archive/issue_events_095783.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11867#event-95783"
}
```



---

archive/issue_events_095784.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11867#event-95784"
}
```



---

archive/issue_events_095785.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11867#event-95785"
}
```



---

archive/issue_events_095786.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11867#event-95786"
}
```



---

archive/issue_events_095787.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11867#event-95787"
}
```



---

archive/issue_events_095788.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11867#event-95788"
}
```



---

archive/issue_events_095789.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11867#event-95789"
}
```



---

archive/issue_events_095790.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11867#event-95790"
}
```



---

archive/issue_comments_126349.json:
```json
{
    "body": "<a id='comment:7'></a>\nHello,\n\nin the meantime the fourth patch of Singular release 4.0 is available at \nhttp://www.mathematik.uni-kl.de/ftp/pub/Math/Singular/SOURCES/4-0-0/\n\nAmong the refactoring it contains a lot of bugfixes (and likely some new bugs)",
    "created_at": "2014-08-28T15:40:36Z",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11867#issuecomment-126349",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

<a id='comment:7'></a>
Hello,

in the meantime the fourth patch of Singular release 4.0 is available at 
http://www.mathematik.uni-kl.de/ftp/pub/Math/Singular/SOURCES/4-0-0/

Among the refactoring it contains a lot of bugfixes (and likely some new bugs)



---

archive/issue_events_095791.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-04-30T08:08:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11867#event-95791"
}
```



---

archive/issue_comments_126350.json:
```json
{
    "body": "<a id='comment:9'></a>\nWe have been using singular 4.0 since long. Right now:\n\n```\nsingular................................4.1.0p3.p2\n```",
    "created_at": "2018-04-30T08:08:38Z",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11867#issuecomment-126350",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:9'></a>
We have been using singular 4.0 since long. Right now:

```
singular................................4.1.0p3.p2
```



---

archive/issue_events_095792.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-04-30T08:08:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11867#event-95792"
}
```



---

archive/issue_events_095793.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2018-04-30T08:11:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11867#event-95793"
}
```



---

archive/issue_events_095794.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2018-04-30T08:11:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11867#event-95794"
}
```



---

archive/issue_comments_126351.json:
```json
{
    "body": "**Reviewer:** Fran\u00e7ois Bissey",
    "created_at": "2018-04-30T08:11:57Z",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11867#issuecomment-126351",
    "user": "https://github.com/kiwifb"
}
```

**Reviewer:** François Bissey



---

archive/issue_comments_126352.json:
```json
{
    "body": "<a id='comment:10'></a>\nRight.",
    "created_at": "2018-04-30T08:11:57Z",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11867#issuecomment-126352",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:10'></a>
Right.



---

archive/issue_events_095795.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "label": "wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11867#event-95795"
}
```



---

archive/issue_comments_126353.json:
```json
{
    "body": "<a id='comment:11'></a>\nclosing positively reviewed duplicates",
    "created_at": "2018-05-18T17:16:26Z",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11867#issuecomment-126353",
    "user": "https://github.com/videlec"
}
```

<a id='comment:11'></a>
closing positively reviewed duplicates



---

archive/issue_events_095796.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11867#event-95796"
}
```



---

archive/issue_events_095797.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11867",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11867#event-95797"
}
```

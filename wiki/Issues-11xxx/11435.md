# Issue 11435: empty partition causes error for down_list

archive/issues_011263.json:
```json
{
    "body": "\n```\nsage: p = Partition([])\nsage: p.down_list()\n---------------------------------------------------------------------------\nIndexError                                Traceback (most recent call last)\n\n/Volumes/Scratch/Users/duser1/sage/<ipython console> in <module>()\n\n/Users/duser1/sage/local/lib/python2.6/site-packages/sage/combinat/partition.pyc in down_list(self)\n    643             [[2, 2, 1], [3, 1, 1], [3, 2]]\n    644         \"\"\"\n--> 645         return [p for p in self.down()]\n    646 \n    647     def dominates(self, p2):\n\n/Users/duser1/sage/local/lib/python2.6/site-packages/sage/combinat/partition.pyc in down(self)\n    622                 yield Partition(p[:i] + [ p[i]-1 ] + p[i+1:])\n    623 \n--> 624         last = p[-1]\n    625         if last == 1:\n    626             yield Partition(p[:-1])\n\n/Users/duser1/sage/local/lib/python2.6/site-packages/sage/combinat/combinat.pyc in __getitem__(self, key)\n    905             <type 'list'>\n    906         \"\"\"\n--> 907         return self._list.__getitem__(key)\n    908 \n    909     def __iter__(self):\n\nIndexError: list index out of range\nsage: \n\n```\n\n\n**Assignee:** sage-combinat\n\n**CC:**  @jbandlow\n\n**Author:** Paul-Olivier Dehaye\n\n**Reviewer:** Nathann Cohen\n\n**Merged:** sage-4.7.2.alpha1\n\nIssue created by migration from https://trac.sagemath.org/ticket/11435\n\n",
    "closed_at": "2011-08-03T14:37:56Z",
    "created_at": "2011-06-06T23:06:02Z",
    "labels": [
        "component: combinatorics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.2",
    "title": "empty partition causes error for down_list",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/11435",
    "user": "https://trac.sagemath.org/admin/accounts/users/pdehaye"
}
```

```
sage: p = Partition([])
sage: p.down_list()
---------------------------------------------------------------------------
IndexError                                Traceback (most recent call last)

/Volumes/Scratch/Users/duser1/sage/<ipython console> in <module>()

/Users/duser1/sage/local/lib/python2.6/site-packages/sage/combinat/partition.pyc in down_list(self)
    643             [[2, 2, 1], [3, 1, 1], [3, 2]]
    644         """
--> 645         return [p for p in self.down()]
    646 
    647     def dominates(self, p2):

/Users/duser1/sage/local/lib/python2.6/site-packages/sage/combinat/partition.pyc in down(self)
    622                 yield Partition(p[:i] + [ p[i]-1 ] + p[i+1:])
    623 
--> 624         last = p[-1]
    625         if last == 1:
    626             yield Partition(p[:-1])

/Users/duser1/sage/local/lib/python2.6/site-packages/sage/combinat/combinat.pyc in __getitem__(self, key)
    905             <type 'list'>
    906         """
--> 907         return self._list.__getitem__(key)
    908 
    909     def __iter__(self):

IndexError: list index out of range
sage: 

```


**Assignee:** sage-combinat

**CC:**  @jbandlow

**Author:** Paul-Olivier Dehaye

**Reviewer:** Nathann Cohen

**Merged:** sage-4.7.2.alpha1

Issue created by migration from https://trac.sagemath.org/ticket/11435





---

archive/attachments_015589.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11435-Downlist_on_empty_partition-pod.patch",
    "asset_url": "tarball://root/attachments/ticket11435/trac_11435-Downlist_on_empty_partition-pod.patch",
    "created_at": "2011-06-07T15:40:50Z",
    "issue": "https://github.com/sagemath/sage/issues/11435",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11435/trac_11435-Downlist_on_empty_partition-pod.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/pdehaye"
}
```



---

archive/issue_comments_117884.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_11435-Downlist_on_empty_partition-pod.patch](https://github.com/sagemath/sage/files/ticket11435/trac_11435-Downlist_on_empty_partition-pod.patch)\n\nfixed",
    "created_at": "2011-06-07T15:40:50Z",
    "issue": "https://github.com/sagemath/sage/issues/11435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11435#issuecomment-117884",
    "user": "https://trac.sagemath.org/admin/accounts/users/pdehaye"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_11435-Downlist_on_empty_partition-pod.patch](https://github.com/sagemath/sage/files/ticket11435/trac_11435-Downlist_on_empty_partition-pod.patch)

fixed



---

archive/issue_events_090570.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/pdehaye",
    "created_at": "2011-06-07T15:40:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11435",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11435#event-90570"
}
```



---

archive/issue_comments_117885.json:
```json
{
    "body": "<a id='comment:2'></a>\nGood to go `:-)`\n\nNathann",
    "created_at": "2011-07-14T12:31:40Z",
    "issue": "https://github.com/sagemath/sage/issues/11435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11435#issuecomment-117885",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:2'></a>
Good to go `:-)`

Nathann



---

archive/issue_events_090571.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-07-14T12:31:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11435",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11435#event-90571"
}
```



---

archive/issue_events_090572.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-07-14T12:31:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11435",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11435#event-90572"
}
```



---

archive/issue_events_090573.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-19T10:16:54Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11435",
    "milestone": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11435#event-90573"
}
```



---

archive/issue_events_090574.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-19T10:16:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11435",
    "milestone": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11435#event-90574"
}
```



---

archive/issue_comments_117886.json:
```json
{
    "body": "<a id='comment:4'></a>\nDear author/reviewer: please add your real name in the Author and Reviewer fields.",
    "created_at": "2011-07-19T10:17:26Z",
    "issue": "https://github.com/sagemath/sage/issues/11435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11435#issuecomment-117886",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'></a>
Dear author/reviewer: please add your real name in the Author and Reviewer fields.



---

archive/issue_comments_117887.json:
```json
{
    "body": "**Author:** Paul-Olivier Dehaye",
    "created_at": "2011-07-19T15:37:15Z",
    "issue": "https://github.com/sagemath/sage/issues/11435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11435#issuecomment-117887",
    "user": "https://github.com/nathanncohen"
}
```

**Author:** Paul-Olivier Dehaye



---

archive/issue_comments_117888.json:
```json
{
    "body": "**Reviewer:** Nathann Cohen",
    "created_at": "2011-07-19T15:37:15Z",
    "issue": "https://github.com/sagemath/sage/issues/11435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11435#issuecomment-117888",
    "user": "https://github.com/nathanncohen"
}
```

**Reviewer:** Nathann Cohen



---

archive/issue_comments_117889.json:
```json
{
    "body": "**Merged:** sage-4.7.2.alpha1",
    "created_at": "2011-08-03T14:37:56Z",
    "issue": "https://github.com/sagemath/sage/issues/11435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11435#issuecomment-117889",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-4.7.2.alpha1



---

archive/issue_events_090575.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-08-03T14:37:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11435",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11435#event-90575"
}
```



---

archive/issue_events_090576.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-08-03T14:37:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11435",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11435#event-90576"
}
```

# Issue 11365: Bad formatting in hasse_diagram

archive/issues_011193.json:
```json
{
    "body": "See [this page](http://www.sagemath.org/doc/reference/sage/combinat/posets/hasse_diagram.html), at the definition of complements and further.  Must be missing some colons or something.\n\n**Assignee:** mvngu\n\n**CC:**  sage-combinat\n\n**Author:** Minh Van Nguyen\n\n**Reviewer:** Mariah Lenox\n\n**Merged:** sage-4.7.1.alpha2\n\nIssue created by migration from https://trac.sagemath.org/ticket/11365\n\n",
    "closed_at": "2011-05-31T17:06:43Z",
    "created_at": "2011-05-21T02:52:34Z",
    "labels": [
        "component: documentation",
        "trivial",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.1",
    "title": "Bad formatting in hasse_diagram",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/11365",
    "user": "https://github.com/kcrisman"
}
```
See [this page](http://www.sagemath.org/doc/reference/sage/combinat/posets/hasse_diagram.html), at the definition of complements and further.  Must be missing some colons or something.

**Assignee:** mvngu

**CC:**  sage-combinat

**Author:** Minh Van Nguyen

**Reviewer:** Mariah Lenox

**Merged:** sage-4.7.1.alpha2

Issue created by migration from https://trac.sagemath.org/ticket/11365





---

archive/issue_comments_116884.json:
```json
{
    "body": "**Author:** Minh Van Nguyen",
    "created_at": "2011-05-21T06:18:37Z",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11365#issuecomment-116884",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Author:** Minh Van Nguyen



---

archive/issue_events_089861.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2011-05-21T06:18:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11365#event-89861"
}
```



---

archive/attachments_015460.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac-11365_hasse-diagram-typos.patch",
    "asset_url": "tarball://root/attachments/ticket11365/trac-11365_hasse-diagram-typos.patch",
    "created_at": "2011-05-21T06:18:37Z",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11365/trac-11365_hasse-diagram-typos.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```



---

archive/issue_comments_116885.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac-11365_hasse-diagram-typos.patch](https://github.com/sagemath/sage/files/ticket11365/trac-11365_hasse-diagram-typos.patch)",
    "created_at": "2011-05-21T06:18:37Z",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11365#issuecomment-116885",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:1'></a>
**Attachment:** [trac-11365_hasse-diagram-typos.patch](https://github.com/sagemath/sage/files/ticket11365/trac-11365_hasse-diagram-typos.patch)



---

archive/issue_comments_116886.json:
```json
{
    "body": "**Reviewer:** Mariah Lenox",
    "created_at": "2011-05-24T13:56:35Z",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11365#issuecomment-116886",
    "user": "https://trac.sagemath.org/admin/accounts/users/mariah"
}
```

**Reviewer:** Mariah Lenox



---

archive/issue_comments_116887.json:
```json
{
    "body": "<a id='comment:2'></a>\nApplied patch, did 'sage -b', then './sage -docbuild reference html',\nand verified that the patch correctly formated the tests and examples of the functions 'complements()', 'cover_relations()', and 'cover_relations_iterator()'.  Positive review.",
    "created_at": "2011-05-24T13:56:35Z",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11365#issuecomment-116887",
    "user": "https://trac.sagemath.org/admin/accounts/users/mariah"
}
```

<a id='comment:2'></a>
Applied patch, did 'sage -b', then './sage -docbuild reference html',
and verified that the patch correctly formated the tests and examples of the functions 'complements()', 'cover_relations()', and 'cover_relations_iterator()'.  Positive review.



---

archive/issue_events_089862.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mariah",
    "created_at": "2011-05-24T13:56:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11365#event-89862"
}
```



---

archive/issue_events_089863.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mariah",
    "created_at": "2011-05-24T13:56:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11365#event-89863"
}
```



---

archive/issue_comments_116888.json:
```json
{
    "body": "<a id='comment:3'></a>\nHi Minh,\n\nThanks for the improvements!\n\nLuckily, it seems like this will commute smoothly with #10998. So I leave the positive review. But please synchronise with us next time!\n\nCheers,",
    "created_at": "2011-05-25T09:22:58Z",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11365#issuecomment-116888",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:3'></a>
Hi Minh,

Thanks for the improvements!

Luckily, it seems like this will commute smoothly with #10998. So I leave the positive review. But please synchronise with us next time!

Cheers,



---

archive/issue_comments_116889.json:
```json
{
    "body": "**Merged:** sage-4.7.1.alpha2",
    "created_at": "2011-05-31T17:06:43Z",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11365#issuecomment-116889",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-4.7.1.alpha2



---

archive/issue_events_089864.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-31T17:06:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11365#event-89864"
}
```



---

archive/issue_events_089865.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-31T17:06:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11365#event-89865"
}
```

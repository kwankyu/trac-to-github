# Issue 11365: Bad formatting in hasse_diagram

archive/issues_011193.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nSee [this page](http://www.sagemath.org/doc/reference/sage/combinat/posets/hasse_diagram.html), at the definition of complements and further.  Must be missing some colons or something.\n\nCC:  @sagetrac-sage-combinat\n\nComponent: **documentation**\n\nAuthor: **Minh Van Nguyen**\n\nReviewer: **Mariah Lenox**\n\nMerged: **sage-4.7.1.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11365_\n\n",
    "closed_at": "2011-05-31T17:06:43Z",
    "created_at": "2011-05-21T02:52:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20trivial%20/%205",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bad formatting in hasse_diagram",
    "type": "issue",
    "updated_at": "2011-05-31T17:06:43Z",
    "url": "https://github.com/sagemath/sage/issues/11365",
    "user": "https://github.com/kcrisman"
}
```
<div id="comment:0"></div>

See [this page](http://www.sagemath.org/doc/reference/sage/combinat/posets/hasse_diagram.html), at the definition of complements and further.  Must be missing some colons or something.

CC:  @sagetrac-sage-combinat

Component: **documentation**

Author: **Minh Van Nguyen**

Reviewer: **Mariah Lenox**

Merged: **sage-4.7.1.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/11365_





---

archive/issue_events_149640.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-05-21T02:52:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11365#event-149640"
}
```



---

archive/issue_events_149641.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-05-21T02:52:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11365#event-149641"
}
```



---

archive/issue_events_149642.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-05-21T02:52:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20trivial%20/%205",
    "label_color": "fff9e5",
    "label_name": "p: trivial / 5",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11365#event-149642"
}
```



---

archive/issue_events_149643.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-05-21T02:52:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11365#event-149643"
}
```



---

archive/issue_events_149644.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2011-05-21T02:52:34Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "subject": "https://github.com/kcrisman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11365#event-149644"
}
```



---

archive/issue_comments_114738.json:
```json
{
    "body": "Author: **Minh Van Nguyen**",
    "created_at": "2011-05-21T06:18:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11365#issuecomment-114738",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Minh Van Nguyen**



---

archive/issue_events_149645.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2011-05-21T06:18:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11365#event-149645"
}
```



---

archive/issue_comments_114739.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac-11365_hasse-diagram-typos.patch.gz](https://github.com/sagemath/sage/files/ticket11365/trac-11365_hasse-diagram-typos.patch.gz)**",
    "created_at": "2011-05-21T06:18:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11365#issuecomment-114739",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac-11365_hasse-diagram-typos.patch.gz](https://github.com/sagemath/sage/files/ticket11365/trac-11365_hasse-diagram-typos.patch.gz)**



---

archive/issue_comments_114740.json:
```json
{
    "body": "Reviewer: **Mariah Lenox**",
    "created_at": "2011-05-24T13:56:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11365#issuecomment-114740",
    "user": "https://github.com/sagetrac-mariah"
}
```

Reviewer: **Mariah Lenox**



---

archive/issue_comments_114741.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nApplied patch, did 'sage -b', then './sage -docbuild reference html',\nand verified that the patch correctly formated the tests and examples of the functions 'complements()', 'cover_relations()', and 'cover_relations_iterator()'.  Positive review.",
    "created_at": "2011-05-24T13:56:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11365#issuecomment-114741",
    "user": "https://github.com/sagetrac-mariah"
}
```

<div id="comment:2" align="right">comment:2</div>

Applied patch, did 'sage -b', then './sage -docbuild reference html',
and verified that the patch correctly formated the tests and examples of the functions 'complements()', 'cover_relations()', and 'cover_relations_iterator()'.  Positive review.



---

archive/issue_events_149646.json:
```json
{
    "actor": "https://github.com/sagetrac-mariah",
    "created_at": "2011-05-24T13:56:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11365#event-149646"
}
```



---

archive/issue_events_149647.json:
```json
{
    "actor": "https://github.com/sagetrac-mariah",
    "created_at": "2011-05-24T13:56:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11365#event-149647"
}
```



---

archive/issue_comments_114742.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nHi Minh,\n\nThanks for the improvements!\n\nLuckily, it seems like this will commute smoothly with #10998. So I leave the positive review. But please synchronise with us next time!\n\nCheers,",
    "created_at": "2011-05-25T09:22:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11365#issuecomment-114742",
    "user": "https://github.com/nthiery"
}
```

<div id="comment:3" align="right">comment:3</div>

Hi Minh,

Thanks for the improvements!

Luckily, it seems like this will commute smoothly with #10998. So I leave the positive review. But please synchronise with us next time!

Cheers,



---

archive/issue_comments_114743.json:
```json
{
    "body": "Merged: **sage-4.7.1.alpha2**",
    "created_at": "2011-05-31T17:06:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11365#issuecomment-114743",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.1.alpha2**



---

archive/issue_events_149648.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-31T17:06:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11365#event-149648"
}
```



---

archive/issue_events_149649.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-31T17:06:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11365",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11365#event-149649"
}
```

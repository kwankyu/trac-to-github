# Issue 11452: serious (but trivial to fix) bug in hidden markov model "generate sequence" function

archive/issues_011280.json:
```json
{
    "body": "The generate sequence function for continuous hidden markov models is missing a break statement, which causes the function to incorrectly choose the last state as the starting state for all generated sequences.  This is a *critical* (mathematically very wrong) but trivial to fix bug.  Fix attached. \n\n**Assignee:** amhou\n\n**Author:** William Stein\n\n**Reviewer:** Bill Cauchois\n\n**Merged:** sage-4.7.1.alpha4\n\nIssue created by migration from https://trac.sagemath.org/ticket/11452\n\n",
    "closed_at": "2011-06-15T15:24:37Z",
    "created_at": "2011-06-09T01:11:48Z",
    "labels": [
        "component: statistics",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.1",
    "title": "serious (but trivial to fix) bug in hidden markov model \"generate sequence\" function",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/11452",
    "user": "https://github.com/williamstein"
}
```
The generate sequence function for continuous hidden markov models is missing a break statement, which causes the function to incorrectly choose the last state as the starting state for all generated sequences.  This is a *critical* (mathematically very wrong) but trivial to fix bug.  Fix attached. 

**Assignee:** amhou

**Author:** William Stein

**Reviewer:** Bill Cauchois

**Merged:** sage-4.7.1.alpha4

Issue created by migration from https://trac.sagemath.org/ticket/11452





---

archive/attachments_015605.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11452.patch",
    "asset_url": "tarball://root/attachments/ticket11452/trac_11452.patch",
    "created_at": "2011-06-09T01:22:16Z",
    "issue": "https://github.com/sagemath/sage/issues/11452",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11452/trac_11452.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_118084.json:
```json
{
    "body": "**Attachment:** [trac_11452.patch](https://github.com/sagemath/sage/files/ticket11452/trac_11452.patch)",
    "created_at": "2011-06-09T01:22:16Z",
    "issue": "https://github.com/sagemath/sage/issues/11452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11452#issuecomment-118084",
    "user": "https://github.com/williamstein"
}
```

**Attachment:** [trac_11452.patch](https://github.com/sagemath/sage/files/ticket11452/trac_11452.patch)



---

archive/issue_events_090757.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-06-09T01:22:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11452",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11452#event-90757"
}
```



---

archive/issue_comments_118085.json:
```json
{
    "body": "<a id='comment:2'></a>\nApplies to Sage 4.7, passes all doctests, appears to fix the bug. Positive review!",
    "created_at": "2011-06-10T22:32:52Z",
    "issue": "https://github.com/sagemath/sage/issues/11452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11452#issuecomment-118085",
    "user": "https://trac.sagemath.org/admin/accounts/users/wcauchois"
}
```

<a id='comment:2'></a>
Applies to Sage 4.7, passes all doctests, appears to fix the bug. Positive review!



---

archive/issue_events_090758.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/wcauchois",
    "created_at": "2011-06-10T22:32:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11452",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11452#event-90758"
}
```



---

archive/issue_events_090759.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/wcauchois",
    "created_at": "2011-06-10T22:32:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11452",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11452#event-90759"
}
```



---

archive/issue_comments_118086.json:
```json
{
    "body": "**Author:** William Stein",
    "created_at": "2011-06-14T21:04:56Z",
    "issue": "https://github.com/sagemath/sage/issues/11452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11452#issuecomment-118086",
    "user": "https://github.com/jdemeyer"
}
```

**Author:** William Stein



---

archive/issue_comments_118087.json:
```json
{
    "body": "**Reviewer:** Bill Cauchois",
    "created_at": "2011-06-14T21:04:56Z",
    "issue": "https://github.com/sagemath/sage/issues/11452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11452#issuecomment-118087",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Bill Cauchois



---

archive/issue_events_090760.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-15T15:24:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11452",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11452#event-90760"
}
```



---

archive/issue_events_090761.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-15T15:24:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11452",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11452#event-90761"
}
```



---

archive/issue_comments_118088.json:
```json
{
    "body": "**Merged:** sage-4.7.1.alpha4",
    "created_at": "2011-06-15T15:24:37Z",
    "issue": "https://github.com/sagemath/sage/issues/11452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11452#issuecomment-118088",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-4.7.1.alpha4

# Issue 11631: L-series attached to cusp forms are broken

archive/issues_011459.json:
```json
{
    "body": "\n```\nsage: C = CuspForms(43,2)\nsage: N = C.newforms('a')\nsage: f = N[1]\nsage: L = f.cuspform_lseries()\nBoom!\n```\n\nThere are two issues:\n\n  (1) the code doesn't work at all when the degree of the form is > 1, which is the main interesting case,\n\n  (2) the name \"cuspform_lseries\" is bad, since f is already a cuspform, and we use the name \"lseries\" in all other places (e.g., elliptic curves, abelian varieties), so it is hard to find.  In fact, I didn't even think to look for cuspform_lseries, instead only finding this via lots of grepping and reading source code.\n\nSo to fix this issue, I think (1) the bug should get fixed, and (2) the name should be changed (actually *deprecate* the old name as explained in the developers guide and introduce the name lseries).\n\nSee also #12015.\n\nThe part about deprecating cuspform_lseries() is moved to #16917. \n\nDepends on #16916\n\n**Assignee:** @williamstein\n\n**Branch/Commit:** [1939a9cc8480d42441bb4f71a784ac85e934b80b](https://github.com/sagemath/sagetrac-mirror/commit/1939a9cc8480d42441bb4f71a784ac85e934b80b)\n\n**Reviewer:** Michael Neururer\n\n**Author:** Gonzalo Tornar\u00eda\n\nIssue created by migration from https://trac.sagemath.org/ticket/11631\n\n",
    "closed_at": "2014-09-08T08:48:43Z",
    "created_at": "2011-07-26T17:58:32Z",
    "labels": [
        "component: number theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "title": "L-series attached to cusp forms are broken",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/11631",
    "user": "https://github.com/williamstein"
}
```

```
sage: C = CuspForms(43,2)
sage: N = C.newforms('a')
sage: f = N[1]
sage: L = f.cuspform_lseries()
Boom!
```

There are two issues:

  (1) the code doesn't work at all when the degree of the form is > 1, which is the main interesting case,

  (2) the name "cuspform_lseries" is bad, since f is already a cuspform, and we use the name "lseries" in all other places (e.g., elliptic curves, abelian varieties), so it is hard to find.  In fact, I didn't even think to look for cuspform_lseries, instead only finding this via lots of grepping and reading source code.

So to fix this issue, I think (1) the bug should get fixed, and (2) the name should be changed (actually *deprecate* the old name as explained in the developers guide and introduce the name lseries).

See also #12015.

The part about deprecating cuspform_lseries() is moved to #16917. 

Depends on #16916

**Assignee:** @williamstein

**Branch/Commit:** [1939a9cc8480d42441bb4f71a784ac85e934b80b](https://github.com/sagemath/sagetrac-mirror/commit/1939a9cc8480d42441bb4f71a784ac85e934b80b)

**Reviewer:** Michael Neururer

**Author:** Gonzalo Tornaría

Issue created by migration from https://trac.sagemath.org/ticket/11631





---

archive/issue_comments_121853.json:
```json
{
    "body": "<a id='comment:1'></a>\nI have now written (or am writing) similar (but more powerful) functionality for psage.  It'll end up in this directory of psage:  psage/lseries/\n\n\nhttp://code.google.com/p/purplesage/source/browse/#hg%2Fpsage%2Flseries",
    "created_at": "2011-08-01T18:04:53Z",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11631#issuecomment-121853",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
I have now written (or am writing) similar (but more powerful) functionality for psage.  It'll end up in this directory of psage:  psage/lseries/


http://code.google.com/p/purplesage/source/browse/#hg%2Fpsage%2Flseries



---

archive/issue_comments_121854.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -14,3 +14,5 @@\n   (2) the name \"cuspform_lseries\" is bad, since f is already a cuspform, and we use the name \"lseries\" in all other places (e.g., elliptic curves, abelian varieties), so it is hard to find.  In fact, I didn't even think to look for cuspform_lseries, instead only finding this via lots of grepping and reading source code.\n \n So to fix this issue, I think (1) the bug should get fixed, and (2) the name should be changed (actually *deprecate* the old name as explained in the developers guide and introduce the name lseries).\n+\n+See also #12015.\n``````\n",
    "created_at": "2011-11-12T18:20:54Z",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11631#issuecomment-121854",
    "user": "https://github.com/williamstein"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -14,3 +14,5 @@
   (2) the name "cuspform_lseries" is bad, since f is already a cuspform, and we use the name "lseries" in all other places (e.g., elliptic curves, abelian varieties), so it is hard to find.  In fact, I didn't even think to look for cuspform_lseries, instead only finding this via lots of grepping and reading source code.
 
 So to fix this issue, I think (1) the bug should get fixed, and (2) the name should be changed (actually *deprecate* the old name as explained in the developers guide and introduce the name lseries).
+
+See also #12015.
``````




---

archive/issue_events_093088.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11631#event-93088"
}
```



---

archive/issue_events_093089.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11631#event-93089"
}
```



---

archive/issue_events_093090.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11631#event-93090"
}
```



---

archive/issue_events_093091.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11631#event-93091"
}
```



---

archive/issue_events_093092.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11631#event-93092"
}
```



---

archive/issue_events_093093.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11631#event-93093"
}
```



---

archive/issue_events_093094.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11631#event-93094"
}
```



---

archive/issue_events_093095.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11631#event-93095"
}
```



---

archive/issue_comments_121855.json:
```json
{
    "body": "**Branch:** [u/tornaria/11631](https://github.com/sagemath/sagetrac-mirror/tree/u/tornaria/11631)",
    "created_at": "2014-09-01T20:00:08Z",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11631#issuecomment-121855",
    "user": "https://github.com/tornaria"
}
```

**Branch:** [u/tornaria/11631](https://github.com/sagemath/sagetrac-mirror/tree/u/tornaria/11631)



---

archive/issue_comments_121856.json:
```json
{
    "body": "<a id='comment:7'></a>\nInspired in the patch at #12015, I added a parameter to `cuspform_lseries()` so one can specify which of the conjugates of the newform to use (defaults to 0). The order is arbitrary as it comes from the `embeddings()` method for `self.base_ring()`.\n\nIn the long term #12015 is much more general, but this is a good fix and much easier to review and merge.\n\n---\n**New commits:**\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/74b6a679543f8dafb58eb53df1d3971bd1dd9f07\">74b6a67</a></td><td><code>Trac #16916: fix precision issues in cuspform_lseries()</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ae26b4a0141de1b89738fb2e3cebd9447f21af12\">ae26b4a</a></td><td><code>Trac #16916: avoid precision issues in cuspform_lseries</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/13dbf33c15030e0af2a938e707e46b7d69d90a9b\">13dbf33</a></td><td><code>remove tabs</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1939a9cc8480d42441bb4f71a784ac85e934b80b\">1939a9c</a></td><td><code>Trac #11631: fix cuspform_lseries() for non-rational newforms</code></td></tr></table>\n",
    "created_at": "2014-09-01T20:00:08Z",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11631#issuecomment-121856",
    "user": "https://github.com/tornaria"
}
```

<a id='comment:7'></a>
Inspired in the patch at #12015, I added a parameter to `cuspform_lseries()` so one can specify which of the conjugates of the newform to use (defaults to 0). The order is arbitrary as it comes from the `embeddings()` method for `self.base_ring()`.

In the long term #12015 is much more general, but this is a good fix and much easier to review and merge.

---
**New commits:**
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/74b6a679543f8dafb58eb53df1d3971bd1dd9f07">74b6a67</a></td><td><code>Trac #16916: fix precision issues in cuspform_lseries()</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ae26b4a0141de1b89738fb2e3cebd9447f21af12">ae26b4a</a></td><td><code>Trac #16916: avoid precision issues in cuspform_lseries</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/13dbf33c15030e0af2a938e707e46b7d69d90a9b">13dbf33</a></td><td><code>remove tabs</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1939a9cc8480d42441bb4f71a784ac85e934b80b">1939a9c</a></td><td><code>Trac #11631: fix cuspform_lseries() for non-rational newforms</code></td></tr></table>




---

archive/issue_comments_121857.json:
```json
{
    "body": "**Author:** tornaria",
    "created_at": "2014-09-01T20:00:08Z",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11631#issuecomment-121857",
    "user": "https://github.com/tornaria"
}
```

**Author:** tornaria



---

archive/issue_comments_121858.json:
```json
{
    "body": "**Commit:** [1939a9cc8480d42441bb4f71a784ac85e934b80b](https://github.com/sagemath/sagetrac-mirror/commit/1939a9cc8480d42441bb4f71a784ac85e934b80b)",
    "created_at": "2014-09-01T20:00:08Z",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11631#issuecomment-121858",
    "user": "https://github.com/tornaria"
}
```

**Commit:** [1939a9cc8480d42441bb4f71a784ac85e934b80b](https://github.com/sagemath/sagetrac-mirror/commit/1939a9cc8480d42441bb4f71a784ac85e934b80b)



---

archive/issue_events_093096.json:
```json
{
    "actor": "https://github.com/tornaria",
    "created_at": "2014-09-01T20:00:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11631#event-93096"
}
```



---

archive/issue_comments_121859.json:
```json
{
    "body": "**Dependencies:** #16916",
    "created_at": "2014-09-01T20:00:52Z",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11631#issuecomment-121859",
    "user": "https://github.com/tornaria"
}
```

**Dependencies:** #16916



---

archive/issue_comments_121860.json:
```json
{
    "body": "<a id='comment:9'></a>\nReview: This patch allows one to compute with L-functions of non-rational new forms and fixes the problem in the description.",
    "created_at": "2014-09-01T22:28:12Z",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11631#issuecomment-121860",
    "user": "https://trac.sagemath.org/admin/accounts/users/mneururer"
}
```

<a id='comment:9'></a>
Review: This patch allows one to compute with L-functions of non-rational new forms and fixes the problem in the description.



---

archive/issue_comments_121861.json:
```json
{
    "body": "**Reviewer:** Michael Neururer",
    "created_at": "2014-09-01T22:28:38Z",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11631#issuecomment-121861",
    "user": "https://trac.sagemath.org/admin/accounts/users/mneururer"
}
```

**Reviewer:** Michael Neururer



---

archive/issue_comments_121862.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -16,3 +16,5 @@\n So to fix this issue, I think (1) the bug should get fixed, and (2) the name should be changed (actually *deprecate* the old name as explained in the developers guide and introduce the name lseries).\n \n See also #12015.\n+\n+The part about deprecating cuspform_lseries() is moved to #16917. \n``````\n",
    "created_at": "2014-09-02T09:34:55Z",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11631#issuecomment-121862",
    "user": "https://github.com/tornaria"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -16,3 +16,5 @@
 So to fix this issue, I think (1) the bug should get fixed, and (2) the name should be changed (actually *deprecate* the old name as explained in the developers guide and introduce the name lseries).
 
 See also #12015.
+
+The part about deprecating cuspform_lseries() is moved to #16917. 
``````




---

archive/issue_comments_121863.json:
```json
{
    "body": "<a id='comment:11'></a>\nI moved the part about deprecation to #16917 so the bugfix can be reviewed and merged independently.",
    "created_at": "2014-09-02T09:34:55Z",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11631#issuecomment-121863",
    "user": "https://github.com/tornaria"
}
```

<a id='comment:11'></a>
I moved the part about deprecation to #16917 so the bugfix can be reviewed and merged independently.



---

archive/issue_events_093097.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mneururer",
    "created_at": "2014-09-02T14:34:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11631#event-93097"
}
```



---

archive/issue_events_093098.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mneururer",
    "created_at": "2014-09-02T14:34:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11631#event-93098"
}
```



---

archive/issue_comments_121864.json:
```json
{
    "body": "**Changing author** from \"tornaria\" to \"Gonzalo Tornar\u00eda\".",
    "created_at": "2014-09-07T10:20:13Z",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11631#issuecomment-121864",
    "user": "https://github.com/tornaria"
}
```

**Changing author** from "tornaria" to "Gonzalo Tornaría".



---

archive/issue_events_093099.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-08T08:48:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11631#event-93099"
}
```



---

archive/issue_events_093100.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "bbb459cffd1d950ed7e87e282f8af6c6d810d2df",
    "created_at": "2014-09-08T08:48:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11631#event-93100"
}
```



---

archive/issue_comments_121865.json:
```json
{
    "body": "**Changing branch** from \"[u/tornaria/11631](https://github.com/sagemath/sagetrac-mirror/tree/u/tornaria/11631)\" to \"[1939a9cc8480d42441bb4f71a784ac85e934b80b](https://github.com/sagemath/sagetrac-mirror/commit/1939a9cc8480d42441bb4f71a784ac85e934b80b)\".",
    "created_at": "2014-09-08T08:48:43Z",
    "issue": "https://github.com/sagemath/sage/issues/11631",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11631#issuecomment-121865",
    "user": "https://github.com/vbraun"
}
```

**Changing branch** from "[u/tornaria/11631](https://github.com/sagemath/sagetrac-mirror/tree/u/tornaria/11631)" to "[1939a9cc8480d42441bb4f71a784ac85e934b80b](https://github.com/sagemath/sagetrac-mirror/commit/1939a9cc8480d42441bb4f71a784ac85e934b80b)".

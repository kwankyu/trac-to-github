# Issue 11471: Add i18n capabilities to the Flask notebook

archive/issues_011299.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout",
        "https://github.com/qed777",
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\n\n\nCC:  @dandrake aquino.luizclaudio@gmail.com @robert-marik @jasongrout\n\nComponent: **notebook**\n\nReviewer: **Keshav kini, Luiz Aquino, Dan Drake, Tim Dumol, Mike Hansen**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11471_\n\n",
    "closed_at": "2012-09-05T07:16:34Z",
    "created_at": "2011-06-14T00:02:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add i18n capabilities to the Flask notebook",
    "type": "issue",
    "updated_at": "2012-09-05T07:16:34Z",
    "url": "https://github.com/sagemath/sage/issues/11471",
    "user": "https://github.com/mwhansen"
}
```
<div id="comment:0"></div>



CC:  @dandrake aquino.luizclaudio@gmail.com @robert-marik @jasongrout

Component: **notebook**

Reviewer: **Keshav kini, Luiz Aquino, Dan Drake, Tim Dumol, Mike Hansen**

_Issue created by migration from https://trac.sagemath.org/ticket/11471_





---

archive/issue_events_151340.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2011-06-14T00:02:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-151340"
}
```



---

archive/issue_events_151341.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2011-06-14T00:02:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-151341"
}
```



---

archive/issue_comments_116282.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThis is a port of the code at #9428 to the Flask notebook.  The code should be a bit cleaner (not having to pass around username everywhere).  This should apply on top of http://code.google.com/r/rkirov-flask/ (changeset 503:79b88d03adf0).",
    "created_at": "2011-06-15T22:41:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-116282",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

This is a port of the code at #9428 to the Flask notebook.  The code should be a bit cleaner (not having to pass around username everywhere).  This should apply on top of http://code.google.com/r/rkirov-flask/ (changeset 503:79b88d03adf0).



---

archive/issue_comments_116283.json:
```json
{
    "body": "Author: **Luiz Aquino, Dan Drake, Tim Dumol, Mike Hansen**",
    "created_at": "2011-06-15T22:41:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-116283",
    "user": "https://github.com/mwhansen"
}
```

Author: **Luiz Aquino, Dan Drake, Tim Dumol, Mike Hansen**



---

archive/issue_events_151342.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2011-06-15T22:41:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-151342"
}
```



---

archive/issue_events_151343.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2011-06-15T22:41:48Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-151343"
}
```



---

archive/issue_events_151344.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2011-06-15T22:41:48Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-151344"
}
```



---

archive/issue_events_151345.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-06-15T22:41:48Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-151345"
}
```



---

archive/issue_events_151346.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2011-06-15T22:41:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-151346"
}
```



---

archive/issue_comments_116284.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAttachment: **[trac_11471.patch.gz](https://github.com/sagemath/sage/files/ticket11471/trac_11471.patch.gz)**\n\nhmmm, i am getting\n\"ImportError: No module named i18n\"\nafter installing babel and flask-babel.\n\nIs this a python package or a missing file?",
    "created_at": "2011-06-19T03:01:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-116284",
    "user": "https://github.com/rkirov"
}
```

<div id="comment:3" align="right">comment:3</div>

Attachment: **[trac_11471.patch.gz](https://github.com/sagemath/sage/files/ticket11471/trac_11471.patch.gz)**

hmmm, i am getting
"ImportError: No module named i18n"
after installing babel and flask-babel.

Is this a python package or a missing file?



---

archive/issue_comments_116285.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nIt's a import that shouldn't be there.",
    "created_at": "2011-06-19T03:39:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-116285",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:4" align="right">comment:4</div>

It's a import that shouldn't be there.



---

archive/issue_comments_116286.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nDo you know offhand if this was merged into the sage notebook?",
    "created_at": "2011-12-17T21:35:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-116286",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:5" align="right">comment:5</div>

Do you know offhand if this was merged into the sage notebook?



---

archive/issue_events_151347.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-06-23T09:08:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-151347"
}
```



---

archive/issue_comments_116287.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nYes, in [this commit](https://github.com/sagemath/sagenb/commit/a11b6f6b9b2049dd22a25b7c4f8e246606075331/).",
    "created_at": "2012-06-23T09:08:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-116287",
    "user": "https://github.com/kini"
}
```

<div id="comment:6" align="right">comment:6</div>

Yes, in [this commit](https://github.com/sagemath/sagenb/commit/a11b6f6b9b2049dd22a25b7c4f8e246606075331/).



---

archive/issue_comments_116288.json:
```json
{
    "body": "Reviewer: **Keshav kini**",
    "created_at": "2012-06-23T09:08:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-116288",
    "user": "https://github.com/kini"
}
```

Reviewer: **Keshav kini**



---

archive/issue_events_151348.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-06-23T09:08:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-151348"
}
```



---

archive/issue_events_151349.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-06-23T09:08:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-151349"
}
```



---

archive/issue_events_151350.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-06-23T09:08:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-151350"
}
```



---

archive/issue_events_151351.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-09-05T07:16:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-151351"
}
```



---

archive/issue_events_151352.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-09-05T07:16:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-151352"
}
```



---

archive/issue_comments_116289.json:
```json
{
    "body": "Changed reviewer from **Keshav kini** to **Keshav kini, Luiz Aquino, Dan Drake, Tim Dumol, Mike Hansen**",
    "created_at": "2012-09-05T07:16:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-116289",
    "user": "https://github.com/jdemeyer"
}
```

Changed reviewer from **Keshav kini** to **Keshav kini, Luiz Aquino, Dan Drake, Tim Dumol, Mike Hansen**



---

archive/issue_comments_116290.json:
```json
{
    "body": "Changed author from **Luiz Aquino, Dan Drake, Tim Dumol, Mike Hansen** to none",
    "created_at": "2012-09-05T07:16:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-116290",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Luiz Aquino, Dan Drake, Tim Dumol, Mike Hansen** to none

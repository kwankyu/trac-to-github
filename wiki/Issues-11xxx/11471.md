# Issue 11471: Add i18n capabilities to the Flask notebook

archive/issues_011299.json:
```json
{
    "body": "\n\n**Assignee:** @jasongrout, @qed777, @williamstein\n\n**CC:**  @dandrake aquino.luizclaudio@gmail.com @robert-marik @jasongrout\n\n**Reviewer:** Keshav kini, Luiz Aquino, Dan Drake, Tim Dumol, Mike Hansen\n\nIssue created by migration from https://trac.sagemath.org/ticket/11471\n\n",
    "closed_at": "2012-09-05T07:16:34Z",
    "created_at": "2011-06-14T00:02:04Z",
    "labels": [
        "component: notebook",
        "enhancement",
        "invalid"
    ],
    "title": "Add i18n capabilities to the Flask notebook",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/11471",
    "user": "https://github.com/mwhansen"
}
```


**Assignee:** @jasongrout, @qed777, @williamstein

**CC:**  @dandrake aquino.luizclaudio@gmail.com @robert-marik @jasongrout

**Reviewer:** Keshav kini, Luiz Aquino, Dan Drake, Tim Dumol, Mike Hansen

Issue created by migration from https://trac.sagemath.org/ticket/11471





---

archive/issue_comments_118442.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis is a port of the code at #9428 to the Flask notebook.  The code should be a bit cleaner (not having to pass around username everywhere).  This should apply on top of http://code.google.com/r/rkirov-flask/ (changeset 503:79b88d03adf0).",
    "created_at": "2011-06-15T22:41:31Z",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-118442",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
This is a port of the code at #9428 to the Flask notebook.  The code should be a bit cleaner (not having to pass around username everywhere).  This should apply on top of http://code.google.com/r/rkirov-flask/ (changeset 503:79b88d03adf0).



---

archive/issue_comments_118443.json:
```json
{
    "body": "**Author:** Luiz Aquino, Dan Drake, Tim Dumol, Mike Hansen",
    "created_at": "2011-06-15T22:41:31Z",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-118443",
    "user": "https://github.com/mwhansen"
}
```

**Author:** Luiz Aquino, Dan Drake, Tim Dumol, Mike Hansen



---

archive/issue_events_090978.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2011-06-15T22:41:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "label": "enhancement",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-90978"
}
```



---

archive/issue_comments_118444.json:
```json
{
    "body": "**Assignee:** @jasongrout, @qed777, @williamstein",
    "created_at": "2011-06-15T22:41:48Z",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-118444",
    "user": "https://github.com/mwhansen"
}
```

**Assignee:** @jasongrout, @qed777, @williamstein



---

archive/issue_events_090979.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2011-06-15T22:41:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "label": "component: notebook",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-90979"
}
```



---

archive/attachments_015656.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11471.patch",
    "asset_url": "tarball://root/attachments/ticket11471/trac_11471.patch",
    "created_at": "2011-06-19T03:01:32Z",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11471/trac_11471.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/rkirov"
}
```



---

archive/issue_comments_118445.json:
```json
{
    "body": "<a id='comment:3'></a>\n**Attachment:** [trac_11471.patch](https://github.com/sagemath/sage/files/ticket11471/trac_11471.patch)\n\nhmmm, i am getting\n\"ImportError: No module named i18n\"\nafter installing babel and flask-babel.\n\nIs this a python package or a missing file?",
    "created_at": "2011-06-19T03:01:32Z",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-118445",
    "user": "https://trac.sagemath.org/admin/accounts/users/rkirov"
}
```

<a id='comment:3'></a>
**Attachment:** [trac_11471.patch](https://github.com/sagemath/sage/files/ticket11471/trac_11471.patch)

hmmm, i am getting
"ImportError: No module named i18n"
after installing babel and flask-babel.

Is this a python package or a missing file?



---

archive/issue_comments_118446.json:
```json
{
    "body": "<a id='comment:4'></a>\nIt's a import that shouldn't be there.",
    "created_at": "2011-06-19T03:39:44Z",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-118446",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:4'></a>
It's a import that shouldn't be there.



---

archive/issue_comments_118447.json:
```json
{
    "body": "<a id='comment:5'></a>\nDo you know offhand if this was merged into the sage notebook?",
    "created_at": "2011-12-17T21:35:03Z",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-118447",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:5'></a>
Do you know offhand if this was merged into the sage notebook?



---

archive/issue_events_090980.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-06-23T09:08:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-90980"
}
```



---

archive/issue_comments_118448.json:
```json
{
    "body": "<a id='comment:6'></a>\nYes, in [this commit](https://github.com/sagemath/sagenb/commit/a11b6f6b9b2049dd22a25b7c4f8e246606075331/).",
    "created_at": "2012-06-23T09:08:12Z",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-118448",
    "user": "https://github.com/kini"
}
```

<a id='comment:6'></a>
Yes, in [this commit](https://github.com/sagemath/sagenb/commit/a11b6f6b9b2049dd22a25b7c4f8e246606075331/).



---

archive/issue_comments_118449.json:
```json
{
    "body": "**Reviewer:** Keshav kini",
    "created_at": "2012-06-23T09:08:25Z",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-118449",
    "user": "https://github.com/kini"
}
```

**Reviewer:** Keshav kini



---

archive/issue_events_090981.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-06-23T09:08:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-90981"
}
```



---

archive/issue_events_090982.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-06-23T09:08:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-90982"
}
```



---

archive/issue_events_090983.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-06-23T09:08:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "milestone": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-90983"
}
```



---

archive/issue_events_090984.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-09-05T07:16:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-90984"
}
```



---

archive/issue_events_090985.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-09-05T07:16:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11471#event-90985"
}
```



---

archive/issue_comments_118450.json:
```json
{
    "body": "**Changing reviewer** from \"Keshav kini\" to \"Keshav kini, Luiz Aquino, Dan Drake, Tim Dumol, Mike Hansen\".",
    "created_at": "2012-09-05T07:16:34Z",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-118450",
    "user": "https://github.com/jdemeyer"
}
```

**Changing reviewer** from "Keshav kini" to "Keshav kini, Luiz Aquino, Dan Drake, Tim Dumol, Mike Hansen".



---

archive/issue_comments_118451.json:
```json
{
    "body": "**Changing author** from \"Luiz Aquino, Dan Drake, Tim Dumol, Mike Hansen\" to \"\".",
    "created_at": "2012-09-05T07:16:34Z",
    "issue": "https://github.com/sagemath/sage/issues/11471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11471#issuecomment-118451",
    "user": "https://github.com/jdemeyer"
}
```

**Changing author** from "Luiz Aquino, Dan Drake, Tim Dumol, Mike Hansen" to "".

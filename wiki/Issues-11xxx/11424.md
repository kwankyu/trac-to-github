# Issue 11424: Build Integer from Unicode

archive/issues_011252.json:
```json
{
    "body": "Currently, Sage fails to build Integer from Unicode\n\n```\nsage: Integer(u'1')\nTypeError Traceback (most recent call last)\n...\nTypeError: unable to coerce <type 'unicode'> to an integer\n```\n\nThe two lines patch attached to this ticket, allow such feature\n\n```\nsage: Integer(u'1')\n1\nsage: Integer(u'0X13')\n19\n```\n\n**Assignee:** @videlec\n\n**Keywords:** Integer, unicode\n\n**Author:** Vincent Delecroix\n\n**Reviewer:** Mariah Lenox\n\n**Merged:** sage-4.7.2.alpha1\n\nIssue created by migration from https://trac.sagemath.org/ticket/11424\n\n",
    "closed_at": "2011-07-28T10:25:08Z",
    "created_at": "2011-06-03T12:50:20Z",
    "labels": [
        "component: basic arithmetic",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.2",
    "title": "Build Integer from Unicode",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/11424",
    "user": "https://github.com/videlec"
}
```
Currently, Sage fails to build Integer from Unicode

```
sage: Integer(u'1')
TypeError Traceback (most recent call last)
...
TypeError: unable to coerce <type 'unicode'> to an integer
```

The two lines patch attached to this ticket, allow such feature

```
sage: Integer(u'1')
1
sage: Integer(u'0X13')
19
```

**Assignee:** @videlec

**Keywords:** Integer, unicode

**Author:** Vincent Delecroix

**Reviewer:** Mariah Lenox

**Merged:** sage-4.7.2.alpha1

Issue created by migration from https://trac.sagemath.org/ticket/11424





---

archive/issue_events_090468.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2011-06-03T12:57:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11424#event-90468"
}
```



---

archive/issue_comments_117761.json:
```json
{
    "body": "<a id='comment:2'></a>\nPatch fixes the reported problem.  Did 'make testlong' and all tests passed.  Positive review!",
    "created_at": "2011-06-10T18:05:30Z",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11424#issuecomment-117761",
    "user": "https://trac.sagemath.org/admin/accounts/users/mariah"
}
```

<a id='comment:2'></a>
Patch fixes the reported problem.  Did 'make testlong' and all tests passed.  Positive review!



---

archive/issue_events_090469.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mariah",
    "created_at": "2011-06-10T18:05:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11424#event-90469"
}
```



---

archive/issue_events_090470.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mariah",
    "created_at": "2011-06-10T18:05:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11424#event-90470"
}
```



---

archive/issue_comments_117762.json:
```json
{
    "body": "**Reviewer:** Mariah Lenox",
    "created_at": "2011-06-10T18:05:30Z",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11424#issuecomment-117762",
    "user": "https://trac.sagemath.org/admin/accounts/users/mariah"
}
```

**Reviewer:** Mariah Lenox



---

archive/issue_comments_117763.json:
```json
{
    "body": "<a id='comment:3'></a>\n1. The commit message of the patch should be changed.\n   2. Could you give a less trivial example in the doctest?  At least something with more than one digit and something hexadecimal (`Integer(u'0x2A')`) or so...",
    "created_at": "2011-06-14T20:43:03Z",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11424#issuecomment-117763",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'></a>
1. The commit message of the patch should be changed.
   2. Could you give a less trivial example in the doctest?  At least something with more than one digit and something hexadecimal (`Integer(u'0x2A')`) or so...



---

archive/issue_events_090471.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-14T20:43:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11424#event-90471"
}
```



---

archive/issue_events_090472.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-14T20:43:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11424#event-90472"
}
```



---

archive/issue_events_090473.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2011-06-19T06:52:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11424#event-90473"
}
```



---

archive/issue_events_090474.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2011-06-19T06:52:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11424#event-90474"
}
```



---

archive/attachments_015577.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11424-unicode_to_integer.patch",
    "asset_url": "tarball://root/attachments/ticket11424/trac_11424-unicode_to_integer.patch",
    "created_at": "2011-06-19T06:52:11Z",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11424/trac_11424-unicode_to_integer.patch",
    "user": "https://github.com/videlec"
}
```



---

archive/issue_comments_117764.json:
```json
{
    "body": "<a id='comment:4'></a>\n**Attachment:** [trac_11424-unicode_to_integer.patch](https://github.com/sagemath/sage/files/ticket11424/trac_11424-unicode_to_integer.patch)\n\nReplying to [@jdemeyer](#comment%3A3):\n\nThanks for comment.\n\n> 1. The commit message of the patch should be changed.\n\nI add the number of the ticket and a short description.\n\n> 2. Could you give a less trivial example in the doctest?  At least something with more than one digit and something hexadecimal (`Integer(u'0x2A')`) or so...\n\nDone.",
    "created_at": "2011-06-19T06:52:11Z",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11424#issuecomment-117764",
    "user": "https://github.com/videlec"
}
```

<a id='comment:4'></a>
**Attachment:** [trac_11424-unicode_to_integer.patch](https://github.com/sagemath/sage/files/ticket11424/trac_11424-unicode_to_integer.patch)

Replying to [@jdemeyer](#comment%3A3):

Thanks for comment.

> 1. The commit message of the patch should be changed.

I add the number of the ticket and a short description.

> 2. Could you give a less trivial example in the doctest?  At least something with more than one digit and something hexadecimal (`Integer(u'0x2A')`) or so...

Done.



---

archive/issue_comments_117765.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -12,4 +12,6 @@\n ```\n sage: Integer(u'1')\n 1\n+sage: Integer(u'0X13')\n+19\n ```\n``````\n",
    "created_at": "2011-06-19T06:52:11Z",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11424#issuecomment-117765",
    "user": "https://github.com/videlec"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -12,4 +12,6 @@
 ```
 sage: Integer(u'1')
 1
+sage: Integer(u'0X13')
+19
 ```
``````




---

archive/issue_events_090475.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mariah",
    "created_at": "2011-06-21T18:21:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11424#event-90475"
}
```



---

archive/issue_events_090476.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mariah",
    "created_at": "2011-06-21T18:21:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11424#event-90476"
}
```



---

archive/issue_comments_117766.json:
```json
{
    "body": "<a id='comment:5'></a>\nApplied patch, did 'sage -b' and 'make testlong'.  All tests passed.  Positive review!",
    "created_at": "2011-06-21T18:21:51Z",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11424#issuecomment-117766",
    "user": "https://trac.sagemath.org/admin/accounts/users/mariah"
}
```

<a id='comment:5'></a>
Applied patch, did 'sage -b' and 'make testlong'.  All tests passed.  Positive review!



---

archive/issue_events_090477.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-22T09:55:57Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "milestone": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11424#event-90477"
}
```



---

archive/issue_events_090478.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-22T09:55:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "milestone": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11424#event-90478"
}
```



---

archive/issue_events_090479.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-28T10:25:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11424#event-90479"
}
```



---

archive/issue_events_090480.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-28T10:25:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11424#event-90480"
}
```



---

archive/issue_comments_117767.json:
```json
{
    "body": "**Merged:** sage-4.7.2.alpha1",
    "created_at": "2011-07-28T10:25:08Z",
    "issue": "https://github.com/sagemath/sage/issues/11424",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11424#issuecomment-117767",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-4.7.2.alpha1

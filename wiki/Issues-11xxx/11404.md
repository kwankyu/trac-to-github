# Issue 11404: Allow sage -upgrade / -update from local directory.

archive/issues_011232.json:
```json
{
    "body": "For testing purposes it would be very convenient if one could update/upgrade from a local directory. This is currently not possible because urllib has no concept of a directory listing to get the list of updated spkgs. The attached patch fixes this.\n\nApply trac_11404_upgrade_from_directory.patch to the sage_scripts repository.\n\n**Assignee:** GeorgSWeber\n\n**CC:**  @jdemeyer @novoselt\n\n**Reviewer:** Volker Braun, John Palmieri, Jeroen Demeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/11404\n\n",
    "closed_at": "2014-02-04T11:23:49Z",
    "created_at": "2011-05-30T17:08:21Z",
    "labels": [
        "component: build",
        "bug",
        "wontfix"
    ],
    "title": "Allow sage -upgrade / -update from local directory.",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/11404",
    "user": "https://github.com/vbraun"
}
```
For testing purposes it would be very convenient if one could update/upgrade from a local directory. This is currently not possible because urllib has no concept of a directory listing to get the list of updated spkgs. The attached patch fixes this.

Apply trac_11404_upgrade_from_directory.patch to the sage_scripts repository.

**Assignee:** GeorgSWeber

**CC:**  @jdemeyer @novoselt

**Reviewer:** Volker Braun, John Palmieri, Jeroen Demeyer

Issue created by migration from https://trac.sagemath.org/ticket/11404





---

archive/attachments_015542.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11404_upgrade_from_directory.patch",
    "asset_url": "tarball://root/attachments/ticket11404/trac_11404_upgrade_from_directory.patch",
    "created_at": "2011-05-30T17:10:07Z",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11404/trac_11404_upgrade_from_directory.patch",
    "user": "https://github.com/vbraun"
}
```



---

archive/issue_comments_117455.json:
```json
{
    "body": "**Attachment:** [trac_11404_upgrade_from_directory.patch](https://github.com/sagemath/sage/files/ticket11404/trac_11404_upgrade_from_directory.patch)\n\nInitial patch",
    "created_at": "2011-05-30T17:10:07Z",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11404#issuecomment-117455",
    "user": "https://github.com/vbraun"
}
```

**Attachment:** [trac_11404_upgrade_from_directory.patch](https://github.com/sagemath/sage/files/ticket11404/trac_11404_upgrade_from_directory.patch)

Initial patch



---

archive/issue_comments_117456.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n For testing purposes it would be very convenient if one could update/upgrade from a local directory. This is currently not possible because urllib has no concept of a directory listing to get the list of updated spkgs. The attached patch fixes this.\n \n-\n+Apply trac_11404_upgrade_from_directory.patch to the sage_scripts repository.\n``````\n",
    "created_at": "2011-05-30T17:11:25Z",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11404#issuecomment-117456",
    "user": "https://github.com/vbraun"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 For testing purposes it would be very convenient if one could update/upgrade from a local directory. This is currently not possible because urllib has no concept of a directory listing to get the list of updated spkgs. The attached patch fixes this.
 
-
+Apply trac_11404_upgrade_from_directory.patch to the sage_scripts repository.
``````




---

archive/issue_events_090248.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2011-05-30T17:11:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11404#event-90248"
}
```



---

archive/issue_comments_117457.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis seems to work with either `sage -upgrade /full/path/to/sage` or `sage -upgrade file:///full/path...`.  It doesn't work with relative paths.  Should I expect it to?\n\nI also think this behavior should be documented somewhere, like in the help messages in sage-sage and at the comments at the top of sage-update.\n\nBy the way, there is a typo in the docstring for `spkg_list_local_directory`: change `spgk` to `spkg`.\n\nFinally, I agree that this would be a very useful addition to Sage.  I've wanted Sage to have this capability myself.",
    "created_at": "2011-07-17T02:36:45Z",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11404#issuecomment-117457",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'></a>
This seems to work with either `sage -upgrade /full/path/to/sage` or `sage -upgrade file:///full/path...`.  It doesn't work with relative paths.  Should I expect it to?

I also think this behavior should be documented somewhere, like in the help messages in sage-sage and at the comments at the top of sage-update.

By the way, there is a typo in the docstring for `spkg_list_local_directory`: change `spgk` to `spkg`.

Finally, I agree that this would be a very useful addition to Sage.  I've wanted Sage to have this capability myself.



---

archive/issue_comments_117458.json:
```json
{
    "body": "**Reviewer:** John Palmieri",
    "created_at": "2011-07-17T02:36:45Z",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11404#issuecomment-117458",
    "user": "https://github.com/jhpalmieri"
}
```

**Reviewer:** John Palmieri



---

archive/issue_events_090249.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2011-07-17T02:36:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11404#event-90249"
}
```



---

archive/issue_events_090250.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2011-07-17T02:36:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11404#event-90250"
}
```



---

archive/issue_events_090251.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11404#event-90251"
}
```



---

archive/issue_events_090252.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11404#event-90252"
}
```



---

archive/issue_events_090253.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11404#event-90253"
}
```



---

archive/issue_events_090254.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11404#event-90254"
}
```



---

archive/issue_events_090255.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-02-04T07:59:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11404#event-90255"
}
```



---

archive/issue_comments_117459.json:
```json
{
    "body": "**Changing author** from \"Volker Braun\" to \"\".",
    "created_at": "2014-02-04T07:59:32Z",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11404#issuecomment-117459",
    "user": "https://github.com/jdemeyer"
}
```

**Changing author** from "Volker Braun" to "".



---

archive/issue_comments_117460.json:
```json
{
    "body": "**Changing reviewer** from \"John Palmieri\" to \"Volker Braun, John Palmieri, Jeroen Demeyer\".",
    "created_at": "2014-02-04T07:59:32Z",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11404#issuecomment-117460",
    "user": "https://github.com/jdemeyer"
}
```

**Changing reviewer** from "John Palmieri" to "Volker Braun, John Palmieri, Jeroen Demeyer".



---

archive/issue_comments_117461.json:
```json
{
    "body": "<a id='comment:6'></a>\nObsolete due to git (and in fact, upgrading from a local directory is possible, just put whatever stuff you need in `upstream/`).",
    "created_at": "2014-02-04T07:59:32Z",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11404#issuecomment-117461",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'></a>
Obsolete due to git (and in fact, upgrading from a local directory is possible, just put whatever stuff you need in `upstream/`).



---

archive/issue_events_090256.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-02-04T07:59:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11404#event-90256"
}
```



---

archive/issue_events_090257.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-02-04T07:59:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11404#event-90257"
}
```



---

archive/issue_events_090258.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-04T11:23:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "label": "wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11404#event-90258"
}
```



---

archive/issue_events_090259.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-04T11:23:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11404#event-90259"
}
```



---

archive/issue_events_090260.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-04T11:23:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11404",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11404#event-90260"
}
```

# Issue 23231: Public access to underlying polynomial in q-adic elements

archive/issues_022994.json:
```json
{
    "body": "In number fields, there is a method for accessing the polynomial underlying an element:\n\n```\nsage: S.<x> = QQ[]\nsage: K.<a> = NumberField(x^3 -x + 27)\nsage: a.polynomial()\nx\n```\nThere should be a similar method for `p`-adic extensions.\n\n**Keywords:** sd87\n\n**Reviewer:** David Roe\n\nIssue created by migration from https://trac.sagemath.org/ticket/23231\n\n",
    "closed_at": "2017-12-12T08:23:33Z",
    "created_at": "2017-06-13T19:18:39Z",
    "labels": [
        "component: padics",
        "minor",
        "enhancement",
        "wontfix"
    ],
    "title": "Public access to underlying polynomial in q-adic elements",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/23231",
    "user": "https://github.com/roed314"
}
```
In number fields, there is a method for accessing the polynomial underlying an element:

```
sage: S.<x> = QQ[]
sage: K.<a> = NumberField(x^3 -x + 27)
sage: a.polynomial()
x
```
There should be a similar method for `p`-adic extensions.

**Keywords:** sd87

**Reviewer:** David Roe

Issue created by migration from https://trac.sagemath.org/ticket/23231





---

archive/issue_events_208863.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-07-23T04:13:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/23231",
    "milestone": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23231#event-208863"
}
```



---

archive/issue_comments_351387.json:
```json
{
    "body": "**Reviewer:** David Roe",
    "created_at": "2017-07-23T04:13:17Z",
    "issue": "https://github.com/sagemath/sage/issues/23231",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23231#issuecomment-351387",
    "user": "https://github.com/roed314"
}
```

**Reviewer:** David Roe



---

archive/issue_comments_351388.json:
```json
{
    "body": "<a id='comment:1'></a>\nSee #14825",
    "created_at": "2017-07-23T04:13:17Z",
    "issue": "https://github.com/sagemath/sage/issues/23231",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/23231#issuecomment-351388",
    "user": "https://github.com/roed314"
}
```

<a id='comment:1'></a>
See #14825



---

archive/issue_events_208864.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-07-23T04:13:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23231",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23231#event-208864"
}
```



---

archive/issue_events_208865.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-07-23T04:13:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23231",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23231#event-208865"
}
```



---

archive/issue_events_208866.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-07-23T04:13:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23231",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23231#event-208866"
}
```



---

archive/issue_events_208867.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-12T08:23:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/23231",
    "label": "wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23231#event-208867"
}
```



---

archive/issue_events_208868.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-12T08:23:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/23231",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23231#event-208868"
}
```



---

archive/issue_events_208869.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-12T08:23:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/23231",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/23231#event-208869"
}
```

# Issue 15467: Partitions return wrong result for obvious reasons

archive/issues_015230.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nAs reported there : https://groups.google.com/d/topic/sage-combinat-devel/vyMs7PQ8KbA/discussion\n\nThis patch makes the constructor of Partitions scream instead of returning garbage.\n\nCC:  @sagetrac-sage-combinat\n\nComponent: **combinatorics**\n\nAuthor: **Nathann Cohen**\n\nBranch/Commit: **[u/ncohen/15467](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15467) @ [`5993eac`](https://github.com/sagemath/sagetrac-mirror/commit/5993eac06a1654326b0f992b6f4a73b67c362f6f)**\n\nReviewer: **Andrew Mathas**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15467_\n\n",
    "closed_at": "2013-12-17T21:51:26Z",
    "created_at": "2013-11-29T16:16:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Partitions return wrong result for obvious reasons",
    "type": "issue",
    "updated_at": "2014-10-01T17:02:19Z",
    "url": "https://github.com/sagemath/sage/issues/15467",
    "user": "https://github.com/nathanncohen"
}
```
<div id="comment:0"></div>

As reported there : https://groups.google.com/d/topic/sage-combinat-devel/vyMs7PQ8KbA/discussion

This patch makes the constructor of Partitions scream instead of returning garbage.

CC:  @sagetrac-sage-combinat

Component: **combinatorics**

Author: **Nathann Cohen**

Branch/Commit: **[u/ncohen/15467](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15467) @ [`5993eac`](https://github.com/sagemath/sagetrac-mirror/commit/5993eac06a1654326b0f992b6f4a73b67c362f6f)**

Reviewer: **Andrew Mathas**

_Issue created by migration from https://trac.sagemath.org/ticket/15467_





---

archive/issue_events_218339.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-11-29T16:16:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "milestone_number": null,
    "milestone_title": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15467#event-218339"
}
```



---

archive/issue_events_218340.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-11-29T16:16:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15467#event-218340"
}
```



---

archive/issue_events_218341.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-11-29T16:16:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15467#event-218341"
}
```



---

archive/issue_events_218342.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-11-29T16:16:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15467#event-218342"
}
```



---

archive/issue_comments_192736.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n+As reported there : https://groups.google.com/d/topic/sage-combinat-devel/vyMs7PQ8KbA/discussion\n \n+This patch makes the constructor of Partitions scream instead of returning garbage.\n``````\n",
    "created_at": "2013-11-29T17:35:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15467#issuecomment-192736",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
+As reported there : https://groups.google.com/d/topic/sage-combinat-devel/vyMs7PQ8KbA/discussion
 
+This patch makes the constructor of Partitions scream instead of returning garbage.
``````




---

archive/issue_events_218343.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-11-29T17:35:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15467#event-218343"
}
```



---

archive/issue_comments_192737.json:
```json
{
    "body": "Branch: **[u/ncohen/15467](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15467)**",
    "created_at": "2013-11-29T17:35:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15467#issuecomment-192737",
    "user": "https://github.com/nathanncohen"
}
```

Branch: **[u/ncohen/15467](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15467)**



---

archive/issue_comments_192738.json:
```json
{
    "body": "Commit: **[`5993eac`](https://github.com/sagemath/sagetrac-mirror/commit/5993eac06a1654326b0f992b6f4a73b67c362f6f)**",
    "created_at": "2013-11-29T17:49:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15467#issuecomment-192738",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[`5993eac`](https://github.com/sagemath/sagetrac-mirror/commit/5993eac06a1654326b0f992b6f4a73b67c362f6f)**



---

archive/issue_comments_192739.json:
```json
{
    "body": "<div id=\"comment:2\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><code>[5993eac](https://github.com/sagemath/sagetrac-mirror/commit/5993eac)</code></td><td><code>trac #15467: Partitions should check its input \"a bit\" more carefully</code></td></tr></table>\n",
    "created_at": "2013-11-29T17:49:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15467#issuecomment-192739",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><code>[5993eac](https://github.com/sagemath/sagetrac-mirror/commit/5993eac)</code></td><td><code>trac #15467: Partitions should check its input "a bit" more carefully</code></td></tr></table>




---

archive/issue_comments_192740.json:
```json
{
    "body": "Reviewer: **Andrew Mathas**",
    "created_at": "2013-12-05T14:53:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15467#issuecomment-192740",
    "user": "https://github.com/AndrewAtLarge"
}
```

Reviewer: **Andrew Mathas**



---

archive/issue_events_218344.json:
```json
{
    "actor": "https://github.com/AndrewAtLarge",
    "created_at": "2013-12-05T14:53:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15467#event-218344"
}
```



---

archive/issue_events_218345.json:
```json
{
    "actor": "https://github.com/AndrewAtLarge",
    "created_at": "2013-12-05T14:53:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15467#event-218345"
}
```



---

archive/issue_events_218346.json:
```json
{
    "actor": "https://github.com/AndrewAtLarge",
    "created_at": "2013-12-05T14:53:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "milestone_number": null,
    "milestone_title": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15467#event-218346"
}
```



---

archive/issue_events_218347.json:
```json
{
    "actor": "https://github.com/AndrewAtLarge",
    "created_at": "2013-12-05T14:53:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "milestone_number": null,
    "milestone_title": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15467#event-218347"
}
```



---

archive/issue_comments_192741.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nDoesn't break anything and fixes some bugs, so positive review.\n\nI've bumped the version number to 6 since the commits come from git.",
    "created_at": "2013-12-05T14:53:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15467#issuecomment-192741",
    "user": "https://github.com/AndrewAtLarge"
}
```

<div id="comment:3" align="right">comment:3</div>

Doesn't break anything and fixes some bugs, so positive review.

I've bumped the version number to 6 since the commits come from git.



---

archive/issue_comments_192742.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nWow, cool ! Thank you very much for reviewing that `:-)`\n\nNathann",
    "created_at": "2013-12-05T14:54:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15467#issuecomment-192742",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">comment:4</div>

Wow, cool ! Thank you very much for reviewing that `:-)`

Nathann



---

archive/issue_events_218348.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "milestone_number": null,
    "milestone_title": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15467#event-218348"
}
```



---

archive/issue_events_218349.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15467#event-218349"
}
```



---

archive/issue_events_218350.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-17T21:51:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15467#event-218350"
}
```



---

archive/issue_events_218351.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "3125f9f362276e4bb61b69bb559961b865c09587",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2013-12-17T21:51:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15467#event-218351"
}
```



---

archive/issue_comments_192743.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nJust out of curiosity, would it be okay for me to add an enhancement ticket asking that we could eventually add the ability to combine (say) `parts_in` with other options?  That would have been useful for me in a class tomorrow, but maybe there is no need.  Or maybe such a ticket already exists!",
    "created_at": "2014-10-01T15:18:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15467#issuecomment-192743",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:7" align="right">comment:7</div>

Just out of curiosity, would it be okay for me to add an enhancement ticket asking that we could eventually add the ability to combine (say) `parts_in` with other options?  That would have been useful for me in a class tomorrow, but maybe there is no need.  Or maybe such a ticket already exists!



---

archive/issue_comments_192744.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\n> Just out of curiosity, would it be okay for me to add an enhancement ticket asking that we could eventually add the ability to combine (say) `parts_in` with other options?  That would have been useful for me in a class tomorrow, but maybe there is no need.  Or maybe such a ticket already exists!\n\nI do not know if such a ticket exists. I do not believe that it takes more than a couple of hours to write this feature (in Python), however.\n\nNathann",
    "created_at": "2014-10-01T16:00:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15467#issuecomment-192744",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:8" align="right">comment:8</div>

> Just out of curiosity, would it be okay for me to add an enhancement ticket asking that we could eventually add the ability to combine (say) `parts_in` with other options?  That would have been useful for me in a class tomorrow, but maybe there is no need.  Or maybe such a ticket already exists!

I do not know if such a ticket exists. I do not believe that it takes more than a couple of hours to write this feature (in Python), however.

Nathann



---

archive/issue_comments_192745.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nOk, I've opened #17081.",
    "created_at": "2014-10-01T17:02:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15467",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15467#issuecomment-192745",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:9" align="right">comment:9</div>

Ok, I've opened #17081.

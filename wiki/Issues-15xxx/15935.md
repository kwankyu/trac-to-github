# Issue 15935: New version of graphviz

archive/issues_015698.json:
```json
{
    "body": "For some reason, i get a segfault when i try to system-wide graphviz from sage, and the current experimental spkg fails to install, so i updated it. It now installs on my systems and works ok.\n\nhttps://drive.google.com/file/d/0B_B7eBQ-W5NGNVBSR3RCTlJRMEk/edit?usp=sharing\n\n**CC:**  @nthiery @rlmill @mwhansen @nathanncohen @jdemeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/15935\n\n",
    "closed_at": "2014-03-13T22:21:33Z",
    "created_at": "2014-03-13T15:11:43Z",
    "labels": [
        "component: packages: experimental",
        "enhancement",
        "duplicate"
    ],
    "title": "New version of graphviz",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/15935",
    "user": "https://github.com/miguelmarco"
}
```
For some reason, i get a segfault when i try to system-wide graphviz from sage, and the current experimental spkg fails to install, so i updated it. It now installs on my systems and works ok.

https://drive.google.com/file/d/0B_B7eBQ-W5NGNVBSR3RCTlJRMEk/edit?usp=sharing

**CC:**  @nthiery @rlmill @mwhansen @nathanncohen @jdemeyer

Issue created by migration from https://trac.sagemath.org/ticket/15935





---

archive/issue_events_142704.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2014-03-13T15:21:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15935",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15935#event-142704"
}
```



---

archive/issue_comments_204475.json:
```json
{
    "body": "<a id='comment:1'></a>\nSee #15933. Looks like you created 3 copies of this ticket.",
    "created_at": "2014-03-13T15:21:19Z",
    "issue": "https://github.com/sagemath/sage/issues/15935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15935#issuecomment-204475",
    "user": "https://github.com/mezzarobba"
}
```

<a id='comment:1'></a>
See #15933. Looks like you created 3 copies of this ticket.



---

archive/issue_events_142705.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2014-03-13T15:21:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15935",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15935#event-142705"
}
```



---

archive/issue_events_142706.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-03-13T19:32:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15935",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15935#event-142706"
}
```



---

archive/issue_events_142707.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-03-13T19:32:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15935",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15935#event-142707"
}
```



---

archive/issue_events_142708.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-13T22:21:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15935",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15935#event-142708"
}
```



---

archive/issue_events_142709.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-13T22:21:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15935",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15935#event-142709"
}
```



---

archive/issue_events_142710.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-13T22:21:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15935",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15935#event-142710"
}
```

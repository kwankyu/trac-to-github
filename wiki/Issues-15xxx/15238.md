# Issue 15238: Wrapper around Pari's matsolvemod to solve systems of linear congruences

archive/issues_015001.json:
```json
{
    "body": "Add a wrapper around Pari's matsolvemod. This returns a small solution to a system of linear congruences of the form A*X = B mod D, where A is an integral matrix, B is an integral vector and D is either a nonnegative integral vector or a nonnegative integer. \n\n**CC:**  @pjbruin\n\n**Branch:** [u/smueller/ticket/15238](https://github.com/sagemath/sagetrac-mirror/tree/u/smueller/ticket/15238)\n\n**Commit:** [fbf86ecf3c040d6414f1b5afb4d367b521695dd9](https://github.com/sagemath/sagetrac-mirror/commit/fbf86ecf3c040d6414f1b5afb4d367b521695dd9)\n\nIssue created by migration from https://trac.sagemath.org/ticket/15238\n\n",
    "created_at": "2013-09-28T16:11:49Z",
    "labels": [
        "component: linear algebra",
        "minor",
        "enhancement"
    ],
    "title": "Wrapper around Pari's matsolvemod to solve systems of linear congruences",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/15238",
    "user": "https://trac.sagemath.org/admin/accounts/users/smueller"
}
```
Add a wrapper around Pari's matsolvemod. This returns a small solution to a system of linear congruences of the form A*X = B mod D, where A is an integral matrix, B is an integral vector and D is either a nonnegative integral vector or a nonnegative integer. 

**CC:**  @pjbruin

**Branch:** [u/smueller/ticket/15238](https://github.com/sagemath/sagetrac-mirror/tree/u/smueller/ticket/15238)

**Commit:** [fbf86ecf3c040d6414f1b5afb4d367b521695dd9](https://github.com/sagemath/sagetrac-mirror/commit/fbf86ecf3c040d6414f1b5afb4d367b521695dd9)

Issue created by migration from https://trac.sagemath.org/ticket/15238





---

archive/issue_comments_190370.json:
```json
{
    "body": "**Branch:** [u/smueller/ticket/15238](https://github.com/sagemath/sagetrac-mirror/tree/u/smueller/ticket/15238)",
    "created_at": "2013-09-28T16:13:17Z",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15238#issuecomment-190370",
    "user": "https://trac.sagemath.org/admin/accounts/users/smueller"
}
```

**Branch:** [u/smueller/ticket/15238](https://github.com/sagemath/sagetrac-mirror/tree/u/smueller/ticket/15238)



---

archive/issue_comments_190371.json:
```json
{
    "body": "<a id='comment:2'></a>\n**Branch pushed to git repo; I updated commit sha1.** **New commits:**\n<table><tr><td>[changeset:fbf86ec]</td><td><code>changed doctests</code></td></tr></table>\n",
    "created_at": "2013-09-28T16:50:13Z",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15238#issuecomment-190371",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:2'></a>
**Branch pushed to git repo; I updated commit sha1.** **New commits:**
<table><tr><td>[changeset:fbf86ec]</td><td><code>changed doctests</code></td></tr></table>




---

archive/issue_comments_190372.json:
```json
{
    "body": "**Commit:** [fbf86ecf3c040d6414f1b5afb4d367b521695dd9](https://github.com/sagemath/sagetrac-mirror/commit/fbf86ecf3c040d6414f1b5afb4d367b521695dd9)",
    "created_at": "2013-09-28T16:50:13Z",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15238#issuecomment-190372",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

**Commit:** [fbf86ecf3c040d6414f1b5afb4d367b521695dd9](https://github.com/sagemath/sagetrac-mirror/commit/fbf86ecf3c040d6414f1b5afb4d367b521695dd9)



---

archive/issue_comments_190373.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-ADD DESCRIPTION\n+Add a wrapper around Pari's matsolvemod. This returns a small solution to a system of linear congruences of the form A*X = B mod D, where A is an integral matrix, B is an integral vector and D is either a nonnegative integral vector or a nonnegative integer. \n``````\n",
    "created_at": "2013-09-29T09:58:19Z",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15238#issuecomment-190373",
    "user": "https://trac.sagemath.org/admin/accounts/users/smueller"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1 @@
-ADD DESCRIPTION
+Add a wrapper around Pari's matsolvemod. This returns a small solution to a system of linear congruences of the form A*X = B mod D, where A is an integral matrix, B is an integral vector and D is either a nonnegative integral vector or a nonnegative integer. 
``````




---

archive/issue_events_134881.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/smueller",
    "created_at": "2013-09-29T09:58:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "label": "minor",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-134881"
}
```



---

archive/issue_events_134882.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-03T10:21:08Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-134882"
}
```



---

archive/issue_events_134883.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-03T10:21:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-134883"
}
```



---

archive/issue_events_134884.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-134884"
}
```



---

archive/issue_events_134885.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-134885"
}
```



---

archive/issue_events_134886.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-134886"
}
```



---

archive/issue_events_134887.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-134887"
}
```



---

archive/issue_events_134888.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-134888"
}
```



---

archive/issue_events_134889.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-134889"
}
```



---

archive/issue_events_134890.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-134890"
}
```



---

archive/issue_events_134891.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-134891"
}
```



---

archive/issue_events_134892.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-134892"
}
```

# Issue 15968: Fix small issue in Constructions document

archive/issues_015731.json:
```json
{
    "body": "See [this sage-devel thread](https://groups.google.com/forum/#!topic/sage-devel/JEcY0lqfibI).\n\nBasically, \n\n```\nSage uses divisors(n) for the number (usually denoted d(n)) of divisors of n and sigma(n,k) for the sum of the kth powers of the divisors of n (so divisors(n) and sigma(n,0) are the same). For example:\n```\nis wrong - we can most easily change this to have \n\n```\nSage uses divisors(n) for the list of (positive) divisors of n and sigma(n,k) for the sum of the kth powers of the divisors of n (so sum(divisors(n)) and sigma(n,0) are the same). For example:\n```\n\n**Reviewer:** Vincent Delecroix\n\nIssue created by migration from https://trac.sagemath.org/ticket/15968\n\n",
    "closed_at": "2018-05-18T17:16:26Z",
    "created_at": "2014-03-18T00:38:39Z",
    "labels": [
        "component: documentation",
        "minor",
        "bug",
        "wontfix"
    ],
    "title": "Fix small issue in Constructions document",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/15968",
    "user": "https://github.com/kcrisman"
}
```
See [this sage-devel thread](https://groups.google.com/forum/#!topic/sage-devel/JEcY0lqfibI).

Basically, 

```
Sage uses divisors(n) for the number (usually denoted d(n)) of divisors of n and sigma(n,k) for the sum of the kth powers of the divisors of n (so divisors(n) and sigma(n,0) are the same). For example:
```
is wrong - we can most easily change this to have 

```
Sage uses divisors(n) for the list of (positive) divisors of n and sigma(n,k) for the sum of the kth powers of the divisors of n (so sum(divisors(n)) and sigma(n,0) are the same). For example:
```

**Reviewer:** Vincent Delecroix

Issue created by migration from https://trac.sagemath.org/ticket/15968





---

archive/issue_events_142966.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15968",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15968#event-142966"
}
```



---

archive/issue_events_142967.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15968",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15968#event-142967"
}
```



---

archive/issue_events_142968.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15968",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15968#event-142968"
}
```



---

archive/issue_events_142969.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15968",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15968#event-142969"
}
```



---

archive/issue_events_142970.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-04-30T12:25:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15968",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15968#event-142970"
}
```



---

archive/issue_events_142971.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-04-30T12:25:45Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15968",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15968#event-142971"
}
```



---

archive/issue_comments_204903.json:
```json
{
    "body": "<a id='comment:3'></a>\nDone since long... see\n\nhttps://doc.sagemath.org/html/en/constructions/number_theory.html#divisors",
    "created_at": "2018-04-30T12:25:45Z",
    "issue": "https://github.com/sagemath/sage/issues/15968",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15968#issuecomment-204903",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'></a>
Done since long... see

https://doc.sagemath.org/html/en/constructions/number_theory.html#divisors



---

archive/issue_events_142972.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-01T12:29:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15968",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15968#event-142972"
}
```



---

archive/issue_events_142973.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-01T12:29:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15968",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15968#event-142973"
}
```



---

archive/issue_comments_204904.json:
```json
{
    "body": "**Reviewer:** Vincent Delecroix",
    "created_at": "2018-05-01T12:29:37Z",
    "issue": "https://github.com/sagemath/sage/issues/15968",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15968#issuecomment-204904",
    "user": "https://github.com/videlec"
}
```

**Reviewer:** Vincent Delecroix



---

archive/issue_comments_204905.json:
```json
{
    "body": "<a id='comment:5'></a>\nclosing positively reviewed duplicates",
    "created_at": "2018-05-18T17:16:26Z",
    "issue": "https://github.com/sagemath/sage/issues/15968",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15968#issuecomment-204905",
    "user": "https://github.com/videlec"
}
```

<a id='comment:5'></a>
closing positively reviewed duplicates



---

archive/issue_events_142974.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15968",
    "label": "wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15968#event-142974"
}
```



---

archive/issue_events_142975.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15968",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15968#event-142975"
}
```



---

archive/issue_events_142976.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15968",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15968#event-142976"
}
```

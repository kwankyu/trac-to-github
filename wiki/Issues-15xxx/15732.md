# Issue 15732: OS X app fails with no warning when run on read only file system (e.g. dmg)

archive/issues_015495.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nWhen run directly from the disk image the OS X application should at least warn that it won't work instead of silently refusing to start.\n\nComponent: **user interface**\n\nKeywords: **mac app**\n\nAuthor: **Ivan Andrus**\n\nBranch: **[`eb13458`](https://github.com/sagemath/sagetrac-mirror/commit/eb134581fc1b354948e99e150e6c5fdd63cd9120)**\n\nReviewer: **Volker Braun**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15732_\n\n",
    "closed_at": "2014-06-26T01:50:40Z",
    "created_at": "2014-01-25T22:42:33Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "OS X app fails with no warning when run on read only file system (e.g. dmg)",
    "type": "issue",
    "updated_at": "2014-08-12T02:34:22Z",
    "url": "https://github.com/sagemath/sage/issues/15732",
    "user": "https://github.com/gvol"
}
```
<div id="comment:0"></div>

When run directly from the disk image the OS X application should at least warn that it won't work instead of silently refusing to start.

Component: **user interface**

Keywords: **mac app**

Author: **Ivan Andrus**

Branch: **[`eb13458`](https://github.com/sagemath/sagetrac-mirror/commit/eb134581fc1b354948e99e150e6c5fdd63cd9120)**

Reviewer: **Volker Braun**

_Issue created by migration from https://trac.sagemath.org/ticket/15732_





---

archive/issue_events_222570.json:
```json
{
    "actor": "https://github.com/gvol",
    "created_at": "2014-01-25T22:42:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15732#event-222570"
}
```



---

archive/issue_events_222571.json:
```json
{
    "actor": "https://github.com/gvol",
    "created_at": "2014-01-25T22:42:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15732#event-222571"
}
```



---

archive/issue_events_222572.json:
```json
{
    "actor": "https://github.com/gvol",
    "created_at": "2014-01-25T22:42:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15732#event-222572"
}
```



---

archive/issue_events_222573.json:
```json
{
    "actor": "https://github.com/gvol",
    "created_at": "2014-01-25T22:42:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15732#event-222573"
}
```



---

archive/issue_comments_198211.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nI have run into this on numerous occasions but had gotten a semi-useful message.  But you are right that this should be really clear, maybe suggesting also to \"drag to Applications\" or \"read the README\" or something.  Not everyone knows what a README is, after all.",
    "created_at": "2014-01-29T15:57:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15732#issuecomment-198211",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:1" align="right">comment:1</div>

I have run into this on numerous occasions but had gotten a semi-useful message.  But you are right that this should be really clear, maybe suggesting also to "drag to Applications" or "read the README" or something.  Not everyone knows what a README is, after all.



---

archive/issue_events_222574.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15732#event-222574"
}
```



---

archive/issue_events_222575.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15732#event-222575"
}
```



---

archive/issue_events_222576.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15732#event-222576"
}
```



---

archive/issue_events_222577.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15732#event-222577"
}
```



---

archive/issue_comments_198212.json:
```json
{
    "body": "Commit: **[`eb13458`](https://github.com/sagemath/sagetrac-mirror/commit/eb134581fc1b354948e99e150e6c5fdd63cd9120)**",
    "created_at": "2014-06-03T05:08:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15732#issuecomment-198212",
    "user": "https://github.com/gvol"
}
```

Commit: **[`eb13458`](https://github.com/sagemath/sagetrac-mirror/commit/eb134581fc1b354948e99e150e6c5fdd63cd9120)**



---

archive/issue_comments_198213.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nHere are some changes to show a (modal) dialog in the case that it's being run from a read-only file system e.g. disk image.\n\nI also included some fixes to warnings.  So that's what those other changes are.",
    "created_at": "2014-06-03T05:08:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15732#issuecomment-198213",
    "user": "https://github.com/gvol"
}
```

<div id="comment:4" align="right">comment:4</div>

Here are some changes to show a (modal) dialog in the case that it's being run from a read-only file system e.g. disk image.

I also included some fixes to warnings.  So that's what those other changes are.



---

archive/issue_comments_198214.json:
```json
{
    "body": "Author: **Ivan Andrus**",
    "created_at": "2014-06-03T05:08:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15732#issuecomment-198214",
    "user": "https://github.com/gvol"
}
```

Author: **Ivan Andrus**



---

archive/issue_comments_198215.json:
```json
{
    "body": "Branch: **[u/iandrus/mac-app-readwrite](https://github.com/sagemath/sagetrac-mirror/tree/u/iandrus/mac-app-readwrite)**",
    "created_at": "2014-06-03T05:08:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15732#issuecomment-198215",
    "user": "https://github.com/gvol"
}
```

Branch: **[u/iandrus/mac-app-readwrite](https://github.com/sagemath/sagetrac-mirror/tree/u/iandrus/mac-app-readwrite)**



---

archive/issue_events_222578.json:
```json
{
    "actor": "https://github.com/gvol",
    "created_at": "2014-06-03T05:19:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15732#event-222578"
}
```



---

archive/issue_comments_198216.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nTwo questions:\n* What is `project.xcworkspace/xcshareddata/Sage.xccheckout`?  It doesn't seem to fit in either of the categories you mention, though it looks like a harmless informational file.\n* What versions of OS X have you tested this on so far?  Does it rely on a newer Xcode?  (Just asking.)",
    "created_at": "2014-06-03T12:11:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15732#issuecomment-198216",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:6" align="right">comment:6</div>

Two questions:
* What is `project.xcworkspace/xcshareddata/Sage.xccheckout`?  It doesn't seem to fit in either of the categories you mention, though it looks like a harmless informational file.
* What versions of OS X have you tested this on so far?  Does it rely on a newer Xcode?  (Just asking.)



---

archive/issue_comments_198217.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nReplying to [@kcrisman](#comment%3A6):\n> Two questions:\n> * What is `project.xcworkspace/xcshareddata/Sage.xccheckout`?  It doesn't seem to fit in either of the categories you mention, though it looks like a harmless informational file.\n\nIt's a file the new Xcode uses to keep track of repositories.  Since we now use git and Xcode understands git, it got created.  Apparently it should be checked in to version control, so I did:\nhttp://stackoverflow.com/questions/18340453/should-xccheckout-files-in-xcode5-be-ignored-under-vcs\n\nBut yes, it *should be* harmless.\n\n> * What versions of OS X have you tested this on so far?  Does it rely on a newer Xcode?  (Just asking.)\n\nI have only tested it on Mavericks, though I intentionally did **not** upgrade the Xcode project so it *should* work with any version of Xcode that it did before.",
    "created_at": "2014-06-03T14:15:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15732#issuecomment-198217",
    "user": "https://github.com/gvol"
}
```

<div id="comment:7" align="right">comment:7</div>

Replying to [@kcrisman](#comment%3A6):
> Two questions:
> * What is `project.xcworkspace/xcshareddata/Sage.xccheckout`?  It doesn't seem to fit in either of the categories you mention, though it looks like a harmless informational file.

It's a file the new Xcode uses to keep track of repositories.  Since we now use git and Xcode understands git, it got created.  Apparently it should be checked in to version control, so I did:
http://stackoverflow.com/questions/18340453/should-xccheckout-files-in-xcode5-be-ignored-under-vcs

But yes, it *should be* harmless.

> * What versions of OS X have you tested this on so far?  Does it rely on a newer Xcode?  (Just asking.)

I have only tested it on Mavericks, though I intentionally did **not** upgrade the Xcode project so it *should* work with any version of Xcode that it did before.



---

archive/issue_comments_198218.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2014-06-25T01:16:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15732#issuecomment-198218",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_222579.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-06-25T01:16:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15732#event-222579"
}
```



---

archive/issue_events_222580.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-06-25T01:16:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15732#event-222580"
}
```



---

archive/issue_comments_198219.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nThanks, Volker, I'm woefully behind on reviewing.  One would think I have a real job not involving Sage or something...  Still haven't tested it on 10.4 but I guess it isn't supported anyway ;-)",
    "created_at": "2014-06-25T13:29:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15732#issuecomment-198219",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:9" align="right">comment:9</div>

Thanks, Volker, I'm woefully behind on reviewing.  One would think I have a real job not involving Sage or something...  Still haven't tested it on 10.4 but I guess it isn't supported anyway ;-)



---

archive/issue_comments_198220.json:
```json
{
    "body": "Changed branch from **[u/iandrus/mac-app-readwrite](https://github.com/sagemath/sagetrac-mirror/tree/u/iandrus/mac-app-readwrite)** to **[`eb13458`](https://github.com/sagemath/sagetrac-mirror/commit/eb134581fc1b354948e99e150e6c5fdd63cd9120)**",
    "created_at": "2014-06-26T01:50:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15732#issuecomment-198220",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/iandrus/mac-app-readwrite](https://github.com/sagemath/sagetrac-mirror/tree/u/iandrus/mac-app-readwrite)** to **[`eb13458`](https://github.com/sagemath/sagetrac-mirror/commit/eb134581fc1b354948e99e150e6c5fdd63cd9120)**



---

archive/issue_events_222581.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-06-26T01:50:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15732#event-222581"
}
```



---

archive/issue_events_222582.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7b87328a8f1b90844acfb2c3d66ee02790f96f0e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-06-26T01:50:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15732#event-222582"
}
```



---

archive/issue_comments_198221.json:
```json
{
    "body": "Changed commit from **[`eb13458`](https://github.com/sagemath/sagetrac-mirror/commit/eb134581fc1b354948e99e150e6c5fdd63cd9120)** to none",
    "created_at": "2014-08-12T02:34:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15732#issuecomment-198221",
    "user": "https://github.com/kcrisman"
}
```

Changed commit from **[`eb13458`](https://github.com/sagemath/sagetrac-mirror/commit/eb134581fc1b354948e99e150e6c5fdd63cd9120)** to none



---

archive/issue_comments_198222.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nThe `NSModalResponse` causes problems on everything except 10.9.  See #16796.",
    "created_at": "2014-08-12T02:34:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15732#issuecomment-198222",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:11" align="right">comment:11</div>

The `NSModalResponse` causes problems on everything except 10.9.  See #16796.

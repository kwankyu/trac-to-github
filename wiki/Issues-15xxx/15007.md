# Issue 15007: adding diff() and differentiate() aliases to polynomial rings for consistency

archive/issues_014770.json:
```json
{
    "body": "\n```\nsage: R.<y> = ZZ[]\nsage: y.diff()\nTraceback (click to the left of this block for traceback)\n...\nAttributeError:\n'sage.rings.polynomial.polynomial_integer_dense_flint.Polynomial_integer\\\n_dense_flint' object has no attribute 'diff'\n```\ndoes not work. Neither does `y.differentiate()`. You can only do the desired operation with\n\n```\nsage: y.derivative()\n1\n```\n\nIn the symbolic ring all of the above is allowed:\n\n```\nsage: var('z')\nz\nsage: z.diff()\n1\nsage: z.differentiate()\n1\nsage: z.derivative()\n1\n```\n\nFor consistency aliases `.diff()` and `.differentiate()` should be added to all polynomial rings.\n\n**Keywords:** diff, derivative, polynomial ring\n\n**Reviewer:** Jeroen Demeyer\n\n**Author:** Jonathan Hayase\n\n**Merged:** sage-5.13.rc0\n\nIssue created by migration from https://trac.sagemath.org/ticket/15007\n\n",
    "closed_at": "2013-12-07T08:26:14Z",
    "created_at": "2013-08-06T09:42:52Z",
    "labels": [
        "component: algebra",
        "trivial",
        "enhancement",
        "good first issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.13",
    "title": "adding diff() and differentiate() aliases to polynomial rings for consistency",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/15007",
    "user": "https://github.com/dkrenn"
}
```

```
sage: R.<y> = ZZ[]
sage: y.diff()
Traceback (click to the left of this block for traceback)
...
AttributeError:
'sage.rings.polynomial.polynomial_integer_dense_flint.Polynomial_integer\
_dense_flint' object has no attribute 'diff'
```
does not work. Neither does `y.differentiate()`. You can only do the desired operation with

```
sage: y.derivative()
1
```

In the symbolic ring all of the above is allowed:

```
sage: var('z')
z
sage: z.diff()
1
sage: z.differentiate()
1
sage: z.derivative()
1
```

For consistency aliases `.diff()` and `.differentiate()` should be added to all polynomial rings.

**Keywords:** diff, derivative, polynomial ring

**Reviewer:** Jeroen Demeyer

**Author:** Jonathan Hayase

**Merged:** sage-5.13.rc0

Issue created by migration from https://trac.sagemath.org/ticket/15007





---

archive/issue_comments_186590.json:
```json
{
    "body": "<a id='comment:1'></a>\nI will work on this as my first ticket.",
    "created_at": "2013-09-08T00:24:02Z",
    "issue": "https://github.com/sagemath/sage/issues/15007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15007#issuecomment-186590",
    "user": "https://trac.sagemath.org/admin/accounts/users/PythonNut"
}
```

<a id='comment:1'></a>
I will work on this as my first ticket.



---

archive/attachments_020794.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "polynomial_ring_aliasing.patch",
    "asset_url": "tarball://root/attachments/ticket15007/polynomial_ring_aliasing.patch",
    "created_at": "2013-09-11T04:13:03Z",
    "issue": "https://github.com/sagemath/sage/issues/15007",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket15007/polynomial_ring_aliasing.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/PythonNut"
}
```



---

archive/issue_comments_186591.json:
```json
{
    "body": "**Attachment:** [polynomial_ring_aliasing.patch](https://github.com/sagemath/sage/files/ticket15007/polynomial_ring_aliasing.patch)\n\nAlias .diff() and .differentiate() to .derivative() in polynomial rings",
    "created_at": "2013-09-11T04:13:03Z",
    "issue": "https://github.com/sagemath/sage/issues/15007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15007#issuecomment-186591",
    "user": "https://trac.sagemath.org/admin/accounts/users/PythonNut"
}
```

**Attachment:** [polynomial_ring_aliasing.patch](https://github.com/sagemath/sage/files/ticket15007/polynomial_ring_aliasing.patch)

Alias .diff() and .differentiate() to .derivative() in polynomial rings



---

archive/issue_events_132307.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/PythonNut",
    "created_at": "2013-09-11T04:14:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15007",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15007#event-132307"
}
```



---

archive/issue_comments_186592.json:
```json
{
    "body": "**Author:** Jonathan Hayase",
    "created_at": "2013-12-06T10:56:09Z",
    "issue": "https://github.com/sagemath/sage/issues/15007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15007#issuecomment-186592",
    "user": "https://github.com/jdemeyer"
}
```

**Author:** Jonathan Hayase



---

archive/issue_comments_186593.json:
```json
{
    "body": "**Merged:** sage-5.13.rc0",
    "created_at": "2013-12-07T08:26:14Z",
    "issue": "https://github.com/sagemath/sage/issues/15007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15007#issuecomment-186593",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.13.rc0



---

archive/issue_comments_186594.json:
```json
{
    "body": "<a id='comment:4'></a>\nLooks good.",
    "created_at": "2013-12-07T08:26:14Z",
    "issue": "https://github.com/sagemath/sage/issues/15007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15007#issuecomment-186594",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'></a>
Looks good.



---

archive/issue_comments_186595.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2013-12-07T08:26:14Z",
    "issue": "https://github.com/sagemath/sage/issues/15007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15007#issuecomment-186595",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_events_132308.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-07T08:26:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15007",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15007#event-132308"
}
```



---

archive/issue_events_132309.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-07T08:26:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15007",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15007#event-132309"
}
```

# Issue 15014: Let gf2x build a shared library on Cygwin

archive/issues_014777.json:
```json
{
    "body": "This is related to #13354.\nReported upstream which waits for a proper fix form my side...\n\nUse:\n* http://boxen.math.washington.edu/home/jpflori/spkg/gf2x-1.1.p0.spkg\n\n\n**CC:**  @kcrisman @dimpase @tscrim\n\n**Keywords:** gf2x spkg cygwin\n\n**Upstream:** Reported upstream. Developers acknowledge bug.\n\n**Reviewer:** Travis Scrimshaw\n\n**Author:** Jean-Pierre Flori\n\n**Merged:** sage-5.12.beta2\n\nIssue created by migration from https://trac.sagemath.org/ticket/15014\n\n",
    "closed_at": "2013-08-16T21:20:03Z",
    "created_at": "2013-08-06T21:33:17Z",
    "labels": [
        "component: porting: cygwin",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.12",
    "title": "Let gf2x build a shared library on Cygwin",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/15014",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpflori"
}
```
This is related to #13354.
Reported upstream which waits for a proper fix form my side...

Use:
* http://boxen.math.washington.edu/home/jpflori/spkg/gf2x-1.1.p0.spkg


**CC:**  @kcrisman @dimpase @tscrim

**Keywords:** gf2x spkg cygwin

**Upstream:** Reported upstream. Developers acknowledge bug.

**Reviewer:** Travis Scrimshaw

**Author:** Jean-Pierre Flori

**Merged:** sage-5.12.beta2

Issue created by migration from https://trac.sagemath.org/ticket/15014





---

archive/attachments_020798.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "gf2x-1.1.p0.diff",
    "asset_url": "tarball://root/attachments/ticket15014/gf2x-1.1.p0.diff",
    "created_at": "2013-08-06T21:39:33Z",
    "issue": "https://github.com/sagemath/sage/issues/15014",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket15014/gf2x-1.1.p0.diff",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpflori"
}
```



---

archive/issue_comments_186630.json:
```json
{
    "body": "**Attachment:** [gf2x-1.1.p0.diff](https://github.com/sagemath/sage/files/ticket15014/gf2x-1.1.p0.diff)\n\nSpkg diff, for review only.",
    "created_at": "2013-08-06T21:39:33Z",
    "issue": "https://github.com/sagemath/sage/issues/15014",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15014#issuecomment-186630",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpflori"
}
```

**Attachment:** [gf2x-1.1.p0.diff](https://github.com/sagemath/sage/files/ticket15014/gf2x-1.1.p0.diff)

Spkg diff, for review only.



---

archive/issue_events_132360.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/jpflori",
    "created_at": "2013-08-06T21:40:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15014",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15014#event-132360"
}
```



---

archive/issue_comments_186631.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good to me. Built on cygwin64 and I'll double-check on cygwin32 in a bit and report if there's a failure.",
    "created_at": "2013-08-07T00:24:35Z",
    "issue": "https://github.com/sagemath/sage/issues/15014",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15014#issuecomment-186631",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:2'></a>
Looks good to me. Built on cygwin64 and I'll double-check on cygwin32 in a bit and report if there's a failure.



---

archive/issue_comments_186632.json:
```json
{
    "body": "**Reviewer:** Travis Scrimshaw",
    "created_at": "2013-08-07T05:40:20Z",
    "issue": "https://github.com/sagemath/sage/issues/15014",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15014#issuecomment-186632",
    "user": "https://github.com/tscrim"
}
```

**Reviewer:** Travis Scrimshaw



---

archive/issue_comments_186633.json:
```json
{
    "body": "<a id='comment:3'></a>\nAlso works on cygwin32.",
    "created_at": "2013-08-07T05:40:20Z",
    "issue": "https://github.com/sagemath/sage/issues/15014",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15014#issuecomment-186633",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'></a>
Also works on cygwin32.



---

archive/issue_events_132361.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-08-07T05:40:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15014",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15014#event-132361"
}
```



---

archive/issue_events_132362.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-08-07T05:40:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15014",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15014#event-132362"
}
```



---

archive/issue_comments_186634.json:
```json
{
    "body": "**Merged:** sage-5.12.beta2",
    "created_at": "2013-08-16T21:20:03Z",
    "issue": "https://github.com/sagemath/sage/issues/15014",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15014#issuecomment-186634",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.12.beta2



---

archive/issue_events_132363.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T21:20:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15014",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15014#event-132363"
}
```



---

archive/issue_events_132364.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T21:20:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15014",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15014#event-132364"
}
```

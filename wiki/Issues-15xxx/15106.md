# Issue 15106: Allow sage --spkg from a different directory

archive/issues_014869.json:
```json
{
    "body": "I'd like to be able to do the following from within the directory of a spkg:\n\n```\nsage --spkg .   # current directory\n```\nbut that doesn't work properly (it would create a file `..spkg`).\n\nTherefore, I propose that\n\n```\nsage --spkg DIR\n```\nfirst does a `cd DIR/..` and then creates the package from there.\n\nThe attached patch also adds some additional checks, for example that a changelog entry exists in `SPKG.txt`.\n\n**Apply** [attachment:15106_sage_pkg_chdir.patch\u200b](https://github.com/sagemath/sage/files/ticket15106/971ee2d28395569b7f6b3405368c136d.patch\u200b) to `$SAGE_LOCAL/bin`.\n\n**Reviewer:** Jeroen Demeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/15106\n\n",
    "closed_at": "2014-02-04T21:11:59Z",
    "created_at": "2013-08-27T07:37:08Z",
    "labels": [
        "component: scripts",
        "enhancement",
        "wontfix"
    ],
    "title": "Allow sage --spkg from a different directory",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/15106",
    "user": "https://github.com/jdemeyer"
}
```
I'd like to be able to do the following from within the directory of a spkg:

```
sage --spkg .   # current directory
```
but that doesn't work properly (it would create a file `..spkg`).

Therefore, I propose that

```
sage --spkg DIR
```
first does a `cd DIR/..` and then creates the package from there.

The attached patch also adds some additional checks, for example that a changelog entry exists in `SPKG.txt`.

**Apply** [attachment:15106_sage_pkg_chdir.patch​](https://github.com/sagemath/sage/files/ticket15106/971ee2d28395569b7f6b3405368c136d.patch​) to `$SAGE_LOCAL/bin`.

**Reviewer:** Jeroen Demeyer

Issue created by migration from https://trac.sagemath.org/ticket/15106





---

archive/attachments_020911.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "15106_sage_pkg_chdir.patch",
    "asset_url": "tarball://root/attachments/ticket15106/15106_sage_pkg_chdir.patch",
    "created_at": "2013-08-27T08:24:15Z",
    "issue": "https://github.com/sagemath/sage/issues/15106",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket15106/15106_sage_pkg_chdir.patch",
    "user": "https://github.com/jdemeyer"
}
```



---

archive/issue_comments_188113.json:
```json
{
    "body": "**Attachment:** [15106_sage_pkg_chdir.patch](https://github.com/sagemath/sage/files/ticket15106/15106_sage_pkg_chdir.patch)",
    "created_at": "2013-08-27T08:24:15Z",
    "issue": "https://github.com/sagemath/sage/issues/15106",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15106#issuecomment-188113",
    "user": "https://github.com/jdemeyer"
}
```

**Attachment:** [15106_sage_pkg_chdir.patch](https://github.com/sagemath/sage/files/ticket15106/15106_sage_pkg_chdir.patch)



---

archive/issue_comments_188114.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -11,3 +11,7 @@\n sage --spkg DIR\n ```\n first does a `cd DIR/..` and then creates the package from there.\n+\n+The attached patch also adds some additional checks, for example that a changelog entry exists in `SPKG.txt`.\n+\n+**Apply** [attachment:15106_sage_pkg_chdir.patch\u200b](https://github.com/sagemath/sage/files/ticket15106/971ee2d28395569b7f6b3405368c136d.patch\u200b) to `$SAGE_LOCAL/bin`.\n``````\n",
    "created_at": "2013-08-27T08:26:40Z",
    "issue": "https://github.com/sagemath/sage/issues/15106",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15106#issuecomment-188114",
    "user": "https://github.com/jdemeyer"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -11,3 +11,7 @@
 sage --spkg DIR
 ```
 first does a `cd DIR/..` and then creates the package from there.
+
+The attached patch also adds some additional checks, for example that a changelog entry exists in `SPKG.txt`.
+
+**Apply** [attachment:15106_sage_pkg_chdir.patch​](https://github.com/sagemath/sage/files/ticket15106/971ee2d28395569b7f6b3405368c136d.patch​) to `$SAGE_LOCAL/bin`.
``````




---

archive/issue_events_133378.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-27T08:26:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15106",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15106#event-133378"
}
```



---

archive/issue_events_133379.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15106",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15106#event-133379"
}
```



---

archive/issue_events_133380.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15106",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15106#event-133380"
}
```



---

archive/issue_comments_188115.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2014-02-04T08:20:32Z",
    "issue": "https://github.com/sagemath/sage/issues/15106",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15106#issuecomment-188115",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_comments_188116.json:
```json
{
    "body": "**Changing author** from \"Jeroen Demeyer\" to \"\".",
    "created_at": "2014-02-04T08:20:32Z",
    "issue": "https://github.com/sagemath/sage/issues/15106",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15106#issuecomment-188116",
    "user": "https://github.com/jdemeyer"
}
```

**Changing author** from "Jeroen Demeyer" to "".



---

archive/issue_events_133381.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-02-04T08:20:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15106",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15106#event-133381"
}
```



---

archive/issue_comments_188117.json:
```json
{
    "body": "<a id='comment:3'></a>\nShould be closed since sage-pkg is obsolete.",
    "created_at": "2014-02-04T08:20:32Z",
    "issue": "https://github.com/sagemath/sage/issues/15106",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15106#issuecomment-188117",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'></a>
Should be closed since sage-pkg is obsolete.



---

archive/issue_events_133382.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-02-04T08:20:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15106",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15106#event-133382"
}
```



---

archive/issue_events_133383.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-02-04T08:20:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15106",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15106#event-133383"
}
```



---

archive/issue_events_133384.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-04T21:11:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15106",
    "label": "wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15106#event-133384"
}
```



---

archive/issue_events_133385.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-04T21:11:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15106",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15106#event-133385"
}
```



---

archive/issue_events_133386.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-04T21:11:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15106",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15106#event-133386"
}
```

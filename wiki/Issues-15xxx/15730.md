# Issue 15730: small problems with CHomP, followup to #15161

archive/issues_015493.json:
```json
{
    "assignees": [],
    "body": "<div id=\"comment:0\"></div>\n\nThis is a followup to #15161, fixing some problems which arose from the changes there. In particular, if CHomP was installed, then the changes at #15161 led to some doctest failures. These have mostly been corrected, except for failures as in #12607 and also that CHomP seems to be broken on my machine, so it is not computing homology groups correctly. I think we should put off actually fixing CHomP to #15202.\n\nComponent: **algebraic topology**\n\nAuthor: **John Palmieri**\n\nBranch/Commit: **[u/jhpalmieri/chain](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/chain) @ [`b02b283`](https://github.com/sagemath/sagetrac-mirror/commit/b02b283e6a3da0159708b7a373df1a96c9265f1a)**\n\nReviewer: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15730_\n\n",
    "closed_at": "2014-02-02T16:57:16Z",
    "created_at": "2014-01-24T22:26:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20algebraic%20topology",
        "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "small problems with CHomP, followup to #15161",
    "type": "issue",
    "updated_at": "2014-02-02T16:57:16Z",
    "url": "https://github.com/sagemath/sage/issues/15730",
    "user": "https://github.com/jhpalmieri"
}
```
<div id="comment:0"></div>

This is a followup to #15161, fixing some problems which arose from the changes there. In particular, if CHomP was installed, then the changes at #15161 led to some doctest failures. These have mostly been corrected, except for failures as in #12607 and also that CHomP seems to be broken on my machine, so it is not computing homology groups correctly. I think we should put off actually fixing CHomP to #15202.

Component: **algebraic topology**

Author: **John Palmieri**

Branch/Commit: **[u/jhpalmieri/chain](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/chain) @ [`b02b283`](https://github.com/sagemath/sagetrac-mirror/commit/b02b283e6a3da0159708b7a373df1a96c9265f1a)**

Reviewer: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/15730_





---

archive/issue_events_222543.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-01-24T22:26:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15730",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15730#event-222543"
}
```



---

archive/issue_events_222544.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-01-24T22:26:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15730",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebraic%20topology",
    "label_color": "0000ff",
    "label_name": "c: algebraic topology",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15730#event-222544"
}
```



---

archive/issue_events_222545.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-01-24T22:26:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15730",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15730#event-222545"
}
```



---

archive/issue_events_222546.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-01-24T22:26:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15730",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15730#event-222546"
}
```



---

archive/issue_events_222547.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-01-24T22:27:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15730",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15730#event-222547"
}
```



---

archive/issue_events_222548.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15730",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15730#event-222548"
}
```



---

archive/issue_events_222549.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15730",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15730#event-222549"
}
```



---

archive/issue_comments_198202.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nLGTM. Just to note, I'm getting two errors, which weren't there before I installed the CHomP spkg, but the first is noted in #12607:\n\n```\nsage -t chain_complex.py\n**********************************************************************\nFile \"chain_complex.py\", line 1135, in sage.homology.chain_complex.ChainComplex_class.homology\nFailed example:\n    D.homology()\nExpected:\n    {0: 0, 1: 0, 4: 0, 5: 0}\nGot:\n    {}\n**********************************************************************\nFile \"chain_complex.py\", line 1397, in sage.homology.chain_complex.ChainComplex_class.torsion_list\nFailed example:\n    C.torsion_list(5)\nExpected:\n    [(2, [1]), (3, [3])]\nGot:\n    [(2, []), (3, [])]\n**********************************************************************\n```",
    "created_at": "2014-02-02T06:54:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15730",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15730#issuecomment-198202",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">comment:3</div>

LGTM. Just to note, I'm getting two errors, which weren't there before I installed the CHomP spkg, but the first is noted in #12607:

```
sage -t chain_complex.py
**********************************************************************
File "chain_complex.py", line 1135, in sage.homology.chain_complex.ChainComplex_class.homology
Failed example:
    D.homology()
Expected:
    {0: 0, 1: 0, 4: 0, 5: 0}
Got:
    {}
**********************************************************************
File "chain_complex.py", line 1397, in sage.homology.chain_complex.ChainComplex_class.torsion_list
Failed example:
    C.torsion_list(5)
Expected:
    [(2, [1]), (3, [3])]
Got:
    [(2, []), (3, [])]
**********************************************************************
```



---

archive/issue_events_222550.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-02-02T06:54:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15730",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15730#event-222550"
}
```



---

archive/issue_events_222551.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-02-02T06:54:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15730",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15730#event-222551"
}
```



---

archive/issue_comments_198203.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2014-02-02T06:54:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15730",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15730#issuecomment-198203",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_222552.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-02T16:57:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15730",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15730#event-222552"
}
```



---

archive/issue_events_222553.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-02T16:57:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15730",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15730#event-222553"
}
```

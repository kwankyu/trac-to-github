# Issue 15091: export TERMINFO in sage shell script

archive/issues_014854.json:
```json
{
    "body": "the place of terminfo is hardcoded in libtinfo.so which presents a problem for relocating the Sage tree. Hence, we should set the TERMINFO variable to make sure ncurses can still find terminal information after such a relocation.\n\nSee: https://groups.google.com/forum/#!topic/sage-support/J2_zYXXAFGk\n\n**CC:**  @vbraun\n\n**Reviewer:** Volker Braun\n\n**Author:** Martin Albrecht\n\n**Merged:** sage-5.12.beta4\n\nIssue created by migration from https://trac.sagemath.org/ticket/15091\n\n",
    "closed_at": "2013-08-28T06:55:28Z",
    "created_at": "2013-08-23T21:01:13Z",
    "labels": [
        "component: relocation",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.12",
    "title": "export TERMINFO in sage shell script",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/15091",
    "user": "https://github.com/malb"
}
```
the place of terminfo is hardcoded in libtinfo.so which presents a problem for relocating the Sage tree. Hence, we should set the TERMINFO variable to make sure ncurses can still find terminal information after such a relocation.

See: https://groups.google.com/forum/#!topic/sage-support/J2_zYXXAFGk

**CC:**  @vbraun

**Reviewer:** Volker Braun

**Author:** Martin Albrecht

**Merged:** sage-5.12.beta4

Issue created by migration from https://trac.sagemath.org/ticket/15091





---

archive/attachments_020893.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac15091_terminfo.patch",
    "asset_url": "tarball://root/attachments/ticket15091/trac15091_terminfo.patch",
    "created_at": "2013-08-23T21:03:58Z",
    "issue": "https://github.com/sagemath/sage/issues/15091",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket15091/trac15091_terminfo.patch",
    "user": "https://github.com/malb"
}
```



---

archive/issue_comments_187779.json:
```json
{
    "body": "**Attachment:** [trac15091_terminfo.patch](https://github.com/sagemath/sage/files/ticket15091/trac15091_terminfo.patch)",
    "created_at": "2013-08-23T21:03:58Z",
    "issue": "https://github.com/sagemath/sage/issues/15091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15091#issuecomment-187779",
    "user": "https://github.com/malb"
}
```

**Attachment:** [trac15091_terminfo.patch](https://github.com/sagemath/sage/files/ticket15091/trac15091_terminfo.patch)



---

archive/issue_events_133192.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2013-08-23T21:04:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15091",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15091#event-133192"
}
```



---

archive/issue_comments_187780.json:
```json
{
    "body": "<a id='comment:2'></a>\nThanks, looks good!\n\nI'm making this a blocker since afaik we do have a policy to support relocation and this patch is both simple and necessary.",
    "created_at": "2013-08-23T21:49:13Z",
    "issue": "https://github.com/sagemath/sage/issues/15091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15091#issuecomment-187780",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:2'></a>
Thanks, looks good!

I'm making this a blocker since afaik we do have a policy to support relocation and this patch is both simple and necessary.



---

archive/issue_events_133193.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-08-23T21:49:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15091",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15091#event-133193"
}
```



---

archive/issue_events_133194.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-08-23T21:49:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15091",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15091#event-133194"
}
```



---

archive/issue_comments_187781.json:
```json
{
    "body": "**Reviewer:** Volker Braun",
    "created_at": "2013-08-23T21:49:13Z",
    "issue": "https://github.com/sagemath/sage/issues/15091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15091#issuecomment-187781",
    "user": "https://github.com/vbraun"
}
```

**Reviewer:** Volker Braun



---

archive/issue_events_133195.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-08-23T21:49:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15091",
    "label": "blocker",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15091#event-133195"
}
```



---

archive/issue_comments_187782.json:
```json
{
    "body": "**Author:** Martin Albrecht",
    "created_at": "2013-08-23T21:49:13Z",
    "issue": "https://github.com/sagemath/sage/issues/15091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15091#issuecomment-187782",
    "user": "https://github.com/vbraun"
}
```

**Author:** Martin Albrecht



---

archive/issue_comments_187783.json:
```json
{
    "body": "**Merged:** sage-5.12.beta4",
    "created_at": "2013-08-28T06:55:28Z",
    "issue": "https://github.com/sagemath/sage/issues/15091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15091#issuecomment-187783",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.12.beta4



---

archive/issue_events_133196.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-28T06:55:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15091",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15091#event-133196"
}
```



---

archive/issue_events_133197.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-28T06:55:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15091",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15091#event-133197"
}
```

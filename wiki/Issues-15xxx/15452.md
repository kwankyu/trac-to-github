# Issue 15452: Fix latex representation bug in operation table

archive/issues_015215.json:
```json
{
    "body": "As noted in <http://ask.sagemath.org/question/3254/undefined-control-sequence-when-showing>\n\n```\nsage: Z5=Integers(5)\nsage: show(Z5.multiplication_table())\n```\n\nMakes pdfTeX go nuts. This is due to a rogue backslash.\n\n**Branch:** [u/defeo/ticket/15452](https://github.com/sagemath/sagetrac-mirror/tree/u/defeo/ticket/15452)\n\n**Commit:** [b8bd642bad77f801ac1b5f0c5c2c919f06babbad](https://github.com/sagemath/sagetrac-mirror/commit/b8bd642bad77f801ac1b5f0c5c2c919f06babbad)\n\n**Reviewer:** Luca De Feo\n\nIssue created by migration from https://trac.sagemath.org/ticket/15452\n\n",
    "closed_at": "2013-11-26T10:57:45Z",
    "created_at": "2013-11-25T20:54:31Z",
    "labels": [
        "component: misc",
        "minor",
        "bug",
        "duplicate"
    ],
    "title": "Fix latex representation bug in operation table",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/15452",
    "user": "https://github.com/defeo"
}
```
As noted in <http://ask.sagemath.org/question/3254/undefined-control-sequence-when-showing>

```
sage: Z5=Integers(5)
sage: show(Z5.multiplication_table())
```

Makes pdfTeX go nuts. This is due to a rogue backslash.

**Branch:** [u/defeo/ticket/15452](https://github.com/sagemath/sagetrac-mirror/tree/u/defeo/ticket/15452)

**Commit:** [b8bd642bad77f801ac1b5f0c5c2c919f06babbad](https://github.com/sagemath/sagetrac-mirror/commit/b8bd642bad77f801ac1b5f0c5c2c919f06babbad)

**Reviewer:** Luca De Feo

Issue created by migration from https://trac.sagemath.org/ticket/15452





---

archive/issue_comments_194868.json:
```json
{
    "body": "**Branch:** [u/defeo/ticket/15452](https://github.com/sagemath/sagetrac-mirror/tree/u/defeo/ticket/15452)",
    "created_at": "2013-11-25T20:58:39Z",
    "issue": "https://github.com/sagemath/sage/issues/15452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15452#issuecomment-194868",
    "user": "https://github.com/defeo"
}
```

**Branch:** [u/defeo/ticket/15452](https://github.com/sagemath/sagetrac-mirror/tree/u/defeo/ticket/15452)



---

archive/issue_comments_194869.json:
```json
{
    "body": "**Author:** Luca De Feo",
    "created_at": "2013-11-25T21:00:00Z",
    "issue": "https://github.com/sagemath/sage/issues/15452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15452#issuecomment-194869",
    "user": "https://github.com/defeo"
}
```

**Author:** Luca De Feo



---

archive/issue_comments_194870.json:
```json
{
    "body": "**Commit:** [b8bd642bad77f801ac1b5f0c5c2c919f06babbad](https://github.com/sagemath/sagetrac-mirror/commit/b8bd642bad77f801ac1b5f0c5c2c919f06babbad)",
    "created_at": "2013-11-25T21:00:00Z",
    "issue": "https://github.com/sagemath/sage/issues/15452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15452#issuecomment-194870",
    "user": "https://github.com/defeo"
}
```

**Commit:** [b8bd642bad77f801ac1b5f0c5c2c919f06babbad](https://github.com/sagemath/sagetrac-mirror/commit/b8bd642bad77f801ac1b5f0c5c2c919f06babbad)



---

archive/issue_events_137395.json:
```json
{
    "actor": "https://github.com/defeo",
    "created_at": "2013-11-25T21:00:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15452",
    "milestone": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15452#event-137395"
}
```



---

archive/issue_events_137396.json:
```json
{
    "actor": "https://github.com/defeo",
    "created_at": "2013-11-25T21:00:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15452",
    "milestone": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15452#event-137396"
}
```



---

archive/issue_events_137397.json:
```json
{
    "actor": "https://github.com/defeo",
    "created_at": "2013-11-25T21:00:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15452",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15452#event-137397"
}
```



---

archive/issue_comments_194871.json:
```json
{
    "body": "<a id='comment:3'></a>\nI think this is a duplicate of #15210.",
    "created_at": "2013-11-25T21:19:11Z",
    "issue": "https://github.com/sagemath/sage/issues/15452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15452#issuecomment-194871",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:3'></a>
I think this is a duplicate of #15210.



---

archive/issue_events_137398.json:
```json
{
    "actor": "https://github.com/defeo",
    "created_at": "2013-11-25T21:24:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15452",
    "milestone": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15452#event-137398"
}
```



---

archive/issue_comments_194872.json:
```json
{
    "body": "<a id='comment:4'></a>\nRight. That's why trac was showing no difference from master! Let's close this.",
    "created_at": "2013-11-25T21:24:40Z",
    "issue": "https://github.com/sagemath/sage/issues/15452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15452#issuecomment-194872",
    "user": "https://github.com/defeo"
}
```

<a id='comment:4'></a>
Right. That's why trac was showing no difference from master! Let's close this.



---

archive/issue_events_137399.json:
```json
{
    "actor": "https://github.com/defeo",
    "created_at": "2013-11-25T21:27:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15452",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15452#event-137399"
}
```



---

archive/issue_events_137400.json:
```json
{
    "actor": "https://github.com/defeo",
    "created_at": "2013-11-25T21:27:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15452",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15452#event-137400"
}
```



---

archive/issue_events_137401.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-11-26T10:57:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15452",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15452#event-137401"
}
```



---

archive/issue_events_137402.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-11-26T10:57:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15452",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15452#event-137402"
}
```



---

archive/issue_events_137403.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-11-26T10:57:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15452",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15452#event-137403"
}
```



---

archive/issue_comments_194873.json:
```json
{
    "body": "**Changing author** from \"Luca De Feo\" to \"\".",
    "created_at": "2013-11-26T10:57:45Z",
    "issue": "https://github.com/sagemath/sage/issues/15452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15452#issuecomment-194873",
    "user": "https://github.com/jdemeyer"
}
```

**Changing author** from "Luca De Feo" to "".



---

archive/issue_comments_194874.json:
```json
{
    "body": "**Reviewer:** Luca De Feo",
    "created_at": "2013-11-26T10:57:45Z",
    "issue": "https://github.com/sagemath/sage/issues/15452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15452#issuecomment-194874",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Luca De Feo

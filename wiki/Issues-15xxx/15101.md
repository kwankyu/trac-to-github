# Issue 15101: SAGE_DOC source location

archive/issues_014864.json:
```json
{
    "body": "The docbuilder turns documentation source into documentation. Currently, there is no notion of \"installing the documentation\", nor of VPATH builds. As soon as there will be, SAGE_DOC must point to the documentation, not the source code, but the source code location is still required to make some doctests pass.\n\nI think it's best to introduce a variable `SAGE_DOC_SRC` that points to the documentation source code.\n\n\n\n\n\n**CC:**  @jdemeyer @kiwifb\n\n**Branch:** [u/felixs/doc_src](https://github.com/sagemath/sagetrac-mirror/tree/u/felixs/doc_src)\n\n**Commit:** [9cc2282312bdc28ef3ca51b11bb6330a2e26aa8c](https://github.com/sagemath/sagetrac-mirror/commit/9cc2282312bdc28ef3ca51b11bb6330a2e26aa8c)\n\n**Reviewer:** Jeroen Demeyer\n\n**Author:** Felix Salfelder\n\nIssue created by migration from https://trac.sagemath.org/ticket/15101\n\n",
    "closed_at": "2016-06-12T12:02:30Z",
    "created_at": "2013-08-26T12:02:31Z",
    "labels": [
        "component: scripts",
        "enhancement",
        "invalid"
    ],
    "title": "SAGE_DOC source location",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/15101",
    "user": "https://trac.sagemath.org/admin/accounts/users/felixs"
}
```
The docbuilder turns documentation source into documentation. Currently, there is no notion of "installing the documentation", nor of VPATH builds. As soon as there will be, SAGE_DOC must point to the documentation, not the source code, but the source code location is still required to make some doctests pass.

I think it's best to introduce a variable `SAGE_DOC_SRC` that points to the documentation source code.





**CC:**  @jdemeyer @kiwifb

**Branch:** [u/felixs/doc_src](https://github.com/sagemath/sagetrac-mirror/tree/u/felixs/doc_src)

**Commit:** [9cc2282312bdc28ef3ca51b11bb6330a2e26aa8c](https://github.com/sagemath/sagetrac-mirror/commit/9cc2282312bdc28ef3ca51b11bb6330a2e26aa8c)

**Reviewer:** Jeroen Demeyer

**Author:** Felix Salfelder

Issue created by migration from https://trac.sagemath.org/ticket/15101





---

archive/issue_comments_187902.json:
```json
{
    "body": "**Commit:** [9cc2282312bdc28ef3ca51b11bb6330a2e26aa8c](https://github.com/sagemath/sagetrac-mirror/commit/9cc2282312bdc28ef3ca51b11bb6330a2e26aa8c)",
    "created_at": "2013-10-31T17:12:10Z",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15101#issuecomment-187902",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

**Commit:** [9cc2282312bdc28ef3ca51b11bb6330a2e26aa8c](https://github.com/sagemath/sagetrac-mirror/commit/9cc2282312bdc28ef3ca51b11bb6330a2e26aa8c)



---

archive/issue_comments_187903.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Branch pushed to git repo; I updated commit sha1.** **New commits:**\n<table><tr><td>[changeset:9cc2282]</td><td><code>sage-bin: sage executable, fix docbuild call</code></td></tr></table>\n",
    "created_at": "2013-10-31T17:12:10Z",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15101#issuecomment-187903",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:1'></a>
**Branch pushed to git repo; I updated commit sha1.** **New commits:**
<table><tr><td>[changeset:9cc2282]</td><td><code>sage-bin: sage executable, fix docbuild call</code></td></tr></table>




---

archive/issue_events_133296.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/felixs",
    "created_at": "2013-10-31T17:14:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15101#event-133296"
}
```



---

archive/issue_events_133297.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-29T18:16:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15101#event-133297"
}
```



---

archive/issue_events_133298.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-29T18:16:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15101#event-133298"
}
```



---

archive/issue_comments_187904.json:
```json
{
    "body": "<a id='comment:3'></a>\nNeeds to be rebased.",
    "created_at": "2013-12-29T18:16:22Z",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15101#issuecomment-187904",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'></a>
Needs to be rebased.



---

archive/issue_events_133299.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15101#event-133299"
}
```



---

archive/issue_events_133300.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15101#event-133300"
}
```



---

archive/issue_events_133301.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15101#event-133301"
}
```



---

archive/issue_events_133302.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15101#event-133302"
}
```



---

archive/issue_events_133303.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15101#event-133303"
}
```



---

archive/issue_events_133304.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15101#event-133304"
}
```



---

archive/issue_comments_187905.json:
```json
{
    "body": "<a id='comment:7'></a>\ndone, see #20030, #20321",
    "created_at": "2016-04-01T19:32:02Z",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15101#issuecomment-187905",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:7'></a>
done, see #20030, #20321



---

archive/issue_events_133305.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-04-01T19:32:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15101#event-133305"
}
```



---

archive/issue_events_133306.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-04-01T19:32:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15101#event-133306"
}
```



---

archive/issue_events_133307.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-04-01T19:32:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15101#event-133307"
}
```



---

archive/issue_events_133308.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-01T19:33:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15101#event-133308"
}
```



---

archive/issue_events_133309.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-01T19:33:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15101#event-133309"
}
```



---

archive/issue_comments_187906.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2016-04-01T19:33:55Z",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15101#issuecomment-187906",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_events_133310.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-12T12:02:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15101#event-133310"
}
```



---

archive/issue_events_133311.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-12T12:02:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15101",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15101#event-133311"
}
```
